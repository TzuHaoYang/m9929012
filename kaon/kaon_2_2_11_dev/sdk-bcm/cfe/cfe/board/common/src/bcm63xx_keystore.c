...
//#define GET_BUF (void*)0x82616fc0  /*hack keep it until we get a board*/
...
static key_store_err_t key_store_get_data(key_store_key_info_t* key_info,
                                        Booter1EncrArgs* enc,
                                        void* dst)
{
        /*Copy data to the destination*/
        key_store_err_t rc =  KEY_STORE_ERR_INVALID;
        unsigned int data_sz, 
                        ecode = CH4_TO_U32(EKEY);
        uint8_t *buf = _get_buf(key_info->size); 
        if (!buf) {
            goto err;
        }
        if (_type2size(KEY_STORE_DATA_GET_TYPE(key_info->type_state), &data_sz)) {
        ...
        switch(KEY_STORE_DATA_GET_STATE(key_info->type_state)) {
                ...
                case KEY_STORE_DATA_STATE_FLD_ENCR:
                        /*Decrypting */
                        aes_cbc128_decrypt((uint32_t*)buf,/*dst*/ 
                                        (uint32_t*)&key_info->data[0],/*src*/
                                        key_info->size, /*data len*/
                                        enc->bek,/*ek*/
                                        enc->biv); /*iv*/ 
                        /*key_sz is expected size of the key per type*/
                        memcpy(dst, buf, data_sz);
                        break;
#endif
                case KEY_STORE_DATA_STATE_RAW:
                        memcpy(dst, &key_info->data[0], data_sz);
                        break;
                default:
                        goto err;
        }
        rc = KEY_STORE_ERR_SUCC; 
err:
        if (buf) {
            _release_buf(buf);
        }
                                ...
                                void* data)
{
        ...
        key_store_err_t  rc = KEY_STORE_ERR_INVALID;
        key_store_hdr_t *hdr;
        key_store_key_info_t *key_info, *buf = NULL;
        ...
        /* while iterating through key info array was OK. The key_info  
          may become 4byte unaligned pointe which is a an issue for the cipher*/
        buf = _get_buf(sizeof(key_store_key_info_t) + key_info->size);
        if (!buf) {
            goto err;
        }
        lib_memcpy(buf, key_info, sizeof(key_store_key_info_t) + key_info->size);
        if (key_store_get_data(buf, 
