...
#else
#define DBG_PRINT_CRED 
#endif

 
extern int otp_map_write(uint32_t* data, key_store_data_type_t data_type);
extern int otp_map_read(uint32_t* data, key_store_data_type_t data_type);
                                ...
                                sec_state_t cur_state)
{
        ...
        switch(cur_state) {
                case SEC_STATE_UNSEC:
                        if (req_state == KEY_STORE_REQ_TRANSIT_FLD || 
...
#if defined (_BCM96878_)
                                {
                                        uint32_t val = 0x1;
                                        (void) res;
                                        if (otp_map_write(&val, KEY_STORE_DATA_TYPE_BROM_MODE)) {
                                             PRINT4CH(EOTP);
                                             goto err;
                                         }
                                         val = 0x7;  
                                         if (otp_map_write(&val, KEY_STORE_DATA_TYPE_CUST_BROM_MODE)) {
                                              PRINT4CH(EOTP);
                                              goto err;
                                          }
                                }
				...
				mid[0] = ntohs((uint16_t)mid[0]);
                                ...
                                (void)res; 
                                /* FUSING FLD MID*/
                                if (otp_map_write(&mid[0], KEY_STORE_DATA_TYPE_MID)) {
                                        PRINT4CH(EOTP);
                                        goto err;
                                }
                                if (otp_map_read(&mid[1], KEY_STORE_DATA_TYPE_MID)) {
                                        PRINT4CH(EOTP);
                                        goto err;
                                }
                                if (memcmp(&mid[0], &mid[1], sizeof(uint16_t))) {
                                ...
                                /* FUSING FLD RoE*/
                                if (otp_map_write((uint32_t*)ek_iv,
                                       KEY_STORE_DATA_TYPE_KEY_AES_CBC_128_EK)) {
                                        PRINT4CH(ESOT);
                                        goto err;
                                }
                                mem = (uint32_t*)_get_buf(sizeof(ek_iv)); 
                                if (otp_map_read((uint32_t*)mem,
                                       KEY_STORE_DATA_TYPE_KEY_AES_CBC_128_EK)) {
                                        PRINT4CH(ESOT);
                                        goto err;
                                }
                                ...
                                /* FUSING FLD RoT */
                                if (otp_map_write((uint32_t*)hash, KEY_STORE_DATA_TYPE_HASH)) {
                                        PRINT4CH(ESOT);
                                        goto err;
                                }
                                _resize_buf((void*)mem,sizeof(hash));
                                if (otp_map_read((uint32_t*)mem, KEY_STORE_DATA_TYPE_HASH)) {
                                        PRINT4CH(ESOT);
                                        goto err;
                                }
