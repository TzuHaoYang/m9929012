...
static inline int __fuse_sko_rows_ecc(uint32_t* obj, otp_obj_map_t *map)
{
	...
	uint32_t crc = CRC32_INIT_VALUE;
        ...
        crc = (~crc);
        if (__fuse_row_ecc(&crc, map, map->row + i, crc)) {
           goto err;
        }
#ifdef DEBUG_OTP_MAP
...
static inline int __otp_map_op(uint32_t* data, key_store_data_type_t data_type, int dir)
{
    switch (data_type) {
...
int otp_map_write(uint32_t* data, key_store_data_type_t data_type)
{
    return __otp_map_op(data, data_type, 1);
}

int otp_map_read(uint32_t* data, key_store_data_type_t data_type)
{
    return __otp_map_op(data, data_type, 0);
}
