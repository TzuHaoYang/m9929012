...
static int cpu_ring_and_scheduler_init(void)
{
    printf("CPU ring init GPL\n");
    return 0;
}

#endif //CONFIG_GPL_RDP
...
{
    unsigned long mem_end = mem_usablesize;
...
static int xrdp_init(void)
{
    ...
    /* fpm pool DDR0 virtual base address */
...
#if defined(_BCM96878_)
{
        ...
        ag_drv_unimac_rdp_command_config_get(4, &cmd);
        cmd.loop_ena = 1;
        cmd.rx_ena = 1;
        cmd.tx_ena = 1;
        ag_drv_unimac_rdp_command_config_set(4, &cmd);
}
#endif
#endif
#endif
...
static int bcm63xx_impl3_ether_write(cfe_devctx_t *ctx, iocb_buffer_t *buffer)
{
    bcmenet_sc_t *sc = (bcmenet_sc_t *)ctx->dev_softc;
    bdmf_error_t rc;
#if !defined(CPU_TX_RING_COPY) && !defined(XRDP_SIMPLE_CFE)
    RDD_CPU_TX_DESCRIPTOR_DTS cpu_tx_descriptor;
    pbuf_t pbuf;
#elif defined(XRDP_SIMPLE_CFE)
    pbuf_t pbuf;
#endif
...
#else
    rc = rdd_cpu_tx(pbuf.fpm_bn, pbuf.length, sc->active_tx_port);
#endif
...
static void cfe_mac_unset_by_phy(uint32_t port)
{
    mac_dev_t *mac_dev = mac_devices[port];
    /* Skip not working ports */
    if (phy_devices[port])
    {
        phy_devices[port]->link = 0; /* Set link down */
        mac_dev = mac_devices[port];
        mac_dev_disable(mac_dev);
    }
}
...
static int bcm63xx_impl3_ether_close(cfe_devctx_t *ctx)
{
    ...
    for (;i< CFE_NUM_OF_PORTS; ++i)
        cfe_mac_unset_by_phy(i);
    return 0;
}
...
static void bcm63xx_impl3_ether_poll(cfe_devctx_t *ctx, int64_t ticks)
{
    bcmenet_sc_t *sc = (bcmenet_sc_t *)ctx->dev_softc;
    int i, link;
#if defined(CONFIG_BRCM_IKOS) && ( defined(_BCM96846_) || defined(_BCM96856_) || defined(_BCM96878_))
