...
unsigned short g_raw_flash_write_offset = 0;
...
#if (defined (_BCM96838_) || defined (_BCM96848_) || defined (_BCM96878_) || defined (_BCM96858_) || defined (_BCM96846_) || defined (_BCM96856_) ) && (NONETWORK == 0)
...
#endif
#if (defined(_BCM96858_) || defined(_BCM96846_) || defined(_BCM96856_) || defined(_BCM96878_)) && (NONETWORK == 0)
...
{
    if (g_force_mode || (flash_get_flash_type() == FLASH_IFC_SPI && !disable_nor_raw_partition))
    {
...
static inline void format_host_file_name(char *hostImageName, char *imageName, char *host_ip)
{

    if(imageName != NULL && host_ip != NULL && hostImageName != NULL)
    {
        if (strchr(imageName, ':'))
        {
            if(strlen(imageName) < BOOT_FILENAME_LEN + BOOT_IP_LEN - 1 )
                strcpy(hostImageName, imageName);
        }
        else if(strlen(imageName) < BOOT_FILENAME_LEN - 1 )
        {
            strcpy(hostImageName, host_ip);
...
static int ui_cmd_flash_image(ui_cmdline_t *cmd,int argc,char *argv[])
{
    ...
    if (!imageName)
         imageName=bootInfo.flashFileName;
    memset(hostImageName, '\0', sizeof(hostImageName));
    format_host_file_name(hostImageName, imageName, bootInfo.hostIp);
...
#if defined(CFG_DT)
static int ui_cmd_set_bootarg(ui_cmdline_t *cmd,int argc,char *argv[])
{
    char* append = cmd_getarg(cmd, 0);
    char* cmdline  = cmd_getarg(cmd, 1);

    if (cmdline == NULL || (*append != 'r'&& *append != 'a')) {
        printf("invalid argument!\r\n");
        return -1;
    }

    return blparms_set_bootcmd(cmdline, *append == 'r' ? 0 : 1);
}
...
static int ui_cmd_load_dtb(ui_cmdline_t *cmd,int argc,char *argv[])
{
    ...
    if (dtbName)
    {
        memset(hostDtbName, '\0', sizeof(hostDtbName));
        format_host_file_name(hostDtbName, dtbName, bootInfo.hostIp);
    }
    else
    {
...
static int ui_cmd_write_whole_image(ui_cmdline_t *cmd,int argc,char *argv[])
{
    ...
    {
        res = writeWholeImageRaw(ptr, res, g_raw_flash_write_offset);
    }
    else
#endif
    {
...
void cfe_go(cfe_loadargs_t *la)
{
    ...
    cfe_sec_set_rng_access_permissions();
#endif
	    ...
	    pDest = pDestBndry + 1024;
	       ...
	       memcpy((void *)(pDest - SEC_S_SIGNATURE), (void *)(pDest + atoi(pCurTag->kernelLen) - SEC_S_SIGNATURE), SEC_S_SIGNATURE);
            ...
            /* ARM linux kernel compiled for virtual address at 0xc0008000,
               ...
               On ARM64, When calculating the physical load address, cferam masks lower 28 bits of 
               the entry address provided by the vmlinux elf file.
               e.g. in case of linux-4.1 the entry point is 0xffffffc000080000
               which translates to 0x80000.
               But in case of linux-4.19, the entry point 
               has changed to 0xffffff8008080000 which translates to 0x8080000.
               Due to this the kernel gets loaded at 134MB which is beyond 128MB,
               which causes an exception while loading the kernel in safe mode or board with 128MB DDR.
               Solution is to only mask 24 bit instead of 28
            */
            image_hdr.la = (image_hdr.la&0xffffff);
            image_hdr.entrypt = (image_hdr.entrypt&0xffffff);
#endif
...
#if (INC_NAND_FLASH_DRIVER==1)
static int bootNandImageFromRootfs(int start_blk, int end_blk, int bootImg, int rootfsIdx)
{
    char brcmMagic[] = {'B','R','C','M'};
    uint8_t *image = NULL;
...
#error "SEC_S_SIGNATURE must be 4 byte aligned"
    ...
    /* ARM linux kernel compiled for virtual address at 0xc0008000, 
       ...
       Solution is to only mask 24 bit instead of 28
    */
    la.la_address =(long)(((uintptr_t)image_hdr.la)&0xffffff);
    la.la_entrypt = (long)(((uintptr_t)image_hdr.entrypt)&0xffffff);
#else
    la.la_address = (long)image_hdr.la;
    la.la_entrypt = (long)image_hdr.entrypt;
#endif
    ...
    defined(_BCM963138_) || defined(_BCM963148_) || defined(_BCM963178_) || \
    defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM96846_) || defined(_BCM96878_) || \
    defined(_BCM963158_) || defined (_BCM96856_) || defined(_BCM947622_) || \
    defined(_BCM96878_)
    ...
    if (ret) {
        ...
        */
        ...
        set_nand_chosen_root(rootfsIdx);
    ...
    if( pTag1 && pTag2 ) {
        ...
        { // swap the booted image if we have the override set unless the state was JFFS2/UBI BOOT_SET_*_IMAGE_ONCE
            if (state == BOOT_SET_NEW_IMAGE)
                ...
                state = BOOT_SET_NEW_IMAGE;
#ifdef CONFIG_CFE_FAILSAFE_BOOT
            if  (cfe_boot_st_error() == CFE_BOOT_ERR_CRIT) {
                 state = BOOT_SET_OLD_IMAGE;
            }
#endif
        }
...
static int auto_run(run_info_t* ar)
{
    ...
    if (ar->boot_opt == 'f'/* && !ar->img_id[id_img_boot]*/) {
        ...
        if (bcm_otp_is_boot_secure()) 
        {
            printf("Device in Secure Mode. Booting from host is disabled!!\n");
            ret = -1;
        } 
        else 
#endif            
...
#if !defined(_BCM947189_) && !defined(_BCM963268_) && !defined(_BCM963381_) && !defined(_BCM96838_)
...
int dump_nand(unsigned int addr, unsigned int end)
{
    ...
    int status;
    unsigned char *buf =  KMALLOC(pageSize + spareSize, sizeof(void*));
    ...
    for (; addr < end; addr += pageSize)
    {
        ...
        fwd.page = PAGE;
        fwd.offset = 0;
        fwd.amount = pageSize + spareSize;
        fwd.data = buf;

        status = flash_dev_specific_cmd(READ_PAGE, &fwd, &no_ecc);

        printf("------------------ block: %d, page: %d ------------------\n", BLOCK, PAGE);
        ui_dumpaddr(buf, pageSize, (int)(addr - (uintptr_t)buf));
        printf("----------- spare area for block %d, page %d -----------\n", BLOCK, PAGE);
        ui_dumpaddr(&buf[pageSize], spareSize, (int)(pageSize - (uintptr_t) &buf[pageSize]));
...
static int ui_cmd_read_nand(ui_cmdline_t *cmd, int argc, char *argv[])
{
    no_ecc = 2;
    return(ui_cmd_dump_nand(cmd, argc, argv));
}
...
static int ui_cmd_find_bad_bits(ui_cmdline_t *cmd, int argc, char *argv[])
{
    char *pszFirst = cmd_getarg(cmd, 0);
    char *pszSecond = cmd_getarg(cmd, 1);
    char *pszThird = cmd_getarg(cmd, 2);
    ...
    static int addr = 0;
    int end, i, no_ecc;
    unsigned char *buf1 = KMALLOC(pageSize + spareSize, sizeof(void*));
    unsigned char *buf2 = KMALLOC(pageSize + spareSize, sizeof(void*));
    flash_write_data_t fwd;
    ...
    for (; addr < end; addr += pageSize)
    {
        ...
        fwd.data = buf1;
        no_ecc = 0;
        if (flash_dev_specific_cmd(READ_PAGE, &fwd, &no_ecc) != FLASH_API_OK)
            printf("Error reading block %d\n", BLOCK);
        ...
        for (i = 0; i < pageSize + spareSize; i++)
            if(buf1[i] != buf2[i])
                printf("offset 0x%04x, ECC on: 0x%02x, ECC off: 0x%02x\n", i, buf1[i], buf2[i]);
    }
...
static int ui_cmd_nand_cmd(ui_cmdline_t *cmd, int argc, char *argv[])
{
    ...
    int item = 0, read = 0;
    uint16_t value;
    unsigned char data[MAX_READ_VALUES];
    ...
    if (!argc)
    {
        printf("Generic NAND command, a=address then address, c=command then command, w=write data then values to write, r=read data then # bytes to read\n"
               ...
               "    read parameter page byte 100:  nc c 0xEC a 0 c 5 a 100 0 c 0xE0 r 1\n\n"
               "Serial NAND:\n"
               "ex. get device ID:                 nc w 0x9F 0 r 8\n"
               "    read config (get feature):     nc w 0xF 0xB0 r 1\n"
               "    read status (get feature):     nc w 0xF 0xC0 r 1\n"
               "    reset part:                    nc w 0xFF\n");
        return(0);
    }
    ...
    if (flash_get_flash_type() == FLASH_IFC_SPINAND)
    {
        ...
        while(argc--)
        {
            if ( (argv[item][0] == 'c') || (argv[item][0] == 'a') || (argv[item][0] == 'w') ||
                 (argv[item][0] == 'C') || (argv[item][0] == 'A') || (argv[item][0] == 'W') )
            { // ignore superfluous commands
                item++;
                continue;
            }
            ...
            if ( (argv[item][0] == 'r') ||
                 (argv[item][0] == 'R') )
            { // if a read is requested get the amount and stop since we can't support a command any more compound
                read = fwd.offset = atoi(argv[++item]);
                break;
            }

            // otherwise get the write data
            data[fwd.amount++] = atoi(argv[item++]);
        }

        flash_dev_specific_cmd(NAND_COMMAND, &fwd, NULL);
    }
...
static int ui_cmd_mark_block_bad(ui_cmdline_t *cmd, int argc, char *argv[])
{
    ...
    if(flash_get_flash_type() ==  FLASH_IFC_NAND)
        printf("If you are bad block testing in Linux you will need to erase the bad block table (BBT) so it will be rebuilt and this newly marked bad block outside of Linux will be recognised in Linux\n");
...
static int ui_cmd_write_data(ui_cmdline_t *cmd, int argc, char *argv[])
{
    ...
    unsigned int pageSize = flash_dev_specific_cmd(GET_PAGE_SIZE, NULL, NULL);
    unsigned int spareSize = flash_dev_specific_cmd(GET_SPARE_SIZE, NULL, NULL);
    ...
    buf = KMALLOC(pageSize + spareSize, sizeof(void*));
    ...
    printf("\nNow think carefully.  Do you really,\n"
        "really want to write block 0x%x, page 0x%x, offset 0x%x with data ", atoi(pszBlock), atoi(pszPage), atoi(pszOffset));
    i = 3;
    while(argv[i] != NULL)
    {
        buf[atoi(pszOffset) + i - 3] = atoi(argv[i]);
    ...
    fwd.block = atoi(pszBlock);
    fwd.page = atoi(pszPage);
    fwd.offset = poke ? atoi(pszOffset) : 0;
    fwd.amount = poke ? i - 3 : pageSize + spareSize;
    fwd.data = poke ? &buf[atoi(pszOffset)] : buf;
    no_ecc = 1;

    flash_dev_specific_cmd(WRITE_PAGE, &fwd, &no_ecc);
...
static int ui_cmd_poke_data(ui_cmdline_t *cmd, int argc, char *argv[])
{ // reads page into NAND cache, modifies data in NAND cache, then initiates page program with ECC off, can be used to set or clear bits
    no_ecc = 2;

    return(ui_cmd_write_data(cmd, argc, argv));
}


static int ui_cmd_modify_data(ui_cmdline_t *cmd, int argc, char *argv[])
{ // reads block, modifies data in memory, erases block, then writes modified data back to block with ECC off, can be used to set or clear bits
    char *pszBlock = cmd_getarg(cmd, 0);
    char *pszPage = cmd_getarg(cmd, 1);
    char *pszOffset = cmd_getarg(cmd, 2);
    char *pszData = cmd_getarg(cmd, 3);
    ...
    unsigned int pageSize = flash_dev_specific_cmd(GET_PAGE_SIZE, NULL, NULL);
    unsigned int spareSize = flash_dev_specific_cmd(GET_SPARE_SIZE, NULL, NULL);
    unsigned int blockSize = flash_get_sector_size(0);

    unsigned int pages = blockSize / pageSize;
    ...
    buf = KMALLOC(pages * (pageSize + spareSize), sizeof(void*));
    ...
    for (page = 0; page < pages; page++)
    {
        ...
        fwd.offset = 0;
        fwd.amount = pageSize + spareSize;
        fwd.data = &buf[page * (pageSize + spareSize)];
        no_ecc = 0;

        status = flash_dev_specific_cmd(READ_PAGE, &fwd, &no_ecc);
        if(update) // fill spare area with 0xFF if updating ECC
            memset(&buf[(page * (pageSize + spareSize)) + pageSize], 0xFF, spareSize);
    ...
    // modify page in block
    printf("\nNow think carefully.  Do you really,\n"
        "really want to write block 0x%x, page 0x%x, offset 0x%x with data ", atoi(pszBlock), atoi(pszPage), atoi(pszOffset));
    i = 3;
    while(argv[i] != NULL)
    {
        buf[(atoi(pszPage) * (pageSize + spareSize)) + atoi(pszOffset) + i - 3] = atoi(argv[i]);
        printf("0x%x ", atoi(argv[i]));
        i++;
    }
    printf("?");
    ...
    // erase block
    status = atoi(pszBlock);
    flash_dev_specific_cmd(FORCE_ERASE, &status, NULL);
    ...
    for (page = 0; page < pages; page++)
    {
        ...
        status = flash_dev_specific_cmd(WRITE_PAGE, &fwd, &no_ecc);
    }
...
static int ui_cmd_update_data(ui_cmdline_t *cmd, int argc, char *argv[])
{ // reads block, modifies page in memory, erases block, then writes modified data back to block with ECC on, can be used to set or clear bits
    no_ecc = 2;

    return(ui_cmd_modify_data(cmd, argc, argv));
}
#endif
...
static int ui_cmd_force_parallel_nand(ui_cmdline_t *cmd, int argc, char *argv[])
{
    int temp = g_force_mode;
    printf("Forcing parallel NAND read/write\n");
    g_force_mode = 3;
    flash_init(); // do flash init again, but with g_force_mode on will now default to SPI NAND
    g_force_mode = temp;
    return(0);
...
static int ui_cmd_enable_raw_flash_write(ui_cmdline_t *cmd, int argc, char *argv[])
{
    ...
    if (argc >= 1)
    {
        g_raw_flash_write_offset=atoi(cmd_getarg(cmd, 0));
        printf("Using %lx for raw flash write\n", g_raw_flash_write_offset);
    }
    return(0);
}
...
extern unsigned int swr_read( unsigned int ps, unsigned int reg);
...
static int swrr(ui_cmdline_t *cmd, int argc, char *argv[])
{
	...
	if(pss && pregs)
	{
		...
		if(reg > 9 || ps >= SWR_LAST || ps < SWR_FIRST)
		{
			printf("reg %d, ps %d\n", reg, ps);
			return 0;
		}
		...
		printf("power_supply_in_hex, reg_in_hex\n");
...
static int swrw_cmd(ui_cmdline_t *cmd, int argc, char *argv[])
{
	char *pss   = cmd_getarg(cmd, 0);
    char *pregs = cmd_getarg(cmd, 1);
	...
	if(pss && pregs && vals )
	{
        ...
        printf("power_supply_in_hex, reg_in_hex, val_in_hex\n");
...
static int ui_cmd_phy(ui_cmdline_t *cmd,int argc,char *argv[])
{
    ...
    !defined (_BCM947189_) && !defined (_BCM96846_) && !defined (_BCM96856_) && !defined (_BCM96878_)
    if (argc < 2) {
...
WrongParams:
      printf("%s\n", lport_phy_usage);
      return -1;
#elif defined (_BCM96846_) || defined (_BCM96856_) || defined(_BCM96878_)
      char *pszPhyId = cmd_getarg(cmd, 0);
      char *pszReg = cmd_getarg(cmd, 1);
      char *pszVal = cmd_getarg(cmd, 2);
      int phyid;
...
WrongParams:
    ...
    defined _BCM94908_  || defined _BCM963158_ || defined PMC_IMPL_3_X
...
#define MAX_SEL_NUM                    16
#define MAX_TRIM_NUM                   16

#define STARTING_SEL                   11 /* 1.008V */       
#define STARTING_TRIM                  8  /* 0mV */

#define SEL_STEP                       28 /* 28m per selection step */
#define TRIM_STEP                      4 /* 4m per selection step */

int sel_tab[MAX_SEL_NUM] = {
    0xf,          /* 0.700V - nominal value, actual voltage are chip dependent*/ 
    0xe,          /* 0.728V */
    0xd,          /* 0.756V */
    0xc,          /* 0.784V */
    0xb,          /* 0.812V */
...
static void set_ldo_vreg_ctrl(int sel, int trim)
{
    TOPCTRL->LdoCtl = ((trim<<LDO_VREG_CTRL_TRIM_SHIFT)&LDO_VREG_CTRL_TRIM_MASK) | ((sel<<LDO_VREG_CTRL_SELECT_SHIFT)&LDO_VREG_CTRL_SELECT_MASK);

    cfe_usleep(250*1000);
}

/* based on Greg's SetLDO python script */
static int set_1p0_ldo_voltage(int target_volt)
{
    int cur_volt, delta;
    int sel, trim, ret;
    int sel_idx, trim_idx;

    /* Initialize the VREGCNTL to about 1.0V */
    sel_idx = STARTING_SEL;
    sel = sel_tab[sel_idx];
    trim_idx = STARTING_TRIM;
    trim = trim_tab[trim_idx];
    set_ldo_vreg_ctrl(sel, trim);

    /* Now step through the LDO Select settings and then the LDO trim settings to adjust vlotage 
       within 5% of the target voltage */
    
    while (1) {
        if ((ret = get_pvtmon_result(kV_1p00_1, 0, &cur_volt)))
            break;

        trim = (TOPCTRL->LdoCtl&LDO_VREG_CTRL_TRIM_MASK)>>LDO_VREG_CTRL_TRIM_SHIFT;
        sel = (TOPCTRL->LdoCtl&LDO_VREG_CTRL_SELECT_MASK)>>LDO_VREG_CTRL_SELECT_SHIFT;
        /*printf("current voltage %d, sel %d trim %d ldoctl reg 0x%x\n", 
	  cur_volt, sel, trim, TOPCTRL->LdoCtl);*/

        delta = cur_volt - target_volt;
        if (delta == 0)
            break;

        if (delta < 0) {
            delta = -delta;
            if ((sel_idx == (MAX_SEL_NUM-1)) && (trim_idx == (MAX_TRIM_NUM-1))) {
                 printf("Settings have maxed out. Cannot make a change\n");
                 break;
            }

            if (delta >= SEL_STEP) {
                if (sel_idx == (MAX_SEL_NUM-1) )
                    trim_idx++; /* already reach max sel, can only adjust trim */
                else
                    sel_idx++;
	    } else if (delta >= TRIM_STEP)
                trim_idx++;
            else
                break;
        } else {
            if ((sel_idx == 0) && (trim_idx == 0)) {
                 printf("Settings have maxed out. Cannot make a change\n");
                 break;
            }
...
#if defined(_BCM963178_) || defined(_BCM947622_)
static int ui_cmd_vreg(ui_cmdline_t *cmd,int argc,char *argv[])
{
    int select;
    char* pszVoltage = NULL;
    int cur_volt, target_volt, res;

    if ((argc != 1)&&(argc != 2)) {
        printf("invalid nubmer of parameters\n");
        return -1;
    }

    select = atoi(cmd_getarg(cmd, 0));
    if (select < kV_0p85_0 || select > kV_3p30) {
        printf("invalid vr select id %d\n", select);
        return -1;
    }

    if ((pszVoltage = cmd_getarg(cmd, 1))) {
        if (select != kV_1p00_1) {
           printf("Only support vr id %d for voltage change\n", kV_1p00_1);
           return -1;
        }
        target_volt = atoi(pszVoltage);
        printf("votage regulator %d setting voltage to %dmV...\n", select, target_volt);
        res = set_1p0_ldo_voltage(target_volt);
        if (res)
            return res;
    }

    res = get_pvtmon_result(select, 0, &cur_volt);
    if (res)
        return res;

    printf("votage regulator %d voltage set to %dmV\n", select, cur_volt);
    return 0;
}
#endif
...
static int ui_cmd_set_force_mode(ui_cmdline_t *cmd,int argc,char *argv[]) {
    ...
    if (g_force_mode == 0)
    {
        ...
        brcm_cmd_addcmd("wd",
                   ui_cmd_write_data,
                   NULL,
                   "Write data to NAND, reads page into memory, overwrites bytes and writes back NAND page, can only be used to clear bits",
                   "eg. wd [block] [page] [offset] [data...]",
                   "",
                   BRCM_UI_CMD_TYPE_RESTRICTED);
        ...
        brcm_cmd_addcmd("pd",
                   ui_cmd_poke_data,
                   NULL,
                   "Poke NAND data, reads page into NAND cache, overwrites bytes there and programs page, can only be used to clear bits (mainly for XTX)",
                   "eg. pd [block] [page] [offset] [data...]",
                   "",
                   BRCM_UI_CMD_TYPE_RESTRICTED);
        ...
        brcm_cmd_addcmd("md",
                   ui_cmd_modify_data,
                   NULL,
                   "Modify NAND data, erases/writes data in a page of NAND preserving ECC, can be used to set or clear bits",
                   "eg. wd [block] [page] [offset] [data...]",
                   "",
                   BRCM_UI_CMD_TYPE_RESTRICTED);
        ...
        brcm_cmd_addcmd("ud",
                   ui_cmd_update_data,
                   NULL,
                   "Update NAND data, erases/writes data in a page of NAND updating ECC, can be used to set or clear bits",
                   "eg. wd [block] [page] [offset] [data...]",
                   "",
                   BRCM_UI_CMD_TYPE_RESTRICTED);
        ...
        brcm_cmd_addcmd("pn",
                   ui_cmd_force_parallel_nand,
                   NULL,
                   "Force read/writes to parallel NAND",
                   "eg. pn",
                   "",
                   BRCM_UI_CMD_TYPE_RESTRICTED);
#endif
...
#if defined(_BCM963178_) || defined(_BCM947622_)
        brcm_cmd_addcmd("vreg",
                   ui_cmd_vreg,
                   NULL,
                   "set/get voltage regulator output voltage",
                   "eg. to set voltage: vreg vr_id mV. to read: vreg vr_id",
                   "",
                   BRCM_UI_CMD_TYPE_RESTRICTED);
#endif
...
#if defined (_BCM963138_) || defined (_BCM963148_) || defined (_BCM94908_) || defined (_BCM963158_) || defined (_BCM963178_) || defined (_BCM947622_)
...
static int  ui_cmd_pmc_close_avs(ui_cmdline_t *cmd,int argc,char *argv[]) 
{
    ...
    if (!(argc == 2 || argc == 3
#if defined _BCM963158_ || defined PMC_IMPL_3_X
          || argc == 4 || argc == 5
#endif
          )) {
    ...
    case 2:
    case 4:
      margin_mv_slow = (uint16_t)atoi(cmd_getarg(cmd, 0));
      margin_mv_fast = (uint16_t)atoi(cmd_getarg(cmd, 1));
      if (argc == 2)
          break;
      maximum_mv = (uint16_t)atoi(cmd_getarg(cmd, 2));
      minimum_mv = (uint16_t)atoi(cmd_getarg(cmd, 3));
      break;
    case 3:
    case 5:
      island = (uint32_t)atoi(cmd_getarg(cmd, 0));
      margin_mv_slow = (uint16_t)atoi(cmd_getarg(cmd, 1));
      margin_mv_fast = (uint16_t)atoi(cmd_getarg(cmd, 2));
      if (argc == 3)
          break;
      maximum_mv = (uint16_t)atoi(cmd_getarg(cmd, 3));
      minimum_mv = (uint16_t)atoi(cmd_getarg(cmd, 4));
      break;
    }

    if (minimum_mv && maximum_mv && (minimum_mv > maximum_mv)) {
        printf("Invalid parameters: min %d > max %d and not firmware default\n",
    ...
    if( res == 0 )
        printf("close avs with [island %d] margin_mv slow %d fast %d "
#if defined _BCM963158_ || defined PMC_IMPL_3_X
               "[max_mv %d min_mv %d (0 means using firmware default)] "
#endif
               "succeeded\n",
               island, margin_mv_slow, margin_mv_fast
#if defined _BCM963158_ || defined PMC_IMPL_3_X
...
static int  ui_cmd_pmc_cmd(ui_cmdline_t *cmd,int argc,char *argv[])
{
    ...
    if ( IS_DIGIT(cmd_getarg(cmd, 0)[0]) ) {
        ...
        if (strcmp(cmd_getarg(cmd, 0), "on") == 0) 
            ...
            pmc_reset();
#ifndef _BCM96838_
        else if  (strcmp(cmd_getarg(cmd, 0), "pvtmon") == 0) {
            int select = atoi(cmd_getarg(cmd, 1));
            int island = atoi(cmd_getarg(cmd, 2));
            int val;
            ...
            printf("pvtmon island %d select %d value %d.%03d %c\n",
                island, select, val/1000, (val > 0 ? val : -val)%1000,
                select ? 'V' : 'C');
        } else if  (strcmp(cmd_getarg(cmd, 0), "tracktemp") == 0) {
                char *p = cmd_getarg(cmd, 1);
                ...
                if (!p || !strcmp(p, "status")) {
                        ...
                        if (res) return res;
                        printf("tracktemp status is %s\n", status ? "on" : "off");
                } else if (!strcmp(p, "on")) {
                        res = pmc_set_tracktemp(1);
                } else if (!strcmp(p, "off")) {
                        res = pmc_set_tracktemp(0);
                } else {
                        res = -1; 
                }
        }
#endif // #ifndef _BCM96838_
        else
#endif
        {
...
#if (defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM963158_) || defined(_BCM963138_) || defined(_BCM963381_) || \
...
static int ui_init_bcm63xx_cmds(void)
{
    ...
    brcm_cmd_addcmd("closeavs",
           ...
           "pmc close avs cmd",
#if defined _BCM963158_ || defined PMC_IMPL_3_X
           "eg. closeavs [island] margin_mv_slow margin_mv_fast [maximum_mv minimum_mv]",
#else
           "eg. closeavs [island] margin_mv_slow margin_mv_fast",
#endif
           "",
           BRCM_UI_CMD_TYPE_RESTRICTED);
    brcm_cmd_addcmd("pmc",
...
#ifndef _BCM96838_
           "\neg. pmc pvtmon [select [island]]"
           "\neg. pmc tracktemp [status|on|off]"
#endif
           ,
           "",
           BRCM_UI_CMD_TYPE_RESTRICTED);
    ...
    brcm_cmd_addcmd("ba",
               ui_cmd_set_bootarg,
               NULL,
               "set linux kernel boot argument",
               "ba a/r \"boot cmdline string\"\n\n"
               "e.g ba r \"replacement cmdline\" ba a \"append cmdline\"",
               "",
               BRCM_UI_CMD_TYPE_RESTRICTED);
#endif
    ...
    brcm_cmd_addcmd("rn",
               ui_cmd_read_nand,
               NULL,
               "Read NAND contents along with spare area with ECC off",
               "eg. rn [block] [page] [total pages to view] or rd [address] [total pages to view]",
               "",
               BRCM_UI_CMD_TYPE_RESTRICTED);
    ...
    brcm_cmd_addcmd("bb",
               ui_cmd_find_bad_bits,
               NULL,
               "Find NAND flipped bits by comparing a read with and without ECC",
               "eg. bb [block] [page] [total pages to view]",
               "",
               BRCM_UI_CMD_TYPE_RESTRICTED);
...
#ifdef BCM_OPTEE
    /* Set OPTEE boot parameters */
    memset(&params, 0, sizeof(params));
    memcpy(params.ddk, derived_device_key, sizeof(params.ddk));
    params.ddk_len = sizeof(derived_device_key);
    /* Run OPTEE, drop to interactive console if failed */
    if (optee_init(&params) == -1) {
       breakIntoCfe = 1;
    }
    /* Remove the parameters from the memory */
    memset(&params, 0, sizeof(params));
#endif
