...
int dtb_del_cma_rsvmem_device(int dtb_id)
{
...
int dtb_getprop_cma_rsvmem_size(int dtb_id, const char *name, uint64_t* size)
{
    char dt_node_name[64];
    ...
    ret = _dtb_getprop_cma_rsvmem_size(dtb_ptr, dt_node_name, size);
    if (ret != 0) {
...
int dtb_setprop_cma_rsvmem_size(int dtb_id, const char *name, uint64_t size)
{
    char dt_node_name[64];
    char* dtb_ptr = get_dtb_ptr(dtb_id);
    int ret;
...
int dtb_del_cma_rsvmem(int dtb_id, const char *name)
{
    char dt_node_name[64];
    char* dtb_ptr = get_dtb_ptr(dtb_id);
    int ret;
...
int dtb_set_bootargs(int dtb_id, char* bootargs, int append )
{
    ...
    if (append) {
        ...
        if (propdata) {
            lib_strncat(new_bootargs, propdata, DT_BOOTARGS_MAX_SIZE);
        }
    } else {
        strncpy(new_bootargs, bootargs, DT_BOOTARGS_MAX_SIZE);
    }
...
int dtb_set_chosen_root(int dtb_id, char * root_device_full_pathname )
{
    ...
    ret=dtb_set_bootargs(dtb_id, bootargs, 1);
err_out:
    if (bootargs) {
        KFREE(bootargs);
   }
...
{
    const void *propdata =NULL;
    int nodeoffset, res = -1,proplen = 0, bootarg_len = 0;
    ...
    bootargs = KMALLOC(DT_BOOTARGS_MAX_SIZE,0); 
    if (!bootargs) {
         goto err_out;
    }

    bootarg_len = sprintf(bootargs, " %s %s0x%llx ", DT_BOOTARGS, DT_BOOTARGS_UPD_OPT, initrd_size);
    if (bootarg_len+proplen > DT_BOOTARGS_MAX_SIZE) {
        printf("Not enough space to append initrd boot arg %s\n", bootargs);
        goto err_out;
    }
    if (propdata) {
        lib_strncat(bootargs, propdata, DT_BOOTARGS_MAX_SIZE);
    }
...
#if defined(_BCM96858_) || defined(_BCM94908_) || defined(_BCM963158_) || defined(_BCM96846_) ||\
    defined(_BCM96856_) || defined(_BCM963138_) || defined(_BCM963178_)  || defined(_BCM947622_) || defined(_BCM96878_)
int dtb_set_nr_cpus(int dtb_id)
{
    unsigned int nr_cpus, nr_bp_cpus = 0;
    int ret;
#if defined(_BCM96846_) || defined(_BCM96856_) || defined(_BCM963138_) || defined(_BCM96878_)
    int max_num_of_cpu = 2;
#elif defined(_BCM963178_)
    int max_num_of_cpu = 3;
#else
