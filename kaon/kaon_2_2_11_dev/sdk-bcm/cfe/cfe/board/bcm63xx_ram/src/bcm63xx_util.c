...
void cfe_mem_info(void)
{
    ...
    printf("Free Memory        :  0x%08lx - 0x%08lx size %u\n", MEMORY_AREA_FREE_MEM, 
                                                          MEMORY_AREA_RSRV_ADDR + mem_usablesize - 1,
                                                          (MEMORY_AREA_RSRV_ADDR + mem_usablesize) - MEMORY_AREA_FREE_MEM);
    printf("\n");
}
...
static int verifyImageDDRType(uint32_t wfiFlags)
{
#if defined(CFG_DDR_TYPE_CHECK)
     uint32_t memcfg;
     uint32_t brdtype, imgtype;
     int rc = -1;

     printf("Verify image ddr type...\n");
     if( BpGetMemoryConfig(&memcfg) == BP_SUCCESS ) {
         brdtype = memcfg&BP_DDR_TYPE_MASK;
         imgtype = wfiFlags&WFI_FLAG_DDR_TYPE_MASK;
         printf("Board ddr type %s\n", brdtype == BP_DDR_TYPE_DDR3 ? "DDR3" : "DDR4");
         if( imgtype == WFI_FLAG_DDR_TYPE_NONE ) {
             printf("Image ddr type not set.");
             if( g_force_mode == 0 ) {
                 printf("Use force cmd to flash anyway if you are sure the image has the same ddr type as your board!!!\n");
                 rc = -1;
         } else {
                 printf("Force cmd enabled. Flash anyway...\n");
                 rc = 0;
             }
         } else {
             printf("Image ddr type %s\n", imgtype == WFI_FLAG_DDR_TYPE_DDR3 ? "DDR3" : "DDR4");
             if( (imgtype == WFI_FLAG_DDR_TYPE_DDR3 && brdtype != BP_DDR_TYPE_DDR3) ||
                 (imgtype == WFI_FLAG_DDR_TYPE_DDR4 && brdtype != BP_DDR_TYPE_DDR4) ) {
                 printf("Invalid image ddr type, flash aborted!!!\n");
                 rc = -1;
             } else
                 rc = 0;
         }
...
static int getSplitFromPartition(int imageNumber)
{
...
int flashImage(uint8_t *imagePtr)
{
    ...
    /* if the image has cfe, check ddr type */
    if (cfeSize) 
    {
        if (verifyImageDDRType(pTag->imageFlags) != 0)
            return -1;
    }
...
// which has the pre-Cferom binary
...
#if defined(_BCM963158_) || defined (_BCM963178_) || defined (_BCM947622_)
    if( (flash_get_flash_type() == FLASH_IFC_SPI) || (flash_get_flash_type() == FLASH_IFC_HS_SPI) )
        offset = IMAGE_OFFSET;
#endif
	...
	uint32_t newBtBlkCnt = 0;                           /* number of boot blocks in new image */
        ...
        {
            /* New image has at least one boot block (headerless cferom in block 0) */
           if( verifyImageDDRType(wfiTag.wfiFlags) != 0 )  /* check cfe rom DDR type */
                goto err_out;
...
int getBootLine(int setdef)
{
    ...
    if( rc == 0 ) {
        ...
        if (cfe_mailbox_status_isset()) {
           unsigned int ver = BCM_MBOX_VER_GET();
...
int changeDDRConfig(void)
{
    ...
    || defined(_BCM963158_) || defined(_BCM96846_) || defined(_BCM96856_) || defined(_BCM963178_)  || defined(_BCM947622_) || defined(_BCM947189_) || defined(_BCM96878_)
...
#if !defined(_BCM96846_) && !defined(_BCM947189_) && !defined(_BCM96878_)
...
int cfe_set_cpu_freq(int freqMHz)
{
   ...
   /* switch back to VCO PLL clock */
    ...
    ctrl_reg.Bits.byp_wait = 0;
    WriteBPCMRegister(PMB_ADDR_BIU_PLL, PLLBPCMRegOffset(resets), ctrl_reg.Reg32);
#elif defined(_BCM963178_) || defined(_BCM947622_)
    int mdiv;
    ...
    if(cfe_cpu_speed == freqMHz*1000000) 
        return 0;    

    pll_ch_freq_set(PMB_ADDR_BIU_PLL, 0, mdiv);
#else
    printf("cpu freq set not implemented\n");
#endif
...
static char root_device_name[NVRAM_BOOTLINE_LEN];
void set_nand_chosen_root(int booted_img_idx)
{
    int split_type = getSplitFromPartition(booted_img_idx);
    ...
    switch(split_type)
    {
        ...
        case SPLIT_FS:
	{
            strncpy(root_device_name, "ubi:rootfs_ubifs ubi.mtd=0 rootfstype=ubifs",NVRAM_BOOTLINE_LEN);
            break;
	}
        case SQSPLIT_FS:
        case SQUBI_FS:
	{
            strncpy(root_device_name, "/dev/ubiblock0_0 ubi.mtd=0 ubi.block=0,0 rootfstype=squashfs", NVRAM_BOOTLINE_LEN);
            break;
	}
        default:
	{
            strncpy( root_device_name, "mtd:rootfs rootfstype=jffs2", NVRAM_BOOTLINE_LEN);
            break;
	}
    }
}

char * get_nand_chosen_root(void)
{
