...
extern unsigned long mem_usablesize;
...
static int cfe_ddr_speed = 0;
    ...
    defined(_BCM96846_) || defined (_BCM96856_) || defined (_BCM96878_)
...
static void cfe_startup_info(void)
{
...
#elif  defined(_BCM96846_) || defined (_BCM96878_)
    xprintf("ARM Cortex A7 Dual Core: %dMHz",cfe_cpu_speed/1000000);
#elif  defined(_BCM963178_)
    xprintf("ARM Cortex A7 Triple Core: %dMHz",cfe_cpu_speed/1000000);
#elif  defined(_BCM947622_)
    xprintf("ARM Cortex A7 Quad Core: %dMHz",cfe_cpu_speed/1000000);
    ...
    xprintf("\n");
#endif
#if defined(_BCM96838_) || defined(_BCM96848_) || defined(_BCM96858_) || defined(_BCM96846_) || defined(_BCM96856_) || defined (_BCM96878_)
...
void cfe_main(int a,int b)
{
    ...
    {
    blparms_init();
#if defined (_BCM963138_) || defined(_BCM963148_) || defined(_BCM947189_) || defined(_BCM96846_) || defined(_BCM963148_)
...
//   printf("L2C_FILT_START 0x%x L2C_FILT_END 0x%x\n", *((unsigned int*)0x8001dc00), *((unsigned int*)0x8001dc04));
   ...
   static unsigned long linuxStartAddr=0x18000;
...
#elif defined(_BCM963178_)
    cfe_cpu_speed = ((MISC->miscStrapBus)&MISC_STRAP_BUS_CPU_SLOW_FREQ) ? 300*1000000 : 1500*1000000;
    cfe_set_cpu_freq(1500);
    pll_ch_freq_set(PMB_ADDR_BIU_PLL, 1, 3000/600); // raise ACEBIU clock rate to 600 MHz
#elif defined(_BCM947622_)
    unsigned int clk_index, cpuMHz;
    cfe_cpu_speed = ((MISC->miscStrapBus)&MISC_STRAP_BUS_CPU_SLOW_FREQ) ? 300*1000000 : 1500*1000000;
    if (!bcm_otp_get_cpu_clk(&clk_index))
        cpuMHz = cpu_speed_table[clk_index];
    else
        cpuMHz = 300;
    cfe_set_cpu_freq(cpuMHz);
...
static void cfe_set_prompt(void)
{
    ...
    defined(_BCM96856_) || defined(_BCM963178_) || defined(_BCM947622_) || defined(_BCM96878_)
...
static void cfe_init_sdram_size(void)
{
    ...
    mem_usablesize = mem_totalsize << 10;
#if defined(CONFIG_ARM)
    /* ARMv7 only support 32 bit virtual address. Limit to 1G accessible memory
       for CFE RAM so we can have 0x40000-00000 to 0x8000-0000 for directly mapped
       uncached address */
    if (mem_usablesize > SZ_1G)
        mem_usablesize = SZ_1G;
#endif
