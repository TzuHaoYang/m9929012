...
static char blparms_bootcmd[BLPARMS_BOOTCMD_LEN] = {0};
...
/* set the os boot command line buffer. If append, start the buffer with +.
   otherwise overwrite the whole buffer */
int blparms_set_bootcmd(char* cmdline, int append)
{
    if (append) {
        if (blparms_bootcmd[0] != '+' ) { 
           /* this must be the first time for cmdline append, clear the buffer 
              insert + at the beginning */
           memset(blparms_bootcmd, 0x0, BLPARMS_BOOTCMD_LEN);
           blparms_bootcmd[0] = '+';
           strncpy(&blparms_bootcmd[1], cmdline, BLPARMS_BOOTCMD_LEN-1);
        } else {
            lib_strncat(blparms_bootcmd, " ", BLPARMS_BOOTCMD_LEN);
            lib_strncat(blparms_bootcmd, cmdline, BLPARMS_BOOTCMD_LEN);
        }
    } else {
        memset(blparms_bootcmd, 0x0, BLPARMS_BOOTCMD_LEN);
        strncpy(blparms_bootcmd, cmdline, BLPARMS_BOOTCMD_LEN);
    }
...
static int is_cma_rsvmem = 0;

static int is_cma_rsvmem_enabled(void)
{
    ...
    if (dtb_get_prop(DTB_ID_CFE, dt_node_name, "reg", &len)) {
        ret = 1;
    }
    return ret;
}

static void del_all_cma_nodes(void)
{
    char dt_node_name[64];
    ...
    dtb_del_cma_rsvmem_device(DTB_ID_CFE);

    return;
}

static int get_rsvmem_size(char* name, uint64_t* size)
{
    char dt_node_name[64];
    uint64_t base_addr;
    int ret = -1;
    ...
    if (is_cma_rsvmem) {
        ret = dtb_getprop_cma_rsvmem_size(DTB_ID_CFE, name, size);
    } else {
        sprintf(dt_node_name, "%s%s", DT_RSVD_PREFIX_STR, name);
        ret = dtb_getprop_reg(DTB_ID_CFE, DT_RSVD_NODE_STR, dt_node_name, &base_addr, size);
    }
...
static int set_rsvmem_offset_size(char* name, uint64_t offset, uint64_t size)
{
    char dt_node_name[64];
    int ret = -1;

    if (is_cma_rsvmem) {
        ret = dtb_setprop_cma_rsvmem_size(DTB_ID_CFE, name, size);
    } else {
        sprintf(dt_node_name, "%s%s", DT_RSVD_PREFIX_STR, name);
        ret = dtb_set_reserved_memory(DTB_ID_CFE, dt_node_name, offset, size);
    }

    return ret;
}

static int del_rsvmem(char* name)
{
    char dt_node_name[64];
    int ret = -1;

    if (is_cma_rsvmem) {
        ret = dtb_del_cma_rsvmem(DTB_ID_CFE, name);
    } else {
        sprintf(dt_node_name, "%s%s", DT_RSVD_PREFIX_STR, name);
        ret = dtb_del_reserved_memory(DTB_ID_CFE, dt_node_name);        
    }
...
void set_reserved_memory(void)
{
    ...
    uint64_t sdram_size = mem_end;
    uint64_t rsrv_mem_required = 0;
    uint64_t adsl_rsv_size = 0;
    char dt_node_name[64];
#if defined(_CFE_REDUCED_XRDP_) || defined(LEGACY_RDP) || defined(DUMMY_RDP)
    int rdp_size[2], dhd_size[3], i, rdp_def_size, chk_dt;
    uint8_t bp_dhd_size;
    uint64_t dt_rdp_size = 0, dt_dhd_size = 0;
...
#endif
    uint32_t alignment_required = SZ_2M;
    ...
    is_cma_rsvmem = is_cma_rsvmem_enabled();
    if (is_cma_rsvmem)
        alignment_required = SZ_1M;

    /* DSL memory must be reserved first toward end of 256MB */
    if (get_rsvmem_size(ADSL_BASE_ADDR_STR, &adsl_rsv_size) == 0) {
        if (sdram_size < adsl_rsv_size) {
            printf("Not enough memory to reserve dsl memory 0x%x total memory 0x%x!\n", 
                adsl_rsv_size, sdram_size);
            adsl_rsv_size = 0;
        }
        ...
        if (adsl_rsv_size) {
            ...
            set_rsvmem_offset_size(ADSL_BASE_ADDR_STR, mem_end, adsl_rsv_size);
        } else
            del_rsvmem(ADSL_BASE_ADDR_STR);
    }
    /* adjust memory based on alignment requirement */
    rsrv_mem_required = ALIGN(alignment_required, rsrv_mem_required);
    mem_end = ALIGN_FLR(alignment_required, mem_end);
    ...
    for( i = 0; i < 2; i++ ) {
        ...
        rdp_size[i] = ALIGN(alignment_required, rdp_size[i]*SZ_1M);

	if (chk_dt) {
            if (get_rsvmem_size(rdp_str[i], &dt_rdp_size) == 0 ) {
    ...
    for( i = 0; i < 3; i++ ) {
        ...
        dhd_size[i] = ALIGN(alignment_required, dhd_size[i]*SZ_1M);

        /* check the device tree value, use that if it is large than nvram value */
        if( chk_dt ) {
            if (get_rsvmem_size(dhd_str[i], &dt_dhd_size) == 0 ) {
    ...
    if ( sdram_size < rsrv_mem_required)
    {
        ...
        * RDP reserved memory size and try again */
        rsrv_mem_required -= (rdp_size[0] + rdp_size[1]);
        rdp_size[0] = ALIGN(alignment_required, RDP_PARAM1_DEF_DDR_SIZE);
        rdp_size[1] = ALIGN(alignment_required, RDP_PARAM2_DEF_DDR_SIZE);
        rsrv_mem_required += rdp_size[0] + rdp_size[1];
        if (sdram_size < rsrv_mem_required) {
            printf("Not enough rdp and dhd memory to reserve 0x%x total memory 0x%x!\n", 
               rsrv_mem_required, sdram_size);
            return;
        }
    }

    for( i = 0; i < 2; i++ ) {
        if (set_rsvmem_offset_size(rdp_str[i], mem_end - rdp_size[i], rdp_size[i]) == 0) {
            mem_end = mem_end - rdp_size[i];
        } else
            rsrv_mem_required -= rdp_size[i];
    }

    for( i = 0; i < 3; i++ ) {
        if( dhd_size[i] != 0 ) {
            if (set_rsvmem_offset_size(dhd_str[i], mem_end - dhd_size[i], dhd_size[i]) == 0) {
                mem_end = mem_end - dhd_size[i];
            } else
                rsrv_mem_required -= dhd_size[i];
        }
        else
            del_rsvmem(dhd_str[i]);        
    }
#endif
    ...
    if (is_cma_rsvmem ) {
        ...
        if (rsrv_mem_required == 0 ) {
            ...
            rsrv_mem_required = ALIGN(SZ_4M, rsrv_mem_required);
            mem_end = ALIGN_FLR(SZ_4M, mem_end);
            printf("Total 0x%08x bytes CMA reserved memory @ 0x%08x\n", 
                (uint32_t)rsrv_mem_required, (uint32_t)mem_end);

            sprintf(dt_node_name, "%s%s", DT_RSVD_PREFIX_STR, CMA_BASE_ADDR_STR);
            dtb_set_reserved_memory(DTB_ID_CFE, dt_node_name, mem_end, rsrv_mem_required);
        }
#ifndef CONFIG_ARM64
        /* disable system default cma for armv7. For armv8, 4.1 kernel has bug that cause
           dma_allocate_coherent to fail when default cma is disabled. It is fixed in 4.19.
           Turn this back on when we migrate to 4.19 to save meemory.
         */
        dtb_set_bootargs(DTB_ID_CFE, "cma=0M", 1);
#endif
    }
}
...
{
    /* boot cmd line priority:
       1. CMDLINE_FORCE/OVERRIDE from PROFILE. This is done by linux kernel ignore dtb chosen
          bootarg and use CONFIG_CMDLINE directly
       2. CFE command replace dtb chosen bootarg.
       3. CFE command append dtb and PROFILE BUILD_CFE_APPEND_BOOT_CMDLINE append dtb.The CFE
           command and profile flag are accumulative.  
    */
...
#endif
    if (blparms_bootcmd[0])
        printf("CFE modified boot cmdline \"%s\"\n", blparms_bootcmd);

    if (blparms_bootcmd[0] && blparms_bootcmd[0] != '+' ) {
        /* replace bootarget from cfe cmd input */
        dtb_set_bootargs(DTB_ID_CFE, blparms_bootcmd, 0);
    } else {
        /* append bootarget from runtime cfg, cfe cmd input and profile input */
#if defined(_BCM963138_)
        if(get_nr_cpus(&nr_cpus) == 0 )
        {
            if(nr_cpus == 1 )
                dtb_set_bootargs(DTB_ID_CFE, "nosmp", 1);
        }
        if(cpu_limited == 1)
            dtb_set_bootargs(DTB_ID_CFE, "nosmp", 1);
#endif
        ...
        dtb_set_chosen_root(DTB_ID_CFE, get_nand_chosen_root());
#else
#if INC_EMMC_FLASH_DRIVER
        dtb_set_bootargs(DTB_ID_CFE, "rootwait", 1);
        dtb_set_chosen_root(DTB_ID_CFE, get_emmc_chosen_root());
#endif
#endif        
     
        if (erase_psi)
            dtb_set_bootargs(DTB_ID_CFE, "erase_psi", 1);

        if (blparms_bootcmd[0] == '+')
            dtb_set_bootargs(DTB_ID_CFE, &blparms_bootcmd[1], 1);

#ifdef CONFIG_CFE_APPEND_BOOT_CMDLINE
        if (strlen(CONFIG_CFE_APPEND_BOOT_CMDLINE)) {
            printf("CFE appending \"%s\" from PROFILE to boot cmdline\n", CONFIG_CFE_APPEND_BOOT_CMDLINE);
            dtb_set_bootargs(DTB_ID_CFE, CONFIG_CFE_APPEND_BOOT_CMDLINE, 1);
        }
#endif
    }
    return;
...
#endif
#if defined (_BCM96858_) || defined (_BCM94908_) || defined(_BCM963158_) || defined(_BCM96878_) || \
    defined(_BCM96846_) || defined(_BCM96856_) || defined(_BCM963138_) || defined(_BCM963178_) || defined(_BCM947622_)
...
extern unsigned long fmw_arg0;
    ...
    dtb_prepare(DTB_ID_CFE, DTB_PREPARE_FDT, NULL, (unsigned char*)CFG_DTB_ADDRESS, 4096);
    dtb_set_bootargs(DTB_ID_CFE, "console=ttyS0,1562500 lpj=800000 ikosboot ", 1);
#else
