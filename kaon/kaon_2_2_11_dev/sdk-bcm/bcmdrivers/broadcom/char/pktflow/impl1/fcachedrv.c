...
struct {
    DECLARE_HASHTABLE(hash, 6); /*64 hash buckets */
...
int fc_netdev_is_valid(void *dev_p)
{
...
int fc_blog_check_netdevs( Blog_t *blog_p )
{
	...
	if(blog_p->tx_dev_p)	
        ...
        {
			dev_p = DEVP_DETACH_DIR(blog_p->virt_dev_p[i]);
...
/*
 *------------------------------------------------------------------------------
 * Function Name: fcacheDrvHostNetdevProcfs
 * Description  : Handler to list Host Device 
 *------------------------------------------------------------------------------
 */
static inline ssize_t fcacheDrvHostNetdevProcfs(struct file *file, char __user *page,
								  size_t len, loff_t *offset)
{
    int index = 0;
    int bytes = 0;
    ...
    /*dump only devices that fit in a page */

	if ( *offset == 0 ) 
	{
		bytes += sprintf( p + bytes, PKTFLOW_MODNAME PKTFLOW_VER_STR "\n" );
		bytes += sprintf( p + bytes, " idx		fc_ndev      netdev        name \n" );
		...
		hash_for_each(fc_ndev_db_g.hash, bucket, fc_ndev_p, hnode)
		{
			...
			bytes += sprintf( p+bytes, "%3u %px %px %s \n",
...
static struct file_operations fcacheDrvHostNetdevProcfs_proc = {
        .read = fcacheDrvHostNetdevProcfs,
};
 ...
 */  
    ...
    if ( fcacheDrv_g.proc_fs_created == 0 )
    {
        ...
        entry = proc_create_data(FCACHE_MISC_PROCFS_DIR_PATH "/host_netdev", S_IRUGO,
            NULL, &fcacheDrvHostNetdevProcfs_proc, (void*)NULL);
        if (!entry)
        {
