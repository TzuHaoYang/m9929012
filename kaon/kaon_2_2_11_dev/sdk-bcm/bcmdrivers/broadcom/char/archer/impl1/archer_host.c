...
archer_mode_t archer_mode_g = ARCHER_HOST_MODE_DEFAULT;
...
archer_host_hash_table_t archer_host_hash_table_g;

static int archer_host_hash_table_add(BlogEthAddr_t *addr_p)
{
    ...
    {
        archer_host_hash_table_entry_t *entry_p = &bucket_p->entry[entry_index];
    ...
    archer_host_hash_table_g.enabled = 0;

    bcm_print("Could not Add MAC Address to Hash Table: " BLOG_ETH_ADDR_FMT,
              addr_p->u8[0], addr_p->u8[1], addr_p->u8[2],
              addr_p->u8[3], addr_p->u8[4], addr_p->u8[5]);

    return -1;
}

static int archer_host_hash_table_delete(BlogEthAddr_t *addr_p)
{
    int bucket_index = archer_host_hash_table_bucket_index(addr_p);
    archer_host_hash_table_bucket_t *bucket_p =
        &archer_host_hash_table_g.bucket[bucket_index];
    ...
    for(entry_index=0; entry_index<ARCHER_HOST_HASH_TABLE_ENTRY_MAX; ++entry_index)
    {
        ...
        if(entry_p->valid)
        {
            if(entry_p->addr.u16[0] == addr_p->u16[0] &&
               entry_p->addr.u16[1] == addr_p->u16[1] &&
               entry_p->addr.u16[2] == addr_p->u16[2])
            {
                memset(entry_p, 0, sizeof(archer_host_hash_table_entry_t));
    ...
    __logError("Could not Delete MAC Address from Hash Table: " BLOG_ETH_ADDR_FMT,
               addr_p->u8[0], addr_p->u8[1], addr_p->u8[2],
               addr_p->u8[3], addr_p->u8[4], addr_p->u8[5]);
...
static void archer_host_hash_table_init(void)
{
    memset(&archer_host_hash_table_g, 0, sizeof(archer_host_hash_table_t));

    archer_host_hash_table_g.enabled = 1;
}
...
static ARCHER_HOST_ERROR_t archer_host_mac_addr_table_add(uint16_t *index_p, const void *val_p)
{
    ...
    for(ii = 0; ii < ARCHER_HOST_MAC_ADDR_TABLE_SIZE; ++ii)
    {
        ...
        if(!ref_count)
        {
            ...
            /* Add MAC Address to Hash Table */
            archer_host_hash_table_add(&entry->mac_addr);
...
static ARCHER_HOST_ERROR_t archer_host_mac_addr_table_delete(uint16_t index)
{
    ...
    if(ref_count)
    {
        ...
        if(!ref_count)
        {
            /* Delete MAC Address from Hash Table */
            archer_host_hash_table_delete(&host_mac_addr);
        }
...
static void archer_host_mac_addr_table_dump(void)
{
    ...
    bcm_print("idx                  MAC  mac_ref  hash_table (%s)\n",
              archer_host_hash_table_g.enabled ? "Enabled" : "Disabled");
    ...
    for(ii = 0; ii < ARCHER_HOST_MAC_ADDR_TABLE_SIZE; ++ii)
    {
        ...
        if(ref_count)
        {
            bcm_print("%3u  <%02x:%2x:%02x:%02x:%02x:%02x>  %7d  %s\n", ii, 
                      mac_addr.u8[0], mac_addr.u8[1], mac_addr.u8[2],
                      mac_addr.u8[3], mac_addr.u8[4], mac_addr.u8[5], ref_count,
                      archer_host_hash_table_match(&mac_addr) ? "Yes" : "No");
        }
    }
}
