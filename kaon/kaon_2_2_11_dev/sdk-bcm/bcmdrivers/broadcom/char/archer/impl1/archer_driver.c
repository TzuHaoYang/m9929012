...
#define ARCHER_DRIVER_SYSPORT_TM_PHY_2_5G_MBS  150000
#define ARCHER_DRIVER_SYSPORT_TM_PHY_1G_MBS    35000
#define ARCHER_DRIVER_DPI_MBS                  2000
...
static int archer_driver_enet_phy_speed_set(void *arg_p)
{
    ...
    int mbs = (phy_speed->kbps > 1000000) ? ARCHER_DRIVER_SYSPORT_TM_PHY_2_5G_MBS :
        ARCHER_DRIVER_SYSPORT_TM_PHY_1G_MBS;
    int logical_port;
    int ret;
    ...
    if(!sysport_tm_phy_set(logical_port, kbps, mbs))
    {
        bcm_print("%s: sysport_tm port shaper set to %d kbps, %d mbs (phy speed %d kbps)\n",
                  phy_speed->dev->name, kbps, mbs, phy_speed->kbps);
    }
...
static int archer_dpi_command(unsigned long arg)
{
    archer_dpi_arg_t dpi_arg;
    int min_mbs;
    int max_mbs;
    int ret = 0;

    copy_from_user(&dpi_arg, (void *)arg, sizeof(archer_dpi_arg_t));

    switch(dpi_arg.cmd)
    {
        case ARCHER_DPI_CMD_MODE_SET:
            ret = archer_dpi_mode_set(dpi_arg.mode);

            break;

        case ARCHER_DPI_CMD_MODE_GET:
            archer_dpi_mode_get(&dpi_arg.mode);

            copy_to_user((void *)arg, &dpi_arg, sizeof(archer_dpi_arg_t));

            break;

        case ARCHER_DPI_CMD_STATS:
            archer_dpi_stats();

            break;

        case ARCHER_SQ_CMD_QUEUE_SET:
            ret = archer_dpi_sq_queue_set(dpi_arg.queue_index,
                                          dpi_arg.min_kbps, ARCHER_DRIVER_DPI_MBS,
                                          dpi_arg.max_kbps, ARCHER_DRIVER_DPI_MBS);
            break;

        case ARCHER_SQ_CMD_QUEUE_GET:
            ret = archer_dpi_sq_queue_get(dpi_arg.queue_index,
                                          &dpi_arg.min_kbps, &min_mbs,
                                          &dpi_arg.max_kbps, &max_mbs);
            if(!ret)
            {
                copy_to_user((void *)arg, &dpi_arg, sizeof(archer_dpi_arg_t));
            }
        ...
        case ARCHER_SQ_CMD_PORT_SET:
            ret = archer_dpi_sq_port_set(dpi_arg.min_kbps, ARCHER_DRIVER_DPI_MBS);

            break;

        case ARCHER_SQ_CMD_PORT_GET:
            ret = archer_dpi_sq_port_get(&dpi_arg.min_kbps, &min_mbs);
            if(!ret)
            {
                copy_to_user((void *)arg, &dpi_arg, sizeof(archer_dpi_arg_t));
            }
        ...
        case ARCHER_SQ_CMD_ARBITER_SET:
            ret = archer_dpi_sq_arbiter_set(dpi_arg.arbiter);

            break;

        case ARCHER_SQ_CMD_ARBITER_GET:
            ret = archer_dpi_sq_arbiter_get(&dpi_arg.arbiter);
            if(!ret)
            {
                copy_to_user((void *)arg, &dpi_arg, sizeof(archer_dpi_arg_t));
            }
        ...
        default:
            __logError("Invalid cmd %d", dpi_arg.cmd);
...
EXPORT_SYMBOL(archer_dpi_mode_set);
