...
void i5MessageAPCapabilityReportReceive(i5_message_type *pmsg)
{
  ...
  rc |= i5TlvAPCapabilitiesTypeExtract(pmsg);
  rc |= i5TlvAPRadioBasicCapabilitiesTypeExtract(pmsg);
  rc |= i5TlvAPHTCapabilitiesTypeExtract(pmsg);
  rc |= i5TlvAPVHTCapabilitiesTypeExtract(pmsg);
  i5MessageFree(pmsg);
}
...
void i5MessageChannelSelectionRequestReceive(i5_message_type *pmsg)
{
  ...
  /* look for vendor specific channel set TLV. If present
   * process vendor TLV for interface and skip channel preference
   * type extract routine
   */
  rc |= i5MessageGetVendorSpecificTlvWithCb(pmsg);

  i5Trace(" rc[%d] after parsing vendor TLV \n", rc);
  i5MessageReset(pmsg);
  if (rc != 0) {
    /* Either No vendor TLV found or error in TLV processing,
     * let channel preference TLV extract routine to process
     * channel selection request message.
     */
    i5Trace("No vendor TLV found, parse channel preference TLV \n");
    rc = 0;
