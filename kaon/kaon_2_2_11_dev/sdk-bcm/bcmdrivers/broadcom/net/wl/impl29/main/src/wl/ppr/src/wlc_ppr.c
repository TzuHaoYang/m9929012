...
void ppr_delete(osl_t *osh, ppr_t* pprptr)
{
	ASSERT((pprptr->ch_bw == WL_TX_BW_20) || (pprptr->ch_bw == WL_TX_BW_40) ||
		(pprptr->ch_bw == WL_TX_BW_80) || (pprptr->ch_bw == PPR_BW_MAX) ||
	...
	for (chain = WL_TX_CHAINS_1; chain <= max_chain; chain++) {
		...
		if (len + DSSS_TLV_LEN + BCM_TLV_HDR_SIZE <= tlv_buf_len) {
			...
			if (bw_pwrs != NULL) {
				...
				if (powers) {
					memcpy(dsss_tlv->data, powers, sizeof(ppr_dsss_rateset_t));
					found = TRUE;
				}
			}
			...
			case PPR_RGRP_DSSS_ID:
				{
					...
					ppr_set_dsss(pprptr, dsss_tlv->bw, dsss_tlv->chains,
						(ppr_dsss_rateset_t*)dsss_tlv->data);
				}
				break;
			case PPR_RGRP_OFDM_ID:
				{
					...
					ppr_set_ofdm(pprptr, ofdm_tlv->bw, ofdm_tlv->mode,
						ofdm_tlv->chains,
						(ppr_ofdm_rateset_t*)(ofdm_tlv->data));
				}
				break;
			case PPR_RGRP_MCS_ID:
				{
					...
					ppr_set_vht_mcs(pprptr, mcs_tlv->bw, mcs_tlv->nss,
						...
						(ppr_vht_mcs_rateset_t*)mcs_tlv->data);
				}
				break;
			default:
				ASSERT(0);
...
uint32 ppr_get_tlv_size(ppr_t* pprptr, wl_tx_bw_t bw, uint32 max_chain)
{
	...
	case WL_TX_BW_40:
		/* 20IN40 and 40 */
		ret = 2*mcs_ofdm_size + dsss_size;
		break;
	case WL_TX_BW_80:
		/* 20IN80, 40IN80,80 */
		ret = 3*mcs_ofdm_size + dsss_size;
		break;
	case WL_TX_BW_160:
		/* 20IN160, 40IN160, 80IN160, 160 */
		ret = 4*mcs_ofdm_size + dsss_size;
		break;
	case WL_TX_BW_8080:
		/* 20IN8080, 40IN8080, 80IN8080, 8080, 8080CHAN2 */
		ret = 5*mcs_ofdm_size + dsss_size;
		break;
	default:
		break;
	};
