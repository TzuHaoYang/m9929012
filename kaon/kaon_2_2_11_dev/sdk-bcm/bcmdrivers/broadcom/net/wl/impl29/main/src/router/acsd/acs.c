 ...
 *
 * $Id: acs.c 767645 2018-09-20 05:59:23Z $
 */
...
acs_fcs_retrieve_config(acs_chaninfo_t *c_info, char * prefix)
{
	...
	if ((str = nvram_get(strcat_r(prefix, "acs_ignore_txfail_on_far_sta", tmp))) == NULL) {
		fcs_info->acs_ignore_txfail_on_far_sta = ACS_IGNORE_TXFAIL_ON_FAR_STA;
#ifdef MULTIAP
		/* For MultiAP ignore tx fail on far sta should be enabled always */
		if (((str = nvram_get(strcat_r(prefix, "map", tmp))) != NULL) && atoi(str))
			fcs_info->acs_ignore_txfail_on_far_sta = 1;
#endif /* MULTIAP */
	} else {
		fcs_info->acs_ignore_txfail_on_far_sta = atoi(str);
	}
	...
	if ((str = nvram_get(strcat_r(prefix, "acs_far_sta_rssi", tmp))) == NULL) {
		...
		if (((str = nvram_get(strcat_r(prefix, "map", tmp))) != NULL) && atoi(str)) {
			if ((str = nvram_get(strcat_r(prefix, "wbd_weak_sta_cfg", tmp))) != NULL) {
				...
				fcs_info->acs_far_sta_rssi = wbd_rssi;
			}
		}
#endif /* MULTIAP */
	} else {
...
acs_pick_chanspec_common(acs_chaninfo_t *c_info, int bw, int score_type)
{
	...
	if ((!acs_is_initial_selection(c_info)) && (!ACS_FCS_MODE(c_info))) {
		for (i=0; i < chstats->count; i++) {
			stats = &chstats->stats[i];
			chspec = wf_chspec_ctlchspec(c_info->selected_chspec);
			if (chspec == stats->chanspec) {
				...
				if (txop_score < ACS_TXOP_LIMIT) {
					...
					ACSD_INFO("timestamp %dtxop_score %d\n",c_info->timestamp,
						txop_score);
					break;
				} else {
					ACSD_INFO("staying on current channel %x score %d\n",
						c_info->selected_chspec, txop_score);
...
#if defined(MULTIAP) && defined(DSLCPE)
        /* when MULTIAP is defined, get mode to fixed mode for STA
         * radio */
        if(nvram_get("acsd_set_fixmode")) 
        {
            char conf_word[128];
            acs_safe_get_conf(conf_word, sizeof(conf_word),"wbd_mode");
            if (strcmp(conf_word, "")) {
                if(atoi(conf_word)==2) {
                    acs_safe_get_conf(conf_word, sizeof(conf_word),
                            strcat_r(prefix, "mode", tmp));
                    if (!strcmp(conf_word, "sta"))
...
acs_candidate_score_txpwr(ch_candidate_t *candi, acs_chaninfo_t* c_info)
{
	...
	bool is_hp = !ACS_IS_LOW_POW_CH(wf_chspec_ctlchan(candi->chspec), is_eu);		/* is high power ch */
	...
	if (is_eu) { /* ETSI/EDCRS_EU/Europe */
		...
		/* Prefer DFS high power, DFS low power, low power channel, weather */
		score = 3 - (1 + is_hp + is_dfs - 3 * is_we);
	} else { /* FCC/US */
		/* Prefer High power (DFS/non), DFS, low power non-DFS channels */
		score = 2 - (2 * is_hp + is_dfs);
	}
	candi->chscore[CH_SCORE_TXPWR].score = score;
	ACSD_INFO(" txpower score is %d chanspec %d\n",score, candi->chspec);
}
...
static bool
acs_check_for_nondfs_chan(acs_chaninfo_t *c_info, int bw)
{
	int i;
	ch_candidate_t *candi;
	bool ret = FALSE;
...
acs_fcs_tx_idle_check(acs_chaninfo_t *c_info)
{
	...
	ret = wl_iovar_get(c_info->name, "counters", cntbuf, ACSD_WL_CNTBUF_SIZE);
	if (ret < 0) {
...
int acs_update_rssi(acs_chaninfo_t *c_info, unsigned char *addr)
{
	...
	ACSD_FCS("txfail sta intf:%s Mac:%02x:%02x:%02x:%02x:%02x:%02x RSSI: %d\n",
		c_info->name, addr[0], addr[1], addr[2], addr[3],
		addr[4], addr[5], dtoh32(scb_val.val));
	if (dtoh32(scb_val.val) < fcs_info->acs_far_sta_rssi) {
		c_info->sta_status |= ACS_STA_EXIST_FAR;
	} else {
...
acs_get_traffic_info(acs_chaninfo_t * c_info, acs_traffic_info_t *t_info)
{
	...
	if (wl_iovar_get(c_info->name, "counters", cntbuf, ACSD_WL_CNTBUF_SIZE) < 0) {
