 ...
 *
 * $Id: wbd_shared.c 782533 2019-12-23 06:42:04Z $
 */
...
wbd_retrieve_common_config(wbd_info_t *info)
{
	...
	if (str) {
		num = sscanf(str, "%u %u %u", &info->max.tm_actframe,
			&info->max.tm_offchan_actframe,
			&info->max.tm_bcn_req_frame);
	}
	if (!str || num != 3) {
		info->max.tm_actframe = WBD_TM_SLV_ACT_FRAME_INTERVAL;
		info->max.tm_offchan_actframe = WBD_TM_SLV_OFFCHAN_AF_INTERVAL;
	...
	/* Get NVRAM: Is beacon report rcpi to rssi converion required */
	if (blanket_get_config_val_int(NULL, WBD_NVRAM_CONV_RCPI_TO_RSSI,
		WBD_DEF_CONV_RCPI_TO_RSSI)) {
		info->flags |= WBD_INFO_FLAGS_CONV_RCPI_RSSI;
	}
	...
	WBD_DEBUG("%s=%d\n %s=%d\n"
		...
		WBD_NVRAM_TM_SLV_ACT_FRAME_INTERVAL, info->max.tm_actframe,
		info->max.tm_offchan_actframe, info->max.tm_bcn_req_frame,
