 ...
 *
 * $Id: wlconf.c 772418 2019-02-22 12:20:20Z $
 */
...
static void  wlconf_revinfo_dtoh( wlc_rev_info_t *rev) {

	rev->vendorid = etoh32(rev->vendorid);	/* PCI vendor id */
	rev->deviceid = etoh32(rev->deviceid); 	/* device id of chip */
	rev->radiorev = etoh32(rev->radiorev);	/* radio revision */
	rev->chiprev = etoh32(rev->chiprev);	/* chip revision */
	rev->corerev = etoh32(rev->corerev);	/* core revision */
	rev->boardid = etoh32(rev->boardid);	/* board identifier (usu. PCI sub-device id) */
	rev->boardvendor = etoh32(rev->boardvendor);	/* board vendor (usu. PCI sub-vendor id) */
	rev->boardrev = etoh32(rev->boardrev);	/* board revision */
	rev->driverrev = etoh32(rev->driverrev);	/* driver version */
	rev->ucoderev = etoh32(rev->ucoderev);	/* microcode version */
	rev->bus = etoh32(rev->bus);		/* bus type */
	rev->chipnum = etoh32(rev->chipnum);	/* chip number */
	rev->phytype = etoh32(rev->phytype);	/* phy type */
	rev->phyrev = etoh32(rev->phyrev);		/* phy revision */
	rev->anarev = etoh32(rev->anarev);		/* anacore rev */
	rev->chippkg = etoh32(rev->chippkg);	/* chip package info */
	rev->nvramrev = etoh32(rev->nvramrev);	/* nvram revision number */
}
#endif
...
{
#ifdef DSLCPE_ENDIAN
	int ret=wl_ioctl(ifname, cmd, pval, sizeof(int));
	*pval=etoh32(*pval);
	return ret;
#else
...
#ifdef DSLCPE_ENDIAN
	key.len = htoe32(key.len);
	key.flags = htoe32(key.flags);
#endif
...
#ifdef DSLCPE_ENDIAN
	dfs_frcd->version = etoh16(dfs_frcd->version);
#endif
	...
	foreach(val, nvram_safe_get("wl_dfs_pref"), next) {
		if (atoi(val)) {
			...
			if (dfs_frcd->chspec_list.num > 6)
				return;
#ifdef DSLCPE_ENDIAN			
			dfs_frcd->chspec_list.list[dfs_frcd->chspec_list.num++] = htoe16(chanspec);
#else
...
#ifdef DSLCPE_ENDIAN
	dfs_frcd->version = htoe16(DFS_PREFCHANLIST_VER);
	dfs_frcd->chspec = htoe16(dfs_frcd->chspec);
	dfs_frcd->chspec_list.num = htoe32(dfs_frcd->chspec_list.num);
#else
...
#ifdef DSLCPE_ENDIAN
		txparams[aci].max_rate = etoh16(txparams[aci].max_rate);
#endif
...
wlconf_set_wme(char *name, char *prefix)
{
	...
	/* WME Tx parameters setting */
	{
		...
		for (j = 0; j < AC_COUNT; j++) {
			...
			txparams[j].short_retry = htoe32(atoi(srl));

			/* update short fallback limit */
			txparams[j].short_fallback =htoe32(atoi(sfbl));

			/* update long retry limit */
			txparams[j].long_retry =htoe32(atoi(lrl));

			/* update long fallback limit */
			txparams[j].long_fallback = htoe32(atoi(lfbl));

			/* update max rate */
			txparams[j].max_rate = htoe32(atoi(maxrate));
#else
			/* update short retry limit */
...
#define AUTOWPA(cfg) ((cfg) == (WPA_AUTH_PSK | WPA2_AUTH_PSK | WPA2_AUTH_FT))
	...
	*/
...
#ifdef DSLCPE_ENDIAN
			psk.key_len = htoe16(psk.key_len);
			psk.flags = htoe16(psk.flags);
#endif
	...
	if (val & (WPA2_AUTH_PSK | WPA2_AUTH_UNSPECIFIED | WPA2_AUTH_FT)) {
		int phytype;
		char var[8];
		int band, fbt, wbd_fbt;

		/* Set mfp only if fbt is not enabled. */
		fbt = atoi(nvram_safe_get(strcat_r(prefix, "fbt", tmp)));
		wbd_fbt = atoi(nvram_safe_get(strcat_r(prefix, "wbd_fbt", tmp)));
		if ((fbt <= 0) && (wbd_fbt <= 0)) {
			...
			/* Disable MFP explicitly, if FBT is enabled. This is required because
			 * MFP flag is persistent across 'rc restart' and in MAP onboarding
			 * FBT may be getting enabled at a later stage. So if it is not disabled
			 * explicitly, a reboot will be required to disable MFP
			 */
			val = WL_MFP_DISABLE;
		}
...
#ifdef DSLCPE_ENDIAN
			retry_limit.tid = htoe32(j);
			retry_limit.retry =htoe32(atoi(retry));
#else
			retry_limit.tid = j;
...
#ifdef DSLCPE_ENDIAN
		trfmgmt->dst_port = htoe16(trfmgmt->dst_port);
		trfmgmt->src_port = htoe16(trfmgmt->src_port);
		trfmgmt->prot = htoe16(trfmgmt->prot);
		trfmgmt->flags = htoe16(trfmgmt->flags);
		trfmgmt->priority = htoe32(trfmgmt->priority);
#endif
...
#ifdef DSLCPE_ENDIAN
			trf_mgmt_dwm_filter->flags = htoe16(trf_mgmt_dwm_filter->flags);
			trf_mgmt_dwm_filter->priority = htoe32(trf_mgmt_dwm_filter->priority);
			trf_mgmt_dwm_filter->dscp = htoe32(trf_mgmt_dwm_filter->dscp);
#endif
...
#ifdef DSLCPE_ENDIAN
		trf_mgmt_config.flags = htoe32(trf_mgmt_config.flags);
		trf_mgmt_config.host_ip_addr = htoe32(trf_mgmt_config.host_ip_addr);
		trf_mgmt_config.host_subnet_mask = htoe32(trf_mgmt_config.host_subnet_mask);
		trf_mgmt_config.downlink_bandwidth = htoe32(trf_mgmt_config.downlink_bandwidth);
		trf_mgmt_config.uplink_bandwidth = htoe32(trf_mgmt_config.uplink_bandwidth);
#endif
		...
		if (tm_filters_configured || dwm_filters_configured) {
...
#ifdef DSLCPE_ENDIAN
			trf_mgmt_config.trf_mgmt_enabled = htoe32(1);
#else
...
#ifdef DSLCPE_ENDIAN
				trf_mgmt_filter_list->num_filters = htoe32(trf_mgmt_filter_list->num_filters);
			...
			if (dwm_filters_configured) {
				dscp_filterlen =  trf_mgmt_dwm_filter_list->num_filters *
					...
					OFFSETOF(trf_mgmt_filter_list_t, filter);
#ifdef DSLCPE_ENDIAN
				trf_mgmt_dwm_filter_list->num_filters = htoe32(trf_mgmt_dwm_filter_list->num_filters);
...
#ifdef DSLCPE_ENDIAN
		ieinfo->pktflag = etoh32(ieinfo->pktflag);
#endif
...
#ifdef DSLCPE_ENDIAN
	ie_setbuf->vndr_ie_buffer.iecount = htoe32(ie_setbuf->vndr_ie_buffer.iecount);
#endif
...
#ifdef DSLCPE_ENDIAN
	ie_setbuf->vndr_ie_buffer.iecount = htoe32(ie_setbuf->vndr_ie_buffer.iecount);
	ie_setbuf->vndr_ie_buffer.vndr_ie_list[0].pktflag = htoe32(ie_setbuf->vndr_ie_buffer.vndr_ie_list[0].pktflag);
#endif
...
wlconf(char *name)
{
...
#else
	/* DSLCPE platfrom does not need to restore default from wlconf
	 * as it support runtime restart and configure comes from DM 
	 */
	restore_defaults = 0;
#endif
...
#ifdef DSLCPE_ENDIAN
			msglevel64.low = htoe32(val);
#else
			msglevel64.low = val;
#endif
			val = (int)strtoul(nvram_get("wl_msglevel2"), NULL, 0);
#ifdef DSLCPE_ENDIAN
			msglevel64.high = htoe32(val);
#else
			msglevel64.high = val;
#endif
...
#ifdef DSLCPE_ENDIAN
			ssid.SSID_len = htoe32(ssid.SSID_len);
#endif
...
#ifdef DSLCPE_ENDIAN
		roam.val = htoe32(WLC_ROAM_NEVER_ROAM_TRIGGER);
		roam.band = htoe32(WLC_BAND_ALL);
#else
	...
	/* Set DWDS and Multi-Ap only for AP or STA modes */
	for (i = 0; i < bclist->count; i++) {
		int map_val = 0;
		val = 0;
		bsscfg = &bclist->bsscfgs[i];
		...
		if (ap || apsta) {
			 ...
			 */
			 if (wlconf_get_wbd_slave_mode(bsscfg->ifname) &&
				!strtol(nvram_safe_get("map_multi_hop"), NULL, 0)) {
				val = 1;
			}
#endif /* BCM_WBD */
...
#ifdef DSLCPE_ENDIAN
	val = htoe32(val+(WL_RADIO_SW_DISABLE << 16));
	...
	*(next = buf) = '\0';
...
#ifdef DSLCPE_ENDIAN
		country_spec.rev = htoe32(atoi(country_rev));
#else
...
#ifdef DSLCPE_ENDIAN
		country_spec.rev = etoh32(country_spec.rev);
#endif
	...
	if ((rev.chipnum == BCM5357_CHIP_ID) || (rev.chipnum == BCM53572_CHIP_ID)) {
		...
		if (val > 0) {
			...
			nvram_commit();
#ifdef DSLCPE_ENDIAN
			setbuf.sleep = htoe32(val);
			setbuf.delay = htoe32(1);
#else
			setbuf.sleep = val;
			setbuf.delay = 1;
#endif
...
#ifdef DSLCPE_ENDIAN
		val = htoe32(atoi(nvram_safe_get(strcat_r(prefix, "tpc_db", tmp))));
#else
	...
	if (WLCONF_PHYTYPE_11N(phytype)) {
		struct {
			int bandtype;
#ifdef DSLCPE_ENDIAN //Endian issue
			int bw_cap;
#else
			uint8 bw_cap;
#endif
		...
		if (val != -1) 
			...
			wl_iovar_setint(name, "vhtmode", 1);
#endif
...
#ifdef DSLCPE_ENDIAN
		param.bandtype = htoe32(bandtype);
		param.bw_cap = htoe32(val);
#else
...
#ifdef DSLCPE_WLCSM_EXT
		/* in case it is auto channel, set init channel to prevent random
		 * init channel number when DFS enabled */
		if(wlcsm_trim_str(str)[0]=='0') {
			if(bandtype==WLC_BAND_5G) {
				switch(nbw){
					case WL_CHANSPEC_BW_40:
						chanspec = wf_chspec_aton("36l");
						break;
					case WL_CHANSPEC_BW_80:
						chanspec = wf_chspec_aton("36/80");
						break;
					case WL_CHANSPEC_BW_160:
						chanspec = wf_chspec_aton("36/160");
						break;
					default:
						chanspec = wf_chspec_aton("36");
						break;
				}
			} else
				chanspec = wf_chspec_aton("1");
		} else
#endif
...
#ifdef DSLCPE_ENDIAN
			ci.target_channel = etoh32(ci.target_channel);
#endif
	...
	if (bandtype == WLC_BAND_2G) {
		...
		/* Set gmode */
#ifdef DSLCPE_ENDIAN
		gmode = htoe32(atoi(nvram_safe_get(strcat_r(prefix, "gmode", tmp))));
#else
		...
		if (nvram_match(tmp, "auto")) {
			override = htoe32(WLC_PROTECTION_AUTO);
			control = htoe32(WLC_PROTECTION_CTL_OVERLAP);
		}
#else
		if (nvram_match(tmp, "auto")) {
	...
	*/
	...
	/* Security and ATM percentage settings for each BSS Configuration */
	for (i = 0; i < bclist->count; i++) {
		bsscfg = &bclist->bsscfgs[i];
		wlconf_security_options(name, bsscfg->prefix, bsscfg->idx,
				mac_spoof, wet || sta || apsta || psta || psr);

		/* Set airtime precentage for each BSS */
		val = atoi(nvram_safe_get(strcat_r(bsscfg->prefix, "atm_bssperc", tmp)));
		if (val) {
			WL_BSSIOVAR_SETINT(name, "atm_bssperc", bsscfg->idx, val);
		}
	}
...
#ifdef DSLCPE_ENDIAN
	val = htoe32(atoi(nvram_safe_get(strcat_r(prefix, "percal_delay", tmp))));
#else
	...
	/* set crsmincal threshold, do crsmincal if remianed time to next tbtt > threshold */
	str = nvram_get(strcat_r(prefix, "crsmincal_thresh_ms", tmp));
	if (str) {
		/* user specified phy cal period. */
		val = atoi(str);
		WL_IOVAR_SET(name, "crsmincal_thresh_ms", &val, sizeof(val));
	}

	/* Set antenna */
#ifdef DSLCPE_ENDIAN
	val = htoe32(atoi(nvram_safe_get(strcat_r(prefix, "antdiv", tmp))));
#else
...
legacy_mode:
		...
		if (obss_coex || channel == 0) {
			if (WLCONF_PHYTYPE_11N(phytype)) {
				...
				/* select a channel */
#ifdef DSLCPE_ENDIAN
			        val = htoe32(wlconf_auto_channel(name));
#else
...
#ifdef DSLCPE_ENDIAN
		maclist->count = htoe32(maclist->count);
#endif
...
wlconf_start(char *name)
{
	...
	if (ap || apsta || wds) {
		/* Set lazy WDS mode */
#ifdef DSLCPE_ENDIAN
		val = htoe32(atoi(nvram_safe_get(strcat_r(prefix, "lazywds", tmp))));
#else
...
#ifdef DSLCPE_ENDIAN
		setbuf.bsscfg_idx = htoe32(bclist->bsscfgs[i].idx);
#else
		setbuf.bsscfg_idx = bclist->bsscfgs[i].idx;
#endif
		setbuf.enable = 0;
	...
	if (!wmf_bss_disab &&
		...
		val = atoi(nvram_safe_get(strcat_r(prefix, "wmf_ucigmp_query", tmp)));
		(void)wl_iovar_setint(name, "wmf_ucast_igmp_query", val);
		val = atoi(nvram_safe_get(strcat_r(prefix, "wmf_ucast_upnp", tmp)));
		(void)wl_iovar_setint(name, "wmf_ucast_upnp", val);
	}
...
#if !defined(DSLCPE) || defined (DSLCPE_WLCONF_MAIN)
int
main(int argc, char *argv[])
{
#ifdef DSLCPE_WLCSM_EXT
		WLCSM_SET_TRACE("wlconf");
#endif
