 ...
 *
 * $Id: vis_linux_main.c 714335 2017-08-04 05:02:21Z $
 */
...
get_counter_from_driver()
{
	...
	while (curnode != NULL) {
		if (curnode->enabled) {
			if ((g_configs.ispscan == 0 && isfirsttime == 1) ||
				(g_configs.ispscan != 0 && (g_configs.isstart != 0 ||
				g_configs.isautostart != 0)) || curnode->dutset.isscan == 1) {
				/*
...
main(int argc, char **argv)
{
	...
	char *value = nvram_get("vis_pscan");
	if (value)
		g_configs.ispscan = atoi(value);
