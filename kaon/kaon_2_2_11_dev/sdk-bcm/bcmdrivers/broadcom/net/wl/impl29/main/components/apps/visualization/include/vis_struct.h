 ...
 *
 * $Id: vis_struct.h 714335 2017-08-04 05:02:21Z $
 */
...
typedef struct configs_t_ {
	...
	uint32 ispscan; /* Whether to scan periodically */
} configs_t;
...
typedef struct ap_info_t_ {
	...
	char ssid[SSID_FMT_BUF_LEN];
#ifdef DSLCPE
	/* when int16 or int8 is casted to integer and use atoi() function to assign value
	   its value is not correctly assigned in BE platform, simple choice is to use int32
