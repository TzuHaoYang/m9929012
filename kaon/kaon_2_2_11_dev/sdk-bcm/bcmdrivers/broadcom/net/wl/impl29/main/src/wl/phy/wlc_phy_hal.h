 ...
 *
 * $Id: wlc_phy_hal.h 745487 2018-02-08 10:27:15Z $
 */
...
#define PHY_PERICAL_DELAY_EU 60 /* dly (ms) btw each cal for mphase if REGION_EU for Adaptivity */
...
#define PHY_CAP_2P5MHZ		0x00002000	/* 2.5 MHz channels supported */
#define PHY_CAP_5MHZ		0x00004000	/* 5   MHz channels supported */
#define PHY_CAP_10MHZ		0x00008000	/* 10  MHz channels supported */
...
extern void wlc_phy_core1_alt_reg_clk_dis(wlc_phy_t *ppi);
extern void wlc_phy_core1_alt_reg_clk_en(wlc_phy_t *ppi);
...
extern void wlc_phy_recover_phycrs_stuck(wlc_phy_t *ppi);
...
extern int wlc_phy_restore_cal_cache(wlc_phy_t *ppi);
#ifdef WLOLPC
...
#if defined BCMDBG || defined(PHYTXERR_DUMP)
...
extern void wlc_phy_swdiv_enab(wlc_phy_t *ppi, bool _swdiv);
...
extern int wlc_tof_seq_upd_dly(wlc_phy_t *ppi, bool tx, uint8 core, bool mac_suspend);
extern int wlc_phy_tof_seq_params(wlc_phy_t *ppi);
#else
...
extern int wlc_phy_kvalue(wlc_phy_t *ppi, chanspec_t chanspec, uint32 rspecidx, uint32 *kip,
	uint32 *ktp, uint8 seq_en);
#else
...
extern uint8 wlc_phy_get_rsdb_dfs_scan_in_progress(phy_info_t *pi);
extern void wlc_phy_set_rsdb_dfs_scan_in_progress(phy_info_t *pi, uint8 status);
#endif // endif
...
extern void wlc_phyhal_txpwrcap_set_cellstatus(wlc_phy_t *pih, int32 cellstatus);
extern int wlc_phy_txpwrcap_tbl_set(wlc_phy_t *pih, wl_txpwrcap_tbl_t *txpwrcap_tbl);
extern int wlc_phy_txpwrcap_tbl_get(wlc_phy_t *pih, wl_txpwrcap_tbl_t *txpwrcap_tbl);
extern bool wlc_phy_txpwrcap_get_enabflg(wlc_phy_t *pih);
#ifdef WLC_SW_DIVERSITY
extern void wlc_phy_txpwrcap_to_shm(wlc_phy_t *pih, uint8 tx_ant, uint8 rx_ant);
...
#ifndef DONGLEBUILD
	#ifndef IMPLICIT_TXBF
		#define IMPLICIT_TXBF
		#define TXBF_RPCAL_ENAB(aci)	(aci->_txbf_rpcal)
	#endif
#else
	#ifdef IMPLICIT_TXBF
		#define TXBF_RPCAL_ENAB(aci)	(aci->_txbf_rpcal)
	#else
		#define TXBF_RPCAL_ENAB(aci)	(0)
	#endif
#endif /* !DONGLEBUILD */
#ifdef IMPLICIT_TXBF
extern bool wlapi_scan_in_progress(wlc_phy_shim_info_t *physhim);
extern uint8 wlapi_implicitTXBF_enabled(wlc_phy_shim_info_t *physhim);

#endif /* IMPLICIT_TXBF */
uint32 wlapi_get_time_left_for_next_tbtt(wlc_phy_shim_info_t *physhim);
#endif	/* _wlc_phy_h_ */
