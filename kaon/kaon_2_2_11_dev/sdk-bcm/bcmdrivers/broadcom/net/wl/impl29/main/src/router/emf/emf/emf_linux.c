 ...
 *
 * $Id: emf_linux.c 734775 2017-12-06 09:19:26Z $
 */
...
#ifdef DSLCPE
#include <linux/version.h> /* DSLCPE */
#else
...
void *wl_wmf_get_igsc(int ifindex) {
...
void *wl_wmf_scbfind(int ifindex,unsigned char *mac) {
	struct net_device *from_device= dev_get_by_index(&init_net, ifindex);
	void *ret=NULL;
	if(from_device) {
		struct priv_link *wlif= *(priv_link_t **)netdev_priv(from_device);
...
EXPORT_SYMBOL(mcast_snooping_notifer);
#endif

#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 0)
#ifdef DSLCPE
EXPORT_SYMBOL(wmf_ipv6_enabled);
EXPORT_SYMBOL(emfc_ipv6_input);
EXPORT_SYMBOL(emfc_remove_sta);
#endif
