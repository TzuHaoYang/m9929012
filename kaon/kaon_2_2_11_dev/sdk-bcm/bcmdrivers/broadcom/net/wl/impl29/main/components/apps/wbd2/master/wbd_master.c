 ...
 *
 * $Id: wbd_master.c 773176 2019-03-14 09:36:06Z $
 */
...
wbd_master_retrieve_blanket_nvram_config(wbd_blanket_master_t *wbd_master)
{
	...
	WBD_DEBUG("%s window[%d] cnt[%d] dwell_time[%d] BH OPT Try[%d] Flags[0x%08x]\n",
		WBD_NVRAM_BOUNCE_DETECT, wbd_master->bounce_cfg.window, wbd_master->bounce_cfg.cnt,
		wbd_master->bounce_cfg.dwell_time, wbd_master->max_bh_opt_try, wbd_master->flags);
...
{
	WBD_INFO("WBD going to shutdown. Signal received : %d\n", sig);
	...
	int if_band)
{
	...
	wbd_master_add_metric_report_policy(g_wbdinfo, al_mac, radio_mac, if_band);
...
main(int argc, char *argv[])
{
	...
	ret = ieee1905_module_init(g_wbdinfo, MAP_MODE_FLAG_CONTROLLER,
		(MAP_IS_CONTROLLER(g_wbdinfo->map_mode) ? 1 : 0));
	WBD_ASSERT();
