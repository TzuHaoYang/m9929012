...
#ifdef DSLCPE
extern int vis_debug_level; /* Indicates the level of debug message to be printed on the console */

extern vis_wl_interface_t *g_wlinterface; /* Head pointer of all the WL interface adapters info */
extern configs_t			g_configs; /* Global config */
extern long				g_timestamp; /* Global timestamp. when the data collection started */

extern char server_ip_address[]; /* Server's IP address */
extern char gateway_ip_address[]; /* Gateway IP address */
#else
