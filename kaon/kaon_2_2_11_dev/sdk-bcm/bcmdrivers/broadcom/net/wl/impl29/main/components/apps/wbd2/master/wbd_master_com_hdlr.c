 ...
 *
 * $Id: wbd_master_com_hdlr.c 773376 2019-03-19 11:34:17Z $
 */
...
static nvram_list_t nvram_params[] = {
	...
	{NVRAM_MAP_MULTIHOP, WBD_NVRAM_FLAG_STR, 0}
};
...
static int wbd_master_send_vndr_nvram_set_cmd(i5_dm_device_type *i5_device);
/* ------------------------------------ Static Declarations ------------------------------------ */
...
{
	char *str;
	int ret = WBDE_OK, num = 0, fbt, fbt_overds, fbt_reassoc_tm;
	...
	master->tbss_info.tbss_algo = blanket_get_config_val_int(NULL, WBD_NVRAM_TBSS_ALGO, 0);
	...
	master->tbss_info.tbss_wght = blanket_get_config_val_int(NULL, WBD_NVRAM_TBSS_WGHT_IDX, 0);
	...
	WBD_DEBUG("BKTID[%d] nws_adv_thld[%d] tbss_stacnt_thld[%d] tbss_algo[%d] tbss_wght[%d] "
		"min_phyrate_boundary[%d] max_phyrate_boundary[%d]\n", master->bkt_id,
		master->tbss_info.nws_adv_thld, master->tbss_info.tbss_stacnt_thld,
		master->tbss_info.tbss_algo, master->tbss_info.tbss_wght,
		master->tbss_info.tbss_min_phyrate, master->tbss_info.tbss_max_phyrate);

	/* read weightages from NVRAM */
	str = blanket_nvram_safe_get(WBD_NVRAM_TBSS_WGHT);
	if (str) {
		num = sscanf(str, "%d %d %d %d %d %d %x",
			...
			&weightage_cfg.flags);
		if (num == 7) {
			memcpy(&master->tbss_info.wght_cfg, &weightage_cfg,
				sizeof(master->tbss_info.wght_cfg));
		} else {
			WBD_WARNING("BKTID[%d] %s : %s = %s\n", master->bkt_id,
				wbderrorstr(WBDE_INV_NVVAL), WBD_NVRAM_TBSS_WGHT, str);
		}
	}

	WBD_DEBUG("BKTID[%d] %s w_rssi[%d] w_hops[%d] w_sta_cnt[%d] w_phyrate[%d] flags[0x%X]\n",
		master->bkt_id, WBD_NVRAM_TBSS_WGHT,
		master->tbss_info.wght_cfg.w_rssi,
		master->tbss_info.wght_cfg.w_hops,
		master->tbss_info.wght_cfg.w_sta_cnt,
		master->tbss_info.wght_cfg.w_phyrate,
		master->tbss_info.wght_cfg.flags);

	/* Read threshold index and prepare threshold config from predefined entries - 2G */
	master->tbss_info.tbss_thld_2g = blanket_get_config_val_int(NULL,
	...
	master->tbss_info.tbss_thld_5g = blanket_get_config_val_int(NULL,
	...
	i5_dm_clients_type *i5_assoc_sta, wbd_cmd_weak_client_resp_t *cmdweakclientresp)
{
	...
	unsigned int agent_count, b_bss_count;
	WBD_ENTER();
	...
	if ((wbd_master->bounce_cfg.cnt == 0) ||
		((bounce_sta_entry != NULL) &&
		(bounce_sta_entry->state == WBD_BOUNCE_DWELL_STATE))) {
		cmdweakclientresp->dwell_time = ((wbd_master->bounce_cfg.cnt == 0) ?
			wbd_master->bounce_cfg.dwell_time :
			(wbd_master->bounce_cfg.dwell_time - bounce_sta_entry->run.dwell_time));
	...
	if (I5_CLIENT_IS_BSTA(i5_assoc_sta)) {
		...
		/* If there is not more than one slave, cant steer the weak client */
		if ((agent_count = i5DmCountNumOfAgents()) <= 1) {
			WBD_WARNING("BKTID[%d] Slave["MACDBG"] For STA["MACDBG"] Count[%d]. %s\n",
				master_info->bkt_id, MAC2STRDBG(i5_bss->BSSID),
				MAC2STRDBG(i5_assoc_sta->mac), agent_count,
				wbderrorstr(WBDE_NO_SLAVE_TO_STEER));
			ret = WBDE_NO_SLAVE_TO_STEER;
			goto end;
		}
	...
	i5_dm_clients_type *i5_assoc_sta)
{
	...
	if (wbd_bcn_rpt && (!eacmp(&(i5_device->DeviceId), &(wbd_bcn_rpt->neighbor_al_mac))) &&
		...
		if (I5_CLIENT_IS_BSTA(i5_assoc_sta)) {
			ret = wbd_master_prepare_backhaul_beacon_request(i5_assoc_sta, &bcn_req);
		} else {
			ret = wbd_master_prepare_beacon_request(&master_info->ap_chan_report,
				i5_assoc_sta, &bcn_req);
		}
		if (ret == WBDE_OK) {
	...
	while (i5_iter_device != NULL) {
		...
		if (I5_IS_MULTIAP_AGENT(i5_iter_device->flags) && i5_iter_device->vndr_data) {
			...
			ieee1905_send_unassoc_sta_link_metric_query(i5_iter_device->DeviceId,
				&uassoc_req);
	...
	foreach_i5glist_item(iter_dev, i5_dm_device_type, ctlr_topology->device_list) {
		...
		foreach_i5glist_item(iter_ifr, i5_dm_interface_type, iter_dev->interface_list) {
			...
			WBD_INFO("WBD_FBT_ENAB Enabled for  : "
				"Device["MACDBG"] IFR["MACDBG"] chanspec [0x%04X] "
				"MDID[%d] FT_CAP[%d] FT_REASSOC[%d] NumBSS[%d]\n",
				MAC2STRDBG(iter_dev->DeviceId),
				MAC2STRDBG(iter_ifr->InterfaceId),
				iter_ifr->chanspec,
			...
			foreach_i5glist_item(iter_bss, i5_dm_bss_type, iter_ifr->bss_list) {
				...
				/* If FBT Data for this BSS, not present, go for next */
				if (!strlen(bss_vndr->r0kh_id) || !strlen(bss_vndr->r0kh_key)) {
					continue;
				}
				...
				memcpy(new_fbt_bss.bssid, iter_bss->BSSID,
					sizeof(new_fbt_bss.bssid));
				memcpy(new_fbt_bss.bss_br_mac, bss_vndr->br_addr,
...
/* Check if FBT is already configured on the Device or not */
static int
wbd_master_is_fbt_configured_on_device(unsigned char *device_id)
{
	int ret = WBDE_OK, ret_val = 0;
	i5_dm_device_type *device;
	i5_dm_interface_type *iter_ifr;
	i5_dm_bss_type *iter_bss;
	wbd_bss_item_t *bss_vndr; /* 1905 BSS item vendor data */
	wbd_master_info_t *master;
	WBD_ENTER();

	if (!device_id) {
		WBD_ERROR("Invalid Device\n");
		goto end;
	}
	...
	foreach_i5glist_item(iter_ifr, i5_dm_interface_type, device->interface_list) {
		...
		/* Find matching Master for this Interface's Band, to get FBT Config */
		WBD_DS_GET_MASTER_INFO(wbd_get_ginfo(), WBD_BKT_ID_BR0, master, (&ret));
		if (ret != WBDE_OK) {
			ret = WBDE_OK;
			continue;
	...
	WBD_DEBUG("FBT is %s on Device["MACDBG"].\n",
		ret_val ? "Configured" : "NOT Configured", MAC2STRDBG(device_id));
	WBD_EXIT();
	return ret_val;
}
...
wbd_master_broadcast_fbt_config_resp_cmd()
{
	...
	/* If FBT_CONFIG_RESP data entries < 0, means FBT is disabled */
	...
	foreach_i5glist_item(iter_dev, i5_dm_device_type, ctlr_topology->device_list) {
		...
		/* Loop for devices, for which FBT is already configured, skip non-FBT devices */
		if (!wbd_master_is_fbt_configured_on_device(iter_dev->DeviceId)) {
			continue;
		}
	...
	unsigned char *tlv_data, unsigned short tlv_data_len)
{
	...
	foreach_glist_item(fbt_item_p, fbt_config_req.entry_list) {
		...
		i5_bss = NULL;
		bss_vndr_data = NULL; /* 1905 BSS item vendor data */

		fbt_data = (wbd_fbt_bss_entry_t *)fbt_item_p;

		/* Find BSS from source device */
		WBD_SAFE_FIND_I5_BSS_IN_DEVICE(neighbor_al_mac, fbt_data->bssid, i5_bss, &ret);
		WBD_ASSERT_ARG(i5_bss->vndr_data, WBDE_INV_ARG);
		WBD_INFO("Master Found Matching BSSID["MACDBG"] SSID[%s]\n",
			MAC2STRDBG(i5_bss->BSSID), i5_bss->ssid.SSID);

		/* Fetch BSS Vendor data */
		bss_vndr_data = (wbd_bss_item_t *)i5_bss->vndr_data;

		/* Copy FBT Data of this BSS, to Respective Agent's BSS Vendor data */
		memcpy(bss_vndr_data->r0kh_id, fbt_data->r0kh_id,
			sizeof(bss_vndr_data->r0kh_id));
		memcpy(bss_vndr_data->r0kh_key, fbt_data->r0kh_key,
			sizeof(bss_vndr_data->r0kh_key));
		memcpy(bss_vndr_data->br_addr, fbt_data->bss_br_mac,
			sizeof(bss_vndr_data->br_addr));
		...
		WBD_INFO("From DEVICE["MACDBG"] BRDG["MACDBG"] BSS["MACDBG"] "
			...
			"MDID[%d] FT_CAP[%d] FT_REASSOC[%d]\n", MAC2STRDBG(neighbor_al_mac),
			MAC2STRDBG(bss_vndr_data->br_addr), MAC2STRDBG(i5_bss->BSSID),
			bss_vndr_data->r0kh_id, strlen(bss_vndr_data->r0kh_id),
			bss_vndr_data->r0kh_key, strlen(bss_vndr_data->r0kh_key),
...
validate:
		...
		for (i = 0; i < REGCLASS_MAX_COUNT; i++) {
			...
			if (!WBD_MCHAN_ENAB(info->flags) && !dedicated_bh) {
				/* Prepare vendor message for agent. As MULTI-AP topology is
				 * operating in single channel operation, BRCM agent use this
				 * information only to set the channel and ignore channel
...
void
wbd_master_add_metric_report_policy(wbd_info_t *info, unsigned char *al_mac,
...
wbd_controller_remove_client_timer_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	foreach_i5glist_item(device, i5_dm_device_type, i5_topology->device_list) {
		...
		sta = i5DmFindClientInDevice(device, (unsigned char *)&remove_client_arg->sta_mac);
		if (sta != NULL && sta->vndr_data != NULL) {
			...
			if (timercmp(&assoc_sta->assoc_time, &assoc_time, <)) {
				...
				/* if remove client message send failed. recreate the timer */
				if (ret != WBDE_OK) {
					recreate = TRUE;
				}
			}
	...
	unsigned char *bssid, unsigned char *sta_mac, ieee1905_sta_link_metric *metric)
{
	...
	i5_dm_interface_type *i5_ifr;
	i5_dm_bss_type *i5_bss;
	i5_dm_clients_type *i5_assoc_sta, *i5_bh_opt_sta;
	wbd_master_info_t *master;
	wbd_assoc_sta_item_t *assoc_sta, *bh_opt_sta;
	...
	/* If the backhaul optimization is running */
	if (I5_CLIENT_IS_BSTA(i5_assoc_sta) && WBD_MINFO_IS_BH_OPT_RUNNING(master->flags)) {
		/* If the STA is weak, just cancel the backhaul optimization of a STA. Also set the
		 * flag to pending so that, it restarts backhaul optimization once it is done.
		 * Else just exit, the TBSS timer which is created as part of backhaul optimization
		 * will take care.
		 */
		if (sta_is_weak) {
			i5_bh_opt_sta = wbd_ds_get_bh_opt_sta(WBD_FLAGS_ASSOC_ITEM_BH_OPT);
			if (i5_bh_opt_sta) {
				bh_opt_sta = (wbd_assoc_sta_item_t*)i5_bh_opt_sta->vndr_data;
				if (bh_opt_sta) {
					bh_opt_sta->flags &= ~WBD_FLAGS_ASSOC_ITEM_BH_OPT;
					master->flags &= ~WBD_FLAGS_MASTER_BH_OPT_RUNNING;
					master->flags |= WBD_FLAGS_MASTER_BH_OPT_PENDING;
				}
			}
		} else {
			/* Sending weak cancel is not required, because if the STA is weak before,
			 * then the backhaul optimization will not be running
			 */
			goto end;
		}
	}
	...
	wbd_monitor_sta_item_t *monitor_sta)
{
	...
	if (WBD_RCPITORSSI_ENAB((wbd_get_ginfo()->flags))) {
		monitor_sta->bcn_rpt_rssi = WBD_RCPI_TO_RSSI(rmrep_bcn->rcpi);
	} else {
		monitor_sta->bcn_rpt_rssi = (int8)rmrep_bcn->rcpi;
	}
			...
			if (i5_assoc_sta && assoc_sta) {
				if (WBD_RCPITORSSI_ENAB((wbd_get_ginfo()->flags))) {
					assoc_sta->stats.bcn_rpt_rssi =
						WBD_RCPI_TO_RSSI(rmrep_bcn->rcpi);
				} else {
					assoc_sta->stats.bcn_rpt_rssi =
						(int8)rmrep_bcn->rcpi;
				}
				...
				WBD_WARNING("STA["MACDBG"] not found in BSS["MACDBG"]'s "
					...
					MAC2STRDBG(i5_bss->BSSID));
				/* If monitor STA item not exists add it */
				monitor_sta = wbd_master_add_to_monitor_list(i5_bss,
					report->sta_mac);
				if (monitor_sta) {
					wbd_master_update_beacon_rssi(wbd_bcn_rpt,
						rmrep_bcn, monitor_sta);
				}
			}
		}
	}
	...
	unsigned short tlv_data_len)
{
	...
	WBD_DEBUG(" ZWDFS msg: cntrl_chan[%d], opclass[%d], reason[%d] \n",
		zwdfs_msg.cntrl_chan, zwdfs_msg.opclass, zwdfs_msg.reason);
	...
	foreach_i5glist_item(i5_iter_device, i5_dm_device_type, ctlr_topology->device_list) {
		...
		foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type,
			...
			if (band !=
				...
				WBD_DEBUG(" band[%d] , if_band[%d] for "MACF"\n", band,
			...
			WBD_INFO("Send ZWDFS MSG from Device["MACDBG"] to Device["MACDBG"]"
				"for interface["MACF"]\n", MAC2STRDBG(ieee1905_get_al_mac()),
				MAC2STRDBG(i5_iter_device->DeviceId),
				ETHERP_TO_MACF(i5_iter_ifr->InterfaceId));
