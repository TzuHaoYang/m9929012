 ...
 *
 * $Id: wbd_master_control.c 772626 2019-02-28 05:48:49Z $
 */
...
#define WBD_IS_MONITOR_STA_RSSI_VALID(sta) (WBD_RSSI_VALID((sta)->bcn_rpt_rssi) && \
					WBD_RSSI_VALID((sta)->rssi))

/* Macros to check whether assoc or unassoc rssi is recent
 * compared to beacon report rssi
*/
#define ASSOC_RSSI_RECENT(sta)	((sta)->stats.sta_tm > (sta)->stats.bcn_tm)
#define MONITOR_RSSI_RECENT(sta, master)	((sta)->monitor_tm > \
					((sta)->bcn_tm + (master)->max.tm_wd_tbss))

#define WBD_MASK_BIT7_TO_BIT5	0xE0
#define WBD_MASK_BIT7_TO_BIT6	0xC0

#define WBD_FSPL_DIFF_2G_5G	7
...
static void wbd_master_start_bh_opt(unsigned char *sta_mac);
	...
	i5_dm_bss_type *parent_bss)
{
	...
	/* Now calculate the RSSI based on the parent devices Tx Power Limit */
	...
	WBD_DEBUG("CUR_BSS["MACDBG"] PARENT_BSS["MACDBG"] RSSI[%d] RSSI_CHANGE[%d] RSSI_BCN[%d] "
		"EST_RSS[%d] CUR_TXPWR[%d] PARENT_TXPWR[%d] BASE_TXPWR[%d]\n",
...
wbd_tbss_get_estimated_sta_rssi(wbd_monitor_sta_item_t *sta, int monitor_rssi, int bcn_rssi,
	time_t monitor_tm, time_t bcn_tm, i5_dm_bss_type *curr_bss, i5_dm_bss_type *monitor_bss,
	i5_dm_bss_type *bcn_bss)
{
	int est_rssi = 0, est_rssi_bcn = 0;
	i5_dm_bss_type *bss = NULL;

	if (WBD_IS_MONITOR_STA_RSSI_VALID(sta) &&
		!MONITOR_RSSI_RECENT(sta, wbd_get_ginfo())) {
		/* stamon and beacon report rssi is present
		 * and beacon report rssi is recent
		 * consider both rssi
		 */
		est_rssi = sta->rssi;
		est_rssi_bcn = sta->bcn_rpt_rssi;
		bss = curr_bss;
	} else if (WBD_RSSI_VALID(sta->rssi)) {
		/* stamon rssi is present consider it */
		est_rssi = sta->rssi;
		bss = curr_bss;
	} else if (monitor_tm <	bcn_tm + (wbd_get_ginfo())->max.tm_wd_tbss) {
		/* Both stamon and beacon rssi are not
		 * available. Beacon rssi is recent
		 * estimate it based on stamon
		 * or beacon rssi available for other
		 * bss on this device
		 */
		...
		/* Both stamon and beacon rssi are not
		 * available for this bss and stamon
		 * rssi on other bss is recent
		 */
		est_rssi = monitor_rssi;
		bss = monitor_bss;
	}
	WBD_DEBUG("STA: RSSI[%d] BCN_RSSI[%d] STAMONITOR_TM[%lu] STA_BCN_TM[%lu]\n",
		sta->rssi, sta->bcn_rpt_rssi, (unsigned long)sta->monitor_tm,
		(unsigned long)sta->bcn_tm);
	WBD_DEBUG("ESTIMATE: MONITOR_RSSI[%d] RSSI_BCN[%d] MONITOR_TM[%lu] BEACON_TM[%lu]\n",
		monitor_rssi, bcn_rssi,	(unsigned long)monitor_tm, (unsigned long)bcn_tm);
	return wbd_tbss_get_estimated_rssi(est_rssi, est_rssi_bcn, curr_bss, bss);
}
	...
	wbd_get_score_param_t *tbss_param)
{
	int rssi = 0, n_rssi = 0;
	int n_hops = 0;
	int total_sta_cnt = 0, assoc_cnt = 0, unassoc_cnt = 0, n_sta_cnt = 0;
	int phyrate = 0, n_phyrate = 0;
	int n_nss = 0, bss_nss = 0;
	int n_tx_rate = 0;
	int node_score = 0;
	int nmin_sta_cnt = master_info->tbss_info.tbss_stacnt_thld;
	i5_dm_interface_type *i5_ifr;
	i5_dm_device_type *i5_device;
	...
	if (master_info->tbss_info.wght_cfg.flags & WBD_WGHT_FLAG_RSSI) {
		/* Check for validity of RSSI bcoz, sta_monitor may not have found the RSSI */
	...
	if (master_info->tbss_info.wght_cfg.flags & WBD_WGHT_FLAG_HOPS) {
		n_hops = device_vndr_data->normalized_hops;
	}

	if (master_info->tbss_info.wght_cfg.flags & WBD_WGHT_FLAG_STACNT) {
		...
		unassoc_cnt = total_sta_cnt - assoc_cnt;
		if (assoc_cnt <= nmin_sta_cnt)
			n_sta_cnt = 100;
		else
			n_sta_cnt = (((float)unassoc_cnt / (float)total_sta_cnt) * 100);
	}

	if (master_info->tbss_info.wght_cfg.flags & WBD_WGHT_FLAG_UPLINKRT) {
		...
		if (check_rule_mask == WBD_WGHT_FLAG_UPLINKRT) {
			...
			n_phyrate = phyrate;
		} else if (phyrate >= master_info->tbss_info.tbss_max_phyrate) {
			n_phyrate = 100;
		} else if (phyrate < master_info->tbss_info.tbss_min_phyrate) {
			n_phyrate = 0;
		} else {
			n_phyrate = (((float)(phyrate - master_info->tbss_info.tbss_min_phyrate)/
				(float)(master_info->tbss_info.tbss_max_phyrate -
				master_info->tbss_info.tbss_min_phyrate))*
				100);
		}
	}

	if ((master_info->tbss_info.wght_cfg.flags & WBD_WGHT_FLAG_NSS) &&
	...
	if ((master_info->tbss_info.wght_cfg.flags & WBD_WGHT_FLAG_TX_RATE) &&
...
scoring:
	...
	node_score = ((master_info->tbss_info.wght_cfg.w_rssi * n_rssi)+
		(master_info->tbss_info.wght_cfg.w_hops * n_hops)+
		(master_info->tbss_info.wght_cfg.w_sta_cnt * n_sta_cnt)+
		(master_info->tbss_info.wght_cfg.w_phyrate * n_phyrate)+
		(master_info->tbss_info.wght_cfg.w_nss * n_nss)+
		(master_info->tbss_info.wght_cfg.w_tx_rate * n_tx_rate));
	WBD_TBSS("For STA["MACF"] on Node["MACDBG"]: weight_flags=0x%x, rssi=%d, "
		...
		MAC2STRDBG(i5_bss->BSSID), master_info->tbss_info.wght_cfg.flags,
...
wbd_tbss_algo_nws(wbd_master_info_t *master_info, i5_dm_clients_type *weak_sta, bool is_dual_band)
{
	...
	check_rule_mask = ((master_info->tbss_info.wght_cfg.flags) & (WBD_WGHT_FLAG_ALL));
	...
	foreach_i5glist_item(i5_device, i5_dm_device_type, i5_topology->device_list) {
		...
		if (i5_device == parent_device) {
			...
			if (!WBD_RSSI_VALID(est_rssi) || !ASSOC_RSSI_RECENT(assoc_sta)) {
			...
			foreach_i5glist_item(i5_bss, i5_dm_bss_type, i5_ifr->bss_list) {
				...
				if (i5_bss == parent_bss) {
					...
					stats.rssi = wbd_tbss_get_estimated_rssi(est_rssi,
						est_rssi_bcn, i5_bss, parent_bss);
					...
					if (sta) {
						...
						stats.rssi = wbd_tbss_get_estimated_sta_rssi(
							sta, est_rssi, est_rssi_bcn, monitor_tm,
	...
	if (target_bss) {
		...
		snprintf(logmsg, sizeof(logmsg), CLI_CMD_LOGS_FOUND_TBSS_ALGO_NWS,
			...
			master_info->tbss_info.wght_cfg.flags, tbss_param.rssi, tbss_param.n_rssi,
...
wbd_master_get_threshold_fail_count(wbd_master_info_t *master_info, i5_dm_bss_type *i5_bss,
	wbd_wl_sta_stats_t *stats, struct ether_addr *mac)
{
...
wbd_tbss_algo_sof(wbd_master_info_t *master_info, i5_dm_clients_type *weak_sta, bool is_dual_band)
{
	...
	foreach_i5glist_item(device, i5_dm_device_type, i5_topology->device_list) {
		...
		if (device == parent_device) {
			est_rssi = WBD_RCPI_TO_RSSI(weak_sta->link_metric.rcpi);
			if (!WBD_RSSI_VALID(est_rssi) ||
				!ASSOC_RSSI_RECENT(assoc_sta)) {
				est_rssi_bcn = assoc_sta->stats.bcn_rpt_rssi;
			}
			parent_monitor_bss = parent_bss;
			bcn_monitor_bss = parent_bss;
			...
			if (monitor_sta || bcn_sta) {
				if (monitor_sta) {
					...
					monitor_tm = monitor_sta->monitor_tm;
				}
				if (bcn_sta) {
					bcn_tm = bcn_sta->bcn_tm;
					est_rssi_bcn = bcn_sta->bcn_rpt_rssi;
				...
				if (bss == parent_bss) {
					...
					if (monitor_sta) {
						stats.rssi = wbd_tbss_get_estimated_sta_rssi(
							...
							monitor_tm, bcn_tm, bss,
							parent_monitor_bss, bcn_monitor_bss);

						fail_cnt = wbd_master_get_threshold_fail_count(
							master_info, bss, &stats,
							(struct ether_addr*)weak_sta->mac);
	...
	bool is_dual_band)
{
	WBD_INFO("BKTNAME[%s] For STA["MACDBG"] DualBand[%d] Finding Target BSS Using Algo[%d]\n",
		master_info->bkt_name, MAC2STRDBG(weak_sta->mac), is_dual_band,
		master_info->tbss_info.tbss_algo);

	/* Update the values required for TBSS calculation */
	wbd_master_update_all_devices_tbss_values();

	return (predefined_tbss_algo[master_info->tbss_info.tbss_algo])(master_info, weak_sta,
		is_dual_band);
}
			...
			foreach_i5glist_item(i5_bss, i5_dm_bss_type, i5_ifr->bss_list) {
				...
				foreach_i5glist_item(i5_sta, i5_dm_clients_type,
					...
					if (assoc_sta &&
						...
						WBD_DEBUG("Device["MACDBG"] IFR["MACDBG"] "
							"BSS["MACDBG"] STA["MACDBG"]. "
							"TBSS Timer created for BSTA\n",
							MAC2STRDBG(i5_device->DeviceId),
...
wbd_master_start_bh_opt(unsigned char *sta_mac)
{
	...
	if ((bsta_count = wbd_ds_count_bstas()) <= 0) {
		WBD_INFO("Number of BSTAs[%d]. No need of backhaul Optimization\n", bsta_count);
		goto end;
	}
	...
	if (sta_mac && (!(ETHER_ISNULLADDR((struct ether_addr*)sta_mac)))) {
		 ...
		 * count by 1
		 */
		if (wbd_ds_is_bsta_device_has_bbss(sta_mac)) {
	...
	if (b_bss_count <= 1) {
		WBD_INFO("Number of Backhaul BSS[%d]. Cannot manage backhaul further\n",
			b_bss_count);
		goto end;
	}
...
wbd_master_start_bh_opt_timeout(bcm_usched_handle *hdl, void *arg)
{
	...
	WBD_DEBUG("Backhaul Optimization Timer Callback\n\n");
...
wbd_master_create_bh_opt_timer(i5_dm_clients_type *i5_sta)
{
	...
	if (WBD_MINFO_IS_BH_OPT_TIMER(master_info->flags) ||
		...
		WBD_INFO("Already Backhaul Optimization %s%s%s\n",
			WBD_MINFO_IS_BH_OPT_TIMER(master_info->flags) ? "Timer Created" : "",
