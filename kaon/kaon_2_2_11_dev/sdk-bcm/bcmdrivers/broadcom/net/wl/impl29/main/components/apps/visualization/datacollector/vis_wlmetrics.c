...
#ifdef DSLCPE_ENDIAN
	iov_pktq_log->version=dtoh32(iov_pktq_log->version);	
#endif
...
#ifdef DSLCPE_ENDIAN
	prec_mask = dtoh32( PREC_DUMPV(logv04->counter_info[index], logv05->counter_info[index]));
#else
...
#ifdef DSLCPE_ENDIAN
			counters.airtime=dtoh32(counters.airtime);
#endif
		} else {
...
#ifdef DSLCPE_ENDIAN
		counters.acked=dtoh32(counters.acked);
		counters.busy=dtoh32(counters.busy);
		counters.dropped=dtoh32(counters.dropped);
		counters.full_dropped=dtoh32(counters.full_dropped);
		counters.max_avail=dtoh32(counters.max_avail);
		counters.max_used=dtoh32(counters.max_used);
		counters.ps_retry=dtoh32(counters.ps_retry);
		counters.queue_capacity=dtoh32(counters.queue_capacity);
		counters.requested=dtoh32(counters.requested);
		counters.retry=dtoh32(counters.retry);
		counters.retry_drop=dtoh32(counters.retry_drop);
		counters.rtsfail=dtoh32(counters.rtsfail);
		counters.sacrificed=dtoh32(counters.sacrificed);
		counters.saved=dtoh32(counters.saved);
		counters.selfsaved=dtoh32(counters.selfsaved);
		counters.stored=dtoh32(counters.stored);
		counters.suppress=dtoh32(counters.suppress);
		counters.throughput=dtoh32(counters.throughput);
		counters.time_delta=dtoh32(counters.time_delta);
		counters.txrate_main=dtoh32(counters.txrate_main);
		counters.txrate_succ=dtoh32(counters.txrate_succ);
...
#ifdef DSLCPE_ENDIAN
	params->num_addrs=htod32(params->num_addrs);
	//WLCSM_TRACE(WLCSM_TRACE_DBG,"%s: num_addr:%d \r\n",(char *)wl,params->num_addrs );	
#endif
...
#ifdef DSLCPE_ENDIAN
		stas_listout->sta[i].rssi = dtoh32(peer_info->rssi);
#else
