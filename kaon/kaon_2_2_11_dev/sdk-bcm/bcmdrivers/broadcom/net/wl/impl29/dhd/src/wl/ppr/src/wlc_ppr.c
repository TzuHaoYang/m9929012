...
{
	memset(pprptr, (uint8)WL_RATE_DISABLED, ppr_size(bw));
	pprptr->ch_bw = bw;
}
...
{
	memset((uchar*)&pprptr->ppr_bw, (uint8)WL_RATE_DISABLED, ppr_pwrs_size(pprptr->ch_bw));
}
...
void ppr_delete(osl_t *osh, ppr_t* pprptr)
{
	ASSERT((pprptr->ch_bw == WL_TX_BW_20) || (pprptr->ch_bw == WL_TX_BW_40) ||
		(pprptr->ch_bw == WL_TX_BW_80) || (pprptr->ch_bw == WL_TX_BW_160) ||
		(pprptr->ch_bw == WL_TX_BW_8080) || (pprptr->ch_bw == PPR_BW_MAX) ||
		IS_ULB_BW(pprptr->ch_bw));
#ifndef BCMDRIVER
	BCM_REFERENCE(osh);
	free(pprptr);
#else
	...
	ppr_dsss_rateset_t* dsss)
{
	...
	if (cnt == 0) {
		...
		memset(dsss->pwr, (uint8)WL_RATE_DISABLED, sizeof(*dsss));
	}
	return cnt;
}
	...
	ppr_ofdm_rateset_t* ofdm)
{
	...
	if (cnt == 0) {
		...
		memset(ofdm->pwr, (uint8)WL_RATE_DISABLED, sizeof(*ofdm));
	}
	return cnt;
}
	...
	wl_tx_chains_t tx_chains, ppr_ht_mcs_rateset_t* mcs)
{
	...
	if (cnt == 0) {
		...
		memset(mcs->pwr, (uint8)WL_RATE_DISABLED, sizeof(*mcs));
	}
...
{
	memset((uchar*)&pprptr->ppr_bw, (uint8)val, ppr_pwrs_size(pprptr->ch_bw));
}
...
/* Support in WL utility to convert old v2 serialized ppr data to v3 */
#ifndef BCMDRIVER

/* This marks the start of a packed structure section. */
#include <packed_section_start.h>

/* V2 Structures and Defines required for conversion of serialized data */
/* V2 did not have support for 4 chains. */
#define PPR_V2_SERIALIZATION_VER	2
#define PPR_V2_MAX_TX_CHAIN_MASK	0x00000003 /* mask of Tx chains */
#define PPR_BEAMFORMING			0x00000004 /* bit indicates BF is on */

/** Per band tx powers */
typedef BWL_PRE_PACKED_STRUCT struct pprpb_v2 {
	/* start of 20MHz tx power limits */
	int8 p_1x1dsss[WL_RATESET_SZ_DSSS];		/* Legacy CCK/DSSS */
	int8 p_1x1ofdm[WL_RATESET_SZ_OFDM];		/* 20 MHz Legacy OFDM transmission */
...
} BWL_POST_PACKED_STRUCT pprpbw_v2_t;

/** Structure to contain ppr values for a 20MHz channel */
typedef BWL_PRE_PACKED_STRUCT struct ppr_v2_bw_20 {
	/* 20MHz tx power limits */
	pprpbw_v2_t b20;
} BWL_POST_PACKED_STRUCT ppr_v2_bw_20_t;

/** Structure to contain ppr values for a 40MHz channel */
typedef BWL_PRE_PACKED_STRUCT struct ppr_v2_bw_40 {
	/* 40MHz tx power limits */
	pprpbw_v2_t b40;
	/* 20in40MHz tx power limits */
	pprpbw_v2_t b20in40;
} BWL_POST_PACKED_STRUCT ppr_v2_bw_40_t;

/** Structure to contain ppr values for an 80MHz channel */
typedef BWL_PRE_PACKED_STRUCT struct ppr_v2_bw_80 {
	/* 80MHz tx power limits */
	pprpbw_v2_t b80;
	/* 20in80MHz tx power limits */
	pprpbw_v2_t b20in80;
	/* 40in80MHz tx power limits */
	pprpbw_v2_t b40in80;
} BWL_POST_PACKED_STRUCT ppr_v2_bw_80_t;

/** Structure to contain ppr values for a 160MHz channel */
typedef BWL_PRE_PACKED_STRUCT struct ppr_v2_bw_160 {
	/* 160MHz tx power limits */
	pprpbw_v2_t b160;
	/* 20in160MHz tx power limits */
	pprpbw_v2_t b20in160;
	/* 40in160MHz tx power limits */
	pprpbw_v2_t b40in160;
	/* 80in160MHz tx power limits */
	pprpbw_v2_t b80in160;
} BWL_POST_PACKED_STRUCT ppr_v2_bw_160_t;

/** Structure to contain ppr values for an 80+80MHz channel */
typedef BWL_PRE_PACKED_STRUCT struct ppr_v2_bw_8080 {
	/* 80+80MHz chan1 tx power limits */
	pprpbw_v2_t b80ch1;
	/* 80+80MHz chan2 tx power limits */
	pprpbw_v2_t b80ch2;
	/* 80in80+80MHz (chan1 subband) tx power limits */
	pprpbw_v2_t b80in8080;
	/* 20in80+80MHz (chan1 subband) tx power limits */
	pprpbw_v2_t b20in8080;
	/* 40in80+80MHz (chan1 subband) tx power limits */
	pprpbw_v2_t b40in8080;
} BWL_POST_PACKED_STRUCT ppr_v2_bw_8080_t;

/**
 * This is the initial implementation of the structure we're hiding. It is sized to contain only
 * the set of powers it requires, so the union is not necessarily the size of the largest member.
 */
typedef BWL_PRE_PACKED_STRUCT struct ppr_v2 {
	wl_tx_bw_t ch_bw;
	...
	BWL_PRE_PACKED_STRUCT union {
		...
		ppr_v2_bw_40_t ch40;
		ppr_v2_bw_80_t ch80;
		ppr_v2_bw_160_t ch160;
		ppr_v2_bw_8080_t ch8080;
	} ppr_bw;
} BWL_POST_PACKED_STRUCT ppr_v2_t;

/* This marks the end of a packed structure section. */
#include <packed_section_end.h>

/* Copy individual fields from V2 to V3 structure. Structure offsets
 * are different. Hence we need to copy each array separately.
 */
static void
ppr_v2_copy_serdata(pprpbw_t *pobuf, const pprpbw_v2_t *pinbuf, uint32 flag)
{
	uint chain = flag & PPR_V2_MAX_TX_CHAIN_MASK;
	bool bf = (flag & PPR_BEAMFORMING) != 0;

	memcpy(pobuf->p_1x1dsss, pinbuf->p_1x1dsss, sizeof(pinbuf->p_1x1dsss));
	memcpy(pobuf->p_1x1ofdm, pinbuf->p_1x1ofdm, sizeof(pinbuf->p_1x1ofdm));
	memcpy(pobuf->p_1x1vhtss1, pinbuf->p_1x1vhtss1, sizeof(pinbuf->p_1x1vhtss1));

	if (chain > 1) {
		memcpy(pobuf->p_1x2dsss, pinbuf->p_1x2dsss,
				sizeof(pinbuf->p_1x2dsss));
		memcpy(pobuf->p_1x2cdd_ofdm, pinbuf->p_1x2cdd_ofdm,
				sizeof(pinbuf->p_1x2cdd_ofdm));
		memcpy(pobuf->p_1x2cdd_vhtss1, pinbuf->p_1x2cdd_vhtss1,
				sizeof(pinbuf->p_1x2cdd_vhtss1));
		memcpy(pobuf->p_2x2stbc_vhtss1, pinbuf->p_2x2stbc_vhtss1,
				sizeof(pinbuf->p_2x2stbc_vhtss1));
		memcpy(pobuf->p_2x2vhtss2, pinbuf->p_2x2vhtss2,
				sizeof(pinbuf->p_2x2vhtss2));
	}

	if (chain > 2) {
		memcpy(pobuf->p_1x3dsss, pinbuf->p_1x3dsss,
				sizeof(pinbuf->p_1x3dsss));
		memcpy(pobuf->p_1x3cdd_ofdm, pinbuf->p_1x3cdd_ofdm,
				sizeof(pinbuf->p_1x3cdd_ofdm));
		memcpy(pobuf->p_1x3cdd_vhtss1, pinbuf->p_1x3cdd_vhtss1,
				sizeof(pinbuf->p_1x3cdd_vhtss1));
		memcpy(pobuf->p_2x3stbc_vhtss1, pinbuf->p_2x3stbc_vhtss1,
				sizeof(pinbuf->p_2x3stbc_vhtss1));
		memcpy(pobuf->p_2x3vhtss2, pinbuf->p_2x3vhtss2,
				sizeof(pinbuf->p_2x3vhtss2));
		memcpy(pobuf->p_3x3vhtss3, pinbuf->p_3x3vhtss3,
				sizeof(pinbuf->p_3x3vhtss3));
	}

	if (bf) {
		memcpy(pobuf->p_1x2txbf_ofdm, pinbuf->p_1x2txbf_ofdm,
				sizeof(pinbuf->p_1x2txbf_ofdm));
		memcpy(pobuf->p_1x2txbf_vhtss1, pinbuf->p_1x2txbf_vhtss1,
				sizeof(pinbuf->p_1x2txbf_vhtss1));
		memcpy(pobuf->p_2x2txbf_vhtss2, pinbuf->p_2x2txbf_vhtss2,
				sizeof(pinbuf->p_2x2txbf_vhtss2));

		if (chain > 2) {
			memcpy(pobuf->p_1x3txbf_ofdm, pinbuf->p_1x3txbf_ofdm,
					sizeof(pinbuf->p_1x3txbf_ofdm));
			memcpy(pobuf->p_1x3txbf_vhtss1, pinbuf->p_1x3txbf_vhtss1,
					sizeof(pinbuf->p_1x3txbf_vhtss1));
			memcpy(pobuf->p_2x3txbf_vhtss2, pinbuf->p_2x3txbf_vhtss2,
					sizeof(pinbuf->p_2x3txbf_vhtss2));
			memcpy(pobuf->p_3x3txbf_vhtss3, pinbuf->p_3x3txbf_vhtss3,
					sizeof(pinbuf->p_3x3txbf_vhtss3));
		}
	}
}

/** Returns the number of bands for a specific bandwidth bw */
static uint ppr_v2_bands_by_bw(wl_tx_bw_t bw)
{
	switch (bw) {
		case WL_TX_BW_2P5:
		case WL_TX_BW_5:
		case WL_TX_BW_10:
		case WL_TX_BW_20:
			return sizeof(ppr_v2_bw_20_t)/sizeof(pprpbw_v2_t);
		case WL_TX_BW_40:
			return sizeof(ppr_v2_bw_40_t)/sizeof(pprpbw_v2_t);
		case WL_TX_BW_80:
			return sizeof(ppr_v2_bw_80_t)/sizeof(pprpbw_v2_t);
		case WL_TX_BW_160:
			return sizeof(ppr_v2_bw_160_t)/sizeof(pprpbw_v2_t);
		case WL_TX_BW_8080:
			return sizeof(ppr_v2_bw_8080_t)/sizeof(pprpbw_v2_t);
		default:
			ASSERT(0);
			return 0;
	}
...
/* Copy */
static void
ppr_deser_v2_cp(uint8 *outbuf, uint8 *inbuf, uint32 flag, wl_tx_bw_t bw, uint16 per_band_size)
{
	uint i;
	uint bands;
	bands = ppr_v2_bands_by_bw(bw);

	/* Set default rate */
	memset(outbuf, (uint8)WL_RATE_DISABLED, ppr_pwrs_size(bw));

	for (i = 0; i < bands; i++) {
		/* For each band, convert V2 to V3 */
		ppr_v2_copy_serdata((pprpbw_t *) outbuf, (pprpbw_v2_t *) inbuf, flag);

		outbuf += sizeof(pprpbw_t);	/* Jump to next band */
		inbuf += per_band_size;		/* Jump to next band */
	}
}

/* Master routine to convert v2 serialized data to v3 */
/* This is required for trunk WL to be compatible with Bison based drivers */
tx_pwr_rpt_t *
convert_v2_ser_ppr_to_v3(uint8 *v2_ser)
{
	int i;
	uint8 *ppr_ser;
	uint8 *ppr_v2_ser;
	tx_pwr_rpt_t *ppr_wl = NULL;
	tx_pwr_rpt_t *ppr_v2_wl = (tx_pwr_rpt_t *) v2_ser;
	...
	if (v2_ser == NULL) {
		fprintf(stderr, "Serialized data empty\n");
		return NULL;
	}

	if (*(uint8 *)ppr_v2_wl->pprdata != PPR_V2_SERIALIZATION_VER) {
		/* This routine only supports V2 to V3 conversion */
		fprintf(stderr, "Unsupported serialize version %d\n", *v2_ser);
		return NULL;
	}
	...
	memset(ppr_wl, (uint8)WL_RATE_DISABLED, ppr_rpt_size);
	...
	ppr_v2_wl = (tx_pwr_rpt_t *) v2_ser;

	/* Copy the tx_pwr_rpt header from v2 to v3 buffer */
	memcpy(ppr_wl, ppr_v2_wl, sizeof(tx_pwr_rpt_t));

	ppr_ser = ppr_wl->pprdata;
	ppr_v2_ser = ppr_v2_wl->pprdata;

	/* Copy each of reg, board and target blocks from v2 to v3 */
	for (i = 0; i < WL_TXPPR_SER_BUF_NUM; i++) {
		ppr_deser_header_t *ser_head = (ppr_deser_header_t *) ppr_v2_ser;
		wl_tx_bw_t ch_bw = ser_head->bw;
		uint32 flags = NTOH32(ser_head->flags);
		uint16 per_band_size = NTOH16(ser_head->per_band_size);

		/* Copy the deser_header */
		memcpy(ppr_ser, ppr_v2_ser, sizeof(*ser_head));
		((ppr_deser_header_t *)ppr_ser)->version = PPR_SERIALIZATION_VER;

		/* Copy over the individual contents */
		ppr_deser_v2_cp((uint8 *) (ppr_ser + sizeof(*ser_head)),
				(uint8 *) (ppr_v2_ser + sizeof(*ser_head)),
				flags, ch_bw, per_band_size);

		ppr_ser += pprsize;
		ppr_v2_ser += ppr_v2_wl->ppr_len;
	}

	/* Also, correct the version and len */
	ppr_wl->ppr_len  = pprsize;
	ppr_wl->version  = TX_POWER_T_VERSION;
