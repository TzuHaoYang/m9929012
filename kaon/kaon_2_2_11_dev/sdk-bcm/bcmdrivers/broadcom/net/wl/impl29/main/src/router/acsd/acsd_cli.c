 ...
 *
 * $Id: acsd_cli.c 770319 2018-12-12 06:25:50Z $
 */
...
do_dump_acs_record(acs_cli_info_t *ctx, int rcount)
{
	...
	printf("Timestamp(ms) ACS-Trigger  ChanSpec  BG-Noise  CCA-Count Idle\n");
	for (i = 0; i < count; i++) {
		time_t ltime = ntohl(result->timestamp);
		printf("%8u \t%s \t%8x \t%d \t%d \t%d\n",
			(uint32)ltime,
