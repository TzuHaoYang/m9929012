 ...
 *
 * $Id: d11.h 740970 2018-01-15 12:32:02Z $
 */
...
#define NFIFO_EXT               6              /* 6 traditional FIFOs */
...
typedef volatile struct _d11regs {
	...
	union {
		...
		struct {
			...
			uint16	PAD[3];				/* b34-b38 */
			uint16	XmtTemplatePtrOffset; /* b3a */
			uint16	PAD[482];		/* 0xb3c - 0xEFE */
		} d11acregs;
	} u;
...
/* This defines the collection of supp reasons (includine none)
...
#define	D11_PRE40_M_TKIP_TSC_TTAK	(0x18c * 2)
...
#define M_SWDIV_SWCTRL_ANT0		(74*2)
#define M_SWDIV_SWCTRL_REG		(75*2)
...
#define M_MFGTEST_UOTARXTEST       ((0x726 * 2) + (0x63 * 2) + (2) * 2)
...
#define D11AC_M_SMPL_COL_BMP	(0x1c7 * 2)          /* Trigger bitmap for sample collect */
#define D11AC_M_SMPL_COL_CTL	(0x1c8 * 2)          /* Sample collect type */
...
#define	MHF1_FORCE_SEND_BCN_AC	0x0800	/* Force send bcn, even if rcvd from peer STA (IBSS) */
#define	MHF1_FORCE_SEND_BCN	0x1000	/* Force send bcn, even if rcvd from peer STA (IBSS) */
...
#define MHF2_RSDB_DFS_SCAN	0x8000		/* RSDB DFS SCAN: blank radar detection   */
...
#define MHF3_ULB_BW		0x2000		/* update bw in cts2self frame for ulb mode */
...
#define MHF5_TXLOFT_WAR         0x1000		/* Enable TX LOFT supression war. Used for ULB */
#define MHF5_ULB_PROTECT_EN     0x1000		/* Enable ULB Protection Mechanism. Used forLOFT */
...
#define PRXS1_ACPHY_HWRSSI_ENABLE	0x0100 /* HW RSSI enabled or disabled */
#define PRXS1_ACPHY_ACI_MITIGATION	0x0200 /* ACI mitigation on or off */
#define PRXS1_ACPHY_TR_SWITCH_STATE0	0x1000 /* tr switch positon on core 0 */
#define PRXS1_ACPHY_TR_SWITCH_STATE1	0x2000 /* tr switch positon on core 1 */

/* ACPHY PhyRxStatus_2: */
#define PRXS2_ACPHY_HWRSSI_qdB_ANT0 0x0003 /* qdB bits for hw rssi on core 0 */
#define PRXS2_ACPHY_HWRSSI_qdB_ANT1 0x000C /* qdB bits for hw rssi on core 1 */
#define PRXS2_ACPHY_HWRSSI_qdB_ANT2 0x0030 /* qdB bits for hw rssi on core 2 */

/* Get qdB bits */
#define ACPHY_HWRSSI_qdB_ANT0(rxs)  (ltoh16((rxs)->PhyRxStatus_2) & \
						PRXS2_ACPHY_HWRSSI_qdB_ANT0)
#define ACPHY_HWRSSI_qdB_ANT1(rxs)  ((ltoh16((rxs)->PhyRxStatus_2) & \
						PRXS2_ACPHY_HWRSSI_qdB_ANT1) >> 2)
#define ACPHY_HWRSSI_qdB_ANT2(rxs)  ((ltoh16((rxs)->PhyRxStatus_2) & \
						PRXS2_ACPHY_HWRSSI_qdB_ANT2) >> 4)
/* Whether HW RSSI is enabled or not */
#define ACPHY_HWRSSI_ENABLE(rxs)    ((ltoh16((rxs)->PhyRxStatus_1) & \
						PRXS1_ACPHY_HWRSSI_ENABLE) >> 8)
#define ACPHY_STATUS_BYTE_3(rxs) ((ltoh16((rxs)->PhyRxStatus_1) & 0xFF00) >> 8)
/* Get TR switch state */
#define ACPHY_TR_SWITCH_STATE0(rxs)  ((ltoh16((rxs)->PhyRxStatus_1) & \
						PRXS1_ACPHY_TR_SWITCH_STATE0) >> 12)
#define ACPHY_TR_SWITCH_STATE1(rxs)  ((ltoh16((rxs)->PhyRxStatus_1) & \
						PRXS1_ACPHY_TR_SWITCH_STATE1) >> 13)
...
#define M_CCASTATS_PTR	(M_PSM_SOFT_REGS + (0x1b * 2))	/* corerev >= 40 */

#ifdef CCATXOP_ROM_COMPAT
#define M_CCA_STATS_BLK (0x3b4 * 2)	/* corerev >= 40 */
#endif /* CCATXOP_ROM_COMPAT */
...
#define M_CCA_SUSP_L        (0x12 * 2)
#define M_CCA_SUSP_H        (0x13 * 2)
#define M_CCA_WIFI_L        (0x14 * 2)
#define M_CCA_WIFI_H        (0x15 * 2)
#define M_CCA_EDCRS_L       (0x16 * 2)
#define M_CCA_EDCRS_H       (0x17 * 2)
/* 0x24 - 0x2a */
#define M_CCA_TXNODE_L	    (0x18)
#define M_CCA_TXNODE_H	    (0x19)
#define M_CCA_RXNODE_L	    (0x1a)
#define M_CCA_RXNODE_H	    (0x1b)
#define M_CCA_XXOBSS_L	    (0x38) /* + 32 */
#define M_CCA_XXOBSS_H	    (0x39)
...
//#define M_HWACI_EN_IND  (M_PSM_SOFT_REGS_EXT + 0x32)    /* Indicate ucode of HW-ACI status */
...
#define	M_UCODE_MACSTAT1_PTR	(M_PSM_SOFT_REGS + (0x1c * 2))
/* BCMC timeout for corerev >= 40 */
#define M_BCMC_TIMEOUT  (0x13 * 2)			/* max time waiting for BCMC in PM */
#define M_BCMC_MOREDATA_TIMEOUT	(0x27 * 2)		/* max time waiting for BCMC in PM  */
							/* if MoreData=1 in the last BCMC frame */

/* BCMC timeout for corerev >= 33 and <= 39 */
#define M_BCMCPS_BLK_PTR	(0x3f * 2)		/* BCMC PS block ptr */

#define M_BCMCPS_TIMEOUT		0		/* max time waiting for BCMC in PM  */
#define M_BCMCPS_MOREDATA_TIMEOUT	1		/* max time waiting for BCMC in PM  */
							/* if MoreData=1 in the last BCMC frame */
...
/* current bt power for each task */
#define	M_BTCX_BT_TXPWR			(118 * 2)

#define C_BTCX_AGGOFF_BLE		(1 << 0)
#define C_BTCX_AGGOFF_A2DP		(1 << 1)

/* Oxygen related SHM locations */
#define M_BTCX_IBSS_TSF_L		(0x1e6 * 2)
#define M_BTCX_IBSS_TSF_ML		(0x1e7 * 2)
#define M_BTCX_IBSS_TSF_ESCO_L		(0x1e8 * 2)
#define PRE40_M_BTCX_IBSS_TSF_L		(0x3ba * 2)
#define PRE40_M_BTCX_IBSS_TSF_ML		(0x3bb * 2)
#define PRE40_M_BTCX_IBSS_TSF_ESCO	(0x3bc * 2)
...
#define ADDR_BMP_IBSS_STA_AC		(1 << 0)	/* IBSS Station (STA) */
#define ADDR_BMP_IBSS_STA		(1 << 3)	/* IBSS Station (STA) */
...
/* BFM coremask for corerev >= 40 only */
#define M_COREMASK_BFM		((M_COREMASK_BLK + 2) * 2) /* byte-1/0: NSS2, Implicit txbf */
#define M_COREMASK_BFM1		((M_COREMASK_BLK + 3) * 2) /* byte-1/0: NSS4, NSS3 */
...
/* WLCX SHM definition for corerev < 40 */
#define PRE40_M_WLCX_BLK		(0x38a)		/* dual radio coex */
/* bitmap for the feature */
#define PRE40_M_WLCX_CONFIG		((PRE40_M_WLCX_BLK + 0xD) * 2)
/* duration(in us) for protection */
#define PRE40_M_WLCX_GUARD_INTV		((PRE40_M_WLCX_BLK + 0xE) * 2)
/* GPIO to be used for 5357/43235 */
#define PRE40_M_WLCX_GPIO_CONFIG	((PRE40_M_WLCX_BLK + 0xF) * 2)
...
#define DBGST_TRANSWAKE		7 	/* transient wake state */
...
*/
#define M_MYMAC_ADDR_L                (0x1c4 *2)
#define M_MYMAC_ADDR_M                (0x1c5 *2)
#define M_MYMAC_ADDR_H                (0x1c6 *2)

/* Shared Memory address that tells if Implicit TXBf cal is to be done */
#define M_TXBFCAL_STATUS		(0xc80 * 2)
...
/* IFS and NAV ihr offsets for ULB */
#define SIFS_RX_TX_TX		0x140
#define IFS_SLOT		0x142
#define NAV_CTL			0x180
...
/* IFS and NAV ihr values for ULB */
#define SIFS_RX_TX_TX_10M	0x5e5e
#define NAV_CTL_10M		0x78
#define IFS_SLOT_10M		0x040e
#define IFS_SLOT_BPHY_10M	0x0424
#define SIFS_RX_TX_TX_5M	0xbfbf
#define NAV_CTL_5M		0xf0
#define IFS_SLOT_5M		0x081c
#define IFS_SLOT_BPHY_5M	0x0848
#define SIFS_RX_TX_TX_20_40_80M	0x3e3e
#define NAV_CTL_20_40_80M	0x3c
#define IFS_SLOT_20_40_80M	0x0207
#define IFS_SLOT_BPHY_20_40_80M	0x0212
...
#define SHM_MBSS_UCODE_AC_BASE	(0x184 * 2)
...
/* # of total AMPDUs tx'd */
#define M_TXAMPDU_CNT	(M_UCODE_MACSTAT + OFFSETOF(macstat_t, txampdu))
/* # of rx'ed block acks */
#define M_RXBA_CNT	(M_UCODE_MACSTAT + OFFSETOF(macstat_t, rxback))
...
#define M_BFCFG_PTR		(0x58 * 2)
#define C_BFI_REFRESH_THR_OFFSET	1
#define C_BFI_NDPA_TXLMT_OFFSET		2
#define C_BFI_BFI_NRXC_OFFSET		3
#define C_BFI_MLBF_LUT_OFFSET		4  // for corerev < 64 only

/* shmem defined with prefix M_ are in shmem */
#define shm_addr(base, offset)  (((base)+(offset))*2)

#define M_BFI_BLK_SIZE			16
/* definition for entries in per-client BFI_BLK */
...
#define C_BFI_BFE_STAINFO_POS           12
 ...
 */
#define C_BFI_BFRIDX_POS_VALID_SHIFT    8
...
#define M_LTECX_ACTUALTX_DURATION       (167*2)
...
#define M_LTECX_PWRCAP_CORE0		(190*2)
#define M_LTECX_PWRCAP_CORE0_FAILSAFE	(191*2)
#define M_LTECX_PWRCAP_CORE1		(201*2)
#define M_LTECX_PWRCAP_CORE1_FAILSAFE	(202*2)
#define M_LTECX_PWRCAP_CORE0_FAILSAFE_ANT	(204*2)
...
/* Phy mode setting for ucode */
#define M_MODE_CORE		(M_PSM_SOFT_REGS_EXT + 0x1c)	/* 0x19c for phymode */
/* CORE0 MODE */
...
#define M_PMU_L					(M_PSM_SOFT_REGS_EXT + 0x22) /* PM Dur timer */
#define M_PMU_H					(M_PSM_SOFT_REGS_EXT + 0x24) /* PM Dur timer */
