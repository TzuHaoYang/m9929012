...
pdftm_dump(const pdftm_t *ftm, struct bcmstrbuf *b)
{
	...
	pdftm_dump_config(ftm_cmn->config, b);
	bcm_bprintf(b, "\tcaps: 0x%04x\n",	ftm_cmn->caps);

	bcm_bprintf(b, "\tenable bss: ");
	for (i = ftm_cmn->enabled_bss_len - 1; i >= 0; i--) {
		bcm_bprintf(b, "%02x", ftm_cmn->enabled_bss[i]);
	}
	bcm_bprintf(b, "\n");
	...
	pdftm_dump_counters(WL_PROXD_SESSION_ID_GLOBAL, ftm_cmn->cnt, b);

	bcm_bprintf(b, "\tmru idx %d\n", ftm_cmn->mru_idx);
	pdftm_dump_sched(ftm_cmn->sched, b);
	bcm_bprintf(b, "\tlast sid %d\n", ftm_cmn->last_sid);
#ifdef WL_FTM_RANGE
	pdftm_dump_ranging_ctx(ftm_cmn->rctx, b);
#endif /* WL_FTM_RANGE */
	bcm_bprintf(b, "\tlast tsf: %u.%u\n", FTM_LOG_TSF_ARG(ftm_cmn->last_tsf));
