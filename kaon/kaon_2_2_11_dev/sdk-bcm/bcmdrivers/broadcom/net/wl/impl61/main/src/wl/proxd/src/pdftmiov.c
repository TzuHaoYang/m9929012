...
#define DIG_NAN_MAP_ID tlv_data.config.nan_map_id
...
#if defined(BCMDBG_DUMP) || defined(FTM_DONGLE_DEBUG)
	...
	const wl_proxd_tlv_t *req_tlvs, int req_len, bool enable)
{
	...
	err = ftm_iov_unpack(ftm, req_tlvs, req_len, ftm->ftm_cmn->dig, &bsscfg);
	if (err != BCME_OK)
		goto done;

	enabled = FTM_BSSCFG_FTM_ENABLED(ftm->ftm_cmn, bsscfg);
	if (enabled && enable) {
	...
	if (enable) {
		...
		if (RSDB_ENAB(wlc->pub) && wlc_bsscfg_primary(wlc)) {
			wlc_bsscfg_t* cfg_inactv = wlc->bsscfg[RSDB_INACTIVE_CFG_IDX];
			if (FTM_BSSCFG_FTM_ENABLED(ftm->ftm_cmn, cfg_inactv)) {
				pdftm_bsscfg_clear_options(ftm, cfg_inactv);
				pdftm_free_sessions(ftm, cfg_inactv, FALSE);
			}
		}
#endif /* WLRSDB */
	...
	const wl_proxd_tlv_t *req_tlvs, int req_len)
{
	...
	if (dig->tlv_data.config.event_mask)
		ftm_cmn->config->event_mask = ltoh32_ua(dig->tlv_data.config.event_mask);

	if (dig->tlv_data.config.rx_max_burst) {
		ftm_cmn->config->rx_max_burst = ftm_iov_adjust_num_burst(ftm,
			ltoh16_ua(dig->tlv_data.config.rx_max_burst));
	}

	sncfg = ftm_cmn->config->session_defaults;
	...
	if (dig->tlv_data.config.debug_mask)
		ftm_cmn->config->debug = ltoh32_ua(dig->tlv_data.config.debug_mask);
#endif // endif

	if (dig->DIG_DEV_ADDR)
		memcpy(&ftm_cmn->config->dev_addr, dig->DIG_DEV_ADDR,
			sizeof(ftm_cmn->config->dev_addr));
	...
	const wl_proxd_tlv_t *req_tlvs, int req_len)
{
	...
	err = ftm_iov_unpack(ftm, req_tlvs, req_len, ftm_cmn->dig, &bsscfg);
	if (err != BCME_OK)
		goto done;
	...
	const wl_proxd_tlv_t *req_tlvs, int req_len)
{
	...
	if (ftm->ftm_cmn->rctx != (wlc_ftm_ranging_ctx_t *) NULL) {
	...
	/* create a ranging context */
	err = ftm_iov_ranging_ctx_from_dig(ftm, bsscfg, ftm->ftm_cmn->dig, &ftm->ftm_cmn->rctx);
	if (err != BCME_OK)
		goto done;

	/* start ranging */
	err = wlc_ftm_ranging_start(ftm->ftm_cmn->rctx);
	if (err != BCME_OK) {
		FTM_LOGIOV(ftm, (("wl%d: %s: failed to start FTM ranging, err=%d\n",
			FTM_UNIT(ftm), __FUNCTION__, err)));

		/* clean up */
		err2 = wlc_ftm_ranging_set_flags(ftm->ftm_cmn->rctx,
		...
		/* destroy the ranging ctx */
		wlc_ftm_ranging_destroy(&ftm->ftm_cmn->rctx);
	}
	...
	const ftm_iov_tlv_dig_info_t *tlv_info)
{
	uint16 len;
	uint16 num_sessions;
	pdftm_cmn_t *ftm_cmn = dig->ftm->ftm_cmn;
	...
	const uint8 *data, int len)
{
	...
	*buf += sizeof(uint16);
	for (i = 0, nsn = 0; i < ftm_cmn->max_sessions; ++i) {
	...
	ASSERT(nsn == ftm_cmn->cnt->sessions);
	...
	const uint8 *data, int len)
{
	...
	if (dig->tlv_data.out.ranging_info) {
		pdftm_ranging_pack_sessions(ftm_cmn->rctx, buf, len);
	}
	else
#endif /* WL_FTM_RANGE */
	{
	...
	int rsp_max, wl_proxd_tlv_t *rsp_tlvs, int *rsp_len)
{
	...
	ftm_info.caps = ftm_cmn->caps;
	ftm_info.max_sessions = ftm_cmn->max_sessions;
	ftm_info.num_sessions = ftm_cmn->num_sessions;
	ftm_info.rx_max_burst = ftm_cmn->config->rx_max_burst;

	dig->DIG_NAN_MAP_ID = &ftm_cmn->config->nan_map_id;
	dig->DIG_DEV_ADDR = (uint8 *) &ftm_cmn->config->dev_addr;
	...
	int rsp_max, wl_proxd_tlv_t *rsp_tlvs, int *rsp_len)
{
	...
	ASSERT(FTM_BSSCFG_FTM_ENABLED(ftm->ftm_cmn, sn->bsscfg));
	...
	int rsp_max, wl_proxd_tlv_t *rsp_tlvs, int *rsp_len)
{
	...
	ftm_cmn->cnt->tsf_lo = (uint32)(cur_tsf & 0xffffffffULL);
	ftm_cmn->cnt->tsf_hi = (uint32)(cur_tsf >> 32);

	err =  ftm_iov_get_cnt(ftm, ftm_cmn->cnt, rsp_max, rsp_tlvs, rsp_len);
	...
	int rsp_max, wl_proxd_tlv_t *rsp_tlvs, int *rsp_len)
{
	...
	/* initialize digest and populate fields */
	dig = pdftm_iov_dig_init(ftm, NULL);
	err = wlc_ftm_ranging_get_info(ftm->ftm_cmn->rctx, &ranging_info);
	if (err != BCME_OK)
		goto done;
...
{
	ASSERT(FTM_VALID(ftm));
	pdftm_iov_dig_reset(ftm, sn, ftm->ftm_cmn->dig);
	return ftm->ftm_cmn->dig;
}
