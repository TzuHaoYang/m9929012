...
#define AVS_THREAD_EVENT_DEPTH		4		/* (FD only) */
...
BCMATTACHFN(wl_avs_attach)(wlc_info_t *wlc)
{
	...
	/*
	 * Temporary
	 * Check if 6710 to hold AVS and run manually
	 */
	if (BCM6710_CHIP(wlc->pub->sih->chip))
		avs->interval = 0;
	...
	if (hnd_thread_create("avs_thread", &avs_thread_stack, sizeof(avs_thread_stack),
	    ...
	    AVS_THREAD_EVENT_DEPTH, &avs->thread) != OSL_EXT_SUCCESS) {
...
#if defined(BCMDBG_DUMP) || defined(AVS_ENABLE_STATUS)
