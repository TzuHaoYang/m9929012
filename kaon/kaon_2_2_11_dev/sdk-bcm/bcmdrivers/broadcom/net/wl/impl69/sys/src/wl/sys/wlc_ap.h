 ...
 *
 * $Id: wlc_ap.h 793323 2020-11-23 11:29:09Z $
 */
#ifndef _WLC_AP_H_
#define _WLC_AP_H_
...
extern ratespec_t wlc_ap_forced_bcn_rspec_get(wlc_info_t *wlc);
extern void wlc_ap_oce_forced_bcn_rspec_set(wlc_info_t *wlc, ratespec_t rspec);
...
void wlc_ap_start_upr_fd_timer(wlc_info_t *wlc);
#endif /* WL_WIFI_6GHZ || WL_OCE && WL_OCE_AP */
#endif /* _WLC_AP_H_ */
#ifdef WL_MBSSID
uint16 wlc_ap_get_maxbss_count(wlc_ap_info_t *ap);
bool wlc_ap_get_block_mbssid(wlc_info_t *wlc);
#endif /* WL_MBSSID */
#if BAND6G || defined(WL_OCE_AP)
void wlc_ap_update_static_rnr_nbr_info(wlc_info_t* wlc, wlc_bsscfg_t *bsscfg);
void wlc_ap_bcast_probe_rsp_update_required(wlc_info_t* wlc, wlc_bsscfg_t *cfg);
#else
#define wlc_ap_update_static_rnr_nbr_info(a, b)		do {} while (0)
#define wlc_ap_bcast_probe_rsp_update_required(a, b)	do {} while (0)
#endif /* BAND6G || WL_OCE_AP */
uint8* wlc_write_transmit_power_envelope_ie_6g(wlc_info_t *wlc,
	chanspec_t chspec, uint8 *cp, int buflen);
#ifdef WL11AC
uint wlc_csa_action_calc_pwr_env_ie_len(wlc_info_t *wlc, chanspec_t chspec);
int wlc_csa_action_write_pwr_env_ie(wlc_info_t *wlc, chanspec_t chspec, uint8 *cp, int buflen);
#endif /* WL11AC */
