 ...
 *
 * $Id: wlc_ampdu_rx.c 791707 2020-10-07 22:52:31Z $
 */
...
#define AMPDU_RESP_TIMEOUT_B		300	/**< # of ms wo resp prog with brcm peer */
...
#define AMPDU_RESP_MAX_RXSTUCK		3
...
#define AMPDU_RESP_RXSTUCK_CNT_MASK		0x0F
#define AMPDU_RESP_PREV_RXSTUCK_CNT_MASK	0xF0
#define AMPDU_RESP_PREV_RXSTUCK_CNT_OFFSET	4

#define AMPDU_RESP_INC_RXSTUCK_CNT(resp) \
	{ \
		ASSERT((resp->rxstuck_cnt & AMPDU_RESP_RXSTUCK_CNT_MASK) != \
			AMPDU_RESP_RXSTUCK_CNT_MASK); \
		resp->rxstuck_cnt += 1; \
	}

#define AMPDU_RESP_RXSTUCK_CNT_MAX_REACHED(resp) \
	((resp->rxstuck_cnt & AMPDU_RESP_RXSTUCK_CNT_MASK) >= AMPDU_RESP_MAX_RXSTUCK)

#define AMPDU_RESP_SET_PREV_RXSTUCK_CNT(resp) \
	{ \
		resp->rxstuck_cnt = ((resp->rxstuck_cnt & AMPDU_RESP_RXSTUCK_CNT_MASK) << \
			AMPDU_RESP_PREV_RXSTUCK_CNT_OFFSET) | \
			(resp->rxstuck_cnt & AMPDU_RESP_RXSTUCK_CNT_MASK); \
	}

#define AMPDU_RESP_PREV_RXSTUCK_CNT_SAME(resp) \
	((resp->rxstuck_cnt & AMPDU_RESP_RXSTUCK_CNT_MASK) == \
	((resp->rxstuck_cnt & AMPDU_RESP_PREV_RXSTUCK_CNT_MASK) >> \
	AMPDU_RESP_PREV_RXSTUCK_CNT_OFFSET))

#define AMPDU_RESP_RESET_RXSTUCK_CNT(resp)	(resp->rxstuck_cnt = 0)
...
typedef struct scb_ampdu_tid_resp {
	...
	uint8 rxstuck_cnt;	/**< number of times rx stuck limit was reached */
	...
	scb_ampdu_rx_t *scb_ampdu, uint8 tid)
{
	...
	if (AMPDU_HOST_REORDER_ENAB(wlc->pub) && (resp != NULL)) {
		AMPDU_RESP_INC_RXSTUCK_CNT(resp);
		if (AMPDU_RESP_RXSTUCK_CNT_MAX_REACHED(resp)) {
			AMPDU_RESP_RESET_RXSTUCK_CNT(resp);
			wlc_ampdu_rx_send_delba(wlc->ampdu_rx, scb, tid, FALSE,
				DOT11_RC_UNSPECIFIED);
		}
	}
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
			if ((resp = scb_ampdu->resp[tid])) {
				/* reset rxstuck_cnt if it did not increase in this last second */
				if (AMPDU_HOST_REORDER_ENAB(wlc->pub) && (resp->rxstuck_cnt != 0)) {
					if (AMPDU_RESP_PREV_RXSTUCK_CNT_SAME(resp)) {
						AMPDU_RESP_RESET_RXSTUCK_CNT(resp);
					} else {
						AMPDU_RESP_SET_PREV_RXSTUCK_CNT(resp);
					}
				}
				continue;
			}
	...
	bool is_first_mpdu)
{
	...
	scb_ampdu->aggn++;
	scb_ampdu->agglen = pkttotlen(wlc->osh, p);
	...
	cur_aggn = scb_ampdu->aggn;
	cur_aggn = (cur_aggn << ULMU_NF_AGGN);
	if (scb_ampdu->ema_aggn != 0) {
		ULMU_MOVING_AVG(&scb_ampdu->ema_aggn, cur_aggn,
			ULMU_EMA_ALPHA);
	} else {
		scb_ampdu->ema_aggn = cur_aggn;
	}
	scb_ampdu->aggn = 0;

	cur_agglen_nf = (scb_ampdu->agglen << ULMU_NF_AGGLEN);
	if (scb_ampdu->ema_agglen != 0) {
		ULMU_MOVING_AVG(&scb_ampdu->ema_agglen, cur_agglen_nf, ULMU_EMA_ALPHA);
	} else {
...
wlc_ampdu_rx_dump(ampdu_rx_info_t *ampdu_rx, struct bcmstrbuf *b)
{
	...
	if (cnt->rxampdu) {
		...
		bcm_bprintf(b, "su: %-12d ", (cnt->rxampdu - amdbg->rxampdu_hetb));
		bcm_bprintf(b, "hetb: %-12d ", amdbg->rxampdu_hetb);
		bcm_bprintf(b, "\n");
	}
	...
	bcm_bprintf(b, "avg rxmpduperampdu %d rxampdu_in_bytes %d\n",
		...
		scb_ampdu->ema_agglen >> ULMU_NF_AGGLEN);
		}
	}
		...
		FOREACH_BSS_SCB(wlc->scbstate, &scbiter, bsscfg, scb) {
			...
			for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
				...
				if (resp && resp->queued) {
					...
					WL_AMPDU_RX(("%s tid %d has reordering packets <%u>.\n",
						bcm_ether_ntoa(&scb->ea, eabuf),
						tid, resp->queued));
				}
			}
...
wlc_ampdu_ulmu_reqbytes_get(ampdu_rx_info_t *ampdu_rx, scb_t *scb)
{
	...
	recv_bytes = scb_ampdu->recv_bytes;
	scb_ampdu->recv_bytes = 0;
