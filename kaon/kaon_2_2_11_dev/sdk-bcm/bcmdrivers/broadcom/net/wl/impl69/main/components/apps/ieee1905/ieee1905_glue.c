...
static void i5GlueDeleteVLANInterface(char *ifname)
{
  ...
  snprintf(vprim, sizeof(vprim), "%s.%d", ifname, i5_config.policyConfig.prim_vlan_id);
  ...
  for (item_p = dll_head_p(&i5_config.policyConfig.ts_policy_list.head);
    ...
    snprintf(vsec, sizeof(vsec), "%s.%d", ifname, ts_policy->vlan_id);
...
static i5_vlan_ifr_node *i5GlueAddVlanIfr(char *ifname)
{
  i5_vlan_ifr_node *ifr_node;
...
void i5GlueCreateTSBSSRules(char *ifr)
{
  ...
  snprintf(cmd, sizeof(cmd), "ebtables -t broute -A BROUTING -i %s -p 0x8100 -j DROP;", ifr);
  system(cmd);
  i5Trace("%s\n", cmd);
}
...
void i5GlueRemoveTSBSSRules(char *ifr)
{
  ...
  snprintf(cmd, sizeof(cmd), "ebtables -t broute -D BROUTING -i %s -p 0x8100 -j DROP;", ifr);
  system(cmd);
  i5Trace("%s\n", cmd);
}
...
void i5GlueCreateVLAN(char *ifname, int isWireless)
{
  ...
  char prim_bridge[I5_MAX_IFNAME], sec_bridge[I5_MAX_IFNAME], nvname[125];
  char sec_bridge_nvname[I5_MAX_IFNAME];
  char *hwstp = NULL;
  dll_t *item_p;
  ieee1905_ts_policy_t *ts_policy;
  int idx = 0;
  ...
  for (item_p = dll_head_p(&i5_config.policyConfig.ts_policy_list.head);
    ...
    I5STRNCPY(sec_bridge, nvram_safe_get(nvname), sizeof(sec_bridge));
    if (strlen(sec_bridge) <= 0) {
      i5TraceDirPrint("Secondary Bridge Name Not Known\n");
      /* Need to create secondary bridge */
      snprintf(sec_bridge, sizeof(sec_bridge), "%s%d", I5_WLCFG_BRIDGE_PREFIX, idx);
      snprintf(sec_bridge_nvname, sizeof(sec_bridge_nvname), "%s%d", I5_WLCFG_BRIDGE_PREFIX, idx);
      /* Set NVRAM ex: lan1_ifname=br1 */
      nvram_set(nvname, sec_bridge);
      /* Set NVRAM ex: br1_ifname=br1 */
      nvram_set(sec_bridge_nvname, sec_bridge);
      eval("brctl", "addbr", sec_bridge);
    }

    /* Create secondary vlan interface for guest network */
    i5CreateSecondaryVLAN(ifname, ts_policy->vlan_id, sec_bridge, prim_bridge);
  }
