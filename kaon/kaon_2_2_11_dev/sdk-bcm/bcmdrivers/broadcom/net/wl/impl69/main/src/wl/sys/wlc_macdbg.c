 ...
 *
 * $Id: wlc_macdbg.c 793901 2020-12-14 16:52:46Z $
 */
...
wlc_write_d11reg(wlc_info_t *wlc, int idx, int type, uint16 addr, uint32 w_val)
{
	...
	case D11REG_TYPE_IHR16:
		if (addr < PIHR2K_BASE) {
			...
			wlc_bmac_write_ihr(wlc->hw, (addr - PIHR_BASE) >> 1, w_val16);
		}
		break;
	case D11REG_TYPE_SCR:
...
wlc_dump_d11cnts(wlc_info_t *wlc, struct bcmstrbuf * b)
{
	...
	char percent[7];
	const char airtime_type_names[AIRTIME_ENUM_MAX][15] =
	...
	sprintf(percent, "(%d%%)", tot_upd ? tot_tx_upd * 100 / tot_upd : 0);
	...
	sprintf(percent, "(%d%%)", delta_time ? tot_tx_airtime * 100 / delta_time : 0);
...
wlc_dump_grphist(wlc_info_t *wlc, struct bcmstrbuf * b)
{
	...
	bcm_bprintf(b, "(sndupd_seq sgi grp_idx_bmp)| (txvidx0 ... txvidxN : txvbmp_cand)|"
		" u0-3: (nssmcs fifo_grp_idx txvidx) (...)... |(txvbmp_grp)\n");
	for (i = 0; i < GRP_HIST_NUM; i++) {
		uint16 curidx = ((grp_hist_idx + i + 1) & (GRP_HIST_NUM - 1));
		uint16 txvbmp_cand = 0, txvbmp_grp = 0;
		uint16 grp_idx_bmp = (grp_hist_blk[curidx].info.grp_ext ? 3 : 1) <<
					grp_hist_blk[curidx].info.grp_idx;
		bcm_bprintf(b, "(0x%03x %d 0x%02x)| (",
			grp_hist_blk[curidx].info.m2vsnd_cnt,
			grp_hist_blk[curidx].info.sgi, grp_idx_bmp);
		txvidx_mand_hist[grp_hist_blk[curidx].txvidx_cand & 0xf]++;
		for (j = 0; j < 4; j++) {
			if (j > grp_hist_blk[curidx].info.nusr_cand) {
				bcm_bprintf(b, "-- ");
			} else {
		...
		for (j = 0; j < 4; j++) {
			if (grp_hist_blk[curidx].usr[j].u16 == 0xffff) {
				bcm_bprintf(b, "(N/A)\t\t");
			} else {
				bcm_bprintf(b, "(0x%02x %02d %02d)\t",
					grp_hist_blk[curidx].usr[j].nssmcs,
					grp_hist_blk[curidx].usr[j].fifo_grp_idx,
					grp_hist_blk[curidx].usr[j].txvidx);
					txvidx_grp_hist[grp_hist_blk[curidx].usr[j].txvidx]++;
					txvbmp_grp |= (1 << (grp_hist_blk[curidx].usr[j].txvidx));
			}
		}
		bcm_bprintf(b, "|(0x%x)\n", txvbmp_grp);
	}
...
wlc_dump_ucode_fatal(wlc_info_t *wlc, uint reason)
{
	...
	psmdebug = (D11REV_GE(pub->corerev, 128)) ?
		wlc_read_shm(wlc, M_PSMWDS_PC(wlc)) : R_REG(osh, D11_PSM_DEBUG(wlc));
	pc = (D11REV_GE(pub->corerev, 128)) ?
		R_REG(osh, D11_PSM_DEBUG(wlc)) : psmdebug;
	...
	WL_PRINT(("PC :\n"));
	if (D11REV_GE(pub->corerev, 128)) {
		val32[0] = wlc_read_shm(wlc, M_PSMWDS_PC(wlc));
...
wlc_dump_psmx_fatal(wlc_info_t *wlc, uint reason)
{
	...
	psmdebug_x = (D11REV_GE(pub->corerev, 128)) ?
		wlc_read_shmx(wlc, MX_PSMWDS_PC(wlc)) : R_REG(osh, D11_PSM_DEBUG_psmx(wlc));
	pc_x = (D11REV_GE(pub->corerev, 128)) ?
		R_REG(osh, D11_PSM_DEBUG_psmx(wlc)) : psmdebug_x;
	...
	if (D11REV_GE(pub->corerev, 128)) {
		val32[0] = wlc_read_shmx(wlc, MX_PSMWDS_PC(wlc));
		WL_PRINT(("0x%-8x 0x%-8x 0x%-8x 0x%-8x\n",
			val32[0], val32[0], val32[0], val32[0]));
	} else {
		for (k = 0; k < 64; k += 4) {
...
wlc_macdbg_txs_ppdu_info(wlc_macdbg_info_t *macdbg, tx_status_t* txs)
{
	...
	ismu = txs->status.s5 & TX_STATUS64_MUTX;
	epoch = TX_STATUS128_EPOCH(txs->status.s5);
	if (!ismu || p_ppdu->epoch == -1 || epoch != p_ppdu->epoch) {
		// this is a new PPDU
		p_ppdu->epoch = epoch;
		p_ppdu->ismu = ismu;
		p_ppdu->pktBw = TX_STATUS64_MU_BW(txs->status.s3);
		p_ppdu->mutype = TX_STATUS128_MUTYP(txs->status.s5);
		p_ppdu->txtime = TX_STATUS128_TXDUR(txs->status.s2);
		p_ppdu->ntxs_recv = 0;
		if (ismu) {
			if (p_ppdu->mutype == TX_STATUS_MUTP_HEOM) {
				p_ppdu->nusrs = TX_STATUS128_HEOM_NUSRS(txs->status.s3);
			} else {
				gbmp = TX_STATUS64_MU_GBMP(txs->status.s4);
	...
	if (!ismu) {
		...
		if (p_ppdu->mutype == TX_STATUS_MUTP_HEOM ||
			...
			p_ppdu->usr_info[usr_idx].ruidx = TX_STATUS128_HEOM_RUIDX(txs->status.s4);
		} else {
			p_ppdu->usr_info[usr_idx].ruidx = -1;
		}
		p_ppdu->usr_info[usr_idx].mcs = TX_STATUS64_MU_MCS(txs->status.s4);
		p_ppdu->usr_info[usr_idx].nss =	TX_STATUS64_MU_NSS(txs->status.s4) + 1;
...
wlc_macdbg_txs_ulmu_info(wlc_macdbg_info_t *macdbg, tx_status_t* txs)
{
	...
	airtime = (TGTXS_LSIG(txs->status.s2) * 4 / 3) + 20; /* lsig_len * 8 / 6 + lsig_preamble */
