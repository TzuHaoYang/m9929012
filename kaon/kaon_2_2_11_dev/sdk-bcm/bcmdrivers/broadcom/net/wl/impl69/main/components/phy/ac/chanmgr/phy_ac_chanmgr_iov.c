 ...
 *
 * $Id: phy_ac_chanmgr_iov.c 791920 2020-10-13 16:39:06Z $
 */
...
static const bcm_iovar_t phy_ac_chanmgr_iovars[] = {
	...
	{"phy_loftcompshift_ovrd", IOV_PHY_LOFTCOMPSHIFT_OVRD, IOVF_SET_DOWN, 0, IOVT_INT8, 0},
#endif /* BCMDBG  || WLTEST */
	...
	void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	switch (aid) {
		...
		case IOV_SVAL(IOV_PHY_LOWRATETSSI_OVRD):
			...
			break;
		case IOV_SVAL(IOV_PHY_VCO_PLL_ADJ_MODE): {
			*ret_int_ptr = phy_ac_chanmgr_enable_vco_pll_adj_mode(pi, TRUE, int_val);
			break;
		}
		...
		case IOV_GVAL(IOV_PHY_LOFTCOMPSHIFT_OVRD): {
			err = phy_ac_chanmgr_iovar_get_loftcompshift_ovrd(chanmgri, ret_int_ptr);
			break;
		}
		case IOV_SVAL(IOV_PHY_LOFTCOMPSHIFT_OVRD): {
			err = phy_ac_chanmgr_iovar_set_loftcompshift_ovrd(chanmgri, int_val);
			break;
		}
