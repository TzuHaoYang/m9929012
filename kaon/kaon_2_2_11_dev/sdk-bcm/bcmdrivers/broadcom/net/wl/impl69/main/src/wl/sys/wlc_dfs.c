 ...
 *
 * $Id: wlc_dfs.c 793445 2020-11-27 01:45:01Z $
 */
...
enum wlc_dfs_iov {
	...
	IOV_DFS_NOP = 14,		/* Duration of non-occupency period */
	IOV_DFS_HANDLE_RADAR_ONSTA = 15,	/* Disabled by default. Radar detection logic for
						 * customers with custom STA + AP without
						 * WET/DWDS/MAP.
						 */
	IOV_LAST
};
...
static const bcm_iovar_t wlc_dfs_iovars[] = {
	...
	{"dfs_handle_radar_onsta", IOV_DFS_HANDLE_RADAR_ONSTA, 0, 0, IOVT_BOOL, 0},
	{NULL, 0, 0, 0, 0, 0}
};
 ...
 * 43684 (A0/A1 corerev=128, B0/B1 corerev=129), 6750/6755 (corerev=130), 6710 (corerev=131),
 * and 6715 (corerev=132)
 */
#define DFS_HAS_SUBBAND_INFO(wlc) (\
		D11REV_IS(wlc->pub->corerev, 65) || D11REV_IS(wlc->pub->corerev, 129) || \
		D11REV_IS(wlc->pub->corerev, 130) || D11REV_IS(wlc->pub->corerev, 131) || \
		D11REV_IS(wlc->pub->corerev, 132))

/* real BW160 is available only for 43684 d11rev 128 (A0/A1), 129 (B0/B1), and 6715 d11rev 132 */
#define DFS_REAL_BW160(wlc)		(D11REV_IS(wlc->pub->corerev, 129) || \
		D11REV_IS(wlc->pub->corerev, 132))
...
/* no. sub bands in 160MHz channel */
#define SUBBANDS_IN_160MHZ	8
...
struct wlc_dfs_info {
	...
	dfs_scan_cmn_t *dfs_scan;	/* dfs_scan structure. */
	...
	uint32 hndl_radarsta;		/* Handle radar detection logic for customers with
					 *  custom STA + AP without WET/DWDS/MAP.
					 */
#endif /* SLAVE_RADAR */
	uint8 radarch_len;			/* No. of 20MHz chan on which radar is detected */
	uint8 radarch[SUBBANDS_IN_160MHZ];	/* channels on which radar is detected */
	chanspec_t old_chanspec;	/* useful if radio wants to switch back to
					 * old channels after performing full time
					 * cac
					 */
};
...
#if defined(BGDFS)
static int wlc_dfs_scan(wlc_dfs_info_t *dfs, wl_chan_switch_t *csa, bool preclear_only);
...
wlc_dfs_updown_cb(void *ctx, bsscfg_up_down_event_data_t *updown_data)
{
	...
	if (!ap_bsscfg_up_exist) {
		...
		if (WL11H_ENAB(wlc) && dfs->radar &&
		    wlc_radar_chanspec(wlc->cmi, radio_chanspec) && !wlc->is_edcrs_eu) {
			wlc_set_quiet_chanspec(wlc->cmi, radio_chanspec);
		}
	}
...
#ifdef SLAVE_RADAR
	/* Handle to enable radar detection for customers
	 * with custom STA + AP without WET/DWDS/MAP. Disabled by default.
	 */
	dfs->hndl_radarsta = 0;
#endif /* SLAVE_RADAR */
...
wlc_dfs_csa_each_up_ap(wlc_info_t *wlc, wl_chan_switch_t *csa, bool docs)
{
	...
	if (WLC_IS_CSA_TIMER_IN_PROGRESS(wlc) ||
		(APSTA_ENAB(wlc->pub) && BSSCFG_STA(wlc->primary_bsscfg) &&
			BSSCFG_IS_CSA_IN_PROGRESS(wlc->primary_bsscfg))) {
		WL_DFS(("wl%d: already CSA timer running, not required to send CSA per ap cfg \n",
			wlc->pub->unit));
		return;
	}
	...
	/* Transmit CSA from each cfg, update beacon and probe response */
	FOREACH_UP_AP(wlc, idx, apcfg) {
		wlc_csa_do_csa(wlc->csa, apcfg, csa, docs);
	}

	if (csa->mode != DOT11_CSA_MODE_ADVISORY) {
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	{
		bool preclear_only = FALSE;
		wl_chan_switch_t csa;
		chanspec_t chspec = phy_utils_get_chanspec(WLC_PI(wlc));
		bool bw_160_80p80 = CHSPEC_IS160(chspec) || CHSPEC_IS8080(chspec);

		if (p_len >= (2 * sizeof(int))) {
			WL_DFS(("wl%d: Requested dfs_ap_move with %d/0x%04x %d/0x%04x\n",
				wlc->pub->unit, int_val, int_val, int_val2, int_val2));
			if (int_val2 == WL_DFS_AP_MOVE_STUNT) {
				preclear_only = TRUE;
			} else {
				WL_DFS(("wl%d: dfs_ap_move unknown second param %d ignored",
				wlc->pub->unit, int_val2));
			}
		}
		 ...
		 */
		if (APSTA_ENAB(wlc->pub) && BSSCFG_STA(wlc->primary_bsscfg) &&
		...
		if ((int16)chspec == -2) {
			...
			wlc_channel_send_chan_event(wlc, wlc->primary_bsscfg,
				WL_CHAN_REASON_DFS_AP_MOVE_STUNT,
		...
		WL_DFS(("wl%d: %s scan channel: %04x, current channel %04x\n", wlc->pub->unit,
				__FUNCTION__, chspec, phy_utils_get_chanspec(WLC_PI(wlc))));

		/* reject if the chanspec is already current radio chanspec */
		if (chspec == phy_utils_get_chanspec(WLC_PI(wlc))) {
			err = BCME_BADCHAN;
			break;
		}
		...
		if ((CHSPEC_BW(chspec) != CHSPEC_BW(phy_utils_get_chanspec(WLC_PI(wlc)))) &&
			!DFS_REAL_BW160(wlc)) {
			WL_DFS(("wl%d: Scan channel bandwidth %d must match current bandwidth %d\n",
					wlc->pub->unit, CHSPEC_BW(chspec),
					CHSPEC_BW(phy_utils_get_chanspec(WLC_PI(wlc)))));
			err = BCME_BADCHAN;
			break;
		}
		...
		if (wlc->is_edcrs_eu && !wlc_quiet_chanspec(wlc->cmi, chspec) &&
			...
			err = wlc_dfs_scan(dfs, &csa, preclear_only);
			if (err != BCME_OK) {
...
#ifdef WL_AP_CHAN_CHANGE_EVENT
		wlc_channel_send_chan_event(wlc, wlc->primary_bsscfg,
			WL_CHAN_REASON_DFS_AP_MOVE_START, chspec);
#endif /* WL_AP_CHAN_CHANGE_EVENT */
		break;
	}
...
#ifdef SLAVE_RADAR
	case IOV_GVAL(IOV_DFS_HANDLE_RADAR_ONSTA):
		*ret_int_ptr = dfs->hndl_radarsta;
		break;

	case IOV_SVAL(IOV_DFS_HANDLE_RADAR_ONSTA):
		dfs->hndl_radarsta = int_val;
		break;
#endif /* SLAVE_RADAR */
...
#ifdef WL11AX
	/* If HE enabled, also send OMI indication for HE clients */
	if (HE_ENAB(wlc->pub) && mode_switch_sched) {
		wlc_he_upd_omi_rxnss(wlc->hei, (max_nss - 1));
	}
#endif /* WL11AX */
...
wlc_dfs_bg_downgrade_wlc(wlc_info_t *wlc)
{
	...
	/* If HE enabled, also send OMI indication for HE clients */
	if (HE_ENAB(wlc->pub) && mode_switch_sched) {
		wlc_he_upd_omi_rxnss(wlc->hei, (max_nss - 2));
	}
#endif /* WL11AX */
...
wlc_dfs_bg_scan_prep(wlc_info_t *wlc)
{
	...
	dfs->scan_both = wlc_radar_chanspec(wlc->cmi, phy_utils_get_chanspec(WLC_PI(wlc)));
...
wlc_dfs_bg_upgrade_phy(wlc_dfs_info_t *dfs)
{
	...
	phy_wd_override(WLC_PI(wlc), TRUE);
...
static int
wlc_dfs_scan(wlc_dfs_info_t *dfs, wl_chan_switch_t *csa, bool preclear_only)
{
	wlc_info_t *wlc = dfs->wlc;
	int ret_val = 0;
	...
	if (preclear_only) {
		if (wlc->is_edcrs_eu) {
			dfs->move_stunted = TRUE;
			WL_DFS(("wl%d %s preclear_only\n", wlc->pub->unit, __FUNCTION__));
		} else {
			WL_DFS(("wl%d: ignoring preclear_only request on non-EDCRS-EU",
					wlc->pub->unit));
		}
	}
	...
	if (ret_val == BCME_OK) {
		/* Start dfs scan */
		wlc_dfs_scan_start(dfs, csa->chspec);
	...
	switch (dfs->dfs_scan->modeswitch_state)
	{
		...
		case DFS_MODESW_DOWNGRADE_IN_PROGRESS:
			if (new_state == MODESW_DN_AP_COMPLETE) {
				...
				/* Start dfs scan */
				wlc_dfs_scan_start(dfs, dfs->dfs_scan->csa.chspec);
			}
			break;
...
wlc_dfs_discard_due_to_txblanking(wlc_dfs_info_t *dfs)
{
	...
	int separation;
	bool discard = FALSE;
	uint16 bw_flag = CHSPEC_BW(phy_utils_get_chanspec(WLC_PI(wlc))), bw_mhz = CHAN_BW_20MHZ;
	 ...
	 */
	if (CHSPEC_IS2G(phy_utils_get_chanspec(WLC_PI(wlc)))) {
		return FALSE;
	}
#endif /* BGDFS_2G */
	...
	separation = wlc_dfs_get_chan_separation(wlc,
			phy_utils_get_chanspec(wlc->pi), dfs->sc_chspec);
...
wlc_dfs_scan_complete_sc(wlc_dfs_info_t *dfs, int reason, int return_home_chan)
{
	...
	chanspec_t target_chspec = phy_utils_get_chanspec(WLC_PI(wlc));
	...
	} else if (reason == DFS_SCAN_S_RADAR_FREE) {
		if (!(dfs->move_stunted)) {
			rc = WL_CHAN_REASON_DFS_AP_MOVE_SUCCESS;
		} else {
			/* Pass event for stunt success to userspace,
			 * wbd use this information to maintain CAC
			 * state machine for radio.
			 */
			rc = WL_CHAN_REASON_DFS_AP_MOVE_STUNT_SUCCESS;
		}
	} else {
		send_event = FALSE;
	}
	...
	if (send_event) {
		wlc_channel_send_chan_event(wlc, wlc->primary_bsscfg, rc, target_chspec);
	}
#endif /* WL_AP_CHAN_CHANGE_EVENT */
...
wlc_dfs_scan_cleanup(wlc_dfs_info_t *dfs, bool return_home_chan, bool upgrade)
{
	...
	if (return_home_chan && BGDFS_ENAB(wlc->pub) && !PHYMODE_BGDFS_ACTIVE(dfs->phymode)) {
...
wlc_dfs_watchdog(void *ctx)
{
	...
	if (WLC_MODESW_ENAB(wlc->pub) && BGDFS_ENAB(dfs->wlc->pub) &&
			PHYMODE_BGDFS_ACTIVE(dfs->phymode) && dfs->dfs_scan->inprogress) {
		int ret_val;
		if ((ret_val = wlc_modesw_omn_master_set(wlc->modesw, WL_OMNM_MOD_ZDFS, 0))
				!= BCME_OK) {
			WL_DFS(("wl%d: %s: zdfs/bgdfs could not get lock as OMN master (%d)\n",
	...
	if (dfs->oos_ch_switch_at == wlc->pub->now) {
		wlc_dfs_to_backup_channel(dfs, TRUE, wlc->primary_bsscfg);
	}
}
...
wlc_dfs_get_dfs_status_all(wlc_dfs_info_t *dfs, uint8 *arg, int len)
{
	...
	sub0->chanspec = phy_utils_get_chanspec(WLC_PI(wlc));
...
wlc_radar_report_handler(void *arg)
{
	...
	wlc_disassociate_client(wlc->primary_bsscfg, FALSE);
...
wlc_dfs_chanspec_oos(wlc_dfs_info_t *dfs, chanspec_t chanspec)
{
	...
	FOREACH_20_SB_EFF(chanspec, channel, last_channel) {
		...
		if (!has_subband_info || (has_subband_info && (radar_sb & pos_20))) {
			dfs->radarch[dfs->radarch_len] = channel;
			dfs->radarch_len++;
...
wlc_dfs_valid_forced_chanspec(wlc_dfs_info_t *dfs)
{
	...
	uint i;
	chanspec_t cur_ch = phy_utils_get_chanspec(WLC_PI(wlc));
...
{
	wlc_info_t *wlc = dfs->wlc;
	chanspec_t chspec, cur_ch = phy_utils_get_chanspec(WLC_PI(wlc));
	...
	while (bw_idx < bw_list_len) {
		bw = bw_list[bw_idx++];
		if (!wlc_is_valid_bw(wlc, wlc->primary_bsscfg, BAND_5G_INDEX, bw)) {
		...
		if ((bw == WL_CHANSPEC_BW_8080) || (bw == WL_CHANSPEC_BW_160)) {
			...
			chspec = wf_ctlchspec20_from_chspec(cur_ch);
		} else
			break;
...
wlc_radar_detected(wlc_dfs_info_t *dfs, bool scan_core)
{
	...
	static const struct {
		...
		{RADAR_TYPE_UK1, "UK1"},
		{RADAR_TYPE_UK2, "UK2"},
		{RADAR_TYPE_NEW_JP_1, "NEW-JP-1"},
		{RADAR_TYPE_NEW_JP_2, "NEW-JP-2"},
		{RADAR_TYPE_NEW_JP_3, "NEW-JP-3"},
		{RADAR_TYPE_NEW_JP_4, "NEW-JP-4"},
		{RADAR_TYPE_NEW_JP_5, "NEW-JP-5"},
		{RADAR_TYPE_NEW_JP_6, "NEW-JP-6"},
		{RADAR_TYPE_NEW_JP_7, "NEW-JP-7"},
		{RADAR_TYPE_NEW_JP_8, "NEW-JP-8"},
	...
	if ((CHSPEC_IS8080(wlc->chanspec) || (CHSPEC_IS160(wlc->chanspec) &&
		...
		radar_type = phy_radar_detect(WLC_PI(wlc),
			&radar_detected, FALSE, TRUE);
		radar_type_u80 =
			phy_radar_detect(WLC_PI(wlc),
	...
	if ((CHSPEC_IS8080(chspec) || (CHSPEC_IS160(chspec) &&
		...
		LOWER_80_SB(chspec))) && (wlc->band->bandtype != WLC_BAND_2G)) {
	...
	if ((wlc_11h_get_spect_state(wlc->m11h, cfg) & radar_sim_mask) ||
		...
		WL_DFS(("wl%d: ###### DETECTED A RADAR on Chanspec 0x%x subband 0x%x type %d "
			"type_80 %d on core %d\n", wlc->pub->unit, chspec,
			dfs->radar_subbands, radar_type, radar_type_u80, scan_core));
...
wlc_dfs_send_event(wlc_dfs_info_t *dfs, chanspec_t target_chanspec, bool radar_on_sc)
{
	...
	if (radar_on_sc) {
		...
		radar_data.current_chanspec = phy_utils_get_chanspec(WLC_PI(wlc));
	}
	...
	wlc_bss_mac_event(wlc, wlc->primary_bsscfg, WLC_E_RADAR_DETECTED, NULL, 0,
...
wlc_dfs_cacstate_ism_set(wlc_dfs_info_t *dfs)
{
	...
	dfs->dfs_cac.status.chanspec_cleared = phy_utils_get_chanspec(WLC_PI(wlc));
	 /* clear the channel */
	wlc_clr_quiet_chanspec(wlc->cmi, dfs->dfs_cac.status.chanspec_cleared);
	...
	if (BGDFS_ENAB(wlc->pub) && PHYMODE_BGDFS_ACTIVE(dfs->phymode)) {
		if (wlc_radar_detected(dfs, TRUE)) {
			...
			if (dfs->dfs_cac.ism_monitor == TRUE) {
				...
				WL_DFS(("wl%d: dfs : current chanspec %s is maintained as channel "
						"switching is disabled\n", wlc->pub->unit,
						wf_chspec_ntoa_ex(phy_utils_get_chanspec(wlc->pi),
								chanbuf)));
				...
				wlc_dfs_chanspec_oos(dfs, phy_utils_get_chanspec(WLC_PI(wlc)));
...
wlc_dfs_to_backup_channel(wlc_dfs_info_t *dfs, bool radar_detected, wlc_bsscfg_t *cfg)
{
	...
	if (dfs->dfs_cac.ism_monitor == TRUE) {
		...
		WL_DFS(("wl%d: dfs : current chanspec 0x%x is maintained as channel switching is"
			" disabled.\n",
			wlc->pub->unit, phy_utils_get_chanspec(WLC_PI(wlc))));
		return;
	}
...
wlc_dfs_cacstate_cac(wlc_dfs_info_t *dfs)
{
...
#if defined(BGDFS)
	if ((BGDFS_ENAB(wlc->pub)) && DFS_SCAN_IN_PROGRESS(dfs)) {
	...
	if (wlc_radar_detected(dfs, FALSE) == TRUE) {
		/* if already fall back channel is decided before initiating CAC, go
		 * for that.
		 */
		if (WLC_IS_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc)) {
			WLC_CLR_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc);
			wlc_dfs_chanspec_oos(dfs, phy_utils_get_chanspec(WLC_PI(wlc)));
			if (!dfs->old_chanspec || wf_chspec_malformed(dfs->old_chanspec)) {
				WL_ERROR(("wl%d: %s:- Malformed DFS old chanspec, "
					"WLC_CLR_SWITCH_TO_OLD_CHANSPEC logic should have"
				       " avoid this condition...\n", wlc->pub->unit,
				       __FUNCTION__));
				ASSERT(0);
			}
			wlc_do_chanswitch(wlc->primary_bsscfg, dfs->old_chanspec);
		...
		if (WL11H_STA_ENAB(wlc) && wlc_dfs_get_radar(wlc->dfs)) {
			...
			wlc_assoc_change_state(wlc->primary_bsscfg, AS_DFS_CAC_FAIL);
			WL_DFS(("wl%d: dfs radar detected \n", wlc->pub->unit));
		}
#endif /* SLAVE_RADAR */
		if (wlc_radar_chanspec(wlc->cmi, phy_utils_get_chanspec(WLC_PI(wlc))) == TRUE) {
	...
	if (!dfs->dfs_cac.duration) {
		bool any_ap_up = FALSE;
		int idx;
		wlc_bsscfg_t *apcfg;

		BCM_REFERENCE(any_ap_up);
#if defined(SLAVE_RADAR)
		if (WL11H_STA_ENAB(wlc) && wlc_dfs_get_radar(wlc->dfs)) {
			if (!wlc_cac_is_clr_chanspec(dfs, phy_utils_get_chanspec(WLC_PI(wlc)))) {
				wlc_cac_do_clr_chanspec(dfs, phy_utils_get_chanspec(WLC_PI(wlc)));
			}
		}
#endif /* SLAVE_RADAR */
		...
		/* MAP Requirement to switch back to previous chanspec after performing
		 * CAC on new chanspec.
		 *
		 * Possible cases:
		 * - Pure AP with MBSS, CSA issued for DFS channel: Once CAC finishes,
		 *   Radio switch back to previous channnel.
		 *
		 * - APSTA case && sta cfg up && associated: CAC is not required as
		 *   as APSTA supposed to follow upstream AP.
		 *
		 * - APSTA case && sta cfg down && keep_ap_up TRUE: As sta interface
		 *   is down hence sta should not accept scan request from wpa_supplicant
		 *   or from driver. Hence sta cfg wont be able to initiate CAC state
		 *   machine under SLAVE_RADAR.
		 *
		 * - APSTA case && sta cfg roaming && keep_ap_up TRUE:
		 *   TODO: Pending issue
		 *	DFS state machine switching to IDLE instead of CAC state
		 *	hence whole MAP requirement not working.
		 *
		 */
		if (WLC_IS_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc)) {
			WLC_CLR_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc);
			...
			/* inform user about CAC done and switching to ISM state */
			wlc_dfs_cac_state_change(dfs, WL_DFS_CACSTATE_ISM);
#ifdef STA
			if (!BSSCFG_AP(wlc->primary_bsscfg) &&
				(wlc_assoc_get_as_state(wlc->primary_bsscfg) == AS_DFS_CAC_START)) {
				/* Intentionally Abort assoc in case sta is trying to associate
				 * to upstream AP. Let sta try once again with
				 * !WLC_IS_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC.
				 */
				wlc_assoc_change_state(wlc->primary_bsscfg, AS_DFS_CAC_FAIL);
			}
#endif // endif
		...
		if (WL11H_STA_ENAB(wlc) && wlc_dfs_get_radar(wlc->dfs) &&
			...
			wlc_assoc_change_state(wlc->primary_bsscfg, AS_DFS_ISM_INIT);
			wlc_join_attempt_select(wlc->primary_bsscfg);
		}
#endif /* SLAVE_RADAR */
	}
}
...
wlc_dfs_cacstate_ism(wlc_dfs_info_t *dfs)
{
...
#ifdef SLAVE_RADAR
		!((BSSCFG_STA(cfg) && (DWDS_ENAB(cfg) || MAP_ENAB(cfg) || dfs->hndl_radarsta)) ||
	...
	wlc_dfs_chanspec_oos(dfs, chspec);
#endif /* WLTEST */

	if (dfs->dfs_cac.ism_monitor == TRUE) {
		/* channel switching is disabled */
		WL_DFS(("wl%d: dfs: current chanspec %s is maintained\n",
		        wlc->pub->unit, wf_chspec_ntoa_ex(chspec, chanbuf1)));
		return;
	}
#ifndef WLTEST
	wlc_dfs_chanspec_oos(dfs, chspec);
	...
	if (!BSSCFG_AP(cfg) && wlc_dfs_get_radar(wlc->dfs) && cfg->current_bss) {
		...
		wlc_csa_send_rrm_report_and_csa_to_upstream_ap(wlc->csa, cfg, &csa,
			dfs->radarch_len, dfs->radarch);
	}
#endif	/* SLAVE_RADAR */
	/* Move to a different channel when radar was seen during ISM */
	if (AP_ENAB(wlc->pub) && ap_up) {
		/* It is possible that radar detected simultaneously on Upstream AP
		 * and Repeater. Based on timing:
		 * - If repeater receives CSA from upstream AP following radar on AP,
		 *   Repeater wont send rrm report from repeater sta as well csa from
		 *   per ap cfg, received CSA will allow all AP cfg to send csa frames
		 *   and beacon updation for all associated clients.
		 */
		wlc_dfs_csa_each_up_ap(wlc, &csa, FALSE);
	} else {
	...
	WL_DFS(("wl%d: dfs : state to %s chanspec current %s next %s at %dms, starting CSA"
		" process\n",
		wlc->pub->unit, wlc_dfs_cacstate_str(dfs->dfs_cac.status.state),
		wf_chspec_ntoa_ex(phy_utils_get_chanspec(WLC_PI(wlc)), chanbuf1),
 ...
 */
static bool
wlc_dfs_process_action_frame_status(wlc_dfs_info_t *dfs, wlc_bsscfg_t *bsscfg, uint txstatus)
{
	wlc_info_t *wlc = dfs->wlc;

	if ((txstatus & TX_STATUS_ACK_RCV) || !bsscfg) {
		...
		if (dfs->actionfr_retry_counter < WLC_RADAR_REPORT_RETRY_LIMIT) {
			...
			wlc_11h_send_basic_report_radar(wlc, bsscfg,
...
wlc_dfs_send_action_frame_complete(wlc_info_t *wlc, uint txstatus, void *arg)
{
	...
	ASSERT((int)(uintptr)arg != WLC_BSSCFG_INVALID_IDX);
	bsscfg = wlc_bsscfg_find(wlc, (int)(uintptr)arg, NULL);
	ASSERT(bsscfg != NULL);

	if (wlc_dfs_process_action_frame_status(dfs, bsscfg, txstatus) == TRUE) {
		/* delete the timer */
...
wlc_dfs_cacstate_init(wlc_dfs_info_t *dfs)
{
	...
	phy_radar_detect_enable(WLC_PI(wlc), dfs->radar != 0);
	...
	if (wlc_radar_chanspec(wlc->cmi, chspec) == TRUE) {
		bool sta_up, any_ap_up, sta_associated = FALSE;
		int idx;
		wlc_bsscfg_t *cfg;

		sta_up = any_ap_up = FALSE;
		dfs->dfs_cac_enabled = TRUE;
		wlc_dfs_timer_add(dfs);
#if defined(BGDFS)
		if (BGDFS_ENAB(wlc->pub) &&
		...
		WL_REGULATORY(("wl%d: CAC or ISM state for cactime[%d] skip_pre_ism[%d]"
			" any_ap_up[%d] sta_up[%d] cac_clr[%d] for CHANSPEC[%x]\n",
			wlc->pub->unit,	dfs->dfs_cac.cactime, skip_pre_ism, any_ap_up,
			sta_up, wlc_cac_is_clr_chanspec(dfs, chspec), chspec));
		...
		if (dfs->dfs_cac.cactime && !skip_pre_ism) {
			if (sta_up && sta_associated) {
				WL_REGULATORY(("wl%d: switch to ISM state\n", wlc->pub->unit));
				wlc_dfs_cacstate_ism_set(dfs);
			} else if (any_ap_up ||
#ifdef SLAVE_RADAR
					!wlc_cac_is_clr_chanspec(dfs, chspec) ||
#endif /* SLAVE_RADAR */
					FALSE) {
...
wlc_set_dfs_cacstate(wlc_dfs_info_t *dfs, int state, wlc_bsscfg_t *cfg)
{
	...
	/* reset radar settings when wl up */
	if (wlc->pub->up)
		phy_radar_tuning_reset(WLC_PI(wlc));
...
wlc_dfs_set_radar(wlc_dfs_info_t *dfs, int radar, uint subband)
{
	...
	chanspec_t chanspec = phy_utils_get_chanspec(WLC_PI(wlc));
	...
	if (radar == (int)WL_RADAR_SIMULATED || radar == (int)WL_RADAR_SIMULATED_SC) {
		...
		if (DFS_HAS_SUBBAND_INFO(wlc)) {
			...
			if ((subband & 0xFF) == 0 && dfs->dfs_cac.ism_monitor) {
				wlc_clr_quiet_chanspec(wlc->cmi, phy_utils_get_chanspec(wlc->pi));
				bzero(dfs->chan_blocked, sizeof(dfs->chan_blocked));
			}
#endif /* WLTEST */
...
#endif /* BGDFS_2G */
	{
		if (wlc_get_band(wlc, BAND_5G_INDEX) == NULL) {
			return BCME_BADBAND; /* 5G band is not supported */
		}
	}

	dfs->radar = (uint32)radar;

	phy_radar_detect_enable(WLC_PI(wlc), radar != 0);
	...
	/* Handle the PM mode for pure Wi-Fi client case if "wl radar" is set to 0/1 */
	if ((!wlc_is_ap_interface_up(wlc)) && WL11H_STA_ENAB(wlc) && cfg &&
		wlc_radar_chanspec(wlc->cmi, cfg->current_bss->chanspec) && cfg->associated) {
		if (dfs->radar == 0) {
...
#endif /* SLAVE_RADAR */
void
wlc_dfs_save_old_chanspec(wlc_dfs_info_t *dfs, chanspec_t chspec)
{
	ASSERT(dfs != NULL);

	dfs->old_chanspec = chspec;
}

void
wlc_dfs_reset_old_chanspec(wlc_dfs_info_t *dfs)
{
	ASSERT(dfs != NULL);
...
bool
wlc_dfs_cac_in_progress(wlc_dfs_info_t *dfs)
{
	ASSERT(dfs);

	switch (dfs->dfs_cac.status.state) {
