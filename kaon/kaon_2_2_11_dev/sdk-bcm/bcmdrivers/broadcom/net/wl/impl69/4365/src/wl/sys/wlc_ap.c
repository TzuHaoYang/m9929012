 ...
 *
 * $Id: wlc_ap.c 792675 2020-11-02 04:26:25Z $
 */
...
enum {
	...
	IOV_MAP_PROFILE ,		/**< get/set multiap profile */
	IOV_MAP_8021Q_SETTINGS ,	/**< get/set multiap default 802.1Q Settings */
#endif	/* MULTIAP */
...
static const bcm_iovar_t wlc_ap_iovars[] = {
	...
	{"map_profile", IOV_MAP_PROFILE, (IOVF_SET_DOWN),  IOVT_UINT8, 0},
	{"map_8021q_settings", IOV_MAP_8021Q_SETTINGS, 0,  IOVT_UINT16, 0},
#endif	/* MULTIAP */
	{"bcnprs_txpwr_offset", IOV_BCNPRS_TXPWR_OFFSET,
	(0), IOVT_UINT8, 0
	},
	{"ucast_disassoc_on_bss_down", IOV_UCAST_DISASSOC_ON_BSS_DOWN,
	0, IOVT_BOOL, 0},
	{NULL, 0, 0, 0, 0 }
};
		...
		if (scb != NULL) {
			...
			if (mac_mode_deny_client && !scb) {
				...
				wlc_bss_mac_event(wlc, bsscfg, WLC_E_AUTH_FAIL, &scb->ea,
					WLC_E_STATUS_SUCCESS, status, auth_alg, body,
					body_len);
			}
		}
	...
	assoc_decision_t * dc, struct scb *scb, wlc_assoc_req_t * param)
{
	...
	param = SCB_ASSOC_CUBBY((wlc_ap_info_pvt_t *)ap, scb);
	if (!param || !param->body) {
	...
	if ((scb->WPA_auth == WPA3_AUTH_SAE_PSK) && (scb->pmkid_included) &&
	    (param->status == DOT11_SC_SUCCESS) && (scb->state & PENDING_ASSOC)) {
		/* This is response to pending assoc state. Clear state, continue with done */
		wlc_scb_clearstatebit(scb, PENDING_ASSOC);
		/* If response was failure then assoc_approved would be FALSE */
		param->status = DOT11_SC_SUCCESS;
	...
	if (param->status != DOT11_SC_SUCCESS) {
		if (WLEXTSTA_ENAB(wlc->pub))
			...
			wlc_bss_mac_event(wlc, bsscfg, reassoc ? WLC_E_REASSOC_FAIL :
				WLC_E_ASSOC_FAIL, &hdr->sa, WLC_E_STATUS_SUCCESS, param->status,
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_CLOSEDNET):
		*ret_int_ptr = bsscfg->closednet_nobcprbresp;
		break;
...
wlc_ap_scb_state_upd_cb(void *ctx, scb_state_upd_data_t *notif_data)
{
	 ...
	 * attribute assoc disallowed to 0.
	*/

	if (wlc_ap_stas_associated(wlc->ap) < appvt->maxassoc) {
		/* Update mbo attr to assoc allowed */
		wlc_mbo_update_attr_assoc_disallowed(scb->bsscfg, 0);
	}
#endif /* WL_MBO && !WL_MBO_DISABLED */
}
#endif /* WL_GLOBAL_RCLASS */
