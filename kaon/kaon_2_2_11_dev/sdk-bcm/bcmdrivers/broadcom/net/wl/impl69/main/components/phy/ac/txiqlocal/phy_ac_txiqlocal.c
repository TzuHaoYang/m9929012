 ...
 *
 * $Id: phy_ac_txiqlocal.c 791962 2020-10-14 17:19:05Z $
 */
	...
	FOREACH_ACTV_CORE(pi, phyrxchain, core) {
		...
		*/
		...
		if (Biq2byp) {
			...
			if (CHSPEC_IS2G(pi->radio_chanspec)) {
				// 2g is not using auxlna_path/mainGm_path yet
				if (0) {
				RADIO_REG_LIST_START
					...
					MOD_RADIO_REG_20708_ENTRY(pi, TX2G_CFG1_OVR, 0,
						ovr_txdb_mx_gc_cas, 0x1)
					MOD_RADIO_REG_20708_ENTRY(pi, TX2G_MIX_GC_REG, 0,
						txdb_mx_gc_cas, 0x3f)
				RADIO_REG_LIST_EXECUTE(pi, core);
				}
			} else {
	...
	uint8 multilo_cal_cnt)
{
	 ...
	 */
	if (!ACMAJORREV_40(pi->pubpi->phy_rev) && !ACMAJORREV_130(pi->pubpi->phy_rev) &&
	...
	if ((ACMAJORREV_40(pi->pubpi->phy_rev))) {
		if (CHSPEC_IS20(pi->radio_chanspec) && CHSPEC_ISPHY5G6G(pi->radio_chanspec)) {
			...
			if (ti->prerxcal == 0) {
				...
				FOREACH_CORE(pi, core) {
					...
					wlc_phy_cal_txiqlo_coeffs_acphy(pi, CAL_COEFF_WRITE_BIQ2BYP,
					                                coeffs, PI_INTER_COEFFS_AB,
					                                core);
					/* Print out Results a & b */
					PHY_CAL(("\tcore-%d: a/b = (%4d,%4d)\n", core,
					        (int16)accal->txiqlocal_biq2byp_coeffs[core*2+0],
					        (int16)accal->txiqlocal_biq2byp_coeffs[core*2+1]));
				} /* for core */
			}
	...
	txgain_setting_t orig_txgain[], bool suspend)
{
	...
	/* BCM7271 CRDOT11ACPHY-2219 radioreg crash WAR */
	if (ACMAJORREV_37(pi->pubpi->phy_rev) || ACMAJORREV_GE47(pi->pubpi->phy_rev)) {
		wlc_phy_resetcca_acphy(pi);
	}
	...
	*/
	...
	FOREACH_ACTV_CORE(pi, coremask, core) {
		...
		wlc_phy_cal_txiqlo_coeffs_acphy(pi, CAL_COEFF_READ, &d_reg,
			TB_OFDM_COEFFS_D, core);
	if (Biq2byp == 1) {
		ab_int[0] = (int16) accal->txiqlocal_biq2byp_coeffs[2*core + 0];
		ab_int[1] = (int16) accal->txiqlocal_biq2byp_coeffs[2*core + 1];
	}
                             ...
                             bool for_iqcal, bool init_adc_inside, uint16 ADCcore, bool champ)
{
	...
	if (is_tssi) {
		...
		if (for_idle) {
			...
			if (RADIOID_IS(pi->pubpi->radioid, BCM20698_ID) &&
				...
				FOREACH_CORE(pi, core) {
					if (core < pll_cores) {
						orig_reg_vals[core].orig_afediv_cfg1_ovr =
							READ_RADIO_PLLREG_20708(pi,
								AFEDIV_CFG1_OVR, core);
						orig_reg_vals[core].orig_afediv_reg3 =
							READ_RADIO_PLLREG_20708(pi,
								AFEDIV_REG3, core);
					}
				}
		...
		if (for_idle && RADIOID_IS(pi->pubpi->radioid, BCM20698_ID) &&
			...
			FOREACH_CORE(pi, core) {
				if (core < pll_cores) {
					WRITE_RADIO_PLLREG_20708(pi, AFEDIV_CFG1_OVR, core,
						orig_reg_vals[core].orig_afediv_cfg1_ovr);
					WRITE_RADIO_PLLREG_20708(pi, AFEDIV_REG3, core,
						orig_reg_vals[core].orig_afediv_reg3);
				}
			}
			MOD_PHYREG(pi, TSSIMode, tssiADCSel, 0);
