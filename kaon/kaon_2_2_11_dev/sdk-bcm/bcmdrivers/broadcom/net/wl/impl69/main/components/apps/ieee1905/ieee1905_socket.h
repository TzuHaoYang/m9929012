...
#define I5_SOCKET_FLAG_BLOCKING   0x02  /* This socket is blocking state(STP) */

/* Helper MACRO for _i5_socket_type flags */
#define I5_SOCKET_IS_LOOPBACK(flags)  ((flags) & I5_SOCKET_FLAG_LOOPBACK)
#define I5_SOCKET_IS_BLOCKING(flags)  ((flags) & I5_SOCKET_FLAG_BLOCKING)
...
typedef struct _i5_socket_type {
  ...
  union {
    struct {
      ...
      timer_elem_type        *pSTPStateChangeTmr;
...
typedef struct {
  ...
  unsigned int          topologyQueryTimeoutMsec; /* Timeout for topology query */
  unsigned int          topologyQueryRetries; /* Number of times topology query to be retried */
#ifdef MULTIAP
  ...
  unsigned char m1_max_timeout; /* M1 Timeout maximum value */
  ...
  time_t ap_cap_collection_time;  /* AP Capability Collection time */
  unsigned char map_profile;  /* MultiAP Profile */
  unsigned char ctrl_autoconfig_msg_count; /* autoconfig search msgs sent by controller at bootup */
#endif /* MULTIAP */
  unsigned char selfLinkMetricTimeout; /* Timeout in sec to Automatically get link metrics */
#if defined(USEBCMUSCHED)
