 ...
 *
 * $Id: wlc_assoc.h 793809 2020-12-10 11:37:45Z $
 */
...
extern void wlc_assoc_check_drain_complete(wlc_info_t *wlc);
...
/* wlc_roam:state variables different states */
#define WLC_ROAM_STATE_ENABLED		0	/* Roaming enabled for Low RSSI and on disconnect */
#define WLC_ROAM_STATE_DISABLED		1	/* Roaming disabled completely */
#define WLC_ROAM_STATE_LOW_RSSI		2	/* Roaming enabled only on low RSSI */

#define WLC_ROAM_ENABLED(roam)		((roam)->state == WLC_ROAM_STATE_ENABLED)
#define WLC_ROAM_DISABLED(roam)		((roam)->state == WLC_ROAM_STATE_DISABLED)
#define WLC_ROAM_ON_LOW_RSSI(roam)	((roam)->state == WLC_ROAM_STATE_LOW_RSSI)

struct wlc_roam {
	uint	bcn_timeout;		/**< seconds w/o bcns until loss of connection */
	bool	assocroam;		/**< roam to preferred assoc band in oid bcast scan */
	uint8	state;			/**< Roam state. State can have values WLC_ROAM_STATE_XXX */
	...
	chanvec2g5g6g_t roam_chn_cache;	/**< split roam scan channel cache */
	...
	bool	multi_ap;	/**< TRUE when there is more than one AP for the SSID on one band */
...
extern int wlc_ext_auth_tx_complete(wlc_info_t *wlc, wlc_bsscfg_t *cfg, void *pkt, void *arg);
