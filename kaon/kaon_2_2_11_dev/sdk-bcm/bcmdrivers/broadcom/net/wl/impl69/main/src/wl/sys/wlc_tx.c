 ...
 *
 * $Id: wlc_tx.c 793809 2020-12-10 11:37:45Z $
 *
 */
...
wlc_low_txq_map_pkts(wlc_info_t *wlc, wlc_txq_info_t *qi, map_pkts_cb_fn cb, void *ctx)
{
	...
	for (item_p = dll_head_p(&txq->active); !dll_end(&txq->active, item_p);
		...
		if (wlc->active_queue == qi && low_txq_map.pkt_cnt > 0) {
			/* Packtes remove from low txq */
			WL_NONE(("wl%d %s: updating fifo %d TXPKTPEND=%d decrement by %d!\n",
				wlc->pub->unit, __FUNCTION__, fifo, TXPKTPENDGET(wlc, fifo),
				low_txq_map.pkt_cnt));
			TXPKTPENDDEC(wlc, fifo, low_txq_map.pkt_cnt);
		}
...
wlc_txq_fill(txq_info_t *txqi, txq_t *dest_txq)
{
	...
	for (ac = AC_COUNT - 1; ac >= 0; ac--) {
		...
		wlc_low_txq_enq_list(txqi, dest_txq, fifo_idx, &temp_q, pkt_cnt);
	} /* for */
#ifdef WLTAF
...
bcmc_pkt_q_check(wlc_info_t *wlc, struct scb *bcmc_scb, wlc_pkt_t pkt)
{
	...
	if (wlc_apps_bcmc_ps_enqueue(wlc, bcmc_scb, pkt) < 0) {
		...
		TX_PKTDROP_COUNT(wlc, bcmc_scb, TX_PKTDROP_RSN_PS_ENQ_FAILED);
		PKTFREE(wlc->osh, pkt, TRUE);
	}
...
wlc_pull_q(wlc_info_t *wlc, uint ac_fifo, struct spktq *output_q, uint *fifo_idx)
{
	...
	max_pkt_cnt = __txq_swq_space(wlc, txq_swq);
...
wlc_send_deauth_on_txcmplt(wlc_info_t *wlc, uint txstatus, void *arg)
{
	...
	wlc_senddeauth(wlc, bsscfg, scb, &cbarg->ea, &bsscfg->BSSID,
		&bsscfg->cur_etheraddr,  DOT11_RC_8021X_AUTH_FAIL, TRUE);
...
wlc_prep_sdu(wlc_info_t *wlc, struct scb *scb, void **pkts, int *npkts, uint fifo)
{
	...
	if (SCB_PS_TXFIFO_BLK(scb)) {
		WL_PS(("wl%d: %s: TXFIFO blocked due to ps_txfifo_blk\n",
			wlc->pub->unit, __FUNCTION__));
...
wlc_tx_open_datafifo(wlc_info_t *wlc)
{
	 ...
	 */
	if (block_datafifo && TXPKTPENDTOT(wlc) == 0) {
#ifdef WL_BSSCFG_TX_SUPR
...
#if !defined(WL_PS_SCB_TXFIFO_BLK)
		if (block_datafifo & DATA_BLOCK_PS) {
	...
	if (wlc->bcmcfifo_drain && TXPKTPENDGET(wlc, TX_BCMC_FIFO) == 0) {
		...
		if ((!WLPKTFLAG_AMPDU(pkttag)) && (frag == 0)) {
			...
			if (delta >= 0) {
				...
				if (AMSDU_TX_ENAB(wlc->pub) &&
					...
					WL_INFORM(("edcf_txop changed, update AMSDU\n"));
...
wlc_tx_fill_link_entry(wlc_info_t *wlc, scb_t *scb, d11linkmem_entry_t *link_entry)
{
	...
	link_entry->RtsDurThresh = htol16(BCM_UINT16_MAX); /* default to -1 (=disabled) */
#ifdef WLMCNX
	if (MCNX_ENAB(wlc->pub)) {
	...
	ratespec_t rspec_override)
{
	...
	if (SCB_PROXD(scb)) {
		...
		if (rspec_override == wlc->band->rspec_override) {
			...
			WL_INFORM(("wl%d: %s, override: %x for scb: %s, using basic spec\n",
				wlc->pub->unit, __FUNCTION__, rspec_override, eabuf));
			special_rate_block = WLC_RLM_SPECIAL_RATE_BASIC;
		}
	} else if ((parms.type == FC_TYPE_MNG) ||
	...
	if (SCB_DLOFDMA(scb) && WLPKTFLAG_AMPDU(parms.pkttag) &&
		...
		/* Check if MU FIFO */
		wlc_fifo_isMU(wlc->fifo, scb, ac)) &&
		!wlc_fifo_is_ulofdma(wlc->fifo, scb)) {
...
#ifdef AP
#ifdef WL_PS_STATS
extern void wlc_apps_upd_pstime(struct scb *scb);
#endif /* WL_PS_STATS */
...
#endif /* MBSS */
	if (wlc->main_ap_bsscfg != NULL) {
		struct scb *bcmc_scb = WLC_BCMCSCB_GET(wlc, wlc->main_ap_bsscfg);
		if (bcmc_scb != NULL) {
	...
	for (i = 0; i < WLC_HW_NFIFO_INUSE(wlc); i++, txq_swq++) {
		...
		if (!PIO_ENAB((wlc)->pub)) {
			...
			HWA_PKTPGR_EXPR({
				...
				hwa_pktpgr_txfifo_shadow_reclaim(hwa_dev,
					WLC_HW_MAP_TXFIFO(wlc, i));
				// Remove specific packets in PageOut Req Q to SW shadow.
				hwa_pktpgr_pageout_ring_shadow_reclaim(hwa_dev,
					WLC_HW_MAP_TXFIFO(wlc, i));
			});
#endif // endif
		...
		WL_INFORM(("MQ: %s: fifo %d: collected %d pkts\n", __FUNCTION__,
		...
		*/
		...
		if ((TXPKTPENDGET(wlc, i) > 0) && (pkt_cnt > 0)) {
			 ...
			 */
			wlc_txfifo_complete(wlc, NULL, i, pkt_cnt);
		}
...
wlc_tx_fifo_scb_flush(wlc_info_t *wlc, struct scb *remove)
{
...
#ifdef WL_TXPKTPEND_SYNC
	if ((wlc_scb_tot_inflt_fifocnt(wlc, remove) != 0)) {
		...
		WL_ERROR(("wl%d %s "MACF" BCMC %d not flushed %u\n",
			...
			wlc_scb_tot_inflt_fifocnt(wlc, remove)));
	 ...
	 */
	if (!wlc->psm_watchdog_debug) {
		ASSERT(wlc_scb_tot_inflt_fifocnt(wlc, remove) == 0);
	}
#endif /* WL_TXPKTPEND_SYNC */
}
...
wlc_txq_dump_amsdu_txq(wlc_info_t *wlc, wlc_bsscfg_t * cfg, struct scb *scb, struct bcmstrbuf *b)
{
	...
	if (wlc->ami &&
		...
		for (idx = 0; idx < NUMPRIO; idx++) {
			if ((queued = wlc_amsdu_tx_queued_pkts(wlc->ami, scb, idx))) {
