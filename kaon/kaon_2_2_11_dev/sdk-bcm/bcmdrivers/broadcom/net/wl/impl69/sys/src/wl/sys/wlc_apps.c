 ...
 *
 * $Id: wlc_apps.c 793359 2020-11-24 12:05:23Z $
 */
...
wlc_apps_print_ps_stuck_info(wlc_info_t *wlc, struct scb *scb)
{
	...
	WL_ERROR(("ps_txfifo_blk: %d,  pkts_inflt_fifocnt_tot:%d\n",
		SCB_PS_TXFIFO_BLK(scb), wlc_scb_tot_inflt_fifocnt(wlc, scb)));
...
wlc_apps_bcmc_scb_ps_on(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	...
	if (bcmc_scb->ps_starttime == 0) {
		bcmc_scb->ps_on_cnt++;
		bcmc_scb->ps_starttime = (uint32_t)jiffies_to_msecs(jiffies);
	}
#endif /* WL_PS_STATS */
...
wlc_apps_scb_ps_on(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (scb->ps_starttime == 0) {
		scb->ps_on_cnt++;
		scb->ps_starttime = (uint32_t)jiffies_to_msecs(jiffies);
	}
#endif /* WL_PS_STATS */
	if (wlc->clk) {
	...
	if (TXPKTPENDTOT(wlc) > 0) {
		...
		if (wlc->datablock_starttime == 0) {
			...
			wlc->datablock_cnt++;
			wlc->datablock_starttime = (uint32_t)WLC_GET_CURR_TIME(wlc);
...
wlc_apps_process_pend_ps(wlc_info_t *wlc)
{
	...
	if ((wlc->block_datafifo & DATA_BLOCK_PS) && TXPKTPENDTOT(wlc)) {
		/* Common queues has suppressed BCMC frames and packets are pending in TXFIFO
	...
	if (wlc->datablock_starttime != 0) {
		...
		uint32 datablock_now = (uint32_t)WLC_GET_CURR_TIME(wlc);
...
wlc_apps_psq(wlc_info_t *wlc, void *pkt, int prec)
{
	...
	if (WLPKTTAG(pkt)->flags3 & WLF3_SUPR) {
		...
		WL_PS(("wl%d.%d:%s(): psq length exceeded\n", wlc->pub->unit,
			WLC_BSSCFG_IDX(SCB_BSSCFG(scb)), __FUNCTION__));
		return FALSE;
...
_wlc_apps_ps_enq(void *ctx, struct scb *scb, void *pkt, uint prec)
{
	...
	if (!wlc_apps_psq(wlc, pkt, prec)) {
		...
		if (PROP_TXSTATUS_ENAB(wlc->pub)) {
			 ...
			 */
			WL_PS(("wl%d.%d: %s ps pkt %p suppressed AID %d\n", wlc->pub->unit,
...
#endif /* PROP_TXSTATUS */
		{
			WL_PS(("wl%d.%d: %s ps pkt %p discarded AID %d\n", wlc->pub->unit,
				WLC_BSSCFG_IDX(SCB_BSSCFG(scb)), __FUNCTION__, pkt, SCB_AID(scb)));
		}
#ifdef WLTAF
...
wlc_apps_ps_enq(void *ctx, struct scb *scb, void *pkt, uint prec)
{
	...
	_wlc_apps_pvb_dec_in_transit(wlc, scb);
...
wlc_apps_pvb_upd_in_transit(wlc_info_t *wlc, struct scb *scb, uint vqdepth, bool op)
{
	...
	if (PROP_TXSTATUS_ENAB(wlc->pub) && BCMPCIEDEV_ENAB()) {
		...
		*/
		psq_len = pktq_n_pkts_tot(&scb_psinfo->psq) + scb_psinfo->in_transit;
#ifdef WLCFP
		if (CFP_ENAB(wlc->pub)) {
			psq_len += wlc_ampdu_scb_txpktcnt(wlc->ampdu_tx, scb);
		}
#endif /* WLCFP */
		if (psq_len < max_depth) {
...
static INLINE void
_wlc_apps_pvb_dec_in_transit(wlc_info_t *wlc, struct scb *scb)
{
#if defined(BCMPCIEDEV)
	if (PROP_TXSTATUS_ENAB(wlc->pub) && BCMPCIEDEV_ENAB()) {
		struct apps_wlc_psinfo *wlc_psinfo;
...
/* Assumption is the first up bss is the transmitted bss */
	...
	/* For now assuming the first up bsscfg is the transmitted bsscfg
	 * once we have code in place to identify any bsscfg as transmitted
...
wlc_apps_pvb_update(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (SCB_PS(scb) || SCB_TWTPS(scb)) {
...
#endif // endif
#ifdef WLCFP
		if (CFP_ENAB(wlc->pub) && (pktq_total == 0)) {
		...
		ps_count = ((scb->apsd.ac_delv == AC_BITMAP_NONE ||
			...
			pktq_total : wlc_apps_apsd_ndelv_count(wlc, scb));
#ifdef PROP_TXSTATUS
...
#endif /* PROP_TXSTATUS */
#ifdef WLTAF
		/* The TAF scheduler might not release traffic to a station in PS mode. So it could
		 * be that the ps queue is empty, despite higher pending traffic. So we check that
		 * case.
		 */
...
wlc_apps_psp_resp_complete(wlc_info_t *wlc, void *pkt, uint txs)
{
	...
	if (scb_psinfo->psp_pending) {
		...
		if (scb_psinfo->psp_flags & PS_PSP_REQ_PEND) {
			...
			WL_PS(("wl%d.%d %s send more frame.\n", wlc->pub->unit,
				WLC_BSSCFG_IDX(SCB_BSSCFG(scb)), __FUNCTION__));
...
wlc_apps_suppr_twt_frame_enq(wlc_info_t *wlc, void *pkt)
{
	...
	if (SCB_PSINFO(wlc->psinfo, scb) == NULL) {
		return TRUE;
	}
...
wlc_apps_twt_enter(wlc_info_t *wlc, scb_t *scb)
{
	...
	WL_TWT(("%s Enter AID %d current PS %d (%d/%d)\n", __FUNCTION__, SCB_AID(scb), SCB_PS(scb),
		wlc_scb_tot_inflt_fifocnt(wlc, scb), SCB_PS_TXFIFO_BLK(scb)));
#else /* ! WL_PS_SCB_TXFIFO_BLK */
	WL_TWT(("%s Enter AID %d current PS %d (%d)\n", __FUNCTION__, SCB_AID(scb), SCB_PS(scb),
		wlc_scb_tot_inflt_fifocnt(wlc, scb)));
#endif /* ! WL_PS_SCB_TXFIFO_BLK */
...
wlc_apps_apsd_send(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (scb_psinfo->ext_qos_null ||
	    ((wlc_apps_apsd_delv_count(wlc, scb) == 0) &&
#ifdef WLSQS
		(wlc_wlfc_scb_host_pkt_avail(wlc, scb) == FALSE) &&
#endif // endif
		TRUE)) {
#ifdef WLTDLS
		if (BSS_TDLS_ENAB(wlc, scb->bsscfg) &&
	...
	struct dot11_header *h, bool last_frag)
{
	...
	if ((wlc_apps_apsd_delv_count(wlc, scb) > 0) ||
#ifdef WLSQS
		wlc_wlfc_scb_host_pkt_avail(wlc, scb) ||
#endif // endif
		FALSE)
	{
		h->fc |= htol16(FC_MOREDATA);
...
#ifdef WLCFP
	if (CFP_ENAB(wlc->pub) && (delv_count == 0)) {
	...
	FOREACH_UP_AP(wlc, i, bsscfg) {
		if (!(bsscfg->flags & WLC_BSSCFG_PS_OFF_TRANS)) { /* Ignore BSS in PS-OFF trans */
			...
			if ((bss_info->ps_nodes) ||
				...
				if (bcmc_scb->ps_starttime == 0) {
					bcmc_scb->ps_on_cnt++;
					bcmc_scb->ps_starttime =
						(uint32_t)jiffies_to_msecs(jiffies);
				}
#endif /* WL_PS_STATS */
...
{
	uint32 ps_delta;
	uint32 ps_now = (uint32_t)jiffies_to_msecs(jiffies);
...
#ifdef PROP_TXSTATUS
void
wlc_apps_pvb_dec_in_transit(wlc_info_t *wlc, struct scb *scb)
{
	_wlc_apps_pvb_dec_in_transit(wlc, scb);
}
...
wlc_cfp_apps_ps_send(wlc_info_t *wlc, struct scb *scb, void *pkt, uint8 prio)
{
	...
	/* Enqueue the PS-Poll response at higher precedence level */
	if (!cpktq_prec_enq(wlc, &qi->cpktq, pkt, WLC_PRIO_TO_HI_PREC(prio))) {
		WL_ERROR(("wl%d.%d: %s: txq full, frame discarded\n",
		          wlc->pub->unit, WLC_BSSCFG_IDX(SCB_BSSCFG(scb)), __FUNCTION__));
		PKTFREE(wlc->osh, pkt, TRUE);
		wlc_apps_apsd_usp_end(wlc, scb);
