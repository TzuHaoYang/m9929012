 ...
 *
 * $Id: wlc_test.c 793358 2020-11-24 11:25:24Z $
 */
...
static const bcm_iovar_t test_iovars[] = {
	...
	{"pkteng_nav", IOV_PKTENG_NAV, 0, 0, IOVT_BOOL, 0},
...
#define WLC_TEST_PKTENGCMD_ULTX_ACMASK		0xf /* ulofdma blk all tx until rx trigger */
...
struct wlc_test_info {
	...
	uint32 nav;				/* Fixed NAV for QoS Null */
#endif /* TESTBED_AP_11AX */
};
...
static void update_pkt_eng_nav(wlc_info_t *wlc, uint32 nav);
#endif /* TESTBED_AP_11AX */
...
wlc_nvram_dump(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	if (wlc->pub->vars) {
		...
		if (wlc->dump_args) {
			args = wlc->dump_args;
			if (!strncmp(args, "-p", strlen("-p"))) {
				args += strlen("-p");
				p = bcm_atoi(args);
				if (p > (VARS_PAGE_NUM - 1)) {
					p = -1;
				}
			}
		}

		if ((wlc->pub->sromrev >= 18) && p >= 0) {
			page = vars_dump->pages[p];
			if (page) {
				while (*page != '\0') {
					/* 2 is for string terminator and '\n' */
					page_len += strlen(page) + 2;
					if (page_len > VARS_PAGE_SIZE) {
						break;
					}
					bcm_bprintf(b, "%s\n", page);
					page += strlen(page) + 1;
...
wlc_sim_pm_timer(void *arg)
{
	...
	wl_del_timer(wlc->wl, test->sim_pm_timer);
	switch (test->sim_pm_state) {
	case SIM_PM_STATE_AWAKE:
		test->sim_pm_state = SIM_PM_STATE_ASLEEP;
		...
		timeout = (test->sim_pm_cycle - test->sim_pm_up) / 1000;
		break;
	case SIM_PM_STATE_ASLEEP:
		test->sim_pm_state = SIM_PM_STATE_AWAKE;
		timeout = test->sim_pm_up / 1000;
		/* Fallthrough */
	...
	if (test->sim_pm_state != SIM_PM_STATE_DISABLE)
		wl_add_timer(wlc->wl, test->sim_pm_timer, timeout, FALSE);
	return;
}
#endif // endif
...
#if (defined(WLTEST) && !defined(WLTEST_DISABLED))
	if (!(test->sim_pm_timer = wl_init_timer(wlc->wl, wlc_sim_pm_timer, wlc, "simpmtimer"))) {
...
#if (defined(WLTEST) && !defined(WLTEST_DISABLED))
	if (test->sim_pm_timer) {
		wl_free_timer(wlc->wl, test->sim_pm_timer);
		test->sim_pm_timer = NULL;
	}
#endif // endif
	...
	void *params, uint plen, void *arg, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_PKTENG): {
		...
		uint corerev_minor = wlc->hw->corerev_minor;
#endif // endif
		...
		if ((pkteng_flags == WL_PKTENG_PER_TX_START) ||
			...
			if (!(D11REV_IS(corerev, 61) && D11MINORREV_LT(corerev_minor, 5)) &&
			...
			if ((pkteng_info == NULL) ||
				...
				do {
					...
					pktlist[i] = wlc_tx_testframe(wlc, &pkteng.dest, sa,
						...
						bsscfg_perif, userdata, userdata_len, NULL);
	...
	case IOV_SVAL(IOV_PKTENG_PKT_TMR):
		...
		break;
	case IOV_GVAL(IOV_PKTENG_NAV):
		*ret_int_ptr = test->nav;
		break;
	case IOV_SVAL(IOV_PKTENG_NAV):
		update_pkt_eng_nav(wlc, int_val);
		break;
	...
	case IOV_GVAL(IOV_SIM_PM):
		{
			...
			sim_pm->enabled = (test->sim_pm_state != SIM_PM_STATE_DISABLE) ? 1 : 0;
			sim_pm->cycle = (uint16)uS2TU(test->sim_pm_cycle);
			sim_pm->up = (uint16)uS2TU(test->sim_pm_up);
			break;
		}
	...
	case IOV_SVAL(IOV_SIM_PM):
		{
			...
			if (enable) {
				if (test->sim_pm_state == SIM_PM_STATE_DISABLE) {
					wlc_get(wlc, WLC_GET_PM, &test->sim_pm_saved_PM);
				...
				if (mboolisset(wlc->hw->wake_override, WLC_WAKE_OVERRIDE_TEST_APPS))
					...
					test->sim_pm_state = SIM_PM_STATE_ASLEEP;
				test->sim_pm_cycle = TU2uS(def ? SIM_PM_DEF_CYCLE :
					sim_pm->cycle);
				test->sim_pm_up = TU2uS(def ? SIM_PM_DEF_UP : sim_pm->up);
			}
			else {
				if (test->sim_pm_state == SIM_PM_STATE_DISABLE)
					break;
				wlc_iovar_setint(wlc, "wd_disable", 0);
				test->sim_pm_state = SIM_PM_STATE_DISABLE;
				wlc_mhf(wlc, MHF1, MHF1_ULP, 0, WLC_BAND_AUTO);
				wlc_set(wlc, WLC_SET_PM, test->sim_pm_saved_PM);
			}
			wl_del_timer(wlc->wl, test->sim_pm_timer);
			wl_add_timer(wlc->wl, test->sim_pm_timer, test->sim_pm_up / 1000,
				FALSE);
		}
		break;
#endif // endif
...
wlc_tx_testframe_len_dur_isvalid(wlc_info_t *wlc, ratespec_t rate_override, uint len)
{
	...
	if (wlc->primary_bsscfg->PLCPHdr_override == WLC_PLCP_LONG)
		shortpreamble = FALSE;
	else
		shortpreamble = TRUE;
...
wlc_tx_testframe(wlc_info_t *wlc, struct ether_addr *da, struct ether_addr *sa,
	...
	wlc_bsscfg_t *bsscfg_perif,
	void *userdata, int userdata_len, struct scb *scb)
{
	void *p;
	bool shortpreamble;
	uint16 txh_off = 0;
...
#define WAIT_COUNT_XI	100	/* wait count for tx interrupt */
	...
	if (STS_RX_ENAB(wlc->pub) || STS_XFER_PHYRXS_ENAB(wlc->pub)) {
	...
	if (STS_RX_ENAB(wlc->pub) || STS_RX_OFFLOAD_ENAB(wlc->pub)) {
		...
		do {
			...
			if (STS_RX_OFFLOAD_ENAB(wlc->pub)) {
				if (wlc_bmac_offload_rxsts_new_ready(wlc_hw))
					break;
			}
#endif /* WLC_OFFLOADS_RXSTS */
		} while ((rx_sts_list.rx_head == NULL) && (delay_cnt >= 0));

		wlc_bmac_recv_process_sts(wlc_hw, RX_FIFO, &rx_list, &rx_sts_list, 0);
	}
#endif /* STS_FIFO_RXEN || WLC_OFFLOADS_RXSTS */

#if defined(STS_XFER)
	if (STS_XFER_PHYRXS_ENAB(wlc->pub)) {
		wlc_sts_xfer_bmac_recv(wlc, RX_FIFO, &rx_list);
	}
#endif /* STS_XFER */
	...
	if (PKTISRXCORRUPTED(osh, rx_p)) {
		PKTRESETRXCORRUPTED(osh, rx_p);
...
#if defined(STS_XFER)
	if (STS_XFER_PHYRXS_ENAB(wlc->pub)) {
		wlc_sts_xfer_bmac_recv_done(wlc, RX_FIFO);
	}
#endif /* STS_XFER */
...
wlc_test_pkteng_cmd_use_rspec(wlc_test_info_t *test)
{
	...
	if ((rspec & WL_RSPEC_OVERRIDE_RATE) != 0)  {
		 ...
		 * format is supported
		 wlc_compute_plcp(wlc, wlc->primary_bsscfg, rspec, 6, 0, plcp);
...
wlc_test_pkteng_set_shmx_block(wlc_test_info_t *test)
{
	...
	for (i = 0; i < info->nusrs; i++) {
		...
		if ((ruidx >= 61) && (ruidx <= 64) && usr->scb) {
			if (!(wlc_he_get_peer_caps(wlc->hei, usr->scb) & SCB_HE_DL_242TONE)) {
				ruidx -= 61;
				ruidx *= 2;
...
wlc_test_pkteng_addsta(wlc_test_info_t *test, int usridx)
{
	...
	scb = wlc_scblookup(wlc, wlc->primary_bsscfg, &usr->ea);
...
#ifdef WL11AX
	wlc_he_update_scb_state(wlc->hei, wlc->band->bandtype, scb, &he_cap, NULL, NULL);
	...
	wl_pkteng_cmd_params_t* params)
{
	...
	!strncmp(keystr, "en", strlen("en"))) {
		...
		/* Also, set AC MASK */
		wlc_bmac_write_shm(wlc->hw, M_ULTX_ACMASK(wlc), WLC_TEST_PKTENGCMD_ULTX_ACMASK);
	} else {
		if (usridx == -1) {
			i = 0;
...
static void
update_pkt_eng_nav(wlc_info_t *wlc, uint32 nav)
{
	wlc_test_info_t *test = wlc->testi;
...
uint32
update_pkt_eng_get_nav_override(wlc_info_t *wlc)
{
	wlc_test_info_t *test = wlc->testi;
