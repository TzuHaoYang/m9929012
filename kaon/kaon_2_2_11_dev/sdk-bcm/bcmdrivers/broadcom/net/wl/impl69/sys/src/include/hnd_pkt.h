 ...
 *
 * $Id: hnd_pkt.h 793650 2020-12-04 09:10:40Z $
 */
...
#define PKTCOPYFLAGS(osh, lb1, lb2)      ({BCM_REFERENCE(osh); LBP(lb1)->flags \
		= LBP(lb2)->flags;})
#define PKTFLAGS(lb)				(LBP(lb)->flags)
...
#define PKTFRAGPKTFLAGS(osh, lb)  (((LBFP(lb)->flist.finfo[LB_FRAG_MAX].pktflags & 0xF0) >> 4))
#define PKTFRAGRESETPKTFLAGS(osh, lb)  (((LBFP(lb)->flist.finfo[LB_FRAG_MAX].pktflags) &= 0xF))
#define PKTFRAGSETPKTFLAGS(osh, lb, flags) ({BCM_REFERENCE(osh); \
	((LBFP(lb)->flist.finfo[LB_FRAG_MAX].pktflags) |= (flags << 4));})
