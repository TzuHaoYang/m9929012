 ...
 *
 * $Id: wlc_txc.c 791560 2020-10-01 08:23:54Z $
 */
...
/* scb cubby */
static uint wlc_txc_scb_secsz(void *ctx, struct scb *scb);
...
BCMATTACHFN(wlc_txc_attach)(wlc_info_t *wlc)
{
	...
	cubby_params.context = txc;
	cubby_params.fn_init = wlc_txc_scb_init;
	cubby_params.fn_deinit = wlc_txc_scb_deinit;
	cubby_params.fn_secsz = wlc_txc_scb_secsz;
	cubby_params.fn_dump = wlc_txc_scb_dump;
	priv->scbh = wlc_scb_cubby_reserve_ext(wlc, sizeof(scb_txc_info_t *),
		&cubby_params);
	if (priv->scbh < 0) {
...
void*
wlc_scb_txc_info(wlc_info_t *wlc, struct scb *scb)
{
	wlc_txc_t *txc = (wlc_txc_t *)wlc->txc;
	return SCB_TXC_INFO(txc, scb);
}
...
wlc_txc_cp(wlc_tx_rtc_t *rtc, void *pkt, uint *flags)
{
	wlc_info_t *wlc;
	scb_txc_info_t *sti;
	d11txhdr_t *txh;
	...
	htc_code = 0;
	if (SCB_QOS(scb) && rtc->he_htc_enab) {
		htc_enab = wlc_he_htc_rtc_tx(rtc, pkt);
		if (htc_enab) {
			htc_code = rtc->htc_code;
	...
	if (htc_enab) {
		...
		/* create room to insert htc+, move rest of hdr */
	...
	uint8 prio, uint16 txh_off, uint d11hdr_len)
{
...
#if defined(BCMDBG)
	/* validation moved from txc_cp */
	if (!(sti->txhlen >= (D11_TXH_SHORT_LEN(wlc) + DOT11_A3_HDR_LEN) &&
		sti->txhlen < wlc->txhroff)) {
		WL_ERROR(("wl%d: %s: txhlen=%d should be >= %d && < %d from txc of SCB "
		MACF"\n", wlc->pub->unit, __FUNCTION__, sti->txhlen,
		(int)((D11_TXH_SHORT_LEN(wlc) + DOT11_A3_HDR_LEN)),
		wlc->txhroff, ETHER_TO_MACF(scb->ea)));
	}
	ASSERT(sti->txhlen >= (D11_TXH_SHORT_LEN(wlc) + DOT11_A3_HDR_LEN) &&
		sti->txhlen < wlc->txhroff);
#endif /* BCMDBG */
	...
	if (CFP_ENAB(wlc->pub) == TRUE) {
		if (WLPKTTAG(pkt)->flags & WLF_MIMO) {
			wlc_cfp_tcb_upd_cache_state(wlc, scb, TRUE);
		} else {
			wlc_cfp_tcb_upd_cache_state(wlc, scb, FALSE);
		}
	}
#endif // endif
}
...
/* scb cubby */
static uint
wlc_txc_scb_secsz(void *ctx, struct scb *scb)
{
	wlc_txc_t *txc = (wlc_txc_t *)ctx;
	wlc_txc_info_priv_t *priv = &txc->priv;
	wlc_info_t *wlc = priv->wlc;
	...
	if (scb && !SCB_INTERNAL(scb) && WLC_TXC_ENAB(wlc)) {
		size = sizeof(scb_txc_info_t);
	}
	return size;
}
...
wlc_txc_scb_init(void *ctx, struct scb *scb)
{
	...
	*psti = wlc_scb_sec_cubby_alloc(wlc, scb, wlc_txc_scb_secsz(ctx, scb));
...
wlc_txc_scb_deinit(void *ctx, struct scb *scb)
{
	...
	wlc_scb_sec_cubby_free(wlc, scb, sti);
	*psti = NULL;
}
...
wlc_txc_scb_dump(void *ctx, struct scb *scb, struct bcmstrbuf *b)
{
	...
	bcm_bprhex(b, "\ttxh ", TRUE, sti->txh, txc->priv.wlc->txhroff);
