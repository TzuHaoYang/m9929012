 ...
 *
 * $Id: wlc_assoc.c 794271 2020-12-30 18:49:23Z $
 */
...
#define WLC_EXT_AUTH_TIMEOUT	1500	/* external auth timeout */
...
/* Auth timeout access macro */
#define BSSCFG_AUTH_TIMEOUT(cfg) \
	(BSSCFG_EXT_AUTH(cfg)) ? WLC_EXT_AUTH_TIMEOUT : WECA_AUTH_TIMEOUT + 10
...
static int wlc_assoc_scan_prep(wlc_bsscfg_t *cfg, wl_join_scan_params_t *scan_params,
	const struct ether_addr *bssid, const chanspec_t* chanspec_list, int n_channels);
...
static wlcband_t *get_band_by_bandtype(wlc_info_t *wlc, int bandtype);
...
ins_entry:
	...
	bc = cfg;
	do {
		wlc_bsscfg_t *temp = wlc->as->assoc_req[i];
		wlc->as->assoc_req[i] = bc;
		bc = temp;
...
wlc_assoc_req_process_next(wlc_info_t *wlc)
{
	...
	if ((cfg = wlc->as->assoc_req[0]) == NULL) {
...
wlc_assoc_req_remove_entry(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	for (i = 0; i < WLC_MAXBSSCFG; i ++) {
		...
		for (; i < WLC_MAXBSSCFG - 1; i ++) {
			if ((wlc->as->assoc_req[i] = wlc->as->assoc_req[i + 1]) == NULL) {
				i = i + 1;
				break;
			}
		}
		wlc->as->assoc_req[i] = NULL;
		err = BCME_OK;
		break;
	}

	/* if we cleared the wlc->as->assoc_req[] list, update ps_ctrl */
	if (as_in_progress_already != AS_IN_PROGRESS(wlc)) {
		wlc_set_wake_ctrl(wlc);
	}
	return err;
} /* wlc_assoc_req_remove_entry */
	...
	bool as_in_prog)
{
	...
	if (!BSSCFG_IS_PRIMARY(cfg) && (cfg->type != BSSCFG_TYPE_GENERIC)) {
...
/**
 * Prepare for join by using the default parameters in wlc->default_bss except SSID which comes from
 * cfg->SSID.
 *
 * @param[out] cfg->target_bss   Is modified to match wlc->default_bss
 */
int
	...
	wl_join_scan_params_t *scan_params, wl_join_assoc_params_t *assoc_params)
{
	...
	int n_channels = 0; /**< number of channels within chanspec list */
	int err;
	...
	if ((err = wlc_join_assoc_params_parse(wlc, cfg, assoc_params, &bssid,
			&chanspec_list, &n_channels)) != BCME_OK) {
		WL_ASSOC(("wl%d.%d: wlc_join_assoc_params_parse failed. err %d\n",
		          wlc->pub->unit, WLC_BSSCFG_IDX(cfg), err));
		goto fail;
	}
	return wlc_assoc_scan_prep(cfg, scan_params, bssid, chanspec_list, n_channels);
...
void
wlc_try_join_start(wlc_bsscfg_t *cfg, wl_join_scan_params_t *scan_params,
	wl_join_assoc_params_t *assoc_params)
{
	...
	/* Do not initiate roam if it is disabled or only low RSSI roam is enabled */
	if (cfg->roam && (WLC_ROAM_DISABLED(cfg->roam) || WLC_ROAM_ON_LOW_RSSI(cfg->roam))) {
...
wlc_bss_pref_score(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi, bool band_rssi_boost, uint32 *prssi)
{
	...
	band_type = CHSPEC_BANDTYPE(bi->chanspec);
	 ...
	 */
	if (cfg->join_pref_rssi_delta.band == band_type && band_rssi_boost &&
	 ...
	 */
	if (band_rssi_boost &&
	    CHSPEC_BANDTYPE(cfg->current_bss->chanspec) != band_type &&
		...
		WL_SRSCAN(("  prof: boost rssi: band=%d delta=%d rssi=%d",
			band_type, cfg->roam->roam_rssi_boost_delta, rssi));
	...
	for (j = 0, weight = 0; j < (int)join_pref->fields; j++) {
		...
		case WL_JOIN_PREF_BAND:
			...
			value = (band_type == join_pref->band) ? 1 : 0;
			break;
...
wlc_populate_join_pref_score(wlc_bsscfg_t *cfg, bool for_roam, join_pref_t *join_pref)
{
	...
	wlc_bss_info_t **bip = wlc->as->join_targets->ptrs;
	int i, j;
#if defined(BCMDBG) || defined(WLMSG_ASSOC)
	char eabuf[ETHER_ADDR_STR_LEN];
#endif /* BCMDBG || WLMSG_ASSOC */

	for (i = 0; i < (int)wlc->as->join_targets->count; i++) {
...
wlc_sort_on_join_pref_score(wlc_bsscfg_t *cfg, join_pref_t *join_pref)
{
	...
	if (wlc->as->join_targets->count > 1) {
		/* sort join_targets by join preference score in increasing order */
		for (k = (int)wlc->as->join_targets->count; --k >= 0;) {
			done = TRUE;
			for (j = 0; j < k; j++) {
				if (join_pref[j].score > join_pref[j+1].score) {
					...
					if ((CHSPEC_BANDTYPE(bip[j]->chanspec) == WLC_BAND_5G) &&
					(CHSPEC_BANDTYPE(bip[j+1]->chanspec) == WLC_BAND_2G)) {
						force_sort = TRUE;
					} else if (CHSPEC_BANDTYPE(bip[j]->chanspec) ==
						CHSPEC_BANDTYPE(bip[j+1]->chanspec)) {
...
wlc_cook_join_targets(wlc_bsscfg_t *cfg, bool for_roam, int cur_rssi)
{
	...
	uint32 join_pref_score_size = wlc->as->join_targets->count * sizeof(*join_pref);
	...
	WL_SRSCAN(("wl%d: RSSI is %d; %d roaming target[s]; Join preference fields "
		"are 0x%x", WLCWLUNIT(wlc), cur_rssi, wlc->as->join_targets->count,
		cfg->join_pref->fields));
	WL_ASSOC(("wl%d: RSSI is %d; %d roaming target[s]; Join preference fields "
		"are 0x%x\n", WLCWLUNIT(wlc), cur_rssi, wlc->as->join_targets->count,
		cfg->join_pref->fields));
	WL_ROAM(("ROAM: RSSI %d, %d roaming targets\n", cur_rssi,
		wlc->as->join_targets->count));
	...
	bip = wlc->as->join_targets->ptrs;
	bss_cnt = wlc->as->join_targets->count;
...
#if defined(WL_OCE) && defined(WL_OCE_STA)
	...
	if (for_roam &&
		...
		for (i = (int)wlc->as->join_targets->count - 1; i >= 0; i--) {
		...
		if (WBTEXT_ACTIVE(wlc->pub)) {
			...
			if (wlc_wnm_update_join_pref_score(wnm, cfg,
				wlc->as->join_targets, join_pref, cur_pref_score)) {
			...
			for (i = 0; i < (int)wlc->as->join_targets->count; i++) {
				...
				if (bip[i]->flags2 & WLC_BSS2_MAX_STA_CNT) {
					wlc_zero_pref(&join_pref[i]);
				}
			}
		...
		wlc->as->join_targets_last = j;
		WL_SRSCAN(("  result: %d target(s)", wlc->as->join_targets_last));
		WL_ASSOC(("  result: %d target(s)\n", wlc->as->join_targets_last));
	...
	if (wlc->pub->wlfeatureflag & WL_SWFL_WLBSSSORT)
		(void)wl_sort_bsslist(wlc->wl, bip, wlc->as->join_targets_last);

#ifdef OPPORTUNISTIC_ROAM
	if (!wlc->as->join_targets_last)
		return;
	best_ap_idx = (int)wlc->as->join_targets->count - 1;
...
wlc_reassoc(wlc_bsscfg_t *cfg, wl_reassoc_params_t *reassoc_params)
{
	...
	int n_channels = reassoc_params->chanspec_num;
	...
	return wlc_assoc_scan_prep(cfg, NULL, bssid, chanspec_list, n_channels);
} /* wlc_reassoc */
	...
	const struct ether_addr *bssid, const chanspec_t *chanspec_list, int n_channels)
{
	...
	return wlc_assoc_scan_start(cfg, scan_params, bssid, chanspec_list, n_channels);
}
...
/**
 * For 'split roam' scan, this function populates a caller-supplied array with chanspecs to scan.
 *
 * @param[inout] chanspecs Array to be populated, containing chanspecs to be scanned later on
 * @param[inout] count     Number of chanspecs in array chanspecs[]
 * @param[inout] hot_count Number of chanspecs with likely favorable scan results in chanspecs[]
 */
static void
split_roam_populate_chanspec_list(wlc_info_t *wlc, wlc_roam_t *roam, wlc_bsscfg_t *cfg,
	chanspec_t* chanspecs, int *count)
{
	enum wlc_bandunit bandunit;
	chanvec_t *chanvec;		/**< array containing one bit per channel */
	struct wlcband *band;
	uint channel;
	chanspec_t chanspec;

	/* Always include home channel as the first to be scanned */
	chanspecs[*count] = wf_ctlchspec20_from_chspec(cfg->current_bss->chanspec);
	(*count)++;

	FOREACH_WLC_BAND(wlc, bandunit) {
		chanvec = GET_CHANVEC(&roam->roam_chn_cache, bandunit);
		band = wlc_get_band(wlc, bandunit);
		FOREACH_WLC_BAND_CHANNEL20(band, channel) {
			if (isset(chanvec->vec, channel)) {
				chanspec = CH20MHZ_CHSPEC2(channel,
					BANDTYPE_CHSPEC(band->bandtype));
				/* Home channel is already included */
				if (chanspecs[0] != chanspec) {
					chanspecs[*count++] = chanspec;
					(*count)++;
					WL_SRSCAN(("  channel %d", channel));
					WL_ASSOC(("  channel %d\n", channel));
				}
			}
		}
 ...
 * @param[in] cfg->assoc->type  Indicates type of assoc: e.g. AS_ASSOCIATION or AS_ROAM.
 * @param[in] chanspec_list     NULL/n_channels means 'all available chanspecs'
 */
static int
	...
	const struct ether_addr *bssid, const chanspec_t *chanspec_list, int n_channels)
{
	...
	if (assoc) {
...
#ifdef WLRCC /* RCC: Roam Channel Cache */
		 ...
		 */
			...
			if (roam->split_roam_phase == 0) {
				...
				chanspecs_size = SCAN_MAX_CHANSPECS * sizeof(chanspec_t);
				chanspecs = (chanspec_t *)MALLOCZ(wlc->pub->osh, chanspecs_size);
				if (chanspecs == NULL) {
					WL_ERROR((WLC_BSS_MALLOC_ERR, WLCWLUNIT(wlc),
						WLC_BSSCFG_IDX(cfg), __FUNCTION__,
						(int)chanspecs_size, MALLOCED(wlc->pub->osh)));
					err = BCME_NOMEM;
					goto done;
				}
				split_roam_populate_chanspec_list(wlc, roam, cfg, chanspecs,
					&n_channels);
				chanspec_list = chanspecs;
			}

			/* Increment the split roam scan phase counter (cyclic). */
			roam->split_roam_phase = (roam->split_roam_phase == 0) ? 1 : 0;
		} else {
	...
	if (wlc->stas_associated > 0 &&
		...
		for (idx = 0; idx < n_channels; idx ++) {
			...
			if ((ctlchan & ~WL_CHANSPEC_BW_MASK) ==
			    (home_ctlchan & ~WL_CHANSPEC_BW_MASK)) {
				break;
			}
		}
		if (n_channels == 0 || idx < n_channels) {
	...
	if (ASSOC_CACHE_ASSIST_ENAB(wlc) && wlc->scan_results->count > 0) {
		...
		if (WLRCC_ENAB(wlc->pub)) {
			if ((chanspec_list == NULL) && roam->rcc_valid && !roam->roam_fullscan) {
				...
				n_channels = roam->n_rcc_channels; /* considering to use 1 */
				...
				WL_ROAM(("RCC: chanspec_list 0x%X, rcc valid %d, roam_fullscan %d, "
					...
					roam->roam_fullscan, n_channels));
			}
			roam->roam_fullscan = FALSE;
		}

		if (n_channels) {
			WL_SRSCAN(("scan start: %d channel(s)", n_channels));
		...
		*/
		...
		err = wlc_scan(wlc->scan, bss_type, bssid, 1, &ssid, scan_type, nprobes,
			active_time, passive_time, chanspec_list, n_channels, 0, TRUE,
			wlc_assoc_scan_complete, wlc, FALSE, scan_flag, cfg,
			SCAN_ENGINE_USAGE_NORM, NULL, NULL, sa_override);
	...
	/* clean scan_results list thats filled in wlc_assoc_cache_eval when scancache enabled */
	if (ASSOC_CACHE_ASSIST_ENAB(wlc))
		wlc_bss_list_free(wlc, wlc->scan_results);
...
wlc_speedy_recreate_fail(wlc_bsscfg_t *cfg)
{
	...
	wlc_bss_list_free(wlc, wlc->as->join_targets);
...
wlc_join_done(wlc_bsscfg_t *cfg, struct ether_addr *bssid, int bss_type, int status)
{
	...
	if (as->req_msch_hdl) {
		 ...
		 */
		if (!wlc_sta_timeslot_onchannel(cfg) && wlc_txqueue_active(wlc, cfg)) {
...
wlc_assoc_scan_complete(void *arg, int status, wlc_bsscfg_t *cfg)
{
	...
	bool for_roam;
	uint8 ap_count[MAXBANDS] = {0}; /**< for each band, the number of ap's that were scanned */
	uint i;
#if defined(BCMDBG) || defined(WLMSG_ASSOC)
	char *ssidbuf = NULL;
	const char *ssidstr;
...
#ifdef WLFBT
	    (!BSSCFG_IS_FBT(cfg) || !(cfg->WPA_auth & (WPA2_AUTH_FT | WPA3_AUTH_SAE_FBT))) &&
#endif /* WLFBT */
	    TRUE) {
	...
	if (WLCAUTOWPA(cfg)) {
		if (wlc_bss_list_expand(cfg, wlc->scan_results, wlc->as->join_targets)) {
			WL_ASSOC_ERROR(("wl%d: wlc_bss_list_expand failed\n", WLCWLUNIT(wlc)));
			wlc_join_done_int(cfg, WLC_E_STATUS_FAIL);
			goto exit;
		}
	} else {
		wlc_bss_list_xfer(wlc->scan_results, wlc->as->join_targets);
	}

	wlc->as->join_targets_last = wlc->as->join_targets->count;

	if (wlc->as->join_targets->count > 0) {
		WL_ASSOC(("wl%d: SCAN for %s%s: SSID scan complete, %d results for \"%s\"\n",
			WLCWLUNIT(wlc), msg_pref, msg_name,
			wlc->as->join_targets->count, ssidstr));
	...
	if (wlc->as->join_targets->count > 0) {
		...
		if (!cfg->associated || !for_roam) {
			...
			memset(&roam->roam_chn_cache, 0, sizeof(roam->roam_chn_cache));
		...
		/* Identify Multi AP Environment (more than 1 AP per band) */
		for (i = 0; i < wlc->as->join_targets->count; i++) {
			ap_count[CHSPEC_BANDUNIT(bip[i]->chanspec)]++;
			/* Build the split roam scan channel cache. */
			if (!roam->roam_chn_cache_locked) {
				chanspec_t chspec = bip[i]->chanspec;
				enum wlc_bandunit bu = CHSPEC_BANDUNIT(chspec);
				chanvec_t *chvec = GET_CHANVEC(&roam->roam_chn_cache, bu);

				setbit(chvec->vec, wf_chspec_ctlchan(chspec));
			}
		}

		if (ap_count[BAND_2G_INDEX] > 1 || ap_count[BAND_5G_INDEX] > 1 ||
		    ap_count[BAND_6G_INDEX] > 1) {
		...
		if (for_roam && !(as->flags & AS_F_CACHED_ROAM)) {
			...
			WL_ASSOC(("wl%d: ROAM: %d roam target%s after pruning\n",
				WLCWLUNIT(wlc), wlc->as->join_targets_last,
				(wlc->as->join_targets_last == 1) ? "" : "s"));
		} else {
	...
	if (wlc->as->join_targets_last > 0) {
		...
		if (for_roam &&
			(!WLFMC_ENAB(wlc->pub) ||
			cfg->roam->reason != WLC_E_REASON_INITIAL_ASSOC))
			wlc_roam_set_env(cfg, wlc->as->join_targets->count);
 ...
 */
int
wlc_roam_scan(wlc_bsscfg_t *cfg, uint reason, chanspec_t *list, uint32 n_channels)
{
	wlc_info_t *wlc = cfg->wlc;
	wlc_roam_t *roam = cfg->roam;
	int ret = BCME_OK;
	...
	if (cfg->roam_scan_params && cfg->roam_scan_params->active_time) {
		...
		/* original */
		wlc_assoc_scan_prep(cfg, NULL, NULL, list, n_channels);
	}
...
wlc_join_wsec_filter(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if ((bi->wpa2.flags & RSN_FLAGS_MFPR) && !(BSSCFG_IS_MFP_SUPPORTED(cfg))) {
 ...
 *
 * This function is called for a STA either joining a BSS or adopting an IBSS.
 *
 * @param[in] cfg->target_bss  Contains the BSS to join
 */
void
wlc_join_attempt(wlc_bsscfg_t *cfg)
{
	...
	for (; (wlc->as->join_targets_last > 0 && wlc->as->join_targets_last < MAXBSS);
		wlc->as->join_targets_last--) {
		wlc_rateset_t rateset;
		chanspec_t chanspec;
		...
		target_band = wlc->bandstate[CHSPEC_BANDUNIT(bi->chanspec)];
		WL_ROAM(("JOIN: checking [%d] %s\n", wlc->as->join_targets_last - 1,
		...
		if (!ISSIM_ENAB(wlc->pub->sih) &&
		    !wlc_join_chanspec_filter(cfg, wf_ctlchspec20_from_chspec(bi->chanspec))) {
		...
		memcpy(&rateset /* dst */, &bi->rateset /* src */, sizeof(bi->rateset));
		...
		if (APSTA_ENAB(wlc->pub)) {
			...
			if ((scb == NULL) && (!IS_SINGLEBAND(wlc))) {
				...
				FOREACH_WLC_BAND(wlc, bandunit) {
					...
					scb = wlc_scbfindband(wlc, cfg, &bi->BSSID, bandunit);
				}
			}
...
void
wlc_assoc_check_drain_complete(wlc_info_t *wlc)
{
	wlc_bsscfg_t *cfg;
	...
	if (cfg->assoc->state == AS_WAIT_TX_DRAIN &&
		pktq_empty(WLC_GET_CQ(cfg->wlcif->qi)) && TXPKTPENDTOT(wlc) == 0) {
...
wlc_join_check_ap_need_csa(wlc_info_t *wlc, wlc_bsscfg_t *cfg, chanspec_t chanspec, uint state)
{
	wlc_assoc_t *as = cfg->assoc;
	bool need_csa = FALSE;
	chanspec_t curr_chanspec = phy_utils_get_chanspec(WLC_PI(wlc));
...
wlc_join_ap_do_csa(wlc_info_t *wlc, chanspec_t tgt_chanspec)
{
	...
	if (WL11H_ENAB(wlc)) {
		FOREACH_UP_AP(wlc, apidx, apcfg) {
			...
			if ((phy_utils_get_chanspec(WLC_PI(wlc)) == apcfg->current_bss->chanspec) &&
#ifdef WLMCHAN
				(!MCHAN_ENAB(wlc->pub) ||
...
wlc_join_BSS_limit_caps(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if (bi->flags & WLC_BSS_HT) {
		...
		if (!ht_wsec) {
			...
			bi->chanspec = wf_ctlchspec20_from_chspec(bi->chanspec);
			/* disable VHT and SGI80 also */
			bi->flags2 &= ~(WLC_BSS2_VHT | WLC_BSS2_SGI_80);
			bi->flags3 &= ~(WLC_BSS3_HE);
		}
	}
} /* wlc_join_BSS_limit_caps */

/**
 * @param[in] cfg->target_bss    Contains IBSS to join
 */
static void
...
#ifdef WL_SAE
static int
wlc_assoc_start_extauth(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct ether_addr *bssid, int auth_alg)
{
	...
	struct scb *scb, uint8 mcsallow)
{
	...
	if (BSSCFG_EXT_AUTH(cfg) && (auth == DOT11_SAE)) {
		WL_ERROR(("wl%d: JOIN: External auth Triggered", WLCWLUNIT(wlc)));
	...
	(void)wlc_assoc_sendauth(wlc, cfg, &scb->ea, &bi->BSSID, scb,
	                   ...
	                   AS_SENT_AUTH_1);
} /* wlc_join_BSS_sendauth */

#ifdef WL11AX
typedef struct {
	he_cap_ie_t **cap_ie;
	he_6g_band_caps_ie_t **ext_cap_ie;
	he_op_ie_t **op_ie;
...
static int
_wlc_parse_he_6g_caps_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	he_6g_band_caps_ie_t **ext_cap_ie = ctx;

	*ext_cap_ie = (he_6g_band_caps_ie_t *)data->ie;
	...
	uint8 *ies, uint ies_len, wlc_parse_assoc_resp_t *parsp)
{
	...
	wlc_iem_tag_t parse_tag[] = {
		DOT11_MNG_HE_CAP_ID,
		DOT11_MNG_HE_6G_BAND_CAPS_ID,
		DOT11_MNG_HE_OP_ID,
	};
	wlc_iem_pe_t parse_cb[] = {
		{_wlc_parse_he_cap_ie, parsp->cap_ie},
		{_wlc_parse_he_6g_caps_ie, parsp->ext_cap_ie},
...
/** called when an auth req is to be sent */
static void
	...
	chanspec_t chanspec, uint8 mcsallow, uint flags)
{
	...
	if (flags & JOIN_BSS_FLAG_VHT_CAP) {
		vht_cap_ie_t *vht_cap_ie_p = NULL; /**< extracted from received beacon */
		...
		int target_bss_band = CHSPEC_BANDTYPE(chanspec);
		...
		if (BAND_2G(target_bss_band) || WLC_VHT_FEATURES_RATES(wlc->pub)) {
			prop_tlv = wlc_vht_read_features_ie(wlc->vhti, parse, len,
					&vht_ratemask, &prop_tlv_len, target_bss);
		}

		if  (prop_tlv) {
			vht_cap_ie_p = wlc_vht_copy_cap_ie(wlc->vhti, prop_tlv,
					prop_tlv_len, &vht_cap_ie);
			vht_op_ie_p = wlc_vht_copy_op_ie(wlc->vhti, prop_tlv,
		...
		wlc_vht_update_scb_state(wlc->vhti, target_bss_band, scb,
				vht_cap_ie_p, vht_op_ie_p, vht_ratemask);
	} else if (SCB_VHT_CAP(scb) &&
	           ((target_bss->flags2 & WLC_BSS2_VHT) != WLC_BSS2_VHT)) {
			wlc_vht_update_scb_state(wlc->vhti,
				CHSPEC_BANDTYPE(chanspec), scb, NULL, NULL, 0);
	}
#endif /* WL11AC */
	...
	if (flags & JOIN_BSS_FLAG_HE_CAP) {
		...
		uint len = target_bss->bcn_prb_len - sizeof(struct dot11_bcn_prb);
		uint8 *parse = (uint8*)target_bss->bcn_prb + sizeof(struct dot11_bcn_prb);
		wlc_parse_assoc_resp_t parsp = {&he_cap_ie, &he_6g_caps_ie, &he_op_ie};
		...
		wlc_he_update_scb_state(wlc->hei, CHSPEC_BANDTYPE(chanspec), scb,
			he_cap_ie, he_op_ie, he_6g_caps_ie);
...
wlc_join_bss(wlc_bsscfg_t *cfg)
{
	...
	if (WLC_ASSOC_IS_PROHIBITED_CHANSPEC(wlc, chanspec) == TRUE) {
		...
		wlc_set_countrycode(wlc->cmi, country_abbrev, 0);
	...
	req.flex.bf.min_dur = MSCH_MIN_ONCHAN_TIME;
	req.interval = 0;
...
wlc_assoc_chanspec_change_cb(void* handler_ctxt, wlc_msch_cb_info_t *cb_info)
{
	...
	chanspec_t curbss_chspec = cfg->current_bss->chanspec;
	wlc_info_t *wlc = cfg->wlc;
	wlc_bss_info_t *bi;
	wlc_assoc_t *as = cfg->assoc;
	...
	WL_ASSOC(("%s chspec 0x%x curbss_chspec 0x%x type %d type_specific %p\n", __FUNCTION__,
		chanspec, curbss_chspec, cb_info->type, OSL_OBFUSCATE_BUF(cb_info->type_specific)));

	if (cb_info->type & MSCH_CT_ON_CHAN) {
		wlc_clr_quiet_chanspec(wlc->cmi, chanspec);
		/* after setting new chanspec, Mark older/current bss chanspec as passive */
		if (WL11H_ENAB(wlc) && wlc_radar_chanspec(wlc->cmi, curbss_chspec) &&
				!wlc_quiet_chanspec(wlc->cmi, curbss_chspec)) {
			wlc_set_quiet_chanspec(wlc->cmi, curbss_chspec);
		}
		wlc_mute(wlc, OFF, 0);
		wlc_txqueue_start(wlc, cfg, chanspec);
...
/**
 * Channel has been switched to the target channel, next step is to transmit an auth-req
 * This function will populate cfg->target_bss with the BSS to join
 *
 * @param[in] bi       Information on the BSS that will be joined
 */
static void
           ...
           chanspec_t chanspec)
{
	...
	if (target_bss->flags & WLC_BSS_WME) {
		...
		flags |= ((target_bss->flags2 & WLC_BSS2_VHT) && BSS_VHT_ENAB(wlc, cfg)) ?
...
wlc_assoc_timeout(void *arg)
{
	...
	*/
	...
	/* It is Timeout !!! Unregister the msch timeslot */
	if (as->req_msch_hdl) {
		if (!wlc_sta_timeslot_onchannel(cfg) && wlc_txqueue_active(wlc, cfg)) {
			wlc_txqueue_end(wlc, cfg);
		}
	...
	if (ASSOC_RECREATE_ENAB(wlc->pub) &&
		...
		as->bss_retries++;
	} else if (as->state == AS_IDLE && wlc->as->sta_retry_time &&
...
wlc_assoc_recreate_timeout(wlc_bsscfg_t *cfg)
{
	...
	if (WOWL_ENAB(wlc->pub) && BSSCFG_IS_PRIMARY(cfg))
...
wlc_join_recreate(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	...
	if (bsscfg->BSS) {
		scb = wlc_scbfindband(wlc, bsscfg, &bi->BSSID, CHSPEC_BANDUNIT(bi->chanspec));
	...
	if (bsscfg->BSS && scb != NULL) {
		...
		WL_ASSOC(("wl%d: JOIN: scb     Band:%s Flags:0x%x Cfg %p\n",
		          WLCWLUNIT(wlc),
		          wlc_bandunit_name(scb->bandunit),
...
wlc_assoc_abort(wlc_bsscfg_t *cfg)
{
	...
	WL_ASSOC(("wlc_assoc_abort state=%d type=%d\n", as->state, as->type));
...
wlc_assoc_change_state(wlc_bsscfg_t *cfg, uint newstate)
{
	...
	if ((oldstate == AS_SCAN) && (newstate == AS_IDLE) && SCAN_IN_PROGRESS(wlc->scan)) {
	 ...
	 if (PWRSTATS_ENAB(wlc->pub)) {
		...
		if (BSSCFG_IS_PRIMARY(cfg) && cfg->BSS &&
	...
	if ((newstate == AS_DFS_CAC_START) || (newstate == AS_DFS_ISM_INIT)) {
		if ((!wlc_is_ap_interface_up(wlc)) && WL11H_STA_ENAB(wlc) &&
			...
			bool to_radar = FALSE;
			wlc_bss_info_t *bi;
			bi = wlc->as->join_targets->ptrs[wlc->as->join_targets_last];
			if (bi != NULL) {
				from_radar = (wlc_radar_chanspec(wlc->cmi,
					cfg->current_bss->chanspec) == TRUE);
				to_radar = wlc_radar_chanspec(wlc->cmi, bi->chanspec);
...
wlc_sradar_ap_select_channel(wlc_info_t *wlc, chanspec_t curr_chanspec)
{
	...
	if ((list = MALLOC(wlc->osh, listlen)) == NULL) {
		...
		wlc_get_valid_chanspecs(wlc->cmi, list,
			curr_chanspec ? CHSPEC_BANDUNIT(curr_chanspec) : BAND_5G_INDEX,
			curr_chanspec ? wlc_chspec_bw2bwcap_bit(CHSPEC_BW(curr_chanspec)) :
				WLC_BW_40MHZ_BIT, abbrev);
	}
...
wlc_sta_assoc_upd(wlc_bsscfg_t *cfg, bool state)
{
	...
	if (BSSCFG_STA(cfg) && APSTA_ENAB(wlc->pub) && (state == FALSE)) {
		WL_INFORM(("wl%d: upstream connection lost, check if downstream AP should be"
			" disabled or not\n", wlc->pub->unit));
		/* check whether downstream AP should be disabled or not */
	...
	WL_ASSOC(("wl%d:%s chanspec 0x%x num_160mhz_assocs %d, any_sta_in_160mhz: %d\n",
		wlc->pub->unit,	__FUNCTION__, wlc->chanspec, wlc->num_160mhz_assocs,
...
/**
 * This function is called for a STA that either joins a BSS or adopts an IBSS.
 * cfg->target_bss points at the bss to adopt.
 */
static void
wlc_join_adopt_bss(wlc_bsscfg_t *cfg)
{
	...
	if (target_bss->bss_type == DOT11_BSSTYPE_INFRASTRUCTURE) {
		WL_ASSOC(("wl%d.%d: JOIN: join BSS \"%s\" on chanspec %s\n",
			WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg),
			ssidbuf, wf_chspec_ntoa_ex(target_bss->chanspec, chanbuf)));
	} else {
		WL_ASSOC(("wl%d.%d: JOIN: join IBSS \"%s\" on chanspec %s\n",
			WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg),
			ssidbuf, wf_chspec_ntoa_ex(target_bss->chanspec, chanbuf)));
	}
	...
	if (as->type == AS_ASSOCIATION) {
		roam->reason = WLC_E_REASON_INITIAL_ASSOC;
		WL_ASSOC(("wl%d.%d: ROAM: roam_reason cleared to 0x%x\n",
			WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), WLC_E_REASON_INITIAL_ASSOC));
	...
	WL_ASSOC(("wl%d.%d: ROAM: initial rssi %d\n",
		WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), roam->prev_rssi));
	WL_SRSCAN(("wl%d.%d: ROAM: initial rssi %d\n",
		WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), roam->prev_rssi));
	...
	WL_APSTA_UPDN(("wl%d.%d: Reporting link up on config 0 for STA joining %s\n",
		WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), cfg->BSS ? "a BSS" : "an IBSS"));
	...
	if (target_bss->bss_type == DOT11_BSSTYPE_INFRASTRUCTURE) {
		...
		WL_ASSOC(("wl%d.%d: JOIN: BSS case...waiting for the next beacon...\n",
			WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg)));
		wlc_assoc_change_state(cfg, AS_WAIT_RCV_BCN);
		wlc_bss_assoc_state_notif(wlc, cfg, as->type, AS_WAIT_RCV_BCN);
	...
	wlc_rateset_t *rateset)
{
	uint i;
	uint8 rate;
	int band = CHSPEC_BANDTYPE(bi->chanspec);
...
/**
 * Return true if we support one or more rates in the target's OperationRateset 'rateset'. Does not
 * modify any of the caller supplied arguments.
 *
 * @param[in] wlc
 * @param[in] cfg
 * @param[in] bi
	...
	wlc_rateset_t *rateset)
{
	 ...
	 */
	wlc_rateset_ht_bw_mcs_filter(&dst,
	...
	wlc_bss_info_t *bi, wlc_rateset_t *rateset)
{
	...
	return TRUE;
} /* wlc_join_verify_filter_membership */
	...
	struct dot11_bcn_prb *bcn, int bcn_len)
{
	...
	if (cfg->BSS) {
		...
		tim = bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_TIM_ID);
...
/** cfg->target_bss contains the bss that this function will adopt */
void
	...
	struct dot11_bcn_prb *bcn, int bcn_len)
{
	...
	if (target_bss->bss_type == DOT11_BSSTYPE_INFRASTRUCTURE) {
		...
		wlc_join_adopt_ibss_params(wlc, cfg); /* writes into e.g. current_bss rateset */
	}
...
wlc_auth_tx_complete(wlc_info_t *wlc, uint txstatus, void *arg)
{
	...
	/* call from _wlc_pcb_pktfree_fn_invoke */
	if (!txstatus) {
		WL_ERROR(("%s, txstatus = 0!!", __FUNCTION__));
		return;
	}
	...
	wl_add_timer(wlc->wl, as->timer, BSSCFG_AUTH_TIMEOUT(cfg), 0);
}
...
void
wlc_process_assocresp_decision(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, assoc_decision_t *dc)
{
	wlc_assocresp_client_next(wlc, bsscfg, dc, NULL);
}

static void
wlc_assocresp_client_next(wlc_info_t *wlc, wlc_bsscfg_t *cfg,
	assoc_decision_t * dc, struct scb *scb)
{
	...
	if ((MBSS_ENAB(wlc->pub) || PSTA_ENAB(wlc->pub) || !BSSCFG_IS_PRIMARY(cfg)) &&
		...
		key = wlc_keymgmt_get_bss_tx_key(wlc->keymgmt,
			(BSSCFG_PSTA(cfg) ? wlc->primary_bsscfg : cfg), FALSE, &key_info);
#else /* PSTA */
	...
	struct dot11_management_header *hdr, uint8 *body, uint body_len)
{
	...
	ftpparm.assocresp.assoc_oui = &assoc_oui;
	...
	/* Association success */
	cfg->AID = ltoh16(assoc->aid) & DOT11_AID_MASK;

	/* UL OFDMA FIFOs will be allocated if all these are true:
	 * 1. Station is capable of supporting UL OFDMA.
	 * 2. Station has AP interface. Meaning, it is a repeater.
	 * 3. Associated AP is capable of supporting UL OFDMA clients.
	 */
	...
	if (SCB_HE_CAP(scb) && HE_ULMU_STA_ENAB(wlc->pub) && ap_interface) {
		wlc_ulmu_alloc_fifo(wlc, scb);
	}
...
#ifdef MFP
			(WLC_MFP_ENAB(wlc->pub) && SCB_MFP(scb)) ||
#endif /* MFP */
			FALSE) {
#ifdef WLAMPDU
			if (SCB_AMPDU(scb)) {
...
#endif /* WLAMPDU */
#ifdef WLAMSDU
			wlc_amsdu_tx_scb_disable(wlc, scb);
#endif /* WLAMSDU */
		} else {
#ifdef WLAMSDU
	...
	uint8 *body, uint body_len, bool short_preamble)
{
	...
	if (auth_status != DOT11_SC_SUCCESS && auth_status != DOT11_SC_SAE_HASH_TO_ELEMENT) {
#ifdef WLFBT
		...
		if (!(BSSCFG_EXT_AUTH(cfg) &&
			...
			(auth_status == DOT11_SC_SAE_HASH_TO_ELEMENT)))) {
...
#if defined(WL_SAE)
	if (BSSCFG_EXT_AUTH(cfg) && (auth_alg == DOT11_SAE)) {
...
wlc_clear_hw_association(wlc_bsscfg_t *cfg, bool mute_mode)
{
	...
	if (!DEVICEREMOVED(wlc)) {
		...
		if (mute_mode == ON &&
		    wlc_quiet_chanspec(wlc->cmi, phy_utils_get_chanspec(WLC_PI(wlc))))
			wlc_mute(wlc, ON, 0);
...
_wlc_join_start_ibss(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int bss_type)
{
	...
	*/
	 ...
	 */
#if BAND5G /* RADAR */
	if (phy_utils_get_chanspec(WLC_PI(wlc)) != bi.chanspec) {
	...
	wlc_rateset_ht_bw_mcs_filter(&bi.rateset,
	...
	phy_chanmgr_create_ctx(WLC_PI(wlc), bi.chanspec);
#endif // endif
...
wlc_join_start_ibss(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int8 bss_type)
{
	...
	(void)wlc_join_start_prep(wlc, cfg); /* populates cfg->target_bss */
...
_wlc_disassociate_client(wlc_bsscfg_t *cfg, bool send_disassociate, pkcb_fn_t fn, void *arg)
{
	...
	wlc_bss_info_t *current_bss = cfg->current_bss;
	wlc_bss_info_t *target_bss = cfg->target_bss;
	chanspec_t bss_chspec = current_bss->chanspec;
	bool mute_mode = ON;
#if defined(BCMDBG) || defined(WLMSG_ASSOC)
	char chanbuf[CHANSPEC_STR_LEN];
#endif // endif
	...
	if (cfg->BSS) {
		...
		if (send_disassociate) {
			if (ETHER_ISNULLADDR(cfg->BSSID.octet)) {
				...
				WL_ASSOC(("wl%d: JOIN: skipping DISASSOC to %s since we lost "
					    "contact.\n", WLCWLUNIT(wlc), eabuf));
			} else if ((wlc_radar_chanspec(wlc->cmi, bss_chspec) &&
				wlc_quiet_chanspec(wlc->cmi, bss_chspec)) ||
			        wlc_restricted_chanspec(wlc->cmi, bss_chspec)) {
				...
				if (fn != NULL) {
					...
					WL_ASSOC(("wl%d: JOIN: skipping DISASSOC to %s"
						...
						WLCWLUNIT(wlc), eabuf,
						wf_chspec_ntoa_ex(bss_chspec, chanbuf)));

				} else if (!wlc_quiet_chanspec(wlc->cmi, bss_chspec) &&
					(bss_chspec == phy_utils_get_chanspec(WLC_PI(wlc)))) {
			...
			} else if (bss_chspec == phy_utils_get_chanspec(WLC_PI(wlc))) {
				WL_ASSOC(("wl%d: JOIN: sending DISASSOC to %s\n",
				    WLCWLUNIT(wlc), eabuf));
	...
	if (!AP_ACTIVE(wlc) && BSSCFG_IS_PRIMARY(cfg)) {
	...
	if (cfg->BSS) {
		wlc_lq_rssi_snr_noise_bss_sta_ma_reset(wlc, cfg, CHSPEC_BANDUNIT(bss_chspec),
...
#endif /* WL_FILS */
		{
			if (BSSCFG_EXT_AUTH(cfg) && (auth_type == DOT11_SAE))
				return;
			wlc_assoc_continue_post_auth1(cfg, scb);
		}
		goto do_event;
	...
	uint assoc_status, bool reassoc, uint bss_type)
{
	...
	if (status == WLC_E_STATUS_SUCCESS) {
		...
		if ((!wlc_is_ap_interface_up(wlc)) && WL11H_STA_ENAB(wlc) &&
			...
			if (!wlc_radar_chanspec(wlc->cmi, phy_utils_get_chanspec(WLC_PI(wlc)))) {
				cfg->pm->pm_modechangedisabled = FALSE;
				wlc_set_pm_mode(wlc, cfg->pm->pm_oldvalue, cfg);
				wlc->mpc = TRUE;
				wlc_radio_mpc_upd(wlc);
		...
		if (AP_ACTIVE(wlc)) {
			/* performing a channel change, all up ap scb's need to be cleaned up */
			mchan_stago_disab = TRUE;
#ifdef WLMCHAN
			mchan_stago_disab = !MCHAN_ENAB(wlc->pub) ||
				wlc_mchan_stago_is_disabled(wlc->mchan);
			if (!MCHAN_ENAB(wlc->pub))
#endif // endif
				FOREACH_UP_AP(wlc, idx, apcfg) {
					/* Clean up scbs only when there is a chanspec change */
					if (phy_utils_get_chanspec(WLC_PI(wlc)) !=
		...
		if (BSSCFG_IS_FBT(cfg) && SUP_ENAB(wlc->pub) &&
			(cfg->WPA_auth & (WPA2_AUTH_FT | WPA3_AUTH_SAE_FBT)) && reassoc)
	...
	if (status != WLC_E_STATUS_SUCCESS) {
		...
		*/
		...
		if ((status == WLC_E_STATUS_NO_NETWORKS ||
			...
			} else if ((wlc->as->sta_retry_time > 0) &&
...
wlc_roamscan_start(wlc_bsscfg_t *cfg, uint roam_reason)
{
	...
	/* Original roaming */
		...
		if (roam_metric < wlc->band->roam_trigger) {
			if (roam->scan_block || WLC_ROAM_DISABLED(roam)) {
				err = BCME_EPERM;
				WL_ASSOC(("ROAM: roam_metric=%d; block roam scan request(%u,%d)\n",
				          roam_metric, roam->scan_block, roam->state));
			} else {
...
wlc_roamscan_complete(wlc_bsscfg_t *cfg)
{
	...
	if (roam->fullscan_count == 1) {
		WL_ASSOC(("wl%d.%d: %s: Building roam candidate cache from driver roam scans\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__));
		wlc_build_roam_cache(cfg, wlc->as->join_targets);
	}
...
wlc_build_roam_cache(wlc_bsscfg_t *cfg, wlc_bss_list_t *candidates)
{
	...
	if ((roam->roam_type == ROAM_FULL) || (roam->split_roam_phase == 1)) {
		...
		if (cfg->associated && !ETHER_ISNULLADDR(&cfg->BSSID)) {
			...
			chanspec = wf_ctlchspec20_from_chspec(cfg->current_bss->chanspec);
	...
	for (i = candidates->count - 1; (i >= 0) &&
		...
		chanspec = wf_ctlchspec20_from_chspec(candidates->ptrs[i]->chanspec);
	...
	if (roam->ap_environment != AP_ENV_DETECT_NOT_USED) {
		if (nentries > 1 && roam->ap_environment != AP_ENV_DENSE) {
			...
			FOREACH_WLC_BAND(wlc, bandunit) {
				...
				if (band->roam_trigger == WLC_AUTO_ROAM_TRIGGER -
				    WLC_ROAM_TRIGGER_STEP) {
					band->roam_trigger = WLC_AUTO_ROAM_TRIGGER;
					WL_ASSOC(("wl%d: %s: Setting roam trigger on bandunit %u "
						  "to %d\n", wlc->pub->unit, __FUNCTION__,
						  band->bandunit, WLC_AUTO_ROAM_TRIGGER));
#ifdef WLABT
					if (WLABT_ENAB(wlc->pub) && !IS_SINGLEBAND(wlc) &&
						roam->prev_bcn_timeout != 0 && band == this_band) {
...
#endif /* WLABT */
				} else if (band->roam_trigger != WLC_AUTO_ROAM_TRIGGER) {
			...
			FOREACH_WLC_BAND(wlc, bandunit) {
				band = wlc->bandstate[bandunit];
				if (band->roam_trigger == WLC_AUTO_ROAM_TRIGGER) {
					band->roam_trigger -= WLC_ROAM_TRIGGER_STEP;
					WL_ASSOC(("wl%d: %s: Setting roam trigger on bandunit %u "
						  "to %d\n", wlc->pub->unit, __FUNCTION__,
						  band->bandunit, band->roam_trigger));
#ifdef WLABT
					if (WLABT_ENAB(wlc->pub) && !IS_SINGLEBAND(wlc) &&
						!P2P_ACTIVE(wlc) && band == this_band) {
...
wlc_roam_motion_detect(wlc_bsscfg_t *cfg)
{
	...
	if (!roam->motion &&
		...
		FOREACH_WLC_BAND(wlc, bandunit) {
			band = wlc->bandstate[bandunit];
			band->roam_trigger += MOTION_DETECT_TRIG_MOD;
			band->roam_delta -= MOTION_DETECT_DELTA_MOD;
		}
	...
	if (BSSCFG_STA(cfg) &&
		...
		for (i = 0; i < wlc->scan_results->count; i++) {
			if (WLC_IS_CURRENT_SSID(cfg, (char*)bip[i]->SSID, bip[i]->SSID_len) &&
			    join_pref->band == CHSPEC_BANDTYPE(bip[i]->chanspec)) {
...
#ifdef WLFBT
			if (BSSCFG_IS_FBT(cfg) &&
					(cfg->WPA_auth & (WPA2_AUTH_FT | WPA3_AUTH_SAE_FBT))) {
...
wlc_assoc_keying_in_progress(wlc_info_t *wlc)
{
	...
	FOREACH_BSS(wlc, idx, cfg) {
		...
		if (!ETHER_ISNULLADDR(&cfg->BSSID) && !wlc_portopen(cfg)) {
			scbband = CHSPEC_BANDUNIT(cfg->current_bss->chanspec);
			if ((scb = wlc_scbfindband(wlc, cfg, &cfg->BSSID, scbband)) != NULL) {
				uint32 time, tmo = 0;
...
wlc_mac_request_assoc(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int req)
{
	...
	if (AS_IN_PROGRESS(wlc)) {
		wlc_bsscfg_t *bc = wlc->as->assoc_req[0];
	...
	uint *roam_delta)
{
	...
	if (band->radioid == NORADIO_ID) {
		...
		*roam_trigger = BAND_5G6G(band->bandtype) ? WLC_5G_ROAM_TRIGGER :
		        WLC_2G_ROAM_TRIGGER;
		*roam_delta = BAND_5G6G(band->bandtype) ? WLC_5G_ROAM_DELTA :
		        WLC_2G_ROAM_DELTA;
		WL_INFORM(("wl%d: wlc_roam_defaults: USE GENERIC ROAM THRESHOLD "
		           ...
		           *roam_trigger, *roam_delta, band->radioid,
		           wlc_bandunit_name(band->bandunit)));
	}
	...
	if (band->roam_prof) {
		...
		if (WBTEXT_ENAB(wlc->pub)) {
			...
			if (WL_MAX_ROAM_PROF_BRACKETS > 1) {
				...
				band->roam_prof[0].roam_trigger = BAND_5G6G(band->bandtype) ?
	...
	const struct ether_addr *addr, uint deauth_reason, uint bss_type)
{
	...
	if (DOT11_RC_INACTIVITY == deauth_reason) {
		scb = wlc_scbfind(wlc, bsscfg, addr);
		rssi.rssi = wlc_lq_rssi_last_get(wlc, bsscfg, scb);
...
{
	bool hwrs_scb = scb == wlc->band->hwrs_scb;
	ASSERT(scb);

	if (!hwrs_scb && scb->sent_deauth) {
		WL_ERROR(("wl%d: %s: already sent deauth for scb "MACF"\n",
			wlc->pub->unit, __FUNCTION__, ETHERP_TO_MACF(da)));
		return;
	}

	WL_ASSOC(("wl%d.%d: %s: send deauth for "MACF" with reason %d\n", wlc->pub->unit,
		WLC_BSSCFG_IDX(cfg), __FUNCTION__, ETHERP_TO_MACF(da), reason_code));

	wlc_senddeauth_ex(wlc, cfg, scb, da, &cfg->BSSID, &cfg->cur_etheraddr,
		reason_code, FALSE, !hwrs_scb);
}
...
wlc_deauth_sendcomplete(wlc_info_t *wlc, uint txstatus, void *arg)
{
	...
	if ((txstatus & TX_STATUS_MASK) == TX_STATUS_NO_ACK) {
		WL_ASSOC(("wl%d.%d: %s: no ACK from "MACF" for Deauth (txs suppr: 0x%x)\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
			__FUNCTION__, ETHER_TO_MACF(scb->ea),
			((txstatus & TX_STATUS_SUPR_MASK) >> TX_STATUS_SUPR_SHIFT)));
	} else {
		WL_ASSOC(("wl%d.%d: %s: deauth complete for scb "MACF"\n", wlc->pub->unit,
			WLC_BSSCFG_IDX(bsscfg), __FUNCTION__, ETHERP_TO_MACF(&scb->ea)));
		/* Let cleanup happen in wlc_ap_scb_unusable_stas_timeout. */
		acked = TRUE;
	}
	...
	if (BSSCFG_STA(bsscfg)) {
...
#endif /* WL_HAPD_WDS */
		{
			 ...
			 * from the inactivity timeout context in
			 * wlc_ap_scb_unusable_stas_timeout(). Scb state change
			 * will happen from wlc_ap_scb_unusable_stas_timeout()
			 */
			...
			SCB_MARK_DEL(scb);
			if (acked) {
				scb->sent_deauth->deauth_acked = TRUE;
			} else {
				scb->sent_deauth = NULL;
				MFREE(wlc->osh, arg, sizeof(wlc_deauth_send_cbargs_t));
			}
...
static void
wlc_roam_period_update(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	if (wlc->band->roam_prof) {
		wlc_roam_t *roam = cfg->roam;
		int idx = roam->roam_prof_idx;
...
bool
wlc_roam_scan_islazy(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool roam_scan_isactive)
{
	wlcband_t *band = wlc->band;

	if (!IS_SINGLEBAND(wlc)) {
		band = wlc->bandstate[CHSPEC_BANDUNIT(cfg->current_bss->chanspec)];
	}
...
bool
wlc_lazy_roam_scan_suspend(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	wlcband_t *band = wlc->band;
...
/** Assume already checked that this is background roam scan */
bool
wlc_lazy_roam_scan_sync_dtim(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	wlcband_t *band = wlc->band;
...
static void
wlc_roam_prof_update(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool reset)
{
	wlc_roam_t *roam = cfg->roam;
	wlcband_t *band = wlc->band;
	...
	if (wlc->band->roam_prof) {
		 ...
		 */
		if (!IS_SINGLEBAND(wlc) && reset) {
			enum wlc_bandunit bandunit;
			wlcband_t *otherband;
			FOREACH_WLC_BAND(wlc, bandunit) {
				otherband = wlc->bandstate[bandunit];
				if (otherband == band) {
					continue; // only act on non-current band(s)
				}
...
static void
wlc_roam_prof_update_default(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	wlc_roam_t *roam = cfg->roam;
	wlcband_t *band;
	enum wlc_bandunit bandunit;
...
wlc_handle_ap_lost(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_STA(cfg) && APSTA_ENAB(wlc->pub)) {
		wlc_sta_assoc_upd(cfg, FALSE);
...

{
	...
	if (ap_scb) {
		...
		WL_ASSOC_ERROR(("bcn_loss:\tNow: %d AP RSSI: %d BI: %d\n",
			wlc->pub->now, wlc->primary_bsscfg->link->rssi,
			bss->beacon_period));
...
wlc_assoc_chanspec_sanitize(wlc_info_t *wlc, chanspec_list_t *list, int len, wlc_bsscfg_t *cfg)
{
	...
	for (i = 0; i < (int)chanspec_num; i++) {
		...
		if (!wlc_valid_chanspec_db(wlc->cmi, chanspec)) {
			/* Country Default : if channel is not supported by phy, reject the channel
			*/
...
wlc_roamprof_is_cu_enabled(wlc_info_t *wlc, int bandtype, int16 idx)
{
	...
	ASSERT((idx >= 0) && (idx < WL_MAX_ROAM_PROF_BRACKETS));
	ASSERT(bandtype == WLC_BAND_2G || bandtype == WLC_BAND_5G || bandtype == WLC_BAND_6G);
	band = wlc->bandstate[wlc_bandtype2bandunit(bandtype)];
	...
	FOREACH_AS_STA(wlc, idx, cfg) {
		if (cfg->up) {
			...
			if (update_tsf_cfprep) {
				...
				bi = wlc->primary_bsscfg->current_bss->beacon_period << 10;
...
static const bcm_iovar_t assoc_iovars[] = {
...
#ifdef WLSCANCACHE
	{"assoc_cache_assist", IOV_ASSOC_CACHE_ASSIST, 0, 0, IOVT_BOOL, 0},
#endif // endif
	{"IBSS_join_only", IOV_IBSS_JOIN_ONLY, IOVF_OPEN_ALLOW, 0, IOVT_BOOL, 0},
	...
	{"ibss_coalesce_allowed", IOV_IBSS_COALESCE_ALLOWED, (IOVF_OPEN_ALLOW), 0,
	IOVT_BOOL, 0},
	{"assoc_listen", IOV_ASSOC_LISTEN, 0, 0, IOVT_UINT16, 0},
#ifdef WL_ASSOC_RECREATE
	{"assoc_recreate", IOV_ASSOC_RECREATE, IOVF_OPEN_ALLOW, 0, IOVT_BOOL, 0},
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_STA_RETRY_TIME):
		*ret_int_ptr = (int32)asi->sta_retry_time;
		break;
	...
	case IOV_SVAL(IOV_STA_RETRY_TIME):
		...
		asi->sta_retry_time = (uint32)int_val;
		break;
	...
	case IOV_SVAL(IOV_ROAM_OFF):
		roam->state = (uint8)int_val;
		break;

	case IOV_GVAL(IOV_ROAM_OFF):
		*ret_int_ptr = (int32)roam->state;
		break;
	...
	case IOV_GVAL(IOV_ROAM_CHANNELS_IN_HOTLIST): { /* returns a list of chanspecs */
		wl_uint32_list_t *list = (wl_uint32_list_t *)arg;
		uint i, j;
		chanvec_t *chanvec; /**< bitvec of either 2G+5G or of 6G channels */
		chanspec_t band;
		...
		list->count = 0;
		for (j = 0 ; j < 2 ; j++) { /* first iterate over 2g+5g, then over 6g channels */
			if (j == 0) {
				chanvec = &roam->roam_chn_cache.vec2g5g;
				band = WL_CHANSPEC_BAND_2G;
			} else {
				chanvec = &roam->roam_chn_cache.vec6g;
				band = WL_CHANSPEC_BAND_6G;
			}

			for (i = 0; i < sizeof(chanvec_t); i++) {
				uint8 chn_list = chanvec->vec[i];
				uint32 ch_idx = i << 3;
				while (chn_list) {
					if (band == WL_CHANSPEC_BAND_2G &&
					    ch_idx >= CH_MIN_5G_CHANNEL) {
						band = WL_CHANSPEC_BAND_5G;
					}
					if (chn_list & 1) {
						if (list->count >= WL_NUMCHANSPECS) {
							err = BCME_BUFTOOSHORT;
							break;
						}
						list->element[list->count++] =
							(WL_CHANSPEC_BW_20 | band | ch_idx);
					}
					ch_idx++;
					chn_list >>= 1;
				}
	...
	case IOV_GVAL(IOV_ROAM_PROF): /* roam profile */
	{
		enum wlc_bandunit bandunit;
		wlcband_t *band;
		...
		if (!BAND_2G(int_val) && !BAND_5G(int_val) && !BAND_6G(int_val)) {
			err = BCME_BADBAND;
			break;
		}

		bandunit = wlc_bandtype2bandunit(int_val);
		band = wlc->bandstate[bandunit];
	...
	case IOV_SVAL(IOV_ROAM_PROF):  /* roam profile */
	{
		int i, np;
		enum wlc_bandunit bandunit;
		wlcband_t *band;
...
wlc_assoc_doioctl(void *ctx, uint32 cmd, void *arg, uint len, struct wlc_if *wlcif)
{
		...
		if (!wlc->clk) {
			...
			*/
			if ((uint)len >= ETHER_ADDR_LEN) {
				...
				/* Handling the WLC_DISASSOC with scb_val_t argument */
				if ((uint)len == sizeof(scb_val_t)) {
					scb_val_t *scbval = (scb_val_t *)arg;
					bcopy(&scbval->ea, (void *)&sta_mac, ETHER_ADDR_LEN);
				} else {
	...
	case WLC_REASSOC: {
		...
		if ((bcmerror == BCME_BUSY) && (BSSCFG_STA(bsscfg)) && BSSCFG_IS_PRIMARY(bsscfg)) {
	...
	case WLC_GET_ROAM_TRIGGER:
		/* bcmerror checking */
		if ((bcmerror = wlc_iocbandchk(wlc, (int*)arg, len, &bandtype, FALSE)))
			break;
		...
		band = get_band_by_bandtype(wlc, (int)bandtype);
		if (band != NULL) {
			*pval = band->roam_trigger;
		}
		break;
	...
	case WLC_SET_ROAM_TRIGGER: {
		...
		if (bandtype == WLC_BAND_ALL) {
			FOREACH_WLC_BAND(wlc, bandunit) {
				band = wlc->bandstate[bandunit];
				trigger_dbm = (val < 0) ? val :
						wlc_roam_trigger_logical_dbm(wlc, band, val);
				band->roam_trigger_def = trigger_dbm;
				band->roam_trigger = trigger_dbm;
			}
		} else {
			band = get_band_by_bandtype(wlc, (int)bandtype);
			if (band != NULL) {
	...
	case WLC_GET_ROAM_DELTA:
		...
		/* Return value for specified band */
		band = get_band_by_bandtype(wlc, (int)bandtype);
		if (band != NULL) {
			*pval = band->roam_delta;
		}
		break;
...
{
	wlc_assoc_info_t *asi = wlc->as;
	if (asi->join_targets_last && ((int)asi->join_targets_last + (--offset) >= 0) &&
	    ((int)asi->join_targets_last + offset < (int)asi->join_targets->count)) {
		return asi->join_targets->ptrs[asi->join_targets_last + offset];
	}
...
BCMATTACHFN(wlc_assoc_attach)(wlc_info_t *wlc)
{
...
#ifdef STA
	if ((asi->join_targets = MALLOCZ(wlc->osh, sizeof(wlc_bss_list_t))) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		goto fail;
	}
	...
	/* wlc_scb_bss_state_upd needs to be registered after wlc_assoc_bss_state_upd */
	if (wlc->scbstate) {
		if (wlc_bsscfg_state_upd_register(wlc, wlc_scb_bss_state_upd,
			wlc->scbstate) != BCME_OK) {
			WL_ERROR(("wl%d: %s: wlc_bsscfg_state_upd_register failed\n",
				wlc->pub->unit, __FUNCTION__));
			goto fail;
		}
...
#ifdef STA
	if (asi->join_targets != NULL) {
		wlc_bss_list_free(wlc, asi->join_targets);
		MFREE(wlc->osh, asi->join_targets, sizeof(wlc_bss_list_t));
	}
#endif // endif
...
#ifdef STA
void
wlc_roam_update_time_since_bcn(wlc_bsscfg_t *bsscfg, uint8 new_time)
{
	if (bsscfg->roam == NULL) {
...
wlc_assoc_do_dfs_reentry_cac_if_required(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	if ((wlc_radar_chanspec(wlc->cmi, bi->chanspec)) &&
		...
		wlc_enable_mac(wlc);
		phy_radar_detect_enable(WLC_PI(wlc), TRUE);
		wlc_assoc_timer_del(wlc, cfg);
		wl_add_timer(wlc->wl, as->timer,
			(wlc_dfs_get_cactime_ms(wlc->dfs) + DELAY_10MS), FALSE);
		wlc_assoc_change_state(cfg, AS_DFS_CAC_START);
		ret = wlc_set_dfs_cacstate(wlc->dfs, ON, wlc->primary_bsscfg);
		if (ret == BCME_OK) {
...
wlc_assoc_disable_apcfg_on_repeater(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (!APSTA_ENAB(wlc->pub)) {
		WL_INFORM(("wl%d: Radio is not in APSTA mode, invalid request to"
			" disable downstream AP.\n", wlc->pub->unit));
		return;
	}

	primary_cfg = wlc->primary_bsscfg;
	if (!(BSSCFG_STA(primary_cfg) || DWDS_ENAB(primary_cfg) || MAP_ENAB(primary_cfg) ||
#if defined(WET) || defined(WET_DONGLE)
		(WET_ENAB(wlc) || WET_DONGLE_ENAB(wlc)) ||
#endif /* WET || WET_DONGLE */
		primary_cfg->_psta)) {

		WL_INFORM(("wl%d: primary cfg is not sta, not disabling downstream AP\n",
			wlc->pub->unit));
		return;
	}

	if (primary_cfg->disable_ap_up) {
		WL_INFORM(("wl%d: Downstream AP of APSTA are already brought down\n",
			wlc->pub->unit));
		return;
	}

	if (primary_cfg->associated) {
		WL_INFORM(("wl%d: sta associated to upstream AP, let downstream AP run\n",
			wlc->pub->unit));

		return;
	}

	/* cases if upstream connection lost:
	 * - Keep_ap_up = 0, disable downstream ap
	 * - Keep_ap_up = 1, enable downstream ap irrespective of home chanspec
	 * - keep_ap_up = 2, also default option at init time.
	 *	Enable downstream ap if home chanspec is Non DFS.
	 *	Reason for avoiding enable ap on DFS home chanspec is to avoid
	 *	overhead or repeatative CAC if sta is scanning for upstream AP.
	 */
	if (wlc->keep_ap_up == WLC_KEEP_AP_UP_ALWAYS) {
		WL_INFORM(("wl%d: keep_ap_up[%d] Not disabling downstream AP\n", wlc->pub->unit,
			wlc->keep_ap_up));

		if ((WL11H_ENAB(wlc) && wlc_radar_chanspec(wlc->cmi, wlc->home_chanspec))) {
			/* connection got lost, treat downstream AP as normal AP and it should
			 * do CAC
			 */
			FOREACH_UP_AP(wlc, i, apcfg) {
				wlc_set_dfs_cacstate(wlc->dfs, ON, apcfg);
				break;
			}
		}
		return;
	...
	if (wlc->keep_ap_up == WLC_KEEP_AP_UP_ON_NON_DFS) {
		if (!wlc_radar_chanspec(wlc->cmi, wlc->home_chanspec)) {
			WL_INFORM(("wl%d: keep_ap_up[%d], Not disabling downstream AP on"
				" Non DFS channel[%x] when upstream connection not present\n",
				wlc->pub->unit, wlc->keep_ap_up, wlc->home_chanspec));
			return;
		}
	}

	WL_ASSOC(("wl%d: disable all downstream ap \n", wlc->pub->unit));
	...
	wlc_set_dfs_cacstate(wlc->dfs, OFF, wlc->primary_bsscfg);
	wlc_disassociate_client(cfg, FALSE);
	if (!WLC_ROAM_DISABLED(cfg->roam)) {
		cfg->roam->active = FALSE;
	}
} /* wlc_assoc_disable_apcfg_on_repeater */
#endif /* STA */
/* ******** WORK IN PROGRESS ******** */
