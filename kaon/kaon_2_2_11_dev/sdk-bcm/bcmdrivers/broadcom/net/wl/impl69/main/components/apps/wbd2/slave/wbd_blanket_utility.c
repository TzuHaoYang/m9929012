 ...
 *
 * $Id: wbd_blanket_utility.c 792603 2020-10-30 08:45:10Z $
 */
	...
	ieee1905_sta_traffic_stats *traffic_stats, ieee1905_vendor_data *out_vndr_tlv)
{
	...
	if (out_vndr_tlv) {
		...
		if (sta->band & (WBD_BAND_LAN_5GL | WBD_BAND_LAN_5GH)) {
			cmd.vndr_metrics.sta_cap |= WBD_TLV_ASSOC_STA_CAPS_BAND_5G;
		}
		if (sta->band & WBD_BAND_LAN_6G) {
			cmd.vndr_metrics.sta_cap |= WBD_TLV_ASSOC_STA_CAPS_BAND_6G;
		}
...
wbd_blanket_set_dfs_chan_clear(char *ifname, ieee1905_chan_pref_rc_map *chan_pref)
{
	...
	band = blanket_opclass_to_band(chan_pref->regclass);
	if (band != WL_CHANSPEC_BAND_5G) {
		WBD_WARNING("[%s]: DFS channel are only in 5G band. Skip channel clear "
			"for regclass [%d] band [%d] # of channels [%d\n",
			ifname, chan_pref->regclass, band, chan_pref->count);
		return;
	}
	...
	for (i = 0; i < chan_pref->count; i++) {
		chanspec = blanket_prepare_chanspec(chan_pref->channel[i],
			chan_pref->regclass, bw, band);
...
wbd_slave_disconnect_all_bstas(i5_dm_interface_type *i5_ifr)
{
	...
	foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type, i5_self_device->interface_list) {
		...
		blanket_deauth_sta(i5_iter_ifr->ifname, (void *)i5_iter_ifr->bssid, 0);
	}
end:
	WBD_EXIT();
}
