...
/* default auto calibration period is 120 seconds - adjustable via IOVAR */
#define WL_OLPC_AUTOCAL_PERIOD 120
#define WL_OLPC_AUTOCAL_ENAB_DEF 0
...
static const bcm_iovar_t olpc_iovars[] = {
	...
	{"olpc_autocal", IOV_OLPC_AUTOCAL, 0, 0, IOVT_BOOL, 0},
	{"olpc_autocal_period", IOV_OLPC_AUTOCAL_PER, 0, 0, IOVT_UINT32, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
struct wlc_olpc_eng_info_t {
	...
	struct wl_timer *autocal_timer;	/* timer for automatic calibration operations */
	uint32 autocal_period;		/* period for autocal timer */
	bool autocal_enabled;		/* 1 = autocal enabled */
	uint32	cal_last_time;		/* time stamp of last calibration */
	uint64	total_olpc_cals;	/* number of OLPC calibrations since "up" */
};
...
static void
wlc_olpc_autocal_timer(void *arg);
...
static void
wlc_olpc_add_timer_dbg(wlc_olpc_eng_info_t *olpc, struct wl_timer *timer, uint to, bool prd,
	const char *fname, int line)
{
	...
	wl_add_timer(olpc->wlc->wl, timer, to, prd);
}

static bool
wlc_olpc_del_timer_dbg(wlc_olpc_eng_info_t *olpc, struct wl_timer *timer, const char *fname,
	int line)
{
	WL_OLPC_DBG(olpc, ("wl%d: %s(%d): wl_del_timer: tsf %u\n",
		olpc->wlc->pub->unit, fname, line,
		R_REG(olpc->wlc->osh, D11_TSFTimerLow(olpc->wlc))));
	return wl_del_timer(olpc->wlc->wl, timer);
}
#define WLC_OLPC_ADD_TIMER(olpc, timer, to, prd) \
	wlc_olpc_add_timer_dbg(olpc, timer, to, prd, __FUNCTION__, __LINE__)
#define WLC_OLPC_DEL_TIMER(olpc, timer) \
	wlc_olpc_del_timer_dbg(olpc, timer, __FUNCTION__, __LINE__)
#else /* DEBUG_OLPC_TIMER */
#define WLC_OLPC_ADD_TIMER(olpc, timer, to, prd) \
	wl_add_timer((olpc)->wlc->wl, (olpc)->timer, (to), (prd))
#define WLC_OLPC_DEL_TIMER(olpc, timer) wl_del_timer((olpc)->wlc->wl, (olpc)->timer)
#endif /* DEBUG_OLPC_TIMER */
#define WLC_OLPC_FREE_TIMER(olpc, timer)	wl_free_timer((olpc)->wlc->wl, (olpc)->timer)
...
static void wlc_olpc_bsscfg_updn(void *ctx, bsscfg_up_down_event_data_t *evt)
{
	...
	/* Need a cal if this is the first VAP going up, primary or otherwise */
	if (evt->up) {
		if (!wlc_olpc_find_other_up_on_chan(olpc->wlc, evt->bsscfg)) {
			WL_OLPC(olpc, ("%s: 1st BSS (%p) up on chan now\n",
				__FUNCTION__, OSL_OBFUSCATE_BUF(evt->bsscfg)));
			/* check for cal in progress */
			if (evt->bsscfg->current_bss) {
				cspec = evt->bsscfg->current_bss->chanspec;
				chan = wlc_olpc_get_chan_ex(wlc, cspec, &err, FALSE);
				if (chan && wlc_olpc_chan_has_active_cal(olpc, chan)) {
					WL_OLPC(olpc,
						("%s: 1st BSS up, terminate cal\n", __FUNCTION__));
					wlc_olpc_chan_terminate_active_cal(wlc, olpc->cur_chan);
				}
			}
			wlc_olpc_eng_hdl_chan_update_ex(olpc->wlc, olpc->npkts, TRUE);
		} else {
			WL_OLPC(olpc, ("%s: BSS (%p) up, ignored (others already up)\n",
				__FUNCTION__, OSL_OBFUSCATE_BUF(evt->bsscfg)));
		}
	} else {
		/* Else if last VAP going down, reset engine */
		WL_OLPC(olpc, ("%s: BSS (%p) down\n",
			__FUNCTION__, OSL_OBFUSCATE_BUF(evt->bsscfg)));
		if (!wlc_olpc_find_other_up_on_chan(olpc->wlc, evt->bsscfg)) {
			WL_OLPC_ENTRY(olpc, ("%s: No BSSs up, freeing chan\n", __FUNCTION__));
			...
			if (evt->bsscfg->current_bss) {
				...
				if (chan) {
					WL_OLPC_ENTRY(olpc,
						("%s: FREEing chan struct\n", __FUNCTION__));
					wlc_olpc_chan_rm(olpc, chan);
					WL_OLPC_DBG(olpc,
						("%s: calling olpc_eng_reset\n", __FUNCTION__));
					wlc_olpc_eng_reset(olpc);
				}
			}
		} else {
			WL_OLPC(olpc, ("%s: BSS (%p) down, ignore (others still up)\n",
				__FUNCTION__, OSL_OBFUSCATE_BUF(evt->bsscfg)));
		}
	}
...
{
	WL_OLPC_ENTRY(wlc->olpc_info, ("Entry:%s\n", __FUNCTION__));
	...
	if (cur_chan) {
		...
		cur_chan->cal_pkts_outstanding = 0;
#if WL_OLPC_IOVARS_ENAB
		cur_chan->cores_cal_pkts_sent = 0;
#endif /* WL_OLPC_IOVARS_ENAB */
	}
	/* delete any timer */
	WLC_OLPC_DEL_TIMER(wlc->olpc_info, cal_timer);
	...
	WL_OLPC_ENTRY(wlc->olpc_info, ("Exit:%s\n", __FUNCTION__));
}
...
{
	int8 sdmmin;
	int min_txpwr_limit = 128; // max txpwr is limited to 128qdbm
	int err = 0;
	...
	while (WL_OLPC_BW_ITER_HAS_NEXT(txbw)) {
		...
		if (txchains > 0) {
			err = ppr_get_vht_mcs_min(txpwr, txbw,
				WL_TX_NSS_1, WL_TX_MODE_NONE, WL_TX_CHAINS_1, &sdmmin);
			if (err == BCME_OK) {
				WL_OLPC_ENTRY(olpc, ("wl%d:%s: nss1 min=%d sdmmin=%d\n",
		...
		if (txchains > 1) {
			...
			err = ppr_get_vht_mcs_min(txpwr, txbw,
				WL_TX_NSS_1, WL_TX_MODE_CDD, WL_TX_CHAINS_2, &sdmmin);

			if (err == BCME_OK) {
				WL_OLPC_ENTRY(olpc, ("wl%d:%s: nss1_cdd1 min=%d sdmmin=%d\n",
					olpc->wlc->pub->unit, __FUNCTION__,
					min_txpwr_limit, sdmmin));
		...
		if (txchains > 2) {
			...
			err = ppr_get_vht_mcs_min(txpwr, txbw,
				WL_TX_NSS_1, WL_TX_MODE_CDD, WL_TX_CHAINS_3, &sdmmin);

			if (err == BCME_OK) {
				WL_OLPC_ENTRY(olpc, ("wl%d:%s: nss1_cdd2 min=%d sdmmin=%d\n",
					olpc->wlc->pub->unit, __FUNCTION__,
					min_txpwr_limit, sdmmin));
		...
		if (txchains > 3) {
			...
			err = ppr_get_vht_mcs_min(txpwr, txbw,
				WL_TX_NSS_1, WL_TX_MODE_CDD, WL_TX_CHAINS_4, &sdmmin);

			if (err == BCME_OK) {
				WL_OLPC_ENTRY(olpc, ("wl%d:%s: nss1_cdd3 min=%d sdmmin=%d\n",
					olpc->wlc->pub->unit, __FUNCTION__,
					min_txpwr_limit, sdmmin));
...
wlc_olpc_chan_needs_olpc(wlc_olpc_eng_info_t *olpc, chanspec_t chan)
{
	...
	phy_info_t *pi = WLC_PI(olpc->wlc);
	int8 qdbm;
	bool override;
	bool force_check = FALSE;

	if (olpc && olpc->wlc->band && pi) {
		if (olpc->olpc_override) {
			olpc->core_idx = 0;
			return TRUE;
		}
		wlc_phy_txpower_get(pi, &qdbm, &override);
		...
		if (olpc->last_chan_chk != chan ||
			...
			for (; core_idx < cores; core_idx++) {
				max_pwr = MIN(max_pwr,
					wlc_phy_calc_ppr_pwr_cap(pi, core_idx));
			}
			tssi_thresh = (int16)wlc_phy_tssivisible_thresh(pi);
		...
		if (qdbm < wlc_phy_tssivisible_thresh(pi)) {
...
wlc_olpc_eng_pkt_complete(wlc_info_t *wlc, void *pkt, uint txs)
{
	...
	if (olpc_chan->cores_cal == wlc->stf->txchain) {
		...
		phy_chanmgr_create_ctx(pi, wlc->chanspec);
#ifdef WLOLPC
		...
		olpc->total_olpc_cals++;
		olpc->cal_last_time = olpc->wlc->pub->now;
		wlc_olpc_stf_override_revert(wlc);
...
#endif /* WLOLPC */
		if ((err = phy_cache_restore_cal(pi)) != BCME_OK) {
		...
		tssi_visi_thresh = wlc_phy_tssivisible_thresh(pi);
		WL_OLPC_DBG(wlc->olpc_info,
		...
		if (qdbm < tssi_visi_thresh) {
			...
			if ((txpwr = ppr_create(wlc->osh, PPR_CHSPEC_BW(wlc->chanspec))) != NULL) {
				...
				err = wlc_phy_txpower_set(pi, qdbm, FALSE, txpwr);
				WL_OLPC_DBG(wlc->olpc_info,
			...
			/* At the end of ANY OLPC calibration, e.g. txchain update,
			 * primary bss up, olpc_cal_force IOVAR, etc. (and others
			 * could be added), check if the autocal timer needs to be
			 * reloaded.
			 */
			wlc_olpc_autocal_timer_reload(olpc);
		} else {
			WL_OLPC_DBG(wlc->olpc_info,
	...
	if (olpc_chan->cal_pkts_outstanding == 0) {
		...
		WLC_OLPC_DEL_TIMER(olpc, cal_timer); // just in case
		WLC_OLPC_ADD_TIMER(olpc, cal_timer, 0, 0);
	...
	void *params, uint p_len, void *arg, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	tssi_thresh = (int16)wlc_phy_tssivisible_thresh(wlc->pi);
	wlc_phy_txpower_get(wlc->pi, &qdbm, &override);
	...
	switch (actionid) {
		...
		case IOV_GVAL(IOV_OLPC_CAL_DONE):
			*ret_int_ptr = olpc->olpc_cal_done;
			break;
		case IOV_SVAL(IOV_OLPC_AUTOCAL):
			if (olpc->autocal_enabled != bool_val) {
				olpc->autocal_enabled = bool_val;
				if (olpc->autocal_enabled && (qdbm < tssi_thresh)) {
					WLC_OLPC_DEL_TIMER(olpc, autocal_timer);
					WLC_OLPC_ADD_TIMER(olpc, autocal_timer,
						olpc->autocal_period, 0);
				} else {
					WLC_OLPC_DEL_TIMER(olpc, autocal_timer);
				}
			}
		...
		case IOV_GVAL(IOV_OLPC_AUTOCAL):
			*ret_int_ptr = olpc->autocal_enabled;
			break;

		case IOV_GVAL(IOV_OLPC_AUTOCAL_PER):
			*ret_int_ptr = olpc->autocal_period / 1000; // convert to sec
			break;

		case IOV_SVAL(IOV_OLPC_AUTOCAL_PER):
			/* range checking 30 < val < 300 - units in seconds */
			if (int_val < 30) {
				WL_OLPC_DBG(olpc, ("wl%d:%s: olpc_autocal_period too low (<30)\n",
					wlc->pub->unit, __FUNCTION__));
				err = BCME_RANGE;
			}
			if (int_val > 300) {
				WL_OLPC_DBG(olpc, ("wl%d:%s: olpc_autocal_period too high (>300)\n",
					wlc->pub->unit, __FUNCTION__));
				err = BCME_RANGE;
			}
			if (err != BCME_RANGE) {
				/* check if it actually was changed, save and restart timer */
				if (olpc->autocal_period != int_val * 1000) {

					olpc->autocal_period = int_val * 1000;
					if (olpc->autocal_enabled && (qdbm < tssi_thresh)) {
						WLC_OLPC_DEL_TIMER(olpc, autocal_timer);
						WLC_OLPC_ADD_TIMER(olpc, autocal_timer,
							olpc->autocal_period, 0);
...
wlc_dump_olpc(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "\nOLPC is %s - count = %d, last = %u, now = %u",
		(qdbm < tssi_thresh) ? "On" : "Off", olpc->total_olpc_cals,
		olpc->cal_last_time, olpc->wlc->pub->now);
	bcm_bprintf(b, "\nautocal: enabled = %d, period = %d",
		olpc->autocal_enabled, olpc->autocal_period / 1000);

	txpwrindex = wlc_phy_txpwr_idx_get_acphy(wlc->pi);
	bcm_bprintf(b, "\ntxpwrindex for core{0...3}: %d %d %d %d",
		txpwrindex & 0xff,
		(txpwrindex & 0xff00) >> 8,
		(txpwrindex & 0xff0000) >> 16,
		(txpwrindex & 0xff000000) >> 24);

	bcm_bprintf(b, "\ntssivisi_thresh = %d (0x%x) qdBm (%d dBm)",
		tssi_thresh, tssi_thresh, tssi_thresh/4);
	...
	divrem = ABS(divrem);
	divquo = ABS(divquo);
	bcm_bprintf(b, "\nTxPower is %d qdbm, %s%d.%d dbm  Override is %s\n",
	       qdbm, neg ? "-" : "", divquo, divrem, override ? "On" : "Off");
...
BCMATTACHFN(wlc_olpc_eng_attach)(wlc_info_t *wlc)
{
	...
	/* Set up OLPC auto calibration */
	olpc_info->autocal_timer = wl_init_timer(wlc->wl, wlc_olpc_autocal_timer, olpc_info,
		"olpctimer2");

	if (olpc_info->cal_timer == NULL) {
		WL_ERROR(("wl%d: %s: wl_init_timer for OLPC autocal timer failed\n",
			wlc->pub->unit, __FUNCTION__));
		goto fail;
	}

	olpc_info->autocal_period = WL_OLPC_AUTOCAL_PERIOD * 1000;  // convert to msec
	olpc_info->autocal_enabled = WL_OLPC_AUTOCAL_ENAB_DEF;

	/* register module up/down, and iovar callbacks */
...
BCMATTACHFN(wlc_olpc_eng_detach)(struct wlc_olpc_eng_info_t *olpc_info)
{
	...
	if (olpc_info->cal_timer) {
		WLC_OLPC_DEL_TIMER(olpc_info, cal_timer);
		WLC_OLPC_FREE_TIMER(olpc_info, cal_timer);
		olpc_info->cal_timer = NULL;
	}
	if (olpc_info->autocal_timer) {
		WLC_OLPC_DEL_TIMER(olpc_info, autocal_timer);
		WLC_OLPC_FREE_TIMER(olpc_info, autocal_timer);
		olpc_info->autocal_timer = NULL;
	}
...
wlc_olpc_eng_up(void *hdl)
{
	...
	if (olpc_info->autocal_enabled) {
		WLC_OLPC_ADD_TIMER(olpc_info, autocal_timer, olpc_info->autocal_period, 0);
	}
	olpc_info->total_olpc_cals = 0;
...
wlc_olpc_cal_timer(void *arg)
{
	...
	if (olpc->cur_chan->cores_cal == olpc->last_txchain_chk) {
		...
		WLC_OLPC_DEL_TIMER(olpc, cal_timer);
...
/** OLPC autocal timer callback function */
static void
wlc_olpc_autocal_timer(void *arg)
{
	wlc_olpc_eng_info_t *olpc = (wlc_olpc_eng_info_t *)arg;

	WL_OLPC_ENTRY(olpc, ("%s - Entry at %u (last %u)\n", __FUNCTION__,
		olpc->wlc->pub->now, olpc->cal_last_time));

	/* execute calibration */
	wlc_olpc_eng_hdl_chan_update(olpc);

	/* and check auto cal timer reload */
	WL_OLPC_DBG(olpc, ("%s call to olpc_autocal_timer_reload\n", __FUNCTION__));
	wlc_olpc_autocal_timer_reload(olpc);
}

void
wlc_olpc_autocal_timer_reload(wlc_olpc_eng_info_t *olpc)
{
	int tssi_thresh;
	int8 qdbm;
	bool override;
	...
	wlc_phy_txpower_get(olpc->wlc->pi, &qdbm, &override);

	if (olpc->autocal_enabled && (qdbm < tssi_thresh)) {
		WLC_OLPC_DEL_TIMER(olpc, autocal_timer);
		WLC_OLPC_ADD_TIMER(olpc, autocal_timer, olpc->autocal_period, 0);
		WL_OLPC_DBG(olpc, ("%s reloading olpc_autocal_timer with %d seconds\n",
			__FUNCTION__, (olpc->autocal_period/1000)));
	} else {
		WL_OLPC_DBG(olpc, ("%s no autocal reload. enable = %d, qdbm = %d, "
			"tssi_thresh = %d\n", __FUNCTION__,
			olpc->autocal_enabled, qdbm, tssi_thresh));
	}
}
...
bool
wlc_olpc_eng_autocal_enab(wlc_olpc_eng_info_t *olpc)
{
	return (olpc->autocal_enabled);
}
#endif /* WLOLPC */
