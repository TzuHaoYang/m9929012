...
#define SCB_FRAG_BUF_OFFSET_NOT_SET 0xFFu

/* scb cubby */
typedef struct scb_frag {
	void	*fragbuf[NUMPRIO];	/**< defragmentation buffer per prio */
	uint16	fragresid[NUMPRIO];	/**< #bytes unused in frag buffer per prio */
	uint8	fragbufbodyoffset[NUMPRIO];	/**< defragmentation buffer offset to frame body
						* when FC_WEP is set.  Otherwise, set to
						* SCB_FRAG_BUF_OFFSET_NOT_SET
						*/
} scb_frag_t;
...
uint8 *
wlc_defrag_first_frag_body_get(wlc_info_t* wlc, struct scb *scb, uint8 prio)
{
	wlc_frag_info_t *frag = wlc->frag;
	uint8 *body = NULL;
	scb_frag_t *scb_frag = SCB_FRAG(frag, scb);

	if (scb_frag && scb_frag->fragbuf[prio] &&
			scb_frag->fragbufbodyoffset[prio] != SCB_FRAG_BUF_OFFSET_NOT_SET) {
		body = (uint8*)PKTDATA(wlc->osh, scb_frag->fragbuf[prio]);
		body += scb_frag->fragbufbodyoffset[prio];
	}
	return body;
}
	...
	struct wlc_frminfo *f)
{
	...
	if (f->fc & FC_WEP) {
		scb_frag->fragbufbodyoffset[prio] = f->pbody - PKTDATA(scb->bsscfg->wlc->osh, f->p);
	} else {
		scb_frag->fragbufbodyoffset[prio] = SCB_FRAG_BUF_OFFSET_NOT_SET;
	}
