 ...
 *
 * $Id: wlc_bsscfg.h 792726 2020-11-03 05:27:25Z $
 */
...
#define BSSCFG_IS_PRIMARY(cfg)    ((cfg) == (cfg)->wlc->primary_bsscfg)
/* Check if a BSSCFG is the main AP or not */
#define BSSCFG_IS_MAIN_AP(cfg)    ((cfg) == (cfg)->wlc->main_ap_bsscfg)

#define BSSCFG_BANDUNIT(cfg) (CHSPEC_BANDUNIT((cfg)->current_bss->chanspec))
...
#define FOREACH_IBSS(wlc, idx, cfg) \
	for (idx = 0; (int) idx < WLC_MAXBSSCFG; idx++) \
		if (((cfg = (wlc)->bsscfg[idx]) != NULL) && BSSCFG_IBSS(cfg))
...
typedef struct wlc_mbssid_tag_list {
	wlc_iem_tag_t  mbss_tag[MAX_TAG_MBSSID_IE];
	uint8  mbss_tags_len;
	wlc_iem_tag_t  mbss_non_inheritance[MAX_TAG_MBSSID_IE];
	uint8  mbss_non_inheritance_len;
	wlc_iem_tag_t  mbss_non_inheritance_ext[MAX_TAG_MBSSID_IE];
	uint8  mbss_non_inheritance_ext_len;
} wlc_mbssid_tag_list_t;

/**
 * Stores information about the relation between this local node and a BSS. Information about the
 * BSS that was extracted from a beacon or probe response is stored in struct wlc_bss_info instead.
 */
struct wlc_bsscfg {
	...
	bool		is_awdl;
	int8 rateset;				/**< bss rateset, e.g. WLC_BSSCFG_RATESET_OFDM */
	...
	uint8	map_profile;		/* Multi-AP Profile */
	uint16	map_prim_vlan_id;	/* Multi-AP Default 802.1Q Settings Primary VLAN ID */
#endif	/* MULTIAP */
#if defined(WLATM_PERC)
	uint8	bssperc;		/* airtime percentage per bsscfg */
	uint8	sched_bssperc;		/* Schedule context airtime percentage per bsscfg */
#endif /* WLATM_PERC */
	rsnxe_ie_info_t *rsnxe_info;
	uint8   ext_rsn_cap_len;        /* Extended RSN Capabilities length */
	uint8*  ext_rsn_cap;			/* Extended RSN Capabilities */
...
#endif /* WL_WIFI_6GH || WL_OCE_AP */
	uint32 wds_bitmap;	/* wds interface bit map per scbs */
	wlc_mbssid_tag_list_t *mbssid_tag_list;
#ifdef WL_MBSSID
...
#define BSSCFG_IS_MFP_SUPPORTED(cfg) (BSSCFG_IS_MFP_CAPABLE(cfg) || BSSCFG_IS_MFP_REQUIRED(cfg))
#else
#define BSSCFG_IS_MFP_CAPABLE(cfg)	FALSE
#define BSSCFG_IS_MFP_REQUIRED(cfg)	FALSE
#define BSSCFG_IS_MFP_SUPPORTED(cfg) FALSE
#define BSSCFG_SET_MFP_CAPABLE(cfg)
#define BSSCFG_SET_AKM_REQUIRES_MFP(cfg)
#define BSSCFG_CLEAR_AKM_REQUIRES_MFP(cfg)
#endif // endif
...
#define IS_MAP_PROF1_STA_ASSOC_DISALLOWED(cfg) ((cfg)->map_attr & \
		MAP_EXT_ATTR_PROFILE1_STA_ASSOC_DISALLOWED)
#define IS_MAP_PROF2_STA_ASSOC_DISALLOWED(cfg) ((cfg)->map_attr & \
		MAP_EXT_ATTR_PROFILE2_STA_ASSOC_DISALLOWED)
#else
#define MAP_ENAB(cfg)					FALSE
#define IS_MAP_PROF1_STA_ASSOC_DISALLOWED(cfg)		FALSE
#define IS_MAP_PROF2_STA_ASSOC_DISALLOWED(cfg)		FALSE
#endif	/* MULTIAP */
...
#define _BSS_N_ENAB(wlc, cfg) \
	(N_ENAB((wlc)->pub) && !((cfg)->flags & WLC_BSSCFG_11N_DISABLE))

#ifndef BAND6G_USING_HT_IES
#define BSS_N_ENAB(wlc, cfg) (_BSS_N_ENAB((wlc), (cfg)) && BSSCFG_BANDUNIT(cfg) != BAND_6G_INDEX)
#else
#define BSS_N_ENAB(wlc, cfg) _BSS_N_ENAB((wlc), (cfg))
...
#define BSS_OCE_ENAB(wlc, bsscfg)	wlc_bsscfg_is_oce_enabled((wlc), (bsscfg))
extern bool wlc_bsscfg_is_oce_enabled(wlc_info_t *wlc, wlc_bsscfg_t *cfg);
#else
#define BSS_OCE_ENAB(wlc, bsscfg)	0
...
#ifdef WL_GLOBAL_RCLASS
extern void wlc_bsscfg_update_rclass(wlc_info_t* wlc);
extern bool wlc_cur_opclass_global(wlc_info_t* wlc);
#endif /* WL_GLOBAL_RCLASS */
bool wlc_is_ap_interface_up(wlc_info_t *wlc);
int wlc_bsscfg_apsta_is_downstream_ap_up_possible(wlc_info_t *wlc, wlc_bsscfg_t *cfg);
#ifdef WL11AX
...
bool wlc_bsscfg_is_probe_response_required(wlc_bsscfg_t *cfg, uint8 *body, uint8 body_len);
