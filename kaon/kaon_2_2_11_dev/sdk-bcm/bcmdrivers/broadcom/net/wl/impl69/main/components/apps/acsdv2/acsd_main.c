 ...
 *
 * $Id: acsd_main.c 793530 2020-12-01 08:15:57Z $
 */
...
acsd_proc_client_req(void)
{
	...
	if (!d_info->cmd_buf)
		d_info->cmd_buf = acsd_malloc(ACSD_BUFSIZE_8K);

	buf = d_info->cmd_buf;

	/* get command from client */
	if ((rcount = sread(d_info->conn_fd, buf, ACSD_BUFSIZE_8K)) < 0) {
...
acs_channel_trigger(acs_chaninfo_t *c_info, char *ifname, unsigned char *addr)
{
	...
	if (!acs_bgdfs_attempt_on_txfail(c_info)) {
		...
		if (c_info->switch_reason == APCS_DFS_REENTRY && !c_info->cur_is_dfs) {
			...
			if (!(ret = acs_csa_handle_request(c_info))) {
				...
				if (c_info->switch_reason == APCS_DFS_REENTRY) {
					chanim_upd_acs_record(c_info->name, c_info->chanim_info,
							c_info->selected_chspec, APCS_DFS_REENTRY);
				} else {
					chanim_upd_acs_record(c_info->name, c_info->chanim_info,
...
acsd_main_loop(struct timeval *tv)
{
	...
	if (d_info->event_fd !=  ACSD_DFLT_FD && FD_ISSET(d_info->event_fd, &fdset)) {
		...
		if (!acs_is_mode_check(ifname)) {
			ACSD_INFO("avoid channel switch when ACSD is not in AP mode: %s\n", ifname);
			return;
		}
