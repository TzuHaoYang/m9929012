 ...
 *
 * $Id: phy_ac_rxgcrs.c 790786 2020-09-03 01:04:28Z $
 */
...
typedef struct {
	...
	phy_ac_rxg_params_attach_t tbl6glo, tbl6gmd, tbl6ghi, tbl6ghi2;
...
static void wlc_phy_mclip_agc_rxgainctrl(phy_info_t *pi);
	...
	if (IS_28NM_RADIO(pi) || IS_16NM_RADIO(pi)) {
		...
		for (i = 0; i < 2; i++) {
			...
			if (PHY_BAND5G_ENAB(pi)) {
				...
				if (ACMAJORREV_129(pi->pubpi->phy_rev) && BF_ELNA_5G(pi_ac)) {
...
phy_ac_rxgcrs_populate_rxg_params_band_specific(phy_ac_rxgcrs_info_t *rxgcrsi)
{
	...
	uint16 subband_6G_end_freq_mhz[4] = {0, 0, 0, 0};

	if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
		subband_6G_end_freq_mhz[0] = 6351;
		subband_6G_end_freq_mhz[1] = 6510;
		subband_6G_end_freq_mhz[2] = 6851;
		subband_6G_end_freq_mhz[3] = 7200;
	}
	if (CHSPEC_IS2G(pi->radio_chanspec)) {
		...
		((ACMAJORREV_47(pi->pubpi->phy_rev) && (RADIOREV(pi->pubpi->radiorev) > 2)) ||
		ACMAJORREV_130(pi->pubpi->phy_rev))) {
		rxg_params->rftbls = &rfbands->tbl6g;
	} else if (CHSPEC_IS6G(pi->radio_chanspec) && ACMAJORREV_129(pi->pubpi->phy_rev)) {
		if (CHAN6G_FREQ(CHSPEC_CHANNEL(pi->radio_chanspec)) < subband_6G_end_freq_mhz[0]) {
			rxg_params->rftbls = &rfbands->tbl6glo;
		} else if (CHAN6G_FREQ(CHSPEC_CHANNEL(pi->radio_chanspec))
			< subband_6G_end_freq_mhz[1]) {
			rxg_params->rftbls = &rfbands->tbl6gmd;
		} else if (CHAN6G_FREQ(CHSPEC_CHANNEL(pi->radio_chanspec))
			< subband_6G_end_freq_mhz[2]) {
			rxg_params->rftbls = &rfbands->tbl6ghi;
		} else {
			rxg_params->rftbls = &rfbands->tbl6ghi2;
		}
	} else {
		rxg_params->rftbls = &rfbands->tbl5g;
	}
...
BCMATTACHFN(phy_ac_rxgcrs_std_params_attach)(phy_ac_rxgcrs_info_t *ri)
{
	...
	if (ACMAJORREV_37(pi->pubpi->phy_rev) || ACMAJORREV_47(pi->pubpi->phy_rev) ||
	...
	if (ACMAJORREV_40_128(pi->pubpi->phy_rev) || IS_4364_1x1(pi) ||
		IS_4364_3x3(pi)) {
...
wlc_dynamic_ed_thresh(phy_info_t *pi)
{
	...
	uint32 edCount;
	int32 adj_ed_thresh, curr_ed_thresh;
	bool edcrs_flag;
	phy_info_acphy_t *pi_ac = pi->u.pi_acphy;
	...
	if (curr_ed_thresh == 0) {
		wlc_phy_adjust_ed_thres_acphy(pi_ac->rxgcrsi, &curr_ed_thresh, FALSE);
                                     ...
                                     bool trloss, bool lna1byp, uint8 *gidx)
{
	...
	if (elna_present == 1) {
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			...
			if (CHSPEC_CHANNEL(pi->radio_chanspec) < 100) {
				elna_gain[core] = pi_ac->sromi->femrx_5g[core].elna;
			} else if (CHSPEC_CHANNEL(pi->radio_chanspec) < 132) {
	...
	bool trloss, bool lna1byp, uint8 clipgain, uint8 *gidx)
{
	...
	int8 k, idx, maxgain_this_stage, gain;
	...
	*/
	...
	if (!apply_desense && !pi_ac->rxgcrsi->desense_forced_dB) {
		...
		*/
		...
		if (CHSPEC_IS2G(pi->radio_chanspec) && !ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
...
wlc_phy_upd_lna1_lna2_gaintbls_acphy(phy_info_t *pi, uint8 lna12)
{
	...
	if (lna12 == LNA1_ID && ACMAJORREV_40(pi->pubpi->phy_rev)) {
		if (CHSPEC_ISPHY5G6G(pi->radio_chanspec) &&
...
phy_ac_set_20708_RSSI_detectors(phy_info_t *pi, uint8 core, uint8 *thresholds)
{
	...
	MOD_RADIO_REG_20708(pi, RXDB_WRSSI1_REG1, core, rxdb_wrssi1_threshold_high, w1_high);
...
static void wlc_phy_mclip_agc_rssi_setup_rev130(phy_info_t *pi)
{
	...
	const uint8 clipcnt_thresh_bw160[9] = {160, 160, 160, 80, 80, 80, 160, 160, 160};
	const uint8 clipcnt_thresh_bw80[9] = {80, 80, 80, 80, 80, 80, 80, 80, 80};
	const uint8 clipcnt_thresh_bw40[9] = {40, 40, 40,  40,  40,  40,  40,  40,  40};
	uint8 clipcnt_thresh_bw20_2G[9] = {20, 20, 20,  30,  30,  30,  30,  30,  30};
	uint8 clipcnt_thresh_bw20_5G[9] = {20, 20, 20,  20,  20,  20,  20,  20,  20};
...
static void wlc_phy_mclip_agc_rssi_setup_rev131(phy_info_t *pi)
{
	...
	const uint8 ana_thld_20710rX_5g[7]  = {13, 7, 7, 7, 3, 3, 3};
	const uint8 clipcnt_thresh_bw160[9] = {160, 160, 160, 160, 160, 160, 160, 160, 160};
	const uint8 clipcnt_thresh_bw80[9] = {80, 80, 80, 80, 80, 80, 80, 80, 80};
	const uint8 clipcnt_thresh_bw40[9] = {40, 40, 40,  40,  40,  40,  40,  40,  40};
	uint8 clipcnt_thresh_bw20_2G[9] = {20, 20, 20,  30,  30,  30,  30,  30,  30};
...
static void wlc_phy_mclip_agc_rxgainctrl(phy_info_t *pi)
{
	...
	int8 high_sens_adjust_list_2g_rev40[] = {10, 10, 10, 10};
...
phy_ac_rxgcrs_lesi(phy_ac_rxgcrs_info_t *rxgcrsi, bool on, uint8 delta_halfdB)
{
	...
	uint8 low_regime_rev130_5g = 80;
	uint8 low_regime_rev130_6g = 70;
		...
		*/
		...
		if (ACMAJORREV_47(pi->pubpi->phy_rev) || ACMAJORREV_129(pi->pubpi->phy_rev) ||
			...
			if (CHSPEC_IS2G(pi->radio_chanspec)) {
				...
				ACPHYREG_BCAST(pi, LowPowerRegimeControlUpperThreshold0, upper);
				ACPHYREG_BCAST(pi, LowPowerRegimeControlLowerThreshold0, upper - 6);
			}
			...
			if (!ACMAJORREV_130(pi->pubpi->phy_rev)) {
				...
				MOD_PHYREG(pi, lesiHTPktGain, LesiHTAgcPktgainWait, 0x22);
				MOD_PHYREG(pi, lesiHTPktGain, LesiHTAGCStrtAdjst, chspec_is20 ?
