 ...
 *
 * $Id: wlu.c 793898 2020-12-14 16:11:01Z $
 */
...
static cmd_func_t wl_dump_networks, wl_revinfo;
...
static cmd_func_t wl_country_abbrev_override;
...
static int wl_chan_info_channel(void *wl, chanspec_t chanspec, bool report_invalid);
...
cmd_t wl_cmds[] = {
	...
	"Set the short retry limit.  (integer [1, 15])" },
	{ "lrl", wl_int, WLC_GET_LRL, WLC_SET_LRL,
	"Set the long retry limit.  (integer [1, 15])" },
	...
	{ "6g_rate", wl_rate, WLC_GET_VAR, WLC_SET_VAR,
	"Force a fixed rate for data frames in the 6G band:\n\n"
	RATE_6G_USAGE
	},
	{ "6g_mrate", wl_rate, WLC_GET_VAR, WLC_SET_VAR,
	...
	{ "mclx_mode", wl_mclx_mode, WLC_GET_VAR, -1,
	...
	"\tUsage: wl roam_prof_2g 2g|5g|6g flags rssi_upper rssi_lower delta, boost_thresh "
	...
	"\t\tExample: PHY: AC, Band 2.4G, CCK rates only, with basic rates, "
	...
	{ "country_rev", wl_country_rev, WLC_GET_VAR, -1,
	...
	{ "macfilter_dump", wl_macfltr_dump, WLC_GET_FLTR_COUNT, WLC_SET_FLTR_COUNT,
	...
	{ "scan_ver", wl_scan_ver, WLC_GET_VAR, -1,
	...
	{"rnr_list", wl_rnr_list, WLC_GET_VAR, WLC_SET_VAR,
	...
	{ "country_abbrev_override", wl_country_abbrev_override, WLC_GET_VAR, WLC_SET_VAR,
	...
	"\tgpaio pmu_lnaldo\n\n"
	"\tgpaio pmu_adcldo\n\n"
	"\tgpaio tempsense_vbg_p\n\n"
	"\tgpaio tempsense_vbe_p\n\n"
	"\tgpaio bg_vctat_cal\n\n"
	"\tgpaio bg_vctat_uncal\n\n"
	"\tgpaio bg_vptat\n\n"
	"\tgpaio clear\n\n"},
#endif // endif
	...
	"wl mac_capture [--reset] [--stop] [--logic_analyzer] [-sel <gpio_sel>] "
	"[-s <start_ptr>] [-e <stop_ptr>] [-sz <size>] [-sm <store_mask>] "
	"[-mm <match_mask>] [-mv <match_val>] [-xm <transition_mask>] "
	"[-trigger <trigger_val>] [-tm <trigger_mask>] [--stopmode] [--expert]\n"
	...
	{"dyntxc", wl_dyntxc, WLC_GET_VAR, WLC_SET_VAR,
	"Configure the dynamic tx control settings\n"
	#if defined(BCMINTERNAL)
	"\t  -s: Configure the supported STA type\n"
	"\t       b0: STA supports up to 3 steams bfe_sts_cap\n"
	"\t       b1: STA supports up to 4 steams bfe_sts_cap\n"
	"\t       b2: STA supports HT\n"
	#endif
	...
	{ "fifo", wl_fifo_cmd, WLC_GET_VAR, WLC_SET_VAR,
	"Set/Get FIFO module's parameters.\n"
	...
	{ "pktdrop_stats", wlu_pktdrop_stats, WLC_GET_VAR, -1,
	"pktdrop_stats [ client-mac-address [ full | brief ]] [ clear ]" },
	...
	{ "extended_ops_support", wl_varint, WLC_GET_VAR, WLC_SET_VAR,
	"cfg support for non hostpad applications\n"
	"\t0 - disable  some cfg ops(which might affect hostapd)\n"
	"\t1 - enable  all cfg ops \n"},
	{ "rsnie", wl_rsn_ie, -1, WLC_SET_VAR,
	"Set RSNIE for an interface\n"
...
dload_clm(void *wl, uint32 datalen, unsigned char *org_buf, int ds_id)
{
	...
	if ((new_buf = (unsigned char *)CALLOC(size2alloc)) != NULL) {
...
process_clm_data(void *wl, char *clmfn, int ds_id)
{
	...
	if ((new_buf = CALLOC(clm_filelen)) == NULL) {
...
process_txcap_data(void *wl, char *txcapfn)
{
	...
	if ((new_buf = CALLOC(txcap_filelen)) == NULL) {
		fprintf(stderr, "unable to allocate %u bytes based on input file size!\n",
			txcap_filelen);
		ret = -ENOMEM;
		goto error;
...
process_cal_data(void *wl, char *calfn)
{
	...
	if ((new_buf = CALLOC(cal_filelen)) == NULL) {
		fprintf(stderr, "unable to allocate %u bytes based on input file size!\n",
			cal_filelen);
		ret = -ENOMEM;
		goto error;
...
wl_macreg(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argc >= 3) {
		if (!str2bandtype(argv[2], &rwt.band, BND_IS_STRING)) {
	...
	/* Fourth arg: band */
	if (argc >= 4) {
		if (!str2bandtype(argv[3], &rwt.band, BND_IS_STRING)) {
			return BCME_USAGE_ERROR;
		}
	}
 ...
 * the band can be a/b/6g/all or omitted. "all"(set only) means all supported bands. A blank means
 * 'current band'.
 */
static int
wl_band_elm(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argc >= 1) {
		if (str2bandtype(argv[0], &band, BND_IS_STRING_INC_BAND_ALL)) {
			x.band = band;
		} else {
	...
	/* Second arg: band */
	if (argc >= 2) {
		if (!str2bandtype(argv[1], &band, BND_IS_STRING_INC_BAND_ALL)) {
			return BCME_USAGE_ERROR;
		}
		x.band = band;
	}
...
static int
wlu_dump_nvram(void *wl, char *dump_buf, char **argv)
{
	int ret;
	int sromrev = 0;
	srom_rw_t *srt;
	...
	/* Read the srom and find out the sromrev */
	srt = (srom_rw_t *)buf;
	srt->byteoff = htod32(0);
	srt->nbytes = htod32(2 * SROM4_WORDS);

	ret = wlu_get(wl, WLC_GET_SROM, buf, WLC_IOCTL_MAXLEN);
	if (ret >= 0) {
		if (srt->buf[SROM18_SIGN] == SROM18_SIGNATURE) {
			sromrev = 18;
		}
	}
	...
	if (sromrev >= 18) {
		for (page = 0; page < VARS_PAGE_NUM; page++) {
			snprintf(dump_buf, WL_DUMP_BUF_LEN, "nvram -p%d", page);
			if ((ret = wlu_iovar_getbuf(wl, "dump", dump_buf, (int)strlen(dump_buf),
...
wlu_dump(void *wl, cmd_t *cmd, char **argv)
{
	...
	} else if (!strncmp(*argv, str_nvram, sizeof(str_nvram))) {
		ret = wlu_dump_nvram(wl, dump_buf, argv);
...
#if	defined(linux) || defined(_CFE_) || defined(DONGLEBUILD) || \
	defined(BWL_NO_INTERNAL_STDLIB_SUPPORT)
/* linux: ffs is in the standard C library */
...
parsecis(char *b, char **argv, int sromrev)
{
	...
	while (srv->tag != 0xFF) {
		...
		if (srv->tag == OTP_RAW) {
			...
			par = CALLOC(strlen(srv->params)+1);
			if (!par)
				return BCME_NOMEM;
			...
			if (sromrev == 18) {
				...
				if (srv->off > LEGACY_PA_OFFSET_END) {
					if ((srv->off >= SROM18_5GB5_PA)) {
						bw160_antoff = (path * SROM18_6GEXT_PA_OFFSET_ANT);
					} else {
						bw160_antoff = (path * SROM18_PA_OFFSET_ANT);
					}
				}
			} else if (sromrev == 13) {
				if (path == 0) {
...
/* Deprecated. Use "bg_mrate" or "a_mrate". does not support the 6ghz band. */
...
wl_rate(void *wl, cmd_t *cmd, char **argv)
{
	...
	option_name = CALLOC(option_name_len);
	if (option_name == NULL) {
...
wl_pmkid_info(void *wl, cmd_t *cmd, char**argv)
{
	...
	if (!*++argv) {
		...
		if ((pmkid_info = (pmkid_list_t *)CALLOC(len)) == NULL)	{
...
wl_default_rateset(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* band type */
	if (!str2bandtype(argv[0], &bandtype, BND_IS_NUMERICAL)) {
		fprintf(stderr, "Wrong band type: %s\n", *argv);
		return BCME_USAGE_ERROR;
	}

	rates_info.bandtype = bandtype;
...
static int
wl_mclx_mode(void *wl, cmd_t *cmd, char **argv)
{
	int err;
	uint32 val = 0;
	...
	if (!*argv) {
		...
		if (MCLX_ENAB(val)) {
			printf("mclx_mode: TRUE\n");
			printf("band_2G: txchain=0x%x\n", (val & MCLX_2G_TXCHAIN_MASK) >>
			       MCLX_2G_TXCHAIN_SHIFT);
			printf("band_2G: rxchain=0x%x\n", (val & MCLX_2G_RXCHAIN_MASK) >>
			       MCLX_2G_RXCHAIN_SHIFT);
			printf("band_5G: txchain=0x%x\n", (val & MCLX_5G_TXCHAIN_MASK) >>
			       MCLX_5G_TXCHAIN_SHIFT);
			printf("band_5G: rxchain=0x%x\n", (val & MCLX_5G_RXCHAIN_MASK) >>
			       MCLX_5G_RXCHAIN_SHIFT);
			printf("band_6G: txchain=0x%x\n", (val & MCLX_6G_TXCHAIN_MASK) >>
			       MCLX_6G_TXCHAIN_SHIFT);
			printf("band_6G: rxchain=0x%x\n", (val & MCLX_6G_RXCHAIN_MASK) >>
			       MCLX_6G_RXCHAIN_SHIFT);
		} else {
			printf("mclx_mode: FALSE\n");
		}
		return 0;
	} else {
		return BCME_UNSUPPORTED;
	...
	if (arg1 == WL_DFS_AP_MOVE_ABORT || arg1 == WL_DFS_AP_MOVE_STUNT) {
		...
		if (val != INVCHANSPEC) {
			if (!argv[1]) {
				...
				char *endptr = NULL;
				int vals[] = {val, htod32(strtol(argv[1], &endptr, 0))};
				err = wlu_var_setbuf_sm(wl, cmd->name, vals, sizeof(vals));
			}
		} else {
			err = BCME_USAGE_ERROR;
		}
...
wlc_traffic_thresh(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!stricmp(param, "-dump")) {
		void *dump_buf = CALLOC(sizeof(struct get_traffic_thresh));
...
wl_cfp_dump(void *wl, cmd_t *cmd, char **argv)
{
	...
	dump_buf = CALLOC(WLC_IOCTL_MEDLEN);
	if (dump_buf == NULL) {
		fprintf(stderr, "Failed to allocate dump buffer of %d bytes\n", WLC_IOCTL_MEDLEN);
		return BCME_NOMEM;
	}
...
wl_version(void *wl, cmd_t *cmd, char **argv)
{
	...
	dump_buf = CALLOC(WLC_IOCTL_SMLEN);
	if (dump_buf == NULL) {
		fprintf(stderr, "Failed to allocate dump buffer of %d bytes\n", WLC_IOCTL_SMLEN);
		return BCME_NOMEM;
	}
...
wl_roamparms(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		params_size = WL_MAX_ROAMSCAN_DATSZ_V2;
	} else {
		params_size = WL_MAX_ROAMSCAN_DATSZ_V1;
	}
	...
	if (err) {
		...
		printf("scan_type: %d\n", dtoh32(((wl_scan_params_v1_t *)params)->scan_type));
		printf("nprobes: %d\n", dtoh32(((wl_scan_params_v1_t *)params)->nprobes));
		printf("active_time: %d\n", dtoh32(((wl_scan_params_v1_t *)params)->active_time));
		printf("passive_time: %d\n", dtoh32(((wl_scan_params_v1_t *)params)->passive_time));
		printf("home_time: %d\n", dtoh32(((wl_scan_params_v1_t *)params)->home_time));
		if (version == WL_SCAN_VERSION_MAJOR_V2) {
			/* print out the channels, if any */
			if (((wl_scan_params_v2_t *)params)->channel_num) {
				uint32 i;
				uint32 chcount =
					dtoh32(((wl_scan_params_v2_t *)params)->channel_num);
				printf("Channels:\n");
				for (i = 0; i < chcount; i++)
					printf("Channel number 0x%x\n",
						dtoh16(
						((wl_scan_params_v2_t *)params)->channel_list[i]));
			}
			/* No ssids to print out, ever */
		} else {
			/* print out the channels, if any */
			if (((wl_scan_params_v1_t *)params)->channel_num) {
				uint32 i;
				uint32 chcount =
					dtoh32(((wl_scan_params_v1_t *)params)->channel_num);
				printf("Channels:\n");
				for (i = 0; i < chcount; i++)
					printf("Channel number 0x%x\n",
						dtoh16(
						((wl_scan_params_v1_t *)params)->channel_list[i]));
	...
	if ((err = wl_scan_parse_params(wl, cmd, argv, &params_size,
			&scan_parse_params, version)) != BCME_OK) {
		goto done;
	}
	err = wl_scan_prep(wl, cmd, argv, params, &params_size, &scan_parse_params, version);
	if (err)
		goto done;
...
wl_roamchannels(void *wl, cmd_t *cmd, char **argv)
{
	...
	params = (wl_roam_channel_list_t *)CALLOC(params_size);
	if (params == NULL) {
...
wl_roam_prof(void *wl, cmd_t *cmd, char **argv)
{
	...
	++argv;
	rp.v1.ver = wl_rp_ver;
	if (*argv == NULL || !str2bandtype(*argv, &rp.v1.band, BND_IS_STRING)) {
...
wl_scan(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* get the scan version to decide which scan_version to use */
	if ((err = wlu_var_getbuf(wl, "scan_ver", NULL, 0, &ptr)) == BCME_OK) {
		if (ptr != NULL) {
			ver = (wl_scan_version_t *)ptr;
			if (ver->scan_ver_major == WL_SCAN_VERSION_MAJOR_V2) {
				version = WL_SCAN_VERSION_MAJOR_V2;
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		...
		params_size += WL_SCAN_PARAMS_FIXED_SIZE_V1 + WL_NUMCHANNELS * sizeof(uint16);
	}
...
wl_escan(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		if ((err = wl_scan_prep(wl, cmd, argv, &((wl_escan_params_v2_t *)params)->params,
			&params_size, &scan_parse_params, version)) == BCME_OK) {
			((wl_escan_params_v2_t *)params)->version = htod32(ESCAN_REQ_VERSION_V2);
			params_size += OFFSETOF(wl_escan_params_v2_t, params);
		}
	} else {
		if ((err = wl_scan_prep(wl, cmd, argv, &((wl_escan_params_v1_t *)params)->params,
				&params_size, &scan_parse_params, version)) == BCME_OK) {
	...
	if (!err) {
			...
			((wl_escan_params_v1_t *)params)->sync_id = htod16(rand() & 0xffff);
#else
			((wl_escan_params_v1_t *)params)->sync_id = htod16(4321);
...
wl_iscan(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		params_size += (WL_SCAN_PARAMS_FIXED_SIZE_V2 +
			OFFSETOF(wl_iscan_params_v2_t, params)) +
			(WL_NUMCHANNELS * sizeof(uint16));
	} else {
		params_size = (WL_SCAN_PARAMS_FIXED_SIZE_V1 +
			OFFSETOF(wl_iscan_params_v1_t, params)) +
			(WL_NUMCHANNELS * sizeof(uint16));
	}
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		if ((err = wl_scan_prep(wl, cmd, argv, &((wl_iscan_params_v2_t *)params)->params,
				&params_size, &scan_parse_params, version)) == BCME_OK) {
			((wl_iscan_params_v2_t *)params)->version = htod32(ISCAN_REQ_VERSION_V2);
			params_size += OFFSETOF(wl_iscan_params_v2_t, params);
		}
	} else {
		if ((err = wl_scan_prep(wl, cmd, argv, &((wl_iscan_params_v1_t *)params)->params,
				&params_size, &scan_parse_params, version)) == BCME_OK) {
			params_size += OFFSETOF(wl_iscan_params_v1_t, params);
			((wl_iscan_params_v1_t *)params)->version = htod32(ISCAN_REQ_VERSION_V1);
			params_size += OFFSETOF(wl_iscan_params_v1_t, params);
		}
	}

	if (!err) {
		((wl_iscan_params_v1_t *)params)->action = htod16(action);
		((wl_iscan_params_v1_t *)params)->scan_duration = htod16(iscan_duration);
...
wl_reassoc(void *wl, cmd_t *cmd, char **argv)
{
	...
	params = (wl_reassoc_params_t *)CALLOC(params_size);
	if (params == NULL) {
		fprintf(stderr, "Error allocating %d bytes for scan params\n", params_size);
		return BCME_NOMEM;
	}
...
wl_dump_networks(void *wl, cmd_t *cmd, char **argv)
{
	...
	dump_buf_orig = dump_buf = CALLOC(WL_DUMP_BUF_LEN);
	if (dump_buf == NULL) {
		fprintf(stderr, "Failed to allocate dump buffer of %d bytes\n", WL_DUMP_BUF_LEN);
		return BCME_NOMEM;
	}
		...
		switch (val_xtlv->id) {
			case WL_RMC_RPT_XTLV_BSS_INFO:
			{
				...
				printf("\t\tReason code for last full scan: %s(%d)\n",
					reason_name ? reason_name : "<UNKNOWN_CODE>",
					bss_info->reason);
				...
				printf("\t\tLast status code for not roaming: %s(%d)\n",
					status_name ? status_name : "<UNKNOWN_CODE>",
					bss_info->status);
			}
				break;
...
wl_channels_in_country(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!str2bandtype(*argv, &cic->band, BND_IS_STRING)) {
...
wl_join_prescanned(void *wl, wl_join_params_t *join_params, uint *join_params_size)
{
	...
	if ((destbuf = CALLOC(WL_DUMP_BUF_LEN)) == NULL) {
...
wl_join(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((join_params = CALLOC(join_params_size)) == NULL) {
	...
	if (passive) {
		...
		if ((extjoin_params = CALLOC(extjoin_size)) == NULL) {
...
wl_band(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv == NULL) {	/* get current band */
		...
		if (!str2bandtype(*argv, &band, BND_IS_STRING_INC_BAND_AUTO)) {
...
/* get country rev list */
static int wl_country_rev(void *wl, cmd_t *cmd, char **argv)
{
	int err = 0;
	unsigned int i;
	...
	if (*argv && strlen(*argv) < 4) {
		if ((err = wlu_iovar_getbuf(wl, cmd->name, *argv, strlen(*argv) + 1,
				buf, WLC_IOCTL_MEDLEN)) < 0) {
		...
		fprintf(stderr, "%s: country abbr unspecified\n", cmd->name);
		err = BCME_USAGE_ERROR;
	}
...
wl_country(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!stricmp(*argv, "list")) {
		...
		if (*++argv) {
			cl->band_set = TRUE;
			if (!str2bandtype(*argv, &cl->band, BND_IS_STRING)) {
				printf("unsupported band: %s\n", *argv);
				return BCME_UNSUPPORTED;
			}
		} else {
...
wl_actframe(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((af_params = (wl_af_params_t *) CALLOC(WL_WIFI_AF_PARAMS_SIZE)) == NULL) {
	...
	/* add the channel (and bandtype) */
	if (argc > 3 && argv[3]) {
		chanspec_t chspec = wf_chspec_aton(argv[3]);
		af_params->channel = htod32(chspec & WL_CHANSPEC_CHAN_MASK);
		af_params->bandtype = htod32(CHSPEC_BANDTYPE(chspec));
	}
...
{
	uint channel, last_channel, band_index;
	chanspec_t chanspec;
	int err;
	bool report_invalid;
	struct band_info {
		...
		{CH_MAX_2G_CHANNEL, WL_CHANSPEC_BAND_2G},
		{MAXCHANNEL, WL_CHANSPEC_BAND_5G},
		{MAXCHANNEL, WL_CHANSPEC_BAND_6G}
	};

	BCM_REFERENCE(last_channel);
	...
	if (*++argv != NULL) {
		...
		if (chanspec == 0) {
			fprintf(stderr,
			        "error: could not parse \"%s\" as a chanspec\n",
					*argv);
			fprintf(stderr, "Usage: %s [chanspec]\n", cmd->name);
			err = BCME_USAGE_ERROR;
		} else {
			report_invalid = TRUE;
			err = wl_chan_info_channel(wl, chanspec, report_invalid);
		}

		return err;
	}

	/* report on all valid channels */
	report_invalid = FALSE;
	for (band_index = 0; band_index < 3; band_index++) {
		struct band_info *bi = &band_info[band_index];

		for (channel = 0; channel <= bi->max_channel; channel++) {
			chanspec = wf_create_20MHz_chspec(channel, bi->band);
			if (chanspec == INVCHANSPEC) {
				continue;
			}
...
static int
wl_chan_info_channel(void *wl, chanspec_t chanspec, bool report_invalid)
{
	uint32 bitmap;
	uint32 chanspec_arg;
	uint32 minutes;
	...
	chanspec_arg = (uint32)chanspec;
	chanspec_arg = wl_chspec32_to_driver(chanspec_arg);

	err = wlu_iovar_getbuf(wl, "per_chan_info",
	                       (char*)&chanspec_arg, sizeof(chanspec_arg),
	                       buf, WLC_IOCTL_SMLEN);
	if (err < 0) {
		char buf_chanspec[CHANSPEC_STR_LEN] = "";
		wf_chspec_ntoa(chanspec, buf_chanspec);
		fprintf(stderr,
		        "error: per_chan_info err for chanspec 0x%04X \"%s\"\n",
		        chanspec, buf_chanspec);
		goto exit;
	}

	bitmap = dtoh32(*(uint32 *)buf);
	...
	printf("Channel %d\t", wf_chspec_primary20_chan(chanspec));

	if (bitmap & WL_CHAN_BAND_5G) {
		printf("5G Band");
	} else if (bitmap & WL_CHAN_BAND_6G) {
		printf("6G Band");
		/* mark Preferred Scanning Channel */
		if (wf_chspec_primary20_chan(chanspec) % 16 == 5) {
			printf(", PSC");
		}
	} else {
		printf("2G Band");
...
int
wl_macfltr_dump(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	uint i;
	struct ether_addr *ea;
	fltr_list_t *fltrlist = (fltr_list_t *) buf;
	if (!*++argv) {
		if (cmd->get < 0)
			return -1;
		if ((ret = wlu_get(wl, cmd->get, fltrlist, WLC_IOCTL_MEDLEN)) < 0)
			return ret;
		fltrlist->count = dtoh32(fltrlist->count);
		printf("Total requests suppressed = %d\n", fltrlist->total_supr_count);
		printf("%16s : prob: auth\n", cmd->name);
		for (i = 0; i < fltrlist->count; i++) {
			ea = &(fltrlist->unit[i].ea);
			printf("%s: %4d:%4d\n", wl_ether_etoa(ea),
				fltrlist->unit[i].prob_fltr_count,
				fltrlist->unit[i].auth_fltr_count);
		}
		return BCME_OK;
	} else {
		if (cmd->set < 0)
			return -1;
		if (!strncmp(*argv, "reset", strlen("reset")))
			return wlu_set(wl, cmd->set, fltrlist, sizeof(int));
	}
	return BCME_USAGE_ERROR;
}
...
wl_assoc_pref(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argv[1]) {
		...
		else if (!strcmp(argv[1], "6g"))
			assoc_pref = WLC_BAND_6G;
...
wl_lifetime(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (strcmp(param, "be") == 0)
		...
		ac = AC_VO;
	else if (strcmp(param, "txfifo") == 0)
		ac = WL_LIFETIME_TXFIFO;
	else {
		fprintf(stderr, "unexpected param %s\n", param);
		return BCME_USAGE_ERROR;
	}
...
wl_bitvecext(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argv[1]) {
		...
		eventmask_msg = (eventmsgs_ext_t*)CALLOC(send_iovar_datasize);
	...
	/* get */
		...
		eventmask_msg = (eventmsgs_ext_t*)CALLOC(EVENTMSGS_EXT_STRUCT_SIZE);
		if (eventmask_msg == NULL) {
		/*
...
wl_escan_event_check(void *wl, cmd_t *cmd, char **argv)
{
	...
	data = (char*)CALLOC(ESCAN_EVENTS_BUFFER_SIZE);
...
wl_event_check(void *wl, cmd_t *cmd, char **argv)
{
	...
	data = (char*)CALLOC(WL_EVENTS_BUFFER_SIZE);
...
wl_hostipv6_extended(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (!*++argv) {
		...
		cfg = (wl_icmp_ipv6_cfg_t *)CALLOC(buf_size);
		if (!cfg) {
...
wl_hmap(void *wl, cmd_t *cmd, char **argv)
{
	...
	BCM_REFERENCE(cmd);
	hmap_params = (pcie_hmap_t *)CALLOC(len);
	if (hmap_params == NULL) {
...
wl_pwrstats(void *wl, cmd_t *cmd, char **argv)
{
	...
	p_query = (wl_pwrstats_query_t *)CALLOC(len);
	if (p_query == NULL) {
		fprintf(stderr, "malloc failed to allocate %d bytes\n", len);
		return -1;
	}
...
static int
wl_country_abbrev_override(void *wl, cmd_t *cmd, char **argv)
{
	int err = 0;
	void *ptr;
	char *abbrv;
	...
	if (argv[1]) {	/* SET */
		len = strlen(argv[1]);
		if (len > (WLC_CNTRY_BUF_SZ-1)) {
			printf("%s: Invalid abbrev \n", __FUNCTION__);
			return BCME_BADLEN;
		}
		if ((err = wlu_var_setbuf(wl, cmd->name, argv[1], len) < 0)) {
			printf("%s: fail to set (%d)\n", __FUNCTION__, err);
		}
	} else {
		if ((err = wlu_var_getbuf(wl, cmd->name, NULL, 0, &ptr) < 0)) {
			printf("%s: fail to get (%d)\n", __FUNCTION__, err);
			return err;
		}
		abbrv = (char *)ptr;
		if (abbrv[0]) {
			abbrv[WLC_CNTRY_BUF_SZ-1] = 0;
			printf("%s\n", abbrv);
		} else {
			printf("no abbrev overide set\n");
		}
	}
...
static void
wl_cca_print_measurement(cca_congest_channel_req_t *results, bool print_header)
{
	uint32 dur;
	int ibss_per, obss_per, inter_per, i;

	if (print_header) {
		printf("chan dur      ibss           obss           interf       time\n");
	}

	for (i = 0; i < results->num_secs; i++) {
		cca_congest_t *chptr = &results->secs[i];

		if ((dur = chptr->duration) == 0) {
			continue;
		}

		/* Percentages */
		ibss_per = chptr->congest_ibss * 100 /dur;
		obss_per = chptr->congest_obss * 100 /dur;
		inter_per = chptr->interference * 100 /dur;

		printf("%-3u %4d %4u %2d%% %-6s %4u %2d%% %-6s %4u %2d%% %-6s %d\n",
		       CHSPEC_CHANNEL(results->chanspec), dur,
		       chptr->congest_ibss, ibss_per, cca_level(ibss_per, IBSS_MED, IBSS_HI),
		       chptr->congest_obss, obss_per, cca_level(obss_per, OBSS_MED, OBSS_HI),
		       chptr->interference, inter_per,
		       cca_level(inter_per, INTERFER_MED, INTERFER_HI),
		       chptr->timestamp);
	}
}
...
wl_cca_get_stats(void *wl, cmd_t *cmd, char **argv)
{
	...
	void *ptr = NULL;
	char *param, *val_p;
	int base, limit, j, channel, tmp_channel, err = 0;
	...
	for (channel = base; channel <= limit; channel++) {
		...
		if (!wf_chspec_valid(req.chanspec)) {
			continue;
		}
		...
		avg[avg_chan_idx] = (cca_congest_channel_req_t *)
			CALLOC(sizeof(cca_congest_channel_req_t));
		...
		if (do_individ) {
			wl_cca_print_measurement(results, print_header);
			print_header = 0;
		}
	}
	...
	for (j = 0; j < avg_chan_idx; j++) {
		...
		printf("%-3u %4d      %2d%% %-6s      %2d%% %-6s      %2d%% %-6s %d\n",
		       CHSPEC_CHANNEL(avg[j]->chanspec), avg[j]->secs[0].duration,
		       avg[j]->secs[0].congest_ibss, cca_level(ibss_per, IBSS_MED, IBSS_HI),
		       avg[j]->secs[0].congest_obss, cca_level(obss_per, OBSS_MED, OBSS_HI),
		       avg[j]->secs[0].interference,
		       cca_level(inter_per, INTERFER_MED, INTERFER_HI),
		       avg[j]->num_secs);
	}
...
#ifndef SERVER_RESPONSE_MAX_BUF_LEN
#define SERVER_RESPONSE_MAX_BUF_LEN 8192
#endif // endif

static int
wl_rnr_list(void *wl, cmd_t *cmd, char **argv)
{
	char *param;
	int err = BCME_OK;
	rnr_list_req_t *req;
	uint8 *ptr = (uint8 *)malloc(SERVER_RESPONSE_MAX_BUF_LEN);
	uint8 *output;
	if (ptr == NULL) {
		printf("No memory allocated for rnr_list\n");
		return BCME_NOMEM;
	} else {
		memset(ptr, 0, SERVER_RESPONSE_MAX_BUF_LEN);
		req = (rnr_list_req_t *)ptr;
	}
	argv++;
	while ((param = *argv++) != NULL) {
		if (stricmp(param, "-h") == 0) {
			...
			free(req);
			return err;
		} else if (stricmp(param, "-a") == 0) {
			char* val;
			wlc_scan_wlioctl_prof_list_t *prof_list =
				(wlc_scan_wlioctl_prof_list_t *)req->prof_list;
			wlc_scan_wlioctl_prof_t *profile =
			       (wlc_scan_wlioctl_prof_t *) prof_list->profile;
			req->ver = 0;
			req->len = sizeof(rnr_list_req_t);
			req->cmd_type = RNR_LIST_SET;
			req->count = 1;
			prof_list->profile_count = 1;
			prof_list->chanspec = INVCHANSPEC;
			param = *argv++;
			while (param != NULL) {
				val = *argv++;
				if (stricmp(param, "-c") == 0) {
					prof_list->chanspec = (int)strtoul(val, NULL, 0);
				} else if (stricmp(param, "-r") == 0) {
					if (stricmp(val, "TRUE") && stricmp(val, "1")) {
						prof_list->rnr = FALSE;
					} else {
						prof_list->rnr = TRUE;
					}
				} else if (stricmp(param, "-s") == 0) {
					strcpy((char *)profile->SSID, (const char*)val);
					profile->ssid_length = strlen(val);
				} else if (stricmp(param, "-ss") == 0) {
					profile->short_ssid = (uint32)strtoul(val, NULL, 0);
					profile->is_short_ssid = TRUE;
				} else if (stricmp(param, "-bs") == 0) {
					if (!wl_ether_atoe(val, &profile->bssid)) {
						err = BCME_USAGE_ERROR;
						break;
					}
					profile->bssid_valid = TRUE;
				} else if (stricmp(param, "-bp") == 0) {
					profile->bss_params = (int)strtoul(val, NULL, 0);
				} else {
					printf("Parameter %s not supported\n", val);
					err = BCME_USAGE_ERROR;
					break;
			...
			if (prof_list->chanspec == INVCHANSPEC) {
				printf("Wrong chanspec is set x%x\n", prof_list->chanspec);
				err = BCME_USAGE_ERROR;
				break;
			}
			err = wlu_var_setbuf(wl, cmd->name, ptr, sizeof(rnr_list_req_t));
			if (err) {
				break;
			}
			free(req);
			return err;
		} else if (stricmp(param, "-d") == 0) {
			req->ver = 0;
			req->len = 0;
			req->count = 0;
			req->cmd_type = RNR_LIST_DEL;
			err = wlu_var_setbuf(wl, cmd->name, ptr, sizeof(rnr_list_req_t));
			if (err) {
				break;
			}
			...
			printf("Parameter %s not supported %s:%d\n", param, __FUNCTION__, __LINE__);
			err = BCME_USAGE_ERROR;
			break;
		}
	}
	if (err) {
		printf("Error in setting values %d\n", err);
		free(req);
		return err;
	}
	/* no parameter is to get rnr_list
	 */
	req->ver = 0;
	req->len = 0;
	req->count = 0;
	req->cmd_type = RNR_LIST_GET;
	if ((err = wlu_var_getbuf(wl, cmd->name, ptr,
			sizeof(rnr_list_req_t), (void **)&output)) < 0) {
		free(req);
		return err;
	} else {
		int i, j;
		rnr_list_req_t *results = (rnr_list_req_t *)output;
...
wl_pktqstatus(void *wl, cmd_t *cmd, char **argv)
{
	...
	pktq_status_param = (scb_pktq_status_t *)CALLOC(sizeof(*pktq_status_param));
...
static int
wl_rate_histo_map_print(wl_rate_histo_map1_t *map1, wl_rate_histo_map2_t *map2)
{
	...
	if (map1) {
		arr = &map1->arr[0];
		arr_len = WL_HISTO_MAP1_ARR_LEN;
		seconds = map1->seconds;
		recent_index = map1->recent_index;
		recent_type = map1->recent_type;
	} else {
		arr = &map2->arr[0];
		arr_len = WL_HISTO_MAP2_ARR_LEN;
		seconds = map2->seconds;
		recent_index = map2->recent_index;
		recent_type = map2->recent_type;
	}
	vhtRateUsed = (recent_type == WL_HISTO_RATE_TYPE_VHT);
	for (i = 0; i < arr_len; i++) {
		if (arr[i] == 0) {
			continue;
		}
	...
	if (tot == 0) {
		...
		printf("total:%d, recent rate type: %s, duration: %ds\n", tot,
			(recent_type < WL_HISTO_RATE_NUM_TYPES ?
			type_str[recent_type] : "Unknown"),
			seconds);
		printf("\ttotal Legacy/non: %u/%u, non-Legacy @NSS1/2/3/4: %u/%u/%u/%u, "
			"non-Legacy @20/40/80MHz: %u/%u/%u, VHT/HE@MCS 9-11: %d\n",
			totLeg, totVHT,
	...
	if (totLeg > 0) {
		...
		for (i = 0; i < lim; i++) {
			percent = PERCENT(arr[i], tot);
			wl_rate_histo_map2str(WL_HISTO_RATE_TYPE_LEGACY,
				i, arr[i], percent, (i == recent_index), out_str);
			printf("%-16s ", out_str);
	...
	for (bwIdx = 0; bwIdx < 3; bwIdx++) {
		...
		for (j = 0; j < lim; j++, i++) {
			...
			percent = PERCENT(arr[i], tot);
			wl_rate_histo_map2str(WL_HISTO_RATE_TYPE_VHT, j, arr[i],
				percent, (i == recent_index), out_str);
			printf("%-16s ", out_str);
	...
	for (i = 0; i < arr_len; i++) {
		...
		wl_rate_histo_map2str(rtype, j, arr[i], percent,
			(i == recent_index), out_str);
...
{
	wl_rate_histo_maps1_t *maps1;
	wl_rate_histo_maps2_t *maps2;
	size_t rpt_len = sizeof(*rpt);
	size_t maps_len;

	if (rpt->ver == WL_HISTO_VER_1) {
		maps_len = sizeof(*maps1);
	} else if (rpt->ver == WL_HISTO_VER_2) {
		maps_len = sizeof(*maps2);
	} else {
	...
	if (rpt->length < (rpt->fixed_len + maps_len)) {
		printf("Report map buffer length too short: %d < %d = %d+%d\n", rpt->length,
			(int) (rpt->fixed_len + maps_len),
			(int) rpt->fixed_len, (int) maps_len);
		return BCME_BUFTOOSHORT;
	}
	...
	if (rpt->ver == WL_HISTO_VER_1) {
		...
		printf("\nRx ");
		wl_rate_histo_map_print(&maps1->rx, NULL);
		printf("\nTx ");
		wl_rate_histo_map_print(&maps1->tx, NULL);
		printf("\n");
	} else {
		printf("Peer MAC %s\n", wl_ether_etoa(&rpt->ea));
		maps2 = (wl_rate_histo_maps2_t *)WL_HISTO_DATA(rpt);
		printf("\nRx ");
		wl_rate_histo_map_print(NULL, &maps2->rx);
		printf("\nTx ");
		wl_rate_histo_map_print(NULL, &maps2->tx);
		printf("\n");
	}
...
/* get rate histogram report V2 */
static int
wl_rate_histo_report_v2(void *wl, cmd_t *cmd, char **argv)
{
	int err;
	int rpt_len = sizeof(wl_rate_histo_report_t) + sizeof(wl_rate_histo_maps2_t);
...
wl_rate_histo_report(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (err == BCME_IOCTL_ERROR) {
		argv--;
		err = wl_rate_histo_report_v2(wl, cmd, argv);
	} else if (err == BCME_OK) {
		err = wl_rate_histo_report_print(rpt);
	}
...
wl_taf_def(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv == NULL) {
		...
		snprintf((char *)taf_def->text, WLC_IOCTL_MAXLEN - sizeof(*taf_def), "%s", *argv);
		...
		err = wlu_iovar_setbuf(wl, cmd->name, taf_def, paramlen, taf_def, WLC_IOCTL_MAXLEN);
	}
...
wl_setiproute(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* allocate the max storage */
	if ((route_tbl = CALLOC(route_tbl_len)) == NULL) {
...
wl_pcie_bus_throughput_params(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv) { /* Set the bus throughput params */
		params = (pcie_bus_tput_params_t *)CALLOC(sizeof(pcie_bus_tput_params_t));
		if (params == NULL) {
...
wl_gpaio(void *wl, cmd_t *cmd, char **argv)
{
	...
	} else if (strcmp("tempsense_vbg_p", argv[1]) == 0) {
		option = GPAIO_TEMPSENSE_VBG_P;
	} else if (strcmp("tempsense_vbe_p", argv[1]) == 0) {
		option = GPAIO_TEMPSENSE_VBE_P;
	} else if (strcmp("bg_vctat_cal", argv[1]) == 0) {
		option = GPAIO_BG_VCTAT_CAL;
	} else if (strcmp("bg_vctat_uncal", argv[1]) == 0) {
		option = GPAIO_BG_VCTAT_UNCAL;
	} else if (strcmp("bg_vptat", argv[1]) == 0) {
		option = GPAIO_BG_VPTAT;
 ...
 * -mv <trigger val> -mm <trigger mask>: Store matching value / mask
 * -xm <trans mask>: Store on transition on masked bits
 * -strigger <stop trigger> : stop trigger value (revid>=129)
 * -psm <1/0>: Use PSM GPIOMON mem (revid>=129)
 * -tpl <1/0>: Use TXE template
 * -lm <logmode> (0/1/3: unconditional/valid_pc/valid_pc && log_en)
 */
static int
wl_mac_captr(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((sc_prms = CALLOC(sc_prms_size)) == NULL) {
	...
	/* Initilize sc_prms */
	sc_prms->optn_bmp |= (3 << WL_MACCAPT_LOGMODE); /* Enable log_en mode by default */
	sc_prms->optn_bmp |= (1 << WL_MACCAPT_FILTER); /* Enable filter mode by default */
	...
	while (*++argv) {
		...
		else if (!stricmp(*argv, "-tv") ||
		!stricmp(*argv, "--trigger_value") ||
		!stricmp(*argv, "--trigger") ||
		!stricmp(*argv, "-trigger")) {
			if (!*++argv) {
				fprintf(stderr, "%s %s: expected argument "
				...
				// by default trigger mode is coupled with stop mode
				sc_prms->optn_bmp |= (1 << WL_MACCAPT_STOPMODE);
			}
		}

		/* specify sample capture stop trigger value */
		else if (!stricmp(*argv, "-stv") ||
		!stricmp(*argv, "--strigger_value") ||
		!stricmp(*argv, "--strigger") ||
		!stricmp(*argv, "--etrigger") ||
		!stricmp(*argv, "-strigger") ||
		!stricmp(*argv, "-etrigger")) {
			if (!*++argv) {
				fprintf(stderr, "%s %s: expected argument "
				...
				sc_prms->tr_val =
					htod32((uint32)(strtoul(*argv, &endptr, 0)));
				sc_prms->optn_bmp |= (1 << WL_MACCAPT_STOPTRIG);
				// by default stop trigger mode does not care about stopmode
			}
		}
		...
		/* specified sample capture stop mode */
		else if (!stricmp(*argv, "-st") || !stricmp(*argv, "--stopmode")) {
			sc_prms->optn_bmp |= (1 << WL_MACCAPT_STOPMODE);
		}

		/* expert mode, everything manully input */
		else if (!stricmp(*argv, "--expert")) {
			sc_prms->optn_bmp |= (1 << WL_MACCAPT_EXPERT);
		}

		/* no print after set */
		else if (!stricmp(*argv, "--noprint")) {
			is_print = FALSE;
		}

		/* use psm for sample capture */
		else if (!strncmp(*argv, "-psm", sizeof("-psm"))) {
			if (!*++argv) {
				fprintf(stderr, "%s %s: expected argument "
					"after \"-psm\" keyword but command line ended.\n",
					wlu_av0, cmd_name);
				ret = BCME_USAGE_ERROR;
				goto exit;
			} else {
				bval = strtoul(*argv, &endptr, 0) == 0 ? 0 : 1;
				sc_prms->optn_bmp |= (bval << WL_MACCAPT_USEPSM);
			}
		}

		/* use txe tpl for sample capture */
		else if (!strncmp(*argv, "-tpl", sizeof("-tpl"))) {
			if (!*++argv) {
				fprintf(stderr, "%s %s: expected argument "
					"after \"-tpl\" keyword but command line ended.\n",
					wlu_av0, cmd_name);
				ret = BCME_USAGE_ERROR;
				goto exit;
			} else {
				bval = strtoul(*argv, &endptr, 0) == 0 ? 0 : 1;
				sc_prms->optn_bmp |= (bval << WL_MACCAPT_USETXE);
			}
		}

		/* set sample capture log mode */
		else if (!stricmp(*argv, "-lm") || !stricmp(*argv, "-logmode")) {
			if (!*++argv) {
				fprintf(stderr, "%s %s: expected argument "
					"after \"-lm\" keyword but command line ended.\n",
					wlu_av0, cmd_name);
				ret = BCME_USAGE_ERROR;
				goto exit;
			} else {
				bval = (htod32((uint32)(strtoul(*argv, &endptr, 0))) & 0x3);
				bval = (bval == 2) ? 0 : bval;
				sc_prms->optn_bmp &= ~(3 << WL_MACCAPT_LOGMODE);
				sc_prms->optn_bmp |= (bval << WL_MACCAPT_LOGMODE);
				if (bval != 0) {
					sc_prms->optn_bmp |= (1 << WL_MACCAPT_FILTER);
				} else {
					sc_prms->optn_bmp &= ~(1 << WL_MACCAPT_FILTER);
				}
			}
		...
		/* set psm controlling gpio mon */
		else if (!stricmp(*argv, "-ps") || !stricmp(*argv, "-psmsel")) {
			if (!*++argv) {
				fprintf(stderr, "%s %s: expected argument "
					"after \"-ps\" keyword but command line ended.\n",
					wlu_av0, cmd_name);
				ret = BCME_USAGE_ERROR;
				goto exit;
			} else {
				sc_prms->optn_bmp |= (1 << WL_MACCAPT_PSMSEL);
				sc_prms->psmsel_bmp = htod32((uint32)(strtoul(*argv, &endptr, 0)));
				if (sc_prms->psmsel_bmp < WL_MACCAP_SEL_PSMR0 ||
					sc_prms->psmsel_bmp > WL_MACCAP_SEL_MAX) {
					fprintf(stderr, "%s %s: invalid parameter "
						"after \"-ps\" keyword. bmp 1/2/4: r0/r1/x\n",
						wlu_av0, cmd_name);
					sc_prms->psmsel_bmp = WL_MACCAP_SEL_PSMR0;
				}
			}
		}
	}
	...
	if (!argv[1]) {
		...
		if (mu_group.forced == WL_MU_GROUP_MODE_FORCED) {
...
#else
				grp_opt_m = mu_group.forced_group_option[m];
#endif // endif
				printf("  Option %d: ", m);
				for (n = 0; n < WL_MU_GROUP_NUSER_MAX; n++) {
			...
			if (mu_group.auto_group_num > 0) {
				printf("Latest recommended groups: %d group(s)\n",
				    mu_group.auto_group_num);
			}
#else
		{
			printf("mu_group: new method %d (%s)\n",
				mu_group.group_method, mu_group.group_method_name);
			if (mu_group.auto_group_num > 0) {
				printf("Latest recommended groups (%d groups)\n",
				    mu_group.auto_group_num);
			}
#endif /* WL_MU_GROUP_PARAMS_VERSION < 4 */
...
static int wl_dyntxc(void *wl, cmd_t *cmd, char **argv)
{
	wl_txs_dyntxc_info_t dyntxc;
	int err = -1;
	//int len = 0;
	uint32 val;

	memset(&dyntxc, 0, sizeof(wl_txs_dyntxc_info_t));
	if (!argv[1]) {
		/* read mode */
		if ((err = wlu_iovar_get(wl, "dyntxc", &dyntxc,
			sizeof(wl_txs_dyntxc_info_t))) < 0) {
			return err;
		}
		printf("dyntxc enable:%d", dyntxc.enable);
		printf("  feature:%d", dyntxc.feature);
		printf("  training period:%d", dyntxc.subperiod_num);
		printf("  candidate set:%d {", dyntxc.coefset);
		...
		/* set mode */
		err = wlu_iovar_set(wl, cmd->name, &dyntxc, sizeof(dyntxc));
	}
...
static int wl_muclients(void *wl, cmd_t *cmd, char **argv)
{
	...
	buf_size = MAX(sizeof(max_clients_t), strlen(cmd->name)+1);
	m = (max_clients_t *) CALLOC(buf_size);

	memset(m, 0, buf_size);
	if ((err = wlu_iovar_getbuf(wl, cmd->name, NULL, 0, m, buf_size)) < 0) {
		return err;
	}
	...
	if (!argv[1]) {
		...
		printf("DL MU-MIMO :%d HE DL-OFDMA :%d HE UL-OFDMA :%d\n",
			dtoh16(m->dlmmu), dtoh16(m->dlofdma), dtoh16(m->ulofdma));
	} else {
		argv++;
		do {
			char *s = *argv++;
...
wl_utrace_capture(void *wl, cmd_t *cmd, char **argv)
{
	...
	buff = CALLOC(WLC_UTRACE_LEN + capture_args_size);
	if (buff == NULL) {
		fclose(fptr);
		return BCME_NOMEM;
	}
...
dump_bss_info(void *bi_generic)
{
	...
	if (version != LEGACY_WL_BSS_INFO_VERSION && (bi->n_cap || bi->he_cap)) {
		if (bi->he_cap) {
			printf("HE Capable:\n");
		} else if (bi->vht_cap) {
			printf("VHT Capable:\n");
	...
	printf("QBSS Channel Utilization: 0x%x (%d %s)\n", bi->qbss_chan_util,
		bi->qbss_chan_util * 100 / (uint8)WLC_QBSS_LOAD_CHAN_FREE_MAX, "%");
		...
		if (rsn_info.akm) {
			...
			for (i = 0; i < akm_count; i++) {
				...
				if (!wlu_bcmp(suite->oui, std_oui, 3)) {
					if ((suite->type == RSN_AKM_SAE_PSK) ||
					(suite->type == RSN_AKM_SAE_FBT))
					{
						is_wpa3 = 1;
					}
					else {
			...
			if (is_wpa3 && is_wpa2)
				printf("RSN (WPA2 + WPA3-SAE):\n");
			else if (is_wpa3 && !is_wpa2)
				printf("RSN (WPA3-SAE):\n");
			else if (!is_wpa3 && is_wpa2)
				printf("RSN (WPA2):\n");
		}
...
wl_musched_cmn_cmd(void *wl, cmd_t *cmd, char **argv, uint16 flags)
{
	...
	if ((musched_params = CALLOC(musched_cmd_param_size)) == NULL) {
...
wl_txbfcfg_cmd(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((txbfcfg_params = CALLOC(txbfcfg_params_size)) == NULL) {
...
wl_srcfg_cmd(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((srcfg_params = CALLOC(srcfg_params_size)) == NULL) {
	...
	if (*argv) {
		srcfg_params->val = strtoul(*argv, NULL, 0);
		srcfg_params->num_vals = 1;
		fprintf(stdout, "%lld", (uint64)(srcfg_params->val));
	} else {
		srcfg_params->num_vals = 0;
	}
...
wl_monitor_config(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((mntcfg_params = CALLOC(mntcfg_param_size)) == NULL) {
...
wl_mutx_ac(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((mutype = *++argv) == NULL) {
		...
		if (reply->mask == 0)
		{
			...
			if (reply->mask & (1 << (WL_MU_DLMMU - 1)))
				printf(" dlmmu");
			if (reply->mask & (1 << (WL_MU_DLOFDMA - 1)))
				printf(" heofdma");
			if (reply->mask & (1 << (WL_MU_ULOFDMA - 1)))
				printf(" ulofdma");
		}
		printf("\n");
	} else {
		...
		{
			do {
				if (strcmp(mutype, "dlmmu") == 0)
					mask |= 1 << (WL_MU_DLMMU - 1);
				else if (strcmp(mutype, "heofdma") == 0)
					mask |= 1 << (WL_MU_DLOFDMA - 1);
				else if (strcmp(mutype, "ulofdma") == 0)
					mask |= 1 << (WL_MU_ULOFDMA - 1);
				else {
					fprintf(stderr, "Invalid AC param %s\n", param);
					return BCME_USAGE_ERROR;
				}
...
static int
wl_fifo_cmd(void *wl, cmd_t *cmd, char **argv)
{
	int err = 0;
	char *param;
	mu_type_t mutype, start, end;
	wl_fifo_msg_t fifo_msg, *reply;
	void *ptr = NULL;
	char *endptr = NULL;
	int8 value;
	char txTypeStr[WL_MU_TX_TYPE_MAX][10] = {"dlmmu", "heofdma", "ulofdma", "twt"};
	uint8 i;

	memset(&fifo_msg, 0, sizeof(wl_fifo_msg_t));
	/* Skip the command */
	if ((param = *++argv) == NULL)
		return BCME_USAGE_ERROR;
	...
	if (!strncmp(*argv, "-h", strlen("-h")) ||
		...
		/* if the first string is not help or -h, it is the key word string */
		strncpy(fifo_msg.keystr, *argv,
			MIN(strlen(*argv), WL_FIFO_CMD_KEYSTR_MAXLEN));
		argv++;
	}

	if (*argv == NULL) {
		mutype = WL_MU_TX_TYPE_MAX;
	} else if (strcmp(*argv, "dlmmu") == 0)
		mutype = WL_MU_DLMMU;
	else if (strcmp(*argv, "ofdma") == 0)
		mutype = WL_MU_DLOFDMA;
	else if (strcmp(*argv, "twt") == 0)
		mutype = WL_MU_TWT;
	else {
		fprintf(stderr, "Invalid mutype param %s\n", param);
		return BCME_USAGE_ERROR;
	}
	if (mutype == WL_MU_TX_TYPE_MAX) {
		start = WL_MU_DLMMU;
		end = mutype - 1;
	} else {
		start = mutype;
	...
	if (((param = *++argv) == NULL) || (mutype == WL_MU_TX_TYPE_MAX)) {
		/* Get FIFO count */
		printf("FIFO %s: ", fifo_msg.keystr);
		for (i = start; i <= end; i++)
		{
			fifo_msg.mutype = htod16(i);
			if ((err = wlu_var_getbuf(wl, cmd->name, &fifo_msg, sizeof(wl_fifo_msg_t),
				&ptr)) < 0)
			{
				return err;
			}
			reply = (wl_fifo_msg_t *) ptr;
			reply->value = dtoh16(reply->value);
			printf("%s=%d ", txTypeStr[i-1], reply->value);
		}
		printf("\n");
	} else {
		value = strtol(param, &endptr, 0);
		fifo_msg.mutype = htod16(mutype);
		fifo_msg.value = htod16(value);
		err = wlu_var_setbuf(wl, cmd->name, &fifo_msg, sizeof(wl_fifo_msg_t));
	}
	return err;
}
