 ...
 *
 * $Id: wlc_csa.c 793762 2020-12-09 03:39:11Z $
 */
...
enum {
	...
	IOV_MAX_CHANNEL_SWITCH_TIME	/* Set/get Max Channel Switch Time */
};

static const bcm_iovar_t wlc_csa_iovars[] = {
#ifdef AP
	{"csa", IOV_CHANSPEC_SWITCH, (IOVF_SET_UP), 0, IOVT_BUFFER, sizeof(wl_chan_switch_t)},
	{"mcst", IOV_MAX_CHANNEL_SWITCH_TIME, (IOVF_SET_UP), 0, IOVT_UINT32, 0},
#endif /* AP */
	{NULL, 0, 0, 0, 0, 0}
};
...
static int wlc_send_action_switch_channel_ex(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg,
	...
	uint8 radarch_len, uint8 *radarch);
static void wlc_send_public_action_switch_channel(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg,
	const struct ether_addr *dst, wl_chan_switch_t *csa, bool protected_dual);
...
#endif // endif
static uint wlc_csa_calc_mcst_ie_len(void *ctx, wlc_iem_calc_data_t *data);
static int wlc_csa_write_mcst_ie(void *ctx, wlc_iem_build_data_t *data);
#endif /* AP */
#ifdef STA
...
static void wlc_csa_process_vs_radarch_csa_ie(wlc_info_t *wlc, bcm_tlv_t *ies, uint32 ies_len,
		uint8 *radarch_len, uint8 **radarch);
...
#define CSA_DEF_MAX_CHANNEL_SWITCH_TIME	500	/* Unit of TU */
...
BCMATTACHFN(wlc_csa_attach)(wlc_info_t *wlc)
{
	...
	csam->wlc = wlc;
	csam->mcst = CSA_DEF_MAX_CHANNEL_SWITCH_TIME;
...
#endif /* WL11AC */
	if (wlc_iem_add_build_fn_mft(wlc->iemi, bcnfstbmp, DOT11_MNG_MAX_CHANNEL_SWITCH_TIME_ID,
	      wlc_csa_calc_mcst_ie_len, wlc_csa_write_mcst_ie, csam) != BCME_OK) {
		WL_ERROR(("wl%d: %s: wlc_iem_add_build_fn failed, mcst in bcn\n",
		          wlc->pub->unit, __FUNCTION__));
		goto fail;
	}
#endif /* AP */
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_CHANSPEC_SWITCH): {
		...
		if (!(N_ENAB(wlc->pub) || CHSPEC_ISLE20(csa->chspec))) {
			err = BCME_BADCHAN;
			break;
		}
		if (!wlc_dfs_valid_ap_chanspec(wlc, csa->chspec)) {
			WL_ERROR(("wl%d: %s: CSA to radar detected quiet chanspec:0x%x\n",
				wlc->pub->unit, __FUNCTION__, csa->chspec));
			err = BCME_BADCHAN;
			break;
		}

		/* Check for STA or APSTA in primary cfg */
		if (BSSCFG_STA(wlc->primary_bsscfg) && (wlc->primary_bsscfg->associated)) {
			if (BSSCFG_STA(bsscfg)) {
				...
				wlc_send_action_switch_channel_ex(csam, bsscfg, &bsscfg->BSSID,
					csa, DOT11_SM_ACTION_CHANNEL_SWITCH, 0, NULL);
				break;
		...
		if (APSTA_ENAB(wlc->pub) && (wlc->primary_bsscfg) &&
			BSSCFG_STA(wlc->primary_bsscfg)) {
			/*
			 * sta_assoc	keep_ap_up	CAC_possible_on_repeater_ap
			 * 0		0		0
			 * 0		1		1
			 * 1		0		0
			 * 1		1		1
		...
		if (WLC_IS_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc)) {
			/* already cac is going and received new csa request.
			 * Honour new csa
			 */
			WLC_CLR_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc);
		}
		/*
		 * Address MAP requirement: Perform CAC on new chanspec, and return
		 * to previous chanspec after CAC.
		 * To adddress this, properiatery information passed in mode field
		 * of CSA control block to cater new MAP requirement.
		 *
		 * Enhancement to existing CSA IOVAR syntax:
		 * wl csa <mode-2(properiatery)> <count> chanspec.
		 * Properiatery mode-2 falls back to NO_TX mode after saving properiatery
		 * information in BSSCFG_SWICTH_TO_OLD_CHANSPEC_AFTER_CAC passed from
		 * MAP via IOVAR.
		 */
		if ((csa->mode == DOT11_CSA_PROPRIETARY_MODE_DO_CAC_AND_RETURN_TO_OLD_CHAN) &&
			wlc_radar_chanspec(wlc->cmi, csa->chspec)) {

			/* Usage of mode: DOT11_CSA_PROPRIETARY_MODE_DO_CAC_AND_RETURN_TO_OLD_CHAN
			 * Switch to new chanspec, Initiate CAC on it. After completion of
			 * CAC or radar detection come back to previous chanspec.
			 */
			WLC_SET_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc);
	...
	case IOV_SVAL(IOV_MAX_CHANNEL_SWITCH_TIME): {
		/* switch time field is 3 octets only */
		if ((int_val >= (1 << 24)) || (int_val < 0)) {
			err = BCME_RANGE;
		} else {
			csam->mcst = int_val;
		}
	...
	case IOV_GVAL(IOV_MAX_CHANNEL_SWITCH_TIME): {
		*ret_int_ptr = csam->mcst;
		break;
	}
#endif /* AP */
	default:
...
wlc_csa_scb_init(void *ctx, struct scb *scb)
{
	...
	*pcsa_scb = wlc_scb_sec_cubby_alloc(wlc, scb, wlc_csa_scb_secsz(wlc, scb));
...
wlc_csa_timeout(void *arg)
{
	...
	struct scb_iter scbiter;
	struct scb *scb;
	chanspec_t local_chanspec, csa_chanspec;
	ASSERT(csa != NULL);
	...
	csa_chanspec = csa->csa.chspec;

	WL_INFORM(("wl%d: csa timeout for cfg[%d] \n", wlc->pub->unit, WLC_BSSCFG_IDX(cfg)));
...
#endif /* WL_DFS_TEST_MODE */
		{
			wlc_do_chanswitch(cfg, csa_chanspec);
			WLC_CLR_CSA_TIMER(wlc);
		}

		FOREACH_UP_AP(wlc, idx, apcfg) {
			wlc_11h_set_spect_state(wlc->m11h, apcfg, NEED_TO_SWITCH_CHANNEL, 0);
			FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
			...
			wlc_csa_train_txpower(csam, apcfg);
		}
	...
	FOREACH_UP_AP(wlc, idx, apcfg) {
		/* Send Channel Switch Indication to upper (OS) layer. */
		wlc_bss_mac_event(wlc, apcfg, WLC_E_CSA_COMPLETE_IND, NULL,
			WLC_E_STATUS_SUCCESS, 0, 0, &csa->csa.mode, sizeof(csa->csa.mode));
		wlc_csa_cb_notif(csam, cfg, csa->csa, BCME_OK, CSA_CHANNEL_CHANGE_END,
			csa_chanspec, local_chanspec);
	}
	...
	if (APSTA_ENAB(wlc->pub) && BSSCFG_STA(cfg) &&
			wlc_radar_chanspec(wlc->cmi, csa_chanspec)) {
		WL_REGULATORY(("wl%d %s PHY MUTED now csa_chanspec 0x%x local_chanspec 0x%x\n",
			wlc->pub->unit, __FUNCTION__, csa_chanspec, local_chanspec));
		wlc_mute(wlc, ON, PHY_MUTE_FOR_PREISM);
		wlc->apsta_muted = TRUE;
	}
#endif /* STA */
}
...
wlc_csa_process_channel_switch(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg)
{
	...
	ASSERT(csa != NULL);
	band = wlc->bandstate[CHSPEC_BANDUNIT(csa->csa.chspec)];
	vht_nor_he = !VHT_ENAB_BAND(wlc->pub, band->bandtype) &&
			!HE_ENAB_BAND(wlc->pub, band->bandtype);
	...
	*/
	...
	if (band && CHSPEC_IS40(csa->csa.chspec) &&
		...
		csa->csa.chspec = wf_ctlchspec20_from_chspec(csa->csa.chspec);
	}
	...
	if (!wlc_valid_chanspec_db(wlc->cmi, csa->csa.chspec)) {
		 ...
		 */
		csa->csa.chspec = wlc_next_chanspec(
					wlc->cmi,
					CH20MHZ_CHSPEC2(CH_MAX_2G_CHANNEL + 1, WL_CHANSPEC_BAND_2G),
					CHAN_TYPE_CHATTY, 0);
	...
	if (BSSCFG_STA(cfg) && BSSCFG_IS_PRIMARY(cfg) && APSTA_ENAB(wlc->pub)) {
		...
		/* timer already started for sta, Prevent new csa timer
		 * on ap cfg in wlc_csa_count_down.
		 */
		WLC_START_CSA_TIMER(wlc);
	}
	...
	/* TODO: Blocking datafifo will also block management/control frames */
	if (WL11H_ENAB(wlc) && csa->csa.mode == DOT11_CSA_MODE_NO_TX) {
		wlc_set_quiet_chanspec(wlc->cmi, wlc->home_chanspec);
		if (phy_utils_get_chanspec(WLC_PI(wlc)) == wlc->home_chanspec) {
		...
		wlc_bsscfg_t *cfg)
{
	...
	chanspec_t band = CHSPEC_BAND(ht_chanspec);
	...
	switch (wide_bw_ie->channel_width) {
		case VHT_OP_CHAN_WIDTH_80:
			if (chan1 == 0) { /* implies 80MHz */
				return wf_channel2chspec(ht_ctl, WL_CHANSPEC_BW_80, band);
			}
#if defined(WL11AC_160) || defined(WL11AC_80P80)
			if (wf_valid_160MHz_center_chan(chan1, band)) {
				return wf_channel2chspec(ht_ctl, WL_CHANSPEC_BW_160, band);
			}
	...
	uint8 *body, int body_len)
{
	...
	/* If secondary channel offset is NOT found in wlc_recv_csa_action,
	 * then try finding using reg class.
	 */
	extch = csam->sec_chan_off;
	...
	WL_REGULATORY(("wl%d: %s: mode %d, reg %d, channel %d, count %d extch %d chspec 0x%x\n",
		wlc->pub->unit, __FUNCTION__, action_hdr->b.mode, action_hdr->b.reg,
		action_hdr->b.channel, action_hdr->b.count, extch, cs_req.chspec));
	...
	if (body_len > (int)(sizeof(struct dot11y_action_ext_csa))) {
		...
		wide_bw_chan_switch_ie = (dot11_wide_bw_chan_switch_ie_t*)
			bcm_find_tlv_min_bodylen(ies, ies_len,
			DOT11_MNG_WIDE_BW_CHANNEL_SWITCH_ID,
	...
	if (!wlc_dfs_valid_ap_chanspec(wlc, cs_req.chspec)) {
		WL_ERROR(("wl%d: %s: CSA to radar detected quiet chanspec:0x%x\n",
			wlc->pub->unit, __FUNCTION__, cs_req.chspec));
		return;
	}
#ifdef WDS
...
wlc_write_wide_bw_csa_ie(wl_chan_switch_t *cs, uint8 *cp, int buflen, wlc_info_t *wlc)
{
	...
	/* For 160/80p80, update the CCFS1. Table 9-276 IEEE P802.11-REVmd/D3.4 */
	if (CHSPEC_IS160(cs->chspec)) {
...
static uint8 *
wlc_write_vs_radarch_csa_ie(uint8 *cp, uint8 *radarch, uint8 radarch_len, int buflen)
{
	dot11_brcm_radarch_ie_t *radarch_ie;

	/* perform buffer length check. */
	/* if not big enough, return buffer untouched */
	BUFLEN_CHECK_AND_RETURN((TLV_HDR_LEN + BRCM_RADARCH_IE_LEN + radarch_len), buflen, cp);

	radarch_ie = (dot11_brcm_radarch_ie_t *)cp;
	radarch_ie->id = DOT11_MNG_VS_ID;
	radarch_ie->len = BRCM_RADARCH_IE_LEN + radarch_len;

	bcopy(BRCM_PROP_OUI, radarch_ie->oui, DOT11_OUI_LEN);
	radarch_ie->type = BRCM_RADARCH_IE_TYPE;

	bcopy(radarch, radarch_ie->radarch, radarch_len);

	cp += (TLV_HDR_LEN + BRCM_RADARCH_IE_LEN + radarch_len);
...
wlc_csa_calc_ext_csa_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if ((data->cbparm->ht || data->cbparm->he) &&
		BSSCFG_AP(cfg) &&
		wlc_csa_get_csa_count(csam, cfg) > 0)
...
static uint
wlc_csa_calc_mcst_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	wlc_csa_info_t *csam = (wlc_csa_info_t *)ctx;
	wlc_bsscfg_t *cfg = data->cfg;

	if (BSSCFG_AP(cfg) && wlc_csa_get_csa_count(csam, cfg) > 0) {
		return DOT11_MAX_CHANNEL_SWITCH_TIME_IE_LEN + TLV_HDR_LEN;
	}
...
static int
wlc_csa_write_mcst_ie(void *ctx, wlc_iem_build_data_t *data)
{
	wlc_csa_info_t *csam = (wlc_csa_info_t *)ctx;
	wlc_bsscfg_t *cfg = data->cfg;

	if (BSSCFG_AP(cfg) && wlc_csa_get_csa_count(csam, cfg) > 0) {
		dot11_max_channel_switch_time_ie_t *mcst_ie;

		mcst_ie = (dot11_max_channel_switch_time_ie_t *)data->buf;
		mcst_ie->id = DOT11_MNG_ID_EXT_ID;
		mcst_ie->len = DOT11_MAX_CHANNEL_SWITCH_TIME_IE_LEN;
		mcst_ie->id_ext = EXT_MNG_MAX_CHANNEL_SWITCH_TIME_ID;

		mcst_ie->switch_time[0] = (csam->mcst & 0xff);
		mcst_ie->switch_time[1] = ((csam->mcst >> 8) & 0xff);
		mcst_ie->switch_time[2] = ((csam->mcst >> 16) & 0xff);
	}
	...
	uint8 *params, int len)
{
	...
	if ((params < end) &&
	    (tag = bcm_find_tlv(params, len, DOT11_MNG_EXT_CSA_ID)) != NULL) {
	...
	if (!csa_ie_found && (params < end) &&
	    (tag = bcm_find_tlv(params, len, DOT11_MNG_CHANNEL_SWITCH_ID)) != NULL) {
		bool err = FALSE;
		...
		if (tag && tag->len == BRCM_EXTCH_IE_LEN) {
			...
			tag = bcm_find_tlv(params, len, DOT11_MNG_EXT_CHANNEL_OFFSET);
			if (tag) {
	...
	if ((params < end) &&
		(tag = bcm_find_tlv(params, len, DOT11_MNG_CHANNEL_SWITCH_WRAPPER_ID)) != NULL) {
		...
		wide_bw_ie = (dot11_wide_bw_chan_switch_ie_t *)
			bcm_find_tlv_min_bodylen(tag->data, tag->len,
					DOT11_MNG_WIDE_BW_CHANNEL_SWITCH_ID,
					DOT11_WIDE_BW_SWITCH_IE_LEN);
		if (wide_bw_ie == NULL) {
	...
	if (csa_ie_found) {
		WL_REGULATORY(("%s: Found a CSA, count = %d csa->chspec 0x%x extch %d reg %d\n",
				__FUNCTION__, csa->count, csa->chspec, extch, csa->reg));
	}
...
static void
wlc_csa_process_vs_radarch_csa_ie(wlc_info_t *wlc, bcm_tlv_t *ies, uint32 ies_len,
		uint8 *radarch_len, uint8 **radarch)
{
	bcm_tlv_t *radarch_ie;
	uint8 radarch_subtype = BRCM_RADARCH_IE_TYPE;
	uint8 i = 0;

	radarch_ie = bcm_find_vendor_ie(ies, ies_len, BRCM_PROP_OUI,
			&radarch_subtype, 1);
	if (radarch_ie != NULL &&
			radarch_ie->len > BRCM_RADARCH_IE_LEN) {
		*radarch = ((dot11_brcm_radarch_ie_t *)radarch_ie)->radarch;
		*radarch_len = radarch_ie->len - BRCM_RADARCH_IE_LEN;
	}

	if (APSTA_ENAB(wlc->pub)) {
		/* Repeater received client csa from another repeater in daisy chain
		 * topology. Marking of OOS channel in repeater is not required.
		 */
		return;
	}

	for (i = 0; i < *radarch_len; i++) {
		uint ch = (*radarch)[i];
		wl_set_chan_info_t chan_info = {0};
		chan_info.chanspec = (WL_CHANSPEC_BW_20 | WL_CHANNEL_2G5G_BAND(ch) | ch);
	...
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
...
#endif /* SLAVE_RADAR */
	wl_chan_switch_t csa;
	uint8 *radarch = NULL, radarch_len = 0;
	uint16 band;
	...
	if (N_ENAB(wlc->pub)) {
		...
		ext_ie = bcm_find_tlv(ies, ies_len, DOT11_MNG_EXT_CHANNEL_OFFSET);
		if (ext_ie != NULL &&
	...
	if (extch == DOT11_EXT_CH_NONE) {
		chspec = (WL_CHANSPEC_BW_20 | band | csa_ie->channel);
	} else {
		chspec = wlc_ht_chanspec(wlc, csa_ie->channel, extch, cfg);
	}
...
#endif /* AP && WL11AC */
	WL_REGULATORY(("wl%d: wlc_recv_csa_action: mode %d, channel %d, count %d, "
			"extension %d chspec 0x%x\n",
			wlc->pub->unit, csa_ie->mode, csa_ie->channel, csa_ie->count,
			extch, chspec));
	if (!wlc_dfs_valid_ap_chanspec(wlc, chspec)) {
		WL_ERROR(("wl%d: %s: CSA to radar detected quiet chanspec:0x%x\n",
			wlc->pub->unit, __FUNCTION__, chspec));
		return;
	}
#ifdef SLAVE_RADAR
	if (APSTA_ENAB(wlc->pub) && !BSSCFG_AP(wlc->primary_bsscfg) && BSSCFG_AP(cfg) &&
		(wlc->primary_bsscfg->current_bss)) {
		...
		if (ap_up) {
			...
			/* Mark channels in BRCM_RADARCH_IE_TYPE vendor IE as inactive */
			if (csa_ie->mode == DOT11_CSA_MODE_NO_TX) {
				wlc_csa_process_vs_radarch_csa_ie(wlc, ies, ies_len,
						&radarch_len, &radarch);
			}
			...
			/* prepare csa info */
			wlc_csa_prepare_csa_info(wlc, wlc->primary_bsscfg, chspec,
				DOT11_CSA_MODE_NO_TX, CSA_BROADCAST_ACTION_FRAME, &csa);
			/* send rrm report and csa to upstream AP */
			wlc_csa_send_rrm_report_and_csa_to_upstream_ap(csam, wlc->primary_bsscfg,
				&csa, radarch_len, radarch);
...
wlc_csa_send_rrm_report_and_csa_to_upstream_ap(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg,
	wl_chan_switch_t *csa, uint8 radarch_len, uint8 *radarch)
{
	wlc_info_t *wlc = csam->wlc;
	...
	wlc_send_unicast_action_switch_channel(csam, cfg, &cfg->BSSID, csa,
		DOT11_SM_ACTION_CHANNEL_SWITCH, radarch_len, radarch);
...
wlc_csa_quiet_mode(wlc_csa_info_t *csam, uint8 *tag, uint tag_len)
{
	...
	csa_ie = (dot11_chan_switch_ie_t *)
	        bcm_find_tlv(tag, tag_len, DOT11_MNG_CHANNEL_SWITCH_ID);
...
#ifdef AP
/*   per AP cfg:
 * - send BCAST csa action frame.
 * - register to tbtt interrupt for beacon and probe response update.
 * - register for csa notification callback
 */
int
	...
	chanspec_t chanspec, uint8 mode, uint8 count, uint8 reg_class, uint8 frame_type)
{
	...
	wlc_csa_cb_notif(csam, cfg, csa->csa, BCME_OK, CSA_CHANNEL_CHANGE_START_INFRA, chanspec,
...
#endif /* WLMCHAN */
	/* This is wrong place to mute datafifo, csa frames for multi bss wont
	 * get opportunity to transmit in the medium.
	 * TODO: mute datafifo in caller based on mode.
	 */
	...
	uint8 radarch_len, uint8 *radarch)
{
	...
	if (action_id == DOT11_SM_ACTION_CHANNEL_SWITCH) {
		...
		if (VHT_ENAB(wlc->pub)) {
			body_len += wlc_csa_action_calc_pwr_env_ie_len(wlc, csa->chspec);
			txpwr_ie = TRUE;
		}
#endif /* WL11AC */
		if ((csa->mode == DOT11_CSA_MODE_NO_TX) && wlc_dfs_get_radar(wlc->dfs)) {
			if (radarch_len && radarch) {
				radar_vs_ie = TRUE;
				body_len += radarch_len + TLV_HDR_LEN + BRCM_RADARCH_IE_LEN;
			}
		}
	} else
	...
	if (action_id == DOT11_SM_ACTION_CHANNEL_SWITCH) {
		...
		if (txpwr_ie) {
			wlc_csa_action_write_pwr_env_ie(wlc, csa->chspec,
					cp, BUFLEN(cp, bufend));
			cp += wlc_csa_action_calc_pwr_env_ie_len(wlc, csa->chspec);
		}
#endif /* WL11AC */
		if (radar_vs_ie) {
			cp = wlc_write_vs_radarch_csa_ie(cp, radarch, radarch_len,
				BUFLEN(cp, bufend));
		}
	} else
	...
	WL_REGULATORY(("wl%d: %s: Send CSA (id=%d) Action frame\n",
	...
	const struct ether_addr *dst, wl_chan_switch_t *csa, bool protected_dual)
{
	...
	bool txpwr_ie = FALSE;
#endif /* WL11AC */
	uint8   act_category = DOT11_ACTION_CAT_PUBLIC;
	...
	/* As per Table 9-332 of IEEE Std 802.11-2016, CSA public action
	 * frame(action id 4/DOT11_PUB_ACTION_CHANNEL_SWITCH) can be a
	 * Protected Dual when MFP is enabled. If MFP is enabled, change the
	 * category from public action to protected dual public action.
	 */
	if (protected_dual) {
		act_category = DOT11_ACTION_CAT_PDPA;
	}

	if ((p = wlc_frame_get_action(wlc, dst, &cfg->cur_etheraddr,
			&cfg->BSSID, body_len, &pbody, act_category)) == NULL) {
		return;
	}
...
#endif /* WL11AC */
	action_hdr = (struct dot11_action_frmhdr *)pbody;
	action_hdr->category = act_category;
	...
	if (wide_bw_ie)
		cp = wlc_write_wide_bw_csa_ie(csa, cp, BUFLEN(cp, bufend), wlc);
	if (txpwr_ie) {
		wlc_csa_action_write_pwr_env_ie(wlc, csa->chspec, cp, BUFLEN(cp, bufend));
		cp += wlc_csa_action_calc_pwr_env_ie_len(wlc, csa->chspec);
	}
#endif /* WL11AC */
	...
	WL_REGULATORY(("wl%d: %s: Send CSA Public Action frame\n", wlc->pub->unit, __FUNCTION__));
...
wlc_send_action_switch_channel(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg)
{
	...
	csa = CSA_BSSCFG_CUBBY(csam, cfg);
	ASSERT(csa != NULL);
	if (!wlc_dfs_valid_ap_chanspec(wlc, csa->csa.chspec)) {
		WL_ERROR(("wl%d: %s: CSA to radar detected quiet chanspec:0x%x\n",
			wlc->pub->unit, __FUNCTION__, csa->csa.chspec));
		return BCME_BADCHAN;
	}
	if (csa->csa.frame_type == CSA_UNICAST_ACTION_FRAME) {
		...
		FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
			...
			if ((SCB_ASSOCIATED(scb) || SCB_LEGACY_WDS(scb)) && !SCB_ISMULTI(scb) &&
				...
				bcmerror = wlc_send_action_switch_channel_ex(csam, cfg, &scb->ea,
					&csa->csa, DOT11_SM_ACTION_CHANNEL_SWITCH, 0, NULL);
				...
				if (bcmerror < 0) {
					...
					if (N_ENAB(wlc->pub)) {
						wlc_send_public_action_switch_channel(csam,
						  scb->bsscfg, &scb->ea, &csa->csa, FALSE);
		...
		wlc_send_action_switch_channel_ex(csam, cfg, &ether_bcast,
			&csa->csa, DOT11_SM_ACTION_CHANNEL_SWITCH, 0, NULL);
		wlc_send_public_action_switch_channel(csam, cfg, &ether_bcast, &csa->csa, FALSE);
#ifdef MFP
		/* Send Protected Dual Of Public Action when MFP is capable/required */
		if (WLC_MFP_ENAB(wlc->pub) &&
				(BSSCFG_IS_MFP_CAPABLE(cfg) || BSSCFG_IS_MFP_REQUIRED(cfg))) {
...
wlc_csa_count_down(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg)
{
	...
	if (--csa->csa.count == 0) {
		...
		wlc_block_datafifo(wlc, DATA_BLOCK_QUIET, DATA_BLOCK_QUIET);
		WL_INFORM(("wl%d: start per radio single CSA timer, raised with cfg[%d]\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(cfg)));
		WLC_START_CSA_TIMER(wlc);
	}
}
