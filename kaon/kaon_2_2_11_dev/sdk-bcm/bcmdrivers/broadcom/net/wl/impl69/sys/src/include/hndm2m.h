...
extern void m2m_dump(osl_t *osh, bool verbose); /* M2M state, includeing users */
extern void m2m_regs(osl_t *osh, bool verbose); /* M2M Core registers */
extern void m2m_stats(osl_t *osh);              /* M2M statistics */
...
extern m2m_dd_key_t m2m_dd_usr_register(osl_t *osh, uint32 usr_idx, uint32 eng_idx,
                void *usr_cbdata, m2m_dd_done_cb_t usr_done_cb,
                m2m_dd_wake_cb_t usr_wake_cb, uint16 usr_wake_thr);

/** Determine available space */
extern int m2m_dd_avail(osl_t *osh, m2m_dd_key_t m2m_dd_key);

/** Request a WAKE callback on DD availability in a M2M engine */
extern int m2m_dd_wake_request(osl_t *osh, m2m_dd_key_t m2m_dd_key);

/** Initiate a m2m data transfer, returns a copy key, or M2M_INVALID. */
extern m2m_dd_cpy_t m2m_dd_xfer(osl_t *osh, m2m_dd_key_t m2m_dd_key,
          dma64addr_t *xfer_src, dma64addr_t *xfer_dst, int xfer_len,
          void *xfer_arg, uint32 xfer_op); /* NOOP, COMMIT, RESUME */

#if !defined(DONGLEBUILD)
/** m2m_core_regs::int_regs[eng_idx]::intstatus secondary ISR handler */
extern bool m2m_dd_isr(void *cbdata, bool *wantdpc);

/** m2m_core_regs::int_regs[eng_idx]::intstatus secondary DPC handler */
extern bool m2m_dd_worklet(void *cbdata);

/** Get the M2M eng pointer */
void * m2m_dd_eng_get(osl_t *osh, m2m_dd_key_t key);

/** Get the SI flag for the M2M system */
uint m2m_dd_si_flag_get(osl_t *osh);

/** Disable interrupts for a particular M2MDMA channel */
extern void m2m_intrsoff(osl_t *osh, void *m2m_eng);

/** Enable interrupts for a particular M2MDMA channel */
extern void m2m_intrson(osl_t *osh, void *m2m_eng);
