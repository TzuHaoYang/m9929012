 ...
 *
 * $Id: wlc_he.c 793165 2020-11-17 06:01:44Z $
 */
...
#define HE_PPET_MAX_RUBITMAP	0xF	/* 1111 = 0x7 => 242, 484, 996 and 2x996 */
...
BCMATTACHFN(wlc_he_init_defaults)(wlc_he_info_t *hei)
{
	...
	setbits((uint8 *)&def_6g_caps_info, sizeof(def_6g_caps_info),
		...
		HT_CAP_MIMO_PS_OFF);
	setbits((uint8 *)&def_6g_caps_info, sizeof(def_6g_caps_info),
		HE_6G_CAP_MAX_MPDU_LEN_IDX, HE_6G_CAP_MAX_MPDU_LEN_FSZ, VHT_CAP_MPDU_MAX_8K);
...
wlc_he_bss_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	*pbhei = bhei;
	...
	bhei->rts_thresh = BCM_UINT16_MAX;	/* (-1) always disabled by default */
...
wlc_he_send_htc_indication_complete(wlc_info_t *wlc, uint txstatus, void *arg)
{
	...
	if (txstatus & TX_STATUS_ACK_RCV) {
		...
		/* retry HTC indication */
		wlc_he_send_htc_indication(wlc, bsscfg, scb);
	} else {
		/* reach retry limit */
...
wlc_he_upd_omi_rxnss(wlc_he_info_t *hei, uint8 new_rxnss)
{
	...
	FOREACH_UP_AP(wlc, idx, cfg) {
		scb_iter_t scbiter;
		uint omi_bw = (CHSPEC_BW(wlc->chanspec) - WL_CHANSPEC_BW_20) >>
...
wlc_he_disassoc_state_change_cb(void *ctx, bss_disassoc_notif_data_t *notif_data)
{
	...
	if (notif_data->state == DAN_ST_DISASSOC_CMPLT) {
		...
		ASSERT((val | MHF6_AVOID_HETB_RU26));
...
wlc_he_write_cap_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (TXBF_ENAB(wlc->pub)) {
		if ((wlc_txbf_get_bfe_cap(wlc->txbf, cfg) & TXBF_HE_SU_BFE_CAP) &&
			!wlc_disable_bfe_for_smth(wlc->txbf, cfg)) {
			setbit((uint8*)&cap->phy_cap, HE_PHY_SU_BEAMFORMEE_IDX);
		} else {
			clrbit((uint8*)&cap->phy_cap, HE_PHY_SU_BEAMFORMEE_IDX);
		...
		if (wlc_txbf_get_bfr_cap(wlc->txbf) & TXBF_HE_MU_BFR_CAP) {
			...
			setbit((uint8*)&cap->phy_cap, HE_PHY_TRG_SU_BFM_FEEDBACK_IDX);
		}
		...
		if (BSSCFG_AP(cfg) && wlc_txbf_get_bfr_cap(wlc->txbf) & TXBF_HE_CQI_BFR_CAP) {
			setbit((uint8*)&cap->phy_cap, HE_PHY_TRG_CQI_FEEDBACK_IDX);
		}
...
wlc_he_write_op_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (CHSPEC_IS6G(chspec)) {
		...
		_6ghz_op_info->control |= wlc->pub->_regulatory_info << 3;
		if (ccfs0 == 0) {
...
wlc_he_read_6ghz_op_ie(he_op_6ghz_op_info_t *_6ghz_op_info, wlc_bss_info_t *bi)
{
	...
	case HE_6GHZ_OP_CTRL_BW8080_160:
		...
		if (ABS((int)(ccfs1 - ccfs0)) == CH_40MHZ_APART) {
			...
			bss_bw_flag = WLC_BSS2_160MHZ;
			center_chan = ccfs1;
		} else {
			chspec_bw = WL_CHANSPEC_BW_8080;
			bss_bw_flag = WLC_BSS2_8080MHZ;
		}
		break;
	}
 ...
 */
he_op_ie_t *
wlc_he_copy_op_ie(wlc_he_info_t *hei, uint8 *tlvs, int tlvs_len, he_op_ie_t* op_ie)
{
	bcm_tlv_t *op_ie_tlv;
	...
	he_op_ie_t *opie, he_6g_band_caps_ie_t *he_6g_caps_ie)
{
	...
	if (he_6g_caps_ie != NULL) {
		...
		/* If peer device is from BRCM, we should not ignore SMPS mode in beacon */
		if (!(scb->flags2 & SCB2_IGN_SMPS) || SCB_IS_BRCM(scb)) {
			bool mimops_enabled = FALSE;
			bool mimops_rtsmode = FALSE;
			uint8 sm_pwr_control = getbits((uint8*)&he_6g_caps_ie->caps_info,
			...
			if (wlc_stf_mimops_on_info_received(wlc, scb, FALSE,
...
wlc_he_htc_process_omi(wlc_info_t* wlc, scb_t *scb, uint32 omi_data)
{
	...
	if (nss_change) {
		if (HE_ULMU_ENAB(wlc->pub) && SCB_HE_CAP(scb) && SCB_ULOFDMA(scb)) {
	...
	if (ulmu_dis_change) {
		...
		wlc_ulmu_admit_client(wlc, scb, admit);
#ifdef TESTBED_AP_11AX
