 ...
 *
 * $Id: hnd_lbuf.h 793650 2020-12-04 09:10:40Z $
 */
...
#define lb_is_rx_corrupted(lb) ({ \
	ASSERT(lb_sane(lb)); \
	(((lb)->flags & LBF_RX_CORRUPTED) != 0); \
})

#define lb_set_rx_corrupted(lb) ({ \
	ASSERT(lb_sane(lb)); \
	(lb)->flags |= LBF_RX_CORRUPTED; \
})
...
static inline bool
lb_is_rx_corrupted(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	return ((lb->flags & LBF_RX_CORRUPTED) != 0);
}

static inline void
lb_set_rx_corrupted(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	lb->flags |= LBF_RX_CORRUPTED;
}
...
static inline void
lb_reset_rx_corrupted(struct lbuf *lb)
{
	ASSERT(lb_sane(lb));
	lb->flags &= ~LBF_RX_CORRUPTED;
}
