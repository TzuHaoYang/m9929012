 ...
 *
 * $Id: wbd_master_com_hdlr.c 792985 2020-11-10 07:44:47Z $
 */
	...
	i5_dm_clients_type *i5_assoc_sta)
{
	...
	if ((now - bss_vndr->apmetrics_timestamp) >= info->max.tm_wd_tbss) {
		/* Send AP Metrics Query message to the agent where STA associated */
		...
		for (i = 0; (i < reg_class_count) &&
			...
			for (j = 0; j < rc_chan_map[i].count; j++) {
				if (rc_chan_map[i].regclass > REGCLASS_5G_40MHZ_LAST) {
					/* use centre channel to compare. 6G 20MHz also will
					 * come here. It is ok because both center
					 * and control channel are same in 20MHz
					 */
...
static int
wbd_master_chspeclist_to_string(char *s_chlist, unsigned int s_chlist_size,
...
wbd_master_process_nonoperable_channel_update(i5_dm_interface_type *in_ifr, int dynamic)
{
	...
	foreach_i5glist_item(iter_dev, i5_dm_device_type, i5_topology->device_list) {
		...
		foreach_i5glist_item(iter_ifr, i5_dm_interface_type, iter_dev->interface_list) {
			...
			ret = wbd_master_chspeclist_to_string(s_chlist, chlist_sz,
				&ifr_vndr_data->static_exclude_chlist);

			/* Convert DYNAMIC ChspecList to Str of Hex separated by space */
			ret = wbd_master_chspeclist_to_string(d_chlist, chlist_sz,
	...
	uint8 *data, uint8 datalen)
{
	...
	assoc_sta = (wbd_assoc_sta_item_t*)sta->vndr_data;
	/* Read FC to get assoc type for parsing ie for given id and type in an (re)assoc frame */
	...
	ieee1905_chscan_report_msg *chscan_rpt)
{
	...
	for (iter_pry = 0; iter_pry < chan_count; iter_pry ++) {
		...
		WBD_DEBUG("IFR[%s] Channel[%u]: Chan_Count[%d] ",
			ifname, pry_list[iter_pry].channel, iter_pry);
...
/* Processes Requested/Independent Channel Scan Report by Controller */
static void
wbd_master_process_req_or_indep_channel_scan_rpt(unsigned char *src_al_mac, time_t ts_chscan_rpt,
	ieee1905_chscan_report_msg *chscan_rpt)
{
	WBD_DEBUG("Independent Channel Scan Report Received. Process it further.\n");
}
	...
	ieee1905_chscan_report_msg *chscan_rpt)
{
	...
	foreach_iglist_item(emt_p, ieee1905_chscan_result_item, chscan_rpt->chscan_result_list) {
		...
		/* Extract Channel */
		emt_p->chanspec_20 = CH20MHZ_CHSPEC2(emt_p->channel,
			blanket_opclass_to_band(emt_p->opclass));
	...
	foreach_iglist_item(emt_p, ieee1905_chscan_result_item, chscan_rpt->chscan_result_list) {
		...
		wbd_master_fwd_channelscan_rpt_to_acsd(src_al_mac, emt_p);
	}
	...
	ieee1905_chscan_report_msg *chscan_rpt)
{
	...
	/* Check if, It is OnBoot ChScan Report sent by the device */
	if (device_vndr->flags & WBD_DEV_FLAG_ONBOOT_CHSCAN_REQ_SENT) {

		/* Reset Onboot Channel Scan Request Send Flag */
		device_vndr->flags &= ~WBD_DEV_FLAG_ONBOOT_CHSCAN_REQ_SENT;
	...
	/* It is Requested/Independent ChScan Report : Process it IF
	 * Fresh ChScan Request sent to device OR Indep ChScan Reporting is enabled
	 */
	} else if ((device_vndr->flags & WBD_DEV_FLAG_FRESH_CHSCAN_REQ_SENT) ||
		WBD_BKT_INDEP_CHSCAN_ENAB(wbd_get_ginfo()->wbd_master->flags)) {

		/* Processes Requested/Independent ChScan Report by Controller */
		wbd_master_process_req_or_indep_channel_scan_rpt(src_al_mac,
			ts_chscan_rpt, chscan_rpt);
	}
