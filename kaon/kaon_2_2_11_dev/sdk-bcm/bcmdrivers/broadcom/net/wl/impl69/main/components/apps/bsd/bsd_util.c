...
#define WLAN_APPS_LOGS(origin, str)    wlcsm_apps_log(origin, str)
...
void bsd_set_staprio(bsd_info_t *info)
{
	...
	foreach(var_intf, bsd_ifnames, next_intf) {
		...
		while (ptr) {
			...
			strncpy(ioctl_buf, "staprio", strlen("staprio") + 1);
...
/* config info->staprio list */
int bsd_retrieve_static_maclist(bsd_info_t *info, bsd_bssinfo_t *bssinfo)
{

	int ret = BSD_OK, size, idx, count = 0;
	...
	if (maclist->count > 0 && maclist->count < 128) {
		...
		for (idx = 0; idx < maclist->count; idx++) {
			if ((info->enable_flag & BSD_FLAG_ENABLED) &&
				wl_wlif_mac_present_in_old_maclist("bsd", &maclist->ea[idx])) {
				continue;
			}
			if ((info->enable_flag & BSD_FLAG_WBD_ENABLED) &&
				wl_wlif_mac_present_in_old_maclist("wbd", &maclist->ea[idx])) {
				continue;
			}
			memcpy(&(bssinfo->static_maclist->ea[count]), &maclist->ea[idx],
				ETHER_ADDR_LEN);
			count++;
		}

		bssinfo->static_maclist->count = count;
...
void bsd_retrieve_config(bsd_info_t *info)
{
	...
	info->block_sta_timeout = BSD_BLOCK_STA_2G_TIMEOUT;
...
void bsd_bssinfo_cleanup(bsd_info_t *info)
{
	...
	for (idx = 0; idx < info->max_ifnum; idx++) {
		...
		for (bssidx = 0; bssidx < WL_MAXBSSCFG; bssidx++) {
			...
			maclist = bssinfo->maclist;
			while (maclist) {
				BSD_INFO("Clean maclist "MACF" on %s\n",
					ETHER_TO_MACF(maclist->addr), bssinfo->ifnames);
				next_mac = maclist->next;

				/* Unblock mac before removing sta. */
				wl_wlif_unblock_mac(bssinfo->wlif_hdl, maclist->addr, 0);
...
int bsd_get_bssid(bsd_bssinfo_t *bssinfo, struct ether_addr *out_bssid)
{
	...
	BSD_INFO("ifname : %s BSSID : "MACF"\n", bssinfo->ifnames, ETHERP_TO_MACF(out_bssid));
	ret = BSD_OK;
...
int bsd_info_init(bsd_info_t *info)
{
	...
	int err = BSD_FAIL;
	char bsd_ifnames[80] = "";
	char bsd_ifnames_sorted[NVRAM_MAX_VALUE_LEN] = "";
	bool rpc = FALSE;
	char *appname = "bsd";
#ifdef BCM_WBD
	int chspec = 0x00;
	...
	foreach(var_intf, bsd_ifnames_sorted, next_intf) {
		 ...
		 * DWDS AP BSS comes up little late, So getting in loop.
		 */
		counter = 0;
		while (1) {
			...
			if (bsd_get_bssid(bssinfo, &ea) != BSD_OK) {
				if (!(counter % 10)) {
					BSD_INFO("ifname[%s] get bssid failure. counter %u\n",
						name, counter);
				}
			...
			if (bsd_wl_ioctl(bssinfo, WLC_GET_SSID, &ssid, sizeof(ssid)) < 0) {
				BSD_INFO("ifname[%s] get ssid failure\n", name);
				sleep(BSD_GET_VALID_BSSID_TIMEOUT);
				continue;
			}
			ssid.SSID[ssid.SSID_len] = '\0';
			BSDSTRNCPY(bssinfo->ssid, (char *)(ssid.SSID), sizeof(bssinfo->ssid) - 1);
			BSD_INFO("ifname[%s] ssid[%s] band[%d] counter[%u]\n",
				name, ssid.SSID, band, counter);
			counter++;
		...
		bsd_retrieve_static_maclist(info, bssinfo);
				...
				if (now - sta->timestamp > info->maclist_timeout) {
					...
					if (remove) {
						...
						wl_wlif_unblock_mac(bssinfo->wlif_hdl,
							sta->addr, 0);
#ifdef CMWIFI_RDKB
						snprintf(log_buf, sizeof(log_buf), "Remove STA "MACF" from blacklist on %s by timeout\n",
			...
			while (sta) {
				...
				if ((eacmp(&(sta->addr), addr) == 0) &&
					...
					bsd_update_steering_record(info, &(sta->addr), bssinfo,
						sta->to_bssinfo,
						sta->steering_flags | BSD_STEERING_RESULT_SUCC);
#else
					bsd_update_steering_record(info, &(sta->addr), bssinfo,
...
#ifdef CMWIFI_RDKB
		bsd_update_steering_record(info, &(sta->addr), bssinfo, sta->to_bssinfo,
			sta->steering_flags | result);
#else
...
bool bsd_is_chan_passive(bsd_bssinfo_t *bssinfo)
{
	...
	FOREACH_20_SB(bssinfo->chanspec, sub_channel) {
		memset(ioctl_buf, 0, sizeof(ioctl_buf));
		strncpy(ioctl_buf, "per_chan_info", strlen("per_chan_info")+1);
...
/* Got the BTM response from STA, inform steering library */
void
bsd_bss_trans_response_event(bsd_info_t *info, char *ifname, uint8 remote, char *pkt, int bytes)
{
	bsd_bssinfo_t *bssinfo;

	BSD_ENTER();

	/* Find bss info from ifname */
	bssinfo = bsd_bssinfo_by_ifname(info, ifname, remote);
	if (!bssinfo) {
		BSD_INFO("Not found steerable ifname:%s\n", ifname);
		return;
	...
	wl_wlif_process_bss_trans_resp_event(bssinfo->wlif_hdl, pkt, bytes);
