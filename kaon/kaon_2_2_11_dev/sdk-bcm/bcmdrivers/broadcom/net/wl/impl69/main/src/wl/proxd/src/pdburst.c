 ...
 *
 * $Id: pdburst.c 793903 2020-12-14 18:06:08Z $
 */
...
#define TOF_43694_AVB_TXL       150000
...
pdburst_get_kval(pdburst_t *burstp, bool initiator, bool seq_en)
{
	...
	FTM_TRACE(("Radio-chanpec %x, burst-config-chanspec %x\n",
		phy_utils_get_chanspec(WLC_PI(wlc)), ftm_chanspec));
	...
	if (!k) {
		if (initiator) {
			wlc_phy_kvalue(WLC_PI(wlc), phy_utils_get_chanspec(WLC_PI(wlc)),
	...
	int32 *gd, int32 *adj, bool initiator, uint8 ts_id)
{
	...
	if (burstp->configp->chanspec == phy_utils_get_chanspec(WLC_PI(wlc)))
...
#else
		if ((frame_bw == TOF_BW_20MHZ_INDEX) &&
			(CHSPEC_CHANNEL(phy_utils_get_chanspec(WLC_PI(wlc))) > 14)) {
#endif /* WL_RANGE_SEQ */
	...
	d11rxhdr_t *rxh, struct ether_addr *peer)
{
	...
	*/
	...
	if (!p) {
		TOF_PRINTF(("%s:burst/ssn not created or already freed\n", __FUNCTION__));
		return;
	}
