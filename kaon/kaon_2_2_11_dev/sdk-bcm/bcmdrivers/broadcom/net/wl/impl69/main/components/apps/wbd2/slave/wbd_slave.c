 ...
 *
 * $Id: wbd_slave.c 793875 2020-12-14 07:35:29Z $
 */
...
wbd_read_blanket_slave_nvrams(wbd_blanket_slave_t *bkt_slave)
{
	char *str;
	int num = 0;
	wbd_best_bsta_wght_t weightage_cfg = {0};
	wbd_best_bsta_ident_info_t *bsta_info;
	WBD_ENTER();

	bsta_info = &bkt_slave->best_bsta_info;
	...
	WBD_INFO("AP Auto Config Search Treshold[%d] scantype_flags[%d]\n",
	...
	bsta_info->bsta_wght = (uint8)blanket_get_config_val_uint(NULL,
		WBD_NVRAM_BEST_BSTA_WGHT_IDX, 0);
	if (bsta_info->bsta_wght >= wbd_get_max_best_bsta_wght())
		bsta_info->bsta_wght = 0;
	memcpy(&bsta_info->wght_cfg, wbd_get_best_bsta_wght(bkt_slave),
		sizeof(bsta_info->wght_cfg));

	/* Get NVRAM : Best BSTA boundaries */
	str = blanket_nvram_prefix_safe_get(NULL, WBD_NVRAM_BEST_BSTA_BOUNDARIES);
	if (str) {
		num = sscanf(str, "%d %d %d %d", &bsta_info->bsta_min_phyrate,
			&bsta_info->bsta_max_phyrate,
			&bsta_info->bsta_min_rssi,
			&bsta_info->bsta_max_rssi);
	}
	if (!str || num != 4) {
		/* Default BSTA boundary */
		bsta_info->bsta_min_phyrate = WBD_BEST_BSTA_MIN_PHYRATE_BOUNDARY;
		bsta_info->bsta_max_phyrate = WBD_BEST_BSTA_MAX_PHYRATE_BOUNDARY;
		bsta_info->bsta_min_rssi = WBD_BEST_BSTA_MIN_RSSI_BOUNDARY;
		bsta_info->bsta_max_rssi = WBD_BEST_BSTA_MAX_RSSI_BOUNDARY;
	}
	blanket_log_default_nvram("%s=%d %d %d %d\n", WBD_NVRAM_BEST_BSTA_BOUNDARIES,
		bsta_info->bsta_min_phyrate,
		bsta_info->bsta_max_phyrate,
		bsta_info->bsta_min_rssi, bsta_info->bsta_max_rssi);

	bsta_info->preferred_band = (uint8)blanket_get_config_val_uint(NULL, WBD_NVRAM_PREFERRED_BH,
			WBD_DEF_PREFERRED_BH_BAND);

	WBD_INFO("tbss_wght[%d] min_phyrate_boundary[%d] max_phyrate_boundary[%d] "
		"min_rssi_boundary[%d] max_rssi_boundary[%d] preferredband[%x]\n",
		bsta_info->bsta_wght, bsta_info->bsta_min_phyrate,
		bsta_info->bsta_max_phyrate, bsta_info->bsta_min_rssi,
		bsta_info->bsta_max_rssi, bsta_info->preferred_band);

	/* read weightages from NVRAM */
	str = blanket_nvram_safe_get(WBD_NVRAM_BEST_BSTA_WGHT);
	if (str) {
		num = sscanf(str, "%d %d %d %hx",
			&weightage_cfg.w_rssi,
			&weightage_cfg.w_max_rate,
			&weightage_cfg.w_stored_bssid,
			&weightage_cfg.flags);
		if (num == 4) {
			memcpy(&bsta_info->wght_cfg, &weightage_cfg,
				sizeof(bsta_info->wght_cfg));
		} else {
			WBD_WARNING("%s : %s = %s\n", wbderrorstr(WBDE_INV_NVVAL),
				WBD_NVRAM_BEST_BSTA_WGHT, str);
		}
	}

	WBD_INFO("%s w_rssi[%d] w_phyrate[%d] w_stored_bssid[%d] flags[0x%X]\n",
		WBD_NVRAM_BEST_BSTA_WGHT, bsta_info->wght_cfg.w_rssi,
		bsta_info->wght_cfg.w_max_rate,
		bsta_info->wght_cfg.w_stored_bssid,
		bsta_info->wght_cfg.flags);
...
wbd_init_slave(wbd_info_t *info)
{
	...
	ret = wbd_add_timers(info->hdl, info, WBD_SEC_MICROSEC(info->max.tm_update_chspec),
...
main(int argc, char *argv[])
{
	...
	/* Set Process Name */
	WBDSTRNCPY(g_wbd_process_name, "Slave", sizeof(g_wbd_process_name));
	...
	/* If any of the bSTA is associated, inform IEEE1905 to create VLAN interface. Also we
	 * make virtual AP up if there are more than one STA interfaces before calling init_slave.
	 * Because, in wbd_init_slave we call the get BSSID and obviously it fails for the virtual
	 * AP interfaces where the primary STA interface is not associated. So, we are not adding
	 * the BSS into the BSS list. When M2 comes we check whether the number of BSS configured
	 * is matching with the number of M2 TLVs. If not matching, we do earse all the bSS and set
	 * it again and do rc restart. As we dont add the BSS due to BSSID fail, the rc restart
	 * continues forever.
	 */
	if ((i5_ifr = wbd_slave_is_any_bsta_associated(TRUE, TRUE))) {
		unsigned short vlan_id = 0;

		wbd_ieee1905_set_bh_sta_params(IEEE1905_BH_STA_ROAM_DISB_VAP_UP, i5_ifr);
#ifdef MULTIAPR2
		blanket_get_primary_vlan_id(i5_ifr->ifname, &vlan_id);
#endif /* MULTIAPR2 */
		WBD_INFO("ifname[%s] is associated and VLAN ID[%d]\n", i5_ifr->ifname, vlan_id);
		ieee1905_bSTA_associated_to_backhaul_ap(i5_ifr->InterfaceId, i5_ifr->ifname,
			vlan_id, vlan_id ? 1 : 0);
	} else {
		WBD_INFO("None of the bSTAs associated\n");
	}
