 ...
 *
 * $Id: wlc_airtime.c 791430 2020-09-25 15:39:00Z $
 *
 */
...
#define AIRTIME_PLCP_VHT_US		32
	...
	if (RSPEC_ISVHT(rspec) || RSPEC_ISHE(rspec)) {
		...
		ret = (rspec & WL_RSPEC_SHORT_PREAMBLE) ?
...
airtime_trailer_time(uint32 flg, uint ctl_rspec, uint ack_rspec)
{
	...
	uint ack_rate_kbps = wf_rspec_to_rate(ack_rspec);
	...
	if (ctl_rspec == ack_rspec)
	{
		...
		ctl_rate_kbps = wf_rspec_to_rate(ctl_rspec);
	}
...
{
	return airtime_packet_time_us(flg, wf_rspec_to_rate(rspec),
...
BCMFASTPATH uint
wlc_airtime_payload_time_us_rate(uint32 flg, uint32 rate, uint size_in_bytes)
{
	return airtime_payload_time_us(flg,  rate, size_in_bytes);
}
...
{
	return airtime_packet_time_us(flg, wf_rspec_to_rate(ctl_rspec),
