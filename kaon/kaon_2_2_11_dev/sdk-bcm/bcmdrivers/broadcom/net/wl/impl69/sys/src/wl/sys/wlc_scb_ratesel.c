 ...
 *
 * $Id: wlc_scb_ratesel.c 791430 2020-09-25 15:39:00Z $
 */
...
static uint wlc_scb_ratesel_scb_secsz(void *context, struct scb *scb);
...
BCMATTACHFN(wlc_scb_ratesel_attach)(wlc_info_t *wlc)
{
	...
	if ((wrsi->rsi = wlc_ratesel_attach(wlc)) == NULL) {
		WL_ERROR(("%s: wlc_ratesel_attach failed\n", __FUNCTION__));
		goto fail;
	}
	...
	cubby_params.fn_secsz = wlc_scb_ratesel_scb_secsz;
...
static uint
wlc_scb_ratesel_scb_secsz(void *context, struct scb *scb)
{
...
#ifdef WME_PER_AC_TX_PARAMS
		if (!RATELINKMEM_ENAB(wlc->pub)) {
			cubby_size = ALIGN_SIZE((AC_COUNT * wrsi->cubby_sz), sizeof(uint32));
		} else
#endif // endif
		{
			cubby_size = ALIGN_SIZE(wrsi->cubby_sz, sizeof(uint32));
		}
#ifdef WL_MU_TX
		/* itxs_cubby is always per AC */
		cubby_size += ALIGN_SIZE((AC_COUNT * wrsi->itxs_cubby_sz), sizeof(uint32));
#endif /* WL_MU_TX */
	}
...
wlc_scb_ratesel_scb_init(void *context, struct scb *scb)
{
	...
	secptr = wlc_scb_sec_cubby_alloc(wlc, scb, wlc_scb_ratesel_scb_secsz(context, scb));
	if (secptr != NULL) {
#ifdef WL_MU_TX
		cubby_info->scb_itxs_cubby = (rcb_itxs_t *) secptr;
		secptr += ALIGN_SIZE((AC_COUNT * wrsi->itxs_cubby_sz), sizeof(uint32));
#endif /* WL_MU_TX */
		cubby_info->scb_cubby = (rcb_t *) secptr;
	}
...
wlc_scb_ratesel_scb_deinit(void *context, struct scb *scb)
{
	...
	if (cubby_info->scb_itxs_cubby) {
		...
		ASSERT(cubby_info->scb_cubby != NULL);
	}
#endif /* WL_MU_TX */
	...
	ratespec_t rspec, bool rspec_overide)
{
	...
	if (!rspec_overide) {
		...
		/* Populate SGI info accrordingly */
		if (RSPEC_ISHE(rspec) && sgi_tx != AUTO) {
			 /* For HE, populate sgi info only if sgi_tx is overridden */
...
ratespec_t
wlc_scb_ratesel_get_opstats(wlc_ratesel_info_t *wrsi, struct scb *scb, uint8 ac, uint16 flags)
{
	rcb_t *state;
	state = wlc_scb_ratesel_get_cubby(wrsi, scb, ac);
	if (state == NULL) {
...
#ifdef WL_MU_TX
uint32
wlc_scb_ratesel_get_muboost(wlc_ratesel_info_t *wrsi, struct scb *scb, uint8 ac, uint8* num_users)
{
	rcb_t *state;
	uint32 rate;
	state = wlc_scb_ratesel_get_cubby(wrsi, scb, ac);
	...
	rate = wlc_ratesel_get_muboost(state, num_users);
	return rate;
}
#endif /* WL_MU_TX */
	...
	bool sgi, bool fbr, uint8 ac)
{
	...
	wlc_ratesel_upd_txstatus_normalack(state, txs, sfbl, lfbl, tx_mcs, sgi, fbr);
}
...
/* Get the ulrt rspec */
ratespec_t
wlc_scb_ratesel_get_ulrt_rspec(wlc_ratesel_info_t *wrsi, struct scb *scb, uint8 rtidx)
{
	rcb_t *state;

	state = wlc_scb_ratesel_get_cubby(wrsi, scb, 0);
	ASSERT(state);
	return wlc_ratesel_get_ulrt_rspec(state, rtidx);
}

/* Get the ulrt trssi */
uint8
wlc_scb_ratesel_get_ulrt_trssi(wlc_ratesel_info_t *wrsi, struct scb *scb)
{
	rcb_t *state;

	state = wlc_scb_ratesel_get_cubby(wrsi, scb, 0);
	ASSERT(state);
	return wlc_ratesel_get_ulrt_trssi(state);
}

void
wlc_scb_ratesel_restore_txrspec(wlc_ratesel_info_t *wrsi, struct scb *scb)
{
	rcb_t *state;

	state = wlc_scb_ratesel_get_cubby(wrsi, scb, 0);
	ASSERT(state);
	return wlc_ratesel_restore_txrspec(state);
}

bool
wlc_scb_ratesel_is_start_sp(wlc_ratesel_info_t *wrsi, struct scb *scb)
{
	rcb_t *state;

	state = wlc_scb_ratesel_get_cubby(wrsi, scb, 0);
	ASSERT(state);
	return wlc_ratesel_is_start_sp(state);
}
...
wlc_scb_ratesel_upd_txs_blockack(wlc_ratesel_info_t *wrsi, struct scb *scb, tx_status_t *txs,
	...
	uint8 mcs /* non-HT format */, bool sgi, uint8 ac)
{
	rcb_t *state;
 ...
 */
void
wlc_scb_ratesel_siso_downgrade(wlc_info_t *wlc, struct scb *scb)
{
	int i = 0;
	bool siso = FALSE;
...
wlc_scb_ratesel_init(wlc_info_t *wlc, struct scb *scb)
{
	...
	if ((wlc->stf->ldpc_tx == AUTO) || (wlc->stf->ldpc_tx == ON)) {
		if (rs_init.bw < BW_80MHZ && SCB_HT_LDPC_CAP(scb))
	...
	if (WLC_HT_GET_SGI_TX(wlc->hti) == AUTO) {
		if (SCB_HE_CAP(scb)) {
			rs_init.sgi_tx |= (SGI_BW20 | SGI_BW40 | SGI_BW80 | SGI_BW160);
		} else {
	...
	for (ac = 0; ac < WME_MAX_AC(wlc, scb); ac++) {
		...
		WL_RATE(("%s: scb 0x%p ac %d state 0x%p bw %s op_txstreams %d"
			...
			wlc->stf->op_txstreams, wlc->band->bandtype, SCB_VHT_CAP(scb),
			rs_init.vht_ratemask, SCB_HE_CAP(scb)));
...
static int
wlc_scb_ratesel_cubby_sz(void)
{
	return (sizeof(struct ratesel_cubby));
}

void
wlc_scb_ratesel_rssi_enable(rssi_ctx_t *ctx)
{
...
void
wlc_scb_ratesel_rssi_disable(rssi_ctx_t *ctx)
{
	struct scb *scb = (struct scb *)ctx;
...
int
wlc_scb_ratesel_get_rssi(rssi_ctx_t *ctx)
{
	struct scb *scb = (struct scb *)ctx;
...
} /* wlc_scb_ratesel_get_ppr_rates_bitmp */
	...
	wl_rate_histo_report_t *req, void *buf, int len)
{
	...
	if (req != NULL) {
		if (req->ver != WL_HISTO_VER_2) {
		    return BCME_VERSION;
		}
		if (req->type != WL_HISTO_TYPE_RATE_MAP2) {
	...
	if (scb->histo == NULL) {
		scb->histo = (wl_rate_histo_maps2_t *) MALLOCZ(wlc->osh,
			sizeof(wl_rate_histo_maps2_t));
		if (!scb->histo) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			return BCME_ERROR;
	...
	memcpy(&rpt->data, scb->histo, sizeof(wl_rate_histo_maps2_t));

	/* reset on read */
	bzero(scb->histo, sizeof(wl_rate_histo_maps2_t));
	scb->histo->rx.seconds = scb->histo->tx.seconds = wlc->pub->now;

	return BCME_OK;
} /* wlc_scb_get_rate_histo */

void
wlc_scb_histo_add(wl_rate_histo_map2_t *map, ratespec_t rspec, int32 count)
{
	uint8 rIdx = RSPEC2HISTO(rspec) % WL_NUM_VHT_RATES;
	uint8 bW = RSPEC2BW(rspec), bIdx = (bW > 0 && bW < 5) ? (bW - 1) : 0;
	uint8 rType = RSPEC2ENCODING(rspec);
	uint8 lidx = (rType == WL_HISTO_RATE_TYPE_LEGACY) ? (rIdx % WL_NUM_LEGACY_RATES) :
		((WL_NUM_LEGACY_RATES + WL_NUM_VHT_RATES * bIdx) + rIdx);

	if (rspec && bW && lidx < WL_HISTO_MAP2_ARR_LEN) {
		map->recent_type = rType;
		map->recent_index = lidx;
		if (count > 0) {
			map->arr[lidx] += count;
		}
...
#ifdef WLC_DTPC
uint8
wlc_scb_ratesel_change_epoch(wlc_ratesel_info_t *wrsi, struct scb *scb)
{
	rcb_t *state = SCB_RATESEL_CUBBY(wrsi, scb, LINK_BW_ENTRY);
	return wlc_ratesel_change_epoch(state);
}
#endif /* WLC_DTPC */
