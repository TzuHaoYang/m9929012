 ...
 *
 * $Id: wlc_misc.c 792827 2020-11-05 04:50:49Z $
 */
...
wlc_is_4way_msg(wlc_info_t *wlc, void *pkt, int offset, wpa_msg_t msg)
{
...
#endif /* BCM_DHDHDR && DONGLEBUILD */
	{
...
#ifdef MULTIAP
		/* For MultiAP we have started sending VLAN Tagged EAPOL. So, if the ethertype
		 * is VLAN, get the ether type after VLAN TAG
		 */
		...
		/* Check if it is EAPOL or not. If the ethertype is VLAN, check is the next
		 * ethertype is EAPOL or not
		 */
		if (!(lsh->dsap == 0xaa &&
		      lsh->ssap == 0xaa &&
		      lsh->ctl == 0x03 &&
