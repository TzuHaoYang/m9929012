 ...
 *
 * $Id: wlc_probresp.c 793063 2020-11-12 05:15:16Z $
 */
	...
	struct ether_addr *da, ratespec_t rate_override)
{
	...
	if ((p = wlc_frame_get_mgmt(wlc, FC_PROBE_RESP, da, &bsscfg->cur_etheraddr,
		...
		wlc_bcn_prb_body(wlc, FC_PROBE_RESP, bsscfg, pbody, &len);
	...
	uint8 *body, int body_len)
{
	...
	if (BSS_OCE_ENAB(cfg->wlc, cfg) &&
		isset(&(cfg->wlc->nbr_discovery_cap), WLC_NBR_DISC_CAP_PRB_RSP_CANCEL) &&
		...
		WL_INFORM(("%s: wl%d: skip probe response to ["MACF"] as Max channel time"
			...
			" Probe response\n", __FUNCTION__, mprobresp->wlc->pub->unit,
			ETHERP_TO_MACF(&hdr->sa)));
		return;
	}
	if (BSS_OCE_ENAB(cfg->wlc, cfg) && wlc_oce_find_cap_ind_attr(body, body_len)) {
...
fail_oce_check:
	...
	if (!(cfg->closednet) &&
		((CHSPEC_BANDTYPE(cfg->current_bss->chanspec) == WLC_BAND_6G) || oce_prb_rqst)) {
...
wlc_prbreq_matches(wlc_bsscfg_t *cfg, bcm_tlv_t *ssid, he_short_ssid_list_ie_t *s_ssid)
{
	...
	if (wlc_rrm_ssid_is_in_nbr_list(cfg, &tmp_ssid, s_ssid)) {
		return TRUE;
	}
#endif /* BAND6G || WL_OCE_AP */
	return FALSE;
