 ...
 *
 * $Id: phy_chanmgr_api.h 792280 2020-10-22 01:44:37Z $
 */
...
void wlc_phy_chanspec_set(phy_info_t *pi, chanspec_t chanspec);
void wlc_phy_chanspec_radio_set(phy_info_t *pi, chanspec_t newch);
bool wlc_phy_is_txbfcal(phy_info_t *pi);
bool wlc_phy_is_smth_en(phy_info_t *pi);
void wlc_phy_smth_per_nss_SUTxbf(phy_info_t *pi, uint8 uint_val);
void wlc_phy_bypass_idletssi_init(phy_info_t *pi, bool force);
...
int phy_tdcs_is_enable(phy_info_t *pi);
...
void phy_chanmgr_sr_enable(phy_info_t *pi, uint8 set_val);
void phy_chanmgr_srg_bssColor(phy_info_t *pi, uint8 set_val, uint16 BM0, uint16 BM1,
	uint16 BM2, uint16 BM3);

/*     VSDB, RVSDB Module related definitions         */
uint8 phy_chanmgr_vsdb_sr_attach_module(phy_info_t *pi, chanspec_t chan0, chanspec_t chan1);
int phy_chanmgr_vsdb_sr_detach_module(phy_info_t *pi);
uint8 phy_chanmgr_vsdb_sr_set_chanspec(phy_info_t *pi, chanspec_t chanspec, uint8 *last_chan_saved);
int phy_chanmgr_vsdb_force_chans(phy_info_t *pi, uint16 * chans, uint8 set);
