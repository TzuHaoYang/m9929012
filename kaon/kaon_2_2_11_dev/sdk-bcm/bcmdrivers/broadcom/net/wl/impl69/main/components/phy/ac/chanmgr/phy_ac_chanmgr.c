 ...
 *
 * $Id: phy_ac_chanmgr.c 793819 2020-12-10 21:46:06Z $
 */
...
static void phy_ac_chanmgr_enable_adcdiv_dacdiv(phy_ac_chanmgr_info_t *chanmgri,
	bool enable);
	...
	phy_chanmgr_info_t *cmn_info)
{
	...
	if (ACMAJORREV_40_128(pi->pubpi->phy_rev) && !phy_ac_chanmgr_attach_paprr(ac_info)) {
...
WLBANDINITFN(phy_ac_chanmgr_init_chanspec)(phy_type_chanmgr_ctx_t *ctx)
{
	...
	if (!(ACMAJORREV_4(pi->pubpi->phy_rev))) {
		if (TINY_RADIO(pi) || (ACMAJORREV_40_128(pi->pubpi->phy_rev) &&
		    (pi->u.pi_acphy->sromi->srom_low_adc_rate_en))) {
...
static const uint32 clip2_tbl_maj130[] =
	{0xa987433, 0xa987644, 0xa987655, 0xaa98766, 0xaa98777, 0xaaa9888, 0xaaaaa99};
...
static const uint8 avvmid_set47_1[13][5][8] = {
	{
	/* http://confluence.broadcom.com/pages/viewpage.action?pageId=431104865 */
		{5, 5, 5, 5, 95, 95, 90, 95},
		{4, 4, 4, 4, 75, 80, 75, 82},
...
static const uint8 avvmid_set129[8][5][8] = {
	{
	/* http://confluence.broadcom.com/pages/viewpage.action?pageId=431104865 */
...
static const uint8 avvmid_set130[4][5][8] = {
	{
		{4, 4, 4, 4, 125, 125, 125, 125},
		{4, 4, 4, 4,  90,  90,  90,  90},
		{4, 4, 4, 4,  90,  90,  90,  90},
		{4, 4, 4, 4,  90,  90,  90,  90},
		{4, 4, 4, 4,  90,  90,  90,  90}},  /* pdet_id = 0 MCM */
	{
		{3, 3, 3, 3, 142, 140, 142,  140},
		{6, 6, 6, 6,  60,  60,  60,  60},
		{6, 6, 6, 6,  60,  60,  60,  60},
		{6, 6, 6, 6,  60,  60,  60,  60},
		{6, 6, 6, 6,  60,  60,  60,  60}},  /* pdet_id = 1 MCM P120 */
	{
...
static const uint32 rfseqext_rev47_2G_dacdiv10[2][11][2] = {
	{
...
static const uint32 rfseqext_rev51_2g5g[2][11][2] = {
	/* See http://confluence.broadcom.com/x/nOIbIw */
	{
		{0x0072, 0x00069983}, {0x0032, 0x00068CC3}, {0x0012, 0x00068883},
		{0x0012, 0x00068883}, {0x0012, 0x00068883}, {0x0032, 0x00068CC3},
		{0x0072, 0x00049983}, {0x0032, 0x00048CC3}, {0x0012, 0x00048883},
		{0x000A, 0x00048E87}, {0x000A, 0x00068E87}}, /* normal ADC rate mode */
	{
		{0x0070, 0x01069983}, {0x0030, 0x02068CC3}, {0x0010, 0x02068883},
		{0x0010, 0x02068883}, {0x0010, 0x02068883}, {0x0030, 0x02068CC3},
		{0x0072, 0x00049983}, {0x0032, 0x00048CC3}, {0x0012, 0x00048883},
		{0x000A, 0x00048E87}, {0x0008, 0x02068E87}}, /* low ADC rate mode */
};
...
static const uint32 rfseqext_rev51_6gl[2][11][2] = {
	{
		...
		{0x000A, 0x00049097}, {0x000A, 0x00069097}}, /* normal ADC rate mode */
	{
		{0x0068, 0x0106B1B3}, {0x0028, 0x02069EDB}, {0x0008, 0x02069293},
		{0x0008, 0x02069293}, {0x0008, 0x02069293}, {0x0028, 0x02069EDB},
		{0x006A, 0x0004B1B3}, {0x002A, 0x00049EDB}, {0x000A, 0x00049293},
		{0x000A, 0x00049097}, {0x0008, 0x02069097}}, /* low ADC rate mode */
};
...
static const uint32 rfseqext_rev51_6gu[2][11][2] = {
	{
		...
		{0x000A, 0x000492A7}, {0x000A, 0x000692A7}}, /* normal ADC rate mode */
	{
		{0x0068, 0x0106BDE3}, {0x0028, 0x02069EF3}, {0x0008, 0x020692A3},
		{0x0008, 0x020692A3}, {0x0008, 0x020692A3}, {0x0028, 0x02069EF3},
		{0x006A, 0x0004BDE3}, {0x002A, 0x00049EF3}, {0x000A, 0x000492A3},
		{0x000A, 0x000492A7}, {0x0008, 0x020692A7}}, /* low ADC rate mode */
};

static const uint32 rfseqext_rev51_2G_dacdiv10[2][11][2] = {
	{
		{0x0072, 0x00069583}, {0x0032, 0x00068CC3}, {0x0012, 0x00068883},
		{0x0012, 0x00068883}, {0x0012, 0x00068883}, {0x0032, 0x00068CC3},
		{0x0072, 0x00049583}, {0x0032, 0x00048CC3}, {0x0012, 0x00048883},
		{0x000A, 0x00048E87}, {0x000A, 0x00068E87}}, /* normal ADC rate mode */
	{
		{0x0070, 0x01069583}, {0x0030, 0x02068CC3}, {0x0010, 0x02068883},
		{0x0010, 0x02068883}, {0x0010, 0x02068883}, {0x0030, 0x02068CC3},
		{0x0072, 0x00049583}, {0x0032, 0x00048CC3}, {0x0012, 0x00048883},
		{0x000A, 0x00048E87}, {0x0008, 0x02068E87}}, /* low ADC rate mode */
};
...
static void wlc_phy_set_reg_on_bw_change_acphy_majorrev40_128(phy_info_t *pi);
...
static void chanspec_setup_phy_ACMAJORREV_40_128(phy_info_t *pi);
...
static void chanspec_tune_phy_ACMAJORREV_40_128(phy_info_t *pi);
...
wlc_phy_set_mfLessAve_on_reset(phy_info_t *pi)
{
	if (ACREV_GE(pi->pubpi->phy_rev, 32)) {
		...
		/* Retain reset values for 4347 and 6878 */
		} else if (!(ACMAJORREV_40_128(pi->pubpi->phy_rev))) {
			FOREACH_CORE(pi, core) {
				MOD_PHYREGCE(pi, crsControlu, core, mfLessAve, 0);
	...
	*/
	...
	if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
		...
		*/
		...
		/* Disable clip2 as its causing some issues in single core jammer test */
		FOREACH_CORE(pi, core)
...
wlc_phy_set_reg_on_reset_majorrev1_3(phy_info_t *pi)
{
	...
	if ((CHSPEC_IS2G(pi->radio_chanspec) && BF3_AGC_CFG_2G(pi->u.pi_acphy)) ||
		(CHSPEC_ISPHY5G6G(pi->radio_chanspec) && BF3_AGC_CFG_5G(pi->u.pi_acphy))) {
...
static void
wlc_phy_set_reg_on_reset_majorrev40_128(phy_info_t *pi)
{
	uint16 temp;
	uint8 core;

	/* 4347, 6878 - forcing CoreConfig to 2x2, RTL default is 4x4 */
	...
	MOD_PHYREG(pi, RxStatPwrOffset0, rx_status_pwr_offset0,
		ACMAJORREV_40_128(pi->pubpi->phy_rev) ? 44 : 46);
...
wlc_phy_set_reg_on_reset_acphy(phy_info_t *pi)
{
	...
	/* Needed for 4347B0 and 6878 */
	if (ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
		/* WAR for CRDOT11ACPHY-2664: keep interlvr clock on */
		/* Needed for 4347B0 and 6878, Fixed for phy_maj44 */
		if (ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
	...
	*/
	...
	if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
		...
		FOREACH_CORE(pi, core) {
			/* No limit for now on max analog gain */
			MOD_PHYREGC(pi, HpFBw, core, maxAnalogGaindb, 64);
			MOD_PHYREGC(pi, DSSScckPktGain, core, dsss_cck_maxAnalogGaindb, 64);
		}
	}
	...
	*/
	...
	if (ACMAJORREV_47_129(pi->pubpi->phy_rev)) {
		...
		WRITE_PHYREG(pi, TxMacDelay, TXMAC_MACDELAY_DEFAULT);
	} else if (!ACMAJORREV_0(pi->pubpi->phy_rev) && !ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
	...
	if (ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
		wlc_phy_set_reg_on_reset_majorrev40_128(pi);
	}
	...
	if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
		...
		*/
		...
		// Disable MRC SIG QUAL for 6710A0 before the bringup
		...
		*/
		...
		if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
			FOREACH_CORE(pi, core) {
				...
				sz = (uint8) (sizeof(clip2_tbl_maj129) / sizeof(uint32));
				wlc_phy_table_write_acphy(pi, tbl, sz, 0x1, 32, clip2_tbl_maj129);
			}
		} else if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
			FOREACH_CORE(pi, core) {
				uint32 tbl = ACPHY_TBL_ID_MCLPAGCCLIP2TBL0 + 32 * core;
				sz = (uint8) (sizeof(clip2_tbl_maj130) / sizeof(uint32));
				wlc_phy_table_write_acphy(pi, tbl, sz, 0x1, 32, clip2_tbl_maj130);
			}
	...
	*/
	...
	if (ACMAJORREV_47_51(pi->pubpi->phy_rev) || ACMAJORREV_GE129(pi->pubpi->phy_rev)) {
		...
		if (ACMAJORREV_47_130(pi->pubpi->phy_rev)) {
			...
			/* for rev131, it needs to align with VASIP configurations */
			bfdLut_cqi_step = 0x4;
			num_decmp_rpts = 2;
		} else { // if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
			bfdLut_cqi_step = 0x5;
	...
	if (ACMAJORREV_47_51_129_130(pi->pubpi->phy_rev)) {
...
wlc_phy_set_regtbl_on_band_change_majrev_ge40(phy_info_t *pi)
{
	...
	if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
		/* 6878 should NOT follow 4347 in rfseqext programming */
...
wlc_phy_set_regtbl_on_band_change_acphy(phy_info_t *pi)
{
	 ...
	 */
	if (ACMAJORREV_3(pi->pubpi->phy_rev)) {
		uint16 enablepd = ((CHSPEC_ISPHY5G6G(pi->radio_chanspec)) ? 1 : 0);
	...
	if (ACMAJORREV_40(pi->pubpi->phy_rev) &&
		...
		/* 6878 does not want to set low power logen mode */
		...
		if (CHSPEC_IS160(pi->radio_chanspec)) {
			FOREACH_CORE(pi, core) {
				MOD_PHYREGCE(pi, sarAfeCompCtrl, core, sarAfePhaseSel,
					ACMAJORREV_130(pi->pubpi->phy_rev)? 0: 1);
			}
		}
#ifndef BCMQT
#ifdef WLVASIP
		/* this part of code should be excluded from 6715
			once it supports HW RU allocation
		*/
		ruAllocParams = (pi->radio_chanspec & WL_CHANSPEC_BW_MASK) >> WL_CHANSPEC_BW_SHIFT;
...
wlc_phy_set_sfo_on_chan_change_acphy(phy_info_t *pi, chanspec_t chanspec)
{
	...
	if (!TINY_RADIO(pi)) {
		if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
			...
			fc = wlc_phy_chan2freq_20694(pi, chanspec, &chan_info);
		} else {
...
wlc_phy_farrow_setup_nontiny(phy_info_t * pi)
{
	...
	if (ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
		...
		wlc_phy_farrow_setup_28nm(pi, 1 /* DAC rate mode */ );
		/* Configure AFE div */
		wlc_phy_radio20708_afe_div_ratio(pi, 1, 0, 0, 0);
	} else if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
		wlc_phy_farrow_setup_28nm(pi, 1 /* DAC rate mode */ );
		/* Configure AFE div */
		wlc_phy_radio20710_afe_div_ratio(pi, use_ovr);
	} else if (ACMAJORREV_37(pi->pubpi->phy_rev)) {
		wlc_phy_farrow_setup_28nm(pi, 1 /* DAC rate mode */ );
	} else {
...
chanspec_setup_regtbl_on_chan_change(phy_info_t *pi)
{
	...
	if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
		/* 6878 skip for now, need to review nvram first */
	...
	if (ACMAJORREV_130(pi->pubpi->phy_rev) && ACMINORREV_0(pi)) {
...
wlc_phy_set_reg_on_bw_change_acphy_noHWPhyReset(phy_info_t *pi)
{
	...
	if (ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
		wlc_phy_set_reg_on_bw_change_acphy_majorrev40_128(pi);
	}
...
static void
wlc_phy_set_reg_on_bw_change_acphy_majorrev40_128(phy_info_t *pi)
{
...
wlc_phy_set_reg_on_bw_change_acphy_majorrev130(phy_info_t *pi)
{
	...
	// obss reg
	WRITE_PHYREG(pi, obssreg0_c0,
		chspec_is20 ? 0x37f : chspec_is40 ? 0x37f : chspec_is80 ? 0x340 : 0x321);
	WRITE_PHYREG(pi, obssreg0_c10,
		chspec_is20 ? 0x360 : chspec_is40 ? 0x360 : chspec_is80 ? 0x286 : 0x236);
	WRITE_PHYREG(pi, obssreg0_c11,
		chspec_is20 ? 0x96 : chspec_is40 ? 0x96 : chspec_is80 ? 0xbd : 0xdb);
	WRITE_PHYREG(pi, obssreg0_c20,
		chspec_is20 ? 0x3b3 : chspec_is40 ? 0x3b3 : chspec_is80 ? 0x287 : 0x228);
	WRITE_PHYREG(pi, obssreg0_c21,
		chspec_is20 ? 0xe8 : chspec_is40 ? 0xe8 : chspec_is80 ? 0xf0 : 0xf7);
...
wlc_phy_tx_farrow_setup_28nm(phy_info_t *pi, uint16 dac_rate_mode)
{
	...
	if (ISSIM_ENAB(pi->sh->sih)) {
		if (ACMAJORREV_GE47(pi->pubpi->phy_rev) &&
			...
			/* 4347 and 6878 QT AFE CLK MODEL */
			fc = (CHSPEC_IS2G(pi->radio_chanspec)) ?
...
wlc_phy_populate_recipcoeffs_acphy(phy_info_t *pi)
{
	...
	if (ACMAJORREV_0(pi->pubpi->phy_rev) || ACMAJORREV_5(pi->pubpi->phy_rev)) {
		...
		nbanks = (ACMAJORREV_51_131(pi->pubpi->phy_rev)) ? 1 : 4;
		for (i = 0; i < nbanks; i++) {
			wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_BFECONFIG,
...
phy_ac_chanmgr_chan2freq(phy_info_t *pi, chanspec_t chanspec)
{
	...
	if (RADIOID_IS(pi->pubpi->radioid, BCM2069_ID)) {
		...
		const chan_info_radio20694_rffe_t *chan_info;
		freq = wlc_phy_chan2freq_20694(pi, chanspec, &chan_info);
...
{
	bool band5g = CHSPEC_ISPHY5G6G(pi->radio_chanspec);
...
phy_ac_chanmgr_core2core_sync_setup(phy_ac_chanmgr_info_t *chanmgri, bool enable)
{
	...
	if (ACMAJORREV_130(pi->pubpi->phy_rev) && !ISSIM_ENAB(pi->sh->sih)) {
		// FIXME: Need to review this for 6715
		// Enabled for Veloce only for now
		return;
	}
	...
	if (enable &&
		...
		/* Setup */
		if (ACMAJORREV_51_131(pi->pubpi->phy_rev)) {
			phy_ac_chanmgr_enable_adcdiv_dacdiv(pi->u.pi_acphy->chanmgri, TRUE);
		} else {
	...
	if (!enable &&
		...
		if (ACMAJORREV_51_131(pi->pubpi->phy_rev)) {
			phy_ac_chanmgr_enable_adcdiv_dacdiv(pi->u.pi_acphy->chanmgri, FALSE);
		} else {
...
phy_ac_chanmgr_core2core_sync_dac_clks(phy_ac_chanmgr_info_t *chanmgri, bool enable)
{
	...
	if (enable) {
		...
		if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
			...
			MOD_RADIO_PLLREG_20708(pi, AFEDIV_REG2, pll_num,
					afediv_pu_clk_buffers, 0);
			MOD_RADIO_PLLREG_20708(pi, AFEDIV_CFG1_OVR, pll_num,
					ovr_afediv_pu_clk_buffers, 0);

			/* Remove dac outbuf overrides */
			MOD_RADIO_PLLREG_20708(pi, AFEDIV_CFG1_OVR, pll_num,
					ovr_afediv_outbuf_dac_pu, 0);
			MOD_RADIO_PLLREG_20708(pi, AFEDIV_REG0, pll_num,
					afediv_pu_outbuf_dac, 0);
			MOD_RADIO_PLLREG_20708(pi, AFEDIV_CFG1_OVR, pll_num,
					ovr_afediv_pu_inbuf, 0);
			MOD_RADIO_PLLREG_20708(pi, AFEDIV_REG0, pll_num,
					afediv_pu_inbuf, 0);
...
static void
phy_ac_chanmgr_enable_adcdiv_dacdiv(phy_ac_chanmgr_info_t *chanmgri, bool enable)
{
	phy_info_t *pi = chanmgri->pi;
	uint8 core;

	/* Assert that phy is supported by this function */
	ASSERT(ACMAJORREV_51(pi->pubpi->phy_rev) || ACMAJORREV_131(pi->pubpi->phy_rev));

	/* This function is designed to keep dac/adc dividers powered on
	 * by radio override with reset disabled to avoid changes in clock/clock phase
	 * If ADCDIV needs to be reprogrammed, ADCDIV1 should be kept constant and
	 * ADCDIV2 should be set to 1 for Rx and a higher value as needed for Tx TSSI.
	 * This is to avoid any ADC clock phase changes at Rx->Tx or Tx->Rx
	 */
	if (enable) {
		/* Setup */
		FOREACH_CORE(pi, core) {
			/* ensure via override that dividers are powered up */
		...
		/* ensure dividers are set */
		wlc_phy_force_rfseq_acphy(pi, ACPHY_RFSEQ_RX2TX);
		OSL_DELAY(10);
		wlc_phy_force_rfseq_acphy(pi, ACPHY_RFSEQ_TX2RX);

		FOREACH_CORE(pi, core) {
			/* ensure via override that outbufs are powered up
			 * and resets are disabled
			 */
			if (ACMAJORREV_51(pi->pubpi->phy_rev)) {
				MOD_RADIO_REG_20704(pi, AFEDIV_REG0, core,
				                    afediv_dac_div_reset, 0);
				...
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
				                    ovr_afediv_outbuf_dac_pu, 1);
				MOD_RADIO_REG_20704(pi, AFEDIV_REG0, core,
				                    afediv_adc_div_reset, 0);
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
				                    ovr_afediv_adc_div_reset, 1);
				MOD_RADIO_REG_20704(pi, AFEDIV_REG0, core,
				                    afediv_outbuf_adc_pu, 1);
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
		...
		/*
		 *  - Restore direct control for adc/dac dividers
		 *  - should be done after resetcca (SWWLAN-191901)
		 */
		if (ACMAJORREV_51(pi->pubpi->phy_rev)) {
			FOREACH_CORE(pi, core) {
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
				                    ovr_afediv_outbuf_dac_pu, 0);
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
				                    ovr_afediv_adc_div_reset, 0);
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
				                    ovr_afediv_outbuf_adc_pu, 0);
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
				                    ovr_afediv_dac_div_reset, 0);
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
				                    ovr_afediv_dac_div_pu, 0);
				MOD_RADIO_REG_20704(pi, AFEDIV_CFG1_OVR, core,
				                    ovr_afediv_adc_div_pu, 0);
			}
		} else if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
			FOREACH_CORE(pi, core) {
				MOD_RADIO_REG_20710(pi, AFEDIV_CFG1_OVR, core,
...
wlc_phy_rxcore_setstate_acphy(phy_info_t *pi, uint8 rxcore_bitmask, uint8 phytxchain)
{
	...
	if (ACMAJORREV_4(pi->pubpi->phy_rev) || ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
#ifdef OCL
		if (PHY_OCL_ENAB(pi->sh->physhim)) {
...
wlc_phy_toggle_resetcca_hwbit_acphy(phy_info_t *pi)
{
	...
	/* AVS: Chips which need half clock freq during resertcca toggle */
	if (ACMAJORREV_129(pi->pubpi->phy_rev))
		reduce_clk_freq = TRUE;

	/* Backend, BP & dot11mac clock = half freq */
	if (reduce_clk_freq)
...
static void
chanspec_tune_phy_ACMAJORREV_40_128(phy_info_t *pi)
{
	phy_info_acphy_t *pi_ac = pi->u.pi_acphy;
	...
	if (!SCAN_RM_IN_PROGRESS(pi)) {
		...
		if (CCT_INIT(pi_ac) || CCT_BW_CHG(pi_ac) || CCT_BAND_CHG(pi_ac)) {
			...
			/* Enable LESI for 4347 and 6878 from the NVRAM file */
			if (CHSPEC_IS2G(pi->radio_chanspec)) {
...
wlc_phy_low_pwr_logen_setting(phy_info_t *pi, bool mimo_bf_enable)
{
	...
	/* 6878 does not need low power mode for logen */
	ASSERT(!ACMAJORREV_128(pi->pubpi->phy_rev));
...
static void
chanspec_setup_phy_ACMAJORREV_40_128(phy_info_t *pi)
{
	uint8 mac_suspend = 1;
...
{
	if (ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
		chanspec_setup_phy_ACMAJORREV_40_128(pi);
	} else if (ACMAJORREV_37(pi->pubpi->phy_rev))
...
chanspec_tune_phy(phy_info_t *pi)
{
	if (ACMAJORREV_5(pi->pubpi->phy_rev))
		...
		chanspec_tune_phy_ACMAJORREV_40_128(pi);
	else if (ACMAJORREV_47(pi->pubpi->phy_rev))
...
chanspec_papr_enable(phy_info_t *pi)
{
	...
	if (!pi_ac->chanmgri->cfg->srom_paprdis) {
		...
		} else if (!PHY_IPA(pi) && ACMAJORREV_40_128(pi->pubpi->phy_rev)) {
			enable = TRUE;
		}
	}
	if ((ACMAJORREV_32(pi->pubpi->phy_rev) || ACMAJORREV_33(pi->pubpi->phy_rev)) &&
...
chanspec_tune_txpath(phy_info_t *pi)
{
	...
	if (RADIOID_IS(pi->pubpi->radioid, BCM2069_ID)) {
		...
		const chan_info_radio20694_rffe_t *chan_info;
		freq = wlc_phy_chan2freq_20694(pi, pi->radio_chanspec, &chan_info);
	} else if (RADIOID_IS(pi->pubpi->radioid, BCM20698_ID)) {
		const chan_info_radio20698_rffe_t *chan_info;
...
chanspec_fw_enab(phy_info_t *pi)
{
	...
	/* 4347 and 6878 QT BBMULT settings */
	if (ACMAJORREV_40_128(pi->pubpi->phy_rev) && ISSIM_ENAB(pi->sh->sih)) {
		uint8 ncore_idx;
		uint16 val = 50;
	...
	uint16 phymode)
{
	...
	case  PHYMODE_BGDFS:
		if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
			...
			MOD_PHYREG(pi, RfctrlCoreLpfCT3, lpf_bq1_bw, bwidx);
			MOD_PHYREG(pi, RfctrlCoreLpfCT3, lpf_bq2_bw, bwidx);
			wlc_phy_radio20708_afe_div_ratio(pi, 1, chanspec_sc, 1, 0);
	...
	case 0:
		if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
			...
			MOD_PHYREG(pi, RfctrlOverrideLpfCT3, lpf_bq1_bw, 0);
			MOD_PHYREG(pi, RfctrlOverrideLpfCT3, lpf_bq2_bw, 0);
			wlc_phy_radio20708_afe_div_ratio(pi, 1, 0, 0, 1);
	...
	switch (phymode) {
		case  PHYMODE_BGDFS:
			if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
				...
				/* attention: when this function is brought-up for 6715, where
				 * we have a session of code for majorrev == 130, sarAfePhaseSel
				 * has to be set to 0 for 160mhz instead
				 */
...
static void wlc_phy_ul_mac_aided_war(phy_info_t *pi)
{
	 ...
	 * is already used 4347 and 6878 for MHF4_NOPHYHANGWAR.
...
static void wlc_phy_ul_mac_aided(phy_info_t *pi)
{
	...
	if (ACMAJORREV_47_51_129_130_131(pi->pubpi->phy_rev)) {
		...
		if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
			MOD_PHYREG(pi, Config6_MAC_aided_trig_frame, he_mac_aided_LESI_disable, 1);
