 ...
 *
 * $Id: wlc_mbss.c 793459 2020-11-27 08:21:54Z $
 */
...
static uint wlc_nontrans_bss_rsnie_check(wlc_bsscfg_t *trans_cfg, wlc_bsscfg_t *nontrans_cfg);
static uint wlc_nontrans_bss_extcap_check(wlc_bsscfg_t *trans_cfg, wlc_bsscfg_t *nontrans_cfg);
static uint wlc_nontrans_bss_md_check(wlc_bsscfg_t *trans_cfg, wlc_bsscfg_t *nontrans_cfg);
static uint wlc_nontrans_bss_rsnxe_check(wlc_bsscfg_t *trans_cfg, wlc_bsscfg_t *nontrans_cfg);
...
/* List of IE's which might differ in the non transmitted BSS
 * and function to decide adding to non-inheritance IE or Non transmitted profile.
 * IE's has to be added in ascending order as per spec
 */
nontrans_bcnprb_t  nt_mbss_beacon_probe[] = {
	{DOT11_MNG_RSN_ID, wlc_nontrans_bss_rsnie_check},
	{DOT11_MNG_MDIE_ID, wlc_nontrans_bss_md_check},
	{DOT11_MNG_EXT_CAP_ID, wlc_nontrans_bss_extcap_check},
	{DOT11_MNG_RSNXE_ID, wlc_nontrans_bss_rsnxe_check},
	{0, NULL}
};
...
mbss_bsscfg_up(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_IS_MAIN_AP(cfg)) {
		if (MBSSID_ENAB(wlc->pub, wlc->band->bandtype)) {
			...
			wlc->pub->bcn_tmpl_len =
				D11AC_BCN_TMPL_LEN + WLC_MAXBSSCFG * D11AC_BCN_PER_NONTX_LEN;
...
#ifdef WL_MBSSID
	if (MBSSID_ENAB(wlc->pub, wlc->band->bandtype)) {
		wlc_bsscfg_set_ext_cap(cfg, DOT11_EXT_CAP_MBSSID, 1);
		wlc_bsscfg_set_ext_cap(cfg, DOT11_EXT_CAP_COMPLETE_NONTXBSSID_PROF, 1);
	}
#endif /* WL_MBSSID */
...
wlc_mbss16_write_beacon(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		...
		tso_hdr_size = 0;
#endif /* WLTOEHW */
		len = (uint16) PKTLEN(wlc->osh, pkt) - tso_hdr_size;
...
wlc_mbss_update_probe_resp(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool suspend)
{
	...
	/* Generate probe body */
	wlc_bcn_prb_body(wlc, FC_PROBE_RESP, cfg, pbody, &len);
...
wlc_prq_directed(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, wlc_prq_info_t *info)
{
	...
	pkt = PKTGET(wlc->osh, wlc->txhroff + len, TRUE);
	if (pkt == NULL) {
...
wlc_mbss_calc_mbssid_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	FOREACH_UP_AP(wlc, i, bsscfg) {
		if (bsscfg != main_bsscfg) {
			...
			while (nt_mbss_beacon_probe[j].fn != NULL) {
				nt_mbss_beacon_probe[j].fn(main_bsscfg, bsscfg);
				j ++;
			}
			total_mbssid_len +=
	...
	wlc_iem_calc_data_t *data, uint16 *tlv_len)
{
	...
	*tlv_len += len;
	...
	ASSERT(mbssid_tag_list->mbss_tags_len <= MAX_TAG_MBSSID_IE);
	ASSERT(mbssid_tag_list->mbss_non_inheritance_len <= MAX_TAG_MBSSID_IE);
	ASSERT(mbssid_tag_list->mbss_non_inheritance_ext_len <= MAX_TAG_MBSSID_IE);

	for (i = 0; i < mbssid_tag_list->mbss_tags_len; i ++) {
		data->tag = mbssid_tag_list->mbss_tag[i];
		len = wlc_mbssid_calc_append_subelement(ctx,
			data, mbssid_tag_list->mbss_tag[i], tlv_len);
		total_mbss_len += len;
	}
	if (mbssid_tag_list->mbss_non_inheritance_ext_len ||
		mbssid_tag_list->mbss_non_inheritance_len) {
		len = wlc_mbssid_calc_append_subelement(ctx,
...
wlc_mbss_calc_multibssid_noninheritance_ie(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	return WLC_MBSS_MBSSID_NONINHERITANCE_FIXED_LEN +
		bsscfg->mbssid_tag_list->mbss_non_inheritance_len +
		bsscfg->mbssid_tag_list->mbss_non_inheritance_ext_len;
}
...
wlc_mbss_build_multibssid_index_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (data->ft == FC_BEACON) {
		...
		mbssid_index_ie->bssid_index = index;
...
wlc_mbss_build_multibssid_noninheritance_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (bsscfg->mbssid_tag_list->mbss_non_inheritance_len) {
		non_inheritance_ie->var_list[len++] =
			bsscfg->mbssid_tag_list->mbss_non_inheritance_len;
		for (i = 0; i < bsscfg->mbssid_tag_list->mbss_non_inheritance_len; i++)	{
			non_inheritance_ie->var_list[len++] =
				bsscfg->mbssid_tag_list->mbss_non_inheritance[i];
		}
	} else {
		non_inheritance_ie->var_list[len++] = 0;
	}

	if (bsscfg->mbssid_tag_list->mbss_non_inheritance_ext_len) {
		non_inheritance_ie->var_list[len++] =
			bsscfg->mbssid_tag_list->mbss_non_inheritance_ext_len;
		for (i = 0; i < bsscfg->mbssid_tag_list->mbss_non_inheritance_ext_len; i++) {
			non_inheritance_ie->var_list[len++] =
				bsscfg->mbssid_tag_list->mbss_non_inheritance_ext[i];
		}
	} else {
		non_inheritance_ie->var_list[len++] = 0;
	}
...
static uint wlc_nontrans_bss_rsnie_check(wlc_bsscfg_t *trans_cfg, wlc_bsscfg_t *nontrans_cfg)
{
	uint32 WPA_auth_trans = trans_cfg->WPA_auth;
	uint wsec_trans = trans_cfg->wsec;
	wlc_mbssid_tag_list_t *mbssid_tag_list = nontrans_cfg->mbssid_tag_list;
	uint32 WPA_auth_nontrans = nontrans_cfg->WPA_auth;
	uint wsec_nontrans = nontrans_cfg->wsec;

	ASSERT(mbssid_tag_list);

	if ((WPA_auth_trans == WPA_auth_nontrans) && (wsec_trans == wsec_nontrans)) {
		return BCME_OK;
	}

	if ((bcmwpa_includes_rsn_auth(WPA_auth_nontrans)) &&
			WSEC_ENABLED(wsec_nontrans)) {
		if (mbssid_tag_list->mbss_tags_len == MAX_TAG_MBSSID_IE) {
			return BCME_ERROR;
		}
		mbssid_tag_list->mbss_tag[mbssid_tag_list->mbss_tags_len] = DOT11_MNG_RSN_ID;
		mbssid_tag_list->mbss_tags_len++;
	} else if ((bcmwpa_includes_rsn_auth(WPA_auth_trans)) &&
			WSEC_ENABLED(wsec_trans)) {
		if (mbssid_tag_list->mbss_non_inheritance_len == MAX_TAG_MBSSID_IE) {
			return BCME_ERROR;
		}
		mbssid_tag_list->mbss_non_inheritance[mbssid_tag_list->mbss_non_inheritance_len] =
			DOT11_MNG_RSN_ID;
		mbssid_tag_list->mbss_non_inheritance_len++;
	}
	return BCME_OK;
...
static uint wlc_nontrans_bss_extcap_check(wlc_bsscfg_t *trans_cfg, wlc_bsscfg_t *nontrans_cfg)
{
	...
	if (!nontrans_cfg->ext_cap_len && !trans_cfg->ext_cap_len) {
		return BCME_OK;
	}
	if (nontrans_cfg->ext_cap_len == 0 && trans_cfg->ext_cap_len != 0) {
		if (mbssid_tag_list->mbss_non_inheritance_len == MAX_TAG_MBSSID_IE) {
			return BCME_ERROR;
		}
		mbssid_tag_list->mbss_non_inheritance[mbssid_tag_list->mbss_non_inheritance_len] =
	...
	for (i = 0; i < trans_cfg->ext_cap_len; i++) {
		if (nontrans_cfg->ext_cap[i] != trans_cfg->ext_cap[i]) {
			goto add_mbss_tag;
		}
	}
	return BCME_OK;
add_mbss_tag:
	...
	mbssid_tag_list->mbss_tag[mbssid_tag_list->mbss_tags_len] = DOT11_MNG_EXT_CAP_ID;
	mbssid_tag_list->mbss_tags_len++;
	return BCME_OK;
}
...
static uint wlc_nontrans_bss_md_check(wlc_bsscfg_t *trans_cfg, wlc_bsscfg_t *nontrans_cfg)
{
	...
	if (wlc_fbt_enabled(wlc->fbt, nontrans_cfg) && !wlc_fbt_enabled(wlc->fbt, trans_cfg)) {
		if (mbssid_tag_list->mbss_tags_len == MAX_TAG_MBSSID_IE) {
			return BCME_ERROR;
		}
		mbssid_tag_list->mbss_tag[mbssid_tag_list->mbss_tags_len] = DOT11_MNG_MDIE_ID;
		mbssid_tag_list->mbss_tags_len++;
	} else if (!wlc_fbt_enabled(wlc->fbt, nontrans_cfg) &&
			wlc_fbt_enabled(wlc->fbt, trans_cfg)) {
		if (mbssid_tag_list->mbss_non_inheritance_len == MAX_TAG_MBSSID_IE) {
			return BCME_ERROR;
		}
		mbssid_tag_list->mbss_non_inheritance[mbssid_tag_list->mbss_non_inheritance_len] =
...
static uint wlc_nontrans_bss_rsnxe_check(wlc_bsscfg_t *trans_cfg, wlc_bsscfg_t *nontrans_cfg)
{
	wlc_mbssid_tag_list_t *mbssid_tag_list = nontrans_cfg->mbssid_tag_list;
	wlc_info_t *wlc = trans_cfg->wlc;
	uint8 trans_rsnxe[16];
	uint8 nontrans_rsnxe[16];
	uint8 trans_rsnxelen = 0;
	uint8 nontrans_rsnxelen = 0;
	int i;

	ASSERT(mbssid_tag_list);
	wlc_akm_host_rsnxe(wlc, trans_cfg, (uint8 *)&trans_rsnxe[0], &trans_rsnxelen);
	wlc_akm_host_rsnxe(wlc, nontrans_cfg, (uint8 *)&nontrans_rsnxe[0], &nontrans_rsnxelen);
	if (!trans_rsnxelen && !nontrans_rsnxelen) {
		return BCME_OK;
	}
	if (nontrans_rsnxelen == 0 && trans_rsnxelen != 0) {
		if (mbssid_tag_list->mbss_non_inheritance_len == MAX_TAG_MBSSID_IE) {
			return BCME_ERROR;
		}
		mbssid_tag_list->mbss_non_inheritance[mbssid_tag_list->mbss_non_inheritance_len] =
	...
	if (nontrans_rsnxelen != trans_rsnxelen) {
		goto add_rsnxe_tag;
	} else {
		for (i = 0; i < trans_rsnxelen; i++) {
			if (trans_rsnxe[i] != nontrans_rsnxe[i]) {
				goto add_rsnxe_tag;
			}
		}
		return BCME_OK;
...
add_rsnxe_tag:
	...
	mbssid_tag_list->mbss_tag[mbssid_tag_list->mbss_tags_len] = DOT11_MNG_RSNXE_ID;
	mbssid_tag_list->mbss_tags_len++;
	return BCME_OK;
}
#endif /* WL_MBSSID */
