 ...
 *
 * $Id: wbd_ds.h 792985 2020-11-10 07:44:47Z $
 */
...
/* Blanket IDs */
#define WBD_BKT_ID_BR0			0
#define WBD_BKT_ID_BR1			1
...
#define WBD_MBO_NP_CHAN_ATTR_OPCLASS_LEN	1 /* 1 byte */
#define WBD_MBO_NP_CHAN_ATTR_PREF_LEN		1 /* 1 byte */
#define WBD_MBO_NP_CHAN_ATTR_REASON_LEN		1 /* 1 byte */
#define WBD_MBO_NP_ATTR_FIXED_LEN	(WBD_MBO_NP_CHAN_ATTR_OPCLASS_LEN + \
						WBD_MBO_NP_CHAN_ATTR_PREF_LEN + \
						WBD_MBO_NP_CHAN_ATTR_REASON_LEN)

#define WBD_MBO_NP_SUBELEMENT_OPCLASS_OFFSET		5
#define WBD_MBO_OUI_LEN					3
#define WBD_MBO_OUI_TYPE_LEN				1
#define WBD_MBO_NP_SUBELEMENT_FIXED_LEN	 (WBD_MBO_NP_ATTR_FIXED_LEN + \
						WBD_MBO_OUI_LEN + WBD_MBO_OUI_TYPE_LEN)

#define WBD_MBO_NP_SUBELEMENT_CHAN_OUI_TYPE		2

#define WBD_MBO_ATTRIBUTE_ID_OFFSET			0
#define WBD_MBO_ATTRIBUTE_LEN_OFFSET			1
#define WBD_MBO_ATTRIBUTE_OUI_OFFSET			2
#define WBD_MBO_WNM_SUBELEMENT_ID_AND_LEN		2
#define WBD_MBO_EMPTY_SUBELEMENT_LIST_LEN		4
#define WBD_MBO_WNM_NOTIFICATION_MIN_SUBELEMENT_LEN	2
#define WBD_MBO_CHAN_LIST				59
#define WFA_OUI_TYPE_MBO				0x16
#define WBD_MBO_ASSOC_ATTRIBUTE_ID_OFFSET		4
#define WBD_MBO_ASSOC_MIN_SUBELEMENT_LEN		2
#define WBD_MBO_ASSOC_SUBELEMENT_ID_AND_LEN		2
...
typedef struct wbd_tbss_ident_info {
	...
	uint8 tbss_thld_6g;		/* 6G Target BSS finding threshold index */
	...
	wbd_tbss_thld_t thld_cfg_6g;	/* 6G Threshold config for finding target BSS */
...
typedef struct wbd_fbt_bss_entry {
	...
	unsigned char blanket_id;		/* Blanket ID: br0 or br1 */
	unsigned char al_mac[MAC_ADDR_LEN];	/* AL MAC of this device */
...
typedef struct wbd_per_bss_mbo_params {
	uint8 ap_attr;	/* mbo capability i.e. cellular capable or not */
	uint8 mbo_enable;	/* mbo enable on BSS or not */
} __attribute__ ((__packed__)) wbd_per_bss_mbo_params_t;
#endif /* MULTIAPR2 */
...
typedef struct wbd_sta_mbo_np_chan_list_g {
	dll_t	node;
	uint8	opclass;
	uint8	pref;
	uint8	reason;
	uint8	list_len;
	uint8	*list; /* Max channels present in opclass eg: E-1 opclass - 131 */
} wbd_sta_mbo_np_chan_list_t_g;
...
typedef struct wbd_assoc_sta_item {
	...
	uint8 bh_opt_count;		/* Number of times TBSS tried in backhaul Optimization */
	...
	wbd_glist_t np_chan_entry_list; /* sta non preferred channel list */
	uint8 band;			/* Bands supported by STA */
...
/* Information for Slave's Channel Scan Request Per Radio timer callback */
typedef struct wbd_escan_per_radio_arg {
	char ifname[IFNAMSIZ + 2]; /* Interface on which Scan needs to be Performed */
	ieee1905_per_radio_opclass_list ifr; /* Per Radio Scan Request Object */
	int retry_count; /* Current Scan Retry Count, in case of Scan failure */
	bool last_scan_aborted; /* Flag indicating Previous Scan got Aborted due to this Req */
} wbd_escan_per_radio_arg_t;
...
#define WBD_DEV_FLAG_NVRAM_SET			0x0001	/* NVRAM set vendor message sent to
							 * device
							 */
#define WBD_DEV_FLAG_ONBOOT_CHSCAN_REQ_SENT	0x0002	/* OnBoot ChScan Request sent to device */
#define WBD_DEV_FLAG_CHAN_PREFERENCE_TIMER	0x0004	/* Created channel preference query timer */
#define WBD_DEV_FLAG_CHAN_SELECT_TIMER		0x0008	/* Created channel selection query timer */
#define WBD_DEV_FLAG_ONBOOT_CHSCAN_TIMER	0x0010	/* Created OnBoot ChScan Request timer */
#define WBD_DEV_FLAG_FRESH_CHSCAN_REQ_SENT	0x0020	/* Fresh ChScan Request sent to device */
...
typedef struct wbd_device_item {
	...
	uint8 n_hops;				/* Number of hops value for TBS */
...
#if defined(MULTIAPR2)
	ieee1905_per_radio_opclass_list indep_chscan_req; /* Independent CH Scan Req data :
		* This data would be required when ESCAN Event comes to Slave, Slave should know,
		* for which Radio+Opclass+Channels the Independent Escan was run
		*/
	ieee1905_per_radio_opclass_list last_fresh_chscan_req; /* Last Fresh CH Scan Req data :
		* This data would be required when ESCAN Event comes to Slave, Slave should know,
		* for which Radio+Opclass+Channels the last Escan was run
		*/
	time_t ts_last_fresh_chscan_recvd; /* Timestamp when Last { FRESH } ChannelScan
		* Req Recvd: Used to decide, if Current { FRESH } Channel Scan Request is Too Soon
		*/
	wbd_escan_per_radio_arg_t *chscan_ifr_arg; /* Pointer to Retry Channel Scan Timer Arg */

	chanspec_t cac_rqst_chspec;	/* chanspec from controller to intiate cac rqst */
	uint8 cac_rqst_method;	/* cac method from controller to initiate cac rqst */
	uint8 cac_rqst_action;	/* cac action from controller to initiate cac rqst */
	uint8 cac_cur_state;	/* current state of CAC on radio */
	uint8 cac_compl_status;	/* CAC cmpletion status on radio */

	uint8 cac_method_flag;	/* Continous, dedicated radio, dimention reduced and time sliced */
	uint8 n_opclass;	/* number of opclass supports in E-4 */
	uint16 old_chanspec;	/* chanspec before starting cac start request */
#endif /* MULTIAPR2 */
	uint8 dfs_event_mask[1];	/* Flags indicate wl dfs events to be ignored or not */
	ieee1905_glist_t static_exclude_chlist; /* Statically Non-operable Chanspecs
		* reported by this Agent's this Radio to Controller in Radio Capability Report.
		* Controller should avoid selecting these Chspecs by setting exclude_chlist in ACSD
		* List of i5_chspec_item_t type objects
		*/
	ieee1905_glist_t dynamic_exclude_chlist; /* Dynamically Non-Operable Chanspecs
		* reported by this Agent's this Radio to Controller in Channel Preference Report.
		* Controller should avoid selecting these Chspecs by setting exclude_chlist in ACSD
		* List of i5_chspec_item_t type objects
		*/
} wbd_ifr_item_t;
...
/* Weightage parameters for finding best backhaul STA */
typedef struct wbd_best_bsta_wght {
	int w_rssi;		/* Weight for RSSI */
	int w_max_rate;		/* Weight for maximum rate this interface can operate */
	int w_stored_bssid;	/* Weight for sotred BSSID */
	uint16 flags;		/* WBD_BEST_BSTA_WGHT_FLAG_XXX, Flags indicates which
				 * params to consider
				 */
} wbd_best_bsta_wght_t;

/* Information of Best Backhaul STA identification */
typedef struct wbd_best_bsta_ident_info {
	uint8 bsta_wght;		/* Best Backhaul STA finding weightage index */
	uint8 preferred_band;		/* Band preferred if multiple the BSTAs has same score */
	int bsta_min_phyrate;		/* Phyrate below this is normalized to 0 */
	int bsta_max_phyrate;		/* Phyrate above this is normalized to 100 */
	int bsta_min_rssi;		/* RSSI below this is normalized to 0  */
	int bsta_max_rssi;		/* RSSI above this is normalized to 100 */
	unsigned char bh_steer_bssid[MAC_ADDR_LEN];	/* Recent backhaul steer request BSSID */
	unsigned char bh_steer_sta[MAC_ADDR_LEN];	/* Recent backhaul steer request STA MAC */
	wbd_best_bsta_wght_t wght_cfg;	/* Weightage config for finding best backhaul STA */
} wbd_best_bsta_ident_info_t;

/* Information of Slave in Blanket */
struct wbd_blanket_slave {
	uint32 flags;			/* Bit flags of type WBD_BKT_SLV_FLAGS_XXX */
	uint8 scantype_flags;		/* Channel Scan Capab Flag : OnBoot / Requested / Both */
	...
	uint8 n_ap_auto_config_search_thld;	/* MAX threshold of ap auto configuration search
						 ...
						 */
	wbd_best_bsta_ident_info_t best_bsta_info; /* Best Backhaul STA Identification info */
	wbd_info_t *parent;		/* keeps track of its parent */
#if defined(MULTIAPR2)
	/* Unsuccessful Association config Policy */
...
struct wbd_master_info {
	...
	wbd_weak_sta_policy_t metric_policy_6g;	/* Metric report policy configuration for 6G band */
	wbd_weak_sta_policy_t metric_policy_bh;	/* Metric report policy configuration of backhaul */
...
/* Helper Macro to get the CtrlAgent device, with error check and error handling */
#define WBD_SAFE_GET_I5_CTRLAGENT_DEVICE(device, find_ret) \
		do { \
			(device) = wbd_ds_get_ctrlagent_i5_device((find_ret)); \
...
/* Traverse I5's Interfaces to find a BSS based on ifname, with Error check */
#define WBD_SAFE_FIND_I5_BSS_IFNAME_IN_IFR(i5_ifr, ifname, i5_bss, find_ret) \
		do { \
			(i5_bss) = wbd_ds_get_i5_bss_match_ifname_in_ifr((i5_ifr), \
					(ifname), (find_ret)); \
			if (((*(find_ret)) != WBDE_OK) || !(i5_bss)) { \
				ret = (*(find_ret)); \
				WBD_WARNING("ifname[%s] %s\n", \
					ifname, \
					wbderrorstr(ret)); \
				goto end; \
			} \
...
/* Traverse Blanket Slave's br0 list to find a Slave based on ifname */
extern wbd_slave_item_t* wbd_ds_find_slave_ifname_in_blanket_slave(wbd_blanket_slave_t *wbd_slave,
	char *ifname, int* error);
...
extern int wbd_ds_find_duplicate_sta_in_controller(struct ether_addr *sta_mac,
	struct timeval *out_assoc_time, i5_dm_clients_type **out_i5_sta);
...
/* Helper function to get the Controller's Agent Device */
i5_dm_device_type* wbd_ds_get_ctrlagent_i5_device(int *error);
...
i5_dm_bss_type* wbd_ds_get_i5_bss_match_ifname_in_ifr(i5_dm_interface_type *i5_ifr,
	char *ifname, int *error);
...
extern i5_dm_clients_type *wbd_ds_get_bh_opt_pending_sta(unsigned char *sta_mac);
