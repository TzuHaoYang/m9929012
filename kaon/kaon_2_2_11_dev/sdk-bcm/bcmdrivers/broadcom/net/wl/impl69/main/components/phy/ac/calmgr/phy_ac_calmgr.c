 ...
 *
 * $Id: phy_ac_calmgr.c 791921 2020-10-13 17:27:25Z $
 */
...
phy_ac_calmgr_singleshot(phy_info_t *pi, uint8 searchmode, acphy_cal_result_t *accal)
{
	...
	if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
		...
		if (ACMAJORREV_40(pi->pubpi->phy_rev)) {
			...
			if (!nonbf_mode && CHSPEC_ISPHY5G6G(pi->radio_chanspec)) {
	...
	if (PHY_PAPDEN(pi) && ACMAJORREV_51_129_131(pi->pubpi->phy_rev)) {
