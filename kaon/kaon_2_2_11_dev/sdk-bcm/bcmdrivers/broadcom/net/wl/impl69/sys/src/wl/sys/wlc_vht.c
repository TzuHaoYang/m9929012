 ...
 *
 * $Id: wlc_vht.c 793345 2020-11-24 06:30:15Z $
 */
...
struct wlc_vht_info {
	...
	int vht_max_pwr;	/* used by VHT Transmit Power Envelope element */
	uint16 ovrd_ratemask;	/* override vht ratemask if client does not advertise
				 * prop vht ie but include prop brcm ht ie
				 */
};

/** properties pertaining to a remote node ('scb cubby') */
...
static uint wlc_vht_scb_secsz(void *context, struct scb *scb);
static int wlc_vht_scb_init(void *context, struct scb *scb);
static void wlc_vht_scb_deinit(void *context, struct scb *scb);

static int wlc_vht_bss_init(void *context, wlc_bsscfg_t *cfg);
...
static int wlc_vht_parse_brcm_ht_ie(void *ctx, wlc_iem_parse_data_t *data);
#endif // endif
...
static int wlc_vht_parse_ext_cap_ie(void *ctx, wlc_iem_parse_data_t *data);

/* iovars */
static int
wlc_vht_doiovar(void *context, uint32 actionid,
...
static const bcm_iovar_t vht_iovars[] = {
	...
	{"bssacmode", IOV_BSS_ACMODE, (0), 0, IOVT_BOOL, 0},
	{"vht_brcm_ratemask", IOV_VHT_BRCM_RATEMASK, (0), 0, IOVT_UINT16, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
BCMATTACHFN(wlc_vht_attach)(wlc_info_t *wlc)
{
	...
	if (TDLS_ENAB(wlc->pub)) {
		...
		if (wlc_ier_add_build_fn(wlc->ier_tdls_srs, DOT11_MNG_OPER_MODE_NOTIF_ID,
			wlc_vht_calc_oper_mode_ie_len,
			wlc_vht_write_oper_mode_ie, vhti) != BCME_OK) {
			WL_ERROR(("wl%d: %s: wlc_ier_add_build_fn failed,"
				"op mode notif in setupresp\n",
				wlc->pub->unit, __FUNCTION__));
			goto fail;
		}
		...
		if (wlc_ier_add_build_fn(wlc->ier_tdls_srs, DOT11_MNG_VS_ID,
			wlc_vht_tdls_calc_brcm_ie_len,
			wlc_vht_tdls_write_brcm_ie, vhti) != BCME_OK) {
			WL_ERROR(("wl%d: %s: wlc_iem_add_build_fn failed, vht brcm ie\n",
				wlc->pub->unit, __FUNCTION__));
			goto fail;
		}
	...
	if (wlc_iem_vs_add_parse_fn_mft(wlc->iemi, arqfstbmp, WLC_IEM_VS_IE_PRIO_BRCM_HT,
	/* if (wlc_iem_vs_add_parse_fn_mft(wlc->iemi, arqfstbmp, WLC_IEM_VS_IE_PRIO_BRCM, */
	                                wlc_vht_parse_brcm_ht_ie, vhti) != BCME_OK) {
		WL_ERROR(("wl%d: %s: wlc_vht_parse_brcm_ht_ie, brcm ie in assocreq\n",
		          wlc->pub->unit, __FUNCTION__));
		goto fail;
	}
#endif /* AP */
	...
	uint16 ext_cap_parse_fstbmp =
	        ...
	        FT2BMP(FC_BEACON) |
	        0;
	if (wlc_iem_add_parse_fn_mft(wlc->iemi, ext_cap_parse_fstbmp, DOT11_MNG_EXT_CAP_ID,
			wlc_vht_parse_ext_cap_ie, vhti) != BCME_OK) {
		WL_ERROR(("wl%d: %s wlc_iem_add_parse_fn failed, ext cap ie\n",
		          wlc->pub->unit, __FUNCTION__));
		goto fail;
	}
	}
	...
	cubby_params.context = vhti;
	cubby_params.fn_init = wlc_vht_scb_init;
	cubby_params.fn_deinit = wlc_vht_scb_deinit;
	cubby_params.fn_secsz = wlc_vht_scb_secsz;
	cubby_params.fn_dump = wlc_vht_scb_dump;
	vhti->scb_handle = wlc_scb_cubby_reserve_ext(wlc, sizeof(struct wlc_vht_scb_cubby),
		&cubby_params);
	...
	if (wlc_bsscfg_cubby_reserve(wlc, 0, wlc_vht_bss_init, NULL, NULL, (void *)vhti) < 0) {
...
static uint
wlc_vht_scb_secsz(void *context, struct scb *scb)
{
	uint size = 0;
	if (scb && !SCB_INTERNAL(scb)) {
		size = sizeof(struct wlc_vht_scb_info);
	}
	return size;
}
...
wlc_vht_scb_init(void *context, struct scb *scb)
{
	...
	SCB_VHT_INFO(vhti, scb) = wlc_scb_sec_cubby_alloc(vhti->wlc, scb,
			wlc_vht_scb_secsz(context, scb));
...
wlc_vht_scb_deinit(void *context, struct scb *scb)
{
	...
	if (SCB_VHT_INFO(vhti, scb)) {
		wlc_scb_sec_cubby_free(vhti->wlc, scb, SCB_VHT_INFO(vhti, scb));
...
wlc_vht_upd_txbf_cap(wlc_vht_info_t *vhti, uint8 bfr, uint8 bfe, uint32 *cap)
{
	...
	*cap &= ~(VHT_CAP_INFO_SU_BEAMFMR |
	...
	if (bfr || bfe) {
		*cap |= (wlc_phy_get_bfe_ndp_recvstreams(WLC_PI(wlc)) <<
...
#define WLC_VHT_FEATURES_2G_ADV_ENAB		0x00000008
	...
	switch (actionid) {
		case IOV_GVAL(IOV_VHTFEATURES):
			...
			/* VHT features return: bit 0 2G VHT,bit1 VHT ext rates, set if enabled */
				...
				*ret_int_ptr = 0;
				...
				if (WLC_VHT_FEATURES_GET(wlc->pub, WL_VHT_FEATURES_2G_ADV))
						*ret_int_ptr |= WLC_VHT_FEATURES_2G_ADV_ENAB;
				/*
		...
		case IOV_SVAL(IOV_VHTFEATURES):
			if (!WLC_PHY_VHT_CAP(wlc->band)) {
				...
				err = BCME_BADOPTION;
				break;
			} else if (!BAND_ENABLED(wlc, BAND_2G_INDEX) &&
				(int_val & WLC_VHT_FEATURES_2G_ENAB)) {
				/* Abort if we try to enable 2G VHT on a non-2G capable device */
				err = BCME_BADOPTION;
				break;
			...
			/* VHT features set value:bit 0 2G VHT,bit1 VHT ext rates, set if enabled */
				...
				if (int_val & WLC_VHT_FEATURES_2G_ADV_ENAB) {
					WLC_VHT_FEATURES_SET(wlc->pub, WL_VHT_FEATURES_2G_ADV);
				}
				else {
					WLC_VHT_FEATURES_CLR(wlc->pub, WL_VHT_FEATURES_2G_ADV);
				}
				/*
		...
		case IOV_SVAL(IOV_VHTMODE):
	       ...
	       case IOV_SVAL(IOV_BSS_ACMODE):
			...
			if (int_val != 0) {
			       bsscfg->flags &= ~WLC_BSSCFG_VHT_DISABLE;
		       } else
				bsscfg->flags |= WLC_BSSCFG_VHT_DISABLE;
			break;

	       case IOV_GVAL(IOV_BSS_ACMODE):
			*ret_int_ptr = (int32)(((bsscfg->flags & WLC_BSSCFG_VHT_DISABLE) != 0) ?
				FALSE : TRUE);
			break;
		case IOV_GVAL(IOV_VHT_BRCM_RATEMASK):
			*ret_int_ptr = (int32)(vhti->ovrd_ratemask);
			break;
		case IOV_SVAL(IOV_VHT_BRCM_RATEMASK):
			vhti->ovrd_ratemask = int_val & WL_VHT_FEATURES_RATES_ALL;
			break;
		default:
			err = BCME_UNSUPPORTED;
			break;
...
BCMATTACHFN(wlc_vht_init_defaults)(wlc_vht_info_t *vhti)
{
	...
	/* VHT Capabilities IE (802.11 sec 8.4.2.160) */
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		cap = VHT_CAP_MPDU_MAX_8K;
	} else {
		cap = VHT_CAP_MPDU_MAX_11K; /* AMPDU limit for AMSDU agg, support max 11KB */
	}
	...
	} else if (WLC_8080MHZ_CAP_PHY(wlc)) {
	...
	/* Extended NSS BW Support has to be enabled/updated when 160/80p80
	 * is operational at reduced NSS than max operational NSS.
	 * Table 9-274 IEEE P802.11-REVmd/D3
	 */
	...
	vhti->ovrd_ratemask = WL_VHT_FEATURES_STD_RATES_5G;
} /* wlc_vht_init_defaults */
 ...
 */
void
wlc_vht_update_ampdu_cap(wlc_vht_info_t *vhti, uint8 rx_factor, uint8 mpdu_density)
{
	WL_TRACE(("wl%d: %s(%04x)\n", vhti->wlc->pub->unit, __FUNCTION__, rx_factor));

	/* Update the Max AMPDU Rx Exponent */
	vhti->vht_cap.vht_cap_info &= ~(VHT_CAP_INFO_AMPDU_MAXLEN_EXP_MASK);
	vhti->vht_cap.vht_cap_info |= (rx_factor << VHT_CAP_INFO_AMPDU_MAXLEN_EXP_SHIFT);
}
#endif /* WLAMPDU */

void wlc_vht_update_mcs_cap_ext(wlc_vht_info_t *vhti)
{
	wlc_info_t *wlc = vhti->wlc;
	wlc->default_bss->vht_capabilities = vhti->vht_cap.vht_cap_info;
	wlc->default_bss->vht_rxmcsmap = vhti->vht_cap.rx_mcs_map;
...
wlc_vht_update_mcs_cap(wlc_vht_info_t *vhti)
{
	...
	if (WLC_MIMOPS_ENAB(wlc->pub)) {
		WL_STF_MIMO_PS_INFO((" WLC STF MIMO PS : rx mcs map %x tx mcs map %x default %x\n",
...
wlc_vht_update_defaults(wlc_vht_info_t *vhti)
{
		...
		wlc->default_bss->vht_rxmcsmap = vhti->vht_cap.rx_mcs_map;
		wlc->default_bss->vht_txmcsmap = vhti->vht_cap.tx_mcs_map;
		wlc->default_bss->vht_txmcsmap_prop = wlc->default_bss->rateset.vht_mcsmap_prop;
}

/* copy ie int passed in struct, then fix endianness  */
vht_cap_ie_t *
wlc_vht_copy_cap_ie(wlc_vht_info_t *vhti, uint8 *tlvs, int tlvs_len, vht_cap_ie_t* cap_ie)
{
	...
	ASSERT(cap_ie);
	cap_ie_tlv = bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_VHT_CAP_ID);
	if (cap_ie_tlv) {
		if (cap_ie_tlv->len >= VHT_CAP_IE_LEN) {
			...
			IEM_INFO(("wl%d: %s: std len %d does not match %d\n",
...
/**
 * This function is e.g. called by wlc_rx.c on beacon reception, or by the assoc module, as well as
 * via IEM callbacks when scanning. Note: does *not* copy the typ nor len field of the TLV.
 *
 * @param[in]  vhti
 * @param[in]  tlvs
 * @param[in]  tlvs_len
 * @param[out] op_ie              IE into which the 'data' field of the source IE is copied
 * @return     pointer to vht_op_ie
 */
vht_op_ie_t *
wlc_vht_copy_op_ie(wlc_vht_info_t *vhti, uint8 *tlvs, int tlvs_len, vht_op_ie_t* op_ie)
{
	bcm_tlv_t *op_ie_tlv;
	...
	op_ie_tlv = bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_VHT_OPERATION_ID);
	if (op_ie_tlv) {
...
uint8 *
wlc_vht_read_features_ie(wlc_vht_info_t *vhti, uint8 *tlvs, int tlvs_len,
 ...
 */
static int
wlc_vht_bss_init(void *ctx, wlc_bsscfg_t *cfg)
{

	wlc_bsscfg_set_ext_cap(cfg, DOT11_EXT_CAP_OPER_MODE_NOTIF, TRUE);

	/* XXX, let's configure maximum number of MSDUs in an A-MSDU, as this
	 * helps achieve better throughput specialy for smaller packets. Also
	 * when HWA_RXFILL_BUILD is enabled.  During the debugging I don't
	 * see the wlc_ampdu_resp_timeout continuous sweep out when I disable
	 * HWA_RXFILL_BUILD.
	*/
	/* Bit 63 - 64
	...
	*/
	/* Set to 3 for 8 MSDU in AMSDU maximum */
	wlc_bsscfg_set_ext_cap(cfg, DOT11_EXT_CAP_NUM_MSDU, TRUE);
	wlc_bsscfg_set_ext_cap(cfg, DOT11_EXT_CAP_NUM_MSDU+1, TRUE);
...
wlc_write_vht_cap_ie(wlc_bsscfg_t *cfg, vht_cap_ie_t *cap_ie)
{
	...
	uint32 cap;
	const uint8 bw_cap = wlc->band->bw_cap;
...
#endif // endif
	{
		/* If MU receive not compiled/disabled, don't advertise MU BFE capability */
		cap &= ~VHT_CAP_INFO_MU_BEAMFMEE;
	}
	WL_TRACE(("%s: cap info=%04x\n", __FUNCTION__, cap));
	 ...
	 * Table 9-274 of IEEE P802.11-REVmd/D3.4, July 2020
	 */
	/* bw <= 80MHz */
	cap |= VHT_CAP_CHAN_WIDTH_SUPPORT_MANDATORY;
	...
	if (WL_BW_CAP_160MHZ(bw_cap) && !no160_inlocale) {
		/* 43684 can support 160Mhz @ 4x4 */
		if (WLC_PHY_160_FULL_NSS(wlc)) {
			cap |= (!WLC_8080MHZ_CAP_PHY(wlc)) ? VHT_CAP_CHAN_WIDTH_SUPPORT_160 :
				VHT_CAP_CHAN_WIDTH_SUPPORT_160_8080;
			wlc_vht_set_ext_nss_bw_sup(wlc->vhti, 0);
		} else if (WLC_PHY_160_HALF_NSS(wlc)) {
			/* 4366 can support 160Mhz @ 2x2 */
			cap &= ~(VHT_CAP_CHAN_WIDTH_SUPPORT_160);
...
wlc_vht_tdls_calc_brcm_ie_len(void * ctx, wlc_iem_calc_data_t * data)
{
	...
	if (encap & VHT_IE_ENCAP_VHTIE) {
		...
		case TDLS_SETUP_CONFIRM:
			...
			break;
		default:
			WL_ERROR(("wl%d: %s: Unsupported action_field type %d\n", WLCWLUNIT(wlc),
...
wlc_vht_tdls_parse_brcm_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	if ((CHSPEC_IS2G(cfg->current_bss->chanspec)) || WLC_VHT_FEATURES_RATES(wlc->pub)) {
		prop_tlv = wlc_vht_read_features_ie(vhti, data->ie, data->ie_len,
		                                    &vht_ratemask, &prop_tlv_len, bi);

		if (prop_tlv == NULL)
			return BCME_OK;

		if (wlc_vht_copy_cap_ie(vhti, prop_tlv, prop_tlv_len, &vht_cap_ie) != NULL)
			wlc_vht_parse_cap(&vht_cap_ie, bi);

		if (wlc_vht_copy_op_ie(vhti, prop_tlv, prop_tlv_len, &vht_op_ie) != NULL) {
...
wlc_vht_ie_len_csa_wrapper(wlc_bsscfg_t *cfg, uint32 flags)
{
	...
	ielen = TLV_HDR_LEN + sizeof(dot11_vht_transmit_power_envelope_ie_t);
		...
		chanspec_t ht_chanspec, uint8 ht_ccfs2)
{
	...
	chanspec_t ht_band = CHSPEC_BAND(ht_chanspec);
	chanspec_t ret_chspec = INVCHANSPEC;  /**< chspec that is returned by this function */
	...
	if (ht_band == WL_CHANSPEC_BAND_2G && vht_op_ie->chan_width != VHT_OP_CHAN_WIDTH_20_40) {
		return INVCHANSPEC; /* 2g band does not support > 40MHz bandwidth */
	}
	...
	switch (vht_op_ie->chan_width) {
		case VHT_OP_CHAN_WIDTH_80:
			if (ccfs1 == 0) { /* implies 80MHz */
				ret_chspec = wf_channel2chspec(ht_ctl, WL_CHANSPEC_BW_80, ht_band);
			}
#if defined(WL11AC_160) || defined(WL11AC_80P80)
			else if (1 &&
#ifndef BAND6G_USING_VHT_IES
				(ht_band == WL_CHANSPEC_BAND_5G) &&
#endif /* BAND6G_USING_VHT_IES */
				wf_valid_160MHz_center_chan(ccfs1, ht_band)) {
				ret_chspec = wf_channel2chspec(ht_ctl, WL_CHANSPEC_BW_160, ht_band);
			} else {
				ret_chspec = wf_chspec_get8080_chspec(ht_ctl, ccfs0, ccfs1);
			}
			break;
		...
		default:
			ret_chspec = ht_chanspec;
			break;
	}

	if (ret_chspec == 0) {
		ret_chspec = INVCHANSPEC;
	}

	return ret_chspec;
} /* wlc_vht_op_ie_chanspec */
		...
		chanspec_t vht_op_ie_chanspec)
{
	...
	return wf_channel2chspec(wf_chspec_ctlchan(vht_op_ie_chanspec), omn_bw,
	                                           CHSPEC_BAND(vht_op_ie_chanspec));
}
#endif /* WL_MODESW */
...
wlc_vht_upd_rate_mcsmap_prop_ex(wlc_vht_info_t *vhti, struct scb *scb, uint16 rxmcsmap)
{
	...
	if ((cubby_info->ratemask & WL_VHT_FEATURES_1024QAM) &&
		(WLC_VHT_FEATURES_GET(vhti->wlc->pub, WL_VHT_FEATURES_1024QAM))) {
...
wlc_vht_upd_rate_mcsmap(wlc_vht_info_t *vhti, struct scb *scb)
{
	...
	if (BSSCFG_AP(cfg)) {
		rxmcsmap = wlc_rateset_vht_filter_mcsmap(rxmcsmap,
...
wlc_vht_calc_cap_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (BAND_2G(vhti->wlc->band->bandtype) &&
		!WLC_VHT_FEATURES_2G_ADV(vhti->wlc->pub))
		return 0;
...
wlc_vht_write_cap_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (BAND_2G(wlc->band->bandtype) &&
		!WLC_VHT_FEATURES_2G_ADV(wlc->pub)) {
		return BCME_OK;
	}
...
wlc_vht_tdls_parse_op_ie(void *ctx, wlc_iem_parse_data_t *parse)
{
	...
	if (VHT_ENAB_BAND(wlc->pub, wlc->band->bandtype) &&
		wlc_vht_copy_op_ie(vhti, parse->ie, parse->ie_len, &vht_op_ie) && bi->chanspec) {
		chspec = wlc_vht_chanspec(vhti, &vht_op_ie, bi->chanspec, FALSE, FALSE,
				bi->ht_ccfs2);

		if (chspec != INVCHANSPEC) {
			bi->chanspec = chspec;
			bi->flags2 |= WLC_BSS2_80MHZ;
		}
	}
...
#ifdef AP
/* call back function to check if there is any legacy brcm HT IEs
 * If yes, then use override brcm proprietary ratemask if following
 * conditions are all true:
 * 1) override brcm ratemask is non zero (iovar'able)
 * 2) this client advertises legacy brcm IE
 * 3) it is a vht client
 * 4) it does not advertise proprietary vht IE
 * 5) its max rx nss is 3
 */
static int
wlc_vht_parse_brcm_ht_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	wlc_vht_info_t *vhti = (wlc_vht_info_t *)ctx;
	wlc_iem_ft_pparm_t *ft;
	vht_cap_ie_t *vht_cap;
	uint8 max_nss;

	if (vhti->ovrd_ratemask == 0) {
		return BCME_OK;
	}
	...
	if (data->pparm == NULL || !data->pparm->vht) {
		return BCME_OK;
	}

	if (((ft = data->pparm->ft) == NULL) || (ft->assocreq.vht_cap_ie == NULL)) {
		return BCME_OK;
	}

	vht_cap = (vht_cap_ie_t *)(ft->assocreq.vht_cap_ie + TLV_BODY_OFF); // skip 1st 2B to body
	max_nss = VHT_MAX_SS_SUPPORTED(vht_cap->rx_mcs_map);

	WL_RATE(("%s frame_type 0x%x vht_ratemask 0x%x rx_mcs_map 0x%x max_nss %d\n", __FUNCTION__,
		data->ft, ft->assocreq.vht_ratemask, vht_cap->rx_mcs_map, max_nss));

	if (ft->assocreq.vht_ratemask == 0 && max_nss == 3) {
		/* It is a BRCM client, vht-capable, 3x3, but does not
		 * advertise prop VHT IE. Go ahead to enable proprietary rate_mask
		 */
		ft->assocreq.vht_ratemask = vhti->ovrd_ratemask;
	}
	return BCME_OK;
}
...
wlc_vht_parse_brcm_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	case FC_REASSOC_REQ:
		...
		if (BAND_2G(wlc->band->bandtype) || WLC_VHT_FEATURES_RATES(wlc->pub)) {
			...
			prop_tlv =
			     wlc_vht_read_features_ie(vhti, data->ie, data->ie_len,
			...
			ftpparm->assocreq.vht_cap_ie = (uint8 *)
			        bcm_find_tlv(prop_tlv, prop_tlv_len, DOT11_MNG_VHT_CAP_ID);
			ftpparm->assocreq.vht_op_ie = (uint8 *)
			        bcm_find_tlv(prop_tlv, prop_tlv_len, DOT11_MNG_VHT_OPERATION_ID);
		}
		break;
	}
...
wlc_vht_oper_mode_ie_allowed(uint16 ft, wlc_iem_cbparm_t *cbparm)
{
	...
	case FC_REASSOC_REQ:
		...
		ASSERT(scb != NULL);
		if (ftcparm->assocreq.target->flags2 &  WLC_BSS2_OPER_MODE) {
			ret = TRUE;
		}
		break;
	case FC_ASSOC_RESP:
...
/** VHT/HE Operating Mode Notification element DOT11_MNG_OPER_MODE_NOTIF_ID */
static uint
...
wlc_vht_parse_oper_mode_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	if (!data->pparm->ht && !data->pparm->vht && !data->pparm->he)
		return BCME_OK;
	...
	dot11_oper_mode_notif_ie_t *ie)
{
	...
	tlv = bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_OPER_MODE_NOTIF_ID);
	if (tlv) {
...
#ifdef WL11AX
		/* First call he module to update rateset based upon oper_mode, then the vht part
		 * since he module will not call rate init.
		 */
		if (SCB_HE_CAP(scb)) {
			wlc_he_recv_af_oper_mode(wlc, scb, oper_mode);
...
wlc_vht_parse_cap(vht_cap_ie_t *cap, wlc_bss_info_t *bi)
{
	if (cap != NULL) {
		...
		if (vht_cap & VHT_CAP_INFO_SGI_80MHZ)
			bi->flags2 |= WLC_BSS2_SGI_80;

		if (vht_cap & VHT_CAP_INFO_SGI_160MHZ)
			bi->flags2 |= WLC_BSS2_SGI_160;
...
wlc_vht_scan_parse_cap_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	if (wlc_vht_copy_cap_ie(vhti, data->ie, data->ie_len, &cap) != NULL)
...
wlc_vht_scan_parse_brcm_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	uint8 vht_ratemask = 0;
	wlc_iem_ft_pparm_t *ftpparm = data->pparm->ft;
	uint beacon_band = ftpparm->scan.bandtype;
	...
	if (BAND_2G(beacon_band) || WLC_VHT_FEATURES_RATES(wlc->pub)) {
		...
		if (wlc_vht_copy_cap_ie(vhti, prop_tlv, prop_tlv_len, &cap) != NULL)
			wlc_vht_parse_cap(&cap, ftpparm->scan.result);
	}
...
wlc_vht_scan_parse_op_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	if ((op_p = wlc_vht_copy_op_ie(vhti, data->ie, data->ie_len, &op_ie)) != NULL) {
		...
		if (CHSPEC_IS80(bi->chanspec))
			...
			bi->flags2 |= WLC_BSS2_8080MHZ;
		else if (CHSPEC_IS160(bi->chanspec))
			bi->flags2 |= WLC_BSS2_160MHZ;
	}

	return BCME_OK;
} /* wlc_vht_scan_parse_op_ie */

#ifdef WL_STF_ARBITRATOR
/* after setting arbitrated chains need to update mcsmap */
void
wlc_vht_set_ac_mode(wlc_vht_info_t *vhti)
{
	wlc_vht_up((void *)vhti);
}
...
wlc_vht_up(void *ctx)
{
	...
	/* publish operational streams based CAP */
	...
	rx_mcsmap = wlc_rateset_vht_filter_mcsmap(rateset.vht_mcsmap,
		rxstreams, (WLC_VHT_FEATURES_MCS_GET(wlc->pub)));

	/* update vht mcs for tx */
	wlc_rateset_vhtmcs_build(&rateset, txstreams);
	tx_mcsmap = wlc_rateset_vht_filter_mcsmap(rateset.vht_mcsmap,
	...
	tx_mcsmap_prop = wlc_rateset_vht_filter_mcsmap(rateset.vht_mcsmap_prop,
	...
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		uint32 max_rxmpdu_field, max_rxmpdu;
		max_rxmpdu_field =
		    (vhti->vht_cap.vht_cap_info & VHT_CAP_INFO_MAX_MPDU_LEN_MASK);

		switch (max_rxmpdu_field) {
		    case VHT_CAP_MPDU_MAX_4K:
			max_rxmpdu = VHT_MPDU_LIMIT_4K;
			break;
		    case VHT_CAP_MPDU_MAX_8K:
			max_rxmpdu = VHT_MPDU_LIMIT_8K;
			break;
		    case VHT_CAP_MPDU_MAX_11K:
			max_rxmpdu = VHT_MPDU_LIMIT_11K;
			break;
		    default:
			max_rxmpdu = 0;
			break;
		}
		wlc_bmac_write_shm(wlc->hw, M_MAXRXMPDU_LEN(wlc), max_rxmpdu);
	}
	...
	if (enable) {
		...
		FOREACH_WLC_BAND(wlc, bandunit) {
			...
			if ((band->bandtype == WLC_BAND_5G) ||
				WLC_VHT_FEATURES_GET(wlc->pub, WL_VHT_FEATURES_2G)) {
				wlc_rateset_vhtmcs_build(&band->defrateset,
				...
				if (WLC_VHT_FEATURES_GET(wlc->pub, WL_VHT_FEATURES_1024QAM)) {
					...
					band->defrateset.vht_mcsmap_prop =
						VHT_PROP_MCS_MAP_NONE_ALL;
				}
				band->hw_rateset.vht_mcsmap = band->defrateset.vht_mcsmap;
				band->hw_rateset.vht_mcsmap_prop = band->defrateset.vht_mcsmap_prop;
			} else {
				band->defrateset.vht_mcsmap = VHT_CAP_MCS_MAP_NONE_ALL;
				band->hw_rateset.vht_mcsmap = VHT_CAP_MCS_MAP_NONE_ALL;
				band->defrateset.vht_mcsmap_prop = VHT_PROP_MCS_MAP_NONE_ALL;
				band->hw_rateset.vht_mcsmap_prop = VHT_PROP_MCS_MAP_NONE_ALL;
			}
		...
		FOREACH_WLC_BAND(wlc, bandunit) {
			...
			band->defrateset.vht_mcsmap = VHT_CAP_MCS_MAP_NONE_ALL;
			band->hw_rateset.vht_mcsmap = VHT_CAP_MCS_MAP_NONE_ALL;
			band->defrateset.vht_mcsmap_prop = VHT_CAP_MCS_MAP_NONE_ALL;
			band->hw_rateset.vht_mcsmap_prop = VHT_CAP_MCS_MAP_NONE_ALL;
...
/**
 * Adjust the information on a remote node according to the caller provided parameters. Similar to
...
*/
void
wlc_vht_update_scb_state(wlc_vht_info_t *vhti, enum wlc_bandunit band, struct scb *scb,
	vht_cap_ie_t *vht_cap_ie, vht_op_ie_t *vht_op_ie, uint8 vht_ratemask)
{
	...
	/* Clear capabilities if the current band does not allow VHT or if there is no
	 * vht cap IE.
	 */
	if (!VHT_ENAB_BAND(wlc->pub, band) || vht_cap_ie == NULL) {
		...
		WL_TRACE(("Skipping vht upd: cap=%p op=%p\n",
			OSL_OBFUSCATE_BUF(vht_cap_ie), OSL_OBFUSCATE_BUF(vht_op_ie)));
		return;
	}

	if (VHT_ENAB_BAND(wlc->pub, band)) {
	...
	*/
	new_rateset.vht_mcsmap = wlc_rateset_vht_filter_mcsmap(
				vht_cap_ie->rx_mcs_map,
				wlc->stf->op_txstreams,
				WLC_VHT_FEATURES_MCS_GET(wlc->pub));

	new_rateset.vht_mcsmap_prop = wlc_rateset_vht_filter_mcsmap(
				vht_cap_ie->rx_mcs_map,
				wlc->stf->op_txstreams,
				WLC_VHT_FEATURES_PROP_MCS_GET(wlc->pub));

	/* If we are AP, save the client's MCS map from the VHT Cap */
	if (BSSCFG_AP(bsscfg)) {
	...
	if (cubby_info->rxmcsmap != new_rateset.vht_mcsmap) {
		...
		wlc_vht_upd_rate_mcsmap_prop_ex(wlc->vhti, scb,
				new_rateset.vht_mcsmap_prop);
		/* For STA, update the MCS map as new_rateset.vht_mcsmap */
	...
	if (amsdu_enabled) {
		...
		max_txmpdu_len = vht_cap_ie->vht_cap_info & VHT_CAP_INFO_MAX_MPDU_LEN_MASK;
	...
	max_exp = (vht_cap_ie->vht_cap_info & VHT_CAP_INFO_AMPDU_MAXLEN_EXP_MASK);
	max_exp >>= VHT_CAP_INFO_AMPDU_MAXLEN_EXP_SHIFT;

	wlc_ampdu_scb_set_params(wlc, scb, -1, (uint8)max_exp, -1);

	/* reinit ratesel if rxmcsmap, sgi, ldpc  changed */
	if (reinit_ratesel) {
	...
	if (TXBF_ENAB(wlc->pub)) {
...
#ifdef WL_MU_TX
			if ((wlc->mutx != NULL) &&
				(vht_cap_ie->vht_cap_info & VHT_CAP_INFO_MU_BEAMFMEE)) {
		...
		wlc_txbf_scb_state_upd(wlc->txbf, scb, (uint8 *) &vht_cap_ie->vht_cap_info,
			sizeof(vht_cap_ie->vht_cap_info), TXBF_CAP_TYPE_VHT);
	}
#endif /*  WL_BEAMFORMING */
} /* wlc_vht_update_scb_state */
 ...
 */
void
wlc_vht_bcn_scb_upd(wlc_vht_info_t *vhti, enum wlc_bandunit band, struct scb *scb,
	...
	uint8 vht_ratemask)
{
	...
	wlc_vht_upd_prop_mcsmap(cfg->wlc, vht_ratemask, bss);
	wlc_vht_upd_prop_mcsmap(cfg->wlc, vht_ratemask, cfg->target_bss);

	if ((ht_cap != NULL) && (vht_cap != NULL) && cfg->associated &&
	    (bss->flags2 & WLC_BSS2_VHT) && (bss->flags & WLC_BSS_WME)) {
		wlc_vht_update_scb_state(vhti, band, scb, vht_cap, vht_op, vht_ratemask);
		if (BSSCFG_STA(cfg) && vht_cap != NULL) {
			bss->vht_rxmcsmap = vht_cap->rx_mcs_map;
			bss->vht_txmcsmap = vht_cap->tx_mcs_map;
		}
		return;
	}

	if (SCB_VHT_CAP(scb)) {
		wlc_vht_update_scb_state(vhti, band, scb, NULL, NULL, 0);
		return;
	}
}
...
#ifdef WL_MU_TX
	if (MU_TX_ENAB(wlc)) {
		wlc_mutx_update_scb_oper_mode(wlc->mutx, scb, oper_mode);
	}
#endif // endif
	...
	ratespec_t rspec, struct wl_rxsts *sts)
{
	...
	if (RSPEC_ISVHT(rspec)) {
		chanspec_subband_t ctl_sb = CHSPEC_CTL_SB(sts->chanspec);
		...
		if (CHSPEC_IS8080(sts->chanspec) || CHSPEC_IS160(sts->chanspec)) {
			if (bw == WL_RSPEC_BW_20MHZ) {
				...
				if (ctl_sb <= WL_CHANSPEC_CTL_SB_LUU) {
...
wlc_vht_bsscfg_up(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
	...
	bi->flags2 &= ~(WLC_BSS2_VHT | WLC_BSS2_80MHZ | WLC_BSS2_SGI_80
			| WLC_BSS2_8080MHZ | WLC_BSS2_160MHZ | WLC_BSS2_SGI_160);
	if (BSS_VHT_ENAB(wlc, cfg)) {
		...
		/* get and save max_pwr for the VHT Transmit Power Envelope element */
		wlc_channel_tx_power_target_min_max(wlc, bi->chanspec, &min_pwr, &max_pwr);
		wlc->vhti->vht_max_pwr = max_pwr;
	}
}
...
static int
wlc_vht_parse_ext_cap_ie(void *ctx, wlc_iem_parse_data_t *data)
{
...
wlc_vht_is_40MHz(wlc_info_t* wlc, void* p, struct scb* scb)
{
	...
	return (RSPEC_BW(rspec) == WL_RSPEC_BW_40MHZ);
}
...
wlc_vht_get_mcs(wlc_info_t* wlc, void* p, struct scb* scb)
{
	...
	return (RSPEC_ISSGI(rspec) & WL_RSPEC_VHT_MCS_MASK);
}
...
wlc_send_action_vht_oper_mode(wlc_vht_info_t *vhti, wlc_bsscfg_t *bsscfg,
	...
	if ((p = wlc_frame_get_mgmt(wlc, FC_ACTION, ea,
	         &bsscfg->cur_etheraddr, &bsscfg->BSSID, body_len, &pbody)) == NULL) {
	...
	if (fromSMPSLearning == FALSE) {
		ahdr->mode = wlc_get_oper_mode(vhti, bsscfg);
	} else {
