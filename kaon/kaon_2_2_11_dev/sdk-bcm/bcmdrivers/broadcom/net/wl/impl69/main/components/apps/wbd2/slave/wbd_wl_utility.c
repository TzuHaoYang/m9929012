 ...
 *
 * $Id: wbd_wl_utility.c 791992 2020-10-15 11:36:03Z $
 */
...
wbd_wl_fill_interface_info(wbd_wl_interface_t *ifr_info)
{
	...
	/* Get BSSID */
	ret = blanket_get_bssid(ifr_info->ifr.ifr_name, &ifr_info->bssid);
	if (ETHER_ISNULLADDR(&ifr_info->bssid)) {
		WBD_DEBUG("Failed to get bssid for the interface ifname [%s] \n",
