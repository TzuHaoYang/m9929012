 ...
 *
 * $Id: wluc_rrm.c 792132 2020-10-20 09:15:57Z $
 */
...
wl_rrm_nbr_list_v1(void *wl, cmd_t *cmd, char **argv)
{
	...
	for (i = 0; i < list_cnt; i++) {
		...
		printf("ssid %s chanspec 0x%04x preference 0x%02x"
			"bss_params 0x%02x tpe_psb 0x%02x\n",
			nbr_elt->ssid.SSID, nbr_elt->chanspec, nbr_elt->bss_trans_preference,
			nbr_elt->bss_params, nbr_elt->tpe_psd);
...
wl_rrm_nbr_add_nbr_v1(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* bss_params useful for 6G information */
	if (argc >= 10 && argv[9]) {
		nbr_elt.bss_params = htod32(strtoul(argv[9], NULL, 0));
	}

	if (argc >= 11 && argv[10]) {
		nbr_elt.tpe_psd = htod32(strtoul(argv[10], NULL, 0));
	}

	if (argc >= 12 && argv[11]) {
		nbr_elt.tbtt_len = htod32(strtoul(argv[11], NULL, 0));
	}
	if (argc >= 13 && argv[12]) {
		nbr_elt.tbtt_offset = htod32(strtoul(argv[12], NULL, 0));
	}
