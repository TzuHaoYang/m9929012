 ...
 *
 * $Id: wlc_lq.c 792523 2020-10-29 09:46:36Z $
 */
...
#define USE_128_BYTES_FOR_CCA_STATS			128
...
static int8 wlc_lq_chanim_offchan_noise_mode(int8 *noise, int count);
...
static void wlc_lq_update_tbtt(void *ctx, wlc_mcnx_intr_data_t *notif_data)
{
	...
	if (phy_utils_get_chanspec(wlc->pi) != cfg->current_bss->chanspec) {
...
static int
wlc_lq_noise_lte_median(int* values)
{
...
wlc_lq_bss_dump(void *ctx, wlc_bsscfg_t *cfg, struct bcmstrbuf *b)
{
	...
	if (!BSSCFG_STA(cfg) || BSSCFG_IBSS(cfg) || !cfg->associated)
		return;
...
wlc_lq_ant_bcn_rssi(void *ctx, bss_rx_bcn_notif_data_t *bcn)
{
	...
	/* validate scb */
	if (!(cfg->associated && (bcn->scb == wlc_scbfind(wlc, cfg, &cfg->BSSID)))) {
		WL_ERROR(("wl%d: wlc_lq_ant_bcn_rssi scb NULL\n", wlc->pub->unit));
		return;
	}
...
/*
 * This function removes the noise spikes and calculate
 * the avarage of most occured noise values(+5db or -5db)
 */
static int8
wlc_lq_chanim_offchan_noise_mode(int8 *noise, int count)
{
	int8 result = 0;
	int max_cnt = 0;
	int i, j;
	...
	for (i = 0; i < count; ++i) {
		...
		for (j = 0; j < count; ++j) {
			if (noise[i]-5 <= noise[j] && noise[j] <= noise[i]+5) {
				++itr_cnt;
				sum += noise[j];
			}
		}
	...
	if (SCAN_IN_PROGRESS(wlc->scan)) {
		...
		for (i =  0; i < cnt; i++) {
			...
			*/
			...
			if (result) {
				noise[valid_cnt] = result;
				valid_cnt++;
			}
		}
		if (valid_cnt) {
			result = wlc_lq_chanim_offchan_noise_mode(noise, valid_cnt);
		}
	}
	...
	wlc_chanim_stats_us_t *cur_stats_us, bool chan_switch)
{
	...
	if (wlc->pub->up) {
		...
		for (i = 0; i < CCASTATS_MAX; i++) {
			if (i == CCASTATS_TXOP) {
				cur_stats_us->chanim_stats_us.ccastats_us[i] =
					acc_us->ccastats_us[i] * slottime;
				if (cur_stats_us->chanim_stats_us.ccastats_us[i] >
						cur_stats_us->chanim_stats_us.total_tm) {
					cur_stats_us->chanim_stats_us.ccastats_us[i] =
						cur_stats_us->chanim_stats_us.total_tm;
				}
			} else {
				cur_stats_us->chanim_stats_us.ccastats_us[i] =
					acc_us->ccastats_us[i];
			}
		}
			...
			for (i = 0; i < CCASTATS_MAX; i++) {
				if (i == CCASTATS_TXOP) {
					if_info->chanim_stats_us.ccastats_us[i]	=
						(if_info->acc_us.ccastats_us[i]-
						if_info->last_acc_us.ccastats_us[i]) * slottime;
					if (if_info->chanim_stats_us.ccastats_us[i] >
							if_info->chanim_stats_us.total_tm) {
						if_info->chanim_stats_us.ccastats_us[i] =
							if_info->chanim_stats_us.total_tm;
					}
				} else {
...
wlc_lq_channel_qa_sample_req(wlc_info_t *wlc)
{
	...
	lqi->channel_qa_channel = CHSPEC_CHANNEL(phy_utils_get_chanspec(WLC_PI(wlc)));
		...
		if (WLRM_ENAB(wlc->pub) && wlc->rm_info->rm_state->rpi_active) {
			if (wlc_rm_rpi_sample(wlc->rm_info, noise_dbm))
				wlc_lq_noise_sample_request(wlc, WLC_NOISE_REQUEST_RM,
						CHSPEC_CHANNEL(phy_utils_get_chanspec(wlc->pi)));
		}
	}
#else
...
#ifdef WLCHANIM
/* return channel utilization as a number between 0(free) and 255(100% used) */
uint8
wlc_lq_get_current_chan_utilization(wlc_info_t *wlc)
{
	wl_chanim_stats_t *list = NULL;
	uint8 cca_stats[USE_128_BYTES_FOR_CCA_STATS];
	int buflen = USE_128_BYTES_FOR_CCA_STATS;
	int busy;
	uint8 chan_util;

	memset(&cca_stats, 0, sizeof(cca_stats));

	wlc_lq_chanim_get_stats(wlc->chanim_info, (wl_chanim_stats_t*)&cca_stats[0], &buflen,
		WL_CHANIM_COUNT_ONE);

	list =  (wl_chanim_stats_t*)&cca_stats[0];

	/* busy = inbss + obss + nocat + nopkt + good_tx + bad_tx */
