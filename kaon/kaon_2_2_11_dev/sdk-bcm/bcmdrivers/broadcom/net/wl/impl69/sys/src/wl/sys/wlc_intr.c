 ...
 *
 * $Id: wlc_intr.c 793816 2020-12-10 18:05:09Z $
 */
...
void BCMFASTPATH
wlc_intr_process_rxfifo_interrupts(wlc_info_t *wlc)
{
	wlc_hw_info_t *wlc_hw = wlc->hw;
	...
	if (BCMSPLITRX_ENAB()) {
		...
		if (STS_RX_ENAB(wlc->pub) && wlc_get_fifo_interrupt(wlc, STS_FIFO)) {
			...
			if (wlc_hw->rxpen_list[RXPEN_LIST_IDX0].rx_head) {
				...
				hwa_worklet_invoke(dev, HWA_COMMON_INTSTATUS_D11BDEST0_INT_MASK);
			}
			...
			W_REG(wlc_hw->osh, &(D11Reggrp_intctrlregs(wlc_hw, STS_FIFO)->intstatus),
				DEF_RXINTMASK);
		}
#endif /* STS_FIFO_RXEN */
...
#if defined(STS_XFER)
		if (STS_XFER_PHYRXS_FIFO_ENAB(wlc->pub) && wlc_get_fifo_interrupt(wlc, STS_FIFO)) {
			/* check for fif0-0 intr */
			if (!(wlc_hw->dma_rx_intstatus & RX_INTR_FIFO_0) &&
				wlc_sts_xfer_phyrxs_rxpend(wlc, RX_FIFO)) {
				WL_INFORM(("Got STS_FIFO intr for fifo-0 pending packets\n"));
				wlc_hw->dma_rx_intstatus |= RX_INTR_FIFO_0;
			}
...
wlc_intstatus(wlc_info_t *wlc, bool in_isr)
{
	...
	if (macintstatus & MI_DMAINT) {
		...
		ASSERT(D11REV_LT(wlc->pub->corerev, 130));
