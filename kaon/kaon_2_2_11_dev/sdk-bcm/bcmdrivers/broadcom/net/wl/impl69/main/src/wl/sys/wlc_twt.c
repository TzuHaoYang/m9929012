 ...
 *
 * $Id: wlc_twt.c 793882 2020-12-14 10:11:24Z $
 */
...
typedef struct twt_indv_desc {
	...
	uint32		cnt_rlm_unavailable;	/* Counter; No RLM entry available to schedule SP */
...
struct wlc_twt_info {
	...
	uint32		cnt_rlm_not_programmed;	/* Counter; Skipped RLM programming due to full */
	uint32		cnt_b2b_missed_rlm;	/* back 2 back failed RLM programming */
};
...
wlc_twt_dump(void *ctx, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "Failed RLM programming, all RLM blocks in use=%d\n",
		twti->cnt_rlm_not_programmed);
	...
	if (twti->itwt_count) {
		...
		while (scb_twt) {
			for (i = 0; i < ARRAYSIZE(scb_twt->indv); i++) {
				...
				bcm_bprintf(b, "      SP not programmed due to unavailability RLM"
					" count=%d\n", indv->cnt_rlm_unavailable);
				if (indv->cnt_pretwt) {
...
{
	/* TWT requester functionality is currently not supported, in STA mode TWT is unsupported */
	return FALSE;
}
...
wlc_twt_build_af(scb_t *scb, uint8 dialog_token, uint8 **body, bool individual)
{
	...
	*body += S1G_AF_TWT_SETUP_TWT_IE_OFF;
	...
	/* Currently the Information Frame handling is implemented, but it is WFA R2 feature */
...
wlc_twt_af_info_recv(wlc_twt_info_t *twti, scb_t *scb, uint8 *body, uint body_len, uint16 fc)
{
	...
	if (twt_info & TWT_INFO_ALL_TWT) {
		...
		/* find the the info */
		if (!wlc_twt_find_fid(scb_twt, flow_id)) {
	...
	if (next_twt_size) {
		...
		if (next_twt_size == TWT_INFO_NEXTTWT_SIZE_1) {
			...
			if (twt_l < tsf_l) {
				if ((tsf_l - twt_l) > 0x7fffffff) {
					...
					/* Problem. specified TWT is in past, so just set
					 * it to TSF. It will make it wake up on next TWT.
					 */
					twt_l = tsf_l;
				}
			}
		...
		if (next_twt_size == TWT_INFO_NEXTTWT_SIZE_2) {
			 ...
			 * be assumed in past. That is if the twt_h is 1 smaller then tsf_h or
	...
	scb_t *scb, uint8 *body, uint body_len)
{
	...
	WL_TWT(("wl%d: %s Processing AF with action id %d, AID %d, PM %d\n", twti->wlc->pub->unit,
		__FUNCTION__, action_id, SCB_AID(scb), (ltoh16(hdr->fc) & FC_PM) ? 1 : 0));
	if (WL_TWT_ON()) {
	...
	case WLC_TWT_AF_PEND_SETUP_ITWT_RSP:
		...
		if (!indv) {
			...
			if (no_ack) {
				...
				/* iTWT setup complete */
				if (wlc_twt_setup_itwt_complete(twti, scb, indv) != BCME_OK) {
					WL_ERROR(("%s: Setup failed, tearing it down\n",
						__FUNCTION__));
					wlc_twt_teardown_itwt(twti, scb, indv, FALSE);
				}
			}
	...
	uint32 twt_l, uint32 twt_h)
{
	...
	if (flow_id == TWT_INFO_ALL_FLOW_ID) {
		...
		/* flow_id has already been validated, create single for loop */
		start = 0;
		stop = 1;
	}
...
skip_rlm_programming:
	...
	for (i = 0; i < ARRAYSIZE(twt_rlm_arr); i++) {
		twt_rlm = &twt_rlm_arr[i];
		if (twt_rlm->usr_count) {
			if (i == 0) {
				twti->cnt_sp_missed_almost++;
			}
			...
			if (usrnum != 0) {
				/* This is a problematic situation. There is no RLM available to
				 * program the SPs. This can happen (rarely) because ucode is to
				 * busy. It can also be a side effect of an upcoming PSM WD. This
				 * situation is handled by not programming the schedule, but drop
				 * it completly. It is counted and logged, and if 8 subsequent
				 * programmings fail then an ASSERT will be given.
				 */
				twti->cnt_rlm_not_programmed++;

				WL_TWT(("%s: Lacking RLM space TSF(l)=0x%08x schedid=0x%04x\n",
					__FUNCTION__, tsf_l, twt_rlm->schedid));

				for (idx = 0; idx < twt_rlm->usr_count; idx++) {
					rlm_id = (twt_rlm->usr_info[idx] & TSB_USR_IDX_MASK)
						>> TSB_USR_IDX_SHIFT;
					scb_twt = twti->scb_twt_list;
					while (scb_twt) {
						if (scb_twt->rlm_id == rlm_id) {
							break;
						}
						scb_twt = scb_twt->next;
					}
					/* ASSERT if not found. The same list was used to build the
					 * info, so this can not happen.
					 */
					ASSERT(scb_twt);
					scb_twt->indv[0].cnt_rlm_unavailable++;

					WL_TWT(("	[%d].id = %d dur = %d AID = %d\n",
						idx, rlm_id,
						((twt_rlm->usr_info[idx] & TSB_USR_SPDUR_MASK)
						>> TSB_USR_SPDUR_SHIFT) + 1,
						SCB_AID(scb_twt->indv[0].scb)));
				}

				twti->cnt_b2b_missed_rlm++;
				if (twti->cnt_b2b_missed_rlm > TWT_B2B_RLM_FAIL_MAX) {
					ASSERT(0);
				}
				goto failed_rlm_programming;
			} else {
				twti->cnt_b2b_missed_rlm = 0;
			}
