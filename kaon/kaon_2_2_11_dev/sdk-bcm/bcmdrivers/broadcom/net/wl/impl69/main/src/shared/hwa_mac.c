...
#else
// WAR for rev131,  fetch all d11b resource and free.
#define D11B_AUDIT_ENABLED(corerev)	HWAREV_LE(corerev, 131)
...
hwa_rxfill_preinit(hwa_rxfill_t *rxfill)
{
	...
	*/
	...
	rxfill->config.rx_size = HWA_PP_RXLFRAG_DATABUF_LEN;

	HWA_ERROR(("%s config rph<%u> offset d11<%u> len<%u> addr<%u>\n",
	...
	if (dev->d11b_axi) {
		...
		// Let's force to 8B alignment in case the dmasel is set.
		...
		bzero(memory, mem_sz);
		HWA_ERROR(("%s rxfifo_ring +memory[%p,%u]\n", HWA1b, memory, mem_sz));
	}
	...
	if (D11B_AUDIT_ENABLED(dev->corerev)) {
		...
		if ((memory = MALLOCZ(dev->osh, mem_sz)) == NULL) {
			...
			bzero(memory, mem_sz);
			HWA_ERROR(("%s d11b_audit_table +memory[%p,%u]\n", HWA1b, memory, mem_sz));
...
hwa_rxfill_init(hwa_rxfill_t *rxfill)
{
	...
	HWA_PKTPGR_EXPR({
		pp_desc_burst = (0U
			 ...
			 */
			| BCM_SBIT(HWA_RX_D0DEST_INTRAGGR_SEQNUM_CFG_PP_DESC_BURST_LEN)
			/* Time out value to be used to force the descriptors transferred out.
			 * This is specified in 64 microseconds.
			 */
			| BCM_SBF(8, HWA_RX_D0DEST_INTRAGGR_SEQNUM_CFG_PP_DESC_TIMER)
			| 0U);
	});
...
hwa_rxfill_d11b_fetch_all(hwa_dev_t *dev, uint32 core)
{
	...
	HWA_ERROR(("d11bdest_ring_wrindex<0x%x,%d>\n", u32, wr_idx));
	...
	HWA_ERROR(("d11bdest_ring_rdindex<0x%x,%d>\n", u32, rd22_idx));
	...
	HWA_ERROR(("d11bdest_ring_wrindex_dir<0x%x,%d>\n", u32, wr_idx_dir));
	...
	HWA_ERROR(("%s reclaim core<%u> rxbuffers RD<%u> WR<%u> WRDIR<%u> dirty <%u>\n",
...
hwa_rxfill_rxbuffer_reclaim(hwa_dev_t *dev, uint32 core)
{
	...
	HWA_ERROR(("%s d11b_audit_total<%d> = recycle_count<%d> + d11b_audit_dirty<%d>\n",
	...
	while ((elem_ix = bcm_ring_cons(&rxfill->d11b_audit_state,
		...
		if (HWAREV_IS(dev->corerev, 131)) {
			...
			// B0: CRBCAHWA-633, RECYCLE_STATUS::recycle_freed11b_mode(bit 27).
		...
		// A0: Free it.
		if (HWAREV_IS(dev->corerev, 131) && dev->d11b_recycle_war) {
			// Free to Host.
			hwa_rxpath_queue_rxcomplete_fast(dev, audit_elem->host_pktid);
...
#if defined(STS_XFER)
			/* TODO: STS_XFER is not supported in Full Dongle */
			ASSERT(0);
#endif /* STS_XFER */
			break;
		}
...
hwa_rxfill_pagein_rx_req(hwa_dev_t *dev, uint32 core, uint16 rxpkt_ready, bool bound)
{
	int pkt_count;
	uint32 u32;
	int pktpgr_trans_id = HWA_FAILURE;
	...
	if (rxpkt_ready == 0) {
		HWA_PP_DBG(HWA_PP_DBG_2A, "  No ready rxpkt, return\n");
		return HWA_SUCCESS;
	}

ReqN:

	pkt_count     = MIN(dev->rx_pkt_count_max, rxpkt_ready);
	...
	dev->pktpgr.pgi_rxpkt_in_trans += pkt_count;
	dev->pktpgr.pgi_rxpkt_req++;

	// Next req if any.
	rxpkt_ready -= pkt_count;
	if (rxpkt_ready && dev->pktpgr.pgi_rxpkt_req < dev->rx_pkt_req_max) {
		goto ReqN;
	}
	...
	int pkt_count, hwa_pp_lbuf_t * pktlist_head, hwa_pp_lbuf_t * pktlist_tail)
{
	...
	if (dev->reinit) {
		...
		if (pkt_count != 0) {
			HWA_ASSERT(PKTLINK(pktlist_tail) == 0U);
		...
		return; // Return here
	} else if (rxpkt_ready > dev->pktpgr.pgi_rxpkt_in_trans) {
		rxpkt_ready -= dev->pktpgr.pgi_rxpkt_in_trans;
	...
	uint32 core, uint32 arg4)
{
	...
	PKTSETPOOL_RX(dev->osh, dev->osh, FALSE, NULL);

	// 2B:len
	rxh = (d11rxhdr_t *)PKTDATA(dev->osh, rxlbuf);
	HWA_ASSERT(PKTLEN(dev->osh, rxlbuf) ==
	...
	PKTSETLEN(dev->osh, rxlbuf, (wlc->hwrxoff +
		D11RXHDR_ACCESS_VAL(rxh, wlc->pub->corerev, RxFrameSize)));
...
hwa_txfifo_init(hwa_txfifo_t *txfifo)
{
	...
	u32 = (0U
		...
		| BCM_SBF(1,
			HWA_TXDMA_HWA_TXDMA2_CFG2_TXDMA_AGGR_AQM_DESCRIPTOR_THRESHOLD2) // 15 mpdu
#else
		| BCM_SBF(0,
			HWA_TXDMA_HWA_TXDMA2_CFG2_TXDMA_AGGR_AQM_DESCRIPTOR_THRESHOLD)
#endif // endif
...
hwa_txfifo_dma_reclaim(struct hwa_dev *dev, uint32 core)
{
	...
	int16 pktcnt;
	wlc_info_t *wlc;
	NO_HWA_PKTPGR_EXPR(hwa_txfifo_t *txfifo);
	NO_HWA_PKTPGR_EXPR(hwa_ring_t *pktchain_ring); // S2H pktchain ring context
	...
	for (i = 0; i < WLC_HW_NFIFO_INUSE(wlc); i++) {
		...
		HWA_PKTPGR_EXPR({
			...
			hwa_pktpgr_txfifo_shadow_reclaim(dev,
				WLC_HW_MAP_TXFIFO(wlc, i));
			// Remove specific packets in PageOut Req Q to SW shadow.
			hwa_pktpgr_pageout_ring_shadow_reclaim(dev,
				WLC_HW_MAP_TXFIFO(wlc, i));
		});
	...
	uint16 pkt_count, uint16 mpdu_count)
{
	...
	if (pkt_local) {
		...
		if (dev->pktpgr.pspl_test_en) {
			hwa_pktpgr_push_req(dev, fifo_idx, HWA_PP_CMD_NOT_TAGGED,
				pktchain_head, pktchain_tail, pkt_count, mpdu_count);
			return FALSE;
		}
#endif /* PSPL_TX_TEST */
		// Transmit TxLfrags: *NoResponse
		//req_cmd.pageout_req_pktlist.trans        = HWA_PP_PAGEOUT_PKTLIST_WR;
		req_cmd.pageout_req_pktlist.trans        = HWA_PP_PAGEOUT_PKTLIST_NR;
		req_cmd.pageout_req_pktlist.fifo_idx     = (uint16)fifo_idx;
	...
	shadow32 = (hwa_txfifo_shadow32_t *)txfifo->txfifo_shadow;
	shadow32[fifo_idx].hw.pkt_count += pkt_count;
	shadow32[fifo_idx].hw.mpdu_count += mpdu_count;

	HWA_PP_DBG(HWA_PP_DBG_3B, "  >>PAGEOUT::REQ %s pkts %3u/%3u "
		"list[0x%p(%d) .. 0x%p(%d)] fifo %3u ==%s(%d)==>\n\n",
		pkt_local ? "PKTLOCAL" : "PKTLIST_NR", pkt_count, mpdu_count,
...
failure:
	HWA_WARN(("%s PAGEOUT::REQ %s failure fifo<%u> head<%p> tail<%p>"
		" pkt<%u> mpdu<%u>\n", HWA3b, pkt_local ? "PKTLOCAL" : "PKTLIST_NR",
...
#ifdef HWA_PKTPGR_BUILD
static int hwa_txstat_pagein_req(hwa_dev_t *dev, wlc_txs_pkg16_t *pkg, uint32 elem_ix);
#endif // endif
	...
	int pkt_count, hwa_pp_lbuf_t *pktlist_head, hwa_pp_lbuf_t *pktlist_tail)
{
	...
	if (myshadow32->pkt_head == NULL) {
		...
		myshadow32->pkt_head = HWA_PTR2UINT(pktlist_head);
		myshadow32->pkt_tail = HWA_PTR2UINT(pktlist_tail);
		myshadow32->pkt_count = pkt_count;
		myshadow32->mpdu_count = mpdu_count;
	} else {
		HWA_PP_DBG(HWA_PP_DBG_4A, "  %s: Queue-tail %d/%d packet [%p .. %p] to shadow %d "
			"[%x .. %x] count %d/%d\n", __FUNCTION__, mpdu_count, pkt_count,
			pktlist_head, pktlist_tail, fifo_idx, myshadow32->pkt_head,
			myshadow32->pkt_tail, myshadow32->pkt_count, myshadow32->mpdu_count);

		txlbuf = HWA_UINT2PTR(hwa_pp_lbuf_t, myshadow32->pkt_tail);
	...
	// Decrease HW count
	HWA_ASSERT(myshadow32->hw.pkt_count >= pkt_count);
	HWA_ASSERT(myshadow32->hw.mpdu_count >= mpdu_count);
	myshadow32->hw.pkt_count -= pkt_count;
	myshadow32->hw.mpdu_count -= mpdu_count;
	...
	int fifo_idx, int mpdu_count)
{
	...
	hwa_txfifo_shadow_enq(dev, fifo_idx, 1, 1,
		txlbuf_local, txlbuf_local);
}
...
static void
hwa_txstat_pagein_rsp_tagged(hwa_dev_t *dev, int fifo_idx, int mpdu_count,
	hwa_pp_lbuf_t *pktlist_head, hwa_pp_lbuf_t *pktlist_tail)
{
	uint16 mpdu;
	int pkt_count;
	hwa_pp_lbuf_t *txlbuf, *txlbuf_link;
	void *p;

	txlbuf = pktlist_head;

	// tagged response is came from hwa_pktpgr_txfifo_shadow_reclaim().
	// DATA frame and MGMT frame will be link together.
	// So enqueue to shadow list individually.
	for (mpdu = 0; mpdu < mpdu_count - 1; ++mpdu) {
		...
		if (PKTISMGMTTXPKT(dev->osh, txlbuf)) {
			hwa_txstat_pagein_rsp_fixup_local(dev, txlbuf, fifo_idx, 1);
		} else {
			...
			for (; PKTNEXT(dev->osh, p); p = PKTNEXT(dev->osh, p))
				pkt_count++;
			// Link the pktlist to shadow list.
			hwa_txfifo_shadow_enq(dev, fifo_idx, 1, pkt_count,
				txlbuf, txlbuf);
		}
	...
	hwa_pp_lbuf_t *pktlist_head, hwa_pp_lbuf_t *pktlist_tail)
{
	...
	if (pkt_count == 0 || mpdu_count == 0) {
		HWA_ERROR(("%s: Over pagein? pkt_count<%d> mpdu_count<%d>\n",
			__FUNCTION__, pkt_count, mpdu_count));
		return;
	}
	...
	HWA_PP_DBG(HWA_PP_DBG_4A, "  <<PAGEIN::RSP TXSTAT MPDUS : mpdu %3u pkt %3u "
		...
		txstat->elem_ix_ack, pktpgr_trans_id);
	...
	if (tagged == HWA_PP_CMD_TAGGED) {
		hwa_txstat_pagein_rsp_tagged(dev, fifo_idx, mpdu_count,
			pktlist_head, pktlist_tail);
		return;
	}

	if (PKTISMGMTTXPKT(dev->osh, txlbuf)) {
		hwa_txstat_pagein_rsp_fixup_local(dev, txlbuf, fifo_idx, mpdu_count);
	} else {
		for (mpdu = 0; mpdu < mpdu_count - 1; ++mpdu) {
			hwa_txstat_pagein_rsp_fixup(dev, txlbuf, mpdu_count);
		...
		// Link the pktlist to shadow list.
		hwa_txfifo_shadow_enq(dev, fifo_idx, mpdu_count, pkt_count,
			pktlist_head, pktlist_tail);
	}
...
static int // PAGEIN REQUEST TXSTATUS
hwa_txstat_pagein_req(hwa_dev_t *dev, wlc_txs_pkg16_t *pkg, uint32 elem_ix)
{
	wlc_info_t *wlc;
	hwa_txstat_t *txstat;
	...
	if ((txserr = hwa_txstat_read_txs_pkg16(wlc, pkg)) == BCME_OK) {
		...
		// Get frameid, ncons
		frameid = (pkg->word[0] & TXS_FID_MASK) >> TXS_FID_SHIFT;
		status_bits = pkg->word[0] & TXS_STATUS_MASK;
		ncons = ((status_bits & TX_STATUS40_NCONS) >> TX_STATUS40_NCONS_SHIFT);
		HWA_PP_DBG_EXPR(seq = pkg->word[1] & TXS_SEQ_MASK);
	...
	req.fifo_idx     = WLC_HW_MAP_TXFIFO(wlc, D11_TXFID_GET_FIFO(wlc, frameid));
	...
	HWA_PP_DBG(HWA_PP_DBG_4A, "  >>PAGEIN::REQ TXSTATUS: mpdu %3u fifo %3u elem_ix %d "
		"==TXS-REQ(%d)==>\n\n", req.mpdu_count,	req.fifo_idx, elem_ix, pktpgr_trans_id);
 ...
 * This issue have fixed in HWA r132 RTL but 6715A0.
 */
bool
hwa_txstat_mismatch_bypass(struct hwa_dev *dev)
{
	...
	if (!txstat->status_mismatch) {
		txstat->status_mismatch = TRUE;
		return TRUE;
	} else {
		return FALSE;
	}
...
hwa_txstat_mismatch_reset(struct hwa_dev *dev)
{
	...
	txstat->status_mismatch = FALSE;
}
#endif /* HWA_REVISION_ID == 131 */

void // For debug
hwa_txfifo_pageout_pktlist_rsp(hwa_dev_t *dev, uint8 pktpgr_trans_id,
	int fifo_idx, int mpdu_count, int total_txdma_desc, hwa_pp_lbuf_t *pkt_local)
{
	// Audit pre-conditions
	HWA_AUDIT_DEV(dev);

	HWA_PP_DBG(HWA_PP_DBG_3B, "  <<PAGEOUT::RSP PKTLIST : mpdu %3u pkt @ 0x%p fifo %3u "
		"<==TXPKTLIST-RSP(%d)==\n\n", mpdu_count, pkt_local, fifo_idx, pktpgr_trans_id);

	// Dump OvfQ
	// hwa_txfifo_dump_ovfq(dev, NULL, fifo_idx);
}
...
// Sanity check and fixup head and end
static void
_hwa_txfifo_pagemgr_pull_rsp_fixup(hwa_dev_t *dev, hwa_pp_lbuf_t *pp_lbuf)
{
	// Audit pre-conditions
	HWA_AUDIT_DEV(dev);
	...
	// Set head and end.
	PKTSETTXBUFRANGE(dev->osh, pp_lbuf, PKTPPBUFFERP(pp_lbuf), HWA_PP_PKTDATABUF_BYTES);
}

// NOTE: pktlist_head and pktlist_tail address space are in DDBM.
void
hwa_txfifo_pull_pktchain_xmit_request(hwa_dev_t *dev, uint8 pktpgr_trans_id,
	...
	hwa_pp_lbuf_t *pktlist_head, hwa_pp_lbuf_t *pktlist_tail)
{
	...
	HWA_ASSERT(PKTLINK(pktlist_tail) == 0U);
	for (pkt = 0; pkt < pkt_count - 1; ++pkt) {
		_hwa_txfifo_pagemgr_pull_rsp_fixup(dev, pp_lbuf);
	...
	req.trans        = HWA_PP_PAGEOUT_PKTLIST_NR;
	req.fifo_idx     = (uint16)fifo_idx;
	...
	HWA_PP_DBG(HWA_PP_DBG_MGR_TX, "  >>PAGEOUT::REQ PULL:PKTLIST_NR pkts %3u/%3u "
		"list[0x%p(%d) .. 0x%p(%d)] fifo %3u ==PULL:PKTLIST_NR(%d)==>\n\n",
...
void // free locally allocated TxLfrag
hwa_txfifo_pageout_local_rsp(hwa_dev_t *dev, uint8 pktpgr_trans_id,
	int fifo_idx, int mpdu_count, int total_txdma_desc, hwa_pp_lbuf_t *pkt_local)
{
	HWA_PP_DBG(HWA_PP_DBG_3B, "  <<PAGEOUT::RSP PKTLOCAL : mpdu %3u pkt @ 0x%p fifo %3u "
		"<==TXLOCAL-RSP(%d)==\n\n", mpdu_count, pkt_local, fifo_idx, pktpgr_trans_id);

	// Locally allocated TxLfrag will be reused for txs processing after hwa_txstat_pagein_rsp
}
...
enum {
	...
	IOV_HWA_RX_PKT_COUNT_MAX,
	IOV_HWA_RX_PKT_REQ_MAX,
	IOV_LAST
};
...
static const bcm_iovar_t hwa_iovars[] = {
	...
	{"hwa_rx_pkt_count_max", IOV_HWA_RX_PKT_COUNT_MAX, (0), 0, IOVT_UINT32, 0},
	{"hwa_rx_pkt_req_max", IOV_HWA_RX_PKT_REQ_MAX, (0), 0, IOVT_UINT32, 0},
#endif // endif
	{NULL, 0, 0, 0, 0, 0}
};
	...
	void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_HWA_PSPL):
		*ret_int_ptr = dev->pktpgr.pspl_test_en;
		break;
	case IOV_SVAL(IOV_HWA_PSPL):
		dev->pktpgr.pspl_test_en = bool_val;
		break;
#endif /* PSPL_TX_TEST */
	...
	case IOV_SVAL(IOV_HWA_D11B_RECYCLE_WAR):
		...
		break;
	case IOV_SVAL(IOV_HWA_RX_PKT_COUNT_MAX):
		dev->rx_pkt_count_max = int_val;
		break;
	case IOV_SVAL(IOV_HWA_RX_PKT_REQ_MAX):
		dev->rx_pkt_req_max = int_val;
		break;
#endif /* HWA_PKTPGR_BUILD */
	default:
...
hwa_down(void *hdl)
{
	...
	// in hwa_txfifo_pageout_local_rsp.
