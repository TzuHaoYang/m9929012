 ...
 *
 * $Id: wlc_rx.c 793636 2020-12-03 23:15:11Z $
 *
 */
#include <wlc_cfg.h>
#include <typedefs.h>
...
#else
		WLCNTINCR(pub->_cnt->rxrunt);
		RX_PKTDROP_COUNT(wlc, scb, RX_PKTDROP_RSN_RUNT_FRAME);
		toss_reason = RX_PKTDROP_RSN_RUNT_FRAME;
		goto toss_silently;
#endif /* WLINTERNAL */
	}
...
wlc_recvdata_ordered(wlc_info_t *wlc, struct scb *scb, struct wlc_frminfo *f)
{
	...
	if (!f->ismulti) {
		...
		/* Drop fragmented multicast frames */
		toss_reason = RX_PKTDROP_RSN_RXFRAG_ERR;
		goto toss;
	}
#ifdef STA
	...
	if (OCE_ENAB(wlc->pub) && (wlc->pub->now % 60 == 0)) {
		...
		if (CHSPEC_IS2G(D11RXHDR_ACCESS_VAL(&wrxh->rxhdr, wlc->pub->corerev, RxChan))) {
			if (!wlc_erp_find(wlc, body, body_len, NULL, NULL)) {
				wlc_oce_update_control_field(wlc->oce, OCE_11b_ONLY_AP_PRESENT);
			}
			wlc_oce_detect_11b_environment(wlc->oce);
		}

		wlc_recv_scan_parse_bcn_prb(wlc, wrxh, &hdr->bssid, TRUE, body, body_len, &bi);
                 ...
                 uint8 *body, int body_len, wlc_d11rxhdr_t *wrxh, uint8 *plcp)
{
...
#ifdef BCMDBG /* Only to debug for MAP-R2: presence of arrival of gas request in \
	Firmware */
	if ((action_category == DOT11_ACTION_CAT_PUBLIC) ||
		(action_category == DOT11_ACTION_CAT_PDPA)) {
		/* WL_ERROR is by default msglevel set, no need to set explictly */
		WL_ERROR(("action frame with category[%d] rcvd \n", action_category));
	}
#endif /* BCMDBG */
	...
	*/
	...
	Firmware */
			if ((action_category == DOT11_ACTION_CAT_PUBLIC) ||
				(action_category == DOT11_ACTION_CAT_PDPA)) {
				WL_ERROR(("generate event[E_ACTION_FRAME_RX] for AF"
					" with category[%d] \n", action_category));
			}
#endif /* BCMDBG */
		...
		if (BSS_VHT_ENAB(wlc, bsscfg) || BSS_HE_ENAB(wlc, bsscfg)) {
			if (scb != NULL) {
				...
				if ((wlc_vht_get_scb_opermode_enab(wlc->vhti, scb))) {
					 ...
					 */
					if (SCB_HE_CAP(scb)) {
						wlc_he_recv_af_oper_mode(wlc, scb, ap_oper_mode);
					} else
#endif /* WL11AX */
					{
						wlc_vht_update_scb_oper_mode(wlc->vhti, scb,
						                             ap_oper_mode);
					}
				}
			}
	...
	struct dot11_management_header *hdr, uint8 *body, int bcn_len, bool *cur_bss_bcn)
{
	...
	*cur_bss_bcn = FALSE;
	...
	while (tags_len >= TLV_HDR_LEN &&
...
#ifdef WL11AC
				bcn_bandtype = (bcn_channel <= CH_MAX_2G_CHANNEL ?
		...
		case DOT11_MNG_HE_OP_ID:
			he_op_ie_p = wlc_he_copy_op_ie(wlc->hei, tags, tag_len, &he_op_ie);
			break;
#endif /* STA */
	...
	*cur_bss_bcn = TRUE;
...
#ifdef WL11AX
	if ((BSSCFG_IBSS(bsscfg)) && HE_ENAB_BAND(wlc->pub, bcn_bandtype) && scb != NULL) {
		if (he_cap_ie) {
			wlc_he_bcn_scb_upd(wlc->hei, bcn_bandtype, scb, he_cap_ie,
				he_op_ie_p, he_6g_caps_ie);
		} else {
	...
	pparm.body = (uint8 *)bcn;
	pparm.ht = N_ENAB(wlc->pub);
#if !defined(BAND6G_USING_HT_IES)
	pparm.ht = (pparm.ht && bcn_bandtype != WLC_BAND_6G);
#endif /* !BAND6G_USING_HT_IES) */
#ifdef WL11AC
	pparm.vht = VHT_ENAB(wlc->pub);
#ifndef BAND6G_USING_VHT_IES
	pparm.vht = (pparm.vht && bcn_bandtype != WLC_BAND_6G);
	...
	if (err != BCME_OK) {
		...
		if (err == BCME_REPLAY) {
			...
			WLCNTSCB_COND_INCR((f.key_info.algo != CRYPTO_ALGO_OFF) ? 1 : 0,
				scb->scb_stats.rx_succ_replay_failures);
#ifdef WL_BSS_STATS
			if (f.key_info.algo != CRYPTO_ALGO_OFF) {
				WLCIFCNTINCR(scb, rx_replay_failures);
			}
#endif /* WL_BSS_STATS */
			if (BSSCFG_AP(bsscfg) &&
