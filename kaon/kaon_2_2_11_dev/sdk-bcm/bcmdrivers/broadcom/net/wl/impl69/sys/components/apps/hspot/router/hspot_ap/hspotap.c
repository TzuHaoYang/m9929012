...
#define HSIOV_WL_RRM_NBR_REPORT		"rrm_nbr_report"
#define HSIOV_WL_WMF_BSS_ENABLE		"wl_wmf_bss_enable"
...
#define HSCMD_ESR			"esr"
#define HSCMD_UESA			"uesa"
...
/* Location URI FQDN List */
#define ENC_HELD			"held"
#define ENC_SUPL			"supl"

#define DEF_HELD_URI			"held.example.com"
#define URILIST_ID1			"held.example.com:held"
...
typedef struct
{
	...
	/* Location URI FQDN List */
	bcm_decode_hspot_anqp_location_uri_fqdn_query_t pubid_uri_fqdn_list;
...
static int update_qosmap_ie(hspotApT *hspotap);
...
static void encode_anqp_nbr_report(hspotApT *hspotap, bcm_encode_t *pkt);
/* ------------------------------ Static Function Declaration -------------------------------- */
...
hspotap_free(void)
{
	...
	for (i = 0; i < hspotap_num; i++) {
		...
		if ((strlen(hspotaps[i]->prefix) > 0) && hspotaps[i]->ifr &&
...
decode_hspot_flags(char *prefix, hspotApT *hspotap, char* err_nvram)
{
	...
	if ((hspotap->osen_ie_enabled && !find_in_list(ptr, NVVAL_WPA2)) &&
		(hspotap->osen_ie_enabled && !find_in_list(ptr, "osen"))) {
		hspotap->osen_ie_enabled = 0;
		set_hspot_flag(prefix, HSFLG_OSEN, 0);
	...
	/* QoS MapSet IE Enabled --------------------------------------------------- */
	hspotap->qosMapSet_ie_enabled = get_hspot_flag(prefix, HSFLG_QOSMAP_EN);
...
reset_iw_ie(hspotApT *hspotap, bool bInit, unsigned int flag)
{
	...
	if (!bInit) {
		...
		if (flag & 0x0020) {
			hspotap->iwIe.isEsr = FALSE;
			set_hspot_flag(hspotap->prefix, HSFLG_IWESR_EN, 0);
		}
		if (flag & 0x0040) {
			hspotap->iwIe.isUesa = FALSE;
			set_hspot_flag(hspotap->prefix, HSFLG_IWUESA_EN, 0);
		}
	} else {
		if (flag & 0x0001) {
			hspotap->iw_ie_enabled = FALSE;
		...
		if (flag & 0x0008) {
			...
			if (!strToEther(HESSID_ID1, &hspotap->iwIe.hessid)) {
				HS20_ERROR(HSERR_ARGV_MAC_ERROR);
				err = -1;
			} else {
		...
		if (flag & 0x0020) {
			hspotap->iwIe.isEsr = FALSE;
			set_hspot_flag(hspotap->prefix, HSFLG_IWESR_EN, IWESR_ID1);
		}
		if (flag & 0x0040) {
			hspotap->iwIe.isUesa = FALSE;
			set_hspot_flag(hspotap->prefix, HSFLG_IWUESA_EN, IWUESA_ID1);
		}
	}
	nvram_commit();
	return err;
...
reset_hspot_osup_list(hspotApT *hspotap, bool bInit, unsigned int flag)
{
	...
	if (!bInit) {
		...
		if (flag & 0x0002) {
			...
			for (iter = hspotap->osuplist.osuProviderCount;
					iter < MAX_OSU_PROVIDERS; iter++)
				memset(&hspotap->osuplist.osuProvider[iter].name, 0,
					sizeof(hspotap->osuplist.osuProvider[iter].name));
		}
		/* OSU_Server_URI */
...
static int
reset_u11_location_pubid_uri_list(hspotApT *hspotap, bool bInit)
{
	int ret, err = 0;
	char varname[NVRAM_MAX_PARAM_LEN] = {0};
	memset(&hspotap->pubid_uri_fqdn_list, 0,
			sizeof(bcm_decode_hspot_anqp_location_uri_fqdn_query_t));

	if (!bInit) {
		ret = nvram_unset(strcat_r(hspotap->prefix, NVNM_PUBIDURIFQDNLIST, varname));
		if (ret) {
			HS20_ERROR(HSERR_NVRAM_UNSET_FAILED, varname);
			err = -1;
		}
	} else {
		hspotap->pubid_uri_fqdn_list.isDecodeValid = TRUE;
		hspotap->pubid_uri_fqdn_list.count = 1;
		strncpy_n(hspotap->pubid_uri_fqdn_list.data[0].name, ENC_HELD, URI_FQDN_SIZE + 1);
		hspotap->pubid_uri_fqdn_list.data[0].nameLen = strlen(ENC_HELD);
		hspotap->pubid_uri_fqdn_list.data[0].encoding = (uint8)LOCATION_ENCODING_HELD;

		ret = nvram_set(strcat_r(hspotap->prefix, NVNM_PUBIDURIFQDNLIST, varname),
				URILIST_ID1);
		if (ret) {
			HS20_ERROR(HSERR_NVRAM_SET_FAILED, varname, URILIST_ID1);
			err = -1;
		}
	}
...
static void
encode_u11_lci(hspotApT *hspotap, bcm_encode_t *pkt)
{
	uint8 buffer[BUFF_512] = {0};
	bcm_encode_t duple;
	uint16 buflen = 0;
	int err;
	...
	err = wl_lci(hspotap->ifr, 0, &buflen, buffer);

	if (buflen > 0) {
		bcm_encode_anqp_lci(pkt, buflen, buffer);
	}
}

static void
encode_u11_civic(hspotApT *hspotap, bcm_encode_t *pkt)
{
	uint8 buffer[BUFF_512] = {0};
	bcm_encode_t duple;
	uint16 buflen = 0;
	...
	err = wl_civic(hspotap->ifr, 0, &buflen, buffer);

	if (buflen > 0) {
		bcm_encode_anqp_civic(pkt, buflen, buffer);
	}
}

static void
encode_u11_location_pubid_uri_list(hspotApT *hspotap, bcm_encode_t *pkt)
{
	uint8 buffer[BUFF_256] = {0};
	bcm_encode_t name; int i;
	...
	for (i = 0; i < hspotap->pubid_uri_fqdn_list.count; i++) {
		bcm_encode_hspot_anqp_ap_location_public_id_uri_data(&name,
			hspotap->pubid_uri_fqdn_list.data[i].encoding,
			hspotap->pubid_uri_fqdn_list.data[i].nameLen,
			hspotap->pubid_uri_fqdn_list.data[i].name);
	}

	bcm_encode_anqp_ap_location_public_id_uri(pkt,
		bcm_encode_length(&name), bcm_encode_buf(&name));
}

static void
encode_anqp_nbr_report(hspotApT *hspotap, bcm_encode_t *pkt)
{
	int ret = 0;
	uint8* nbr_list_buf = NULL;
	uint16 buf_len = 1024;
	uint16 bytes_rd = 0;
	int len_copied = 0;

	nbr_list_buf = malloc(buf_len);
	if (!nbr_list_buf) {
		HS20_ERROR(HSERR_MALLOC_FAILED);
		goto exit;
	}
	memset(nbr_list_buf, 0, buf_len);
	ret = wl_get_nbr_list(hspotap->ifr, nbr_list_buf, buf_len, &bytes_rd);
	if (bytes_rd && (ret == 0)) {
		len_copied = bcm_encode_anqp_neighbor_report(pkt, bytes_rd, nbr_list_buf);
	} else {
		HS20_ERROR(HSERR_IOVAR_FAILED, HSIOV_WL_RRM_NBR_REPORT);
	}
exit:
	if (nbr_list_buf) {
		free(nbr_list_buf);
...
/* ------------------------------------------------------------------------------ */
static int
hspot_cmd_esr_handler(hspotApT *hspotap,
	char **argv, char *txData, bool *set_tx_data)
{
	int err = 0;
	if (argv[0] == NULL) {
		HS20_ERROR(HSERR_ARGV_ERROR, HSCMD_ESR);
		return -1;
	}

	hspotap->iwIe.isEsr = (atoi(argv[0]) != 0);
	HS20_INFO(PRNT_FLG, HSCMD_ESR, hspotap->iwIe.isEsr);
	set_hspot_flag(hspotap->prefix, HSFLG_IWESR_EN, hspotap->iwIe.isEsr);
	err = update_iw_ie(hspotap, FALSE);
	return err;
}
...
/* ------------------------------------------------------------------------------ */
static int
hspot_cmd_uesa_handler(hspotApT *hspotap,
	char **argv, char *txData, bool *set_tx_data)
{
	int err = 0;
	if (argv[0] == NULL) {
		HS20_ERROR(HSERR_ARGV_ERROR, HSCMD_UESA);
		return -1;
	}

	hspotap->iwIe.isUesa = (atoi(argv[0]) != 0);
	HS20_INFO(PRNT_FLG, HSCMD_UESA, hspotap->iwIe.isUesa);
	set_hspot_flag(hspotap->prefix, HSFLG_IWUESA_EN, hspotap->iwIe.isUesa);
	err = update_iw_ie(hspotap, FALSE);
	return err;
}
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	reset_iw_ie(hspotap, TRUE, 0x007F);
	...
	hspotap->qosMapSet_ie_enabled = FALSE;
	set_hspot_flag(hspotap->prefix, HSFLG_QOSMAP_EN, hspotap->qosMapSet_ie_enabled);
	update_qosmap_ie(hspotap);
	...
	reset_u11_location_pubid_uri_list(hspotap, TRUE);
	...
	char **argv, char *txData, bool *set_tx_data)
{
	...
	hspotap->qosMapSet_ie_enabled = TRUE;
	set_hspot_flag(hspotap->prefix, HSFLG_QOSMAP_EN, hspotap->qosMapSet_ie_enabled);
	err = update_qosmap_ie(hspotap);
...
update_dgaf_disable(hspotApT *hspotap)
{
	...
	if (isDgafDisabled) {
		...
		/* Disable wmf_bss_enable NVRAM if enable, as it converts the Multicast
		 * Ether destination address in Ether header to Unicast Multicast address
		 * if available, Hence Block multicast feature not able to work in Disallowing
		 * Group addressable Multicast frames from APUT
		 */
		if (nvram_match(strcat_r(hspotap->prefix, "wmf_bss_enable", tmp), "1")) {
			if (wl_wmf_bss_enable(hspotap->ifr, 0) < 0) {
				err = -1;
				HS20_ERROR(HSERR_IOVAR_FAILED, HSIOV_WL_WMF_BSS_ENABLE);
			}
		}
	} else {
		...
		/* Based on existing setting of wmf_bss_enable NVRAM, reset wmf_bss_enable
		 * at full dongle or in NIC model
		 */
		if (nvram_match(strcat_r(hspotap->prefix, "wmf_bss_enable", tmp), "1")) {
			if (wl_wmf_bss_enable(hspotap->ifr, 1) < 0) {
				err = -1;
				HS20_ERROR(HSERR_IOVAR_FAILED, HSIOV_WL_WMF_BSS_ENABLE);
			}
		}
...
update_iw_ie(hspotApT *hspotap, bool disable)
{
	...
	if (hspotap->iw_ie_enabled) {
		...
		bcm_encode_ie_interworking(&ie, hspotap->iwIe.accessNetworkType,
			hspotap->iwIe.isInternet, hspotap->iwIe.isAsra,
			hspotap->iwIe.isEsr, hspotap->iwIe.isUesa,
...
static int
update_qosmap_ie(hspotApT *hspotap)
{
	int err = 0, iter = 0;
	...
	if (hspotap->qosMapSet_ie_enabled) {
		bcm_encode_t ie;
		uint8 buffer[BUFF_256] = {0};
...
#ifdef __CONFIG_DHDAP__
		if (is_dhd) {
			err = dhd_ie(hspotap->ifr, DOT11_MNG_QOS_MAP_ID,
			bcm_encode_length(&ie) - 4, bcm_encode_buf(&ie) + 4);
		}
#endif // endif
...
#ifdef __CONFIG_DHDAP__
		if (is_dhd) {
			err = dhd_ie(hspotap->ifr, DOT11_MNG_QOS_MAP_ID, 0, 0);
		}
#endif // endif
...
process_cli_command(hspotApT *hspotap, char **argv, char *txData)
{
	...
	} else if (strcmp(argv[0], HSCMD_ESR) == 0) {
		argv++;
		err = hspot_cmd_esr_handler(hspotap, argv, txData, &set_tx_data);
	} else if (strcmp(argv[0], HSCMD_UESA) == 0) {
		argv++;
		err = hspot_cmd_uesa_handler(hspotap, argv, txData, &set_tx_data);
...
tcp_receive_handler(void *handle, char *rxData, char *txData)
{
	...
	(void)process_cli_command(current_hspotap, argv, txData);
}
/* =========================== TCP_PORT SUPPORT ========================== */
	...
	if (anqp.anqpQueryListLength > 0) {
		...
		for (i = 0; i < queryList.queryLen; i++) {
			...
			case ANQP_ID_AP_GEOSPATIAL_LOCATION:
				encode_u11_lci(hspotap, &rsp);
				break;
			case ANQP_ID_AP_CIVIC_LOCATION:
				encode_u11_civic(hspotap, &rsp);
				break;
			case ANQP_ID_AP_LOCATION_PUBLIC_ID_URI:
				encode_u11_location_pubid_uri_list(hspotap, &rsp);
				break;
			...
			case ANQP_ID_NEIGHBOR_REPORT:
				encode_anqp_nbr_report(hspotap, &rsp);
				break;
			default:
				break;
			}
	...
	if (anqp.hspot.iconRequestLength > 0) {
		...
		if (bcm_decode_hspot_anqp_icon_request(&ie, &request)) {
			...
			if (strlen(filename) >  0) {
				snprintf(fullpath, sizeof(fullpath), "%s/%s", ICONPATH, filename);
...
main(int argc, char **argv)
{
	...
	while ((ifr = wlif(total_ifr)) != NULL) {
		...
		/* AP Location URI/FQDN List ---------------------------------------- */
		if (!decode_u11_urifqdn_list(prefix, &hspotap->pubid_uri_fqdn_list))
			HS20_WARNING(HSERR_NVRAM_NOT_DEFINED_EX, prefix, NVNM_PUBIDURIFQDNLIST);
		...
		wl_wnm_url(ifr, 0, 0);
		if (get_hspot_flag(hspotap->prefix, HSFLG_ANQP)) {
			wl_app_serve_anqp_rqst(hspotap->ifr, 1);
		}
