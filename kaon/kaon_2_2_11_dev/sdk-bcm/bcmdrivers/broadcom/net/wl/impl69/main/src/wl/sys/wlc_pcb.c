 ...
 *
 * $Id: wlc_pcb.c 791560 2020-10-01 08:23:54Z $
 */
...
/* Callback function for TX packet */
...
/* Callback function for RX packet */
...
BCMATTACHFN(wlc_pcb_attach)(wlc_info_t *wlc)
{
	...
	/* Register with OSL the callback to be called when PKTFREE is called for Rx packet */
	PKTFREESETRXCB(wlc->osh, _wlc_pcb_rxpktfree_fn_invoke, pcbi);
...
/**
 * Determine whether a specific callback <fn,arg> had been registered for this packet.
 */
int BCMFASTPATH
wlc_pcb_pkt_is_registered(wlc_pcb_info_t *pcbi, pkcb_fn_t fn, void *arg, void *pkt)
{
	wlc_pkttag_t *pt = WLPKTTAG(pkt);
	uint16 idx = pt->callbackidx;
	pkt_cb_t *pcb;
...
#if defined(STS_XFER)
	/* TODO: Instead of calling for every Rx packet.
	 * Implement a packet callback decriptor in Rx path and register a packet callback only for
	 * Rx packet with valid PhyRx status
	 */
	if (STS_XFER_PHYRXS_ENAB(pcbi->wlc->pub)) {
		wlc_sts_xfer_phyrxs_free(pcbi->wlc, pkt);
	}
#endif /* STS_XFER */
}
...
wlc_pcb_pktfree_cb_unregister(wlc_pcb_info_t *pcbi, wlc_pcb_pktfree_fn_t fn, void* ctxt)
{
	...
	/* check and wait to protect pktfree_callback content if someone using on it */
	SPINWAIT((pcbi->pktfree_callback->entered == TRUE), 100);
	ASSERT(pcbi->pktfree_callback->entered == FALSE);
