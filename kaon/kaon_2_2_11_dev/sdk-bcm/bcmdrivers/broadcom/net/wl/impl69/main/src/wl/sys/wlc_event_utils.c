 ...
 *
 * $Id: wlc_event_utils.c 792446 2020-10-27 15:04:44Z $
 */
...
wlc_if_event(wlc_info_t *wlc, uint8 what, struct wlc_if *wlcif)
{
	...
	uint8 r;
	wlc_bsscfg_t *primary = wlc->primary_bsscfg;
...
wlc_event_if(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_event_t *e, const struct ether_addr *addr)
{
	...
	if (addr != NULL && !ETHER_ISMULTI(addr) && cfg != NULL &&
		...
		else if (wlc->primary_bsscfg != NULL) {
			_wlc_event_if(wlc, e, cfg, wlcif, wlc->primary_bsscfg->wlcif);
		}
	} else if (wlc->primary_bsscfg != NULL) {
		_wlc_event_if(wlc, e, NULL, NULL, wlc->primary_bsscfg->wlcif);
	}
}
