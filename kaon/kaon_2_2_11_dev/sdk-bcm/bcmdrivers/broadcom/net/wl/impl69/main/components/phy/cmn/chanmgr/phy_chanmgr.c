 ...
 *
 * $Id: phy_chanmgr.c 792280 2020-10-22 01:44:37Z $
 */
...
bool
wlc_phy_is_txbfcal(phy_info_t *pi)
{
...
bool
wlc_phy_is_smth_en(phy_info_t *pi)
{
	phy_chanmgr_info_t *chanmgri = pi->chanmgri;
	phy_type_chanmgr_fns_t *fns = chanmgri->fns;
...
void
wlc_phy_smth_per_nss_SUTxbf(phy_info_t *pi, uint8 uint_val)
{
	phy_chanmgr_info_t *chanmgri = pi->chanmgri;
	phy_type_chanmgr_fns_t *fns = chanmgri->fns;

	if ((fns->smth_per_nss_SUTxbf != NULL) && ISACPHY(pi)) {
		return (fns->smth_per_nss_SUTxbf)(fns->ctx, uint_val);
	}
}

void
wlc_phy_bypass_idletssi_init(phy_info_t *pi, bool force)
{
	phy_chanmgr_info_t *chanmgri = pi->chanmgri;
	phy_type_chanmgr_fns_t *fns = chanmgri->fns;
 ...
 */
void
wlc_phy_chanspec_radio_set(phy_info_t *pi, chanspec_t newch)
{
...
void
wlc_phy_chanspec_set(phy_info_t *pi, chanspec_t chanspec)
{
	phy_chanmgr_info_t *chanmgri = pi->chanmgri;
	phy_type_chanmgr_fns_t *fns = chanmgri->fns;
...
void
phy_chanmgr_sr_enable(phy_info_t *pi, uint8 set_val)
{
	phy_type_chanmgr_fns_t *fns = pi->chanmgri->fns;

	if (fns->sr_enable != NULL)
		(fns->sr_enable)(pi, set_val);
}

void
phy_chanmgr_srg_bssColor(phy_info_t *pi, uint8 set_val, uint16 BM0, uint16 BM1,
	uint16 BM2, uint16 BM3)
{
	phy_type_chanmgr_fns_t *fns = pi->chanmgri->fns;

	if (fns->srg_bssColor != NULL)
		(fns->srg_bssColor)(pi, set_val, BM0, BM1, BM2, BM3);
}
...
#ifdef WLSRVSDB
void
phy_sr_vsdb_reset(phy_info_t *pi)
{
...
#ifdef WLSRVSDB
int
phy_chanmgr_vsdb_force_chans(phy_info_t *pi, uint16* vsdb_chans, uint8 set)
{
	uint16 chans[2];
	...
	if (set) {
		...
		phy_chanmgr_vsdb_sr_attach_module(pi, chans[0], chans[1]);
		...
		phy_chanmgr_vsdb_sr_detach_module(pi);
...
int
phy_chanmgr_vsdb_sr_detach_module(phy_info_t *pi)
{
	uint8 i;

	/* Disable the flags */
	phy_sr_vsdb_reset(pi);
 ...
 */
uint8
phy_chanmgr_vsdb_sr_attach_module(phy_info_t *pi, chanspec_t chan0, chanspec_t chan1)
{
	uint8 i;
 ...
 */
uint8
phy_chanmgr_vsdb_sr_set_chanspec(phy_info_t *pi, chanspec_t chanspec, uint8 *last_chan_saved)
{
