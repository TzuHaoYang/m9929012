...
wlc_cfp_pktlist_free(wlc_info_t *wlc, void *p)
{
	...
	PKTCNTR_INC(wlc, PKTCNTR_CFP_PKTLIST_FREE, count);
...
wlc_cfp_scb_chain_sendup(wlc_info_t *wlc, scb_cfp_t * scb_cfp, uint8 prio)
{
	...
	defined(BCMDBG_MU) || defined(DUMP_MURX)))
		...
		/* If HWA_RXDATA_BUILD is defined then AMSDU subframe header gets
		 * already validated through HWA filtermap, so skipping here.
		 */
#if !defined(HWA_RXDATA_BUILD)
		if (WLPKTTAG(p)->flags & WLF_HWAMSDU) {
			...
			if (!wlc_amsdu_validate_sf_hdr(wlc, scb,
				(struct ether_header *)PKTDATA(wlc->osh, p))) {
				WL_ERROR(("%s: AMSDU deagg attack; Invalid AMSDU\n", __FUNCTION__));
