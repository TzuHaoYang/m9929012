 ...
 *
 * $Id: wlc_lq.c 792224 2020-10-21 06:29:15Z $
 */
...
static void wlc_lq_chanim_us_accum(chanim_info_t* c_info, chanim_cnt_us_t *cur_cnt,
	chanim_acc_us_t *acc);
static void wlc_lq_copy_to_acc_us(wlc_info_t *wlc, chanim_acc_us_t *acc_us,
	wlc_chanim_stats_us_t *stats_us);
...
wlc_lq_chanim_us_accum(chanim_info_t* c_info, chanim_cnt_us_t *cur_cnt_us, chanim_acc_us_t *acc_us)
{
	...
	for (i = 0; i < CCASTATS_MAX; i++) {
		if (last_cnt_us->ccastats_cnt[i] || acc_us->chanspec) {
			if ((cur_cnt_us->ccastats_cnt[i] == 0) &&
				(last_cnt_us->ccastats_cnt[i] != 0)) {
				return;
			}
	...
	if ((cur_cnt_us->busy_tm == 0) && (last_cnt_us->busy_tm != 0)) {
		return;
	}
	...
	if ((cur_cnt_us->rxcrs_pri20 == 0) && (last_cnt_us->rxcrs_pri20 != 0)) {
		return;
	}
	...
	if ((cur_cnt_us->rxcrs_sec20 == 0) && (last_cnt_us->rxcrs_sec20 != 0)) {
		return;
	}
	...
	if ((cur_cnt_us->rxcrs_sec40 == 0) && (last_cnt_us->rxcrs_sec40 != 0)) {
		return;
	}
...
static void
wlc_lq_copy_to_acc_us(wlc_info_t *wlc, chanim_acc_us_t *acc_us, wlc_chanim_stats_us_t *stats_us)
{
	int i = 0;
	...
	acc_us->busy_tm = stats_us->chanim_stats_us.busy_tm;
	acc_us->rxcrs_pri20 = stats_us->chanim_stats_us.rxcrs_pri20;
	acc_us->rxcrs_sec20 = stats_us->chanim_stats_us.rxcrs_sec20;
	acc_us->rxcrs_sec40 = stats_us->chanim_stats_us.rxcrs_sec40;
	for (i = 0; i < CCASTATS_MAX; i++) {
		if (i == CCASTATS_TXOP) {
			acc_us->ccastats_us[i] =
				stats_us->chanim_stats_us.ccastats_us[i] / slottime;
		} else {
			acc_us->ccastats_us[i] =
				stats_us->chanim_stats_us.ccastats_us[i];
		}
	}
}
	...
	if (if_info == NULL) {
		...
		for (i = 0; i < ifaces->num_ifs; i++) {
			if (ifaces->if_info[i].chanspec == 0) {
				...
				wlc_lq_copy_to_acc_us(wlc, if_info->acc_us, if_info->stats_us);
                                 ...
                                 chanspec_t prev_chanspec)
{
	...
	/*
	 * Since we are switching to new channel update  accumulated counter
	 * with new channel stats
	 */
	last_cnt_us->start_tm = OSL_SYSUPTIME();
	last_cnt_us->total_tm = if_info->stats_us->chanim_stats_us.total_tm;
	wlc_lq_copy_to_acc_us(wlc, if_info->acc_us, if_info->stats_us);
			...
			for (j = 0; j < CCASTATS_MAX; j++) {
				if (j == CCASTATS_TXOP) {
					if_info->chanim_stats_us.ccastats_us[j]	=
						...
						if_info->last_acc_us->ccastats_us[j]) * slottime;
					if (if_info->chanim_stats_us.ccastats_us[j] >
