 ...
 *
 * $Id: wlc_pspretend.c 792130 2020-10-20 09:02:25Z $
 */
...
/* Returns TRUE if packet should be freed, FALSE in case it is requeued */
bool
	...
	uint supr_status)
{
	...
	if (supr_status == TX_STATUS_SUPR_PPS) {
		...
		if (SCB_PS_PRETEND(scb) &&
#if defined(WL_PS_SCB_TXFIFO_BLK)
			((wlc->block_datafifo & DATA_BLOCK_PS) || SCB_PS_TXFIFO_BLK(scb)) &&
