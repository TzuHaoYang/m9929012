 ...
 *
 * $Id: phy_ac_rssi.c 791399 2020-09-25 01:50:58Z $
 */
...
wlc_phy_rssi_get_chan_freq_range_acphy(phy_info_t *pi, uint8 core_segment_mapping, uint8 core)
{
	...
	if (ACMAJORREV_33(pi->pubpi->phy_rev)) {
		...
		if (phy_get_phymode(pi) == PHYMODE_80P80) {
			...
			if (PRIMARY_FREQ_SEGMENT == core_segment_mapping)
				chanspec = wf_chspec_primary80_chspec(chanspec);
			else if (SECONDARY_FREQ_SEGMENT == core_segment_mapping)
				chanspec = wf_chspec_secondary80_chspec(chanspec);
		}
	}
	...
	if (CHSPEC_IS2G(chanspec)) {
		...
		if (RADIOID_IS(pi->pubpi->radioid, BCM2069_ID)) {
			...
			PHY_ERROR(("wl%d: %s: invalid radio ID: %x\n", pi->sh->unit, __FUNCTION__,
				pi->pubpi->radioid));
			ASSERT(FALSE);
			return 0;
		}
...
void
phy_ac_rssi_subbandrssi_setup(phy_info_t *pi, uint8 mode)
{
	PHY_TRACE(("%s %d\n", __FUNCTION__, mode));
	...
	// measurement setting
	MOD_PHYREG(pi, trigger_counter, perBandRssiCalWaitCnt, 32);
	MOD_PHYREG(pi, trigger_counter, perBandRssiCalLesiWaitCnt, 16);

	// average len, N x 0.8us
	MOD_PHYREG(pi, perBandRssi_rssi_accum_samps, perBandRssiAccumSamps, 2);

	// pwr to dBm
	MOD_PHYREG(pi, per_band_rssi_ctrl, perBandRssiUseQdbm, 0);

	// 1 = max, 0 = linear average
	MOD_PHYREG(pi, per_band_rssi_ctrl, report_max_avg, mode);

	MOD_PHYREG(pi, perBandRssi_const_qdb, perBandRssiConstQdB, 184);

	WRITE_PHYREG(pi, perBandRssiTROffset_0, 0x7);
	WRITE_PHYREG(pi, perBandRssiTROffset_1, 0x7);
	WRITE_PHYREG(pi, perBandRssiTROffset_2, 0x7);
	WRITE_PHYREG(pi, perBandRssiTROffset_3, 0x7);
	WRITE_PHYREG(pi, perBandRssiTempOffset_1_0, 0);
	WRITE_PHYREG(pi, perBandRssiTempOffset_3_2, 0);

	if (CHSPEC_IS160(pi->radio_chanspec)) {
		WRITE_PHYREG(pi, perBandRssiBwScaling_0_1, 0x0019);
		WRITE_PHYREG(pi, perBandRssiBwScaling_2_3, 0x1900);
	} else if (CHSPEC_IS80(pi->radio_chanspec)) {
		WRITE_PHYREG(pi, perBandRssiBwScaling_0_1, 0xfc00);
		WRITE_PHYREG(pi, perBandRssiBwScaling_2_3, 0x00fc);
	} else if (CHSPEC_IS40(pi->radio_chanspec)) {
		WRITE_PHYREG(pi, perBandRssiBwScaling_0_1, 0xf2f2);
		WRITE_PHYREG(pi, perBandRssiBwScaling_2_3, 0x0);
	} else {
		WRITE_PHYREG(pi, perBandRssiBwScaling_0_1, 0x00f2);
		WRITE_PHYREG(pi, perBandRssiBwScaling_2_3, 0x0);
	}
}

int
phy_ac_rssi_subbandintf_enable(phy_info_t *pi, bool set_val)
{
	phy_ac_rssi_data_t *data = pi->u.pi_acphy->rssii->data;
	...
	if (set_val) {
		// subband interferance
		MOD_PHYREG(pi, perBandRssi_gain_settle_len,
			perBandRssiTrigGainSettleLen, 16);
		MOD_PHYREG(pi, perBandRssi_rssi_trigger_accum_samps,
			perBandRssiTrigAccumSamps, 2);

		// 40mhz clk rate, repeat measurement every 4us.
		MOD_PHYREG(pi, perBandRssi_rssi_trigger_measure_gap,
			perBandRssiTrigGap, 80);
		MOD_PHYREG(pi, per_band_rssi_ctrl, trigger_measure_enable, 1);
	} else {
		MOD_PHYREG(pi, per_band_rssi_ctrl, trigger_measure_enable, 0);
	}

	data->subbandintf_en = set_val;
	return BCME_OK;
}
