/*
 * GAS state machine functions which implements the GAS protocol as defined in 802.11u.
 * GAS is used e.g. by ANQPO
 *
...
static gasEventCallback_t * get_gasEventCallback(bcm_gas_t *gas);

/* ----------------------------------------------------------- */

/** To prevent ROMming shdat issue because of ROMmed functions accessing RAM */
static gasEventCallback_t* BCMRAMFN(get_gasEventCallback)(bcm_gas_t *gas)
{
	wlc_info_t *wlc = (wlc_info_t *)gas->drv;
	return (wl_gas_get_gasEventCallback(wlc));
}
...
static int add_entry(bcm_gas_t *gas)
{
	...
	while (*curr != 0) {
		if ((memcmp(&(*curr)->peer, &gas->peer,	sizeof((*curr)->peer)) == 0) &&
			((*curr)->chspec20 == gas->chspec20) && ((*curr)->drv == gas->drv)) {
#ifdef BCMDBG
			WL_PRMAC(("already existing instance %p", &(*curr)->peer));
#endif // endif
			return FALSE;
		}
		curr = &(*curr)->next;
...
static bcm_gas_t *find_entry(chanspec_t chspec20, struct ether_addr *mac, int isIncoming)
{
	bcm_gas_t **head, **curr;
...
find_gas_entry_core(uint32 packet_id, bcm_gas_t *list)
{
	...
	while (curr != NULL) {
		...
		if (packet_id == gas_ptr_id) {
			WL_NONE(("%s incoming: found %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(curr)));
			return curr;
		}
		curr = curr->next;
	}
...
int bcm_gas_is_active(chanspec_t chspec20, struct ether_addr *mac, uint8 dialogToken,
	int isIncoming)
{
	bcm_gas_t *gas;

	gas = find_entry(chspec20, mac, isIncoming);
...
bcm_gas_t *bcm_gas_create(struct bcm_gas_wl_drv_hdl *drv, int bsscfg_idx,
	void *wl_drv_if, chanspec_t chspec20, struct ether_addr *dst, bool isIncoming)
{
	bcm_gas_t *gas;
	...
	void (*fn)(void *context, bcm_gas_t *gas, bcm_gas_event_t *event))
{
	...
	gasEventCallback_t *p = wl_gas_get_gasEventCallback(wlc);
	p->fn = fn;
	p->context = context;
	return TRUE;
}
...
int bcm_gas_unsubscribe_event(void *context, void (*fn)(void *context,
	...
	wl_event_msg_t *wlEvent, uint8 *data, uint32 length)
{
	...
	case WLC_E_ACTION_FRAME_RX:
	{
		...
		chanspec_t chspec20 = ntoh16(frameInfo->chspec20);
		uint8 *actFrame = (uint8 *) (frameInfo + 1);
		uint32 actFrameLen = length - sizeof(wl_event_rx_frame_data_t);
		bcm_decode_t dec;
		bcm_decode_gas_t gasDecode;

		WL_P2PO(("WLC_E_ACTION_FRAME_RX on channel %d chspec %04x\n",
			CHSPEC_CHANNEL(chspec20), chspec20));
		if (WL_P2PO_ON()) {
		...
		if (bcm_decode_init(&dec, actFrameLen, actFrame) &&
			...
			gas = find_entry(chspec20, src, isIncoming);
			if (gas != 0 &&
...
{
	gasEventCallback_t *p = get_gasEventCallback(gas);
...
static void tx_action_frame(bcm_gas_t *gas, chanspec_t chspec20, uint32 responseTimeout,
...
static void retransmit(bcm_gas_t *gas)
{
	if (gas->retransmit < gas->maxRetransmit) {
		...
		if (wl_actframe(gas->drv, gas->bsscfgIndex,
			BCMGAS_PKT_ID(gas), gas->tx.chspec20, gas->tx.responseTimeout,
	...
	uint8 dialogToken, uint8 advertisementProtocol,	uint16 reqLen, uint8 *req)
{
	...
	tx_action_frame(gas, gas->chspec20, responseTimeout, &gas->peer,
	...
	uint8 advertisementProtocol, uint16 rspLen, uint8 *rsp)
{
	...
	tx_action_frame(gas, gas->chspec20, responseTimeout,
	...
	uint8 advertisementProtocol, uint16 rspLen, uint8 *rsp)
{
	...
	tx_action_frame(gas, gas->chspec20, (fragmentId & MORE_BIT) ?
...
static void rx_query_response(bcm_gas_t *gas, int dataLen, uint8 *data)
{
	...
	WL_NONE(("%s: len=%u txrsp=%p data=%p\n", __FUNCTION__,
		gas->txResponse.length, OSL_OBFUSCATE_BUF(gas->txResponse.data),
...
static state_func_t g_table[STATE_LAST] = {
	state_idle,
	state_tx_request,
	state_tx_response,
	state_tx_comeback_request,
	...
	bcm_decode_gas_t *gasDecode, int dataLen, uint8 *data)
{
	...
	if (gas->state < STATE_LAST && event < EVENT_LAST) {
		...
		(gas_get_table()[gas->state])(gas, event, gasDecode, dataLen, data);
