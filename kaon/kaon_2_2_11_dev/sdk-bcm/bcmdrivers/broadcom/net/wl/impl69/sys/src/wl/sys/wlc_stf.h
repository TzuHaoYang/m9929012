 ...
 *
 * $Id: wlc_stf.h 793742 2020-12-08 17:25:14Z $
 */
...
/** Cubby pertaining to a particular remote node */
#define STF_SCB_CUBBY(stf, scb) \
	((wlc_stf_scb_cubby_t *)SCB_CUBBY((scb), (stf)->scbh_cubby))

#define WLC_STF_GET_SCB_MIMOPS_ENAB(stf, scb) \
	(STF_SCB_CUBBY((stf), (scb))->mimops_enabled)

#define WLC_STF_GET_SCB_MIMOPS_RTS_ENAB(stf, scb) \
	(STF_SCB_CUBBY((stf), (scb))->mimops_rtsmode)

struct wlc_stf_arb_mps_info {
	wlc_info_t  *wlc;
	int cfgh_hwcfg;
	int cfgh_arb;		/**< handle from cubby reserve */
	int cfgh_mimops;	/**< handle from cubby reserve */
};

typedef struct wlc_stf_ocl_info wlc_stf_ocl_info_t;

/** READ ONLY. public properties pertaining to a particular remote node ('scb cubby') */
typedef struct wlc_stf_scb_cubby {
	/* fields are READ ONLY */
	bool mimops_enabled;	/**< cached state: a mimo ps mode is enabled */
	bool mimops_rtsmode;	/**< cached state: TRUE=RTS mimo, FALSE=no mimo */
	/* above READ ONLY */
} wlc_stf_scb_cubby_t;
...
struct wlc_stf {
	wlc_info_t  *wlc;
	int	cfgh_cubby;		/**< BSSCFG cubby handle */
	int	scbh_cubby;	/**< SCB cubby handle (pertaining to a particular remote node) */
	...
	uint8	mimops_mode;		/** local node advertized sm power save support */
	...
	uint8  mclx_2g5g_txchain_mask;	/* MCLX mode user restricted chain masks */
	uint8  mclx_2g5g_rxchain_mask;
	uint8  mclx_6g_txchain_mask;
	uint8  mclx_6g_rxchain_mask;
...
extern uint8 wlc_stf_get_pwrperrate(wlc_info_t *wlc, ratespec_t rspec,
	uint16 spatial_map);
extern int wlc_stf_get_power_offset_per_ratespec(wlc_info_t *wlc, ratespec_t rspec, uint8 *offset,
...
void wlc_stf_set_cfg_mimops_PM(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 v);
uint8 wlc_stf_get_cfg_mimops_PM(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg);
void wlc_stf_mimops_action_send(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 mimops_mode);
int wlc_stf_send_action_mimops(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 mimops_mode);
#ifdef WL_MIMOPS_CFG
void wlc_stf_set_mimops_ActionPM(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 action_pm);
#endif /* WL_MIMOPS_CFG */
void wlc_stf_mimops_cap_update(wlc_info_t *wlc, uint8 mimops_mode);
bool wlc_stf_is_scb_dynamic_smps(wlc_info_t *wlc, struct scb *scb);
bool wlc_stf_mimops_on_info_received(wlc_info_t *wlc, struct scb *scb, bool reinit_ratesel,
