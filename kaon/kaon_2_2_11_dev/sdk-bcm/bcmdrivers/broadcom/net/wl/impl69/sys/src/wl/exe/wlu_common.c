 ...
 *
 * $Id: wlu_common.c 792988 2020-11-10 08:38:49Z $
 */
...
wl_sta_info_all(void *wl, cmd_t *cmd)
{
	...
	if (err != BCME_IOCTL_ERROR) {
		...
		maclist = CALLOC(WLC_IOCTL_MEDLEN);
		if (!maclist) {
		...
		if (!strncmp(*argv, "del", strlen("del"))) {
			...
			while (*argv && maclist->count < max) {
				...
				if (!found) {
					...
					printf("WARNING: entry %s is already "
					"in list\n", wl_ether_etoa(ea));
				}
				argv++;
			}
...
int
wl_scan_ver(void *wl, cmd_t *cmd, char **argv)
{
	void *ptr;
	int err;
	...
	if (*argv) {
		fprintf(stderr,
				"\"%s\" scan_ver iovar doesn't take any arguments\n", *argv);
		return BCME_USAGE_ERROR;
	}
...
int
wl_rsn_ie(void *wl, cmd_t *cmd, char **argv)
{
	int err = 0;
	int ret;
	int bsscfg_idx = 0;
	...
	if (length > 0) {
		if ((err = get_ie_data((uchar *)argv[3], tlv->data, length))) {
			fprintf(stderr, "Error parsing data arg\n");
			free(tlv);
			return err;
		}
	...
	if (bsscfg_idx == -1)
		err = wlu_var_setbuf(wl, "rsnie", tlv, malloc_size);
	else
		err = wlu_bssiovar_setbuf(wl, "rsnie", bsscfg_idx,
			tlv, malloc_size, buf, WLC_IOCTL_MAXLEN);
...
add_one_batched_cmd(int cmd, void *cmdbuf, int len)
{
	...
	new_cmd = CALLOC(sizeof(wl_seq_cmd_pkt_t));
...
wlu_set(void *wl, int cmd, void *cmdbuf, int len)
{
	...
	if (cmd_batching_mode) {
		err = add_one_batched_cmd(cmd, cmdbuf, len);
	}
	else {
...
wl_reg(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argc >= 2) {
		if (str2bandtype(argv[1], &bandtype, BND_IS_STRING)) {
			x.band = bandtype;
		} else {
	...
	if (argc >= 3) {
		if (str2bandtype(argv[2], &bandtype, BND_IS_STRING)) {
			x.band = bandtype;
		} else {
	...
	vndr_ie_setbuf_t **buf, int *buf_len)
{
	...
	ie_setbuf = (vndr_ie_setbuf_t *) CALLOC(buflen);
...
int
wl_scan_init_scan_parse_params(wl_scan_parse_param_t *params)
{
	memcpy(&params->bssid, &ether_bcast, ETHER_ADDR_LEN);
	params->bss_type = DOT11_BSSTYPE_ANY;
	params->scan_type = 0;
	params->scan_type_2 = 0;
	params->nprobes = -1;
...
int
wl_scan_parse_params(void *wl, cmd_t *cmd, char **argv, int *size,
	wl_scan_parse_param_t *params, uint16 version)
{
	int val = 0;
	char key[64];
	int keylen;
	...
	while ((p = *argv) != NULL) {
		...
		if (version >= 2) {
			if (opt == 'R' || !strcmp(key, "RNR")) {
				if (val != 0) {
					params->scan_type_2 |= (WLC_SCANFLAGS_RNR >> 17);
				}
			}
			if (opt == 'P' || !strcmp(key, "PSC")) {
				if (val != 0) {
					params->scan_type_2 |= (WLC_SCANFLAGS_PSC >> 17);
				}
			}
		}

		if (opt == 's' || !strcmp(key, "ssid") || positional_param) {
			nssid = wl_parse_ssid_list(valstr, params->ssid, nssid,
...
int
wl_scan_prep(void *wl, cmd_t *cmd, char **argv, void *params, int *params_size,
	wl_scan_parse_param_t *scan_parse_params, uint16 version)
{
	char *p;
	int err = 0;
	int i;
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		...
		params_v2->scan_type_2 = scan_parse_params->scan_type_2;
	}
	...
	/* copy the parsed scan params to scan_params_v2 */
	nchan = scan_parse_params->channel_num;
	nssid = scan_parse_params->nssid;

	params_v1->scan_type = scan_parse_params->scan_type;
	params_v1->bss_type = scan_parse_params->bss_type;
	params_v1->bssid = scan_parse_params->bssid;

	params_v1->nprobes = htod32(scan_parse_params->nprobes);
	params_v1->active_time = htod32(scan_parse_params->active_time);
	params_v1->passive_time = htod32(scan_parse_params->passive_time);
	params_v1->home_time = htod32(scan_parse_params->home_time);

	for (i = 0; i < nchan; i++) {
		if (version == WL_SCAN_VERSION_MAJOR_V2) {
			params_v2->channel_list[i] =
				htodchanspec(scan_parse_params->channel_list[i]);
		}
		else {
	...
	for (i = 0; i < nssid; i++) {
		scan_parse_params->ssid[i].SSID_len = htod32(scan_parse_params->ssid[i].SSID_len);
	}

	/* For a single ssid, use the single fixed field */
	if (nssid == 1) {
		nssid = 0;
		memcpy(&params_v1->ssid, &scan_parse_params->ssid[0],
			sizeof(scan_parse_params->ssid[0]));
	}

	/* Copy ssid array if applicable */
	if (nssid > 0) {
		if (version == WL_SCAN_VERSION_MAJOR_V2) {
			i = OFFSETOF(wl_scan_params_v2_t, channel_list) + nchan * sizeof(uint16);
		} else {
			i = OFFSETOF(wl_scan_params_v1_t, channel_list) + nchan * sizeof(uint16);
		}
		...
		p = (char*)params + i;
		memcpy(p, scan_parse_params->ssid, nssid * sizeof(wlc_ssid_t));
		p += nssid * sizeof(wlc_ssid_t);
	} else {
		if (version == 2) {
			p = (char*)params_v2->channel_list + nchan * sizeof(uint16);
		} else {
			p = (char*)params_v1->channel_list + nchan * sizeof(uint16);
		}
	}

	params_v1->channel_num = htod32((nssid << WL_SCAN_PARAMS_NSSID_SHIFT) |
	                             (nchan & WL_SCAN_PARAMS_COUNT_MASK));
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		*params_size = p - (char*)params_v2 + nssid * sizeof(wlc_ssid_t);
	} else {
		*params_size = p - (char*)params_v1 + nssid * sizeof(wlc_ssid_t);
	}
exit:
	return err;
}
...
static const char * wlu_rx_pktdrop_reason_strings[RX_PKTDROP_RSN_SIZE] =
	{ RX_PKTDROP_RSN_STRINGS_ARRAY_INITIALIZER };
static const char * wlu_tx_pktdrop_reason_strings[TX_PKTDROP_RSN_SIZE] =
	{ TX_PKTDROP_RSN_STRINGS_ARRAY_INITIALIZER };
static const char * wlu_pktdrop_brief_reason_strings[PKTDROP_BRF_RSN_SIZE] =
	{ PKTDROP_BRF_RSN_STRINGS_ARRAY_INITIALIZER };

/*
 * Prepares the iovar ioctl buffer.  This will be the command string
 * prepared at the user level and passed into the wlc driver.
 * It is the null terminated command string, always followed by the
 * pktdrop_stats_ioctl_request_t structure.  Since alignment is not
 * guaranteed, the structure is copied one byte at a time.
 *
 * The flag carries bits to request enable/disable/clear functions.
 *
 * If mac address is all zeros, it means the request is applicable to the
 * global (per wlc) pktdrop stats table, otherwise it relates to a specific
 * client of the specified mac address.  The mac address is binary (raw bytes)
 * and is NOT printable as is.  Below is the ioctl REQUEST byte format:
 *
 * pktdrop_stats\0<unaligned pktdrop_stats_ioctl_request_t structure>
 *
 * Note that the wlc driver ALWAYS responds with something.
 *
 */
static void
prepare_pktdrop_stats_ioctl_request(unsigned char *buf, const char *cmd_string,
	unsigned int flags, struct ether_addr *ea)
{
	unsigned char *c = buf;
	pktdrop_stats_ioctl_request_t reqst;

	/* copy the iovar cmd & advance the pointer past the terminating null */
	strcpy((char*) c, cmd_string);
	c += (strlen(cmd_string) + 1);

	/* now prepare the reqst structure and append it */
	reqst.flags = flags;
	if (ea) {
		memcpy(&reqst.sta_mac, ea, ETHER_ADDR_LEN);
	} else {
		memset(&reqst.sta_mac, 0, ETHER_ADDR_LEN);
	}

	memcpy(c, &reqst, sizeof(pktdrop_stats_ioctl_request_t));
}

/*
 * returns how many characters are till the end of string
 * in the last line of the string (after the LAST
 * newline character).  This funky looking function
 * is needed since we have some pkt drop descriptions
 * which extend beyond one line and we want to still
 * be able to print them padded.
 */
static int
chars_after_last_newline(const char *s)
{
	int c, i;
...
static void
print_padded(const char *s, int width, char pad)
{
	int remainder;

	printf("%s ", s);
	remainder = width - chars_after_last_newline(s);
	if (remainder > 0) {
		while (remainder--) {
			putchar(pad);
		}
...
static void
display_pktdrop_counters_v1(struct ether_addr *client_macp,
	void *ctrs_v1)
{
	int i, index;
	uint64 *uintp;
	uint64 value;
	char buffer[128];
	pktdrop_stats_ioctl_response_v1_t *ctrs;
	const int padded_size = 70;

	ctrs = (pktdrop_stats_ioctl_response_v1_t*) ctrs_v1;

	if (ctrs->flags & PKTDROP_COUNTERS_FEATURE_DISABLE_BIT) {
		printf("This feature is not enabled on this interface\n");
		return;
	}
	if (ctrs->flags & PKTDROP_COUNTERS_INVALID_STA_BIT) {
		printf("no such client\n");
		return;
	}

	uintp = &ctrs->start_of_counter_value_pairs[0];

	if (ctrs->full_counters_rx_count || ctrs->full_counters_tx_count) {

		printf("packet drop full reasons & counts:\n");

		/* print rx drop reasons & counts */
		for (i = 0; i < ctrs->full_counters_rx_count; i++) {
			index = *uintp++;
			value = *uintp++;
			if (index >= RX_PKTDROP_RSN_SIZE) {
				sprintf(buffer,
					"rx drop reason %d (description unavailable)",
					index);
				print_padded(buffer, padded_size, '.');
			} else {
				print_padded(wlu_rx_pktdrop_reason_strings[index],
					padded_size, '.');
			}
			printf(" %llu\n", value);
		}

		/* print tx drop reasons & counts */
		for (i = 0; i < ctrs->full_counters_tx_count; i++) {
			index = *uintp++;
			value = *uintp++;
			if (index >= TX_PKTDROP_RSN_SIZE) {
				sprintf(buffer,
					"tx drop reason %d (description unavailable)",
					index);
				print_padded(buffer, padded_size, '.');
			} else {
				print_padded(wlu_tx_pktdrop_reason_strings[index],
					padded_size, '.');
			}
			printf(" %llu\n", value);
		}
	...
	if (ctrs->brief_counters_count) {
		printf("packet drop brief reasons & counts:\n");
		for (i = 0; i < ctrs->brief_counters_count; i++) {
			index = *uintp++;
			value = *uintp++;
			if (index >= PKTDROP_BRF_RSN_SIZE) {
				sprintf(buffer,
					"tx drop reason %d (description unavailable)",
					index);
				print_padded(buffer, padded_size, '.');
			} else {
				print_padded(wlu_pktdrop_brief_reason_strings[index],
					padded_size, '.');
			}
			printf(" %llu\n", value);
		}
...
static void
wlu_display_pktdrop_stats(struct ether_addr *client_macp, void *buf)
{
	pktdrop_stats_version_info_t *ctrs_ver;

	ctrs_ver = (pktdrop_stats_version_info_t*) buf;
	switch (ctrs_ver->version) {
		case PKTDROP_COUNTERS_VERSION_1:
			display_pktdrop_counters_v1(client_macp, buf);
			break;
		default:
			fprintf(stderr,
				"cannot display packet drop counters: unknown version %u\n",
				ctrs_ver->version);
	}
}

static int
keyw_present(const char *keyw, char *parsed_word)
{
	return (strncmp(keyw, parsed_word, strlen(keyw)) == 0);
}

/*
 * process the "wl pktdrop_stats" command in the user space.
 */

static void
pktdrop_stats_usage(void)
{
	fprintf(stderr, "usage: wl pktdrop_stats [ enable | disable ]\n");
	fprintf(stderr, "       wl pktdrop_stats "
		"[ <client mac addr> [ full | brief ]] | [ clear ]\n");
}

/*
 * A very rudimentary syntax checker for the pktdrop command
 */
static int
pktdrop_command_syntax_error(int argc, int mac_count, int enable,
	int disable, int clear, int full, int brief)
{
	/* this is ok */
	if (argc == 0) return 0;

	/* there can only be one mac address specified */
	if (mac_count > 1) return 1;

	/* these can occur only once */
	if ((enable + disable + full + brief + clear) > 1) return 1;

	/* these must have a mac */
	if (full || brief) {
		if (mac_count <= 0) return 1;
	}

	/* these cannot have a mac */
	if ((enable + disable + mac_count) > 1) return 1;

	return 0;
}

int
wlu_pktdrop_stats(void *wl, cmd_t *cmd, char **argv)
{
	int argc, rv = 0, size;
	int enable = 0, disable = 0, clear = 0, full = 0, brief = 0;
	unsigned int flags = 0;
	unsigned char *big_buf;
	int mac_count = 0;
	struct ether_addr client_mac;
	struct ether_addr *client_macp = NULL;
	...
	while (*argv) {
		if (keyw_present("enable", *argv)) {
			enable = 1;
			flags |= PKTDROP_COUNTERS_FEATURE_ENABLE_BIT;
			flags &= (~PKTDROP_COUNTERS_FEATURE_DISABLE_BIT);
		} else if (keyw_present("disable", *argv)) {
			disable = 1;
			flags |= PKTDROP_COUNTERS_FEATURE_DISABLE_BIT;
			flags &= (~PKTDROP_COUNTERS_FEATURE_ENABLE_BIT);
		} else if (keyw_present("clear", *argv)) {
			clear = 1;
			flags |= PKTDROP_COUNTERS_CLEAR_BIT;
		} else if (keyw_present("full", *argv)) {
			full = 1;
			flags |= PKTDROP_FULL_COUNTERS_ENABLE_BIT;
			flags &= (~PKTDROP_FULL_COUNTERS_DISABLE_BIT);
		} else if (keyw_present("brief", *argv)) {
			brief = 1;
			flags |= PKTDROP_FULL_COUNTERS_DISABLE_BIT;
			flags &= (~PKTDROP_FULL_COUNTERS_ENABLE_BIT);

		/* only other alternative is a station mac address */
		} else {
			rv = wl_ether_atoe(*argv, &client_mac);
			if (rv) {
				mac_count++;
				client_macp = &client_mac;
			} else {
				pktdrop_stats_usage();
				return -1;
			}
	...
	if (pktdrop_command_syntax_error(argc, mac_count, enable, disable, clear,
		full, brief)) {
			pktdrop_stats_usage();
			return -1;
	}

	/* if no args, global/wlc counters are assumed */
	if (argc == 0) client_macp = NULL;

	/* if a mac address is entered and is null, global/wlc counters are assumed */
	if ((argc == 1) && (mac_count == 1)) {
		if (ETHER_ISNULLADDR(client_macp)) {
			client_macp = NULL;
		}
	}

	/* get the biggest buffer */
	size = PKTDROP_STATS_BUFFER_SIZE;
	big_buf = (unsigned char*) CALLOC(size);
	if (!big_buf) {
		fprintf(stderr, "could not allocate a buffer of %d bytes\n", size);
		return -1;
	}

	/* issue the ioctl command */
	prepare_pktdrop_stats_ioctl_request(big_buf, cmd->name, flags, client_macp);
	rv = wlu_get(wl, cmd->get, big_buf, size);
	if (rv) {
		fprintf(stderr, "This feature is not supported "
			"by wlan driver (failed: %d)\n", rv);
		goto done;
	}

	/* display the results; wlc driver always responds */
	wlu_display_pktdrop_stats(client_macp, big_buf);
done:
	free(big_buf);
	return rv;
}
...
/**
 * a band specification that was entered on the commmand line needs to be translated into something
 * that the driver understands.
 *
 * @return  TRUE if the string argument was recognized as a band argument.
 */
bool
str2bandtype(char *str, uint32 *band, enum bandstring_e str_type)
{
	if (str_type == BND_IS_NUMERICAL) {
		if (!stricmp(str, "5")) {
			*band = WLC_BAND_5G;
		} else if (!stricmp(str, "2")) {
			*band = WLC_BAND_2G;
		} else if (!stricmp(str, "6")) {
			*band = WLC_BAND_6G;
		} else {
			return FALSE;
		}
	} else {
		if (!stricmp(str, "a") || !stricmp(str, "5g") || !stricmp(str, "5G")) {
			*band = WLC_BAND_5G;
		} else if (!stricmp(str, "b") || !stricmp(str, "g") || !stricmp(str, "2g") ||
		           !stricmp(str, "2.4G")) {
			*band = WLC_BAND_2G;
		} else if (!stricmp(str, "6g")) {
			*band = WLC_BAND_6G;
		} else if (str_type == BND_IS_STRING_INC_BAND_ALL && !stricmp(str, "all")) {
			*band = WLC_BAND_ALL;
		} else if (str_type == BND_IS_STRING_INC_BAND_AUTO && !stricmp(str, "auto")) {
			*band = WLC_BAND_AUTO;
		} else {
			return FALSE;
		}
