 ...
 *
 * $Id: phy_txiqlocal.c 791888 2020-10-13 04:07:17Z $
 */
...
int
phy_txiqlocal_get_target_tssi(phy_info_t *pi, int32* tssi_target)
{
	phy_type_txiqlocal_fns_t *fns = pi->txiqlocali->priv->fns;

	if (fns->get_calidx != NULL) {
		return (fns->get_target_tssi)(fns->ctx, tssi_target);
	} else {
		return BCME_UNSUPPORTED;
	}
}
...
int
phy_txiqlocal_set_target_tssi(phy_info_t *pi, int32 tssi_target)
{
	phy_type_txiqlocal_fns_t *fns = pi->txiqlocali->priv->fns;

	if (fns->set_calidx != NULL) {
		return (fns->set_target_tssi)(fns->ctx, tssi_target);
	} else {
		return BCME_UNSUPPORTED;
	}
}
int
phy_txiqlocal_get_tssi_search_enable(phy_info_t *pi, int32* tssi_search_en)
{
	phy_type_txiqlocal_fns_t *fns = pi->txiqlocali->priv->fns;

	if (fns->get_tssi_search_enable != NULL) {
		return (fns->get_tssi_search_enable)(fns->ctx, tssi_search_en);
	} else {
		return BCME_UNSUPPORTED;
	}
}
...
int
phy_txiqlocal_set_tssi_search_enable(phy_info_t *pi, int32 tssi_search_en)
{
	phy_type_txiqlocal_fns_t *fns = pi->txiqlocali->priv->fns;

	if (fns->set_tssi_search_enable != NULL) {
		return (fns->set_tssi_search_enable)(fns->ctx, tssi_search_en);
	} else {
		return BCME_UNSUPPORTED;
	}
}
