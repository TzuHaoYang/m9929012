 ...
 *
 * $Id: phy_tpc.c 793479 2020-11-30 03:20:51Z $
 */
...
/* attach/detach */
phy_tpc_info_t *
BCMATTACHFN(phy_tpc_attach)(phy_info_t *pi, int bandtype)
{
	phy_tpc_info_t *info;
	...
	if ((info->data->ru_pwroffset = ppr_ru_offset_create(pi->sh->osh)) == NULL) {
		PHY_ERROR(("wl%d: %s: out of memory, malloc ru_pwroffset fail\n",
				pi->sh->unit, __FUNCTION__));
		goto fail;
	}
...
BCMATTACHFN(phy_tpc_detach)(phy_tpc_info_t *info)
{
	...
	if (info->data->ru_pwroffset != NULL) {
		ppr_ru_offset_delete(pi->sh->osh, info->data->ru_pwroffset);
	}
...
#ifdef WL_SARLIMIT
	wlc_phy_sar_limit_set(pi, wlc_phy_get_cached_sar_lims(pi->txpwr_cache, pi->radio_chanspec));
#endif // endif
...
wlc_phy_txpower_recalc_target(phy_info_t *pi, ppr_t *txpwr_reg, ppr_t *txpwr_targets)
{
	...
	ppr_ru_offset_clear(pi->tpci->data->ru_pwroffset);
#endif /* WL11AX */
...
#ifdef WL_UCM
	/* Update SISO response power */
	phy_btcx_ucm_update_only_ack_pwr_offset(pi);
#endif /* WL_UCM */
	...
	uint8 filter_offset)
{
	...
	for (i = 0; i < WL_RATESET_SZ_DSSS; i++) {
		dsss->pwr[i] = max_pwr - ((po & 0xf) * 2) - (filter_offset * 2);
		po >>= 4;
	}
}
...
#ifdef WLOLPC
void
wlc_phy_update_olpc_cal(phy_info_t *pi, bool set, bool dbg)
{
	phy_tpc_info_t *ti = pi->tpci;
	phy_type_tpc_fns_t *fns = ti->priv->fns;
...
int8
wlc_phy_calc_ppr_pwr_cap(phy_info_t *pi, uint8 core)
{
...
#ifdef FCC_PWR_LIMIT_2G
static bool
wlc_phy_isvalid_fcc_pwr_limit(phy_info_t *pi)
{
s	uint prev_ch = CHSPEC_CHANNEL(pi->previous_chanspec);
...
void
wlc_phy_prev_chanspec_set(phy_info_t *pi, chanspec_t prev_chanspec)
{
...
void
wlc_phy_txpower_limit_set(phy_info_t *pi, ppr_t *txpwr, chanspec_t chanspec)
{
#ifdef FCC_PWR_LIMIT_2G
	...
	if ((data->cfg->fccpwrch12 > 0 || data->cfg->fccpwrch13 > 0) &&
		...
		if (wlc_phy_isvalid_fcc_pwr_limit(pi)) {
			wlc_phy_fcc_pwr_limit_set(pi, TRUE);
		}
	}
#endif /* FCC_PWR_LIMIT_2G */
...
int
wlc_phy_txpower_set(phy_info_t *pi, int8 qdbm, bool override, ppr_t *reg_pwr)
{
	phy_tpc_info_t *ti = pi->tpci;
	phy_type_tpc_fns_t *fns = ti->priv->fns;
...
void
wlc_phy_txpower_sromlimit(phy_info_t *pi, chanspec_t chanspec, uint8 *min_pwr,
...
int
wlc_phy_txpower_get_current(phy_info_t *pi, ppr_t *reg_pwr, phy_tx_power_t *power)
{
	phy_tpc_info_t *ti = pi->tpci;
	phy_type_tpc_fns_t *fns = ti->priv->fns;
	int ret;
	...
	wlc_phy_get_ppr_board_limits(pi, power->ppr_board_limits);
	 ...
	 * target power fields
	 */
	if ((ret = wlc_phy_get_est_pout(pi, power->est_Pout,
...
#ifdef WL11AX
int
wlc_phy_txpower_get_txctrl(phy_info_t *pi, phy_tx_power_t *power)
{
	int ret;
	...
	ret = wlc_phy_get_txctrl_mu(pi, &power->txctrl_info);
	if (ret != BCME_OK) {
		PHY_ERROR(("wl%d: %s: PHY func fail: %d\n", pi->sh->unit,
			__FUNCTION__, ret));
	}
...
void
wlc_phy_get_tx_power_offset(phy_info_t *pi, ppr_t *txpwr_offset)
{
	ppr_copy_struct(pi->tx_power_offset, txpwr_offset);
}

void
wlc_phy_get_ppr_board_limits(phy_info_t *pi, ppr_t *ppr_board_limits)
{
	ppr_t *txpwr_srom;
	uint8 min_pwr;
	...
	if ((txpwr_srom = ppr_create(pi->sh->osh,
		...
		wlc_phy_txpower_sromlimit(pi, pi->radio_chanspec, &min_pwr, txpwr_srom, 0);
...
bool
wlc_phy_txpower_hw_ctrl_get(phy_info_t *pi)
{
...
#ifdef FCC_PWR_LIMIT_2G
void
wlc_phy_fcc_pwr_limit_set(phy_info_t *pi, bool enable)
{
	phy_tpc_info_t *ti = pi->tpci;
	phy_type_tpc_fns_t *fns = ti->priv->fns;
...
bool
wlc_phy_fcc_pwr_limit_get(phy_info_t *pi)
{
	return pi->tpci->data->fcc_pwr_limit_2g;
}
...
void
wlc_phy_sar_limit_set(phy_info_t *pi, uint32 int_val)
{
	uint core;
...
int
wlc_phy_is_rsdb_mode(phy_info_t *pi)
{
	uint8 pi_idx, active_pi_count = 0;
	phy_info_t *pi_temp;

	FOREACH_PI(pi, pi_idx) {
		pi_temp = phy_get_pi(pi, pi_idx);
		if (pi_temp->sh->clk)
			active_pi_count++;
	}
...
void
wlc_phy_dynamic_sarctrl_set(phy_info_t *pi, bool isctrlon)
{
	...
	if (isctrlon || is_rsdb_mode) {
		...
		if (is_rsdb_mode) {
			...
			if (CHSPEC_IS2G(phy_chanmgr_get_home_chanspec(chanmgri))) {
				if (isctrlon) {
					/* SAR ENABLE ON */
					if (pi->tpci->data->fcc_pwr_limit_2g) {
						sarctrlmap =
						pi->tpci->data->cfg->dynamic_sarctrl_2g_2;
					} else {
						sarctrlmap =
							pi->tpci->data->cfg->dynamic_sarctrl_2g;
					}
				} else {
					/* SAR_ENABLE OFF */
					sarctrlmap = pi->tpci->data->cfg->dynamic_sarctrl_2g_1;
				}
			} else {
				if (isctrlon) {
					/* SAR ENABLE ON */
					if (pi->tpci->data->fcc_pwr_limit_2g) {
						sarctrlmap =
						pi->tpci->data->cfg->dynamic_sarctrl_5g_2;
					} else {
						sarctrlmap =
							pi->tpci->data->cfg->dynamic_sarctrl_5g;
					}
				} else {
					/* SAR_ENABLE OFF */
					sarctrlmap = pi->tpci->data->cfg->dynamic_sarctrl_5g_1;
				}
			}
		} else {
			/* NON-RSDB detected */
			...
			if (CHSPEC_IS2G(phy_chanmgr_get_home_chanspec(chanmgri))) {
				if (pi->tpci->data->fcc_pwr_limit_2g) {
					sarctrlmap = pi->tpci->data->cfg->dynamic_sarctrl_2g_2;
				} else {
					sarctrlmap = pi->tpci->data->cfg->dynamic_sarctrl_2g;
				}
			} else {
				if (pi->tpci->data->fcc_pwr_limit_2g) {
					sarctrlmap = pi->tpci->data->cfg->dynamic_sarctrl_5g_2;
				} else {
					sarctrlmap = pi->tpci->data->cfg->dynamic_sarctrl_5g;
				}
			}
#if (MAX_RSDB_MAC_NUM > 1)
		}
	...
	wlc_phy_sar_limit_set_percore(pi->pubpi, sarctrlmap);
}
#endif /* WL_SAR_SIMPLE_CONTROL */

void
BCMRAMFN(wlc_phy_avvmid_txcal)(phy_info_t *pi, wlc_phy_avvmid_txcal_t *val, bool set)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
...
void
wlc_phy_txpwr_percent_set(phy_info_t *pi, uint8 txpwr_percent)
{
	pi->tpci->data->txpwr_percent = txpwr_percent;
}

void
wlc_phy_txpwr_degrade(phy_info_t *pi, uint8 txpwr_degrade)
{
	pi->tpci->data->txpwr_degrade = txpwr_degrade;
}

int
wlc_phy_txpower_core_offset_set(phy_info_t *pi, struct phy_txcore_pwr_offsets *offsets)
{
...
int
wlc_phy_txpower_core_offset_get(phy_info_t *pi, struct phy_txcore_pwr_offsets *offsets)
{
	int err = BCME_UNSUPPORTED;
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
...
/* user txpower limit: in qdbm units with override flag */
int
wlc_phy_txpower_get(phy_info_t *pi, int8 *qdbm, bool *override)
{
	ASSERT(qdbm != NULL);
...
/* user txpower limit: in qdbm units with override flag */
int
wlc_phy_neg_txpower_set(phy_info_t *pi, uint qdbm)
{
	if (pi->sh->up) {
		if (SCAN_INPROG_PHY(pi)) {
			PHY_TXPWR(("wl%d: Scan in progress, skipping txpower control\n",
...
int
phy_tpc_txbackoff_set(phy_info_t *pi, int16 txpwr_offset)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
	int retval = BCME_OK;
...
bool
phy_tpc_txbackoff_is_enabled(phy_info_t *pi)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
	bool retval = FALSE;
...
bool
phy_tpc_txbackoff_enable(phy_info_t *pi, bool enable)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
	bool retval = FALSE;
...
int
phy_tpc_txbackoff_dump(phy_info_t *pi, phy_txpwrbackoff_info_t *pdata)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
...
int16
phy_tpc_get_vtherm_pwrbackoff(phy_info_t *pi)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
	int16 retval = 0;
...
void
BCMRAMFN(wlc_phy_set_country)(phy_info_t *pi, const char *val)
{
	pi->tpci->data->ccode_ptr = val;
}

int8
BCMRAMFN(wlc_phy_maxtxpwr_lowlimit)(phy_info_t *pi)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
...
int wlc_phy_get_est_pout(phy_info_t *pi, uint8* est_Pout,
...
#ifdef WL11AX
int
wlc_phy_get_txctrl_mu(phy_info_t *pi, phy_txctrl_info_t* txctrl_info)
{
	phy_tpc_info_t *tpci = pi->tpci;
	phy_type_tpc_fns_t *fns = tpci->priv->fns;
	phy_type_tpc_ctx_t *ctx = fns->ctx;
...
int32
phy_tpc_get_min_power_limit(phy_info_t *pi)
{
	return (int32)pi->min_txpower;
}

uint8
phy_tpc_get_target_min(phy_info_t *pi)
{
	uint8 core;
...
void
phy_tpc_get_target_max_per_core(phy_info_t *pi, phy_tx_targets_per_core_t *max_per_core)
{
	memcpy(max_per_core->pwr, pi->tx_power_max_per_core, sizeof(*max_per_core));
}

uint8
phy_tpc_get_target_max(phy_info_t *pi)
{
	uint8 core;
	uint8 tx_pwr_max = 0;
...
uint8
phy_tpc_get_power_backoff(phy_info_t *pi)
{
...
bool
phy_tpc_ipa_ison(phy_info_t *pi)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
...
#ifdef BCMDBG
void
phy_tpc_dump_txpower_limits(phy_info_t *pi, ppr_t* txpwr)
{
	int i;
 ...
 */
void
phy_tpc_set_txpower_hw_ctrl(phy_info_t *pi, bool hwpwrctrl)
{
	phy_type_tpc_fns_t *fns = pi->tpci->priv->fns;
...
void phy_tpc_set_txpwr_cache(phy_info_t *pi, tx_pwr_cache_entry_t* cacheptr)
{
