 ...
 *
 * $Id: wlc_scb.c 791589 2020-10-02 21:48:13Z $
 */
...
wlc_scbfree(wlc_info_t *wlc, struct scb *scbd)
{
	...
	if (wlc_scb_tot_inflt_fifocnt(wlc, scbd)) {
		WL_INFORM(("%s: %d packets pending for "MACF" in Tx FIFO\n", __FUNCTION__,
			wlc_scb_tot_inflt_fifocnt(wlc, scbd), ETHER_TO_MACF(scbd->ea)));
	}
...
#endif /* AP */
victim_found:
	WL_ASSOC(("wl%d.%d: %s: reclaim scb %s, idle %d sec\n",  WLC_UNIT(wlc),
		WLC_BSSCFG_IDX(scb->bsscfg), __FUNCTION__,
...
wlc_scb_setstatebit(wlc_info_t *wlc, struct scb *scb, uint8 state)
{
	...
	WL_ASSOC(("wl%d.%d: %s: "MACF": set bits 0x%02x %s\n",
		WLC_UNIT(wlc), WLC_BSSCFG_IDX(scb->bsscfg),
		__FUNCTION__, ETHER_TO_MACF(scb->ea), state,
		wlc_scb_state_ascii(state, state_str)));
	...
	WL_ASSOC(("wl%d.%d: %s: "MACF": oldstate = 0x%02x, scb->state = 0x%02x %s\n",
		...
		oldstate, scb->state,
		wlc_scb_state_ascii(scb->state, state_str)));
