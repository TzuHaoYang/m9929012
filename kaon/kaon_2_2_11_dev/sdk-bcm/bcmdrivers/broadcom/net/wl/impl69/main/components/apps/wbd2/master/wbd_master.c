 ...
 *
 * $Id: wbd_master.c 793876 2020-12-14 07:46:14Z $
 */
...
wbd_master_retrieve_blanket_nvram_config(wbd_blanket_master_t *wbd_master)
{
	...
	/* Independent Channel Scan Reporting is enabled or not */
	if (blanket_get_config_val_int(NULL, NVRAM_MAP_CHSCAN_INDEP_SCAN,
		DEF_MAP_CHSCAN_INDEP_SCAN)) {
		wbd_master->flags |= WBD_FLAGS_BKT_INDEP_CHSCAN_ENAB;
	}
...
wbd_ieee1905_ap_auto_config_resp_cb(i5_dm_device_type *i5_device)
{
	...
	int is_controller_shutdown_in_progress = 0;
	...
	if (blanket_get_config_val_int(NULL, NVRAM_MAP_BRINGDOWN_CONTROLLER, 0)) {
		wbd_master_update_multiap_mode_to_agent();
		wbd_do_rc_restart_reboot(WBD_FLG_NV_COMMIT | WBD_FLG_RC_RESTART);
		is_controller_shutdown_in_progress = 1;
	}

	WBD_EXIT();

	return is_controller_shutdown_in_progress;
}
