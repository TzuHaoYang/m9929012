 ...
 *
 * $Id: phy_ac_tpc.c 792709 2020-11-02 17:21:46Z $
 */
#include <phy_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
static const uint32 lowrate_tssi_delay_set47_1[13][2] = {
	{470,470},  /* pdet_id = 0 */
	{470,470},  /* pdet_id = 1 */
	{470,470},  /* pdet_id = 2 */
	{470,470},  /* pdet_id = 3 */
...
static const uint32 lowrate_tssi_delay_set130[4][3] = {
	/* 2G/5G/5G-80MHz */
...
#if BAND6G
	if (SROMREV(pi->sh->sromrev) >= 18 && (BAND_6G(ti->bandtype))) {
...
#define MINPWRRANGE_11DBM	44
	...
	if (RADIOREV(pi->pubpi->radiorev) == 4 || RADIOREV(pi->pubpi->radiorev) == 8 ||
		ACMAJORREV_2(pi->pubpi->phy_rev) || ACMAJORREV_5(pi->pubpi->phy_rev) ||
		ACMAJORREV_4(pi->pubpi->phy_rev) || ACMAJORREV_40_128(pi->pubpi->phy_rev) ||
		IS_4364_1x1(pi)) {
...
wlc_phy_txpwrctrl_pwr_setup_srom12_acphy(phy_info_t *pi)
{
	...
	if (ACMAJORREV_128(pi->pubpi->phy_rev) || ACMAJORREV_51_131(pi->pubpi->phy_rev)) {
		...
		if (chan_freq_range < WL_CHAN_FREQ_RANGE_5G_BAND5) {
			if (PHY_AS_80P80(pi, pi->radio_chanspec)) {
				...
				phy_ac_tpc_get_paparams_srom12(pi,
					chan_freq_range, a, b, c, d);
			}
		} else {
#if BAND6G
			/* Get additional 6g pwrdet params from SROM for current subband */
			phy_ac_tpc_get_5gext_paparams_srom18(pi, chan_freq_range, a, b, c, d);
#endif /* BAND6G */
		}
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
		...
		FOREACH_CORE(pi, core) {
			...
			case WL_CHAN_FREQ_RANGE_5G_BAND4_40:
				/* Adjust index for 40M */
				a[core] =  pwrdet->pwrdet_a_40[core][chan_freq_range -
					WL_CHAN_FREQ_RANGE_2G_40];
				b[core] =  pwrdet->pwrdet_b_40[core][chan_freq_range -
					WL_CHAN_FREQ_RANGE_2G_40];
				c[core] =  pwrdet->pwrdet_c_40[core][chan_freq_range -
					WL_CHAN_FREQ_RANGE_2G_40];
				d[core] =  pwrdet->pwrdet_d_40[core][chan_freq_range -
					WL_CHAN_FREQ_RANGE_2G_40];
				break;
			case WL_CHAN_FREQ_RANGE_5G_BAND0_80:
			case WL_CHAN_FREQ_RANGE_5G_BAND1_80:
			...
			case WL_CHAN_FREQ_RANGE_5G_BAND4_80:
				/* Adjust index for 80M */
				a[core] =  pwrdet->pwrdet_a_80[core][chan_freq_range -
					WL_CHAN_FREQ_RANGE_5G_BAND0_80];
				b[core] =  pwrdet->pwrdet_b_80[core][chan_freq_range -
					WL_CHAN_FREQ_RANGE_5G_BAND0_80];
				c[core] =  pwrdet->pwrdet_c_80[core][chan_freq_range -
					WL_CHAN_FREQ_RANGE_5G_BAND0_80];
				d[core] =  pwrdet->pwrdet_d_80[core][chan_freq_range -
					WL_CHAN_FREQ_RANGE_5G_BAND0_80];
				break;
			default:
				PHY_ERROR(("wl%d: %s: pwrdet core%d: a=%d b=%d c=%d d=%d\n",
	...
	if (IS_FCBS(pi) && pi->phy_fcbs->FCBS_INPROG)
		...
		if (ACMAJORREV_32(pi->pubpi->phy_rev) || ACMAJORREV_33(pi->pubpi->phy_rev) ||
		    ACMAJORREV_37(pi->pubpi->phy_rev) || ACMAJORREV_51_131(pi->pubpi->phy_rev)) {
...
static bool BCMATTACHFN(wlc_phy_txpwr_5gext_srom18_read)(phy_type_tpc_ctx_t *ctx)
{
	...
	srom18_5gext_pwrdet_t *pwrdet_5gext = pi->pwrdet_ac_5gext;
	uint8 b = 0, core;
	uint8 i = 0, maxval = 0, maxval_n160 = 0;
	ASSERT(pi->sh->sromrev >= 18);

	maxval = (CH_6G_BANDEXT + 1) * 4;
	maxval_n160 = (CH_6G_BANDEXT) * 4;
	/* read pwrdet params for each band/subband/bandwidth */
	for (i = 0; i < maxval; i = i + 4) {
		if (i < maxval_n160) {
			/* 6GEXT_BANDS_20MHz */
	...
	FOREACH_CORE(pi, core) {
		...
		if (core < 3) {
			...
			pwrdet_5gext->pdoffset20in160[core][2] =
					PDOFFSET(pi, rstr_pdoffset20in160m5gcore3_2, 3);
			pwrdet_5gext->pdoffset40in160[core][0] =
...
#if BAND6G
	if (SROMREV(pi->sh->sromrev) >= 18 && (BAND_6G(tpci->ti->bandtype))) {
		srom18_5gext_pwrdet_t *pwrdet_5gext = pi->pwrdet_ac_5gext;
		switch (freq_range) {
...
wlc_mac_register_init_for_hwppr(phy_info_t *pi)
{
	...
	uint16 tpcTargetPwr = 108, tpcBackOffTb = 6;		/* reg(TXE_PPR_CFG1) */
	...
	uint16 AdjPwr = 5, maxAllowedTxPwr = 127;		/* reg(TXE_PPR_CTL0) */
