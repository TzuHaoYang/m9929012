 ...
 *
 * $Id: wlc_rate.c 793899 2020-12-14 16:19:21Z $
 */
 ...
 */
void
wlc_rateset_ht_mcs_upd(wlc_rateset_t *rs, uint8 nstreams)
{
	int i;
 ...
 *     'mcsallow': bitmask carrying bits such as WLC_MCS_ALLOW_HT and WLC_MCS_ALLOW_VHT.
 */
void
	...
	uint xmask, uint8 mcsallow)
{
	...
	/* Combination of CCK rates with HT,VHT or HE rates is disallowed */
	ASSERT(rates != WLC_RATES_CCK ||
	       (mcsallow & (WLC_MCS_ALLOW_HT | WLC_MCS_ALLOW_VHT | WLC_MCS_ALLOW_1024QAM |
		WLC_MCS_ALLOW_HE)) == 0);

	/* Note that this is not standard compliant to allow VHT but not HT.
	 * See 802.11-2016 section 21.1.1
	 */
	/* VHT rates can only be enabled when HT rates are enabled as well */
	ASSERT(rates == WLC_RATES_CCK || !(mcsallow & WLC_MCS_ALLOW_VHT) ||
		(mcsallow & WLC_MCS_ALLOW_HT));

	/* QAM1024 VHT rates can only be enabled when VHT rates are enabled as well */
	ASSERT(rates == WLC_RATES_CCK || !(mcsallow & WLC_MCS_ALLOW_1024QAM) ||
		(mcsallow & WLC_MCS_ALLOW_VHT));

	if (mcsallow & WLC_MCS_ALLOW_HT) {
		if (src != dst)
			memcpy(&dst->mcs[0], &src->mcs[0], MCSSET_LEN);
		if (!(mcsallow & WLC_MCS_ALLOW_PROP_HT))
			wlc_rate_clear_prop_11n_mcses(dst->mcs);
	} else {
		wlc_rateset_ht_mcs_clear(dst);
	}
 ...
 * @param mcsallow    Bitmask carrying bits such as WLC_MCS_ALLOW_HT and WLC_MCS_ALLOW_VHT
 */
void
	...
	uint8 mcsallow, uint8 bw, uint8 txstreams)
{
	...
	wlc_rateset_ht_bw_mcs_filter(&rs_sel, bw);
	wlc_rateset_ht_mcs_upd(&rs_sel, txstreams);
	...
	bool ldpc, uint8 ratemask, uint8 txstreams)
{
	...
	return reprate;
} /* wlc_get_highest_rate */

void
wlc_rateset_ht_mcs_clear(wlc_rateset_t *rateset)
{
	uint i;
...
void
wlc_rateset_ht_mcs_build(wlc_rateset_t *rateset, uint8 nstreams)
{
	bcopy(&cck_ofdm_mimo_rates.mcs[0], &rateset->mcs[0], MCSSET_LEN);
	wlc_rateset_ht_mcs_upd(rateset, nstreams);
}

/** Based on bandwidth passed, allow/disallow MCS 32 in the rateset */
void
wlc_rateset_ht_bw_mcs_filter(wlc_rateset_t *rateset, uint8 bw)
{
	BCM_REFERENCE(bw);
...
/*
 * @param[in] vht_mcsmap   Bitmap, two bits per stream.
 * @param[in] nstreams     Number of streams
 * @param[in] mcsallow     A two bits value
 * return                  Filtered bitmap, two bits per stream
 */
uint16
wlc_rateset_vht_filter_mcsmap(uint16 vht_mcsmap, uint8 nstreams, uint8 mcsallow)
{
	int stream;
