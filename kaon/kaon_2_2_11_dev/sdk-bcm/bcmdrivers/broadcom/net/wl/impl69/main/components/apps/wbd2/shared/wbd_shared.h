 ...
 *
 * $Id: wbd_shared.h 791992 2020-10-15 11:36:03Z $
 */
...
/* MBO oui length and offset */
#define MIN_MBO_IE_LEN				4 /* OUI[3] + OUI_TYEP[1] */
...
/* All the NVRAMs for SSID */
typedef struct wbd_cmd_vndr_ssid_nvram {
	ieee1905_ssid_type ssid;			/* SSID */
	uint8 n_nvrams;					/* Number NVRAMs for this SSID */
	wbd_cmd_vndr_nvram_t *nvrams;			/* List of NVRAMs */
} wbd_cmd_vndr_ssid_nvram_t;
...
typedef struct wbd_cmd_vndr_nvram_set {
	...
	uint8 mac[MAC_ADDR_LEN];			/* Interface MAC address for SSID nvrams */
	uint8 n_ssid;					/* Number of NVRAMs based on SSID */
	wbd_cmd_vndr_ssid_nvram_t *ssid_nvrams;		/* NVRAMs based on SSID */
...
extern void wbd_strrep(char *list, char c_old, char c_new);

/* In the space-separated/null-terminated list(haystack), find total number of "needle" s */
extern int wbd_count_needles(char *haystack);
...
void wbd_get_basic_common_vlan_config(ieee1905_config *config, uint16 *sec_vlan_id);
 ...
 * @param config		Config to be passed to IEEE1905 module
 *
