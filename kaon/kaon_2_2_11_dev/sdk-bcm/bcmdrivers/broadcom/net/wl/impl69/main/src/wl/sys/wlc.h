 ...
 *
 * $Id: wlc.h 793742 2020-12-08 17:25:14Z $
 */
...
#define BCNMISC_SPARE			(1<<3)	/* available */
#define BCNMISC_MONITOR			(1<<4)	/* bcns promisc mode override for monitor */
#define BCNMISC_AXSTA_OBSS_NBWRU	(1<<5)	/* bcns promisc mode override for OBSS
						 * Narrow Bandwidth RU tolerance check
						 */
...
/* roam->type values */
/* ROAM_FULL executes a full broadcast scan across all channels. The purpose of this scan is not
 * only to find possible join targets, but to also collect information about the surrounding
 * environment. BSSID and channel information from the AP's with strongest RSSI is saved away in the
 * roam cache for later use.
 */
#define ROAM_FULL		0
/* ROAM_PARTIAL scans only those channels that are represented in the roam cache. Partial roam scans
 * occur at periodic intervals.
 */
#define ROAM_PARTIAL		1
/* ROAM_SPLIT_PHASE: the full scan will be split into two phases with the channels in neighbor list
 * being scanned in 1st phase. The rest of channel will be scanned in 2nd phase if no good roaming
 * target can be found in 1st phase. The source of neighbor list may come from the AP neighbor list
 * of beacon frame; If no AP list available from beacon, the list is filled with accumulated
 * previous scan results which includes at least the home channel.
 */
...
#define BAND5G_GAP(bandunit, ch) \
		(((bandunit) != BAND_5G_INDEX) ? 0 : \
			((ch) == CH_MAX_5G_SUB1_CHANNEL) ? 32 : \
			((ch) == CH_MAX_5G_SUB2_CHANNEL) ? 1 : 0)

#ifdef WFA_6G_NEW_CHANNELIZATION
/* For 6G channel 2 */
#define BAND6G_GAP(bandunit, ch) \
		(((bandunit) != BAND_6G_INDEX) ? 0 : \
			((ch) == CH_MIN_6G_CHANNEL) ? -3 : \
			((ch) == 2) ? -1 :0)
#else
#define BAND6G_GAP(bandunit, ch) (0)
#endif /* WFA_6G_NEW_CHANNELIZATION */

/**
 * FOREACH_WLC_BAND_CHANNEL20() iterates over all 20MHz channels in the given band using
 * the channel range and spacing information from the band. It also takes the gap in 5G
 * band into account by properly doing additional increment.
 */
#define FOREACH_WLC_BAND_CHANNEL20(band, ch) \
	for ((ch) = (band)->first_ch; (ch) <= (band)->last_ch; \
		(ch) += (band)->ch_spacing + BAND5G_GAP((band)->bandunit, ch) \
		+ BAND6G_GAP((band)->bandunit, ch))
...
#define WLC_CORENUM_PHY(w)	(phy_utils_get_corenum(WLC_PI((w))))
...
/* 43684 & 6715 support 80Mhz/160Mhz @ 4x4 */
/* 6756 supports 80/160Mhz @ 2x2 */
#define WLC_PHY_160_FULL_NSS(wlc) (ACREV_IS(wlc->band->phyrev, 47) || \
	ACREV_IS(wlc->band->phyrev, 130) || ACREV_IS(wlc->band->phyrev, 131))
...
#define WLC_RNR_UPDATE_METHOD_STATIC	0
#define WLC_RNR_UPDATE_METHOD_SCAN	1
...
/* Reduced Neibor Report timer */
#define RNR_UPDATE_START_PERIOD		1000 /* Initial scan period right after wl up in ms */
#define RNR_UPDATE_STATIC_PERIOD	1000 /* msec for rnr update with static popluation of nbr */
#define RNR_UPDATE_SCAN_PERIOD		10000 /* msec for rnr update with AP scan */
#define RNR_UPDATE_ADD_TIMER(wlc, to)	wl_add_timer(wlc->wl, (wlc)->rnr_update_timer, (to), 0)
#define RNR_UPDATE_DEL_TIMER(wlc)	wl_del_timer(wlc->wl, (wlc)->rnr_update_timer)
#define RNR_UPDATE_FREE_TIMER(wlc)	wl_free_timer(wlc->wl, (wlc)->rnr_update_timer)

#define WLC_NBR_DISC_CAP_FD_BCAST_PRB_RSP_MASK			0X00000060 /* bit 5 and 6 */
#define WLC_NBR_DISC_CAP_BOTH_FD_BCAST_PRB_RSP_ENABLED		0X03
#define WLC_NBR_DISC_CAP_FD_BCAST_PRB_RSP_BIT_SHIFT			0X05

#define IS_WLC_NBR_DISC_CAP_BOTH_FD_BCAST_PRB_RSP_BIT_SET(val)		\
	((((val) & WLC_NBR_DISC_CAP_FD_BCAST_PRB_RSP_MASK) >>			\
		WLC_NBR_DISC_CAP_FD_BCAST_PRB_RSP_BIT_SHIFT) ==		\
		WLC_NBR_DISC_CAP_BOTH_FD_BCAST_PRB_RSP_ENABLED)

#define WL_PRB_RSP_CANCEL_GRACE_PERIOD		4 /* msec */

#define UNSOLICITED_BCAST_PRB_RSP_TX_PERIOD	20 /* ms */

#define UPR_FD_START_TIMER		0X01
#define WLC_UPR_FD_STOP_TIMER(upr_fd_info)	(((upr_fd_info)->flags) &= ~UPR_FD_START_TIMER)
#define WLC_UPR_FD_MAX_PKTS_IN_FLIGHT		1
...
struct wlcband {
	...
	uint8		first_ch;		/**< first channel in this band */
	uint8		last_ch;		/**< last channel in this band */
	uint8		ch_spacing;		/**< 20MHz channel spacing in this band */
};
...
* XXX: to be removed. There is no wlc[x] anymore, no RSDB
*/
...
#define WLC_USER_WAKE_SPARE_0x8		0x00000008
...
/* WL_PKTCNTR allows tracking  number of packets passing through different stages in wl driver.
 * The counters are cleared on read.
 * These counters can be polled by an user every second (to get packets per second stats)
 * or as needed.
 * This facility can also aid dev/local debugging where new packet counters can be temporarily
 * added for specific scenarios in WL driver.
 * Following set of packet counters have been added for the no-runner, no-flowcache NIC datapath.
 * Similar counters can be added for other datapaths too.
 * Any new entry for a new counter must be added before PKTCNTR_LAST.
 *
 * PKTCNTR_WL_IN_TXQ_TOT		- # pkts queued at WL ingress wl->txq
 * PKTCNTR_WL_IN_TXQ_DROP		- # pkts dropped when wl->txq_thresh is reached
 * PKTCNTR_WLPF_PKTLIST_CFP		- # pkts transposed from WL pktfwd pktlist into CFP pktlist
 * PKTCNTR_WLPF_PKTLIST_PKTC    - # pkts transposed from WL pktfwd pktlist into packet chain (PKTC)
 * PKTCNTR_WLPF_PKTLIST_SINGLE  - # pkts transferred one by one from WL pktlist to WL driver
 * PKTCNTR_CFP_ENQ_SCB_AMPDU    - # pkts enqueued into SCB AMPDU Queues from CFP entry point
 * PKTCNTR_CFP_PKTLIST_FREE     - # pkts freed up in CFP pktlist
 * PKTCNTR_PKTC_ENQ_SCB_AMPDU   - # pkts enqueued into SCB AMPDU Queues via legacy PKTC path
 * PKTCNTR_PKTC_SCB_AMPDU_DROP  - # pkts dropped due to SCB AMPDU Queue being full
 * PKTCNTR_AMSDU_PASSTHROUGH    - # pkts where AMSDU agg is skipped in legacy path
 * PKTCNTR_CFP_AMPDU_RELEASE    - # pkts released from SCB AMPDU Q in CFP path
 * PKTCNTR_AMPDU_RELEASE        - # pkts released from SCB AMPDU Q in legacy path
 * PKTCNTR_MPDU_ENQ_TXFIFO      - # MPDUs enqueued into Tx FIFO
 * PKTCNTR_MPDU_TXS_AQMC_FREE   - # MPDUs freed up on successful txstatus
 * PKTCNTR_LAST
 *
 * Use "wl dump pktcntr" to check the packet counters.
 *
 *
 * Sample output for downstream traffic via CFP:
 *
 * # wl dump pktcntr
 * WL Driver Packet Counters:
 * --------------------------
 *  Id      PktCntr                         Cnt
 *
 *  0       PKTCNTR_WL_IN_TXQ_TOT           5
 *  1       PKTCNTR_WL_IN_TXQ_DROP          0
 *  2       PKTCNTR_WLPF_PKTLIST_CFP        6697188
 *  3       PKTCNTR_WLPF_PKTLIST_PKTC       162
 *  4       PKTCNTR_CFP_ENQ_SCB_AMPDU       6697188
 *  5       PKTCNTR_CFP_PKTLIST_FREE        0
 *  6       PKTCNTR_PKTC_ENQ_SCB_AMPDU      4
 *  7       PKTCNTR_PKTC_SCB_AMPDU_DROP     0
 *  8       PKTCNTR_AMSDU_PASSTHROUGH       0
 *  9       PKTCNTR_CFP_AMPDU_RELEASE       1521487
 *  10      PKTCNTR_AMPDU_RELEASE           1194
 *  11      PKTCNTR_MPDU_ENQ_TXFIFO         1522849
 *  12      PKTCNTR_MPDU_TXS_AQMC_FREE      1522541
 */

#define FOREACH_PKTCNTR(ENUMDEF)					\
	ENUMDEF(PKTCNTR_WL_IN_TXQ_TOT)					\
	ENUMDEF(PKTCNTR_WL_IN_TXQ_DROP)					\
	ENUMDEF(PKTCNTR_WLPF_PKTLIST_CFP)				\
	ENUMDEF(PKTCNTR_WLPF_PKTLIST_PKTC)				\
	ENUMDEF(PKTCNTR_WLPF_PKTLIST_SINGLE)			\
	ENUMDEF(PKTCNTR_CFP_ENQ_SCB_AMPDU)				\
	ENUMDEF(PKTCNTR_CFP_PKTLIST_FREE)				\
	ENUMDEF(PKTCNTR_PKTC_ENQ_SCB_AMPDU)				\
	ENUMDEF(PKTCNTR_PKTC_SCB_AMPDU_DROP)			\
	ENUMDEF(PKTCNTR_AMSDU_PASSTHROUGH)				\
	ENUMDEF(PKTCNTR_CFP_AMPDU_RELEASE)				\
	ENUMDEF(PKTCNTR_AMPDU_RELEASE)					\
	ENUMDEF(PKTCNTR_MPDU_ENQ_TXFIFO)				\
	ENUMDEF(PKTCNTR_MPDU_TXS_AQMC_FREE)				\
	ENUMDEF(PKTCNTR_LAST)

#define PKTCNTR_GENERATE_ENUM(ENUM) ENUM,

#define PKTCNTR_GENERATE_STRING(STRING) #STRING,

typedef enum {
	FOREACH_PKTCNTR(PKTCNTR_GENERATE_ENUM)
} pktcntr_enum;

#endif   /* WL_PKTCNTR */

#ifdef WL_PKTCNTR
#define PKTCNTR_INC(wlc, cntr_id, cnt)		wlc->pktcntr_tbl[cntr_id] += cnt;
#else /* !WL_PKTCNTR */
...
/* per opclass and chan, TBTT information for list of neighbors to push in RNR IE */
#define WL_RNR_INFO_MAX_NBR_PER_OPCLASS_CHAN		5
typedef struct wlc_rnr_info {
	uint8 rclass;
	uint8 chan;
	uint8 count;		/* nbr entries with same chan and rclass */
	uint8 max_count;	/* max nbr entries allowed */
	uint8 *pbuf;		/* ptr to nbr entries */
	uint8 tbtt_len;		/* prepare tbtt of len in RNR */
} wlc_rnr_info_t;

typedef struct wlc_upr_fd_info {
	wlc_hrt_to_t	*timer;	/* Delayed scheduler timer */
	wlc_bsscfg_t	*cfg;
	void		*pkt;	/* lbuf packet from PKTGET, reuse it */
	uint16		flags;
	uint8		period;
	uint32		n_pkts_xmit;	/* pkts xmitted till timer stop */
	uint32		n_pkts_xmit_fail;
	uint32		alloc_fail;	/* Number of allocation (probe response) failures */
	uint8		count;
	uint8		max_per_interval;
	uint8		one_time_alloc;	/* it should be only 1 */
	uint8		pad;
	uint32		n_pktget;
	uint32		n_pktfree;
	uint32		in_flight;
...
struct wlc_info {
	...
	wlc_cmn_info_t	*cmn;			/**< pointer to common part of wlc variables */
	...
	struct hwa_dev *hwa_dev;		/**< backpoint to HWA device */
#endif // endif
	/* up and down */
	...
	phy_info_t	*pi;			/**< pointer to active per-phy info */
	...
	bool		PSpoll;			/**< whether there is outstanding PS-Poll frame */
	...
	wlc_bsscfg_t	**bsscfg;		/**< set of BSS configurations, idx 0 is default and
						 * always valid
						 */
	wlc_bsscfg_t	*primary_bsscfg;	/**< the primary (first) bsscfg (AP or STA) */
	wlc_bsscfg_t	*main_ap_bsscfg;	/**< First bsscfg configured as AP (can be NULL) */
	...
	bool		any_wds_peer_160mhz;	/**< Any WDS peer in 160MHz */
	...
	uint		block_time;		/**< stall start time (milliseconds) */
	int		block_timeout;		/**< stall detection time (milliseconds) */
	...
	uint		block_timeout_txavail_thr;	/**< CCASTATs threshold for TXOP+TXDUR */
	uint		block_timeout_rxcca_thr;	/**< CCASTATs threshold for high RXCCA */
	wl_chanim_stats_t *block_chanstats;	/**< CHANIM STATs pointer for datablock */
#endif /* BCMDBG_TXSTALL */
	bool		bcmcfifo_drain;		/**< TX_BCMC_FIFO is set to drain */
	bool		ps_scb_txfifo_blk;	/**< prohibit posting frames of PS sta to tx fifo */
	uint16		ps_txfifo_blk_scb_cnt;	/**< No of stations in PS fifo block state */
	...
	uint16		lifetime_txfifo;	/**< TX FIFO pkt lifetime in 1024-us units */
	 ...
	 */
	bool		apsd_sta_usp;		/**< Unscheduled Service Period in progress STA */
	...
	/**< Small table for duplicate detections for cases where SCB does not exist */
	struct sa_seqctl ctl_dup_det[SA_SEQCTL_SIZE];
	...
	wlc_txq_info_t	*active_queue;		/**< txq for currently active transmit context */
	...
	bool		txfifo_detach_transition_queue_del; /**< del detach tran q when sync done */
	...
	mbool		gptimer_stay_awake_req; /**< bitfield used to keep device awake
						 ...
						 */
	uint16		wake_event_timeout;	/**< timeout in seconds to trigger wake_event */
	...
	/* STA */
	bool		reset_triggered_pmoff;	/**< tells if reset took place -> turned off PM */
	...
	*/
	...
	uint		iscan_result_last;	/**< scanresult index last iscanresult */
	...
	bcm_notif_module_t *notif;		/**< Notifier module. */

	bcm_mpm_mgr_h	mem_pool_mgr;		/**< Memory pool manager handle. */
	uint16		fabid;
	...
	/* MBO */
	wlc_mbo_info_t *mbo_ap;         /* MBO_AP instance */
	wlc_mbo_info_t *mbo_sta;	/* MBO_STA instance */
...
#ifdef WL_PKTCNTR
	uint32 pktcntr_tbl[PKTCNTR_LAST];
#endif // endif
	...
	bool		active_udpv4;
	bool		active_tcp;
	bool		atm_perc;		/* airtime percentage statistics */

#ifdef WL_PKTDROP_STATS
	bool pktdrop_stats_feature_enabled;
...
/* data_block_ps cunters */
	...
	uint32 datablock_starttime;	/* Starting time of the data_block_ps duration */
	uint32 datablock_mintime;
	uint32 datablock_maxtime;
	uint32 datablock_tottime;	/* Aggregated time for the average time calculation */
	uint32 pktpend_min;
	uint32 pktpend_max;
	uint32 pktpend_tot;		/* Aggregated count for the average count calculation */
#endif /* WL_PS_STATS */
	...
	uint32 pktpend_sync_drop;	/* Counter for txpktpend inflight packets dropped */
#endif /* WL_TXPKTPEND_SYNC */
	uint32 bcn_txerr;
	uint32 bcn_txerr_thresh;
	...
	struct wl_timer *rnr_update_timer;
	wlc_upr_fd_info_t	*upr_fd_info;

	uint32		rnr_update_period;	/* rnr scan timer period */
	uint8		rnr_update_method;	/* Method to create Reduced Neighbor report
						 * static = 0
						 * scan = 1
						 */
	uint8		skip_build_rnr_ie;	/* skip adding rnr ie in beacon and probe
						 * response
						 */
#endif /* WL_WIFI_6GHZ || WL_OCE_AP */
#if BAND6G || defined(WL_OCE_AP)
	uint32		nbr_discovery_cap;			/* 6G/OCE capabilities */
#endif /* BAND6G || WL_OCE_AP */
	bool		dsmps_war;       /* = TRUE dyn SMPS enabled STAs excluded from MU */
#ifdef BCM_CSIMON
	wlc_csimon_info_t *csimon_info;	/* CSIMON info */
#endif // endif
#if defined(STS_XFER)
	wlc_sts_xfer_info_t *sts_xfer_info;	/* Tx and PhyRx Status transfer */
#endif /* STS_XFER */
#if BAND6G || defined(DEBUG_TBTT) || (defined(WL_OCE) && defined(WL_OCE_AP)) || \
	defined(WL_OCE_AP)
	uint32		prev_TBTT;		/**< TSF when last TBTT indicated */
	bool		bad_TBTT;		/**< lost the race and iterated */
#endif /* BAND6G || DEBUG_TBTT || (WL_OCE && WL_OCE_AP */
	bool		defrateset_override;	/* true if default tx/rx mcs maps have been changed
						 * via 'rateset' IOVAR.
						 */
	bool		short_ssid_list; /** is supported if TRUE */
	bool		short_aqmdd; /** is supported if TRUE */
	wlc_multibssid_info_t *mbssid_i; /**< multiple BSSID information */
#ifdef WL_CFG80211_NIC
	bool            extended_ops_support;
#endif /* WL_CFG80211_NIC */
	bool		apsta_muted;	/* bool to check phy muted for Repeater. To sync
					 * with rootAP beacons on DFS channels.
					 */
	wlc_txpwr_eirp_t      reg_eirp;
	wlc_txpwr_eirp_psd_t  reg_eirp_psd;
	int tpe_eirp;
	bool		mclx_mode;		/* identifies MCLX cards for 2x2^2x2 mode */
	bool		mclx_mode_band_changed;	/* band changed, chains need updating */
#if defined(BCMDBG)
...
#define WLC_NBR_DISC_CAP_COLOCATED_RADIO		2
#define WLC_NBR_DISC_CAP_IN_BAND_DISCOVERY		3
#define WLC_NBR_DISC_CAP_OOB_DISCOVERY			4
...
struct wlc_txh_info {
	...
	uint16 FbwInfo;			/* for corerev >= 40 only */
	uint16 d11FrameSize;
	uint hdrSize;
	d11txhdr_t* hdrPtr;
	void* d11HdrPtr;
...
#define WLC_IS_MATCH_SSID(ssid1, ssid2, len1, len2) \
...
extern void *wlc_senddeauth(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct scb *scb,
	...
	uint16 reason_code, bool psdontq);
extern void *wlc_senddeauth_ex(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct scb *scb,
...
extern void wlc_update_upr_fd_template(wlc_info_t *wlc, bool suspend);
extern void wlc_bss_update_upr_fd_template(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool suspend);

extern void wlc_prepare_upr_template(wlc_info_t *wlc, uint type,
	ratespec_t bcn_rate, wlc_bsscfg_t *cfg, uint16 *buf, int *len);

#if defined(WL_OCE) && defined(WL_OCE_AP)
extern void wlc_prepare_fd_template(wlc_info_t *wlc, uint type, ratespec_t bcn_rspec,
...
/* MPC off request spare request, was used by tsync */
#define MPC_OFF_REQ_SPARE_NOT_USED (1 << 1)
...
#ifdef WL_BSS_STATS
extern int wlc_wlent_stats(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
...
typedef struct {
	wlc_rateset_t *sup;
	wlc_rateset_t *ext;
	chanspec_t *chspec;			/**< points at a 20 mhz chspec */
...
extern void wlc_update_phy_tdcs(wlc_info_t *wlc);
...
extern void wlc_scb_flush_queues(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, struct scb *scb);
...
extern void wlc_update_txpktsuccess_stats(wlc_info_t *wlc, struct scb *scb, bool broadcast,
	uint32 pkt_len, uint8 prio, uint16 npkts);
...
#define WLC_MBSP_SEL_SAE_H2E	(1<<3)  /* SAE HASH to element only */
...
void wlc_process_multiap_ie(wlc_info_t *wlc, struct scb *scb, multiap_ie_t *multiap_ie, uint16 ft);
#endif	/* MULTIAP */
#ifdef WL_CFG80211
...
#if BAND6G || defined(WL_OCE_AP)
extern void wlc_rnr_update_timer(void *ctx);
#endif /* WL_WIFI_6GHZ || WL_OCE_AP */
#ifdef WL_CFG80211_NIC
void wl_set_extended_ops_support(struct wl_info *wl, bool value);
#endif /* WL_CFG80211_NIC */

#ifdef WL_PKTDROP_STATS

void wlc_pktdrop_stats_enable(wlc_info_t *wlc);
void wlc_pktdrop_stats_disable(wlc_info_t *wlc);

void wlc_pktdrop_full_counters_enable(wlc_info_t *wlc);
void wlc_pktdrop_full_counters_disable(wlc_info_t *wlc);

void scb_pktdrop_brief_counters_enable(wlc_info_t *wlc, struct scb *scb);
void scb_pktdrop_brief_counters_disable(wlc_info_t *wlc, struct scb *scb);

void scb_pktdrop_full_counters_enable(wlc_info_t *wlc, struct scb *scb);
void scb_pktdrop_full_counters_disable(wlc_info_t *wlc, struct scb *scb);

void wlc_pktdrop_counter_update(wlc_info_t *wlc, struct scb *scb,
	int rx, int reason);

/*
 * Since both the rx & tx toss reson enums start from 0, distinguishing
 * them as being rx or tx toss reason needs an extra indicator.  These
 * are those needed indicators.
 */
#define TX_TOSS_REASON		0
#define RX_TOSS_REASON		1

static INLINE void
RX_PKTDROP_COUNT(wlc_info_t *wlc, struct scb *scb, int rsn)
{
	if (wlc->pktdrop_stats_feature_enabled) {
		wlc_pktdrop_counter_update(wlc, scb, RX_TOSS_REASON, rsn);
	}
}

static INLINE void
TX_PKTDROP_COUNT(wlc_info_t *wlc, struct scb *scb, int rsn)
{
	if (wlc->pktdrop_stats_feature_enabled) {
		wlc_pktdrop_counter_update(wlc, scb, TX_TOSS_REASON, rsn);
	}
}

#else /* !WL_PKTDROP_STATS */

#define RX_PKTDROP_COUNT(wlc, scb, rsn)	BCM_REFERENCE(rsn);
#define TX_PKTDROP_COUNT(wlc, scb, rsn) BCM_REFERENCE(rsn);
