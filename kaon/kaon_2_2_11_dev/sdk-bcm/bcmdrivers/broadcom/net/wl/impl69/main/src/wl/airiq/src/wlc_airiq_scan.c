...
airiq_update_scan_config(airiq_info_t *airiqh, airiq_config_t *sc)
{
	...
	for (i = 0; i < airiqh->scan.channel_cnt; i++) {
		airiqh->scan.real_fft_count[i]  = 0;
		airiqh->scan.real_capture_time[i] = 0;
	}
#endif // endif
	...
	if (cnt <= 0) {
		WL_ERROR(("wl%d: %s: Invalid chanspec count %d\n",
			WLCWLUNIT(airiqh->wlc), __FUNCTION__, cnt));
...
chanspec_list_valid(airiq_info_t *airiqh, airiq_config_t *sc)
{
	...
	if ((airiqh->phy_mode == PHYMODE_3x3_1x1) &&
		...
		IS_43794MC(airiqh->wlc->deviceid) || IS_43794MC2(airiqh->wlc->deviceid) )) {
		return TRUE;
	} else if (!IS_SINGLEBAND(airiqh->wlc)) {
		return TRUE;
	} else {
		/* For single band radio check if it supports requested band */
		for (i = 0; i < sc->chanspec_cnt; i++) {
			if (CHSPEC_BANDTYPE(sc->chanspec_list[i]) !=
...
static void wlc_airiq_scantimer_3p1(void *arg)
{
	...
	if (!airiqh) {
		WL_ERROR(("wl%d: %s(%d): null invalid airiq handle\n",
	...
	if (airiqh->scan.scan_start) {
		...
		if(airiqh->m2m_dma_inprogress) {
			if((tsf- airiqh->dma_start_tsf) > 1000*DMA_TIMEOUT_MS) {
				WL_AIRIQ(("wl%d: %s(%d): Scan aborted M2M DMA timed out.\n",
					WLCWLUNIT(airiqh->wlc), __FUNCTION__, __LINE__));
				wlc_airiq_set_enable(airiqh, FALSE);
				airiqh->scan.run_phycal = FALSE;
				airiqh->wlc->scan->state &= ~(SCAN_STATE_PROHIBIT | SCAN_STATE_PASSIVE);
				...
				// extend time by 1ms interval , wait for DMA to finish
				wl_add_timer(airiqh->wlc->wl, airiqh->timer, 1, 0);
				return;
			}
		}
#if defined(BCMDBG) || defined(WLTEST)
		airiqh->scan.real_fft_count[airiqh->scan.channel_idx] = airiqh->fft_count;
		airiqh->scan.real_capture_time[airiqh->scan.channel_idx] = tsf - airiqh->chan_tsf;
		if(airiqh->scan.channel_idx == (chancnt - 1)) {
			/* copy last scan stats*/
			airiqh->last_scan_stats.channel_cnt = chancnt;
			memcpy(airiqh->last_scan_stats.chanspec_list, airiqh->scan.chanspec_list, chancnt*sizeof(chanspec_t));
			memcpy(airiqh->last_scan_stats.real_fft_count, airiqh->scan.real_fft_count, chancnt*sizeof(uint32));
			memcpy(airiqh->last_scan_stats.real_capture_time, airiqh->scan.real_capture_time, chancnt*sizeof(uint32));
			memcpy(airiqh->last_scan_stats.capture_count, airiqh->scan.capture_count, chancnt*sizeof(uint32));
		}
#endif // endif
...
void wlc_airiq_scantimer(void *arg)
{
	...
	if (airiqh->scan_type != SCAN_TYPE_AIRIQ) {
		WL_ERROR(("wl%d: %s(%d): invalid AIRIQ 4x4 scan type: 0x%X\n", WLCWLUNIT(airiqh->wlc),
	...
	if (airiqh->tx_suspending) {
		if (wlc_tx_suspended(airiqh->wlc)) {
			...
			/* wait */
			WL_PRINT(("wl%d: %s: tx still suspending. TXPKTPENDTOT=%d\n",
				WLCWLUNIT(airiqh->wlc), __FUNCTION__, TXPKTPENDTOT(airiqh->wlc)));
	...
	if (!airiqh->wlc->pub->up) {
		WL_AIRIQ(("wl%d: %s(%d): wl not up. Scan aborted/Interface down.\n",
			WLCWLUNIT(airiqh->wlc), __FUNCTION__, __LINE__));
		wlc_airiq_set_enable(airiqh, FALSE);
		airiqh->scan.run_phycal = FALSE;
	...
	if (!airiqh->scan_enable) {
		WL_AIRIQ(("wl%d: %s(%d): wl%d scan stopped.\n", WLCWLUNIT(airiqh->wlc),
...
int wlc_airiq_start_scan_phase2(airiq_info_t *airiqh)
{
	...
	if (airiqh->phy_mode != PHYMODE_3x3_1x1) {
		...
		wlc_ap_mute(airiqh->wlc, TRUE, NULL, WLC_AP_MUTE_SCAN);
                wlc_suspend_mac_and_wait(airiqh->wlc);
                WL_AIRIQ(("%s: Suspending all TX FIFOs \n", __FUNCTION__));
                for (i = 0; i < WLC_HW_NFIFO_INUSE(airiqh->wlc); i++) {
                        wlc_bmac_tx_fifo_suspend(airiqh->wlc->hw,i);
                }
                wlc_enable_mac(airiqh->wlc);
	}
...
int wlc_airiq_scan_abort(airiq_info_t *airiqh, bool upgrade)
{
	...
	airiqh->scan.run_phycal = FALSE;
	wlc_airiq_set_enable(airiqh, FALSE);
	wl_del_timer(airiqh->wlc->wl, airiqh->timer);
	...
	wlc_mute(airiqh->wlc, OFF, 0);
        WL_AIRIQ(("%s: Resuming all TX FIFOs \n", __FUNCTION__));
        for (i = 0; i < WLC_HW_NFIFO_INUSE(airiqh->wlc); i++) {
                wlc_bmac_tx_fifo_resume(airiqh->wlc->hw,i);
        }
#ifdef STA
	...
	wlc_radio_mpc_upd(airiqh->wlc);
#endif /* STA */
	WL_AIRIQ(("wl%d: %s(%d): upgrading: phymode=%d(%d) modesw_state=%d, enab=%d\n",
...
lte_u_chanspec_list_valid(airiq_info_t *airiqh, airiq_config_t *sc)
{
	...
	for (i = 0; i < sc->chanspec_cnt; i++) {
		if (! BAND_5G(CHSPEC_BANDTYPE(sc->chanspec_list[i]))) {
			WL_AIRIQ(("wl%d: %s: BW is not 5G BW of channel [%d] = %x\n",
				WLCWLUNIT(airiqh->wlc), __FUNCTION__,
				i, CHSPEC_BANDTYPE(sc->chanspec_list[i])));
			return FALSE;
		}
	}
...
lte_u_update_scan_config(airiq_info_t *airiqh, airiq_config_t *sc)
{
	...
	default:
		airiqh->lte_u_scan_configured = FALSE;
		airiqh->scan.channel_cnt  = 0;
		WL_ERROR(("wl%d: %s: unknown bw 0x%x\n", WLCWLUNIT(airiqh->wlc),
			__FUNCTION__, bw));
		break;
	}
...
//TODO	pere wlc_phy_clear_tssi(WLC_PPI(airiqh->wlc));
