 ...
 *
 * $Id: acsd_chanim.c 788900 2020-07-14 08:57:11Z $
 */
...
void
chanim_upd_acs_record(char *ifname, chanim_info_t *ch_info, chanspec_t selected, uint8 trigger)
{
	...
	SEND_CEVENT_A2C_EXT(ifname, 0, trigger, CEVENT_A2C_ST_ACS_CH_SW, 0, &selected,
			sizeof(chanspec_t));
}
...
acsd_chanim_query(acs_chaninfo_t * c_info, uint32 count, uint32 ticks)
{
	...
	if (BAND_6G(c_info->rs_info.band_type)) {
		input_band = WL_CHANSPEC_BAND_6G;
	} else if (BAND_5G(c_info->rs_info.band_type)) {
