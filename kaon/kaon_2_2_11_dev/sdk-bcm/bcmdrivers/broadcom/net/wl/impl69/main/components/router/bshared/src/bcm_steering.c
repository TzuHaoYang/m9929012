 ...
 *
 * $Id: bcm_steering.c 791369 2020-09-24 11:10:07Z $
 */
...
static void
wl_wlif_get_maclist_filename(char *appname, char *fname)
{
	snprintf(fname, PATH_MAX, STEERINGFILE_NAME_FORMAT, STEERINGFILE_DIR_NAME,
		STEERINGFILE_PREFIX, appname, STEERINGFILE_POSTFIX);
}
static int
...
wl_wlif_restore_old_maclist_timer(wl_wlif_hdl_data_t *hdl)
{
	...
	wl_wlif_get_maclist_filename(hdl->appname, fname);
	if (!(fp = fopen(fname, "r"))) {
...
int
wl_wlif_mac_present_in_old_maclist(char *appname, struct ether_addr *addr)
{
	FILE *fp;
	char fname[PATH_MAX];
	struct maclist_entry entry;
	...
	wl_wlif_get_maclist_filename(appname, fname);
	if (!(fp = fopen(fname, "r"))) {
		WLIF_BSSTRANS("Error opening file %s : %s\n", fname, strerror(errno));
		return -1;
	}
	while (fgets(line, sizeof(line), fp) != NULL) {
		wl_wlif_get_maclist_entry(line, &entry);
		bcm_ether_atoe((const char *)entry.macaddr, &ea);
		if ((eacmp(addr, (struct ether_addr *)&ea) == 0)) {
			WLIF_BSSTRANS("%s: "MACF" found in old maclist\n",
				appname, ETHER_TO_MACF(ea), timeout);
			fclose(fp);
			return 1;
		}
	}
