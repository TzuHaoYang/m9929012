...
static taf_list_t ** taf_ias_get_list_head_ptr(void * context)
{
	...
	if (method->active_list) {
		...
		WL_TAFM2(method, "recombine active to main list\n");
...
static INLINE void taf_ias_get_pktrate(taf_scb_cubby_t *scb_taf, int tid, uint32 aggsf,
	...
	int len, taf_rspec_index_t rindex, taf_source_type_t s_idx, taf_list_type_t type)
{
	...
	taf_ias_get_pktrate(scb_taf, tid, ias_sched->aggsf, rindex, s_idx, &pkt_rate, &byte_rate);
	...
	taf_rate *result)
{
	...
	case TAF_RSPEC_MU_DL_BOOST:
		...
		case TAF_RSPEC_UL:
			...
			result->rspec |= HE_GI_TO_RSPEC(WL_RSPEC_HE_2x_LTF_GI_1_6us);
			result->rspec |= wlc_scb_ratesel_get_link_bw(wlc, scb) << WL_RSPEC_BW_SHIFT;
			break;
#endif /* TAF_ENABLE_UL */
		default:
			ret = FALSE;
...
static INLINE void taf_ias_pkt_overhead(wlc_taf_info_t *taf_info, taf_scb_cubby_t* scb_taf,
	taf_scheduler_scb_stats_t* scb_stats, taf_rspec_index_t rindex)
{
	...
	for (rindex = TAF_RSPEC_SU_DL; rindex < NUM_TAF_RATES; rindex++) {
		...
		if ((!TAF_RIDX_IS_BOOST(rindex) &&
			...
			if (scb_stats->ias.data.byte_rate[rindex] == 0) {
				WL_ERROR(("wl%u %s: "MACF" zero byte rate (type %u, rspec 0x%x)\n",
					WLCWLUNIT(TAF_WLCT(taf_info)), __FUNCTION__,
					TAF_ETHERC(scb_taf), rindex, rate.rspec));
		...
		switch (method->type) {
			...
			case TAF_ATOS2:
				...
				score = taf_ias_decay_score(scb_taf->score[item->type],
					&ias_cfg(method).coeff, elapsed, &time_correction);
...
static INLINE bool taf_ias_fifo_overlap(taf_method_info_t* method, taf_scb_cubby_t **scb_taf_p,
	taf_scb_cubby_t *scb_taf_test, uint8 count, int tid)
{
	...
	taf_list_t *list, taf_ias_uni_state_t *uni_state, int tid)
{
	...
	while (list) {
		...
		if (TAF_OFDMA_OPT(method, PAIR)) {
			uint32 pro_limit = (scb_taf->score[TAF_TYPE_DL] *
				ias_cfg(method).mu_pair[mu_user_count]) >> TAF_COEFF_IAS_MAX_BITS;
			...
			if (diff > pro_limit) {
				WL_TAFM2(method, "score diff %u "MACF"%s is too large "
					...
					pro_limit, ias_cfg(method).mu_pair[mu_user_count]);
				break;
			}
		}
	...
	if (est_inter_total == 0) {
		...
		WL_TAFM1(method, "intermediate traffic count %u so sweep %s release this frame\n",
	...
	taf_list_t *list, taf_ias_uni_state_t *uni_state, int tid)
{
	...
	TAF_ASSERT(ias_cfg(method).mu_mimo_rel_limit);
	...
	if (est_inter_total == 0) {
		user_rel_limit =
			TAF_MICROSEC_TO_UNITS(ias_cfg(method).mu_mimo_rel_limit[mu_user_count - 1]);
...
static INLINE void taf_ias_sched_active_list(taf_method_info_t *method)
{
...
{
	taf_ias_get_pktrate(scb_taf, tid, ias_sched->aggsf, rindex, s_idx,
	...
	int tid, taf_release_context_t *context, taf_schedule_state_t op_state)
{
	...
	for (list = method->list; list; list = list->next) {
		...
		emptied = taf_ias_sched_send(method, scb_taf, tid, uni_state, ias_sched,
			context, op_state, list->type, agg_hold);
	...
	if (taf_info->op_state == TAF_MARKUP_REAL_PACKETS &&
		...
		for (tid = 0; tid < NUMPRIO; tid++) {
			...
			if ((ias_sched = TAF_IAS_TID_STATE(scb_taf, tid))) {
				...
				if (ias_sched->released_units_limit[TAF_TYPE_DL] >= 0) {
					/* set release limit to 1 (minimum release)  */
					ias_sched->released_units_limit[TAF_TYPE_DL] = 1;
				}
			}
		}
	}
	...
	switch (status) {
		...
		case TAF_TXPKT_STATUS_REQUEUED:
			if (TAF_IS_TAGGED(WLPKTTAG(p))) {
				WL_TAFM1(method, MACF" tid %u %s\n",
					TAF_ETHERC(scb_taf), tid, taf_txpkt_status_text[status]);
			}
			/* fall through */
	...
	struct bcmstrbuf* b)
{
	...
	if (!strcmp(cmd, "mu_mimo_limit")) {
		...
		if (ias_cfg(method).mu_mimo_rel_limit == NULL) {
			return err;
		}
		cmd += strlen(cmd) + 1;
		...
		if (*cmd) {
			...
			if (idx >= 2 && idx <= TAF_MAX_MU_MIMO) {
				uint32 score = ias_cfg(method).mu_mimo_rel_limit[idx - 1];

				err = wlc_taf_param(&cmd, &score, 0, TAF_MICROSEC_MAX, b);
				if (err == TAF_IOVAR_OK_SET) {
					ias_cfg(method).mu_mimo_rel_limit[idx - 1] = score;
				}
				result->misc = ias_cfg(method).mu_mimo_rel_limit[idx - 1];
			}
		} else {
			int idx;
			...
			for (idx = 1; idx < TAF_MAX_MU_MIMO; idx++) {
				bcm_bprintf(b, " %2u:   %4u\n", idx + 1,
					ias_cfg(method).mu_mimo_rel_limit[idx]);
			}
			err = BCME_OK;
		}
		return err;
