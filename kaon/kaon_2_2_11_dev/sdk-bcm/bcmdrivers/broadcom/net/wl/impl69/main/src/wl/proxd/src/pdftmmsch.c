 ...
 *
 * $Id: pdftmmsch.c 793903 2020-12-14 18:06:08Z $
 */
...
pdftm_msch_cb(void *handler_ctx, wlc_msch_cb_info_t *cb_info)
{
	...
	if (cb_info->type & MSCH_CT_PRE_ONCHAN) {
		...
		if (!BSSCFG_AP(sn->bsscfg)) {
			wlc_txqueue_start(ftm->wlc, sn->bsscfg, cb_info->chanspec);
		}
		...
		/* Dont start queue if STA and chancpec is same as sta chanspec */
		if (!BSSCFG_AP(sn->bsscfg)) {
			wlc_txqueue_end(ftm->wlc, sn->bsscfg);
		}
		...
		if (sn->state <= WL_PROXD_SESSION_STATE_BURST) {
			...
			FTM_LOGSCHED(ftm, (("%s[%d]: wl%d type %d state %d Stopping Burst\n",
				__FUNCTION__, __LINE__, FTM_UNIT(ftm), cb_info->type, sn->state)));
			if (sn->state == WL_PROXD_SESSION_STATE_BURST && sn->ftm_state &&
			    sn->ftm_state->burst) {
				/* tell the peer we are done */
				sn->status = WL_PROXD_E_CANCELED;
				FTM_INIT_SESSION_NOTIFY_INFO(&notify_info, NULL,
					&sn->config->burst_config->peer_mac,
					WL_PROXD_EVENT_SESSION_END, 0, 0, NULL, 0);
				pdftm_notify(sn->ftm, sn->bsscfg,
...
pdftm_msch_end(pdftm_t *ftm, pdftm_session_t *sn)
{
	...
	if (!(ftm->flags & FTM_FLAG_SCHED_INITED))
		goto done;
		...
		for (i = 0; i < ftm->max_sessions; i++) {
			...
			if ((sst != NULL) && sn->ext_sched && sn->ext_sched->req_hdl) {
				if (!(sst->flags & FTM_SESSION_STATE_MSCH_SCHEDULED)) {
					...
					err = wlc_msch_timeslot_unregister(ftm->wlc->msch_info,
						(wlc_msch_req_handle_t **)&sn->ext_sched->req_hdl);
					sst->flags &= ~FTM_SESSION_STATE_BURST_REQ;
				}
			}
			if (sn->ext_sched) {
				MFREE(FTM_OSH(ftm), sn->ext_sched, sizeof(*sn->ext_sched));
				sn->ext_sched = NULL;
			}
		}
	} else {
