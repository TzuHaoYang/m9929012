 ...
 *
 * $Id: pdftmsn.c 793903 2020-12-14 18:06:08Z $
 */
	...
	wl_proxd_session_state_t new_state, wl_proxd_event_type_t *event_type)
{
	...
	*event_type = WL_PROXD_EVENT_NONE;
	...
	if (ftm->flags & FTM_FLAG_SCHED_ACTIVE) {
		err = WL_PROXD_E_BAD_STATE;
		goto done;
	}
	...
	SAFE_DECR(ftm->sched->in.start_wait);
	UPDATE_STATUS(sn, status);
	sn->state = new_state;
	err = pdftm_sched_session(ftm, sn);
done:
	...
	wl_proxd_session_state_t new_state, wl_proxd_event_type_t *event_type)
{
	...
	*event_type = WL_PROXD_EVENT_NONE;
	...
	UPDATE_STATUS(sn, status);
	sn->state = new_state;
	SAFE_DECR(ftm->sched->in.started);
	...
	wl_proxd_session_state_t new_state, wl_proxd_event_type_t *event_type)
{
	...
	*event_type = WL_PROXD_EVENT_NONE;
	...
	SAFE_DECR(ftm->sched->in.sched_wait);
	UPDATE_STATUS(sn, status);
	sn->state = new_state;
	...
	case WL_PROXD_SESSION_STATE_BURST:
		...
		*event_type = WL_PROXD_EVENT_BURST_START;
		...
		FTM_CNT(ftm, sn, burst);
		ftm->sched->burst_sn = sn;
	...
	wl_proxd_session_state_t new_state, wl_proxd_event_type_t *event_type)
{
	...
	*event_type = WL_PROXD_EVENT_BURST_END;
	...
	ftm->sched->burst_sn = NULL;
	SAFE_DECR(ftm->sched->in.burst);
	UPDATE_STATUS(sn, status);
	sn->state = new_state;
	...
	wl_proxd_session_state_t new_state, wl_proxd_event_type_t *event_type)
{
	...
	*event_type = WL_PROXD_EVENT_NONE;
	...
	if (sn->flags & FTM_SESSION_TX_PENDING &&
		ftm->wlc->state != WLC_STATE_GOING_DOWN &&
		ftm->wlc->state != WLC_STATE_DOWN) {
		goto done;
	}
...
ftm_sn_alloc_sid(pdftm_t *ftm, wl_proxd_session_id_t *out_sid)
{
	...
	err = BCME_NORESOURCE;
	for (limit = 0, sid = ftm->last_sid; limit <= ftm->max_sessions; ++limit) {
		sid = (sid + 1) | 0x8000;
	...
	wl_proxd_session_state_t state)
{
	...
	for (i = ftm->mru_idx; i < ftm->max_sessions; ++i) {
		sn = ftm->sessions[i];
		if (ftm_session_match(sn, sid, peer_mac, flags, state))
			goto done;
	}

	for (i = 0; i < ftm->mru_idx; ++i) {
		sn = ftm->sessions[i];
		if (ftm_session_match(sn, sid, peer_mac, flags, state))
			goto done;
	}
	...
	bool *created)
{
	...
	*created = FALSE;
	...
	pdftm_copy_session_config(ftm, sn->config, ftm->config->session_defaults);
	...
	++(ftm->cnt->sessions);
	if (ftm->cnt->max_sessions < ftm->cnt->sessions) {
		ftm->cnt->max_sessions = ftm->cnt->sessions;
	}
...
pdftm_free_sessions(pdftm_t *ftm, wlc_bsscfg_t *bsscfg, bool detach)
{
	...
	if (FTM_BSSCFG_SECURE(wlc_ftm_get_handle(ftm->wlc), bsscfg)) {
		phy_rxgcrs_sel_classifier(WLC_PI(ftm->wlc),
			TOF_CLASSIFIER_BPHY_ON_OFDM_ON);
	}

	if (!ftm->sessions || !ftm->max_sessions)
		goto done;
	...
	for (i = 0; i < ftm->max_sessions; ++i) {
		...
		wlc_ftm_delete_session(ftm, sn->sid);
		...
		MFREE(FTM_OSH(ftm), sn, sizeof(*sn));
		ftm->sessions[i] = NULL;
		SAFE_DECR(ftm->num_sessions);
	}

	MFREE(FTM_OSH(ftm), ftm->sessions, sizeof(pdftm_session_t *) * ftm->max_sessions);
	ftm->max_sessions = 0;
done:;
}
...
pdftm_start_session(pdftm_session_t *sn)
{
	...
	if (SCAN_IN_PROGRESS(ftm->wlc->scan)) {
		FTM_LOGPROTO(ftm, (("wl%d.%d: %s: SCAN in progress - abort it \n",
			FTM_UNIT(ftm), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__)));
		wlc_scan_abort(ftm->wlc->scan, WLC_E_STATUS_ABORT);
	}
...
pdftm_stop_session(pdftm_session_t *sn, wl_proxd_status_t status)
{
	...
	ASSERT(sn->ftm->sessions[sn->idx] == sn);
...
pdftm_get_burst_config(pdftm_t *ftm, pdftm_session_t *sn, wl_proxd_session_flags_t *flags)
{
	pdftm_session_config_t *sncfg;
	ASSERT(FTM_VALID(ftm));
	sncfg = !sn ? ftm->config->session_defaults : sn->config;
	if (sncfg) {
		*flags = sncfg->flags;
...
int pdftm_start_session_tsf_scan(pdftm_session_t *sn)
{
	...
	scan_chanspec = sn->config->burst_config->chanspec;

	sncfg = sn->config;
	ssid.SSID_len = 0;
	err = wlc_scan_request(ftm->wlc, DOT11_BSSTYPE_ANY,
