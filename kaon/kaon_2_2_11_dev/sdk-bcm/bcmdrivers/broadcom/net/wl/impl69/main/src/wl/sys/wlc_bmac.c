 ...
 *
 * $Id: wlc_bmac.c 793901 2020-12-14 16:52:46Z $
 */
...
static uint16 xmtfifo_sz[][NFIFO_LEGACY] = {
	...
	{ 32, 98, 22, 22, 14, 5 },	/* corerev 30: 8192, 25088, 5632, 5632, 3584, 1280 */
...
#define D11MAC_BMC_MACMEM_MAX_BYTES_REV130_2	(768 * 1024)
#define D11MAC_BMC_SYSMEM_MAX_BYTES_REV132	((1024 + 512) * 1024)   /* 1.5M bytes maximum */
...
#define D11MAC_BMC_RXFIFO0_IDX_IS132		(NFIFO_EXT_REV132 + 1)
#define D11MAC_BMC_RXFIFO1_IDX_IS132		(NFIFO_EXT_REV132 + 2)
...
#define D11MAC_BMC_MAXFIFOS_IS132		73
...
#define D11MAC_BMC_MAXFIFOS(rev)	(D11REV_IS(rev, 132) ? D11MAC_BMC_MAXFIFOS_IS132: \
 ...
 */
static int bmc_fifo_list_is132[D11MAC_BMC_MAXFIFOS_IS132] =
...
{
	return	D11REV_IS(rev, 132) ? bmc_fifo_list_is132 :
	        D11REV_IS(rev, 131) ? bmc_fifo_list_is131 :
	        D11REV_GE(rev, 130) ? bmc_fifo_list_is130 :
	        D11REV_GE(rev, 128) ? bmc_fifo_list_ge128 :
	        bmc_fifo_list_lt80;
}

#define D11MAC_BMC_RXFIFO0_IDX(rev)	(D11REV_IS(rev, 132) ? D11MAC_BMC_RXFIFO0_IDX_IS132: \
					(D11REV_IS(rev, 131) ? D11MAC_BMC_RXFIFO0_IDX_IS131: \
					(D11REV_IS(rev, 130) ? D11MAC_BMC_RXFIFO0_IDX_IS130: \
					(D11REV_GE(rev, 128) ? D11MAC_BMC_RXFIFO0_IDX_GE128: \
					D11MAC_BMC_RXFIFO0_IDX_LT80))))
#define D11MAC_BMC_RXFIFO1_IDX(rev)	(D11REV_IS(rev, 132) ? D11MAC_BMC_RXFIFO1_IDX_IS132: \
					(D11REV_IS(rev, 131) ? D11MAC_BMC_RXFIFO1_IDX_IS131: \
...
static uint32 wlc_setband_inact(wlc_hw_info_t *wlc_hw, enum wlc_bandunit bandunit);
static void
wlc_bmac_setband(wlc_hw_info_t *wlc_hw, enum wlc_bandunit bandunit, chanspec_t chanspec);
...
static void wlc_bmac_init_core_reset_disable_fn(wlc_hw_info_t *wlc_hw);
static void wlc_bmac_core_reset(wlc_hw_info_t *wlc_hw, uint32 flags, uint32 resetflags);
static void wlc_bmac_core_disable(wlc_hw_info_t *wlc_hw, uint32 bits);
...
static void wlc_setxband(wlc_hw_info_t *wlc_hw, enum wlc_bandunit bandunit);
...
static int wlc_bmac_phase1_phy_attach(wlc_hw_info_t *wlc_hw);
...
static void wlc_bmac_corerev_minor(wlc_hw_info_t *wlc_hw);
...
wlc_bmac_update_slot_timing(wlc_hw_info_t *wlc_hw, bool shortslot)
{
	...
	if (shortslot) {
		...
		W_REG(osh, D11_IFS_SLOT(wlc_hw), (uint16)0x0207);	/* APHY_SLOT_TIME */
		wlc_bmac_write_shm(wlc_hw, M_DOT11_SLOT(wlc_hw), APHY_SLOT_TIME);
	} else {
		/* 11g long slot: 11b timing */
		W_REG(osh, D11_IFS_SLOT(wlc_hw), (uint16)0x0212);	/* BPHY_SLOT_TIME */
...
WLBANDINITFN(wlc_ucode_bsinit)(wlc_hw_info_t *wlc_hw)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 132)) {
		wlc_write_inits(wlc_hw, d11ax130bsinitvals132);
#if defined(WL_PSMR1)
		if (PSMR1_ENAB(wlc_hw)) {
			wlc_write_inits(wlc_hw, d11ax130bsinitvalsr1_132);
		}
#endif // endif
		wlc_write_inits(wlc_hw, d11ax130bsinitvalsx132);
		...
		if (D11MINORREV_IS(wlc_hw->corerev_minor, 2)) {
			wlc_write_inits(wlc_hw, d11ax47bsinitvals129_2);
#if defined(WL_PSMR1)
			if (PSMR1_ENAB(wlc_hw)) {
				wlc_write_inits(wlc_hw, d11ax47bsinitvalsr1_129_2);
			}
#endif // endif
			wlc_write_inits(wlc_hw, d11ax47bsinitvalsx129_2);
		} else {
		...
		wlc_write_inits(wlc_hw, d11ac24bsinitvals56);
	...
	// restore IFS settings per BW
	wlc_bmac_upd_ifs(wlc_hw);
	return err;
} /* wlc_ucode_bsinit */

/** switch to new band but leave it inactive */
static uint32
WLBANDINITFN(wlc_setband_inact)(wlc_hw_info_t *wlc_hw, enum wlc_bandunit bandunit)
{
	...
	phy_radio_xband(wlc_hw->band->pi);
	...
	if ((plcp[0] | plcp[1] | plcp[2])) {
		...
		if ((gid > VHT_SIGA1_GID_TO_AP) && (gid < VHT_SIGA1_GID_NOT_TO_AP)) {
			if (D11REV_LT(wlc->hw->corerev, 128)) {
				murate = D11RXHDR_LT80_ACCESS_VAL(rxhdr, MuRate);
...
static void
wlc_print_ge129_mac_rxhdr(wlc_info_t *wlc, d11rxhdr_ge129_t *ge129_rxh)
{
	ASSERT(D11REV_GE(wlc->pub->corerev, 129));

	printf("\nwl%d: MAC H/W RX Status\n", wlc->pub->unit);

	/* Printing individual fields of hw rx status */
	printf("\tRxFrmSz:0x%04x dmaflgs:0x%02x fifo:0x%02x\n", ge129_rxh->RxFrameSize,
		ge129_rxh->dma_flags, ge129_rxh->fifo);
	printf("\tmrxs:0x%04x RxFrmSz_0:0x%04x HdrConvSt:0x%04x\n", ge129_rxh->mrxs,
		ge129_rxh->RxFrameSize_0, ge129_rxh->HdrConvSt);
	printf("\tfiltermap:0x%08x pktclass:0x%04x flwid:%04x errflgs:%04x\n", ge129_rxh->filtermap,
		ge129_rxh->pktclass, ge129_rxh->flowid, ge129_rxh->errflags);
}

/* Print uCode generated Rx Status */
static void
wlc_print_ge129_ucode_rxhdr(wlc_info_t *wlc, d11rxhdr_ge129_t *ge129_rxh)
{
	ASSERT(D11REV_GE(wlc->pub->corerev, 129));
	...
	printf("\tRxSt1:0x%04x RxSt2:0x%04x RxChan:%04x\n", ge129_rxh->RxStatus1,
		ge129_rxh->RxStatus2, ge129_rxh->RxChan);
	printf("\tAvbRxTime L:0x%04x\n", ge129_rxh->AvbRxTimeL);
	printf("\tUlRtInfo 0x%04x\n", ge129_rxh->UlRtInfo);
	printf("\tRxTsfTime H:0x%04x L:0x%04x RxSt3:0x%04x\n", ge129_rxh->RxTsfTimeH,
		ge129_rxh->RxTSFTime, ge129_rxh->RxStatus3);
}

/* Print PHY Rx Status */
static void
wlc_print_ge129_phy_rxhdr(wlc_info_t *wlc, d11rxhdr_ge129_t *ge129_rxh)
{
	uint16 *ptr;
	uint16 len;
	...
	ptr = D11PHYRXSTS_GE129_ACCESS_REF((d11rxhdr_t *)ge129_rxh, PhyRxStatus_0);
	len = D11PHYRXSTS_GE129_ACCESS_VAL((d11rxhdr_t *)ge129_rxh, PhyRxStatusLen);
...
static void
wlc_print_ge129_rxhdr(wlc_info_t *wlc, d11rxhdr_t *rxh)
{
	d11rxhdr_ge129_t *ge129_rxh;
	ASSERT(rxh);
	ASSERT(D11REV_GE(wlc->pub->corerev, 129));

	ge129_rxh =  &rxh->ge129;
	if (ge129_rxh) {
		ASSERT(RXHDR_GE129_DMA_FLAGS_RXS_TYPE(rxh) != RXS_INVALID);

		/* Print MAC RX status first */
		wlc_print_ge129_mac_rxhdr(wlc, ge129_rxh);

		if (RXS_GE128_VALID_UCODE_STS(rxh, wlc->pub->corerev)) {
			/* Print uCode RX status */
			wlc_print_ge129_ucode_rxhdr(wlc, ge129_rxh);
		}

		if (RXS_GE128_VALID_PHYRXSTS(rxh, wlc->pub->corerev)) {
			/* Print PHY RX status */
			wlc_print_ge129_phy_rxhdr(wlc, ge129_rxh);
		}
	}
}
...
/* Check if new phyrxsts is ready */
bool
wlc_bmac_offload_rxsts_new_ready(wlc_hw_info_t *wlc_hw)
{
	ASSERT(STS_RX_OFFLOAD_ENAB(wlc_hw->wlc->pub));
	return (wlc_offload_rxsts_new_ready(wlc_hw->wlc->offl));
}
...
#if defined(STS_FIFO_RXEN) || defined(WLC_OFFLOADS_RXSTS)
static int wlc_bmac_prxs_stats_dump(void *ctx, struct bcmstrbuf *b);
...
/* "K-lookahead WAR" to discard these orphan PhyRx Statuses */
static void
wlc_bmac_recv_process_sts_done(wlc_hw_info_t *wlc_hw, rx_list_t *rx_sts_list, uint fifo)
{
#ifdef BCMDBG
	...
	while (curr != NULL) {
...
#else
		if (IS_STSSEQ_ADVANCED(STSBUF_SEQ(curr),
			wlc_hw->cons_seq[idx])) {
			break;
		}
#endif /* BCMPCIEDEV */
		...
		if (curr != NULL) {
			...
			/* No pending PhyRx Status avialable,
			 * update read index to last consumed seq index
			 */
			wlc_hw->prxs_rd_idx = wlc_hw->cons_seq[idx];
		}

#ifdef BCMDBG
		if ((rxh_seq_dbg->seq[STSBUF_SEQ(prev)] != (uint16)~0)) {
			wlc_info_t *wlc = wlc_hw->wlc;
			struct bcmstrbuf b;
			...
			BCM_REFERENCE(wlc);
			WL_ERROR(("%s: ERROR Rx AMPDU released without linking PhyRx sts seq[%d]\n",
				__FUNCTION__, STSBUF_SEQ(prev)));
			WL_ERROR(("rxh_seq_dbg::rd_idx[%d] wr_idx[%d] seq[%d]=%d\n",
				rxh_seq_dbg->rd_idx, rxh_seq_dbg->wr_idx,
				STSBUF_SEQ(prev), rxh_seq_dbg->seq[STSBUF_SEQ(prev)]));
			...
			if (buf != NULL) {
				...
				if (STS_RX_OFFLOAD_ENAB(wlc->pub)) {
					...
					wlc_offload_wl_dump(wlc->offl, &b);
					WL_PRINT(("%s", buf));
				}
#endif /* WLC_OFFLOADS_RXSTS */
...
} /* wlc_bmac_recv_process_sts_done() */
		...
		sts_buff_t *sts_buff, uint8 rxh_offset)
{
	...
	*/
	...
	rxh_seq = D11RXHDR_GE129_SEQNUM(rxh);
	ASSERT(!IS_D11RXHDRSHORT(rxh, wlc_hw->corerev));
...
wlc_bmac_prxs_stats_dump(void *ctx, struct bcmstrbuf *b)
{
	...
	if (wlc_hw->rxpen_list[RXPEN_LIST_IDX0].rx_head) {
		...
		rxh = (d11rxhdr_t *)(PKTDATA(wlc_hw->osh, p) + rxh_offset);
		if (!IS_D11RXHDRSHORT(rxh, wlc->pub->corerev)) {
			last = start = D11RXHDR_GE129_SEQNUM(rxh);
		}
		while (p != NULL) {
			rxh = (d11rxhdr_t *)(PKTDATA(wlc_hw->osh, p) + rxh_offset);
			if (!IS_D11RXHDRSHORT(rxh, wlc->pub->corerev)) {
				last = D11RXHDR_GE129_SEQNUM(rxh);
			}
			p = PKTLINK(p);
			cnt++;
		}
		...
		uint8 rxh_offset)
{
	...
	while (curr_p != NULL) {
		rxh = (d11rxhdr_t *)(PKTDATA(wlc_hw->osh, curr_p) + rxh_offset);
		curr_seq = rxh_seq = D11RXHDR_GE129_SEQNUM(rxh);
		if (!IS_D11RXHDRSHORT(rxh, wlc->pub->corerev)) {
			ASSERT(rxh_seq < STS_SEQNUM_MAX);
	...
	wlc_bmac_recv_process_sts_done(wlc_hw, &wlc_hw->rx_sts_list, fifo);
	...
	while (curr_p != NULL) {
		rxh = (d11rxhdr_t *)(PKTDATA(wlc_hw->osh, curr_p) + rxh_offset);
		if (!IS_D11RXHDRSHORT(rxh, wlc->pub->corerev)) {
			curr_seq = D11RXHDR_GE129_SEQNUM(rxh);
		}
		curr_p  = PKTLINK(curr_p);
	}
...
#if defined(STS_FIFO_RXEN) || defined(WLC_OFFLOADS_RXSTS) || defined(STS_XFER)

#endif /* STS_FIFO_RXEN || WLC_OFFLOADS_RXSTS || STS_XFER */
 ...
 */
bool BCMFASTPATH
wlc_bmac_recv(wlc_hw_info_t *wlc_hw, uint fifo, bool bound, wlc_worklet_info_t *worklet)
{
	void *p;
	void *head = NULL;
	void *tail = NULL;
	...
	*/
	...
	if ((fifo != RX_FIFO2) && CFP_RCB_ENAB(wlc->cfp)) {
		want_dpc = wlc_cfp_bmac_recv(wlc_hw, fifo, worklet);
		goto wlc_bmac_recv_done;
	}
...
#if defined(BCMDBG) || defined(WLMSG_PRHDRS) || defined(BCM_11AXHDR_DBG)
		if (D11REV_GE(wlc_hw->corerev, 129) && WL_PRHDRS_ON() && WL_APSTA_RX_ON()) {
			wlc_print_ge129_rxhdr(wlc, rxh);
		}
#endif /* BCMDBG || WLMSG_PRHDRS || BCM_11AXHDR_DBG */
	...
	if (head0 != NULL) {
		...
		if (one_chain && (head != NULL)) {
			...
			want_dpc = (rx_list.rxfifocnt >= bound_limit);
#else
			worklet->processed += n;
			want_dpc = (n >= bound_limit);
#endif /* BULKRX_PKTLIST */
			goto wlc_bmac_recv_done;
		}
	}
	...
	while ((p = head) != NULL) {
		...
		if (!(IS_D11RXHDRSHORT(&wrxh->rxhdr, corerev))) {
			...
			phy_rssi_compute_rssi(wlc_hw->band->pi, wrxh);
		}
...
#if defined(WLCFP) || defined(PKTC) || defined(PKTC_DONGLE)
wlc_bmac_recv_done:
#endif /* WLCFP || PKTC || PKTC_DONGLE */

#if defined(STS_XFER)
	if (STS_XFER_PHYRXS_ENAB(wlc->pub)) {
		wlc_sts_xfer_bmac_recv_done(wlc, fifo);
	}
#endif /* STS_XFER */
#if defined(BCMPCIEDEV) && defined(WLC_OFFLOADS_RXSTS)
	want_dpc = (want_dpc ? want_dpc :
		(wlc_hw->rxpen_list[RXPEN_LIST_IDX1].rx_head != NULL));
#endif // endif
	return want_dpc;
} /* wlc_bmac_recv */
...
int
wlc_bmac_activate_srvsdb(wlc_hw_info_t *wlc_hw, chanspec_t chan0, chanspec_t chan1)
{
	int err = BCME_ERROR;
...
void
wlc_bmac_deactivate_srvsdb(wlc_hw_info_t *wlc_hw)
{
	wlc_hw->sr_vsdb_active = FALSE;
...
static void
wlc_bmac_srvsdb_set_band(wlc_hw_info_t *wlc_hw, enum wlc_bandunit bandunit, chanspec_t chanspec)
{
	uint32 macintmask;
	uint32 mc;
	wlc_info_t *wlc = wlc_hw->wlc;
	...
	if ((mc & MCTL_EN_MAC) != 0) {
		if (mc == 0xffffffff)
			...
			WL_MAC(("wl%d: wlc_phy_init:MAC running! mc=0x%x\n",
				wlc_hw->unit, mc));
	...
	wlc_bmac_update_slot_timing(wlc_hw,
	        BAND_5G6G(wlc_hw->band->bandtype) ? TRUE : wlc_hw->shortslot);
	...
	if (BAND_5G6G(wlc_hw->band->bandtype))
	 ...
	 * so wlc_worklet() will re-enable interrupts when done.
	 */
...
wlc_bmac_srvsdb_set_chanspec(wlc_hw_info_t *wlc_hw, chanspec_t chanspec, bool mute,
	bool fastclk, enum wlc_bandunit bandunit, bool band_change)
{
	uint8 switched;
	...
	wl_txpwrcap_tbl_t *txpwrcap_tbl, int* cellstatus)
{
	...
	if (SRHWVSDB_ENAB(wlc_hw->pub)) {
		...
		vsdb_switch |= (wlc_hw->sr_vsdb_active &&
		                sr_engine_enable(wlc_hw->sih, MAC_CORE_UNIT_0, IOV_GET, FALSE) > 0);
		...
		if (wlc_hw->band->bandunit != bandunit) {
			...
			if (wlc_hw->up) {
				if (SRHWVSDB_ENAB(wlc_hw->pub) && vsdb_switch) {
					...
					if (vsdb_status) {
						WL_MAC(("SRVSDB Switch DONE successfully \n"));
...
#endif /* SRHWVSDB */
			if (wlc_hw->up) {
				phy_info_t *pi = wlc_hw->bandstate[bandunit]->pi;
	...
	if (!wlc_hw->up) {
		...
		if (!IS_SINGLEBAND(wlc_hw->wlc)) {
			...
			if (phy_init_pending(wlc_hw->band->pi)) {
				phy_init(wlc_hw->band->pi, chanspec);
			} else {
			...
			if (chanspec != phy_utils_get_chanspec(wlc_hw->band->pi)) {
#ifdef SRHWVSDB
				if (SRHWVSDB_ENAB(wlc_hw->pub) && vsdb_switch) {
					vsdb_status = wlc_bmac_srvsdb_set_chanspec(wlc_hw,
...
void *
wlc_bmac_dmatx_peeknexttxp(wlc_info_t *wlc, int fifo)
{
...
/* PKTPGR: Stop PageOutReq, PageInReq rings
 * SW, HW shadow map
 * PageInRsp, PageOutReq map
 * Start PageOutReq, PageInReq rings
 */
void
...
#if defined(BCMHWA)
	HWA_PKTPGR_EXPR(hwa_pktpgr_map_pkts_prep(hwa_dev));
#endif // endif
...
#if defined(BCMHWA)
	HWA_PKTPGR_EXPR({
		hwa_pktpgr_ring_map_pkts(hwa_dev, cb, ctx);
		hwa_pktpgr_map_pkts_done(hwa_dev);
	});
#endif // endif
...
#if defined(STS_FIFO_RXEN) || defined(STS_XFER)
					if (i == STS_FIFO) {
						continue;
					}
#endif /* STS_FIFO_RXEN || STS_XFER */
#if defined(BCMHWA)
					HWA_RXPATH_EXPR({
					if (i == RX_FIFO || i == RX_FIFO1) {
						continue;
					}});
#endif // endif
	...
	if (BCMSPLITRX_ENAB()) {
		...
		if (POOL_ENAB(wlc->pub->pktpool_rxlfrag)) {
			if (wlc_hw->di[RX_FIFO]) {
				if (reg == TRUE) {
					pktpool_avail_register(wlc->pub->pktpool_rxlfrag,
							wlc_pktpool_avail_cb, wlc_hw);
				} else {
					pktpool_avail_deregister(wlc->pub->pktpool_rxlfrag,
							wlc_pktpool_avail_cb, wlc_hw);
				}
			}
		}
		...
		if (POOL_ENAB(wlc->pub->pktpool)) {
			if (wlc_hw->di[PKT_CLASSIFY_FIFO]) {
				if (reg == TRUE) {
					...
					pktpool_avail_deregister(wlc->pub->pktpool,
							wlc_sharedpktpool_avail_cb, wlc_hw);
				}
			}
		}
...
BCMUCODEFN(wlc_bmac_set_dma_burstlen_pcie)(wlc_hw_info_t *wlc_hw, bmac_dmactl_t *dmactl)
{
	...
	WL_MAC(("MRRS Read from config reg %x \n", mrrs));
#ifdef WLTEST
	...
	WL_MAC(("MPS Read from config reg %x \n", mps));
...
#ifdef BCMDBG
	WL_MAC(("Read from config regs: %x %x max_speed %s, max_width x%d, neg_speed %s"
	...
	bmac_dmactl_t *dmactl)
{
...
#ifdef BCMPCIEDEV
		if (BCMPCIEDEV_ENAB() && (BUSCORETYPE(wlc_hw->sih->buscoretype) == PCIE2_CORE_ID)) {
			burstlen_cap = 1;
		}
#endif /* BCMPCIEDEV */
	}
...
#ifdef BCMDBG
	{
		...
		WL_MAC(("wl%d: DMA tx outstanding reads: %s(value: %d) burstlen_cap: %d,"
			"channel_switch: %d\n"
 ...
 * aligned address. This is achieved by aligning each phyrx struct (d11phyrxsts_t) array element
...
wlc_bmac_phyrxsts_alloc(wlc_hw_info_t *wlc_hw)
{
	...
	rxphysts_objsz = sizeof(d11phyrxsts_t);
	if (STS_RX_ENAB(wlc_hw->wlc->pub)) {
		/* size calculation for d11phyrxsts_t array (alignment requirement for eg 684b0) */
		rxphysts_objsz = ALIGN_SIZE(rxphysts_objsz, sts_mp_align);
	}

	wlc_hw->sts_phyrx_alloc_sz = (rxphysts_objsz * STSBUF_MP_N_OBJ);
	wlc_hw->sts_phyrx_alloc_sz += sts_mp_align;
#ifdef BCM_SECURE_DMA
	...
	/* sets a global variable used by e.g. the D11PHYRXSTS_GE129_ACCESS_REF() macro */
	...
	for (i = 0; i < STSBUF_MP_N_OBJ; i++) {
		wlc_hw->sts_buff_t_array[i].phystshdr =
		         (d11phyrxsts_t*)(sts_phyrx_array + i * rxphysts_objsz);
...
BCMUCODEFN(wlc_bmac_attach_dmapio)(wlc_hw_info_t *wlc_hw, bool wme)
{
	...
	if (PIO_ENAB_HW(wlc_hw)) {
		...
		HWA_RXFILL_EXPR({
			if (hwa_dev) {
				...
				if (HWA_HME_MACIFS_RX_EN(hwa_dev)) {
					dma_attach_flags |= BCM_DMA_HWA_HME_RX;
				}
			}
		});
		...
		HWA_TXFIFO_EXPR({
			if (hwa_dev) {
				...
				if (HWA_HME_MACIFS_TX_EN(hwa_dev)) {
					dma_attach_flags |= BCM_DMA_HWA_HME_TX;
				}
			}
		});
#endif /* BCMHWA */
		...
		ASSERT(BCMEXTRAHDROOM >= wlc->txhroff);
#endif // endif
...
#if defined(STS_FIFO_RXEN) || defined(STS_XFER)
		if (STS_RX_ENAB(wlc->pub) || D11REV_IS(wlc_hw->corerev, 129)) {
			di = dma_attach_ext(wlc_hw->dmacommon, osh, name, wlc_hw->sih,
				DMAREG(wlc_hw, DMA_TX, 3), DMAREG(wlc_hw, DMA_RX, 3),
				/* PhyRx Status buffers are already 8 bytes aligned */
				(dma_attach_flags & ~BCM_DMA_RX_ALIGN_8BYTE),
				WLC_HW_MAP_TXFIFO(wlc, 3), ntxd, tune->nrxd_sts, tune->rxbufsz_sts,
				0, tune->bufpost_sts, D11PHYRXSTS_RECV_OFFSET, &wl_msg_level);
		} else
#endif /* STS_FIFO_RXEN || STS_XFER */
		{
			di = dma_attach_ext(wlc_hw->dmacommon, osh, name, wlc_hw->sih,
				DMAREG(wlc_hw, DMA_TX, 3), NULL, dma_attach_flags,
				WLC_HW_MAP_TXFIFO(wlc, 3), ntxd, 0, 0, 0, 0, 0, &wl_msg_level);
		}
...
#else /* ! (STS_FIFO_RXEN || WLC_OFFLOADS_RXSTS) */
		...
		if (D11REV_GE(wlc_hw->corerev, 128)) {
			if (BCM_DMA_CT_ENAB(wlc)) {
				...
				for (i = NFIFO_LEGACY; i < WLC_HW_NFIFO_TOTAL(wlc) -
					WLC_DEEP_RING_NFIFO; i++) {
				...
				/* attach deeper dma rings with remaining FIFOs */
				for (; i < WLC_HW_NFIFO_TOTAL(wlc); i++) {
					snprintf(name, sizeof(name), rstr_wlD, unit, i);
					di = dma_attach_ext(wlc_hw->dmacommon, osh, name,
						wlc_hw->sih,
						(void*)(uintptr)(
						...
						NULL, dma_attach_flags,
						WLC_HW_MAP_TXFIFO(wlc, i),
						tune->ntxd_lfrag, 0, 0, -1, 0, 0, &wl_msg_level);
					if (di == NULL) {
						WL_ERROR(("wl%d: dma_attach inddma fifo %d failed"
							"\n", unit, i));
						goto dma_attach_fail;
...
#if defined(DMA64DD_SHORT)
				if (wlc->short_aqmdd) {
					/* Use new AQM 8 bytes descriptor format */
					dma_attach_flags |= BCM_DMA_DESC_ONLY_8B_FLAG;
				}
#endif // endif
		...
		if (HWA_ENAB(wlc->pub)) {
			...
			for (i = 0; i < RX_FIFO2; i++) {
				...
				fifo_depth = 0;
				dma_get_fifo_info(wlc_hw->di[i], &fifo_base, &fifo_depth, FALSE);
			...
			for (i = 0; i < WLC_HW_NFIFO_TOTAL(wlc); i++) {
				...
				fifo_depth = 0;
				dma_get_fifo_info(wlc_hw->di[i], &fifo_base, &fifo_depth, TRUE);
				PHYSADDR64HISET(fifo_addr, PHYSADDRHI(fifo_base));
				PHYSADDR64LOSET(fifo_addr, PHYSADDRLO(fifo_base));
				...
				aqm_fifo_depth = 0;
				dma_get_fifo_info(wlc_hw->aqm_di[i], &aqm_fifo_base,
					&aqm_fifo_depth, TRUE);
	...
	if (BCMSPLITRX_ENAB()) {
		WL_MAC(("wlc_bmac_attach_dmapio: enable split RX\n"));
	...
	if (PKT_CLASSIFY()) {
		WL_MAC(("wlc_bmac_attach_dmapio: Enable PKT_CLASSIFY\n"));
		/* enable host flags to do ucode frame classification */
		wlc_bmac_enable_rx_hostmem_access(wlc_hw, TRUE);
	}

	if (RXFIFO_SPLIT()) {
		WL_MAC(("wlc_bmac_attach_dmapio: link F0/F1 dma \n"));
		dma_link_handle(wlc_hw->di[RX_FIFO1], wlc_hw->di[RX_FIFO]);
	}

#if defined(STS_FIFO_RXEN) || defined(STS_XFER)
	if (STS_RX_ENAB(wlc_hw->wlc->pub) || D11REV_IS(wlc_hw->corerev, 129)) {
	...
	if (D11REV_GE(wlc_hw->corerev, 129)) {
		...
		dma_set_sts_xfer_phyrxs(wlc_hw->di[RX_FIFO], TRUE);

		if (wlc_hw->di[RX_FIFO1]) {
			dma_set_sts_xfer_phyrxs(wlc_hw->di[RX_FIFO1], TRUE);
		}

		if (wlc_hw->di[RX_FIFO2]) {
			dma_set_sts_xfer_phyrxs(wlc_hw->di[RX_FIFO2], TRUE);
		}
	}
#endif /* STS_XFER */
...
/* Change the DMA table location */
#if defined(BCMPCIEDEV) && defined(BCMHWA) && defined(BCMHME) && \
	(defined(HWA_RXFILL_BUILD) || defined(HWA_TXFIFO_BUILD))
int
wlc_bmac_hme_macifs_upd(wlc_info_t *wlc, dma64addr_t hmeaddr, uint hmelen)
{
	int i, ret = BCME_OK;
	uint32 fifo_depth;
	dmaaddr_t fifo_base;
	dma64addr_t fifo_addr;
	uint fifo_flags, aqm_fifo_flags;
	wlc_hw_info_t *wlc_hw = wlc->hw;
	HWA_RXFILL_EXPR(dma64regs_t *d64regs);
	HWA_TXFIFO_EXPR(uint32 aqm_fifo_depth);
	HWA_TXFIFO_EXPR(dmaaddr_t aqm_fifo_base);
	...
	ASSERT(!PIO_ENAB_HW(wlc_hw));

	// Update HME macifs memory address to hnddma.
	ASSERT(wlc_hw->dmacommon != NULL);
	dma_common_hme_upd(wlc_hw->dmacommon, hmeaddr, hmelen);

	// Check all di and aqm_di
	...
	for (i = 0; i < WLC_HW_NFIFO_TOTAL(wlc_hw->wlc); i++) {
		...
		if (wlc_hw->di[i]) {
			ret = dma64_hme_alloc(wlc_hw->di[i], &fifo_flags);
			if (ret != BCME_OK)
				return ret;
		}

		if (wlc_hw->aqm_di[i]) {
			ret = dma64_hme_alloc(wlc_hw->aqm_di[i], &aqm_fifo_flags);
			if (ret != BCME_OK)
				return ret;
		}
		...
		//Rx
		HWA_RXFILL_EXPR({
		if (fifo_flags & BCM_DMA_HWA_HME_RX) {
			fifo_depth = 0;
			dma_get_fifo_info(wlc_hw->di[i], &fifo_base, &fifo_depth, FALSE);
			PHYSADDR64HISET(fifo_addr, PHYSADDRHI(fifo_base));
			PHYSADDR64LOSET(fifo_addr, PHYSADDRLO(fifo_base));
			PHYSADDR64HISET(fifo_addr, PHYSADDR64HI(hmeaddr)|SI_PCIE_DMA_H32);

			d64regs = DMAREG(wlc_hw, DMA_RX, i);
			hwa_rxfill_fifo_attach(wlc, 0, i, fifo_depth, fifo_addr,
				(uint32)&d64regs->ptr, TRUE);
		}});

		//Tx
		HWA_TXFIFO_EXPR({
		if ((fifo_flags & BCM_DMA_HWA_HME_TX) ||
			(aqm_fifo_flags & BCM_DMA_HWA_HME_TX)) {
			ASSERT(aqm_fifo_flags & BCM_DMA_HWA_HME_TX);
			ASSERT(fifo_flags & BCM_DMA_HWA_HME_TX);
			...
			aqm_fifo_depth = 0;
			dma_get_fifo_info(wlc_hw->aqm_di[i], &aqm_fifo_base, &aqm_fifo_depth, TRUE);
			PHYSADDR64HISET(aqm_fifo_addr, PHYSADDRHI(aqm_fifo_base));
			PHYSADDR64LOSET(aqm_fifo_addr, PHYSADDRLO(aqm_fifo_base));
			PHYSADDR64HISET(aqm_fifo_addr, PHYSADDR64HI(hmeaddr)|SI_PCIE_DMA_H32);

			(void)hwa_txfifo_config(hwa_dev, 0, WLC_HW_MAP_TXFIFO(wlc, i),
				fifo_addr, fifo_depth, aqm_fifo_addr, aqm_fifo_depth, TRUE);
		}});
	}

	return ret;
}
#endif /* BCMPCIEDEV && BCMHWA && BCMHME && (HWA_RXFILL_BUILD || HWA_TXFIFO_BUILD) */
...
wlc_bmac_sr_verify_ex(wlc_hw_info_t *wlc_hw, struct bcmstrbuf *b, int sr_core)
{
	...
	if (sr_core == SRENG2) {
		...
		phy_vasip_set_clk(wlc_hw->band->pi, FALSE);
	} else {
...
BCMPREATTACHFN(wlc_bmac_process_ucode_sr)(wlc_hw_info_t *wlc_hw)
{
	...
	if (SR_ENAB()) {
		wlc_hw->regs = (d11regs_t *)si_setcore(wlc_hw->sih, D11_CORE_ID, MAC_CORE_UNIT_0);
		wlc_bmac_sr_init(wlc_hw, MAC_CORE_UNIT_0);
	}
#endif // endif
...
BCMATTACHFN(wlc_bmac_register_iovt_all)(wlc_hw_info_t *hw, wlc_iocv_info_t *ii)
{
	...
	FOREACH_WLC_BAND(hw->wlc, bandunit) {
		wlc_hwband_t *band = hw->bandstate[bandunit];
...
static void wlc_bmac_board_analog_rf_filters(wlc_hw_info_t *wlc_hw)
{
	...
	if ((boardname = getvar(wlc_hw->vars, "boardid")) != NULL) {
		...
		if (!strncmp(boardname, RF_FILTER_BOARD_NAME, strlen(RF_FILTER_BOARD_NAME))) {
			...
			if (IS_SINGLEBAND_5G(wlc_hw->wlc)) {
...
static void
wlc_bmac_corerev_minor(wlc_hw_info_t *wlc_hw)
{
	if ((D11REV_IS(wlc_hw->corerev, 129) || D11REV_IS(wlc_hw->corerev, 130))) {
		uint32 minor = (0xffff & R_REG(wlc_hw->osh, D11_Workaround(wlc_hw)))
...
/**
 * restrict bitmask indicating supported bands taking phy capabilities
 * and compile-time options into account.
 */
static void
BCMATTACHFN(wlc_bmac_restrict_bandmask)(wlc_hw_info_t *wlc_hw)
{
	uint bandmask = wlc_hw->_bandmask;
	uint phymask;

#if NCONF
	if (CHIPID(wlc_hw->sih->chip) == BCM43217_CHIP_ID) {
		bandmask = (1 << BAND_2G_INDEX);
	}
#endif /* NCONF */

	/* multi-band devid can still be single band due to phy capabilities */
	phymask = 0;
	if (wlc_hw->phy_cap & PHY_PREATTACH_CAP_SUP_2G)
		phymask |= (1 << BAND_2G_INDEX);
	if (wlc_hw->phy_cap & PHY_PREATTACH_CAP_SUP_5G)
		phymask |= (1 << BAND_5G_INDEX);
	if (wlc_hw->phy_cap & PHY_PREATTACH_CAP_SUP_6G)
		phymask |= (1 << BAND_6G_INDEX);

	bandmask &= phymask;
	/* finally apply compile time bandmask */
	bandmask &= BANDMASK;

	if (!bandmask)
		WL_ERROR(("wl%d: no bands for devid %u\n", wlc_hw->unit,
			wlc_hw->deviceid));

	wlc_hw->_bandmask = bandmask;
}
...
BCMATTACHFN(wlc_bmac_attach)(wlc_info_t *wlc, uint16 vendor, uint16 device, uint unit,
	bool piomode, osl_t *osh, volatile void *regsva, uint bustype, void *btparam)
{
	wlc_hw_info_t *wlc_hw;
	char *macaddr = NULL;
	char *vars;
	...
	wlc->cmn->num_d11_cores = si_numd11coreunits(wlc->pub->sih);
...
no_devid_override:
	...
	if ((ISSIM_ENAB(wlc_hw->sih)) &&
	    ((CHIPID(wlc_hw->sih->chip) == BCM4360_CHIP_ID) ||
	     ...
	     (CHIPID(wlc_hw->sih->chip) == BCM43526_CHIP_ID) ||
	     (CHIPID(wlc_hw->sih->chip) == BCM4352_CHIP_ID))) {
		device = (CHIPID(wlc_hw->sih->chip) == BCM4352_CHIP_ID) ?
			BCM4352_D11AC_ID : BCM4360_D11AC_ID;
	}

	/* verify again the device is supported */
	wlc_hw->_bandmask = wlc_get_bandmask(device);
	if (wlc_hw->_bandmask == 0) {
...
#ifdef BCMHWA
	{
		...
		/* Check if chip support HWA */
		hwa_regs = si_setcore(wlc->pub->sih, HWA_CORE_ID, 0);
		if (hwa_regs == NULL) {
			WL_ERROR(("wl%d: chip does not support HWA\n", unit));
			WL_HWA_DEVP(wlc) = (struct hwa_dev*)NULL;
		} else {
#ifndef DONGLEBUILD
			//use PCIE register PCIE2_BAR0_CORE2_WIN2 for HWA register space
			si_backplane_pci_config_bar0_core2_win2(wlc->pub->sih,
				si_addrspace(wlc_hw->sih, CORE_SLAVE_PORT_0, CORE_BASE_ADDR_0));
			//adjust the hwa_regs with offset for PCIE2_BAR0_CORE2_WIN2
			hwa_regs = (void *)(hwa_regs + PCIE2_BAR0_CORE2_WIN2_OFFSET);
#endif /* !DONGLEBUILD */
			// For FD, init the Global HWA instance
			// For NIC mode, init the per wl radio HWA instance
			WL_HWA_DEVP(wlc) = hwa_attach(wlc, wlc_hw->deviceid, osh, hwa_regs,
				wlc_hw->sih, unit, bustype);

			si_setcoreidx(wlc_hw->sih, coreidx);

			if (WL_HWA_DEVP(wlc) == NULL) {
			...
			hwa_mac_config(WL_HWA_DEVP(wlc), HWA_HC_HIN_REGS, 0, wlc_hw->regs, 0U);

			/* Set the MAC_AXI_BASE */
			HWA_RXDATA_EXPR(hwa_mac_config(WL_HWA_DEVP(wlc), HWA_MAC_AXI_BASE, 0, NULL,
				si_addrspace(wlc_hw->sih, CORE_SLAVE_PORT_1, CORE_BASE_ADDR_0)));

			/* Set the MAC_BASE_ADDR */
			hwa_mac_config(WL_HWA_DEVP(wlc), HWA_MAC_BASE_ADDR, 0, NULL,
				si_addrspace(wlc_hw->sih, CORE_SLAVE_PORT_0, CORE_BASE_ADDR_0));

			/* Set the FrmTxStatus offset */
			HWA_TXSTAT_EXPR(hwa_mac_config(WL_HWA_DEVP(wlc), HWA_MAC_FRMTXSTATUS, 0,
				NULL, D11_FrmTxStatus_OFFSET(wlc_hw)));

			HWA_TXFIFO_EXPR({
				/* Set the MAC IND XMT Pointer */
				hwa_mac_config(WL_HWA_DEVP(wlc), HWA_MAC_IND_XMTPTR, 0, NULL,
					D11_inddma_OFFSET(wlc_hw) + OFFSETOF(dma64regs_t, ptr));
				/* Set the MAC IND QSel */
				hwa_mac_config(WL_HWA_DEVP(wlc), HWA_MAC_IND_QSEL, 0, NULL,
					D11_IndAQMQSel_OFFSET(wlc_hw));
			});
		}
	}
#endif /* BCMHWA */
...
#if defined(WLC_OFFLOADS_RXSTS)
	/* rev130/131/132 use M2M channel for phyrxsts, just like rev129, but has no rxfifo-3 */
	wlc->pub->_sts_offld_rxen = D11REV_GE(wlc_hw->corerev, 130);
#endif /* WLC_OFFLOADS_RXSTS */

	wlc_bmac_corereset(wlc_hw, 0);
	...
	wlc_hw->boardflags4 = (uint32)getintvar_slicespecific(wlc_hw, vars, rstr_boardflags4);
	...
	wlc_bmac_restrict_bandmask(wlc_hw);
	if (wlc_hw->_bandmask == 0) {
		err = 36;
		goto fail;
	}
	...
	wlc->pub->_bandmask = wlc_hw->_bandmask;
	wlc->pub->corerev_minor = wlc_hw->corerev_minor;

	WL_MAC(("wlc_bmac_attach, deviceid 0x%x bands %x\n", wlc_hw->deviceid,
		wlc_hw->_bandmask));
...
#if defined(PKTC) || defined(PKTC_DONGLE)
	WL_MAC(("wlc_bmac_attach, pktc<%d> pktc_tx<%d>\n",
	...
	WL_MAC(("RXOFFSET Programming : datafiforxoff %d d11rxoff %d hwrxoff %d \n",
		wlc->datafiforxoff, wlc->d11rxoff, wlc->hwrxoff));

	wlc->hwrxoff_pktget = (wlc->hwrxoff % 4) ?  wlc->hwrxoff : (wlc->hwrxoff + 2);

	wlc->txhroff = D11REV_GE(wlc_hw->corerev, 128) ? D11AX_TXOFF :
	               D11REV_GE(wlc_hw->corerev, 40)  ? D11AC_TXOFF : D11_TXOFF;
	...
	if (WLC_CT_HW_SUPPORTED(wlc)) {
		if (D11REV_GE(wlc_hw->corerev, 128)) {
			...
			wlc->_dma_ct = TRUE;
#if defined(DMA64DD_SHORT)
			if (D11REV_GE(wlc_hw->corerev, 132)) {
				wlc->short_aqmdd = TRUE;
			}
#endif // endif
		} else if (!wlc->_dma_ct) {
	...
	/* initialize software state for each core and band */
	FOREACH_WLC_BAND_UNORDERED(wlc_hw->wlc, bandunit) {
		wlc_setxband(wlc_hw, bandunit);

		wlc_hw->band->bandunit = bandunit;
		wlc_hw->band->bandtype = wlc_bandunit2bandtype(bandunit);

		switch (wlc_hw->band->bandtype) {
		case WLC_BAND_6G:
			wlc->band->first_ch = CH_MIN_6G_CHANNEL;
			wlc->band->last_ch = CH_MAX_6G_CHANNEL;
			wlc->band->ch_spacing = CH_20MHZ_APART;
			break;
		case WLC_BAND_5G:
			wlc->band->first_ch = CH_MIN_5G_CHANNEL;
			wlc->band->last_ch = CH_MAX_5G_CHANNEL;
			wlc->band->ch_spacing = CH_20MHZ_APART;
			break;
		case WLC_BAND_2G:
			wlc->band->first_ch = CH_MIN_2G_CHANNEL;
			wlc->band->last_ch = CH_MAX_2G_CHANNEL;
			wlc->band->ch_spacing = CH_5MHZ_APART;
			break;
		}
		 ...
		 */
		wlc->band->bandunit = bandunit;
		wlc->band->bandtype = wlc_bandunit2bandtype(bandunit);
		...
		WL_NONE(("wl%d: %s: bandunit %d bandtype %d coreidx %d\n", unit,
		         ...
		         wlc->core->coreidx));
		if ((wlc_hw->band->pi = phy_module_attach(wlc_hw->phy_sh,
				(void *)(uintptr)wlc_hw->regs,
		...
		phy_utils_get_phyversion(wlc_hw->band->pi, &wlc_hw->band->phytype,
			&wlc_hw->band->phyrev, &wlc_hw->band->radioid, &wlc_hw->band->radiorev,
			&wlc_hw->band->phy_minor_rev);

		wlc_hw->band->core_flags = phy_utils_get_coreflags(wlc_hw->band->pi);
...
#endif /* WLSMC */
#ifdef WLWARENG
	if (wlc_wareng_hw_supported(wlc_hw->corerev))
		wlc_wareng_download(wlc_hw);
#endif /* WLWARENG */
#endif /* DONGLEBUILD */
...
//	return err;
	...
	WL_MAC(("wl%d: %s: board 0x%x macaddr: %s\n", unit, __FUNCTION__,
	...
	if (BCMSPLITRX_ENAB()) {
		...
		if (POOL_ENAB(wlc->pub->pktpool)) {
			...
			WL_ERROR(("%s: PKT Pool not available split RX mode \n", __FUNCTION__));
		}
...
BCMATTACHFN(wlc_bmac_info_init)(wlc_hw_info_t *wlc_hw)
{
	...
	/* initial value for receive interrupt lazy control */
	...
	wlc_hw->chanspec = CH20MHZ_CHSPEC2(1, WL_CHANSPEC_BAND_2G);
	...
	if (WLRXOV_ENAB(wlc->pub))
		wlc_bmac_set_defmacintmask(wlc_hw, MI_RXOV, MI_RXOV);
#endif // endif

#if defined(WL_OCE_AP)
	wlc_bmac_set_defmacintmask(wlc_hw, MI_BCNSUCCESS, MI_BCNSUCCESS);
#endif // endif
...
#if defined(WLC_OFFLOADS_RXSTS)
		/* Stop D11 MAC from interrupting host */
		wlc_bmac_set_defmacintmask(wlc_hw, MI_DMAINT, ~MI_DMAINT);
#endif /* WLC_OFFLOADS_RXSTS */
	}
	...
	if (D11REV_GE(wlc_hw->corerev, 130)) {
		/* TODO: STS_XFER_TXS_ENAB - Stop MI_TFS from interrupting host */
...
BCMATTACHFN(wlc_bmac_detach)(wlc_info_t *wlc)
{
	...
	FOREACH_WLC_BAND_UNORDERED(wlc_hw->wlc, bandunit) {
		band = wlc_hw->bandstate[bandunit];
		if (band->pi) {
			/* Detach this band's phy */
			phy_module_detach(band->pi);
			band->pi = NULL;
		}
	}
	...
	if (wlc_hw->btc->wlc_btc_params) {
		MFREE(wlc->osh, wlc_hw->btc->wlc_btc_params,
			BTCX_SHM_BACKUP_SIZE*sizeof(uint16));
...
BCMINITFN(wlc_bmac_init)(wlc_hw_info_t *wlc_hw, chanspec_t chanspec)
{
	...
	wlc_setxband(wlc_hw, CHSPEC_BANDUNIT(chanspec));
	wlc_phy_chanspec_radio_set(wlc_hw->band->pi, chanspec);
	wlc_hw->chanspec = chanspec;
	...
	if (D11REV_IS(wlc_hw->corerev, 61)) {
		si_pmu_set_mac_rsrc_req(wlc_hw->sih, MAC_CORE_UNIT_0);
	}
...
BCMINITFN(wlc_bmac_up_prep)(wlc_hw_info_t *wlc_hw)
{
	...
	if (BUSTYPE(wlc_hw->sih->bustype) == PCI_BUS) {
...
#if !defined(DONGLEBUILD)
#if defined(BCMHWA) && defined(HWA_TXSTAT_BUILD)
		/* Enable hwa interrupt in PCIEIntMask for NIC */
		if (HWA_ENAB(wlc_hw->wlc->pub)) {
			coremask |= (1 << hwa_si_flag(WL_HWA_DEVP(wlc_hw->wlc)));
		}
#endif /* BCMHWA && HWA_TXSTAT_BUILD */

#if defined(WLC_OFFLOADS_M2M_INTR)
		/* Enable M2M core interrupt in PCIEIntMask for NIC */
		if (D11REV_GE(wlc_hw->corerev, 130)) {
			uint si_flag;
			si_flag = wlc_offload_m2m_si_flag(wlc_hw->wlc->offl);
			coremask |= (1 << si_flag);
		}
#endif /* WLC_OFFLOADS_M2M_INTR */
...
#if defined(STS_XFER_M2M_INTR)
		/* Enable M2M core interrupt in PCIEIntMask for NIC */
		if (STS_XFER_PHYRXS_M2M_ENAB(wlc_hw->wlc->pub)) {
			uint si_flag;
			si_flag = wlc_sts_xfer_m2m_si_flag(wlc_hw->wlc);
			coremask |= (1 << si_flag);
		}
#endif /* STS_XFER_M2M_INTR */
		...
		if (D11REV_GE(wlc_hw->corerev, 129)) {
			uint si_flag;
			si_flag = wlc_csimon_m2m_si_flag(wlc_hw->wlc);
			coremask |= (1 << si_flag);
			CSIMON_DEBUG(" si_flag 0x%x coremask 0x%x\n", si_flag, coremask);
		}
#endif /* BCM_CSIMON  && ! IS_BCA_2x2AX_BUILD */

#if defined(WL_AIR_IQ)
		/* Enable M2M core interrupt in PCIEIntMask for NIC */
		if (D11REV_GE(wlc_hw->corerev, 129)) {
			uint si_flag;
			si_flag = wlc_airiq_m2m_si_flag(wlc_hw->wlc);
			coremask |= (1 << si_flag);
		}
#endif /* WL_AIR_IQ   */
#endif /* !DONGLEBUILD */
...
BCMINITFN(wlc_bmac_up_finish)(wlc_hw_info_t *wlc_hw)
{
	...
	ASSERT(WLPROBRESP_SW_ENAB(wlc_hw->wlc) || !UCPRS_DISABLED(wlc_hw->wlc->pub));
...
BCMUNINITFN(wlc_bmac_down_prep)(wlc_hw_info_t *wlc_hw)
{
	...
	callbacks += phy_down(wlc_hw->band->pi);
...
BCMUNINITFN(wlc_bmac_hw_down)(wlc_hw_info_t *wlc_hw)
{
	if (BUSTYPE(wlc_hw->sih->bustype) == PCI_BUS) {
...
#ifdef BCMECICOEX
		/* seci down */
		if (BCMCCSECICOEX_ENAB_BMAC(wlc_hw))
		...
		if (SR_ENAB() && sr_cap(wlc_hw->sih)) {
			sr_engine_enable(wlc_hw->sih, MAC_CORE_UNIT_0, IOV_SET, FALSE);
		}
...
#if defined(STS_XFER)
	if (STS_XFER_PHYRXS_FIFO_ENAB(wlc_hw->wlc->pub)) {
		W_REG(wlc_hw->osh, (D11Reggrp_intrcvlazy(wlc_hw, STS_FIFO)), intrcvlazy);
	}
#endif /* STS_XFER */
...
wlc_bmac_mhf(wlc_hw_info_t *wlc_hw, uint8 idx, uint16 mask, uint16 val, int bands)
{
	...
	const uint16 addr[] = {M_HOST_FLAGS(wlc_hw), M_HOST_FLAGS2(wlc_hw), M_HOST_FLAGS3(wlc_hw),
		M_HOST_FLAGS4(wlc_hw), M_HOST_FLAGS5(wlc_hw), M_HOST_FLAGS6(wlc_hw)};
#if defined(WL_PSMX)
	const uint16 addrx[] = {
	...
	case WLC_BAND_ALL:
		...
		break;
	case WLC_BAND_6G:
		band = wlc_hw->bandstate[BAND_6G_INDEX];
		break;
	case WLC_BAND_5G:
	...
	if (bands == WLC_BAND_ALL) {
		FOREACH_WLC_BAND(wlc_hw->wlc, bandunit) {
			wlc_hwband_t *b;

			b = wlc_hw->bandstate[bandunit];
			b->mhfs[idx] = ((b->mhfs[idx] & ~mask) | val);
		}
	}
} /* wlc_bmac_mhf */
...
wlc_bmac_ifsctl1_regshm(wlc_hw_info_t *wlc_hw, uint32 mask, uint32 val)
{
	...
	wlc_bmac_write_shm(wlc_hw, M_IFSCTL1(wlc_hw), w);
}

#ifdef WL_PROXDETECT
#define WL_PROXD_KEEP_AVBCLK_ENABLED 1
/**
 * Proximity detection service - enables a way for mobile devices to pair based on relative
 * proximity.
 */
void
wlc_enable_avb_timer(wlc_hw_info_t *wlc_hw, bool enable)
{
	osl_t *osh;
	uint32 macctrl1 = 0;
	uint32 clkst = 0;
	bool keep_avbclk_enabled = WL_PROXD_KEEP_AVBCLK_ENABLED;
	...
	if (enable) {
		if (keep_avbclk_enabled) {
			wlc_get_avb_timer_reg(wlc_hw, &clkst, &macctrl1);
			if (!(macctrl1 & MCTL1_AVB_ENABLE)) {
...
void
wlc_enable_avb_timer_war(wlc_hw_info_t *wlc_hw, bool enable)
{
	osl_t *osh = wlc_hw->osh;
...
void
wlc_get_avb_timer_reg(wlc_hw_info_t *wlc_hw, uint32 *clkst, uint32 *maccontrol1)
{
	osl_t *osh;
...
void
wlc_get_avb_timestamp(wlc_hw_info_t *wlc_hw, uint32* ptx, uint32* prx)
{
	osl_t *osh;
...
{
	return (D11REV_IS(corerev, 129) || D11REV_IS(corerev, 132));
}
#endif /* WL_PSMR1 */

/* Write psm_maccommand */
void
wlc_bmac_psm_maccommand(wlc_hw_info_t *wlc_hw, uint16 val)
{
	...
	const struct ether_addr *addr)
{
	...
	W_REG(osh, D11_rcm_ctl(wlc_hw), (uint16)(RCM_INC_DATA | match_reg_offset));
...
/**
 * @param[in] offset  Byte offset into the transmit FIFO RAM, should be a uint32 multiple.
 */
void
...
{
	uint shm_bcn_tpl0_base; /**< byte offset into transmit FIFO RAM */
...
void
wlc_bmac_update_synthpu(wlc_hw_info_t *wlc_hw)
{
...
WLBANDINITFN(wlc_bmac_bsinit)(wlc_hw_info_t *wlc_hw, chanspec_t chanspec, bool chanswitch_path)
{
	...
	/* phymode switch requires phyinit */
	phy_chanmgr_bsinit(wlc_hw->band->pi, chanspec,
		(!chanswitch_path) || phy_init_pending(wlc_hw->band->pi));
...
wlc_bmac_4349_btcx_prisel_war(wlc_hw_info_t *wlc_hw)
{
	...
	/* Apply the setting to D11 core unit one always */
	WL_MAC(("MIMO: PRISEL ISSUE WORKAROUND\n"));
...
wlc_bmac_phy_reset(wlc_hw_info_t *wlc_hw)
{
	...
	if (WLCISNPHY(wlc_hw->band) && NREV_GE(wlc_hw->band->phyrev, 3) &&
		...
		if (ACMAJORREV_47_51_129_130(wlc_hw->band->phyrev)) {
			/* read idletssi before phy_reset */
			wlc_idletssi_force(wlc_hw->band->pi, tssival, TRUE);
		}
		...
		if ((wlc_hw->band->phyrev == 47) && (wlc_hw->band->phy_minor_rev == 2)) {
			...
			if (phyfgc == 0) {
				W_REG(osh, D11_PSM_PHY_CTL(wlc),
					(uint16)(val | PHYCTL_PHYRST | PHYCTL_PHYFGC));
				OSL_DELAY(2);
				W_REG(osh, D11_PSM_PHY_CTL(wlc),
					(uint16)(val & ~(PHYCTL_PHYRST | PHYCTL_PHYFGC)));
			} else {
				W_REG(osh, D11_PSM_PHY_CTL(wlc), (uint16)(val | PHYCTL_PHYRST));
				OSL_DELAY(2);
				W_REG(osh, D11_PSM_PHY_CTL(wlc), (uint16)(val & ~(PHYCTL_PHYRST)));
			}
	...
	if (ACMAJORREV_47_51_129_130(wlc_hw->band->phyrev)) {
		...
		W_REG(osh, D11_PSM_PHY_CTL(wlc), (uint16)(val_phyctl | PHYCTL_PHYRSTSMC_GE128));

		W_REG(wlc->osh, D11_PHY_REG_ADDR(wlc), (uint16)PHYREG_SMCBUFFERADDR);
		val = R_REG(wlc->osh, D11_PHY_REG_DATA(wlc));
		W_REG(wlc->osh, D11_PHY_REG_DATA(wlc), (uint16)(val | PHYREG_SMCBUFFERADDR_M2SRST));

		W_REG(osh, D11_PSM_PHY_CTL(wlc), (uint16)(val_phyctl & ~(PHYCTL_PHYRSTSMC_GE128)));
	}

	/* Reset WAR-engine after PHYRESET */
	if ((wlc_hw->band->phyrev == 130)) {
		// phy cpu reset top
		...
		si_wrapperreg(wlc_hw->sih, AI_IOCTRL, 1 << 21, 1 << 21);

		// Release phy cpu reset core
		si_wrapperreg(wlc_hw->sih, AI_IOCTRL, 1 << 20, 1 << 20);
	}
	 ...
	 */
	if (CHIPID(wlc_hw->sih->chip) != BCM43217_CHIP_ID) {
		phy_utils_set_bwstate(wlc_hw->band->pi, CHSPEC_BW(wlc_hw->chanspec));
	}
	...
	if (wlc_hw->band->pi != NULL)
		phy_ana_reset(wlc_hw->band->pi);
	/* restore idletssi after phy_reset to keep TPC healthy */
	if (WLCISACPHY(wlc_hw->band) && ACMAJORREV_47_51_129_130(wlc_hw->band->phyrev)) {
		wlc_idletssi_force(wlc_hw->band->pi, tssival, FALSE);
	}
...
static void
WLBANDINITFN(wlc_bmac_setband)(wlc_hw_info_t *wlc_hw, enum wlc_bandunit bandunit,
                               chanspec_t chanspec)
{
	wlc_info_t *wlc = wlc_hw->wlc;
	uint32 macintmask;
 ...
 */
static void
WLBANDINITFN(wlc_setxband)(wlc_hw_info_t *wlc_hw, enum wlc_bandunit bandunit)
{
...
wlc_bmac_4360_pcie2_war(wlc_hw_info_t* wlc_hw, uint32 vcofreq)
{
	...
	WL_MAC(("wl%d: pcie gen2 link speed: %d\n", wlc_hw->unit, linkspeed));
...
BCMINITFN(wlc_bmac_hw_up)(wlc_hw_info_t *wlc_hw)
{
	...
	if (BUSTYPE(wlc_hw->sih->bustype) == PCI_BUS) {
		...
		if ((CHIPID(wlc_hw->sih->chip) == BCM4360_CHIP_ID) ||
			...
			/* changing the avb vcoFreq as 510M (from default: 500M) */
			/* Tl clk 127.5Mhz */
				WL_MAC(("wl%d: %s: settng clock to %d\n",
...
static void
wlc_bmac_rsdb_set_mimomode(wlc_info_t *wlc)
{
	wlc_pub_t	*pub;
	uint sicoreunit;
	uint32 backup_mask;
	uint32 phymode = PHYMODE_MIMO;
	...
	/* Set up the PHY MODE for RSDB PHY */
...
BCMINITFN(wlc_bmac_corereset)(wlc_hw_info_t *wlc_hw, uint32 flags)
{
	...
	if (wlc_hw->noreset) {
		wlc_hw->macintstatus = 0;	/* skip wl_worklet after down */
	 ...
	 */
	if (wlc_bmac_rsdb_cap(wlc_hw)) {
		wlc_bmac_rsdb_set_mimomode(wlc_hw->wlc);
	}
...
BCMINITFN(wlc_bmac_btc_init)(wlc_hw_info_t *wlc_hw)
{
	...
	if (wlc_hw->boardflags & BFL_BTCOEX) {
		...
		} else if (BCMCCSECICOEX_ENAB_BMAC(wlc_hw)) {
			si_seci_init(wlc_hw->sih, SECI_MODE_SECI);
		} else if (BCMSECICOEX_ENAB_BMAC(wlc_hw) || BCMGCICOEX_ENAB_BMAC(wlc_hw)) {
...
wlc_update_splitrx_mode(wlc_hw_info_t *wlc_hw, bool mode, bool init)
{
	...
	WL_MAC(("wl: %d %s :  pub up %d int %d mode %d \n",
	...
	/* Enable bit */
	if (mode) {
		OR_REG(wlc_hw->osh, D11_RcvHdrConvCtrlSts(wlc_hw), (uint16)HDRCONV_USR_ENAB);
	} else {
		AND_REG(wlc_hw->osh, D11_RcvHdrConvCtrlSts(wlc_hw), (uint16)~HDRCONV_USR_ENAB);
	}
...
BCMINITFN(wlc_coreinit)(wlc_hw_info_t *wlc_hw)
{
	...
	uint i = 0;
	osl_t *osh = wlc_hw->osh;
	uint16 shm_mswcap_l; /** driver to ucode feature capability enable */
	uint16 shm_mswcap_h;
	...
	if (D11REV_IS(wlc_hw->corerev, 132)) {
		wlc_write_inits(wlc_hw, d11ax130initvals132);
#if defined(WL_PSMR1)
		if (PSMR1_ENAB(wlc_hw)) {
			wlc_write_inits(wlc_hw, d11ax130initvalsr1_132);
		}
#endif // endif
		wlc_write_inits(wlc_hw, d11ax130initvalsx132);
	} else if (D11REV_IS(wlc_hw->corerev, 131)) {
		wlc_write_inits(wlc_hw, d11ax129initvals131);
		wlc_write_inits(wlc_hw, d11ax129initvalsx131);
	} else if (D11REV_IS(wlc_hw->corerev, 130)) {
		if (D11MINORREV_IS(wlc_hw->corerev_minor, 2)) {
			wlc_write_inits(wlc_hw, d11ax131initvals130_2);
			wlc_write_inits(wlc_hw, d11ax131initvalsx130_2);
		} else {
		...
		if (D11MINORREV_IS(wlc_hw->corerev_minor, 2)) {
			wlc_write_inits(wlc_hw, d11ax47initvals129_2);
#if defined(WL_PSMR1)
			if (PSMR1_ENAB(wlc_hw)) {
				wlc_write_inits(wlc_hw, d11ax47initvalsr1_129_2);
			}
#endif // endif
			wlc_write_inits(wlc_hw, d11ax47initvalsx129_2);
		} else {
		...
		if (wlc_hw->wlc->pub->corerev_minor == 5) {
			...
			wlc_write_inits(wlc_hw, d11ac40initvals61_1);
		}
	} else if (D11REV_IS(wlc_hw->corerev, 65)) {
		wlc_write_inits(wlc_hw, d11ac33initvals65);
		wlc_write_inits(wlc_hw, d11ac33initvalsx65);
	} else if (D11REV_IS(wlc_hw->corerev, 56)) {
		wlc_write_inits(wlc_hw, d11ac24initvals56);
	} else if (D11REV_IS(wlc_hw->corerev, 49)) {
	...
	W_REG(wlc_hw->osh, D11_SLOW_CTL(wlc_hw), (uint16)0x0);
	W_REG(wlc_hw->osh, D11_SLOW_TIMER_L(wlc_hw), (uint16)0x0);
	W_REG(wlc_hw->osh, D11_SLOW_TIMER_H(wlc_hw), (uint16)0x10);
#endif /* ATE_BUILD */
	...
	/* Use new AQM 8 bytes descriptor format */
	if (wlc->short_aqmdd) {
		OR_REG(osh, D11_ctdma_ctl(wlc_hw), 0x1);
	}
#endif // endif
	...
	shm_mswcap_l = 0;
	/* Enable A-MSDU defragment feature */
	if (D11REV_GE(wlc_hw->corerev, 128)) {
		/* only >=128 ucode is sensitive to shm(M_MACHW_CAP_L) */
		setbit(&shm_mswcap_l, MSWCAP_GE128_DEFRAG_ON_NBIT);
	}

	shm_mswcap_h = (uint16)((wlc_hw->machwcap >> 16) & 0xffff);
	if (D11REV_GE(wlc_hw->corerev, 40)) {
		shm_mswcap_h &= (1 << C_MCAP_TKIPMIC_NBIT);
	} else {
		shm_mswcap_h &= ((1 << C_MCAP_TKIPMIC_NBIT) | (1 << C_MCAP_MBSS_NBIT) |
		            (1 << C_MCAP_BTCX_NBIT));
	}

	wlc_bmac_write_shm(wlc_hw, M_MACHW_CAP_L(wlc_hw), shm_mswcap_l);
	wlc_bmac_write_shm(wlc_hw, M_MACHW_CAP_H(wlc_hw), shm_mswcap_h);
#if defined(WL_PSMR1)
	/* Mirror PSMr0 settings */
	if (PSMR1_ENAB(wlc_hw)) {
		wlc_bmac_write_shm1(wlc_hw, M_MACHW_CAP_L(wlc_hw), shm_mswcap_l);
		wlc_bmac_write_shm1(wlc_hw, M_MACHW_CAP_H(wlc_hw), shm_mswcap_h);
	}
#endif /* WL_PSMR1 */
	...
	AND_REG(osh, D11_IFS_CTL(wlc_hw), (uint16)0x0FFF);
	W_REG(osh, D11_IFS_AIFSN(wlc_hw), (uint16)EDCF_AIFSN_MIN);
	...
	if (BCM4369_CHIP(wlc_hw->sih->chip)) {
		...
		W_REG(osh, D11_TSF_CLK_FRAC_L(wlc_hw), (uint16)0x5114);
		W_REG(osh, D11_TSF_CLK_FRAC_H(wlc_hw), (uint16)0x5); /* 'h5_5114 = 348436 */
	...
	*/
	...
	WL_MAC(("wl%d: CORE INIT : mode %d pktclassify %d rxsplit %d  hdr conversion %d"
		" DMA_CT %s short_aqmdd %d\n", wlc_hw->unit, BCMSPLITRX_MODE(), PKT_CLASSIFY(),
		RXFIFO_SPLIT(), wlc_hw->hdrconv_mode, wlc->_dma_ct ? "Enabled":"Disabled",
		wlc->short_aqmdd));
	...
	*/
	if (!(D11REV_GE(wlc_hw->corerev, 65) || D11REV_IS(wlc_hw->corerev, 61))) {
		OR_REG(osh, D11_PSMCoreCtlStat(wlc_hw), (uint16)PSM_CORE_CTL_REHE);
	}

#if defined(HNDBME) && defined(LINUXSTA_PS)
	bme_reinit(wlc_hw->sih, osh);
...
wlc_dma_pio_init(wlc_hw_info_t *wlc_hw)
{
	...
	if (!PIO_ENAB_HW(wlc_hw)) {
...
#if defined(STS_XFER)
				if (i == STS_FIFO) {
					ASSERT(STS_XFER_PHYRXS_FIFO_ENAB(wlc->pub));
					dma_sts_rxinit(wlc_hw->di[i]);
				}
#endif /* STS_XFER */
				wlc_bmac_dma_rxfill(wlc_hw, i);
			}
		...
		if (RXFIFO_SPLIT()) {
			...
			/* copy count value */
			W_REG(wlc_hw->osh, D11_RCV_COPYCNT_Q1(wlc_hw), (uint16)tune->copycount);
...
wlc_bmac_switch_macfreq(wlc_hw_info_t *wlc_hw, uint8 spurmode)
{
	...
	if (EMBEDDED_2x2AX_CORE(wlc_hw->sih->chip)) {
		...
		if (spurmode == 1) {
			...
			/* MAC 242.2504MHz */
			W_REG(osh, D11_TSF_CLK_FRAC_L(wlc_hw), (uint16)0x3A1F);
			W_REG(osh, D11_TSF_CLK_FRAC_H(wlc_hw), (uint16)0x4);
		}
	} else if (
		...
		if (spurmode == WL_SPURAVOID_ON2) {	/* 126Mhz */
			W_REG(osh, D11_TSF_CLK_FRAC_L(wlc_hw), (uint16)0x2082);
			W_REG(osh, D11_TSF_CLK_FRAC_H(wlc_hw), (uint16)0x8);
		} else if (spurmode == WL_SPURAVOID_ON1) {	/* 123Mhz */
			W_REG(osh, D11_TSF_CLK_FRAC_L(wlc_hw), (uint16)0x5341);
			W_REG(osh, D11_TSF_CLK_FRAC_H(wlc_hw), (uint16)0x8);
		} else {	/* 120Mhz */
			W_REG(osh, D11_TSF_CLK_FRAC_L(wlc_hw), (uint16)0x8889);
			W_REG(osh, D11_TSF_CLK_FRAC_H(wlc_hw), (uint16)0x8);
		}
	} else if (BCM53573_CHIP(wlc_hw->sih->chip) ||
		...
		0)
	{
		...
		W_REG(osh, D11_TSF_CLK_FRAC_L(wlc_hw), (uint16)(clk_frac & 0xffff));
		W_REG(osh, D11_TSF_CLK_FRAC_H(wlc_hw), (uint16)((clk_frac >> 16) & 0xffff));
...
BCMINITFN(wlc_gpio_init)(wlc_hw_info_t *wlc_hw)
{
	...
	if (WLSWDIV_ENAB(wlc_hw->wlc) &&
		...
		gm |= gc |= val;
		OR_REG(osh, D11_PSM_GPIOEN(wlc_hw), (uint16)gm);
		WL_MAC(("%s: GPIOctrl=0x%x, psm_gpio_oe=0x%x\n", __FUNCTION__,
			si_gpiocontrol(wlc_hw->sih, 0, 0, GPIO_DRV_PRIORITY),
			R_REG(osh, D11_PSM_GPIOEN(wlc_hw))));
	}
#endif /* WLC_SW_DIVERSITY */

	WL_MAC(("wl%d: gpiocontrol mask 0x%x value 0x%x\n", wlc_hw->unit, gm, gc));
...
BCMUCODEFN(wlc_ucode_download)(wlc_hw_info_t *wlc_hw)
{
	...
	if (TRUE) {
		...
		if (ctdma && load_mu_ucode) {
			WL_MAC(("%s: wl%d: Loading MU diags ucode\n",
				__FUNCTION__, wlc_hw->unit));
		} else {
			WL_MAC(("%s: wl%d: Loading non-MU diags ucode\n",
				__FUNCTION__, wlc_hw->unit));
		}
		if (D11REV_IS(wlc_hw->corerev, 132)) {
			ASSERT(!load_mu_ucode); /* MU ucode is not ready yet for rev132 */
			ucode32 = d11ucode_maindiag132;
			nbytes = d11ucode_maindiag132sz;
			ucodex32 = d11ucodex_maindiag132;
			nbytes_x = d11ucodex_maindiag132sz;
		} else if (D11REV_IS(wlc_hw->corerev, 131)) {
			...
			if (D11MINORREV_IS(wlc_hw->corerev_minor, 1)) {
				...
				ucode32 = d11ucode_maindiag130_2;
				nbytes = d11ucode_maindiag130_2sz;
				ucodex32 = d11ucodex_maindiag130_2;
				nbytes_x = d11ucodex_maindiag130_2sz;
			} else {
				ucode32 = d11ucode_maindiag130;
				nbytes = d11ucode_maindiag130sz;
				ucodex32 = d11ucodex_maindiag130;
			...
			if (D11MINORREV_IS(wlc_hw->corerev_minor, 2)) {
				ucode32 = d11ucode_maindiag129_2;
				nbytes = d11ucode_maindiag129_2sz;
				ucodex32 = d11ucodex_maindiag129_2;
				nbytes_x = d11ucodex_maindiag129_2sz;
			} else if (D11MINORREV_IS(wlc_hw->corerev_minor, 1)) {
	...
	if (D11REV_IS(wlc_hw->corerev, 132)) {
		...
		WL_MAC(("%s: wl%d: Loading 132 MU ucode\n",
		          __FUNCTION__, wlc_hw->unit));
		wlc_ucode_write(wlc_hw, d11ucode_mu132, d11ucode_mu132sz);
		wlc_ucodex_write(wlc_hw, d11ucodex_mu132, d11ucodex_mu132sz);
	} else if (D11REV_IS(wlc_hw->corerev, 131)) {
		ASSERT(load_mu_ucode != 0);
		if (BCMSECICOEX_ENAB_BMAC(wlc_hw)) {
			WL_ERROR(("%s: wl%d: Loading 131 MU SECI BTCX ucode\n",
				__FUNCTION__, wlc_hw->unit));
			wlc_ucode_write(wlc_hw, d11ucode_btcxeci131, d11ucode_btcxeci131sz);
			wlc_ucodex_write(wlc_hw, d11ucodex_btcxeci131, d11ucodex_btcxeci131sz);
		} else if (BTCX_ENAB(wlc_hw)) {
			WL_ERROR(("%s: wl%d: Loading 131 MU 3-wire BTCX ucode\n",
				__FUNCTION__, wlc_hw->unit));
			wlc_ucode_write(wlc_hw, d11ucode_btcxmu131, d11ucode_btcxmu131sz);
			wlc_ucodex_write(wlc_hw, d11ucodex_btcxmu131, d11ucodex_btcxmu131sz);
		} else {
			WL_ERROR(("%s: wl%d: Loading 131 MU non-BTCX ucode\n",
				__FUNCTION__, wlc_hw->unit));
		...
		ASSERT(load_mu_ucode != 0);
		WL_MAC(("%s: wl%d: Loading 130 MU ucode\n",
		          __FUNCTION__, wlc_hw->unit));
		if (D11MINORREV_IS(wlc_hw->corerev_minor, 1)) {
			if (BTCX_ENAB(wlc_hw)) {
				wlc_ucode_write(wlc_hw, d11ucode_btcxmu130_1,
						d11ucode_btcxmu130_1sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_btcxmu130_1,
						d11ucodex_btcxmu130_1sz);
			} else {
				wlc_ucode_write(wlc_hw, d11ucode_mu130_1, d11ucode_mu130_1sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_mu130_1, d11ucodex_mu130_1sz);
			}
		} else if (D11MINORREV_IS(wlc_hw->corerev_minor, 2)) { /* embedded 2x2 ax 160 mhz */
			if (BTCX_ENAB(wlc_hw)) {
				wlc_ucode_write(wlc_hw, d11ucode_btcxmu130_2,
						d11ucode_btcxmu130_2sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_btcxmu130_2,
						d11ucodex_btcxmu130_2sz);
			} else {
				wlc_ucode_write(wlc_hw, d11ucode_mu130_2, d11ucode_mu130_2sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_mu130_2, d11ucodex_mu130_2sz);
			}
		} else {
			if (BTCX_ENAB(wlc_hw)) {
				wlc_ucode_write(wlc_hw, d11ucode_btcxmu130,
						d11ucode_btcxmu130sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_btcxmu130,
						d11ucodex_btcxmu130sz);
			} else {
		...
		WL_MAC(("%s: wl%d: Loading 129 WFA MU non-BCTX ucode\n",
			__FUNCTION__, wlc_hw->unit));
#else
		WL_MAC(("%s: wl%d: Loading 129 MU ucode\n", __FUNCTION__, wlc_hw->unit));
#endif /* TESTBED_AP_11AX */
		if (D11MINORREV_IS(wlc_hw->corerev_minor, 2)) {
#ifdef TESTBED_AP_11AX
			wlc_ucode_write(wlc_hw, d11ucode_wfa129_2, d11ucode_wfa129_2sz);
			wlc_ucodex_write(wlc_hw, d11ucodex_wfa129_2, d11ucodex_wfa129_2sz);
#else
			if (BTCX_ENAB(wlc_hw)) {
				wlc_ucode_write(wlc_hw, d11ucode_btcxmu129_2,
						d11ucode_btcxmu129_2sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_btcxmu129_2,
						d11ucodex_btcxmu129_2sz);
			} else {
				wlc_ucode_write(wlc_hw, d11ucode_mu129_2, d11ucode_mu129_2sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_mu129_2, d11ucodex_mu129_2sz);
			}
#endif /* TESTBED_AP_11AX */
		} else if (D11MINORREV_IS(wlc_hw->corerev_minor, 1)) {
#ifdef TESTBED_AP_11AX
			wlc_ucode_write(wlc_hw, d11ucode_wfa129_1, d11ucode_wfa129_1sz);
			wlc_ucodex_write(wlc_hw, d11ucodex_wfa129_1, d11ucodex_wfa129_1sz);
#else
			if (BTCX_ENAB(wlc_hw)) {
				...
				wlc_ucode_write(wlc_hw, d11ucode_mu129_1, d11ucode_mu129_1sz);
				wlc_ucodex_write(wlc_hw, d11ucodex_mu129_1, d11ucodex_mu129_1sz);
			}
#endif /* TESTBED_AP_11AX */
		} else {
...
wlc_write_inits(wlc_hw_info_t *wlc_hw, const d11init_t *inits)
{
	...
	for (i = 0; inits[i].addr != 0xffff; i++) {
		...
		data_val = inits[i].value + offset_val;
		if (inits[i].addr >= PIHR2K_BASE) {
			/* address could exceed 4KB core window, use indirect reg access method */
			uint ihr_word_addr = (inits[i].addr - PIHR_BASE) / 2;
			ASSERT(inits[i].size == 2);
			wlc_bmac_write_ihr(wlc_hw, ihr_word_addr, (uint16)(data_val));
		} else if (inits[i].size == 2) {
			W_REG(osh, (uint16*)(uintptr)(base+inits[i].addr), (uint16)(data_val));
		} else if (inits[i].size == 4) {
			W_REG(osh, (uint32*)(uintptr)(base+inits[i].addr), data_val);
		}
	}
}
...
wlc_bmac_wowlucodex_start(wlc_hw_info_t *wlc_hw)
{
	...
	if ((GET_MACINTSTATUS_X(wlc_hw->osh, wlc_hw) & MI_MACSSPNDD) == 0) {
		WL_ERROR(("wl%d: %s: ucode psmx did not self-suspend!\n", wlc_hw->unit,
			__FUNCTION__));
...
wlc_bmac_wowlucode_start(wlc_hw_info_t *wlc_hw)
{
	...
	WL_MAC(("wl%d: wlc_bmac_wowlucode_start mctrl=0x%x  0x%x\n", wlc_hw->unit, mctrl,
...
#if defined(WL_PSMR1)
	if (PSMR1_ENAB(wlc_hw)) {
		WL_MAC(("wl%d: wlc_bmac_wowlucode_start mctrl1=0x%x  0x%x\n", wlc_hw->unit,
			mctrl, R_REG(wlc_hw->osh, D11_MACCONTROL_r1(wlc_hw))));
	}
#endif /* WL_PSMR1 */

	WL_MAC(("wl%d: wlc_bmac_wowlucode_start (mctrl | MCTL_PSM_JMP_0)=0x%x 0x%x\n",
	...
	if ((GET_MACINTSTATUS(wlc_hw->osh, wlc_hw) & MI_MACSSPNDD) == 0) {
		WL_ERROR(("wl%d: %s: ucode did not self-suspend!\n",
			wlc_hw->unit, __FUNCTION__));
		WL_HEALTH_LOG(wlc_hw->wlc, MACSPEND_WOWL_TIMOUT);
		err = BCME_ERROR;
...
wlc_ucode_txant_set(wlc_hw_info_t *wlc_hw)
{
	...
	if (D11REV_GE(wlc_hw->corerev, 40)) {
		WL_MAC(("wl%d: %s: need rev40 update\n", wlc_hw->unit, __FUNCTION__));
		return;
	}
...
wlc_intrs_enabled(wlc_hw_info_t *wlc_hw)
{
	...
	/**
	 * Checking only D11 MAC interrupts is suffice, as D11 & M2M interrupts are
...
#define MUTE_DATA_FIFO	TX_DATA_FIFO
	...
	if (on) {
		...
		/* if a virtual AP interface is present,
		 * set MBSSID base address to all 0xFF on mute
		 */
		if (MBSS_ENAB(wlc_hw->wlc->pub)) {
			...
			FOREACH_UP_AP(wlc, idx, cfg) {
				wlc_write_shm(wlc, M_MBS_BSSID0(wlc), 0xFFFFu);
				wlc_write_shm(wlc, M_MBS_BSSID1(wlc), 0xFFFFu);
				wlc_write_shm(wlc, M_MBS_BSSID2(wlc), 0xFFFFu);
				break;
			}
		}
#endif /* MBSS */
...
#if defined(STS_FIFO_RXEN) || defined(STS_XFER)
	if (fifo == STS_FIFO) {
		ASSERT(STS_RX_ENAB(wlc->pub) || STS_XFER_PHYRXS_FIFO_ENAB(wlc->pub));
...
wlc_bmac_tx_fifo_suspend(wlc_hw_info_t *wlc_hw, uint tx_fifo)
{
	...
	{
		WL_MAC(("%s, not suspending FIFO %d, PSMx not yet ready\n",
			__FUNCTION__, tx_fifo));
		return;
	}
#endif /* WL_PSMX */
...
#if defined(BCMHWA) && defined(HWA_TXFIFO_BUILD)
			if (hwa_txfifo_dma_active(hwa_dev, 0,
				WLC_HW_MAP_TXFIFO(wlc_hw->wlc, i)) == 0)
#else
	...
	NO_HWA_PKTPGR_EXPR({
		for (i = 0; i < WLC_HW_NFIFO_TOTAL(wlc_hw->wlc); i++) {
			if (isset(fifo_bitmap, i)) {
				...
				hwa_txfifo_clear_ovfq(hwa_dev, 0,
					WLC_HW_MAP_TXFIFO(wlc_hw->wlc, i));
			}
		}
	});
#endif /* BCMHWA) && defined(HWA_TXFIFO_BUILD) */
...
#if defined(BCMHWA) && defined(HWA_TXSTAT_BUILD)
	if (HWA_ENAB(wlc_hw->wlc->pub)) {
		hwa_txstat_reclaim(WL_HWA_DEVP(wlc_hw->wlc), 0, FALSE);
		HWA_PKTPGR_EXPR(hwa_pktpgr_txstatus_wait_to_finish(WL_HWA_DEVP(wlc_hw->wlc), -1));
	} else
#endif /* BCMHWA && HWA_TXSTAT_BUILD */
	{
		bool fatal = FALSE;
	...
	if (D11REV_GE(wlc_hw->corerev, 40)) {
		...
		for (i = 0; i < WLC_HW_NFIFO_TOTAL(wlc_hw->wlc); i++) {
			...
			if (wlc_bmac_tx_fifo_suspended(wlc_hw, i)) {
				WL_MAC(("%s: suspend request when dma%d suspended, "
...
void
wlc_bmac_print_txstatus(wlc_hw_info_t *wlc_hw, tx_status_t* txs)
{
...
/* word_offset is the raw offset after subtracting PIHR_BASE and div by 2 */
uint16
wlc_bmac_read_ihr(wlc_hw_info_t *wlc_hw, uint word_offset)
{
	uint16 v;
	W_REG(wlc_hw->osh, D11_objaddr(wlc_hw), OBJADDR_IHR_SEL | word_offset);
	(void)R_REG(wlc_hw->osh, D11_objaddr(wlc_hw));
...
wlc_bmac_write_scr(wlc_hw_info_t *wlc_hw, uint offset, uint16 v)
{
	...
	W_REG(wlc_hw->osh, D11_objaddr(wlc_hw), OBJADDR_SCR_SEL | offset);
	(void)R_REG(wlc_hw->osh, D11_objaddr(wlc_hw));
	W_REG(wlc_hw->osh, D11_objdata(wlc_hw), (uint32)v);
}
...
wlc_bmac_suspend_mac_and_wait(wlc_hw_info_t *wlc_hw)
{
	...
	if (!(R_REG(osh, D11_MACINTSTATUS(wlc_hw)) & MI_MACSSPNDD)) {
		...
		if ((CHIPID(wlc_hw->sih->chip) == BCM43217_CHIP_ID) &&
			wlc_phy_is_edcrs_high(WLC_PI(wlc)))
			return;

		if (CHIPID(wlc_hw->sih->chip) != BCM43217_CHIP_ID)
			wlc_dump_ucode_fatal(wlc, PSM_FATAL_SUSP);

		wlc_macdbg_psmwd_trigger(wlc->macdbg, WLC_MACDBG_PSMWD_MACSUSP);
...
#ifdef BCMECICOEX
	if (BCMSECICOEX_ENAB_BMAC(wlc_hw) && wlc_bmac_btc_mode_get(wlc_hw) != 0) {
		si_seci_clkreq(wlc_hw->sih, FALSE);
	}
#endif /* BCMECICOEX */
...
#ifdef BCMECICOEX
	if (BCMSECICOEX_ENAB_BMAC(wlc_hw) && wlc_bmac_btc_mode_get(wlc_hw) != 0) {
		si_seci_clkreq(wlc_hw->sih, TRUE);
	}
#endif /* BCMECICOEX */
...
wlc_bmac_ifsctl_vht_set(wlc_hw_info_t *wlc_hw, int ed_sel)
{
	...
	case WL_CHANSPEC_BW_8080:
		...
		if (wlc_hw->band->mhfs[MHF1] & MHF1_D11AC_DYNBW) {
			WL_MAC(("%s: cannot enable dynbw fow bw160 for now!\n",
				__FUNCTION__));
		}
		break;
	...
	if (!err) {
		...
		wlc_phy_conditional_suspend(wlc_hw->band->pi, &suspend);

		/* update phyreg NsyncscramInit1:scramb_dyn_bw_en */
		wlc_acphy_set_scramb_dyn_bw_en(wlc_hw->band->pi, enable);

		wlc_phy_conditional_resume(wlc_hw->band->pi, &suspend);
	}
...
wlc_bmac_core_phypll_ctl(wlc_hw_info_t* wlc_hw, bool on)
{
	...
	if (on) {
		if (D11REV_IS(wlc_hw->corerev, 30)) {
			...
			OR_REG(osh, D11_PSMCoreCtlStat(wlc_hw), (uint16)req_bits);
		...
		if (D11REV_IS(wlc_hw->corerev, 30)) {
			...
			AND_REG(osh, D11_PSMCoreCtlStat(wlc_hw), (uint16)~req_bits);
			tmp = R_REG(osh, D11_PSMCoreCtlStat(wlc_hw));
		} else {
			req_bits = CCS_ERSRC_REQ_D11PLL | CCS_ERSRC_REQ_PHYPLL;
...
wlc_coredisable(wlc_hw_info_t* wlc_hw)
{
	...
	/* radio off */
	phy_radio_switch(wlc_hw->band->pi, OFF);

	/* turn off analog core */
	phy_ana_switch(wlc_hw->band->pi, OFF);
...
wlc_bmac_recover_pkts(wlc_info_t *wlc, uint queue)
{
	...
	if (intstatus) {
		...
		while ((p = GETNEXTTXP(wlc, queue))) {
			wlc_txfifo_complete(wlc, NULL, queue, 1);
...
#if defined(WL_ULMU) && defined(WL11AX)
		if (HE_ULMU_ENAB(wlc->pub)) {
			/* relcaim all remaining utxd in TRIG fifo */
			wlc_ulmu_reclaim_utxd(wlc, NULL);
		}
#endif /* WL_ULMU && WL11AX */

#if defined(BCMHWA) && defined(HWA_TXSTAT_BUILD)
		if (HWA_ENAB(wlc->pub)) {
			hwa_txstat_reclaim(WL_HWA_DEVP(wlc), 0, TRUE);
		}
#endif /* (BCMHWA && HWA_TXSTAT_BUILD) */
...
#if defined(BCMHWA) && defined(HWA_TXFIFO_BUILD)
		if (HWA_ENAB(wlc->pub)) {
			hwa_txfifo_dma_reclaim(WL_HWA_DEVP(wlc), 0);
		}
...
#if defined(STS_FIFO_RXEN) || defined(STS_XFER)
				if (i == STS_FIFO) {
					ASSERT(STS_RX_ENAB(wlc->pub) ||
						STS_XFER_PHYRXS_FIFO_ENAB(wlc->pub));
...
wlc_bmac_dump_phy(wlc_hw_info_t *wlc_hw, const char *name, struct bcmstrbuf *b)
{
	...
	if (!strcmp(name, "radioreg") || !strcmp(name, "phyreg") || !strcmp(name, "phytbl")) {
		ret = phy_dbg_dump(wlc_hw->band->pi, name, b);
		ta_ok = TRUE;
	} else
#endif // endif
	{
		...
		FOREACH_WLC_BAND(wlc_hw->wlc, bandunit) {
			...
			band = wlc_hw->bandstate[bandunit];
			if (pi == band->pi) {
				continue; /* skip pi's that were already dumped */
			}
			pi = band->pi;
			ret = phy_dbg_dump(pi, name, b);
		}
	}
...
wlc_bmac_dump_clr(wlc_hw_info_t *wlc_hw, const char *name)
{
	...
	/* dump if 'name' is a phy dump */
	if (ret == BCME_NOTFOUND) {
		ret = phy_dbg_dump_clr(wlc_hw->band->pi, name);
	}
#endif // endif
...
static void
wlc_bmac_pkteng_handle_rx(wlc_hw_info_t *wlc_hw, wlc_worklet_info_t *worklet)
{
	wlc_info_t *wlc = wlc_hw->wlc;
	uint32 macintstatus;
	uint32 fifo0_int;
	...
	if (macintstatus & MI_DMAINT) {
		...
		if (fifo0_int) {
			W_REG(wlc->osh, &(D11Reggrp_intctrlregs(wlc_hw, RX_FIFO)->intstatus),
				DEF_RXINTMASK);
			wlc_bmac_recv(wlc_hw, RX_FIFO, TRUE, worklet);
		}
	}
}
 ...
 */
static int
wlc_bmac_pkteng_rx_start(wlc_hw_info_t *wlc_hw, wl_pkteng_t *pkteng, phy_info_t *pi, uint32 cmd,
...
static int
wlc_bmac_pkteng_rx_stop(wlc_hw_info_t *wlc_hw, phy_info_t *pi)
{
 ...
 */
static int
wlc_bmac_pkteng_tx_start(wlc_hw_info_t *wlc_hw, wl_pkteng_t *pkteng, phy_info_t *pi, uint32 cmd,
	bool is_sync, bool is_sync_unblk, void *p[], int npkts)
{
	...
	if (cmd == WL_PKTENG_PER_TX_HETB_START) {
		val = MFGTEST_HETB_TXMODE;
	} else if (cmd == WL_PKTENG_PER_TX_HETB_WITH_TRG_START) {
		val = MFGTEST_HETBTRG_TXMODE;
	}
	...
	if (pkteng_cache->status == PKTENG_CACHE_RUNNING) {
		...
		WL_MAC(("%s Caching pkteng TX request for %d pacekts\n",
	...
	if ((cmd == WL_PKTENG_PER_TX_START) || (cmd == WL_MUPKTENG_PER_TX_START) ||
		(cmd == WL_PKTENG_PER_TX_HETB_START) ||
		(cmd == WL_PKTENG_PER_TX_HETB_WITH_TRG_START)) {
	...
	if (pkteng->nframes) {
		...
		switch (cmd) {
		case WL_PKTENG_PER_TX_HETB_START:
			val = MFGTEST_HETB_TXMODE_FRMCNT;
			break;
		case WL_PKTENG_PER_TX_HETB_WITH_TRG_START:
			val = MFGTEST_HETBTRG_TXMODE_FRMCNT;
			break;
		default:
	...
	if (is_sync) {
		...
		do {
			...
			if (other_wlc && other_wlc->hw->pkteng_status == PKTENG_RX_BUSY) {
				wlc_bmac_pkteng_handle_rx(other_wlc->hw, &worklet);
			}
...
static int
wlc_bmac_pkteng_tx_stop(wlc_hw_info_t *wlc_hw, phy_info_t *pi, void *p[])
{
	wlc_info_t *wlc = wlc_hw->wlc;
	int status;
	...
	if (wlc_hw->pkteng_status == PKTENG_IDLE || wlc_hw->pkteng_status == PKTENG_TX_BUSY) {
		...
		if (status & MFGTEST_TXMODE) {
			...
			if (D11REV_GE(wlc_hw->corerev, 128)) {
				...
				/* clear bit blocking tx until rx trigger to stop cleanly */
				shm_val &= ~(1 << HE_ULTX_STS_BLOCK_NBIT);
...
wlc_bmac_radio_hw(wlc_hw_info_t *wlc_hw, bool enable, bool skip_anacore)
{
	...
	if (enable) {
		...
		if (!skip_anacore)
			phy_ana_switch(wlc_hw->band->pi, ON);
		phy_radio_switch(wlc_hw->band->pi, ON);
...
/** update auto d11 shmdef specific to ucode download type */
void
...
#endif // endif
#ifdef WL_CCA_STATS_MESH
	if (((D11REV_IS(wlc_hw->corerev, 61) && D11MINORREV_IS(wlc_hw->corerev_minor, 5)) ||
			D11REV_IS(wlc_hw->corerev, 42) || D11REV_IS(wlc_hw->corerev, 56) ||
			D11REV_IS(wlc_hw->corerev, 65) || D11REV_GE(wlc_hw->corerev, 128)) &&
			wlc_get_cca_mesh_enable(wlc_hw->wlc)) {
		int i;
		for (i = 0; i < CCA_MESH_MAX; i++) {
			int j = i*4;
			cca_counts->txnode[i] = wlc_bmac_read_counter(wlc_hw, 0,
				M_MESHCCA_TXNODE0(wlc_hw) + j*2,
				M_MESHCCA_TXNODE0(wlc_hw)+ 2 + j*2);
			cca_counts->rxnode[i] = wlc_bmac_read_counter(wlc_hw, 0,
				M_MESHCCA_RXNODE0(wlc_hw) + j*2,
				M_MESHCCA_RXNODE0(wlc_hw)+ 2 + j*2);
		}
		cca_counts->xxobss = wlc_bmac_read_counter(wlc_hw, 0,
			M_MESHCCA_OBSSNODE(wlc_hw),
			M_MESHCCA_OBSSNODE(wlc_hw)+ 2);
	}
#endif /* WL_CCA_STATS_MESH */
...
wlc_bmac_obss_stats_read(wlc_hw_info_t *wlc_hw, wlc_bmac_obss_counts_t *obss_counts)
{
	...
	if (((D11REV_IS(wlc_hw->corerev, 61) && D11MINORREV_IS(wlc_hw->corerev_minor, 5)) ||
		...
		for (i = 0; i < CCA_MESH_MAX; i++) {
			int j = i*4;
			obss_counts->txnode[i] = wlc_bmac_read_counter(wlc_hw, 0,
				M_MESHCCA_TXNODE0(wlc_hw) + j*2,
				M_MESHCCA_TXNODE0(wlc_hw)+ 2 + j*2);
			obss_counts->rxnode[i] = wlc_bmac_read_counter(wlc_hw, 0,
				M_MESHCCA_RXNODE0(wlc_hw) + j*2,
				M_MESHCCA_RXNODE0(wlc_hw)+ 2 + j*2);
		}
		obss_counts->xxobss = wlc_bmac_read_counter(wlc_hw, 0,
			M_MESHCCA_OBSSNODE(wlc_hw),
			M_MESHCCA_OBSSNODE(wlc_hw)+ 2);
	}
#endif /* WL_CCA_STATS_MESH */
...
BCMATTACHFN(wlc_bmac_btc_param_attach)(wlc_info_t *wlc)
{
	...
	if ((wlc_hw->btc->wlc_btc_params = (uint16 *)
		MALLOCZ(wlc->osh, BTCX_SHM_BACKUP_SIZE*sizeof(uint16))) == NULL) {
		WL_ERROR(
	...
	for (indx = 0; indx < BTCX_SHM_BACKUP_SIZE; indx++) {
...
BCMINITFN(wlc_bmac_btc_param_init)(wlc_hw_info_t *wlc_hw)
{
	...
	if (wlc_hw->btc->wlc_btc_params && wlc_hw->btc->btc_init_flag) {
		wlc_bmac_copyto_shm(wlc_hw, wlc_hw->btc->bt_shm_addr,
				wlc_hw->btc->wlc_btc_params,
				BTCX_SHM_BACKUP_SIZE * sizeof(uint16));
	} else {
		...
		if (wlc->btc_param_vars) {
			/* if any btc_params exist in nvram, overwrite shared memory */
...
{
	if ((*pval < BTCX_SHM_BACKUP_SIZE) && (wlc_hw->btc->bt_shm_addr)) {
...
wlc_bmac_btc_mode_set(wlc_hw_info_t *wlc_hw, int btc_mode)
{
...
#if !defined(BCMECICOEX)
	WL_ERROR(("wl%d: %s: BTCX sw feature not supported.\n", wlc_hw->unit,
		__FUNCTION__));
	return BCME_UNSUPPORTED;
#endif // endif
	...
	if (btc_mode != WL_BTC_DISABLE) {
		if ((wlc_hw->btc->wire >= WL_BTC_3WIRE) && !MCAP_BTCX_SUPPORTED_WLCHW(wlc_hw)) {
			WL_ERROR(("wl%d: %s: Bad option wire:%d machwcap:0x%x; machwcap1:0x%x\n",
				wlc_hw->unit, __FUNCTION__, wlc_hw->btc->wire, wlc_hw->machwcap,
				wlc_hw->machwcap1));
...
wlc_bmac_btc_params_set(wlc_hw_info_t *wlc_hw, int int_val, int int_val2)
{
	...
	if (int_val >= BTC_PARAMS_FW_START_IDX) {
		...
		if (!(wlc_hw->up)) {
			if (!(wlc_hw->btc->wlc_btc_params))
				return BCME_ERROR;
			if (int_val < BTCX_SHM_BACKUP_SIZE) {
...
BCMUNINITFN(wlc_bmac_btc_params_save)(wlc_hw_info_t *wlc_hw)
{
	...
	if (wlc_hw->clk) {
		...
		if (wlc_btc_params && bt_shm_addr) {
			wlc_bmac_copyfrom_shm(wlc_hw, bt_shm_addr,
					wlc_btc_params,
					BTCX_SHM_BACKUP_SIZE * sizeof(uint16));
		}
	} else {
...
wlc_bmac_btc_gpio_disable(wlc_hw_info_t *wlc_hw)
{
	...
	if (wlc_hw->clk)
		AND_REG(wlc_hw->osh, D11_PSM_GPIOEN(wlc_hw), (uint16)~wlc_hw->btc->gpio_out);
...
wlc_bmac_btc_gpio_enable(wlc_hw_info_t *wlc_hw)
{
	...
	OR_REG(wlc_hw->osh, D11_PSM_GPIOEN(wlc_hw), (uint16)wlc_hw->btc->gpio_out);
...
wlc_full_reset(wlc_hw_info_t *wlc_hw, uint32 val)
{
	...
	if (val & 2) {
		...
		WL_MAC(("wl%d: %s: starting chipc watchdog\n",
		           wlc_hw->unit, __FUNCTION__));
	...
	WL_MAC(("wl%d: %s: about to wl_init()\n", wlc_hw->unit, __FUNCTION__));
	...
	WL_MAC(("wl%d: %s: back from wl_init()\n", wlc_hw->unit, __FUNCTION__));
	WL_NONE(("wl%d: %s: up %d, hw->up %d, sbclk %d, clk %d, hw->clk %d, fastclk %d\n",
	         wlc_hw->unit, __FUNCTION__, wlc_hw->wlc->pub->up, wlc_hw->up,
	         wlc_hw->sbclk, wlc_hw->wlc->clk, wlc_hw->clk, wlc_hw->forcefastclk));

	if (val & 8) {
		WL_MAC(("wl%d: %s: calling wl_down()\n", wlc_hw->unit, __FUNCTION__));
		wl_down(wlc_hw->wlc->wl);

		WL_MAC(("wl%d: %s: calling wl_up()\n", wlc_hw->unit, __FUNCTION__));
		wl_up(wlc_hw->wlc->wl);
	}
	WL_MAC(("wl%d: %s(0x%x): done in %dmS\n", wlc_hw->unit, __FUNCTION__, val,
	           OSL_SYSUPTIME() - start));
} /* wlc_full_reset */
#endif	/* BPRESET */
...
BCMINITFN(wlc_bmac_txfifo_sz_chk)(wlc_hw_info_t *wlc_hw)
{
	...
	for (fifo_nu = 0; fifo_nu < NFIFO_LEGACY; fifo_nu++) {
		...
		WL_MAC(("wl%d: %s: FIFO %d block config, "
...
BCMINITFN(wlc_corerev_fifosz_validate)(wlc_hw_info_t *wlc_hw, uint16 *buf)
{
	...
	if (err != 0) {
		WL_ERROR(("%s: txfifo mismatch: ucode size %d driver size %d index %d\n",
			__FUNCTION__,
...
static const bmc_params_t bmc_params_132 = {
	0,   /**< .rxq_in_bm = rx queues are allocated in BM */
	128, /**< .rxq0_buf = rx queue 0: 128 buffers of 512 bytes = 64KB */
	128, /**< .rxq1_buf = rx queue 1: 128 buffers of 512 bytes = 64KB */
	0,   /**< .rxbmmap_is_en = */
...
{
	if (D11REV_IS(wlc_hw->corerev, 132))
		wlc_hw->bmc_params = &bmc_params_132;
	else if (D11REV_IS(wlc_hw->corerev, 131))
...
static void
wlc_bmac_bmc_smac_template(wlc_hw_info_t *wlc_hw, int tplbuf, uint32 doublebufsize)
{
	wlc_info_t *wlc = wlc_hw->wlc;
	osl_t *osh;
	int i;
	...
	*/
	...
	if (D11REV_IS(wlc_hw->corerev, 65) || D11REV_GE(wlc_hw->corerev, 128)) {
		...
		if (D11REV_IS(wlc_hw->corerev, 131)) {
			...
			if (D11MINORREV_IS(wlc_hw->corerev_minor, 2)) {
				ASSERT(fifo_sz == D11MAC_BMC_MACMEM_MAX_BYTES_REV130_2);
			} else {
				ASSERT(fifo_sz == D11MAC_BMC_MACMEM_MAX_BYTES_REV130);
			}
		...
		if (D11REV_GE(wlc_hw->corerev, 128)) {
			...
			W_REG(osh, D11_TXE_SCHED_CTL_ALTBASE(regs, wlc_hw->regoffsets), (uint16)0);
		}
	}
	...
	if (wlc_hw->bmc_params->rxq_in_bm) { /* TRUE for rev(65,66), FALSE for rev>=128 */
		...
		if (wlc_hw->bmc_params->rxbmmap_is_en) {  /* RXBMMAP is enabled */
			...
			W_REG(osh, D11_RCV_BM_ENDPTR_Q0(wlc_hw),
				(uint16)(((tplbuf + rxq0buf) << 7) - 1));

			W_REG(osh, D11_RCV_BM_STARTPTR_Q1_ALTBASE(regs, wlc_hw->regoffsets),
				(uint16)((tplbuf + rxq0buf) << 7));
			W_REG(osh, D11_RCV_BM_ENDPTR_Q1_ALTBASE(regs, wlc_hw->regoffsets),
				(uint16)(((tplbuf + rxq0buf + rxq1buf) << 7) - 1));
		...
		W_REG(osh, D11_RCV_FIFO_CTL_ALTBASE(regs,
			wlc_hw->regoffsets), (uint16)0x101);	/* sel and reset q1 */
		W_REG(osh, D11_RCV_FIFO_CTL_ALTBASE(regs,
			wlc_hw->regoffsets), (uint16)0x001);	/* sel and reset q0 */
	}
	...
	for (i = 0; i < num_of_fifo; i++) {
		...
		if (D11REV_GE(wlc_hw->corerev, 56)) {
			if (D11REV_GE(wlc_hw->corerev, 128)) {
				...
				*/
				W_REG(osh, D11_BMCAllocCtl_ALTBASE(regs, wlc_hw->regoffsets),
				(uint16)((alloc_thresh << BMCAllocCtl_AllocThreshold_SHIFT_Rev50) |
					alloc_cnt));
			}
			 ...
			 */
			W_REG(osh, D11_MsduThreshold_ALTBASE(regs, wlc_hw->regoffsets),
				(uint16)0x8);
		} else {
			W_REG(osh, D11_BMCAllocCtl_ALTBASE(regs, wlc_hw->regoffsets),
			(uint16)((alloc_thresh << BMCAllocCtl_AllocThreshold_SHIFT) | alloc_cnt));
		}
		...
		if (D11REV_GE(wlc_hw->corerev, 128)) {
			...
			/* Enable this fifo */
			W_REG(osh, D11_BMCCmd_ALTBASE(regs, wlc_hw->regoffsets), (uint16)(fifo |
	...
	if (D11REV_GE(wlc_hw->corerev, 128)) {
		...
		for (i = 0; i < 3; i++) {
			...
			for (j = 0; j < 4; j++) {
				...
				(uint16)((bmc_statsel[j] << D11MAC_BMC_NBIT_STASEL_GE128) | fifo |
	...
	for (i = 0; i < tplbuf; i ++) {
		...
		if (end_idx >= tplbuf)
			end_idx = tplbuf - 1;
		W_REG(osh, D11_MSDUEntryStartIdx_ALTBASE(regs, wlc_hw->regoffsets), (uint16)i);
		W_REG(osh, D11_MSDUEntryEndIdx_ALTBASE(regs, wlc_hw->regoffsets), (uint16)end_idx);
		W_REG(osh, D11_MSDUEntryBufCnt_ALTBASE(regs, wlc_hw->regoffsets),
			(uint16)(end_idx - i + 1));
	...
	if (wlc_bmac_rsdb_cap(wlc_hw)) {
		...
		for (i = 0; i < num_of_fifo; i++) {
			...
			if (D11REV_GE(wlc_hw->corerev, 128)) {
				...
				W_REG(osh, D11_BMCCmd_ALTBASE(regs, wlc_hw->regoffsets),
					(uint16)(fifo | (1 << 10) | (1 <<  BMCCmd_Enable_SHIFT)));
			}
		}

		wlc_bmac_bmc_smac_template(wlc_hw, tplbuf, doublebufsize);
		...
		*/
		if (D11REV_GE(wlc_hw->corerev, 56)) {
			W_REG(osh, D11_XmtTemplatePtrOffset_ALTBASE(sregs, wlc_hw->regoffsets),
				(uint16)(D11MAC_BMC_TPL_BYTES_PERCORE / 4));
		}
	}
...
wlc_bmac_bmc_dump(wlc_hw_info_t *wlc_hw, struct bcmstrbuf *b)
{
	...
	for (i = 0; i < num_of_fifo; i++) {
		...
		for (j = 0; j < num_statsel; j++, p++) {
			W_REG(osh, D11_BMCStatCtl(wlc_hw),
			(uint16)((j << statsel_nbit) | (fifoid & (bmask_fifosel)) |
			(seltype << D11MAC_BMC_SELTYPE_NBIT)));
		...
		for (i = 0; i < num_of_fifo; i++) {
			...
			if (D11REV_GE(wlc_hw->corerev, 128)) {
				...
				W_REG(osh, D11_BMCCmd1(wlc_hw),
				    (uint16)((seltype << BMCCMD1_SELTYPE_NBIT) |
				             (fifoid << BMCCMD1_SELNUM_NBIT) |
				             (0x3 << BMCCMD1_RDSRC_SHIFT_GE128)));
			} else {
				W_REG(osh, D11_BMCCmd1(wlc_hw), (uint16)((bmccmd1 &
				~(0xf << BMCCMD1_TIDSEL_SHIFT)) | (fifo << BMCCMD1_TIDSEL_SHIFT)
				| (0x3 << BMCCMD1_RDSRC_SHIFT_LT128)));
			}

			if ((fifo == rxq0_idx || fifo == rxq1_idx) &&
				D11REV_LT(wlc_hw->corerev, 132)) {
				bcm_bprintf(b,
	...
	chanspec_t chanspec, bool scan_in_progress, bool roam_in_progress)
{
	...
	if (!wlc_hw->wlc->primary_bsscfg)
		return;

	if (wlc_hw->wlc->primary_bsscfg->associated) {
		infrachan = CHSPEC_CHANNEL(wlc_hw->wlc->primary_bsscfg->current_bss->chanspec);
	}
	else {
		infrachan = 0;
	}
	...
	if (BCMCOEX_ENAB_BMAC(wlc_hw) && !scan_in_progress && !roam_in_progress) {
		...
		if ((wlc_hw->wlc->primary_bsscfg->associated &&
			(CHSPEC_CHANNEL(chanspec) == infrachan)) ||
...
int
wlc_bmac_srvsdb_force_set(wlc_hw_info_t *wlc_hw, uint8 force)
{
		...
		for (delay_val = 0; delay_val < BTCX_FLUSH_WAIT_MAX_MS * 10; delay_val++) {
			if (BCMSECICOEX_ENAB_BMAC(wlc_hw)) {
				/* In GCI, both task type and buffer count are available
				   in the same word
				*/
				W_REG(wlc_hw->osh, D11_BTCX_ECI_ADDR(wlc_hw), (uint16)1);
				eci_m = R_REG(wlc_hw->osh, D11_BTCX_ECI_DATA(wlc_hw));
				a2dp_buffer = ((eci_m >> 8) & 0xf);
				bt_task = eci_m  & 0x3f;
			} else {
				W_REG(wlc_hw->osh, D11_BTCX_ECI_ADDR(wlc_hw), (uint16)0);
				eci_m = R_REG(wlc_hw->osh, D11_BTCX_ECI_DATA(wlc_hw));
...
pri_wlan:
		...
		OR_REG(wlc_hw->osh, D11_BTCX_TRANSCTL(wlc_hw), (uint16)BTCX_TRANS_TXCONF);
		delay_val = 0;
...
wlc_bmac_exclusive_reg_access_core0(wlc_hw_info_t *wlc_hw, bool set)
{
	...
	ASSERT(phy_get_phymode(wlc_hw->band->pi) == PHYMODE_MIMO);
	if (set)
...
wlc_bmac_enable_tx_hostmem_access(wlc_hw_info_t *wlc_hw, bool enabled)
{
	if (BCMPCIEDEV_ENAB()) {
		...
		WL_MAC(("wlc_bmac_enable_tx_hostmem_access \n"));
		if (!enabled)  {
...
wlc_bmac_rsdb_cap(wlc_hw_info_t *wlc_hw)
{
	...
	return (wlc_hw->num_mac_chains > 1) ? TRUE : FALSE;
}
...
wlc_bmac_core_reset(wlc_hw_info_t *wlc_hw, uint32 flags, uint32 resetflags)
{
	...
	if (si_setcore(wlc_hw->sih, SYSMEM_CORE_ID, 0)) {
		...
		if (BUSTYPE(wlc_hw->sih->bustype) == SI_BUS) {
			uint16 d11mac_sysm_startaddr_h = 0;
			uint16 d11mac_sysm_startaddr_l = 0;
...
wlc_bmac_process_split_fifo_pkt(wlc_hw_info_t *wlc_hw, void* p, uint8 rxh_offset)
{
	...
	/* length & conv status */
	fifo0len = ltoh16(D11RXHDR_GE129_ACCESS_VAL(rxh, RxFrameSize_0));
	convstatus = ltoh16(D11RXHDR_GE129_ACCESS_VAL(rxh, HdrConvSt));
	hdr_converted = ((convstatus & HDRCONV_ENAB) ? TRUE : FALSE);

	if (hdr_converted) {
		/* Successfull Header conversion */
		/* Check for max host frag length */
		if (fifo0len > PKTFRAGLEN(wlc_hw->osh, p, LB_FRAG1)) {
			WL_MAC(("%s: Invalid host length : Pkt %p  fifo0len 0x%x "
		 ...
		 */
		if ((wlc_hw->hdrconv_mode) && RXS_GE128_VALID_UCODE_STS(rxh, wlc->pub->corerev)) {
			uint16 rxstatus1 = ltoh16(D11RXHDR_GE129_ACCESS_VAL(rxh, RxStatus1));
			uint16 rxstatus2 = ltoh16(D11RXHDR_GE129_ACCESS_VAL(rxh, RxStatus2));
			uint8 dma_flags = (D11RXHDR_GE129_ACCESS_VAL(rxh, dma_flags));
...
#if defined(STS_FIFO_RXEN) || defined(STS_XFER)
		case STS_FIFO :
			return (uint8)(STS_RX_ENAB(hw->wlc->pub) ||
					STS_XFER_PHYRXS_FIFO_ENAB(hw->wlc->pub));
			break;
#endif /* STS_FIFO_RXEN || STS_XFER */
		default :
...
{
	W_REG(wlc_hw->osh, D11_BTCX_ECI_ADDR(wlc_hw), (uint16)reg_num);
...
wlc_bmac_dump_list(wlc_hw_info_t *wlc_hw, struct bcmstrbuf *b)
{
	...
	return phy_dbg_dump_list(wlc_hw->band->pi, b);
}
...
wlc_bmac_dump_dump(wlc_hw_info_t *wlc_hw, struct bcmstrbuf *b)
{
	...
	return phy_dbg_dump_dump(wlc_hw->band->pi, b);
}
...
wlc_bmac_enable_mac_clkgating(wlc_info_t *wlc)
{
	...
	if (D11REV_IS(corerev, 61)) { /* 4347/6878 */
		OR_REG(osh, D11_PSMPowerReqStatus(wlc_hw), (uint16)(1 << AUTO_MEM_STBY_RET_SHIFT));
		...
		OR_REG(osh, D11_BMCCTL(wlc_hw), (uint16)(1 << BMCCTL_CLKGATEEN_SHIFT));
		 ...
		 */
		W_REG(osh, D11_CLK_GATE_REQ0(wlc_hw), (uint16)(
		...
		W_REG(osh, D11_CLK_GATE_REQ1(wlc_hw), (uint16)(
		...
		W_REG(osh, D11_CLK_GATE_REQ2(wlc_hw), (uint16)(
		...
		W_REG(osh, D11_CLK_GATE_STRETCH0(wlc_hw), (uint16)(
		...
		W_REG(osh, D11_CLK_GATE_DIV_CTRL(wlc_hw), (uint16)val);

		W_REG(osh, D11_CLK_GATE_PHY_CLK_CTRL(wlc_hw), (uint16)(
		...
		W_REG(osh, D11_CLK_GATE_EXT_REQ0(wlc_hw), (uint16)(
		...
		W_REG(osh, D11_CLK_GATE_EXT_REQ1(wlc_hw), (uint16)(
		...
		W_REG(osh, D11_CLK_GATE_STS(wlc_hw), (uint16)val);
	}
	else {
		ASSERT(0);
	}
...
#if !defined(BCMHME)
#error "PKTPGR need BCMHME support"
#endif // endif

/**
 * SW may optionally BME transfer the additional data_buffer to a host memory
 * segment before a PAGEOUT request. As host may use a 64bit host addressing
 * the 32bit Lbuf.CONTROL.data does not suffice. SW will set the Lbuf.CONTROL.length
 * to 0 to indicate to HW that a TxDMA descriptor need not be programmed using the
 * Lbuf.CONTROL.data. The value of Lbuf.CONTROL.data may be non-NULL even when
 * Lbuf.CONTROL.length is 0. HW continues to use the relocation rule: if Lbuf.CONTROL.data
 * is in the default data_buffer range, then relocate else do not relocate on a PAGEOUT.
 */
static int BCMFASTPATH
wlc_bmac_hwa_xfer_data_to_host(osl_t *osh, void *p, bool full_copy)
{
	uint headroom;
	uint32 pktlen;
	int ret;
	...
	ASSERT(PKTNEXT(osh, p) == NULL);

	// Both LBF_MGMT_TX_PKT and LBF_BUF_ALLOC need to do HME copy.
	// LBF_MGMT_TX_PKT request HME copy when data size is bigger than HWA_PP_PKTDATABUF_BYTES.
	// LBF_BUF_ALLOC always request HME copy.
	// If there is data in host, only copy the first HWA_PP_PKTDATABUF_TXFRAG_MAX_BYTES bytes
	if (full_copy) {
		// prhex(" MGMTTXPKT RAW", (uint8 *)PKTDATA(osh, p), PKTLEN(osh, p));

		// Do BME copy if valid data range is out of LBUF:data_buffer 128B range.
		// Or data is not at same segment as LBUF [PKTISBUFALLOC].
		// XXX: CRBCAHWA-660 For 6715A0, We also need to do BME copy
		// if data is located to Lfrag boundary(LBUFMGMTSZ + headroom + pktlen = 256)
		headroom = PKTHEADROOM(osh, p);
		pktlen = PKTLEN(osh, p);
		if (PKTISBUFALLOC(osh, p) ||
			headroom + pktlen >= HWA_PP_PKTDATABUF_TXFRAG_MAX_BYTES) {
			// LBF_MGMT_TX_PKT: both LBUF header and data buffer are from PKTGET(heap).
			// LBF_BUF_ALLOC: PKT fetch case.

			// Use hndbme wrapper routines
			ret = hme_d2h_xfer((const void *)PKTDATA(osh, p), &hme64, pktlen,
				TRUE, HME_USER_LCLPKT);
			if (ret < 0)
				return ret;

			// Save headroom.
			PKTSAVEHEADROOM(osh, p, headroom);

			// Defer PKT pointers adjustment because wlc_txprep_pkt_get_hdrs in below
			// will access it.

			// Save HME LCLPKT info, and mark it as HMEDATA.
			// NOTE: MGMTPKT's host_pktid[0] is 0 so we are safe to use fraginfo fields
			// to store the HME LCLPKT info.
			PKTSETHMELEN(osh, p, pktlen);
			PKTSETHME_HI(osh, p, HADDR64_HI(hme64));
			PKTSETHME_LO(osh, p, HADDR64_LO(hme64));
			PKTSETHMEDATA(osh, p);
		}
	} else {
		HADDR64_HI_SET(hme64, PKTHME_HI(osh, p));
		HADDR64_LO_SET(hme64, PKTHME_LO(osh, p));

		// Update the first HWA_PP_PKTDATABUF_TXFRAG_MAX_BYTES bytes of the data
		ret = hme_d2h_xfer((const void *)PKTDATA(osh, p), &hme64,
			HWA_PP_PKTDATABUF_TXFRAG_MAX_BYTES, TRUE, HME_USER_NOOP);
		if (ret < 0)
			return ret;
	}
...
*/
static int BCMFASTPATH
wlc_bmac_hwa_convert_to_3bswpkt(wlc_info_t *wlc, uint fifo, void *p, uint32 *pkt_count)
{
	d11txhdr_t *txh;
	struct dot11_header *d11_hdr;
	void *head, *prev;
	osl_t *osh;
	uint8 *txd, ac, num_desc;
	uint32 txdma_flags;
	uint32 count, frag_num;
	...
	WL_TRACE(("==> wl%d: %s: convert 3a -> 3b swpkt on fifo %d\n",
		wlc->pub->unit, __FUNCTION__, fifo));

	/* We only need to care about txd updated for re-transmission */
	if (PKTISHWA3BPKT(osh, p)) {

		// XXX, FIXME. Need to back here to review again.
		HWA_ERROR(("******** PKTISHWA3BPKT: Re-transmission p<%p>\n", p));
		if (PKTISMGMTTXPKT(osh, p)) {
			if (wlc_bmac_hwa_xfer_data_to_host(osh, p, TRUE))
				return BCME_NORESOURCE;
		} else if (PKTHASHMEDATA(osh, p)) {
			if (wlc_bmac_hwa_xfer_data_to_host(osh, p, FALSE))
				return BCME_NORESOURCE;
		}
		...
		for (; p; p = PKTNEXT(osh, p)) {
			...
			if (p == head) {
				/* get num_desc */
				num_desc = HWAPKTNDESC(p);
			}
		}
		goto upd_aqm;
	}

	if (PKTDATAISLOCAL(osh, p)) {
		if (wlc_bmac_hwa_xfer_data_to_host(osh, p, TRUE))
			return BCME_NORESOURCE;
	}

	// Set terminater
	// XXX, HW use HostDataLen[0] as identifier to process HostData0,
	// and then use HostPktId[1..3] as identifier to process HostData1..3.
	// Tx fetched packet doesn't have frag_num but we need to keep the
	// HostPktId[0].
	frag_num = PKTFRAGTOTNUM(osh, p);
	if (frag_num < HWA_PP_PKT_FRAGINFO_MAX) {
		if (frag_num == 0) {
			// Keep HostPktId[0] info
			ASSERT(frag_num == LB_FRAG1);
			PKTSETFRAGLEN(osh, p, LB_FRAG1, 0);
		}
		else {
			PKTSETHOSTPKTID(p, frag_num, 0);
		}
	}

	/* AMSDU use PKTNEXT to link MSDUs when aggsf > 4 */
	for (; p; p = PKTNEXT(osh, p)) {
		count++;
		num_desc += PKTLEN(osh, p) ? 1 : 0;
		...
		if (BCMLFRAG_ENAB() && PKTISTXFRAG(osh, p)) {
			frag_num = PKTFRAGTOTNUM(osh, p);
			if (frag_num == 1) {
				if (HWAPKTHLEN(p, 0) != PKTFRAGTOTLEN(osh, p)) {
					WL_PRINT(("WARNING: count %d hwapkthlen %d "
						"pktfragtotlen %d\n", count, HWAPKTHLEN(p, 0),
						PKTFRAGTOTLEN(osh, p)));
					HWA_PKT_DUMP_EXPR(_hwa_txpost_dump_pkt(p, NULL,
						"Len-Mismatch", count, TRUE, TRUE));
					ASSERT(0);
				}
			}

			num_desc += PKTFRAGTOTNUM(osh, p);
		} else
#endif /* BCMLFRAG */
		{
			HWAPKTSETHLEN(p, 0, 0);
		 ...
		 */
		HWAPKTSETNDESC(p, 0);
		HWAPKTSETAMSDUTLEN(p, 0);
		HWAPKTSETTXDMAFLAGS(p, 0);
...
upd_aqm:
	...
	* in dongle  packetpager mode we should set
	* not coherent and is pcie.
	*/

	// Assign to first pkt
	// NOTE: pkttotlen count PKTFRAGTOTLEN which has sum of all host length in host_datalen
	// in wlc_amsdu_single_txlfrag_fixup already
	HWAPKTSETNDESC(head, num_desc);
	HWAPKTSETAMSDUTLEN(head, pkttotlen(osh, head) - (uint)((uint8*)d11_hdr - txd));
	HWAPKTSETTXDMAFLAGS(head, txdma_flags);

	*pkt_count = count;

	// PKT pointers adjustment here after wlc_txprep_pkt_get_hdrs.
	if (PKTHASHMEDATA(osh, head)) {
		/* All data is at HME, free local allocated buffer */
		if (PKTISBUFALLOC(osh, head)) {
			PKTBUFFREE(osh, head);
		}

		// Set HME low address to data
		//PKTSETDATA(osh, head, PKTHME_LO(osh, head));

		// Adjust data, head, and end
		PKTADJDATA(osh, head, (uchar *)PKTHME_LO(osh, head),
			PKTHMELEN(osh, head), 0);
...
wlc_bmac_hwa_convert_to_3bswpkt(wlc_info_t *wlc, uint fifo, void *p, uint32 *pkt_count)
{
	...
	if (PKTISHWA3BPKT(osh, p)) {
		for (; p; p = PKTNEXT(osh, p)) {
			/* get pkt_count */
			count++;
			txpkt = (void *)LFRAG2HWAPKT(p);
			prev_txpkt = txpkt;
			if (p == head) {
				/* get num_desc */
				num_desc = HWAPKTNDESC(txpkt);
			}
		}
		goto upd_aqm;
	}
	...
	for (; p; p = PKTNEXT(osh, p)) {
		count++;
		txpkt = (void *)LFRAG2HWAPKT(p);
		HWAPKTSETDATA(txpkt, PKTDATA(osh, p));
		HWAPKTSETLEN(txpkt, PKTLEN(osh, p));
		num_desc += HWAPKTLEN(txpkt) ? 1 : 0;
...
#ifdef BCMLFRAG
		if (BCMLFRAG_ENAB() && PKTISTXFRAG(osh, p)) {
			HWAPKTSETHLEN(txpkt, PKTFRAGTOTLEN(osh, p));
			if (HWAPKTHLEN(txpkt)) {
				num_desc++;
				PHYSADDR64LOSET(HWAPKTHDATA(txpkt),
					(uint32)PKTFRAGDATA_LO(osh, p, LB_FRAG1));
				PHYSADDR64HISET(HWAPKTHDATA(txpkt),
					(uint32)PKTFRAGDATA_HI(osh, p, LB_FRAG1));
			}
		} else
#endif // endif
		{
			HWAPKTSETHLEN(txpkt, 0);
		}

		/* Reset num_desc, total len and txdma_flags, we will fix it up later at first pkt.
		 * Only first pkt need to carry these info.
		 */
		HWAPKTSETNDESC(txpkt, 0);
		HWAPKTSETAMSDUTLEN(txpkt, 0);
		HWAPKTSETTXDMAFLAGS(txpkt, 0);

		//Set up HWA SWPKT link list, it can be different with 3a output.
		if (prev_txpkt) {
			HWAPKTSETNEXT(prev_txpkt, LFRAG2HWAPKT(p));
		}
		prev_txpkt = txpkt;
...
upd_aqm:
	...
	//Assign to first pkt
	HWAPKTSETNDESC(txpkt, num_desc);
	HWAPKTSETAMSDUTLEN(txpkt, pkttotlen(osh, head) - (uint)((uint8*)d11_hdr - txd));
	HWAPKTSETTXDMAFLAGS(txpkt, txdma_flags);
...
#ifdef BCM_BUZZZ
	/* QUE events using logical fifo index as in PKT events for AMPDU/AMSDU */
	BUZZZ_KPI_QUE1(KPI_QUE_MAC_WR_UPD, 2, hwa_txpkt->mpdu_count, fifo);
#endif /* BCM_BUZZZ */
...
chain_hwa_txpkt:
		if (hwa_txpkt->head) {
			...
			if (hwa_txpkt->fifo != fifo) {
				WL_MAC(("wl%d: Commit current fifo-%d, new fifo-%d coming.\n",
			...
			err = wlc_bmac_hwa_convert_to_3bswpkt(wlc, fifo, p, &pkt_count);
			if (err < 0) {
				WL_ERROR(("wl%d: ERROR @ %s(%d): err<%d>!!!\n", wlc->pub->unit,
					__FUNCTION__, __LINE__, err));
				return BCME_BUSY;
			}

			// SW temporary HWA Tx packet list for HWA3B commit.
#if defined(HWA_PKTPGR_BUILD)
			HWAPKTSETLINK(hwa_txpkt->tail, p);
			// A MPDU with PKTNEXT means > 4-in-1 AMSDU.
			hwa_txpkt->tail = p;
#else
			HWAPKTSETNEXT(hwa_txpkt->tail, LFRAG2HWAPKT(p));
			hwa_txpkt->tail = (void *)LFRAG2HWAPKT(pktlast(wlc->osh, p));
#endif // endif
			...
			hwa_txpkt->tail = p;
#else
			hwa_txpkt->head = (void *)LFRAG2HWAPKT(p);
			hwa_txpkt->tail = (void *)LFRAG2HWAPKT(pktlast(wlc->osh, p));
#endif // endif
			hwa_txpkt->pkt_count = pkt_count;
		...
		if (!PKTISMGMTTXPKT(wlc->osh, p)) {
			WL_ERROR(("wl%d: ERROR @ %s(%d): Why flags 0x%x FIXME!!!\n",
				wlc->pub->unit, __FUNCTION__, __LINE__, PKTFLAGS(p)));
			ASSERT(0);
		}
	...
	if (BCM_DMA_CT_ENAB(wlc)) {
		...
		if (!(*wlc_hw->txavail_aqm[fifo])) {
			WL_MAC(("wl%d %s: NO AQM descriptors available, fifo=%d \n",
				wlc->pub->unit, __FUNCTION__, fifo));
			return BCME_BUSY;
		}
		...
		if (err < 0) {
			WL_ERROR(("wl%d %s: dma_txfast failed, fifo=%d \n", wlc->pub->unit,
				__FUNCTION__, fifo));
			return BCME_BUSY;
		}
...
void
wlc_bmac_classify_fifo_suspend(wlc_hw_info_t *wlc_hw)
{
...
wlc_bmac_classify_fifo_resume(wlc_hw_info_t *wlc_hw, bool rollback)
{
	...
	if (!wlc_hw->classify_fifo_suspend_cnt && PKT_CLASSIFY_EN(RX_FIFO2) &&
		...
		wlc_worklet_info_t worklet;
		wlc_bmac_recv(wlc_hw, RX_FIFO2, FALSE, &worklet);
	}
}
		...
		txd_range_t range, uint16 ncons, bool nonAMPDU)
{
	...
	for (npkt = 0; npkt < ncons; npkt++)
	{
		 ...
		 * the errror handling
		 */
		WL_MAC(("wl%d: %s: AQM-fifo %d reclaim err=%u, AMPDU=%u nc=%u np=%u\n",
...
#if defined(BCMDBG)
		{
			...
			for (i = 0; i < WLC_HW_NFIFO_INUSE(wlc); i++) {
				...
				if (npkts == 0) continue;
				WL_ERROR(("wl%d: %s: FIFO-%d TXPEND = %d TX-DMA%d =>\n",
					wlc->pub->unit, __FUNCTION__, i, npkts, i));
...
wlc_bmac_dma_getnexttxp(wlc_info_t *wlc, uint fifo, txd_range_t range)
{
	...
	if (txp == NULL) {
		if (range != HNDDMA_RANGE_ALL) {
			WL_ERROR(("wl%d: %s Could not reclaim data dd(hwa) fifo=%d\n",
				wlc->pub->unit, __FUNCTION__, fifo));
		}
		return NULL;
	}
...
wlc_bmac_dma_getnexttxp(wlc_info_t *wlc, uint fifo, txd_range_t range)
{
	...
	if (BCM_DMA_CT_ENAB(wlc)) {
		...
		if (dma_getnexttxdd(WLC_HW_AQM_DI(wlc, fifo), range, &txdma_flags) != BCME_OK) {
			WL_INFORM(("wl%d: %s: could not reclaim AQM-fifo %d descriptor\n",
				wlc->pub->unit, __FUNCTION__, fifo));
#if defined(BCMDBG)
			{
			int npkts;
		...
		if (txp == NULL) {
			WL_ERROR(("wl%d: %s Could not reclaim data dd fifo=%d\n",
				wlc->pub->unit, __FUNCTION__, fifo));
			return NULL;
		}
	...
	if (txp == NULL) {
		WL_INFORM(("wl%d: %s Could not reclaim data dd fifo=%d\n",
			wlc->pub->unit, __FUNCTION__, fifo));
		return NULL;
	}
	return txp;
...
{
	if (si_iscoreup(wlc_hw->sih)) {
		// XXX: Determine exact shm
		wlc_bmac_write_shm(wlc_hw, M_GPIO_TX_INHIBIT_TOUT(wlc), wlc_hw->tx_inhibit_tout);
	}
}
...
wlc_bmac_notify_gpio_tx_inhibit(wlc_info_t *wlc)
{
	...
	* Either one of the bits being set indicates an edge transition on the gpio.
	...
	int txinhibit_int = wlc_bmac_read_shm(wlc->hw, M_MACINTSTATUS_EXT(wlc)) &
...
{
#ifdef BCM_DMA_CT
	uint16 en = enabled ? 0x1 : 0x0;

	if (D11REV_GE(wlc_hw->corerev, 65) && wlc_hw->clk) {
		W_REG(wlc_hw->osh, D11_CTMode(wlc_hw), en);
	}
#endif // endif
}
...
wlc_rx_invalid_length_handle(wlc_info_t* wlc, void* p, d11rxhdr_t *rxh)
{
	...
	haddr64 = PKTFRAGDATA_HI(wlc->osh, p, LB_FRAG1);
	haddr64 = (haddr64 << (NBITS(uint32))) | (PKTFRAGDATA_LO(wlc->osh, p, LB_FRAG1));
...
/**
 * Get the current snapshot of the fifo's that are ready
 *
 * Utility funtion that can be invoked in debug scenarios, Not during a real datapath
 *
 * @param wlc_hw	wlc hw info structure
 * @param fifordy	fifo ready bitmap, 1 indicates that fifo is ready.
 * @param cnt		return value for the frame count per fifo
 * @return		frame count array
 */
void
wlc_bmac_hfifo_rdy_get(wlc_hw_info_t *wlc_hw, uint16 fifordy[])
{
	osl_t *osh = wlc_hw->osh;
	uint corerev = wlc_hw->corerev;

	if (D11REV_GE(corerev, 128)) {
		fifordy[0] = R_REG(osh, D11_AQMF_READY0(wlc_hw));
		fifordy[1] = (R_REG(osh, D11_AQMF_READY1(wlc_hw)) << 16);
		fifordy[2] = R_REG(osh, D11_AQMF_READY2(wlc_hw));
		fifordy[3] = (R_REG(osh, D11_AQMF_READY3(wlc_hw)) << 16);
		fifordy[4] = R_REG(osh, D11_AQMF_READY4(wlc_hw));
	} else {
		if (D11REV_GE(corerev, 65)) {
			fifordy[0] = R_REG(osh, D11_AQMFifoRdy_L(wlc_hw));
			fifordy[1] = (R_REG(osh, D11_AQMFifoRdy_H(wlc_hw)) << 16);
		} else if (D11REV_GE(corerev, 40)) {
			fifordy[0] = R_REG(osh, D11_AQMFifoReady(wlc_hw));
		} else {
			fifordy[0] = R_REG(osh, D11_xmtfifordy(wlc_hw));
		}
	}
}

/**
 * Get the frame currently in all the aqm fifo's
 *
 * Utility funtion that can be invoked in debug scenarios, Not during a real datapath
 *
 * @param wlc_hw	wlc hw info structure
 * @param frmcnt	return value for the frame count per fifo
 * @return		frame count array
 */
void
wlc_bmac_hfifo_frmcnt_get(wlc_hw_info_t *wlc_hw, uint16 frmcnt[])
{
	osl_t *osh = wlc_hw->osh;
	uint16 cur_qid, i;

	if (D11REV_LT(wlc_hw->corerev, 128)) {
		return;
	}

	wlc_bmac_suspend_mac_and_wait(wlc_hw);
	cur_qid = R_REG(osh, D11_AQM_QMAP(wlc_hw));
	for (i = 0; i < WLC_HW_NFIFO_TOTAL(wlc_hw->wlc); i++) {
		W_REG(osh, D11_AQM_QMAP(wlc_hw), i);
		frmcnt[i] = R_REG(osh, D11_AQMF_STATUS(wlc_hw)) & 0x7f;
	}
	W_REG(osh, D11_AQM_QMAP(wlc_hw), cur_qid);
	wlc_bmac_enable_mac(wlc_hw);
}

/**
 * Update IFS Registers according to channel BW
 *
 * @param wlc_hw	wlc hw info structure
 */
void
wlc_bmac_upd_ifs(wlc_hw_info_t *wlc_hw)
{
	osl_t *osh = wlc_hw->osh;
	uint16 he_dly = 0x3e;
	uint16 nonhe_dly = 0x16;

	if (D11REV_LT(wlc_hw->corerev, 129)) {
		return;
	}

	wlc_bmac_suspend_mac_and_wait(wlc_hw);
	if (!ACREV_IS(wlc_hw->band->phyrev, 129)) {
		if (CHSPEC_IS20(wlc_hw->chanspec)) {
			he_dly = 0x1;
			nonhe_dly = 0x29;
		} else if (CHSPEC_IS40(wlc_hw->chanspec)) {
			he_dly = 0xa;
			nonhe_dly = 0x31;
		} else if (CHSPEC_IS80(wlc_hw->chanspec)) {
			he_dly = 0xd;
			nonhe_dly = 0x34;
		} else if (CHSPEC_IS160(wlc_hw->chanspec)) {
			he_dly = 0x10;
			nonhe_dly = 0x37;
		}
	} else {
	...
	W_REG(osh, D11_IFS_EDCAPRI(wlc_hw), (uint16)0x14e4);
	W_REG(osh, D11_IFS_SIFS_RX_TX_TX(wlc_hw),
		(uint16)(he_dly | (uint16)((he_dly & 0xff) << 8)));
	W_REG(osh, D11_IFS_SIFS_NAV_TX(wlc_hw), (uint16)(he_dly | (2 << 8)));
	W_REG(osh, D11_IFS_EDCAPRI(wlc_hw), (uint16)0x4e4);
	W_REG(osh, D11_IFS_SIFS_RX_TX_TX(wlc_hw),
		(uint16)(nonhe_dly | ((nonhe_dly & 0xff) << 8)));
	W_REG(osh, D11_IFS_SIFS_NAV_TX(wlc_hw), (uint16)(nonhe_dly | (2 << 8)));
	wlc_bmac_enable_mac(wlc_hw);
}
