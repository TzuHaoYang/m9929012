 ...
 *
 * $Id: acsd_svr.h 792570 2020-10-29 21:03:03Z $
 */
...
#define WL_CHSPEC_DEF_5G_L	0xD028		/* Default 5GL chanspec from Channel 40 */
#define WL_CHSPEC_DEF_5G_H	0xD099		/* Default 5GH chanspec from Channel 153 */
#define WL_RADIO_CHAN_5GL	0x0018
#define WL_RADIO_CHAN_5GH	0x0007
#define BAND_5G(band) (band == WLC_BAND_5G)
#define BAND_6G(band) (band == WLC_BAND_6G)
...
#define ACS_CS_SCAN_DWELL_ACTIVE_6G 110 /* ms */
#define ACS_CS_SCAN_DWELL_PASSIVE_6G 110 /* ms */
...
#define ACS_CI_SCAN_EXPIRE	3600  /* sec: how long to expire an scan entry */
...
#ifndef ACS_NUMCHANNELS
#ifdef WL_NUMCHANNELS
#define ACS_NUMCHANNELS	WL_NUMCHANNELS
#else
#define ACS_NUMCHANNELS		64
#endif /* WL_NUMCHANNELS */
#endif /* ACS_NUMCHANNELS */
...
typedef enum {
	...
	ACS_POLICY_DEFAULT6G	= 2,
	ACS_POLICY_CUSTOMIZED1	= 3,
	ACS_POLICY_USER		= 4,
...
#define ACS_MAX_LIST_LEN			128 /* Max Possible
	* Non-Operable (Exclude) Chanspecs in a Radio
	*/
#define ACS_MAX_VECTOR_LEN			(ACS_MAX_LIST_LEN * 7) /* Max Possible
	* Length of a comma separated string having Non-Operable Chanspecs in a Radio
...
#define TXFAIL_EVENT_TIMEOUT_ETSI		720		/* 12 min */
#define TXFAIL_EVENT_TIMEOUT_FCC		180		/* 3 min */
...
#ifdef BAND6G
#define ACS_CHANIM_BUF_LEN			(4*1024)
#else /* BAND6G */
#define ACS_CHANIM_BUF_LEN			(2*1024)
#endif /* BAND6G */
#define ACS_TXPWR_BUF_LEN			(2*1024)
#define ACS_MAX_TXPWR				33
#define ACS_TXPWR_DEFAULT			16
#define WL_CHANSPEC_TXPWR_FIXED_LEN		(sizeof(wl_chanspec_txpwr_max_t) \
...
#define WL_CS_SCAN_TIMEOUT_6G 13 /* Timeout in second for CS scan event from driver */
...
#define ACS_DFS_MOVE_BACK_DEFAULT	1
#define ACS_BUF_LEN			256
#define ACS_CFG_FILE			"/tmp/acsd_cfg"
#define ACS_CFG_KEYWORD_LAST_DFS_CH	"Last DFS Chspec:"
...
typedef struct acs_conf_chspec_s {
	...
	uint16 rep_excl_count;
	chanspec_t rep_clist[ACS_MAX_LIST_LEN];
} acs_conf_chspec_t;
...
typedef struct {
	chanspec_t chanspec;
	uint8 chan_avail;
	uint16 rep_nbr_nctrl;  /* No of Bss in control channel */
	uint16 rep_nbr_next20; /* No of neighbors in ext20 */
	uint16 rep_nbr_next40; /* No of neighbors in ext40 */
	uint16 rep_nbr_next80; /* No of neighbors in ext80 */
	uint8 rep_nbr_util; /* Channel utilization */
	int8 rep_nbr_noise; /* noise */
	uint8 rep_excl_enable; /* flag to enable/disable chanspec based on excl list */
} acs_perchan_stats_t;
...
struct acs_chaninfo {
	...
	uint32 acs_ac_flag;
	bool non_dfs_present;
	uint8 acs_pref_6g_psc_pri;
	bool fallback_legacy_txpwr;
	wl_chanspec_txpwr_max_t *txpwr_params;
	uint8 last_device_mac[6];
	uint8 radio_mac[6];
	acs_perchan_stats_t *perchan_stats;
	uint8 perchan_count;
	uint8 device_count;
	uint8 radiomac_count;
	uint8 acs_dfs_move_back;
	chanspec_t last_dfs_chspec;
};
...
extern int acs_escan_prep_cs(acs_chaninfo_t *c_info, void *params,
	int *params_size, uint16 version);
extern int acs_escan_prep_ci(acs_chaninfo_t *c_info, void *params,
	int *params_size, uint16 version);
...
extern void chanim_upd_acs_record(char *ifname, chanim_info_t *ch_info, chanspec_t selected,
	uint8 trigger);
...
extern int acs_get_scan_ver(acs_chaninfo_t *c_info, wl_scan_version_t *ver);
extern int acs_set_escan_params(acs_chaninfo_t *c_info, void *params,
	int params_size);
...
extern int acs_derive_bw_from_given_chspec(acs_chaninfo_t * c_info, chanspec_t chspec);
#ifdef DEBUG
...
extern int acs_get_rs_info(acs_chaninfo_t * c_info, char* prefix);
extern int acs_change_from_dfs_to_nondfs(acs_chaninfo_t * c_info);
extern int acs_get_txpwr_info(acs_chaninfo_t *c_info, wl_chanspec_txpwr_max_t *param, int param_len,
	char *buf, int buf_len);
extern void acs_invalidate_exclusion_channels(ch_candidate_t *candi, int count,
	acs_conf_chspec_t *excl_chans);
extern void acs_invalidate_rep_exclusion_channels(ch_candidate_t *candi, int count,
	acs_conf_chspec_t *excl_chans);
extern bool acs_found_chspec_in_list(chanspec_t chspec, chanspec_t *list, uint16 list_len);
extern int acs_build_repeater_stats(acs_chaninfo_t *c_info);
extern int bcm_ether_atoe(const char *p, struct ether_addr *ea);
extern int acs_get_cfg_from_file(acs_chaninfo_t *c_info, const char *cfg_keyword, int *cfg_val);
