 ...
 *
 * $Id: wlc_rate_sel.c 793720 2020-12-08 08:32:21Z $
 */
...
/* care sgi */
#define CUR_RATESPEC(state)	(RATESPEC_OF_I(state, state->rt.rateid))
...
static uint8 wlc_ratesel_getfbrateid(rcb_t *state, uint8 rateid, bool is_ul);
...
static void wlc_ratesel_ulrt_upd_rate(rcb_t *state);
#endif // endif
#ifdef WL_LPC
...
wlc_ratesel_dump_rcb(rcb_t *rcb, int32 ac, struct bcmstrbuf *b)
{
	...
	if (state_cmn->rateid < rcb->active_rates_num) {
...
wlc_ratesel_set_fixrate(rcb_t *state, int ac, uint8 val)
{
	...
	if (val != DUMMY_UINT8 && rateid >= state->active_rates_num) {
		...
		if (val != DUMMY_UINT8) {
			state->rt.rateid = rateid;
			if (state->rt.rateid >= state->mcs_baseid)
...
wlc_ratesel_scbdump(rcb_t *state, struct bcmstrbuf *b)
{
	...
	/* summary */
	if (state_cmn->rateid < state->active_rates_num) {
		if (state->fixrate != DUMMY_UINT8) {
			if (state->fixrate & FIXPRI_FLAG)
		...
		bcm_bprintf(b, "rid %d %s 0x%x sgi_sts %d epoch %d ncf %u pncf %u"
			"skips %u nupds %u\n",
			state_cmn->rateid, IS_MCS(CUR_RATESPEC(state)) ? "mcs" : "rate",
	...
	bcm_bprintf(b, "ulrt counters: sp_probing %d sp_status %d rate_update %d rate_store %d\n",
		state->ulrt.nsp_probe, state->ulrt.nsp_status,
		state->ulrt.nrt_update, state->ulrt.nrt_restore);

	if (state->spmode == SP_NORMAL) {
		if (state_cmn->mcs_sp_id >= state->active_rates_num) {
			...
			bcm_bprintf(b, "\tspatial mcs x%02x bw %u sp_nupd %u\n\t",
				WL_RSPEC_RATE_MASK & RATESPEC_OF_I(state, state_cmn->mcs_sp_id),
				RSPEC2BW(RATESPEC_OF_I(state, state_cmn->mcs_sp_id)),
				state_cmn->sp_nupd);
		...
		if (state_cmn->extsp_ixtoggle == I_PROBE)
			...
			ratespec_t spr;
			spr = RATESPEC_OF_I(state, state_cmn->extsp_xpr_id);
	...
	bcm_bprintf(b, "%sulrt: aid %d state 0x%x last RU %d ncfails %d nupd %d\n",
		ULDL(state_cmn), state->aid, state->ulrt.state, state->ulrt.ruidx,
		state_cmn->ncfails, state_cmn->nupd);
	for (i = 0; i < RATESEL_MFBR_NUM; i++) {
		uint32 curd, curf, curp;
		curd = state->ulrt.gdelim[i];
		curf = state->ulrt.gfcs[i];
		curp = (curd == 0) ? 0 : (curf << RATESEL_EMA_NF) / curd;
		bcm_bprintf(b, "\trt%d [0x%02x %d / %d p %4d] n %d\n",
			i, WL_RSPEC_RATE_MASK & state->ulrt.txrspec[i],
			curf, curd, curp, state->ulrt.nupd[i]);
	}
	/* report on vertical rate */
	bcm_bprintf(b, "\tcur [%sx%02xb%d t %d p %d]\n\tfbr [%sx%02xb%d t %d p %d]"
		    "\n\tdn  [%sx%02xb%d t %d p %d]\n\tup  [%sx%02xb%d t %d p %d]\n",
 ...
 */
static uint8
wlc_ratesel_getfbrateid(rcb_t *state, uint8 rateid, bool is_ul)
{

#if defined(D11AC_TXD)
	return state->fbrid[rateid];
#else
	return ((state->rt.gotpkts & GOTPKTS_INIT) ? state->fbrid[rateid] : 0);
#endif /* D11AC_TXD */
}
...
wlc_ratesel_getmcsfbr(rcb_t *state, uint8 ac, uint8 plcp0)
{
	...
	if (mcsid != -1)
		fbrid = wlc_ratesel_getfbrateid(state, (uint8)mcsid, 0);
...
wlc_ratesel_next_rate(rcb_t *state, int8 incdec, uint8 *next_rateid,
	ratespec_t *next_rspec, uint8 rateid)
{
	ratespec_t rspec = RATESPEC_OF_I(state, rateid);

	if (incdec == +1) {
		*next_rateid = state->uprid[rateid];
		if (*next_rateid == rateid) {
				return FALSE;
		}
	} else if (incdec == -1) {
		/* drop the rate */
		*next_rateid = state->dnrid[rateid];
		if (*next_rateid == rateid)
			return FALSE;
...
wlc_ratesel_init_nss2(rcb_t *state, uint16 *mcs_filter, bool bw_auto, uint32 minrt, uint32 maxrt)
{
	...
	if (bw_auto) {
		...
		} else if (state->bwcap >= BW_80MHZ) {
			/* for bw80, add: c0s2/c1s2_b40, c0s2_b80 (27/54/58.5 Mbps) */
			wlc_ratesel_addmcs(state, mcs_filter, minrt, maxrt, 0, nss, BW_40MHZ);
			wlc_ratesel_addmcs(state, mcs_filter, minrt, maxrt, 1, nss, BW_40MHZ);
			wlc_ratesel_addmcs(state, mcs_filter, minrt, maxrt, 0, nss, BW_80MHZ);
		}
	...
	uint32 min_rate, uint start_rate)
{
	...
	*/
	...
	state->rt.rateid = wlc_ratesel_find_startidx(state, start_rate);
	state->rt.gotpkts = 0;
} /* wlc_ratesel_filter */
...
wlc_ratesel_init(ratesel_info_t *rsi, ratesel_init_t *init)
{
	...
	if (state->spmode == SP_NORMAL) {
		...
		nss = wlc_ratespec_nss(RATESPEC_OF_I(state, state->rt.rateid));
	...
	state->ulrt.txrspec[0] = RATESPEC_OF_I(state, state->ulrt.rt.rateid);
	state->ulrt.trssi = ULMU_OFDMA_TRSSI_INIT;
	state->ulrt.ncf_lmt = NCFAIL_LMT0;
	...
	if (state->spmode == SP_NORMAL) {
		...
		nss = wlc_ratespec_nss(RATESPEC_OF_I(state, state->ulrt.rt.rateid));
		/* start from probing into higher stream rate */
		if (nss > 1 && nss < state->mcs_streams)
			state->ulrt.rt.mcs_sp_col = SPATIAL_MCS_COL2;
	}

	/* initialize rest of the ulrt txrspec */
	rateid = state->ulrt.rt.rateid;
	fbrid = wlc_ratesel_getfbrateid(state, rateid, 1);
	rspec = RATESPEC_OF_I(state, rateid);
	...
	for (i = 1; i < RATESEL_MFBR_NUM; i++) {
		state->ulrt.txrspec[i] = HE_RSPEC(0, nss);
	}

	if (mcs >= 4) {
		state->ulrt.txrspec[1] = RATESPEC_OF_I(state, fbrid);
		state->ulrt.txrspec[2] = RATESPEC_OF_I(state, fbrid-1);
		if (state->ulrt.rt.gotpkts)
			state->ulrt.txrspec[3] = RATESPEC_OF_I(state, fbrid-2);
	} else if (mcs >= 2) {
		state->ulrt.txrspec[1] = RATESPEC_OF_I(state, fbrid);
	}
...
wlc_ratesel_load_psri(rcb_t *state, rcb_rtcmn_t *state_cmn)
{
	...
	cur_rspec = RATESPEC_OF_I(state, state_cmn->rateid);
	...
	if (SP_MODE(state) == SP_NORMAL) {
		...
		if (mcs_extsp_xpr > 0)
			mcs_extsp_xpr_sel = wlc_ratesel_mcsbw2id(state,
				(uint8) mcs_extsp_xpr, state->bw[state_cmn->rateid]);
...
wlc_ratesel_sanitycheck_psr(rcb_t *state, rcb_rtcmn_t *state_cmn)
{
	...
	cur_rspec = RATESPEC_OF_I(state, state_cmn->rateid);
	fbrid = wlc_ratesel_getfbrateid(state, state_cmn->rateid, state_cmn->is_ul);
	...
	uint ntx_succ, uint ntx, uint16 txbw)
{
	...
	if (state->rsi->ratesel_sp_algo) { /* throughput-based sp algo */
		...
		cu_rate = RSPEC2RATE500K(RATESPEC_OF_I(state, state_cmn->rateid));
		...
		if (state_cmn->sp_nupd == 0) {
			...
			/* use moving average */
			UPDATE_MOVING_AVG(&state_cmn->mcs_sp_thrt0, thrt0,
				state->rsi->psr_ema_alpha);
			UPDATE_MOVING_AVG(&state_cmn->mcs_sp_thrt1, thrt1,
				state->rsi->psr_ema_alpha);
		}
		RL_SP0(("%ssp_nupd %u : (mx%02x %d) -> (mx%02x %d) thrt_pri/sp 0x%x 0x%x "
			"sp_thrt_pri/sp 0x%x 0x%x\n",
			ULDL(state_cmn), state_cmn->sp_nupd,
			WL_RSPEC_RATE_MASK & RATESPEC_OF_I(state, state_cmn->rateid),
	...
	uint ntx_succ, uint ntx)
{
	...
	ratespec_t cur_rspec = RATESPEC_OF_I(state, state_cmn->rateid);
	uint8 extsp_rateid;
	...
	if (state->rsi->ratesel_sp_algo) { /* throughput-based sp algo */
		...
		if (state_cmn->sp_nupd == 0) {
			...
			UPDATE_MOVING_AVG(&state_cmn->extsp_thrt0, thrt0,
				state->rsi->psr_ema_alpha);
			UPDATE_MOVING_AVG(&state_cmn->extsp_thrt1, thrt1,
				state->rsi->psr_ema_alpha);
		}
...
wlc_ratesel_checksgi(rcb_t *state, bool is_sgi)
{
	...
	if (state->sgi_bwen == 0 || state_dl->rateid < state->mcs_baseid)
		return FALSE;
	...
	if (state->sgi_state == SGI_RESET) {
		...
		if (state_dl->nupd >= state->rsi->ema_nupd_thres &&
			...
			if (state->sgi_bwen & (1 << state->bw[state_dl->rateid])) {
...
#endif // endif
{
	...
	if (!wlc_ratesel_next_rate(state, -1, &down_rateid, &down_rspec, *rateid))
		return FALSE;

	rsi = state->rsi;
	prate_cur = RSPEC2RATE500K(RATESPEC_OF_I(state, *rateid));
...
make_decision:
	...
	if (decision) {
		...
		ratespec_t prv_rspec = RATESPEC_OF_I(state, *rateid);
		*rateid = down_rateid;
		...
		if (rsi->hopping && SP_MODE(state) == SP_NORMAL && state_cmn->sp_nupd > 0) {
			uint sp_stream = wlc_ratespec_nss(
					RATESPEC_OF_I(state, state_cmn->last_mcs_sp_id));
...
wlc_ratesel_goup(rcb_t *state, rcb_rtcmn_t *state_cmn)
{
	...
	if (!wlc_ratesel_next_rate(state, +1, &up_rateid, &up_rspec, *rateid))
		return FALSE;

	cur_rspec = RATESPEC_OF_I(state, *rateid);
...
wlc_ratesel_start_probe(rcb_t *state, rcb_rtcmn_t *state_cmn)
{
	...
	if (SP_MODE(state) == SP_NORMAL) {
		...
		if ((state_cmn->mcs_sp_flgs & SPFLGS_VAL_MASK) &&
			...
			ASSERT(state_cmn->mcs_sp_id < state->active_rates_num);
			if (((state_cmn->nupd + state_cmn->cur_sp_nupd) % state_cmn->mcs_sp_Pmod)
				...
				RL_SP0(("%said %d nupd %d cur_sp_nupd %d "
					...
					state_cmn->mcs_sp_id - state->mcs_baseid,
					state_cmn->rateid - state->mcs_baseid, state_cmn->mcs_sp_K,
		...
		if (((state_cmn->nupd + state_cmn->cur_sp_nupd) %
			...
			if (valid) {
				...
				extsp_rspec = RATESPEC_OF_I(state, state_cmn->extsp_xpr_id);
				if (state_cmn->extsp_ixtoggle == I_PROBE)
...
wlc_ratesel_change_sp(rcb_t *state, rcb_rtcmn_t *state_cmn)
{
	...
	if (state_cmn->sp_nupd < SPATIAL_M || state_cmn->mcs_sp_id == state_cmn->rateid ||
	    state_cmn->mcs_sp_id >= state->active_rates_num)
		return FALSE;
		...
		if (state_cmn->extsp_ixtoggle == I_PROBE) {
			...
			if (state->rsi->ratesel_sp_algo)
				RL_SP1(("%s%s: aborting X-PROBE probe mcs = 0x%x nupd %d "
					...
					ULDL(state_cmn), __FUNCTION__,
					WL_RSPEC_RATE_MASK &
					known_rspec[state->select_rates[state_cmn->extsp_xpr_id]],
					state_cmn->sp_nupd, state_cmn->extsp_thrt1,
					state_cmn->extsp_thrt0));
			else
...
wlc_ratesel_pick_rate(rcb_t *state, bool is_probe, bool is_sgi)
{
	...
	/* sanity check */
	ASSERT(state_dl->rateid < state->active_rates_num);
	...
	if (change_rate) {
		...
		if (DTPC_ACTIVE(state)) {
			...
			wlc_dtpc_rate_change(state->rsi->wlc->dtpc, state->scb,
				CUR_RATESPEC(state), !(prv_rateid > state_dl->rateid),
				state_dl->epoch,
	...
	uint8 tx_mcs, bool sgi, bool fbr)
{
	...
	RL_MORE(("%s fid 0x%x: rid %d %d psr_cur/fbr %d %d [nrtx ncrx nftx acked fbr sgi] ="
		...
		RSPEC2RATE500K(CUR_RATESPEC(state)), state_dl->psri[PSR_CUR_IDX].psr,
...
wlc_ratesel_ulrt_clear_ratestat(rcb_t *state, bool change_epoch)
{
	...
	RL_MORE(("%s%s: clear ulrt stats [epoch %d]\n",
		ULDL(&state->ulrt.rt), __FUNCTION__, state->ulrt.rt.epoch));
...
wlc_ratesel_ulrt_pick_rate(rcb_t *state, bool is_probe)
{
	...
	/* sanity check */
	ASSERT(state_ul->rateid < state->active_rates_num);

	prv_rateid = state_ul->rateid;
	prv_rspec = RATESPEC_OF_I(state, state_ul->rateid);
	...
	if (is_probe) {
		if (SP_MODE(state) == SP_NORMAL)
			...
			if ((change_rate = wlc_ratesel_change_extsp(state, state_ul))) {
				/* check if need to change epoch */
				cur_rspec = RATESPEC_OF_I(state, state_ul->rateid);
				if (!IS_MCS(prv_rspec) || !IS_MCS(cur_rspec))
					change_epoch = TRUE;
			}
		}
		...
		if (WL_RATE_ON() && change_rate) {
			...
			if (prv_rateid != state_ul->rateid) {
				/* the rate has just changed */
				WL_RATE(("%srate_changed: %sx%02xb%d %s %sx%02xb%d (%d %d) after "
					"%2u used and %u skipped txs epoch %d\n", ULDL(state_ul),
					IS_MCS(prv_rspec) ? "m" : "",
					WL_RSPEC_RATE_MASK & prv_rspec,
					RSPEC2BW(prv_rspec),
					...
					RSPEC2RATE500K(RATESPEC_OF_I(state, state_ul->rateid)),
					state_ul->nupd, state_ul->nskip, state_ul->epoch));
			} else {
				/* the rate has just changed due to a rateset change */
				WL_RATE(("%srate_changed(from-oob): %sx%02x DN %sx%02x (%d %d) "
					"after %2u used and %u skipped txs\n", ULDL(state_ul),
					IS_MCS(prv_rspec) ? "m" : "",
					WL_RSPEC_RATE_MASK & prv_rspec,
...
wlc_ratesel_ulrt_restore_txrspec(rcb_t *state)
{
	...
	wlc_ratesel_ulrt_upd_rate(state);
...
wlc_ratesel_ulrt_upd_txs(rcb_t *state, ratesel_tgtxs_t *tgtxs)
{
	...
	fbrd = fbrf = 0;
	for (i = 1; i < RATESEL_MFBR_NUM; i++) {
		fbrd += state->ulrt.gdelim[i];
		fbrf += state->ulrt.gfcs[i];
	}
	...
	if (tgtxs->is_sppkt) {
		...
		cur_mcs = RATESPEC_OF_I(state, state_ul->rateid) & WL_RSPEC_RATE_MASK;
		tx_mcs = tgtxs->txrspec & WL_RSPEC_RATE_MASK;
		if (IS_SP_PKT(state, cur_mcs, tx_mcs)) {
			RL_SP0(("%s%s aid %d: spatial probe rcvd: mx%02x (pri: mx%02x) "
		...
		/* check if in SP period */
		if (state->ulrt.start_sp) {
			/* restore txrspec back after SP period */
	...
	/* case 1 and 3 */
	if ((curf << 1) > curd || (curd > 0 && (fbrf << 1) > fbrd))
		state_ul->ncfails = 0;
	else if (fbrd > 0)
		state_ul->ncfails++;

	alpha = state_ul->gotpkts ? ((state_ul->nupd <= state->rsi->ema_nupd_thres) ?
		state->rsi->psr_ema_alpha0 : state->rsi->psr_ema_alpha) : RATESEL_EMA_ALPHA_INIT;
	...
	if (fbrd > 0) {
		fbrpsr = (fbrf << RATESEL_EMA_NF) / fbrd;
...
static void
wlc_ratesel_ulrt_upd_rate(rcb_t *state)
{
	uint8 rateid, fbrid;
	uint8 mcs, nss, i;
	...
	if (rateid != state_ul->rateid && rateid >= state->active_rates_num) {
		rateid = state->active_rates_num - 1;
		state_ul->rateid = rateid;
	}
	if (rate_probe) {
		/* save previous txrspec before forming SP rspec */
		for (i = 0; i < RATESEL_MFBR_NUM; i++)
	...
	mcs = RSPEC_HE_MCS(state->ulrt.txrspec[0]);
	nss = RSPEC_HE_NSS(state->ulrt.txrspec[0]);
	/* initialize txrspec default rspec */
	for (i = 1; i < RATESEL_MFBR_NUM; i++) {
		state->ulrt.txrspec[i] = HE_RSPEC(0, nss);
	}
	...
	RL_ULRX(("%s%s: txrspec[0..3]=[%x, %x, %x, %x] trssi=%d\n", ULDL(state_ul), __FUNCTION__,
		state->ulrt.txrspec[0] & WL_RSPEC_RATE_MASK,
		state->ulrt.txrspec[1] & WL_RSPEC_RATE_MASK,
		state->ulrt.txrspec[2] & WL_RSPEC_RATE_MASK,
		state->ulrt.txrspec[3] & WL_RSPEC_RATE_MASK,
...
{
	int rtidx = tgtxs->rtidx;
	uint8 reason = TGTXS_REASON(txs->status.s1);
	uint8 trssi;
	bool is_taf = FALSE;
	rcb_rtcmn_t *state_ul = &state->ulrt.rt;
	int k, txs_res;
	...
	state->ulrt.gdelim[rtidx] = LIMADD(state->ulrt.gdelim[rtidx], UINT32_MAX, tgtxs->gdelim);
	state->ulrt.gfcs[rtidx] = LIMADD(state->ulrt.gfcs[rtidx], UINT32_MAX, tgtxs->gfcs);
	...
	if (is_taf) {
		...
		trssi = D11TRIGUI_TRSSI(TGTXS_USRINFOP3(txs->status.ack_map2));
	...
	LIMINC_UINT32(state->ulrt.nupd[rtidx]);
	LIMINC_UINT32(state_ul->nupd);

	RL_ULRX(("%s%s: aid %d rtidx %d txcnt %d rspec 0x%06x gdelim %d gfcs %d "
		"nupd_cur[%1d]/tot %d %d trssi %d reason %d\n",
		ULDL(state_ul), __FUNCTION__, state->aid, rtidx, tgtxs->txcnt,
		tgtxs->txrspec, tgtxs->gdelim, tgtxs->gfcs,
		rtidx, state->ulrt.nupd[rtidx], state_ul->nupd,
		state->ulrt.trssi, reason));

	BCM_REFERENCE(reason);
	BCM_REFERENCE(txs_res);
#ifdef WL_ULRT_DRVR
	/* check for ul fixed rate */
	...
	if (TXS_DISC != txs_res) {
		...
		if (wlc_ulmu_is_drv_spctl_en(state->rsi->wlc->ulmu) &&
		    !rate_changed &&
		    wlc_ratesel_start_probe(state, state_ul)) {
			LIMINC_UINT32(state->ulrt.nsp_probe);
			do_probe = TRUE;
		    }
		...
		if (rate_changed)
			RL_CHG(("%s%s: aid %d rate_changed %s[mx%02x -> mx%02x : %d -> %d]\n",
				...
				WL_RSPEC_RATE_MASK & RATESPEC_OF_I(state, prv_rateid),
				WL_RSPEC_RATE_MASK & RATESPEC_OF_I(state, state_ul->rateid),
				RSPEC2RATE500K(RATESPEC_OF_I(state, prv_rateid)),
				RSPEC2RATE500K(RATESPEC_OF_I(state, state_ul->rateid))));
		BCM_REFERENCE(prv_rateid);
	}
done:
...
void
wlc_ratesel_restore_txrspec(rcb_t *state)
{
	if (state->ulrt.start_sp)
		wlc_ratesel_ulrt_restore_txrspec(state);
}
	...
	uint16 *flags)
{
	...
	*flags = 0;
	...
	if (rateid != state_dl->rateid && rateid >= state->active_rates_num) {
		rateid = state->active_rates_num - 1;
		state_dl->rateid = rateid;
	}
	...
	fbrateid = wlc_ratesel_getfbrateid(state, state_dl->rateid, 0);
	...
	ratesel_txs_t *rs_txs, uint8 flags)
{
	...
	if (txs_mutype == TX_STATUS_MUTP_VHTMU || txs_mutype == TX_STATUS_MUTP_HEMM) {
		/* mumimo */
		grp_bmp = TX_STATUS64_MU_GBMP(txs->status.s4);
		num_users = bcm_bitcount(&grp_bmp, sizeof(grp_bmp));
		muboost = rate100kbps * num_users;
	} else {
		num_users = TX_STATUS128_HEOM_NUSRS(txs->status.s3);
		muboost = rate100kbps;
	}
