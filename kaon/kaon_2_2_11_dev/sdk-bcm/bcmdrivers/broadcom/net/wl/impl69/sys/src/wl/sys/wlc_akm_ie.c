 ...
 *
 * $Id: wlc_akm_ie.c 793459 2020-11-27 08:21:54Z $
 */
...
static const bcm_iovar_t akm_iovars[] = {
	{"wpa_cap", IOV_WPA_CAP, IOVF_OPEN_ALLOW, 0, IOVT_UINT16, 0},
	{"wpaie", IOV_WPAIE, IOVF_OPEN_ALLOW, 0, IOVT_BUFFER, 0},
	{"rsnxe", IOV_RSNXE, (IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, 0},
#ifdef TESTBED_AP_11AX
	{"rsnie", IOV_RSNIE, (IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, 0},
#endif /* TESTBED_AP_11AX */
	{"ext_sup_rates", IOV_EXT_SUP_RATES, (IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
typedef struct {
	...
	uint32 ptksa_cntr;
	uint8 host_rsnxe_len;		/* Host RSNXE IE length */
	uint8 *host_rsnxe;		/* Host RSNXE IE TLV buffer */
	uint8 ext_sup_rates_len;	/* Host ext sup rates length */
	uint8 *ext_sup_rates;		/* Host ext sup rates TLV buffer */
#ifdef TESTBED_AP_11AX
	uint8 rsnie_len;		/* use provided RSNIE length */
	uint8 *rsnie;			/* user provided RSNIE TLV buffer */
#endif /* TESTBED_AP_11AX */
} bss_akm_info_t;
#define WLC_PTKSA_CNTR_HOST_OVERRIDE 0x80
#define WLC_PTKSA_CNTR_MASK 0x03
...
static int wlc_write_oui_and_type(int *buflen, uint *wpa_len, wpa_suite_t* list,
	const char* src_oui, uint8 type, uint16 *count);
#ifdef AP
...
/**
 * Returns the multicast cipher defined as the weakest enabled encryption in the order: WEP, TKIP,
 * AES.
 */
uint8
wlc_wpa_mcast_cipher(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (WSEC_WEP_ENABLED(wsec)) {
		...
		/* If wsec does not specify WEP key size, so use the default key algo */
		wlc_keymgmt_get_wsec_info(wlc->keymgmt, cfg, &wsec_info);
		if (wsec_info.cur_algos & (1 << CRYPTO_ALGO_AES_GCM)) {
			cipher = WPA_CIPHER_AES_GCM;
		}
		else if (wsec_info.cur_algos & (1 << CRYPTO_ALGO_AES_GCM256)) {
...
wlc_wpa_cap(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *cap, int len)
{
	...
	/* RSN capabilities */
	cntr = (bai->ptksa_cntr & WLC_PTKSA_CNTR_HOST_OVERRIDE) ?
		(bai->ptksa_cntr & WLC_PTKSA_CNTR_MASK) : WLC_REPLAY_CNTRS_VALUE;
	cntr = BSS_WME_ENAB(wlc, cfg) ? cntr : 0;
	...
	if (BSSCFG_AP(cfg) &&
	    WPA2_AUTH_IS_FILS(cfg->WPA_auth) &&
	    (bai->wpa2_preauth == TRUE))
		cap[0] |= WPA_CAP_WPA2_PREAUTH;
	cap[1] = 0;
...
static int
wlc_write_oui_and_type(int *buflen, uint *wpa_len, wpa_suite_t* list, const char* src_oui,
	uint8 type, uint16 *count)
{
	ASSERT(buflen && wpa_len && list && src_oui && count);
	if (*buflen < WPA_SUITE_LEN) {
		WL_ERROR(("%s, Error: not enough buffer space\n", __FUNCTION__));
		ASSERT(0);
		return BCME_BUFTOOSHORT;
	}
	memcpy(list[*count].oui, src_oui, DOT11_OUI_LEN);
...
wlc_wpa_set_cap(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *cap, int len)
{
	...
	if (BSSCFG_AP(cfg)) {
		...
		WL_INFORM(("wl%d: wlc_wpa_set_cap: setting the wpa2 preauth %d\n", wlc->pub->unit,
			bai->wpa2_preauth));
		ret_val = 0;
		cntr = (cap[0] & WPA_CAP_REPLAY_CNTR_MASK) >> WPA_CAP_REPLAY_CNTR_SHIFT;

		if (cntr == 0) {
			bai->ptksa_cntr = 0;
		} else if ((cntr > WLC_REPLAY_CNTRS_VALUE) ||
			((cntr != WPA_CAP_4_REPLAY_CNTRS) &&
			(cntr != WPA_CAP_16_REPLAY_CNTRS))) {
			ret_val = BCME_BADARG;
		} else {
			bai->ptksa_cntr = cntr | WLC_PTKSA_CNTR_HOST_OVERRIDE;
		}
	}
	return ret_val;
}
...
wlc_write_wpa_ie_safe(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *buf, int buflen)
{
	...
	if (BSSCFG_IBSS(cfg)) {
		...
		if (WSEC_AES_ENABLED(wsec)) {
			/* length check */
			/* if buffer too small, return untouched buffer */
			if (wlc_write_oui_and_type(&buflen, &WPA_len, ucast->list,
				WPA_OUI, WPA_CIPHER_AES_CCM, &count)) {
				return orig_buf;
			}
		}
		...
		if (WPA_auth & WPA_AUTH_UNSPECIFIED) {
			/* length check */
			/* if buffer too small, return untouched buffer */
			if (wlc_write_oui_and_type(&buflen, &WPA_len, auth->list,
				WPA_OUI, RSN_AKM_UNSPECIFIED, &count)) {
				return orig_buf;
			}
		}
...
wlc_write_rsn_ie_safe(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *buf, int buflen)
{
	...
	if (WSEC_AES_ENABLED(wsec)) {
		...
		wlc_keymgmt_get_wsec_info(wlc->keymgmt, cfg, &wsec_info);
		if (wsec_info.cur_algos & (1 << CRYPTO_ALGO_AES_GCM)) {
			ucast->list[count++].type = WPA_CIPHER_AES_GCM;
		}
		else if (wsec_info.cur_algos & (1 << CRYPTO_ALGO_AES_GCM256)) {
			ucast->list[count++].type = WPA_CIPHER_AES_GCM256;
		}
		else {
	...
	if (WSEC_TKIP_ENABLED(wsec)) {
		...
		/* if buffer too small, return untouched buffer */
		if (wlc_write_oui_and_type(&buflen, &WPA_len, ucast->list,
			WPA2_OUI, WPA_CIPHER_TKIP, &count)) {
			return orig_buf;
		}
	}
	...
	if ((WPA_auth & (WPA2_AUTH_UNSPECIFIED | WPA2_AUTH_1X_SHA256)) ||
		...
		else if (WPA_auth & WPA3_AUTH_SUITEB) {
			akm_type = RSN_AKM_SUITEB;
		}
#ifdef MFP
		...
		/* if buffer too small, return untouched buffer */
		if (wlc_write_oui_and_type(&buflen, &WPA_len, auth->list,
			WPA2_OUI, akm_type, &count)) {
				return orig_buf;
		}
	}
	if (WPA_auth & WPA2_AUTH_FILS_SHA256) {
		if (wlc_write_oui_and_type(&buflen, &WPA_len, auth->list,
			WPA2_OUI, 0, &count)) {
			return orig_buf;
		}
		auth->list[count++].type = akm_type = RSN_AKM_FILS_SHA256;
	}
	if (WPA_auth & WPA2_AUTH_FILS_SHA384) {
		if (wlc_write_oui_and_type(&buflen, &WPA_len, auth->list,
			WPA2_OUI, 0, &count)) {
			return orig_buf;
		}
		auth->list[count++].type = akm_type = RSN_AKM_FILS_SHA384;
	}
...
#ifdef WLFBT
	if (BSSCFG_IS_FBT(cfg) && (WPA_auth & WPA2_AUTH_FT)) {
		/* length check */
		/* if buffer too small, return untouched buffer */
		akm_type = BSSCFG_IS_FBT_1X(cfg) ? RSN_AKM_FBT_1X : RSN_AKM_FBT_PSK;
		if (wlc_write_oui_and_type(&buflen, &WPA_len, auth->list,
			WPA2_OUI, akm_type, &count)) {
			return orig_buf;
		}
	...
	if (WPA_auth & WPA3_AUTH_SAE_PSK) {
		...
		if (!((WPA_auth & WPA2_AUTH_PSK) || (WPA_auth & WPA2_AUTH_FT))) {
	...
	if (WPA_auth & WPA3_AUTH_SAE_FBT) {
		...
		/* If pure SAE-FT, set MFP to required */
		if (!((WPA_auth & WPA2_AUTH_PSK) || (WPA_auth & WPA2_AUTH_FT))) {
			BSSCFG_SET_AKM_REQUIRES_MFP(cfg);
			BSSCFG_SET_MFP_CAPABLE(cfg);
		} else {
...
#endif /* WL_SAE */
#ifdef WL_DPP
	if (WPA_auth & WPA3_AUTH_DPP) {
		akm_type = RSN_AKM_DPP;
		if (BSSCFG_STA(cfg)) {
			/* if auth algorithm is OPEN SYSTEM, demote the AKM to WPA2 PSK */
			if (cfg->auth == DOT11_OPEN_SYSTEM) {
				akm_type = RSN_AKM_PSK;
		...
		/* if buffer too small, return untouched buffer */
		if (wlc_write_oui_and_type(&buflen, &WPA_len, auth->list,
			WPA3_OUI, akm_type, &count)) {
			return orig_buf;
		}
		/* If pure DPP, set MFP to required */
	...
	if (WPA_auth & WPA_AUTH_OWE) {
		akm_type = RSN_AKM_OWE;
...
wlc_akm_bcn_calc_rsn_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	bai = BSS_AKM_INFO(akmi, cfg);
	ASSERT(bai);

	/* User provided RSN IE */
	if (bai->rsnie_len && bai->rsnie) {
		rsnie_len = bai->rsnie_len;
		return rsnie_len;
	}
#endif /* TESTBED_AP_11AX */
...
wlc_akm_bcn_write_rsn_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	/* User provided RSN IE */
	if (bai->rsnie_len && bai->rsnie) {
		bcopy((uint8*)bai->rsnie, data->buf, bai->rsnie_len);
		return BCME_OK;
	}
#endif /* TESTBED_AP_11AX */
	/* RSN IE */
...
wlc_rsn_build_ie(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct rsn_parms *rsn, void *pmcast)
{
	...
	if (WSEC_AES_ENABLED(cfg->wsec) && UCAST_AES(rsn)) {
		wlc_keymgmt_get_wsec_info(wlc->keymgmt, cfg, &wsec_info);
		if (wsec_info.cur_algos & (1 << CRYPTO_ALGO_AES_GCM)) {
			ucast->list[0].type = WPA_CIPHER_AES_GCM;
		}
		else if (wsec_info.cur_algos & (1 << CRYPTO_ALGO_AES_GCM256)) {
			ucast->list[0].type = WPA_CIPHER_AES_GCM256;
		}
		else {
	...
	if (cfg->WPA_auth & WPA_AUTH_UNSPECIFIED)
		...
		if (((WLC_MFP_ENAB(wlc->pub) && (cfg->WPA_auth & WPA2_AUTH_UNSPECIFIED)) ||
		   (cfg->WPA_auth & WPA2_AUTH_1X_SHA256)) &&
...
#endif /* WLFBT */
		/* special test mode: ignore AP settings */
		if (((WLC_MFP_ENAB(wlc->pub) && (cfg->WPA_auth & WPA2_AUTH_PSK)) ||
		   (cfg->WPA_auth & WPA2_AUTH_PSK_SHA256)) &&
	...
	else if (cfg->WPA_auth & WPA3_AUTH_SUITEB) {
		mgmt->list[0].type = RSN_AKM_SUITEB;
	}
...
static int
wlc_remove_wpa2_pmkid(wlc_info_t *wlc, bcm_tlv_t *wpa2ie)
{
...
wpa_cipher_enabled(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, int cipher)
{
	...
	case WPA_CIPHER_AES_GCM:
		ret = WSEC_AES_ENABLED(wsec);
		if (ret && (WPA_auth == WPA3_AUTH_SUITEB))
			ret = 0;
		break;
...
wpa_mcast_cipher_allowed(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, int cipher)
{
	...
	case WPA_CIPHER_TKIP:
		...
		break;
	case WPA_CIPHER_AES_CCM:
	case WPA_CIPHER_AES_GCM256:     /* AES (GCM256) */
...
wlc_check_wpaie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 *wpaie, uint32 *auth, uint32 *wsec)
{
	...
	if (ie_len < 22) {
		...
		*auth = WPA_AUTH_UNSPECIFIED;
		...
		if (!bcmwpa_akm2WPAauth(&wpaie[offset], &WPA_auth)) {
...
wlc_check_wpa2ie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, bcm_tlv_t *wpa2ie, struct scb *scb)
{
	...
	uint16 count;
	uint16 pmkid_count;
#if defined(MFP) || defined(WLWNM_AP)
	uint16 rsn_cap;
#endif /* MFP || WLWNM_AP */
	...
	if (bcmp(mcast->oui, WPA2_OUI, DOT11_OUI_LEN) ||
		...
		WL_ERROR(("wl%d: WPA2 mcast cipher %02x:%02x:%02x:%d not enabled\n",
			...
			mcast->type));
		return DOT11_SC_INVALID_GROUP_CIPHER;
	}
	len -= WPA_SUITE_LEN;
	...
	if ((count != 1) ||
		...
		((mgmt->list[0].type == RSN_AKM_SUITEB) &&
		(WPA_auth & WPA3_AUTH_SUITEB)) ||
	       ((mgmt->list[0].type == RSN_AKM_FILS_SHA256) &&
		(WPA_auth & WPA2_AUTH_FILS_SHA256)) ||
	       ((mgmt->list[0].type == RSN_AKM_FILS_SHA384) &&
...
#endif /* WL_SAE */
		((mgmt->list[0].type == RSN_AKM_OWE) &&
		(WPA_auth & WPA_AUTH_OWE)) ||
		((mgmt->list[0].type == RSN_AKM_PSK) &&
		(WPA_auth & WPA2_AUTH_PSK)))) ||
#ifdef WL_DPP
		((bcmp(mgmt->list[0].oui, WPA3_OUI, WPA3_OUI_LEN) == 0) &&
		((mgmt->list[0].type == RSN_AKM_DPP) &&
...
#endif /* WL_SAE */
#ifdef WL_DPP
		/* Check for OUI too, as RSN_AKM_PSK and RSN_AKM_DPP has the same value */
		((bcmp(mgmt->list[0].oui, WPA3_OUI, WPA3_OUI_LEN) == 0) &&
		 ((mgmt->list[0].type == RSN_AKM_DPP) && (WPA_auth == WPA3_AUTH_DPP))) ||
#endif  /* WL_DPP */
		((mgmt->list[0].type == RSN_AKM_OWE) && (WPA_auth == WPA_AUTH_OWE)) ||
		((mgmt->list[0].type == RSN_AKM_SUITEB) && (WPA_auth == WPA3_AUTH_SUITEB)) ||
		(mgmt->list[0].type == RSN_AKM_SHA256_PSK)) {
	...
	if (!bcmwpa_akm2WPAauth((uint8 *)&mgmt->list[0], &WPA_auth)) {
		WL_ERROR(("wl%d: bcmwpa_akm2WPAauth: can't convert AKM %02x%02x%02x%02x.\n",
...
#if defined(MFP) || defined(WLWNM_AP)
	if (len == 0) {
		/* If no rsn_cap, proceed with rsn_cap as 0 */
		rsn_cap = 0;
	} else if (len > 0 && len < RSN_CAP_LEN) {
		/* rsn_cap length can be either 0 or 2 bytes */
		WL_ERROR(("wl%d: rsn_cap length: %u, len: %u bytes\n",
				wlc->pub->unit, RSN_CAP_LEN,  len));
		return DOT11_SC_INVALID_RSNIE_CAP;
	} else {
		rsn_cap = ltoh16_ua(&mgmt->list[count]);
		/* Decrement length after reading RSN capabilities */
		len -= RSN_CAP_LEN;
	}
#endif /* MFP || WLWNM_AP */
	...
	if (WLC_MFP_ENAB(wlc->pub)) {
		...
		if (((mgmt->list[0].type == RSN_AKM_SAE_PSK) ||
			(mgmt->list[0].type == RSN_AKM_SAE_FBT) ||
			/* Check for OUI too, as RSN_AKM_PSK and RSN_AKM_DPP has the same value */
			(((bcmp(mgmt->list[0].oui, WPA3_OUI, WPA3_OUI_LEN) == 0) &&
			  (mgmt->list[0].type == RSN_AKM_DPP))) ||
			(mgmt->list[0].type == RSN_AKM_OWE)) && !(rsn_cap & RSN_CAP_MFPC)) {
			WL_ERROR(("wl%d: MFP is must for SAE/FT-SAE/DPP/OWE capable STA, reject\n",
				wlc->pub->unit));
			return DOT11_SC_ASSOC_MFP_VIOLATION;
		}
	...
	if (WLC_MFP_ENAB(wlc->pub)) {
		...
		if ((bcmp(gmcs->oui, WPA2_OUI, DOT11_OUI_LEN)) ||
			((gmcs->type != WPA_CIPHER_BIP_GMAC_256) &&
			gmcs->type != WPA_CIPHER_BIP)) {
			WL_WSEC(("wl%d: WPA2 Grp Mgmt Ciphr Suite %02x:%02x:%02x:%d not enabled\n",
				wlc->pub->unit, gmcs->oui[0], gmcs->oui[1], gmcs->oui[2],
				gmcs->type));
			return DOT11_SC_CIPHER_OUT_OF_POLICY;
		}
	...
	if (len > 0) {
		WL_INFORM(("wl%d: Unparsed RSNE bytes: %u\n", wlc->pub->unit, len));
	}
...
wlc_parse_wpa_ie(wlc_info_t *wlc, wpa_ie_fixed_t *wpaie, wlc_bss_info_t *bi)
{
	...
	*/
	...
	for (i = 0, j = 0;
		...
		/* get supporting WPA AKMs */
		if (bcmwpa_akm2WPAauth((uint8 *)&mgmt->list[i], &WPA_auth)) {
			bi->WPA_auth_support |= WPA_auth;
		}
	}
...
#endif // endif
#ifdef WLTDLS
			else if (TDLS_ENAB(wlc->pub) && wlc_tdls_isup(wlc->tdls) &&
				IS_TDLS_AKM(mgmt->list[i].type)) {
...
#ifdef WL_DPP
		if (!bcmp(mgmt->list[i].oui, WPA3_OUI, WPA3_OUI_LEN)) {
			switch (mgmt->list[i].type) {
				case RSN_AKM_DPP:
					bi->wpa2.auth[j++] = mgmt->list[i].type;
					break;
				default:
					WL_ERROR(("wl%d: unsupported WPA3 auth %d\n",
						wlc->pub->unit, mgmt->list[i].type));
					return BCME_UNSUPPORTED;
			}
		} else
...
/* module entries */
static int
wlc_akm_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_WPAIE): {
		...
		wlc_ap_find_wpaie(wlc->ap, scb, (uint8 **)&wpaie, &wpaie_len);
		if (wpaie == NULL) {
			err = BCME_NOTFOUND;
			break;
		}

		if (len < (int)wpaie_len) {
			err = BCME_BUFTOOSHORT;
			break;
		}

		bcopy(wpaie, buf, wpaie_len);

		/* clear length of next IE to indicate there is no more  */
		wpaie = (bcm_tlv_t *)(buf + wpaie_len);
		wpaie->len = 0;
	...
	case IOV_SVAL(IOV_RSNXE): {
		bss_akm_info_t *bai;
		bai = BSS_AKM_INFO((wlc_akm_info_t *)ctx, bsscfg);
		...
		if (bai->host_rsnxe_len && bai->host_rsnxe) {
			/* free the existing host rsnxe value if any */
			MFREE(wlc->osh, bai->host_rsnxe, bai->host_rsnxe_len);
			bai->host_rsnxe_len = 0;
			bai->host_rsnxe = NULL;
		}
		...
		bai->host_rsnxe = MALLOCZ(wlc->osh, p_len);
		if (!bai->host_rsnxe) {
			err = BCME_NOMEM;
			break;
		}
		bai->host_rsnxe_len = p_len;
		bcopy((uint8*)params, bai->host_rsnxe, p_len);
		break;
	}

	case IOV_SVAL(IOV_EXT_SUP_RATES): {
		bss_akm_info_t *bai;
		bai = BSS_AKM_INFO((wlc_akm_info_t *)ctx, bsscfg);
		...
		if (bai->ext_sup_rates_len && bai->ext_sup_rates) {
			/* free the existing ext sup rates value if any */
			MFREE(wlc->osh, bai->ext_sup_rates, bai->ext_sup_rates_len);
			bai->ext_sup_rates_len = 0;
			bai->ext_sup_rates = NULL;
		}
		...
		bai->ext_sup_rates = MALLOCZ(wlc->osh, p_len);
		if (!bai->ext_sup_rates) {
			err = BCME_NOMEM;
			break;
		}
		bai->ext_sup_rates_len = p_len;
		bcopy((uint8*)params, bai->ext_sup_rates, p_len);
		break;
	}

#ifdef TESTBED_AP_11AX
	case IOV_SVAL(IOV_RSNIE): {
		bss_akm_info_t *bai;
		bai = BSS_AKM_INFO((wlc_akm_info_t *)ctx, bsscfg);

		if (bai->rsnie_len && bai->rsnie) {
			/* free the existing user rsnie value if any */
			MFREE(wlc->osh, bai->rsnie, bai->rsnie_len);
			bai->rsnie_len = 0;
			bai->rsnie = NULL;
		}
		...
		bai->rsnie = MALLOCZ(wlc->osh, p_len);
		if (!bai->rsnie) {
			err = BCME_NOMEM;
			break;
		}
		bai->rsnie_len = p_len;
		bcopy((uint8*)params, bai->rsnie, p_len);
		break;
	}
#endif /* TESTBED_AP_11AX */
...
wlc_akm_bsscfg_deinit(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	wlc = akm->wlc;
	bai = BSS_AKM_INFO(akm, cfg);

	if (bai && bai->host_rsnxe_len && bai->host_rsnxe) {
		/* free host rsnxe */
		MFREE(wlc->osh, bai->host_rsnxe, bai->host_rsnxe_len);
		bai->host_rsnxe_len = 0;
		bai->host_rsnxe = NULL;
	...
	if (bai && bai->ext_sup_rates_len && bai->ext_sup_rates) {
		/* free host rsnxe */
		MFREE(wlc->osh, bai->ext_sup_rates, bai->ext_sup_rates_len);
		bai->ext_sup_rates_len = 0;
		bai->ext_sup_rates = NULL;
...
#ifdef TESTBED_AP_11AX
	if (bai && bai->rsnie_len && bai->rsnie) {
		/* free user provided rsnie */
		MFREE(wlc->osh, bai->rsnie, bai->rsnie_len);
		bai->rsnie_len = 0;
		bai->rsnie = NULL;
	}
...
static int
wlc_akm_write_dh_params_ie(void *ctx, wlc_iem_build_data_t *data)
{
	wlc_akm_info_t *akmi = (wlc_akm_info_t *)ctx;
	wlc_info_t *wlc = akmi->wlc;
	wlc_iem_ft_cbparm_t *ftcbparm = data->cbparm->ft;
	struct scb *scb = ftcbparm->assocresp.scb;
	bcm_tlv_t *dh_params_ie = (bcm_tlv_t *)data->buf;

	if ((scb != NULL) && (scb->WPA_auth == WPA_AUTH_OWE) &&
		scb->dh_params && scb->dh_params_included) {
		bcopy(scb->dh_params, dh_params_ie, scb->dh_params->len + TLV_HDR_LEN);
		wlc_scb_dh_params_free(wlc, scb);
	}
...
static uint
wlc_akm_calc_dh_params_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	uint16 ie_len = 0;
	wlc_iem_ft_cbparm_t *ftcbparm = data->cbparm->ft;
	struct scb *scb = ftcbparm->assocresp.scb;
	...
	if ((scb != NULL) && (scb->WPA_auth == WPA_AUTH_OWE) &&
		scb->dh_params && scb->dh_params_included) {
		ie_len += scb->dh_params->len + TLV_HDR_LEN;
	}
...
static int
wlc_akm_arsp_write_rsn_ie(void *ctx, wlc_iem_build_data_t *data)
{
	wlc_akm_info_t *akmi = (wlc_akm_info_t *)ctx;
	wlc_info_t *wlc = akmi->wlc;
	wlc_iem_ft_cbparm_t *ftcbparm = data->cbparm->ft;
	struct scb *scb = ftcbparm->assocresp.scb;
	bcm_tlv_t *rsn_ie = (bcm_tlv_t *)data->buf;

	/* Include RSN IE in (re)assoc response when dh params is not included
	 * and RSN IE is present.
	 */
	if ((scb != NULL) && (scb->WPA_auth == WPA_AUTH_OWE) &&
		scb->rsn_ie && !scb->dh_params_included) {
		bcopy(scb->rsn_ie, rsn_ie, scb->rsn_ie->len + TLV_HDR_LEN);
		wlc_scb_rsn_ie_free(wlc, scb);
	}
...
static uint
wlc_akm_arsp_calc_rsn_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	uint16 ie_len = 0;
	wlc_iem_ft_cbparm_t *ftcbparm = data->cbparm->ft;
	struct scb *scb = ftcbparm->assocresp.scb;
	...
	if ((scb != NULL) && (scb->WPA_auth == WPA_AUTH_OWE) &&
		scb->rsn_ie && !scb->dh_params_included) {
		ie_len += scb->rsn_ie->len + TLV_HDR_LEN;
	}
...
static int
wlc_akm_save_rsnxe(wlc_info_t *wlc, rsnxe_ie_info_t *rsnxe_info,
	uint8* rsnxe, uint16 rsnxe_len)
{
	int err = BCME_OK;

	ASSERT(wlc && rsnxe_info);

	if (!wlc || !rsnxe_info || !rsnxe || !rsnxe_len) {
		err = BCME_BADARG;
		goto done;
	}

	if (!rsnxe_info->rsnxe || rsnxe_info->rsnxe_len < rsnxe_len) {
		if (rsnxe_info->rsnxe)
			MFREE(wlc->osh, rsnxe_info->rsnxe, rsnxe_info->rsnxe_len);
		rsnxe_info->rsnxe = MALLOCZ(wlc->osh, rsnxe_len);
		...
		rsnxe_info->rsnxe_len = rsnxe_len;
	}

	bcopy((uint8*)rsnxe, rsnxe_info->rsnxe, rsnxe_len);

	if (rsnxe && isset(rsnxe, DOT11_CAP_SAE_HASH_TO_ELEMENT)) {
		rsnxe_info->rsnxe_cap |= DOT11_CAP_SAE_HASH_TO_ELEMENT;
	}

	if (err != BCME_OK) {
		MFREE(wlc->osh, rsnxe_info->rsnxe, rsnxe_info->rsnxe_len);
		rsnxe_info->rsnxe_len = 0;
	}
...
static uint
wlc_akm_calc_rsnxe_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	wlc_akm_info_t *akmi = (wlc_akm_info_t *)ctx;
	bss_akm_info_t *bai = NULL;
	wlc_info_t *wlc = akmi->wlc;
	...
	bai = BSS_AKM_INFO((wlc_akm_info_t *)ctx, data->cfg);
	ASSERT(bai);

	rsnxe_info = RSNXE_INFO_GET(data->cfg);
	...
	if (bai->host_rsnxe_len && bai->host_rsnxe) {
		err = wlc_akm_save_rsnxe(wlc, rsnxe_info,
				bai->host_rsnxe, bai->host_rsnxe_len);
		if (err) {
			WL_ERROR(("wlc_akm_save_rsnxe err:%d\n", err));
			goto done;
		}
		rsnxe_len = bai->host_rsnxe_len + TLV_HDR_LEN;
	} else {
		/* External auth/supp did not provide rsnxe ie */
		if (rsnxe_info->rsnxe) {
			MFREE(wlc->osh, rsnxe_info->rsnxe, rsnxe_info->rsnxe_len);
...
static int
wlc_akm_write_rsnxe_ie(void *ctx, wlc_iem_build_data_t *data)
{
	wlc_akm_info_t *akmi = NULL;
	wlc_info_t *wlc = NULL;
	wlc_iem_ft_cbparm_t *ftcbparm = NULL;
	struct scb *scb = NULL;
	rsnxe_ie_info_t *rsnxe_info = NULL;
	bcm_tlv_t *rsnxe_ie = NULL;
	int err = BCME_OK;
	bool assoc_ft = FALSE;
	...
	if (data->ft == FC_ASSOC_REQ ||
		...
		data->ft == FC_ASSOC_RESP ||
		data->ft == FC_REASSOC_RESP) {
		assoc_ft = TRUE;
	}
	if (assoc_ft) {
		akmi = (wlc_akm_info_t *)ctx;
		wlc = akmi->wlc;
		ftcbparm = data->cbparm->ft;
		scb = ftcbparm->assocresp.scb;
	}
	rsnxe_info = RSNXE_INFO_GET(data->cfg);
	rsnxe_ie = (bcm_tlv_t *)data->buf;
	if (!rsnxe_info) {
		err = BCME_BADARG;
		goto done;
	}

	if (rsnxe_info->rsnxe_len == 0) {
		goto done;
	}
	...
	/* fixed portion */
	rsnxe_ie->id = DOT11_MNG_RSNXE_ID;
	rsnxe_ie->len = rsnxe_info->rsnxe_len;
	bcopy((uint8*)rsnxe_info->rsnxe, rsnxe_ie->data, rsnxe_info->rsnxe_len);
	if (assoc_ft && !scb) {
		wlc_scb_rsnxe_info_free(wlc, scb);
	}
...
void
wlc_akm_host_rsnxe(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *rsnxe, uint8 *rsnlen)
{
	bss_akm_info_t *bai = NULL;

	bai = BSS_AKM_INFO(wlc->akmi, cfg);
	if (!bai) {
		return;
	}

	if (bai->host_rsnxe_len == 0) {
		*rsnlen = 0;
		return;
	}
	bcopy(bai->host_rsnxe, rsnxe, bai->host_rsnxe_len);
	*rsnlen = bai->host_rsnxe_len;
	return;
}

bool
wlc_akm_is_sae_h2e_enabled(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	bss_akm_info_t *bai;
	...
	/* verifying with TLV data and data len passed from hostapd */
	if (!bai || !bai->ext_sup_rates_len || !bai->ext_sup_rates) {
		return FALSE;
	}

	if ((bai->ext_sup_rates[0] & 0x7f) == DOT11_BSS_MEMBERSHIP_SELECTOR_SAE_H2E_ONLY) {
		WL_INFORM(("wl%d:%d: sae_h2e only enabled\n", cfg->wlc->pub->unit,
			WLC_BSSCFG_IDX(cfg)));
		return TRUE;
	}
...
static int
wlc_akm_scan_parse_rsnxe_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	wlc_akm_info_t *akmi = (wlc_akm_info_t *)ctx;
	wlc_info_t *wlc = akmi->wlc;
	wlc_iem_ft_pparm_t *ftpparm = data->pparm->ft;
	...
	BCM_REFERENCE(bi);
	if (data->ie != NULL) {
		dot11_rsnxe_t *rsnxe_ie_tlv = (dot11_rsnxe_t *)data->ie;
		if (rsnxe_ie_tlv->len >= 1) {
			uint8 *cap = rsnxe_ie_tlv->cap;
			if (cap && isset(cap, DOT11_CAP_SAE_HASH_TO_ELEMENT)) {
				RSNXE_INFO_GET(bi).rsnxe_cap |=
					DOT11_CAP_SAE_HASH_TO_ELEMENT;
			}
		}
	}
...
static int
wlc_akm_arq_parse_rsnxe_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	wlc_akm_info_t *akmi = (wlc_akm_info_t *)ctx;
	wlc_info_t *wlc = akmi->wlc;
	rsnxe_ie_info_t *rsnxe_info = NULL;
	...
	if (data->ie != NULL) {
		...
		if (rsnxe_ie_tlv->len >= 1) {
			/* save copy of RSNXE */
			uint16 rsnxe_len = rsnxe_ie_tlv->len + TLV_HDR_LEN;
			rsnxe_info = RSNXE_INFO_GET(scb);
			if (rsnxe_info == NULL) {
				rsnxe_info = (rsnxe_ie_info_t *)
				MALLOC(wlc->osh, sizeof(rsnxe_ie_info_t));
				if (rsnxe_info == NULL) {
					WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
					wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
					return BCME_NOMEM;
			...
			rsnxe_info->rsnxe = NULL;
			rsnxe_info->rsnxe_len = 0;
			RSNXE_INFO_GET(scb) = rsnxe_info;
			err = wlc_akm_save_rsnxe(wlc, RSNXE_INFO_GET(scb),
				(uint8*)rsnxe_ie_tlv, rsnxe_len);
		}
	}
...
{
	wlc_akm_info_t *akm;
	bsscfg_cubby_params_t cubby;
#ifdef AP
	uint16 bcnfstbmp = FT2BMP(FC_BEACON) | FT2BMP(FC_PROBE_RESP);
	uint16 arspfstbmp = FT2BMP(FC_ASSOC_RESP) | FT2BMP(FC_REASSOC_RESP);
#endif /* AP */
	...
	uint16 rsnxe_build_fstbmp =
		...
		FT2BMP(FC_BEACON) |
		0;
	uint16 rsnxe_scan_fstbmp = FT2BMP(WLC_IEM_FC_SCAN_BCN) |
		FT2BMP(WLC_IEM_FC_SCAN_PRBRSP);
	uint16 rsnxe_arq_fstbmp = FT2BMP(FC_ASSOC_REQ) | FT2BMP(FC_REASSOC_REQ);
	...
	if (wlc_iem_add_build_fn_mft(wlc->iemi, arspfstbmp, DOT11_MNG_DH_PARAM_ID,
		wlc_akm_calc_dh_params_ie_len, wlc_akm_write_dh_params_ie, akm) != BCME_OK) {
		WL_ERROR(("wl%d: wlc_akm_attach: wlc_iem_add_build_fn_mft failed, "
					" diffie hellman params ie\n", wlc->pub->unit));
		goto fail;
	}

	if (wlc_iem_add_build_fn_mft(wlc->iemi, arspfstbmp, DOT11_MNG_RSN_ID,
		wlc_akm_arsp_calc_rsn_ie_len, wlc_akm_arsp_write_rsn_ie, akm) != BCME_OK) {
		WL_ERROR(("wl%d: wlc_akm_attach: wlc_iem_add_build_fn_mft failed, "
					" RSN ie\n", wlc->pub->unit));
		goto fail;
	}
	...
	/* calc/build */
	if (wlc_iem_add_build_fn_mft(wlc->iemi, rsnxe_build_fstbmp, DOT11_MNG_RSNXE_ID,
		wlc_akm_calc_rsnxe_ie_len, wlc_akm_write_rsnxe_ie, akm) != BCME_OK) {
		WL_ERROR(("wl%d: wlc_akm_attach: wlc_iem_add_build_fn_mft failed, "
					" rsnxe ie\n", wlc->pub->unit));
		goto fail;
	}

	if (wlc_iem_add_parse_fn_mft(wlc->iemi, rsnxe_scan_fstbmp, DOT11_MNG_RSNXE_ID,
		wlc_akm_scan_parse_rsnxe_ie, akm) != BCME_OK) {
		WL_ERROR(("wl%d: wlc_sae_attach: wlc_iem_add_parse_fn"
					" failed, rsnxe ie\n", wlc->pub->unit));
		goto fail;
	}

	if (wlc_iem_add_parse_fn_mft(wlc->iemi, rsnxe_arq_fstbmp, DOT11_MNG_RSNXE_ID,
		wlc_akm_arq_parse_rsnxe_ie, akm) != BCME_OK) {
		WL_ERROR(("wl%d: wlc_sae_attach: wlc_iem_add_parse_fn"
					" failed, rsnxe ie\n", wlc->pub->unit));
		goto fail;
	}
	...
	bzero(&cubby, sizeof(cubby));
	cubby.context = akm;
	cubby.fn_init = wlc_akm_bsscfg_init;
	cubby.fn_deinit = wlc_akm_bsscfg_deinit;
	cubby.fn_dump = wlc_akm_bsscfg_dump;
	if ((akm->cfgh = wlc_bsscfg_cubby_reserve_ext(wlc, sizeof(bss_akm_info_t), &cubby)) < 0) {
...
fail:
	MODULE_DETACH(akm, wlc_akm_detach);
	return NULL;
}
