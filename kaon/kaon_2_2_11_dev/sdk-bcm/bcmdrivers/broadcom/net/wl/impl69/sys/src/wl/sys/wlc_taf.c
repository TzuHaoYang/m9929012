...
bool wlc_taf_do_mem_log(void)
{
	return (taf_trace_buf != NULL && taf_trace_buf_len != 0);
}
...
#ifdef TAF_DBG
static INLINE bool taf_dump_q(struct bcmstrbuf *b)
{
#ifdef TAF_DEBUG_VERBOSE
	if (taf_trace_buf && b->size > PRINTF_BUFLEN)
	{
		...
		bcm_bprintf(b, "trace_buf not enabled\n");
	}
	return TRUE;
#endif /* TAF_DEBUG_VERBOSE */
	return FALSE;
...
#if defined(DONGLEBUILD) || defined(PKTC) || defined(WLCFP)
	/* enable by default */
