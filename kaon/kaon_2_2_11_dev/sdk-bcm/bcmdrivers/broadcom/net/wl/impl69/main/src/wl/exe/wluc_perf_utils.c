...
wl_scb_rx_report(void *wl, cmd_t *cmd, char **argv)
{
	...
	iov_rx_report_record_t *rec = NULL;
...
#undef DEVICE_TO_HOST64
		...
		for (tid = 0; tid < NUMPRIO; tid++) {
			...
			case DISPLAY_RAW:
				printf("%7u %8u %8u %8llu "
					...
					ctr->rxmcs, ctr->rxnss,
					ctr->rxmpdu_ofdma, ctr->rxtones, rec->time_delta);
				break;
			case DISPLAY_COOKED:
				{
				...
				if (ctr->rxmpdu) {
					...
					ratio_ofdma = ctr->rxmpdu_ofdma * 100.0 / ctr->rxmpdu;
					...
					if (ctr->rxmpdu_ofdma) {
						tones = ctr->rxtones * 1.0 / ctr->rxmpdu_ofdma;
					}
				}

				wl_float_to_string_rounding(st_nss, nss, ctr->rxmpdu);
				wl_float_to_string_rounding(st_mcs, mcs, ctr->rxmpdu);
				wl_float_to_string_rounding(st_bw, bw, ctr->rxmpdu);
				wl_float_to_string_rounding(st_tones, tones, ctr->rxmpdu);
	...
	/* Add total summary */
	if (rec && display_overall && (display_mode == DISPLAY_COOKED)) {
		ctr = &allc;
