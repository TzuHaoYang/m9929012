 ...
 *
 * $Id: pdftmsched.c 792446 2020-10-27 15:04:44Z $
 */
...
ftm_sched_compute_burst_start(pdftm_t *ftm, pdftm_session_t *sn)
{
...
#endif /* WL_FTM_MSCH */
	ASSERT(ftm->sched->burst_sn == NULL || ftm->sched->in.burst != 0);
	ASSERT(!ftm->sched->in.burst || ftm->sched->burst_sn != NULL);
	...
	if (FTM_SESSION_IS_INITIATOR(sn)) {
		...
		msch_exp = wlc_msch_endtime_timeslot(ftm->wlc->msch_info,
			sn->config->burst_config->chanspec);
	...
	if (!(ftm->sched->in.burst + ftm->sched->in.sched_wait
		+ ftm->sched->in.delay)) {
		goto done;
	}
	...
	for (i = 0; i < ftm->max_sessions; ++i) {
		uint64 start;
		pdftm_session_t *sn2 = ftm->sessions[i];
...
pdftm_sched(void *arg)
{
	...
	ASSERT(ftm->flags & FTM_FLAG_SCHED_INITED);
	ASSERT(!(ftm->flags & FTM_FLAG_SCHED_ACTIVE));

	ftm->flags |= FTM_FLAG_SCHED_ACTIVE;
	...
	/* process end - reclaim */
	pdftm_sched_process_session_end(ftm, 0, ftm->max_sessions - 1);
...
pdftm_init_sched(pdftm_t *ftm)
{
	...
	if (!ftm->sched->timer) {
		ftm->sched->timer = wl_init_timer(FTM_WL(ftm), pdftm_sched,
				ftm, FTM_SCHED_NAME);
		if (!ftm->sched->timer) {
			err = BCME_NORESOURCE;
			goto done;
...
pdftm_wake_sched(pdftm_t *ftm, pdftm_session_t *sn)
{
	...
	ftm_sched_del_timer(ftm->sched);
	ftm_sched_add_timer(ftm->sched, 0);
...
pdftm_end_sched(pdftm_t *ftm)
{
	int err = BCME_OK;
	pdftm_sched_t *sched;
	if (!(ftm->flags & ftm->flags & FTM_FLAG_SCHED_INITED))
		goto done;
	sched = ftm->sched;
