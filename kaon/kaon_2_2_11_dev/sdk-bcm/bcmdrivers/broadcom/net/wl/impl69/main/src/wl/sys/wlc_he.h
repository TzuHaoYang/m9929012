 ...
 *
 * $Id: wlc_he.h 792662 2020-11-01 10:53:24Z $
 */
...
#define SCB_HE_FRAGRX_CAP(v, a)		(SCB_HE_CAP(a) && \
	(wlc_he_get_peer_caps(v, a) & SCB_HE_DYNFRAG_RXEN))

#define WLC_HE_SU_BFR_CAP_PHY(w) \
		((wlc_phy_ac_hecaps1(WLC_PI((w))) & PHY_CAP_HE_SU_BFR) != 0)
#define WLC_HE_SU_MU_BFR_CAP_PHY(w) \
		((wlc_phy_ac_hecaps1(WLC_PI((w))) & PHY_CAP_HE_SU_MU_BFR) != 0)
#define WLC_HE_SU_BFE_CAP_PHY(w) \
		((wlc_phy_ac_hecaps1(WLC_PI((w))) & PHY_CAP_HE_SU_BFE) != 0)
#define WLC_HE_SU_MU_BFE_CAP_PHY(w) \
		((wlc_phy_ac_hecaps1(WLC_PI((w))) & PHY_CAP_HE_SU_MU_BFE) != 0)
...
bool wlc_he_htc_rtc_tx(wlc_tx_rtc_t *rtc, void *pkt);
void wlc_he_htc_rtc_init(wlc_tx_rtc_t *rtc);
...
uint8 wlc_he_get_phy_mu_feedback_ng16(wlc_he_info_t *hei);
/* check if HTC code in this packet is a real rather than a 'dummy' (ONES) code */
bool wlc_he_pkt_has_real_htc(wlc_info_t* wlc, scb_t *scb, void *pkt);
...
#define SCB_HE_FRAGRX_CAP(v, a)			FALSE
...
#define wlc_he_htc_rtc_tx(a, b)			0
#define wlc_he_htc_rtc_init(a)			do {} while (0)
#define wlc_he_htc_send_code(a, b, c, d)
#define wlc_he_pkt_has_real_htc(a, b, c)	0

#define wlc_he_omi_pmq_code(a, b, c, d)		FALSE
#define wlc_he_get_phy_mu_feedback_ng16(a)	0
...
#define HE_ULTX_STS_BLOCK_NBIT			1

/* update scb */
void wlc_he_update_scb_state(wlc_he_info_t *hei, enum wlc_bandunit band, scb_t *scb,
	he_cap_ie_t *capie, he_op_ie_t *opie, he_6g_band_caps_ie_t *he_6g_caps_ie);
void wlc_he_bcn_scb_upd(wlc_he_info_t *hei, enum wlc_bandunit band, struct scb *scb,
	he_cap_ie_t *capie, he_op_ie_t *opie, he_6g_band_caps_ie_t *he_6g_caps);
...
extern void wlc_he_update_ampdu_cap(wlc_he_info_t *hei, uint8 rx_factor, uint8 rx_density);
...
extern bool wlc_he_get_dlmmu_allow(wlc_he_info_t *hei, struct scb *scb);
...
extern void wlc_he_upd_omi_rxnss(wlc_he_info_t *hei, uint8 rxnss);
extern void wlc_he_upd_omi_txnsts(wlc_he_info_t *hei, uint8 txnsts);

bool wlc_he_is_nonbrcm_160sta(wlc_he_info_t *hei, scb_t *scb);
void wlc_he_chanspec_upd(wlc_info_t *wlc);
void wlc_he_process_obssbcn_extcap_elm(wlc_he_info_t *hei, uint8 *tlvs, int tlvs_len);
#ifdef STA
he_op_ie_t *wlc_he_copy_op_ie(wlc_he_info_t *hei, uint8 *tlvs, int tlvs_len, he_op_ie_t* op_ie);
chanspec_t wlc_he_chanspec(wlc_he_info_t *hei, he_op_ie_t *he_op_ie, chanspec_t chspec);
extern void wlc_he_omi_ulmu_disable(wlc_he_info_t *hei, wlc_bsscfg_t *cfg, bool ulmu_disable);
#endif /* STA */
