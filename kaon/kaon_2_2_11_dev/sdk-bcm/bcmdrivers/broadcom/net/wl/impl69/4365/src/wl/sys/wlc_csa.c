 ...
 *
 * $Id: wlc_csa.c 791460 2020-09-28 14:06:30Z $
 */
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_CHANSPEC_SWITCH): {
		...
		if (APSTA_ENAB(wlc->pub) && BSSCFG_STA(wlc_bsscfg_primary(wlc))) {
			if (!wlc->keep_ap_up) {
				break;
			}
		}
...
wlc_csa_timeout(void *arg)
{
	...
	FOREACH_UP_AP(wlc, idx, apcfg) {
		wlc_csa_obss_dynbw_notif_cb_notif(csam, apcfg,
	...
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	...
	if (APSTA_ENAB(wlc->pub) && !BSSCFG_AP(wlc->cfg) && BSSCFG_AP(cfg) &&
		...
		if (ap_up) {
			...
			/* prepare csa info */
			wlc_csa_prepare_csa_info(wlc, wlc->cfg, chspec,
				DOT11_CSA_MODE_NO_TX, CSA_BROADCAST_ACTION_FRAME, &csa);
			/* send rrm report and csa to upstream AP */
			wlc_csa_send_rrm_report_and_csa_to_upstream_ap(csam, wlc->cfg,
				&csa);

			/* Inform all clients associated to downstream APs with csa */
			FOREACH_UP_AP(wlc, idx, apcfg) {
	...
	wl_chan_switch_t *csa)
{
	...
	if (BSSCFG_IS_CSA_IN_PROGRESS(cfg)) {
		WL_INFORM(("wl%d: Already csa is in progress for cfg index[%d], no need of"
			" sending client csa & rrm report to upstream AP, return \n",
			wlc->pub->unit, WLC_BSSCFG_IDX(cfg)));
		return BCME_BUSY;
	}
#ifdef WLDFS
...
#else
	WL_INFORM(("wl%d: DFS not enabled, not sending client csa\n", wlc->pub->unit));
#endif /* WLDFS */
	BCM_REFERENCE(wlc);
	return BCME_OK;
}
