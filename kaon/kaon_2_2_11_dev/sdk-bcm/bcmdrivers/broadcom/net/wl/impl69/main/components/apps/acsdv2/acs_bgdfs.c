 ...
 *
 *	$Id: acs_bgdfs.c 792586 2020-10-30 04:23:58Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
reduce_bw:
	...
	acs_invalidate_rep_exclusion_channels(c_info->bgdfs_candidate[bw], c_info->c_count[bw],
			excl_chans);
	...
	chbw = acs_derive_bw_from_given_chspec(c_info, c_info->cur_chspec);
...
acs_bgdfs_ahead_trigger_scan(acs_chaninfo_t * c_info)
{
	...
	if (!is_etsi && is_dfs && (!c_info->bw_upgradable ||
			c_info->bw_upgradable_timeout > time(NULL))) {
		return BCME_OK;
	}

	/* Allow for pre-clearance when operating on etsi country codes and dfs channel */
	if ((!c_info->bw_upgradable || c_info->bw_upgradable_timeout > time(NULL)) &&
			(!is_etsi)) {
	...
	if (!is_etsi && !(FIXCHSPEC(c_info) || MONITORCHECK(c_info)) &&
...
acs_bgdfs_attempt_on_txfail(acs_chaninfo_t * c_info)
{
	...
	if (ACS_11H_AND_BGDFS(c_info) && !c_info->cur_is_dfs &&
		...
		/* Don't try to initiate BGDFS if selected channel is already pre-cleared */
