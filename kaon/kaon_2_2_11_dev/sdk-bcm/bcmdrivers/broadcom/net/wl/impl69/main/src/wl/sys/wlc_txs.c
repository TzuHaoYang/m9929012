 ...
 *
 * $Id: wlc_txs.c 793911 2020-12-14 21:07:14Z $
 */
...
wlc_dotrigtxstatus(wlc_info_t *wlc, tx_status_t *txs)
{
	...
	tgtxs.is_sppkt = TGTXS_SP(txs->status.s5);
...
#if defined(BCMDBG)
	if (TGTXS_USRIDX(txs->status.s1) == 0) {
...
#if defined(HWA_PKTPGR_BUILD) && (HWA_REVISION_ID == 131)
		if (hwa_txstat_mismatch_bypass(hwa_dev)) {
			/* Fixup framid and continue processing */
			txs->frameid = htol16(txh_info.TxFrameID);
			goto resume_txs;
		}
#endif  /* HWA_PKTPGR_BUILD && HWA_REVISION_ID == 131 */
		/* Show FIFO and SEQ */
...
#if defined(HWA_PKTPGR_BUILD) && (HWA_REVISION_ID == 131)
	hwa_txstat_mismatch_reset(hwa_dev);
resume_txs:
#endif  /* HWA_PKTPGR_BUILD && HWA_REVISION_ID == 131 */
	...
	if (ETHER_ISMULTI(txh_info.TxFrameRA)) {
		...
		if (D11REV_GE(wlc->pub->corerev, 128)) {
			...
			SCB_BS_DATA_CONDADD(bs_data_counters, airtime,
				TX_STATUS128_TXDUR(txs->status.s2));
	...
	if (!SCB_INTERNAL(scb)) {
		...
		if (CEVENT_STATE(wlc->pub) && pkttag->flags & WLF_8021X) {
			wlc_send_cevent(wlc, bsscfg, SCB_EA(scb), 0, 0, 0, NULL, 0,
				CEVENT_D2C_ST_EAP_TX, CEVENT_FRAME_DIR_TX |
	...
	if (was_acked) {
		...
		if (BSSCFG_AP(bsscfg) || BSS_TDLS_ENAB(wlc, bsscfg)) {
			if (!SCB_INTERNAL(scb) && (SCB_ISMULTI(scb) || SCB_ASSOCIATED(scb) ||
			...
			WL_PS(("wl%d: %s: suppress for PM frame 0x%x for "MACF" %s\n",
