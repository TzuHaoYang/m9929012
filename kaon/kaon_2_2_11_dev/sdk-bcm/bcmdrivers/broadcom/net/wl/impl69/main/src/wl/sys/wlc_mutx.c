...
wlc_mutx_admit_clients(wlc_mutx_info_t *mu_info)
{
	...
	if (TXPKTPENDTOT(wlc)) {
		...
		if (mu_info->active && (!LIST_IS_EMPTY(mu_info->evictee_list) ||
			...
			WL_MUTX(("wl%d: %s1:  block data fifo %d\n", wlc->pub->unit, __FUNCTION__,
	...
	if (HE_DLMU_ENAB(wlc->pub)) {
		...
		if (!LIST_IS_EMPTY(mu_info->admit_ofdma_list)) {
			if (TXPKTPENDTOT(wlc)) {
				wlc_block_datafifo(wlc, DATA_BLOCK_MUTX, DATA_BLOCK_MUTX);
				WL_MUTX(("wl%d: %s2:  block data fifo %d\n", wlc->pub->unit,
				__FUNCTION__,
				(wlc->block_datafifo & DATA_BLOCK_MUTX) != 0));
...
#ifdef WL11AX
	if ((mu_scb->tp == HEMMU) && (!wlc_he_get_dlmmu_allow(mu_info->wlc->hei, scb) ||
...
{
	wlc_mutx_info_t *mu_info = wlc->mutx;
	WL_MUTX(("wl%d: %s:  %d txpktpendtot %d\n", wlc->pub->unit, __FUNCTION__,
	        ((wlc->block_datafifo & DATA_BLOCK_MUTX) != 0), TXPKTPENDTOT(wlc)));
