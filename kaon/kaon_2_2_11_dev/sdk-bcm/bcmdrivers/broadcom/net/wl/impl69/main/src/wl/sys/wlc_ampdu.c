 ...
 *
 * $Id: wlc_ampdu.c 793901 2020-12-14 16:52:46Z $
 */
...
#if defined(WLATF_BARE) || defined(PKTQ_LOG)
#include <wlc_airtime.h>
#include <wlc_prot.h>
#include <wlc_prot_g.h>
#include <wlc_prot_n.h>
#endif /* WLATF_BARE || PKTQ_LOG */
#include <wlc_btcx.h>
#include <wlc_txc.h>
#ifdef WL11AC
#include <wlc_vht.h>
...
static INLINE pktq_counters_t * wlc_ampdu_pktqlog_cnt(wlc_info_t* wlc, struct pktq * q, int tid)
{
	...
	if (pktq_cnt == NULL && (q->pktqlog->_prec_log & PKTQ_LOG_AUTO)) {
		...
		if (pktq_cnt) {
			wlc_read_tsf(wlc, &pktq_cnt->_logtimelo, &pktq_cnt->_logtimehi);
		}
	}
	return pktq_cnt;
}
...
static const bcm_iovar_t ampdu_iovars[] = {
	{"ampdu_tx", IOV_AMPDU_TX, (IOVF_SET_DOWN), 0, IOVT_BOOL, 0},
	...
	{"ampdu_mpdu", IOV_AMPDU_MPDU, (0), 0, IOVT_INT8, 0},
	...
	{"ampdu_aggmode", IOV_AMPDU_AGGMODE, (IOVF_SET_DOWN), 0, IOVT_INT8, 0},
...
/** min and max number of bytes that can be released to the lower queue for a particular tid */
...
typedef struct atf_state {
	/* The term 'inflight' is identical to 'released', and means 'handed over to the lower
	 * queue, and the ampdu subsystem has not received a txstatus of the packet yet'.
	 */
...
struct scb_ampdu_tx {
	...
	uint8 max_rxfactor;             /**< max ampdu length exponent (full, including base) */
	uint8 mpdu_density;		/**< min mpdu start spacing, avoids rx ovfl @ remote node */
...
#endif	/* BCMDBG */
	uint8 dynfragtidmask;           /**< indicates which TIDs have dynamic fragment enable */
	uint8 htvht_rxfactor;           /**< partial max ampdu length exponent from HT or VHT IE */
	uint8 he_rxfactor_ext;		/**< max ampdu length exponent extension from HE IE  */
};
...
static void wlc_ampdu_ini_move_window_aqm(ampdu_tx_info_t *ampdu_tx,
...
static bool BCMFASTPATH ampdu_is_exp_seq_noaqm(ampdu_tx_info_t *ampdu_tx, scb_ampdu_tx_t *scb_ampdu,
	scb_ampdu_tid_ini_t *ini, uint16 seq, bool suppress_pkt);
static void wlc_ampdu_dotxstatus_regmpdu_noaqm(ampdu_tx_info_t *ampdu_tx, struct scb *scb,
	void *p, tx_status_t *txs, wlc_txh_info_t *txh_info);
static void wlc_ampdu_ini_move_window_noaqm(ampdu_tx_info_t *ampdu_tx,
	scb_ampdu_tx_t *scb_ampdu, scb_ampdu_tid_ini_t *ini);
...
static bool BCMFASTPATH ampdu_is_exp_seq_aqm(ampdu_tx_info_t *ampdu_tx,
	scb_ampdu_tx_t *scb_ampdu, scb_ampdu_tid_ini_t *ini, uint16 seq, bool suppress_pkt);
...
static void ampdu_ba_state_off(wlc_info_t *wlc, scb_ampdu_tx_t *scb_ampdu,
...
static void wlc_ampdu_flush_scb_tid(void *handle, struct scb *scb, uint8 tid);
static void wlc_ampdu_scb_deactivate(void *context, struct scb *scb);
...
#ifdef WLSQS
static int wlc_sqs_ampdu_evaluate(ampdu_tx_info_t *ampdu_tx, scb_ampdu_tx_t *scb_ampdu,
...
static txmod_fns_t BCMATTACHDATA(ampdu_txmod_fns) = {
	...
	wlc_ampdu_flush_scb_tid,	/* Flush packets for a certain scb/tid */
	wlc_ampdu_scb_deactivate,	/* Handle the deactivation of the feature */
...
static uint32 wlc_ampdu_map_pkts_cb(void *ctx, void *pkt);
...
static void wlc_ampdu_tx_scb_dump_scb(void *ctx, struct scb *scb, struct bcmstrbuf *b);
#endif // endif
...
BCMATTACHFN(wlc_ampdu_tx_cfg_init)(wlc_info_t *wlc, ampdu_tx_config_t *ampdu_tx_cfg)
{
	...
	ampdu_tx_cfg->mpdu_density = AMPDU_DEF_MPDU_DENSITY; /* advertized by this node */
...
#if defined(BCMDBG) || defined(WLTEST) || defined(BCMDBG_AMPDU)
	cubby_params.fn_dump = wlc_ampdu_tx_scb_dump_scb;
#endif // endif
...
static void
wlc_ampdu_scb_deactivate(void *context, struct scb *scb)
{
	ampdu_tx_info_t *ampdu_tx = (ampdu_tx_info_t *)context;
...
#ifdef WLTAF
	wlc_taf_scb_state_update(ampdu_tx->wlc->taf_handle, scb, TAF_AMPDU, NULL,
 ...
 */
void
scb_ampdu_update_config_mu(ampdu_tx_info_t *ampdu_tx, scb_t *scb)
{
...
wlc_ampdu_txpktcnt(void *hdl)
{
	...
	FOREACHSCB(ampdu_tx->wlc->scbstate, &scbiter, scb) {
		pktcnt += wlc_ampdu_scb_txpktcnt(ampdu_tx, scb);
	}
		...
		for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
			...
			if (ini != NULL)
			{
				if ((ini->ba_state == AMPDU_TID_STATE_BA_ON) ||
					...
					/* Send delba */
					wlc_send_delba(wlc, scb, tid, TRUE,
						DOT11_RC_TIMEOUT);
				...
				ampdu_ba_state_off(wlc, scb_ampdu, ini);
...
wlc_ampdu_frameburst_override(ampdu_tx_info_t *ampdu_tx, struct scb *scb)
{
	...
	if (AMPDU_ENAB(ampdu_tx->wlc->pub) && SCB_AMPDU(scb) &&
			(SCB_HT_CAP(scb) || SCB_VHT_CAP(scb) || SCB_HE_CAP(scb))) {
		ampdu_tx_config_t *ampdu_tx_cfg = ampdu_tx->config;
	...
	uint8 ampdu_ini_dead_timeout)
{
	...
	if (ini->dead_cnt >= ampdu_ini_dead_timeout) {
		...
		WL_AMPDU(("ampdu_dbg: wl%d.%d dead_cnt %d tx_in_transit %d ",
			wlc->pub->unit, WLC_BSSCFG_IDX(SCB_BSSCFG(scb)),
	...
	if (BCM_DMA_CT_ENAB(wlc) && (ini->dead_cnt >= ampdu_ini_dead_timeout)) {
		...
		/* trigger a PSMWD if 'psmwd_reason' iovar is set with ampdu */
		wlc_macdbg_psmwd_trigger(wlc->macdbg, WLC_MACDBG_PSMWD_AMPDU);
	}
...

{
	...
	if (ini->retry_bar) {
		...
		if (ini->retry_bar == AMPDU_R_BAR_NO_BUFFER) {
			...
			WL_ERROR(("wl%d: %s(): no memory\n",
				wlc->pub->unit, __FUNCTION__));
			rel_cnt = pktqprec_n_pkts(&scb_ampdu->txq,
				ini->tid) >> 2;
			...
			for (i = 0; i < rel_cnt; i++) {
				...
				TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_NO_BUF);
...
#ifdef AMPDU_NON_AQM
		if (!AMPDU_AQM_ENAB(wlc->pub) &&
			((ini->tx_in_transit == 0) && (ini->non_aqm->rem_window == 0)))
			ini->dead_cnt++;
#endif /* AMPDU_NON_AQM */
	...
	*/
	if (ini->dead_cnt >= MAX(ampdu_ini_dead_timeout, interval)) {
		...
		WL_ERROR(("wl%d: %s: "MACF" cleaning up ini"
			" tid %d due to no progress for %d secs"
			" tx_in_transit %d\n",
			wlc->pub->unit, __FUNCTION__,
			ETHER_TO_MACF(scb->ea), tid, ini->dead_cnt,
			ini->tx_in_transit));
...
static void wlc_ampdu_retry_ba_session(wlc_info_t *wlc, struct scb *scb, scb_ampdu_tid_ini_t *ini)
{
	...
	scb_ampdu = SCB_AMPDU_TX_CUBBY(ampdu_tx, scb);
	/* stop after addba_req_cnt max count is reached */
...
void
wlc_ampdu_check_pending_ba_for_bsscfg(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	struct scb *scb;
	struct scb_iter scbiter;
...
void
wlc_ampdu_check_pending_ba_for_scb(wlc_info_t *wlc, struct scb *scb)
{
	uint8 tid;
	ampdu_tx_info_t *ampdu_tx = wlc->ampdu_tx;
	scb_ampdu_tx_t *scb_ampdu;
 ...
 */
static void
wlc_ampdu_ba_pending_off(wlc_info_t *wlc, struct scb *scb,
	scb_ampdu_tid_ini_t *ini, uint8 tid)
{
	ampdu_tx_info_t *ampdu_tx = wlc->ampdu_tx;
	scb_ampdu_tx_t *scb_ampdu;
	...
	WL_AMPDU(("wl%d: %s: cleaning up tid %d from poff phydebug %08x txop %08x fifordy ",
		wlc->pub->unit, __FUNCTION__, tid, phydebug, txop));
	wlc_bmac_hfifo_rdy_get(wlc->hw, fifordy);
	for (i = NFIFO_U16(wlc)-1; i >= 0; i--) {
		j = NUM_BITS_U16*i;
		WL_AMPDU(("%d_%d:%04x ", j+16, j, fifordy[i]));
	}
	WL_AMPDU(("\n"));
...
static void
wlc_ampdu_ba_off(wlc_info_t *wlc, struct scb *scb,
	scb_ampdu_tid_ini_t *ini, uint8 tid)
{
	ampdu_tx_info_t *ampdu_tx = wlc->ampdu_tx;
	scb_ampdu_tx_t *scb_ampdu;
...
/**
 * Minimal spacing between transmit MPDU's in an AMPDU, used to avoid overflow at the remote node's
 * receiver. This function is called when we locally want to restrain the minimum space between
 * MPDUs that we send.
 *
 * @param[in] mpdu_density  Contrary to intuition, a larger number means 'less dense'.
 */
void
wlc_ampdu_tx_set_mpdu_density(ampdu_tx_info_t *ampdu_tx, uint8 mpdu_density)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if (SCB_AMPDU(scb)) {
			scb_ampdu_tx_t *scb_ampdu = SCB_AMPDU_TX_CUBBY(ampdu_tx, scb);
			scb_ampdu->mpdu_density =
				MAX(scb_ampdu->mpdu_density, ampdu_tx_cfg->mpdu_density);
...
#ifdef WLTAF
	/* Update TAF state */
	wlc_taf_scb_state_update(wlc->taf_handle, scb, TAF_AMPDU, NULL, TAF_SCBSTATE_SOURCE_ENABLE);
#endif // endif
...
#ifdef WLTAF
	/* Update TAF state */
	wlc_taf_scb_state_update(wlc->taf_handle, scb, TAF_AMPDU, NULL,
	...
	scb_ampdu_tx_t *scb_ampdu, scb_ampdu_tid_ini_t *ini, bool force_release)
{
	...
	ampdu_pktq_n_pkts = pktqprec_n_pkts(&scb_ampdu->txq, ini->tid);
	...
	wlc_ampdu_release(wlc->ampdu_tx, scb_ampdu, ini, ampdu_pktq_n_pkts,
		wlc_txq_avail, NULL);
}
#else /* ! (WLCFP && WLSQS) */
...
wlc_ampdu_agg_pktc(void *ctx, struct scb *scb, void *p, uint prec)
{
	...
	while (1) { /* loop over each packet in the caller supplied chain */
		...
		PKTCNTR_INC(wlc, PKTCNTR_PKTC_ENQ_SCB_AMPDU, 1);
		enq_cnt++;
	...
	if (drop_cnt) {
		...
		PKTCNTR_INC(wlc, PKTCNTR_PKTC_SCB_AMPDU_DROP, drop_cnt);
	}
...
wlc_ampdu_agg(void *ctx, struct scb *scb, void *p, uint prec)
{
	...
	TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_AMPDU_Q_FAIL);
	PKTFREE(wlc->osh, p, TRUE);
 ...
 * rate for the maximum and minimum txq release time window. Is not called when TAF is enabled.*
 *
 * @param[inout] atf_state    per-TID ATF state
 * @return                    min and max number of bytes to be released
 */
...
/**
 * Avoids a particular TID from releasing too many bytes to the lower queue.
 * wlc_ampdu_atf_lowat_release() is subsequently called for any TID that is not 'on hold'.
 *
 * @param[in] atf_state   per-tid ATF state
 * @return    TRUE if the TID should hold releasing packets
 */
static INLINE bool
...
/**
 * Decides on a per-TID, rather than a global basis, if packets should be released to the lower
 * queue.
 */
static INLINE bool
...
/**
 * When e.g. an addba is received, the ini needs to be cleaned up. As part of that, the ATF state
 * of that ini needs to be reinitialized.
 */
static void
...
/**
 * Bookkeeping. If a tx mpdu is freed, or if a txstatus was received for the mpdu, then it is no
 * longer 'in flight'.
 */
static void BCMFASTPATH
...
wlc_ampdu_get_taf_scb_tid_pktlen(void *scbh, void *tidh)
{
	...
	return (scb_ampdu && ini) ? pktqprec_n_pkts(&scb_ampdu->txq, ini->tid) : 0;
}
...
{
	return scb_ampdu ? scb_ampdu->max_pdu : 0;
}

#ifdef WLSQS
void *
wlc_ampdu_taf_sqs_link_init(wlc_info_t *wlc, struct scb *scb, uint8 prio)
{
	ampdu_tx_info_t* ampdu_tx = wlc->ampdu_tx;
	scb_ampdu_tx_t *scb_ampdu;
...
wlc_ampdu_pkt_freed(wlc_info_t *wlc, void *pkt, uint txs)
{
...
#if defined(WL_MU_TX)
	/* If packet was freed before being transmitted, update the intermediate txstatus */
	if (txs == TX_STATUS_NO_ACK) {
		wlc_scb_ratesel_init_rcb_itxs(wlc->wrsi, scb, WME_PRIO2AC(ini->tid));
	}
#endif // endif
	...
	if (!acked) {
		...
		if (aqm)
			wlc_ampdu_ini_move_window_aqm(ampdu_tx, scb_ampdu_tx, ini);
#ifdef AMPDU_NON_AQM
		else
			wlc_ampdu_ini_move_window_noaqm(ampdu_tx, scb_ampdu_tx, ini);
#endif /* AMPDU_NON_AQM */
	}
...
#else /* PROP_TXSTATUS */
#ifdef AMPDU_NON_AQM
/** AMPDU_NON_AQM is defined for NIC driver */
#define wlc_ampdu_release_count(ini, qlen, scb_ampdu) \
...
static INLINE void wlc_cfp_ampdu_schedule(scb_cfp_t *scb_cfp, ampdu_tx_info_t *ampdu_tx,
	scb_ampdu_tx_t *scb_ampdu, scb_ampdu_tid_ini_t *ini,
	...
	scb_ampdu_tid_ini_t *ini, bool force, taf_scheduler_public_t *taf)
{
	...
	qlen = pktqprec_n_pkts(&scb_ampdu->txq, ini->tid);
	...
	if (!AMPDU_AQM_ENAB(wlc->pub) &&
		...
		wlc_wlfc_suppr_status_query(wlc, scb)) {
		wlc_ampdu_flush_ampdu_q(ampdu_tx, cfg);
		return FALSE;
	}
#endif /* PROP_TXSTATUS */
...
#ifdef WLATF_BARE
	    /* XXX please note that since the term below is or'ed, it may cause packets to be
	     * released on per-tid criteria, even if the ampdu 'global' conditions would
	     * otherwise have prevented it.
	     */
...
#ifdef WLTAF
		if (taf) {
			uint16 new_qlen = pktqprec_n_pkts(&scb_ampdu->txq, ini->tid);
			if (new_qlen == 0) {
	...
	scb_ampdu_tid_ini_t *ini, uint16 release, uint16 q_avail, taf_scheduler_public_t* taf)
{
...
#ifdef WLATF_BARE
	atf_state_t *atf_state;
	atf_rbytes_t *atf_rbytes; /* max/min bytes to release for a given tid and a given TXOP */
	bool use_atf;
	uint32 max_rbytes;
	uint32 chunk_bytes;
	...
	if (CFP_ENAB(wlc->pub) == TRUE) {
		...
		if (wlc_cfp_tcb_is_EST(wlc, scb, tid, &scb_cfp) &&
			!SCB_PS(scb) &&
#if defined(WL_PS_SCB_TXFIFO_BLK)
	...
	if (!AMPDU_AQM_ENAB(pub)) {
		 ...
		 */
		if (ini->tx_in_transit == 0 && (delta + release > ini->non_aqm->rem_window)) {
			...
			memset(ini->non_aqm->ackpending, 0, sizeof(ini->non_aqm->ackpending));
			memset(ini->non_aqm->barpending, 0, sizeof(ini->non_aqm->barpending));
			delta = 0;
		} else {
	...
	while (do_release && (q_avail > 0)) { /* loop over each packet to release */
...
#ifdef WLTAF
		if (taf && taf->how == TAF_RELEASE_LIKE_IAS && TAF_IS_TAGGED(WLPKTTAG(p))) {
		...
		if (CFP_ENAB(wlc->pub) == TRUE) {
			...
			if (PKTISCFP(p)) {
				...
				/* Reset CFP flags now */
				PKTCLRCFPFLOWID(p, SCB_FLOWID_INVALID);
			}
		...
		if (taf && taf->how == TAF_RELEASE_LIKE_IAS) {
			...
			if (!taf->ias.is_ps_mode)  {
				...
				if ((taf->ias.released_units_limit > 0) &&
					...
					release = 0;
					taf->ias.npkts_for_release = 0;
					taf->ias.extend.units += taf->ias.actual.released_units -
						taf->ias.released_units_limit;
					do_release = FALSE;
	...
	PKTCNTR_INC(wlc, PKTCNTR_AMPDU_RELEASE, chunk_pkts);
	...
	if (!AMPDU_AQM_ENAB(pub)) {
		...
		if ((ini->tx_in_transit == 0) && (ini->non_aqm->rem_window == 0)) {
			WL_ERROR(("wl%d: %s: "MACF" tx_in_transit rem_window both 0"
				"tid %d chunk_pkts %d delta %d suppr_count %d \n",
				pub->unit, __FUNCTION__, ETHER_TO_MACF(scb->ea),
				tid, chunk_pkts, delta, suppr_count));
		}
	}
#endif /* AMPDU_NON_AQM */
 ...
 */
static bool BCMFASTPATH
ampdu_is_exp_seq_aqm(ampdu_tx_info_t *ampdu_tx, scb_ampdu_tx_t *scb_ampdu, scb_ampdu_tid_ini_t *ini,
 ...
 */
static bool BCMFASTPATH
ampdu_is_exp_seq_noaqm(ampdu_tx_info_t *ampdu_tx, scb_ampdu_tx_t *scb_ampdu,
	scb_ampdu_tid_ini_t *ini, uint16 seq, bool suppress_pkt)
{
 ...
 *
 * txparams include <frametype: 11n or 11vht>, <rate info in plcp>
	...
	d11linkmem_entry_t* link_entry)
{
	...
	scb_ampdu_min = MPDU_DENSITY_REV128(scb_ampdu->mpdu_density);
	...
	struct spktq *output_q, int *pkt_cnt, uint fifo)
{
	...
	if (!SCB_AMPDU(scb)) {
		WL_AMPDU(("wl%d: %s: entry func: scb ampdu flag null -- error\n",
			wlc->pub->unit, __FUNCTION__));
		...
		wlc_taf_scb_state_update(ampdu_tx->wlc->taf_handle, scb, TAF_NO_SOURCE, NULL,
			TAF_SCBSTATE_RESET);
#endif // endif
		TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_NON_AMPDU_SCB);
		PKTFREE(osh, p, TRUE);
		*pdu = NULL;
		return err;
	}
	...
	if (!ini || (ini->ba_state == AMPDU_TID_STATE_BA_PENDING_ON) ||
		...
		WL_AMPDU(("wl%d: %s: bailing out for bad ini (%p)/bastate\n",
			wlc->pub->unit, __FUNCTION__, OSL_OBFUSCATE_BUF(ini)));
		WLCNTINCR(ampdu_tx->cnt->orphan);
#ifdef WLTAF
		wlc_taf_txpkt_status(ampdu_tx->wlc->taf_handle, scb, tid, p,
			TAF_TXPKT_STATUS_PKTFREE_DROP);
		wlc_taf_link_state(ampdu_tx->wlc->taf_handle, scb, tid, TAF_NO_SOURCE,
			TAF_LINKSTATE_HARD_RESET);
#endif // endif
		TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_AMPDU_BAD_INI);
		PKTFREE(osh, p, TRUE);
		*pdu = NULL;
		return err;
	}
	...
	if (wlc->block_datafifo) {
		WL_AMPDU_TX(("wl%d: %s: datafifo blocked 0x%x\n",
...
#if defined(WL_PS_SCB_TXFIFO_BLK)
	if (SCB_PS_TXFIFO_BLK(scb)) {
		WL_AMPDU(("wl%d: %s: txfifo blocked for PS STA "MACF"\n",
	...
	struct spktq *output_q, int *pkt_cnt, uint fifo)
{
	...
	while (p) {
		...
		if (ETHER_ISNULLADDR(&h->a1)) {
			...
			TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_BAD_ADDR);
			PKTFREE(osh, p, TRUE);
			err = BCME_ERROR;
			*pdu = NULL;
			break;
		...
		if (WLPKTTAG(p)->flags & WLF_FIFOPKT) {
			 ...
			 */
		} else if (!ampdu_is_exp_seq_noaqm(ampdu_tx, scb_ampdu, ini, seq, suppress_pkt)) {
			TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_INV_SEQ);
			PKTFREE(osh, p, TRUE);
			err = BCME_ERROR;
			*pdu = NULL;
			break;
	...
	struct spktq *output_q, int *pkt_cnt, uint fifo)
{
	...
	uint corerev;
	uint16 fc_type;
	wlc_tx_pktdrop_reason_t toss_reason;
	...
	if (wlc->lifetime_txfifo && D11REV_GE(corerev, 128)) {
		txfifo_expiry = wlc_lifetime_now(wlc) +
			(wlc->lifetime_txfifo << D11_REV128_LIFETIME_SHIFT);
	}
	...
	success_cnt = 0;
	d11buf_avail = D11BUF_POOL_AVAIL();
	...
	while (TRUE) {
		...
		if (ETHER_ISNULLADDR(&h->a1)) {
			WL_AMPDU_ERR(("wl%d: %s: dropping frame with null a1\n",
				wlc->pub->unit, __FUNCTION__));
			toss_reason = TX_PKTDROP_RSN_BAD_ADDR;
			err = BCME_ERROR;
...
#ifdef PROP_TXSTATUS
#endif /* PROP_TXSTATUS */
		} else if (!ampdu_is_exp_seq_aqm(ampdu_tx, scb_ampdu, ini, seq, is_suppr_pkt)) {
			/* If the packet was not retrieved from HW FIFO before
			 * attempting over the air for multi-queue
			 */
#ifdef BCMDBG
			WL_ERROR(("wl%d: %s: "MACF": p %p seq %x pktflags %x\n",
				wlc->pub->unit, __FUNCTION__, ETHER_TO_MACF(scb->ea), p,
				WLPKTTAG(p)->seq, WLPKTTAG(p)->flags));
#endif // endif

			toss_reason = TX_PKTDROP_RSN_INV_SEQ;
			err = BCME_ERROR;
		}
			...
			*machigh = tx_info.MacTxControlHigh;
			...
			if (fill_txh_info) {
				...
				htc = HE_FC_IS_HTC(ltoh16(h->fc)) &&
					wlc_he_pkt_has_real_htc(wlc, scb, p);
				change = TRUE;
		...
		/* Set lifetime for TXFIFO at low txq enqueue time, synchronous with
		 * PKTPEND count and therefore datablock
		 */
		if (txfifo_expiry) {
			if (pkttag->flags & WLF_EXPTIME) {
				pkttag->u.exptime = MIN(txfifo_expiry, pkttag->u.exptime);
			} else {
	...
	scb_ampdu_tid_ini_t *ini)
{
	...
	WL_AMPDU_TX(("wl%d: %s: tid %d start_seq bumped to 0x%x\n",
		ampdu_tx->wlc->pub->unit, __FUNCTION__,
		ini->tid, ini->start_seq));
...
wlc_ampdu_free_chain(ampdu_tx_info_t *ampdu_tx, void *p, tx_status_t *txs)
{
	...
	if (scb) {
		...
		if ((wlc_hw->pkteng_status == PKTENG_TX_BUSY) &&
			(scb->pkts_inflt_fifocnt[prio] < count))
	...
	wlc_txh_info_t *txh_info)
{
	...
	if (!ini || (ini->ba_state == AMPDU_TID_STATE_BA_PENDING_ON)) {
		WL_AMPDU(("wl%d: %s: bail: bad ini (%p) or ba_state (%d) prio %d\n",
			wlc->pub->unit, __FUNCTION__, OSL_OBFUSCATE_BUF(ini),
...
#ifdef WL11AX
	/* Update ampdu stats */
	if ((txs_mutype == TX_STATUS_MUTP_HEMM) || (txs_mutype == TX_STATUS_MUTP_HEOM)) {
		if (txs_mutype == TX_STATUS_MUTP_HEOM) {
			gi = TX_STATUS128_HEOM_CPLTF(txs->status.s4);
		} else {
			gi = RSPEC_HE_LTF_GI(rs_txs.txrspec[rtidx]);
		}
...
#if defined(BCMDBG) || defined(WLTEST) || defined(BCMDBG_AMPDU) || defined(WL_LINKSTAT)
		WLCNTADD(ampdu_tx->amdbg->txvht[rateidx], rs_txs.tx_cnt[rtidx]);
		WLCNTADD(ampdu_tx->amdbg->txvht_succ[rateidx], rs_txs.txsucc_cnt[rtidx]);
#endif /*  defined(BCMDBG) || defined(BCMDBG_DUMP) || defined(WLTEST)
	* ||defined(BCMDBG_AMPDU) || defined(WL_LINKSTAT)
	*/
	} else {
		WL_INFORM(("wl%d: %s[%d] txs_mutype = 0x%x\n",
			wlc->pub->unit, __FUNCTION__, __LINE__, txs_mutype));
	}
#endif /* WL11AX */
...
static void
wlc_ampdu_dotxstatus_trace_dump(wlc_info_t *wlc, struct scb *scb,
	void *p, wlc_txh_info_t *txh_info, uint8 tot_mpdu, uint ncons)
{
	...
	WL_ERROR(("%s:scb 0x%p fid 0x%04x epoch 0x%x scb1 0x%p fid1 0x%04x "
		"epoch1 0x%x pkt 0x%p txh_info1 0x%p flags:0x%x flags2:0x%x "
		"flags3:0x%x seq:0x%x ncons (%d/%d)\n",
		__FUNCTION__, scb, txh_info->TxFrameID,
		((tsoHdrPtr[2] & TOE_F2_EPOCH_MASK) >> TOE_F2_EPOCH_SHIFT),
		WLPKTTAGSCBGET(p), txh_info1.TxFrameID,
		((tsoHdrPtr1[2] & TOE_F2_EPOCH_MASK) >> TOE_F2_EPOCH_SHIFT),
		p, &txh_info1, WLPKTTAG(p)->flags, WLPKTTAG(p)->flags2,
		WLPKTTAG(p)->flags3, WLPKTTAG(p)->seq, tot_mpdu+1, ncons));

	BCM_REFERENCE(tsoHdrPtr);
	BCM_REFERENCE(tsoHdrPtr1);

#if defined(WLC_MACDBG_FRAMEID_TRACE) && defined(BCMDBG)
	/* dump pkt_hist, txs_hist and dma(fifo) pointed by frameid */
	wlc_macdbg_frameid_trace_dump(wlc->macdbg,
		D11_TXFID_GET_FIFO(wlc, txh_info->TxFrameID));
#endif /* WLC_MACDBG_FRAMEID_TRACE && BCMDBG */
}
	...
	void *p, tx_status_t *txs, wlc_txh_info_t *txh_info)
{
	...
	if (!ncons) {
		...
		TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_BAD_NCONS);
		PKTFREE(wlc->osh, p, TRUE);
		ASSERT(ncons);
		return;
	} else if (ncons > 64) {
	...
	*/
	...
	if (supr_status != TX_STATUS_SUPR_NONE) {
		...
		if (supr_status == TX_STATUS_SUPR_LMEM_INVLD) {
			 ...
			 */
			WLCNTADD(wlc->pub->_wme_cnt->tx_expired[ac].packets, ncons);
	...
	while (TRUE) { /* loops over each tx MPDU in the caller supplied tx sts */
...
#ifdef BCMDBG_SWWLAN_38455
		if ((WLPKTTAG(p)->flags & WLF_AMPDU_MPDU) == 0) {
			wlc_ampdu_dotxstatus_trace_dump(wlc, scb, p, txh_info, tot_mpdu, ncons);
...
#ifdef PKTQ_LOG
				if (prec_cnt || actq_cnt || bs_data_counters) {
		...
		if (was_acked) { /* if current MPDU was acked by remote party */
			...
			ack_cnt++;
#endif // endif
			total_acked_pktlens += prec_pktlen;
...
not_retry:		/* don't retry an unacked MPDU, but send BAR if applicable */
				...
				if (supr_status == TX_STATUS_SUPR_NONE) {
					...
					WLCNTCONDINCR(actq_cnt, actq_cnt->dropped);
				}
#endif // endif
			} // if (retry_suppr_pkts)
		} /* if (not acked) */
...
free_and_next:
		...
		if (free_mpdu) {
			...
			if (supr_status == TX_STATUS_SUPR_AGG0) {
				if (D11REV_GE(wlc->pub->corerev, 128)) {
					...
					WL_AMPDU_TX(("wl%d %s: suppressed by continuous AGG0\n",
						wlc->pub->unit, __FUNCTION__));
				} else {
	...
	WL_AMPDU_TX(("wl%d: %s: ncons %d tot_mpdus %d start_seq 0x%04x\n",
		wlc->pub->unit, __FUNCTION__, ncons, tot_mpdu, ini->start_seq));
...
#ifdef BCMDBG
	/* Parse rs_txs to check for obvious errors */
	if (WL_AMPDU_ERR_ON()) {
		for (k = 0; k < rnum; k++) {
			if (rs_txs.txsucc_cnt[k] > rs_txs.tx_cnt[k]) {
				WL_AMPDU_ERR(("%s: rs_txs.txsucc_cnt[%d] > rs_txs.tx_cnt[%d]:"
					" %d > %d\n", __FUNCTION__, k, k,
					rs_txs.txsucc_cnt[k], rs_txs.tx_cnt[k]));
	...
	/* Parse rs_txs according to transmission type */
	if (fix_rate && !txs_mu) {
		...
		succ_count = rs_txs.txsucc_cnt[0];
		try_count = rs_txs.tx_cnt[0];
#ifdef WLSCB_HISTO
		WLSCB_HISTO_TX_IF_HTVHTHE(scb, rs_txs.txrspec[0], succ_msdu);
#endif /* WLSCB_HISTO */
	} else {
		txs_mutype = TX_STATUS_MUTYP(wlc->pub->corerev, txs->status.s5);
		if (txs_mu) { /* MU case */
		...
		if (!txs_mu) {
			ASSERT(rtidx == 0);
			for (k = 1; k < rnum; k++) {
				succ_count += rs_txs.txsucc_cnt[k];
				try_count += rs_txs.tx_cnt[k];
			}
		}
	...
	if (try_count >= succ_count) {
		retry_count = try_count - succ_count;
	} else {
		WL_ERROR(("wl%d: %s: retry miscount %sU (%u, %u, %u, %u) fix_rate %d\n",
			wlc->pub->unit, __FUNCTION__, txs_mu ? "M":"S",
			try_count, succ_count, rnum, txs_mutype, fix_rate));
		retry_count = 0;
	}
#ifdef WL11K
...
#endif // endif
	/*
	 * Handle lots of statistics
	 */
	failed_rts = txs->status.rts_tx_cnt - txs->status.cts_rx_cnt;
	WLCNTADD(ampdu_tx->cnt->nocts, failed_rts);
	AMPDUSCBCNTADD(scb_ampdu->cnt.nocts, failed_rts);
	...
	WLCNTSCBADD(scb_ampdu->ampdu_scb_stats->tx_bytes_total[ini->tid], total_acked_pktlens);
	if (!from_host) {
...
#ifdef PKTQ_LOG
	/* Calculate txrate_succ, txrate_succ txbw_succ and others  */
	if ((prec_cnt || actq_cnt || bs_data_counters) && succ_count) {
		txrate_main = mcs_txrate[rtidx] * try_count;
		if (fix_rate && !txs_mu) {
			txrate_succ = (mcs_txrate[0] * succ_count);
			txbw_succ = mcs_bw[0] * succ_count;
			if (actq_cnt) {
				WLCNTADD(actq_cnt->nss[mcs_nss[0]], succ_count);
			}
			if (prec_cnt) {
				WLCNTADD(prec_cnt->nss[mcs_nss[0]], succ_count);
				if (mcs_mcs[0] < 16) {
					WLCNTADD(prec_cnt->mcs[mcs_mcs[0]], succ_count);
				}
			}
#if defined(SCB_BS_DATA)
			if (bs_data_counters) {
				WLCNTADD(bs_data_counters->txmcs, (mcs_mcs[0] * succ_count));
				WLCNTADD(bs_data_counters->txnss, ((mcs_nss[0] + 1) * succ_count));
				WLCNTADD(bs_data_counters->txbw, (mcs_bw[0] * succ_count));
			}
#endif /* SCB_BS_DATA */
		} else if (!txs_mu) {
			...
			for (k = 0; k < rnum; k++) {
				...
				if (rs_txs.txsucc_cnt[k] == 0) {
					continue;
				}
				_dp_rspec = rs_txs.txrspec[k];
				_dp_succ =  rs_txs.txsucc_cnt[k];

				txrate_succ += mcs_txrate[k] * _dp_succ;
				txbw_succ += mcs_bw[k] * _dp_succ;
				...
				if (actq_cnt) {
					WLCNTADD(actq_cnt->nss[mcs_nss[k]], _dp_succ);
				}
				if (prec_cnt) {
					WLCNTADD(prec_cnt->nss[mcs_nss[k]], _dp_succ);
					if (mcs_mcs[k] < 16) {
						WLCNTADD(prec_cnt->mcs[mcs_mcs[k]], _dp_succ);
					}
					WLCNTCONDADD(is_sgi, prec_cnt->sgi, _dp_succ);
				}
#if defined(SCB_BS_DATA)
				if (bs_data_counters) {
					WLCNTADD(bs_data_counters->txmcs, _dp_succ * mcs_mcs[k]);
					WLCNTADD(bs_data_counters->txnss,
						_dp_succ * (mcs_nss[k] + 1));
					WLCNTADD(bs_data_counters->txbw, _dp_succ * mcs_bw[k]);
				}
#endif /* SCB_BS_DATA */
			}
		} else if (txs_mu) {
			...
			if ((txs_mutype == TX_STATUS_MUTP_VHTMU) ||
				...
				SCB_BS_DATA_CONDADD(bs_data_counters, mu_acked, succ_count);
				...
				SCB_BS_DATA_CONDADD(bs_data_counters, ru_acked, succ_count);
			} else  {
				/* to do/check */
				dp_bw = 0;
				dp_is_sgi = 0;
			...
			txbw_succ = succ_count *
				wlc_ratespec_bw(dp_bw << WL_RSPEC_BW_SHIFT);

			if ((txs_mutype != TX_STATUS_MUTP_VHTMU)) {
				txrate_succ = succ_count *
...
#if defined(SCB_BS_DATA)
			if (bs_data_counters) {
				WLCNTADD(bs_data_counters->txmcs, succ_count * dp_mcs);
				WLCNTADD(bs_data_counters->txnss, succ_count * (dp_nss + 1));
				WLCNTADD(bs_data_counters->txbw, txbw_succ);
			}
#endif /* SCB_BS_DATA */

			if (actq_cnt) {
				WLCNTADD(actq_cnt->nss[dp_nss], succ_count);
			}
			if (prec_cnt) {
				WLCNTADD(prec_cnt->nss[dp_nss], succ_count);
				WLCNTADD(prec_cnt->mcs[dp_mcs], succ_count);
				WLCNTCONDADD(dp_is_sgi, prec_cnt->sgi, succ_count);
			}
		}
		...
		/* save airtime */
		if (D11REV_GE(wlc->pub->corerev, 128)) {
			airtime = TX_STATUS128_TXDUR(txs->status.s2);
		} else {
			airtime = TX_STATUS40_TX_MEDIUM_DELAY(txs);
		}
	}

	/* Finally, update enabled counters */
	if (prec_cnt) {
		WLCNTADD(prec_cnt->rtsfail, failed_rts);
		WLCNTADD(prec_cnt->acked, ack_cnt);
		WLCNTADD(prec_cnt->throughput, (uint64)total_acked_pktlens);
		...
		WLCNTADD(prec_cnt->suppress, suppress_cnt);
		WLCNTADD(prec_cnt->airtime, (uint64)airtime);
	}
	if (actq_cnt) {
		WLCNTADD(actq_cnt->rtsfail, failed_rts);
		WLCNTADD(actq_cnt->acked, ack_cnt);
		WLCNTADD(actq_cnt->throughput, (uint64)total_acked_pktlens);
		WLCNTADD(actq_cnt->txrate_succ, (uint64)txrate_succ);
		WLCNTADD(actq_cnt->retry, retry_count);
	}
#if defined(SCB_BS_DATA)
	if (bs_data_counters) {
		WLCNTADD(bs_data_counters->rtsfail, failed_rts);
		WLCNTADD(bs_data_counters->acked, ack_cnt);
		WLCNTADD(bs_data_counters->throughput, (uint64)total_acked_pktlens);
		WLCNTADD(bs_data_counters->txrate_succ, (uint64)txrate_succ);
		WLCNTADD(bs_data_counters->retry, retry_count);

		WLCNTADD(bs_data_counters->txrate_main, (uint64)txrate_main);
		WLCNTADD(bs_data_counters->airtime, airtime);
	}
#endif /* SCB_BS_DATA */
	if (ampdu_dpdbg) {
		for (k = 0; k < rnum; k++) {
	...
	else for (k = 0; k < rnum; k++) {
		/* Updating ampdu_tx->amdbg and ampdu_tx->cnt */
		uint gi, mcs, nss, rateidx;
		uint8 ampdu_ft;
		BCM_REFERENCE(gi);
		BCM_REFERENCE(mcs);
...
#endif /* WL11AC */
		case FT_HT:
			rateidx = MCS2IDX(rs_txs.txrspec[k] & WL_RSPEC_HT_MCS_MASK);
			ASSERT(rateidx < AMPDU_HT_MCS_ARRAY_SIZE);
#ifdef WLPKTDLYSTAT
			WLCNTADD(ampdu_tx->cnt->txmpdu_cnt[rateidx], rs_txs.tx_cnt[k]);
			WLCNTADD(ampdu_tx->cnt->txmpdu_succ_cnt[rateidx], rs_txs.txsucc_cnt[k]);
#endif /* WLPKTDLYSTAT */
			WLCNTADD(ampdu_tx->amdbg->txmcs[rateidx], rs_txs.tx_cnt[k]);
			WLCNTADD(ampdu_tx->amdbg->txmcs_succ[rateidx], rs_txs.txsucc_cnt[k]);
			if (rs_txs.txrspec[k] & WL_RSPEC_SGI) {
				WLCNTADD(ampdu_tx->cnt->u1.txmpdu_sgi, rs_txs.tx_cnt[k]);
				WLCNTADD(ampdu_tx->amdbg->txmcssgi[rateidx], rs_txs.tx_cnt[k]);
			}
			if (rs_txs.txrspec[k] & WL_RSPEC_STBC) {
				WLCNTADD(ampdu_tx->cnt->u2.txmpdu_stbc, rs_txs.tx_cnt[k]);
				WLCNTADD(ampdu_tx->amdbg->txmcsstbc[rateidx],
					rs_txs.tx_cnt[k]);
			}
			break;

		default:
			WL_INFORM(("wl%d: %s[%d] un-recognized frame type = 0x%x\n",
				wlc->pub->unit, __FUNCTION__, __LINE__, ft[k]));
			break;
		}
	...
	/*
	 * end of handle lots of statistics
	 */

#if defined(WL_MU_TX) && defined(WLCNT)
	if (((MU_TX_ENAB(wlc) && SCB_VHTMU(scb)) || (HE_DLMMU_ENAB(wlc->pub) && SCB_HEMMU(scb))) &&
...
#ifdef WLSCB_HISTO
		WLSCB_HISTO_TX_IF_HTVHTHE(scb, rs_txs.txrspec[0], rs_txs.txsucc_cnt[0] <<
				WLPKTTAG_AMSDU(p));
		WLSCB_HISTO_TX_IF_HTVHTHE(scb, rs_txs.txrspec[1], rs_txs.txsucc_cnt[1] <<
				WLPKTTAG_AMSDU(p));
#endif /* WLSCB_HISTO */
		if (!(txs->status.was_acked)) {
	...
	while (p) {
		...
		if (!ack_recd) {
...
#endif /* PROP_TXSTATUS */
			{
				...
				TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_TOOMANY_RETX);
				WLCNTSCBINCR(scb->scb_stats.tx_failures);
...
#define MCS_TO_RSPEC(mcs, bw, plcp)	((HT_RSPEC(mcs)) | ((bw) << WL_RSPEC_BW_SHIFT)| \
	...
	if (!((bw >= BW_20MHZ) && (bw <= BW_40MHZ))) {
		WL_ERROR(("wl%d: %s: Incorrect bw, airtime computation skipped, bw %d\n",
			wlc->pub->unit, __FUNCTION__, bw));
		goto skip_airtime_computation;
	}
 ...
 */
static void
ampdu_ba_state_off(wlc_info_t *wlc, scb_ampdu_tx_t *scb_ampdu, scb_ampdu_tid_ini_t *ini)
{
	uint16 deq_cnt = 0;
	void *p;
	struct scb *scb = scb_ampdu->scb;
...
#if defined(BCM_PKTFWD_FLCTL)
	__wlc_ampdu_update_link_credits(wlc, scb, ini->tid, deq_cnt, TRUE);
#endif /* BCM_PKTFWD_FLCTL */
	 ...
	 */
#if defined(DONGLEBUILD)
	wlc_txflowcontrol_override(wlc, SCB_WLCIFP(scb_ampdu->scb)->qi,
		OFF, TXQ_STOP_FOR_AMPDU_FLOW_CNTRL);
#endif /* DONGLEBUILD */
#ifdef WLTAF
	wlc_taf_link_state(wlc->taf_handle, scb, ini->tid, TAF_AMPDU,
...
wlc_ampdu_map_pkts_cb(void *ctx, void *pkt)
{
	...
	uint32 map_result = MAP_PKTS_CB_NOOP;
	...
	if ((scb != WLPKTTAGSCBGET(pkt)) ||
		...
		ampdu_tx->tx_intransit--;
#ifdef WLTAF
		/* TAF accounting for dropped packets */
		wlc_taf_txpkt_status(ampdu_tx->wlc->taf_handle, scb, ini->tid, pkt,
			TAF_TXPKT_STATUS_PKTFREE_DROP);
#endif // endif
...
/**
 * Inform TAF/SQS that it should no longer retrieve new data packets for this scb.
 * This function is called when scb is marked for deletion.
 */
void
wlc_ampdu_scb_close_link(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (SCB_AMPDU(scb)) {
		...
		WL_AMPDU(("wl%d: "MACF": scb ampdu close all links.\n",
			wlc->pub->unit, ETHER_TO_MACF(scb->ea)));
		...
		for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
			...
			wlc_taf_link_state(wlc->taf_handle, scb, tid, TAF_AMPDU,
				TAF_LINKSTATE_NOT_ACTIVE);
		}
#endif /* WLTAF */
#ifdef WLCFP
	        if (CFP_ENAB(wlc->pub) == TRUE) {
			/* As scb has been marked for deletion, cfp will be disabled for tx. */
			wlc_cfp_scb_state_upd(wlc->cfp, scb);
			wlc_cfp_tcb_upd_pause_state(wlc, scb, TRUE);
		}
#endif /* WLCFP */
...
ampdu_cleanup_tid_ini(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 tid, bool force)
{
	...
	(void)wlc_txq_pktq_pflush(wlc, &scb_ampdu->txq, ini->tid);
...
wlc_ampdu_recv_addba_resp(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 *body, int body_len)
{
	...
	if ((status != DOT11_SC_SUCCESS) ||
		...
		WL_AMPDU_ERR(("wl%d: %s: "MACF": Failed. status %d wsize %d policy %d\n",
			wlc->pub->unit, __FUNCTION__, ETHER_TO_MACF(scb->ea),
			status, wsize, policy));
		ampdu_ba_state_off(wlc, scb_ampdu_tx, ini);
		WLCNTINCR(ampdu_tx->cnt->rxunexp);
		return;
	}
...
#ifdef BCMDBG
	old_queue = pktqprec_n_pkts(&scb_ampdu_tx->txq, tid);
#endif // endif
...
#ifdef BCMDBG
	if ((old_queue > 0) && ((i = pktqprec_n_pkts(&scb_ampdu_tx->txq, tid)) > old_queue)) {
		WL_ERROR(("wl%u %s: "MACF" tid %u, %u packets in flight re-enqueued on top of "
			"%u, will not preserve packet ordering\n",
			WLCWLUNIT(wlc), __FUNCTION__, ETHER_TO_MACF(scb->ea), tid,
			(i - old_queue), old_queue));
	}
#endif // endif

#ifdef WLNAR
	wlc_nar_addba(wlc->nar_handle, scb, WLC_PRIO_TO_PREC(tid));
#endif /* WLNAR */
...
#ifdef WLATF_BARE
/* Set per TID atf mode. Should not get called when TAF is enabled. */
static void wlc_ampdu_atf_tid_setmode(scb_ampdu_tid_ini_t *ini, uint32 mode)
{
	ASSERT(mode < WLC_AIRTIME_DONGLE);
	wlc_ampdu_atf_reset_state(ini);
...
static void wlc_ampdu_atf_tid_set_release_time(scb_ampdu_tid_ini_t *ini, uint txq_time_allowance_us)
{
	...
	scb = ini->scb;
	ASSERT(scb);
	if ((MU_TX_ENAB(scb->bsscfg->wlc) && SCB_VHTMU(scb)) ||
	    (HE_DLMMU_ENAB(scb->bsscfg->wlc->pub) && SCB_HEMMU(scb))) {
		/* For MU clients, allow double time to meet performance requirements */
		atf_state->txq_time_allowance_us *= 2;
	}
#endif /* WL_MU_TX */
...
/**
 * The ATF release time for all TIDs of particular remote node is the same.
...
wlc_ampdu_atf_ratespec_override(void *ptr)
{
	...
	return wlc_scb_ratesel_get_primary(atf_state->ampdu_tx->wlc, atf_state->ini->scb, NULL);
}
...
#endif // endif
	wlc_send_addba_req(wlc, scb, tid, ampdu_tx_cfg->ba_tx_wsize,
...
/**
 * wlc_ampdu_ba_on_tid() - return if a block ack agreement exists for the given tid.
 *
 * Inputs:
 *	scb	pointer to station control block to examine.
 *	tid	8-bit integer indicating the priority.
 *
 * Returns:
 *	The return value is true if a block ack agreement exists for the given tid.
 */
bool BCMFASTPATH
wlc_ampdu_ba_on_tid(struct scb *scb, uint8 tid)
{
	wlc_info_t *wlc;
	scb_ampdu_tx_t *scb_ampdu;
	scb_ampdu_tid_ini_t *ini;
...
wlc_ampdu_tx_dump(ampdu_tx_info_t *ampdu_tx, struct bcmstrbuf *b)
{
	...
	if (wlc->clk && AMPDU_MAC_ENAB(pub) && !AMPDU_AQM_ENAB(pub)) {
		...
		bcm_bprintf(b, "txampdu %d txmpdu %d txmpduperampdu %d noba %d (%d%%) nocts %d\n",
	...
	HWA_TXPOST_EXPR(HWA_BCMDBG_EXPR({
		uint32 *table_val;
		table_val = hwa_txpost_histogram(hwa_dev, FALSE);
		if (table_val)  {
			bcm_bprintf(b, "ScheCmdDens: unit x%d\n",
				(1 << HWA_TXPOST_HISTOGRAM_UNIT_SHIFT));
			wlc_print_dump_table(b, "           ", table_val, NULL, NULL,
				HWA_TXPOST_HISTOGRAM_MAX, 8, TABLE_NONAME);
		}
	}));
#endif /* BCMHWA */
...
static void
wlc_ampdu_tx_scb_dump_scb(void *ctx, struct scb *scb, struct bcmstrbuf *b)
{
	ampdu_tx_info_t *ampdu_tx = (ampdu_tx_info_t*)ctx;
	scb_ampdu_tx_t *scb_ampdu;
#ifdef BCMDBG
	...
	if (SCB_AMPDU(scb)) {
		scb_ampdu = SCB_AMPDU_TX_CUBBY(ampdu_tx, scb);
		bcm_bprintf(b, MACF": max_pdu %d release %d max_ampdu_size %d mpdu_density %d\n",
			ETHER_TO_MACF(scb->ea),
			scb_ampdu->max_pdu, scb_ampdu->release,
			((1 << scb_ampdu->max_rxfactor) - 1),
			scb_ampdu->mpdu_density);
#ifdef BCMDBG
		bcm_bprintf(b, "\ttxdrop %u txstuck %u "
				  ...
				  scb_ampdu->cnt.nocts, scb_ampdu->cnt.rxaddbaresp);
		for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
			scb_ampdu_tid_ini_t *ini = scb_ampdu->ini[tid];
			if (!ini)
				continue;
			if (!(ini->ba_state == AMPDU_TID_STATE_BA_ON))
				continue;
			bcm_bprintf(b, "tid %d:\tba_state %d ba_wsize %d tx_in_transit %d "
				"rem_window %d\n",
				ini->tid, ini->ba_state, ini->ba_wsize, ini->tx_in_transit,
...
#ifdef WLTAF
			wlc_taf_in_use(ampdu_tx->wlc->taf_handle);
#endif // endif
		}
#endif /* BCMDBG */
	}
...
wlc_ampdu_tx_scb_dump(ampdu_tx_info_t *ampdu_tx, struct bcmstrbuf *b)
{
	...
	if (wlc->dump_args) {
		...
		while ((!strncmp(args, "-mac ", 5)) && (argc < DUMP_AMPDU_ARGV_MAX)) {
			...
			if (scb != NULL) {
				wlc_ampdu_tx_scb_dump_scb(ampdu_tx, scb, b);
				args += 17;
				argc += 17;
...
static void
wlc_ampdu_flush_scb_tid(void *handle, struct scb *scb, uint8 tid)
{
	ampdu_tx_info_t *ampdu_tx = (ampdu_tx_info_t *)handle;
	wlc_info_t *wlc = ampdu_tx->wlc;
	scb_ampdu_tx_t *scb_ampdu = SCB_AMPDU_TX_CUBBY(ampdu_tx, scb);
	uint32 n_pkts_flushed;

	if (scb_ampdu && scb_ampdu->ini[tid] && !pktqprec_empty(&scb_ampdu->txq, tid)) {
		n_pkts_flushed = wlc_txq_pktq_pflush(wlc,  &scb_ampdu->txq, tid);
		BCM_REFERENCE(n_pkts_flushed);
		WL_AMPDU(("wl%d.%d: %s flushing %d packets for "MACF" AID %d tid %d\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(SCB_BSSCFG(scb)), __FUNCTION__,
			n_pkts_flushed, ETHER_TO_MACF(scb->ea), SCB_AID(scb), tid));

		wlc_ampdu_txflowcontrol(wlc, scb_ampdu, scb_ampdu->ini[tid]);
...
wlc_ampdu_clear_tx_dump(ampdu_tx_info_t *ampdu_tx)
{
	...
	HWA_TXPOST_EXPR(HWA_BCMDBG_EXPR((void)hwa_txpost_histogram(hwa_dev, TRUE)));
#endif // endif
	...
	uint16 initiator, uint16 reason)
{
	...
	WL_AMPDU(("wl%d: %s: "MACF": tid %d initiator %d reason %d\n",
		wlc->pub->unit, __FUNCTION__,
...
void
wlc_ampdu_upd_pm(wlc_info_t *wlc, uint8 PM_mode)
{
	int i;
...
#ifdef PROP_TXSTATUS
void
wlc_ampdu_flush_ampdu_q(ampdu_tx_info_t *ampdu, wlc_bsscfg_t *cfg)
{
	struct scb_iter scbiter;
	struct scb *scb = NULL;
	scb_ampdu_tx_t *scb_ampdu;
...
void
wlc_ampdu_send_bar_cfg(ampdu_tx_info_t * ampdu, struct scb *scb)
{
	scb_ampdu_tx_t *scb_ampdu;
	scb_ampdu_tid_ini_t *ini;
	uint32 pktprio;
...
void
wlc_check_ampdu_fc(ampdu_tx_info_t *ampdu, struct scb *scb)
{
	scb_ampdu_tx_t *scb_ampdu;
	scb_ampdu_tid_ini_t *ini;
	int i;
...
/** tx queue profiling */
void
wlc_ampdu_txq_prof_add_entry(wlc_info_t *wlc, struct scb *scb, const char * func, uint32 line)
{
	...
	p_entry->func = func;
	p_entry->line = line;
	p_entry->ampdu_q_len = pktqprec_n_pkts((&scb_ampdu->txq), (ini->tid));
...
void
wlc_ampdu_txq_prof_print_histogram(int entries)
{
	int i, index;
	struct wlc_ampdu_txq_prof_entry *p_entry;
...
wlc_ampdu_txeval_all(wlc_info_t *wlc)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if (SCB_AMPDU(scb)) {
			wlc_ampdu_txeval_alltid(wlc, scb, FALSE);
		}
	}
}
...
int
wlc_ampdu_get_epoch(ampdu_tx_info_t *ampdu_tx, int fifo)
{
	return ampdu_tx->hwagg[fifo].epoch;
}
...
void
wlc_ampdu_get_stats(wlc_info_t *wlc, rrm_stat_group_12_t *g12)
{
	ASSERT(wlc);
	ASSERT(g12);
...
static INLINE uint8 wlc_cfp_ampdu_update_epoch(wlc_tx_rtc_t *rtc, int reason_code,
	bool htc, uint8* tsoHdrPtr);

/* 5.2 TXD fixups after cache updates for MACREV >= 40 & < 128 */
static INLINE void wlc_cfp_ampdu_txd_ge40_fixup(wlc_tx_rtc_t *rtc, void *pkt, d11txhdr_t *txh,
	uint8 *iv_body);
/* 5.2 TXD fixups after cache updates for MACREV >= 128 */
static INLINE void wlc_cfp_ampdu_txd_ge128_fixup(wlc_tx_rtc_t *rtc, void* pkt,
	d11txhdr_t *txh, uint8 *iv_body);

/* 5.1 TXCache Fixups[ dot11 header Section] */
static INLINE void wlc_cfp_ampdu_d11hdr_fixups(wlc_tx_rtc_t *rtc, void* pkt,
	d11txhdr_t *txh, char *da, char *sa);

/* 5. PREPARE: Prepare MPDU for transmission to AQM */
static INLINE void wlc_cfp_ampdu_prepare(wlc_tx_rtc_t *rtc, void *pkt);
/* 4. RELEASE: Release packets from AMPDU Q with aggregation. */
static INLINE uint16 wlc_cfp_ampdu_release(scb_cfp_t *scb_cfp, ampdu_tx_info_t *ampdu_tx,
	scb_ampdu_tx_t *scb_ampdu, scb_ampdu_tid_ini_t *ini,
...
static INLINE void wlc_cfp_ampdu_evaluate(scb_cfp_t *scb_cfp, ampdu_tx_info_t *ampdu_tx,
 ...
 */
static INLINE uint8
wlc_cfp_ampdu_update_epoch(wlc_tx_rtc_t *rtc, int reason_code,  bool htc, uint8* tsoHdrPtr)
{
	struct scb *scb;
	ampdu_tx_info_t *ampdu_tx;
	ampdumac_info_t *hwagg;
	...
	ampdu_tx = rtc->ampdu_tx;
	scb_cfp = rtc->scb_cfp;
	cache_gen = SCB_CFP_CACHE_GEN(scb_cfp);
	scb = rtc->scb;
	fifo = rtc->fifo;
	tid = rtc->tid;
	hwagg = &(ampdu_tx->hwagg[fifo]);
 ...
 */
static INLINE void
wlc_cfp_ampdu_txd_ge128_fixup(wlc_tx_rtc_t *rtc, void* pkt, d11txhdr_t *txh, uint8 *iv_body)
{
	...
	/* Initialize */
	ampdu_tx = (ampdu_tx_info_t *)rtc->ampdu_tx;
	scb_cfp = (scb_cfp_t *)rtc->scb_cfp;
	wlc = rtc->wlc;
	scb = rtc->scb;
	tid = rtc->tid;
	fifo = rtc->fifo;
	...
	frameid = wlc_compute_frameid_rtc(rtc, 0);
...
#endif // endif
	txh->rev128.FrmLen =
		htol16((uint16)(pktlen +  DOT11_FCS_LEN - rtc->d11_txh_len - tsoHdrSize));
	...
	if (!(GE128_FC_2_TXH_FT(FC_TYPE(fc)) ==
		((tsoHdrPtr[1] & TOE_F1_FT_MASK) >> TOE_F1_FT_SHIFT))) {
		WL_ERROR(("wl%d: %s: False (fc=0x%x tsoHdrPtr[1]=0x%x) 0x%x == 0x%x\n",
		wlc->pub->unit, __FUNCTION__, ntoh16(fc), tsoHdrPtr[1],
		GE128_FC_2_TXH_FT(FC_TYPE(fc)),
		((tsoHdrPtr[1] & TOE_F1_FT_MASK) >> TOE_F1_FT_SHIFT)));
	}
	...
	*/
	 ...
	 * Avoiding D11_REV128_GET_MACLOW_PTR macro to bypass
	 * D11REV* branching
	 */

	maclow = D11_REV128_GET_MACLOW_PTR(txh);
	machigh = D11_REV128_GET_MACHIGH_PTR(txh);
	...
	*machigh &= htol16(~D11AC_TXC_SVHT);
	...
	htc = FALSE;
	if ((ampdu_tx->cfp_head_frame) || (hwagg->prev_htc)) {
		/* Evaluate epoch update changes for just the head frame */
		change = TRUE;
		htc = HE_FC_IS_HTC(fc) && wlc_he_pkt_has_real_htc(wlc, scb, pkt);

		/* Reset head frame flag */
		ampdu_tx->cfp_head_frame = FALSE;
		/* TODO: scb and tid based epoch should not be a per packet check */
		if ((scb != hwagg->prev_scb) | (tid != hwagg->prev_tid)) {
			reason_code = AMU_EPOCH_CHG_DSTTID;
	...
	if (change) {
		...
		if (ltoh16(*maclow) & D11AC_TXC_UPD_CACHE) {
			ampdu_tx->cfp_head_frame = TRUE;
		}

		wlc_cfp_ampdu_update_epoch(rtc, reason_code, htc, tsoHdrPtr);
	}
 ...
 */
static INLINE void
wlc_cfp_ampdu_txd_ge40_fixup(wlc_tx_rtc_t *rtc, void *pkt, d11txhdr_t *txh, uint8 *iv_body)
{
	uint16 frameid; /* frameID fixup */
	uint tsoHdrSize; /* TSO header size */
	bool change; /* epoch change */
	...
	pktinfo->FrameLen = htol16((uint16)(rtc->mpdu_bytes +
	...
	if (change) {
		...
		wlc_cfp_ampdu_update_epoch(rtc, reason_code, FALSE, tsoHdrPtr);
	}
 ...
 */
static INLINE void
wlc_cfp_ampdu_d11hdr_fixups(wlc_tx_rtc_t *rtc, void* pkt, d11txhdr_t *txh, char *da, char *sa)
{
	...
	/* dot11 hdr ptr */
	h = (struct dot11_header*) (((char*)txh) + rtc->d11_txh_len);
	/* A3 Fixups */
 ...
 */
static INLINE void
wlc_cfp_ampdu_prepare(wlc_tx_rtc_t *rtc, void *pkt)
{
	...
	HWA_PKT_DUMP_EXPR({
		if (PKTISTXFRAG(PKT_OSH_NA, pkt)) {
			hwa_txpost_dump_pkt(pkt, NULL, "wlc_cfp_ampdu_prepare", 0, TRUE);
		}
	});
#endif // endif
...
#if defined(BCMHWA)
		HWA_PKTPGR_EXPR(wlc_amsdu_single_txlfrag_fixup(wlc, pkt));
#endif // endif
	}
#endif /* BCM_DHDHDR && DONGLEBUILD */
	...
	txh = wlc_txc_cp(rtc, pkt, &flags);

	/* cache mpdu_butes after wlc_txc_cp */
	rtc->mpdu_bytes = pkttotlen(PKT_OSH_NA, pkt);

	/** 5.2 TXCache Fixups [dot11header] */
	if (is_amsdu == 0) {
		/* NON AMSDU: Pickup DA/SA for A3 fixup  from incoming pkt header */
		wlc_cfp_ampdu_d11hdr_fixups(rtc, pkt, txh, (char*)&da, (char*)&sa);
	} else {
		/* AMSDU: Pickup DA/SA for A3 fixup from bsscfg/scb */
		wlc_cfp_ampdu_d11hdr_fixups(rtc, pkt, txh, NULL, NULL);
	}

	/* TXD fixups */
	if (rtc->is_ge128) {
		wlc_cfp_ampdu_txd_ge128_fixup(rtc, pkt, txh, iv_body);
	} else {
		wlc_cfp_ampdu_txd_ge40_fixup(rtc, pkt, txh, iv_body);
	}
	...
	if (rtc->apps_ucast) {
		...
		h = (struct dot11_header*) ((uint8 *)txh + rtc->d11_txh_len);
 ...
 *
 * @param[in] release_cnt	Number of MPDUs to release
 *
 */
static INLINE uint16
wlc_cfp_ampdu_release(scb_cfp_t *scb_cfp, ampdu_tx_info_t *ampdu_tx, scb_ampdu_tx_t *scb_ampdu,
	...
	taf_scheduler_public_t* taf)
{
	...
	uint32 tot_amsdu_cnt = 0;
#endif /* !BCMDBG && !BCMDBG_AMSDU */

	/* Initialize rtc */
	rtc.ampdu_tx = ampdu_tx;
	rtc.ini = ini;
	rtc.scb_cfp = scb_cfp;
	rtc.scb_ampdu = scb_ampdu;
	rtc.wlc = wlc = ampdu_tx->wlc;
	rtc.scb = scb = SCB_CFP_SCB(scb_cfp);

	wlc_he_htc_rtc_init(&rtc);
	rtc.sti = wlc_scb_txc_info(wlc, scb);

	rtc.bsscfg = bsscfg = SCB_CFP_CFG(scb_cfp);

	rtc.is_ge128 = D11REV_GE((wlc)->pub->corerev, 128) ? TRUE : FALSE;
	rtc.tid = tid = ini->tid;
	rtc.mpdu_bytes = 0;
	rtc.d11_txh_len = D11_TXH_LEN_EX(wlc);
...
#if defined(WL_PS_SCB_TXFIFO_BLK)
	ASSERT(SCB_PS_TXFIFO_BLK(scb) == FALSE);
#endif /* WL_PS_SCB_TXFIFO_BLK */
	...
	rtc.fifo = fifo = wlc_fifo_index_get(wlc->fifo, scb, prio2ac[tid]);
	rtc.hw_fifo = WLC_HW_MAP_TXFIFO(wlc, fifo);
	rtc.apps_ucast = BSSCFG_AP(bsscfg) & SCB_PS(scb) & (fifo != TX_BCMC_FIFO);

	ASSERT(fifo < WLC_HW_NFIFO_INUSE(wlc));

	/* XXX WAR for BCAWLAN-221763 and BCAWLAN-221910.
	 * Related to CFP TCB CACHE becomes invalid by fifo_swap after wlc_fifo_index_get()
	 * Long term, this needs restructure about fifo_swap problem.
	 */
	if (!SCB_CFP_TCB_IS_CACHE_VALID(scb_cfp))
		goto release_done;
	...
	if (wlc->lifetime_txfifo && D11REV_GE(wlc->pub->corerev, 128)) {
		txfifo_expiry = wlc_lifetime_now(wlc) +
			(wlc->lifetime_txfifo << D11_REV128_LIFETIME_SHIFT);
	}
	...
	while (continue_release && (pkt = pktq_pdeq(&scb_ampdu->txq, tid))) {
#if defined(BCM_DHDHDR) && defined(DONGLEBUILD)
		ASSERT(D11BUF_POOL_AVAIL());
#endif // endif
		ASSERT(PKTPRIO(pkt) == tid);

#ifdef WLTAF
		if (taf && taf->how == TAF_RELEASE_LIKE_IAS && TAF_IS_TAGGED(WLPKTTAG(pkt))) {
			/* re-sending packets previously released by TAF is design error */
			ASSERT(0);
			wlc_taf_txpkt_status(wlc->taf_handle, scb, tid, pkt,
				TAF_TXPKT_STATUS_REQUEUED);
		}
#endif /*  WLTAF */
		...
		if (!PKTISCFP(pkt)) {
			PKTSETCFPFLOWID(pkt, SCB_FLOWID(scb));
		}
		...
		if (amsdu_in_ampdu) {
			uint32 peek_pkt_totlen = 0, pkt_totlen = 0;
			...
			if ((peek_pkt != NULL) | (!is_amsdu_sf0)) {
#if defined(DONGLEBUILD)
				/* Don't expect any packets with PKTNEXT assigned */
				ASSERT(PKTNEXT(PKT_OSH_NA, pkt) == NULL);
				ASSERT(BCMLFRAG_ENAB() & PKTISFRAG(PKT_OSH_NA, pkt));

				pkt_totlen = PKTLEN(PKT_OSH_NA, pkt) +
					PKTFRAGTOTLEN(PKT_OSH_NA, pkt);

				if (peek_pkt)
					peek_pkt_totlen = PKTLEN(PKT_OSH_NA, peek_pkt) +
						PKTFRAGTOTLEN(PKT_OSH_NA, peek_pkt);
#else
				/*
				 * In NIC mode, packet may arrive as multiple fragments.
				 * In EAP, we tend to not have access to the tunnel stack,
				 * and tier-1 customers may design their stack knowing that
				 * WLAN handles PKTNEXT based fragments.
				 */
				pkt_totlen = pkttotlen(PKT_OSH_NA, pkt);
				if (peek_pkt)
					peek_pkt_totlen = pkttotlen(PKT_OSH_NA, peek_pkt);
#endif /* DONGLEBUILD */
				...
				if (is_amsdu_sf0) {
					...
					if (pad) {
						 ...
						 * end of host data
						 */
						PKTSETFRAGLEN(PKT_OSH_NA, sf1, LB_FRAG1,
							PKTFRAGLEN(PKT_OSH_NA, sf1, LB_FRAG1) +
							pad);
						 ...
						 */
						PKTSETLEN(PKT_OSH_NA, sf1, sf1_totlen + pad);
#endif /* !BCM_DHDHDR */
					}

					PKTSETNEXT(PKT_OSH_NA, pktlast(PKT_OSH_NA, sf1), pkt);
					totlen += pad + pkt_totlen;
					msdu_cnt++;
					sf1 = pkt;
#if !defined(BCM_DHDHDR)
					sf1_totlen = pkt_totlen;
					 ...
					 * We need to maintain histogram for debug build.
					 * So update inside the loop. Otherwise update after
					 * the release loop
					 */
					wlc_cfp_amsdu_tx_counter_histogram_upd(
						wlc, msdu_cnt, totlen);
#else
					/* Total msdu count should be same as deq_cnt */
					tot_amsdu_cnt++;    /* Total AMSDU count */
#endif /* BCMDBG || BCMDBG_AMSDU */
				}
			}
		...
		if (use_atf) {
			...
			if (PKTISFRAG(PKT_OSH_NA, pkt))
				pktbytes += PKTFRAGTOTLEN(PKT_OSH_NA, pkt);
		...
		d11buf_avail--;
#ifdef WLTAF
		if ((taf && taf->complete == TAF_REL_COMPLETE_NULL) & (d11buf_avail == 0)) {
			taf->complete = TAF_REL_COMPLETE_NO_BUF;
		}
#endif /* WLTAF */
#endif /* BCM_DHDHDR && DONGLEBUILD */
		...
		*/
		...
		spktq_enq(&temp_q, pkt);

		mpdu_bytes += rtc.mpdu_bytes;
		/* Release one mpdu at a time if
		 * 1. SCB_PS is true
		 * 2. Header cache is invalidated -
		 *    It could be invalidated while
		 *    processing first mpdu if rate_probe
		 *    is enabled in frameid. We should NOT
		 *    continue CFP path in such case
		 */
		if (SCB_PS(scb) | !SCB_CFP_TCB_IS_CACHE_VALID(scb_cfp)) {
			break;
		}
	} /* while continue_release loop */

#if !defined(BCMDBG) && !defined(BCMDBG_AMSDU)
	wlc_cfp_amsdu_tx_counter_upd(wlc, deq_cnt, tot_amsdu_cnt);
#endif /* !BCMDBG && !BCMDBG_AMSDU */

	ini->tx_in_transit += mpdu_cnt;
	ampdu_tx->tx_intransit += mpdu_cnt;

	SCB_CFP_PKT_CNT(scb_cfp, tid) += mpdu_cnt;
	SCB_PKTS_INFLT_FIFOCNT_ADD(scb, tid, mpdu_cnt);

	/* In CFP path all packets should be cached */
	WLCNTADD(wlc->pub->_cnt->txchit, mpdu_cnt);
	...
	WL_AMPDU_TX(("%s: fifo %d count %d epoch %d txpktpend %d",
		__FUNCTION__, fifo, mpdu_cnt, ampdu_tx->hwagg[fifo].epoch,
		TXPKTPENDGET(wlc, fifo)));
	...
	PKTCNTR_INC(wlc, PKTCNTR_CFP_AMPDU_RELEASE, mpdu_cnt);
	...
	wlc_update_txpktsuccess_stats(wlc, scb, 0, mpdu_bytes, tid, mpdu_cnt);
	...
	if (out_of_order) {
...
#endif /* WLTAF */
		{
			 ...
			 */
			if (SCB_PS(scb)) {
				wlc_ampdu_release(ampdu_tx, scb_ampdu, ini, 1, 1, NULL);
			} else {
				WLC_CFP_AMPDU_AGG_COMPLETE(wlc, ampdu_tx, scb, scb_ampdu,
					ini, TRUE);
			}
		}
	...
	scb_ampdu_tid_ini_t *ini, const bool force_release)
{
	...
	fifo = wlc_fifo_index_get(wlc->fifo, scb, prio2ac[ini->tid]);
	...
	if (wlc->ps_scb_txfifo_blk) {
		/* When atleast one station is in PS with block fifo state then don't release
		 * more than psscb_release_cnt (default-256, 0-disabled).
		 */
		if ((wlc->pub->psscb_release_cnt != 0) &&
			(TXPKTPENDTOT(wlc) > wlc->pub->psscb_release_cnt)) {
			release_cnt = 0;
		}
	}
		...
		(force_release))
	{
		...
		if (wlc_cfp_tcb_is_EST(wlc, scb, ini->tid, NULL)) {
			wlc_cfp_ampdu_release(scb_cfp, ampdu_tx, scb_ampdu, ini,
				release_cnt,
	...
	scb_ampdu_tid_ini_t *ini, const bool force_release)
{
	...
	(void) wlc_cfp_ampdu_evaluate(scb_cfp, ampdu_tx, scb_ampdu, ini, force_release);
}
	...
	scb_cfp_t *scb_cfp)
{
	...
	if (enqueue_status < 0) {
		...
		PKTCNTR_INC(wlc, PKTCNTR_CFP_ENQ_SCB_AMPDU, pktlist_count);
	}
...
wlc_cfp_ampdu_ps_send(wlc_info_t *wlc, struct scb *scb, uint32 tid_bmp, uint32 extra_flags)
{
	...
	wlc_cfp_tcb_is_EST(wlc, scb, ini->tid, &scb_cfp);
	if (!scb_cfp) {
		return FALSE;
	}
	...
	WL_PS(("wl%d.%d: %s AID %d prio %d extra_flags 0x%x\n",
		wlc->pub->unit, WLC_BSSCFG_IDX(scb->bsscfg), __FUNCTION__,
		SCB_AID(scb), prio, extra_flags));
...
#ifdef WLSQS
bool
wlc_sqs_ampdu_capable(wlc_info_t *wlc, struct scb* scb, uint8 prio)
{
	scb_ampdu_tx_t *scb_ampdu;
	ampdu_tx_info_t *ampdu_tx = wlc->ampdu_tx;
	ampdu_tx_config_t *ampdu_tx_cfg;
...
uint16
wlc_scb_ampdu_in_transit_pkts(wlc_info_t *wlc, struct scb* scb, uint8 prio)
{
	scb_ampdu_tx_t *scb_ampdu;
	scb_ampdu_tid_ini_t *ini = NULL;
...
uint16
wlc_scb_ampdu_n_pkts(wlc_info_t *wlc, struct scb* scb, uint8 prio)
{
	scb_ampdu_tx_t *scb_ampdu;
...
/* Return the real packets in scb ampdu Q */
uint16
wlc_scb_ampdu_tbr_pkts(wlc_info_t *wlc, struct scb* scb, uint8 prio)
{
	scb_ampdu_tx_t *scb_ampdu;
...
#ifdef PKTQ_LOG
void
wlc_ampdu_sqs_pktq_log(wlc_info_t *wlc, struct scb* scb, uint8 prio, bool rewind,
	uint16 v_pkts_tot, uint16 v_pkts)
{
	...
	/* Log is updated for AMPDU traffic only. */
	if (pktq->pktqlog) {
		...
		if (prec_cnt && v_pkts_tot > prec_cnt->sqsv_max_used) {
			WLCNTSET(prec_cnt->sqsv_max_used, v_pkts_tot);
		}
	}
}
#endif /* PKTQ_LOG */
	...
	uint8 prio, bool force, taf_scheduler_public_t* taf)
{
	...
	aggsf = amsdu_in_ampdu ? wlc_amsdu_scb_max_sframes(wlc->ami, scb) : 1;

	ampdu_pktq_n_pkts = pktqprec_n_pkts(&scb_ampdu->txq, prio);
	ampdu_pktq_total_pkts = SQS_AMPDU_PKTLEN(flowid, &scb_ampdu->txq, prio);
	...
	WL_AMPDU_TX(("%s: qlen<%u> nptks<%u>\n", __FUNCTION__, qlen,
		scb_ampdu->txq.q[prio].n_pkts));
	if (qlen == 0) {
		goto eval_done;
	}
	...
	if (!AMPDU_AQM_ENAB(ampdu_tx->wlc->pub) &&
		...
		wlc_wlfc_suppr_status_query(ampdu_tx->wlc, scb)) {
		wlc_ampdu_flush_ampdu_q(ampdu_tx, SCB_BSSCFG(scb));
		goto eval_done;
	}
#endif /* PROP_TXSTATUS */
	...
	if (AMPDU_AQM_ENAB(wlc->pub)) {
		...
		if (ps_pretend_limit_transit && !force) {
			...
			limit_transit = (in_transit > ampdu_tx->aqm_max_release[ini->tid]);
		} else {
			limit_transit = FALSE;
		}
...
int
wlc_sqs_ampdu_admit(wlc_info_t *wlc, uint16 flowid, uint8 prio, uint16 v_pkts)
{
	int v2r_request = 0;
	...
	/* Initialize */
	scb_cfp = wlc_scb_cfp_id2ptr(wlc, flowid);
	...
	aggsf = amsdu_in_ampdu ? wlc_amsdu_scb_max_sframes(wlc->ami, scb_ampdu->scb) : 1;
	...
	if (pktlist_emit != NULL) {
		...
		if (CFP_ENAB(wlc->pub) &&
			...
			/* Give one more chance to resume CFP fast path */
			if (wlc_cfp_tcb_is_EST(wlc, scb, ini->tid, NULL)) {
				wlc_cfp_ampdu_release(scb_cfp, wlc->ampdu_tx, scb_ampdu, ini,
		...
		uint16 succ_mpdu, uint16 succ_msdu, uint8 amsdu_sf)
{
	...
	for (r = 0; r < RATESEL_MFBR_NUM; r++) {
		...
		WLSCB_HISTO_TX_IF_HTVHTHE(scb, rs_txs->txrspec[r], txsucc_cnt);
	...
	if (distr_cnt) {
		...
		WLSCB_HISTO_TX_IF_HTVHTHE(scb, rs_txs->txrspec[0], distr_cnt);
	}
...
wlc_ampdu_taf_release(void* ampduh, void* scbh, void* tidh, bool force, taf_scheduler_public_t* taf)
{
	...
	if (!wlc_scb_restrict_can_txeval(wlc)) {
		WL_AMPDU(("%s: wlc_scb_restrict_can_txeval returns FALSE\n", __FUNCTION__));
	...
	/* Check for data fifo status before proceeding with release */
	if (wlc->block_datafifo | isset(wlc->hw->suspended_fifos, prio2fifo[ini->tid])) {
		taf->complete = TAF_REL_COMPLETE_BLOCKED;
		return FALSE;
	}

	qlen = pktqprec_n_pkts(&scb_ampdu->txq, tid);
	...
	if (taf->mode == TAF_RELEASE_MODE_REAL_FAST) {
		...
		if (CFP_ENAB(wlc->pub) && wlc_cfp_tcb_is_ini_valid(wlc, scb, tid, &scb_cfp) &&
			...
			if (n_mpdus_to_release > 0 && wlc_cfp_tcb_is_EST(wlc, scb, tid, NULL)) {
				cfp_pkts_released += wlc_cfp_ampdu_release(scb_cfp, ampdu_tx,
					scb_ampdu, ini,	n_mpdus_to_release,
					WLC_AMSDU_IN_AMPDU(wlc, scb, tid), taf);
	...
	if (pktqprec_n_pkts(&scb_ampdu->txq, tid) == 0) {
		if (taf->how == TAF_RELEASE_LIKE_IAS) {
			taf->ias.was_emptied = TRUE;
			taf->complete = taf->ias.is_ps_mode ?
				TAF_REL_COMPLETE_EMPTIED_PS : TAF_REL_COMPLETE_EMPTIED;
	...
	} else if ((n_mpdus_to_release + cfp_pkts_released > 0) &&
		(taf->complete == TAF_REL_COMPLETE_NOTHING)) {
		taf->complete = TAF_REL_COMPLETE_PARTIAL;
	}
	...
	int32 credits, bool add)
{
	...
	wl_pktfwd_update_link_credits(wlc->wl, flowid, (uint8 *) &scb->ea, prio, credits, add);
}
...
void
wlc_ampdu_pktqlog_enable(wlc_info_t* wlc, struct scb* scb)
{
	ampdu_tx_info_t *ampdu_tx = wlc->ampdu_tx;
	scb_ampdu_tx_t *scb_ampdu = SCB_AMPDU_TX_CUBBY(ampdu_tx, scb);
	int tid;
...
static bool
ampdu_dpstats_dump(wlc_info_t* wlc, scb_ampdu_tx_t *scb_ampdu, mac_log_counters_v06_t* mac_log,
	int tid, bool clear, uint32 timelo, uint32 timehi)
{
	scb_ampdu_dpdbg_t * ampdu_dpdbg = NULL;
	...
	if ((ampdu_dpdbg = ini->ampdu_dpdbg) == NULL) {
		ampdu_dpdbg = (scb_ampdu_dpdbg_t *)MALLOCZ(wlc->osh,
			sizeof(*ampdu_dpdbg));

		if (ampdu_dpdbg) {
			ini->ampdu_dpdbg = ampdu_dpdbg;
...
int
wlc_ampdu_dpstats_dump(wlc_info_t* wlc, struct scb* scb, wl_iov_pktq_log_t* iov_pktq,
	uint8 index, bool clear, uint32 timelo, uint32 timehi, uint32 tid_mask, uint32 flags)
{
	...
	for (tid = 0; tid < NUMPRIO; tid++) {
		...
		if (ampdu_dpstats_dump(wlc, scb_ampdu, mac_log, tid, clear, timelo, timehi)) {
			tid_mask |= (1 << tid);
		} else {
			tid_mask &= ~(1 << tid);
		}
...
static void
ampdu_dpstats_free(ampdu_tx_info_t *ampdu_tx, scb_ampdu_tx_t *scb_ampdu, int tid)
{
	scb_ampdu_dpdbg_t * ampdu_dpdbg = NULL;
...
void
wlc_ampdu_dpstats_free(wlc_info_t* wlc, struct scb* scb)
{
	ampdu_tx_info_t *ampdu_tx = wlc->ampdu_tx;
	scb_ampdu_tx_t *scb_ampdu = SCB_AMPDU_TX_CUBBY(ampdu_tx, scb);
	int tid;
...
/**
 * wlc_ht.c and wlc_vht.c call this function when encountering AMPDU parameters while
 * parsing IEs.
 */
void
wlc_ampdu_scb_set_params(wlc_info_t *wlc, struct scb *scb, uint8 mpdu_density,
	uint8 htvht_rxfactor, uint8 he_rxfactor_ext)
{
	...
	/* parameters are optional; -1 means not provided by caller */
	if (htvht_rxfactor != (uint8)-1) {
		scb_ampdu->htvht_rxfactor = htvht_rxfactor;
		update = TRUE;
	}

	if (he_rxfactor_ext != (uint8)-1) {
		scb_ampdu->he_rxfactor_ext = he_rxfactor_ext;
		update = TRUE;
	}

	if (update == TRUE) {
		scb_ampdu->max_rxfactor = AMPDU_RX_FACTOR_BASE_PWR + scb_ampdu->htvht_rxfactor +
			scb_ampdu->he_rxfactor_ext;

		ASSERT((scb_ampdu->max_rxfactor & ~D11_REV128_MAXRXFACTOR_MASK) == 0);
#ifdef AMPDU_NON_AQM
		/* max_rlen is only used for non-AQM chips */
		scb_ampdu->max_rxlen = MIN((1 << scb_ampdu->max_rxfactor), HE_MAC_AMPDU_MAX_LEN);
#endif /* AMPDU_NON_AQM */
	}

	if (mpdu_density != (uint8)-1) {
		scb_ampdu->mpdu_density = MAX(mpdu_density, ampdu_tx_cfg->mpdu_density);
	}
}

/* This function is used to reset scb ampdu session dead_cnt */
void
wlc_ampdu_scb_reset_alive(wlc_info_t * wlc, struct scb *scb)
{
	ampdu_tx_info_t *ampdu_tx = wlc->ampdu_tx;
	scb_ampdu_tx_t *scb_ampdu = SCB_AMPDU_TX_CUBBY(ampdu_tx, scb);
	int idx;
