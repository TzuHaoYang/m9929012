 ...
 *
 * $Id: wlc_macdbg.h 793901 2020-12-14 16:52:46Z $
 */
...
enum macdbg_psmwd_reason {
	WLC_MACDBG_PSMWD_AMPDU   = 0,
	WLC_MACDBG_PSMWD_TXBCN   = 1,
	WLC_MACDBG_PSMWD_MACSUSP = 2,
	WLC_MACDBG_PSMWD_ULMU    = 3
};
...
void wlc_macdbg_frameid_trace_pkt(wlc_macdbg_info_t *macdbg, void *pkt, d11txhdr_t *txh,
...
void wlc_macdbg_frameid_trace_sync_start(wlc_macdbg_info_t *macdbg);
...
extern void wlc_macdbg_dtrace_log_str(wlc_macdbg_info_t *macdbg, struct scb *scb,
	const char *format, ...);
extern int wlc_macdbg_psmwd_trigger(wlc_macdbg_info_t *macdbg, uint16 reason);
...
extern void wlc_macdbg_txs_ulmu_info(wlc_macdbg_info_t *macdbg, tx_status_t* txs);
#else /* BCMDBG */
...
#define wlc_macdbg_psmwd_trigger(a, b) do {} while (0)
...
#define wlc_macdbg_txs_ulmu_info(a, b);
#endif /* BCMDBG */

#if defined(BCMDBG)
extern uint8 * wlc_macdbg_get_dmadump_buf(wlc_info_t *wlc);
#else
#define wlc_macdbg_get_dmadump_buf(a) NULL
#endif // endif
...
typedef struct {
	uint16 m2vsnd_cnt	: 10; // b[9:0]
	uint16 grp_idx		: 2;  // b[11:10]
	uint16 grp_ext		: 1;  // b[12]
	uint16 nusr_cand	: 2;  // b[14:13]
	uint16 sgi		: 1;  // b[15]
} grp_hist_info_t;
...
typedef union {
	uint16 u16;
	struct {
		uint16 txvidx		: 4; // b[3:0]
		uint16 fifo_grp_idx	: 4; // b[7:4]
		uint16 nssmcs		: 8; // b[15:8]
	};
} grp_hist_usr_t;

#include <packed_section_start.h>
typedef BWL_PRE_PACKED_STRUCT struct {
	grp_hist_info_t	info;		// w[0]
	uint16		txvidx_cand;	// w[1] 4bits per user up to 4 users
	grp_hist_usr_t	usr[4];		// w[5:2]
