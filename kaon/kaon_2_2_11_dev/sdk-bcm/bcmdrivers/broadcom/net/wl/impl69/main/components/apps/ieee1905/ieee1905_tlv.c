    ...
    while (pdmif != NULL) {
      ...
      *pbuf = pdmif->MediaSpecificInfoSize;
      ...
      if (pdmif->MediaSpecificInfoSize > 0) {
        i5WlCfgCreateMediaInfo(pdmif->InterfaceId, pdmif->bssid, pdmif->chanspec,
          pdmif->mapFlags, pdmif->MediaSpecificInfo);
...
int i5TlvAPRadioBasicCapabilitiesTypeExtract(i5_message_type *pmsg, i5_dm_device_type *pdevice)
{
  ...
  while ((rc = i5MessageTlvExtract(pmsg, i5TlvAPRadioBasicCapabilitiesType,
    ...
    if (length >= MAC_ADDR_LEN + 2) {
      ...
      /* Update the Non-operable Channels of an interface, STATIC (0) */
      if (i5_config.cbs.nonoperable_channel_update) {
        i5_config.cbs.nonoperable_channel_update(pdmif, 0);
      }
    } else {
      rc = -1;
      goto end;
...
int i5TlvChannelPreferenceTypeExtract(i5_message_type *pmsg, int isAgent)
{
  ...
  while ((i5MessageTlvExtract(pmsg, i5TlvChannelPreferenceType,
    ...
    if (isAgent && (pdmif->msg_stat_flag == I5_MSG_RECEIVED ||
      pdmif->msg_stat_flag == I5_MSG_DECLINED)) {
      i5Trace("Channel already set by vendor TLV for " I5_MAC_DELIM_FMT
        ". Ignore prefernece TLV\n", I5_MAC_PRM(mac));
      continue;
    }

    if (I5_IS_BSS_STA(pdmif->mapFlags) && (pdmif->roam_enabled == TRUE)) {
      i5Trace("Ignoring channel selection since the interface "I5_MAC_DELIM_FMT
        " is active sta\n", I5_MAC_PRM(mac));
      pdmif->msg_stat_flag = I5_MSG_DECLINED;
      continue;
    }
    ...
    for (i = 0; (i < cp->rc_count) && (idx + 1 < length);) {
      for (k = 0; (k < reg_class_count); k++) {
        if (pvalue[idx] == rc_chan_map[k].regclass)
          break;
      }
      if (k == reg_class_count) {
        /* In certification tests they have a testcase to test the invalid rclass/channel. So,
         * if the certification is enabled send error report. In actual field we cannot do this
         * bcoz new global rclasses got added
         */
        if (I5_IS_MAP_CERT(i5_config.flags)) {
          i5TraceError("Invalid regclass: %d\n", pvalue[idx]);
          goto end;
        } else {
          i5TraceError("Ignoring unsupported regclass: %d\n", pvalue[idx]);
          /* regclass(1byte) + channel count(1byte) + pref & reason(1 byte) +
           * channels(count bytes)
           */
          idx += (3 + pvalue[idx + 1]) ;
          continue;
        }
      ...
      if (pvalue[idx] >  IEEE1905_MAX_RCCHANNELS) {
        i5TraceError("Channel per regclass count %d is more than max allowed channels %d  \n",
        pvalue[idx], IEEE1905_MAX_RCCHANNELS);
        cp->rc_map[i].count = IEEE1905_MAX_RCCHANNELS;
      } else {
        cp->rc_map[i].count = pvalue[idx];
      }
      real_chan_count = pvalue[idx];
      idx++;

      chan_count = rc_chan_ptr->count;
      chan_list = rc_chan_ptr->channel;

      for (j = 0, m = 0; (j < cp->rc_map[i].count) && (idx + 1 < length) && (m < real_chan_count); m++) {
        ...
        if (k == chan_count) {
          /* In certification tests they have a testcase to test the invalid rclass/channel. So,
           * if the certification is enabled send error report. In actual field we cannot do this
           * bcoz new channels got added to the existing rclass
           */
          if (I5_IS_MAP_CERT(i5_config.flags)) {
            i5TraceError("Invalid channel: %d\n", pvalue[idx]);
            goto end;
          } else {
            i5TraceError("Ignoring unsupported channel: %d\n", pvalue[idx]);
            idx++;
            continue;
          }
        }
      ...
      if (m < real_chan_count) {
        idx += (real_chan_count - m);
      }
      if (idx > length)
        break;
...
#if defined(MULTIAPR2)
      /* Clear DFS channel, as indicated by the controller */
      if (isAgent && i5_config.cbs.set_dfs_chan_clear &&
        (cp->rc_map[i].pref != 0) &&
        (cp->rc_map[i].reason == I5_REASON_CHAN_CLEAR_IND)) {
	      i5_config.cbs.set_dfs_chan_clear(pdmif->ifname, &cp->rc_map[i]);
      }
#endif /* MULTIAPR2 */
      i++;
    }
    ...
    if (!isAgent) {
      ...
      /* Update the Non-operable Channels of an interface, DYNAMIC (1) */
      if (i5_config.cbs.nonoperable_channel_update) {
        i5_config.cbs.nonoperable_channel_update(pdmif, 1);
      }
    }
  }
  return 0;
...
int i5TlvAssociatedSTATrafficStatsTypeInsert(i5_message_type *pmsg, unsigned char *mac,
  ieee1905_sta_traffic_stats *traffic_stats, i5_dm_device_type *pDestDevice)
{
  ...
  if (I5_P2APCAP_COUNTER_UNITS_GET(byte_cntr_unit) == I5_P2APCAP_COUNTER_UNITS_KBYTES) {
    bytes_sent = (unsigned int)(traffic_stats->bytes_sent / 1000);
    bytes_recv = (unsigned int) (traffic_stats->bytes_recv / 1000);
  } else if (I5_P2APCAP_COUNTER_UNITS_GET(byte_cntr_unit) == I5_P2APCAP_COUNTER_UNITS_MBYTES) {
    bytes_sent = (unsigned int) (traffic_stats->bytes_sent / (1000 * 1000));
    bytes_recv = (unsigned int) (traffic_stats->bytes_recv/ (1000 * 1000));
  } else {
    bytes_sent = (unsigned int)traffic_stats->bytes_sent;
    bytes_recv = (unsigned int)traffic_stats->bytes_recv;
  }
  ...
  ieee1905_chscan_req_msg *chscan_req)
{
  ...
  for (idx_radio = 0; idx_radio < chscan_req->num_of_radios; idx_radio++) {
    ...
    /* Save : Timestamp when { FRESH } Channel Scan Request Received */
    radio_info->ts_scan_recvd = time(NULL);

    /* Save : Type of Scan { STORED } / { FRESH } */
    radio_info->scan_type = (chscan_req->chscan_req_msg_flag & MAP_CHSCAN_REQ_FRESH_SCAN) ?
      I5_CHSCAN_REQ_FRESH : I5_CHSCAN_REQ_STORED;
	...
	/* Extract Channel Value */
        ...
        if (opclass_info->supported) {
          ...
          if (i5DmISChannelSupported(pdmif, opclass_info->opclass_val,
            opclass_info->chan_list[idx_chan])) {

            setbit(&opclass_info->supported_chan_list, idx_chan);
            setbit(&opclass_info->scan_required, idx_chan);
  ...
  ieee1905_chscan_report_msg *chscan_rpt)
{
  ...
  while ((i5MessageTlvExtract(pmsg, i5TlvChannelScanResultType,
    ...
    emt_p = i5DmFindChannelInScanResult(chscan_rpt, extrt_opclass, extrt_channel);
...
int i5TlvTunneledTypeInsert(i5_message_type *pmsg, unsigned char *payload, uint32 payload_len)
{
  ...
  if (payload_len > I5_MAX_TLV_PAYLOAD_LEN) {
    /* Unlikely situation */
  ...
  i5_dm_device_type *pDestDevice)
{
  ...
  i5_tlv_t *ptlv = (i5_tlv_t *)buf;
  int len = 0;
  unsigned int unicastBytesSent, unicastBytesReceived, multicastBytesSent, multicastBytesReceived;
  unsigned int broadcastBytesSent, broadcastBytesReceived;
  unsigned char byte_cntr_unit = pDestDevice->byte_cntr_unit;
  ...
  if (I5_P2APCAP_COUNTER_UNITS_GET(byte_cntr_unit) == I5_P2APCAP_COUNTER_UNITS_KBYTES) {
    unicastBytesSent = (unsigned int)(pbss->APMetric.unicastBytesSent / 1000);
    unicastBytesReceived = (unsigned int)(pbss->APMetric.unicastBytesReceived / 1000);
    multicastBytesSent = (unsigned int)(pbss->APMetric.multicastBytesSent / 1000);
    multicastBytesReceived = (unsigned int)(pbss->APMetric.multicastBytesReceived / 1000);
    broadcastBytesSent = (unsigned int)(pbss->APMetric.broadcastBytesSent / 1000);
    broadcastBytesReceived = (unsigned int)(pbss->APMetric.broadcastBytesReceived / 1000);
  } else if (I5_P2APCAP_COUNTER_UNITS_GET(byte_cntr_unit) == I5_P2APCAP_COUNTER_UNITS_MBYTES) {
    unicastBytesSent = (unsigned int)(pbss->APMetric.unicastBytesSent / (1000 * 1000));
    unicastBytesReceived = (unsigned int)(pbss->APMetric.unicastBytesReceived / (1000 * 1000));
    multicastBytesSent = (unsigned int)(pbss->APMetric.multicastBytesSent / (1000 * 1000));
    multicastBytesReceived = (unsigned int)(pbss->APMetric.multicastBytesReceived / (1000 * 1000));
    broadcastBytesSent = (unsigned int)(pbss->APMetric.broadcastBytesSent / (1000 * 1000));
    broadcastBytesReceived = (unsigned int)(pbss->APMetric.broadcastBytesReceived / (1000 * 1000));
  } else {
    unicastBytesSent = (unsigned int)pbss->APMetric.unicastBytesSent;
    unicastBytesReceived = (unsigned int)pbss->APMetric.unicastBytesReceived;
    multicastBytesSent = (unsigned int)pbss->APMetric.multicastBytesSent;
    multicastBytesReceived = (unsigned int)pbss->APMetric.multicastBytesReceived;
    broadcastBytesSent = (unsigned int)pbss->APMetric.broadcastBytesSent;
    broadcastBytesReceived = (unsigned int)pbss->APMetric.broadcastBytesReceived;
  }

  memcpy(&buf[len], pbss->BSSID, MAC_ADDR_LEN);
  len += MAC_ADDR_LEN;
  *((unsigned int *)&buf[len]) = htonl(unicastBytesSent);
  len += 4;
  *((unsigned int *)&buf[len]) = htonl(unicastBytesReceived);
  len += 4;
  *((unsigned int *)&buf[len]) = htonl(multicastBytesSent);
  len += 4;
  *((unsigned int *)&buf[len]) = htonl(multicastBytesReceived);
  len += 4;
  *((unsigned int *)&buf[len]) = htonl(broadcastBytesSent);
  len += 4;
  *((unsigned int *)&buf[len]) = htonl(broadcastBytesReceived);
  len += 4;
...
int i5TlvAPExtendedMetricsTypeExtract(i5_message_type *pmsg, i5_dm_device_type *pdevice)
{
  ...
  while (i5MessageTlvExtract(pmsg, i5TlvAPExtendedMetricsType, &length, &pvalue,
    ...
    pvalue += 4;
    i5Trace("Device["I5_MAC_DELIM_FMT"] BSS["I5_MAC_DELIM_FMT"] unicastBytesSent[%llu] "
      "unicastBytesReceived[%llu] multicastBytesSent[%llu] multicastBytesReceived[%llu] "
      "broadcastBytesSent[%llu] broadcastBytesReceived[%llu]\n",
...
int i5TlvCacCompletionTypeExtract(i5_message_type *pmsg)
{
  ...
  int i, j;
  uint8 n_radio;
  ieee1905_cac_completion_list_t *cac_list = NULL;
  int ret = -1;

  if (i5MessageTlvExtract(pmsg, i5TlvCACCompletionReportType, &length, &pvalue,
    i5MessageTlvExtractWithReset) == 0) {

    n_radio = *pvalue++;

    if (length < (i5TlvCACCompletionType_Min_Length * n_radio + 1)) {
      i5TraceError("CAC completion TLV is too small(Length: %d, min. required: %d)\n",
        length, (i5TlvCACCompletionType_Min_Length * n_radio));
      goto end;
    }

    if ((cac_list = (ieee1905_cac_completion_list_t *)malloc(length)) == NULL) {
      i5TraceDirPrint("Out of memory error\n");
      goto end;
    }
    cac_list->count = n_radio;

    for(i = 0; i < n_radio; i++) {
      ieee1905_radio_cac_completion_t *cac_compl = &cac_list->params[i];

      memcpy(cac_compl->mac, pvalue, MAC_ADDR_LEN);
      pvalue += MAC_ADDR_LEN;
      i5TraceInfo("CAC completion on interface["I5_MAC_DELIM_FMT"]\n",
        I5_MAC_PRM(cac_compl->mac));

      cac_compl->opclass = *pvalue++;
      cac_compl->chan = *pvalue++;
      cac_compl->status = *pvalue++;
      cac_compl->n_opclass_chan = *pvalue++;
      i5TraceInfo("Opclass[%d] Channel[%d] CAC completion status [%d] radar pairs count [%d]\n",
        cac_compl->opclass, cac_compl->chan, cac_compl->status, cac_compl->n_opclass_chan);

      if ((cac_compl->status == MAP_CAC_STATUS_RADAR_DETECTED) &&
        (cac_compl->n_opclass_chan == 0)) {
        i5TraceError("Number of pairs should at least 1, cac status is radar detected. "
	  "n_pairs = %d\n", cac_compl->n_opclass_chan);
	goto end;
      } else if ((cac_compl->status != MAP_CAC_STATUS_RADAR_DETECTED) &&
        (cac_compl->n_opclass_chan != 0)) {
        i5TraceError("Number of pairs should be 0, cac status is not radar detected. "
	  "n_pairs = %d\n", cac_compl->n_opclass_chan);
	goto end;
      }

      for (j = 0; j < cac_compl->n_opclass_chan; j++) {
	opclass_chan_list *list = &cac_compl->list[j];

	list->op_class = *pvalue++;
	list->chan = *pvalue++;
        i5TraceInfo("Radar subband Opclass[%d] Channel[%d]\n", list->op_class, list->chan);
      }
      if (cac_compl->n_opclass_chan == 0) {
        pvalue += 2;
      }
    }
    if (i5_config.cbs.process_cac_complete) {
      i5_config.cbs.process_cac_complete(i5MessageSrcMacAddressGet(pmsg), cac_list);
    }
  }
  ret = 0;
...
int i5TlvCacCapabilitiesTypeExtract(i5_message_type *pmsg, i5_dm_device_type *pdevice)
{
  ...
  uint8 n_radio = 0;
  i5_dm_interface_type *pdmif;
  i5_dm_cac_capabilities_t *cac_cap;
  int min_length = 3;

  if (i5MessageTlvExtract(pmsg, i5TlvCACCapabilitiesType, &length, &pvalue,
    i5MessageTlvExtractWithReset) == 0) {
    if (length < min_length) {
      i5TraceError("CAC cabilities TLV is too small(Length: %d)\n", length);
      goto end;
    }

    pvalue += 2; /* Skipping the country code */
    n_radio = *pvalue++;

    min_length += 15 * n_radio;
    if (length < min_length) {
      i5TraceError("CAC cabilities TLV is too small(Length: %d Required: %d)\n",
        length, min_length);
      goto end;
    }

    for(i = 0; i < n_radio; i++) {
      pdmif = i5DmInterfaceFind(pdevice, pvalue);
      if (!pdmif) {
        i5TraceError("Device["I5_MAC_DELIM_FMT"] doesn't have an interface with mac address["
		I5_MAC_DELIM_FMT"]\n", I5_MAC_PRM(pdevice->DeviceId), I5_MAC_PRM(pvalue));
	goto end;
      }

      pvalue += MAC_ADDR_LEN;

      if (pdmif->ApCaps.cac_cap == NULL) {
        pdmif->ApCaps.cac_cap = (i5_dm_cac_capabilities_t *)malloc(sizeof(i5_dm_cac_capabilities_t));
	if (pdmif->ApCaps.cac_cap == NULL) {
          i5TraceDirPrint("Out of memory error\n");
	  goto end;
	}
      }
      cac_cap = pdmif->ApCaps.cac_cap;
      memset(cac_cap, 0, sizeof(*cac_cap));

      cac_cap->n_types = *pvalue++;

      min_length += 8 * (cac_cap->n_types - 1);
      if (length < min_length) {
        i5TraceError("CAC cabilities TLV is too small(Length: %d Required: %d)\n",
          length, min_length);
        goto end;
      ...
      for (j = 0; j < cac_cap->n_types; j++) {
	i5_dm_cac_types_t *cac_type = &cac_cap->types[j];

	cac_type->method = *pvalue++;
	MAP_ENABLE_CAC_METHOD(cac_cap->method_flag, cac_type->method);

	I5_SWAP(pvalue[0], pvalue[2], unsigned char);
	memcpy((uint8*)&(cac_type->time), pvalue, 3);
	pvalue += 3;

	cac_type->n_op_class = *pvalue++;

	min_length += 3 * (cac_type->n_op_class - 1);
        if (length < min_length) {
          i5TraceError("CAC cabilities TLV is too small(Length: %d Required: %d)\n",
            length, min_length);
          goto end;
	...
	for (k = 0; k < cac_type->n_op_class; k++) {
          i5_dm_op_class_list_t *opclass = &cac_type->opclasses[k];

	  opclass->opclass = *pvalue++;
	  opclass->n_chan = *pvalue++;

          min_length += (opclass->n_chan - 1);
          if (length < min_length) {
            i5TraceError("CAC cabilities TLV is too small(Length: %d Required: %d)\n",
              length, min_length);
            goto end;
	  ...
	  memcpy(opclass->chan, pvalue, opclass->n_chan);
	  pvalue += opclass->n_chan;
	}
      }
    }
  }
  if(length != min_length) {
    i5TraceError("CAC cabilities TLV length not correct(Length: %d Required: %d)\n",
      length, min_length);
    goto end;
  }
  return 0;
...
/* Function to extract CAC status */
int i5TlvCacStatusTypeExtract(i5_message_type *pmsg)
{
  ...
  pdevice = i5DmDeviceFind(i5MessageSrcMacAddressGet(pmsg));
  if (pdevice == NULL) {
    i5TraceError("Neighbour device["I5_MAC_DELIM_FMT"] does not exist\n",
      I5_MAC_PRM(i5MessageSrcMacAddressGet(pmsg)));
    goto end;
  }

  if (i5MessageTlvExtract(pmsg, i5TlvCACStatusReportType, &length, &pvalue,
    i5MessageTlvExtractWithReset) == 0) {

    n_chan = *pvalue++;
    min_length = n_chan * sizeof (ieee1905_opclass_chan_info_t);

    if (length < min_length) {
      i5TraceError("CAC Status TLV is too small(Length: %d, min. required: %d)\n",
        length, min_length);
      goto end;
    }

    buf_size = MAXCHANNEL * sizeof (ieee1905_opclass_chan_info_t) +
      sizeof(ieee1905_opclass_chan_info_list_t);
    if (pdevice->active_info == NULL) {
      pdevice->active_info = (ieee1905_opclass_chan_info_list_t *)malloc(buf_size);
    }
    if (pdevice->active_info == NULL) {
      i5TraceDirPrint("Out of memory error\n");
      goto end;
    }
    memset(pdevice->active_info, 0, buf_size);

    pdevice->active_info->n_count = n_chan;
    i5TraceInfo("Device ["I5_MAC_DELIM_FMT"] has %d channels available\n",
      I5_MAC_PRM(pdevice->DeviceId), n_chan);

    for(i = 0; i < n_chan; i++) {
      ieee1905_opclass_chan_info_t *info = &pdevice->active_info->info[i];
      ieee1905_opclass_chan_info_t *tmp = (ieee1905_opclass_chan_info_t *)pvalue;

      info->opclass = tmp->opclass;
      info->chan = tmp->chan;
      info->duration = ntohs(tmp->duration);
      pvalue += sizeof(*tmp);
    }
    n_chan = *pvalue++;
    i5TraceInfo("Device ["I5_MAC_DELIM_FMT"] has %d channels radar detected\n",
      I5_MAC_PRM(pdevice->DeviceId), n_chan);

    if (i5_config.cbs.process_cac_status) {
      i5_config.cbs.process_cac_status(pdevice);
    }
  }
  ret = 0;
