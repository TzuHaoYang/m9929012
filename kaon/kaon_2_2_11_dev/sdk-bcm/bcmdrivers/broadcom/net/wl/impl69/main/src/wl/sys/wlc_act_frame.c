 ...
 *
 * $Id: wlc_act_frame.c 793059 2020-11-12 04:49:08Z $
 */
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_AF): {
		...
		bool do_event = TRUE;
		wl_af_params_t *af = NULL; /**< the command parameters provided by user space */
		...
		if (af->channel != 0 && af->bandtype == 0) {
			/** currently, not all callers use the added 'bandtype' struct member */
			af->bandtype =
				(af->channel <= CH_MAX_2G_CHANNEL) ? WLC_BAND_2G : WLC_BAND_5G;
		}

		do {
			chanspec_t chspec20 = (WL_CHANSPEC_BW_20 | WL_CHANSPEC_BAND_2G);

			if (af->channel != 0) {
				chspec20 = wf_create_20MHz_chspec(af->channel,
					BANDTYPE_CHSPEC(af->bandtype));
			}

			/* use scan to send AF for quiet chanspec */
			if (wlc_quiet_chanspec(wlc->cmi, chspec20)) {
			...
			if (af->channel == 0 ||
			    chspec20 == wf_chspec_ctlchspec(phy_utils_get_chanspec(wlc->pi))) {
				af_continue = TRUE;
			}
...
int
wlc_msch_actionframe(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, chanspec_t chspec20, int reason,
	uint32 dwell_time, struct ether_addr *bssid, wlc_actframe_callback cb_func, void *arg)
{
	...
	chanspec_list[0] = (chspec20 == 0) ? phy_utils_get_chanspec(wlc->pi) : chspec20;
		...
		if (scb != NULL) {
			...
			*/
			if (SCB_ASSOCIATED(scb) && eacmp(&bsscfg->BSSID, &hdr->a3) != 0) {
				WL_ERROR(("wl%d: %s: "MACF": A3 does not match BSSID\n",
			...
			if (SCB_MFP(scb)) {
				if (cat == DOT11_ACTION_CAT_PUBLIC) {
					...
					if (wlc_is_protected_dual_publicaction(act, bsscfg)) {
						WL_ERROR(("wl%d: %s: "MACF": "
							"rx frame has category %d,  "
							"pub action field %d; should have cat %d\n",
							wlc->pub->unit, __FUNCTION__,
							ETHER_TO_MACF(scb->ea),
							cat, act, DOT11_ACTION_CAT_PDPA));
						return -1;
					}
				} else { /* pdpa */
					...
					if (!wlc_is_protected_dual_publicaction(act, bsscfg)) {
						WL_ERROR(("wl%d: %s: "MACF": "
							...
							wlc->pub->unit, __FUNCTION__,
							ETHER_TO_MACF(scb->ea),
							cat, act, DOT11_ACTION_CAT_PUBLIC));
						return -1;
					}
				}
			} else { /* !mfp */
				...
				if (cat == DOT11_ACTION_CAT_PDPA) {
					WL_ERROR(("wl%d: %s: "MACF": rx frame has category %d, "
						"pub action field %d; should have cat %d\n",
						wlc->pub->unit, __FUNCTION__,
						ETHER_TO_MACF(scb->ea),
						cat, act, DOT11_ACTION_CAT_PUBLIC));
	...
	wl_af_params_t *af, bool *do_event)
{
	...
	chanspec_t chanspec_list; /**< a list of just one chanspec to scan */
	wlc_ssid_t ssid;
	int err = BCME_OK;
	...
	chanspec_list = wf_create_20MHz_chspec(af->channel, BANDTYPE_CHSPEC(af->bandtype));
	err = wlc_scan_request(wlc, DOT11_BSSTYPE_ANY, &af->BSSID,
			1, &ssid, -1, 1, af->dwell_time, af->dwell_time,
			-1, &chanspec_list, 1, 0, FALSE, wlc_af_off_channel_callback, wlc,
			WLC_ACTION_SCAN, 0, bsscfg, wlc_af_send, bsscfg);

	if (err != BCME_OK) {
		WL_ERROR(("%s(): wlc_scan_request() failed,"
			"err = %d\n", __FUNCTION__, err));
	}
