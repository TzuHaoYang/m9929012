...
/* Bit mask for finding best backhaul STA based on weightage */
#define WBD_BEST_BSTA_WGHT_FLAG_RSSI		0x0001	/* RSSI */
#define WBD_BEST_BSTA_WGHT_FLAG_MAX_RATE	0x0002	/* Max Rate */
#define WBD_BEST_BSTA_WGHT_FLAG_STORED_BSSID	0x0004	/* Stored BSSID */
#define WBD_BEST_BSTA_WGHT_FLAG_ALL		(WBD_BEST_BSTA_WGHT_FLAG_RSSI | \
							WBD_BEST_BSTA_WGHT_FLAG_MAX_RATE | \
							WBD_BEST_BSTA_WGHT_FLAG_STORED_BSSID)

/* Predefined Best backhaul STA weightage */
static wbd_best_bsta_wght_t predefined_best_bsta_wght[] = {
	/* w_rssi w_max_rate w_stored_bssid flags */
	/* 0 : All weightage */
	{4, 4, 6, 0x07},
	/* End */
	{0, 0, 0, 0x00}
...
#define WBD_MAX_BEST_BSTA_WEIGHTAGE \
	(sizeof(predefined_best_bsta_wght)/sizeof(wbd_best_bsta_wght_t)-1)
...
wbd_slave_find_weak_client(wbd_slave_item_t *slave_item, int *found)
{
	...
	foreach_i5glist_item(i5_sta, i5_dm_clients_type, i5_bss->client_list) {
		...
		if (wbd_is_client_weak(slave_item, i5_sta) == TRUE) {
			...
			WBD_INFO("Band[%d] In BSS "MACF"  Found weak STA "MACDBG"\n",
			...
			if (sta_item->status == WBD_STA_STATUS_WEAK) {
				WBD_INFO("Band[%d] In BSS "MACF"  Weak STA "MACDBG"  became "
					"strong\n", slave_item->band,
	...
	unsigned char *interface_mac, ieee1905_backhaul_link_metric *metric)
{
	...
	WBD_INFO("Ifname[%s] MAC["MACDBG"] DWDS %s txPacketErrors %d transmittedPackets %d "
		"receivedPackets %d rxPacketErrors %d RCPI[%d (%d)] macThroughPutCapacity %d\n",
		...
		metric->rcpi, WBD_RCPI_TO_RSSI(metric->rcpi), metric->macThroughPutCapacity);
...
static int wbd_slave_erase_bss(char *vif_name, int bsscfg_idx)
{
	char prefix[IFNAMSIZ] = {0};
	int ret = -1;
	...
	/* Disable the BSS and remove it from IEEE1905 datamodel */
...
static int wbd_slave_erase_all_bss(char *ifname)
{
	...
	ret = wbd_slave_erase_bss(tmpvif, 0);
	...
	for (idx = 1; idx < WBD_MAX_POSSIBLE_VIRTUAL_BSS; idx++) {
		...
		if (find_in_list(lan_ifnames, tmpvif) ||
			...
			if (wbd_slave_erase_bss(tmpvif, idx) == 0) {
				ret = 0;
			}
		} else {
			break;
	...
	int is_primary, int band)
{
	...
	if ((bss->AuthType & IEEE1905_AUTH_SAE) && (band & BAND_6G)) {
		blanket_nvram_prefix_set(prefix, NVRAM_SAE_PWE, "1");
	}
...
no_match:
	WBD_INFO("prefix[%s] ssid[%s] key[%s] AuthType[%x] akm[%s] EncryptType[%x] crypto[%s] "
		"map_flag[0x%x] isClosed[%d] flags[%d] ret[%d]. %s\n",
		prefix, bss->ssid.SSID, bss->NetworkKey.key, bss->AuthType, akm,
		bss->EncryptType, crypto, bss->map_flag, bss->Closed, bss->flags, ret,
...
erase :
	...
	if (!backhaul_bss_found && !find_in_list(bh_sta_list, ifname)) {
		...
		/* Also add the ifname to NVRAM_MAP_BHSTA_IFNAMES */
...
apply:
	...
	foreach_iglist_item(iter_bss, ieee1905_client_bssinfo_type, *bssinfo_list) {
		...
		WBDSTRNCPY(lan_ifnames, blanket_nvram_safe_get(NVRAM_LAN_IFNAMES),
		...
		for (idx = 1; idx < WBD_MAX_POSSIBLE_VIRTUAL_BSS; idx++) {
			...
			if ((!find_in_list(lan_ifnames, tmpvif)) &&
				(!find_in_list(lan1_ifnames, tmpvif))) {
				wbd_slave_erase_bss(tmpvif, idx);
...
int
wbd_get_max_best_bsta_wght()
{
	return WBD_MAX_BEST_BSTA_WEIGHTAGE;
}

/* Get the weightage policy based on index */
wbd_best_bsta_wght_t*
wbd_get_best_bsta_wght(wbd_blanket_slave_t *bkt_slave)
{
	return &predefined_best_bsta_wght[bkt_slave->best_bsta_info.bsta_wght];
}

/* Get score of a backhaul STA */
static uint32
wbd_slave_get_bsta_score(i5_dm_interface_type *i5_ifr, uint32 check_rule_mask,
	wbd_best_bsta_wght_t *wght_cfg)
{
	int rssi = 0, n_rssi = 0;
	int phyrate = 0, n_phyrate = 0, n_stored_bssid = 0;
	uint32 node_score = 0;
	wbd_blanket_slave_t *wbd_slave = wbd_get_ginfo()->wbd_slave;
	char *nvval;
	...
	blanket_get_bssid(i5_ifr->ifname, &bssid);

	if (wght_cfg->flags & WBD_BEST_BSTA_WGHT_FLAG_RSSI) {
		int max_rssi = wbd_slave->best_bsta_info.bsta_max_rssi;
		int min_rssi = wbd_slave->best_bsta_info.bsta_min_rssi;
		...
		if (!WBD_RSSI_VALID(rssi)) {
			WBD_INFO("For BSTA["MACDBG"] invalid rssi(%d)\n",
				MAC2STRDBG(i5_ifr->InterfaceId), rssi);
			goto scoring;
		}
		 ...
		 * applications
		 */
		if (check_rule_mask == WBD_BEST_BSTA_WGHT_FLAG_RSSI) {
	...
	if (wght_cfg->flags & WBD_BEST_BSTA_WGHT_FLAG_MAX_RATE) {
		/* Find the normalized max rate */
		int max_phyrate = wbd_slave->best_bsta_info.bsta_max_phyrate;
		int min_phyrate = wbd_slave->best_bsta_info.bsta_min_phyrate;

		blanket_get_max_rate(i5_ifr->ifname, &phyrate);

		if (check_rule_mask == WBD_BEST_BSTA_WGHT_FLAG_MAX_RATE) {
	...
	if (wght_cfg->flags & WBD_BEST_BSTA_WGHT_FLAG_STORED_BSSID) {
		/* If the BSSID is stored for this backhaul STA, then give 100 */
		nvval = blanket_nvram_prefix_safe_get(i5_ifr->prefix, NVRAM_BSSID);
...
scoring:
	...
	node_score = ((wght_cfg->w_rssi * n_rssi)+
		(wght_cfg->w_max_rate * n_phyrate)+
		(wght_cfg->w_stored_bssid * n_stored_bssid));
	WBD_INFO("For %s:BSTA["MACDBG"] : weight_flags=0x%x, rssi=%d, "
		"n_rssi=%d, phyrate=%d, n_phyrate=%d, n_stored_bssid=%d, score=%d\n",
		i5_ifr->ifname, MAC2STRDBG(i5_ifr->InterfaceId), wght_cfg->flags,
		rssi, n_rssi, phyrate, n_phyrate, n_stored_bssid, node_score);
...
wbd_slave_choose_best_bh_sta()
{
	...
	wbd_best_bsta_ident_info_t *bsta_info;
	wbd_best_bsta_wght_t *wght_cfg = NULL;

	WBD_SAFE_GET_I5_SELF_DEVICE(i5_self_dev, &ret);

	bsta_info = &wbd_get_ginfo()->wbd_slave->best_bsta_info;
	wght_cfg = &bsta_info->wght_cfg;
	/* Get which rules are selected */
	check_rule_mask = ((wght_cfg->flags) & (WBD_BEST_BSTA_WGHT_FLAG_ALL));
	...
	foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type, i5_self_dev->interface_list) {
		...
		node_score = wbd_slave_get_bsta_score(i5_iter_ifr, check_rule_mask, wght_cfg);
		if (node_score > best_score) {
			best_score = node_score;
			best_ifr = i5_iter_ifr;
		} else if (node_score && (best_score == node_score) &&
			(i5_iter_ifr->band & bsta_info->preferred_band)) {
			best_ifr = i5_iter_ifr;
			WBD_INFO("Selecting %s as %x is matching with %x\n", i5_iter_ifr->ifname,
				i5_iter_ifr->band, bsta_info->preferred_band);
		}
	}
