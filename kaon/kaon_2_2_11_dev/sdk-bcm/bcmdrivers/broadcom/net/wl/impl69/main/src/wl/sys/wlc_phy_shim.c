 ...
 *
 * $Id: wlc_phy_shim.c 793797 2020-12-10 06:31:26Z $
 */
...
void
wlapi_bmac_write_ihr(wlc_phy_shim_info_t *physhim, uint offset, uint16 v)
{
	wlc_bmac_write_ihr(physhim->wlc_hw, (offset - PIHR_BASE) >> 1, v);
}

uint16
wlapi_bmac_read_ihr(wlc_phy_shim_info_t *physhim, uint offset)
{
	return wlc_bmac_read_ihr(physhim->wlc_hw, (offset - PIHR_BASE) >> 1);
}
...
{
	wlc_info_t *wlc = (wlc_info_t *)physhim->wlc;
	return wlc->primary_bsscfg->roam->time_since_bcn;
}
...
void
wlapi_bmac_upd_ifs(wlc_phy_shim_info_t *physhim)
{
	wlc_bmac_upd_ifs(physhim->wlc_hw);
}
