 ...
 *
 * $Id: wlc_dbg.c 791560 2020-10-01 08:23:54Z $
 */
...
wlc_print_dot11_mac_hdr(uint8* buf, int len)
{
	...
	if (kind == FC_RTS) {
		...
		printf("BSSID: %s TA: %s ", a1, a2);
		if (len < 2*ETHER_ADDR_LEN)
			printf("%s ", errstr);
	} else if (kind == FC_CF_END || kind == FC_CF_END_ACK) {
		printf("RA: %s BSSID: %s ", a1, a2);
		if (len < 2*ETHER_ADDR_LEN)
			printf("%s ", errstr);
	} else if (FC_TYPE(fc) == FC_TYPE_DATA) {
		if (!toDS) {
			if (!fromDS) {
				printf("DA: %s SA: %s BSSID: %s ", a1, a2, a3);
			} else {
				printf("DA: %s BSSID: %s SA: %s ", a1, a2, a3);
			}
		} else if (!fromDS) {
			printf("BSSID: %s SA: %s DA: %s ", a1, a2, a3);
		} else {
			printf("RA: %s TA: %s DA: %s ", a1, a2, a3);
		}
		if (len < 3*ETHER_ADDR_LEN) {
			printf("%s ", errstr);
		} else if (len < 20) {
			printf("SeqCtl: ------ %s ", errstr);
		} else {
	...
	if (FC_SUBTYPE_ANY_QOS(FC_SUBTYPE(fc)) && (fc & FC_ORDER)) {
		if (len < 4) {
			printf("HTC: ------ %s ", errstr);
		} else {
			v = buf[0] | (buf[1] << 8);
...
#define RATELINK_DUMP_SIZE	2048
	...
	printf("\tLinkMemIdxTID: 0x%x, RateMemIdx: 0x%x\n",
...
wlc_recv_print_rxh(wlc_info_t *wlc, wlc_d11rxhdr_t *wrxh)
{
	...
	if (D11REV_GE(corerev, 128) && !RXS_GE128_VALID_PHYRXSTS(&wrxh->rxhdr, corerev)) {
		phystatus_0 = 0;
		phystatus_1 = 0;
		phystatus_2 = 0;
		phystatus_3 = 0;
	} else {
		phystatus_0 = D11PHYRXSTS_ACCESS_VAL(rxh, corerev, PhyRxStatus_0);
		phystatus_1 = D11PHYRXSTS_ACCESS_VAL(rxh, corerev, PhyRxStatus_1);
		phystatus_2 = D11PHYRXSTS_ACCESS_VAL(rxh, corerev, PhyRxStatus_2);
		phystatus_3 = D11PHYRXSTS_ACCESS_VAL(rxh, corerev, PhyRxStatus_3);
	}
	...
	printf("RxFrameSize:   %6s (%d)%s\n", lenbuf, len,
		(phystatus_0 & PRXS_SHORTPMBL(corerev)) ? " short preamble" : "");
...
wlc_print_auth(wlc_info_t *wlc, struct dot11_management_header *mng, int len)
{
	...
	if (fc & FC_WEP) {
		body_len -= 8;	/* Skip WEP IV and ICV */
		body += 4;	/* Skip WEP IV */
	}
...
static const struct wlc_id_name_entry dot11_ie_names[] =
{
	...
	{DOT11_MNG_HE_OP_ID, "HE Operation"},
	{DOT11_MNG_EXT_CAP_ID, "Extended Capabilities"},
	{DOT11_MNG_REGCLASS_ID, "Supported Operating Classes"},
	{0, NULL}
};
...
void
wlc_print_dump_table(struct bcmstrbuf *b, char* title,
	uint32 *table_val, char* title_per, uint32 *tsuccess,
	int max_idx, int nbr_colums, table_type_t table_type)
{
#define MAX_SIZE 64
#define MAX_COLUM 12
	int32 table_val_pcent[MAX_SIZE];
	int32 table_per[MAX_SIZE];
	int32 table_per_pcent[MAX_SIZE];
	int32 table_colum[MAX_COLUM];
	int32 table_width[MAX_COLUM];
	uint col, value;
	int sum, last, titlelen = 0, unacked, pcent, i, pad;
	...
	for (i = 0, sum = 0, last = 0; i < max_idx; i++) {
		sum += table_val[i];
		if (table_val[i]) last = i;
	}

	if (!sum)
		return;

	/* Round up the last value to align with last column */
	last = nbr_colums * ((last/nbr_colums) + 1) - 1;

	for (col = 0; col < nbr_colums; col++) {
		table_colum[col] = 0;
		table_width[col] = 1;
	}

	/* Calculate Table Contain */
	for (i = 0; i <= last; i++) {
		col = (i % nbr_colums);
		value = table_val[i];
		pcent = PERCENT(sum, value);
		...
		if (tsuccess) {
			pcent = 0;
			unacked = value - tsuccess[i];
			if (unacked < 0) unacked = 0;
			if ((unacked > 0) && (value))
				pcent = PERCENT(value, unacked);
			table_per[i] = unacked;
	...
	if (title) {
		...
		if (title_per) {
			int length_title_per = strlen(title_per);
			if (length_title_per > titlelen)
				titlelen = length_title_per;
		}
	}
	...
	switch (table_type) {
		case TABLE_RU:
			bcm_bprintf(b, "%*s  ", titlelen, " ");
			for (col = 0; col < nbr_colums; col++) {
				char name[10];
				sprintf(name, "RU%d", wf_he_ru_type_to_tones(col));
				pad = 4 + table_width[col];
				bcm_bprintf(b, "%*s  ", pad, name);
			}
			bcm_bprintf(b, "\n");
			break;
		...
		case TABLE_MCS:
			bcm_bprintf(b, "%*s  ", titlelen, " ");
			for (col = 0; col < nbr_colums; col++) {
				pad = 3 + table_width[col];
				if (col > 9) pad--;

				bcm_bprintf(b, "%*s%d  ", pad, "MCS", col);
			}
			bcm_bprintf(b, "\n");
			break;
		...
		case TABLE_BW:
			...
			value = 10;
			for (col = 0; col < nbr_colums; col++) {
				pad = 2 + table_width[col];
				if (col > 9) pad--;
				value *= 2;

				bcm_bprintf(b, "%*s%d  ", pad, "BW", value);
			}
			bcm_bprintf(b, "\n");
			break;
	...
	bcm_bprintf(b, "%*s: ", titlelen, title);
	...
	for (i = 0; i <= last; i++) {
		...
		bcm_bprintf_val_pcent(b, table_val[i],
			table_val_pcent[i], table_width[col]);
	}
	bcm_bprintf(b, "\n");

	if (tsuccess) {
		bcm_bprintf(b, "%*s: ", titlelen, title_per);
		for (i = 0; i <= last; i++) {
			col = (i % nbr_colums);
			bcm_bprintf_val_pcent(b, table_per[i],
				table_per_pcent[i], table_width[col]);
			if ((col == (nbr_colums - 1)) && (i != last)) {
				bcm_bprintf(b, "\n");
				bcm_bprintf(b, "%*s: ", titlelen, " ");
			}
		}
