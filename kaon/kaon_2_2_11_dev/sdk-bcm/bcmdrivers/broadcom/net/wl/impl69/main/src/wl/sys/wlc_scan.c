 ...
 *
 * $Id: wlc_scan.c 793903 2020-12-14 18:06:08Z $
 */
...
#define WLC_SCAN_DFS_AUTO_REDUCE	0	/* Allow scan on DFS ch when active time/passive
						 ...
						 * time
						 */
#define WLC_SCAN_DFS_DELAY		25	/* Scan is in the vicinity of the radar poll
						  (130 to 150 ms) postphone the scan to 35ms
						*/
	...
	struct ether_addr *sa_override)
{
	...
	if (!(scan_flags & WL_SCANFLAGS_SWTCHAN) &&
		PROXD_ENAB(scan_info->wlc->pub) &&
		wlc_ftm_num_sessions_inprog(wlc_ftm_get_handle(scan_info->wlc))) {
		WL_SCAN_ERROR(("wl%d %s: Reject scan due to FTM in progress\n",
			scan_info->unit, __FUNCTION__));
		ret_val = BCME_SCANREJECT;
		status = WLC_E_STATUS_NEWSCAN;
		goto fail_with_cb;
	...
	int bandinfo)
{
	...
	{
#ifdef WL_SCAN_DFS_HOME
		if (scan_info->dfs_delay) {
			scan_info->state = WLC_SCAN_STATE_PARTIAL;
			WLC_SCAN_ADD_TIMER(scan_info, WLC_SCAN_DFS_DELAY, 0);
		} else
#endif /* WL_SCAN_DFS_HOME */
		{
