 ...
 *
 * $Id: acs.c 792738 2020-11-03 08:25:58Z $
 */
...
/* TODO: Need to revisit 6G policy. Currently 6G policy is a copy of 2G policy */
/* threshld    Channel score weigths values                                      chan */
/* bgn  itf  {  BSS  BUSY  INTF I-ADJ   FCS TXPWR NOISE TOTAL   CNS   ADJ TXOP  DFS,
RBSS, RTXOP}, pick
 */
/* --- ----   ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----  ---  func */
{ -65,  40, {   10,    1,    1,    0,    0,    0,    1,    1,    1,    0,  10,	 0,
5,	5},	NULL}, /* 2G */
{   0, 100, {   10,    0,    1,    1,    0,   10,    1,    1,    1,    1,  10,   5,
5,	5},	NULL}, /* 5G */
{   0, 100, {   10,    0,    1,    1,    0,   10,    1,    1,    1,    1,  10,   0,
5,    5},	NULL}, /* 6G */
{ -55,  45, { -200,    0, -100,  -50,    0,    0,  -50,    0,    1,    0,  -1,   0,
0,    0},	NULL}, /* CUST */
};
...
acs_get_rs_info(acs_chaninfo_t * c_info, char* prefix)
{
	...
	if (str && (strcmp(str, "0") && strcmp(str, "2g0") &&
			strcmp(str, "5g0") && strcmp(str, "6g0"))) {
...
/*
 * acs_pick_chanspec_default6g() - default policy function to pick a chanspec to switch to.
 *
 * c_info:	pointer to the acs_chaninfo_t for this interface.
 * bw:		bandwidth to chose from
 *
 ...
 */
static chanspec_t
acs_pick_chanspec_default6g(acs_chaninfo_t* c_info, int bw)
{
	/* TODO : Need to modify acs_pick_chanspec_common for 6GHz. */
	return acs_pick_chanspec_common(c_info, bw, CH_SCORE_TOTAL);
}
...
acs_default_policy(acs_policy_t *a_pol, acs_policy_index index)
{
	...
	} else if (index == ACS_POLICY_DEFAULT6G) {
		a_pol->chan_selector = acs_pick_chanspec_default6g;
	} else {
...
void acs_set_initial_chanspec(acs_chaninfo_t *c_info)
{
	...
	if (c_info->rs_info.pref_chspec == 0) {
		if (c_info->country_is_edcrs_eu) {
			/* Not allowing acsd to switch to non-dfs channel when
			 * country is set in ETSI country codes
			 */
			return;
		}
		...
		if (c_info->rs_info.band_type == WLC_BAND_5G) {
			...
			if (dis_ch_grp == WL_RADIO_CHAN_5GL) {
				...
				/* Assume that valid non-DFS channel is already set */
				return;
			}
...
static void
acs_get_txpwr_for_allchans(acs_chaninfo_t* c_info)
{
	wl_chanspec_txpwr_max_t params = {0};
	char *txpwrbuf;
	int i, ret = 0, buflen = ACS_TXPWR_BUF_LEN;

	txpwrbuf = acsd_malloc(ACS_TXPWR_BUF_LEN);
	if (!txpwrbuf) {
		ACSD_DEBUG("%s: Memory allocation failed \n", c_info->name);
		c_info->fallback_legacy_txpwr = TRUE;
		return;
	}
	c_info->txpwr_params = (wl_chanspec_txpwr_max_t *) txpwrbuf;

	params.ver = htod16(WL_CHANSPEC_TXPWR_MAX_VER);
	params.len = htod16(WL_CHANSPEC_TXPWR_MAX_LEN);
	params.count = htod32(1);

	ret = acs_get_txpwr_info(c_info, &params, sizeof(wl_chanspec_txpwr_max_t),
			txpwrbuf, buflen);
	if (ret != 0) {
		ACSD_DEBUG("%s: Failed to get txpwr values using iovar and falling back"
				" to old method\n", c_info->name);
		c_info->fallback_legacy_txpwr = TRUE;
		ACS_FREE(txpwrbuf);
		c_info->txpwr_params = NULL;
		return;
	}

	c_info->txpwr_params->ver = dtoh16(c_info->txpwr_params->ver);
	if (c_info->txpwr_params->ver != WL_CHANSPEC_TXPWR_MAX_VER) {
		ACSD_DEBUG("%s: Error: version [%d] mismatch Driver version:%d\n",
				c_info->name, WL_CHANSPEC_TXPWR_MAX_VER, c_info->txpwr_params->ver);
		c_info->fallback_legacy_txpwr = TRUE;
		ACS_FREE(txpwrbuf);
		c_info->txpwr_params = NULL;
		return;
	...
	c_info->txpwr_params->count = dtoh32(c_info->txpwr_params->count);
	if (c_info->txpwr_params->count > (buflen - WL_CHANSPEC_TXPWR_FIXED_LEN) /
		sizeof(chanspec_txpwr_max_t)) {
		ACSD_DEBUG("%s: c_info->txpwr_params count %d is greater than default max"
			"count %d\n", c_info->name, c_info->txpwr_params->count, buflen / 4);
		c_info->fallback_legacy_txpwr = TRUE;
		ACS_FREE(txpwrbuf);
		c_info->txpwr_params = NULL;
		return;
	}
	for (i = 0; i < c_info->txpwr_params->count; i++) {
		c_info->txpwr_params->txpwr[i].chanspec =
			dtoh16(c_info->txpwr_params->txpwr[i].chanspec);
		c_info->txpwr_params->txpwr[i].txpwr_max =
			MIN(c_info->txpwr_params->txpwr[i].txpwr_max,
			ACS_MAX_TXPWR * 4);
	}
}
...
acs_check_bss_is_enabled(char *name, acs_chaninfo_t **c_info_ptr, char *prefix)
{
	...
	*c_info_ptr = acs_info->chan_info[index] =
	...
	memcpy((*c_info_ptr)->name, name, sizeof((*c_info_ptr)->name));
...
acs_adjust_ctrl_chan(acs_chaninfo_t *c_info, chanspec_t chspec)
{
	...
	if (BAND_6G(c_info->rs_info.band_type) && c_info->acs_pref_6g_psc_pri) {
		ACSD_DEBUG("ifname:%s stay on PSC channel when operating on 6GHz band: 0x%04X\n",
	...
	for (; i < max_sb; i++) {
		...
		if (excl_chans && excl_chans->count) {
			if (acs_found_chspec_in_list(selected, excl_chans->clist,
					excl_chans->count)) {
				excl = TRUE;
			}
		}
		...
		if (excl_chans && excl_chans->rep_excl_count) {
			if (acs_found_chspec_in_list(selected, excl_chans->rep_clist,
...
acs_update_status(acs_chaninfo_t * c_info)
{
	...
	/* If DFS move back disable feature is on,
	 * keep track of last DFS chanspec.
	 */
	if (BAND_5G(c_info->rs_info.band_type) && !c_info->acs_dfs_move_back &&
		c_info->cur_is_dfs && c_info->last_dfs_chspec != cur_chspec) {
		ACSD_INFO("%s last_dfs_chspec: 0x%04x != 0x%04x :cur_chspec\n", c_info->name,
				c_info->last_dfs_chspec, c_info->cur_chspec);
	...
	if (CHSPEC_CHANNEL(c_info->selected_chspec) != CHSPEC_CHANNEL(c_info->cur_chspec)) {
		...
		chanim_upd_acs_record(c_info->name, c_info->chanim_info, c_info->cur_chspec,
				APCS_NONACSD);
	...
	ACSD_INFO("%s: chanspec: 0x%x is160_bwcap: %d is_bw_upgradable: %d\n", c_info->name,
			c_info->cur_chspec, c_info->is160_bwcap, c_info->bw_upgradable);
...
acs_check_ifname_is_virtual(char **ifname)
{
	...
	for (i = 0; i < ACS_MAX_IF_NUM; i++) {
		cur_map = &acs_info->acs_ifmap[i];
		if (cur_map == NULL || cur_map->name == NULL || !cur_map->name[0] ||
				strnlen(cur_map->name, IFNAMSIZ) >= IFNAMSIZ) {
			continue;
		}
		for (j = 0; j < ACS_MAX_VIFNAMES; j++) {
			if (acs_info->chan_info[i] != NULL &&
					acs_info->chan_info[i]->vifnames[j] != NULL &&
					acs_info->chan_info[i]->vifnames[j][0] &&
					(strnlen(acs_info->chan_info[i]->vifnames[j], IFNAMSIZ) <
					IFNAMSIZ) &&
				...
				if (!strncmp(acs_info->chan_info[i]->vifnames[j], *ifname,
					...
					ACSD_INFO("found vir intf %s converting to corresponding "
							...
							acs_info->chan_info[i]->vifnames[j],
							cur_map->name, strlen(*ifname));
					*ifname = cur_map->name;
					break;
				}
			}
...
int acs_update_assoc_info(acs_chaninfo_t *c_info)
{
	...
	/* read assoclist */
	list = (struct maclist *)acsd_malloc(ACSD_BUFSIZE_8K);
	memset(list, 0, ACSD_BUFSIZE_8K);
	ACSD_INFO("%s: WLC_GET_ASSOCLIST\n", c_info->name);
	list->count = htod32((ACSD_BUFSIZE_8K - sizeof(int)) / ETHER_ADDR_LEN);
	ret = wl_ioctl(c_info->name, WLC_GET_ASSOCLIST, list, ACSD_BUFSIZE_8K);
	if (ret < 0) {
...
acs_process_cmd(acs_chaninfo_t * c_info, chanspec_t chspec, int dfs_ap_move)
{
	...
	chanim_upd_acs_record(c_info->name, c_info->chanim_info, c_info->selected_chspec,
			APCS_IOCTL);
}
