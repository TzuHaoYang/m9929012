...
typedef struct bsd_steerlib_callback_data {
	...
	bsd_bssinfo_t *bssinfo;			/* bssinfo of STA */
	bsd_bssinfo_t *steer_bssinfo;		/* steer bssinfo ptr. */
	bsd_sta_info_t sta;			/* Copy entire STA data. This is required if the
						 * STA disassociates before getting the callback
						 * from steering library.
						 */
...
#ifdef CMWIFI_RDKB
	ptr->steering_flags = bssinfo->intf_info->steering_flags;
	BSD_STEER("Steering START: steer_state=%d steering_flags=0x%x\n",
		ptr->steer_state, ptr->steering_flags);
#endif // endif
...
#ifdef CMWIFI_RDKB
int bsd_rdk_log_steer_reason(int src_band, int dest_band, uint32 steering_flags)
{
	int steering_reason = RDKB_BS_REASON_UNKNOWN;
	int active, util, rssi, phyrate, upgrade = 0;

	active = steering_flags & BSD_STEERING_POLICY_FLAG_ACTIVE;
	util = steering_flags & BSD_STEERING_POLICY_FLAG_CHAN_OVERSUB;
	rssi = steering_flags & BSD_STEERING_POLICY_FLAG_RSSI;
	phyrate = steering_flags & BSD_STEERING_POLICY_FLAG_PHYRATE;
	upgrade = (src_band == BSD_BAND_2G) && (dest_band == BSD_BAND_5G);

	BSD_STEER("steer_reason: src_band=%d dest_band%d steering_flags0x%x\n", src_band, dest_band, steering_flags);

	if (active) {
		if (upgrade)
			steering_reason = RDKB_BS_REASON_ACTIVE_UPGRADE;
		else if (util)
			steering_reason = RDKB_BS_REASON_ACTIVE_UTIL;
		else if (rssi)
			steering_reason = RDKB_BS_REASON_ACTIVE_RSSI;
		else if (phyrate)
			steering_reason = RDKB_BS_REASON_ACTIVE_PHYRATE;
	}
	else {
		if (rssi)
			steering_reason = RDKB_BS_REASON_POST_RSSI;
		else if (util)
			steering_reason = RDKB_BS_REASON_POST_UTIL;
	}
	return steering_reason;
}
#endif // endif
...
#ifdef CMWIFI_RDKB
	{
		static FILE *bsd_log_fd = NULL;
		char *str = NULL;
		int steer_reason;
		char date_str[RDKB_BS_REASON_STRING_LENGTH] = {0};
		time_t t = time(NULL);
		struct tm tm = *localtime(&t);

		if ((str = nvram_get(WLAN_CM_RDK_BSD_LOG_FILE_NVRAM))) {
			bsd_log_fd = fopen(str, "a");
		}
		else {
			bsd_log_fd = fopen(WLAN_CM_RDK_BSD_LOG_FILE, "a");
		}

		if (bsd_log_fd != NULL) {
			steer_reason = bsd_rdk_log_steer_reason(bssinfo->intf_info->band, to_bssinfo->intf_info->band, reason);
			snprintf(date_str, sizeof(date_str), "%d-%d-%d %d:%d:%d", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
			...
			*/
			...
			if (reason & (BSD_STEERING_RESULT_SUCC | BSD_STEERING_RESULT_FAIL)) {
				fprintf(bsd_log_fd, "%9lu %s %.40s Steering MAC="MACF" FROM_BAND=%d TO_BAND=%d REASON=%d %.25s\n",
					(unsigned long)(time(NULL)), date_str, "steerexecImplCmnHandleAssocUpdateBTM",
					ETHERP_TO_MACF(addr),
					(bssinfo->intf_info->band == BSD_BAND_2G) ? 0 : 1,
					(to_bssinfo->intf_info->band == BSD_BAND_2G) ? 0 : 1,
					steer_reason,
					(reason & BSD_STEERING_RESULT_SUCC) ? "is_complete" : "is_not_complete");
			} else {
				fprintf(bsd_log_fd, "%9lu %s %.40s RDKB_LOG_STEERED MAC="MACF" FROM_BAND=%d TO_BAND=%d REASON=%d %.25s\n",
					(unsigned long)(time(NULL)), date_str, "steerexecImplCmnStartSteer",
					ETHERP_TO_MACF(addr),
					(bssinfo->intf_info->band == BSD_BAND_2G) ? 0 : 1,
					(to_bssinfo->intf_info->band == BSD_BAND_2G) ? 0 : 1,
					steer_reason, "steering_in_progress");
			}
			fflush(bsd_log_fd);
			fclose(bsd_log_fd);
		}
		else {
			fprintf(stderr, "Failed to open bsd log\n");
		}
	}
#endif /* CMWIFI_RDKB */
...
bsd_process_bss_trans_resp(void *data, int response)
{
	...
	bssinfo = callback_data->bssinfo;
	intf_info = bssinfo->intf_info;
	info = callback_data->info;

	/* Find the STA by address if not found use the STA data which is copied before sending
	 * the request. There may be a chance that the STA might have already disassociated
	 */
	if ((sta = bsd_sta_by_addr(info, bssinfo, &callback_data->sta.addr, FALSE)) == NULL) {
		sta = &callback_data->sta;
		BSD_STEER("STA:"MACF" not found\n", ETHER_TO_MACF(sta->addr));
	}
	...
	*/
...
#ifdef CMWIFI_RDKB
		snprintf(log_buf, sizeof(log_buf), "Remove STA "MACF" from blacklist on %s when BTM Reject\n",
			ETHER_TO_MACF(sta->addr), bssinfo->ifnames);
		WLAN_APPS_LOGS("BSD", log_buf);
#endif // endif
	}
	...
	if (response == BSD_BSS_RESPONSE_UNKNOWN) {
		if (bssinfo->intf_info->band != callback_data->steer_bssinfo->intf_info->band) {
			if (!bsd_is_sta_dualband(info, &sta->addr)) {
				...
				BSD_STEER("Skip STA "MACF" no response for single-band sta\n",
					ETHER_TO_MACF(sta->addr));
#ifdef CMWIFI_RDKB
				snprintf(log_buf, sizeof(log_buf), "Remove STA "MACF" from blacklist on %s for single-band sta when BTM Unknown\n",
					ETHER_TO_MACF(sta->addr), bssinfo->ifnames);
				WLAN_APPS_LOGS("BSD", log_buf);
#endif // endif
				goto end;
			...
			BSD_STEER("Skip STA "MACF" no response for not-at-target sta\n",
				ETHER_TO_MACF(sta->addr));
#ifdef CMWIFI_RDKB
			snprintf(log_buf, sizeof(log_buf), "Remove STA "MACF" from blacklist on %s for not-at-target sta when BTM Unknown\n",
				ETHER_TO_MACF(sta->addr), bssinfo->ifnames);
			WLAN_APPS_LOGS("BSD", log_buf);
#endif // endif
			goto end;
		...
		if (info->steer_no_deauth) {
			...
			BSD_STEER("NOT steer STA "MACF" from %s by steer_no_deauth\n",
				ETHER_TO_MACF(sta->addr), bssinfo->ifnames);
#ifdef CMWIFI_RDKB
			snprintf(log_buf, sizeof(log_buf), "Remove STA "MACF" from blacklist on %s by steer_no_deauth\n",
				ETHER_TO_MACF(sta->addr), bssinfo->ifnames);
			WLAN_APPS_LOGS("BSD", log_buf);
#endif // endif
			goto end;
			...
			for (bssidx = 0; bssidx < WL_MAXBSSCFG; bssidx++) {
				...
				if (!strcmp(tmp_bssinfo->ssid,
					...
					if (response != BSD_BSS_RESPONSE_REJECT) {
						...
						wl_wlif_block_mac(tmp_bssinfo->wlif_hdl,
							sta->addr, timeout);
#ifdef CMWIFI_RDKB
						snprintf(log_buf, sizeof(log_buf), "Add STA "MACF" to blacklist on %s when steering\n",
							ETHER_TO_MACF(sta->addr), tmp_bssinfo->ifnames);
						WLAN_APPS_LOGS("BSD", log_buf);
#endif // endif
					}
				}
	...
	if ((info->enable_flag & BSD_FLAG_WBD_ENABLED) && (response != BSD_BSS_RESPONSE_REJECT)) {
	...
	if ((response == BSD_BSS_RESPONSE_UNKNOWN) || (response == BSD_BSS_RESPONSE_REJECT)) {
		ret = 0;
	}
