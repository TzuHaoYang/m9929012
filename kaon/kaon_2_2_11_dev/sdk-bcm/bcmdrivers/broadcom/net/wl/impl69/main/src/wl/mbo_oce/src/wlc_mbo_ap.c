...
typedef struct wlc_mbo_scb_cubby {
	/* flags for associated bss capability, eg. band support */
	uint8 flags[1];
...
static int wlc_mbo_ap_iov_doiovar(void *ctx, uint32 id, void *params, uint params_len,
...
BCMATTACHFN(wlc_mbo_ap_attach)(wlc_info_t *wlc)
{
	...
	/* register module */
	ret = wlc_module_register(wlc->pub, mbo_iovars, "MBO_AP", (void *)mbo,
		wlc_mbo_ap_iov_doiovar, NULL, NULL, NULL);
	if (ret != BCME_OK) {
...
BCMATTACHFN(wlc_mbo_ap_detach)(wlc_mbo_info_t* mbo)
{
	...
	bcm_gas_unsubscribe_event(mbo->wlc, wlc_mbo_gas_event_cb);
	...
	mbo->wlc->pub->cmn->_mbo_ap = FALSE;
	bcm_iov_free_parse_context(&mbo->iov_parse_ctx, (bcm_iov_free_t)mbo_iov_context_free);
	wlc_module_unregister(mbo->wlc->pub, "MBO_AP", mbo);
	MFREE(mbo->wlc->osh, mbo, sizeof(*mbo));
	mbo = NULL;
}

static int
wlc_mbo_ap_iov_doiovar(void *ctx, uint32 id, void *params, uint params_len,
	void *arg, uint arg_len, uint vsize, struct wlc_if *wl_if)
{
	wlc_mbo_info_t *mbo = (wlc_mbo_info_t *)ctx;
	return bcm_iov_doiovar(mbo->iov_parse_ctx, id, params, params_len, arg,
		arg_len, vsize, wl_if);
}
	...
	size_t ilen, uint8 *obuf, size_t *olen)
{
	...
	wlc_mbo_update_attr_assoc_disallowed(dig->bsscfg, data);
fail:
	if (ptr) {
	...
	size_t ilen, uint8 *obuf, size_t *olen)
{
	...
	ptr = pibuf;
	memcpy(pibuf, ibuf, ilen);
	ret = bcm_unpack_xtlv_entry((uint8 **)&pibuf, WL_MBO_XTLV_AP_FWD_GAS_RQST_TO_APP,
		nbytes, &data, BCM_XTLV_OPTION_ALIGN32);
	if (ret != BCME_OK) {
		WL_MBO_ERR(("wl%d: %s: unpacking xtlv failed\n",
			mbo->wlc->pub->unit, __FUNCTION__));
		goto fail;
...
wlc_mbo_parse_ie(void *ctx, wlc_mbo_oce_attr_parse_data_t *data)
{
	...
	switch (data->ft) {
		...
		case FC_REASSOC_REQ: {
			...
			while ((ptr = (uint8*)bcm_find_tlv(local, data->ie_len,
...
wlc_mbo_update_scb_band_cap(wlc_info_t* wlc, struct scb* scb, uint8* data)
{
	...
	mbo_scb = MBO_SCB_CUBBY(mbo, scb);
	/*
	 * band capability mask when bit index refers
	 * to enum wlc_bandunit, eg. bit 1 set means
	 * scb supports 5GHz band (BAND_5G_INDEX = 1).
	 */
...
wlc_mbo_process_bsstrans_resp(wlc_info_t* wlc, struct scb* scb, uint8* body, int body_len)
{
	...
	if (mbo_ie && (mbo_ie->len > (MBO_OUI_LEN + sizeof(mbo_ie->oui_type)))) {
		...
		reject_attr = (wifi_mbo_trans_reason_code_attr_t *)
			bcm_find_tlv(mbo_ie->attr, attr_len,
	...
	struct ether_addr *peer_addr)
{
	...
	ASSERT(wlc->main_ap_bsscfg);
	bcm_gas_set_bsscfg_index(gas, wlc->main_ap_bsscfg->_idx);
...
wlc_mbo_update_attr_assoc_disallowed(wlc_bsscfg_t *cfg, uint8 assoc_disallowed)
{
	...
	if (prev_val != mbc->mbo_assoc_disallowed) {
		...
		wlc_bss_mac_event(wlc, cfg, WLC_E_MBO_CAPABILITY_STATUS,
