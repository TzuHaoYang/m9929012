...
void i5InterfaceEthernetNotifyReceiveOperStatus(i5_socket_type *psock, unsigned char const operStatus)
{
  ...
  i5Trace("ifname %s flags %x Oper Status: %d\n", psock->u.sll.ifname, psock->flags,
    aggregateOperStatus);
  if (IF_OPER_UP == aggregateOperStatus || IF_OPER_UNKNOWN == aggregateOperStatus) {
    i5MessageRestartTopologyDiscovery(psock);
...
/* Timer callback to inform about interface blocked by STP */
void i5InterfaceStpBlockingStateTimeout(void *arg)
{
  i5_socket_type *psock = (i5_socket_type *)arg;
  ...
  i5TimerFree(psock->u.sll.pSTPStateChangeTmr);
  psock->u.sll.pSTPStateChangeTmr = NULL;

  stp_status = i5BrUtilGetPortStpState(psock->u.sll.ifname);
  i5Trace("ifname %s flags %x STP status: %d\n", psock->u.sll.ifname, psock->flags,
    stp_status);
  /* If it's not in blocking state. don't do anything */
  if (stp_status != 0) {
    return;
  }
  ...
  /* We can remove the neighbor entries for those whose local interface is in blocking state */
  foreach_safe_i5glist_item(item, i5_dm_1905_neighbor_type, device->neighbor1905_list, next) {
    next = i5ll_next_p(item);
    if (eacmp(item->LocalInterfaceId, psock->u.sll.mac_address) == 0) {
      i5TraceInfo("ifname %s Removing Neighbor Enrty in Device "I5_MAC_FMT " Neighbor device "
          I5_MAC_FMT " Local Interface " I5_MAC_FMT " Neighbor Interface " I5_MAC_FMT "\n",
          item->localIfname, I5_MAC_PRM(device->DeviceId), I5_MAC_PRM(item->Ieee1905Id),
...
i5_socket_type *i5InterfaceSocketSet(char *ifname, unsigned short media_type, unsigned char const *pNetTechOui)
{
  ...
  /* Update the STP port status in psock of both LLDP and PROTO sockets */
  if (i5BrUtilGetPortStpState(ifname) == 0) {
    pifsock->flags |= I5_SOCKET_FLAG_BLOCKING;
    pifsock->u.sll.pLldpProtoSock->flags |= I5_SOCKET_FLAG_BLOCKING;
  } else {
    pifsock->flags &= ~I5_SOCKET_FLAG_BLOCKING;
    pifsock->u.sll.pLldpProtoSock->flags &= ~I5_SOCKET_FLAG_BLOCKING;
  }
  i5Trace("ifname %s flags %x\n", ifname, pifsock->flags);
                    ...
                    char *real_ifname)
{
  ...
  if ( 0 == bCreateSocket ) {
     ...
     * created. If the VLANs are created using iptable, then do not remove primary if the interface
     * is not wirless
     */
    if (I5_IS_TS_ACTIVE(i5_config.flags)) {
      if (!i5DmIsInterfaceWireless(media_type) && I5_IS_TS_USING_IPTABLE(i5_config.flags)) {
        i5TraceDirPrint("ifname[%s] is not wireless. And it is using iptables. So do not remove "
          "primary interface socket\n", ifname);
      } else {
...
void i5InterfaceAdd(char *ifname, unsigned short matchMediaType)
{
  ...
  unsigned short mediaType;
  int mediaLen = i5TlvMediaSpecificInfoWiFi_Length, is_loopback_listen, radio = 1;
  unsigned char  netTechOui[I5_PHY_INTERFACE_NETTECHOUI_SIZE];
  ...
  char real_ifname[I5_MAX_IFNAME], vlan_ifname[I5_MAX_IFNAME], prefix[I5_MAX_IFNAME];
  ...
  if (I5_IS_TS_ACTIVE(i5_config.flags) && i5_config.policyConfig.prim_vlan_id > 0) {
    snprintf(vlan_ifname, sizeof(vlan_ifname), "%s.%d", ifname,
  ...
  /* Check if wlx_radio is not 0 for primary wirelesss interface */
  if (i5DmIsInterfaceWireless(mediaType) && !i5WlCfgIsVirtualInterface(ifname)) {
    if (i5WlCfgGetPrefix(ifname, prefix, sizeof(prefix)) == 0) {
      radio = strtoul(i5WlCfgGetNvVal(prefix, "radio"), NULL, 0);
      if (!radio) {
        i5Trace("skip ifname[%s] as the %s_radio is %d\n", ifname, prefix, radio);
        return;
      }
    }
  }
...
#if !defined(SUPPORT_IEEE1905_FM)
static void i5InterfaceFlushMacs(char *ifname)
{
  i5_dm_device_type *pdevice = i5DmGetSelfDevice();
  ...
  if ((pdevice != NULL) && (pdevice->BridgingTuplesNumberOfEntries > 0)) {
    ...
    i5Trace("Unable to flush dynamic MACs for %s, no bridge\n", ifname);
  }
}

/* Neighbor removed, remove the MAC entry from bridge */
void i5InterfaceProcessNeighborRemoved(i5_dm_1905_neighbor_type *deletedNeighbor)
{
   ...
   i5InterfaceFlushMacs(deletedNeighbor->localIfname);
}
#endif /* defined(SUPPORT_IEEE1905_FM) */
