...
wlc_csimon_dump(void *ctx, bcmstrbuf_t *b)
{
	...
	for (idx = 0; idx < CSIMON_MAX_STA; idx++) {
		...
		bcm_bprintf(b, "null_frm_cnt %u xfer_to_ddr_cnt %u ack_fail_cnt %u\n"
		 "rec_ovfl_cnt %u xfer_to_ddr_fail_cnt %u rpt_invalid_cnt %u "
		 ...
		 c_info->sta_info[idx].xfer_fail_cnt, c_info->sta_info[idx].rpt_invalid_cnt,
		 c_info->sta_info[idx].rpt_invalid_once_cnt);
...
BCMATTACHFN(wlc_csimon_detach)(wlc_csimon_info_t *ci)
{
	...
	DMA_FREE_CONSISTENT(wlc->osh, (void *)ci->xfer.local_ring_base,
	                    mem_size, pa, NULL);
...
wlc_csimon_sta_config(wlc_csimon_info_t *ctxt, wlc_csimon_sta_config_t *cfg)
{
	...
	case CSIMON_CFG_CMD_DSB:
		if (CSIMON_ENAB(WLCPUB(ctxt))) {
			...
			CSIMON_DEBUG("wl%d deleted tmr %d ms\n", wlc->pub->unit,
			             ctxt->wd_timeout);
#endif /* ! DONGLEBUILD */
		}
		break;
...
wlc_csimon_intrson(wlc_info_t *wlc)
{
	...
	CSIMON_DEBUG("Enabled interrupts for m2m eng %p m2m_usr %d\n", m2m_eng,
	             CSIxfer(wlc).m2m_dd_csi);
