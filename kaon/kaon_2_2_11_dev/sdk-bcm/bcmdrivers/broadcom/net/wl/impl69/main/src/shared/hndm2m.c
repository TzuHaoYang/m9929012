...
typedef struct m2m_sys {
	...
	bool              pci64addr_high;       // high 63rd bit for (PCIe) address
#if defined(DONGLEBUILD)
...
static INLINE void /* thread safe disable interrupts */
__m2m_intrsoff(osl_t *osh, volatile uint32 *reg_intmask)
{
	OSL_INTERRUPT_SAVE_AREA
	OSL_DISABLE
	W_REG(osh, reg_intmask, 0U);
	OSL_RESTORE
...
m2m_core_worklet(void *cbdata)
{
	...
	M2M_DEBUG(M2M_PRINT("M2M CORE WLET: Intstatus 0x%x\n", m2m->intstatus));
...
void /* disable interrupts for a particular channel */
m2m_intrsoff(osl_t *osh, void *m2m_eng)
{
	...
	__m2m_intrsoff(osh, &int_regs->intmask);

} /* m2m_intrsoff() */

void /* enable interrupts for a particular channel */
m2m_intrson(osl_t *osh, void *m2m_eng)
{
	m2m_sys_t *m2m;
	volatile m2m_int_regs_t *int_regs;
	m2m_dd_eng_t *eng = (m2m_dd_eng_t *)m2m_eng;
	...
	__m2m_intrson(osh, &int_regs->intmask, M2M_DD_INTMASK);
...
#if defined(DONGLEBUILD)
	/* eng->intstatus is updated in NIC ISR, so no need to read
	 * from register for NIC mode
	 */
	/* Fetch and clear pending intstatus */
	eng->intstatus = R_REG(m2m->osh, &int_regs->intstatus);
#endif /* DONGLEBUILD */
	W_REG(m2m->osh, &int_regs->intstatus, eng->intstatus);  // clr DD intstatus
	M2M_DEBUG(R_REG(m2m->osh, &int_regs->intstatus)); /* sync read */
	M2M_DEBUG(M2M_PRINT("M2M WLET[%u]: Intstatus 0x%x\n", eng->idx, eng->intstatus));
...
m2m_dd_isr(void *cbdata, uint index, uint32 intstatus)
{
	...
	__m2m_intrsoff(m2m->osh, &int_regs->intmask);
...
m2m_dd_isr(void *cbdata, bool *wantdpc)
{
	...
	/* A bit in this register is set on a channel event when corresponding
	 * field in the ChXIntMask is set. For example, when rcvInt(b16) bit is
	 * set in the Ch0IntMask register, rcvInt(b16) bit in the Ch0IntStatus
	 * register gets set upon the receive interrupt
	 */
	intstatus = R_REG(m2m->osh, &int_regs->intstatus);
	if (intstatus == 0) {
		return FALSE; // Not our interrupt
	}

	/* Save interrupt status */
	eng->intstatus = intstatus;
...
BCMATTACHFN(m2m_fini)(si_t * sih, osl_t * osh)
{
	...
	for (eng_idx = 0U; eng_idx < M2M_DD_ENG; eng_idx++) {
		eng = &m2m->eng[eng_idx];
		M2M_DEBUG(M2M_PRINT("M2M FINI: engine 0x%x\n", eng_idx));
		m2m_int_regs = &m2m->core_regs->int_regs[eng_idx];
		m2m_eng_regs = &m2m->core_regs->eng_regs[eng_idx];

		/* Disable M2M interrupts */
		W_REG(osh, &m2m_int_regs->intmask, 0U);
		W_REG(osh, &m2m_int_regs->intstatus, ~0U); // clr DD intstatus
		/* CLR top level pending interrupt just in case for the engine */
		v32 = 1 << eng_idx;
		W_REG(osh,   &m2m->core_regs->intstatus /* 0x20 */,  v32);
		/* Disable the transmit and receive channels of the M2M engine */
		AND_REG(osh, &m2m_eng_regs->tx.control, ~D64_XC_XE);
		AND_REG(osh, &m2m_eng_regs->rx.control, ~D64_RC_RE);
...
#error "Both HNDM2M && BCMHWA features are enabled"
	...
	if (M2M_USE_PCI64ADDR(m2m)) {
		/* Use high 63rd bit for accessing host DDR over PCIe */
		m2m->pci64addr_high = TRUE;
	}
    ...
    void * xfer_arg, uint32 xfer_op)
{
	...
	if (m2m->pci64addr_high && (eng->idx == M2M_DD_D2H)) {
	...
	if (m2m->pci64addr_high && (eng->idx == M2M_DD_H2D)) {
