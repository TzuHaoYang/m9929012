 ...
 *
 * $Id: wlc_tpc.c 793480 2020-11-30 03:36:12Z $
 */
...
static const bcm_iovar_t wlc_tpc_iovars[] = {
	...
	{"curpower", IOV_CURPOWER, (IOVF_GET_UP), 0, IOVT_UINT8, 0},
	{"curtxctrl", IOV_CURTXCTRL, (IOVF_GET_UP), 0, IOVT_UINT8, 0},
#endif // endif
	...
	dot11_tpc_rep_t *ie, int8 rssi, ratespec_t rspec)
{
	...
	target_pwr = phy_tpc_get_target_max(WLC_PI(wlc));
...
wlc_tpc_get_local_constraint_qdbm(wlc_tpc_info_t *tpc)
{
	...
	if ((tpc->txpwr_local_max != WL_RATE_DISABLED) && wlc->pub->associated &&
		CHSPEC_PRIM20_EQUAL(wlc->chanspec, wlc->home_chanspec)) {
...
wlc_tpc_write_brcm_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (WL11K_ENAB(wlc->pub)) {
		uint8 target_pwr = phy_tpc_get_target_max(WLC_PI(wlc));
		uint8 *cp = data->buf;
	...
	ppr_ru_t **ru_reg_limits)
{
	...
	phy_txpwr->chanspec = phy_utils_get_chanspec(WLC_PI(wlc));
	if (wlc->pub->associated)
...
wlc_tpc_get_current_txctrl(wlc_tpc_info_t *tpc, void *txctrl, uint len)
{
	...
	txctrl_to_wl->cp_ltf_size = power->txctrl_info.cp_ltf_size;
	txctrl_to_wl->heltf_mask = power->txctrl_info.heltf_mask;
	txctrl_to_wl->num_heltf = power->txctrl_info.num_heltf;
	txctrl_to_wl->ldpc_extra = power->txctrl_info.ldpc_extra;
	txctrl_to_wl->afactor = power->txctrl_info.afactor;
	txctrl_to_wl->disamb = power->txctrl_info.disamb;
	txctrl_to_wl->doppler = power->txctrl_info.doppler;
	txctrl_to_wl->ldpc = power->txctrl_info.ldpc;
	txctrl_to_wl->mcs = power->txctrl_info.mcs;
	txctrl_to_wl->dcm = power->txctrl_info.dcm;
	txctrl_to_wl->nss_start_pos = power->txctrl_info.nss_start_pos;
	txctrl_to_wl->nss = power->txctrl_info.nss;
...
wlc_tpc_get_current(wlc_tpc_info_t *tpc, void *pwr, uint len, wlc_bsscfg_t *bsscfg)
{
	...
	if (ppr_type == PPRTYPE_REGLIMITS) {
		if ((reg_limits = ppr_create(wlc->osh,
			PPR_CHSPEC_BW(phy_utils_get_chanspec(WLC_PI(wlc))))) == NULL) {
			WL_ERROR(("wl%d: %s: reglimit create failed\n", wlc->pub->unit,
				__FUNCTION__));
			return BCME_NOMEM;
		}
		local_constraint = wlc_tpc_get_local_constraint_qdbm(tpc);
		wlc_channel_reg_limits(wlc->cmi, phy_utils_get_chanspec(wlc->pi), reg_limits, NULL);
		if (!AP_ONLY(wlc->pub)) {
			ppr_apply_constraint_total_tx(reg_limits, local_constraint);
		}
		pwr_to_wl->chanspec = phy_utils_get_chanspec(WLC_PI(wlc));
		...
		pwr_to_wl->antgain[2] = wlc->bandstate[BAND_6G_INDEX]->antgain;
		...
		if (old_power) {
			...
			if (WLTXPWRCAP_ENAB(wlc)) {
				uint32 caps = phy_txpwrcap_get_caps_inuse(WLC_PI(wlc));
			...
			if (!hwtable_txpwr) {
				...
				if ((err = wlc_stf_get_power_offset_per_ratespec(wlc,
					pwr_to_wl->last_tx_ratespec, &offset, txbf_chains)) !=
					BCME_OK) {
					goto free_power;
				}
				for (idx = 0; (uint) idx < WL_STA_ANT_MAX;
					idx++) {
					if (((txcore & wlc->stf->hw_txchain) >> idx) & 0x1) {
						int8 current_target =
