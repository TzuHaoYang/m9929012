 ...
 *
 * $Id: wlc_wnm.c 793169 2020-11-17 06:54:23Z $
 */

/**
 * @file
 * @brief
 * This file implements a part of 802.11v Wireless Network Management (WNM) features (aka Wi-Fi
 * Alliance Network Power Save) for both AP and STA. The .11v standard was released in 2011 and
 * rolled up into the 802.11-2012 standard. It facilitates members of the BSS exchanging information
 * about the WLAN in order to optimize it.
 *
...
#include <wlc_mbo.h>
#include <mbo.h>
#endif /* WL_MBO && !WL_MBO_DISABLED && (MBO_AP && MBO_STA) */
...
/*
 * Direct Multicast Service (DMS) is a feature of the 802.11v standard. Without the use of DMS a
 * client has to wake up every DTIM interval in order to receive multicast traffic. With DMS, the AP
 * buffers the multicast traffic for certain client, when client wakes up it sends a unicast frame
 * in order to request this traffic. It allows the client to sleep for a longer time and save
 * battery power. Multicast frames are transmitted as unicast over the air, sent at a higher data
 * rate than which It would have been used without DMS.
 *
...
typedef struct wnm_nbr_rep {
	...
	dot11_neighbor_rep_ie_t nbr_elt;	/**< ieee80211 defined structure */
} wnm_nbr_rep_t;

/** BTM (BSS Transition Management) Query Neighbor Report element List */
...
typedef struct wnm_bsstrans_sta_info {
	...
	uint16 nbrlist_size;
	wnm_nbr_rep_t *nbr_rep_head;	/** linked list of ieee80211 defined neighbor reports */
...
static void wlc_wnm_add_neighbor(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg,
	dot11_neighbor_rep_ie_t *nbr_elt);
...
#if defined(BCMDBG) || defined(WLTEST) || defined(DNG_DBGDUMP)
...
static const bcm_iovar_t wnm_iovars[] = {
	...
	{"wnm_bsstrans_resp",
	IOV_WNM_BSSTRANS_RESP, IOVF_BSSCFG_STA_ONLY, 0, IOVT_UINT8, 0},
#endif /* STA */
	{"wnm_btq_nbr_add",
...
static void wlc_wnm_send_bsstrans_request(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg,
	struct scb *scb, uint8 token, uint8 reqmode, uint8 retry_delay, uint8 transition_reason,
	bool add_self);
...
BCMATTACHFN(wlc_wnm_detach)(wlc_wnm_info_t *wnm)
{
	...
	FOREACH_STA(wlc, idx, bsscfg) {
		...
		if (WBTEXT_ENAB(wlc->pub) &&
			...
			if (bsi->resp_timer) {
				wl_free_timer(wlc->wl, bsi->resp_timer);
			}
			if (bsi->nbrlist_valid_timer) {
				wl_free_timer(wlc->wl, bsi->nbrlist_valid_timer);
				bsi->nbrlist_valid_timer = NULL;
			}
			if (bsi->disassoc_imnt_timer) {
				wl_free_timer(wlc->wl, bsi->disassoc_imnt_timer);
...
#ifdef STA
/**
 * BSS Transition Management enables an AP to e.g. request a voice client to transition to a
 * specific AP, or suggest a set of preferred APs to a voice client, due to network load balancing
 * or BSS termination. This helps the voice client identify the best AP to which that client should
 * transition to as that client roams.
 */
static int
wlc_wnm_bsstrans_query_send(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg, wlc_bss_list_t *list)
{
	...
	return BCME_OK;
} /* wlc_wnm_bsstrans_query_send */

/** STA related functionality */
static void
 ...
 * STA related functionality
 *
 * @param[in] body   Contains an .11v neighbor report
 */
static int
	...
	wl_reassoc_params_t *reassoc, uint8 *pref_max)
{
	...
	while (*body_len > 0) {
		...
		if (ngb->id != DOT11_MNG_NEIGHBOR_REP_ID ||
			ngb->len < DOT11_NEIGHBOR_REP_IE_FIXED_LEN) {
			goto skip;
		}
		if (!bcm_valid_tlv(ngb, *body_len)) {
			if (ngb) {
				WL_ERROR(("%s: Invalid tlv! id:%d len: %d\n",
					__FUNCTION__, ngb->id, ngb->len));
			} else {
				WL_ERROR(("%s: Invalid tlv! ngb: is Null\n",
					__FUNCTION__));
			}
			return BCME_ERROR;
		}

		pref = (dot11_ngbr_bsstrans_pref_se_t*) bcm_find_tlv(ngb->data,
			ngb->len - DOT11_NEIGHBOR_REP_IE_FIXED_LEN,
		...
		if (pref_chspec_found && ngb->channel != 0) {
			 ...
			 */
			/* TODO:6GHZ: combine country+operating_class+channel info in neighbor
			 * report to deduce if it is a 6g channel number
			 */
			reassoc->chanspec_list[0] = CH20MHZ_CHSPEC(update_channel);
			WL_WNM(("pref chanspec:0x%x rclass:0x%x chan:0x%x\n",
				reassoc->chanspec_list[0], ngb->reg, update_channel));
		}
	skip:
		/* Continue with next candidate */
		ngb = (dot11_neighbor_rep_ie_t *) bcm_next_tlv((bcm_tlv_t *)ngb, body_len);
	}
	if (!pref_chspec_found) {
		WL_ERROR(("%s: Failed to get preffered AP!ngb id or ngb len is wrong\n",
			__FUNCTION__));
		return BCME_ERROR;
	}
	return BCME_OK;
...
/**
 * STA related functionality. Function to identify if the candidate AP is recommended AP
...
/**
 * STA related functionality. Finds a 802.11 neighbor report, given the cfg, a specific address and
 * channel.
 */
wnm_nbr_rep_t *
...
/** STA related functionality. Get chanspec list from wnm neighbor list */
static int
		...
		chanspec_t *list, uint32 maxsize)
{
	...
	while (nbr_rep) {
		if (nbr_rep->nbr_elt.channel != 0 && idx < maxsize) {
			/* skip when band can not be determined */
			if (wlc_regclass_get_band(wnm->wlc->cmi, nbr_rep->nbr_elt.reg, &band) < 0)
				continue;

			chspec = CH20MHZ_CHSPEC2(nbr_rep->nbr_elt.channel, band);
	...
	return BCME_OK;
} /* wlc_wnm_neighbor_get_chanlist */

/** STA related functionality. Prepare wnm neighbor list from BSS Transition request frame */
static int
	...
	uint8 *body, int *body_len)
{
	...
	while (*body_len > 0) {
		...
		wlc_wnm_add_neighbor(wnm, bsscfg, ngb);
...
/** STA related functionality. Add an element for wnm neighbor list */
static void
wlc_wnm_add_neighbor(wlc_wnm_info_t *wnm, wlc_bsscfg_t *cfg, dot11_neighbor_rep_ie_t *nbr_elt)
{
	wnm_nbr_rep_t *nbr_rep;
	wlc_info_t *wlc;
	wnm_bsstrans_sta_info_t *bsi;
...
/** STA related functionality. delete all elements from wnm neighbor list */
static void
...
#ifdef WL_MBO_WFA_CERT
/**
 * STA related functionality. Place BTQ (BT query) NBR in BTM (Bss Transition Management) query
 * Frame.
 */
static void
...
/** STA related functionality. Function to cleanup the neighbor ap list */
static void
...
/** STA related functionality. Update non neighbor best score in case validity timer is active */
void
...
/** STA related functionality. Function to cleanup AP blacklist related info */
static void
...
/**
 * STA related functionality. Function to check if the current AP needs to be blacklisted
...
/**
 * STA related functionality. Take the bsstrans_req frame body as input and return list of channels
 * in neighbor IEs
 */
static int
...
/** STA related functionality. Allocate, fill and send a bsstrans_resp frame */
static int
	...
	uint8 token, struct scb *scb)
{
	...
	if (status == DOT11_BSSTRANS_RESP_STATUS_ACCEPT) {
		(void)memcpy(resp->data, bssid, ETHER_ADDR_LEN);
	}
	else if (status == DOT11_BSSTRANS_RESP_STATUS_REJ_BSS_LIST_PROVIDED) {
		uint32 max_score;
		uint32 temp_rssi;
...
/**
 * STA related functionality. Invoked after bsstrans_resp(with accept status) is not transmitted for
 * BSSTRANS_RESP_TIMEOUT. Don't wait anymore for bsstrans_resp tx and initiate the pending join.
 */
static void
...
/**
 * STA related functionality. Invoked at the end of roam scan to check if bss-trans-resp
...
/**
 * STA related functionality. This function is invoked for end-product (non-plugfest) functionality.
	...
	uint8 *body, int body_len, bool unicast_req, dot11_bsstrans_req_t* req)
{
	...
	if (req->reqmode & DOT11_BSSTRANS_REQMODE_DISASSOC_IMMINENT) {
		 ...
		 */
		if (bsi->disassoc_imnttmr_active) {
			wl_del_timer(wlc->wl, bsi->disassoc_imnt_timer);
			bsi->disassoc_imnttmr_active = FALSE;
		}

		if (req->disassoc_tmr != 0) {
			wl_add_timer(wlc->wl, bsi->disassoc_imnt_timer,
...
reject:
	...
	return retval;
} /* wlc_wnm_bsstrans_req_process_product */
		...
		uint8 **bufptr, struct scb *scb, uint8 reqmode)
{
	...
	*bufptr += TLV_HDR_LEN + DOT11_NEIGHBOR_REP_IE_FIXED_LEN;
	 ...
	 * SCB in MBO module
	 */
	if (scb && BSS_MBO_ENAB(wlc, scb->bsscfg)) {
	...
	uint8 *body, int body_len)
{
	...
	case DOT11_WNM_ACTION_BSSTRANS_QUERY: {
		...
		if (CEVENT_STATE(wlc->pub)) {
			wlc_send_cevent(wlc, bsscfg, SCB_EA(scb), 0, transquery->reason,
				0, NULL, 0, CEVENT_D2C_ST_BTM_QUERY, CEVENT_FRAME_DIR_RX);
		}
#endif /* BCM_CEVENT */
...
wlc_wnm_tfs_set(wlc_bsscfg_t *bsscfg, wnm_bsscfg_cubby_t *wnm_cfg, wl_tfs_set_t *tfs_set)
{
	...
	if (tfs_set->tfs_id) {
		...
		if (wnm->tclas_cnt == 1 && tfs_set->tclas_proc) {
		...
		if (wnm->tclas_cnt) {
			...
			filter->tclas_head = wnm->tclas_head;
			filter->tclas_cnt = wnm->tclas_cnt;
			wnm->tclas_head = NULL;
			wnm->tclas_cnt = 0;
		}

		/* Send the req frame with ALL filter set not enabled */
	} else if (wnm->tclas_cnt) {
...
/** STA specific functionality. Directed multicast service */
static int
...
#ifdef STA
/** STA specific functionality. This function will do WBTEXT related STA deinitialization on cfg */
static void
...
/** STA specific functionality. This function will do WBTEXT related STA initialization on cfg */
static int
BCMRAMFN(wnm_wbtext_bsscfg_bsi_init)(wlc_bsscfg_t *cfg)
{
	...
	/* init bss_resp timer */
	if ((bsi->resp_timer = wl_init_timer(wlc->wl,
	...
	if ((bsi->nbrlist_valid_timer = wl_init_timer(wlc->wl,
	...
	if ((bsi->disassoc_imnt_timer = wl_init_timer(wlc->wl,
...
/**
 * STA specific functionality.  This function just allocates bsi_mem structure
 * Since struct embedding is present, making it a BCMRAMFN.
 */
static int
...
/** STA specific functionality. This function will do STA related deinitialization on cfg */
static void
...
#endif /* WLWNM_AP */
		/* finally set up beacon and probe response ext cap */
		if (bsscfg->up &&
		    (BSSCFG_AP(bsscfg) || (BSSCFG_IBSS(bsscfg) && !BSS_TDLS_ENAB(wlc, bsscfg)))) {
...
wlc_wnm_timbc_req_ie_process(wlc_info_t *wlc, uint8 *tlvs, int len, struct scb *scb)
{
	...
	ie_tlv = (dot11_timbc_req_ie_t *)
		 bcm_find_tlv(tlvs, len, DOT11_MNG_TIMBC_REQ_ID);
...
/** AP specific functionality. TIM broadcast request element */
static int
...
/** AP specific functionality. TIM broadcast response element */
static uint
...
/**
 * AP specific functionality. The TFS Request frame is sent by a non-AP STA to the AP to request the
 * specified traffic filtering.
 */
static int
	...
	uint8 *tfs_resp_ie, int tfs_resp_ie_len, uint8 token)
{
	...
	return BCME_OK;
} /* wlc_wnm_send_tfs_resp_frame */

/** AP specific functionality. Traffic filtering */
static int
	...
	while (p < p_end) {
		...
		if (se->sub_id == DOT11_TFS_REQ_TFS_SE_ID) {
			...
			if (st_se->resp_st == DOT11_TFS_STATUS_DENY_POLICY &&
			    wnm->tclas_head != NULL && wnm->tclas_cnt > 0) {
				dot11_tfs_se_t *resp_se = (dot11_tfs_se_t *)st_se->data;
				int ext_len = wlc_wnm_tclas_ie_prep(wnm->tclas_head, resp_se->data);
...
/** AP specific functionality. Return true if the frame should be bypass */
static bool
...
wlc_wnm_tclas_cnt_get(uint8 *buf, int buf_len)
{
	...
	while (p < p_end) {
		tlv = bcm_find_tlv(p, (int)(p_end - p), DOT11_MNG_TCLAS_ID);
		if (tlv == NULL)
			break;
		num++;
...
proc_error:
	PKTFREE(wlc->osh, p, FALSE);
	return BCME_ERROR;
} /* wlc_wnm_dms_req_parse_resp_send */
#endif /* WLWNM_AP */
	...
	while (body < body_end) {
		...
		if (dms_desc == NULL) {
			...
			if (dms_resp_st->type == DOT11_DMS_RESP_TYPE_ACCEPT) {
				...
				wlc_sendmgmt(wlc, p, wlc->primary_bsscfg->wlcif->qi, NULL);
...
#ifdef WLWNM_AP
/** AP specific functionality. DOT11_WNM_ACTION_WNM_SLEEP_REQ, WNM Sleep Mode Request */
static int
...
/** AP specific functionality. BSS transition management */
static void
...
/**
 * AP specific functionality. Last 3 arguments are MBO specific extra argument
...
wlc_wnm_send_bsstrans_request(wlc_wnm_info_t *wnm, wlc_bsscfg_t *bsscfg, struct scb *scb,
	uint8 token, uint8 reqmode, uint8 retry_delay, uint8 transition_reason, bool add_self)
{
	wlc_info_t *wlc = wnm->wlc;
	wnm_bsscfg_cubby_t *wnm_cfg;
...
#if defined(WL_MBO) && !defined(WL_MBO_DISABLED) && defined(MBO_AP)
	if (BSS_MBO_ENAB(wlc, bsscfg)) {
		/* MBO IE for mbo client or only for BCAST transition req */
		if ((scb && SCB_MBO(scb)) || (scb == NULL)) {
	...
	if (BSS_MBO_ENAB(wlc, bsscfg)) {
		if (add_self) {
			 ...
			 * candidate preference element with preference value
			 */
			wlc_create_nbr_element_own_bss(wlc, bsscfg, &pdata);
		}
		/* Add MBO IE for mbo client or only for BCAST transition req */
		if ((scb && SCB_MBO(scb)) || (scb == NULL)) {
...
#ifdef BCM_CEVENT
	if (CEVENT_STATE(wlc->pub)) {
		wlc_send_cevent(wlc, bsscfg, SCB_EA(scb), 0, transition_reason, 0, NULL, 0,
			CEVENT_D2C_ST_BTM_REQ, CEVENT_FRAME_DIR_TX);
	}
#endif /* BCM_CEVENT */

	wlc_sendmgmt(wlc, p, bsscfg->wlcif->qi, scb);
} /* wlc_wnm_send_bsstrans_request */
...
#ifdef BCM_CEVENT
	if (CEVENT_STATE(wlc->pub)) {
		wlc_send_cevent(wlc, bsscfg, SCB_EA(scb), 0, bsstrans_resp->status, 0, NULL, 0,
			CEVENT_D2C_ST_BTM_RESP, CEVENT_FRAME_DIR_RX);
	}
#endif /* BCM_CEVENT */
} /* wlc_wnm_parse_bsstrans_resp */
	...
	uint8 *reply_to_bss, bool istx)
{
	...
	return FRAME_NOP;
} /* proxyarp_icmp6_handle */
#endif /* WLWNM_AP */

#ifdef STA
/** STA specific functionality. DOT11_WNM_ACTION_TIMBC_RESP */
int
...
/** STA specific functionality. Traffic filtering */
static int
...
/** STA specific functionality. Merge this function with wlc_wnm_dms_desc_prep() ? */
static int
	...
	uint32 dms_desc_list_len, uint8 **pbody)
{
	...
	p = wlc_frame_get_action(wlc, &wlc->primary_bsscfg->BSSID, &wlc->pub->cur_etheraddr,
		&wlc->primary_bsscfg->BSSID, body_len, &body, DOT11_ACTION_CAT_WNM);
	if (p == NULL)
		return NULL;
	...
	uint8 *body, int body_len)
{
	...
	return BCME_OK;
} /* wlc_wnm_sleep_resp_frame_parse */
	...
	struct scb *scb, uint8 *key, int len)
{
	...
	return err;
} /* wlc_wnm_sleep_resp_keydata_parse */
...
wlc_wnm_sleep_resp_ie_parse(wlc_wnm_info_t *wnm, struct scb *scb, uint8 *p, int len, int key_len)
{
	...
	tlv = bcm_find_tlv(p, len, DOT11_MNG_WNM_SLEEP_MODE_ID);
	if (tlv == NULL)
		return BCME_ERROR;
		...
		wnm_cfg->sleep_state == WNM_SLEEP_EXIT_WAIT)
	{
		...
		case DOT11_WNM_SLEEP_RESP_ACCEPT:
			...
			if (wlc_wnm_sleep_remove_tfs_filter(wnm->wlc->primary_bsscfg) != BCME_OK) {
...
/**
 * STA specific functionality. This function is an entry point to WNM-Sleep Mode extension for
 * PM_FAST. It is called by wlc_bcn_parse_tim_ie each DTIM interval when STA is in PM_FAST mode.
 */
int
...
/** STA specific functionality. This function is called by PM_FAST mode when PS mode is entered */
int
...
/** STA specific functionality. Add TFS for Sleep-Mode */
static int
...
/** STA specific functionality. Remove Sleep-Mode TFS filter */
static int
...
wlc_wnm_proxyarp_packets_handle(wlc_bsscfg_t *bsscfg, void *sdu, frame_proto_t *fp, bool istx)
{
	...
	return BCME_ERROR;
} /* wlc_wnm_proxyarp_packets_handle */
#endif /* WLWNM_AP */
...
} /* wlc_wnm_avg_bssload_calc */

/** STA specific functionality. Process BSS Load IE in beacon */
static int
...
/** STA specific functionality. This get called on every beacon if rssi trigger is hit */
bool
wlc_wnm_bsstrans_roam_required(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int *reason)
{
	...
	return FALSE;
} /* wlc_wnm_bsstrans_roam_required */

/** STA specific functionality. Return FALSE to stop roam scan */
bool
...
wlc_wnm_check_dms_req(wlc_info_t *wlc, struct ether_addr *ea)
{
	...
	wnm_bsscfg_cubby_t *wnm_cfg = WNM_BSSCFG_CUBBY(wnm_info, wlc->primary_bsscfg);
	struct scb *scb = wlc_scbfind(wlc, wlc->primary_bsscfg, ea);
	...
	if (BSSCFG_STA(wlc->primary_bsscfg)) {
		...
		if (req_send)
			wlc_wnm_dms_req_frame_send(wlc->primary_bsscfg, DOT11_DMS_REQ_TYPE_ADD, 0);
	}
}
	...
	wlc_bss_list_t *join_targets, void *join_pref_list, uint32 current_score)
{
	...
	return retval;
} /* wlc_wnm_update_join_pref_score */
	...
	int8 bcn_rssi, uint32 *score)
{
	...
	is_vht = bi->flags2 & WLC_BSS2_VHT;

	if (bi->flags2 & WLC_BSS2_80MHZ) {
		bw = 4;
	...
	if (is_vht) {
		...
		vht_mcsmap = wlc_rateset_vht_filter_mcsmap(bi->rateset.vht_mcsmap,
	...
	*score = rate_500k[i] * nss * bw * bi->qbss_load_chan_free;
	...
	return BCME_OK;
} /* wlc_wnm_bss_pref_score_product */
	...
	wlc_bss_info_t **bip, int trgt_count)
{
	...
	for (i = 0; i < trgt_count; i++) {
...
#ifdef WL11AC
		if (bip[i]->flags2 & WLC_BSS2_VHT) {
			extcap = (dot11_extcap_ie_t *)
				bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_EXT_CAP_ID);
		...
		qbss_load_ie = (dot11_qbss_load_ie_t *)
			bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_QBSS_LOAD_ID);
		...
		if (sta_cnt == 0xffff) {
			bip[i]->flags2 |= WLC_BSS2_MAX_STA_CNT;
		}
	}
	wnm_cfg = WNM_BSSCFG_CUBBY(wnm, cfg);
...
{
	wnm_bsscfg_cubby_t *wnm_cfg = WNM_BSSCFG_CUBBY(wnm, wnm->wlc->primary_bsscfg);
...
#ifdef WL11AC
	if (bsscfg->current_bss->flags2 & WLC_BSS2_VHT) {
		nbrrep->phytype = 9;
	} else
#else
