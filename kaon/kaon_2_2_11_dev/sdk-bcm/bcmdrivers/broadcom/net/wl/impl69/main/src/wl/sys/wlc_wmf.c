 ...
 *
 * $Id: wlc_wmf.c 792887 2020-11-05 21:22:51Z $
 */
...
#if defined(BCM_WLAN_PER_CLIENT_FLOW_LEARNING)
bool wl_wmf_psta_disable(wlc_bsscfg_t *bsscfg)
{
	wmf_info_t *wmf = bsscfg->wlc->wmfi;
	bss_wmf_info_t *bssinfo = BSS_WMF_INFO(wmf, bsscfg);
	ASSERT(bssinfo != NULL);
	return bssinfo->wmf_psta_disable;
}
#endif // endif
	...
	void *txif, int rt_port)
{
	...
	if (!scb || !SCB_ASSOCIATED(scb)) {
		if (p != NULL) {
			TX_PKTDROP_COUNT(wlc, NULL, TX_PKTDROP_RSN_NO_SCB);
			PKTFREE(osh, p, FALSE);
		}
	...
	if (PKTSHARED(p)) {
		...
		if ((pkt = PKTGET(osh, wlc->txhroff + ETHER_HDR_LEN, TRUE)) == NULL) {
			WL_ERROR(("wl%d: %s: PKTGET headroom %d failed\n",
			          wlc->pub->unit, __FUNCTION__, (uint)wlc->txhroff));
 ...
 *
 * Input:       wrapper - pointer to the bss configuration
 *              p     - Pointer to the ethernet packet
 */
static void
wmf_sendup(void *wrapper, void *p)
{
	...
	if (!bssinfo->wmf_instance) {
		if (p != NULL) {
			TX_PKTDROP_COUNT(wlc, NULL, TX_PKTDROP_RSN_WMF_DROP);
...
#endif // endif
	/* Send the packet up */
	wlc_sendup_msdus(wlc, bsscfg, NULL, p);
}
	...
	void *p, bool frombss)
{
	...
	if (ether_type == ETHER_TYPE_8021Q) {
		ehv = (struct ethervlan_header *)PKTDATA(bsscfg->wlc->pub->osh, p);
...
#if defined(BCM_BLOG) && defined(BCM_WLAN_PER_CLIENT_FLOW_LEARNING)
	if (frombss && !((struct sk_buff *)p)->blog_p) {
		struct sk_buff *skb = (struct sk_buff *)p;
		if (!skb->dev)
			skb->dev = wl_bsscfg_get_netdev(bsscfg);
		if (skb->dev && wl_handle_blog_sinit(bsscfg->wlc->wl, skb) == 0)
			return WMF_TAKEN;
	}
#endif /* BCM_BLOG */
	...
	if (skb_adjust)
		PKTPULL(wlc->osh, p, ETH_HLEN);

#ifdef BCM_NBUFF_WLMCAST_IPV6
	if (ether_type == ETHER_TYPE_IPV6)
		ret =  (emfc_ipv6_input(bssinfo->emfci, p, scb, iph, !frombss));
	else
#endif // endif
	ret = (emfc_input(bssinfo->emfci, p, scb, iph, !frombss));

	/* Readjust skb pointer if EMF is not taken it */
	if (skb_adjust && (ret != WMF_TAKEN))
		PKTPUSH(wlc->osh, p, ETH_HLEN);
