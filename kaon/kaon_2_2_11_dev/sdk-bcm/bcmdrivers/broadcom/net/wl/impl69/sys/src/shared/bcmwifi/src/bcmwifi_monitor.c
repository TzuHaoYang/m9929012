...
#define RXHDR_GET_AGG_TYPE(rxh, corerev, corerev_minor) (IS_D11RXHDRSHORT(rxh, corerev) ? \
	((D11RXHDRSHORT_ACCESS_VAL(rxh, corerev, corerev_minor, mrxs) & \
	  RXSS_AGGTYPE_MASK) >> RXSS_AGGTYPE_SHIFT) : D11REV_GE(corerev, 129) ? \
...
#define RXHDR_GET_PAD_LEN(rxh, corerev, corerev_minor) (IS_D11RXHDRSHORT(rxh, corerev) ? \
	        (((D11RXHDRSHORT_ACCESS_VAL(rxh, corerev, corerev_minor, mrxs) & \
		           RXSS_PBPRES) != 0) ? HDRCONV_PAD : 0) : D11REV_GE(corerev, 129) ? \
...
#define D11PHYRXSTS_ACCESS_REF_D(rxh, corerev, member) \
...
wlc_recv_mon_compute_rspec(d11rxhdr_t *rxh, uint8 *plcp,
	uint32 corerev)
{
	ratespec_t rspec;
	uint16 phy_ft;
	...
	phy_ft = *D11PHYRXSTS_ACCESS_REF_D(rxh, corerev, PhyRxStatus_0) & PRXS0_FT_MASK;
	switch (phy_ft) {
		case FT_CCK:
		...
		case FT_VHT:
			...
			break;
		case FT_HE:
			phy_ft_fmt = *D11PHYRXSTS_ACCESS_REF_D(rxh,
...
wl_d11rx_to_rxsts(monitor_info_t* info, monitor_pkt_info_t* pkt_info,
	d11rxhdr_t  *rxhdr, void *pkt, uint16 len, void *pout, int16 *offset,
	wl_phyextract_t  *phy_extract)
{
	...
	plcp += RXHDR_GET_PAD_LEN(rxhdr, corerev, 0);

	ts_tsf = pkt_info->ts.ts_low;
	rx_tsf_l = wlc_recover_tsf32(D11RXHDR_ACCESS_VAL(rxhdr, corerev, RxTSFTime), ts_tsf);

	bzero((void *)&sts, sizeof(wl_rxsts_t));

	sts.mactime = rx_tsf_l;
	if (corerev < 128) {
		sts.antenna = (*D11PHYRXSTS_ACCESS_REF_D(rxhdr, corerev, PhyRxStatus_0)
				& PRXS0_RXANT_UPSUBBAND) ? 1 : 0;
	}
	sts.signal = rssi;
	sts.noise = noise;
	sts.chanspec = D11RXHDR_ACCESS_VAL(rxhdr, corerev, RxChan);
	...
	if (D11REV_GE(corerev, 128)) {
		...
		rspec = wlc_recv_mon_compute_rspec(rxhdr, plcp, info->corerev);
	}
	...
	if ((subtype == FC_SUBTYPE_QOS_DATA) || (subtype == FC_SUBTYPE_QOS_NULL)) {
		...
		if (RSPEC_ISHT(rspec)) {
			...
			if ((plcp[0] | plcp[1] | plcp[2]) &&
				!(D11RXHDR_ACCESS_VAL(rxhdr, corerev, RxStatus2)
					& RXS_PHYRXST_VALID)) {
				/* First MPDU:
	...
	sts.pktlength = D11RXHDR_ACCESS_VAL(rxhdr, corerev, RxFrameSize)
		- D11_PHY_RXPLCP_LEN(info->corerev);
	if (corerev < 128) {
		sts.sq = ((*D11PHYRXSTS_ACCESS_REF_D(rxhdr, corerev, PhyRxStatus_1)
	...
	if (RSPEC_ISCCK(rspec)) {
		...
		if (D11REV_GE(corerev, 128)) {
			...
			sts.preamble = (*D11PHYRXSTS_ACCESS_REF_D(rxhdr,
				corerev, PhyRxStatus_0)
				& PRXS0_SHORTH) ?
		...
		sts.preamble = (uint32)((D11HT_MMPLCPLen(rxhdr) != 0) ?
			WL_RXS_PREAMBLE_HT_MM : WL_RXS_PREAMBLE_HT_GF);
	}
	/* translate error code */
	if (D11RXHDR_ACCESS_VAL(rxhdr, corerev, RxStatus1) & RXS_DECERR)
		sts.pkterror |= WL_RXS_DECRYPT_ERR;
	if (D11RXHDR_ACCESS_VAL(rxhdr, corerev, RxStatus1) & RXS_FCSERR)
		sts.pkterror |= WL_RXS_CRC_ERROR;

	p += RXHDR_GET_PAD_LEN(rxhdr, corerev, 0);
	len -= RXHDR_GET_PAD_LEN(rxhdr, corerev, 0);
...
static uint16
wl_monitor_amsdu(monitor_info_t* info, monitor_pkt_info_t* pkt_info, d11rxhdr_t *rxhdr,
		...
		wl_phyextract_t  *phy_extract)
{
	...
	aggtype = RXHDR_GET_AGG_TYPE(rxhdr, info->corerev, 0);
	...
	case RXS_AMSDU_N_ONE:
		...
		info->headroom  += RXHDR_GET_PAD_LEN(rxhdr, info->corerev, 0);
		*pkt_type = MON_PKT_AMSDU_FIRST;
		if (aggtype == RXS_AMSDU_N_ONE) {
			/* all-in-one AMSDU subframe */
			wl_d11rx_to_rxsts(info, pkt_info, rxhdr, p,
	...
	default:
		...
		if (info->amsdu_len) {
			...
			p += hwrxoff;
			len -= hwrxoff;
			p += RXHDR_GET_PAD_LEN(rxhdr, info->corerev, 0);
			len -= RXHDR_GET_PAD_LEN(rxhdr, info->corerev, 0);
			...
			*pkt_type = MON_PKT_AMSDU_INTERMEDIATE;
			...
			if (aggtype == RXS_AMSDU_LAST) {
				*pkt_type = MON_PKT_AMSDU_LAST;
				wl_d11rx_to_rxsts(info, pkt_info, rxhdr, info->amsdu_pkt,
...
bcmwifi_monitor(monitor_info_t* info, monitor_pkt_info_t* pkt_info,
	...
	uint16 *pkt_type, uint8 dma_flags, void  *phyextract, d11rxhdr_t *rxhdr)
{
	uint16 hwrxoff;
	...
	if (RXHDR_GET_AMSDU(rxhdr, info->corerev, 0)) {
		return wl_monitor_amsdu(info, pkt_info, rxhdr, pkt,
		...
		*pkt_type = MON_PKT_NON_AMSDU;
		...
		info->headroom = D11_PHY_RXPLCP_LEN(info->corerev) + hwrxoff;
		pout = (uint8 *)pout + RXHDR_GET_PAD_LEN(rxhdr, info->corerev, 0);
		info->headroom += RXHDR_GET_PAD_LEN(rxhdr, info->corerev, 0);
		wl_d11rx_to_rxsts(info, pkt_info, rxhdr, pkt, len, pout, offset, phyextract);
		info->headroom = 0;
		return len;
	}
}
