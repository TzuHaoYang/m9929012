...
#define I5_BR_UTIL_POLL_INTERVAL_MS 60000
...
int i5BrUtilGetNameFromPort(const char *brname, int portNum, char *ifname)
{
  ...
  if (dp != NULL)
  {
    while ((ep = readdir(dp)) != NULL) {
      ...
      retval = snprintf(path, MAX_PATH_NAME, "/sys/class/net/%s/brif/%s/port_no", brname, ep->d_name);
      if(retval>0 && ( f = fopen(path, "r"))) {
...
int i5BrUtilDelFdbEntry(const char *ifname, const unsigned char *macaddr)
{
   ...
   i5Trace("Deleting entry ifname %s MAC "I5_MAC_DELIM_FMT"\n", ifname, I5_MAC_PRM(macaddr));
      ...
      if ( cnt )
      {
         ...
         for ( i = 0; i < cnt; i++ )
         {
                 ...
                 (port_no == fdb[i].port_no) )
            {
               ...
               {
                  i5Trace("brname %s idx %d port %d ageing %d MAC "I5_MAC_DELIM_FMT"\n",
                    brname, i, fdb[i].port_no, fdb[i].ageing_timer_value,
                    I5_MAC_PRM(fdb[i].mac_addr));
