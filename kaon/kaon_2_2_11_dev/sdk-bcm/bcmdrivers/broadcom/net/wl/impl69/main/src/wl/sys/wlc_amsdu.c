...
wlc_amsdu_validate_sf_hdr(wlc_info_t *wlc, struct scb *scb, struct ether_header *eh)
{
	...
	if (BSSCFG_AP(bsscfg)) {
		/* For AP mode, SA should match the associated STA mac address and
		 * DA must not look like LLC-SNAP hdr.
		 */
		return ((eacmp(eh->ether_shost, &scb->ea) == 0) &&
			!rfc894chk(wlc, (struct dot11_llc_snap_header *)eh));
	} else {
		/* For STA, the DA must match its own MAC addr. This condition is stronger
		 * than the LLC-SNAP signature check so no need of LLC-SNAP check.
		 */
...
wlc_amsdu_deagg_hw(amsdu_info_t *ami, struct scb *scb, struct wlc_frminfo *f)
{
	...
	while (newpkt != NULL) {
		...
		if ((num_sf == 0) && !wlc_amsdu_validate_sf_hdr(ami->wlc, scb, eh)) {
