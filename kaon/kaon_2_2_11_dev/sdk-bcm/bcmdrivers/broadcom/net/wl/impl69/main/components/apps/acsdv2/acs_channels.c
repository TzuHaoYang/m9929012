 ...
 *
 *	$Id: acs_channels.c 793338 2020-11-24 03:44:17Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
acs_candidate_score_total(ch_candidate_t *candi, acs_chaninfo_t* c_info)
{
	...
	for (i = CH_SCORE_MAX-2; i < CH_SCORE_MAX; i++)
		total += score_p[i].score * score_p[i].weight;
...
acs_channel_compare(acs_chaninfo_t *c_info, chanspec_t cur_ch, chanspec_t sel_ch)
{
	...
	if (c_info->autochannel_through_cli && cur_bw != sel_bw) {
		better = TRUE;
		ACSD_INFO("%s: acs_boot_only:%d, cur_bw:%d sel_bw:%d\n",
				c_info->name, c_info->acs_boot_only, cur_bw, sel_bw);
	}
