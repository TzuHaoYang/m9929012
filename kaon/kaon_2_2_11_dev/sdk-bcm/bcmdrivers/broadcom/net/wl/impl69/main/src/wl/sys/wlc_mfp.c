 ...
 *
 * $Id: wlc_mfp.c 791480 2020-09-29 06:49:23Z $
 *
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_MFP_BIP): {
		...
		} else if (memcmp((const uint8 *)BIP_OUI_GCM256_TYPE,
			(uint8 *)arg, WPA_SUITE_LEN) == 0) {
			bss_mfp->bip_type = WPA_CIPHER_BIP_GMAC_256;
		} else {
...
mfp_set_bip(wlc_bsscfg_t *bsscfg, uint8 *bip)
{
	...
	if (memcmp((const uint8 *)BIP_OUI_TYPE, bip, WPA_SUITE_LEN) == 0) {
		bss_mfp->bip_type = WPA_CIPHER_BIP;
	} else if (memcmp((const uint8 *)BIP_OUI_GCM256_TYPE, bip, WPA_SUITE_LEN) == 0) {
		bss_mfp->bip_type = WPA_CIPHER_BIP_GMAC_256;
	} else {
		bss_mfp->bip_type = 0;
	}
	...
	d11rxhdr_t *rxhdr, struct dot11_management_header *hdr, void *p)
{
	...
	ismulti = ETHER_ISMULTI(&hdr->da);
	if (ismulti) { /* bcast/mcast */
		mmic_ie_t *ie;
	...
	/* Nothing more to be done for BC/MC frames */
	if (ismulti) {
		goto done;
	}
