 ...
 *
 * $Id: wlc.c 794257 2020-12-30 02:59:15Z $
 */
...
#include <wlc_airtime.h> /* for e.g. WLC_AIRTIME_DISABLED */
#endif /* WLATM_PERC */
...
#if defined(WL_MBO) && !defined(WL_MBO_DISABLED) && (defined(MBO_STA) || \
	defined(MBO_AP))
#include <wlc_mbo.h>
#endif /* WL_MBO && !WL_MBO_DISABLED ** (MBO_STA || MBO_AP) */
...
static void wlc_pktdrop_stats(wlc_info_t *wlc, void *arg, void *params);
...
#define CALLBACKS_TRACE(unit, name, callbacks, cur_callback)

/**  WLC Global object. Access wlc ptr for given radio unit id */
wlc_info_t* wlc_global[WLC_UNIT_MAX] = { WLC_G_INIT };
...
/**
 * structure for bandmask lookup - see wlc_bmac_fill_bandmask().
 */
struct devid_bandmask {
	uint16 deviceid;
	uint bandmask;
};
...
enum wlc_iov {
	...
	IOV_HEALTH_STATUS = 85,	/* Health status of the card (unused) */
	IOV_SAR_ENABLE = 86,
	IOV_SAR_LIMIT = 87,
	IOV_RESET_CNTS = 88,
	...
	IOV_DYNTXC = 167,
	IOV_FIPS_LOOPBACK_TEST = 168,
	IOV_BLOCK_HE = 169,     /* block HE probe req, auth req, assoc req */
	IOV_FIPS_CMVP_TEST = 170,
	IOV_GET_PKTDROP_STATS = 171,
	IOV_PSSCB_RELEASE_CNT = 172,
	IOV_ATF_PERC_RESET = 173,
#if BAND6G || defined(WL_OCE_AP)
	IOV_NBR_DISCOVERY_CAP = 174,	/**< Neighbor discovery capabilities */
	IOV_SHORT_SSID_LIST = 175,
#endif /* BAND6G || WL_OCE_AP */
#ifdef WL_CFG80211_NIC
	IOV_EXTENDED_OPS_SUPPORT = 176,
#endif /* WL_CFG80211_NIC */
	IOV_MBSSID = 181,	/* enable/disable mbssid */
...
static const bcm_iovar_t wlc_iovars[] = {
	...
	(IOVF_SET_UP|IOVF_WHL|IOVF_OPEN_ALLOW), 0, IOVT_UINT32, 0
	},      /* constructed in wlu.c with txpwr or txpwr1 */
#if defined(BCMNODOWN) || defined(WLP2P) || defined(WLPKTENG) || defined(WLTEST)
	...
	(IOVF_OPEN_ALLOW|IOVF_MFG), 0, IOVT_BOOL, 0
	},
#endif // endif
#ifdef STA
	{"mpc_dur", IOV_MPC_DUR,
	...
	(IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, sizeof(wl_cnt_info_t)
	},
	{"subcounters", IOV_SUBCOUNTERS,
	(IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, OFFSETOF(wl_subcnt_info_t, data)
	},
...
#if defined(WLATM_PERC)
	{"atf", IOV_ATF, 0, 0, IOVT_UINT32, 0 },
#endif /* WLATM_PERC */
#if defined(WLATM_PERC)
	{"atf_perc_reset", IOV_ATF_PERC_RESET, 0, 0, IOVT_VOID, 0 },
#endif /* WLATM_PERC */
#if defined(WLTEST)
	...
	(0), 0, IOVT_BUFFER, sizeof(wl_delta_stats_t)
	},
#endif // endif
	...
	0, 0, IOVT_BUFFER, (sizeof(sar_limit_t))
	},
#endif /* WL_SARLIMIT */
#ifdef WLCNT
...
#endif /* WL11AX */
#ifdef WL_PKTDROP_STATS
	{"pktdrop_stats", IOV_GET_PKTDROP_STATS,
	(0), 0, IOVT_BUFFER, PKTDROP_STATS_BUFFER_SIZE
	},
#endif /* WL_PKTDROP_STATS */
#if defined(PSSCB_RELEASE_WAR)
	{"psscb_release_cnt", IOV_PSSCB_RELEASE_CNT,
	(0), 0, IOVT_UINT32, sizeof(uint32)
	},
#endif /* PSSCB_RELEASE_WAR */
#if BAND6G || defined(WL_OCE_AP)
	{"nbr_discovery_cap", IOV_NBR_DISCOVERY_CAP, 0, 0, IOVT_UINT32, 0},
	{"short_ssid_list", IOV_SHORT_SSID_LIST, 0, 0, IOVT_BOOL, 0},
#endif /* BAND6G || WL_OCE_AP */
#ifdef WL_CFG80211_NIC
	{"extended_ops_support", IOV_EXTENDED_OPS_SUPPORT,
	(0), 0, IOVT_BOOL, 0},
#endif /* WL_CFG80211_NIC */
#ifdef WL_MBSSID
	{"mbssid", IOV_MBSSID, IOVF_SET_DOWN, 0, IOVT_BOOL, 0},		/* enable/disable mbssid */
#endif /* WL_MBSSID */
	{NULL, 0, 0, 0, 0, 0}
};
...
const uint8 prio2fifo[NUMPRIO] = {
	TX_AC_BK_FIFO,	/* 0	BK	AC_BK	Background */
	TX_AC_BE_FIFO,	/* 1	BE	AC_BK	Best Effort */
	TX_AC_BE_FIFO,	/* 2	BE	AC_BK	Best Effort */
	TX_AC_BE_FIFO,	/* 3	BE	AC_BE	Best Effort */
	TX_AC_BE_FIFO,	/* 4	BE	AC_VI	Best Effort */
	TX_AC_BE_FIFO,	/* 5	BE	AC_VI	Best Effort */
	TX_AC_VI_FIFO,	/* 6	VI	AC_VO	Video */
	TX_AC_VO_FIFO	/* 7	VO	AC_VO	Voice */
};
#else /* HOSTDPI */
/** Mapping from 802.1d priority to FIFO */
const uint8 prio2fifo[NUMPRIO] = {
...
const uint8 prio2ac[NUMPRIO] = {
	AC_BK,  /* 0       Background */
	AC_BE,  /* 1       Besteffort-1 */
	AC_BE,  /* 2       Besteffort-2 */
	AC_BE,  /* 3       Besteffort-3 */
	AC_BE,  /* 4       Besteffort-4 */
	AC_BE,  /* 5       Besteffort-5 */
	AC_VI,  /* 6       Video */
	AC_VO	/* 7       Voice */
};
#else /* HOSTDPI */
...
const uint8 wlc_prio2prec_map[] = {
	_WLC_PREC_BK,		/* 0 BK - Background */
	_WLC_PREC_BE,		/* 1 BE - Best Effort */
	_WLC_PREC_BE1,		/* 2 BE - Best Effort */
	_WLC_PREC_BE2,		/* 3 BE - Best Effort */
	_WLC_PREC_BE3,		/* 4 BE - Best Effort */
	_WLC_PREC_BE4,		/* 5 BE - Best Effort */
	_WLC_PREC_VI,		/* 6 VI - Video */
	_WLC_PREC_VO,		/* 7 VO - Voice */
};
#else /* HOSTDPI */
const uint8 wlc_prio2prec_map[] = {
...
const uint8 wlc_prec2prio_map[] = {
	...
	_WLC_PREC_BE1/2,	/* 2 BE - Best effort */
	_WLC_PREC_BE2/2,	/* 3 BE - Best effort */
	_WLC_PREC_BE3/2,	/* 4 BE - Best effort */
	_WLC_PREC_BE4/2,	/* 5 BE - Best effort */
	_WLC_PREC_VI/2,		/* 6 VI - Video */
	_WLC_PREC_VO/2,		/* 7 VO - Voice */
};
#else /* HOSTDPI */
const uint8 wlc_prec2prio_map[] = {
	_WLC_PREC_BE/2,		/* 0 BE - Best effort */
	_WLC_PREC_BK/2,		/* 1 BK - Background */
...
const uint8 tid_service_order[NUMPRIO] = {
	DPI_PRIO_VO,
	DPI_PRIO_VI,
	DPI_PRIO_BE4,
	DPI_PRIO_BE3,
...
#ifdef WL_PKTCNTR
static void wlc_dump_pktcntr(wlc_info_t *wlc, struct bcmstrbuf *b);
#endif /* WL_PKTCNTR */
...
static void wlc_setband(wlc_info_t *wlc, enum wlc_bandunit bandunit);
...
static void wlc_ibss_enable_all(wlc_info_t *wlc);
static void wlc_ibss_disable_all(wlc_info_t *wlc);
...
static int wlc_get_chan_info(wlc_info_t *wlc, chanspec_t chspec, uint32 *presult);
...
static void wlc_reset_atf_perc(wlc_info_t *wlc);
#endif /* WLATM_PERC */

static void wlc_disable_prop_vht_features(wlc_info_t *wlc);
static wlcband_t* wlc_get_band_by_chanspec(wlc_info_t *wlc, chanspec_t chanspec);
static wlcband_t* wlc_get_band_by_bandtype(wlc_info_t *wlc, int bandtype);

static void wlc_init_override_clm(wlc_info_t *wlc);

#ifdef STA
static int wlc_assoc_ie_len_update(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
		wlc_iem_uiel_t *puiel, wlc_assoc_t *as);
#endif /* STA */

static wlcband_t*
wlc_get_band_by_chanspec(wlc_info_t *wlc, chanspec_t chanspec)
{
	enum wlc_bandunit bandunit;

	FOREACH_WLC_BAND(wlc, bandunit) {
		if (bandunit == CHSPEC_BANDUNIT(chanspec)) {
			return wlc->bandstate[bandunit];
		}
	}
...
static wlcband_t*
wlc_get_band_by_bandtype(wlc_info_t *wlc, int bandtype)
{
	wlcband_t *band = NULL;
	enum wlc_bandunit bandunit;

	if (wlc->band->bandtype == bandtype) {
		return wlc->band;
	}

	FOREACH_WLC_BAND(wlc, bandunit) {
		if (wlc->bandstate[bandunit]->bandtype == bandtype) {
...
/**
 * Queries channel info
 * Returns an error if the chanspec is malformed (BCME_BADARG)
 */
static int
wlc_get_chan_info(wlc_info_t *wlc, chanspec_t chspec, uint32 *presult)
{
	uint32 result;
	uint32 dfs_chan_info;
	uint channel;

	/* check for a malformed argument */
	...
	/* only check Primary 20MHz info */
	chspec = wf_chspec_primary20_chspec(chspec);
	channel = CHSPEC_CHANNEL(chspec);

	if (CHSPEC_IS5G(chspec)) {
		result |= WL_CHAN_BAND_5G;
	} else if (CHSPEC_IS6G(chspec)) {
		result |= WL_CHAN_BAND_6G;
	}

	/* validation takes hw aka phy and sw into account */
	if (wlc_valid_chanspec_db(wlc->cmi, chspec)) {
		result |= WL_CHAN_VALID_HW | WL_CHAN_VALID_SW;
...
void
wlc_scb_close_link(wlc_info_t *wlc, struct scb *scb)
{
...
#endif /* PROP_TXSTATUS */
#ifdef WLAMPDU
		wlc_ampdu_scb_close_link(wlc, scb);
#endif /* WLAMPDU */
#ifdef WLNAR
		wlc_nar_scb_close_link(wlc, scb);
#endif /* WLNAR */
	}
}
...
#ifdef PROP_TXSTATUS
	/* close flowring for this scb */
	if (PROP_TXSTATUS_ENAB(wlc->pub) && !SCB_LEGACY_WDS(scb)) {
		wlc_update_scb_bus_flctl(wlc, scb, WLFC_CTL_TYPE_MAC_CLOSE);
	}
#endif /* PROP_TXSTATUS */

	wlc_txmod_flush_pkts(wlc->txmodi, scb, TXMOD_TID_FLUSH_ALL_TID);
...
#endif /* WL11K */
	DBG_PRINT_WAKE_RSN(AS_IN_PROGRESS(wlc));
...
wlc_ps_allowed(wlc_bsscfg_t *cfg)
{
...
#ifdef WL_PM_MUTE_TX
	if (cfg->wlc->pm_mute_tx->state != DISABLED_ST) {
		return pm->PMenabled;
	}
#endif // endif
	...
	if (!cfg->associated ||
	    ...
	    !cfg->dtim_programmed ||
#ifdef WLTDLS
	    !WLC_TDLS_PM_ALLOWED(cfg->wlc, cfg) ||
#endif // endif
	    !WLC_PORTOPEN(cfg))
		return FALSE;
...
wlc_tinydump(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	FOREACH_WLC_BAND_UNORDERED(wlc, bandunit) {
		bcm_bprintf(b, "%s %d ", wlc_bandunit_name(bandunit),
		            wlc->bandstate[bandunit]->rspec_override);
	}

	bcm_bprintf(b, "\nant_rx_ovr %d txant %d\n", wlc->stf->ant_rx_ovr, wlc->stf->txant);
...
wlc_fatal_error(wlc_info_t *wlc)
{
	 ...
	 * if it attemps to perform hard reset, FALSE otherwise. For Linux NIC and RTE, it returns
	 * FALSE.
	 */
...
#ifdef BCMHWA
	if (HWA_ENAB(wlc->pub)) {
		hwa_set_reinit(WL_HWA_DEVP(wlc));
	}
#endif /* BCMHWA */

	wl_init(wlc->wl); /* calls wl_reset and wlc_init */
...
BCMINITFN(wlc_init_chanspec)(wlc_info_t *wlc)
{
	...
	if (CHSPEC_IS40_UNCOND(chanspec) &&
	    (!N_ENAB(wlc->pub) || !VALID_40CHANSPEC_IN_BAND(wlc, CHSPEC_BANDUNIT(chanspec)))) {
		chanspec = wf_ctlchspec20_from_chspec(chanspec);
	}

	/* make sure the channel is on the supported band if we are band-restricted */
	if (wlc->bandlocked || IS_SINGLEBAND(wlc)) {
		/* driver is configured for the current band only,
		 * pick another channel if the chanspec is not on the current band.
		 */
		if (CHSPEC_BANDUNIT(chanspec) != wlc->band->bandunit)
	...
	if (!wlc_valid_chanspec_db(wlc->cmi, chanspec)) {
		...
		wlc_pi_band_update(wlc, CHSPEC_BANDUNIT(chanspec));
		chanspec = wlc_default_chanspec(wlc->cmi, TRUE);
...
BCMINITFN(wlc_enable_edcf)(wlc_info_t *wlc)
{
	if (WME_ENAB(wlc->pub)) {
		...
		OR_REG(wlc->osh, D11_IFS_CTL(wlc), (uint16)IFS_USEEDCF);
		FOREACH_BSS(wlc, i, bsscfg) {
...
BCMINITFN(wlc_reinit)(wlc_info_t *wlc)
{
	...
	if (WLC_IS_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc)) {
		WLC_CLR_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc);
		wlc_dfs_reset_old_chanspec(wlc->dfs);
	}
...
void BCMINITFN(wlc_init)(wlc_info_t *wlc)
{
	...
	/* Choose the right bandunit in case it is not yet synchronized to the chanspec */
	band = wlc_get_band_by_chanspec(wlc, chanspec);
	ASSERT(band != NULL);
	...
	phy_misc_tkip_rifs_war(wlc->pi, WLC_HT_GET_RIFS(wlc->hti));
	...
	wlc_phy_fcc_pwr_limit_set(WLC_PI(wlc), wlc_phy_fcc_pwr_limit_get(pi));
	...
	/* clear ALL data block state but put back DATA_BLOCK_QUIET if it was set */
	wlc_block_datafifo(wlc, DATA_BLOCK_MASK_ALL, wlc->block_datafifo & DATA_BLOCK_QUIET);
...
#ifdef BCM_CSIMON
	/* Initialize Channel State Info (CSI) Monitor */
	...
	if (D11REV_IS(wlc->pub->corerev, 129)) {
		wlc->dyntxc->enable = TRUE;
		wlc_txs_dyntxc_set(wlc->dyntxc);
	}
...
__wlc_set_wake_ctrl(wlc_info_t *wlc, uint32 mc)
{
	...
	if (WL_LEAKYAPSTATS_ENAB(wlc->pub)) {
		if (!wake && awake_before) {
			wlc_leakyapstats_gt_event(wlc, wlc->primary_bsscfg);
		}
	}
#endif /* WL_LEAKY_AP_STATS */
	if (HND_DS_HC_ENAB()) {
...
wlc_set_ps_ctrl(wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_IS_PRIMARY(cfg) && !BSSCFG_IBSS(cfg) &&
#ifdef WLMCNX
		((MCNX_ENAB(wlc->pub) && !wlc->cmn->ps_multista) || !MCNX_ENAB(wlc->pub)) &&
#endif /* WLMCNX */
		TRUE) {
...
#ifdef WL_MBSSID
		/* clear the Transmitted BSSID from the match registers */
		if (MULTIBSSID_ENAB(wlc->pub) && BSSCFG_STA(cfg)) {
			(void) wlc_keymgmt_free_amt(wlc->keymgmt, &cfg->mbssid_transbss_amtidx);
		}
#endif /* WL_MBSSID */
	}
#if defined(WLMCNX)
...
#ifdef WL_MBSSID
		if (MULTIBSSID_ENAB(wlc->pub) && BSSCFG_STA(cfg)) {
			/* Generate a dynamic AMT index for Tranmitted BSSID */
			cfg->mbssid_transbss_amtidx = wlc_keymgmt_alloc_amt(wlc->keymgmt);
			wlc_multibssid_config(wlc, cfg, attr);
		}
#endif // endif
...
wlc_set_home_chanspec(wlc_info_t *wlc, chanspec_t chanspec)
{
	...
	if (wlc->home_chanspec != chanspec) {
		...
		/* Delete all the stations which matches the current chanspec from sta monitor
		 * because, no point in keeping those STAs as we are not sure about the
		 * chanspec of the STA. Checking the chanspec because, no need to remove offchan
		 * STAs as the offchan STA monitor will take care of sniffing from different
		 * channel
		 */
		...
		if (BSSCFG_STA(wlc->primary_bsscfg)) {
			...
			wlc_bsscfg_set_current_bss_chan(wlc->primary_bsscfg, chanspec);
		}

		if ((wlc->stas_associated != 0) && (CHSPEC_IS160(chanspec)))
			wlc->any_sta_in_160mhz = TRUE;
		else
			wlc->any_sta_in_160mhz = FALSE;
		...
		if (WLCISNPHY(wlc->band)) {
			...
			wlc_phy_acimode_noisemode_reset(pi, chanspec, TRUE, TRUE, FALSE);
		}

		/* Update tpe_eirp */
		wlc_channel_tx_power_target_min_max(wlc, wlc->home_chanspec, &min_pwr, &max_pwr);
		wlc->tpe_eirp = max_pwr;
	}

	/* Update tpe_eirp for the case of boot.
	 * For case of boot, wlc->home_chanspec == chanspec and wlc->tpe_eirp == 0
	 */
	if (wlc->tpe_eirp == 0) {
		wlc_channel_tx_power_target_min_max(wlc, wlc->home_chanspec, &min_pwr, &max_pwr);
		wlc->tpe_eirp = max_pwr;
...
#if defined(WL_PWRSTATS)
	if (PWRSTATS_ENAB(wlc->pub)) {
		if (wlc_mimo_siso_metrics_report(wlc)) {
...
wlc_cache_cals(wlc_info_t *wlc)
{
	...
	list->count = 0;
	FOREACH_WLC_BAND(wlc, bandunit) {
		wlc_get_valid_chanspecs(wlc->cmi, list, bandunit, WLC_BW_CAP_20MHZ, abbrev);
	}
	...
	for (i = 0; i < list->count; i++) {
		...
		WL_INFORM(("wl%d: %s: setting chanspec to 0x%04x and running a periodic cal\n",
		           wlc->pub->unit, __FUNCTION__, chanspec));
		phy_chanmgr_create_ctx(pi, chanspec);
...
static int
wlc_cachedcal_tune(wlc_info_t *wlc, uint16 chanspec)
{
	uint16 channel = (chanspec & WL_CHANSPEC_CHAN_MASK);
	...
	if (!wlc_valid_channel20_in_band(wlc->cmi, CHSPEC_BANDUNIT(chanspec), channel)) {
		WL_ERROR(("wl%d: %s: Bad channel %d in band %d, chanspec(0x%04x)\n", wlc->pub->unit,
			__FUNCTION__, channel, CHSPEC_BANDUNIT(chanspec), chanspec));
		return BCME_BADCHAN;
	}
...
wlc_scb_disassoc_cleanup(wlc_info_t *wlc, struct scb *scb)
{
	...
	if (!SCB_LEGACY_WDS(scb)) {
		...
		if (SCB_AMPDU(scb)) {
			WL_AMPDU(("wl%d: "MACF": scb ampdu cleanup\n",
	...
	if (PWRSTATS_ENAB(wlc->pub) && BSSCFG_IS_PRIMARY(bsscfg) &&
...
wlc_BSSinit_rateset_filter(wlc_bsscfg_t *cfg)
{
	...
	if ((wf_chspec_ctlchan(phy_utils_get_chanspec(WLC_PI(wlc))) == 14) &&
	    (wlc_japan(wlc) == TRUE)) {
...
/**
 * Causes the current_bss to adopt the caller specified 'bi' properties
 */
void
wlc_BSSinit(wlc_info_t *wlc, wlc_bss_info_t *bi, wlc_bsscfg_t *cfg, int type)
{
	 ...
	 */
	if (phy_utils_get_chanspec(WLC_PI(wlc)) != current_bss->chanspec) {
		if (WLC_CHAN_COEXIST(phy_utils_get_chanspec(WLC_PI(wlc)), current_bss->chanspec)) {
#ifdef WL_MIMOPS_CFG
			if (WLC_MIMOPS_ENAB(wlc->pub)) {
	...
	if (MCNX_ENAB(wlc->pub) &&
	    (BSSCFG_AP(cfg) ||
	     (BSSCFG_IBSS(cfg) && BSSCFG_IS_PRIMARY(cfg)))) {
...
wlc_bandinit_ordered(wlc_info_t *wlc, chanspec_t chanspec)
{
	 ...
	 */
	if (wlc->bandlocked || IS_SINGLEBAND(wlc)) {
		ASSERT(CHSPEC_BANDUNIT(chanspec) == wlc->band->bandunit);

		start_bu = wlc->band->bandunit;	/* updated in wlc_bandlock() */
	} else {
		/* end with the band as specified by chanspec */
		start_bu = (CHSPEC_BANDUNIT(chanspec) + 1) % MAXBANDS;
	}

	/* make each band operational, software state init */
	FOREACH_WLC_BAND_STARTING_FROM(wlc, start_bu, bandunit) {
		wlc_pi_band_update(wlc, bandunit);

		ASSERT(wlc->bandstate[bandunit]->hwrs_scb != NULL);
...
static void
WLBANDINITFN(wlc_setband)(wlc_info_t *wlc, enum wlc_bandunit bandunit)
{
#ifdef STA
	int idx;
	wlc_bsscfg_t *cfg;
...
BCMATTACHFN(wlc_info_init)(wlc_info_t *wlc, int unit)
{
	...
	if ((wlc->phy_cap & PHY_PREATTACH_CAP_SUP_6G) &&
	    !(wlc->phy_cap & PHY_PREATTACH_CAP_SUP_2G)) {
		wlc->chanspec = CH20MHZ_CHSPEC2(1, WL_CHANSPEC_BAND_6G);
	} else if ((wlc->phy_cap & PHY_PREATTACH_CAP_SUP_5G) &&
		!(wlc->phy_cap & PHY_PREATTACH_CAP_SUP_2G)) {
		/* Assuming cores are bandlocked/band specific */
		wlc->chanspec = CH20MHZ_CHSPEC2(36, WL_CHANSPEC_BAND_5G);
	} else {
		wlc->chanspec = CH20MHZ_CHSPEC2(1, WL_CHANSPEC_BAND_2G);
	}
...
#if BAND6G || defined(WL_OCE_AP)
	wlc->nbr_discovery_cap = 0; /* TODO:6GHZ: WLC_NBR_DISC_CAP_NO_WILDCARD_PRBREQ */
	wlc->rnr_update_method = WLC_RNR_UPDATE_METHOD_STATIC;
#endif /* WL_WIFI_6GHZ || WL_OCE_AP */
#if BAND6G || defined(WL_OCE_AP)
	wlc->short_ssid_list = TRUE;
#endif /* BAND6G || WL_OCE_AP */

	wlc_init_override_clm(wlc);
	wlc->pub->_mbssid_enab = WLC_MBSSID_ENAB_AUTO;
} /* wlc_info_init */
...
BCMATTACHFN(wlc_attach_module_pre)(wlc_info_t *wlc)
{
	...
	MODULE_ATTACH(hrti, wlc_hrt_attach, 11);
...
BCMATTACHFN(wlc_attach_module)(wlc_info_t *wlc)
{
	...
	MODULE_ATTACH(mprobresp, wlc_probresp_attach, 17);
#endif /* WLPROBRESP_SW && !WLPROBRESP_SW_DISABLED */

	MODULE_ATTACH(prot, wlc_prot_attach, 18);
	MODULE_ATTACH(prot_g, wlc_prot_g_attach, 19);
	MODULE_ATTACH(prot_n, wlc_prot_n_attach, 20);

	MODULE_ATTACH(ap, wlc_ap_attach, 21); /* ap_attach() depends on wlc->scbstate */
#ifdef WLLED
	MODULE_ATTACH(ledh, wlc_led_attach, 22);
#endif // endif
#if defined(STA) && defined(WLRM) && !defined(WLRM_DISABLED)
	MODULE_ATTACH(rm_info, wlc_rm_attach, 23);
#endif // endif
#if defined(WLC_OFFLOADS_TXSTS) || defined(WLC_OFFLOADS_RXSTS)
	MODULE_ATTACH(offl, wlc_offload_attach, 24);
#endif /* WLC_OFFLOADS_TXSTS || WLC_OFFLOADS_RXSTS */
#ifdef WET
	MODULE_ATTACH(weth, wlc_wet_attach, 25);
#endif // endif
#ifdef WET_TUNNEL
	MODULE_ATTACH(wetth, wlc_wet_tunnel_attach, 26);
#endif // endif
#ifdef WLAMSDU
	/*
	 ...
	 */
	MODULE_ATTACH(ami, wlc_amsdu_attach, 27);
#endif // endif
#if defined(STS_XFER)
	MODULE_ATTACH(sts_xfer_info, wlc_sts_xfer_attach, 28);
#endif /* STS_XFER */
#ifdef WLAMPDU
...
#if defined(BCMWAPI_WPI) || defined(BCMWAPI_WAI)
	MODULE_ATTACH(wapi, wlc_wapi_attach, 74);
#endif // endif
...
#if defined(WLC_DTPC) && !defined(WLC_DTPC_DISABLED)
	MODULE_ATTACH(dtpc, wlc_dtpc_attach, 112);
#endif /* WLC_DTPC && !WLC_DTPC_DISABLED */
	...
	MODULE_ATTACH(pktc_info, wlc_pktc_attach, 153);
#endif // endif

#if defined(WL_OBSS_DYNBW) && !defined(WL_OBSS_DYNBW_DISABLED)
	MODULE_ATTACH(obss_dynbw, wlc_obss_dynbw_attach, 154);
#endif /* defined(WL_OBSS_DYNBW) && !defined(WL_OBSS_DYNBW_DISABLED) */

#if defined(PROP_TXSTATUS) && defined(PROP_TXSTATUS_ENABLED)
	MODULE_ATTACH(wlfc, wlc_wlfc_attach, 155);
#endif // endif
	MODULE_ATTACH(frag, wlc_frag_attach, 156);
	...
	MODULE_ATTACH(assoc_mgr, wlc_assoc_mgr_attach, 174);
		...
		MODULE_ATTACH(swdiv, wlc_swdiv_attach, 176);
	...
	MODULE_ATTACH(mbo_ap, wlc_mbo_ap_attach, 191);
#endif /* WL_MBO && MBO_AP */
...
#if defined(WL_ULMU) && !defined(WL_ULMU_DISABLED)
	MODULE_ATTACH(ulmu, wlc_ulmu_attach, 212);
#endif /* WL_ULMU && !WL_ULMU_DISABLED */
	MODULE_ATTACH(dyntxc, wlc_txs_dyntxc_attach, 213);
#if defined WLDFS && !defined(WLDFS_DISABLED)
	MODULE_ATTACH(dfs, wlc_dfs_attach, 214);
#endif // endif
#ifdef BCM_CSIMON
	MODULE_ATTACH(csimon_info, wlc_csimon_attach, 215);
#endif // endif
#if defined(WL_MBSSID)
	MODULE_ATTACH(mbssid_i, wlc_multibssid_attach, 216);
#endif /* defined(WL_MBSSID) */
#if defined(WL_MBO) && !defined(WL_MBO_DISABLED) && defined(MBO_STA)
	MODULE_ATTACH(mbo_sta, wlc_mbo_attach, 217);
#endif /* WL_MBO && WL_MBO_DISABLED && MBO_STA */
...
void
wlc_get_override_vendor_dev_id(void *wlc, uint16 *vendorid, uint16 *devid)
{
...
wlc_hwrsscbs_alloc(wlc_info_t *wlc)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		wlcband_t *band;
		band = wlc->bandstate[bandunit];
		if (band->hwrs_scb == NULL) {
			band->hwrs_scb = wlc_hwrsscb_alloc(wlc, band);
		}
		if (!band->hwrs_scb) {
		...
		bcopy(&band->hw_rateset, &band->hwrs_scb->rateset, sizeof(wlc_rateset_t));
	}
...
wlc_hwrsscbs_free(wlc_info_t *wlc)
{
	...
	FOREACH_WLC_BAND_UNORDERED(wlc, bandunit) {
		wlcband_t *bandstate = wlc->bandstate[bandunit];
...
#ifdef WL_PROXDETECT
/* allocate the proxd scbs */
int
wlc_proxdscbs_alloc(wlc_info_t *wlc)
{
	enum wlc_bandunit bandunit;
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		...
		band = wlc->bandstate[bandunit];
		if (band->proxd_scb == NULL) {
			band->proxd_scb = wlc_proxdscb_alloc(wlc, band);
		}
		if (!band->proxd_scb) {
			WL_ERROR(("wl%d: %s: wlc_proxdscbs_alloc failed\n",
			          wlc->pub->unit, __FUNCTION__));
			return BCME_NOMEM;
		}
	}
...
/* free the proxd scbs */
void
wlc_proxdscbs_free(wlc_info_t *wlc)
{
	enum wlc_bandunit bandunit;
	...
	FOREACH_WLC_BAND_UNORDERED(wlc, bandunit) {
		...
		if (bandstate->proxd_scb) {
			wlc_proxdscb_free(wlc, bandstate->proxd_scb);
			bandstate->proxd_scb = NULL;
		}
	}
}
#endif /* WL_PROXDETECT */
...
static const char BCMATTACHDATA(rstr_aa2g)[] = "aa2g";  /**< number of 2g antenna's */
static const char BCMATTACHDATA(rstr_aa5g)[] = "aa5g";  /**< number of 5g/6g antenna's */
static const char BCMATTACHDATA(rstr_aga0)[] = "aga0";   /**< antenna gain 5g/6g, ant #0 */
static const char BCMATTACHDATA(rstr_agbg0)[] = "agbg0"; /**< antenna gain 2g, ant #0 */
...
static const char BCMATTACHDATA(rstr_aga3)[] = "aga3";
static const char BCMATTACHDATA(rstr_agbg3)[] = "agbg3";
static const char BCMATTACHDATA(rstr_aa0)[] = "aa0";     /**< number of 2g antenna's */
static const char BCMATTACHDATA(rstr_aa1)[] = "aa1";     /**< number of 5g/6g antenna's */
static const char BCMATTACHDATA(rstr_ag0)[] = "ag0";     /**< 2ghz antenna gain */
static const char BCMATTACHDATA(rstr_ag1)[] = "ag1";     /**< 5ghz/6ghz antenna gain */
...
wlc_dump_ratesets(void *ctx, struct bcmstrbuf *b)
{
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		...
		wlc_rateset_dump(&wlc->bandstate[bandunit]->hw_rateset, b);
		bcm_bprintf(b, "bandstate[%d]->hwrs_scb->rateset:\n", bandunit);
		wlc_rateset_dump(&wlc->bandstate[bandunit]->hwrs_scb->rateset, b);
		bcm_bprintf(b, "bandstate[%d]->basic_rate:\n", bandunit);
		wlc_dump_basic_rate("Basic", wlc->bandstate[bandunit]->basic_rate,
		                    sizeof(wlc->bandstate[bandunit]->basic_rate), b);
		bcm_bprintf(b, "\n");
	}
...
static void
wlc_init_vars_dump(wlc_info_t *wlc)
{
	uint page = 0, vars_len = 0;
	char *vars = wlc->pub->vars;
	wlc_vars_dump_t *vars_dump = &wlc->pub->vars_dump;

	if (vars == NULL) {
		return;
	}

	for (page = 0; page < VARS_PAGE_NUM; page++) {
		vars_dump->pages[page] = NULL;
	}

	page = 0;
	vars_dump->pages[page] = vars;
	page++;

	while (*vars != '\0') {
		/* 2 is for string terminator and '\n', '\n' is reserved for bcmstrbuf */
		vars_len += strlen(vars) + 2;
		if (vars_len > VARS_PAGE_SIZE) {
			vars_dump->pages[page] = vars;
			vars_len = strlen(vars) + 1;
			page++;
			if (page > (VARS_PAGE_NUM - 1)) {
				break;
			}
		}
		vars += strlen(vars) + 1;
                      ...
                      void *btparam, uint *perr)
{
	...
	/* Check that bcmerrorstrtable has the right number of entries in it */
	/* For ROM offload images, force a reference to bcmerrorstr(). This avoids run-time errors
	 * for ROM-offload targets created w/o error messages if the ROM invokes bcmerrorstr(), and
	 * bcmerrorstr() does not exist in RAM. (If bcmerrorstr() is only contained within
	 * WL_ERROR() macros and errors msgs are disabled, bcmerrorstr() will be excluded from the
	 * RAM image since there are no references to it).
	 */
	(void)bcmerrorstr(0);
#endif // endif
...
#ifdef HNDBME
	/* Initialize the BME service; note: for dongles this only registers osh pointer */
	(void) bme_init(sih, osh);
#endif /* HNDBME */

#ifdef HNDM2M
	/* Initialize the M2MDMA service */
	(void) m2m_init(sih, osh);
#endif /* HNDM2M */
	...
	*/
...
#if defined(WL_PKTCNTR)
	wlc_dump_register(pub, "pktcntr", (dump_fn_t)wlc_dump_pktcntr, (void *)wlc);
#endif /* defined(WL_PKTCNTR) */
	...
	pub->m_coremask_blk_wowl = M_COREMASK_BLK(wlc);
	...
	FOREACH_WLC_BAND_UNORDERED(wlc, j) { /* No FOREACH_WLC_BAND, loop body switches band */
		wlc_pi_band_update(wlc, j); /* switches current band */
...
/* for EAP builds do not add additional default VHT features beyond the defaults */
	...
	FOREACH_WLC_BAND_UNORDERED(wlc, bandunit) {
		wlc_pi_band_update(wlc, bandunit); // park on default band, being the lowest fq one
		break;
	}
	...
	*/
...
#ifdef WL_PROXDETECT
	if (wlc_proxdscbs_alloc(wlc) != BCME_OK) {
		err = 1103;
		goto fail;
	}
#endif // endif
	...
	/* Setup a global pointer to WLC across radios */
	WLC_G(WLC_UNIT(wlc)) = wlc;

	wlc->pub->_upr_fd_sw = UPR_FD_SW_ENAB;
#if BAND6G
	if (BAND_6G(wlc->band->bandtype)) {
		wlc->pub->_upr_fd_sw = UPR_FD_UCODE_ENAB;
	}
#endif /* BAND6G */
...
fail:
	...
	if (sih && !wlc) {
#ifdef HNDBME
		/* De-initialize the BME service */
		(void) bme_deinit(sih, osh);
#endif /* HNDBME */
#ifdef HNDM2M
		/* De-initialize the M2M service */
		(void) m2m_fini(sih, osh);
#endif /* HNDM2M */
...
BCMATTACHFN(wlc_attach_antgain_init)(wlc_info_t *wlc)
{
	...
	if ((antgain == -1) && (wlc->pub->sromrev == 1)) {
		...
		int8 ag[WLC_TXCORE_MAX], max_ag = 0;
		uint i;

		ag[0] = (int8)getintvar_slicespecific(wlc->hw,
				vars, (BAND_5G6G(bandtype) ? rstr_aga0 : rstr_agbg0));
		ag[1] = (int8)getintvar_slicespecific(wlc->hw,
				vars, (BAND_5G6G(bandtype) ? rstr_aga1 : rstr_agbg1));
		ag[2] = (int8)getintvar_slicespecific(wlc->hw, vars,
				(BAND_5G6G(bandtype) ? rstr_aga2 : rstr_agbg2));
		ag[3] = (int8)getintvar_slicespecific(wlc->hw, vars,
				(BAND_5G6G(bandtype) ? rstr_aga3 : rstr_agbg3));
#ifdef WL_ANTGAIN
		...
		*/
		...
		WL_INFORM(("wl%d: %s: SROM %s antenna gain is %d\n", wlc->pub->unit, __FUNCTION__,
			BAND_5G6G(bandtype) ? "5G/6G" : "2G", antgain));
	}
...
BCMATTACHFN(wlc_attach_stf_ant_init)(wlc_info_t *wlc)
{
	...
	aa = (int8)getintvar_slicespecific(wlc->hw,
			vars, (BAND_5G6G(bandtype) ? rstr_aa5g : rstr_aa2g));
	if (aa == 0)
		aa = (int8)getintvar_slicespecific(wlc->hw,
			vars, (BAND_5G6G(bandtype) ? rstr_aa1 : rstr_aa0));
	if ((aa < 1) || (aa > 15)) {
	...
	wlc->band->antgain =
		...
		(BAND_5G6G(bandtype) ? rstr_ag1 : rstr_ag0));
	wlc_attach_antgain_init(wlc);

	wlc->band->sar =
			(int8)getintvar_slicespecific(wlc->hw, vars,
			(BAND_5G6G(bandtype) ? rstr_sar5g : rstr_sar2g));
...
#define AA2DBI		(2 * WLC_TXPWR_DB_FACTOR)
	if (wlc->pub->sih->boardvendor == VENDOR_APPLE) {
		...
		if (wlc->pub->sih->boardtype == BCM94322X9) {
			...
			wlc->band->antgain = BAND_5G6G(wlc->band->bandtype) ?
...
BCMATTACHFN(wlc_timers_deinit)(wlc_info_t *wlc)
{
	...
	if (CSIMON_ENAB(wlc->pub)) {
		wlc_csimon_wd_timer_del(wlc);
	}
#endif /* ! DONEGLEBUILD && BCM_CSIMON */
...
#if defined(WL_ULMU) && !defined(WL_ULMU_DISABLED)
	MODULE_DETACH(wlc->ulmu, wlc_ulmu_detach);
#endif /* WL_ULMU && !WL_ULMU_DISABLED */
#if defined(WL_MUSCHEDULER) && !defined(WL_MUSCHEDULER_DISABLED)
...
#if defined(WLC_DTPC) && !defined(WLC_DTPC_DISABLED)
	MODULE_DETACH(wlc->dtpc, wlc_dtpc_detach);
#endif /* WLC_DTPC */

#if defined(WL_MBO) && defined(MBO_AP)
	MODULE_DETACH(wlc->mbo_ap, wlc_mbo_ap_detach);
#endif /* WL_MBO && MBO_AP */

#if defined(WL_MBO) && !defined(WL_MBO_DISABLED) && defined(MBO_STA)
	MODULE_DETACH(wlc->mbo_sta, wlc_mbo_detach);
#endif /* WL_MBO && WL_MBO_DISABLED && MBO_STA */
...
#endif // endif
#if defined(STS_XFER)
	MODULE_DETACH(wlc->sts_xfer_info, wlc_sts_xfer_detach);
#endif /* STS_XFER */
#ifdef WLAMPDU
...
#endif /* WLPROBRESP_SW && !WLPROBRESP_SW_DISABLED */
#if defined(BCMWAPI_WPI) || defined(BCMWAPI_WAI)
	MODULE_DETACH(wlc->wapi, wlc_wapi_detach);
#endif // endif
#ifdef WOWLPF
...
#endif // endif
	MODULE_DETACH(wlc->dyntxc, wlc_txs_dyntxc_detach);
#ifdef BCM_CSIMON
	MODULE_DETACH(wlc->csimon_info, wlc_csimon_detach);
#endif // endif
#if defined(WL_MBSSID)
	MODULE_DETACH(wlc->mbssid_i, wlc_multibssid_detach);
#endif // endif
} /* wlc_detach_module */
...
BCMATTACHFN(wlc_detach)(wlc_info_t *wlc)
{
	...
	*/
	...
	if (wlc->pub->sih) {
#ifdef HNDBME
		(void) bme_deinit(wlc->pub->sih, wlc->osh);
#endif /* HNDBME */
#ifdef HNDM2M
		(void) m2m_fini(wlc->pub->sih, wlc->osh);
#endif /* HNDM2M */
...
#ifdef BCMHWA
	if (HWA_ENAB(wlc->pub)) {
		hwa_detach(WL_HWA_DEVP(wlc));
		WL_HWA_DEVP(wlc) = NULL;
	}
#endif /* BCMHWA */
	...
	for (i = 0; i < wlc->pub->max_modules; i ++) {
		...
		WL_ERROR(("wl%d: wlc_detach faile: %s module is unregistered\n",
			wlc->pub->unit, wlc->modulecb[i].name));
		ASSERT(0);
	}

	/* Reset the global lookup table */
	WLC_G(WLC_UNIT(wlc)) = NULL;
...
int
wlc_mpc_off_req_set(wlc_info_t *wlc, mbool mask, mbool val)
{
	int ret = BCME_OK;
...
wlc_watchdog(void *arg)
{
...
#if defined(WL_TXPKTPEND_SYNC)
	if (txpktpendtot != TXPKTPENDTOT(wlc)) {
		wlc->pktpend_sync |= TXPKTPEND_SYNC_EN;
		WL_ERROR(("wl%d:%s txpktpendtot %d != TXPKTPENDTOT(wlc) %d",
			wlc->pub->unit, __FUNCTION__, txpktpendtot,
			TXPKTPENDTOT(wlc)));
	}
#else
	...
	if (!AP_ACTIVE(wlc) && (wlc->excess_pm_period || wlc->excess_pmwake->ca_thresh) &&
	    ((wlc->primary_bsscfg->pm->PM && wlc->stas_associated) ||
	     0))
	...
	*/
	...
	/* wlc->pub->up would become FALSE if PSMx WD happens after wlc_bmac_watchdog() */
	if (!wlc->pub->up) {
		return;
	}
	...
	if (AP_ENAB(wlc->pub) && APSTA_ENAB(wlc->pub) && wlc_apup_allowed(wlc)) {
		 ...
		 */
		if (BSSCFG_STA(wlc->primary_bsscfg) &&
			(MAP_ENAB(wlc->primary_bsscfg) || DWDS_ENAB(wlc->primary_bsscfg) ||
#if defined(WET) || defined(WET_DONGLE)
			(WET_ENAB(wlc) || WET_DONGLE_ENAB(wlc)) ||
#endif /* WET || WET_DONGLE */
			(wlc->primary_bsscfg->_psta))) {

			if ((wlc->primary_bsscfg->up) && (wlc->primary_bsscfg->associated)) {
				 ...
				 */
				if (wlc_assoc_get_as_state(wlc->primary_bsscfg) ==
					AS_DFS_CAC_START) {
					...
					wlc_try_join_start(wlc->primary_bsscfg,
						wlc_bsscfg_scan_params(wlc->primary_bsscfg),
						wlc_bsscfg_assoc_params(wlc->primary_bsscfg));
				}
			}
		} else {
			wlc_apsta_restart_ap(wlc);
...
#if defined(WLATM_PERC)
/**
 * scb's contain statistics related to airtime scheduling. These statistics need to be accumulated
 * and updated on a periodic basis.
 */
void
wlc_atm_update_perc(wlc_info_t *wlc)
{
	wlc_bsscfg_t *cfg;
...
BCMINITFN(wlc_up)(wlc_info_t *wlc)
{
	...
	if (wlc->pub->radio_disabled) {
		wlc_radio_monitor_start(wlc);
#if defined(HNDBME) && defined(LINUXSTA_PS)
		/* When the Radio is disabled and Host platfom(Ex: STB) enters Suspend/Resume Cycle
		 * bme_reinit() will not done after resume, It will lead to bme_sync() issues,
		 * if the host platform goes for soft reboot, So handling bme_reint() here.
		 */
		bme_reinit(wlc->hw->sih, wlc->osh);
#endif /* HNDBME && LINUXSTA_PS */
		wlc->state = WLC_STATE_DOWN;
		return 0;
	}
...
#ifdef BCM_CEVENT
	wlc_if_event(wlc, WLC_E_IF_BSSCFG_UP, wlc->primary_bsscfg->wlcif);
#endif /* BCM_CEVENT */
...
BCMUNINITFN(wlc_down)(wlc_info_t *wlc)
{
		...
		FOREACH_BSS(wlc, i, bsscfg) {
			...
			if (BSSCFG_AP(bsscfg)) {
				if (bsscfg->up) {
					...
					cur_callback = wlc_bsscfg_down(wlc, bsscfg);
					callbacks += cur_callback;
					CALLBACKS_TRACE(wlc->pub->unit, "wlc_bsscfg_down",
						callbacks, cur_callback);
				}
				continue;
			}
			...
			cur_callback = wlc_assoc_abort(bsscfg);
			callbacks += cur_callback;
			CALLBACKS_TRACE(wlc->pub->unit, "wlc_assoc_abort", callbacks, cur_callback);
			...
			if (
				...
				cur_callback = wlc_bsscfg_disable(wlc, bsscfg);
				callbacks += cur_callback;
				CALLBACKS_TRACE(wlc->pub->unit, "wlc_bsscfg_disable",
					callbacks, cur_callback);
	...
	cur_callback = wlc_bmac_down_prep(wlc->hw);
	callbacks += cur_callback;
	CALLBACKS_TRACE(wlc->pub->unit, "wlc_bmac_down_prep", callbacks, cur_callback);
	...
	for (i = 0; i < wlc->pub->max_modules; i ++) {
		...
		WL_INFORM(("wl%d: %s: calling DOWN callback for \"%s\"\n",
		           wlc->pub->unit, __FUNCTION__, wlc->modulecb[i].name));
		cur_callback = wlc->modulecb[i].down_fn(wlc->modulecb_data[i].hdl);
		callbacks += cur_callback;
		CALLBACKS_TRACE(wlc->pub->unit, wlc->modulecb[i].name, callbacks, cur_callback);
	}
	...
	cur_callback = wlc_down_del_timer(wlc);
	callbacks += cur_callback;
	CALLBACKS_TRACE(wlc->pub->unit, "wlc_down_del_timer", callbacks, cur_callback);
...
#ifdef WLRXOV
	if (WLRXOV_ENAB(wlc->pub) && wlc->rxov_timer) {
		cur_callback = wl_del_timer(wlc->wl, wlc->rxov_timer);
		if (!cur_callback)
			callbacks++;
		CALLBACKS_TRACE(wlc->pub->unit, "wl_del_timer", callbacks, cur_callback);
	}
#endif // endif
	...
	cur_callback = wlc_led_down(wlc);
	callbacks += cur_callback;
	CALLBACKS_TRACE(wlc->pub->unit, "wlc_led_down", callbacks, cur_callback);
#endif /* WLLED */
	...
	cur_callback = wlc_bmac_down_finish(wlc->hw);
	callbacks += cur_callback;
	CALLBACKS_TRACE(wlc->pub->unit, "wlc_bmac_down_finish", callbacks, cur_callback);
...
#ifdef WLTAF
	/* Reset TAF scheduler state machine */
	wlc_taf_reset_scheduling(wlc->taf_handle, ALLPRIO, TRUE);
#endif // endif
...
#ifdef BCM_CEVENT
	wlc_if_event(wlc, WLC_E_IF_BSSCFG_DOWN, wlc->primary_bsscfg->wlcif);
#endif /* BCM_CEVENT */
...
wlc_set_gmode(wlc_info_t *wlc, uint8 gmode, bool config)
{
	...
	bool ofdm_basic = FALSE;		/* Make 6 and 24 basic rates */
	...
	/* grab the 2G band pointer */
	band = wlc_get_band_by_bandtype(wlc, WLC_BAND_2G);
	if (band == NULL) {
		return BCME_BADBAND;
	}
	...
	if (ofdm_basic) {
		for (i = 0; i < rs.count; i++) {
			if (rs.rates[i] == WLC_RATE_6M || rs.rates[i] == WLC_RATE_24M)
	...
	/* The default rateset is changed to the new one.
...
good:
	...
	bcopy((char*)&new, (char*)&wlc->bandstate[bandunit]->hwrs_scb->rateset,
		sizeof(wlc_rateset_t));

	if (VHT_ENAB_BAND(wlc->pub, wlc->band->bandtype)) {
		/* update wlc->vht_cap. mcs map (802.11 IE format) */
		wlc_vht_update_mcs_cap(wlc->vhti);
		/* updating new vht tx/rx mcs maps to default bss */
		wlc_vht_update_defaults(wlc->vhti);
	}
#if defined(WL11AX)
...
#endif /* WL11AC */
	/* Set up flag to indicate that default tx/rx mcs maps have been changed
	 * via 'rateset' IOVAR. This will prevent the Rx/Tx VHT-MCS maps  being
	 * overwritten after bringing the driver "up".
	 */
	wlc->defrateset_override = TRUE;
...
wlc_bandlock(wlc_info_t *wlc, int val)
{
	...
	if (IS_SINGLEBAND(wlc)) {
		if ((val != WLC_BAND_AUTO) && (val != wlc->band->bandtype)) {
			bandunit = wlc_bandtype2bandunit(val);
			goto badband;
		}
	...
	case WLC_BAND_2G: {
		...
		for (j = 0; j < MAXCHANNEL; j++) {
			if (wlc_valid_channel20_in_band(wlc->cmi, bandunit, j)) {
				valid_channels = TRUE;
				break;
			}
		}
		...
		if (move && (val == WLC_BAND_2G)) {
			/* Need to add b-band rates back in if just switched from a-band.
			 * These get killed in wlc_rate_hwrs_filter_sort_validate() when
			 * switching to band a.
			 */
			FOREACH_BSS(wlc, idx, bsscfg) {
				for (i = 0; i < wlc->band->hw_rateset.count; i++) {
					bsscfg->current_bss->rateset.rates[i] =
						wlc->band->hw_rateset.rates[i];
				}
				bsscfg->current_bss->rateset.count = wlc->band->hw_rateset.count;
			}
		}
...
badband:
	WL_ERROR(("wl%d: Band %s is not allowed\n", wlc->pub->unit, wlc_bandunit_name(bandunit)));
	return BCME_BADBAND;
} /* wlc_bandlock */

int
wlc_change_band(wlc_info_t *wlc, enum wlc_bandunit new_band)
{
	chanspec_t chspec;
	enum wlc_bandunit bandunit;

	/* sanity check arg */
	if (new_band != BAND_2G_INDEX && new_band != BAND_5G_INDEX && new_band != BAND_6G_INDEX) {
		return BCME_RANGE;
	}

	/* switch to first channel in the new band */
	wlc_pi_band_update(wlc, new_band);
	chspec = wlc_default_chanspec(wlc->cmi, FALSE);
	ASSERT(chspec != INVCHANSPEC);

	FOREACH_WLC_BAND_UNORDERED(wlc, bandunit) {
		if (bandunit != new_band) {
			wlc_pi_band_update(wlc, bandunit);
		}
	}
...
wlc_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
	...
	case WLC_SET_SSID: {
		...
		if (BSSCFG_AP(bsscfg)) {
			...
			if (WLC_BSSCFG_IDX(bsscfg) ==
					wlc_oce_get_apcfg_idx_used_for_fd_frame(wlc)) {
				wlc_oce_set_short_ssid(wlc, bsscfg);
			}
#endif /* WL_OCE_AP */
		}
	...
	case WLC_GET_CHANNEL: { /* obsolete IOV, does not support 6G channels */
		channel_info_t *ci = (channel_info_t*)arg;
		int chspec;

		ASSERT(len > (int)sizeof(ci));

		ci->hw_channel = wf_chspec_ctlchan(phy_utils_get_chanspec(WLC_PI(wlc)));
	...
	case WLC_SET_CHANNEL: { /* obsolete IOV, does not support 6G channels */
		int chspec = (WL_CHANSPEC_BW_20 | WL_CHANNEL_2G5G_BAND(val) | val);
		if (val < 0 || (uint)val > MAXCHANNEL) {
	...
	case WLC_SET_PWROUT_PERCENTAGE:
		if ((uint)val > 100 || (uint)val == 0 || (uint)val%10 != 0) {
			/* min unit is 10% */
			/* input cannot be 0 or > 100 */
			bcmerror = BCME_RANGE;
			break;
		}
		...
		if (wlc->pub->up) {
			...
			if (wlc_phy_tssivisible_thresh(pi) < 0 ||
				(ACREV_LT(wlc->band->phyrev, 47))) {
				/* OLPC th has to be set for this feature
				* threshold is always positive value
				 * even under lower power board
				 */
				bcmerror = BCME_EPERM;
				break;
			}
	...
	case WLC_GET_ANTDIV: {
		...
		if (!wlc->pub->up) {
			*pval = wlc->stf->ant_rx_ovr;
			...
			phy_antdiv_get_rx(pi, &phy_antdiv);
			*pval = (int)phy_antdiv;
		}
		break;
	}
	case WLC_SET_ANTDIV:
		...
		bcmerror = phy_antdiv_set_rx(pi, (uint8)val);
		break;
	...
	case WLC_SET_BCNPRD:
		if (val >= DOT11_MIN_BEACON_PERIOD && val <= DOT11_MAX_BEACON_PERIOD) {
			...
			if (wlc->pub->up) {
				WL_ERROR(("wl%d: BCME_NOTDOWN setting BCNPRD\n", wlc->pub->unit));
			}
		} else
			bcmerror = BCME_RANGE;
		break;
	...
	case WLC_SCB_DEAUTHENTICATE_FOR_REASON: {
		...
		if (BSSCFG_STA(bsscfg)) {
			...
			if (bsscfg->BSS) {
				...
				if (scb) {
					WL_ASSOC(("wl%d.%d: %s: send deauth to "MACF" with reason"
						" %d\n", wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
						__FUNCTION__, ETHERP_TO_MACF(apmac), val));
					(void)wlc_senddeauth(wlc, bsscfg, scb, apmac, apmac,
						&bsscfg->cur_etheraddr, (uint16)val, TRUE);
				}
				...
				(void)wlc_senddeauth(wlc, bsscfg, scb, eaptr, &bsscfg->BSSID,
					&bsscfg->cur_etheraddr, (uint16)val, scb ? TRUE : FALSE);
				if (scb) {
...
#ifdef WL_HAPD_WDS
			if ((SCB_LEGACY_WDS(scb)) && ((cmd == WLC_SCB_DEAUTHORIZE) ||
			(cmd == WLC_SCB_DEAUTHENTICATE_FOR_REASON) ||
			(cmd == WLC_SCB_DEAUTHENTICATE))) {
	...
	case WLC_GET_BSS_INFO: {
		...
		if (!wlc->pub->hw_off) {
			...
			current_bss->phy_noise = wlc_lq_noise_ma_upd(wlc, phy_noise_avg(wlc->pi));
		}
	...
	case WLC_GET_BANDLIST: {
		uint nbands = wlc_get_nbands(wlc);

		if (len < sizeof(*pval) * (nbands + 1)) {
			bcmerror = BCME_BUFTOOSHORT;
			break;
		}
		...
		*pval++ = nbands;
		FOREACH_WLC_BAND(wlc, bandunit) {
			*pval++ = wlc->bandstate[bandunit]->bandtype;
		}
		break;
	}
	...
	case WLC_SET_BAND:
		...
		wlc_mac_event(wlc, WLC_E_BAND_CHANGE, NULL,
				0, 0, 0, &val, sizeof(uint));
	...
	case WLC_SET_SHORTSLOT_OVERRIDE:
		 ...
		 * currently on the 5G or 6G band
		 */
		if (BAND_5G6G(wlc->band->bandtype))
			break;
	...
	case WLC_GET_GMODE: {
		wlcband_t *b;
		b = wlc_get_band_by_bandtype(wlc, WLC_BAND_2G);
		if (b != NULL) {
			*pval = b->gmode;
		} else {
	...
	case WLC_SET_SUP_RATESET_OVERRIDE: {
		...
		bcmerror = BCME_BADBAND;
		FOREACH_WLC_BAND(wlc, bandunit) {
			if (wlc->bandstate[bandunit]->gmode) {
				bcmerror = BCME_OK;
			}
		}

		if (bcmerror == BCME_BADBAND) {
			break;
		}
	...
	case WLC_GET_SUP_RATESET_OVERRIDE: {
		...
		if (bcmerror == BCME_BADBAND) {
			break; /* gmode only command when not in gmode */
		}
	...
	case WLC_GET_VALID_CHANNELS: {
		...
		FOREACH_WLC_BAND_UNORDERED(wlc, bandunit) {
			...
			FOREACH_WLC_BAND_CHANNEL20(band, i) {
				if (wlc_valid_chanspec_db(wlc->cmi,
					CH20MHZ_CHSPEC2(i, BANDTYPE_CHSPEC(band->bandtype)))) {
	...
	case WLC_GET_BANDSTATES: {
		...
		FOREACH_WLC_BAND(wlc, bandunit) {
			wlcband_t *b;
			b = wlc->bandstate[bandunit];
			wl_band->bandtype = (uint16) b->bandtype;
			wl_band->bandunit = (uint16) b->bandunit;
			wl_band->phytype = (uint16) b->phytype;
			wl_band->phyrev = (uint16) b->phyrev;
			wl_band++;
		}
		break;
	}
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	*/
	case IOV_SVAL(IOV_QTXPOWER): {
		...
		wlc_phy_txpwr_cache_invalidate(phy_tpc_get_txpwr_cache(pi));
#endif	/* WLTXPWR_CACHE */
...
#ifdef WLOLPC
		if (OLPC_ENAB(wlc) && (wlc_olpc_eng_autocal_enab(wlc->olpc_info)) &&
			(qdbm < wlc_phy_tssivisible_thresh(pi))) {
			/* override sends its own packets via pkteng */
			if (!override) {
				wlc_olpc_eng_hdl_chan_update(wlc->olpc_info);
			}
			wlc_olpc_autocal_timer_reload(wlc->olpc_info);
		}
#endif /* WLOLPC */
		break;
	}
	...
	case IOV_GVAL(IOV_STA_INFO):
	{
		...
		if (ETHER_ISBCAST(ea)) {
			...
			FOREACH_BSS_SCB(wlc->scbstate, &scbiter, bsscfg, scb) {
				if (err != BCME_OK) {
					break;
				} else if (remaining_len < sizeof(*sta_info)) {
					/* The buffer size is not enough for all sta_info.
					 * Return BCME_BUFTOOSHORT will let wl utility to
					 * 1. query assocated sta number
					 * 2. iterate query the sta_info
					 */
	...
	case IOV_SVAL(IOV_WPA_AUTH):
		...
		if (BSSCFG_AP(bsscfg)) {
			if (bsscfg->up)
				...
				bsscfg->WPA_auth = int_val;
			if (bsscfg->WPA_auth & WPA3_AUTH_SUITEB) {
				if (D11REV_LT(wlc->pub->corerev, 130)) {
					WL_ERROR(("%s: revinfo < 130 WPA3 SUITEB "
					"configured\n", __FUNCTION__));
					err = BCME_UNSUPPORTED;
				}
			}
	...
	case IOV_GVAL(IOV_PER_CHAN_INFO): {
		...
		err = wlc_get_chan_info(wlc, (chanspec_t)int_val, &chan_info);
		if (err == BCME_OK) {
			*ret_int_ptr = (uint32)chan_info;
		}
		break;
	}
...
#endif /* WL_PSMX */
#if defined(AP)
			memset((char *)wlc->pub->_ap_cnt, 0, sizeof(*wlc->pub->_ap_cnt));
#endif // endif
			if (wlcif) {
				bzero(wlcif->_cnt, sizeof(wl_if_stats_t));
				WLCNTSET(wlcif->_cnt->version, WL_IF_STATS_T_VERSION);
...
#if !defined(WLATF_BARE)
		if (int_val != WLC_AIRTIME_DISABLED && int_val != WLC_AIRTIME_PERC) {
			err = BCME_UNSUPPORTED;
			break;
		}
#endif /* WLATF_BARE */
		wlc_reset_atf_perc(wlc);

		wlc->atf = int_val;
		wlc->atm_perc = (int_val == WLC_AIRTIME_PERC) ? TRUE : FALSE;
		break;

	case IOV_SVAL(IOV_ATF_PERC_RESET):
		err = BCME_OK;
		wlc_reset_atf_perc(wlc);
		break;
	...
	case IOV_GVAL(IOV_CHANSPECS_DEFSET): {
		...
		FOREACH_WLC_BAND(wlc, bandunit) {
			bwcap = wlc_get_cmn_bwcap(wlc, wlc_bandunit2bandtype(bandunit));
			wlc_get_valid_chanspecs(wlc->cmi, list, bandunit, bwcap, abbrev);
		}
	...
	case IOV_GVAL(IOV_CHANSPECS): {
		...
		if (!bcmp(wlc_channel_ccode(wlc->cmi), abbrev, WLC_CNTRY_BUF_SZ -1) &&
			(wlc->band->bandunit == CHSPEC_BANDUNIT(chanspec))) {
		...
		if (chanspec == 0) {
			...
			wlc_get_valid_chanspecs(wlc->cmi, list, CHSPEC_BANDUNIT(chanspec),
				CHSPEC_IS8080(chanspec) ? WLC_VALID_CHANSPECS_WANT_80P80 :
	...
	case IOV_SVAL(IOV_CHANSPEC): {
		...
		if (!(N_ENAB(wlc->pub) || CHSPEC_ISLE20(chspec))) {
			err = BCME_BADCHAN;
			break;
		}
		...
		if (!to_wlc->pub->up && IS_MBAND_UNLOCKED(to_wlc)) {
			if (to_wlc->band->bandunit != CHSPEC_BANDUNIT(chspec)) {
	...
	case IOV_GVAL(IOV_ANTGAIN): {
		...
		memset(ag, 0, sizeof(*ag) * MAXBANDS);
		FOREACH_WLC_BAND(wlc, bandunit) {
			/* Compute Antenna Gain */
			ag[bandunit] = (wlc->bandstate[bandunit]->antgain >> 2);
		}
		break;
	}
	...
	case IOV_SVAL(IOV_ANTGAIN): {
		...
		FOREACH_WLC_BAND(wlc, j) {
			wlc_pi_band_update(wlc, j);
			/* Compute Antenna Gain */
			antgain = BAND_5G6G(wlc->band->bandtype) ? ag[1] : ag[0];
			wlc->band->antgain = wlc_antgain_calc(antgain);
		}
		break;
	}
#endif // endif
	...
	case IOV_GVAL(IOV_LIFETIME):
	{
		...
		/* Lifetime TX FIFO only supported on rev 128+ */
		if (int_val == WL_LIFETIME_TXFIFO && D11REV_GE(wlc->pub->corerev, 128)) {
			lifetime->ac = int_val;
			/* Convert 1024-us TU to milliseconds */
			lifetime->lifetime =
			    ((wlc->lifetime_txfifo << D11_REV128_LIFETIME_SHIFT) + 500) / 1000;
		} else if (int_val < AC_COUNT) {
			lifetime->ac = int_val;
	...
	case IOV_SVAL(IOV_LIFETIME):
	{
		...
		/* Lifetime for TX FIFO only supported on rev 128+ */
		if (int_val == WL_LIFETIME_TXFIFO && D11REV_GE(wlc->pub->corerev, 128)) {
			const int lifetime_max =
			    (D11_REV128_LIFETIME_MAX << D11_REV128_LIFETIME_SHIFT) / 1000;
			if (lifetime.lifetime <= lifetime_max) {
				/* Convert to 1024usec units */
				wlc->lifetime_txfifo =
				    (lifetime.lifetime * 1000) >> D11_REV128_LIFETIME_SHIFT;
				wlc_rfaware_lifetime_set(wlc, wlc->rfaware_lifetime);
			} else {
				err = BCME_BADARG;
			}
		} else if (lifetime.ac < AC_COUNT && lifetime.lifetime <= WL_LIFETIME_MAX) {
	...
	case IOV_SVAL(IOV_CACHE_CHANSWEEP):
			...
			if (int_val > 0 && int_val < MAXCHANNEL) {
				int_val |= WL_CHANSPEC_BW_20;
			}
			if (!wf_chspec_malformed((uint16)int_val))
	...
	/* This IOVar is used for getting the current rateset */
	case IOV_GVAL(IOV_CURR_RATESET): {
		...
		if (VHT_ENAB_BAND(wlc->pub, wlc->band->bandtype)) {
			...
			for (nss = 1; nss <= wlc->stf->op_txstreams; nss++) {
				...
				if (wlc->pub->associated) {
					uint8 vht_ratemask;
					if (wlc->band->bandtype == WLC_BAND_6G) {
						vht_ratemask = 0; /* no vht in 6g band */
	...
	/* This IOVar is used to get the default rateset */
	...
	/* This IOVar is used to set the given input rateset. Alters current rateset */
	...
	case IOV_GVAL(IOV_DYNTXC): {
		wl_txs_dyntxc_info_t *dyntxc = (wl_txs_dyntxc_info_t *)arg;
		dyntxc->enable = wlc->dyntxc->enable;
		dyntxc->feature = wlc->dyntxc->feature;
		dyntxc->subperiod_num = wlc->dyntxc->subperiod_num;
		dyntxc->coefset = wlc->dyntxc->coefset;
		break;
	}
	case IOV_SVAL(IOV_DYNTXC): {
		wl_txs_dyntxc_info_t *dyntxc = (wl_txs_dyntxc_info_t *)arg;
		wlc->dyntxc->enable = dyntxc->enable;
		wlc->dyntxc->feature = dyntxc->feature;
		wlc->dyntxc->subperiod_num = dyntxc->subperiod_num;
		wlc->dyntxc->coefset = dyntxc->coefset;
		wlc_txs_dyntxc_set(wlc->dyntxc);
		break;
	}
#ifdef WL11AX
...
#endif /* WL11AX */
#ifdef WL_PKTDROP_STATS
	case IOV_GVAL(IOV_GET_PKTDROP_STATS):
		wlc_pktdrop_stats(wlc, arg, params);
		break;
#endif /* WL_PKTDROP_STATS */

#if defined(PSSCB_RELEASE_WAR)
	case IOV_GVAL(IOV_PSSCB_RELEASE_CNT):
		*ret_int_ptr = wlc->pub->psscb_release_cnt;
		break;

	case IOV_SVAL(IOV_PSSCB_RELEASE_CNT):
		wlc->pub->psscb_release_cnt = (uint16) int_val;
		break;
#endif /* PSSCB_RELEASE_WAR */

#if BAND6G || defined(WL_OCE_AP)
	case IOV_GVAL(IOV_NBR_DISCOVERY_CAP): /* neighbor discovery */
		*ret_int_ptr = wlc->nbr_discovery_cap;
	break;

	case IOV_SVAL(IOV_NBR_DISCOVERY_CAP):
	{
		if (IS_WLC_NBR_DISC_CAP_BOTH_FD_BCAST_PRB_RSP_BIT_SET((uint32)int_val)) {
			WL_ERROR(("wl%d: Invalid nbr_discovery_cap[%x] parameters, both"
				" Fils discovery and 20TU bcast probe rsp frame should not"
				" be enabled at same time\n", wlc->pub->unit, (uint32)int_val));

			err = BCME_BADARG;
			break;
		}
		/* release packet, create fresh in next bcn interrupt */
		if (wlc->upr_fd_info && (wlc->upr_fd_info->pkt)) {
			PKTFREE(wlc->osh, wlc->upr_fd_info->pkt, TRUE);
			wlc->upr_fd_info->pkt = NULL;
			wlc->upr_fd_info->one_time_alloc--;
		}
		if (wlc->nbr_discovery_cap != (uint32)int_val) {
			wlc->nbr_discovery_cap = (uint32)int_val;
			/* let update probe response to trigger either UPR or
			 * FD through Software or Ucode
			 */
			wlc_update_probe_resp(wlc, TRUE);
		}
	}
	...
	case IOV_GVAL(IOV_SHORT_SSID_LIST):
		*ret_int_ptr = wlc->short_ssid_list;
	break;

	case IOV_SVAL(IOV_SHORT_SSID_LIST):
		wlc->short_ssid_list = (uint32)int_val;
	break;
#endif /* BAND6G || WL_OCE_AP */

#ifdef WL_CFG80211_NIC
	case IOV_GVAL(IOV_EXTENDED_OPS_SUPPORT):
		*ret_int_ptr = wlc->extended_ops_support;
		break;

	case IOV_SVAL(IOV_EXTENDED_OPS_SUPPORT):
		{
			wlc->extended_ops_support = int_val;
			wl_set_extended_ops_support(wlc->wl, int_val);
		}
		break;
#endif /* WL_CFG80211_NIC */

#ifdef WL_MBSSID
	case IOV_GVAL(IOV_MBSSID):
		*ret_int_ptr = wlc->pub->_mbssid_enab;
		break;

	case IOV_SVAL(IOV_MBSSID):
		wlc->pub->_mbssid_enab = int_val;
		if (!wlc->main_ap_bsscfg) {
			err = BCME_NOTAP;
			break;
		}
		wlc_bsscfg_set_ext_cap(wlc->main_ap_bsscfg, DOT11_EXT_CAP_MBSSID, bool_val);
		wlc_bsscfg_set_ext_cap(wlc->main_ap_bsscfg,
			DOT11_EXT_CAP_COMPLETE_NONTXBSSID_PROF, bool_val);
		/* update beacon/probe resp for AP */
		if (wlc->pub->up && AP_ENAB(wlc->pub)) {
			wlc_bsscfg_t *bsscfg_local;
			struct scb *bcmc_scb;
			int i;
			 ...
			 */
			FOREACH_UP_AP(wlc, i, bsscfg_local) {
				if (!BSSCFG_IS_MAIN_AP(bsscfg_local)) {
					wlc_bsscfg_set_ext_cap(bsscfg_local, DOT11_EXT_CAP_MBSSID,
						bool_val);
					wlc_bsscfg_set_ext_cap(bsscfg_local,
						DOT11_EXT_CAP_COMPLETE_NONTXBSSID_PROF, bool_val);
					bcmc_scb = WLC_BCMCSCB_GET(wlc, bsscfg_local);
	...
	bool need_ies)
{
	...
	if (bi->flags2 & WLC_BSS2_RSSI_INVALID)
		wl_bi.flags |= WL_BSS_FLAGS_RSSI_INVALID;
	if (bi->flags2 & WLC_BSS2_RSSI_INACCURATE)
		wl_bi.flags |= WL_BSS_FLAGS_RSSI_INACCURATE;
	if (bi->flags2 & WLC_BSS2_HS20)
...
#endif /* WL11AX */
	if (bi->max_mbssid_indicator) {
		wl_bi.bcnflags |= WL_BSS_BCNFLAGS_MULTIPLE_BSSID_SET;
		if (bi->mbssid_index) {
			wl_bi.bcnflags |= WL_BSS_BCNFLAGS_NONTRANSMITTED_BSSID;
		}
	}
	...
	wl_bi.qbss_chan_util = (uint8)WLC_QBSS_LOAD_CHAN_FREE_MAX - bi->qbss_load_chan_free;
...
wlc_dump_wlc(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "vendor 0x%x device 0x%x bands %s regs %p\n",
	            wlc->vendorid, wlc->deviceid, bandbuf, OSL_OBFUSCATE_BUF(wlc->regs));
...
#if defined(WL_TXPKTPEND_SYNC)
	bcm_bprintf(b, "pktpend_sync %d pktpend_drop %d\n",
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		...
		bcm_bprintf(b, "  ratespec_override 0x%x mratespec_override 0x%x\n",
			band->rspec_override, band->mrspec_override);
		bcm_bprintf(b,
			"  (ROAMThreshold, ROAMDelta)  default: (%d, %d)  current: (%d, %d)\n",
		...
		if (band->hwrs_scb != NULL) {
			bcm_bprintf(b, "\n-------- %s hwrs scb --------\n",
				wlc_bandunit_name(bandunit));
			wlc_scb_dump_scb(wlc, NULL, band->hwrs_scb, b, -1);
		}
		bcm_bprintf(b, "\n");
	}
	...
	bcm_bprintf(b, "mhf 0x%02x mhf2 0x%02x mhf3 0x%02x mhf4 0x%02x mhf5 0x%02x mhf6 0x%02x\n",
		...
		wlc_bmac_mhf_get(wlc->hw, MHF6, WLC_BAND_AUTO));
	...
	bcm_bprintf(b, "pkt_callback_reg_fail %d\n",
	...
	bcm_bprintf(b, "\tntxd = %d, nrxd = %d, rxbufsz = %d, nrxbufpost = %d, maxscb = %d\n",
	            (D11REV_GE(wlc->pub->corerev, 42)) ? tunables->ntxd_large : tunables->ntxd,
	            (D11REV_GE(wlc->pub->corerev, 42)) ? tunables->nrxd_large : tunables->nrxd,
	            tunables->rxbufsz,
	...
	bcm_bprintf(b, "\tampdu_pktq_size = %d\n", tunables->ampdu_pktq_size);
...
wlc_dump_bss_info(const char *name, wlc_bss_info_t *bi, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "SSID \"%s\" len %d ISBRCM %d ISHT %d ISVHT %d ISHE %d\n",
		...
		((bi->flags & WLC_BSS_HT) != 0), ((bi->flags2 & WLC_BSS2_VHT) != 0),
...
#endif /* WL_PSMX */
#if defined(AP)
		{WL_CNT_XTLV_WLC_AP, sizeof(wl_ap_cnt_t), wlc->pub->_ap_cnt},
#endif // endif
		{0, 0, NULL}
	};
...
static const struct devid_bandmask BCMATTACHDATA(wlc_bandmask_table)[] = {
	/* tri band 2G + 5G + 6G devices */
	{ BCM43684_D11AX2G5G6G_ID,
	((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ BCM6715_D11AX2G5G6G_ID,
	((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX) | (1 << BAND_6G_INDEX)) },

	/* dual band 2G + 5G devices */
	{ BCM6362_D11N_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4360_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4352_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43452_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43602_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4352_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43569_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4359_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4369_D11AX_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4377_D11AX_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4347_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM6878_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4361_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4365_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43684_D11AX_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ EMBEDDED_2x2AX_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ EMBEDDED_2x2AX160_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4366_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM53573_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM47189_D11AC_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM6710_D11AX_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43692_D11AX_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM6715_D11AX_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },

	/* dual band 2G + 6G devices */
	{ EMBEDDED_2x2AX_DEV2G6G_ID,	((1 << BAND_2G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ EMBEDDED_2x2AX160_DEV2G6G_ID,	((1 << BAND_2G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ BCM43684_D11AX2G6G_ID,	((1 << BAND_2G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ BCM6710_D11AX2G6G_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ BCM6715_D11AX2G6G_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_6G_INDEX)) },

	/* dual band 5G + 6G devices */
	{ EMBEDDED_2x2AX_DEV5G6G_ID,	((1 << BAND_5G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ EMBEDDED_2x2AX160_DEV5G6G_ID,	((1 << BAND_5G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ BCM43684_D11AX5G6G_ID,	((1 << BAND_5G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ BCM6710_D11AX5G6G_ID,		((1 << BAND_5G_INDEX) | (1 << BAND_6G_INDEX)) },
	{ BCM6715_D11AX5G6G_ID,		((1 << BAND_5G_INDEX) | (1 << BAND_6G_INDEX)) },

	/* single band 2G devices */
	{ BCM43217_D11N2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM4360_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM4352_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM43569_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM43570_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM43602_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM53573_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM47189_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM4347_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM6878_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM4365_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM4366_D11AC2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM43684_D11AX2G_ID,		(1 << BAND_2G_INDEX) },
	{ EMBEDDED_2x2AX_DEV2G_ID,	(1 << BAND_2G_INDEX) },
	{ EMBEDDED_2x2AX160_DEV2G_ID,	(1 << BAND_2G_INDEX) },
	{ BCM6715_D11AX2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM6710_D11AX2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM43692_D11AX2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM6362_D11N2G_ID,		(1 << BAND_2G_INDEX) },
	{ BCM43227_D11N2G_ID,		(1 << BAND_2G_INDEX) },

	/* single band 5G devices */
	{ BCM4360_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM4352_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM43602_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM4365_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM4366_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM4347_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM53573_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM47189_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM43684_D11AX5G_ID,		(1 << BAND_5G_INDEX) },
	{ EMBEDDED_2x2AX_DEV5G_ID,	(1 << BAND_5G_INDEX) },
	{ EMBEDDED_2x2AX160_DEV5G_ID,	(1 << BAND_5G_INDEX) },
	{ BCM6710_D11AX5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM43692_D11AX5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM6715_D11AX5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM6878_D11AC5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM6362_D11N5G_ID,		(1 << BAND_5G_INDEX) },
	{ BCM43569_D11AC5G_ID,		(1 << BAND_5G_INDEX) },

	/* single band 6G devices */
	{ EMBEDDED_2x2AX_DEV6G_ID,	(1 << BAND_6G_INDEX) },
	{ EMBEDDED_2x2AX160_DEV6G_ID,	(1 << BAND_6G_INDEX) },
	{ BCM43684_D11AX6G_ID,		(1 << BAND_6G_INDEX) },
	{ BCM6710_D11AX6G_ID,		(1 << BAND_6G_INDEX) },
	{ BCM6715_D11AX6G_ID,		(1 << BAND_6G_INDEX) },

	/* chips with blank OTP */
	{ BCM6710_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM6715_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4347_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43227_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43602_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43462_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43522_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43684_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43569_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43570_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4352_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM4360_CHIP_ID,		((1 << BAND_2G_INDEX) | (1 << BAND_5G_INDEX)) },
	{ BCM43217_CHIP_ID,		(1 << BAND_2G_INDEX) },
...
uint
BCMATTACHFN(wlc_get_bandmask)(uint16 deviceid)
{
	int i = 0;

	while (wlc_bandmask_table[i].deviceid != 0) {
		if (wlc_bandmask_table[i].deviceid == deviceid) {
			return wlc_bandmask_table[i].bandmask;
		}
		i++;
	}
...
#else
	int32 offset = 1000 * (OSL_SYSUPTIME() - wlc->lifetime_osltime_cache);
	/* now(us) = TSF cache(us) + elapsed system time(ms * 1000) since TSF cache */
	return wlc->lifetime_tsftimer_cache + offset;
#endif /* DONGLEBUILD */
}
...
{
#if !defined(DONGLEBUILD)
/* Host clock updates asynchronously to MAC's TSF. This introduces errors between
 * actual TSF and the packet queue time in the TXD, proportional to the granularity
 * of the host clock. Subtract a max error to ensure queue time does not appear to
 * be in the future when ucode computes (age = TSF(now) - queuetime < 0). If age < 0,
 * ucode suppresses the packet.
 */
#define CLK_ERR_MAX 2000 /* microseconds */
	if (wlc->pub->up) {
		wlc->lifetime_tsftimer_cache = R_REG(wlc->osh, D11_TSFTimerLow(wlc)) - CLK_ERR_MAX;
...
{
	int evict_prec = -1;
	return wlc_prec_enq_head_ex(wlc, q, pkt, prec, head, &evict_prec);
}

bool BCMFASTPATH
wlc_prec_enq_head_ex(wlc_info_t *wlc, struct pktq *q, void *pkt,
	int prec, bool head, int *evict_prec)
{
	void *p;
	void *dqp = NULL;
	...
	if (q->pktqlog) {
		...
		if (prec_cnt == NULL && (q->pktqlog->_prec_log & PKTQ_LOG_AUTO)) {
			prec_cnt = wlc_txq_prec_log_enable(wlc, q, (uint32)prec, TRUE);
	...
	if (prec_cnt)
	{
		uint32 qlen = pktqprec_n_pkts(q, prec);
...
wlc_send80211_specified(wlc_info_t *wlc, void *sdu, ratespec_t rspec, struct wlc_if *wlcif)
{
	 ...
	 */
	scb = wlc->bandstate[CHSPEC_BANDUNIT(chanspec)]->hwrs_scb;
	if (!CHSPEC_IS2G(chanspec)) {
		...
		if (RSPEC_ISCCK(rspec)) {
			...
			WL_ERROR(("wl%d: %s: rate override with DSSS rate "
			          "incompatible with 5GHz/6Ghz band\n",
			          wlc->pub->unit, __FUNCTION__));
		}
	}
...
#define WLPKTTAG_FLAGS_MOVE(pkt_from, pkt_to) \
	do { \
		...
		WLPKTTAG(pkt_to)->flags3 = WLPKTTAG(pkt_from)->flags3 & \
			(WLF3_NO_PMCHANGE | WLF3_DATA_WOWL_PKT | WLF3_TAF_TAGGED | \
			 WLF3_BYPASS_AMPDU); \
	} while (0)
...
wlc_pkttag_info_move(wlc_info_t *wlc, void *pkt_from, void *pkt_to)
{
	...
	WLPKTTAGSCBSET(pkt_to, WLPKTTAGSCBGET(pkt_from));
	WLPKTTAGBSSCFGSET(pkt_to, WLPKTTAGBSSCFGGET(pkt_from));
...
/**
 * Copies packet flags from a source packet to a (non-last) generated fragment.
 * Contrary to wlc_pkttag_info_move, source packet is not affected.
 */
void
wlc_pkttag_info_frag_cpy(wlc_info_t *wlc, void *pkt_from, void *pkt_to)
{
	/* Make sure not copying to same packet */
	ASSERT(pkt_from != pkt_to);
	...
	WLFC_PKTAG_INFO_MOVE(pkt_from, pkt_to);
	WLPKTTAG_FLAGS_COPY(pkt_from, pkt_to);
	/* Following fields are zero-ed for non-last frags */
	WLPKTTAG(pkt_to)->flags2 = 0;
	WLPKTTAG(pkt_to)->flags3 = 0;
	WLPKTTAG(pkt_to)->callbackidx = 0;
	WLPKTTAG(pkt_to)->shared.packetid = 0;
	/* following fields are cleared (using RX side fields) to clear possible TAF info */
	WLPKTTAG(pkt_to)->pktinfo.misc.snr = 0;
	WLPKTTAG(pkt_to)->pktinfo.misc.rssi = 0;
}
...
/** Create PLCP corresponding to the caller supplied rspec */
void
 ...
 *
 * @param[plcp] out  Starts with HE-SIGA field, *not* the L-STF field.
 *
	...
	uint16 fc, uint8 *plcp)
{
	...
	/**
	 * PLCP bits [20:19] indicate bandwidth :
 ...
 */
uint16
wlc_compute_frame_dur(wlc_info_t *wlc, enum wlc_bandunit bandunit, ratespec_t rate,
	uint8 preamble_type, uint next_frag_len)
{
	uint16 dur, sifs;

	sifs = BANDUNIT_SIFS(bandunit);
 ...
 */
uint16
wlc_compute_rtscts_dur(wlc_info_t *wlc, enum wlc_bandunit bandunit, bool cts_only,
...
wlc_tx_resume(wlc_info_t *wlc)
{
	...
	if (!silence && !wlc_quiet_chanspec(wlc->cmi, phy_utils_get_chanspec(WLC_PI(wlc)))) {
...
#if defined(DEBUG_TBTT) || (defined(WL_OCE) && defined(WL_OCE_AP))
	if (WL_INFORM_ON() || OCE_ENAB(wlc->pub)) {
		wlc->prev_TBTT = R_REG(wlc->osh, D11_TSFTimerLow(wlc));
	}
#endif /* DEBUG_TBTT */
...
/* Need AP defined here to print out capability only if enabled */
	...
	bcm_bprintf(b, "slaveradar ");
#endif // endif
#if defined(SW_PAGING)
	bcm_bprintf(b, "swpaging ");
#endif // endif
#ifdef WFA_6G_NEW_CHANNELIZATION
	bcm_bprintf(b, "wfapf2 ");
...
wlc_set_pmenabled(wlc_bsscfg_t *cfg, bool state)
{
	...
	if ((WLC_BSSCFG_IDX(cfg) == 0) && (cfg->BSS))
		phy_noise_pmstate_set(WLC_PI(cfg->wlc), state);
...
void
wlc_module_set_pmstate(wlc_bsscfg_t *cfg, bool state, mbool moduleId)
{
...
*/
bool
wlc_maccore_wake_state(wlc_info_t *wlc)
{
	int bss_idx;
	wlc_bsscfg_t *cfg;

	FOREACH_AS_BSS(wlc, bss_idx, cfg) {
...
#ifdef WLMCNX
	    (!MCNX_ENAB(wlc->pub) && !BSSCFG_IS_PRIMARY(cfg)) ||
#endif // endif
	    FALSE) {
...
wlc_update_pmstate(wlc_bsscfg_t *cfg, uint txstatus)
{
	...
	if (!BSSCFG_STA(cfg) ||
	    ((BSSCFG_IBSS(cfg) || !cfg->associated) && !BSS_TDLS_ENAB(wlc, cfg)))
		return;
...
wlc_do_chanswitch(wlc_bsscfg_t *cfg, chanspec_t newchspec)
{
	...
	WL_REGULATORY(("wl%d: %s: new chanspec %s\n", wlc->pub->unit, __FUNCTION__,
	...
	if (CHSPEC_BANDUNIT(wlc->home_chanspec) != CHSPEC_BANDUNIT(newchspec))
		band_chg = TRUE;
	...
	if (BSS_TDLS_ENAB(wlc, cfg) || BSS_PROXD_ENAB(wlc, cfg)) {
		WL_ERROR(("wl%d.%d: %s: Not supported yet\n",
			wlc->pub->unit, cfg->_idx, __FUNCTION__));
		return;
	}

	/* cater specific MAP requirement: User wants to perform CAC on new chanspec
	 * but after performing CAC, Radio should switch back to it's previous chanspec
	 */
	if (to_radar && BSSCFG_AP(cfg) &&
		WLC_IS_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc)) {
		/* let DFS state machine to store current chanspec befor switching to
		 * new chanspec, later after performing CAC on newchspec,
		 * Firmware switch back to cfg->current_bss->chanspec.
		 */
		wlc_dfs_save_old_chanspec(wlc->dfs, cfg->current_bss->chanspec);
	}
	...
	if (!BSSCFG_PSTA(wlc->primary_bsscfg)) { /* checking primary cfg */
		 ...
		 * with new chanspec
		 */
		if (BSSCFG_STA(wlc->primary_bsscfg)) { /* checking primary cfg */
			wlc_sync_txfifo_all(wlc, wlc->active_queue, SUPPRESS_FLUSH_FIFO);
			wlc_sta_timeslot_register(wlc->primary_bsscfg); /* checking primary cfg */
			/* before doing bsscfg up operation, enable sta cfg
			 * if down
			 */
			if (!wlc->primary_bsscfg->enable) {
				wlc_bsscfg_enable(wlc, wlc->primary_bsscfg);
			}
		}
#endif /* STA */
...
#ifdef SLAVE_RADAR
	if (WLDFS_ENAB(wlc->pub) && (to_radar)) {
		if (WL11H_STA_ENAB(wlc) && wlc_dfs_get_radar(wlc->dfs)) {
			if (!wlc_is_ap_interface_up(wlc) && !from_radar &&
	...
	*/
	if (band_chg || ETHER_ISNULLADDR(&cfg->BSSID)) {
#ifdef STA
		if (BSSCFG_STA(cfg) && cfg->BSS) {
			if (!BSSCFG_PSTA(cfg)) {
...
wlc_bcn_li_upd(wlc_info_t *wlc)
{
	...
	if (wlc->bcn_li_dtim == 1)
		wlc_write_shm(wlc, M_PS_MORE_DTIM_TBTT(wlc), 0);
	else
	...
	if (MCNX_ENAB(wlc->pub)) {
		if (wlc->bcn_li_dtim == 1) {
			wlc_mcnx_multidtim_upd(wlc->mcnx, wlc->primary_bsscfg, FALSE);
		} else {
			wlc_mcnx_multidtim_upd(wlc->mcnx, wlc->primary_bsscfg, TRUE);
		}
	}
#endif /* WLMCNX */
}
...
wlc_enable_probe_req(wlc_info_t *wlc, uint32 mask, uint32 val)
{
	...
	if (!wlc->pub->up) {
#ifndef ATE_BUILD
		WL_INFORM(("wl%d: %s: state down, deferring setting of host flags\n",
			wlc->pub->unit, __FUNCTION__));
#endif // endif
		return;
	}
	...
	chanspec_t new_chspec)
{
	...
	if (!wlc_is_shared_chanspec(wlc, bsscfg)) {
		phy_chanmgr_destroy_ctx(bpi, bsscfg->current_bss->chanspec);
	}
#endif // endif
	...
	if (phy_chanmgr_create_ctx(bpi, new_chspec) == BCME_OK) {
	...
	if (wlc->chanspec != new_chspec) {
		...
		if (WLC_MODESW_ENAB(wlc->pub)) {
			...
			{
				phy_chanmgr_create_ctx(WLC_PI(wlc), new_chspec);
			}
		}
#endif /* WL_MODESW */
#endif /* !WLMCHAN */
 ...
 * bandunit - BAND_2G_INDEX, BAND_5G_INDEX, ..
...
wlc_is_valid_bw(wlc_info_t *wlc, wlc_bsscfg_t *cfg, enum wlc_bandunit bandunit, uint16 bw)
{
	uint16 locale_flags;
	uint8 bu = bandunit;
	uint16 bandtype = wlc_bandunit2bandtype(bu);
	wlcband_t * band;
	bool band5or6 = (bu == BAND_5G_INDEX && VHT_ENAB_BAND(wlc->pub, bandtype)) ||
	                (bu == BAND_6G_INDEX && HE_ENAB_BAND(wlc->pub, bandtype));

	ASSERT(wlc != NULL);
	ASSERT(cfg != NULL);

	band = wlc->bandstate[bu];

	WL_INFORM(("wl%d: %s: given bandunit: %d, bw: 0x%2x", wlc->pub->unit, __FUNCTION__,
			bu, bw));

	locale_flags = wlc_channel_locale_flags_in_band(wlc->cmi, bu);

	switch (bw) {
#ifdef WL11AC_80P80
		case WL_CHANSPEC_BW_8080:
			return (band5or6 &&
			        !(locale_flags & WLC_NO_160MHZ) && WL_BW_CAP_160MHZ(band->bw_cap));
#endif /* WL11AC_80P80 */
		case WL_CHANSPEC_BW_160:
			return (band5or6 &&
			        !(locale_flags & WLC_NO_160MHZ) && WL_BW_CAP_160MHZ(band->bw_cap));
		case WL_CHANSPEC_BW_80:
			return (band5or6 && !(locale_flags & WLC_NO_80MHZ) &&
	...
	dot11_management_header_t *mgmt, uint32 len)
{
	...
	scb = wlc_scbfind(wlc, bsscfg, &mgmt->da);
	if (scb == NULL) {
		WL_ERROR(("wl%d.%d could not find scb\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
		return BCME_ERROR;
	}

	if (scb->WPA_auth == WPA_AUTH_OWE) {
		bcm_tlv_t *dh_param_ie;
		uint offset = DOT11_MGMT_HDR_LEN + DOT11_ASSOC_RESP_FIXED_LEN;

		/* Find Diffie-Hellman Parameter element */
		dh_param_ie = bcm_find_tlv_ext((uint8 *)mgmt + offset, len - offset,
				DOT11_MNG_ID_EXT_ID, EXT_MNG_DH_PARAM_ID);

		if (dh_param_ie != NULL) {
			scb->dh_params =
				(dh_params_ie_t *) MALLOC(wlc->osh, sizeof(dh_params_ie_t));
			if (scb->dh_params == NULL) {
				WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
					wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
				scb->dh_params_included = 0;
				return BCME_NOMEM;
			}
			/* Save Diffie Hellman IE now, it should be added in the assoc response
			 * generated by firmware.
			 */
			bcopy(dh_param_ie, scb->dh_params, dh_param_ie->len + TLV_HDR_LEN);
			scb->dh_params_included = 1;
		} else {
			bcm_tlv_t *rsn_ie;
			/* Find RSN Information element */
			rsn_ie = bcm_find_tlv((uint8 *)mgmt + offset, len - offset,
					DOT11_MNG_RSN_ID);
			if (rsn_ie != NULL) {
				scb->rsn_ie =
					(bcm_tlv_t *) MALLOC(wlc->osh, rsn_ie->len + TLV_HDR_LEN);
				if (scb->rsn_ie == NULL) {
					WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
						wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
					scb->dh_params_included = 0;
				...
				/* Save RSN IE now, it should be added in the assoc response
				 * generated by firmware.
				 */
				bcopy(rsn_ie, scb->rsn_ie, rsn_ie->len + TLV_HDR_LEN);
			}
			scb->dh_params_included = 0;
		}
	}

	assoc_resp = (struct dot11_assoc_resp *)((uint8 *)mgmt + DOT11_MGMT_HDR_LEN);

	bzero(&decision, sizeof(decision));

	decision.reject_reason = ltoh16(assoc_resp->status);
	if (decision.reject_reason == DOT11_SC_SUCCESS) {
		decision.assoc_approved = TRUE;
	}
	bcopy(&mgmt->da, &decision.da, ETHER_ADDR_LEN);
	...
	dot11_management_header_t *mgmt, uint32 len)
{
	...
	WL_ASSOC(("wl%d: %s: auth frame for MAC %s\n", wlc->pub->unit, __FUNCTION__,
		bcm_ether_ntoa((struct ether_addr *)&mgmt->da, eabuf)));
#endif /* WLMSG_ASSOC */
	...
	if ((pkt = wlc_frame_get_mgmt(wlc, FC_AUTH, &target_mac, &bsscfg->cur_etheraddr,
			&bssid, len - DOT11_MGMT_HDR_LEN, &auth_frm)) == NULL) {
	...
	WL_ASSOC(("wl%d: %s JOIN: sending Auth %d ...\n", WLCWLUNIT(wlc), __FUNCTION__,
		ltoh16(auth->seq)));
	...
	if (BSSCFG_EXT_AUTH(bsscfg)) {
		if ((ret = wlc_sae_assoc_change_state(bsscfg, auth)) != BCME_OK) {
			WL_ERROR(("wl%d: %s:  Wrong Sae Auth seq number \n",
				wlc->pub->unit, __FUNCTION__));
			TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_SAE_AUTH_SEQ_NMBR);
			...
			WL_ERROR(("wl%d: %s:  Couldn't register the tx complete cb \n",
				wlc->pub->unit, __FUNCTION__));
			TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_TX_CB_REGISTRATION);
		}
		if (ret != BCME_OK) {
...
wlc_set_ratetable(wlc_info_t *wlc)
{
	...
	wlc_rateset_ht_mcs_upd(&rs, wlc->stf->op_txstreams);
	wlc_rateset_filter(&rs, &rs, FALSE, WLC_RATES_CCK_OFDM, RATE_MASK_FULL,
	                   wlc_get_mcsallow(wlc, wlc->primary_bsscfg));
 ...
 *
 * Note: the bandwidth bits in the rspec are not checked.
 */
bool
wlc_valid_rate(wlc_info_t *wlc, ratespec_t rspec, int band, bool verbose)
{
	wlc_rateset_t *hw_rateset = NULL;
	uint i;

	if (band == WLC_BAND_AUTO) {
		hw_rateset = &wlc->band->hw_rateset;
	} else {
		wlcband_t *b = wlc_get_band_by_bandtype(wlc, band);
		if (b != NULL) {
			hw_rateset = &b->hw_rateset;
		}
	}

	if (hw_rateset == NULL) {
		return (FALSE); /* device does not support the caller supplied band */
	}
...
wlc_mod_prb_rsp_rate_table(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint frame_len)
{
	...
	sifs = BANDUNIT_SIFS(CHSPEC_BANDUNIT(cfg->current_bss->chanspec));
	rs_dflt = wlc_rateset_get_hwrs(wlc);
	ASSERT(rs_dflt != NULL);
...
wlc_compute_bcntsfoff(wlc_info_t *wlc, ratespec_t rspec, bool short_preamble, bool phydelay)
{
	...
	if (RSPEC_ISHE(rspec)) {
		...
		WL_ERROR(("wl%d: xmit beacon with mcs rate; rspec 0x%x\n",
			wlc->pub->unit, rspec));
	} else if (RSPEC_ISOFDM(rspec)) {
...
wlc_erp_find(wlc_info_t *wlc, void *body, uint body_len, uint8 **erp, int *len)
{
	...
	ie = bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_ERP_ID);
...
/**
 *      Calculates a beacon or probe template and writes it into d11 template memory. The template
 *      contains a plcp and the beacon (or probe) mpdu contents. It does not contain PHY control
 *      words. Beacon PHY control words are constructed in a different driver function.
 *
                     ...
                     uint16 *buf, int *len)
{
	...
	wlc_bcn_prb_body(wlc, type, cfg, body, &body_len); /* mpdu without phy header */
	...
	*len = hdr_len + body_len; /* return actual size */
	...
	if (!MBSS_BCN_ENAB(wlc, cfg) && type == FC_BEACON) {
		...
		uint8 *plcp; /**< points at memory to be written */
	...
	uint16 *bcn, int len, ratespec_t bcn_rspec, bool suspend)
{
	...
	if (BSSCFG_AP(cfg) && !MBSS_BCN_ENAB(wlc, cfg)) {
		...
		cp = (uint8 *)bcm_find_tlv(cp, len, DOT11_MNG_TIM_ID);
		if (cp == NULL) {
...
wlc_bss_set_bcn_rspec(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bss)
{
	...
	/* AP */
	if (BSSCFG_AP(cfg)) {
		rspec = wlc_ap_forced_bcn_rspec_get(wlc);
	}
	...
	ASSERT(wlc_valid_rate(wlc, rspec, CHSPEC_BANDTYPE(bss->chanspec), TRUE));
...
_wlc_bss_update_beacon(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (HWBCN_ENAB(cfg)) { /* Hardware beaconing for this config */
		...
		wlc_bss_set_bcn_rspec(wlc, cfg, current_bss); /* writes wlc->bcn_rspec */
...
wlc_bss_update_probe_resp(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool suspend)
{
	...
	if (BAND_6G(wlc->band->bandtype)) {
		if (WL_UPR_FD_SW_ENAB(wlc)) {
			...
			wlc_ap_bcast_probe_rsp_update_required(wlc, cfg);
		} else if (WL_UPR_FD_UCODE_ENAB(wlc)) {
			wlc_update_upr_fd_template(wlc, TRUE);
		} else {
			/* disable both */
			val = wlc_bmac_mhf_get(wlc->hw, (uint8)0x01, WLC_BAND_AUTO);
			val &= ~MHF2_UNSOL_PRS;
...
/* Beacon and Probe Response frame body, excluding PLCP
 *  max sz  order
 *  8       1 Timestamp
 *  2       2 Beacon interval
...
wlc_bcn_prb_body(wlc_info_t *wlc, uint type, wlc_bsscfg_t *bsscfg, uint8 *bcn, int *len)
{
	...
	cbparm.ht = (BSS_N_ENAB(wlc, bsscfg) && ht_cap) ? TRUE : FALSE;
#ifndef BAND6G_USING_HT_IES
	cbparm.ht = cbparm.ht && cbparm.bandunit != BAND_6G_INDEX;
#endif /* !BAND6G_USING_HT_IES */
#ifdef WL11AC
	cbparm.vht = (BSS_VHT_ENAB(wlc, bsscfg) && ht_cap) ? TRUE : FALSE;
#ifndef BAND6G_USING_VHT_IES
	cbparm.vht = cbparm.vht && cbparm.bandunit != BAND_6G_INDEX;
#endif /* !BAND6G_USING_VHT_IES */
#endif /* WL11AC */
#ifdef WL11AX
	cbparm.he = BSS_HE_ENAB_BAND(wlc, wlc->bandstate[cbparm.bandunit]->bandtype, bsscfg);
	if (cbparm.bandunit != BAND_6G_INDEX) {
		cbparm.he = cbparm.he && ht_cap;
	}
#endif // endif
...
wlc_bss_update_brcm_ie(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_AP(cfg)) {
#ifdef WDS
		if (wlc_wds_lazywds_is_enable(wlc->mwds) && BSSCFG_IS_PRIMARY(cfg))
			flags |= BRF_LZWDS;
#endif // endif
#ifdef BCM_DCS
	...
	ASSERT(brcm_ie->flags2 == 0);

	/* Some STAs have interop issues after adding an additional byte to brcm_ie
	 * for storing new flags (flags2), JIRA:BCAWLAN-220495. Currently, flags2 is
	 * used only for carrying DTPC capability bit. This bit is needed only for the
	 * transmission of IE from STA to AP, not from AP to STA, and therefore we do
	 * not send out the additional byte of brcm_ie for avoiding such interop issues.
	 * Assuming flags2 is the last data member of brcm_ie.
	 */
	brcm_ie->len = BRCM_IE_LEN - TLV_HDR_LEN - sizeof(brcm_ie->flags2);
...
wlc_process_brcm_ie(wlc_info_t *wlc, struct scb *scb, brcm_ie_t *brcm_ie)
{
	...
	/* Stop proceeding if it is the older version of brcm ie that does not have flags2 field */
	if (brcm_ie->len <= (OFFSETOF(brcm_ie_t, flags2) - TLV_HDR_LEN)) {
		return;
	}

#ifdef WLC_DTPC
	if ((brcm_ie->flags2 & BRF2_DTPC_RXCAP) && BSSCFG_AP(SCB_BSSCFG(scb))) {
		scb->flags |= SCB_DTPCCAP;
	}
#endif // endif
} /* wlc_process_brcm_ie */
	...
	const struct ether_addr *sa)
{
	...
	hdr->durid = htol16(cfg->AID | ~DOT11_AID_MASK);
...
wlc_frame_get_ctl(wlc_info_t *wlc, uint len)
{
	...
	osh = wlc->osh;
	if ((p = PKTGET(osh, (wlc->txhroff + len), TRUE)) == NULL) {
		WL_ERROR(("wl%d: %s: pktget error for len %d\n",
			wlc->pub->unit, __FUNCTION__, (wlc->txhroff + len)));
		WLCNTINCR(wlc->pub->_cnt->txnobuf);
		return (NULL);
	}
	...
	uint8 **pbody, uint8 cat)
{
	...
	if (WLC_MFP_ENAB(wlc->pub)) {
		p = wlc_mfp_frame_get_mgmt(wlc->mfp, fc, cat, da, sa, bssid, body_len, pbody, TRUE);
	} else {
    ...
    uint8 **pbody, uint iv_len, uint tail_len)
{
	...
	if ((p = PKTGET(osh, (wlc->txhroff + len), TRUE)) == NULL) {
		...
		uint32 req_len = wlc->txhroff + len;
		...
		if (p == NULL) {
			WL_ERROR(("wl%d: wlc_frame_get_mgmt: pktget error for len %d fc %x\n",
				wlc->pub->unit, (wlc->txhroff + len), fc));
			WLCNTINCR(wlc->pub->_cnt->txnobuf);
			return (NULL);
		}
	}
	...
	*pbody = (uint8*)&hdr[1] + iv_len;
	...
	WL_WSEC(("wl%d: %s: allocated %u bytes; iv %u and tail %u bytes\n",
	         wlc->pub->unit, __FUNCTION__, (wlc->txhroff + len), iv_len, tail_len));
#endif /* WLP2P && BCMDBG */
...
/*
 * return TRUE on succes, toss_reason is not assigned in that case. On failure returns FALSE
 * and toss_reason gives reason of failure.
 */
static bool
wlc_queue_frag(wlc_info_t *wlc, void *p, wlc_bsscfg_t *bsscfg, scb_t *scb, bool ctrl_frame,
	wlc_tx_pktdrop_reason_t *toss_reason, wlc_txq_info_t *qi, bool enq_only)

{
	...
	prio = PKTPRIO(p);
	ASSERT(prio <= MAXPRIO);
	prec = WLC_PRIO_TO_HI_PREC(prio);

	/* Determine if packet should go to APPS or Common Q. If AP and SCB is in PS or
	 * AP and SCB is using TWT then use APPS.
	 */
	if (BSSCFG_AP(bsscfg) &&
		((SCB_PS(scb) && !(WLPKTTAG(p)->flags & WLF_PSDONTQ)) || SCB_TWTPS(scb))) {

		if (wlc_apps_psq(wlc, p, prec)) {
			return TRUE;
		} else {
			if (ctrl_frame) {
				*toss_reason = TX_PKTDROP_RSN_NOT_PSQ2;
			} else {
				*toss_reason = TX_PKTDROP_RSN_NOT_PSQ1;
			}
			return FALSE;
		}
	}

	if (!wlc_ctrlmgmt_pkt_prec((uchar *)PKTDATA(wlc->osh, p))) {
		prec = WLC_PRIO_TO_PREC(prio);
	}

	if (cpktq_prec_enq(wlc, &qi->cpktq, p, prec)) {
		if (!enq_only) {
			wlc_send_q(wlc, qi);
		}
		return TRUE;
	...
	*toss_reason = TX_PKTDROP_RSN_NO_BUF;
	return FALSE;
}
	...
	wlc_bsscfg_t *bsscfg, bool short_preamble, wlc_key_t *key, ratespec_t rate_override)
{
	...
	ether_ismulti = ETHER_ISMULTI(&hdr->da);
	...
	/* Allow deauth frames to be sent when scb is marked for deletion */
	if (SCB_DEL_IN_PROGRESS(scb) ||
		(SCB_MARKED_DEL(scb) &&
		!((FC_TYPE(ltoh16(hdr->fc)) == FC_TYPE_MNG) && (fk == FC_DEAUTH)))) {
		WLPKTTAGBSSCFGSET(p, WLC_BSSCFG_IDX(bsscfg));
	...
	if (WLC_MFP_ENAB(wlc->pub) && (WLPKTTAG(p)->flags & WLF_MFP) && key == NULL) {
		/* In a Repeater, BCMC is usually done by the AP interface. The BSSCFG
		 * derived from HWratesetSCB of repeater will be a STA(primary) bsscfg.
		 * We need the AP's BSSCFG for using the keymgmt hence get the AP's BSSCFG.
		 * After getting AP's BSSCFG, if we have BCMC SCB, use it for SCB.
		 */
		if (APSTA_ENAB(wlc->pub) && ether_ismulti) {
			bsscfg = wlc_bsscfg_find_by_bssid(wlc, &hdr->bssid);
			/* If new bsscfg is AP, use BCMC SCB instead of HWratesetSCB */
			if (BSSCFG_AP(bsscfg) && bsscfg->bcmc_scb) {
				scb = bsscfg->bcmc_scb;
			}
		}

		if (ether_ismulti && BSSCFG_AP(bsscfg)) {
			key = wlc_keymgmt_get_bss_tx_key(wlc->keymgmt, bsscfg, TRUE, NULL);
		} else if (!ether_ismulti && SCB_MFP(scb)) {
...
#ifdef MFP
	/* if valid key and MFP, already start sw encrypt and create tx header */
	if (WLC_MFP_ENAB(wlc->pub) && (WLPKTTAG(p)->flags & WLF_MFP) && key != NULL) {
		uint fifo;
		WLPKTTAGSCBSET(p, scb);
		WLPKTTAGBSSCFGSET(p, WLC_BSSCFG_IDX(bsscfg));
		wlc_scb_get_txfifo(wlc, scb, p, &fifo);
		wlc_pdu_txhdr(wlc, p, scb, fifo);
	}
#endif /* MFP */

	if (wlc_queue_frag(wlc, p, bsscfg, scb, FALSE, &toss_reason, qi, FALSE)) {
		return TRUE;
	}
...
#endif // endif
#ifdef WL11K
	wlc_rrm_stat_qos_counter(wlc, scb, PKTPRIO(p), OFFSETOF(rrm_stat_group_qos_t, txfail));
#endif // endif
	...
	ratespec_t rate_override, bool enq_only)
{
	...
	if (wlc_queue_frag(wlc, p, scb->bsscfg, scb, TRUE, &toss_reason, qi, enq_only)) {
		return TRUE;
	}
 ...
 * This may not be the correct behavior because we are passing the wlc->primary_bsscfg when scanning
 */
void
wlc_sendprobe(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
	const uint8 ssid[], int ssid_len, int n_short_ssid, uint32 short_ssid[],
	...
	ratespec_t ratespec_override, uint8 *extra_ie, int extra_ie_len)
{
	...
	ftcbparm.prbreq.short_ssid = short_ssid;
	ftcbparm.prbreq.short_ssid_len = n_short_ssid * sizeof(uint32);
	bzero(&cbparm, sizeof(cbparm));
	cbparm.ft = &ftcbparm;
...
#ifndef BAND6G_USING_HT_IES
	cbparm.ht = cbparm.ht && BSSCFG_BANDUNIT(bsscfg) != BAND_6G_INDEX;
#endif /* !BAND6G_USING_HT_IES */
#ifdef WL11AC
	cbparm.vht = BSS_VHT_ENAB(wlc, bsscfg);
#ifndef BAND6G_USING_VHT_IES
	cbparm.vht = cbparm.vht && BSSCFG_BANDUNIT(bsscfg) != BAND_6G_INDEX;
#endif /* !BAND6G_USING_VHT_IES */
#endif /* WL11AC */
#ifdef WL11AX
	cbparm.he = BSS_HE_ENAB_BAND(wlc, wlc->bandstate[cbparm.bandunit]->bandtype, bsscfg) ?
	...
	if (wlc_iem_build_frame(wlc->iemi, bsscfg, FC_PROBE_REQ, NULL, &cbparm,
		...
		WL_ERROR(("wl%d: %s: wlc_iem_build_frame failed\n",
		          wlc->pub->unit, __FUNCTION__));
		TX_PKTDROP_COUNT(wlc, NULL, TX_PKTDROP_RSN_IEM_BLD_FRAME_FAIL);
		PKTFREE(wlc->osh, pkt, TRUE);
		return;
	}
	...
	void *tx_cplt_ctx)
{
	...
	if ((auth_status == DOT11_SC_SUCCESS) &&
		...
		if (key_info.algo == CRYPTO_ALGO_OFF) {
			wlc_bsscfg_t *tx_cfg;
			tx_cfg = (BSSCFG_PSTA(cfg)) ? wlc->primary_bsscfg : cfg;
	...
	if (tx_cplt_fn != NULL) {
		if (wlc_pcb_fn_register(wlc->pcb, tx_cplt_fn, tx_cplt_ctx, pkt) != BCME_OK) {
			TX_PKTDROP_COUNT(wlc, scb, TX_PKTDROP_RSN_PCBFN_REGISTER);
			PKTFREE(wlc->osh, pkt, TRUE);
			return NULL;
		}
	}
...
#if defined(BCMDBG) || defined(WLMSG_PRPKT)
	WL_ASSOC(("wl%d: %s JOIN: sending Auth %d ...\n", WLCWLUNIT(wlc), __FUNCTION__, auth_seq));
	...
	if (CEVENT_STATE(wlc->pub)) {
		wlc_send_cevent(wlc, cfg, SCB_EA(scb), (uint16)auth_status, 0, 0, NULL, 0,
			CEVENT_D2C_ST_AUTH_TX, CEVENT_D2C_FLAG_QUEUED | CEVENT_FRAME_DIR_TX);
	}
#endif /* BCM_CEVENT */
                 ...
                 uint32 pktflags, int prio)
{
	...
	if (qos) {
		fc = FC_QOS_NULL;
		len += DOT11_QOS_LEN;
		htc_enab = wlc_he_htc_tx(wlc, scb, p, &htc_code);
		if (htc_enab) {
			fc |= FC_ORDER;
			len += DOT11_HTC_LEN;
		}
...
wlc_nulldata_template(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, struct ether_addr *da)
{
	...
	if (bsscfg->PLCPHdr_override == WLC_PLCP_LONG)
		shortpreamble = FALSE;
	else
		shortpreamble = TRUE;

	rspec = wlc_lowest_basic_rspec(wlc, &bsscfg->current_bss->rateset);
	ASSERT(wlc_valid_rate(wlc, rspec, CHSPEC_BANDTYPE(bsscfg->current_bss->chanspec), TRUE));

	/* add headers */
	wlc_d11hdrs(wlc, pkt, scb, shortpreamble, 0, 1, TX_DATA_FIFO, 0, NULL, NULL, rspec);
...
/**
 * Normally a TIM bit is set in the beacon before sending the deauth frame from the AP,
 * when a station is in power save. The deauth frame is then temporarily stored on the
 * PSQ of the scb.
 * In case an scb will be cleaned up without delay, a deauth frame should be sent immediately
 * as there either won't be a PSQ to store the packet or the deauth frame
 * will be cleaned up before it can be sent.
 * psdontq should be set to true when the scb is cleaned up without delay.
 */
void*
wlc_senddeauth(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct scb *scb,
	struct ether_addr *da, struct ether_addr *bssid,
	struct ether_addr *sa, uint16 reason_code, bool psdontq)
{
	return wlc_senddeauth_ex(wlc, cfg, scb, da, bssid,
		sa, reason_code, psdontq, FALSE);
}

void*
wlc_senddeauth_ex(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct scb *scb,
	struct ether_addr *da, struct ether_addr *bssid,
	struct ether_addr *sa, uint16 reason_code, bool psdontq,
	bool use_cb_fn)
{
	...
	reason = (uint16 *)pbody;
	reason[0] = htol16(reason_code);
	if ((reason_code == DOT11_RC_INACTIVITY) || (scb && !SCB_ASSOCIATED(scb))) {
		/* Sta might be in PS or PPS, but because the reason_code is inactivity it can
		 * be at cell-edge and might not be able to respond to TIM indication.
		 * If sta is no longer in associated state, apps PSQ is unavailable.
		 * Don't enqueue the deauth but sent it right now.
		 */
		psdontq = TRUE;
	}
...
#ifdef BCM_CEVENT
	if (CEVENT_STATE(wlc->pub) && scb && !SCB_INTERNAL(scb)) {
		wlc_send_cevent(wlc, cfg, SCB_EA(scb), 0,
	...
	/* If caller requested to register packet callback function, do it now */
	if (use_cb_fn && scb && (scb != wlc->band->hwrs_scb)) {
	...
	rate_override = scb ? WLC_LOWEST_SCB_RSPEC(scb) : WLC_LOWEST_BAND_RSPEC(wlc->band);
	if (psdontq) {
		/* The scb is about to be cleaned up. To prevent immediate cleanup of the deauth
		 * frame, explicitly passing scb pointer as NULL.
		 * The deauth frame will still be sent when the scb has been freed using the
		 * hwrs_scb. In order not to break MFP, passing the key from the scb.
		 * Don't enqueue the deauth but sent it right now.
		 */
		WLPKTTAG(pkt)->flags |= WLF_PSDONTQ;
#ifdef MFP
		/* setup key for MFP */
		if (scb && WLC_MFP_ENAB(wlc->pub) && (WLPKTTAG(pkt)->flags & WLF_MFP)) {
			if (SCB_ISMULTI(scb) && BSSCFG_AP(cfg))
				key = wlc_keymgmt_get_bss_tx_key(wlc->keymgmt, cfg, TRUE, NULL);
			else if (!SCB_ISMULTI(scb) && SCB_MFP(scb))
				key = wlc_keymgmt_get_scb_key(wlc->keymgmt, scb,
					WLC_KEY_ID_PAIRWISE, WLC_KEY_FLAG_NONE, NULL);
		}
#endif /* MFP */
		scb = NULL;
	}
	if (wlc_queue_80211_frag(wlc, pkt, cfg->wlcif->qi, scb,
		(scb ? scb->bsscfg : NULL), FALSE, key, rate_override)) {
		if (BSS_SMFS_ENAB(wlc, cfg)) {
			(void)wlc_smfs_update(wlc->smfs, cfg, SMFS_TYPE_DEAUTH_TX, reason_code);
		}
		WL_ASSOC(("wl%d.%d: %s: station "MACF" has been notified of deauthentication %s\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__,
			ETHERP_TO_MACF(da), scb?"":"using hwrs_scb"));
		return pkt;
	}
		...
		wlc_iem_uiel_t *puiel, wlc_assoc_t *as)
{
	...
	old_as_ie_len = as->ie_len;
	puiel->ies_len = puiel->ies[TLV_LEN_OFF] + TLV_HDR_LEN;
	as->ie_len = puiel->ies_len;

	if ((new_as_ie = MALLOC(wlc->osh, puiel->ies_len)) == NULL) {
		WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
		ret = BCME_ERROR;
		return ret;
	...
	bcopy(puiel->ies, new_as_ie, puiel->ies_len);

	if (as->ie) {
		MFREE(wlc->osh, as->ie, old_as_ie_len);
		as->ie = NULL;
	}
...
wlc_sendassocreq(wlc_info_t *wlc, wlc_bss_info_t *bi, struct scb *scb, bool reassoc)
{
	...
	/* Update ASSOC IEs length */
	if (type == FC_REASSOC_REQ && (puiel != NULL)) {
	/* puiel is NULL when wl command is used for association
	 * so when you use wl command this IE len handling should be skipped
	 * and handled only when wpa_supplicant is used for association
	 */
		if (wlc_assoc_ie_len_update(wlc, bsscfg, puiel, as) != BCME_OK) {
			WL_ERROR(("%s:For Re-Assoc, update ASSOC IEs length failed\n",
				__FUNCTION__));
			return NULL;
		}
	}
...
wlc_get_revision_info(wlc_info_t *wlc, void *buf, uint len)
{
	...
	if ((BUSTYPE(wlc->pub->sih->bustype) == SI_BUS) &&
		(BUSCORETYPE(wlc->pub->sih->buscoretype) == UBUS_BRIDGE_ID)) {
		/* ensure router host is aware we are not a dongle */
		rinfo->bus	= U_BUS;
	}
 ...
 * indicates if 11n, 11n proprietary, 11ac and/or 11ax MCS rates are allowed.
 *
 * Prerequisite: the caller of this function should have set wlc->band to the band to which the
 * requested mcsmask applies.
 *
...
#ifndef BAND6G_USING_LEGACY_IES
...
BCMATTACHFN(wlc_bss_default_init)(wlc_info_t *wlc)
{
	...
	/* determine the default chanspec for current band */
	chanspec = wlc_default_chanspec_by_band(wlc->cmi, wlc->band->bandunit);
...
static const dot11_mbsp_sel_t mbsp_sel_tbl[] = {
	...
	DOT11_BSS_MEMBERSHIP_SAE_H2E_ONLY,
	DOT11_BSS_MEMBERSHIP_GLK,
	DOT11_BSS_MEMBERSHIP_EPD
};
...
wlc_bss_membership_get(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_AP(cfg) && wlc_akm_is_sae_h2e_enabled(wlc, cfg)) {
		mbsp_sel |= WLC_MBSP_SEL_SAE_H2E;
	}
	return mbsp_sel;
}
	...
	if (BSSCFG_STA(bsscfg) && bsscfg->associated &&
		...
		if ((bcmwpa_is_wpa_auth(bsscfg->WPA_auth) &&
		    ...
		    (bcmwpa_is_rsn_auth(bsscfg->WPA_auth) &&
		     (ie = bcm_find_tlv(parse, len, DOT11_MNG_RSN_ID)) != NULL) ||
#ifdef BCMWAPI_WAI
		    (IS_WAPI_AUTH(bsscfg->WPA_auth) &&
		     (ie = bcm_find_tlv(parse, len, DOT11_MNG_WAPI_ID)) != NULL) ||
#endif // endif
		    FALSE) {

#if defined(WLFBT)
			/* Don't report the association event if it's a fast transition */
			if (BSSCFG_IS_FBT(bsscfg) && (bcmwpa_is_rsn_auth(bsscfg->WPA_auth)) &&
				isup && reason) {
	...
	void *buf, int len)
{
	...
	sta.chanspec = wf_channel2chspec(wf_chspec_ctlchan(bsscfg->current_bss->chanspec),
			...
			WL_CHANSPEC_BW_20)))),
			CHSPEC_BAND(bsscfg->current_bss->chanspec));
	...
	if (SCB_ASSOCIATED(scb)) {
		...
		if (scb->bsscfg) {
			...
			(void) wlc_keymgmt_get_tx_key(wlc->keymgmt, scb, scb->bsscfg,
					&key_info);
...
#ifdef WL_MBO
	if (SCB_MBO(scb)) {
		sta.flags |= WL_STA_MBO_CAP;
	}
#endif /* WL_MBO */
#ifdef MULTIAP
	if (SCB_MAP_CAP(scb)) {
		sta.flags |= WL_STA_MAP;
	}
#endif /* MULTIAP */
	...
	if (SCB_HE_CAP(scb)) {
		...
		sta.he_flags = wlc_he_get_peer_caps(wlc->hei, scb);
		sta.he_omi = wlc_he_get_scb_omi(wlc->hei, scb);
		sta.twt_info = wlc_twt_scb_info(wlc->twti, scb);
	}
#endif /* WL11AX */
	...
	if (len < (int)sizeof(sta_info_t)) {
		...
		if (SCB_MAP_P2_CAP(scb)) {
			sta.map_flags |= WL_MAP_STA_PROFILE2;
		}
	}
...
#endif /* WLWNM_AP */
	wlc_iem_get_sta_info_vendor_oui(wlc, scb, &sta.sta_vendor_oui);
#ifdef WL11K_AP
...
uint16
wlc_wme_get_frame_medium_time(wlc_info_t *wlc, enum wlc_bandunit bandunit,
	ratespec_t rate, uint8 preamble_type, uint mac_len)
{
	uint16 duration;

	duration = (uint16)BANDUNIT_SIFS(bandunit);
...
/** Read a single uint16 from shared memory. SHM 'byte_offset' needs to be an even address. */
uint16
wlc_read_shm(wlc_info_t *wlc, uint byte_offset)
{
	return wlc_bmac_read_shm(wlc->hw, byte_offset);
}

/** Write a single uint16 to shared memory. SHM 'byte_offset' needs to be an even address */
void
wlc_write_shm(wlc_info_t *wlc, uint byte_offset, uint16 v)
{
	wlc_bmac_write_shm(wlc->hw, byte_offset, v);
}

void
wlc_update_shm(wlc_info_t *wlc, uint byte_offset, uint16 v, uint16 mask)
{
	wlc_bmac_update_shm(wlc->hw, byte_offset, v, mask);
}

/**
 * Set a range of shared memory to a value. SHM 'byte_offset' needs to be an even address and Range
 * length 'len' must be an even number of bytes
 */
void
wlc_set_shm(wlc_info_t *wlc, uint byte_offset, uint16 v, int len)
{
	/* byte_offset and len need to be even */
	...
	wlc_bmac_set_shm(wlc->hw, byte_offset, v, len);
}

/**
 * Copy a buffer to shared memory. SHM 'byte_offset' needs to be an even address and
 * Buffer length 'len' must be an even number of bytes
 */
void
wlc_copyto_shm(wlc_info_t *wlc, uint byte_offset, const void* buf, int len)
{
	/* byte_offset and len need to be even */
	ASSERT((byte_offset & 1) == 0);
	ASSERT((len & 1) == 0);
	...
	if (len <= 0)
		return;
	wlc_bmac_copyto_objmem(wlc->hw, byte_offset, buf, len, OBJADDR_SHM_SEL);

}

/**
 * Copy from shared memory to a buffer. SHM 'byte_offset' needs to be an even address and
 * Buffer length 'len' must be an even number of bytes
 */
void
wlc_copyfrom_shm(wlc_info_t *wlc, uint byte_offset, void* buf, int len)
{
	/* byte_offset and len need to be even */
	ASSERT((byte_offset & 1) == 0);
	ASSERT((len & 1) == 0);
	...
	wlc_bmac_copyfrom_objmem(wlc->hw, byte_offset, buf, len, OBJADDR_SHM_SEL);
}

#if defined(WL_PSMX)
/* Read a single uint16 from PSMX shared memory.
 * SHM 'byte_offset' needs to be an even address
 */
uint16
wlc_read_shmx(wlc_info_t *wlc, uint byte_offset)
{
	uint16 val;

	wlc_bmac_copyfrom_objmem(wlc->hw, byte_offset, &val, sizeof(val), OBJADDR_SHMX_SEL);
	return val;
}
 ...
 */
void
wlc_write_shmx(wlc_info_t *wlc, uint byte_offset, uint16 v)
{
	wlc_bmac_copyto_objmem(wlc->hw, byte_offset, &v, sizeof(v), OBJADDR_SHMX_SEL);
}

/* Read a single uint16 from PSMX ihr.
 * 'byte_offset' is in enum address space. IHRx is on offset (x + 0x200) *2.
 */
uint16
wlc_read_macregx(wlc_info_t *wlc, uint byte_offset)
{
	uint16 val;
	ASSERT(byte_offset >= D11REG_IHR_BASE);
	ASSERT(PSMX_ENAB(wlc->pub));

	byte_offset = (byte_offset - D11REG_IHR_BASE) << 1;
	wlc_bmac_copyfrom_objmem(wlc->hw, byte_offset, &val, sizeof(val), OBJADDR_IHRX_SEL);
	return val;
}
 ...
 */
void
wlc_write_macregx(wlc_info_t *wlc, uint byte_offset, uint16 v)
{
	ASSERT(byte_offset >= D11REG_IHR_BASE);
	ASSERT(PSMX_ENAB(wlc->pub));

	byte_offset = (byte_offset - D11REG_IHR_BASE) << 1;
	wlc_bmac_copyto_objmem(wlc->hw, byte_offset, &v, sizeof(v), OBJADDR_IHRX_SEL);
}
#endif /* WL_PSMX */
 ...
 */
uint16
wlc_read_shm1(wlc_info_t *wlc, uint byte_offset)
{
	return wlc_bmac_read_shm1(wlc->hw, byte_offset);
}
 ...
 */
void
wlc_write_shm1(wlc_info_t *wlc, uint byte_offset, uint16 v)
{
	wlc_bmac_write_shm1(wlc->hw, byte_offset, v);
}

/* Read a single uint16 from PSMR1 ihr.
 * IHR1 'byte_offset' should be the host space address. (x + 0x200) *2.
 */
uint16
...
#if defined(DSLCPE)
void
wlc_reset_cnt(wlc_info_t* wlc)
{
	int offset = sizeof(wlc->pub->_cnt->version) + sizeof(wlc->pub->_cnt->length);
	char *hdrptr = (char*)(wlc->pub->_cnt);

	/* clear wlc->pub->cnt */
	bzero(hdrptr + offset, wlc->pub->_cnt->length - offset);
}
#endif  /* DSLCPE */
...
wlc_rfaware_lifetime_set(wlc_info_t *wlc, uint16 lifetime)
{
	...
	/* txfifo-lifetime uses same TXD lifetime construct as WME lifetime */
...
wlc_rfd_intr(wlc_info_t *wlc)
{
	...
	if (rfd) {
		...
		wlc_bsscfg_t *bsscfg;
		FOREACH_BSS(wlc, idx, bsscfg) {
			if ((!BSSCFG_STA(bsscfg)) || BSSCFG_IBSS(bsscfg) || !bsscfg->enable)
				continue;
...
void
wlc_check_txq_fc(wlc_info_t *wlc, wlc_txq_info_t *qi)
{
 ...
 *
 * @param[in] band_index  BAND_2G_INDEX, BAND_5G_INDEX, ...
 */
void
wlc_pi_band_update(wlc_info_t *wlc, enum wlc_bandunit bandunit)
{
	wlc->band = wlc->bandstate[bandunit];
	wlc->pi = wlc->bandinst[bandunit]->pi;
}
...
wlc_devpwrstchg_change(wlc_info_t *wlc, bool hostmem_access_enabled)
{
	...
	if (BCMPCIEDEV_ENAB()) {
		...
		wlc_bmac_enable_rx_hostmem_access(wlc->hw, rx_hostmem_access_enabled);
		wlc_bmac_enable_tx_hostmem_access(wlc->hw, wlc->cmn->hostmem_access_enabled);
	}
#endif /* BCMPCIEDEV */
}
...
*/
uint32
wlc_halt_device(wlc_info_t *wlc)
{
	if (wlc->pub->up) {
		if (wlc->clk) {
#ifdef DONGLEBUILD
...
wlc_generate_pme_to_host(wlc_info_t *wlc, bool pme_state)
{
	...
	/* Assert PME by write to PSM mac intstatus, but clear through write to mac intstatus */
		if (pme_state) {
			...
			OR_REG(wlc->osh, D11_PSM_MAC_INTSTAT_L(wlc), (uint16)MI_PME);
		} else {
	...
	if (enable) {
...
#if defined(WLAMPDU) || defined(WLAMSDU)
			if (SCB_MFP(scb) &&
			    N_ENAB(wlc->pub) &&
			    (scb->wsec == AES_ENABLED)) {
#ifdef WLAMSDU
				wlc_amsdu_tx_scb_enable(wlc, scb);
#endif /* WLAMSDU */
#ifdef WLAMPDU
				if (SCB_AMPDU(scb)) {
		...
		if (flag == AUTHORIZED) {
			...
			if (wlc->pub->up && (SCB_AUTHENTICATED(scb) || SCB_LEGACY_WDS(scb)) &&
				...
				wlc_bss_mac_event(wlc, bsscfg, WLC_E_AUTH_FAIL, &scb->ea,
					WLC_E_STATUS_SUCCESS, DOT11_SC_AUTH_MISMATCH, 0, 0, 0);
			}
		}
	}
 ...
 */
uint32
wlc_current_pmu_time(wlc_info_t *wlc)
{
...
{
	si_srpwr_request(wlc->pub->sih, SRPWR_DMN3_MACMAIN_MASK, SRPWR_DMN3_MACMAIN_MASK);
}

void
wlc_srpwr_request_off(wlc_info_t *wlc)
{
	si_srpwr_request(wlc->pub->sih, SRPWR_DMN3_MACMAIN_MASK, 0);
}

void
wlc_srpwr_request_on_all(wlc_info_t *wlc, bool force_pwr)
{
	if (force_pwr || wlc->clk) {
		wlc_srpwr_request_on(wlc);
	}
}
...
{
	uint st = si_srpwr_stat_spinwait(wlc->pub->sih,
		SRPWR_DMN3_MACMAIN_MASK, SRPWR_DMN3_MACMAIN_MASK);

	BCM_REFERENCE(st);
	ASSERT(st & SRPWR_DMN3_MACMAIN_MASK);
}

#ifdef ENABLE_CORECAPTURE
int
wlc_dump_mem(wlc_info_t *wlc, int type)
{
...
{
	uint st = si_srpwr_stat(wlc->pub->sih);

	return (st & SRPWR_DMN3_MACMAIN_MASK) ? TRUE : FALSE;
}
...
wlc_get_cmn_bwcap(wlc_info_t *wlc, int bandtype)
{
	...
	bandunit = wlc_bandtype2bandunit(bandtype);
	if (bandunit == MAXBANDS) {
		WL_ERROR(("wl%d: %s: Invalid bandtype %d\n", wlc->pub->unit, __FUNCTION__,
			bandtype));
		ASSERT(0);
		goto done;
	}
	bwcap = wlc->bandstate[bandunit]->bw_cap;
...
void
wlc_process_multiap_ie(wlc_info_t *wlc, struct scb *scb, multiap_ie_t *multiap_ie, uint16 ft)
{
	multiap_ext_attr_t *map_attr = NULL;
	multiap_profile_se_t *map_profile = NULL;
	multiap_def_8021Q_settings_se_t *map_8021q_settings;
	int len = 0;
	bcm_tlv_t *tlv = NULL;

	ASSERT(scb != NULL);

	if (!multiap_ie || multiap_ie->len < MAP_IE_FIXED_LEN) {
		return;
	}

	len = multiap_ie->len;
	/* Unset MAP capability and profile */
	scb->flags3 &= ~SCB3_MAP_CAP;
	scb->flags3 &= ~SCB3_MAP_P2_CAP;
	/* Unset VLAN ID before processing (re)assoc response */
	if ((scb->bsscfg) && (ft == FC_ASSOC_RESP || ft == FC_REASSOC_RESP)) {
		scb->bsscfg->map_prim_vlan_id = 0;
	}

	for (tlv = (bcm_tlv_t *)multiap_ie->attr; tlv != NULL;
		tlv = (bcm_tlv_t *)bcm_next_tlv((bcm_tlv_t *)tlv, &len)) {
		if (tlv->id  == MAP_EXT_ATTR) {
			map_attr = (multiap_ext_attr_t *)tlv;

			if (map_attr && (map_attr->attr_val != 0)) {
				scb->flags3 |= SCB3_MAP_CAP;
			}
		} else if (tlv->id == MAP_PROFILE_SE_ID) {
			map_profile = (multiap_profile_se_t *)tlv;

			if (map_profile && (map_profile->map_profile == MAP_PROFILE_2)) {
				scb->flags3 |= SCB3_MAP_P2_CAP;
			}
		} else if (tlv->id == MAP_8021Q_SETTINGS_SE_ID) {
			map_8021q_settings = (multiap_def_8021Q_settings_se_t *)tlv;

			if (scb->bsscfg) {
				scb->bsscfg->map_prim_vlan_id = map_8021q_settings->prim_vlan_id;
			}
		}
	}
}
...
wlc_update_multiap_ie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	...
	multiap_profile_se_t *map_profile;
	multiap_def_8021Q_settings_se_t *map_8021q_settings;
	...
	map_ie->len = MAP_IE_FIXED_LEN + MAP_EXT_ATTR_LEN + TLV_HDR_LEN;

	if (bsscfg->map_attr) {
		map_profile = (multiap_profile_se_t *)
			((uint8 *)map_ie + (map_ie->len + TLV_HDR_LEN));
		if (bsscfg->map_profile && (bsscfg->map_profile >= MAP_PROFILE_2)) {
			map_profile->id = MAP_PROFILE_SE_ID;
			map_profile->len = MAP_PROFILE_SE_LEN;
			map_profile->map_profile = bsscfg->map_profile;
			map_ie->len += MAP_PROFILE_SE_LEN + TLV_HDR_LEN;
		}

		map_8021q_settings = (multiap_def_8021Q_settings_se_t *)
			((uint8 *)map_ie + (map_ie->len + TLV_HDR_LEN));
		if (bsscfg->map_prim_vlan_id && (bsscfg->map_profile >= MAP_PROFILE_2)) {
			map_8021q_settings->id = MAP_8021Q_SETTINGS_SE_ID;
			map_8021q_settings->len = MAP_8021Q_SETTINGS_SE_LEN;
			map_8021q_settings->prim_vlan_id = bsscfg->map_prim_vlan_id;
			map_ie->len += MAP_8021Q_SETTINGS_SE_LEN + TLV_HDR_LEN;
		}
	}
...
static const char *pktcntr_names[] = {
	FOREACH_PKTCNTR(PKTCNTR_GENERATE_STRING)
};

static void
wlc_dump_pktcntr(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	int i;

	bcm_bprintf(b, "WL Driver Packet Counters:\n");
	bcm_bprintf(b, "--------------------------\n");
	bcm_bprintf(b, "Id \tPktCntr\t\t\t\tCnt \n\n");
	for (i = 0; i < PKTCNTR_LAST; i++) {
		bcm_bprintf(b, "%d \t%-30s\t%d\n", i, pktcntr_names[i], wlc->pktcntr_tbl[i]);
		wlc->pktcntr_tbl[i] = 0;
	}
}
#endif /* WL_PKTCNTR */
...
/**
 * Avoids selecting a bandunit (wlc->bandstate[X]) that is unsupported by hardware.
 */
wlcband_t *
wlc_get_band(const wlc_info_t *wlc, enum wlc_bandunit bandunit)
{
	if (bandunit >= MAXBANDS || !BAND_ENABLED(wlc, bandunit))
		return NULL;
...
uint
wlc_get_nbands(struct wlc_info *wlc)
{
	int bandmask = wlc->pub->_bandmask;
	uint nbands = 0;
...
static const struct {
	...
	uint chspec_band;
	const char *name;
} band_unit_conv[MAXBANDS] = {
	{ WLC_BAND_2G, CLM_BAND_2G, WL_CHANSPEC_BAND_2G, "2.4G" },
	{ WLC_BAND_5G, CLM_BAND_5G, WL_CHANSPEC_BAND_5G, "5G" },
	{ WLC_BAND_6G, CLM_BAND_6G, WL_CHANSPEC_BAND_6G, "6G" },
};

int
wlc_bandunit2bandtype(enum wlc_bandunit bu)
{
	ASSERT(bu < MAXBANDS);
	return band_unit_conv[bu].bandtype;
}

enum wlc_bandunit
wlc_bandtype2bandunit(int bandtype)
{
	int i;

	for (i = 0; i < MAXBANDS; i++) {
		if (band_unit_conv[i].bandtype == bandtype)
			break;
	}
	ASSERT(i < MAXBANDS);
	return i;
...
enum clm_band
wlc_bandunit2clmband(enum wlc_bandunit bu)
{
	ASSERT(bu < MAXBANDS);
	return band_unit_conv[bu].clm;
}

uint
wlc_bandunit2chspecband(enum wlc_bandunit bu)
{
	ASSERT(bu < MAXBANDS);
	return band_unit_conv[bu].chspec_band;
}

enum clm_band
wlc_bandtype2clmband(int bandtype)
{
	enum wlc_bandunit bu;

	bu = wlc_bandtype2bandunit(bandtype);
	return band_unit_conv[bu].clm;
}

const char *
wlc_bandunit_name(enum wlc_bandunit bu)
{
	ASSERT(bu < MAXBANDS);
	return band_unit_conv[bu].name;
}

void
wlc_get_bands_str(struct wlc_info *wlc, char *buf, int bufsize)
{
	struct bcmstrbuf b;
	enum wlc_bandunit bandunit;
	...
	FOREACH_WLC_BAND(wlc, bandunit) {
		if (b.buf == b.origbuf)
			bcm_bprintf(&b, "%s", wlc_bandunit_name(bandunit));
		else
			bcm_bprintf(&b, "/%s", wlc_bandunit_name(bandunit));
	}
}
...
static const char wlc_rx_pktdrop_reason_2_brief_reason_map[RX_PKTDROP_RSN_SIZE] =
	{ RX_PKTDROP_RSN_2_BRF_RSN_ARRAY_INITIALIZER };
static const char wlc_tx_pktdrop_reason_2_brief_reason_map[TX_PKTDROP_RSN_SIZE] =
	{ TX_PKTDROP_RSN_2_BRF_RSN_ARRAY_INITIALIZER };

static void
pktdrop_full_counters_clear(wlc_pktdrop_full_counters_t *full)
{
	if (full) {
		memset(full, 0, sizeof(wlc_pktdrop_full_counters_t));
	}
}

static void
pktdrop_brief_counters_clear(wlc_pktdrop_brief_counters_t *brief)
{
	if (brief) {
		memset(brief, 0, sizeof(wlc_pktdrop_brief_counters_t));
	}
}

void
wlc_pktdrop_full_counters_enable(wlc_info_t *wlc)
{
	int sz = sizeof(wlc_pktdrop_full_counters_t);

	if (wlc->pktdrop_full_counters == NULL) {
		wlc->pktdrop_full_counters = MALLOCZ(wlc->osh, sz);
		if (wlc->pktdrop_full_counters == NULL) {
			WL_ERROR(("wl%d: MALLOCZ of %d bytes for wlc full counters failed\n",
				wlc->pub->unit, sz));
		}
	}
}
...
void
wlc_pktdrop_full_counters_disable(wlc_info_t *wlc)
{
	if (wlc->pktdrop_full_counters) {
		MFREE(wlc->osh, wlc->pktdrop_full_counters, sizeof(wlc_pktdrop_full_counters_t));
		wlc->pktdrop_full_counters = NULL;
	}
}

void
scb_pktdrop_brief_counters_enable(wlc_info_t *wlc, struct scb *scb)
{
	int sz = sizeof(wlc_pktdrop_brief_counters_t);

	if (scb->pktdrop_brief_counters == NULL) {
		scb->pktdrop_brief_counters = MALLOCZ(wlc->osh, sz);
		if (scb->pktdrop_brief_counters == NULL) {
			WL_ERROR(("wl%d: MALLOCZ of %d bytes for scb brief counters failed\n",
				wlc->pub->unit, sz));
		}
	}
}
...
void
scb_pktdrop_brief_counters_disable(wlc_info_t *wlc, struct scb *scb)
{
	if (scb->pktdrop_brief_counters) {
		MFREE(wlc->osh, scb->pktdrop_brief_counters, sizeof(wlc_pktdrop_brief_counters_t));
		scb->pktdrop_brief_counters = NULL;
	}
}

void
scb_pktdrop_full_counters_enable(wlc_info_t *wlc, struct scb *scb)
{
	int sz = sizeof(wlc_pktdrop_full_counters_t);

	if (scb->pktdrop_full_counters == NULL) {
		scb->pktdrop_full_counters = MALLOCZ(wlc->osh, sz);
		if (scb->pktdrop_full_counters == NULL) {
			WL_ERROR(("wl%d: MALLOCZ of %d bytes for scb full counters failed\n",
				wlc->pub->unit, sz));
		}
	}
}
...
void
scb_pktdrop_full_counters_disable(wlc_info_t *wlc, struct scb *scb)
{
	if (scb->pktdrop_full_counters) {
		MFREE(wlc->osh, scb->pktdrop_full_counters, sizeof(wlc_pktdrop_full_counters_t));
		scb->pktdrop_full_counters = NULL;
	}
}

/*
 * allocate the full counters for the wlc AND brief counters for
 * ALL the already joined clients.
 *
 * Note that if at any time in the past MALLOCZ failed, some entities
 * may not have had their counters allocated.  So, always scan thru
 * all of them to check, every time, in case MALLOCZ may succeed
 * this time.
 */
void
wlc_pktdrop_stats_enable(wlc_info_t *wlc)
{
	struct scb_iter scbiter;
	struct scb *scb;

	/* mark that the feature is enabled */
	wlc->pktdrop_stats_feature_enabled = 1;

	wlc_pktdrop_full_counters_enable(wlc);
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		if (SCB_IS_UNUSABLE(scb)) continue;
		scb_pktdrop_brief_counters_enable(wlc, scb);
	}
}

/*
 * Free up the wlc full pktdrop counter and all the scb full & brief counters
 */
void
wlc_pktdrop_stats_disable(wlc_info_t *wlc)
{
	struct scb_iter scbiter;
	struct scb *scb;

	/* mark that the feature is disabled */
	wlc->pktdrop_stats_feature_enabled = 0;

	wlc_pktdrop_full_counters_disable(wlc);
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		scb_pktdrop_brief_counters_disable(wlc, scb);
		scb_pktdrop_full_counters_disable(wlc, scb);
	}
}

/*
 * Generates a pktdrop counter information structure
 * to be returned back to the user.  All valid
 * situations will either have 'full' and/or 'brief'
 * set.
 */
static void
wlc_return_pktdrop_counters(void *returned_buffer,
	int pktdrop_stats_feature_enabled,
	int scb_exists,
	wlc_pktdrop_full_counters_t *full,
	wlc_pktdrop_brief_counters_t *brief)
{
	uint64 *uintp;
	uint64 value;
	int i, count;
	pktdrop_stats_ioctl_response_v1_t *dst;

	/* decide what version we are returning */
	dst = (pktdrop_stats_ioctl_response_v1_t*) returned_buffer;
	dst->ver_info.version = PKTDROP_COUNTERS_LATEST_VERSION;

	/* zero out everything first */
	dst->flags = 0;
	dst->full_counters_rx_count = 0;
	dst->full_counters_tx_count = 0;
	dst->brief_counters_count = 0;

	if (!pktdrop_stats_feature_enabled) {
		dst->flags |= PKTDROP_COUNTERS_FEATURE_DISABLE_BIT;
		return;
	}

	if (!scb_exists) {
		dst->flags |= PKTDROP_COUNTERS_INVALID_STA_BIT;
		return;
	}

	uintp = &dst->start_of_counter_value_pairs[0];

	/* encode full counters if any */
	if (full) {

		/* encode all non 0 full rx counters */
		count = 0;
		for (i = RX_PKTDROP_RSN_FIRST; i < RX_PKTDROP_RSN_SIZE; i++) {
			value = full->rx_pktdrop_counters[i];
			if (value) {
				*uintp++ = i;
				*uintp++ = value;
				count++;
		...
		dst->full_counters_rx_count = count;

		/* encode all non 0 full tx counters */
		count = 0;
		for (i = TX_PKTDROP_RSN_FIRST; i < TX_PKTDROP_RSN_SIZE; i++) {
			value = full->tx_pktdrop_counters[i];
			if (value) {
				*uintp++ = i;
				*uintp++ = value;
				count++;
		...
		dst->full_counters_tx_count = count;
	}

	/* now encode brief counters if any */
	if (brief) {
		count = 0;
		for (i = 0; i < PKTDROP_BRF_RSN_SIZE; i++) {
			value = brief->ctrs[i];
			if (value) {
				*uintp++ = i;
				*uintp++ = value;
		...
		dst->brief_counters_count = count;
	}
}

static INLINE void
pktdrop_stats_not_enabled_response(void *returned_buffer)
{
	wlc_return_pktdrop_counters(returned_buffer, 0, 0, NULL, NULL);
}

static INLINE void
pktdrop_stats_invalid_scb_response(void *returned_buffer)
{
	wlc_return_pktdrop_counters(returned_buffer, 1, 0, NULL, NULL);
}

static INLINE void
pktdrop_stats_empty_response(void *returned_buffer)
{
	wlc_return_pktdrop_counters(returned_buffer, 1, 1, NULL, NULL);
}

/*
 * This is how the wlc driver responds to a pktdrop_stats request from the user.
 * Note that a response is ALWAYS returned.  It makes the interface simple.
 */
static void
wlc_pktdrop_stats(wlc_info_t *wlc, void *arg, void *params)
{
	unsigned char *mac;
	int flags, clear;
	struct scb *scb = NULL;
	pktdrop_stats_ioctl_request_t reqst;

	/* get the request; this is UN aligned so copy it byte by byte */
	memcpy(&reqst, params, sizeof(pktdrop_stats_ioctl_request_t));

	flags = reqst.flags;
	mac = &reqst.sta_mac[0];

	/* these are always mutually exclusive */
	if (flags & PKTDROP_COUNTERS_FEATURE_ENABLE_BIT) {
		wlc_pktdrop_stats_enable(wlc);
		pktdrop_stats_empty_response(arg);
		return;
	} else if (flags & PKTDROP_COUNTERS_FEATURE_DISABLE_BIT) {
		wlc_pktdrop_stats_disable(wlc);
		pktdrop_stats_empty_response(arg);
		return;
	}
	...
	if (!wlc->pktdrop_stats_feature_enabled) {
		pktdrop_stats_not_enabled_response(arg);
		return;
	}

	clear = flags & PKTDROP_COUNTERS_CLEAR_BIT;

	/* per radio (wlc) command */
	if (ETHER_ISNULLADDR(mac)) {

		if (clear) {
			pktdrop_full_counters_clear(wlc->pktdrop_full_counters);
		}

		/* This works whether cleared or not */
		wlc_return_pktdrop_counters(arg, 1, 1, wlc->pktdrop_full_counters, NULL);

		return;
	}

	/* if we are here, it is a per scb command so get the scb */
	if (!ETHER_ISMULTI(mac)) {
		scb = wlc_scbapfind(wlc, (struct ether_addr*) mac);
	}

	/* scb does NOT exist */
	if (scb == NULL) {
		pktdrop_stats_invalid_scb_response(arg);
		return;
	}

	/*
	 * if we are here, scb DOES exist, so do other requests
	 */
	if (flags & PKTDROP_FULL_COUNTERS_ENABLE_BIT) {
		scb_pktdrop_full_counters_enable(wlc, scb);
		pktdrop_stats_empty_response(arg);
		return;
	} else if (flags & PKTDROP_FULL_COUNTERS_DISABLE_BIT) {
		scb_pktdrop_full_counters_disable(wlc, scb);
		pktdrop_stats_empty_response(arg);
		return;
	}
	...
	if (clear) {
		pktdrop_brief_counters_clear(scb->pktdrop_brief_counters);
		pktdrop_full_counters_clear(scb->pktdrop_full_counters);
		pktdrop_stats_empty_response(arg);
		return;
	}

	/* if here, copy the client pkt drops out to user */
	wlc_return_pktdrop_counters(arg, 1, 1,
		scb->pktdrop_full_counters,
		scb->pktdrop_brief_counters);
}

/*
 * The functions below updates rx/tx pkt drop counters.
 * In the case of an scb present, it updates
 * brief counters on the scb.  If an scb cannot be found, then the drop
 * counters on the radio are updated.  Note that the drop counters on an scb
 * are brief but on the radio are not, they are full counters.
 */

static INLINE void
increment_full_counter(wlc_pktdrop_full_counters_t *full_ctrs, int rx, int ctr)
{
	if (full_ctrs) {
		if (rx) {
			full_ctrs->rx_pktdrop_counters[ctr]++;
		} else {
			full_ctrs->tx_pktdrop_counters[ctr]++;
		}
	}
}

static INLINE void
increment_scb_pktdrop_counter(struct scb *scb, int rx, int ctr)
{
	const char *map;

	/* scb is just about to be deleted, dont do it */
	if (SCB_IS_UNUSABLE(scb)) {
		return;
	}

	/*
	 * if scb has full counters, update this NOW,
	 * BEFORE ctr is mapped for the brief counters
	 */
	increment_full_counter(scb->pktdrop_full_counters, rx, ctr);

	/* now update its brief counters if any */
	if (scb->pktdrop_brief_counters) {
		/* map the counter and increment the brief reason counter now */
		map = rx ?
			&wlc_rx_pktdrop_reason_2_brief_reason_map[0] :
			&wlc_tx_pktdrop_reason_2_brief_reason_map[0];
		ctr = map[ctr];
		scb->pktdrop_brief_counters->ctrs[ctr]++;
	}
}

void
wlc_pktdrop_counter_update(wlc_info_t *wlc, struct scb *scb, int rx, int toss_reason)
{
	int upper = rx ? RX_PKTDROP_RSN_SIZE : TX_PKTDROP_RSN_SIZE;

	/* not considered a toss reason */
	if (toss_reason < 0)
		return;

	/* this should NEVER occur, its a code bug if it does; FIX IT */
	if (toss_reason >= upper) {
		WL_ERROR(("wlc_pktdrop_counter_update: invalid toss reason %d\n",
			toss_reason));
		return;
	}

	/* if a relevant scb is present, update its counters */
	increment_scb_pktdrop_counter(scb, rx, toss_reason);

	/* also update global interface counters */
	increment_full_counter(wlc->pktdrop_full_counters, rx, toss_reason);
}

#endif /* WL_PKTDROP_STATS */

#if defined(WLATM_PERC)
/* This function is called to reset sta percentage of air time usage */
static void
wlc_reset_atf_perc(wlc_info_t *wlc)
{
	struct scb *scb;
	struct scb_iter scbiter;
	wlc_bsscfg_t *bsscfg;
	...
	FOREACH_BSS(wlc, idx, bsscfg) {
		bsscfg->bssperc = 0;
		bsscfg->sched_bssperc = 0;
	}
}
#endif /* WLATM_PERC) */

void wlc_update_phy_tdcs(wlc_info_t *wlc)
{
	int tdcs_en;

	if (!BAND_5G(wlc->band->bandtype) ||
		!CHSPEC_IS160(wlc->chanspec) ||
		!(WLC_BW_SWITCH_TDCS_EN(wlc) ||
		WLC_BW_SWITCH_ENABLED(wlc))) {
		return;
	}

	tdcs_en = phy_tdcs_is_enable((phy_info_t*)wlc->pi);

	if ((wlc->any_sta_in_160mhz || (wlc->num_160mhz_assocs > 0) ||
			wlc->any_wds_peer_160mhz)) {
		if (tdcs_en) {
...
#if BAND6G || defined(WL_OCE_AP)
void
wlc_update_upr_fd_template(wlc_info_t *wlc, bool suspend)
{
	int idx;
	wlc_bsscfg_t *bsscfg;
	...
	FOREACH_UP_AP(wlc, idx, bsscfg) {
		if (bsscfg->up && (BSSCFG_AP(bsscfg) || BSSCFG_IBSS(bsscfg))) {
			WL_INFORM(("wl%d: Ucode UPR or FD from cfg index[%d]\n", wlc->pub->unit,
				WLC_BSSCFG_IDX(bsscfg)));
			wlc_bss_update_upr_fd_template(wlc, bsscfg, suspend);
			break;
		}
	}
}
...
void
wlc_bss_update_upr_fd_template(wlc_info_t *wlc, wlc_bsscfg_t *cfg, bool suspend)
{
	int len, bcn_tmpl_len;
	uint16 *prb_resp;
	int offset;
	...
	if (!HWPRB_ENAB(cfg) || !BSSCFG_AP(cfg)) {
		WL_INFORM(("wl%d: HW does not support UPR/FD, exit\n", wlc->pub->unit));
		return;
	}
	...
	if (send_bcast_prb_rsp && send_fd) {
		WL_INFORM(("wl%d: both UPR and FD requested, not supported. Exit\n",
			wlc->pub->unit));
		return;
	}

	if (!send_fd && !send_bcast_prb_rsp) {
		WL_INFORM(("wl%d: neither FD nor UPR enabled, return\n", wlc->pub->unit));
		return;
	}
	...
	if (send_bcast_prb_rsp) {
		/* create the probe response template */
		wlc_prepare_upr_template(wlc, FC_PROBE_RESP, wlc->bcn_rspec, cfg, prb_resp, &len);
	}
#if defined(WL_OCE) && defined(WL_OCE_AP)	/* TODO: Remove this check before checkin */
	else {
		wlc_prepare_fd_template(wlc, FC_ACTION, wlc->bcn_rspec, cfg, prb_resp, &len);
	}
#endif // endif
	if (MBSS_ENAB(wlc->pub)) {
		/* tell ucode where to find the probe responses */
		if (D11REV_GE(wlc->pub->corerev, 40)) {
			offset = D11AC_T_BCN0_TPL_BASE +
				(WLC_MAX_AP_BSS(wlc->pub->corerev) * (wlc->pub->bcn_tmpl_len));

			wlc_write_shm(wlc, M_MBS_PRS_TPLPTR(wlc), offset);
		} else { /* for corerev >= 16 the value is in multiple of 4 */
			offset = (D11_T_BCN0_TPL_BASE +
				(WLC_MAX_AP_BSS(wlc->pub->corerev) *
				TPLBLKS_PER_BCN((wlc)->pub->corerev) *
				TXFIFO_SIZE_UNIT((wlc)->pub->corerev))) >> 2;
	...
	/* write the probe response into the template region */
	WL_INFORM(("wl%d: %s:template length[%d]\n", wlc->pub->unit,
			(send_fd ? "FD frame" : "UPR Frame"), len));
	...
	interval = (uint16)(wlc->upr_fd_info->period * 1024);

	/* configure interval */
	wlc_write_shm(wlc, M_UPRS_INTVL_L(wlc), interval);
	...
	if (send_bcast_prb_rsp) {
		wlc_mod_prb_rsp_rate_table(wlc, cfg, (uint16)len);
		wlc_write_shm(wlc, M_UPRS_FD_TSF_LOC(wlc), 24);
	} else {
		wlc_write_shm(wlc, M_UPRS_FD_TSF_LOC(wlc), 28);
	}

	val = wlc_mhf_get(wlc, (uint8)0x01, WLC_BAND_AUTO);

	val |= MHF2_UNSOL_PRS;

	wlc_mhf(wlc, (uint8)0x01, 0xffff, val, WLC_BAND_AUTO);

	WL_INFORM(("wl%d: RSP_TXPCTL0[%d] RSP_TXPCTL1[%d]"
		" M_PRS_FRM_BYTE_SIZE[%d] offset[%x] len[%d]"
		" period[%d] ucode host flag_1[%d]\n",
		wlc->pub->unit,	wlc_read_shm(wlc, M_RSP_TXPCTL0(wlc)),
		wlc_read_shm(wlc, M_RSP_TXPCTL1(wlc)),
		wlc_read_shm(wlc, M_PRS_FRM_BYTESZ(wlc)),
		offset, len, wlc->upr_fd_info->period,
		wlc_mhf_get(wlc, (uint8)0x01, WLC_BAND_AUTO)));

	MFREE(wlc->osh, (void *)prb_resp, bcn_tmpl_len);
}

void
wlc_prepare_upr_template(wlc_info_t *wlc, uint type, ratespec_t bcn_rspec, wlc_bsscfg_t *cfg,
        uint16 *buf, int *len)
{
	uint8 *body;
	struct dot11_management_header *h;
	...
	*len = hdr_len + body_len; /* return actual size */
	...
	/* PLCP for Probe Response frames are filled in from core's rate table */
	mf_len = DOT11_MAC_HDR_LEN + body_len;
	plcp = (uint8 *)buf + wlc_template_plcp_offset(wlc, bcn_rspec);

	wlc_compute_plcp(wlc, cfg, WLC_RATE_6M, mf_len + DOT11_FCS_LEN, FC_FROMDS, plcp);

	WL_INFORM(("wl%d: UPR template body_len[%d] hdr_len[%d]\n", wlc->pub->unit, body_len,
		hdr_len));
	...
	bcopy((char*)&cfg->cur_etheraddr, (char*)&h->sa, ETHER_ADDR_LEN);
	bcopy((char*)&cfg->BSSID, (char*)&h->bssid, ETHER_ADDR_LEN);
} /* wlc_prepare_upr_template */

/* Ugly thing to put under WL_OCE_AP cap..., as 6715 is not enabled with OCE_AP
 * TODO: sort it out
 */
#if defined(WL_OCE) && defined(WL_OCE_AP)
void
wlc_prepare_fd_template(wlc_info_t *wlc, uint type, ratespec_t bcn_rspec, wlc_bsscfg_t *cfg,
        uint16 *buf, int *len)
{
	uint8 *body;
	struct dot11_management_header *h;
	...
	fd_action_frame = (dot11_action_frmhdr_t*) body;

	fd_action_frame->category = DOT11_ACTION_CAT_PUBLIC;
	fd_action_frame->action = DOT11_FILS_ACTION_DISCOVERY;

	wlc_oce_write_fd_info_field(wlc->oce, cfg,
		body + DOT11_ACTION_HDR_LEN, frame_len - DOT11_ACTION_HDR_LEN);
	...
	*len = hdr_len + body_len; /* return actual size */
	...
	WL_INFORM(("wl%d: FD frame body[%d] header[%d]\n", wlc->pub->unit, body_len, hdr_len));
	...
	bcopy((char*)&cfg->cur_etheraddr, (char*)&h->sa, ETHER_ADDR_LEN);
	bcopy((char*)&cfg->BSSID, (char*)&h->bssid, ETHER_ADDR_LEN);
} /* wlc_prepare_fd_template */
#endif /* WL_OCE && WL_OCE_AP */
#endif /* BAND6G || WL_OCE_AP */

static void
wlc_init_override_clm(wlc_info_t *wlc)
{
	int i;
	wlc_pub_t *pub = wlc->pub;

	pub->_override_clm_eirp.txpwr_20 = WLC_OVERRIDE_CLM_DISABLE;
	pub->_override_clm_eirp.txpwr_40 = WLC_OVERRIDE_CLM_DISABLE;
	pub->_override_clm_eirp.txpwr_80 = WLC_OVERRIDE_CLM_DISABLE;
	pub->_override_clm_eirp.txpwr_160 = WLC_OVERRIDE_CLM_DISABLE;
	for (i = 0; i < TPE_PSD_COUNT; i++) {
		pub->_override_clm_eirp_psd.txpwr_psd[i] = WLC_OVERRIDE_CLM_DISABLE;
	}
}
