 ...
 *
 * $Id: wbd_ds.c 792598 2020-10-30 07:58:11Z $
 */
...
wbd_slave_item_t*
wbd_ds_find_slave_ifname_in_blanket_slave(wbd_blanket_slave_t *wbd_slave, char *ifname, int* error)
{
	wbd_slave_item_t *slave_item = NULL;
	dll_t *slave_item_p;
	*error = WBDE_OK;
	...
	/* Traverse br0 slave List */
	foreach_glist_item(slave_item_p, wbd_slave->br0_slave_list) {

		slave_item = (wbd_slave_item_t*)slave_item_p;

		/* Slave Checking Condition */
		if (strcmp(ifname, slave_item->wbd_ifr.ifr.ifr_name) == 0) {
			goto end;
		}
	}

	WBD_DS("Slave[%s]. %s\n", ifname, wbderrorstr(WBDE_DS_UNKWN_SLV));
	*error = WBDE_DS_UNKWN_SLV;
	slave_item = NULL;
end:
	WBD_EXIT();
			...
			foreach_i5glist_item(i5_bss, i5_dm_bss_type, i5_ifr->bss_list) {
				...
				foreach_i5glist_item(sta, i5_dm_clients_type, i5_bss->client_list) {
					...
					if (memcmp(sta->mac, sta_mac, MAC_ADDR_LEN) == 0) {
						WBD_INFO("Duplicate STA check : "MACF" associated "
							"to BSS(%d) "MACF" at "TIMEVALF"\n",
							ETHERP_TO_MACF(sta_mac),
							sta_found_count + 1,
							ETHERP_TO_MACF(i5_bss->BSSID),
							TIMEVAL_TO_TIMEVALF(sta->assoc_tm));
						if ((timercmp(&(sta->assoc_tm),
							/* CSTYLED */
							&assoc_time, >))) {
							assoc_time = sta->assoc_tm;
							i5_recent_sta = sta;
						}
						sta_found_count++;
					}
...
i5_dm_device_type*
wbd_ds_get_ctrlagent_i5_device(int *error)
{
	int ret = WBDE_DS_UN_DEV;
	i5_dm_network_topology_type *i5_topology;
	...
	i5_topology = ieee1905_get_datamodel();
	if (!i5_topology) {
		WBD_ERROR("Error: No Topology from ieee1905_get_datamodel.\n");
		goto end;
	}

	foreach_i5glist_item(i5_device, i5_dm_device_type, i5_topology->device_list) {

		/* Traverse Topology to get Controller Agent */
	...
	WBD_DS("Find Controller Device in Topology : %s\n", wbderrorstr(ret));
	if (error) {
		*error = ret;
	}
	WBD_EXIT();
	return (ret == WBDE_OK) ? i5_device : NULL;
}
...
i5_dm_bss_type*
wbd_ds_get_i5_bss_match_ifname_in_ifr(i5_dm_interface_type *i5_ifr, char *ifname,
	int *error)
{
	int ret = WBDE_OK;
	i5_dm_bss_type *i5_bss = NULL;
	...
	if (!(i5_bss = i5DmGetBSSMatchingIfnameFromInterface(i5_ifr, ifname))) {
		ret = WBDE_DS_UN_BSS;
	}
end:
...
wbd_ds_interface_init(i5_dm_interface_type *i5_ifr)
{
	...
	/* Initialize lists holding STATIC & DYNAMIC Non-operable Chanspecs */
	ieee1905_glist_init(&ifr_vndr_data->static_exclude_chlist);
	ieee1905_glist_init(&ifr_vndr_data->dynamic_exclude_chlist);
...
wbd_ds_interface_deinit(i5_dm_interface_type *i5_ifr)
{
	...
	if (i5_ifr->vndr_data) {
		...
		/* Cleanup lists holding STATIC & DYNAMIC Non-operable Chanspecs */
		i5DmGlistCleanup(&(ifr_vndr_data->static_exclude_chlist));
		i5DmGlistCleanup(&(ifr_vndr_data->dynamic_exclude_chlist));
...
wbd_ds_sta_init(wbd_info_t *info, i5_dm_clients_type *i5_assoc_sta)
{
	...
	if (i5_ifr->chanspec != 0) {
		band = ieee1905_get_band_from_channel(i5_ifr->opClass,
			wf_chspec_ctlchan(i5_ifr->chanspec));
		prb_band |= band;
	}
...
i5_dm_clients_type*
wbd_ds_get_bh_opt_pending_sta(unsigned char *sta_mac)
{
	i5_dm_network_topology_type *i5_topology;
	i5_dm_device_type *i5_device;
	i5_dm_interface_type *i5_ifr;
		...
		foreach_i5glist_item(i5_ifr, i5_dm_interface_type, i5_device->interface_list) {
			foreach_i5glist_item(i5_bss, i5_dm_bss_type, i5_ifr->bss_list) {
				...
				foreach_i5glist_item(i5_sta, i5_dm_clients_type,
					...
					/* Skip last selected bh sta */
					if (sta_mac == i5_sta->mac) {
						continue;
					}
