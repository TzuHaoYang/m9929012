 ...
 *
 * $Id: wlc_stf.c 793742 2020-12-08 17:25:14Z $
 */
...
#include <wlc_pcb.h>		// for wlc_pcb_fn_register()
#ifdef WL11AX
...
/* Value of PHYREG_OBSSPDSREN */
#define OBSSPDSREN_OFF			0x4
#define OBSSPDSREN_HE			0x5
#define OBSSPDSREN_HEVHT		0x7
#define SRPSREN				0x14
#define SRP_OBSSPDSREN_HE		0x15
#define SRP_OBSSPDSREN_HEVHT		0x17

/** Cubby pertaining to the BSS configuration */
#define STF_BSSCFG_CUBBY(stf, cfg) \
	((wlc_stf_bsscfg_cubby_t *)BSSCFG_CUBBY((cfg), (stf)->cfgh_cubby))
...
typedef struct wlc_stf_bsscfg_cubby wlc_stf_bsscfg_cubby_t;
...
static uint8 wlc_stf_core_mask_assign(wlc_stf_info_t *stf, uint core_count);
...
#ifdef WL11AC
static void wlc_stf_update_160mode_txcore(wlc_stf_info_t *stf, chanspec_t chanspec);
#endif // endif
#ifdef OCL
...
static void wlc_stf_mimops_handle_rxchain_update(wlc_info_t *wlc, uint8 mimops_mode);
#if defined(BCMDBG)
static void wlc_stf_bsscfg_cubby_dump(void *ctx, wlc_bsscfg_t *cfg, struct bcmstrbuf *b);
static void wlc_stf_scb_cubby_dump(void *ctx, struct scb *scb, struct bcmstrbuf *b);
#endif // endif
static void wlc_stf_mimops_action_complete(wlc_info_t *wlc, uint txstatus, void *arg);
#ifdef STA
static void wlc_stf_mimops_action(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg);
#endif /* STA */
	...
	void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
		...
		{
#ifdef WL_MODESW
			uint8 oper_mode, op_rxstreams = wlc->stf->op_rxstreams;
			chanspec_t chspec = (bsscfg->associated) ? bsscfg->current_bss->chanspec :
				wlc->default_bss->chanspec;

#ifdef RXCHAIN_PWRSAVE
			if (wlc->ap && wlc_ap_in_rxchain_power_save(wlc->ap)) {
				uint8 rxchain = wlc_rxchain_pwrsave_get_rxchain(wlc);
				op_rxstreams = WLC_BITSCNT(rxchain);
			}
#endif /* RXCHAIN_PWRSAVE */
		...
		case IOV_SVAL(IOV_STF_RXCHAIN):
		{
...
#endif  /* WL_STF_ARBITRATOR */
			{
				...
				if (err == BCME_OK) {
					...
					/* Update rateset */
					wlc_rateset_ht_mcs_build(&wlc->default_bss->rateset,
						wlc->stf->op_rxstreams);
					wlc_rateset_filter(&wlc->default_bss->rateset,
						&wlc->default_bss->rateset, FALSE,
						WLC_RATES_CCK_OFDM, RATE_MASK_FULL,
		...
		case IOV_SVAL(IOV_STF_NSS_TX):
		{
			...
			/* Extract the scb */
			if (!(scb = wlc_scbfind(wlc, wlc->primary_bsscfg,
				&wlc->primary_bsscfg->BSSID))) {
		...
		case IOV_SVAL(IOV_STF_SR_CFG):
			...
			break;
#ifdef WL_MIMOPS_CFG
		case IOV_GVAL(IOV_MIMOPS):
			*ret_int_ptr = wlc_mimops_get(wlc);
			break;

		case IOV_SVAL(IOV_MIMOPS):
			if (!wlc_mimops_set(wlc, bsscfg, int_val)) {
				err = BCME_RANGE;
			}
			break;
#endif /* WL_MIMOPS_CFG */
...
wlc_stf_get_ppr_offset(wlc_info_t *wlc, wl_txppr_t *pbuf)
{
	...
	pbuf->chanspec = phy_utils_get_chanspec(WLC_PI(wlc));
...
wlc_txduty_upd(wlc_info_t *wlc)
{
	...
	if (wlc->pub->up) {
		...
		if ((wlc_stf_duty_cycle_set(wlc, duty_cycle_cck, FALSE, TRUE) == BCME_OK) &&
			(wlc_stf_duty_cycle_set(wlc, duty_cycle_ofdm, TRUE, TRUE) == BCME_OK)) {
#if defined(WL11AX) && defined(STA)
			wlc_bsscfg_t *bsscfg;
			int idx;

			duty_cycle = MIN(duty_cycle_ofdm, duty_cycle_cck);
			/* Initiate duty_cycle-based UL MU disable change */

			/* Make sure immediate OMI is triggered if needed */
			FOREACH_AS_STA(wlc, idx, bsscfg) {
				ASSERT(bsscfg != NULL);
				wlc_he_omi_ulmu_disable(wlc->hei, bsscfg,
					(duty_cycle < NO_DUTY_THROTTLE));
			}
#endif /* WL11AX && STA */
		}
#ifdef WL11AC
 ...
 */
static void
wlc_stf_update_160mode_txcore(wlc_stf_info_t *stf, chanspec_t chanspec)
{
...
wlc_stf_tempsense_upd(wlc_info_t *wlc)
{
	...
	temp_throttle_req = phy_temp_is_heatedup(WLC_PI(wlc)) ?
...
wlc_stf_enter_rxchain_pwrsave(wlc_info_t *wlc)
{
	...
	if (!wlc_ap_stas_associated(wlc->ap))
		phy_chanmgr_pad_online_enable(wlc->pi, FALSE, TRUE);
...
wlc_stf_exit_rxchain_pwrsave(wlc_info_t *wlc, uint8 ht_cap_rx_stbc)
{
	...
	phy_chanmgr_pad_online_enable(wlc->pi, TRUE, TRUE);
}
...
wlc_stf_init_txcore_default(wlc_info_t *wlc)
{
	...
	for (i = 0; i < MAX_CORE_IDX; i++) {
		...
		else if (D11REV_IS(wlc->pub->corerev, 30)) /* for 43217 only */
			wlc->stf->txcore[i][1] =
				wlc_stf_core_mask_assign(wlc->stf, txcore_default[i][0]);
		else
 ...
 */
static uint8
wlc_stf_core_mask_assign(wlc_stf_info_t *stf, uint core_count)
{
 ...
 */
static uint8
wlc_stf_txchain_subval_update(wlc_stf_info_t *stf, uint id, uint8 txchain_subval)
{
	int i;
...
void
wlc_stf_chain_active_set(wlc_info_t *wlc, uint8 active_chains)
{
...
#ifdef WL11AX
	/* Send OMI indication to peers */
	if (HE_ENAB(wlc->pub) && (current_streams != new_streams)) {
		wlc_he_upd_omi_txnsts(wlc->hei, (new_streams - 1));
	}
#endif /* WL11AX */
	/* If packets are enqueued, then wait for it to drain only if switching to fewer chains */
...
wlc_stf_rxchain_set(wlc_info_t* wlc, int32 int_val, bool update)
{
...
#ifdef WL11AX
	/* Send OMI indication to peers */
	if (HE_ENAB(wlc->pub) && (old_rxchain != rxchain)) {
		wlc_he_upd_omi_rxnss(wlc->hei, (rxchain_cnt - 1));
	}
#endif /* WL11AX */
	/* PR81503: need to iterate over active bsscfgs instead of
	 * just using wlc->primary_bsscfg for wlc_stf_mimops_action_send() calls
	 */
	...
	if (rxchain_cnt != old_rxchain_cnt &&
		...
		wlc_stf_mimops_handle_rxchain_update(wlc, mimops_mode);
	}
...
wlc_stf_txcore_shmem_write(wlc_info_t *wlc, bool forcewr)
{
	...
	if (D11REV_GE(wlc->pub->corerev, 129)) {
		/* need to set HETB mask too */
		/* no support for WOWL in these corerevs */
		ASSERT(wlc->stf->shmem_base == wlc->pub->m_coremask_blk);
		/* always write txchain setting */
		wlc_write_shm(wlc, M_COREMASK_HETB(wlc), wlc->stf->txchain);
	}
...
wlc_stf_watchdog(void *ctx)
{
	...
	if (WLC_MIMOPS_ENAB(wlc->pub)) {
		wlc_stf_mimops_watchdog(wlc);
	}
#endif /* WL_MIMOPS_CFG */
#ifdef STA
	FOREACH_AS_STA(wlc, i, cfg) {
		wlc_stf_mimops_action(wlc, cfg);
	}
#endif /* STA */
}
...
BCMATTACHFN(wlc_stf_attach)(wlc_info_t* wlc)
{
	...
	enum wlc_bandunit bandunit;
	wlc_stf_info_t *stf = wlc->stf;
	bsscfg_cubby_dump_t bsscfg_cubby_dump_fn = NULL;
	scb_cubby_dump_t scb_cubby_dump_fn = NULL;
	/* register module */
...
#if defined(BCMDBG)
	bsscfg_cubby_dump_fn = wlc_stf_bsscfg_cubby_dump;
	scb_cubby_dump_fn = wlc_stf_scb_cubby_dump;
#endif // endif

	stf->cfgh_cubby = wlc_bsscfg_cubby_reserve(wlc, sizeof(wlc_stf_bsscfg_cubby_t), NULL, NULL,
			bsscfg_cubby_dump_fn, (void *)stf);
	stf->scbh_cubby = wlc_scb_cubby_reserve(wlc, sizeof(wlc_stf_scb_cubby_t),
			NULL, NULL, scb_cubby_dump_fn, (void *)stf);
	if (stf->cfgh_cubby < 0 || stf->scbh_cubby < 0) {
		WL_ERROR(("wl%d: %s: cubby_reserve failed\n", WLCWLUNIT(wlc), __FUNCTION__));
		goto fail;
	}

	/* init d11 core dependent rate table offset */
	if (D11REV_LT(wlc->pub->corerev, 40)) {
		stf->shm_rt_txpwroff_pos = M_RT_TXPWROFF_POS(wlc);
	} else {
		stf->shm_rt_txpwroff_pos = M_REV40_RT_TXPWROFF_POS(wlc);
	}
	...
	if (WLC_STBC_CAP_PHY(wlc)) {
		stf->ss_algosel_auto = TRUE;
		stf->ss_algo_channel = (uint16)-1; /* Init the default value */
#ifdef WL11N_STBC_RX_ENABLED
		wlc_stf_stbc_rx_ht_update(wlc, HT_CAP_RX_STBC_ONE_STREAM);
		if (stf->op_txstreams > 1) {
			FOREACH_WLC_BAND(wlc, bandunit) {
	...
	if ((stf->txpwr_ctl = ppr_create(wlc->pub->osh, ppr_get_max_bw())) == NULL) {
		goto fail;
	}
#ifdef WL11AX
	if ((stf->ru_txpwr_ctl = ppr_ru_create(wlc->pub->osh)) == NULL) {
		goto fail;
	}
#endif /* WL11AX */
#ifdef WL_SARLIMIT
	if ((stf->txpwr_ctl_qdbm = ppr_create(wlc->osh, ppr_get_max_bw())) == NULL) {
	...
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		stf->max_offset = 0x3f;	/* cap to 31.5 dBm Max */
	} else {
		stf->max_offset = 0x1f;	/* cap to 15.5 dBm Max */
	}

	/* default Spatial mode to ON */
	memset(stf->spatial_mode_config, ON, SPATIAL_MODE_MAX_IDX);
	stf->shmem_base = wlc->pub->m_coremask_blk;

	stf->tempsense_period = WLC_TEMPSENSE_PERIOD;
	...
	if ((temp != 0) && (temp < 0xf)) {
		...
		if (!((ACREV_IS(wlc->band->phyrev, 32) || ACREV_IS(wlc->band->phyrev, 33)) &&
			(temp == 1)))
			stf->tempsense_period = temp;
	}

	/* override tx coremask for cck rates */
	temp = getintvar(wlc->pub->vars, rstr_cck_tx_coremask);
	if (temp != 0) {
		stf->txcore_override[CCK_IDX] = (uint8)temp;
	}

	/* enable tx diversity for cck rates */
	temp = getintvar(wlc->pub->vars, rstr_cck_onecore_tx);
	if (temp != 0) {
		stf->txcore_override[CCK_IDX] |= CCK_TX_DIVERSITY_BIT;
	}

	stf->rssi_pwrdn_disable = TRUE;
	stf->pwr_throttle_mask = stf->hw_txchain;
	stf->txchain_perrate_state_modify = NULL;

	/* initialize mimops_mode */
	stf->mimops_mode = HT_CAP_MIMO_PS_OFF;
	/* SISO device has its MIMO PS set to static disabled */
	if (WLC_BITSCNT(wlc->stf->hw_rxchain) == 1) {
		stf->mimops_mode = HT_CAP_MIMO_PS_ON;
	}
	...
	ocl_info->wlc = wlc;
	stf->ocl_info = ocl_info;
	/* init of ocl_en is controlled by the nvram */
	/* ocl_info->ocl_en = phy_ocl_get_en(WLC_PPI(wlc)); */
	...
	if (WLC_STF_ARB_ENAB(wlc->pub)) {
		...
		if ((stf->arb = wlc_stf_arb_attach(wlc,
		                     (void *)stf->arb_mps_info_hndl)) == NULL) {
			err = BCME_NOMEM;
			goto fail;
		}
	}
	...
	if (WLC_MIMOPS_ENAB(wlc->pub)) {
		if ((err = wlc_stf_mimops_attach(wlc,
		                                 (void *)stf->arb_mps_info_hndl)) != BCME_OK) {
			goto fail;
		}
	}
#endif /* WL_MIMOPS_CFG */

	stf->tx_duty_cycle_thermal = NO_DUTY_THROTTLE;
...
} /* wlc_stf_d11hdrs_phyctl_get_txant_mask */
...
#define DIV_REM(num, div) (((num%div) * 100)/div) /* Return the remainder of division */
	...
	if (offset[rate] == (uint8)(WL_RATE_DISABLED))
		...
		return (offset[rate]);
} /* wlc_stf_get_pwrperrate */

int
wlc_stf_get_power_offset_per_ratespec(wlc_info_t *wlc, ratespec_t rspec, uint8 *offset,
	wl_tx_chains_t txbf_chains)
{
	...
	if ((txpwr_offset = ppr_create(wlc->osh, ppr_get_max_bw())) == NULL) {
		WL_ERROR(("wl%d: %s: txpwr_offset create failed\n", wlc->pub->unit,
			__FUNCTION__));
		return BCME_NOMEM;
	}

	wlc_phy_get_tx_power_offset(WLC_PI(wlc), txpwr_offset);
	...
	if (RSPEC_ISHE(rspec)) {
		rate = (rspec & WL_RSPEC_HE_MCS_MASK);
		ppr_get_he_mcs(txpwr_offset, bw, nss, mode, chains, &he_mcs_pwrs, WL_HE_RT_SU);
		*offset = he_mcs_pwrs.pwr[rate];
	} else if (RSPEC_ISVHT(rspec)) {
		rate = (rspec & WL_RSPEC_VHT_MCS_MASK);
		ppr_get_vht_mcs(txpwr_offset, bw, nss, mode, chains, &mcs_pwrs);
		...
		*offset = mcs_pwrs.pwr[rate];
		...
		rate = ofdm_pwr_idx_table[rate/6];
		ASSERT(rate != 0x80);
		ppr_get_ofdm(txpwr_offset, bw, mode, chains, &ofdm_pwrs);
		*offset = ofdm_pwrs.pwr[rate];
	} else if (RSPEC_ISCCK(rspec)) {
		rate = (uint8)RSPEC2RATE(rspec);
		rate = cck_pwr_idx_table[rate >> 1];
		ASSERT(rate != 0x80);
		ppr_get_dsss(txpwr_offset, bw, chains, &dsss_pwrs);
...
free_ppr:
	ppr_delete(wlc->osh, txpwr_offset);
	return err;
}
...
#ifndef WLTEST
	if (max_txpwr_limit < wlc_phy_maxtxpwr_lowlimit(pi)) {
...
wlc_stf_chain_init(wlc_info_t *wlc)
{
	...
	if (wlc->stf->channel_bonding_cores) {
		phy_stf_chain_init(pi,
			wlc->stf->channel_bonding_cores,
			wlc->stf->channel_bonding_cores);
		phy_stf_chain_set(pi, wlc->stf->channel_bonding_cores,
			wlc->stf->channel_bonding_cores);
	} else {
		phy_stf_chain_init(pi, wlc->stf->hw_txchain, wlc->stf->hw_rxchain);
		phy_stf_chain_set(pi, wlc->stf->txchain, wlc->stf->rxchain);
	}
}
...
BCMUNINITFN(wlc_stf_down)(void *hdl)
{
	...
	if (PWRSTATS_ENAB(wlc->pub)) {
		...
		(void) wlc_mimo_siso_metrics_report(wlc);
	}
#endif /* WL_PWRSTATS */
	wlc->stf->op_txstreams = wlc->stf->txstreams;
	wlc->stf->op_rxstreams = wlc->stf->rxstreams;
...
wlc_stf_ocl_rssi_thresh_handling(wlc_bsscfg_t *bsscfg)
{
	...
	phy_ocl_status_get(pi, &bits, &curr_coremask, NULL);
...
#endif /* WL_PWRSTATS */
			phy_ocl_disable_req_set(pi, OCL_DISABLED_RSSI,
			                            !disable, WLC_OCL_REQ_RSSI);
		}
	}
} /* wlc_stf_ocl_rssi_thresh_handling */

void
wlc_stf_ocl_update_assoc_pend(wlc_info_t* wlc, bool disable)
{
	phy_info_t *pih = WLC_PI(wlc);
	bool bit_present;
	uint16 bits;
	...
	if ((disable && !bit_present) || (!disable && bit_present)) {
		phy_ocl_disable_req_set(pi, OCL_DISABLED_ASPEND,
...
wlc_stf_ocl_assoc_state_cb(void *ctx, bss_assoc_state_data_t *notif_data)
{
	...
	if (!((OCL_ENAB(wlc->pub)) && (BSSCFG_INFRA_STA(cfg)) && BSSCFG_IS_PRIMARY(cfg)))
		return;
...
#ifdef WL_STF_ARBITRATOR
void
wlc_stf_set_arbitrated_chains_complete(wlc_info_t *wlc)
{
	 /* push the updated txant to phytxant (used for txheader) */
	_wlc_stf_phy_txant_upd(wlc);
	/* initialize txcore and spatial policy */
...

{
	...
	*loop_cnt = bw + 1;
} /* wlc_stf_ack_ppr_get_ppr_offset_5g */
...
wlc_stf_srcfg_cmd_get_dispatch(wlc_info_t *wlc, sr_config_t *params, char *outbuf, int outlen)
{
	...
	} else if (!strncmp(params->cmdstr, "srg_obsscolorbmp", strlen("srg_obsscolorbmp"))) {
		for (i = 0; i < 4; i++) {
			bcm_bprintf(&bstr, "srg_obsscolorbmp [%d] = 0x%x \n",
				i, (uint16)(wlc->stf->srg_bssclr_bmp[i]));
		}
	} else if (!strncmp(params->cmdstr, "srg_pbssidbmp", strlen("srg_pbssidbmp"))) {
		for (i = 0; i < 4; i++) {
			bcm_bprintf(&bstr, "srg_pbssidbmp [%d] = 0x%x \n",
				i, (uint16)(wlc->stf->srg_pbssid_bmp[i]));
		}
	} else if (!strncmp(params->cmdstr, "sr_cnts", strlen("sr_cnts"))) {
		bcm_bprintf(&bstr, "sr_opp_cnt %d \n",
				(int16)(wlc_read_shm(wlc, M_D11SROPP_CNT(wlc))));
		bcm_bprintf(&bstr, "sr_tx_cnt %d \n",
				(int16)(wlc_read_shm(wlc, M_D11SRTX_CNT(wlc))));
	} else if (!strncmp(params->cmdstr, "options", strlen("options"))) {

		switch (wlc_read_shm(wlc, M_D11SR_OPTIONS(wlc))) {
			case OBSSPDSREN_OFF: bcm_bprintf(&bstr, "options 0 \n");
				break;
			case OBSSPDSREN_HE: bcm_bprintf(&bstr, "options 1 \n");
				break;
			case OBSSPDSREN_HEVHT: bcm_bprintf(&bstr, "options 2 \n");
				break;
			case SRPSREN: bcm_bprintf(&bstr, "options 4 \n");
				break;
			case SRP_OBSSPDSREN_HE: bcm_bprintf(&bstr, "options 5 \n");
				break;
			case SRP_OBSSPDSREN_HEVHT: bcm_bprintf(&bstr, "options 6 \n");
				break;
	...
	return err;
} /* wlc_stf_srcfg_cmd_get_dispatch */
...
wlc_stf_srcfg_cmd_set_dispatch(wlc_info_t *wlc, sr_config_t *params)
{
	...
	if (!strncmp(params->cmdstr, "nsrg_pdmin", strlen("nsrg_pdmin"))) {
		...
		wlc->stf->srg_bssclr_bmp[0] = (uint16)((uint64)(params->val) & 0xffff);
		wlc->stf->srg_bssclr_bmp[1] = (uint16)(((uint64)(params->val) >> 16) & 0xffff);
		wlc->stf->srg_bssclr_bmp[2] = (uint16)(((uint64)(params->val) >> 32) & 0xffff);
		wlc->stf->srg_bssclr_bmp[3] = (uint16)(((uint64)(params->val) >> 48) & 0xffff);
		phy_chanmgr_srg_bssColor(wlc->pi, 0,
				wlc->stf->srg_bssclr_bmp[0], wlc->stf->srg_bssclr_bmp[1],
				wlc->stf->srg_bssclr_bmp[2], wlc->stf->srg_bssclr_bmp[3]);
	} else if (!strncmp(params->cmdstr, "srg_pbssidbmp", strlen("srg_pbssidbmp"))) {
		wlc->stf->srg_pbssid_bmp[0] = (uint16)((uint64)(params->val) & 0xffff);
		wlc->stf->srg_pbssid_bmp[1] = (uint16)(((uint64)(params->val) >> 16) & 0xffff);
		wlc->stf->srg_pbssid_bmp[2] = (uint16)(((uint64)(params->val) >> 32) & 0xffff);
		wlc->stf->srg_pbssid_bmp[3] = (uint16)(((uint64)(params->val) >> 48) & 0xffff);
		phy_chanmgr_srg_bssColor(wlc->pi, 1,
				wlc->stf->srg_pbssid_bmp[0], wlc->stf->srg_pbssid_bmp[1],
				wlc->stf->srg_pbssid_bmp[2], wlc->stf->srg_pbssid_bmp[3]);
	} else if (!strncmp(params->cmdstr, "options", strlen("options"))) {
		/* set correponding PHY regs */
		phy_chanmgr_sr_enable(wlc->pi, params->val);
		/* Disable SR */
		if (params->val == 0) {
			if ((wlc_mhf_get(wlc, MHF2, WLC_BAND_AUTO) & MHF2_SR_ENABLE)
					? TRUE : FALSE)
				wlc_mhf(wlc, MHF2, MHF2_SR_ENABLE, 0, WLC_BAND_AUTO);
			/* write SR options to shm */
			wlc_write_shm(wlc, M_D11SR_OPTIONS(wlc), (int16)OBSSPDSREN_OFF);
			/* reset counters */
			wlc_write_shm(wlc, M_D11SROPP_CNT(wlc), 0);
			wlc_write_shm(wlc, M_D11SRTX_CNT(wlc), 0);
		/* Enable SR */
		} else {
			if (!((wlc_mhf_get(wlc, MHF2, WLC_BAND_AUTO) & MHF2_SR_ENABLE)
						? TRUE : FALSE))
			...
			switch (params->val) {
				case 1:
					wlc_write_shm(wlc, M_D11SR_OPTIONS(wlc),
							(int16)OBSSPDSREN_HE);
					break;
				case 2:
					wlc_write_shm(wlc, M_D11SR_OPTIONS(wlc),
							(int16)OBSSPDSREN_HEVHT);
					break;
				case 4:
					wlc_write_shm(wlc, M_D11SR_OPTIONS(wlc),
							(int16)SRPSREN);
				...
				case 5:
					wlc_write_shm(wlc, M_D11SR_OPTIONS(wlc),
							(int16)SRP_OBSSPDSREN_HE);
					break;
				case 6:
					wlc_write_shm(wlc, M_D11SR_OPTIONS(wlc),
							(int16)SRP_OBSSPDSREN_HEVHT);
					break;
				default:
					break;
			}
	...
	return err;
} /* wlc_stf_srcfg_cmd_set_dispatch */

/** can even be called when WL_MIMOPS_CFG is undefined */
static void
wlc_stf_mimops_handle_rxchain_update(wlc_info_t *wlc, uint8 mimops_mode)
{
	wlc_bsscfg_t *cfg;
	uint8 idx;

	FOREACH_AS_BSS(wlc, idx, cfg) {
		wlc_stf_set_cfg_mimops_PM(wlc, cfg, mimops_mode);
	}

	if (AP_ENAB(wlc->pub)) {
		phy_stf_chain_set(WLC_PI(wlc), wlc->stf->txchain, wlc->stf->rxchain);
		wlc_stf_mimops_cap_update(wlc, mimops_mode);
		if ((wlc->pub->associated) &&
#ifdef WL_MODESW
			(WLC_MODESW_ENAB(wlc->pub) &&
			!MODE_SWITCH_IN_PROGRESS(wlc->modesw)) &&
#endif // endif
			TRUE) {
			FOREACH_AS_BSS(wlc, idx, cfg) {
				wlc_stf_mimops_action_send(wlc, cfg, mimops_mode);
			}
		}
		return;
	}
...
} /* wlc_stf_mimops_handle_rxchain_update */

#if defined(BCMDBG)
static void
wlc_stf_bsscfg_cubby_dump(void *ctx, wlc_bsscfg_t *bsscfg, struct bcmstrbuf *b)
{
	wlc_stf_info_t *stf = (wlc_stf_info_t*)ctx;
	wlc_stf_bsscfg_cubby_t *cubby = STF_BSSCFG_CUBBY(stf, bsscfg);
	BCM_REFERENCE(cubby);
	ASSERT(cubby != NULL);
} /* wlc_stf_bsscfg_cubby_dump */

static void
wlc_stf_scb_cubby_dump(void *ctx, struct scb *scb, struct bcmstrbuf *b)
{
	wlc_stf_info_t *stf = (wlc_stf_info_t*)ctx;
	wlc_stf_scb_cubby_t *cubby = STF_SCB_CUBBY(stf, scb);
	BCM_REFERENCE(cubby);
	ASSERT(cubby != NULL);
	bcm_bprintf(b,	"mimops_enabled %d mimops_rtsmode %d\n",
			cubby->mimops_enabled, cubby->mimops_rtsmode);
} /* wlc_stf_scb_cubby_dump */
#endif // endif

void
wlc_stf_mimops_action_send(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 mimops_mode)
{
	wlc_stf_bsscfg_cubby_t *cubby = STF_BSSCFG_CUBBY(wlc->stf, bsscfg);
	int err;
	...
	err = wlc_stf_send_action_mimops(wlc, bsscfg, mimops_mode);
...
#ifdef STA
/** transmit action frames if required */
static void
wlc_stf_mimops_action(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	wlc_stf_bsscfg_cubby_t *cubby = STF_BSSCFG_CUBBY(wlc->stf, bsscfg);

	if (cubby->mimops_ActionRetry & WLC_MIMOPS_RETRY_SEND) {
		wlc_stf_mimops_action_send(wlc, bsscfg, cubby->mimops_PM);
	} else if (cubby->mimops_ActionRetry & WLC_MIMOPS_RETRY_NOACK) {
		wlc_stf_mimops_action_send(wlc, bsscfg, cubby->mimops_ActionPM);
	}
}
#endif /* STA */

/** Called after action frame has been transmitted and (hopefully) ack'ed */
static void
wlc_stf_mimops_action_complete(wlc_info_t *wlc, uint txstatus, void *arg)
{
	wlc_bsscfg_t *cfg = wlc_bsscfg_find_by_ID(wlc, (uint16)(uintptr)arg);
	wlc_stf_bsscfg_cubby_t *cubby = STF_BSSCFG_CUBBY(wlc->stf, cfg);
	...
	if (cubby->mimops_ActionPM == HT_CAP_MIMO_PS_ON) {
		if (VHT_ENAB_BAND(wlc->pub, wlc->band->bandtype))
			wlc_vht_update_mcs_cap(wlc->vhti);
		wlc_stf_mimops_cap_update(wlc, HT_CAP_MIMO_PS_ON);
#if defined(WL_PWRSTATS)
		if (PWRSTATS_ENAB(wlc->pub)) {
			wlc_mimo_siso_metrics_snapshot(wlc, FALSE,
...
#ifdef WL_MIMOPS_CFG
	if (WLC_MIMOPS_ENAB(wlc->pub)) {
		wlc_stf_cfg_timer_start(cfg);
	}
#endif /* WL_MIMOPS_CFG */
	cubby->mimops_ActionPending = FALSE;

	/* FIXME: PR81503: This code needs to know what bsscfg just completed the PS
	 * notification instead of just using wlc->primary_bsscfg. We may need to move along a
	 * higher layer state machine that is updating multiple bsscfgs.
	 */
	if (cubby->mimops_ActionPM != cubby->mimops_PM)
		wlc_stf_mimops_action_send(wlc, cfg, cubby->mimops_PM);
} /* wlc_stf_mimops_action_complete */

int
wlc_stf_send_action_mimops(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 mimops_mode)
{
	void *p;
	uint8* pbody;
	uint body_len;
	...
	ASSERT(mimops_mode != 2 && mimops_mode <= 3);
	...
	if (BSSCFG_STA(bsscfg)) {
		wlc_pcb_fn_register(wlc->pcb, wlc_stf_mimops_action_complete,
			(void *)(uintptr)bsscfg->ID, p);
	}

	/*
	 * If sending fails, p will be freed
	 * Return BCME_NOMEM to let the calling function know
	 * sending failed.
	 */
...
#ifdef WL_MIMOPS_CFG
void
wlc_stf_set_mimops_ActionPM(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 action_pm)
{
	wlc_stf_bsscfg_cubby_t *cubby = STF_BSSCFG_CUBBY(wlc->stf, bsscfg);

	cubby->mimops_ActionPM = action_pm;
}
#endif /* WL_MIMOPS_CFG */

uint8
wlc_stf_get_cfg_mimops_PM(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	wlc_stf_bsscfg_cubby_t *cubby = STF_BSSCFG_CUBBY(wlc->stf, bsscfg);

	return cubby->mimops_PM;
}

void
wlc_stf_set_cfg_mimops_PM(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 v)
{
	wlc_stf_bsscfg_cubby_t *cubby = STF_BSSCFG_CUBBY(wlc->stf, bsscfg);
...
/** Updates the local node's advertised capabilities */
void
wlc_stf_mimops_cap_update(wlc_info_t *wlc, uint8 mimops_mode)
{
	wlc->stf->mimops_mode = mimops_mode;
...
/**
 * Called from wlc_ht.c or wlc_vht.c on e.g. action frame reception
 *
 * @return    TRUE if the scb cubby was updated
 */
bool
wlc_stf_mimops_on_info_received(wlc_info_t *wlc, struct scb *scb, bool reinit_ratesel,
                              bool mimops_enabled, bool mimops_rtsmode)
{
	wlc_stf_scb_cubby_t *cubby = STF_SCB_CUBBY(wlc->stf, scb);
	bool ret = FALSE;

	if ((cubby->mimops_enabled != mimops_enabled) ||
	    (cubby->mimops_rtsmode != mimops_rtsmode)) {
		WL_INFORM(("wl%d:%s,"MACF"changd,mimops:[%d %d],rtsmde[%d %d]\n",
			wlc->pub->unit, __FUNCTION__, ETHER_TO_MACF(scb->ea),
			cubby->mimops_enabled, mimops_enabled,
			cubby->mimops_rtsmode, mimops_rtsmode));
		cubby->mimops_enabled = mimops_enabled;
		cubby->mimops_rtsmode = mimops_rtsmode;
		ret = TRUE;
		if (reinit_ratesel == TRUE) {
			wlc_scb_ratesel_init(wlc, scb);
#ifdef WL11AX
			...
			/* Return if d-smps_war is not enabled */
			if (!wlc->dsmps_war) {
				WL_TRACE(("wl%d: %s:Dyn  SMPS WAR disabled \n",
					wlc->pub->unit, __FUNCTION__));
				return ret;
			}
			 ...
			 */
			if (SCB_HE_CAP(scb) && HE_DLMMU_ENAB(wlc->pub)) {
				wlc_mutx_update_hemmu_dsmps(wlc->mutx, scb);
			}
...
bool
wlc_stf_is_scb_dynamic_smps(wlc_info_t *wlc, struct scb *scb)
{
	wlc_stf_scb_cubby_t *cubby;

	if (wlc == NULL || scb == NULL) {
		return FALSE;
	}

	cubby = STF_SCB_CUBBY(wlc->stf, scb);

	return (cubby->mimops_enabled && cubby->mimops_rtsmode);
}
