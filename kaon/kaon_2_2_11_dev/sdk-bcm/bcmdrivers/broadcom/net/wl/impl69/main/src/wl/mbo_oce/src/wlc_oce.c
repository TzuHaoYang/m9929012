...
struct wlc_oce_info {
	...
	uint	non_oce_detect_environment; /* elapsed seconds */
	uint	ap_11b_detect_environment; /* elapsed seconds */
};
...
wlc_oce_watchdog(void *ctx)
{
	...
	if ((oce->wlc->pub->now - oce->non_oce_detect_environment) > 60) {
		oce->oce_cmn_info->control_field &= ~NON_OCE_AP_PRESENT;
		oce->non_oce_detect_environment = oce->wlc->pub->now;
	}

	if ((oce->wlc->pub->now - oce->ap_11b_detect_environment) > 60) {
		oce->oce_cmn_info->control_field &= ~OCE_11b_ONLY_AP_PRESENT;
		oce->ap_11b_detect_environment = oce->wlc->pub->now;
	}
#endif /* WL_OCE_AP */
}
...
void
wlc_oce_detect_11b_environment(wlc_oce_info_t *oce)
{
	oce->ap_11b_detect_environment = oce->wlc->pub->now;
}
...
{
	if (oce->oce_cmn_info->control_field == 0) {
		return;
	}
...
wlc_oce_get_rnr_completeness_attr_len(wlc_oce_info_t *oce, wlc_bsscfg_t *cfg)
{
	...
	if (!isset(&cfg->wlc->nbr_discovery_cap, WLC_NBR_DISC_CAP_RNR_IE)) {
...
wlc_oce_parse_rnr_element(wlc_oce_info_t *oce, uint8* buf, uint16 buflen)
{
	...
	while (len >= NEIGHBOR_AP_INFO_FIELD_HDR_LEN) {
		...
		if ((ap_info.op_class >= BCMWIFI_OP_CLASS_6G_20MHZ) &&
				(ap_info.op_class <= BCMWIFI_OP_CLASS_6G2_20MHZ)) {
...
#endif /* BAND6G */
		WL_INFORM(("Neighbor AP Info Field: channel %d tbtt info count %d "
			"tbtt info len %d\n",
...
wlc_oce_send_fd_frame(wlc_oce_info_t *oce, wlc_bsscfg_t *bsscfg)
{
	...
	if (oce->wlc->upr_fd_info->in_flight >= WLC_UPR_FD_MAX_PKTS_IN_FLIGHT) {
		WL_INFORM(("wl%d: skipping FD, 1 packet still in Queue\n",
			oce->wlc->pub->unit));
		return BCME_ERROR;
	}
	...
	WLPKTTAG(pkt)->flags |= WLF_TX_PKT_UPR_FD;
	...
	if (wlc_queue_80211_frag(oce->wlc, pkt, bsscfg->wlcif->qi,
		...
		oce->wlc->upr_fd_info->in_flight++;
	} else {
		oce->wlc->upr_fd_info->n_pkts_xmit_fail++;
		oce->wlc->upr_fd_info->n_pktfree++;
	}
	return BCME_OK;
}
...
wlc_oce_update_control_field(wlc_oce_info_t *oce, uint8 option)
{
	...
	if (option & OCE_11b_ONLY_AP_PRESENT) {
		oce->ap_11b_detect_environment = oce->wlc->pub->now;
	}
	return BCME_OK;
}
