 ...
 *
 * $Id: soap_x_wancommoninterfaceconfig.c 738255 2017-12-27 22:36:47Z $
 */
#include <upnp.h>
...
)
{
	...
	/* << USER CODE START >> */
	upnp_tlv_set(tlv, (uintptr_t)"Ethernet");
	return OK;
}
/* >> AUTO GENERATED FUNCTION */
...
)
{
	...
	upnp_tlv_set(tlv, (uintptr_t)link_status);
	return OK;
}
/* >> AUTO GENERATED FUNCTION */
...
)
{
	...
	upnp_tlv_set(out_NewWANAccessType, (uintptr_t)"Ethernet");
	upnp_tlv_set(out_NewLayer1UpstreamMaxBitRate, tx);
	upnp_tlv_set(out_NewLayer1DownstreamMaxBitRate, rx);
	upnp_tlv_set(out_NewPhysicalLinkStatus, (uintptr_t)link_status);
	return OK;
}
/* >> AUTO GENERATED FUNCTION */
...
)
{
	...
	sprintf(buf, "uuid:%s:WANConnectionDevice:1", wan_advertise->uuid);
	upnp_tlv_set(out_NewActiveConnDeviceContainer, (uintptr_t)buf);

	sprintf(buf, "urn:upnp-org:serviceId:%s", wan_service->service_id);
	upnp_tlv_set(out_NewActiveConnectionServiceID, (uintptr_t)buf);
	return OK;
}
/* >> AUTO GENERATED FUNCTION */
