 ...
 *
 * $Id: wlconf.c 794023 2020-12-17 09:45:55Z $
 */
...
#define WL_HEIOVAR_SETINT(ifname, iovar, subcmd, val)				\
	if ((ret = wl_heiovar_setint(ifname, iovar, subcmd, val)))		\
		fprintf(stderr, "%s:%d:(%s): setting he "	\
		"iovar \"%s %s\" to 0x%x failed, err = %d\n", \
		        __FUNCTION__, __LINE__, ifname, iovar, subcmd, val, ret);
#define WL_TWTIOVAR_SETINT(ifname, iovar, subcmd, val)				\
	if ((ret = wl_twtiovar_setint(ifname, iovar, subcmd, val)))		\
		fprintf(stderr, "%s:%d:(%s): setting twt "	\
		"iovar \"%s %s\" to 0x%x failed, err = %d\n", \
		        __FUNCTION__, __LINE__, ifname, iovar, subcmd, val, ret);
#define WL_MSCHED_IOVAR_SETINT(ifname, iovar, subcmd, val)			\
	if ((ret = wl_msched_iovar_setint(ifname, iovar, subcmd, val)))	\
		fprintf(stderr, "%s:%d:(%s): setting msched "	\
		"iovar \"%s %s\" to 0x%x failed, err = %d\n", \
		        __FUNCTION__, __LINE__, ifname, iovar, subcmd, val, ret);
#define WL_IOVAR_XTLV_SETINT(ifname, iovar, val, version, cmd_id, xtlv_id)			\
	if ((ret = wl_iovar_xtlv_setint(ifname, iovar, val, version, cmd_id, xtlv_id)))		\
		fprintf(stderr, "%s:%d:(%s): setting xtlv "	\
		"iovar \"%s\" to 0x%x failed, err = %d\n",	\
		        __FUNCTION__, __LINE__, ifname, iovar, (unsigned int)val, ret);
...
#define WL_TWTIOVAR_SETINT(ifname, iovar, subcmd, val) \
		(ret = wl_twtiovar_setint(ifname, iovar, subcmd, val))
#define WL_MSCHED_IOVAR_SETINT(ifname, iovar, subcmd, val) \
		(ret = wl_msched_iovar_setint(ifname, iovar, subcmd, val))
#ifdef __CONFIG_DHDAP__
...
#define WL_CHSPEC_DEF_2G	0x1001		/* Default 2G chanspec on channel 1 */
#define WL_CHSPEC_DEF_5G_L	0xD024		/* Default 5GL chanspec on channel 36 */
#define WL_CHSPEC_DEF_5G_H	0xD095		/* Default 5GH chanspec on channel 149 */
#define WL_CHSPEC_DEF_6G	0x5001		/* Default 6G chanspec on channel 1 */
...
#define WLCONF_WARN_BANNER(fmt, arg...) do {						\
	printf("**************************************************************\n");	\
	printf("* WLCONF_WARNING: "fmt, ##arg); printf(" *\n");				\
	printf("**************************************************************\n");	\
} while (0)

#define MBSSID_ENAB(mbssid, mbssid_band) ((mbssid == -1  && mbssid_band == WLC_BAND_6G) || mbssid == 1)
/* prototypes */
...
#ifdef BCMWAPI_WAI
		if (!strcmp(akm, "wapi"))
			akm_ret_val |= WAPI_AUTH_UNSPECIFIED;
		if (!strcmp(akm, "wapi_psk"))
			akm_ret_val |= WAPI_AUTH_PSK;
#endif /* BCMWAPI_WAI */
	}
	return akm_ret_val;
}
...
wlconf_set_wsec(char *ifname, char *prefix, int bsscfg_idx)
{
...
#ifdef BCMWAPI_WAI
		else if (nvram_match(strcat_r(prefix, "crypto", tmp), "sms4"))
			val = SMS4_ENABLED;
#endif /* BCMWAPI_WAI */
	}
	...
	if ((val & WEP_ENABLED) || (val & TKIP_ENABLED)) {
		WLCONF_WARN_BANNER("Setting WEP and/or TKIP is not recommended");
	}
...
static void
wlconf_config_join_pref(char *name, int bsscfg_idx, int auth_val, int need)
{
	int ret = 0, i = 0;

	if (need && ((auth_val & (WPA_AUTH_UNSPECIFIED | WPA2_AUTH_UNSPECIFIED)) ||
	    CHECK_PSK(auth_val))) {
		uchar pref[] = {
		/* WPA pref, 14 tuples */
		0x02, 0xaa, 0x00, 0x0e,
		...
		/* clear join_pref */
		WL_BSSIOVAR_SET(name, "join_pref", bsscfg_idx, "", 0);
	}

}

#define DEFAULT_UPRFD_PERIOD	20
#define UPR_ENABLE		0x20 /* set 5th bit on nbr_discovery_cap */
#define FD_ENABLE		0x40 /* set 6th bit on nbr_discovery_cap */
#define UPR_FD_SW_ENAB		1
#define UPR_FD_UCODE_ENAB	2

static void
wlconf_configure_upr_and_fd(char *name, char *prefix, uint8 bandtype, struct bsscfg_list *bclist)
{
	struct bsscfg_info *bsscfg = NULL;
	int upr_fd_period = DEFAULT_UPRFD_PERIOD;
	int upr_fd_val = 0;
	int upr_fd_method = 0;	/* ucode or software driven */
	char tmp[100];
	char *str = NULL;
	uint8 nbr_discovery_cap = 0;
	int i;
	int ap;

	if (nvram_get(strcat_r(prefix, "nbr_discovery_cap", tmp)) != NULL) {
		nbr_discovery_cap = atoi(str);
	} else {
		/* If not explicitly, enable RNR by default */
		if (bandtype != WLC_BAND_6G) { /* limiting to 2.4g/5g for now */
			nbr_discovery_cap |= 0x01;
		}
	}

	if (nvram_get(strcat_r(prefix, "upr_fd_enable", tmp)) != NULL) {
		upr_fd_val = atoi(str);
	}
#ifndef WLTEST
	else {
		if (bandtype == WLC_BAND_6G) {
			upr_fd_val = 1; /* Enable UPR */
		}
	}
#endif /* WLTEST */
	...
	/* Adjust  */
	switch (upr_fd_val) {
		case 1 :	/* Enable UPR, Disable FD */
			nbr_discovery_cap |= UPR_ENABLE;
			nbr_discovery_cap &= ~FD_ENABLE;
			break;
		case 2 :	/* Enable FD, Disable UPR */
			nbr_discovery_cap |= FD_ENABLE;
			nbr_discovery_cap &= ~UPR_ENABLE;
			break;
		default:
			printf("Invalid option for UPR or FD for %s\n", name);
	}

	if (nvram_get(strcat_r(prefix, "upr_fd_period", tmp)) != NULL) {
		upr_fd_period = atoi(nvram_safe_get(strcat_r(prefix, "upr_fd_period", tmp)));
	}

	if (nvram_get(strcat_r(prefix, "upr_fd_method", tmp)) != NULL) {
		upr_fd_method = atoi(str);
	} else {
		if (bandtype == WLC_BAND_6G) {
			upr_fd_method = UPR_FD_UCODE_ENAB; /* default: ucode driven for 6G */
		} else {
			upr_fd_method = UPR_FD_SW_ENAB;
		}
	}

	wl_iovar_setint(name, "nbr_discovery_cap", nbr_discovery_cap);
	wl_iovar_setint(name, "upr_fd_sw", upr_fd_method);
	if (upr_fd_period >= 0) {
		wl_iovar_setint(name, "fd_prb_rsp_period", upr_fd_period);
	}

	/* If RNR is enabled, we also need to enable RRM to support this */
	if ((nbr_discovery_cap & 0x01)) {
		int ret = -1;
		for (i = 0; i < bclist->count; i++) {
			...
			if (ap) {
				WL_BSSIOVAR_GET(name, "rrm", bsscfg->idx, &rrm_val,
					sizeof(rrm_val));
				rrm_val |= 0x02; /* enable rnr */
				WL_BSSIOVAR_SETINT(name, "rrm", bsscfg->idx, rrm_val);
			} else {
				WL_BSSIOVAR_GET(name, "rrm", bsscfg->idx, &rrm_val,
					sizeof(rrm_val));
				rrm_val &= ~0x02; /* disable rnr */
				WL_BSSIOVAR_SETINT(name, "rrm", bsscfg->idx, rrm_val);
			}
		}
	}
...
static void
wlconf_configure_mbo_oce(char *primary_ifname, char *name, char *prefix, int bsscfg_idx)
{
	int ret;
	int mbo_ap_enable = 1; /* by default enable */
	int oce_ap_enable = 0;
	char tmp[100];
	//char var[8];
	char *str = NULL;
	...
	str = nvram_safe_get(strcat_r(prefix, "mbo_enable", tmp));
	if (str[0] != '\0') {
		mbo_ap_enable = atoi(str);
	}
	...
	str = NULL;
	str = nvram_safe_get(strcat_r(prefix, "oce_enable", tmp));
	if (str[0] != '\0') {
		oce_ap_enable = atoi(str);
	}

	WL_IOVAR_XTLV_SETINT(name, "oce", oce_ap_enable, WL_OCE_IOV_VERSION,
		WL_OCE_CMD_ENABLE, WL_OCE_XTLV_ENABLE);

	memset(tmp, 0, sizeof(tmp));
}

static void
wlconf_configure_6g_securities(char *name, char *prefix, int unit, int n_vifs)
{

	char tmp[100], tmp1[100], *str;
	int val = 0, ret = 0, i, bandtype;
	int max_no_vifs = 0;

	max_no_vifs = n_vifs;

	if ((max_no_vifs > WL_MAXBSSCFG) || (max_no_vifs == 0)) {
		max_no_vifs = WL_MAXBSSCFG;
	}
	...
	for (i = 0; i < max_no_vifs; i++) {
		memset(tmp1, 0, sizeof(tmp1));
		memset(tmp, 0, sizeof(tmp));
		if (i == 0) {
			snprintf(tmp1, sizeof(tmp1), "wl%d_", unit);
		} else {
			snprintf(tmp1, sizeof(tmp1), "wl%d.%d_", unit, i);
		}
		nvram_set(strcat_r(tmp1, "akm", tmp), "sae");

		memset(tmp, 0, sizeof(tmp));
		nvram_set(strcat_r(tmp1, "sae_pwe", tmp), "1");

		memset(tmp, 0, sizeof(tmp));
		nvram_set(strcat_r(tmp1, "crypto", tmp), "aes");

		memset(tmp, 0, sizeof(tmp));
		nvram_set(strcat_r(tmp1, "mfp", tmp), "2");

		memset(tmp, 0, sizeof(tmp));
		nvram_set(strcat_r(tmp1, "wps_mode", tmp), "disabled");
	}
}
...
#define AUTOWPA(cfg) \
	...
	*/
	...
	wlconf_config_join_pref(name, bsscfg_idx, val, need_join_pref);
...
#ifdef BCMWAPI_WAI
	WL_BSSIOVAR_SETINT(name, "wai_restrict", bsscfg_idx,
		(val & (WAPI_AUTH_UNSPECIFIED | WAPI_AUTH_PSK)));
	val &= ~ (WAPI_AUTH_UNSPECIFIED | WAPI_AUTH_PSK);
#endif // endif
	...
	/* Set WEP keys */
	if (nvram_match(strcat_r(prefix, "wep", tmp), "enabled")) {
		WLCONF_WARN_BANNER("WEP not recommended! Configuring WEP keys ");
	...
	if (val & (WPA2_AUTH_PSK | WPA2_AUTH_UNSPECIFIED | WPA2_AUTH_FT | WPA3_AUTH_SAE_PSK
		| WPA3_AUTH_SAE_FBT | WPA3_AUTH_SUITEB)) {
		int bandtype;
		int def_mfp;
		char *ptr = NULL;

		WL_GETINT(name, WLC_GET_BAND, &bandtype);
		def_mfp = (bandtype == WLC_BAND_6G) ? WL_MFP_REQUIRED: WL_MFP_CAPABLE;
		...
		if (ptr[0] && (ptr[0] != '0') && (val == 0)) {
			/* Invalid nvram, default */
			val = def_mfp;
		...
		/* For station/repeater, wpa_suplicant will handle the mfp setting
		 * if hostapd enabled */
		if ((nvram_match("hapd_enable", "0")) || (is_apmode == TRUE)) {
			WL_BSSIOVAR_SETINT(name, "mfp", bsscfg_idx, WL_MFP_DISABLE);
		}
	}
#endif /* ifdef MFP */
...
#endif // endif
					/* Set to all AX driver to enhance D/L TPUT */
					if (rev.corerev >= 129) {
						WL_IOVAR_SETINT(name, "amsdu_aggsf", 8);
					}
...
static void wlconf_set_txbf(char *name, char *prefix, int bandtype, struct bsscfg_list *bclist)
{
	char *str, tmp[100];
	wlc_rev_info_t rev;
	...
	for (i = 0; i < bclist->count; i++) {
		struct bsscfg_info *bsscfg = &bclist->bsscfgs[i];
		char *var;
		var = nvram_safe_get(strcat_r(bsscfg->prefix, "txbf_bfe_cap", tmp));
		txbf_bfe_cap = atoi(var);
		if (txbf_bfe_cap >= 0) {
			WL_BSSIOVAR_SETINT(name, "txbf_bfe_cap", bsscfg->idx, txbf_bfe_cap);
		}
	}
	...
	if ((str = nvram_get(strcat_r(prefix, "txbf_bfe_override_disable", tmp))) != NULL) {
		/* disallow txbf bfe disable for channel smoothing */
		WL_IOVAR_SETINT(name, "txbf_bfe_override_disable", atoi(str));
	}
...
static void
wlconf_set_taf(char *name, int enable)
{
	...
	if (enable == -1) {
		/* -1 is "auto" mode which means use the driver default */
		return;
	}
...
static void
wlconf_set_taf_ul(char *name, int enable)
{
	char iobuf[sizeof( "taf" )		/* Need some space for the iovar itself, + nul */
		+ sizeof(wl_taf_define_t)
		+ sizeof("ul")		/* "enable" string + terminating nul byte */
		+ sizeof("0")			/* "0" or "1" string + terminating nul byte */
		+ sizeof("")];			/* Final terminating nul byte */
	...
	ptr = taf_def->text;
	ptr += sprintf(ptr, "ul");	/* Set up the command name argument */
	*ptr++ = '\0';			/* add the NUL separator */
	*ptr++ = (enable) ? '1' : '0';	/* enable or disable taf */
	*ptr++ = '\0';			/* add the NUL separator */
	*ptr = '\0';			/* and another NUL to terminate the argument list */
...
wlconf_set_fbt(struct bsscfg_list *bclist)
{
	...
	for (i = 0; i < bclist->count; i++) {
		...
		/* FBT should be enabled only if the akm contains psk2ft or saeft type */
		strnvval = nvram_safe_get(strcat_r(bsscfg->prefix, "akm", tmp));
		if ((find_in_list(strnvval, "psk2ft") == NULL) &&
			(find_in_list(strnvval, "saeft") == NULL)) {
			WLCONF_DBG("i[%d] name[%s] prefix[%s] %sakm[%s]. akm is not psk2ft or saeft\n",
...
wlconf_overwrite_txbf_cap_nvram(char *name, char *prefix)
{
	...
	if (revinfo.corerev >= 129 && revinfo.corerev <= 131) {
		snprintf(val, sizeof(val), "%d",
			TXBF_SU_MU_BFR_CAP | TXBF_HE_SU_MU_BFR_CAP | TXBF_HE_CQI_BFR_CAP);
	} else {
	 ...
	 * Disable MUTX by default for 43692 and 6705 (the same chipid as 43692)
	 */
	if (revinfo.chipnum == BCM43692_CHIP_ID) {
...
wlconf(char *name)
{
	...
	int rxchain_pwrsave = 0, radio_pwrsave = 0, map = 0, map_mode = 0, map_profile = 0;
	...
	int map_cert = atoi(nvram_safe_get("map_cert"));
#endif // endif
	bool set_psp, bss_is_ap;
	int mbssid = -1, mbssid_band;
	...
	if ((str = nvram_get(strcat_r(prefix, "vifs_base", tmp))) != NULL) {
		/* Overwrite vifs base address. The least n bits come from hwaddr */
		/* Please see IEEE Guidelines for Use of EUI, OUI, and CID to select vifs_base. */
		ether_atoe(str, (unsigned char *)vif_addr);
		vif_addr[5] = (vif_addr[5] & ~(max_no_vifs - 1)) | (buf[5] & (max_no_vifs - 1));
	}
	 ...
	 */
#ifdef BCA_CPEROUTER
	if (restore_defaults || !nvram_get("_default_restored_"))
#else
	if (restore_defaults)
#endif // endif
	{
		...
		WL_IOCTL(name, WLC_GET_REVINFO, &rev, sizeof(rev));
		if (rev.corerev >= 129 && rev.corerev <= 132) {
			snprintf(tmp2, sizeof(tmp2), "%d", TXBF_SU_MU_BFE_CAP | TXBF_HE_SU_MU_BFE_CAP);
		} else { // corerev < 129
			snprintf(tmp2, sizeof(tmp2), "%d", 2);
		}

		// For MBSS virtual interfaces
		for (i = 1; i < WL_MAXBSSCFG; i++) {
			// assume tmp1 = wlX.Y_
			snprintf(tmp1, sizeof(tmp1), "wl%d.%d_", unit, i);
			nvram_set(strcat_r(tmp1, "txbf_bfe_cap", tmp), tmp2);
		}

		wlconf_set_current_txparam_into_nvram(name, prefix);
		wlconf_configure_6g_securities(name, prefix, unit, max_no_vifs);
		/* Clear country_code and country_rev. Default values will be
		 * obtained from driver and set later in this function
		 */
	...
	/* Apply message level */
	if (nvram_invmatch(strcat_r(prefix, "msglevel", tmp), "")) {
		struct wl_msglevel2 msglevel64 = {0};

		msglevel64.low = (int) strtoul(nvram_safe_get(
					strcat_r(prefix, "msglevel", tmp)), NULL, 0);
		msglevel64.high = (int) strtoul(nvram_safe_get(
					strcat_r(prefix, "msglevel2", tmp)), NULL, 0);

		WL_IOVAR_SET(name, "msglevel", &msglevel64, sizeof(struct wl_msglevel2));
	} else if (nvram_invmatch("wl_msglevel", "")) {
		struct wl_msglevel2 msglevel64 = {0};

		msglevel64.low = (int) strtoul(nvram_safe_get("wl_msglevel"), NULL, 0);
		msglevel64.high = (int) strtoul(nvram_safe_get("wl_msglevel2"), NULL, 0);
	...
	/* set mbssid */
	str = nvram_get(strcat_r(prefix, "mbssid", tmp));
	if (str && str[0]) {
		val = atoi(str);
		mbssid = val;
		WL_IOVAR_SETINT(name, "mbssid", val);
	}
	...
	if (WLCONF_PHYTYPE_11N(val)) {
		str = nvram_get(strcat_r(prefix, "nband", tmp));
		if (str)
			mbssid_band = atoi(str);
		else {
			WL_GETINT(name, WLC_GET_BAND, &mbssid_band);
		}
	} else {
		mbssid_band = WLCONF_PHYTYPE2BAND(val);
	}

	/* Create addresses for VIFs */
	if (mbss) {
		if (!MBSSID_ENAB(mbssid, mbssid_band)) {
#ifdef BCA_CPEROUTER
		...
		for (i = 1; i < max_no_vifs; i++) {
			...
			if ((ure_mbss && !mbss_ign_mac_valid) || !strcmp(addr, "") ||
				(MBSSID_ENAB(mbssid, mbssid_band) || mbssid == 0)) {
	...
	if (wl_ap_build) {
		...
		for (i = 0; i < bclist->count; i++) {
			...
			/* Do not set SSID if hostapd is enabled */
#ifdef CONFIG_HOSTAPD
	...
	/* If not defined, set to profile-2. Accepting profile-1 to
	 * support R1 for MultiAP R1 testbed
	 */
	map_profile = atoi(nvram_safe_get("map_profile"));
	if  (!map_profile) {
		map_profile = 2;
	}
#ifdef MULTIAP
	...
	/* Following block parses and applies nvram variables for PCIE drive strength configuration.
	 * Example format of nvram variable:
	 * wl0_pcieserdesreg_cmdX=0xf40 0x00 0x0e20
	 * X is integer from 1 to MAX_PCIE_SERDES_COMMAND.
	 */
#define MAX_PCIE_SERDES_COMMAND	9
	for (i = 1; i <= MAX_PCIE_SERDES_COMMAND; i++) {
		snprintf(tmp, sizeof(tmp), "%s%s%d", prefix, "pcieserdesreg_cmd", i);
		...
		if (strcmp(str, "")) {
			...
			while (*str != '\0') {
				...
				if (*str == '\0')
					break;
				val = strtoul(str, &str1, 16);
				if (str == str1)
					break;
				str = str1;
				var_count++;
				memcpy(&var[len], &val, sizeof(val));
				len += sizeof(val);
				if (len == sizeof(var))
					break;
			}
			if (var_count != 3) {
				printf("%s: parsing of pcie serdes register write "
					"arguments failed\n", __FUNCTION__);
				break;
			}
#ifdef __CONFIG_DHDAP__
			if (is_dhd) {
				dhd_iovar_set(name, "pcieserdesreg", var, sizeof(var));
			} else
#endif /* __CONFIG_DHDAP__ */
			{
				wl_iovar_set(name, "pcieserdesreg", var, sizeof(var));
			}
		}
	}
	...
	for (i = 0; i < bclist->count; i++) {
		...
		WL_BSSIOVAR_SETINT(name, "map_profile", bsscfg->idx, map_profile);
#ifdef MULTIAP
		/* If multiap is enabled, and it is onboarded,
		 * for STA interface enable dwds_lb_filter
		 */
		...
		/* If the BSS is backhaul and it has any VLAN to SSID mapping is configured,
		 * then set the primary VLAN ID
		 */
		if ((map_profile >= 2) && (map_val & 0x02) &&
			(strlen(nvram_safe_get("map_ts_vlans")) > 0)) {
			int prim_vlan_id;

			prim_vlan_id = strtol(nvram_safe_get("map_ts_prim_vlan_id"), NULL, 0);
			WL_BSSIOVAR_SETINT(name, "map_8021q_settings", bsscfg->idx, prim_vlan_id);
		} else {
			WL_BSSIOVAR_SETINT(name, "map_8021q_settings", bsscfg->idx, 0);
		}

		/* For backhaul bss set ucast_disassoc_on_bss_down */
		if (map_val == 2) {
			WL_BSSIOVAR_SETINT(name, "ucast_disassoc_on_bss_down", bsscfg->idx, 1);
		}
	}
	...
	if (!wet && !sta) { /* AP Mode(s) */
		WL_IOVAR_SETINT(name, "mpc", OFF);
	} else { /* STA Mode(s) */
		val = strtol(nvram_safe_get(strcat_r(prefix, "sta_mpc_disable", tmp)), NULL, 0);
		WL_IOVAR_SETINT(name, "mpc", (val ? OFF : ON));
	}
	...
	*(next = buf) = '\0';
	...
	if ((country && country[0] != '\0') && (country_rev && country_rev[0] != '\0')) {
		...
		/* Do nvram_commit to prevent losing nvrams after power-cycle */
		nvram_commit();
	}
	...
	/* Set dis_ch_grp_user */
	if ((str = nvram_get(strcat_r(prefix, "dis_ch_grp_user", tmp))) != NULL) {
		uint32 dis_ch_grp_user = (uint32)strtoul(str, NULL, 0);
		WL_IOVAR_SETINT(name, "dis_ch_grp_user", dis_ch_grp_user);
	}
	...
	if ((str = nvram_get(strcat_r(prefix, "chanspec", tmp))) != NULL) {
...
#ifdef CMWIFI_RDKB
		char* tmp_str = nvram_get("acsd2_started");
#endif // endif
		 ...
		 * ACSD will later select the best channel based on maximum available BW.
		 */
#ifdef CMWIFI_RDKB
		if (!strcmp(str, "0") &&
				((tmp_str == NULL) || !strcmp(tmp_str, "0"))) {
#else
		if (!strcmp(str, "0")) {
#endif // endif
			switch (bandtype) {
			...
			case WLC_BAND_5G:
				...
				/* else assume that chanspec is set to valid non-DFS */
				break;
			case WLC_BAND_6G:
				chanspec = WL_CHSPEC_DEF_6G;
				break;
			}
		}
	...
	if (cap_bgdfs160) {
		/* read from nvram only for 160 capable interfaces */
		if (bandtype != WLC_BAND_2G && wlconf_is_160(nvr_chanspec, nvr_bw_cap)) {
			...
			/* if nvram is not (properly) set, leave 160 capability to firmware
	...
	if ((chanspec == 0) && val && !WLCONF_PHYTYPE_11N(phytype)) {
		...
		uint channel;
		uint nctrlsb = 0;
		uint cspecband = BANDTYPE_CHSPEC(bandtype);
		...
		chanspec |= (cspecband | nbw | nctrlsb | channel);
		WL_IOVAR_SETINT(name, "chanspec", (uint32)chanspec);
	}
	...
	for (i = 0; i < bclist->count; i++) {
		...
		/* set APSD */
		str = nvram_safe_get(strcat_r(bsscfg->prefix, "wme_apsd", tmp));
		apsd = strcmp(str, "on") ? 0 : 1;
		WL_BSSIOVAR_SETINT(name, "wme_apsd", bsscfg->idx, apsd);
	}
	...
	*/
	...
	/* it is band-blind. try each band */
	error_bg = wl_iovar_setint(name, "2g_rate", val);
	error_a = wl_iovar_setint(name, "5g_rate", val);
	error_6g = wl_iovar_setint(name, "6g_rate", val);

	if (error_bg && error_a && error_6g) {
		/* none succeeded. Try default rate (card may have changed) */
		val = 0;
	...
	error_6g = wl_iovar_setint(name, "6g_mrate", val);
	...
	if (error_bg && error_a && error_6g) {
		...
		WL_IOVAR_SETINT(name, "6g_mrate", val);
	...
	if (WLCONF_PHYTYPE_11N(phytype)) {
		...
		if (str != NULL) {
			...
			if (vmcsidx >= 0 && vmcsidx <= 11) {
				...
				WL_IOVAR_SETINT(name, "6g_rate", (int)val);
			}
		}
	}
	...
	/* Set beacon loss timeout */
	str = nvram_safe_get(strcat_r(prefix, "txbcn_timeout", tmp));
	if (strlen(str)) {
		WL_IOVAR_SETINT(name, "txbcn_timeout", atoi(str));
	}
	...
	if (cap_11ax) {
		...
		if (str) {
			...
			WL_HEIOVAR_SETINT(name, "he", "enab", (val != 0));
		}

		str = nvram_safe_get(strcat_r(prefix, "umsched_brcm_only", tmp));
		if (str) {
			val = atoi(str);
			WL_MSCHED_IOVAR_SETINT(name, "umsched", "brcm_only", val);
		}

		str = nvram_safe_get(strcat_r(prefix, "twt", tmp));
		if (str && strlen(str)) {
			val = atoi(str);
			WL_TWTIOVAR_SETINT(name, "twt", "enab", val);
		}
	}
	...
	if (!strcmp(str, "auto") || !strcmp(str, "")) {
		...
		WL_IOVAR_SETINT(name, "sgi_tx", atoi(str));
	}
	...
	/* Set up TxBF */
	wlconf_set_txbf(name, prefix, bandtype, bclist);
	...
	/* set TAF UL */
	val = 0;
	str = nvram_get(strcat_r(prefix, "taf_ul", tmp));
	if (str) {
		val = atoi(str);
		wlconf_set_taf_ul(name, val);
	}

	/* configure MBO, MFP  */
	for (i = 0; i < bclist->count; i++) {
		bsscfg = &bclist->bsscfgs[i];
		wlconf_configure_mbo_oce(name, bsscfg->ifname, bsscfg->prefix, bsscfg->idx);
	}

	/* Configure upr/fd if this is not a pure STA */
	if (!sta) {
		wlconf_configure_upr_and_fd(name, prefix, bandtype, bclist);
	}
	...
	if (!((phytype == PHY_TYPE_LCN) || (phytype == PHY_TYPE_AC))) {
		/* Set antenna */
	...
	/* set pspretend for each bss */
	for (i = 0; i < bclist->count; i++) {
		set_psp = FALSE;
		val = 0;
		bsscfg = &bclist->bsscfgs[i];
		str = nvram_safe_get(strcat_r(bsscfg->prefix, "mode", tmp));
		bss_is_ap = (!strcmp(str, "") || !strcmp(str, "ap"));
		if (bss_is_ap) {
	 ...
	 * For a STA - Only in 2G.
	 */
	if (WLCONF_PHYTYPE_11N(phytype)) {
		if ((sta && (bandtype == WLC_BAND_2G)) ||
...
legacy_end:
	...
	if (apsta) {
		char tmp[100];
		char *nvram_keep_ap_up = NULL;
		int keep_ap_up;

		nvram_keep_ap_up = nvram_safe_get(strcat_r(prefix, "keep_ap_up", tmp));
		if (nvram_keep_ap_up && (nvram_keep_ap_up[0] != '\0')) {
			keep_ap_up = atoi(nvram_keep_ap_up);
			WL_IOVAR_SETINT(name, "keep_ap_up", keep_ap_up);
		}
	}
...
#ifdef MULTIAP
		strcat_r(bsscfg->prefix, "map", tmp);
		val = strtol(nvram_safe_get(tmp), NULL, 0);
		if (MAP_IS_AGENT(map_mode) && (val == 2) && !map_cert) {
			/* Disable the backhaul AP interface by default. WBD will enable it once
			 * it discovers the MAP controller
			 */
			disable_map_bh_bss(name, bsscfg->ifname, bsscfg->idx);
		}
#endif /* MULTIAP */
	}
...
wlconf_start(char *name)
{
	...
	if (is_dhd) {
		...
		if ((str = nvram_get(strcat_r(prefix, "dconpoll", tmp))) != NULL &&
				str[0] >= '0' && str[0] <= '9') {
			val = atoi(str);
			snprintf(tmp, sizeof(tmp), "dhd -i %s dconpoll %d", name, val);
			system(tmp);
		}
