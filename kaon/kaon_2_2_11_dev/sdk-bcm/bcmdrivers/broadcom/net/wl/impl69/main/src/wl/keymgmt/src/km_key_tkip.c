 ...
 * $Id: km_key_tkip.c 781451 2019-11-21 03:06:22Z $
 */
	...
	uint8 *body, int body_len, const key_pkt_info_t *pkt_info)
{
	...
	if (FC_TYPE(fc) == FC_TYPE_MNG) {
		...
		ins = PRIO2IVIDX(QOS_PRIO(pkt_info->qc));
	...
	uint8 *body, int body_len, d11txhdr_t *txd)
{
	...
	if (mic_len > 0) {
		...
		if (mic_len > 0) {
			...
			KM_ASSERT((int)PKTTAILROOM(KEY_OSH(key), mic_pkt) >= mic_len);
	...
	if (WLC_KEY_IN_HW(&key->info) && txd != NULL) {
		...
		if (KEY_COREREV_GE128(key)) {
			...
			if (hwmic) {
				...
				rev128_txh->MacControl_0 =
					htol16(ltoh16(rev128_txh->MacControl_0) |
					D11AC_TXC_AMIC);
			}
			goto done;
	...
	uint8 *body, int body_len, const key_pkt_info_t *pkt_info)
{
	...
	if (micl != rx_micl || micr != rx_micr) {
		err = BCME_MICERR;
		KEY_ERR(("wl%d: %s: TKIP MIC failure for frame from " MACDBG " to ...",
			KEY_WLUNIT(key), __FUNCTION__,
			MAC2STRDBG(hdr->ether_shost)));
		KEY_ERR(("..." MACDBG "key idx %d, expected %08x.%08x got %08x.%08x\n",
			MAC2STRDBG(hdr->ether_dhost),
	...
	if (err == BCME_OK) {
		...
		if (PKTISHDRCONVTD(KEY_OSH(key), pkt) &&
			((pkt_info->qc & QOS_AMSDU_MASK) ||
				D11REV_GE(KEY_PUB(key)->corerev, 130))) {
			...
			PKTFRAG_TRIM_TAILBYTES(KEY_OSH(key), pkt,
				TKIP_MIC_SIZE, TAIL_BYTES_TYPE_MIC);
#ifdef BCM43684_HDRCONVTD_ETHER_LEN_WAR
			/* Correct ether length */
			if (PKTISHDRCONVTD(key->wlc->osh, pkt)) {
				uint16 ether_type;

				ether_type = ntoh16(hdr->ether_type);
				if (ether_type < ETHER_TYPE_MIN) {
					uint32 low_addr = PKTFRAGDATA_LO(key->wlc->osh,
						pkt, LB_FRAG1) + ETHER_TYPE_1_OFFSET;

					ether_type -= TKIP_MIC_SIZE;
					ether_type = hton16(ether_type);

					wlc_bmac_write_host_buffer(key->wlc, low_addr,
						(uint8*)&ether_type, ETHER_TYPE_LEN);
				}
			}
#endif /* BCM43684_HDRCONVTD_ETHER_LEN_WAR */
		}
		...
		if (!WLC_KEY_USE_IVTW(&key->info)) {
			...
			KM_UPDATE_IVTW(KEY_KM(key), &key->info, rx_seq_id,
				tkip_key->rx_state.next_seq, TKIP_KEY_SEQ_SIZE, FALSE);
		}
	...
	KEY_LOG(("wl%d: %s: %s tkip mic check pkt@%p- key idx %d mic err status %d\n",
		KEY_WLUNIT(key), __FUNCTION__, ((pkt_info->flags & KEY_PKT_HWDEC) ? "hw" : "sw"),
