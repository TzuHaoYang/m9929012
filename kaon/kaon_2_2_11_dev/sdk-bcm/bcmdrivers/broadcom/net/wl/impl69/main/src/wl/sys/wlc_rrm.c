 ...
 *
 * $Id: wlc_rrm.c 794175 2020-12-23 17:00:47Z $
 */
 ...
 *
 * IPI: idle power indicator. A PHY indication of the total channel power (noise and interference)
 *      as measured in the channel at the receiving antenna connector while the STA is idle, i.e.,
 *      neither transmitting nor receiving a frame.
 * LCI: location configuration information: latitude, longitude, and altitude, with resolution
 *      indicators for each.
 */
...
/* RCPI macros */
#define MIN_OF_POW_RANGE_0_TO_109_5	-1095
#define POW_MIN_LIMIT			-110
#define RCPI_MEAS_NOT_AVAIL		255u
#define RCPI_MAX_VALUE			220u
#define RCPI_MIN_VALUE			0u

/* RSNI macros */
#define SNR_MIN_LIMIT			-10
#define SNR_MAX_LIMIT			117
#define RSNI_MIN_VAL			0u
#define RSNI_MAX_VAL			254u
...
enum {
	...
	IOV_RRM_LCI_REQ = 10,
	IOV_RRM_CIVIC_REQ = 11,			/**< Location Civic Measurement */
...
typedef struct rrm_nbr_rep {
	...
	uint8 *opt_lci;
	uint8 *opt_civic;			/**< Location Civic Measurement */
	uint8 *opt_locid;
} rrm_nbr_rep_t;
...
static int wlc_rrm_bcnreq_scancache(wlc_rrm_info_t *rrm_info, wlc_ssid_t *ssid,
...
#if defined(WL11K_ALL_MEAS) || defined(WL11K_BCN_MEAS) || (defined(WL11K_NBR_MEAS) && \
	defined(STA))
...
#endif /* WL11K_ALL_MEAS || WL11K_BCN_MEAS || WL11K_NBR_MEAS */
...
static int wlc_rrm_send_event(wlc_rrm_info_t *rrm_info, struct scb *scb, char *ie, int len,
...
#if ((defined(WL11K_AP) && defined(WL11k_NBR_MEAS)) && (BAND6G || defined(WL_OCE_AP)))
static int wlc_rrm_find_free_index(wlc_rnr_info_t *pnbr_info, uint8 size);
static int wlc_rrm_find_index(wlc_rnr_info_t *pnbr_info, uint8 chan, uint8 rclass, uint8 size);
static int wlc_rrm_add_tbtt_info_for_nbr(uint8 *pbuf, wlc_bsscfg_t *bsscfg,
	nbr_rpt_elem_t *nbr_elt);
#endif /* 11k_AP && 11k_NBR_MEAS && (BAND6G || OCE_AP) */
...
static void
wlc_rrm_parse_response(wlc_rrm_info_t *rrm_info, struct scb *scb, dot11_rm_ie_t *ie, int len)
{
	bool incap = FALSE;
	bool refused = FALSE;
...
wlc_rrm_ap_chreps_to_chanspec(bcm_tlv_t *tlvs, int tlvs_len, rrm_bcnreq_t *bcn_req)
{
	...
	return channel_count;
} /* wlc_rrm_ap_chreps_to_chanspec */

/** this node was requested to measure a beacon */
static void
	...
	wlc_rrm_req_t *req)
{
	...
	if (tlv_len > 0) {
		...
		tlv = bcm_find_tlv(tlvs, tlv_len, DOT11_RMREP_BCN_LAST_RPT_IND);
		if (tlv && tlv->len > 0) {
...
#ifdef WL11K_ALL_MEAS
/** this node was requested to measure a frame */
static void
...
void
wlc_rrm_stat_qos_counter(wlc_info_t *wlc, struct scb *scb, int tid, uint cnt_offset)
{
...
void
wlc_rrm_stat_chanwidthsw_counter(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	struct scb *scb;
...
} /* wlc_rrm_recv_statreq */

void
wlc_rrm_tscm_upd(wlc_info_t *wlc, struct scb *scb, int tid, uint cnt_offset, uint cnt_val)
{
	rrm_tscm_t *tscm;
	uint32 *counter;
	scb_tscm_t *scb_tscm;
...
void
wlc_rrm_delay_upd(wlc_info_t *wlc, struct scb *scb, uint8 tid, uint32 delay)
{
	...
	int16 cat, int16 subevent)
{
	...
	buf = (char*)MALLOCZ(rrm_info->wlc->osh, WL_RRM_EVENT_BUF_MAX_SIZE);
	if (!buf) {
		return BCME_NOMEM;
	}

	evt = (wl_rrm_event_t*)buf;
	...
	wlc_bss_mac_event(rrm_info->wlc, scb->bsscfg, WLC_E_RRM,
		&scb->ea, 0, 0, 0, buf, WL_RRM_EVENT_BUF_MAX_SIZE);
	WL_RRM(("%s: <0x%x> trigger event [DONE]\n", __FUNCTION__, subevent));

	MFREE(rrm_info->wlc->osh, buf, WL_RRM_EVENT_BUF_MAX_SIZE);
...
wlc_rrm_recv_bcnrep(wlc_rrm_info_t *rrm_info, struct scb *scb, dot11_rm_ie_t *ie)
{
	...
	if (tlv->id == DOT11_RMREP_BCN_FRM_BODY && tlv_len >= (TLV_HDR_LEN + DOT11_BCN_PRB_LEN)) {
		...
		tlv = bcm_find_tlv(tlvs, tlv_len, DOT11_MNG_SSID_ID);
		if (tlv) {
			report_ssid.SSID_len = MIN(DOT11_MAX_SSID_LEN, tlv->len);
			bcopy(tlv->data, report_ssid.SSID, report_ssid.SSID_len);
		}
		htie = bcm_find_tlv(tlvs, tlv_len, DOT11_MNG_HT_CAP);
		mdie = bcm_find_tlv(tlvs, tlv_len, DOT11_MNG_MDIE_ID);
		wpa2ie = bcm_find_tlv(tlvs, tlv_len, DOT11_MNG_RSN_ID);
...
wlc_rrm_begin_scan(wlc_rrm_info_t *rrm_info)
{
	...
	if (bcn_req->scan_type == DOT11_RMREQ_BCN_TABLE) {
		...
		ret = wlc_scan_request(wlc, DOT11_BSSTYPE_ANY, &bcn_req->bssid,
			1, &bcn_req->ssid, bcn_req->scan_type, 1, bcn_req->duration,
			bcn_req->duration, -1, bcn_req->chanspec_list, bcn_req->channel_num, 0,
			TRUE, wlc_rrm_bcnreq_scancb, wlc, WLC_ACTION_SCAN, FALSE, NULL, NULL, NULL);
...
wlc_rrm_get_ap_chrep(const wlc_bss_info_t *bi)
{
	...
	if (bi->bcn_prb && (bi->bcn_prb_len >= DOT11_BCN_PRB_LEN)) {
		...
		ap_chrep = (dot11_ap_chrep_t*)bcm_find_tlv((uint8 *)tlv, bi->bcn_prb_len -
...
static void
wlc_rrm_write_last_bcn_rpt_ind(wlc_rrm_info_t *rrm_info, wlc_bss_info_t *bi,
	uint8 *bufptr)
{
	...
	bss_count = rrm_info->bcnreq->scan_results.count;
	...
	if ((i+1 == bss_count) &&
		...
		last_rpt = 1;
		WL_INFORM(("This is last bcn rpt!!\n"));
	}
	last_rpt_ind->data = last_rpt;
}

/* IEEE spec: 9.4.2.37 RCPI element
 * Table 9-169
 * 0 Represents P < - 109.5 dBm
 * 1 - 219 represents Power levels in the range -109.5 <= P < 0
 * derived by RCPI =  (2 * (P + 110))
 * 220 Represents P >=0 dBm
 */
/* RCPI value normalized by multiplied by 10 */
static uint8
wlc_rrm_get_rcpi_value(wlc_rrm_info_t *rrm_info, int16 p)
{
	uint8 rcpi = RCPI_MEAS_NOT_AVAIL;
	/* multiply by 10 to normalize */
	int16 p_adj = p * 10;
	int16 rcpi_tmp = 0;
	if (p_adj < MIN_OF_POW_RANGE_0_TO_109_5) {
		rcpi = RCPI_MIN_VALUE;
	} else if (p_adj >= 0) {
		rcpi = RCPI_MAX_VALUE;
	} else {
		/* we are here means value of 'p' is restricted
		 * within -109.5 <= p < 0. So, we should not
		 * see any overflow(uint8) and value of below equation
		 * will be within 1 - 219.
		 */
		rcpi_tmp = 2 * (p + (-1 * POW_MIN_LIMIT));
		rcpi = (uint8)rcpi_tmp;
	}
	WL_RRM(("Pow:%d RCPI:%u\n", p, rcpi));
	return rcpi;
}
/* IEEE spec: RSNI element
 * RSNI = (10 * log10((RCPIpower - ANPIpower) / ANPIpower) + 10) * 2
 * SNR_db = 10 * log10((RCPIpower - ANPIpower) / ANPIpower)
 * So, RSNI = (SNR_db + 10) * 2
 * If, SNR <= -10, cap RSNI to 0
 * If, SNR > 117, cap RSNI to 254
 */
static uint8
wlc_rrm_get_rsni_value(wlc_rrm_info_t *rrm_info, int16 snr)
{
	int16 rsni;
	if (snr <= SNR_MIN_LIMIT) {
		rsni = RSNI_MIN_VAL;
	} else if (snr > SNR_MAX_LIMIT) {
		rsni = RSNI_MAX_VAL;
	} else {
		/* we are here means value of 'snr' is restricted
		* within -10 < p < 117. So, we should not
		* see any overflow(uint8) and value of below equation
		* will be within 1 - 254.
		*/
		rsni = ((snr + (-1 * SNR_MIN_LIMIT)) * 2);
	}
	WL_RRM(("SNR:%d RSNI:%u\n", snr, rsni));
	/* RSNI value is restricted to 0 to 254 */
	return (uint8)rsni;
}
...
wlc_rrm_bcnrep_add(wlc_rrm_info_t *rrm_info, wlc_bss_info_t *bi, uint8 *bufptr, uint buflen)
{
	...
	if (bufptr) {
		...
		rmrep_bcn->rcpi = wlc_rrm_get_rcpi_value(rrm_info, bi->RSSI);
		rmrep_bcn->rsni = wlc_rrm_get_rsni_value(rrm_info, bi->SNR);
	...
	if (rrm_info->bcnreq->rep_detail
		...
		if (bufptr) {
			...
			bufptr += TLV_HDR_LEN + DOT11_BCN_PRB_FIXED_LEN;
		}
	...
	/* Add Last Beacon Report Indication subelement */
	if (rrm_info->bcnreq->last_bcn_rpt_ind_req) {
		...
		if (bufptr) {
			...
			wlc_rrm_write_last_bcn_rpt_ind(rrm_info, bi, bufptr);
...
{
	wlc_info_t *wlc = rrm_info->wlc;
	wlc_bss_list_t *bss_list = NULL;
#ifdef BCMDBG
	char eabuf[ETHER_ADDR_STR_LEN];
#endif /* BCMDBG */
	ASSERT(rrm_info->bcnreq->channel_num);

	bss_list = &(rrm_info->bcnreq->scan_results);
...
wlc_rrm_add_ap_chrep(wlc_rrm_info_t *rrm, wlc_bsscfg_t *cfg, uint8 *bufstart)
{
	...
	while (nbr_cnt) {
		...
		pbuf += TLV_HDR_LEN + 1 + chan_count;
	}
} /* wlc_rrm_add_ap_chrep */
...
#endif /* WL11K_ALL_MEAS || WL11K_BCN_MEAS || (WL11K_NBR_MEAS && STA) */
...
static void wlc_rrm_send_txstrmrep(wlc_rrm_info_t *rrm_info)
{
	...
	scb = wlc_scbfind(wlc, rrm_info->cur_cfg, &rrm_info->da);
	wlc_sendmgmt(wlc, p, rrm_info->cur_cfg->wlcif->qi, scb);
} /* wlc_rrm_send_txstrmrep */

static void
wlc_rrm_send_statrep(wlc_rrm_info_t *rrm_info)
{
	wlc_info_t *wlc = rrm_info->wlc;
	dot11_rm_action_t *rm_rep;
	dot11_rm_ie_t *rmrep_ie;
...
wlc_rrm_framerep_add(wlc_rrm_info_t *rrm_info, uint8 *bufptr, uint cnt)
{
	...
	WL_RRM(("%s: rssi=%d, snr=%d,noise_avg=%d, lq_noise=%d, "
		"phytxant=%d, txant=%d\n",
		__FUNCTION__, bsscfg->link->rssi, wlc->primary_bsscfg->link->snr,
...
/**
 * STA specific functionality.
 * FIXME: PR81518: need a wlc_bsscfg_t param to correctly determine addresses and queue for
 * wlc_sendmgmt()
 */
static void
...
#ifdef WLASSOC_NBR_REQ
/** STA specific functionality. Called from sta assocreq_done function via scb state notif */
static void
...
/** AP specific functionality. Send Neighbor Report Response */
static void
	...
	rrm_bsscfg_cubby_t *rrm_cfg)
{
	...
	for (i = 0; i < list_cnt; i++) {
		...
		nbr_elt->bss_params = nbr_rep->nbr_elt.bss_params;
		nbr_elt->tpe_psd = nbr_rep->nbr_elt.tpe_psd;
...
} /* wlc_rrm_remove_regclass_nbr_cnt */

#if BAND6G || defined(WL_OCE_AP)
/** AP specific functionality */
static int
wlc_rrm_find_free_index(wlc_rnr_info_t *pnbr_info, uint8 size)
{
	uint8 index = 0;
	for (; index < size; index++) {
		if ((pnbr_info[index].chan == 0) &&
			(pnbr_info[index].rclass == 0)) {
			return index;
		}
	}
	return -1;
...
/** AP specific functionality */
static int
wlc_rrm_find_index(wlc_rnr_info_t *pnbr_info, uint8 chan, uint8 rclass, uint8 size)
{
	uint8 index = 0;
	for (; index < size; index++) {
		if ((pnbr_info[index].chan == chan) &&
			(pnbr_info[index].rclass == rclass)) {
			return index;
		}
	}
	return -1;
...
/** AP specific functionality */
static int
wlc_rrm_add_tbtt_info_for_nbr(uint8 *pbuf, wlc_bsscfg_t *bsscfg, nbr_rpt_elem_t *nbr_elt)
{
	...
	memcpy(tbtt_info.bssid, &nbr_elt->bssid, ETHER_ADDR_LEN);

	if (nbr_elt->ssid.SSID[0] != '\0') {
		tbtt_info.short_ssid = wlc_calc_short_ssid(nbr_elt->ssid.SSID,
		                                           nbr_elt->ssid.SSID_len);
		htol32_ua_store(tbtt_info.short_ssid, &tbtt_info.short_ssid);
#if defined(WL_OCE) && defined(WL_OCE_AP)
		wlc_oce_set_sssid_for_rnr_completeness_attr(bsscfg, tbtt_info.short_ssid);
#endif /* WL_OCE && WL_OCE_AP */
	}
	tbtt_info.bss_params = nbr_elt->bss_params;
	tbtt_info.tpe_psd = nbr_elt->tpe_psd;

	memcpy(pbuf, (uint8*)&tbtt_info,
		(nbr_elt->tbtt_len != 0 ? nbr_elt->tbtt_len: TBTT_INFO_FIELD_HDR_LEN));
	return BCME_OK;
}

/** AP specific functionality */
int
wlc_rrm_fill_static_rnr_info(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 *pibuf,
	uint8 *len_copied)
{
	wlc_rnr_info_t *rnr_info;
	rrm_bsscfg_cubby_t *rrm_cfg;
	rrm_nbr_rep_t *nbr_rep;
	int ret = BCME_OK;
	uint8 *pnbr_ap_info, *ptraverse;
	uint8 nbr_count, chan, rclass, nbytes_rd, tbtt_len = 0;
	int8 index;
	...
	if (!nbr_count) {
		WL_INFORM(("wl%d: No Neighbors present to prepare RNR IE\n",
			wlc->pub->unit));
		return BCME_OK;
	}
	WL_INFORM(("wl%d: [%d] Neighbor in static RRM list, Prepare RNR IE with this info\n",
		wlc->pub->unit, nbr_count));

	/* allocate memory for holding rclass and chan info */
	rnr_info = (wlc_rnr_info_t*)MALLOCZ(wlc->osh, nbr_count * sizeof(wlc_rnr_info_t));
	if (rnr_info == NULL) {
	...
	for (index = 0; index < nbr_count; index++) {
		rnr_info[index].pbuf = (uint8*)MALLOCZ(wlc->osh,
				(WL_RNR_INFO_MAX_NBR_PER_OPCLASS_CHAN * TBTT_INFO_FIELD_HDR_LEN));

		if (!rnr_info[index].pbuf) {
			WL_ERROR(("wl%d : Malloc failed, return\n",
				wlc->pub->unit));
			return BCME_NOMEM;
		}
		rnr_info[index].max_count = WL_RNR_INFO_MAX_NBR_PER_OPCLASS_CHAN;
	}
	index = 0;

	/* Debug code only */
	WL_INFORM(("wl%d: rnr buffer befor processing static RRM nbr list:\n", wlc->pub->unit));
	for (index = 0; index < nbr_count; index++) {
		WL_INFORM(("wl%d: rnr index[%d] info: chan[%d] rclass[%d] count[%d] pbuf[%p]\n",
			wlc->pub->unit, index, rnr_info[index].chan, rnr_info[index].rclass,
			rnr_info[index].count, rnr_info[index].pbuf));
	}
	/* debug code finish */

	nbr_rep = rrm_cfg->nbr_rep_head;
	while (nbr_rep) {
		/* search for rclass and chan */
		chan = nbr_rep->nbr_elt.channel;
		rclass = nbr_rep->nbr_elt.reg;
		tbtt_len = nbr_rep->nbr_elt.tbtt_len;
		index = wlc_rrm_find_index(rnr_info, chan, rclass, nbr_count);

		if (index == -1) {
			index = wlc_rrm_find_free_index(rnr_info, nbr_count);

			if (index == -1) {
				/* fail condition ...return ? */
				WL_ERROR(("wl%d:%s: No free slots available for NBR ["MACF"]"
					" to add in RNR IE \n", wlc->pub->unit, __FUNCTION__,
					ETHERP_TO_MACF(&nbr_rep->nbr_elt.bssid)));

				ret = BCME_NORESOURCE;
				goto finish;
			}
			rnr_info[index].rclass = rclass;
			rnr_info[index].chan = chan;
			rnr_info[index].tbtt_len =
				(tbtt_len != 0 ? tbtt_len : TBTT_INFO_FIELD_HDR_LEN);

			wlc_rrm_add_tbtt_info_for_nbr(rnr_info[index].pbuf, bsscfg,
				&nbr_rep->nbr_elt);

			rnr_info[index].count++;
		} else {
			if (rnr_info[index].count == rnr_info[index].max_count) {
				/* fail condition ...return ? */
				WL_ERROR(("wl%d:%s: No space available for NBR ["MACF"]"
					" to add in RNR IE, skip this \n", wlc->pub->unit,
					__FUNCTION__, ETHERP_TO_MACF(&nbr_rep->nbr_elt.bssid)));
				nbr_rep = nbr_rep->next;
				continue;
			}
			ptraverse = rnr_info[index].pbuf;
			ptraverse += rnr_info[index].count *
				(tbtt_len != 0 ? tbtt_len : TBTT_INFO_FIELD_HDR_LEN);
			wlc_rrm_add_tbtt_info_for_nbr(ptraverse, bsscfg, &nbr_rep->nbr_elt);
	...
	/* Debug code only */
	WL_INFORM(("wl%d: rnr buffer after populating static RRM nbr list\n", wlc->pub->unit));
	for (index = 0; index < nbr_count; index++) {
		WL_INFORM(("wl%d: rnr index[%d] info: chan[%d] rclass[%d] count[%d] pbuf[%p]\n",
			wlc->pub->unit, index, rnr_info[index].chan, rnr_info[index].rclass,
			rnr_info[index].count, rnr_info[index].pbuf));
	...
	/* Neigbor AP info field is of type:
	 * --------------------------------------------------------------------
	 * | TBTT info HDR | operating class | channel | TBTT information set |
	 * |   2 BYTE      |  1 BYTE         | 1 BYTE  |  <variable>          |
	 * --------------------------------------------------------------------
	 *  TBTT information set:
	 *  ----------------------------------------------------------------
	 *  |TBTT info	| filtered | co-located| TBTT info | TBTT info len |
	 *  | field type|  NBR AP  |   AP      |  count    |   <8 bit>     |
	...
	for (index = 0; index < nbr_count; index++) {
		nbytes_rd = 0;
		if (*len_copied >= 255) {
			/* not allowed, anymore. return */
			return BCME_NORESOURCE;
		}
		if (rnr_info[index].count == 0) {
			/* no need to add in NBR AP info field */
			continue;
		}
		wlc_write_neighbor_ap_info_field(pnbr_ap_info, &rnr_info[index], &nbytes_rd);

		pnbr_ap_info += nbytes_rd;
		*len_copied += nbytes_rd;
	}

	WL_INFORM(("wl%d: after copying to NBR AP buffer:\n", wlc->pub->unit));
	for (index = 0; index < nbr_count; index++) {
		WL_INFORM(("wl%d: rnr index[%d] info: chan[%d] rclass[%d] count[%d] pbuf[%p]\n",
			wlc->pub->unit, index, rnr_info[index].chan, rnr_info[index].rclass,
			rnr_info[index].count, rnr_info[index].pbuf));
...
finish:
	/* Release all memory */
	for (index = 0; index < nbr_count; index++) {
		MFREE(wlc->osh, rnr_info[index].pbuf,
			(WL_RNR_INFO_MAX_NBR_PER_OPCLASS_CHAN * TBTT_INFO_FIELD_HDR_LEN));
		rnr_info[index].pbuf = NULL;
	}

	MFREE(wlc->osh, rnr_info, nbr_count * sizeof(wlc_rnr_info_t));

	return ret;
} /* wlc_rrm_fill_static_rnr_info */

/** AP specific functionality */
int
wlc_write_neighbor_ap_info_field(uint8 *pbuffer, wlc_rnr_info_t *nbr_rnr_info, uint8 *len_copied)
{
	neighbor_ap_info_field_t info;
	uint16 tbtt_info_hdr = 0;

	info.channel = nbr_rnr_info->chan;
	info.op_class = nbr_rnr_info->rclass;

	TBTT_INFO_HDR_SET_LENGTH(tbtt_info_hdr, nbr_rnr_info->tbtt_len);

	if (FALSE /* todo: need to find filter_neighbor_ap */) {
		TBTT_INFO_HDR_SET_FN_AP(tbtt_info_hdr, 1);
	}

	TBTT_INFO_HDR_SET_COUNT(tbtt_info_hdr, (nbr_rnr_info->count - 1));
	htol16_ua_store(tbtt_info_hdr, &info.tbtt_info_header);
	memcpy(pbuffer, (uint8*)&info, NEIGHBOR_AP_INFO_FIELD_HDR_LEN);
	pbuffer += NEIGHBOR_AP_INFO_FIELD_HDR_LEN;
	memcpy(pbuffer, nbr_rnr_info->pbuf, nbr_rnr_info->count * (nbr_rnr_info->tbtt_len));

	pbuffer += nbr_rnr_info->count * (nbr_rnr_info->tbtt_len);
	*len_copied = (nbr_rnr_info->count * (nbr_rnr_info->tbtt_len)) +
			NEIGHBOR_AP_INFO_FIELD_HDR_LEN;
	...
	rrm_bsscfg_cubby_t *rrm_cfg, int addtype)
{
	...
	if (addtype == NBR_ADD_STATIC) {
		...
		if (should_update_beacon) {
			...
			if (bsscfg->up &&
				(BSSCFG_AP(bsscfg) ||
					(BSSCFG_IBSS(bsscfg) && !BSS_TDLS_ENAB(wlc, bsscfg)))) {
...
/** AP specific functionality. Location Civic. */
int
...
} /* wlc_rrm_add_regclass_neighbor_count */
...
/** AP specific functionality. Called from ap assocreq_done function via scb state notif */
static void
	...
	rrm_nbr_rep_t *nbr_rep, int *buflen, uint8 flags)
{
	...
	*buflen -= DOT11_RM_IE_LEN;
	...
	return ptr;
} /* wlc_rrm_create_lci_meas_element */
	...
	rrm_nbr_rep_t *nbr_rep, int *buflen, uint8 flags)
{
	...
	*buflen -= DOT11_RM_IE_LEN;
	...
	return ptr;
} /* wlc_rrm_create_civic_meas_element */
	...
	uint8 *data_ptr, int *buflen)
{
	...
	*buflen -= DOT11_LCI_COLOCATED_BSSID_LIST_FIXED_LEN;
	...
	return data_ptr;
} /* wlc_rrm_create_colocatedbssid_ie */
        ...
        rrm_nbr_rep_t *nbr_rep, wlc_bsscfg_t *cfg)
{
	while (mr_tlvs) {
		if (mr_tlvs->id == DOT11_MNG_MEASURE_REPORT_ID) {
			...
			if ((op_p = wlc_vht_copy_op_ie(vhti, (uint8 *)mr_tlvs,
				...
				if (bw == WL_CHANSPEC_BW_8080) {
					...
					nbr_rep->nbr_elt.chanspec =
						wf_channel2chspec(primary_channel, bw,
							WL_CHANNEL_2G5G_BAND(primary_channel));
				}
			}
			else {
				WL_ERROR(("wl%d: %s: Error parsing wlc_vht_copy_op_ie id: %d, "
					"len %d\n", wlc->pub->unit, __FUNCTION__,
					mr_tlvs->id, mr_tlvs->len));
			}
		}
	...
	dot11_rmreq_ftm_range_t *rmreq_ftmrange, wlc_rrm_req_t *req)
{
	...
	if (rmreq_ftmrange->len <= DOT11_MNG_IE_MREQ_FRNG_FIXED_LEN) {
		WL_ERROR(("wl%d: %s: ftmrange len is too short \n", WLCWLUNIT(wlc),
			__FUNCTION__));
		MFREE(wlc->osh, rrm_info->rrm_frng_req->frng_req, (sizeof(frngreq_t) +
			((DOT11_FTM_RANGE_ENTRY_MAX_COUNT-1) *
			sizeof(frngreq_target_t))));
		rrm_info->rrm_frng_req->frng_req = NULL;
	...
	dot11_rmrep_ftm_range_t *rmrep_ftmrange, int len)
{
	...
	wlc_bss_mac_event(wlc, scb->bsscfg, WLC_E_RRM, &scb->ea,
...
wlc_rrm_create_frngrep_element(uint8 **ptr, rrm_bsscfg_cubby_t *rrm_cfg, frngrep_t *frngrep)
{
	...
	*ptr += sizeof(frngrep->error_entry_count);
	...
	return copy_len;
} /* wlc_rrm_create_frngrep_element */
...
/**
 * STA specific functionality. Configure the roam channel cache manually, for instance if you have
...
wlc_set_roam_channel_cache(wlc_bsscfg_t *cfg, const chanspec_t *chanspecs, int num_chanspecs)
{
	...
	/* Neighbor list from AP may contains more channels than necessary:
	 * Take the first wlc->roam_chn_cache_limit distinctive channels.
	 * (prioritize 5G over 2G/6G channels, and then 6G over 2G channels)
	 */
	for (j = 0; j < MAXBANDS; j++) {
		int i;
		const enum wlc_bandunit order[MAXBANDS] =
			{BAND_5G_INDEX, BAND_6G_INDEX, BAND_2G_INDEX};
		enum wlc_bandunit bu = order[j]; /* chspecs matching this band are processed */
		uint8 channel;
		chanvec_t *chanvec;

		for (i = 0; (i < num_chanspecs) && (num_channel < roam->roam_chn_cache_limit); i++)
		{
			chanspec_t chanspec = chanspecs[i];
			channel = wf_chspec_ctlchan(chanspec);
			chanvec = GET_CHANVEC(&roam->roam_chn_cache, bu);

			if (CHSPEC_BANDUNIT(chanspec) == bu && isclr(chanvec->vec, channel)) {
				WL_SRSCAN(("%u", channel));
				WL_ASSOC(("%u\n", channel));
				setbit(chanvec->vec, channel);
				num_channel++;
			}
		}
	}
	...
	uint8 *body, int body_len)
{
	...
	while (tlvs && bcm_valid_tlv(tlvs, tlv_len) && tlvs->id == DOT11_MNG_NEIGHBOR_REP_ID) {
		...
		pref = (dot11_ngbr_bsstrans_pref_se_t*) bcm_find_tlv(nbr_rep_ie->data,
        ...
        void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_RRM_NBR_ADD_NBR):
	{
		...
		wlc_ap_update_static_rnr_nbr_info(wlc, cfg);
		break;
	}
	...
	int body_len, int8 rssi, ratespec_t rspec)
{
	...
	lmrep->rcpi = wlc_rrm_get_rcpi_value(rrm_info, rssi);
...
wlc_rrm_abort(wlc_info_t *wlc)
{
	...
	(phy_utils_get_chanspec(WLC_PI(wlc)) != rrm_state->chanspec_return)) {
...
wlc_rrm_report_11k(wlc_rrm_info_t *rrm_info, wlc_rrm_req_t *req_block, int count)
{
	...
	for (i = 0; i < count; i++) {
		...
		case DOT11_RMREQ_BCN_PASSIVE:
			...
			if (status != WLC_E_STATUS_SUCCESS) {
				WL_ERROR(("%s: scan status != success\n", __FUNCTION__));
				wlc_rrm_rep_err(rrm_info, DOT11_MEASURE_TYPE_BEACON,
					req->token, DOT11_RMREP_MODE_REFUSED);
				break;
			}
...
wlc_rrm_begin(wlc_rrm_info_t *rrm_info)
{
	...
	chanspec_t radio_chspec = phy_utils_get_chanspec(wlc->pi);
	...
	WL_INFORM(("rrm_chanspec[%x] radio_chanspec[%x]", rrm_chanspec, radio_chspec));

	WL_RRM(("%s: <Enter> rrm_chanspec %d\n", __FUNCTION__, rrm_chanspec));

	/* check for a channel change */
	if (!blocked && rrm_chanspec != 0 &&
	    !CHSPEC_PRIM20_EQUAL(rrm_chanspec, radio_chspec)) {
		...
		if (CHSPEC_CHANNEL(rrm_chanspec) != 0 &&
			...
			rrm_state->chanspec_return = radio_chspec;
			/* skip CFP & TSF update */
		...
		if (rrm_chanspec != 0 &&
		    (wf_chspec_ctlchan(rrm_chanspec) !=
		     wf_chspec_ctlchan(phy_utils_get_chanspec(WLC_PI(wlc)))))
...
wlc_rrm_next_set(wlc_rrm_info_t *rrm_info)
{
	...
	if (wlc->pub->associated && (chanspec != 0) &&
		!CHSPEC_PRIM20_EQUAL(chanspec, wlc->home_chanspec)) {
		if (rrm_state->chanspec_return != 0)
...
#ifdef STA
/** STA specific functionality. Return TRUE if wlc_bss_info_t contains RRM IE, else FALSE */
static bool
wlc_rrm_is_rrm_ie(wlc_bss_info_t *bi)
{
	...
	if (bcm_find_tlv(bcn_parse, bcn_parse_len, DOT11_MNG_RRM_CAP_ID))
		return TRUE;

	return FALSE;
}

/** STA specific functionality. RRM Cap */
static uint
...
bool
wlc_rrm_in_progress(wlc_info_t *wlc)
{
	return WLC_RRM_IN_PROGRESS(wlc);
}
...
static int
wlc_rrm_scb_init(void *ctx, struct scb *scb)
{
	wlc_rrm_info_t *rrm_info = (wlc_rrm_info_t *)ctx;
	wlc_info_t *wlc = rrm_info->wlc;
...
static void
wlc_rrm_scb_deinit(void *ctx, struct scb *scb)
{
	wlc_rrm_info_t *rrm_info = (wlc_rrm_info_t *)ctx;
	wlc_info_t *wlc = rrm_info->wlc;
	rrm_scb_cubby_t *scb_cubby = RRM_SCB_CUBBY(rrm_info, scb);
...
static void
wlc_rrm_scb_dump(void *ctx, struct scb *scb, struct bcmstrbuf *b)
{
	wlc_rrm_info_t *rrm_info = (wlc_rrm_info_t *)ctx;
	rrm_scb_info_t *scb_info = RRM_SCB_INFO(rrm_info, scb);
	rrm_nbr_rep_t *nbr_rep;
...
wlc_rrm_get_nbr_report(wlc_info_t* wlc, wlc_bsscfg_t *bsscfg, uint16 cnt, void* buf)
{
	...
	for (i = 0; i < cnt; i++) {
		...
		memcpy(nbrrep, &nbr_rep->nbr_elt.id,
			(TLV_HDR_LEN + DOT11_NEIGHBOR_REP_IE_FIXED_LEN));
...
#if BAND6G || defined(WL_OCE_AP)
/**
 * A colocated AP with a 6Ghz unit should also respond to a probe request that was received on the
 * 2G or 5G band that contains the same ssid as emitted by the 6G band unit.
 *
 * @return  TRUE if the caller-supplied short-ssid matches one of the elements in the list
 *          of neighbor APs
 */
bool
wlc_rrm_ssid_is_in_nbr_list(wlc_bsscfg_t *cfg, wlc_ssid_t *ssid, he_short_ssid_list_ie_t *s_ssid)
{
	rrm_bsscfg_cubby_t *rrm_cfg;
	rrm_nbr_rep_t *nbr_rep;
	int i;
	...
	if ((cfg->wlc->pub->_rrm != TRUE) || !(cfg->current_bss->capability & DOT11_CAP_RRM)) {
		return FALSE;
	}

	rrm_cfg = RRM_BSSCFG_CUBBY(cfg->wlc->rrm_info, cfg);

	nbr_rep = RRM_NBR_REP_HEAD(rrm_cfg, NBR_ADD_STATIC);

	if (cfg->wlc->rnr_update_method != WLC_RNR_UPDATE_METHOD_STATIC) {
		return FALSE;
	}

	while (nbr_rep != NULL) {
		wlc_ssid_t *nbr = &nbr_rep->nbr_elt.ssid;
		uint32 nbr_short_ssid =
			~hndcrc32(nbr->SSID, nbr->SSID_len, CRC32_INIT_VALUE);

		if (ssid->SSID_len > 0) {
			if (ssid->SSID_len == nbr->SSID_len &&
				bcmp(ssid->SSID, nbr->SSID, ssid->SSID_len) == 0) {
				WL_INFORM(("wl%d: ssid match[%s] in nbr list\n",
					cfg->wlc->pub->unit, nbr->SSID));
				return TRUE;
			}
		}
		if (s_ssid == NULL) {
			/* skip short ssid parsing */
			nbr_rep = nbr_rep->next;
			continue;
		}
		if (cfg->wlc->short_ssid_list != TRUE) {
			/* skip short ssid parsing */
			nbr_rep = nbr_rep->next;
			continue;
		}
		...
		for (i = 0; i < s_ssid->len / sizeof(s_ssid->short_ssid[0]); i++) {
			if (s_ssid->short_ssid[i] == nbr_short_ssid) {
				WL_INFORM(("wl%d: short ssid match[%x] in nbr list\n",
					cfg->wlc->pub->unit, nbr_short_ssid));
				return TRUE;
			}
		}
