 ...
 *
 *	$Id: acs_scan.c 793530 2020-12-01 08:15:57Z $
 */
...
acs_scan_prep(acs_chaninfo_t *c_info, void *params, int *params_size, uint16 version)
{
	...
	params_v1->passive_time = ACS_CS_SCAN_DWELL;
...
int acs_run_normal_ci_scan(acs_chaninfo_t *c_info)
{
	...
	params_v1->nprobes = -1;
	params_v1->active_time = ACS_CI_SCAN_DWELL;
	params_v1->passive_time = ACS_CI_SCAN_DWELL;
	params_v1->home_time = -1;
	params_v1->channel_num = 1; /* 1 channel for each ci scan */
...
acs_run_escan(acs_chaninfo_t *c_info, uint8 scan_type)
{
	...
	if (scan_type == ACS_SCAN_TYPE_CS) {
		...
		if (BAND_6G(c_info->rs_info.band_type)) {
			escan_timeout = time(NULL) + WL_CS_SCAN_TIMEOUT_6G;
		} else {
	...
	srand((unsigned)time(NULL));
	((wl_escan_params_v1_t *)params)->sync_id = htod16(random() & 0xffff);
...
acs_escan_prep_cs(acs_chaninfo_t *c_info, void *params, int *params_size, uint16 version)
{
	...
	params_v1->nprobes = -1;
	if (BAND_6G(c_info->rs_info.band_type)) {
		params_v1->active_time = ACS_CS_SCAN_DWELL_ACTIVE_6G;
		params_v1->passive_time = ACS_CS_SCAN_DWELL_PASSIVE_6G;
	} else {
		params_v1->active_time = ACS_CS_SCAN_DWELL_ACTIVE;
		params_v1->passive_time = ACS_CS_SCAN_DWELL;
	}
