 ...
 *
 * $Id: igmprt.c 710512 2017-07-13 03:54:07Z $
 */
...
char            upstream_interface[10][IFNAMSIZ+1];
// Keven Kuo
int             interface_lo = FALSE;
unsigned int    igmp_debug_level = IGMP_DBG_ERR;
...
igmp_info_print(igmp_router_t * router)
{
	...
	if (!(igmp_debug_level & IGMP_DBG_PRINT_TABLE))
		return;

	printf("#### IGMP Table ####\n");
	printf("%-14s %-9s %-14s %-5s %-14s %-14s\n", "interface", "version",
		   "groups", "mode", "source", "Members");
	for (ifp = router->igmprt_interfaces; ifp;
		 ifp = (igmp_interface_t *) ifp->igmpi_next) {
		printf("%-14s 0x%x\n", (char *) inet_ntoa(ifp->igmpi_addr.s_addr),
			   ifp->igmpi_version);
 ...
 */
igmp_interface_t *
igmp_interface_create(igmp_router_t *igmprt, struct in_addr ifaddr, char *ifname, int ifindex)
{
	igmp_interface_t *ifp;
	struct ip_mreq  mreq;
	u_char i;
	...
	IGMP_DBG("ifname=%s %s\n", ifname, (char *)inet_ntoa(ifaddr.s_addr));
	...
	ifp->igmpi_ifindex = ifindex;

	/* initial value , will be updated after getting an available number */
	ifp->igmpi_vifi = -1;
...
int igmp_interface_handle_upstream(igmp_router_t * router, igmp_interface_t *ifp,
	struct in_addr groupaddr)
{
	...
	IGMP_DBG("Upstream  interface : %s %s \n", ifp->igmpi_name, (char *) inet_ntoa(ifp->igmpi_addr.s_addr));
	IGMP_DBG("grp %s \n", (char *)inet_ntoa(groupaddr.s_addr));
	//printf("Adding membership on interface %s  iface_addr=%s for grp %s  \n", ifp->igmpi_name,
	//       (char *) inet_ntoa(ifp->igmpi_addr.s_addr), (char *)inet_ntoa(groupaddr.s_addr));
	...
	if (!VALID_ADDR(mreq.imr_multiaddr)) {
		IGMP_DBG("Skip for address %s\n", (char *)inet_ntoa(mreq.imr_multiaddr.s_addr));
		return 0;
	}
	/* only if the join is from downstream */
	...
	ret = setsockopt(router->igmprt_up_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP,
				 (void *) &mreq, sizeof(mreq));
	if (ret < 0) {
		IGMP_DBG("IP_ADD_MEMBERSHIP failed ret=%d \n", ret);
		/* perror("igmp_interface_group_add: setsockopt failed."); */
		syslog(LOG_NOTICE, "igmp:IP_ADD_MEMBERSHIP Failed\n");
		return -1;
	}
	IGMP_DBG("IP_ADD_MEMBERSHIP done for %s\n", (char *)inet_ntoa(mreq.imr_multiaddr.s_addr));
...
#else /* HND_FIX */
	sprintf(cmd, "iptables -D FORWARD -i %s -d %s -j DROP 2>/dev/null",
		ifp->igmpi_name, (char *) inet_ntoa(groupaddr));
	bcmSystemEx(cmd, 1);
	sprintf(cmd, "iptables -t filter -I FORWARD 1 -i %s -d %s -j ACCEPT 2>/dev/null",
		ifp->igmpi_name, (char *) inet_ntoa(groupaddr));
	bcmSystemEx(cmd, 1);
#endif /* HND_FIX */
	return 0;
...
igmp_interface_group_add(igmp_router_t * router, igmp_interface_t * ifp, struct in_addr groupaddr)
{
	...
	gp = igmp_group_create(groupaddr);
	if (!gp)
		return gp;
	...
	for (ifp = router->igmprt_interfaces; ifp; ifp = ifp->igmpi_next) {
		if (ifp->igmpi_type == UPSTREAM) {
			igmp_interface_handle_upstream(router, ifp, groupaddr);
		}
	}
	return gp;
}
...
#ifdef HND_FIX
				mcast_filter_del((unsigned char *)ifi->igmpi_name, report->igmpr_group);
#endif // endif
			}
		}
		if (!interface_lo) {
									 ...
									 igmpr_t * report, int len)
{
	 ...
	 */
	IGMP_DBG("igmp_interface_group_add %s\n", (char*)inet_ntoa(report->igmpr_group.s_addr));
...
igmp_interface_print(igmp_interface_t * ifp)
{
	...
	syslog(LOG_NOTICE, " interface %s, %s ver=0x%x name=%s ifindex=%d\n",
		   (char *) inet_ntoa(ifp->igmpi_addr),
		   (ifp->igmpi_type == UPSTREAM) ? "UPSTREAM" : "DOWNSTREAM",
		   ifp->igmpi_version, ifp->igmpi_name, ifp->igmpi_ifindex);
	for (gp = ifp->igmpi_groups; gp; gp = gp->igmpg_next)
		igmp_group_print(gp);
}
...
/*
 * interface_t* igmprt_interface_lookup_ifindex()
 *
 * Lookup a group, identified by the interface index
 */
igmp_interface_t *
igmprt_interface_lookup_ifindex(igmp_router_t * igmprt, int ifindex)
{
	igmp_interface_t *ifp;

	for (ifp = igmprt->igmprt_interfaces; ifp; ifp = ifp->igmpi_next) {
		if (ifp->igmpi_ifindex == ifindex)
			return ifp;
	}
	return NULL;
}
...
igmprt_interface_add(igmp_router_t * igmprt,
					 struct in_addr ifaddr, char *ifname, int ifindex)
{
	igmp_interface_t *ifp;
	 ...
	 */
	if ((ifp = igmp_interface_create(igmprt, ifaddr, ifname, ifindex))) {
...
igmprt_input(igmp_router_t * igmprt, igmp_interface_t * ifp)
{
	 ...
	 */
	ifpi = igmprt_interface_lookup_ifindex(igmprt, ifindex);
	if (ifpi == NULL)
		return;
	/*
	 * Handle the message
	...
	case IGMP_MEMBERSHIP_QUERY:
		...
		if (query->igmpq_code == 0) {
			...
			// receive_membership_query(ifpi,query->igmpq_group,query->igmpq_sources,
			// src.s_addr,query->igmpq_numsrc,srsp);
		}
		break;
...
#define IGMPMSG_NOCACHE  1
	case IGMPMSG_NOCACHE:
		...
		//       ifindex, iph->ip_dst.s_addr, iph->ip_src.s_addr,
		//       ifp->igmpi_ifindex);
...
#endif // endif
{
	...
	val = nvram_safe_get("igmp_debug_level");
	if (strcmp(val, ""))
		igmp_debug_level = strtoul(val, NULL, 0);
	...
	for (vifi = 0, ifp = ifl; ifp; ifp = ifp->ifl_next, vifi++) {
		...
		if (!(ifc = igmprt_interface_add(&router, psin->sin_addr, ifp->ifl_name,
							 ifp->ifl_ifindex)))
			continue;
		numifs++;
		if (!k_proxy_add_vif(((igmp_router_t *) & router)->igmprt_socket,
			psin->sin_addr.s_addr, &vifi))
			continue;
		ifc->igmpi_vifi = vifi;
		numvifs++;
	}
	...
	if (numifs != numvifs) {
		fprintf(stderr, "igmp: numifs %d and numvifs %d mismatch\n",
			numifs, numvifs);
		goto out;
	}
	/*
