 ...
 *
 * $Id: wbd_slave_com_hdlr.c 793717 2020-12-08 06:44:41Z $
 */
...
wbd_slave_steer_retry_timer_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	if (sta->status == WBD_STA_STATUS_NORMAL || sta->status == WBD_STA_STATUS_WEAK) {
		WBD_INFO("Slave["MACF"] STA["MACF"] Status[0x%x] is normal or weak. "
...
{
	int ret = WBDE_OK;
	bool scantype_onboot = FALSE, scanstate_running = FALSE, send_indep_scan = FALSE;
	...
	ieee1905_per_radio_opclass_list *indep_chscan_req;
	WBD_ENTER();
	...
	indep_chscan_req = &ifr_vndr_data->indep_chscan_req;
	...
	/* Fetch Independent Scan Reporting Policy Set by the Controller */
	send_indep_scan = I5_IS_CHSCAN_INDEP_SCAN_ENAB((&(policy_config->chscanrpt_config)));

	/* IF Channel Scan is not Triggered by Controller, Independent Scan Reporting is ON,
	  * And Independent Scan Request data not present, New Indep Scan just begun
	  */
	if (!scanstate_running && send_indep_scan &&
		ETHER_ISNULLADDR((struct ether_addr *)indep_chscan_req->radio_mac)) {

		/* Initialize Independent Chscan Request Obj for this Radio */
		memset(indep_chscan_req, 0, sizeof(*indep_chscan_req));
		ieee1905_glist_init(&indep_chscan_req->opclass_list);
		eacopy(evt_ifr->InterfaceId, indep_chscan_req->radio_mac);
	}
	...
	if (escan_event_status == WLC_E_STATUS_PARTIAL) {
		...
		blanket_get_global_rclass(CH20MHZ_CHSPEC2(bi->ctl_ch,
				CHSPEC_BAND(bi->chanspec)), &bi_opClass);

		WBD_DEBUG("ON ESCAN PARTIAL: Result ifname[%s], MAC["MACDBG"] BSSID["MACF"] "
		...
		if (!nbr_bss) {
			...
			WBD_DEBUG("ON ESCAN PARTIAL: New Neighbor BSS ifname[%s] IFR["MACDBG"] "
				"BSSID["MACF"] Appended\n", ifname,
				MAC2STRDBG(evt_ifr->InterfaceId), ETHER_TO_MACF(bi->BSSID));
		}
		...
		WBD_DEBUG("ON ESCAN PARTIAL: NBR_BSSID["MACF"] NBR_SSID[%s] NBR_RCPI[%d (%d)] "
			...
			chscan_result->scan_status_code, chscan_result->noise,
			chscan_result->utilization);

		/* Fill Independent Chscan Request Obj data for this Radio */
		if (!scanstate_running && send_indep_scan) {

			ieee1905_per_opclass_chan_list *opclass_info = NULL;
			int found_chan_idx = -1;

			/* Find the OpClass & Channel in Independent Scan Request of Radio */
			opclass_info = i5DmFindChannelInScanRequest(indep_chscan_req,
				bi_opClass, bi->ctl_ch, &found_chan_idx);

			if (opclass_info == NULL) {
				/* New Opclass Info. Allocate memory and save it */
				opclass_info = (ieee1905_per_opclass_chan_list *)
					wbd_malloc(sizeof(*opclass_info), &ret);

				ieee1905_glist_append(&indep_chscan_req->opclass_list,
					(dll_t*)opclass_info);

				opclass_info->opclass_val = bi_opClass;
				indep_chscan_req->num_of_opclass++;
				WBD_DEBUG("ON ESCAN PARTIAL: INDEP REQ : New OpClass[%d] Info "
					"ifname[%s] IFR["MACDBG"] BSSID["MACF"] Appended. "
					"Num_OpClass[%d]\n",
					bi_opClass, ifname, MAC2STRDBG(evt_ifr->InterfaceId),
					ETHER_TO_MACF(bi->BSSID),
					indep_chscan_req->num_of_opclass);
			}

			/* Update the OpClass Info fields, If Channel is New */
			if (found_chan_idx == -1) {
				opclass_info->chan_list[opclass_info->num_of_channels] =
					bi->ctl_ch;
				opclass_info->num_of_channels++;
				WBD_DEBUG("ON ESCAN PARTIAL: INDEP REQ : OpClass[%d] "
					"New Channel[%d] Num_Channels[%d]\n",
					bi_opClass, bi->ctl_ch, opclass_info->num_of_channels);
			}
		}
	...
	/* Escan finished : Success / Aborted */
		...
		/* If Channel Scan is not Triggered by SmartMesh Controller AND Independent Scan
		 * Reporting is Disabled by the Controller. Do not process further
		 */
		if (!scanstate_running && !send_indep_scan) {

			WBD_DEBUG("Channel scan is not Triggered by SmartMesh AND Independent Scan "
				"Reporting is OFF. Ifr Flag[%x] Policy[%x]\n", evt_ifr->flags,
				policy_config->chscanrpt_config.chscan_rpt_policy_flag);
			goto end;
		}

		WBD_INFO("ON ESCAN %s: [%s] : Ifanme[%s] Sync_id: %d, Status:%d. \n",
		...
		/* If Result is other than PARTIAL, and it was Requested (not OnBoot) Scan, and
		 * (Channel Scan was Triggered by SmartMesh OR Independent Scan Reporting is ON),
		 * Send Channel Scan Results to Controller
		 */
		if (!scantype_onboot) {

			/* Agent Sends Channel Scan Report to Controller Per Radio : Fresh/Indep */
			wbd_slave_send_chscan_report_per_radio(evt_ifr,
				(scanstate_running) ? (&ifr_vndr_data->last_fresh_chscan_req) :
				(&ifr_vndr_data->indep_chscan_req), 0);

			/* If Independent Scan Completed */
			if (!scanstate_running) {

				/* (RE)Initialize Independent Chscan Request Obj for this Radio */
				if (indep_chscan_req->num_of_opclass > 0) {
					i5DmGlistCleanup(&indep_chscan_req->opclass_list);
					indep_chscan_req->num_of_opclass = 0;
				}
				memset(indep_chscan_req, 0, sizeof(*indep_chscan_req));
				ieee1905_glist_init(&indep_chscan_req->opclass_list);
			}
	...
	switch (evt_type) {
		...
		case WLC_E_AP_CHAN_CHANGE:
		{
			...
			if ((evt_data->length != WL_CHAN_CHANGE_EVENT_LEN_VER_1) ||
				...
				WBD_ERROR("Band[%d] BSS["MACF"] Ifname[%s] WLC_E_AP_CHAN_CHANGE"
			...
			if ((reason == WL_CHAN_REASON_CSA) ||
				...
				/* Full CAC is handled in WLC_E_CAC_STATE_CHANGE event now */
				WBD_WARNING("CSA event [%d] should not come here\n", reason);
				break;
			}
			 ...
			 */
			WBD_INFO("AP_CHAN_CHANGE event: Band[%d] BSS["MACF"] Ifname[%s],"
				" Firmware reason[%d] current chanspec[0x%x], target"
				" chanspec[0x%x]\n", evt_ifr->band, ETHERP_TO_MACF(evt_bss->BSSID),
			...
			memcpy(&zwdfs_msg.mac, evt_bss->BSSID, ETHER_ADDR_LEN);
		...
		case WLC_E_CAC_STATE_CHANGE:
		{
			...
			if (sub0->state == WL_DFS_CACSTATE_PREISM_CAC) {
				...
				WBD_INFO("FULL Time CAC started on "MACDBG". event mask [0x%x]\n",
					MAC2STRDBG(evt_ifr->InterfaceId),
					ifr_vndr_data->dfs_event_mask[0]);

				if (isset(ifr_vndr_data->dfs_event_mask, WL_CHAN_REASON_CSA)) {
					clrbit(ifr_vndr_data->dfs_event_mask, WL_CHAN_REASON_CSA);
					break;
				}
				if (isset(ifr_vndr_data->dfs_event_mask,
					WL_CHAN_REASON_CSA_TO_DFS_CHAN_FOR_CAC_ONLY)) {
					clrbit(ifr_vndr_data->dfs_event_mask,
						WL_CHAN_REASON_CSA_TO_DFS_CHAN_FOR_CAC_ONLY);
					break;
				}
			...
			if (sub0->state == WL_DFS_CACSTATE_ISM) {
				WBD_INFO("ISM started for "MACDBG". cac state [%d] chanspec [0x%x] "
					"event mask [0x%x]\n", MAC2STRDBG(evt_ifr->InterfaceId),
					ifr_vndr_data->cac_cur_state, sub0->chanspec,
					ifr_vndr_data->dfs_event_mask[0]);
#ifdef MULTIAPR2
...
wbd_slave_remove_nonactive_peer_fbt_nvrams(char *prefix, char *old_fbt_aps, char *new_fbt_aps)
{
	...
	uint32 len_old_fbt_aps = strlen(old_fbt_aps), len_new_fbt_aps = strlen(new_fbt_aps);

	/* If NO Old fbt_aps, skip */
	if (len_old_fbt_aps <= 0) {
		goto end;
	}

	/* Get the max length among old and new fbt_aps list */
	max_len = MAX(len_old_fbt_aps, len_new_fbt_aps);
...
/* Prepare r0kh string for HAPD */
static int
wbd_slave_prepare_hapd_r0kh_entry(char *ifname, char *out_val, int out_sz)
{
	char nv_name[WBD_MAX_BUF_512] = {0};
	char fbt_name[WBD_MAX_BUF_128], tar_name[WBD_MAX_BUF_128];
	char *fbt_aps, *next;
	char r0kh[WBD_MAX_BUF_2048];
	 ...
	 */
	snprintf(nv_name, sizeof(nv_name), "%s_fbt_aps", ifname);
	fbt_aps = blanket_nvram_safe_get(nv_name);
	...
	foreach(fbt_name, fbt_aps, next)
	{
		...
		if (!strlen(blanket_nvram_safe_get(nv_name))) {
			continue;
		}

		snprintf(nv_name, sizeof(nv_name), "%s_addr", tar_name);
		addr = blanket_nvram_safe_get(nv_name);

		snprintf(nv_name, sizeof(nv_name), "%s_r0kh_id", tar_name);
		r0kh_id = blanket_nvram_safe_get(nv_name);

		snprintf(nv_name, sizeof(nv_name), "%s_r0kh_key", tar_name);
		fbt_r0_key = blanket_nvram_safe_get(nv_name);

		if (!strlen(addr) || !strlen(r0kh_id) || !strlen(fbt_r0_key)) {
			continue;
		}
		...
		if ((fbt_r0_key != NULL) && (strlen(fbt_r0_key) == (WBD_FBT_KH_KEY_LEN - 1))) {
...
/* Prepare r1kh string for HAPD */
static int
wbd_slave_prepare_hapd_r1kh_entry(char *ifname, char *out_val, int out_sz)
{
	char nv_name[WBD_MAX_BUF_512] = {0};
	char fbt_name[WBD_MAX_BUF_128], tar_name[WBD_MAX_BUF_128];
	char *fbt_aps, *next;
	char r1kh[WBD_MAX_BUF_2048];
	...
	foreach(fbt_name, fbt_aps, next)
	{
		...
		r1kh_id = blanket_nvram_safe_get(nv_name);

		snprintf(nv_name, sizeof(nv_name), "%s_r1kh_key", tar_name);
		fbt_r1_key = blanket_nvram_safe_get(nv_name);

		if (!strlen(addr) || !strlen(r1kh_id) || !strlen(fbt_r1_key)) {
			continue;
		}
		...
		if ((fbt_r1_key != NULL) && (strlen(fbt_r1_key) == (WBD_FBT_KH_KEY_LEN - 1))) {
...
/* Update HAPD with new NVRAMs for Neighbor APs */
static void
wbd_slave_update_hapd_for_peer_fbt_nvrams(bcm_usched_handle *hdl, void *arg)
{
	char* ifname = (char*)arg;
	int ret = WBDE_OK;
	char r0kh[WBD_MAX_BUF_2048 + 2] = {0}, r1kh[WBD_MAX_BUF_2048 + 2] = {0};
	char cmd[WBD_MAX_BUF_128 + WBD_MAX_BUF_2048] = {0};
	WBD_ENTER();

	WBD_ASSERT_ARG(ifname, WBDE_INV_ARG);

	/* Prepare r0kh string for HAPD & Set it to HAPD thourgh HAPD CLI */
	memset(r0kh, 0, sizeof(r0kh));
	wbd_slave_prepare_hapd_r0kh_entry(ifname, r0kh, sizeof(r0kh));
	memset(cmd, 0, sizeof(cmd));
	snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set r0kh \"%s\"", ifname, r0kh);
	WBD_INFO("%s\n", cmd);
	system(cmd);

	/* Prepare r1kh string for HAPD & Set it to HAPD thourgh HAPD CLI */
	memset(r1kh, 0, sizeof(r1kh));
	wbd_slave_prepare_hapd_r1kh_entry(ifname, r1kh, sizeof(r1kh));
	memset(cmd, 0, sizeof(cmd));
	snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set r1kh \"%s\"", ifname, r1kh);
	WBD_INFO("%s\n", cmd);
	system(cmd);

	/* Reload HAPD thourgh HAPD CLI, Post setting new r0kh & r1kh strings to HAPD */
	sleep(1);
	memset(cmd, 0, sizeof(cmd));
	snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s reload", ifname);
	WBD_INFO("%s\n", cmd);
	system(cmd);
...
/* Create timer for Callback fn to Update HAPD with new NVRAMs for Neighbor APs */
static int
wbd_slave_create_hapd_update_for_peer_fbt_nvrams_timer(char* ifname)
{
	int ret = WBDE_OK;
	char *if_name = NULL;
	WBD_ENTER();

	if_name = (char*)wbd_malloc(IFNAMSIZ, &ret);
	WBD_ASSERT_MSG("IF[%s] Failed to alloc hapd_update_for_peer_fbt_nvrams arg\n", ifname);
	WBDSTRNCPY(if_name, ifname, IFNAMSIZ);

	/* Add the timer */
	ret = wbd_add_timers(wbd_get_ginfo()->hdl, (void *)if_name,
		WBD_SEC_MICROSEC((wbd_get_ginfo()->max.tm_process_ap_configure)*3),
		wbd_slave_update_hapd_for_peer_fbt_nvrams, 0);

	/* If Add Timer not Succeeded */
	if (ret != WBDE_OK) {
		WBD_WARNING("Timeout[%d] Slave Failed to create hapd_update_for_peer_fbt_nvrams "
			"timer Error: %d\n",
			((wbd_get_ginfo()->max.tm_process_ap_configure)*3), ret);
		free(if_name);
	}
...
wbd_slave_create_peer_fbt_nvrams(char* ifname, wbd_fbt_bss_entry_t *fbt_bss_info,
	wbd_cmd_fbt_config_t *fbt_config_resp, int peer_triggered, int *error)
{
	int ret = WBDE_OK;
	...
	if (wlx_fbt_aps[0] != '\0') {
		...
		blanket_nvram_prefix_unset(prefix, strcat_r(prefix, NVRAM_FBT_APS, data));
	}
	...
	if (rc_flags) {
		...
		/* In case fbt_generate_local not set & Peer NVRAMs change is Triggered by Peer */
		if (peer_triggered) {

			/* Update HAPD with new NVRAMs for Neighbor APs */
			wbd_slave_create_hapd_update_for_peer_fbt_nvrams_timer(ifname);
		}
	}
...
wbd_slave_process_fbt_config_resp_cmd(unsigned char *neighbor_al_mac,
	unsigned char *tlv_data, unsigned short tlv_data_len)
{
	int ret = WBDE_OK, peer_triggered = 1, fbt_generate_local = WBD_FBT_DEF_GEN_LOCAL;
	...
	/* If FBT_CONFIG_REQ is sent by this Slave, Reset peer_triggered flag, Reset bit */
	...
	foreach(var_intf, wbd_ifnames, next_intf) {
		...
		/* Create the NVRAMs required for FBT for Neighbor APs, If gen Local not set */
		if (!fbt_generate_local) {

			rc_flags |= wbd_slave_create_peer_fbt_nvrams(ifname, matching_fbt_bss_info,
				&fbt_config_resp, peer_triggered, &ret);
		}
	}
	...
	ieee1905_chscan_req_msg *chscan_req)
{
	...
	/* Send Channel Scan Report Message to a Multi AP Device */
	if (chscan_req) {
		ieee1905_send_requested_stored_channel_scan(chscan_req, 0);
	}
...
wbd_slave_escan_run_per_radio(bcm_usched_handle *hdl, void *arg)
{
	...
	if (blanket_escan_prep_cs(ifr, &params->params, &params_size) != 0) {
...
wbd_slave_process_onboot_channel_scan(bcm_usched_handle *hdl, void *arg)
{
	...
	/* Start onboot scan only if all the interfaces are configured. This is to prevent
	 * escan if the device is not onboarded
	 */
	if (i5DmIsAllInterfacesConfigured()) {
		/* Send Requested { FRESH } Channel Scan Report to Controller */
		wbd_slave_process_requested_fresh_channel_scan(NULL, I5_CHSCAN_ONBOOT);
	} else {
		WBD_INFO("Not all the interfaces are configured so, create the onboot scan "
			"timer again\n");
		wbd_remove_timers(hdl, wbd_slave_process_onboot_channel_scan, arg);
		wbd_slave_create_onboot_channel_scan_timer((wbd_info_t*)arg);
	}
	...
	wbd_cmd_weak_client_bsd_t *cmdweakclient)
{
	 ...
	 * and driver. So, BSD has the correct information. So, if WBD gets weak client from
	 * BSD and if that STA is not in WBD assoclist, we can add it if it is in driver
	 */
	if (ret == WBDE_DS_UN_ASCSTA) {
		WBD_INFO("%s: Received WEAK STA notification for "MACF" which is not in assoclist "
			"of BSS "MACDBG". Refreshing assoclist\n",
			i5_bss->ifname, ETHERP_TO_MACF(sta_mac), MAC2STRDBG(i5_bss->BSSID));
	...
	WBD_INFO("Found weak STA "MACF" in BSS "MACDBG"\n", ETHERP_TO_MACF(sta_mac),
		MAC2STRDBG(i5_bss->BSSID));
	...
	wbd_cmd_weak_cancel_bsd_t *cmdweakcancel)
{
	...
	if (assoc_sta->status == WBD_STA_STATUS_WEAK ||
		...
		WBD_INFO("Band[%d] in BSS "MACDBG" Weak STA "MACF" became strong status=%d\n",
...
wbd_slave_store_beacon_report(char *ifname, uint8 *data, struct ether_addr *sta_mac)
{
	...
	if (event->subevent != DOT11_MEASURE_TYPE_BEACON) {
		WBD_ERROR("Ifname[%s] Unhandled event subtype: 0x%2X\n", ifname, event->subevent);
		goto end;
	}
...
wbd_slave_update_chanspec_timer_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	if (info->wbd_slave->flags & WBD_BKT_SLV_FLAGS_CAC_RUNNING) {
		WBD_INFO("CAC start timer running. Skip chanspec check\n");
		goto end;
	}
	...
	foreach_i5glist_item(sifr, i5_dm_interface_type, sdev->interface_list) {
		...
		/* If Channel Scan is running, Do not Report Channel Change */
		if (I5_IS_CHSCAN_RUNNING(sifr->flags)) {
			WBD_DEBUG("Channel Scan is running. Skip chanspec check\n");
			continue;
		}
	...
	unsigned short tlv_data_len)
{
	...
	blanket_get_chanspec(pdmif->ifname, &pdmif->chanspec);
...
wbd_slave_remove_traffic_separation_policy()
{
	...
	foreach(var_intf, nvval, next) {
		...
		if (I5_IS_BSS_GUEST(map_flags)) {
			tmpret = wbd_slave_move_ifname_to_list(var_intf, NVRAM_LAN1_IFNAMES,
				NVRAM_LAN_IFNAMES);
			if (tmpret == 0) {
...
wbd_slave_start_cac_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	foreach_i5glist_item(pdmif, i5_dm_interface_type, sdev->interface_list) {
		if (I5_IS_BSS_STA(pdmif->mapFlags)) {
			WBD_INFO("Interface ["MACF"] is STA, no channel switch in sta mode\n",
				ETHERP_TO_MACF(pdmif->InterfaceId));
			continue;
		}
		if (!pdmif->vndr_data) {
		...
		if (ifr_vndr_data->cac_rqst_action == MAP_CAC_ACTION_RETURN_TO_PREV_CHANNEL) {
			...
			pdmif->chanspec = ifr_vndr_data->cac_rqst_chspec;
		}
...
wbd_slave_process_cac_request(wbd_info_t *info, ieee1905_cac_rqst_list_t *pmsg)
{
	...
	for (i = 0; i < pmsg->count; i++) {
		...
		if (I5_IS_BSS_STA(pdmif->mapFlags)) {
			WBD_INFO("Interface ["MACF"] is STA, no CAC in sta mode\n",
				ETHERP_TO_MACF(pdmif->InterfaceId));
			ifr_vndr_data->cac_cur_state = MAP_CAC_RQST_COMPLETE;
			ifr_vndr_data->cac_compl_status = MAP_CAC_STATUS_RQST_NOT_SUPPORTED;
			continue;
		}
		...
		if (ifr_vndr_data->cac_cur_state) {
			if ((ifr_vndr_data->cac_rqst_chspec == chanspec) &&
		...
		/* 3+1 CAC maynot work proeperly if the current channel is DFS */
		if (cac_method == MAP_CAC_METHOD_MIMO_DIMENSION_REDUCED_CAC) {
			bool is_radar = 0;
			uint8 channel;

			FOREACH_20_SB(pdmif->chanspec, channel) {
				uint bitmap_sb = 0x00;
				blanket_get_chan_info(pdmif->ifname, channel,
					CHSPEC_BAND(pdmif->chanspec), &bitmap_sb);
				if (bitmap_sb & WL_CHAN_RADAR) {
					is_radar = 1;
					break;
				}
			...
			if (is_radar) {
				WBD_WARNING("Interface [%s] is on DFS chanspec [0x%x] "
					"channel [%d]. 3+1 not possible\n", pdmif->ifname,
					pdmif->chanspec, channel);
				ifr_vndr_data->cac_cur_state = MAP_CAC_RQST_COMPLETE;
				ifr_vndr_data->cac_compl_status = MAP_CAC_STATUS_OTHER_ERROR;
				continue;
			}
		}
...
wbd_slave_process_cac_terminate_rqst(wbd_info_t *info, ieee1905_cac_termination_list_t *pmsg)
{
	...
	for (i = 0; i < pmsg->count; i++) {
		...
		if (I5_IS_BSS_STA(pdmif->mapFlags)) {
			WBD_INFO("Interface ["MACF"] is STA, no CAC termination in sta mode\n",
				ETHERP_TO_MACF(pdmif->InterfaceId));
			continue;
		}
		...
		if (ifr_vndr_data->cac_rqst_chspec != chanspec) {
			WBD_WARNING("Ignoring CAC termination request for ["MACF"]. CAC "
				...
				ETHERP_TO_MACF(pdmif->InterfaceId), ifr_vndr_data->cac_rqst_chspec,
	...
	foreach_i5glist_item(sifr, i5_dm_interface_type, sdev->interface_list) {
		...
		if (ifr_vndr_data->cac_compl_status == MAP_CAC_STATUS_RADAR_DETECTED) {
			...
			FOREACH_20_SB(ifr_vndr_data->cac_rqst_chspec, channel) {
				...
				blanket_get_global_rclass(CH20MHZ_CHSPEC2(channel, band),
					&cac_compl->list[i].op_class);
				cac_compl->list[i].chan = channel;
				i++;
			}
			cac_compl->n_opclass_chan = i;
			/* first entry size is already included in cac_compl */
			len += sizeof(opclass_chan_list) * (i - 1);
		}
	}
	if (len) {
...
wbd_slave_prepare_cac_status_payload(uint8 **pbuf, uint16 *payload_len)
{
	...
	if (*radar_chan_count) {
		...
		ptr += sizeof(ieee1905_opclass_chan_info_t);
	}
	...
	for (i = 0; i < g_rc_chan_map_count; i++) {
		...
		for (j = 0; j < g_rc_chan_map[i].count; j++) {
			...
			FOREACH_20_SB(chanspec, channel) {
				...
				chan_info[index].bitmap |= bitmap_sb;
			}
			if (ret < 0) {
				continue;
			}
