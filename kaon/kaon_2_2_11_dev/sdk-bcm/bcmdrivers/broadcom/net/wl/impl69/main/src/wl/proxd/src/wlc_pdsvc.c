 ...
 *
 * $Id: wlc_pdsvc.c 793495 2020-11-30 15:16:32Z $
 */
...
wlc_proxd_AVB_clock_factor(wlc_pdsvc_info_t* pdsvc, uint8 shift, uint32 *ki, uint32 *kt)
{
	...
	if (CHIPID(pdsvc->wlc->pub->sih->chip) == BCM63178_CHIP_ID) {
		clkfracl = R_REG(pdsvc->wlc->osh, D11_TSF_CLK_FRAC_L(pdsvc->wlc->hw));
		switch (clkfracl) {
		case TSF_CLK_FRAC_L_63178_240p0431MHZ:
			/* factor = 1000*2^15/240.0431 */
			factor = 136509;
			break;
		case TSF_CLK_FRAC_L_63178_242p2504MHZ:
			/* factor = 1000*2^15/242.2504 */
			factor = 135265;
			break;
		default:
			WL_ERROR(("wl%d: %s: unknown clock factor for D11_TSF_CLK_FRAC_L 0x%x \n",
				pdsvc->wlc->pub->unit, __FUNCTION__, clkfracl));
			break;
		...
		/** AVB Clock = 242.2504 */
		factor = 135265;
	} else if (CHIPID(pdsvc->wlc->pub->sih->chip) == BCM6710_CHIP_ID) {
		/* factor = 1000*2^15/242.2504 */
		factor = 135265;
	} else if (CHIPID(pdsvc->wlc->pub->sih->chip) == BCM6715_CHIP_ID) {
		/* factor = 1000*2^15/363.375 */
		factor = 90177;
	} else
...
uint16 wlc_proxd_get_tx_subband(wlc_info_t * wlc, chanspec_t chanspec)
{
	...
	if (CHSPEC_IS80(phy_utils_get_chanspec(WLC_PI(wlc)))) {
		/* Target is 80M band */
		tchan = CHSPEC_CHANNEL(phy_utils_get_chanspec(WLC_PI(wlc)));
	...
	} else if (CHSPEC_IS40(phy_utils_get_chanspec(WLC_PI(wlc)))) {
		tchan = CHSPEC_CHANNEL(phy_utils_get_chanspec(WLC_PI(wlc)));
		if (CHSPEC_IS20(chanspec)) {
			ichan = CHSPEC_CHANNEL(chanspec);
			if (ichan == tchan + 2)
...
bool wlc_is_proxd_supported(wlc_info_t *wlc)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 65)) {
		ret = TRUE;
	} else if (D11REV_GE(wlc_hw->corerev, 129)) {
		ret = TRUE;
	}
...
pdsvc_get_bsscfg(wlc_pdsvc_info_t *pdsvc)
{
	...
	wlc = pdsvc->wlc;
	if (!pdsvc->bsscfg) {
		pdsvc->bsscfg = wlc->primary_bsscfg;
	}
	return pdsvc->bsscfg;
}
		...
		if (D11REV_IS(wlc->pub->corerev, 129) &&
			...
			*/
			if (D11REV_GE(wlc->pub->corerev, 129)) {
				avb_cap = (wlc_read_shm(wlc, M_UCODE_CAP_H(wlc)) <<
					   NBITS(uint16)) & EAP_FTM_CAP;
			}

			if (avb_cap && PROXD_ENAB(wlc->pub)) {
...
wlc_proxd_wlc_up(void *context)
{
	...
	if (avb_cap && PROXD_ENAB(pdsvc->wlc->pub)) {
		wlc->pub->cmn->_proxd_ucode_tsync = TRUE;
	}
#endif /* WL_PROXD_UCODE_TSYNC */
