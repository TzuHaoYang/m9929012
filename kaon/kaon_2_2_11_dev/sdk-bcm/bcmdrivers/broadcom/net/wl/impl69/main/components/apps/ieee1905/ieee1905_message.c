...
int i5MessageInsertTlv(i5_message_type *pmsg, unsigned char const *buf, unsigned int len)
{
  ...
  if (len > I5_MAX_TLV_PAYLOAD_LEN) {
    i5TraceDirPrint("Error: Size of the TLV %u exceeds minimum size %zu for TLV %d (%s)\n",
      len, I5_MAX_TLV_PAYLOAD_LEN, (int)((i5_tlv_t *)buf)->type,
...
void i5MessageTopologyResponseReceive(i5_message_type *pmsg)
{
  ...
  if ((pmsg_req = i5MessageMatch(i5_config.i5_mac_address, message_identifier, i5MessageTopologyQueryValue)) != NULL) {
    ...
    /* Send AP capability query only from controller. Also, send it only if there is any change
     * in the interfaces in topology
     */
    if (I5_IS_MULTIAP_CONTROLLER(i5_config.flags) && I5_IS_QUERY_AP_CAP(pdevice->flags)) {
...
void i5MessageMultiAPPolicyConfigRequestSend(i5_dm_device_type *pdevice)
{
  ...
  if (pmsg != NULL) {
    ...
    i5MessageAddVendorSpecificTlvWithCb(pmsg, pdevice->DeviceId, i5MsgMultiAPPolicyConfigRequestValue);
...
void i5MessageChannelSelectionResponseSend(i5_message_type *pmsg_req, uint8 resp_code)
{
  ...
  while (pdmif != NULL) {
   /* For all the operational BSS send the Channel Preference information */
    ...
    if (pdmif->msg_stat_flag == I5_MSG_DECLINED) {
      ...
      */
      i5TlvChannelSelectionResponseTypeInsert(pmsg, pdmif->InterfaceId, 3);
      pdmif->msg_stat_flag = I5_MSG_RESP_SENT;
    }
    pdmif = pdmif->ll.next;
  }
...
void i5MessageTunneledMessageReceive(i5_message_type *pmsg)
{
  ...
  prhex(" msg before source TLV : \n", pmsg->ppkt->pbuf, pmsg->ppkt->length);
