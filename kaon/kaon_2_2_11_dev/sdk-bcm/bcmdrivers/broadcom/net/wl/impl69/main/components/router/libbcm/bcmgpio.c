 ...
 *
 * $Id: bcmgpio.c 520342 2014-12-11 05:39:44Z $
 */
...
typedef struct {
	...
	int *strobe_done;			/* pointer to memory which is used to
	                                         * signal strobe completion
						 */
...
bcmgpio_timercb(bcm_timer_id tid, int gpio_pin)
{
	...
	if (bcmgpio_info[gpio_pin].strobing) {
		...
		if (bcmgpio_info[gpio_pin].timer_count == bcmgpio_info[gpio_pin].on_time) {
			...
			if (bcmgpio_info[gpio_pin].timer_count ==
				...
				if (bcmgpio_info[gpio_pin].tot_strobes > 0) {
					...
					if (bcmgpio_info[gpio_pin].strobe_count ==
					         bcmgpio_info[gpio_pin].tot_strobes) {
						bcmgpio_strobe_stop(gpio_pin);
						bcmgpio_out(bitmask,
						  bcmgpio_info[gpio_pin].orig_state);
...
bcmgpio_strobe_start(int gpio_pin, bcmgpio_strobe_t *strobe_info)
{
	...
	if ((status = bcm_timer_create(strobe_info->timer_module,
	...
	if ((status = bcm_timer_connect(bcmgpio_info[gpio_pin].timer_id,
	          (bcm_timer_cb) bcmgpio_timercb, (int) gpio_pin)) != 0) {
	...
	bcmgpio_info[gpio_pin].strobe_period =
	         strobe_info->strobe_period_in_ms / BCMGPIO_STRB_MS_TIME;
