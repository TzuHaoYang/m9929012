 ...
 *
 * $Id: wlc_rspec.c 793720 2020-12-08 08:32:21Z $
 */
...
static const bcm_iovar_t rspec_iovars[] = {
	...
	{"6g_rate", IOV_6G_RATE, IOVF_OPEN_ALLOW, 0, IOVT_UINT32, 0},
	{"6g_mrate", IOV_6G_MRATE, IOVF_OPEN_ALLOW, 0, IOVT_UINT32, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
static int wlc_set_ratespec_override(wlc_info_t *wlc, enum wlc_bandunit bandunit,
		ratespec_t rspec, bool mcast);
...
static enum wlc_bandunit
actionid2bandunit(uint32 actionid)
{
	enum wlc_bandunit band_unit;
	...
	case IOV_SVAL(IOV_5G_MRATE):
		band_unit = BAND_5G_INDEX;
		break;

	case IOV_GVAL(IOV_6G_RATE):
	case IOV_GVAL(IOV_6G_MRATE):
	case IOV_SVAL(IOV_6G_RATE):
	case IOV_SVAL(IOV_6G_MRATE):
		band_unit = BAND_6G_INDEX;
		break;
	}
	...
	void *params, uint p_len, void *arg, uint a_len, uint val_size, struct wlc_if *wlcif)
{
	...
	bandunit = actionid2bandunit(actionid);
	...
	case IOV_GVAL(IOV_6G_RATE):
	{
		...
		if (mcast) {
			ratespec = wlc->bandstate[bandunit]->mrspec_override;
		} else {
			ratespec = wlc->bandstate[bandunit]->rspec_override;
		}
	...
	case IOV_SVAL(IOV_6G_RATE):
	{
		err = wlc_set_iovar_ratespec_override(wlc, bsscfg, bandunit,
			(ratespec_t)int_val, mcast);
		break;
	}
...
wlc_rspec_to_rts_rspec_ex(wlc_info_t *wlc, ratespec_t rspec, bool use_rspec, bool g_prot)
{
	...
	if ((RSPEC_IS40MHZ(rts_rspec) && (CHSPEC_IS20(wlc->chanspec))) ||
...
int
wlc_set_iovar_ratespec_override(wlc_info_t *wlc, wlc_bsscfg_t *cfg, enum wlc_bandunit bandunit,
	ratespec_t wl_rspec, bool mcast)
{
	ratespec_t rspec;
	int err;

	WL_TRACE(("wl%d: %s: band %s wlrspec 0x%08X mcast %d\n", wlc->pub->unit, __FUNCTION__,
	          wlc_bandunit_name(bandunit), wl_rspec, mcast));

	err = wlc_wlrspec2ratespec(cfg, wl_rspec, &rspec);

	if (!err) {
		err = wlc_set_ratespec_override(wlc, bandunit, rspec, mcast);
	} else {
...
static int
wlc_set_ratespec_override(wlc_info_t *wlc, enum wlc_bandunit bandunit, ratespec_t rspec, bool mcast)
{
	wlcband_t *band;
	...
	WL_TRACE(("wl%d: %s: band %s rspec 0x%08X mcast %d\n", wlc->pub->unit, __FUNCTION__,
	          wlc_bandunit_name(bandunit), rspec, mcast));
	...
	if (HE_ENAB_BAND(wlc->pub, band->bandtype) && ishe) {
	...
	} else if (VHT_ENAB_BAND(wlc->pub, band->bandtype) && isvht) {
...
#if defined(WLAMPDU) && defined(WLATF_BARE)
