...
static int i5CtlSendCAPI(void *pCmd,int argc, char *argv[]);
#if defined(MULTIAPR2)
static int i5CtlSendCHScan(void *pCmd,int argc, char *argv[]);
static int i5CtlSendCACRequest(void *pCmd,int argc, char *argv[]);
static int i5CtlSendCACTermination(void *pCmd,int argc, char *argv[]);
static int i5CtlAssocDisallow(void *pCmd,int argc, char *argv[]);
#endif /* MULTIAPR2 */
#endif /* MULTIAP */
...
t_I5_CTL_CMD i5CtlCmds[] = {
	...
	I5_CTL_CMD_AGENT | I5_CTL_CMD_CONTROLLER},
    {"wpsPBC", "Start wps pbc configuration. (ifname/rfband mode(0-sta, 1-ap))", I5_API_CMD_WPS, i5CtlStartWPS, 3,
	I5_CTL_CMD_AGENT | I5_CTL_CMD_CONTROLLER},
    ...
    {"dme", "Display Extended data model", I5_API_CMD_RETRIEVE_DM_EXT, i5CtlDataModelCmdHandler, 0,
      I5_CTL_CMD_AGENT | I5_CTL_CMD_CONTROLLER },
    {"sendCAPI", "Send CAPI command (datalen filename)", I5_API_CMD_SEND_CAPI, i5CtlSendCAPI, 3,
      I5_CTL_CMD_AGENT | I5_CTL_CMD_CONTROLLER},
#if defined(MULTIAPR2)
    {"sendCHScan", "Send Channel Scan command(DestALID type(1 - Fresh Scan, 0 - Stored Result)",
      I5_API_CMD_SEND_CHSCAN, i5CtlSendCHScan, 3, I5_CTL_CMD_AGENT | I5_CTL_CMD_CONTROLLER},
    {"sendCACRequest", "Send CAC Request message "
     "(<DestALID> <tlv_len> <n_radio> <((mac opclass chan flag) * n_radio)>",
      I5_API_CMD_SEND_CAC_REQUEST, i5CtlSendCACRequest, 7, I5_CTL_CMD_CONTROLLER},
    {"sendCACTerminate", "Send CAC Terminate message "
     "(<DestALID> <tlv_len> <n_radio> <((mac opclass chan) * n_radio)>",
      I5_API_CMD_SEND_CAC_TERMINATION, i5CtlSendCACTermination, 6, I5_CTL_CMD_CONTROLLER},
    {"assocDisallow", "Set Association Disallowed Attribute in Beacon "
      "command(BSSID Reason(0 - Allow Association, 1-5 - Reason)", I5_API_CMD_ASSOC_DISALLOW,
      i5CtlAssocDisallow, 3, I5_CTL_CMD_AGENT},
#endif /* MULTIAPR2 */
#endif /* MULTIAP */
};
...
void i5CtlDataModelPrint(char *pBuf)
{
  ...
  for( devNum = 0; devNum < pDmNet->DevicesNumberOfEntries; devNum++) {
    ...
    printf("\nDevice: " I5_MAC_DELIM_FMT " \"%s\"%s v%d\n",
           I5_MAC_PRM (device->DeviceId), device->friendlyName, strflag, device->profile);
...
static int i5CtlSendCAPI(void *pCmd,int argc, char *argv[])
{
  ...
  data_len = (unsigned int)strtoul(argv[2], NULL, 0);
  msgSize = sizeof(*msg) + data_len;
  ...
  if (i5CtlReadFromFile(argv[3], msg->data, msg->data_len) <= 0) {
    goto end;
  }
...
static int i5CtlSendCHScan(void *pCmd,int argc, char *argv[])
{
  ...
  msg->type = (unsigned char)strtoul(argv[3], NULL, 0);
...
static int i5CtlSendCACRequest(void *pCmd,int argc, char *argv[])
{
  ...
  int rc = 0;
  int rd_bytes = 0;
  uint8 *remove_after_testing = NULL;

  buffer = (unsigned char *)malloc(I5_PACKET_BUF_LEN_512);
  if (!buffer) {
     printf(" Memory allocation failure \n");
     rc = -1;
     goto end;
  }
  memset(buffer, 0, I5_PACKET_BUF_LEN_512);
  msg = (t_I5_API_CAC_REQUEST*)buffer;
  ...
  msg->count = (unsigned char)atoi(argv[3]);

  printf("CAC request for n_radio[%d] \n", msg->count);
  if (!msg->count) {
    printf(" No list for CAC requestm exit \n");
    rc = -1;
    goto end;
  }
  plist = (uint8*)&msg->params[0];
  remove_after_testing = plist;

  for (i = 0, argument = 4; i < msg->count ; i++) {
     unsigned char mac[6];
     if (!i5String2MacAddr(argv[argument++], mac)) {
		printf("Invalid MAC Address\n");
     }
     memcpy((uchar*)plist, mac, MAC_ADDR_LEN); // radio mac
     plist += MAC_ADDR_LEN;
     *plist++ = (unsigned char)atoi(argv[argument++]); // opclass
     *plist++ = (unsigned char)atoi(argv[argument++]); // chan
     *plist++ = (unsigned char)atoi(argv[argument++]); // flags
     rd_bytes += 9; //hardcode for testing(size of ieee1905_cac_request_list)
  }
  /* Remove at checkin */
  prhex(" ===copied data : \n", remove_after_testing, rd_bytes);
  msg->msgInfo.messageId = i5MessageCACRequestValue;

  rc = i5apiTransaction(pI5Cmd->cmd, msg, (sizeof(*msg) + rd_bytes), &pBuf, 0);
  if ( rc != -1 ) {
    t_I5_CTL_CMDRESP_SEND_MSG_TYPE *response = pBuf;
    i5CtlDataModelAlMacPrint(response->srcMacAddr);
    printf("MID=%d\n",response->messageId);
...
static int i5CtlSendCACTermination(void *pCmd,int argc, char *argv[])
{
  ...
  memset(buffer, 0, I5_PACKET_BUF_LEN_512);
  msg = (t_I5_API_CAC_TERMINATION*)buffer;
  ...
  for (i = 0, argument = 4; i < msg->count ; i++) {
     ...
     *plist++ = (unsigned char)atoi(argv[argument++]);// chan
     ...
     rd_bytes += 8; // hardcode(size of ieee1905_cac_termination_list_t)
  }

  msg->msgInfo.messageId = i5MessageCACTerminationValue;
...
static int i5CtlAssocDisallow(void *pCmd,int argc, char *argv[])
{
  t_I5_API_ASSOC_DISALLOW_REQ *msg = NULL;
  t_I5_CTL_CMD *pI5Cmd = (t_I5_CTL_CMD *)pCmd;
  void *pBuf = 0;
  int rc;
  unsigned int msgSize;
  ...
  msgSize = sizeof(*msg);
  msg = (t_I5_API_ASSOC_DISALLOW_REQ*)malloc(msgSize);
  if (NULL == msg) {
    printf("Unable to allocate msg\n");
    return -1;
  }

  /* Store the BSSID in msgInfo.macAddr */
  if (NULL == i5String2MacAddr(argv[2], msg->msgInfo.macAddr)) {
     printf("MAC Address must be of form xx:xx:xx:xx:xx:xx\n");
     goto end;
  }
  msg->reason = (unsigned char)strtoul(argv[3], NULL, 0);
