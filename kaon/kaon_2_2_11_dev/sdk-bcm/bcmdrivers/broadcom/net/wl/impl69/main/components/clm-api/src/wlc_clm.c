...
#define FORMAT_VERSION_MAJOR 25
 ...
 */
static const unsigned char *
skip_multibandwidth_regulatory_powers(const unsigned char *reg_data_ptr)
{
	unsigned char flags;
	do {
		flags = *reg_data_ptr++;
		...
		reg_data_ptr += 1 + CLM_LOC_DSC_REG_REC_LEN * (unsigned int)(*reg_data_ptr);
 ...
 */
static const unsigned char *
skip_base_header(const unsigned char *loc_def, data_dsc_t *ds, const unsigned char **reg_limits,
	const unsigned char **psd_limits)
{
	unsigned char base_flags = loc_def[CLM_LOC_DSC_FLAGS_IDX];
	...
	if (ds->registry_flags2 & CLM_REGISTRY_FLAG2_DEVICE_CATEGORIES) {
		...
		loc_def += 1 + CLM_LOC_DSC_REG_REC_LEN * (int)(*loc_def);
	}
	if (reg_limits && ((loc_def - *reg_limits) < (CLM_LOC_DSC_REG_REC_LEN + 1))) {
		*reg_limits = NULL;
	}
...
static MY_BOOL
ranges_intersect(int l1, int r1, int l2, int r2, int min_intersection)
{
...
static clm_bandwidth_t
active_channel_bandwidth(const clm_limits_params_t *params)
{
 ...
 */
static MY_BOOL
get_dev_cat_reg_dest(unsigned char flags2, clm_device_category_t *dev_cat,
 ...
 */
static MY_BOOL
regulatory_country_information(const locale_data_t *base_loc_data, clm_country_info_t *info)
{
	...
	if (base_loc_data->reg_limits) {
		info->reg_limit_types |= 1 << CLM_REGULATORY_LIMIT_TYPE_CHANNEL;
	}
	if (base_loc_data->psd_limits) {
		info->reg_limit_types |= 1 << CLM_REGULATORY_LIMIT_TYPE_PSD;
	}
	if (base_loc_data->ds->registry_flags2 & CLM_REGISTRY_FLAG2_DEVICE_CATEGORIES) {
		unsigned int rdt;
		const unsigned char *reg_limits[2];
		/* Nonconstant array initializer is nonstandard extension :( */
		reg_limits[0] = base_loc_data->reg_limits;
		reg_limits[1] = base_loc_data->psd_limits;
		for (rdt = 0; rdt < ARRAYSIZE(reg_limits); ++rdt) {
			unsigned char flags;
			const unsigned char *p = reg_limits[rdt];
			if (!p) {
				continue;
			}
			do {
				...
				flags = *p++;
				if (flags & CLM_DATA_FLAG_FLAG2) {
					if (!get_dev_cat_reg_dest(*p++, &dev_cat, &reg_dest)) {
						return FALSE;
					}
				}
				info->device_categories |= 1u << dev_cat;
				info->reg_limit_destinations |= 1u << reg_dest;
				p += 1u + CLM_LOC_DSC_REG_REC_LEN * (unsigned int)(*p);
			} while (flags & CLM_DATA_FLAG_MORE);
		}
	} else {
 ...
 * \param[in] base_ht_loc_data Locale descriptors
 * \param[out] info Country information being filled
 * \return TRUE if no problem with data, FALSE if there is some problem
 *
 */
static MY_BOOL
tx_country_information(const locale_data_t base_ht_loc_data[BH_NUM], clm_country_info_t *info)
{
	unsigned int base_ht_idx;
	for (base_ht_idx = 0; base_ht_idx < BH_NUM; ++base_ht_idx) {
		unsigned char flags, flags2;
		const unsigned char *tx_rec = base_ht_loc_data[base_ht_idx].tx_limits;
		...
		do {
			...
			info->device_categories |= 1 << dev_cat;
...
{
	locale_data_t base_ht_loc_data[BH_NUM];
	const locale_data_t *base_loc_data = &base_ht_loc_data[BH_BASE];
	data_dsc_t *ds;
	if (!locales || !info || ((unsigned)band >= (unsigned)CLM_BAND_NUM)) {
		return CLM_RESULT_ERR;
	}
	bzero(info, sizeof(*info));
	info->flags = (unsigned long)(CLM_FLAG_DFS_NONE | CLM_FLAG_NO_40MHZ | CLM_FLAG_NO_80MHZ |
	...
	if (!fill_base_ht_loc_data(locales, band, base_ht_loc_data)) {
		return CLM_RESULT_ERR;
	}
	if (!regulatory_country_information(&base_loc_data[BH_BASE], info) ||
		!tx_country_information(base_ht_loc_data, info))
	{
		return CLM_RESULT_ERR;
	}
	info->flags |= translate_flags(base_loc_data->base_hdr
		? base_loc_data->base_hdr[CLM_LOC_DSC_FLAGS_IDX] : 0, FTS_BASE) |
	...
	unsigned char bw_to_chan[CLM_BW_NUM], MY_BOOL ru_limits)
{
	...
	const unsigned char *tx_rec = loc_data->tx_limits;
	...
	unsigned int num_limits, MY_BOOL ru_limits)
{
	...
	if (!locales || !per_rate_limits || !num_limits || (num_limits > WL_NUMRATES) ||
		...
		((unsigned)params->device_category >= (unsigned)CLM_DEVICE_CATEGORY_NUM))
	{
		return CLM_RESULT_ERR;
	}
	...
	clm_limits_type_t limits_type, clm_available_he_limits_result_t *result)
{
	...
	for (dev_cat = 0; dev_cat < CLM_DEVICE_CATEGORY_NUM; ++dev_cat) {
		...
		for (rt_idx = 0; rt_idx < ARRAYSIZE(su_base_rates); ++rt_idx) {
			...
			if (limits.su_limits[su_base_rates[rt_idx]] ==
		...
		for (rt_idx = 0; rt_idx < WL_RU_NUMRATES; ++rt_idx) {
			...
			/* Fail on absent HE rate table when there is enabled HE rate (ClmCompiler
			 * bug)
			 */
	...
	clm_channels_t *channels20, clm_channels_t *channels4080)
{
	...
	/* Computing helper information on locales */
	if (!fill_base_ht_loc_data(locales, CLM_BAND_5G, base_ht_loc_data)) {
		return CLM_RESULT_ERR;
	}
	...
	const clm_channels_params_t *params, clm_channels_t *channels)
{
	...
	for (base_ht_idx = 0; base_ht_idx < (int)ARRAYSIZE(base_ht_loc_data); ++base_ht_idx) {
		...
		do {
			...
			if (!((1 << device_category) & params->device_categories)) {
				tx_rec += num_rec * tx_rec_len;
				continue;
			}
			ranges = loc_data->chan_ranges_bw[pg_bw];
	...
	const clm_regulatory_limits_params_t *params, clm_regulatory_power_limits_t *limits)
{
	...
	const clm_channel_comb_set_t *comb_set;
	MY_BOOL qdbm = FALSE;
	MY_BOOL headerless = FALSE;
	clm_result_t ret = CLM_RESULT_NOT_FOUND;
	if (!params) {
		return CLM_RESULT_ERR;
	}
	if (!params || !locales || !limits || ((unsigned)band >= (unsigned)CLM_BAND_NUM) ||
		((unsigned)params->bw >= (unsigned)CLM_BW_NUM) ||
		((unsigned)params->dest >= (unsigned)CLM_REGULATORY_LIMIT_DEST_NUM) ||
		((unsigned)params->device_category >= (unsigned)CLM_DEVICE_CATEGORY_NUM) ||
		!get_loc_def(locales, compose_loc_type[band][BH_BASE], &loc_data))
	{
		return CLM_RESULT_ERR;
	}
	memset(limits->limits, (unsigned char)WL_RATE_DISABLED, sizeof(limits->limits));
	if (!loc_data.base_hdr) {
		return CLM_RESULT_NOT_FOUND;
	}
	...
	do {
		...
		if (headerless) {
			...
			flags = *record++;
			flags2 = (flags & CLM_DATA_FLAG_FLAG2) ? (*record++) : 0;
		}
		num_rec = *record++;
		if (bw_width_to_idx[flags & CLM_DATA_FLAG_WIDTH_MASK] != params->bw) {
		...
		for (; num_rec--; record += CLM_LOC_DSC_REG_REC_LEN)
		{
			...
			/* Skip disabled power record */
			if ((unsigned char)CLM_DISABLED_POWER == (unsigned char)limit) {
				continue;
			}
			if (qdbm) {
				limit /= QDBM_IN_DBM;
			...
			for (num_combs = comb_set->num, comb = comb_set->set; num_combs--; ++comb) {
				...
				for (chan = comb->first_channel; chan <= comb->last_channel;
					chan += comb->stride)
				{
					if (channel_in_range(chan, range, comb_set, 0)) {
	...
	unsigned int channel, int *limit)
{
	...
	if (channel >= (unsigned int)ARRAYSIZE(reg_limits.limits)) {
		return CLM_RESULT_ERR;
	}
	*limit = WL_RATE_DISABLED;
	...
	if ((unsigned)reg_limits.limits[channel] == (unsigned)WL_RATE_DISABLED) {
		return CLM_RESULT_NOT_FOUND;
	}
	...
	clm_power_t *psd_limit)
{
	...
	clm_regulatory_limits_params_t reg_limits_params;
	if (!params || !psd_limit ||
		(channel >= (unsigned int)ARRAYSIZE(reg_limits.limits)))
	{
		return CLM_RESULT_ERR;
	}
	*psd_limit = (clm_power_t)(unsigned char)WL_RATE_DISABLED;
	(void)clm_regulatory_limits_params_init(&reg_limits_params);
