 ...
 *
 * $Id: acsd_svr.h 787401 2020-05-28 16:58:31Z $
 */
...
#define ACS_TXOP_LIMIT 90		/* txop limit for channel change */
#define ACS_TXOP_CHANNEL_SELECT 2	/* change channel after completion of 2 secs */
...
typedef struct acs_fcs_s {
	...
	acs_intfer_trf_thold_t trf_params; /* intfer(txfail) configuration parametres */
...
typedef struct acs_chaninfo {
	...
	uint8 acs_start_on_nondfs;
	uint32 cur_timestamp;		/* cur timestamp */
	uint32 timestamp;		/* timestamp collected from chanim_stats */
	uint8 txop_channel_select;	/* change channel after competion of 2 secs */
	uint8 txop_score;		/* combination of tx+inbss+txop */
	uint8 dfs_reentry;		/* allow dfs_reentry if threshold is met */
...
extern int acs_fcs_tx_idle_check(acs_chaninfo_t *c_info);
...
extern void acs_fcs_retrieve_config(acs_chaninfo_t *c_info, char * prefix);
extern bool acs_fcs_check_for_hp_chan(acs_chaninfo_t *c_info, int bw);
extern bool acs_check_for_nondfs_chan(acs_chaninfo_t *c_info, int bw);
extern bool acs_fcs_check_for_overlap(chanspec_t cur_chspec, chanspec_t candi_chspec);
extern int acs_fcs_ci_scan_finish_check(acs_chaninfo_t * c_info);
