 ...
 *
 * $Id: wlc_ap.c 793901 2020-12-14 16:52:46Z $
 */
...
static const bcm_iovar_t wlc_ap_iovars[] = {
	...
	{"regulatory_info", IOV_REGULATORY_INFO, (IOVF_SET_DOWN), 0, IOVT_UINT8, 0},
...
static uint16 wlc_write_rnr_element(wlc_bsscfg_t *bsscfg, uint8 *cp, uint buf_len);
...
wlc_ap_up(wlc_ap_info_t *ap, wlc_bsscfg_t *bsscfg)
{
	...
	if (CHSPEC_IS40(target_bss->chanspec) &&
		...
		target_bss->chanspec = CH20MHZ_CHSPEC2(channel,
	...
	void *p, bool short_preamble, d11rxhdr_t *rxh)
{
	...
	if (CEVENT_STATE(wlc->pub)) {
		wlc_send_cevent(wlc, bsscfg, &hdr->sa, 0, 0, 0, NULL, 0,
			CEVENT_D2C_ST_AUTH_RX, CEVENT_FRAME_DIR_RX);
	}
#endif /* BCM_CEVENT */
	...
	case 1:
		...
		if (!(scb = wlc_scblookup(wlc, bsscfg, (struct ether_addr *) &hdr->sa))) {
			...
			if ((addr_match == WLC_MACFLTR_ADDR_DENY) ||
				...
				wlc_bss_mac_event(wlc, bsscfg, WLC_E_AUTH_FAIL, &hdr->sa,
...
#ifdef WL_OCE_AP
	if (BSS_OCE_ENAB(wlc, bsscfg) &&
		(wlc_oce_if_valid_assoc(wlc->oce, bsscfg, &wrxh->rssi, &reject_data) ==
			OCE_ASSOC_REJECT_RC_INSUFFICIENT_RSSI)) {

		WL_ASSOC(("wl%d: OCE_AP: *** Reject Association due to insufficient RSSI (%d) "
			"by delta(%d)\n", wlc->pub->unit, wrxh->rssi, reject_data));
...
wlc_ap_process_assocreq_decision(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, assoc_decision_t *dc)
{
	...
	if (scb->state & PENDING_ASSOC) {
		...
		if ((param->status == DOT11_SC_SUCCESS) &&
			(((scb->WPA_auth == WPA3_AUTH_SAE_PSK) && (scb->pmkid_included)) ||
			((scb->WPA_auth == WPA3_AUTH_DPP) && (scb->pmkid_included)) ||
			(scb->WPA_auth == WPA_AUTH_OWE) ||
			((scb->WPA_auth == WPA3_AUTH_SAE_FBT) && (scb->auth_alg == DOT11_SAE)))) {
...
defkey_done:
	...
	if ((param->status == DOT11_SC_SUCCESS) &&
		...
		wlc_bss_mac_event(wlc, bsscfg, WLC_E_ASSOC, &scb->ea, WLC_E_STATUS_SUCCESS, 0,
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		if (WSEC_ENABLED(scb->bsscfg->wsec) && SCB_AUTHORIZED(scb)) {
			...
			if (ap_scb->time_in_decrypt_failure_state >
				...
				WL_ERROR(("wl%d: decryption error for sta, send deauth to sta %s\n",
	...
	/* Find AP's that are enabled but not up to restart */
		...
		if (!phy_utils_ismuted(WLC_PI(wlc)) && !SCAN_IN_PROGRESS(wlc->scan) &&
			...
			if (txbcn_snapshot == appvt->txbcn_snapshot) {
				...
				edcrs = wlc_phy_is_edcrs_high(WLC_PI(wlc));
				if (((appvt->txbcn_inactivity >= appvt->txbcn_timeout) && !edcrs) ||
					...
					wlc_macdbg_psmwd_trigger(wlc->macdbg,
						WLC_MACDBG_PSMWD_TXBCN);
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	*ret_int_ptr = (int32)bsscfg->far_sta_rssi;
	...
	case IOV_SVAL(IOV_UPR_FD_SW):
		...
		break;
	case IOV_GVAL(IOV_REGULATORY_INFO):
		*ret_int_ptr = (int32)wlc->pub->_regulatory_info;
		break;
	case IOV_SVAL(IOV_REGULATORY_INFO):
		if (wlc->pub->_regulatory_info != (int8)int_val) {
			wlc->pub->_regulatory_info = (int8)int_val;
		}
		break;
#endif /* BAND6G || WL_OCE_AP */
...
wlc_ap_upd_onchan(wlc_ap_info_t* ap, wlc_bsscfg_t *cfg)
{
	...
	wlc_channel_send_chan_event(wlc, wlc->primary_bsscfg, WL_CHAN_REASON_ANY,
		wlc->chanspec);
#endif /* WL_AP_CHAN_CHANGE_EVENT */
	FOREACH_UP_AP(wlc, idx, apcfg) {
#ifdef WL_AP_CHAN_CHANGE_EVENT
		wlc_channel_send_chan_event(wlc, apcfg, WL_CHAN_REASON_ANY,
			wlc->chanspec);
#endif /* WL_AP_CHAN_CHANGE_EVENT */
	}
	...
	if (wlc->rnr_update_method == WLC_RNR_UPDATE_METHOD_SCAN) {
		FOREACH_UP_AP(wlc, i, bsscfg) {
			...
			err = wlc_scan_request(wlc, DOT11_BSSTYPE_ANY, &ether_bcast, 1,
					&ssid, DOT11_SCANTYPE_PASSIVE, -1, -1, -1, -1,
...
wlc_rnr_element_build_fn(void *ctx, wlc_iem_build_data_t *data)
{
	...
	switch (data->ft) {
		case FC_BEACON:
		case FC_PROBE_RESP:
			(void)wlc_write_rnr_element(cfg, cp, data->buf_len);
			break;
		default:
			break;
	}
...
static uint16
wlc_write_rnr_element(wlc_bsscfg_t *bsscfg, uint8 *cp, uint buf_len)
{
	ASSERT(buf_len >= (bsscfg->rnr_nbr_ap_info_size + BCM_TLV_HDR_SIZE));
...
wlc_ap_sendnulldata(wlc_ap_info_t *ap, wlc_bsscfg_t *cfg, struct scb *scb)
{
	...
	if (SCB_PS_TXFIFO_BLK(scb)) {
		WL_PS(("wl%d.%d: %s: "MACF" SCB_PS_TXFIFO_BLK pending %d pkts\n",
		       wlc->pub->unit, WLC_BSSCFG_IDX(cfg), __FUNCTION__,
		       ETHER_TO_MACF(scb->ea), wlc_scb_tot_inflt_fifocnt(wlc, scb)));
		return FALSE;
	}
#endif /* WL_PS_SCB_TXFIFO_BLK */
	...
	chanspec_t chspec, uint8 *cp, int buflen)
{
	...
	/* Find the wlc which is capable to operate on the
	* given chanspec to get accurate power numbers.
	* There is a special case during inter band CSA for
	* for RDSB use cases, where other bands information
	* is required before hand to be populated in the beacon.
	*/
	wlc = wlc_find_wlc_for_chanspec(wlc, chspec);
...
wlc_write_transmit_power_envelope_ie_6g(wlc_info_t *wlc,
	chanspec_t chspec, uint8 *cp, int buflen)
{
	uint8 regulatory_info = wlc->pub->_regulatory_info;
	wlc_txpwr_eirp_t      client_lpi_eirp;
	wlc_txpwr_eirp_psd_t  client_lpi_eirp_psd;
	wlc_txpwr_eirp_psd_t  subordinate_lpi_eirp_psd;
	...
	wlc_get_6g_tpe_reg_max_power(wlc_cmi, &locales, &limits_client,
		CLM_REGULATORY_LIMIT_DEST_CLIENT, CLM_DEVICE_CATEGORY_LP, chspec);

	client_lpi_eirp.txpwr_20 = limits_client.limit[0][0] * WLC_TPE_PWR_FACTOR;
	client_lpi_eirp.txpwr_40 = limits_client.limit[1][0] * WLC_TPE_PWR_FACTOR;
	client_lpi_eirp.txpwr_80 = limits_client.limit[2][0] * WLC_TPE_PWR_FACTOR;
	client_lpi_eirp.txpwr_160 = limits_client.limit[3][0] * WLC_TPE_PWR_FACTOR;
	for (i = 0; i < TPE_PSD_COUNT; i++) {
		client_lpi_eirp_psd.txpwr_psd[i] = limits_client.limit[0][1] * WLC_TPE_PWR_FACTOR;
	}

	wlc_get_6g_tpe_reg_max_power(wlc_cmi, &locales, &limits_subord,
		CLM_REGULATORY_LIMIT_DEST_SUBORDINATE, CLM_DEVICE_CATEGORY_LP, chspec);
	for (i = 0; i < TPE_PSD_COUNT; i++) {
		subordinate_lpi_eirp_psd.txpwr_psd[i] =
			limits_subord.limit[0][1] * WLC_TPE_PWR_FACTOR;
	}

	wlc_check_override_clm(wlc, &client_lpi_eirp, &client_lpi_eirp_psd);

	wlc_write_client_eirp(&cp, &client_lpi_eirp, TRANS_PWR_REG_EIRP,
		TRANS_PWR_CAT_DEFAULT, chspec, buflen);
	if (regulatory_info == 0) {
		 wlc_write_client_eirp_psd(&cp, &client_lpi_eirp_psd, TRANS_PWR_REG_EIRP_PSD,
		 TRANS_PWR_CAT_DEFAULT, chspec, buflen);
		 wlc_write_client_eirp_psd(&cp, &subordinate_lpi_eirp_psd, TRANS_PWR_REG_EIRP_PSD,
		 TRANS_PWR_CAT_SUBORDINATE, chspec, buflen);
	}
	if (regulatory_info == 1) {
		 wlc_write_client_eirp_psd(&cp, &client_lpi_eirp_psd, TRANS_PWR_REG_EIRP_PSD,
		 TRANS_PWR_CAT_DEFAULT, chspec, buflen);
	}
	return cp;
