 ...
 *
 * $Id: acsd_cli.c 793076 2020-11-12 09:41:58Z $
 */
...
typedef struct acs_cli_info {
	...
	char cmd_buf[ACSD_BUFSIZE_8K];	/* Server command and response buffer */
} acs_cli_info_t;
...
acsd_variables[] = {	/* Sorted alphabetically for nicer help display */
	...
	"acs_bgdfs_idle_interval", /* acs_bgdfs_idle_interval value */
	"acs_use_escan", /* Run escan if this variable is true */
	"acs_chanim_tx_avail", /* Allow CI scan based on this threshold */
	"acs_enable_dfsr_on_highpwr", /* If set, allow bgdfs from highpwr channels */
	"acs_nonwifi_enable", /* If set, allow downgrade in 2.4G if glitch is more */
	"acs_chanim_glitch_thresh", /* Glitch threshold */
	"acs_pref_6g_psc_pri", /* If set, allow PSC channel only in 6G Band */
	"test_cevent", /* send a cevent to test eventing */
	"acs_dfs_move_back", /* DFS move back disable feature */
	"last_dfs_chspec", /* get last DFS chspec, updated if acs_dfs_move_back = 0 */
	NULL
};
...
acs_dump_reason(ch_candidate_t * candi)
{
	printf("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n",
		...
		(candi->reason & ACS_INVALID_NONPSC) ? "NON_PSC" : "");
}
...
acs_dump_cscore_body(ch_candidate_t * candi)
{
	...
	printf("%7s (0x%04x)  %3s %s", wf_chspec_ntoa(candi->chspec, buf), candi->chspec,
		(candi->is_dfs) ? "dfs" : " - ",
		((candi->valid) ? "" : (candi->reason & ACS_INVALID_NOISE ? "":" Invalid:")));

	if (candi->valid || (candi->reason & ACS_INVALID_NOISE)) {
		acs_dump_ch_score_body(candi->chscore, TRUE);
		if (candi->reason & ACS_INVALID_NOISE) {
			printf(" Reason: INVALID_");
...
do_dump_candidate_int(acs_cli_info_t *ctx, int rcount, bool pretty_display)
{
	...
	for (i = 0; i < count; i++, candi++) {
		...
		for (j = 0; j < CH_SCORE_MAX; j++) {
			...
			ACSD_DEBUG("ntohl score is %d\n", candi->chscore[j].score);
		}
...
create_server_command(acs_cli_info_t *ctx)
{
	...
	len = acs_snprintf(ctx->cmd_buf, ACSD_BUFSIZE_8K, "%s&ifname=%s",
		ctx->cmdarg[CMDARG_COMMAND], ctx->wl_ifname);

	if (ctx->cmdarg[CMDARG_PARAMETER][0]) {
		len += acs_snprintf(ctx->cmd_buf+len, ACSD_BUFSIZE_8K - len,
...
do_command_response(acs_cli_info_t *ctx)
{
	...
	len = sread(ctx->socket, ctx->cmd_buf, ACSD_BUFSIZE_8K-1);
	if (len < 0) {
