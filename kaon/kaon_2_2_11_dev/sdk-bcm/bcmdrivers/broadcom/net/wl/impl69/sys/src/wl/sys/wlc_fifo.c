...
*			DL-MUMIO(VHT+HE) OFDMA ULOFDAM TWT
* Max:
* Reserved (Any AC):
* Per AC:
* Overload Allowed:
...
static void wlc_fifo_swap_packets(wlc_info_t *wlc, struct scb *scb,
...
static int
wlc_fifo_commit(wlc_fifo_info_t *fifo_info, struct scb *scb, mu_type_t mu, uint ac, int fifo_idx)
{
	...
	wlc_fifo_swap_packets(wlc, scb, (const uint8*)fifo_to, ac);
...
static void
wlc_fifo_swap_packets(wlc_info_t *wlc, struct scb *scb, const uint8 *fifo_idx_to, uint8 ac)
{
...
wlc_fifo_free_all(wlc_fifo_info_t *fifo_info, struct scb *scb)
{
	...
	if (wlc_scb_tot_inflt_fifocnt(wlc, scb)) {
		if (SCB_DEL_IN_PROGRESS(scb) || SCB_MARKED_DEL(scb)) {
			...
			wlc_fifo_swap_packets(wlc, scb, ac2sufifo, AC_ALL);
		}
	}
...
bool
wlc_fifo_is_ulofdma(wlc_fifo_info_t *fifo_info, struct scb *scb)
{
	mu_type_t mu;
	int fifo_idx;
	fifo_idx = scb->fifo_idx[AC_BE] - fifo_info->mu_fifo_start_idx;
...
wlc_fifo_mutx_ac_release(wlc_fifo_info_t *fifo_info, wl_mutx_ac_mg_t *mutx_ac_mask)
{
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		if (mu_disable_mask & mu_bitfield) {
			wlc_fifo_swap_packets(wlc, scb, ac2sufifo, ac);
 ...
 * Note: This API is valid for DLMMU only for now.
 */
