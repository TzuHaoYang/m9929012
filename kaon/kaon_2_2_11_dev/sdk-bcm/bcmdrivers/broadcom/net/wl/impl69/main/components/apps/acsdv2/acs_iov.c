 ...
 *
 *	$Id: acs_iov.c 790594 2020-08-28 02:56:31Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
 ...
 * subband_bmp bitmap provides per 20MHz channel information of type bmp_type
 */
int acs_set_chanspec_info(acs_chaninfo_t *c_info, chanspec_t chspec,
		uint8 subband_bmp, acs_chan_info_bmp_t bmp_type)
{
	int ret = BCME_ERROR;
	uint8 sub_channel, pos_20 = LOWER_20_POS_20MHZ;
	chanspec_t sub_chspec;
	uint32 chinfo = 0;
	...
	FOREACH_20_SB(chspec, sub_channel) {
		if (chinfo == WL_CHAN_INACTIVE && !(subband_bmp & pos_20)) {
			pos_20 >>= 1;
			continue;
		}
		sub_chspec = CH20MHZ_CHSPEC(sub_channel);
...
int acs_get_scan_ver(acs_chaninfo_t *c_info, wl_scan_version_t *ver)
{
	return wl_iovar_get(c_info->name, "scan_ver", ver, sizeof(wl_scan_version_t));
}

int acs_set_escan_params(acs_chaninfo_t *c_info, void *params, int params_size)
{
...
int acs_get_txpwr_info(acs_chaninfo_t *c_info, wl_chanspec_txpwr_max_t *param, int param_len,
	char *buf, int buf_len)
{
	return wl_iovar_getbuf(c_info->name, "chanspec_txpwr_max", param, param_len, buf, buf_len);
}
