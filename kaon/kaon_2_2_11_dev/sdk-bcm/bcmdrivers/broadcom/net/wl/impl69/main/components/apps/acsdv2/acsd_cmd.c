 ...
 *
 * $Id: acsd_cmd.c 793076 2020-11-12 09:41:58Z $
 */
...
acsd_proc_cmd(acsd_wksp_t* d_info, char* buf, uint rcount, uint* r_size)
{
	...
	*c++ = '\0';
	...
	if (!strcmp(buf, "csscan") || (!strcmp(buf, "autochannel"))) {
		...
		/* If current channel is DFS, temporarily set to non_DFS channel in order for
		 * CS scan to be successful.
		 */
	...
	if (!strcmp(buf, "dump")) {
		...
		if (!strcmp(param, "help")) {
			...
			*r_size = acs_dfsr_dump(ACS_DFSR_CTX(c_info), buf, ACSD_BUFSIZE_8K);
		}
	...
	if (!strcmp(buf, "get")) {
		...
		if (!strcmp(param, ACS_CMD_TEST_CEVENT)) {
			SEND_CEVENT_A2C(c_info->name, CEVENT_A2C_ST_ACS_CH_SW,
	...
	if (!strcmp(buf, "set")) {
		...
		if (!strcmp(param, "channelscanrpt")) {
			unsigned int nctrl_val = 0, next20_val = 0, next40_val = 0, next80_val = 0;
			...
			for (i = 0; i < c_info->perchan_count; i++) {
				...
				if (strstr(data, "radiomac") && (c_info->radiomac_count == 0)) {
				...
				c_info->perchan_stats[i].rep_nbr_util =	((util_val * 100) / 255);
		...
		if (!strcmp(param, ACS_CMD_TEST_CEVENT)) {
			if (setval == CEVENT_A2C_ST_ACS_CH_SW) {
				SEND_CEVENT_A2C(c_info->name, setval,
						&c_info->selected_chspec, sizeof(chanspec_t));
			} else {
				SEND_CEVENT_A2C(c_info->name, setval, NULL, 0);
			}
			goto done;
		}
