 ...
 *
 * $Id: wlc_txbf.c 792386 2020-10-26 13:30:42Z $
 */
...
static uint16 txbfcfg_def_vals[TXBFCFG_MAX_ENTRIES] = {
	...
	5,				/* sounding age period, 5 * 2 * beaconinterval = 1sec */
};
...
#define IMPBF_REV_GE64_USR_IDX	7
	...
	uint8 bfe_maxsts; /* limit the feedback report wth max nsts (Nc) */
};
...
static const bcm_iovar_t txbf_iovars[] = {
	...
	(IOVF_BSS_SET_DOWN), 0, IOVT_UINT8, 0
	},
	...
	{"txbf_bfe_maxsts", IOV_TXBF_BFE_NSTS_LMT,
	(0), 0, IOVT_BUFFER, 0
	},
	{NULL, 0, 0, 0, 0, 0}
};

#define BF_SOUND_PERIOD_DFT	 (100 * 1000/32)	/* 100 ms, in 32us unit, max 1024 msec */
#define BF_SOUND_PERIOD_DISABLED 0xffff
#define BF_SOUND_PERIOD_MIN	 5	/* 5ms */
#define BF_SOUND_PERIOD_MAX	 1024	/* 1024ms */
	...
	if (D11REV_GT(wlc->pub->corerev, 128)) {
		if ((txbf->txbfcfg_blk_ptr = (txbf_cfg_blk_t *)MALLOCZ(wlc->osh,
			...
			for (i = 0; i < TXBFCFG_MAX_ENTRIES; i++) {
				...
				} else if (i == TXBFCFG_SU_SNDAGE_THRSH) {
					txbf_cfg->offset = S_SNDAGE_THRSH;
					txbf_cfg->val = txbfcfg_def_vals[i];
				} else {
...
wlc_txbf_up(void *context)
{
	...
	*/
	...
	my_bfe_capable = txbf->bfe_capable;
	if (HE_ENAB_BAND(pub, wlc->band->bandtype)) {
		...
		if (my_bfe_capable & TXBF_HE_CQI_BFE_CAP) {
			hecap |= (1 << C_BFECAPHE_CQIEN_NBIT);
		}
		if (my_bfe_capable & TXBF_HE_MU_BFE_CAP) {
			hecap |= (0x1 << C_BFECAPHE_MUEN_NBIT);
		}
		if (my_bfe_capable & TXBF_HE_SU_BFE_CAP) {
	...
	if (VHT_ENAB(pub)) {
		...
		vhtcap |= (wlc_phy_get_bfe_ndp_recvstreams(WLC_PI(wlc)) << C_BFECAPVHT_NR_NBIT);
		if (CHSPEC_IS40(wlc->chanspec)) {
		...
		if (my_bfe_capable & TXBF_MU_BFE_CAP) {
			vhtcap |= (0x1 << C_BFECAPVHT_MUEN_NBIT);
		}
		if (my_bfe_capable & TXBF_SU_BFE_CAP) {
	...
	WL_TXBF(("wl%d: %s bfr capable %d bfe capable %d beamforming mode %d\n",
		...
		(my_bfe_capable > TXBF_MU_BFE_CAP) ? TXBF_MU_BFE_CAP : my_bfe_capable,
		txbf->mode));
	return BCME_OK;
}
...
wlc_txbf_impbf_upd(wlc_txbf_info_t *txbf)
{
	...
	if (D11REV_GT(wlc->pub->corerev, 40)) {
		bool is_caled = wlc_phy_is_txbfcal(WLC_PI(wlc));
...
wlc_txbf_mutx_enabled(wlc_txbf_info_t *txbf, uint8 tx_type)
{
	...
	if (tx_type == VHTMU) {
		return (((wlc->pub->mu_features & MU_FEATURES_MUTX) != 0) &&
				((txbf->bfr_capable & TXBF_MU_BFR_CAP) != 0));
	} else if (tx_type == HEMMU) {
		return (HE_DLMMU_ENAB(wlc->pub) && ((txbf->bfr_capable & TXBF_HE_MU_BFR_CAP) != 0));
	}
	return FALSE;
}
...
BCMATTACHFN(wlc_txbf_init)(wlc_txbf_info_t *txbf)
{
	...
	if (D11REV_GE(corerev, 40)) {
		if (WLC_BITSCNT(wlc->stf->hw_txchain) > 1) {
			...
			if (D11REV_GE(corerev, 129) && D11REV_LT(corerev, 132)) {
	...
	txbf->bfr_spexp = 0;
	txbf->bfe_maxsts = WLC_BITSCNT(wlc->stf->rxchain);
}
...
wlc_txbf_init_exp_ge128(wlc_txbf_info_t *txbf, scb_t *scb, txbf_scb_info_t *bfi)
{
	...
	my_bfe_capable = cfg->bfe_capable;

	if ((!(txbf->bfr_capable || my_bfe_capable)) ||
		WLC_PHY_AS_80P80(wlc, wlc->chanspec)|| (!wlc->clk)) {
		return BCME_OK;
	}

	if (txbf->virtif_disable && !BSSCFG_IS_PRIMARY(cfg) && !BSSCFG_PSTA(cfg)) {
		return BCME_OK;
	}
	...
	if (SCB_HE_CAP(scb)) {
		uint32 he_flags;
		he_flags = wlc_he_get_peer_caps(wlc->hei, scb);
	...
	if (my_bfe_capable && bfi->bfr_capable &&
		((wlc_vht_get_cap_info(wlc->vhti) & VHT_CAP_INFO_SU_BEAMFMEE) ||
		(WLC_HE_SU_BFE_CAP_PHY(wlc) && (my_bfe_capable & TXBF_HE_SU_BFE_CAP)))) {
...
wlc_txbf_init_exp(wlc_txbf_info_t *txbf, scb_t *scb, txbf_scb_info_t *bfi)
{
	...
	if ((!(txbf->bfr_capable || cfg->bfe_capable)) ||
		WLC_PHY_AS_80P80(wlc, wlc->chanspec)|| (!wlc->clk)) {
		return BCME_OK;
	}
	...
	if (!((txbf->bfr_capable && bfi->bfe_capable) ||
		(cfg->bfe_capable && bfi->bfr_capable &&
		(wlc_vht_get_cap_info(wlc->vhti) & VHT_CAP_INFO_SU_BEAMFMEE)))) {
		WL_TXBF(("wl%d %s fail: my bfr/bfe_cap = %d %d, peer bfr/bfe_cap = %d %d\n",
			wlc->pub->unit, __FUNCTION__,
			txbf->bfr_capable, cfg->bfe_capable,
			bfi->bfr_capable, bfi->bfe_capable));
		return BCME_OK;
	}
	...
	if ((txbf->bfr_capable & TXBF_MU_BFR_CAP) && (bfi->bfe_capable & TXBF_MU_BFE_CAP) &&
		...
		if (shmx_idx != BF_SHM_IDX_INV) {
			bfi->flags = MU_BFE;
			if ((cfg->bfe_capable & TXBF_SU_BFE_CAP) &&
	...
	if (!(cfg->bfe_capable && bfi->bfr_capable && (shm_idx != BF_SHM_IDX_INV) &&
		((wlc_vht_get_cap_info(wlc->vhti) & VHT_CAP_INFO_SU_BEAMFMEE)))) {
		goto init_done;
	}
...
wlc_txbf_init_imp(wlc_txbf_info_t *txbf, scb_t *scb, txbf_scb_info_t *bfi)
{
	...
	if ((txbf->flags & WLC_TXBF_FLAG_IMPBF) && txbf->imp &&
		!(txbf->virtif_disable && !BSSCFG_IS_PRIMARY(cfg) && !BSSCFG_PSTA(cfg)) &&
		!WLC_PHY_AS_80P80(wlc, wlc->chanspec) &&
...
wlc_disable_bfe_for_smth(wlc_txbf_info_t *txbf, wlc_bsscfg_t *cfg)
{
	 ...
	 */
	if (BSSCFG_AP(cfg) && wlc_phy_is_smth_en(WLC_PI(wlc)) &&
	    (wlc->band->phyrev >= 32) && (WLC_BITSCNT(wlc->stf->rxchain) > 2) &&
	    !((cfg->bfe_capable & TXBF_SU_BFE_SMTH_OVRD_DIS) > 0)) {
		disable_bfe = TRUE;
	} else {
		disable_bfe = FALSE;
	}
...
wlc_txbf_set_shm_idx(wlc_txbf_info_t *txbf, txbf_scb_info_t *bfi)
{
	...
	if (MU_TX_ENAB(txbf->wlc)) {
		bfe = ((cfg->bfe_capable & TXBF_SU_BFE_CAP) &&
			(bfi->bfr_capable & TXBF_SU_BFR_CAP) &&
			(wlc_vht_get_cap_info(wlc->vhti) & VHT_CAP_INFO_SU_BEAMFMEE)) ||
			(WLC_HE_SU_BFE_CAP_PHY(wlc) &&
			(cfg->bfe_capable & TXBF_HE_SU_BFE_CAP)) ? TRUE : FALSE;
...
wlc_txbf_delete_link_serve_ge128(wlc_txbf_info_t *txbf, scb_t *scb)
{
	...
	bfi->exp_en = FALSE;
	bfi->init_pending = TRUE;
	bfi->BFIConfig0 &= ~(1 << C_LTX_BFIVLD_NBIT);

	/* Make sure the linkmem entry still exists for this scb.
	 * to avoid update_link_entry() call to create a linkmem
	 * if already deleted
	 */
	if (RATELINKMEM_ENAB(wlc->pub) &&
		wlc_ratelinkmem_lmem_isvalid(wlc, scb)) {
...
wlc_txbf_bfe_init_rev128(wlc_txbf_info_t *txbf, txbf_scb_info_t *bfi)
{
	...
	if (HE_ENAB_BAND(pub, wlc->band->bandtype)) {
		...
		bfe_nsts = wlc_phy_get_bfe_ndp_recvstreams(WLC_PI(wlc));
	} else if (txbf->ht_enable) {
		bfe_nsts = HT_CAP_TXBF_CAP_C_BFR_ANT_DEF_VAL;
	}

	// XXX: From PHY: advertising bfe nsts as 3 iso 4 has better gain
	// with 4 streams 43684
	if (bfe_nsts > (txbf->bfe_maxsts - 1)) {
		bfe_nsts = txbf->bfe_maxsts - 1;
	}

	setbits((uint8 *)&bfi->BFIConfig1, sizeof(bfi->BFIConfig1), C_LTX_BFENR_NBIT,
		NBITSZ(C_LTX_BFENR), MIN(bfe_nsts, bfr_nr));
}
...
wlc_txbf_bfr_init_ge128(wlc_txbf_info_t *txbf, txbf_scb_info_t *bfi)
{
	...
	if (SCB_HE_CAP(scb)) {
		 ...
		 */
		mu_cb = 1;
		/* mu_ng := my ng16 cap AND peer's ng16 cap
		 * Note that our default ng16 in phycap is 0 and we prefer ng16=0 as it has better
		 * sounding report resolution. It is expected to have ng16=0 always unless
		 * something changes in the future
		 */
		mu_ng = wlc_he_get_phy_mu_feedback_ng16(wlc->hei);
		mu_ng &= getbits((uint8 *)&bfi->he_cap, sizeof(bfi->he_cap),
			HE_PHY_MU_FEEDBACK_NG16_SUPPORT_IDX, HE_PHY_MU_FEEDBACK_NG16_SUPPORT_FSZ);

		bfrcfg0 |= (mu_cb << C_LTX_MUCB_NBIT);
		bfrcfg0 |= (mu_ng << C_LTX_MUNG_NBIT);
		bfrcfg0 |= (1 << C_LTX_DISAMB_NBIT);

	} else if (SCB_VHT_CAP(scb)) {
		bfi_mode = 1;
	}
	bfrcfg0 |= (bfi_mode << C_LTX_STY_NBIT);

	if ((bfi->flags & MU_BFE) || ((bfi->flags & HE_MU_BFE) && HE_DLMMU_ENAB(wlc->pub))) {
	...
	if (PSMX_ENAB(pub)) {
		...
		bfrStats = ((bw -1) << C_LTX_BFRLBW_NBIT) | sb << C_LTX_BFRSB_NBIT;
		...
		bfrcfg0 |= (nc_idx << C_LTX_BFRNC_NBIT);

		ndps = MIN(bfr_sts, bfe_sts);
		bfrcfg0 |= (ndps << C_LTX_BFRNDPS_NBIT);

		bfrcfg0 |= (1 << C_LTX_BFIVLD_NBIT);
...
wlc_txbf_bfr_init(wlc_txbf_info_t *txbf, txbf_scb_info_t *bfi)
{
	...
	if (PSMX_ENAB(wlc->pub) && (bfi->flags & MU_BFE)) {
		...
		if (!BSSCFG_IS_PRIMARY(scb->bsscfg)) {
...
wlc_txbf_scb_state_upd(wlc_txbf_info_t *txbf, scb_t *scb, uint8* cap_ptr, uint cap_len, int8 cap_tp)
{
	...
	FALSE) {
#endif // endif
		if (!(txbf->bfr_capable || bsscfg->bfe_capable)) {
			return;
		}
#ifdef WL11AX
		if (SCB_HE_CAP(scb)) {
...
void
wlc_txbf_applied2ovr_upd(wlc_txbf_info_t *txbf, bool bfen)
{
...
void
wlc_txbf_rxchain_upd(wlc_txbf_info_t *txbf)
{
	int rxchains;
	wlc_info_t *wlc;
...
void
wlc_txbf_scb_ps_notify(wlc_txbf_info_t *txbf, scb_t *scb, bool ps_on)
{
	txbf_scb_info_t *bfi;
	bfi = TXBF_SCB_INFO(txbf, scb);
...
void
wlc_txbf_txchain_upd(wlc_txbf_info_t *txbf)
{
	...
	*/
	...
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		FOREACHSCB(wlc->scbstate, &scbiter, scb) {
			...
			bfi->BFIConfig0 &= ~(C_LTX_BFRNC_BMSK | C_LTX_BFRNDPS_BMSK);
			bfi->BFIConfig0 |= (nc_idx << C_LTX_BFRNC_NBIT);

			ndps = MIN(bfr_sts, bfe_sts);
			bfi->BFIConfig0 |= (ndps << C_LTX_BFRNDPS_NBIT);
		}
	} else {
 ...
 */
bool
wlc_txbf_bfen(wlc_txbf_info_t *txbf, scb_t *scb,
	...
	for (i = 0; i < TXBF_MUTXV_MAX; i++) {
		...
		if (bfi->bfr_capable) {
			...
			bcm_bprintf(b, "    bfr cap 0x%x, myaddr as bfe %s\n",
				bfi->bfr_capable,
				(scb->bsscfg->bfe_capable & bfi->bfr_capable) ? "ON" : "OFF");
		}
	...
	for (i = 0; i <= txbf->max_link; i++) {
		...
		if (bfi->vht_cap & VHT_CAP_INFO_SU_BEAMFMR)
			bcm_bprintf(b, "\tsu bfr cap, bfe link %s\n",
				(scb->bsscfg->bfe_capable && (wlc_vht_get_cap_info(wlc->vhti) &
...
wlc_txbf_dump_clr(void *ctx)
{
	...
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		...
		wlc_write_shm(wlc, M_VHTSU_TXNDPA_CNT(wlc), 0);
		wlc_write_shm(wlc, M_VHTMU_TXNDPA_CNT(wlc), 0);
		wlc_write_shm(wlc, M_HESU_TXNDPA_CNT(wlc), 0);
		wlc_write_shm(wlc, M_HEMU_TXNDPA_CNT(wlc), 0);
		wlc_write_shm(wlc, M_VHTSU_RXSF_CNT(wlc), 0);
		wlc_write_shm(wlc, M_VHTMU_RXSF_CNT(wlc), 0);
		wlc_write_shm(wlc, M_HESU_RXSF_CNT(wlc), 0);
		wlc_write_shm(wlc, M_HEMU_RXSF_CNT(wlc), 0);
		wlc_write_shm(wlc, M_VHTSU_EXPRXSF_CNT(wlc), 0);
		wlc_write_shm(wlc, M_VHTMU_EXPRXSF_CNT(wlc), 0);
		wlc_write_shm(wlc, M_HESU_EXPRXSF_CNT(wlc), 0);
		wlc_write_shm(wlc, M_HEMU_EXPRXSF_CNT(wlc), 0);
	}
...
wlc_txbf_dump_stats(wlc_txbf_info_t *txbf, bcmstrbuf_t *b)
{
	...
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		bcm_bprintf(b, "txndpa %d (vhtsu %d vhtmu %d hesu %d hemu %d) txndp %d"
			" txbfpoll %d ",
			wlc_read_shm(wlc, M_TXNDPA_CNT(wlc)),
			wlc_read_shm(wlc, M_VHTSU_TXNDPA_CNT(wlc)),
			wlc_read_shm(wlc, M_VHTMU_TXNDPA_CNT(wlc)),
			wlc_read_shm(wlc, M_HESU_TXNDPA_CNT(wlc)),
			wlc_read_shm(wlc, M_HEMU_TXNDPA_CNT(wlc)),
			wlc_read_shm(wlc, M_TXNDP_CNT(wlc)),
			wlc_read_shm(wlc, M_TXBFPOLL_CNT(wlc)));
			bcm_bprintf(b, "txbrp %d txbfm %d\n",
				wlc_read_shm(wlc, M_TXBRPTRIG_CNT(wlc)),
				wlc_read_shm(wlc, M_TXBFM_CNT(wlc)));
		bcm_bprintf(b, "rxsf %d (vhtsu %d vhtmu %d hesu %d hemu %d) ",
			wlc_read_shm(wlc, M_RXSFUCAST_CNT(wlc)),
			wlc_read_shm(wlc, M_VHTSU_RXSF_CNT(wlc)),
			wlc_read_shm(wlc, M_VHTMU_RXSF_CNT(wlc)),
			wlc_read_shm(wlc, M_HESU_RXSF_CNT(wlc)),
			wlc_read_shm(wlc, M_HEMU_RXSF_CNT(wlc)));
		bcm_bprintf(b, "exprxsf %d (vhtsu %d vhtmu %d hesu %d hemu %d)",
			wlc_read_shm(wlc, M_NDPAUSR_CNT(wlc)),
			wlc_read_shm(wlc, M_VHTSU_EXPRXSF_CNT(wlc)),
			wlc_read_shm(wlc, M_VHTMU_EXPRXSF_CNT(wlc)),
			wlc_read_shm(wlc, M_HESU_EXPRXSF_CNT(wlc)),
			wlc_read_shm(wlc, M_HEMU_EXPRXSF_CNT(wlc)));
		bcm_bprintf(b, " rxcqi %d\n",
...
void
wlc_txfbf_update_amt_idx(wlc_txbf_info_t *txbf, int amt_idx, const struct ether_addr *addr)
{
	uint32 i, shm_idx;
	uint16 val;
	wlc_info_t *wlc;
...
wlc_txbf_txbfcfg_write_shmx(wlc_txbf_info_t *txbf, int index)
{
	...
	case TXBFCFG_SU_SNDAGE_THRSH: {
		wlc_bmac_write_scr(wlc->hw, txbf_cfg->offset, txbf_cfg->val);
		break;
	}
	default:
	...
	void *p, uint plen, void *a, uint alen, uint vsize, wlc_if_t *wlcif)
{
	...
	case IOV_GVAL(IOV_TXBF_BFE_CAP):
		*ret_int_ptr = bsscfg->bfe_capable;
		break;
	...
	case IOV_SVAL(IOV_TXBF_BFE_CAP): {
		...
		if (D11REV_GE(pub->corerev, 128)) {
			int_val &= (TXBF_SU_BFE_CAP | TXBF_MU_BFE_CAP | TXBF_HE_SU_MU_BFE_CAP
				| TXBF_HE_CQI_BFE_CAP | TXBF_SU_BFE_SMTH_OVRD_DIS);
		} else if (D11REV_GE(pub->corerev, 65)) {
			int_val &= (TXBF_SU_BFE_CAP | TXBF_MU_BFE_CAP | TXBF_SU_BFE_SMTH_OVRD_DIS);
		} else if (D11REV_GE(pub->corerev, 40)) {
	...
	case IOV_GVAL(IOV_TXBF_TIMER):
		if (txbf->sounding_period == 0)
			...
			*ret_uint_ptr = (uint32)txbf->sounding_period * 32 / 1000;
		break;
	...
	case IOV_SVAL(IOV_TXBF_TIMER):
		if (int_val == -1) /* -1 auto */
			...
			txbf->sounding_period = (uint16)int_val * (1000 / 32);
		wlc_write_shm(wlc, M_BFI_REFRESH_THR(wlc), txbf->sounding_period);
		break;
	...
	case IOV_GVAL(IOV_TXBF_BFE_NSTS_LMT):
		*ret_uint_ptr = txbf->bfe_maxsts;
		break;

	case IOV_SVAL(IOV_TXBF_BFE_NSTS_LMT):
		if (txbf->bfe_maxsts > WLC_BITSCNT(wlc->stf->rxchain)) {
			return BCME_UNSUPPORTED;
		}
		txbf->bfe_maxsts = (uint8) int_val;
		break;
...
void
wlc_txbf_pkteng_tx_start(wlc_txbf_info_t *txbf, scb_t *scb)
{
	uint16 val, bfrctl = 0;
	wlc_info_t *wlc;
	txbf_scb_info_t *bfi;
...
void
wlc_txbf_pkteng_tx_stop(wlc_txbf_info_t *txbf, scb_t *scb)
{
	wlc_info_t * wlc;
	uint16 val;
	txbf_scb_info_t *bfi;
...
wlc_txbf_vht_upd_bfr_bfe_cap(wlc_txbf_info_t *txbf, wlc_bsscfg_t *cfg, uint32 *cap)
{
	...
	wlc = txbf->wlc;
	BCM_REFERENCE(wlc);
	if ((wlc->txbf->virtif_disable && !BSSCFG_IS_PRIMARY(cfg) && !BSSCFG_PSTA(cfg)) ||
		WLC_PHY_AS_80P80(wlc, wlc->chanspec)) {
		bfr = bfe = 0;
	} else {
		bfr = txbf->bfr_capable;
		bfe = wlc_disable_bfe_for_smth(txbf, cfg) ? 0: cfg->bfe_capable;
	}
	wlc_vht_upd_txbf_cap(wlc->vhti, bfr, bfe, cap);
	if ((BSSCFG_AP(cfg) && ((cfg->bfe_capable & TXBF_SU_BFE_SMTH_OVRD_DIS) > 0)) ||
		(!BSSCFG_AP(cfg) && bfe > 0)) {
		if (WLC_BITSCNT(wlc->stf->rxchain) == 4)
			wlc_phy_smth_per_nss_SUTxbf(WLC_PI(wlc), 0x7);
		else if (WLC_BITSCNT(wlc->stf->rxchain) == 3)
			wlc_phy_smth_per_nss_SUTxbf(WLC_PI(wlc), 0x3);
	} else {
		wlc_phy_smth_per_nss_SUTxbf(WLC_PI(wlc), 0x1);
	}
}
#endif /* WL11AC */
	...
	d11linkmem_entry_t *link_entry)
{
	...
	if (bfi) {
		...
		/* shouldn't overwrite bits written by other modules */
...
void
wlc_forcesteer_gerev129(wlc_info_t *wlc, uint8 enable)
{
	wlc_txbf_info_t *txbf;
	scb_t *scb;
	txbf_scb_info_t *bfi;
	...
	if (D11REV_GE(wlc->pub->corerev, 129)) {
		...
		snd_type = 1;
		bfrcfg0 = (snd_type << C_LTX_STY_NBIT);
		bfi->BFIConfig0 = bfrcfg0;

		bw = (wlc->chanspec & WL_CHANSPEC_BW_MASK) >> WL_CHANSPEC_BW_SHIFT;
		bw -= (WL_CHANSPEC_BW_20 >> WL_CHANSPEC_BW_SHIFT);
		bfrStats = (bw << C_LTX_BFRLBW_NBIT) | (0 << C_LTX_BFRSB_NBIT) |
			(wlc->stf->txchain << C_LTX_BFRCMASK_NBIT);
		bfi->BFRStat0 = bfrStats;
	}
#endif /* WL_PSMX */
	if (!txbf->active) {
...
uint8
wlc_txbf_get_bfe_cap(wlc_txbf_info_t *txbf, wlc_bsscfg_t *cfg)
{
	return cfg->bfe_capable;
}
...
wlc_txbf_tbcap_update(wlc_txbf_info_t *txbf, scb_t *scb)
{
	...
	bfi->BFIConfig0 &= ~(1 << C_LTX_TBCAP_NBIT);
	bfi->BFIConfig0 &= ~(1 << C_LTX_MUCAP_NBIT);
	bfi->BFIConfig0 &= ~(1 << C_LTX_CQICAP_NBIT);
...
wlc_txbf_tbcap_check(wlc_txbf_info_t *txbf, scb_t *scb, uint8 mu_type)
{
	...
	if (mu_type == HEMMU) {
		if (!(bfi->BFIConfig0 & (1 << C_LTX_MUCAP_NBIT) &&
			(bfi->BFIConfig0 & (1 << C_LTX_TBCAP_NBIT)))) {
			return ret;
		}
...
void
wlc_txbf_bfecap_init(wlc_txbf_info_t *txbf, wlc_bsscfg_t *bsscfg)
{
	wlc_info_t *wlc = txbf->wlc;
	uint8 my_bfe_capable;

	BCM_REFERENCE(bsscfg);
	my_bfe_capable = WLC_SU_BFE_CAP_PHY(wlc) ? TXBF_SU_BFE_CAP : 0;
	my_bfe_capable |= WLC_MU_BFE_CAP_PHY(wlc) ? TXBF_MU_BFE_CAP : 0;
	if (D11REV_GE(wlc->pub->corerev, 128)) {
		my_bfe_capable |= WLC_HE_SU_BFE_CAP_PHY(wlc) ? TXBF_HE_SU_BFE_CAP : 0;
		my_bfe_capable |= WLC_HE_SU_MU_BFE_CAP_PHY(wlc) ?
			TXBF_HE_SU_MU_BFE_CAP : 0;
	}
	bsscfg->bfe_capable = my_bfe_capable;
}
