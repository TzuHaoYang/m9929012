 ...
 *
 * $Id: wlc_pcb.h 791560 2020-10-01 08:23:54Z $
 */
...
/**
 * Set packet callback for a given packet. It is called at the time a packet is added to the tx
 * path.
 */
 ...
 */
extern int wlc_pcb_pkt_is_registered(wlc_pcb_info_t *pcbi, pkcb_fn_t fn, void *arg, void *pkt);
...
/** Invoke packet callback(s). It is called in the tx path when the tx status is processed. */
...
/** register or unregister callback for all pkts prior to when they are freed */
...
extern int wlc_pcb_pktfree_cb_unregister(wlc_pcb_info_t *pcbi,
	wlc_pcb_pktfree_fn_t fn, void* ctxt);
/** locate first non zero entry in packet class callback, used by A-MSDU */
extern void wlc_pcb_getdata(wlc_pcb_info_t *pcbi,
	wlc_pkttag_t *pt, uint8 *pcb_offset, uint8 *pcb_data);
#endif /* _wlc_pcb_h_ */
