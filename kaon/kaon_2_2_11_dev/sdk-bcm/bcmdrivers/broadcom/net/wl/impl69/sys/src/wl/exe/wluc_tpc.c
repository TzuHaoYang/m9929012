 ...
 *
 * $Id: wluc_tpc.c 788027 2020-06-18 11:48:42Z $
 */
...
static void wl_txpwr_array_row_print(ppr_t* pprptr, int8 channel_bandwidth,
	reg_rate_index_t rate_idx, bool verbose);
static void wl_txpwr_array_print(ppr_t* pprptr, ppr_ru_t* ppr_ru_ptr, int8 channel_bandwidth,
	bool verbose, chanspec_t chspec_band, uint32 hw_txchain, uint32 curpower_flag);
static void wl_txpwr_ppr_print(ppr_t* pprptr, ppr_ru_t* ppr_ru_ptr, int vb, ppr_rate_type_t type,
	clm_rate_group_id_t gid, int8 bw, reg_rate_index_t *rate_index, chanspec_t chspec_band,
...
wl_txpwr_print_summary(tx_pwr_rpt_t *ppr_wl, bool verbose)
{
	...
	if (ppr_wl->version == TX_POWER_T_VERSION) {
		...
		printf("%-23s%d.%d dB\n", "SROM Antgain 6G:",
			DIV_QUO(ppr_wl->antgain[2], 4), DIV_REM(ppr_wl->antgain[2], 4));
	} else if (ppr_wl->version == TX_POWER_T_VERSION45) {
		tx_pwr_rpt_ver45_t *ppr_wl_v45 = (tx_pwr_rpt_ver45_t *)ppr_wl;
		printf("%-23s%d.%d dB\n", "SROM Antgain 2G:",
			DIV_QUO(ppr_wl_v45->antgain[0], 4), DIV_REM(ppr_wl_v45->antgain[0], 4));
		printf("%-23s%d.%d dB\n", "SROM Antgain 5G:",
			DIV_QUO(ppr_wl_v45->antgain[1], 4), DIV_REM(ppr_wl_v45->antgain[1], 4));
	}
...
wl_get_current_power(void *wl, cmd_t *cmd, char **argv)
{
	...
	for (argn = 1; argv[argn]; ++argn) {
		if (!strcmp(argv[argn], "--verbose") || !strcmp(argv[argn], "-v")) {
			verbose = TRUE;
		} else
		if (!strcmp(argv[argn], "--brief") || !strcmp(argv[argn], "-b")) {
			brief = TRUE;
		} else
		if (!strcmp(argv[argn], "-he")) {
			curpower_flag |= CURPOWER_SU;
		} else
		if (!strcmp(argv[argn], "--dlofdma") || !strcmp(argv[argn], "-d")) {
			curpower_flag |= CURPOWER_DLOFDMA;
		} else
		if (!strcmp(argv[argn], "--ub_he") || !strcmp(argv[argn], "-ub")) {
			curpower_flag |= (CURPOWER_DLOFDMA | CURPOWER_UB);
		} else
		if (!strcmp(argv[argn], "--ulofdma") || !strcmp(argv[argn], "-u")) {
	...
	if (legacy_curpower_mode) {
		tx_pwr_rpt_ver45_t *ppr_wl_v45 = NULL;
		pprsize -= ppr_get_he_tlv_size(ppr_target, ppr_chanspec_bw(chanspec),
			PPR_MAX_TX_CHAINS);
		ppr_rpt_size = sizeof(tx_pwr_rpt_ver45_t) + pprsize * WL_LEGACY_TXPPR_SER_BUF_NUM;
		...
		ppr_wl = ppr_wl_buf;
		ppr_wl_v45 = (tx_pwr_rpt_ver45_t *)ppr_wl;
		memset(ppr_wl, WL_RATE_DISABLED, ppr_rpt_size);

		ppr_wl->ppr_len = pprsize;
		ppr_wl->version = TX_POWER_T_VERSION45;
		...
		/* parse */
		if (ppr_wl->version != TX_POWER_T_VERSION45) {
		...
		ppr_ser = ppr_wl_v45->pprdata;

		(void)ppr_convert_from_tlv(ppr_board, ppr_wl_v45->pprdata, ppr_wl->ppr_len);
		...
		for (pprtype_count = 1; pprtype_count <= WL_TXPPR_SER_BUF_NUM; pprtype_count++) {
			...
			if ((err = wlu_get(wl, cmd->get, ppr_wl, ppr_rpt_size)) < 0) {
				ppr_wl->version = TX_POWER_T_VERSION45;
				ppr_rpt_size = (sizeof(tx_pwr_rpt_ver45_t) + pprsize);
				if ((err = wlu_get(wl, cmd->get, ppr_wl, ppr_rpt_size)) < 0) {
					fprintf(stderr, "Error: Curpower failed. %d ", err);
					fprintf(stderr, "Bring up interface and disable mpc "
						"if necessary (wl mpc 0)\n");
			...
			/* parse */
			if (ppr_wl->version == TX_POWER_T_VERSION) {
				pprdata_ref = ppr_wl->pprdata;
			} else if (ppr_wl->version == TX_POWER_T_VERSION45) {
				tx_pwr_rpt_ver45_t *ppr_wl_v45 = (tx_pwr_rpt_ver45_t *)ppr_wl;
				pprdata_ref = ppr_wl_v45->pprdata;
			} else {
				printf("error: version mismatch - driver %d, "
					"wl executable was expecting %d or %d\n",
					ppr_wl->version,
					TX_POWER_T_VERSION45, TX_POWER_T_VERSION);
				err = BCME_ERROR;
				goto exit;
			}
			...
			case PPRTYPE_BOARDLIMITS:
				...
				(void)ppr_convert_from_tlv(ppr_board, pprdata_ref,
					ppr_wl->ppr_len);
				break;
			case PPRTYPE_REGLIMITS:
				...
				(void)ppr_convert_from_tlv(ppr_reg, pprdata_ref,
					ppr_wl->ppr_len);
				break;
	...
	if (brief) {
		wl_txpwr_print_header(ppr_wl->channel_bandwidth, verbose, curpower_flag);
	}

	printf("Regulatory Limits:\n");
	if (brief) {
		wl_txpwr_array_row_print(ppr_reg, ppr_wl->channel_bandwidth,
			get_reg_rate_index_from_ratespec(ppr_wl->last_tx_ratespec), verbose);
	} else {
		wl_txpwr_array_print(ppr_reg, clm_he_txpwr_limit, ppr_wl->channel_bandwidth,
			verbose, chanspec & WL_CHANSPEC_BAND_MASK, hw_txchain, curpower_flag);
	}
	printf("\n");
...
wl_get_current_txctrl(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv != NULL) {
		...
		fprintf(stderr, "Error: No arguments supported\n");
		goto exit;
	}

	if (!is_he) {
		err = BCME_UNSUPPORTED;
		fprintf(stderr, "Error: Only supported for HE\n");
		goto exit;
	}
	...
	if (txctrl.frame_type == WL_FRAME_TYPE_HE ||
		txctrl.frame_type == WL_FRAME_TYPE_VHT) {
		if ((phyrev < 130) || (txctrl.cp_ltf_size == 0xff)) {
			printf("\n=== PER-User TXCTL_EXT Starts Here (Total 0x%x users) ===\n\n",
			       txctrl.n_user);
			for (i = 0; i < txctrl.n_user; i++) {
				uint nss = ((txctrl.mcs_nss_mu[i]>>4) & 0x7) + 1;
				uint mcs = (txctrl.mcs_nss_mu[i]) & 0xf;
				bool bfm_en = (txctrl.bfm_mu[i]>>7) & 0x1;
				uint8 bfm_idx = (txctrl.bfm_mu[i]) & 0x7f;
				uint16 aid_idx = (txctrl.aid_mu[i]) & 0x7ff;
				bool dcm_en = (txctrl.aid_mu[i]>>11) & 0x1;
				uint8 power_loading = (txctrl.aid_mu[i]>>12) & 0xf;
				...
				if (txctrl.frame_type == WL_FRAME_TYPE_VHT) {
					...
					uint8 drop_ind = (txctrl.usridx_mu[i]>>7) & 0x1;
					uint8 usr_idx = (txctrl.usridx_mu[i]) & 0x7f;
			...
			printf("\n=== HESIG EXT fields for UL TB frames ===\n\n");
			printf("	cp_ltf_size	:\t%d\n", txctrl.cp_ltf_size);
			printf("	num_heltf	:\t%d\n", txctrl.num_heltf);
			printf("	heltf_mask	:\t%d\n", (uint)txctrl.heltf_mask);
			printf("	LDPC		:\t%d\n", (uint)txctrl.ldpc);
			printf("	mcs		:\t%d\n", txctrl.mcs);
			printf("	nss		:\t%d\n", txctrl.nss);
			printf("	nss_start_pos	:\t%d\n", txctrl.nss_start_pos);
			printf("	DCM		:\t%d\n", (uint)txctrl.dcm);
			printf("	ldpc_extra	:\t%d\n", (uint)txctrl.ldpc_extra);
			printf("	afactor		:\t%d\n", txctrl.afactor);
			printf("	disamb		:\t%d\n", (uint)txctrl.disamb);
			printf("	Doppler		:\t%d\n", (uint)txctrl.doppler);
		}
...
static void
wl_txpwr_array_row_print(ppr_t *pprptr, int8 channel_bandwidth,
	reg_rate_index_t rate_index, bool verbose)
{
	const char *label;
	txpwr_row_t powers;
	...
	else
	{
		...
		wl_txpwr_print_row(label, ppr_group_table[group_id].chain, powers,
			WL_RATE_DISABLED, channel_bandwidth, verbose, 0);
	}
}
...
static void
wl_txpwr_array_print(ppr_t* pprptr, ppr_ru_t* ppr_ru_ptr, int8 bw, bool verbose,
	chanspec_t chspec_band,	uint32 hw_txchain, uint32 curpower_flag)
{
	...
	for (i = start; i < end; i++) {
		wl_txpwr_ppr_print(pprptr, ppr_ru_ptr, verbose, ppr_group_table[i].rate_type, i, bw,
			&rate_index, chspec_band, hw_txchain, curpower_flag);
	...
	uint32 hw_txchain, uint32 curpower_flag)
{
	...
	if (type == PPR_RATE_DSSS && chspec_band == WL_CHANSPEC_BAND_5G) {
