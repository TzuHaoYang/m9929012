 ...
 *
 * $Id: wbd_master_control.c 792947 2020-11-09 14:10:22Z $
 */
...
/* Predefined TBSS weightage for 6g */
static wbd_tbss_thld_t predefined_tbss_thld_6g[] = {
	/* t_rssi t_hops t_sta_cnt t_uplinkrate flags sof_algos */
	/* 0 : All Threshold and WBD_SOF_ALGO_BEST_NWS */
	{-75, 1, 30, 36, 0x7F, 0},
	/* 1 : All Threshold and WBD_SOF_ALGO_BEST_RSSI */
...
typedef i5_dm_bss_type* (*wbd_tbss_algo_t)(wbd_master_info_t *master_info, i5_dm_clients_type *sta,
	uint8 sta_band, i5_dm_interface_type *i5_sta_ifr);

/* Selecting BSS for steering a STA based on normalized wightage and score(NWS) */
static i5_dm_bss_type*
wbd_tbss_algo_nws(wbd_master_info_t *master_info, i5_dm_clients_type *sta, uint8 sta_band,
	i5_dm_interface_type *i5_sta_ifr);

/* Selecting BSS for steering a STA based on threshold values. Survival of Fittest(SoF) algo */
static i5_dm_bss_type*
wbd_tbss_algo_sof(wbd_master_info_t *master_info, i5_dm_clients_type *sta, uint8 sta_band,
	i5_dm_interface_type *i5_sta_ifr);
...
#define WBD_MAX_THRESHOLD_6G \
	(sizeof(predefined_tbss_thld_6g)/sizeof(wbd_tbss_thld_t)-1)
...
/* Get the number of threshold policies defined for 6G */
int
wbd_get_max_tbss_thld_6g()
{
	return WBD_MAX_THRESHOLD_6G;
}
...
/* Get the threshold policy based on index for 6G */
wbd_tbss_thld_t*
wbd_get_tbss_thld_6g(wbd_master_info_t *master_info)
{
	return &predefined_tbss_thld_6g[master_info->tbss_info.tbss_thld_6g];
}
...
wbd_get_nss_from_radio_caps(i5_dm_interface_type *i5_ifr)
{
	...
	if (i5_ifr->ApCaps.HECaps.Valid) {
		switch (i5_ifr->ApCaps.HECaps.CapsEx & WBD_MASK_BIT7_TO_BIT5) {
			case IEEE1905_AP_HECAP_TX_NSS_1:
				cur_nss = 1;
				break;
			case IEEE1905_AP_HECAP_TX_NSS_2:
				cur_nss = 2;
				break;
			case IEEE1905_AP_HECAP_TX_NSS_3:
				cur_nss = 3;
				break;
			case IEEE1905_AP_HECAP_TX_NSS_4:
				cur_nss = 4;
				break;
			case IEEE1905_AP_HECAP_TX_NSS_8:
				cur_nss = 8;
				break;
		}
	} else if (i5_ifr->ApCaps.VHTCaps.Valid) {
	...
	foreach_i5glist_item(i5_device, i5_dm_device_type, i5_topology->device_list) {
		...
		foreach_i5glist_item(i5_ifr, i5_dm_interface_type, i5_device->interface_list) {
			...
			if (i5_ifr->ApCaps.HECaps.Valid) {
				he = TRUE;
				if (bw == 160) {
					mcs_map = i5_ifr->ApCaps.HECaps.TxBW160MCSMap;
				} else {
					mcs_map = i5_ifr->ApCaps.HECaps.TxBW80MCSMap;
				}
			} else if (i5_ifr->ApCaps.VHTCaps.Valid) {
				mcs_map = i5_ifr->ApCaps.VHTCaps.TxMCSMap;
			}
			blanket_get_txrate(mcs_map, cur_nss, bw, &bss->avg_tx_rate, he);
	...
	i5_dm_bss_type *parent_bss, int adjust)
{
	...
	if (WBD_BAND_TYPE_LAN_2G(cur_ifr->band) && !WBD_BAND_TYPE_LAN_2G(parent_ifr->band)) {
		est_rssi += wbd_get_ginfo()->crossband_rssi_est;
	} else if (WBD_BAND_TYPE_LAN_2G(parent_ifr->band) && !WBD_BAND_TYPE_LAN_2G(cur_ifr->band)) {
		est_rssi -= wbd_get_ginfo()->crossband_rssi_est;
	}
	...
	i5_dm_bss_type *parent_bss, int current_adv_score, int max_score)
{
	...
	for (idx = 0; idx < tbss_scores->count; idx++) {
		...
		if (I5_IS_CTRLAGENT(i5_device->flags)) {
			...
			WBD_INFO("Select master BSS["MACDBG"] Type[%s], score[%d]\n",
				MAC2STRDBG(iter_item->i5_bss->BSSID),
				"Master", iter_item->score);
			goto end;
		} else if (!I5_IS_DWDS(i5_device->flags)) {
			WBD_INFO("Considering Ethernet BSS["MACDBG"] Type[%s], score[%d]\n",
				MAC2STRDBG(iter_item->i5_bss->BSSID),
				"Ethernet",
				iter_item->score);
...
{
	uint8 eth_updated = 0, dedicated_updated = 0, n_hops = 0;
	...
	if (i5_parent_device) {
		...
		device_vndr_data->n_hops = n_hops;
...
/* Traverse STA NON Preferred channel report with tbss channel and opclass to avoid tbss select */
static bool
wbd_tbss_is_sta_preferred_channel(wbd_assoc_sta_item_t *sta, uint8 channel, uint8 opclass)
{
	wbd_glist_t *np_chan_list = (wbd_glist_t*)&sta->np_chan_entry_list;
	wbd_sta_mbo_np_chan_list_t_g *new_channel_list = NULL;
	dll_t *item_p;
	int count;
	bool ret = TRUE;
	...
	if (!sta || !np_chan_list->count) {
		WBD_INFO("STA NULL or STA Non preferred entry list count [%d] \n",
			np_chan_list->count);
		goto end;
	}

	count = np_chan_list->count;

	WBD_INFO("STA Non preferred entry list count [%d] bss channel [%d] bss opclass [%d]\n",
		count, channel, opclass);

	/* Traverse STA NON Preferred channel report */
	foreach_glist_item_ex(item_p, *np_chan_list, count > 0) {
		int i;
		new_channel_list = (wbd_sta_mbo_np_chan_list_t_g*)item_p;
		count--;

		if (new_channel_list->opclass != opclass) {
			continue;
		}

		for (i = 0; i < new_channel_list->list_len; i++) {
			if (new_channel_list->list[i] == channel) {
				WBD_INFO("STA non preferred channel[%d] match tbss channel[%d] so "
					"skip bss\n", new_channel_list->list[i], channel);
				ret = FALSE;
				goto end;
			}
		}
...
scoring:
	...
	WBD_INFO("For STA "MACF" on BSS "MACDBG" - weight_flags=0x%x, rssi=%d, N_RSSI=%d, "
		"N_HOPS=%d, tot_sta=%d, assoc_cnt=%d, N_STA_CNT=%d, phyrate=%d, N_PHYRATE=%d, "
		"bss_nss=%d, max_nss=%d, N_NSS=%d, txrate=%d, max_txrate=%d, N_TXRATE=%d, "
		"SCORE=%d\n", ETHERP_TO_MACF(mac), MAC2STRDBG(i5_bss->BSSID), wght_cfg->flags,
...
/* Selecting BSS for steering a STA based on normalized wightage and score(NWS) */
i5_dm_bss_type*
wbd_tbss_algo_nws(wbd_master_info_t *master_info, i5_dm_clients_type *weak_sta, uint8 sta_band,
	i5_dm_interface_type *i5_sta_ifr)
{
	uint8 band_steer;
	i5_dm_network_topology_type *i5_topology;
	i5_dm_device_type *i5_device, *parent_device, *i5_sta_ifr_device = NULL;
	...
	if (I5_CLIENT_IS_BSTA(weak_sta)) {
		...
		WBD_ASSERT_ARG(i5_sta_ifr, WBDE_INV_ARG);
	...
	WBD_INFO("START NWS for SSID[%s]. bss_count[%d] Max_NSS[%d] Max_AVG_TX_Rate[%d] "
	...
	foreach_i5glist_item(i5_device, i5_dm_device_type, i5_topology->device_list) {
		...
		if (i5_device == parent_device) {
			...
			if (sta || bcn_sta) {
				...
				WBD_DEBUG("STA["MACDBG"] NO Beacon AND Monitor rssi in "
		...
		foreach_i5glist_item(i5_ifr, i5_dm_interface_type, i5_device->interface_list) {
			...
			/* Get the tbss channel from the interface */
			tbss_channel = wf_chspec_primary20_chan(i5_ifr->chanspec);

			/* check tbss channel and opclass with sta non preferred chan report */
			is_pref_chan = wbd_tbss_is_sta_preferred_channel(assoc_sta, tbss_channel,
					i5_ifr->opClass);
			if (!is_pref_chan) {
				WBD_DEBUG("STA["MACDBG"] non preferred channel match so. "
			 ...
			 */
			if (!(sta_band & i5_ifr->band)) {
				WBD_DEBUG("STA["MACDBG"] Band[0x%x] doesn't support Band [0x%x] "
					"of IFR["MACDBG"] in Device["MACDBG"]. Skip it\n",
					MAC2STRDBG(weak_sta->mac), sta_band, i5_ifr->band,
			 ...
			 */
			if (!band_steer && (i5_device == parent_device) && (i5_ifr != parent_ifr)) {
				WBD_INFO("New Device and ParentDevice["MACDBG"] are same AND "
					"band steering is disabled. So skip band steering of "
					"STA["MACDBG"]\n", MAC2STRDBG(i5_device->DeviceId),
					MAC2STRDBG(weak_sta->mac));
				continue;
			}
			...
			foreach_i5glist_item(i5_bss, i5_dm_bss_type, i5_ifr->bss_list) {
				...
				/* If backhaul BSS Profile 1 / 2 assoc disallowed set continue */
				if (I5_CLIENT_IS_BSTA(weak_sta) &&
					I5_IS_BSTA_DISALLOWED(i5_sta_ifr_device->profile,
						i5_bss->mapFlags)) {
					WBD_DEBUG("BSTA profile [%d] Assoc STA disallowed [%x] "
						"set on bss["MACF"], so skip.. \n",
						i5_sta_ifr_device->profile, i5_bss->mapFlags,
						ETHERP_TO_MACF(i5_bss->BSSID));
					continue;
				}
#endif /* MULTIAPR2 */
	...
	if (tbss_scores)
		free(tbss_scores);
	if (target_bss) {
		WBD_INFO("For STA "MACDBG" found Target BSS "MACDBG" score=%d\n",
			MAC2STRDBG(weak_sta->mac),
	...
	wbd_wl_sta_stats_t *stats, struct ether_addr *mac, uint8 is_bsta)
{
	...
	if (is_bsta) {
		...
		if (band & WBD_BAND_LAN_2G) {
			...
			thld_cfg = &master_info->tbss_info.thld_cfg_6g;
		} else {
	...
	if (thld_cfg->flags & WBD_THLD_FLAG_HOPS) {
		n_hops = device_vndr_data->n_hops;
		if (n_hops >= thld_cfg->t_hops)
			fail_cnt++;
	}
	...
	WBD_INFO("BKTNAME[%s] For STA["MACF"] on Slave["MACDBG"]. (value/threshold/enable) "
...
wbd_master_find_target_slave_with_best_rssi(wbd_master_info_t *master_info,
	i5_dm_clients_type *weak_sta, uint8 sta_band, i5_dm_interface_type *i5_sta_ifr)
{
	uint8 band_steer;
	i5_dm_network_topology_type *i5_topology;
	i5_dm_device_type *i5_device, *parent_device, *i5_sta_ifr_device = NULL;
...
/* Selecting BSS for steering a STA based on threshold values. Survival of Fittest(SoF) algo */
i5_dm_bss_type*
wbd_tbss_algo_sof(wbd_master_info_t *master_info, i5_dm_clients_type *weak_sta, uint8 sta_band,
	i5_dm_interface_type *i5_sta_ifr)
{
	uint8 band_steer;
	uint8 is_bsta = 0;
	...
	i5_dm_network_topology_type *i5_topology;
	i5_dm_device_type *device, *parent_device, *i5_sta_ifr_device = NULL;
	...
	foreach_i5glist_item(device, i5_dm_device_type, i5_topology->device_list) {
		...
		foreach_i5glist_item(ifr, i5_dm_interface_type, device->interface_list) {
			...
			/* Get the tbss channel from the interface */
			tbss_channel = wf_chspec_primary20_chan(ifr->chanspec);
			 ...
			 * band of interface, then skip it
			 */
			if (!(sta_band & ifr->band)) {
				WBD_DEBUG("STA["MACDBG"] Band [0x%x] doesn't support Band [0x%x] "
					"of IFR["MACDBG"] in Device["MACDBG"]\n",
					MAC2STRDBG(weak_sta->mac), sta_band, ifr->band,
					MAC2STRDBG(ifr->InterfaceId), MAC2STRDBG(device->DeviceId));
				continue;
			}
			 ...
			 * is associated if the band_steer is disabled
			 */
			if (!band_steer && (device == parent_device) && (ifr != parent_ifr)) {
				WBD_INFO("New Device and ParentDevice["MACDBG"] are same AND "
					"band steering is disabled. So skip band steering of "
					"STA["MACDBG"]\n", MAC2STRDBG(device->DeviceId),
					MAC2STRDBG(weak_sta->mac));
			...
			foreach_i5glist_item(bss, i5_dm_bss_type, ifr->bss_list) {
				...
				if (I5_CLIENT_IS_BSTA(weak_sta) &&
					...
					WBD_DEBUG("BSTA profile [%d] Assoc STA disallowed [%x] "
						"set on bss["MACF"], so skip.. \n",
						i5_sta_ifr_device->profile, bss->mapFlags,
						ETHERP_TO_MACF(bss->BSSID));
					continue;
				}
#endif /* MULTIAPR2 */
	...
	if (node_count == 0) {
		WBD_INFO("All BSS failed to pass threshold check for STA "MACDBG"\n",
			MAC2STRDBG(weak_sta->mac));
		goto end;
	}
	...
	if (node_count == 1) {
		...
		if (parent_bss == tmp_good_bss) {
			WBD_INFO("For STA "MACDBG", only current BSS "MACDBG" passed all "
				"thresholds. Not Steering\n", MAC2STRDBG(weak_sta->mac),
				MAC2STRDBG(tmp_good_bss->BSSID));
			goto end;
		}
		WBD_INFO("For STA "MACDBG" only one BSS "MACDBG" found eligible\n",
			MAC2STRDBG(weak_sta->mac),
			MAC2STRDBG(tmp_good_bss->BSSID));
	 ...
	 */
	if (parent_ifr->band & BAND_2G) {
		thld_cfg = &master_info->tbss_info.thld_cfg_2g;
	} else if (parent_ifr->band & BAND_6G) {
		thld_cfg = &master_info->tbss_info.thld_cfg_6g;
	} else {
		thld_cfg = &master_info->tbss_info.thld_cfg_5g;
	}
	if (thld_cfg->sof_algos == WBD_SOF_ALGO_BEST_RSSI) {
		target_bss = wbd_master_find_target_slave_with_best_rssi(master_info,
			weak_sta, sta_band, i5_sta_ifr);
		goto end;
	} else if (thld_cfg->sof_algos == WBD_SOF_ALGO_BEST_NWS) {
		target_bss = wbd_tbss_algo_nws(master_info, weak_sta, sta_band, i5_sta_ifr);
		goto end;
	}
	...
	uint8 sta_band, i5_dm_interface_type *i5_sta_ifr)
{
	...
	WBD_INFO("BKTNAME[%s] For STA["MACDBG"] Band[%d] Finding Target BSS Using Algo[%d]\n",
		master_info->bkt_name, MAC2STRDBG(weak_sta->mac), sta_band, tbss_algo);

	return (predefined_tbss_algo[tbss_algo])(master_info, weak_sta, sta_band, i5_sta_ifr);
}
	...
	i5_dm_bss_type *tbss)
{
	...
	WBD_INFO("Sending BTM steer req for sta "MACDBG" at bss "MACDBG" to target bss "MACDBG" "
		"opclass=%d] chan=%d\n", MAC2STRDBG(sta_mac), MAC2STRDBG(bssid),
		MAC2STRDBG(tbss->BSSID), bss_info->target_op_class, bss_info->target_channel);
...
{
	uint8 sta_band = WBD_BAND_LAN_INVALID;
	uint8 is_backhaul = 0;
	...
	i5_dm_interface_type *parent_ifr, *i5_sta_ifr = NULL;
	...
	/* For backhaul STA keep sta_band as ALL because, the bands are checked in
	...
	if (I5_CLIENT_IS_BSTA(sta)) {
		...
		if ((i5_sta_ifr = i5DmFindInterfaceFromNetwork(args->sta_mac)) == NULL) {
			WBD_WARNING("bSTA "MACDBG" is not an interface in network. Will not "
				"steer him\n", MAC2STRDBG(args->sta_mac));
			goto end;
		}
		sta_band = ((i5_dm_device_type*)WBD_I5LL_PARENT(i5_sta_ifr))->bsta_band;
	} else {
		parent_ifr = (i5_dm_interface_type*)WBD_I5LL_PARENT(sbss);

		sta_band = ieee1905_get_band_from_channel(parent_ifr->opClass,
			wf_chspec_ctlchan(parent_ifr->chanspec));
		...
		if (prbsta) {
			sta_band |= prbsta->band;
		} else {
			WBD_WARNING("Warning: probe list has no STA["MACDBG"]\n",
					MAC2STRDBG(sta->mac));
		}
		/* Making following assumptions here.
		 * 1. If the STA supports 5G assume that it supports 2G also
		 * 2. If the STA supports 6G assume that it supports both 5G and 2G also
		 * This is becasue there is no reliable way to find out the bands a STA supports
		 */
		if (WBD_BAND_TYPE_LAN_6G(sta_band)) {
			sta_band = WBD_BAND_LAN_ALL;
		} else if (WBD_BAND_TYPE_LAN_5G(sta_band)) {
			sta_band = WBD_BAND_LAN_2G5G;
		}
	}

	tbss = wbd_master_find_target_bss(master_info, sta, sta_band, i5_sta_ifr);
	...
	if (tbss == NULL || tbss == sbss) {
		...
		if (WBD_ASSOC_ITEM_IS_BH_OPT(assoc_sta->flags)) {
			...
			WBD_INFO("For BHSTA["MACDBG"]. There Is no tbss in backhaul optimization. "
				"TotalTry[%d] MaxTry[%d]\n",
		...
		WBD_DEBUG("There Is no tbss sending link metric req again \n");
	...
	if (is_backhaul) {
		...
		if (free_timer && master_info) {
			...
			if (WBD_MINFO_IS_BH_OPT_RUNNING(master_info->flags)) {
				WBD_DEBUG("BH STA optimization is running mflag %x aflag %x\n",
				 ...
				 * backhaul optimization
				 */
				WBD_DEBUG("Restart the backhaul optimization as backhaul sta "
 ...
 */
int
wbd_master_is_bsta_steering_possible(i5_dm_clients_type *i5_sta, i5_dm_interface_type *i5_sta_ifr)
{
	int ret = WBDE_OK, is_possible = 0;
	i5_dm_network_topology_type *i5_topology;
	i5_dm_device_type *i5_parent_device, *i5_device, *i5_self_device, *i5_sta_ifr_device;
	i5_dm_interface_type *i5_self_ifr;
	i5_dm_bss_type *i5_self_bss;
	unsigned char inv_bh_bss_band, inv_bsta_band;
	WBD_ENTER();
	...
	foreach_i5glist_item(i5_device, i5_dm_device_type, i5_topology->device_list) {
		...
		/* Loop for only for Agent Devices with backhaul BSS */
		if (!I5_IS_MULTIAP_AGENT(i5_device->flags) || !I5_HAS_BH_BSS(i5_device->flags)) {
			WBD_DEBUG("Device "MACDBG" is %san agent. It %sbackhaul BSS. Skipping\n",
				MAC2STRDBG(i5_device->DeviceId),
		...
		/* Get the backhaul BSS band other than where the STA is associated.
		 * This will give the information of other backhaul BSS in the same device
		 */
		inv_bh_bss_band = (unsigned char)(i5_device->bh_bss_band & ~i5_self_ifr->band);
		/* Get the backhaul STA band other than the STA interface.
		 * This will give the information of other backhaul STA in the same device
		 */
		inv_bsta_band = (unsigned char)(i5_sta_ifr_device->bsta_band & ~i5_sta_ifr->band);

		/* skip the self device if there is no backhaul BSS on the other band for the
		 * other BSTA in another band
		 */
		if (i5_self_device == i5_device) {
			/* BSTA in other band can be steered to backhaul BSS on that band */
			if (inv_bsta_band & inv_bh_bss_band) {
				is_possible = 1;
				WBD_INFO("Device "MACDBG" is a possible TBSS candidate for "
					"bSTA "MACDBG"\n",
					MAC2STRDBG(i5_device->DeviceId), MAC2STRDBG(i5_sta->mac));
				goto end;
			} else {
				WBD_DEBUG("bSTA "MACDBG" is sassociated to "MACDBG" it. So not a "
					"candidate for TBSS. Skipping\n",
					MAC2STRDBG(i5_sta->mac), MAC2STRDBG(i5_device->DeviceId));
				continue;
			}
		}
...
{
	int ret = WBDE_OK, b_bss_count, bsta_count, agent_bh_bss_count;
	...
	if (wbd_master_is_tbss_created_for_bsta()) {
		WBD_INFO("Backhaul steering in progress. Skipping backhaul optimization\n");
	...
	/* Count the number of backhaul BSS in the network */
...
find_next_sta:
	if (bsta_count <= 0) {
		WBD_INFO("Tried optimization for all backhaul STAs once. Exit\n");
		goto end;
	}
	bsta_count--;
	 ...
	 */
	if (!i5_sta) {
		i5_sta = wbd_ds_get_bh_opt_pending_sta(sta_mac);
	}
	...
	if (!wbd_master_is_bsta_steering_possible(i5_sta, i5_sta_ifr)) {
	...
	/* If the device with STA interface has the backhaul BSS, reduce the backhaul BSS count */
	agent_bh_bss_count = i5DmCountNumOfBSS(WBD_I5LL_PARENT(i5_sta_ifr),
		IEEE1905_MAP_FLAG_BACKHAUL);
	if ((b_bss_count - agent_bh_bss_count) <= 1) {
		WBD_INFO("Number of backhaul BSS where bSTA "MACDBG" can be connected is %d."
			"Cannot do further optimization. Skip it\n",
			MAC2STRDBG(i5_sta->mac), (b_bss_count - agent_bh_bss_count));
		goto end;
	}
	 ...
	 */
	ret = wbd_ds_add_sta_in_peer_devices_monitorlist(master_info, i5_sta);
	if ((ret != WBDE_OK) && (ret != WBDE_DS_STA_EXST)) {
		WBD_INFO("Failed to add STA["MACDBG"] to peer slaves monitorlist\n",
			MAC2STRDBG(i5_sta->mac));
	}
