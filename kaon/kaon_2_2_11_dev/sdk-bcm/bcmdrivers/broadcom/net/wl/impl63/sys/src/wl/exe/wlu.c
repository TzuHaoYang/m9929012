...
cmd_t wl_cmds[] = {
	...
	"Set current or configured 2G/5G channel:\n"
	...
	"\tprimary channel number {1, 5, 9, 13 .. 233} for 20MHz,\n"
...
wl_mutx_ac(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((mutype = *++argv) == NULL) {
		...
		if (reply->mask == 0)
		{
			...
			if (reply->mask & (1 << (WL_MU_VHTMU - 1)))
				printf(" vhtmu");
			if (reply->mask & (1 << (WL_MU_DLOFDMA - 1)))
				printf(" heofdma");
			if (reply->mask & (1 << (WL_MU_HEMMU - 1)))
				printf(" hemu");
		}
		printf("\n");
	} else {
...
wl_fifo_cmd(void *wl, cmd_t *cmd, char **argv)
{
	...
	char *endptr = NULL;
	int8 value;
	char txTypeStr[WL_MU_TX_TYPE_MAX][10] = {"vhtmu", "hemmu", "ofdma", "twt"};
	uint8 i;
	...
	} else if (strcmp(*argv, "vhtmu") == 0)
		mutype = WL_MU_VHTMU;
	else if (strcmp(*argv, "hemmu") == 0)
		mutype = WL_MU_HEMMU;
