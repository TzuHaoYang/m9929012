...
#define CONF_SERVER	"http://c.speedtest.net/speedtest-config.php?x=5p33dy"
#define STATIC_SERVER	"http://c.speedtest.net/speedtest-servers-static.php?x=5p33dy"
...
get_lowest_latency_server(server_config_t *servers, server_config_t *best_server)
{
	...
	for (i = 0; i < CLOSEST_SERVERS_NUM; i++) {
		...
		for (j = 0; j < 3; j++) {
			...
			eval("curl", "--connect-timeout", "5", "--max-time", "5", "-L",
				"-s", "-o", "/tmp/latency.txt", url);
