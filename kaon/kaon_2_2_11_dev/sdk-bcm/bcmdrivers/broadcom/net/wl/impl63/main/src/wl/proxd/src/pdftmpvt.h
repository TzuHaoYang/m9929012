...
#define FTM_TSF_SYNC_SESSION(_sn) (\
	((_sn)->ftm->ftm_cmn->config->flags & WL_PROXD_FLAG_NO_TSF_SYNC) == 0)
#else
...
#define FTM_ERR(_args) do { \
	if (EVENT_LOG_IS_LOG_ON(EVENT_LOG_TAG_PROXD_ERROR)) \
		EVENT_LOG_COMPACT_CAST_PAREN_ARGS(EVENT_LOG_TAG_PROXD_ERROR, _args); \
	} while (0)
...
#define FTM_LOGNAN(_ftm, _args) FTM_PRINTF _args
...
#define FTM_BSSCFG(_ftm) (_ftm)->wlc->cfg
...
#define FTM_BSSCFG_SECURE(_ftm, _bsscfg) isset((_ftm)->ftm_cmn->enabled_bss, \
