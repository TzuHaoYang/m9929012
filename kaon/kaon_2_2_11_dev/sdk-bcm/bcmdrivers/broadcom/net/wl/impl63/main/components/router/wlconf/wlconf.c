 ...
 *
 * $Id: wlconf.c 782984 2020-01-10 05:16:18Z $
 */
		...
		if (amsdu_valid_option) {
			if (amsdu_option_val != OFF) { /* AMPDU (above) has priority over AMSDU */
				if (rev.corerev >= 40) {
					...
					if (rev.corerev >= 129) {
						WL_IOVAR_SETINT(name, "amsdu_aggsf", 4);
					}
...
wlconf(char *name)
{
	...
	/* If not defined or the profile is less than 1, set to profile-2. Accepting profile-1 to
