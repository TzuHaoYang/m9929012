...
if [ "$dev_wan" == "eth0" -o "$dev_wan" == "ppp0" -o "$dev_wan" == "eth0.1" -o "$dev_wan" == "ppp0.1" ]; then
	udb_param="dev_wan=eth0,ppp0"
else
	udb_param="dev_wan=eth0,ppp0,$dev_wan"
fi
...
fwdev_maj=191
fwdev_min=0
iptable_devwanlist="eth0 eth0.1 ppp0 ppp0.1"
indevlist=0
...
start)
	...
	for iptable_devwan in $iptable_devwanlist
	do
		if [ "$iptable_devwan" == "$dev_wan" ]; then
			indevlist=1
		fi
		iptables -t mangle -A $chain -i $iptable_devwan -p udp --sport 68 --dport 67 -j DROP
		iptables -t mangle -A $chain -i $iptable_devwan -p udp --sport 67 --dport 68 -j DROP
		iptables -t mangle -A PREROUTING -i $iptable_devwan -p udp -j $chain
	done
	if [ "$indevlist" == 0 ]; then
