...
#ifdef DSLCPE
static int acs_dfsr_counter_update(dfsr_context_t *ctx, char *if_name, void *cc_info);
#else
...
static dfsr_window_params_t default_params[W_COUNT] = {
	{ seconds:5*60, threshold:3, interval:1 },	/* Immediate: > 3 switches in last 5 minutes */
	{ seconds:7*60*60*24, threshold:5, interval:1 },/* Deferred: > 5 switches in last 7 days */
	{ seconds:30, threshold:10*1024, interval:1 },	/* chan idle if < 100kB of RX/TX in last 30 secs */
...
dfsr_reentry_type_t
#ifdef DSLCPE_ENDIAN
acs_dfsr_chanspec_update(dfsr_context_t *ctx, chanspec_t channel, const char *caller, char *if_name, void *cc_info)
#else
...
#ifdef DSLCPE
			if (acs_dfsr_counter_update(ctx, if_name, cc_info) != BCME_OK) {
#else
...
static int
#ifdef DSLCPE_ENDIAN
acs_dfsr_counter_update(dfsr_context_t *ctx, char *if_name, void  *cc_info)
#else
...
#endif
{
	...
	time_t now = time(0);
#ifdef DSLCPE_ENDIAN
	acs_chaninfo_t *c_info = (acs_chaninfo_t*) cc_info;
#endif
...
#ifdef DSLCPE_ENDIAN
		wlc_cnt->txbyte = dtoh32(wlc_cnt->txbyte);
		wlc_cnt->rxbyte = dtoh32(wlc_cnt->rxbyte);
#endif
...
dfsr_reentry_type_t
#ifdef DSLCPE_ENDIAN
acs_dfsr_activity_update(dfsr_context_t *ctx, char *if_name, void  *cc_info)
#else
