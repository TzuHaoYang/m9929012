 ...
 *
 * $Id: acsd_cmd.c 736682 2017-12-18 05:55:04Z $
 */
...
acs_policy_name(acs_policy_index i)
{
	switch (i) {
		...
		case ACS_POLICY_TXOP:		return "TXOP";
...
acsd_proc_cmd(acsd_wksp_t* d_info, char* buf, uint rcount, uint* r_size)
{
	...
	*c++ = '\0';
	...
	if (!strcmp(buf, "info")) {
		time_t ltime;
		int i;
		const char *mode_str[] = {"disabled", "monitor", "auto", "coex", "11h", "fixchspec"};
	...
	if (!strcmp(buf, "csscan") || (!strcmp(buf, "autochannel"))) {
		...
		if (pick) {
			if(ACS_FCS_MODE(c_info)) {
				...
				c_info->switch_reason = APCS_IOCTL;
	...
	if (!strcmp(buf, "get")) {
		...
		if (!strcmp(param, "mode")) {
			const char *mode_str[] = {"disabled", "monitor", "select", "coex", "11h", "fixchspec"};
		...
		if (!strcmp(param, "acs_far_sta_rssi")) {
			*r_size = sprintf(buf, "%d", c_info->acs_fcs.acs_far_sta_rssi);
			goto done;
		}
...
#ifdef DSLCPE
		if (!strcmp(param, "acs_suspend")) {
			*r_size = sprintf(buf, "Acsd is %s.", g_acsd_suspended ?  "Suspended" : "running");
			goto done;
		}
#endif
		CHANIM_GET(max_acs, "");
	...
	if (!strcmp(buf, "set")) {
		...
		if (!strcmp(param, "acs_far_sta_rssi")) {
			c_info->acs_fcs.acs_far_sta_rssi = setval;
			*r_size = sprintf(buf, "%d", c_info->acs_fcs.acs_far_sta_rssi);
			goto done;
		}
		if (!strcmp(param, "acs_nofcs_least_rssi")) {
