...
#ifdef DSLCPE_ENDIAN
static void pktq_log_counters_endian_convert(uint32 *item, int v4) {

	int count;
	int i=0;
	if(v4)
		count=sizeof(pktq_log_counters_v04_t)/sizeof(uint32);
	else
		count=sizeof(pktq_log_counters_v05_t)/sizeof(uint32);
	for(i=0;i<count;i++) {
		*item=dtoh32(*item);
		item++;
	}
	...
	for (prec = 0; prec < num_prec; prec++) {
		...
		if (iov_pktq_log->version == 5) {
...
#ifdef DSLCPE_ENDIAN
			pktq_log_counters_endian_convert((uint32 *)&counters,0);
#endif
		} else {
			...
			*/
			...
			counters.airtime = 0;
#ifdef DSLCPE_ENDIAN
			pktq_log_counters_endian_convert((uint32 *)&counters,1);
#endif
		}
			...
			if (is_aqm && (prec_mask & (1 << (prec + 1)))) {
				...
				if (iov_pktq_log->version == 5) {
...
#ifdef DSLCPE_ENDIAN
					pktq_log_counters_endian_convert((uint32 *)&hi,0);
#endif
				} else {
					...
					memcpy(&hi, &logv04->counters[index][prec + 1],
						sizeof(pktq_log_counters_v04_t));
#ifdef DSLCPE_ENDIAN
					pktq_log_counters_endian_convert((uint32 *)&hi,1);
#endif
				}
