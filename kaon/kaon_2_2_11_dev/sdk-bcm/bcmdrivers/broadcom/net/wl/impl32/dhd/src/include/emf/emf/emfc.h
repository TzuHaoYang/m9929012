...
typedef struct emfc_mgrp
{
	...
	union {
		...
		struct inv6_addr         mgrp_ipv6;       /* Multicast Group IP Address */
	};
#else
	uint32           mgrp_ip;       /* Multicast Group IP Address */
#endif
...
typedef struct emfc_info
{
	...
	struct inv6_addr	 mgrp_cache_ipv6_addr;  /* ipv6 cached address */
...
#endif
#ifdef DSLCPE  
#define MFDB_MGRP_HASH_IPV6(m)       ((((m).s6_addr32[0])+((m).s6_addr32[1])+((m).s6_addr32[2])+((m).s6_addr32[3])) &7)
