 ...
 *
 * $Id: acsd_chanim.c 769414 2018-11-14 05:20:56Z $
 */
...
#ifdef DSLCPE_ENDIAN
static void
acsd_dtoh_chanim(wl_chanim_stats_t* chanim_stats,acs_chaninfo_t *c_info) {

	int count = chanim_stats->count;
	int i;
	...
	for (i = 0; i < count; i++) {
		...
		stats->bphy_glitchcnt = dtoh32(stats->bphy_glitchcnt);
		stats->bphy_badplcp = dtoh32(stats->bphy_badplcp);
...
static void
#ifdef DSLCPE_ENDIAN
acsd_dump_chanim(wl_chanim_stats_t* chanim_stats,acs_chaninfo_t* c_info) 
#else
...
#ifdef DSLCPE_ENDIAN
		printf("%d\t%d\t%d\t%d", (stats->glitchcnt), (stats->badplcp),
...
static void
#ifdef DSLCPE_ENDIAN
acsd_display_chanim(acs_chaninfo_t *c_info,chanim_stats_t* stats)
#else
...
acsd_update_chanim(acs_chaninfo_t * c_info, chanim_stats_t * stats, uint ticks)
{
	...
	if (acsd_debug_level & ACSD_DEBUG_CHANIM)
#ifdef DSLCPE_ENDIAN
		acsd_display_chanim(c_info, &tmp);
#else
	...
	c_info->txop_score = stats->ccastats[CCASTATS_TXOP] +
...
acsd_chanim_query(acs_chaninfo_t * c_info, uint32 count, uint32 ticks)
{
	...
	if (list->buflen == 0) {
		...
		fprintf(stderr, "Err : your driver has wl_chanim_stats version %d "
...
#ifdef DSLCPE_ENDIAN
		acsd_dtoh_chanim(c_info->chanim_stats, c_info);
#endif
#ifdef DEBUG
#ifdef DSLCPE
		acsd_dump_chanim(c_info->chanim_stats, c_info);
#else
