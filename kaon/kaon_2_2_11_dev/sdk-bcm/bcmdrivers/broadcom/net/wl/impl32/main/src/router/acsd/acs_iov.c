 ...
 *
 *	$Id: acs_iov.c 766654 2018-08-09 06:57:25Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{
#ifdef DSLCPE_ENDIAN
	sub_chspec=htodenum(sub_chspec);
#endif
...
{
#ifdef DSLCPE_ENDIAN
	return wl_iovar_setint(c_info->name, "chanspec", (chspec));
#else
	return wl_iovar_setint(c_info->name, "chanspec", htod32(chspec));
#endif
}

int acs_set_pref_chanspec(acs_chaninfo_t *c_info, chanspec_t prefchspec)
{
	return wl_iovar_setint(c_info->name, "pref_chanspec", 0);
...
#ifdef DSLCPE_ENDIAN
static void _dfs_force_t_endian_convert(acs_chaninfo_t *c_info,wl_dfs_forced_t *inp,int dtoh) {

	int i=0;
	inp->chspec=dtohenum(inp->chspec);
	inp->version=dtoh16(inp->version);
	if(dtoh)
		inp->chspec_list.num=dtoh32(inp->chspec_list.num);
	for(;i<inp->chspec_list.num;i++)
	  inp->chspec_list.list[i]=dtohenum(inp->chspec_list.list[i]);
	if(!dtoh)
		inp->chspec_list.num=dtoh32(inp->chspec_list.num);
}
#endif
...
#ifdef DSLCPE_ENDIAN
	_dfs_force_t_endian_convert(c_info,(wl_dfs_forced_t *)smbuf,1);
#endif
	return ret;
}
...
{
#ifdef DSLCPE_ENDIAN
	_dfs_force_t_endian_convert(c_info,dfs_frcd,0);
#endif
...
#ifdef DSLCPE_ENDIAN
		c_info->country_is_edcrs_eu = is_edcrs_eu;
#else
...
int dcs_handle_request(char* ifname, wl_bcmdcs_data_t *dcs_data,
	uint8 mode, uint8 count, uint8 csa_mode, acs_chaninfo_t *c_info)
#else
int dcs_handle_request(char* ifname, wl_bcmdcs_data_t *dcs_data,
...
#endif
{
	...
	csa.count = count;
#ifdef DSLCPE_ENDIAN
	csa.chspec = htod16(dcs_data->chspec);
#else
