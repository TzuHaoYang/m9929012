 ...
 *
 * $Id: wbd_ds.c 773157 2019-03-14 05:21:34Z $
 */
...
wbd_ds_blanket_master_cleanup(wbd_info_t *info)
{
	...
	foreach_safe_glist_item(master_item_p, info->wbd_master->blanket_list, master_next_p) {
		...
		wbd_ds_master_info_cleanup(master_info, FALSE);
...
wbd_ds_master_info_cleanup(wbd_master_info_t *master_info, bool free_master_mem)
{
	...
	foreach_safe_glist_item(slave_item_p, master_info->slave_list, slave_next_p) {
		...
		wbd_ds_slave_item_cleanup(slave_item, FALSE);
	...
	/* In the end, Remove Master Info itself */
	if (free_master_mem) {
		free(master_info);
		master_info = NULL;
	}
...
wbd_ds_slave_item_cleanup(wbd_slave_item_t *slave_item, bool free_slave_mem)
{
	...
	/* In the end, Remove Slave itself */
	if (free_slave_mem) {
		free(slave_item);
		slave_item = NULL;
	}
	...
	struct timeval *out_assoc_time)
{
	...
	foreach_i5glist_item(device, i5_dm_device_type, i5_topology->device_list) {
		...
		if ((sta = i5DmFindClientInDevice(device, (unsigned char *)sta_mac)) != NULL) {
			...
			if (assoc_sta && out_assoc_time &&
				/* CSTYLED */
				(timercmp(&(assoc_sta->assoc_time), &assoc_time, >))) {
	...
	i5_dm_clients_type *i5_assoc_sta, uint8 map_flags)
{
	...
	while (i5_iter_device != NULL) {
		...
		i5_iter_ifr = (i5_dm_interface_type *)WBD_I5LL_HEAD(i5_iter_device->interface_list);
		while (i5_iter_ifr != NULL) {
			/* For all the BSS in this Interface */
			i5_iter_bss = (i5_dm_bss_type *)WBD_I5LL_HEAD(i5_iter_ifr->bss_list);
			while (i5_iter_bss != NULL) {
				...
				if (!(i5_iter_bss->mapFlags & map_flags)) {
					i5_iter_bss = WBD_I5LL_NEXT(i5_iter_bss);
					continue;
				}
			...
			i5_iter_ifr = WBD_I5LL_NEXT(i5_iter_ifr);
		}
		i5_iter_device = WBD_I5LL_NEXT(i5_iter_device);
	}
	...
	for (key = 0; key < WBD_BOUNCE_HASH_SIZE; key++) {
		...
		while (entry) {
			...
			if (entry->state == WBD_BOUNCE_DWELL_STATE) {
				...
				if (entry->run.dwell_time > wbd_master->bounce_cfg.dwell_time) {
				...
				if (entry->run.window < wbd_master->bounce_cfg.window) {
					/* Within window state, check the steered count */
					if (entry->run.cnt >= wbd_master->bounce_cfg.cnt) {
						/* Move to dwell state */
						entry->timestamp = now;
...
wbd_ds_is_fbt_possible_on_agent()
{
	...
	foreach_i5glist_item(iter_ifr, i5_dm_interface_type, self_device->interface_list) {
		...
		foreach_i5glist_item(iter_bss, i5_dm_bss_type, iter_ifr->bss_list) {
			...
			WBD_INFO("Check FBT Possible on Device["MACDBG"]: BSS[%s] Prefix[%s]\n",
				MAC2STRDBG(ieee1905_get_al_mac()), iter_bss->ifname, prefix);

			/* If FBT enabled then only set the FBT Parameters for this BSS */
