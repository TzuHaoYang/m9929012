 ...
 *
 * $Id: zerowaitdfs.js 682076 2017-01-31 10:13:15Z spalanga $
 */
...
function initZWDPage()
{
	if (isHTTPDWebserver == 1) {
		...
		g_urlGetAPStatus = window.location.protocol + '//localhost/zerowaitdfs.php';
	}
}
...
function parseIsZWDSupportedResp(array)
{
	...
	/* DSLCPE added BLOCK **/
	if (g_isZWDSupported == 0) {
		$("#message_panel").text("No DFS interface found...");
		$("#message_panel").attr("class","message_error");
	} else {
		$("#message_panel").attr("class","message_ok");
		$("#message_panel").text("on interface:"+g_wlName);
	}
	/***End of DSLCPE added Block ***/
