 ...
 *
 * $Id: acsd_main.c 767644 2018-09-20 04:50:39Z $
 */
...
#ifdef DSLCPE
    /* if acsd is suspend, then return after handle client commands */
    if (g_acsd_suspended)
        return;
#endif
	/* handle brcm event */
			...
			case WLC_E_TXFAIL_THRESH:
				{
					...
					if (c_info->acs_fcs.acs_ignore_txfail_on_far_sta) {
						if (c_info->sta_status & ACS_STA_EXIST_FAR) {
							ACSD_FCS(" intf: %s Ignoring txfail for the far "
								"sta %d\n", c_info->name,
								c_info->sta_status);
							break;
						}
...
acsd_watchdog(uint ticks)
{
	...
	for (i = 0; i < ACS_MAX_IF_NUM; i++) {
		...
		if (!acs_is_mode_check(c_info->name)) {
			ACSD_INFO("do not perform scan or no channel change,"
		...
		if ( c_info->txop_channel_select && (-- c_info->txop_channel_select == 0)) {
			...
			if((c_info->timestamp != c_info->cur_timestamp) &&
				...
				if(c_info->cur_chspec != c_info->selected_chspec) {
					...
					ACSD_PRINT("selected_chspec is %x\n",
						c_info->selected_chspec);
				}
			} else {
...
#ifdef DSLCPE
		if (!g_acsd_suspended && (tv.tv_sec == 0 && tv.tv_usec == 0)) {
#else
