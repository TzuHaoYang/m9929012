...
wl_chanim_stats_us_print(void *ptr, uint32 req_count, int version)
{
	...
	if (version < WL_CHANIM_STATS_VERSION_V3) {
	...
	if ((count == 1) && ((req_count == WL_CHANIM_COUNT_US_ONE) ||
		...
		if (version < WL_CHANIM_STATS_VERSION_V3) {
			...
			printf("%-8s  %-8s  %-8s  %-8s  %-8s  %-8s  %-8s  %-8s  %-8s\n",
					...
					"pri20", "sec20", "sec40");
			printf("0x%-6x  ", stats_us->chanspec);
			printf("%-8u  %-8u  %-8u  %-8u  %-8u  %-8u  %-8u  %-8u\n",
					stats_us->tx_tm,
					stats_us->rx_bss, stats_us->rx_obss,
					stats_us->total_tm, stats_us->busy_tm,
					stats_us->rxcrs_pri20, stats_us->rxcrs_sec20,
					stats_us->rxcrs_sec40);
		}
		else {
			unsigned int i;
			...
			printf("%-8s  %-8s  %-8s  %-8s  %-8s  %-8s  %-8s  %-8s  %-8s"
					...
					"pri20", "sec20", "sec40");
			printf("0x%-6x  ", stats_us_v1->chanspec);

			for (i = 0; i < CCASTATS_MAX; i++) {
				printf("%-8u  ", stats_us_v1->ccastats_us[i]);
			}
			printf("%-8u  %-8u  %-8u  %-8u  %-8u\n",
					stats_us_v1->total_tm,
					stats_us_v1->busy_tm,
					stats_us_v1->rxcrs_pri20,
					stats_us_v1->rxcrs_sec20,
					stats_us_v1->rxcrs_sec40);
		}
		...
		if (version <  WL_CHANIM_STATS_VERSION_V3) {
			...
			for (i = 0; i < count; i++) {
				printf("%-10s  %-10s  %-10s  %-10s  %-10s  %-10s  %-10s  %-10s"
					...
					"chanspec", "tx", "inbss", "obss", "nocat", "nopkt",
					"doze", "txop", "goodtx", "badtx",  "total_tm", "busy_tm");
				printf("0x%-8x  ", list_v1->stats_us_v1[i].chanspec);

				for (j = 0; j < CCASTATS_MAX; j++) {
					printf("%-10u  ", list_v1->stats_us_v1[i].ccastats_us[j]);
				}
				printf("%-10u  %-10u\n", list_v1->stats_us_v1[i].total_tm,
						list_v1->stats_us_v1[i].rxcrs_pri20);
			}
		}
	}
	printf("\n");
...
static int wl_chanim_stats_us_dur(void *wl, cmd_t *cmd, char **argv, int version)
{
	...
	param_us.buflen = htod32(sizeof(wl_chanim_stats_us_v1_t));
	if ((err = wlu_var_getbuf(wl, cmd->name, &param_us, sizeof(wl_chanim_stats_us_v1_t),
			&ptr)) < 0) {
	...
	if (*argv != NULL) {
		if (!strcmp(*argv, "us")) {
			...
			if (*argv != NULL) {
				...
				param.buflen = htod32(sizeof(wl_chanim_stats_us_v1_t));
			}
			if ((err = wlu_var_getbuf(wl, cmd->name, &param,
					sizeof(wl_chanim_stats_us_v1_t), &ptr)) < 0) {
				printf("failed to get chanim results\n");
				return err;
			}
