 ...
 *
 * $Id: wbd_ds.h 773157 2019-03-14 05:21:34Z $
 */
...
#define WBD_TBSS_MIN_STA_THLD		1

/* Maximum phyrate boundary above which the normalized value is 100 */
...
#define WBD_STA_METRICS_REPORTING_RSSI_THLD			-60
...
#define WBD_BH_METRICS_REPORTING_RSSI_THLD			-65
...
typedef struct wbd_tbss_ident_info {
	...
	int tbss_wght;			/* Target BSS finding weightage index */
	int tbss_thld_2g;		/* 2G Target BSS finding threshold index */
	int tbss_thld_5g;		/* 5G Target BSS finding threshold index */
	int tbss_algo;			/* Target BSS finding algorithm index */
	int tbss_stacnt_thld;		/* Threshold for minimum STA count in SoF */
	int tbss_min_phyrate;		/* Minimum phyrate boundary below which the normalized
					 * value is 0
					 */
	int tbss_max_phyrate;		/* Maximum phyrate boundary above which the normalized
					 * value is 100
					 */
...
typedef struct wbd_chan_info {
	short int channel;	/* Channel number */
	unsigned int bitmap;	/* Channel parameters i.e. radar sensitive or not */
} wbd_chan_info_t;

typedef struct wbd_interface_chan_info {
	int count;
	wbd_chan_info_t chinfo[WBD_MAXCHANNEL];
} wbd_interface_chan_info_t;
...
struct wbd_slave_item {
	...
	wbd_taf_params_t *taf_info;	/* list of sta with prio and type to share across
					 ...
					 */
	bcm_stamon_config_t stamon_config;	/* Stamon configuration info */
};
...
struct wbd_blanket_master {
	...
	uint8 max_bh_opt_try;		/* Number of TBSS retries for STA in backhaul optimization */
