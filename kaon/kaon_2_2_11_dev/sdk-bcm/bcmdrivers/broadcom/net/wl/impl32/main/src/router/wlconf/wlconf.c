 ...
 *
 * $Id: wlconf.c 782540 2019-12-23 08:36:03Z $
 */
...
wlconf(char *name)
{
	...
	WL_IOVAR_XTLV_SETINT(name, "mbo", atoi(nvram_safe_get(strcat_r(prefix, "mbo_enable", tmp))),
		WL_MBO_IOV_VERSION, WL_MBO_CMD_AP_ENAB, WL_MBO_XTLV_AP_ENAB);
...
#ifdef __CONFIG_DHDAP__
#if defined(__CONFIG_LBR_AGGR__) || defined(DSLCPE)
	/* Check if interface uses dhd adapter */
	is_dhd = !dhd_probe(name);
