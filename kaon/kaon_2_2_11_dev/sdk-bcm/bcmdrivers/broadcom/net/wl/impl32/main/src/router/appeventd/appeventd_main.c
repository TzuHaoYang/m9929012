...
#ifdef BCM_WBD
/* Parse the weak client event data received from wbd. */
static void
appeventd_parse_wbd_weak_client_data(char *pkt, int len)
{
	...
	while (len >= ulen) {
		...
		APPEVENTD_INFO("WBD-Weak Sta: Ifname = %s STA MAC = "MACF" RSSI = %d"
			" Tx-Failures = %d Tx-Rate = %f\n", weak_sta->ifname,
			ETHER_TO_MACF(weak_sta->sta_addr), weak_sta->rssi,
		...
		if ((bytes = recv(sock, pkt, APPEVENTD_BUFSIZE, 0)) > sizeof(app_event_h)) {
			...
			switch (app_event->h.event_id) {
				...
				case APP_E_WBD_SLAVE_WEAK_CLIENT:
					...
					appeventd_parse_wbd_weak_client_data(pkt, len);
					break;
