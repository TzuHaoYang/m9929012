 ...
 *
 * $Id: wlu_linux.c 677493 2017-01-03 12:13:05Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
#define INTERACTIVE_NUM_ARGS			31
...
int wl_get(void *wl, int cmd, void *buf, int len);
...
wl_find(struct ifreq *ifr)
{
...
#ifdef DSLCPE
	int status=BCME_ERROR,wl_index= 1024,cur_index;
#else
	int status;
	int unit;
	void *ptr;
	...
	while (fgets(buf, sizeof(buf), fp)) {
		...
		if (!(name = strsep(&c, ":")) ||
		 (strstr(name, ".")))
			continue;
		/* only check primary interface */
		if ((sscanf(name, "eth%d", &cur_index) > 0) ||
			(sscanf(name, "wl%d", &cur_index) > 0)) {
#else
		if (!(name = strsep(&c, ":")))
			continue;
		strncpy(ifr->ifr_name, name, IFNAMSIZ-1);
		...
		if (wl_get_dev_type(name, dev_type, DEV_TYPE_LEN) >= 0 &&
...
#ifdef DSLCPE
			if (wl_check((void *) name) == 0 && (cur_index < wl_index)) {
					wl_index = cur_index;
					strncpy(ifr->ifr_name, name, IFNAMSIZ-1);
					ifr->ifr_name[IFNAMSIZ-1] = 0;
					status = BCME_OK;
			...
			/* find wl0.x */
			if (wl_get((void *)ifr, WLC_GET_INSTANCE, &unit, sizeof(unit)) < 0)
				break;
			if (unit != 0)
				goto not_found;

			/* find wl0.0 */
			if (wlu_var_getbuf((void *)ifr, "bsscfg_idx", NULL, 0, &ptr) < 0)
				break;
			bssidx = *(int *)ptr;
			if (bssidx != 0)
				goto not_found;
