 ...
 *
 * $Id: wbd_tlv.c 771746 2019-02-07 04:14:47Z $
 */
...
wbd_tlv_decode_bss_capability_report(void* data, unsigned char* tlv_data, unsigned int tlv_data_len)
{
	...
	for (idx = 0; idx < bss_count; idx++) {
		...
		i5_bss->mapFlags = 0;
		if (map_flags & IEEE1905_FRONTHAUL_BSS) {
			i5_bss->mapFlags |= IEEE1905_MAP_FLAG_FRONTHAUL;
		}
		if (map_flags & IEEE1905_BACKHAUL_BSS) {
			i5_bss->mapFlags |= IEEE1905_MAP_FLAG_BACKHAUL;
		}
		if (map_flags & IEEE1905_BACKHAUL_STA) {
			i5_bss->mapFlags |= IEEE1905_MAP_FLAG_STA;
		}
	...
	unsigned char* tlv_data, unsigned int tlv_data_len)
{
	...
	if (tlv_data_len < WBD_MAX_LEN_TLV_REMOVE_CLIENT_REQ) {
		WBD_WARNING("tlv_data_len[%d] < Remove Client Req Len[%d]\n",
			tlv_data_len, WBD_MAX_LEN_TLV_REMOVE_CLIENT_REQ);
		ret = WBDE_TLV_ERROR;
		goto end;
	}
