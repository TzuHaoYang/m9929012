 ...
 *
 * $Id: toad.c 766354 2018-07-31 08:57:42Z $
 *
 */
 ...
 */
#ifdef DSLCPE
static char default_to_wl_ifname[] = "wl0";
#else
...
#ifdef DSLCPE_ENDIAN
		wl_endian_probe(toad_main_ifname(tc));
#endif	
		td.version = htod16(1);
		td.ea = sta->ea;
...
toad_station_list_arrange(toad_context *tc)
{
	...
	if (tc->ordering && toad_station_count(tc)) {
		...
		TOAD_TRACE(tc, ("sorting mbase %p[%zu] sz %zu\n", mbase, mcount, msize));
		...
		TOAD_TRACE(tc, ("re-prioritizing %zu stations\n", mcount));
