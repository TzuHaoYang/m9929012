 ...
 *
 * $Id: wlu.c 780844 2019-11-05 03:11:29Z $
 */
...
static cmd_func_t wl_rand, wl_counters, wl_clear_counters, wl_if_counters;
...
cmd_t wl_cmds[] = {
	...
	"\t  -maxsed: Configure the min SED (in %). SED lower than this will trigger threshold change (if feature is enabled)\n"
...
#ifdef DSLCPE_ENDIAN
	ra.ch = wl_chspec_from_driver(ra.ch);
	ra.count = dtoh32(ra.count);
	ra.radartype = dtoh32(ra.radartype);
	ra.timenow = dtoh32(ra.timenow);
	ra.timefromL = dtoh32(ra.timefromL);
	ra.lp_csect_single = dtoh32(ra.lp_csect_single);
	ra.detected_pulse_index = dtoh32(ra.detected_pulse_index);
	ra.nconsecq_pulses = dtoh32(ra.nconsecq_pulses);
	for (i = 0; i < 10; i++) {
		ra.intv[i] = dtoh32(ra.intv[i]);
		ra.pw[i] = dtoh32(ra.pw[i]);
		ra.fm[i] = dtoh32(ra.fm[i]);
	}
#endif
...
wl_sta_info(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (sta->ver > WL_STA_VER) {
		...
		sta_v7->tx_rspec = dtoh32(sta_v7->tx_rspec);
		sta_v7->rx_rspec = dtoh32(sta_v7->rx_rspec);
		sta_v7->wnm_cap = dtoh32(sta_v7->wnm_cap);
	...
	printf("\t flags 0x%x:%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n",
	       sta->flags,
	...
	if (sta->ver >= WL_STA_VER_V7) {
		...
		if (sta_v7->len >= STRUCT_SIZE_THROUGH(sta_v7, he_flags)) {
...
wl_if_counters(void *wl, cmd_t *cmd, char **argv)
{
	...
	pbuf += sprintf(pbuf, "d11_txfrag %u d11_txmulti %u d11_txretry %u d11_txretrie %u\n",
		(uint32)dtoh32(cnt->txfrag), (uint32)dtoh32(cnt->txmulti),
		(uint32)dtoh32(cnt->txretry), (uint32)dtoh32(cnt->txretrie));

	pbuf += sprintf(pbuf, "d11_txfrmsnt %u\n",
		(uint32)dtoh32(cnt->txfrmsnt));

	PRVAL(rxnobuf);	PRVAL(rxfragerr); PRNL();
	PRVAL(rxrunt); PRNL();

	pbuf += sprintf(pbuf, "d11_rxmulti %u \n",
		(uint32)dtoh32(cnt->rxmulti));
	PRNL();
