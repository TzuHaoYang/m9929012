 ...
 *
 * $Id: airiq_app.c 769064 2018-11-02 16:55:45Z $
 */
...
#define MAX_SCAN_CHAN_COUNT 4
typedef struct {
	int count;
	int chan3x3;
	int channels[MAX_SCAN_CHAN_COUNT];
} scan_chan_t;

const scan_chan_t chan_lut_24GHz_40MHz[] = {
	{
		/* 3/40 = 1L/5U */
		.count = 3,
		.chan3x3 = 3,
...
const scan_chan_t *lookup_24GHz_40MHz_scan(int chan)
{
	int i = 0;

	/* 0 terminated list */
	while (chan_lut_24GHz_40MHz[i].count) {
		if (chan_lut_24GHz_40MHz[i].chan3x3 == chan) {
			return &chan_lut_24GHz_40MHz[i];
		}
		i++;
	}
	return NULL;
                     ...
                     void             *EventData)
{
	...
	for (k = 0; k < msg->Count; k++) {
		...
		PRINTF_EVENT(BSA_EVENT_ID_CHAN_UTIL_WITH_INT,
		    "#eventdata:%2d Channel %3d: Non-wifi: %3d%% Wi-fi: %3d%% Total: %3d%%",
		    k, pcu->ChannelNum, pcu->NonWlanUtil / 100,
		    pcu->WlanUtil / 100, pcu->TotalUtil / 100);
...
int chan_overlap(int chan, int chan2, int bw)
{
	int upper, lower;
	/* If scan_chanspec center frequency overlaps with main chanspec,
	 * return true.
	 */
	switch (bw) {
		...
		case WL_CHANSPEC_BW_80:
			...
			break;
		default:
			fprintf(stderr,"%s: Invalid channel %d bw: 0x%x\n",
						__FUNCTION__, chan, bw);
			exit(-1);
	}
	...
	if (chan2 >= lower && chan2 <= upper && chan != chan2) {
		return 1;
	}
...
/*  */
	...
	chanspec_t chanspec3x3      = scanspec->chanspec3x3;
	int        chan3x3;
/*  */
	...
	if (bband) {
		if (bw == WL_CHANSPEC_BW_20) {
			...
			if (phy_mode == PHYMODE_3x3_1x1 && (chan3x3 == 2 || chan3x3 == 3)) {
				/* scan channel 15 -- 2402 MHz */
				pParam->chanspec_list[cnt]       = 15  | WL_CHANSPEC_BW_20 | WL_CHANSPEC_BAND_2G;
				pParam->dwell_interval_ms[cnt]   = shortdwell;
				pParam->capture_interval_us[cnt] = capture_interval;
				pParam->capture_count[cnt]       = shortfftcnt * 2;
				pParam->core_config[cnt]         = core;
			...
			for (i = 1; i < 15; i++) {
				if (phy_mode == PHYMODE_3x3_1x1 && chan_overlap(chan3x3, i, bw)) {
					continue;
				}
			...
			const scan_chan_t *s = lookup_24GHz_40MHz_scan(chan3x3);

			if (! s) {
				fprintf(stderr, "Can't determine scan for channel %d/40\n", chan3x3);
				exit(-1);
			}
			for (i = 0; i < s->count; i++) {
				pParam->chanspec_list[cnt]       = s->channels[i]  | WL_CHANSPEC_BW_40 | WL_CHANSPEC_BAND_2G;
				pParam->dwell_interval_ms[cnt]   = shortdwell;
				pParam->capture_interval_us[cnt] = capture_interval;
				pParam->capture_count[cnt]       = shortfftcnt * 2;
				pParam->core_config[cnt]         = core;
					...
					if (!ret) {
						if (bcmerr == BCME_UNSUPPORTED) {
							...
							fprintf(stderr, "WARNING: %s: scan in progress (BCME_BUSY %d).\n",
							        __FUNCTION__, bcmerr);
						} else if (bcmerr == BCME_BADCHAN) {
							fprintf(stderr, "Channel change detected.\n");
							checkchan = TRUE;
						} else {
...
void StartUserControlledSWSAScanning(int ifcnt, airiq_scan_req_t *scanreqs)
{
	...
	while (1) {
				...
				if (scanreqs[i].phy_mode == PHYMODE_3x3_1x1 && scanreqs[i].core == 3) {
					...
					if (ret) {
						fprintf(stderr, "%s: iovar failed getting 'chanspec' status %d. chanspec=%x\n",
						        __FUNCTION__, ret, chanspec);
						exit(-1);
					} else if (chanspec != scanspec[i].chanspec3x3) {
						printf("%s: Chanspec 0x%x -> 0x%x New Bandwidth = 0x%x Old Bandwidth = 0x%x\n",
						       __FUNCTION__, scanspec[i].chanspec3x3, chanspec, newbw, scanspec[i].bw);
						scanspec[i].bw = newbw;
						scanspec[i].chanspec3x3 = chanspec;
						ConfigureSWSAScanParam(&airiqScanConfig[i], &scanspec[i]);
					}
				}
			}
		...
		if (scan_cntr == 10) {
			scan_cntr   = 0;
			scan_offset = 977;
			checkchan = 1; /* check for channel change */
		} else {
			scan_offset = 0;
		}
		scan_cntr++;
...
#define STACK_SIZE 0x40000
	...
	strncpy(gMgrsvcFlags.SwsaConfigFile, "/tmp/airiq-custom.cfg", sizeof(gMgrsvcFlags.SwsaConfigFile));
...
int main(int argc, char*argv[])
{
	...
	for (i = 1; i < argc; i++) {
		...
		if (strcmp(argv[i], "-curssithr") == 0) {
			if ((i + 1) < argc) {
				if (argv[i + 1][0] != '-') {
					PRINTF_VERBOSE("CLI: argv[%d] = %s val=%s\n", i, argv[i], argv[i + 1]);
					i++;
					CuRssiThrDbm = atoi(argv[i]);
					if (CuRssiThrDbm <= 0) {
						fprintf(stderr, "Illegal curssithr specified: %s. Must be greater than 0.\n", argv[i]);
						exit(-1);
					}
				}
			}
		...
		if (strcmp(argv[i], "-cuwifidetsnr") == 0) {
			if ((i + 1) < argc) {
				if (argv[i + 1][0] != '-') {
					PRINTF_VERBOSE("CLI: argv[%d] = %s val=%s\n", i, argv[i], argv[i + 1]);
					i++;
					CuWifiDetSnr = atoi(argv[i]);
					if (CuWifiDetSnr <= 0) {
						fprintf(stderr, "Illegal cuwifidetsnr specified: %s. Must be greater than 0.\n", argv[i]);
						exit(-1);
					}
				}
			}
	...
	if (gLogEventMask) {
		...
		/* ////////////////////////////////////////////// */
		OptionData.CuRssiThrDbm = -CuRssiThrDbm;
		status = BsaSetSpecanOption(DevHandle, BSA_SPECAN_OPTION_CU_RSSI_THR, &OptionData);
		if (status != BSA_SUCCESS) {
			fprintf(stderr, "could not set Channel utilization (duty cycle) rssi to -85 dBm. status=%d\n",
			        status);
		}
		...
		/* Adjust Wi-Fi Utilization relative threshold from noise floor */
		OptionData.CuWifiDetSnr = CuWifiDetSnr;
		status = BsaSetSpecanOption(DevHandle, BSA_SPECAN_OPTION_CU_WIFI_DET_SNR, &OptionData);
		if (status != BSA_SUCCESS) {
			fprintf(stderr, "could not set Wi-Fi Channel utilization (duty cycle) detection SNR. status=%d\n",
			        status);
		}

		/* //Adjust Wi-Fi Utilization relative threshold from noise floor
		OptionData.CuWifiRaiseForHighPwr = 40;
		status = BsaSetSpecanOption(DevHandle, BSA_SPECAN_OPTION_CU_WIFI_RAISE_FOR_HIGH_PWR, &OptionData);
		if (status != BSA_SUCCESS) {
			fprintf(stderr, "could not set CuWifiRaiseForHighPwr. status=%d\n", status);
		}
		OptionData.CuWifiRaiseForMidPwr = 40;
		status = BsaSetSpecanOption(DevHandle, BSA_SPECAN_OPTION_CU_WIFI_RAISE_FOR_MID_PWR, &OptionData);
		if (status != BSA_SUCCESS) {
			fprintf(stderr, "could not set CuWifiRaiseForMidPwr. status=%d\n", status);
		}
		OptionData.CuWifiRaiseForHighPwr = 20;
		status = BsaSetSpecanOption(DevHandle, BSA_SPECAN_OPTION_CU_WIFI_RAISE_FOR_LOW_PWR, &OptionData);
		if (status != BSA_SUCCESS) {
			fprintf(stderr, "could not set CuWifiRaiseForLowPwr. status=%d\n", status);
		}
		*/
