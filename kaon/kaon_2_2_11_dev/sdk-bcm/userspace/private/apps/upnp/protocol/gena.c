...
int split_url(char *url, char **host, int *port, char **path)
{
	...
	/* Only accept the LAN side clients */
	{
	    if (! cmsNet_isAddressOnLanSide(*host))
	        break;
	}
...
void uuidstr_create(char *str, int len)
{
    ...
    srand(4);
    for (i = 0; i < 15; i++) {
       d[i] = 0 + (int) (255.0 * rand()/(RAND_MAX + 1.0));
    }
    *((int *)&d[15]) = uuid_count++;
...
void gena_new_subscription(UFILE *up, PService psvc, char *cb, char *sid, char *to, char *nt)
{
    ...
    if (sid || !cb || !nt || !split_url(cb, &host, &port, &path)) {
       ...
       sr = (PSubscription) cmsMem_alloc(sizeof(Subscription), ALLOC_ZEROIZE);
       if (sr) {
	    
	    // store the split callback URL in the subscription record
	    sr->cb_host = cmsMem_strdup(host);
	    sr->cb_path = cmsMem_strdup(path);
	    sr->cb_port = port;

	    // allocate a unique SID for this subscription.
	    strcpy(sr->sid, "uuid:");

	    cmsUtl_generateUuidStrFromRandom(((char *)sr->sid)+5, sizeof(sr->sid)-5);
	    ...
	    UPNP_ERROR(("\tnew subscription cmsMem_alloc failed\n"));
	}
    }
}
...
void delete_subscription(PService psvc, PSubscription sr) 
{
	...
	if (*psr == sr) {
	    *psr = (*psr)->next;
	    ...
	    //
	    cmsMem_free(sr->cb_host);
	    cmsMem_free(sr->cb_path);
	    cmsMem_free(sr);
	    break;
	}
    }
...
static struct net_connection *make_gena_connection(PService psvc, PSubscription sr)
{
    ...
    if (connect(fd, (struct sockaddr *) &sr->addr, sizeof(sr->addr)) == -1) {
       UPNP_INFORM(("%s: connect failed - err %d\n", __FUNCTION__, errno));
       close(fd);
       return NULL;
    }

    c = (struct gena_connection *) cmsMem_alloc(sizeof(struct gena_connection), ALLOC_ZEROIZE);
    if (c == NULL) {
