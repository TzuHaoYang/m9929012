...
void WANCommonInterface_UpdateVars_dev2(PService psvc)
{
   ...
   if (!found && (pathDesc.oid == MDMOID_LA_G))
   {
      Dev2EthLAGObject *ethLagObj=NULL;
      Dev2EthLAGStatsObject *ethLagStatsObj = NULL;
      CmsRet r2;

      found = TRUE;
      setsvcval_by_name(psvc,"WANAccessType", "EthLAG");   /* Ok to use this ? todo */

      r2 = cmsObj_get(MDMOID_LA_G, &pathDesc.iidStack, 0, (void **)&ethLagObj);
      if (r2 != CMSRET_SUCCESS)
      {
         cmsLog_error("Could not get ethlag obj, r2=%d", r2);
      }
      else
      {
         setsvcval_by_name(psvc,"PhysicalLinkStatus", ethLagObj->status);
         cmsObj_free((void **) &ethLagObj);
      }

      r2 = cmsObj_get(MDMOID_LAG_STATS, &pathDesc.iidStack, 0, (void **)&ethLagStatsObj);
      if (r2 != CMSRET_SUCCESS)
      {
         cmsLog_error("Could not get ethlag stats obj, r2=%d", r2);
      }
      else
      {
         sprintf(tmpbuf,"%lu",(ulong)ethLagStatsObj->bytesSent);
         setsvcval_by_name(psvc,"TotalBytesSent",tmpbuf);

         sprintf(tmpbuf,"%lu",(ulong)ethLagStatsObj->bytesReceived);
         setsvcval_by_name(psvc,"ethLagStatsObj",tmpbuf);

         sprintf(tmpbuf,"%lu",(ulong)ethLagStatsObj->packetsSent);
         setsvcval_by_name(psvc,"TotalPacketsSent",tmpbuf);

         sprintf(tmpbuf,"%lu",(ulong)ethLagStatsObj->packetsReceived);
         setsvcval_by_name(psvc,"TotalPacketsReceived",tmpbuf);

         cmsObj_free((void **) &ethLagStatsObj);
      }
   }
