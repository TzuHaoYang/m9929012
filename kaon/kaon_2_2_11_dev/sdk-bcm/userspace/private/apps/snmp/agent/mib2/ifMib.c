...
static INTF_INFO *genWanIntfInfo(_WanIpConnObject *wanIpObj, _WanIpConnStatsObject *wanIpConnSts)
{
    if (!wanIpObj)
    {
        return NULL;
    ...
    /* Add interface */
    pINTF_INFO pIfNew = (pINTF_INFO)calloc(1, sizeof(INTF_INFO));
    if (!pIfNew)
    {
        return NULL;
    }
    ...
    pIfNew->if_name = (char *)strdup(wanIpObj->X_BROADCOM_COM_IfName);

    if (wanIpConnSts)
    {
        pIfNew->if_ibytes    =  wanIpConnSts->ethernetBytesReceived;
        pIfNew->if_ipackets  =  wanIpConnSts->ethernetPacketsReceived;
        pIfNew->if_ierrors   =  wanIpConnSts->X_BROADCOM_COM_RxErrors;
        pIfNew->if_iqdrops   =  wanIpConnSts->X_BROADCOM_COM_RxDrops;
        pIfNew->if_obytes    =  wanIpConnSts->ethernetBytesSent;
        pIfNew->if_opackets  =  wanIpConnSts->ethernetPacketsSent;
        pIfNew->if_oerrors   =  wanIpConnSts->X_BROADCOM_COM_TxErrors;
        pIfNew->if_odrop     =  wanIpConnSts->X_BROADCOM_COM_TxDrops;
    }

    struct ifreq ifrq;

    cmsUtl_strncpy(ifrq.ifr_name, wanIpObj->X_BROADCOM_COM_IfName, sizeof(ifrq.ifr_name));
    if (ioctl(s, SIOCGIFADDR, &ifrq) >= 0)
      pIfNew->if_addr = ifrq.ifr_addr;
...
//   LanWlanStatsObject *wlanStatsObj=NULL;
...
#ifdef BRCM_PKTCBL_SUPPORT
    /* Add WAN interfaces first */
    _WanIpConnObject *wanIpObj = NULL;
    while (cmsObj_getNext(MDMOID_WAN_IP_CONN, &iidStack, (void **) &wanIpObj)
               == CMSRET_SUCCESS)
    {
        _WanIpConnStatsObject *wanIpConnSts = NULL;
        InstanceIdStack iidStatsStack = EMPTY_INSTANCE_ID_STACK;

        if (wanIpObj->enable
            && CMSRET_SUCCESS == cmsObj_getNextInSubTree(MDMOID_WAN_IP_CONN_STATS,
                                                         &iidStack,
                                                         &iidStatsStack,
                                                         (void **) &wanIpConnSts)
           )
        {
            INTF_INFO *pIfNew = genWanIntfInfo(wanIpObj, wanIpConnSts);
            if (pIfNew)
            {
                pIfNew->if_index = index + IF_INDEX_BASE;
                index++;
                ifAddIfToList(pIfNew);
            }
            cmsObj_free((void **) &wanIpConnSts);
   ...
   {
      if(!usbObj->X_BROADCOM_COM_IfName)
      {
...
#ifdef BRCM_PKTCBL_SUPPORT
    /* Add PacketCable endpoints */
    index = 0;      // Reset index
    _POTSFxsObject *pFxsObj = NULL;
    InstanceIdStack iidStatsStack = EMPTY_INSTANCE_ID_STACK;

    while (cmsObj_getNext(MDMOID_POTS_FXS, &iidStack, (void **) &pFxsObj)
               == CMSRET_SUCCESS)
    {
        /* Add interface */
        pINTF_INFO pIfNew = (pINTF_INFO)calloc(1, sizeof(INTF_INFO));
        ...
        {
            memset(pIfNew, 0, sizeof(INTF_INFO));
            pIfNew->if_index = index + IF_INDEX_PKTCBL_BASE;
            pIfNew->if_name = (char *)strdup("Voice Over Cable Interface");
            pIfNew->if_type = IF_IANAIFTYPE_VOICEOVERCABLE;
            if (pFxsObj->enable)
            {
                pIfNew->if_flags |= IFF_UP;
            }
...
pINTF_INFO ifGetIfByIfIndex(int index)
{
  pINTF_INFO pIf;
...
#ifdef BRCM_PKTCBL_SUPPORT
/*****************************************************************************
**  FUNCTION:       sendVoiceCallMgrRestartMsgToSsk
**
**  PURPOSE:        Sends a message to SSK application to restart voice call manager
**
**  INPUT PARMS:    
**
**  OUTPUT PARMS:   
...
**
*****************************************************************************/
static CmsRet sendVoiceCallMgrRestartMsgToSsk( void )
{
   CmsMsgHeader msgHdr = EMPTY_MSG_HEADER;
   CmsRet ret;

   cmsLog_debug("sending restart msg to voice");

   msgHdr.dst = EID_SSK; 
   msgHdr.src = EID_SNMPD;
    ...
    switch (action) 
      ...
      case COMMIT:
...
#ifdef BRCM_PKTCBL_SUPPORT
        /* column check not needed, writes to everything else will return in previous stages and will not reach COMMIT */
        if (index >= IF_INDEX_PKTCBL_BASE)
        {
            // MTA endpoint interfaces
            InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;

            // Get provision state
            int isProvisioned = 0;
            SnmpPktcblObject *snmpObj = NULL;
            cmsObj_get(MDMOID_SNMP_PKTCBL, &iidStack, 0, (void **) &snmpObj);
            if (snmpObj)
            {
                if (snmpObj->pktcMtaDevProvisioningState == CLPKTCPROVSTATE_PASS)
                {
                    isProvisioned = 1;
                }
                cmsObj_free((void **) &snmpObj);
            }
            ...
            while (count > 0
                   && cmsObj_getNext(MDMOID_POTS_FXS, &iidStack, (void **) &pFxsObj)
                           == CMSRET_SUCCESS)
            {
                if (count == index - IF_INDEX_PKTCBL_BASE + 1)
                {
                    // Match found
                    if (IF_ADMIN_STATUS_UP == value || IF_ADMIN_STATUS_DOWN == value)
                    {
                        int mybool = (value == IF_ADMIN_STATUS_UP ? 1 : 0);

                        /* Restart call manager if provisioning has been completed
                         * and status is going from down to up */
                        if (mybool && !pFxsObj->enable && isProvisioned)
                        {
                            sendVoiceCallMgrRestartMsgToSsk();
                        }

                        // Update FXS parameter
                        if (pFxsObj->enable != mybool)
                        {
                            pFxsObj->enable = mybool;
                            if (cmsObj_set(pFxsObj,  &iidStack) != CMSRET_SUCCESS)
                            {
                                cmsLog_error("Can't set Client object\n");
                            }
                        }
                    }
...
#endif /* BRCM_PKTCBL_SUPPORT */
        {
            ...
            /* get ifName first */
            ifrq.ifr_ifindex = index;                       // TBD: This doesn't look right, there is no universal
                                                            //      mapping of SNMP index to interface type.
