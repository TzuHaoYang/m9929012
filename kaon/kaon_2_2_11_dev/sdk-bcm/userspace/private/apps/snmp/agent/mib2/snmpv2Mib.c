             ...
             WRITE_METHOD *write_method)
{
   ...
   if (cmsObj_get(MDMOID_IGD_DEVICE_INFO, &iidStack, 0, (void **) &obj) == CMSRET_SUCCESS)
   {
       ...
       snprintf(outputBuffer, 512,
                "EUE reference design <<HW_REV: %s; VENDOR: Broadcom; BOOTR: %s; SW_REV: %s; MODEL: %s>>",
                obj->hardwareVersion,
                obj->additionalSoftwareVersion,
                obj->softwareVersion,
                obj->modelName);
       *var_len = strlen(outputBuffer);
