...
extern int trapCode;
int snmpTransportIpVer = SNMPTRANSPORTIPVERBITS_IPV4;
...
static int snmpUdpReceive(pTRANSPORT_INFO transport, raw_snmp_info_t *message)
{
    ...
    if ((snmpDebug.transport & SNMP_DEBUG_LEVEL_TRANSPORT_UDP) == 
        ...
        cmsLog_debug("Received %d bytes:", message->request_length);
...
static int snmpUdpSend(pTRANSPORT_INFO pTransport, void* msg)
{
  ...
  {
     if (pTransport->trapEnable && 0 != g_trap_addr.ss_family)
     {
        if (sendto(socket, (char *)message->response,
                   message->response_length, 0,
                   (struct sockaddr *)&g_trap_addr,
                   sizeof(g_trap_addr)) < 0)
        {
           cmsLog_error("sendto failed for trapSocket");
           return (0);
        }
     }
     else
     {
        cmsLog_notice("Trap is not enabled or trap address is invalid, %d",
                     pTransport->trapEnable);
        return (0);
     }
  }
  else
...
static int snmpUdpCreateSocket(int port)
{
  struct sockaddr_storage mySocketAddress;
  ...
  /* Determine IPv4/IPv6 socket to use */
  if (snmpTransportIpVer & (1 << SNMPTRANSPORTIPVERBITS_IPV4))
  {
    cmsLog_notice("Opening IPv4 socket");

    struct sockaddr_in *pV4 = (struct sockaddr_in *)&mySocketAddress;
    pV4->sin_family = AF_INET;
    pV4->sin_addr.s_addr = INADDR_ANY;
    if (sizeof(pV4->sin_port) == 2) {
      pV4->sin_port = htons((short)port);
    } else if (sizeof(pV4->sin_port) == 4) {
      pV4->sin_port = htonl(port);
    } else {
      return -1;
    }
  }
  else if (snmpTransportIpVer & (1 << SNMPTRANSPORTIPVERBITS_IPV6))
  {
    cmsLog_notice("Opening IPv6 socket");

    struct sockaddr_in6 *pV6 = (struct sockaddr_in6 *)&mySocketAddress;
    pV6->sin6_family = AF_INET6;
    pV6->sin6_addr = in6addr_any;
    if (sizeof(pV6->sin6_port) == 2) {
      pV6->sin6_port = htons((short)port);
    } else if (sizeof(pV6->sin6_port) == 4) {
      pV6->sin6_port = htonl(port);
    } else {
      return -1;
    }
  }
  ...
  /* create data port */
  pNew->socket = socket(mySocketAddress.ss_family, SOCK_DGRAM, 0);
  if (pNew->socket  < 0){
    cmsLog_error("socket, family(%d)", mySocketAddress.ss_family);
...
static int snmpRxMsgFromSmd(pTRANSPORT_INFO transport, raw_snmp_info_t *message)
{
   ...
   case CMS_MSG_SNMPD_NOTIFICATION:
      cmsLog_debug("receive CMS_MSG_SNMPD_NOTIFICATION! sendPktcNotification (notification %d)",msg->sequenceNumber);
      sendPktcNotification(msg->sequenceNumber, msg->wordData);
      break;
