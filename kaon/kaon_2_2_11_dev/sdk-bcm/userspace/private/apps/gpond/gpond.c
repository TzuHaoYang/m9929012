...
static void ploamEventHandler(int ploamFd) {
  ...
  if (maskedEventBitmap & BCM_PLOAM_EVENT_ENCR_STATE_CHANGED) {
      BCM_PloamGemEncryptUpd encryptStatus;
      int rc;
      int i;

      printf("gpond: ploamEvent: BCM_PLOAM_EVENT_ENCR_STATE_CHANGED\n");
      rc = gponCtl_getEncryptStateUpdate(&encryptStatus);
      if (rc)
      {
          printf("  gponCtl_getEncryptStateUpdate() returned %d\n", rc);
      }
      else
      {
          for (i=0; i<encryptStatus.numOfEntries; i++)
          {
              printf ("GEM Port %d : %s\n", encryptStatus.encryptState[i].gemPortId,
                      (encryptStatus.encryptState[i].encrypted)? "encrypted" : "not encrypted");
          }
      }
  }
...
static int userPloamEventHandler(int usrPloamFd) {
  ...
  if (readn(usrPloamFd, (uint8_t*)&msg, sizeof(msg)) < 0)
      return 0;    /* Not a failure - do nothing */
