...
#define GPONCTL_PASSWORD_SIZE        36
...
int  getStatsHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
...
int  setMcastEncryptionKeysHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
...
int  setDWELL_TIMERHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
int  getDWELL_TIMERHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
static int  setTO6Handler(PGPON_OPTION_INFO pOptions, int nNumOptions) __attribute__((unused));
static int  getTO6Handler(PGPON_OPTION_INFO pOptions, int nNumOptions) __attribute__((unused));
...
int  decfgTcontAllocIdHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
...
int  setOmciCtrlMskHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
int  getKeyEncryptionKeyHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
int  getPWMParamsHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
int  setPWMParamsHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
int  cfgOmciMirrorHandler(PGPON_OPTION_INFO pOptions, int nNumOptions);
...
GPON_COMMAND_INFO g_gponctl_Cmds[] =
{
    ...
    {"setDWELL_TIMER",   {"--dwell_timer", ""}, setDWELL_TIMERHandler},
    {"getDWELL_TIMER",   {""}, getDWELL_TIMERHandler},
#ifdef SUPPORT_TO6
    {"setTO6",      {"--to6", ""}, setTO6Handler},
    {"getTO6",      {""}, getTO6Handler},
#endif
    ...
    {"transmitMsg",   {"--id", "--data", "--block",
                       ""},                      transmitMsgHandler},
    {"getStats",           {"--reset", ""}, getStatsHandler},
    ...
    {"configGemPort",      {"--index", "--id", "--alloc",
                            "--usQueue", "--dsonly", "--mcast", "--encring", ""},
    ...
    {"configTcontAllocId", {"--tcontIdx", "--allocId", ""},
                                            cfgTcontAllocIdHandler},
    {"deconfigTcontAllocId",
                           {"--tcontIdx", "--allocId", ""},
                                            decfgTcontAllocIdHandler},
    ...
    {"setMcastEncryptionKey",
                           {"--keyIdx", "--encrypt", "--key"},
                                            setMcastEncryptionKeysHandler},
    ...
    {"generatePrbs",       {"--enable", "--mode",
                            ""},            generatePrbsHandler},
    {"setMsk",             {"--valid", "--key",
                            ""},            setOmciCtrlMskHandler},
    {"getKek",             {""},            getKeyEncryptionKeyHandler},
    {"getPWMParams",       {""},            getPWMParamsHandler},
    {"setPWMParams",       {"--mode_cap", "--mode_conf", "--itransinit",
                            "--itxinit", "--ilowpower", "--irxoff",
                            "--iaware", "--ihold", "--ilowpower_doze",
                            "--ilowpower_wsleep", ""},
                                            setPWMParamsHandler},
    {"configOmciMirror",   {"--enable","--portIndex", ""},
                                            cfgOmciMirrorHandler},
};
...
void usage(void)
{
    ...
    printf("       %s getTO1TO2\n", g_gponctl_PgmName);
    printf("       %s setDWELL_TIMER --dwell_timer <ms>\n", g_gponctl_PgmName);
    printf("       %s getDWELL_TIMER\n", g_gponctl_PgmName);
#ifdef SUPPORT_TO6
    printf("       %s setTO6 --to6 <ms>\n", g_gponctl_PgmName);
    printf("       %s getTO6\n", g_gponctl_PgmName);
#endif
    printf("       %s getState\n", g_gponctl_PgmName);
    printf("       %s getSnPwd\n", g_gponctl_PgmName);
    ...
    printf("                      --pwd <xx-xx-xx-xx-xx-xx-xx-xx-xx-xx> (up to 36 bytes)]\n");
    printf("       %s getVersion\n", g_gponctl_PgmName);
    ...
    printf("       %s setMsk\n\n", g_gponctl_PgmName);
    printf("                    [ --valid <0=FALSE|1=TRUE> \n");
    printf("                      --key <xx-xx-xx-xx-xx-xx-xx-xx-xx-xx-xx-xx-xx-xx-xx-xx> (16 bytes) ]\n");
    printf("       %s getKek\n\n", g_gponctl_PgmName);
    printf(":::::: Power Management ::::::\n");
    printf("       %s getPWMParams\n", g_gponctl_PgmName);
    printf("       %s setPWMParams\n", g_gponctl_PgmName);
    printf("                    [ --mode_cap  <0...7: bit0=doze, bit1=sleep, bit2=wsleep>]\n");
    printf("                    [ --mode_conf <0...7: bit0=doze, bit1=sleep, bit2=wsleep>]\n");
    printf("                    [ --itransinit <0...0xFFFFFFFF>]\n");
    printf("                    [ --itxinit <0...0xFFFFFFFF>]\n");
    printf("                    [ --ilowpower <0...0xFFFFFFFFFFFFFFFF>]\n");
    printf("                    [ --irxoff <0...0xFFFFFFFFFFFFFFFF>]\n");
    printf("                    [ --iaware <0...0xFFFFFFFFFFFFFFFF>]\n");
    printf("                    [ --ihold <0...0xFFFFFFFF>]\n");
    printf("                    [ --ilowpower_doze <0...0xFFFFFFFFFFFFFFFF>]\n");
    printf("                    [ --ilowpower_wsleep <0...0xFFFFFFFFFFFFFFFF>]\n");
    printf("       %s configOmciMirror\n", g_gponctl_PgmName);
    printf("                    [ --enable <0=FALSE|1=TRUE> \n");
    printf("                      --portIndex <0=eth0|1=eth1|...> ]\n\n");
} /* usage */
...
static UINT8 pwdStrToNum(char* input, UINT8 *pwd)
{
    ...
    char *pToken = NULL;
    char *pLast = NULL;
    char buf[GPONCTL_PASSWORD_SIZE * 3 + 1];
    UINT8 inputLen;
    UINT8 numBytes;

    inputLen = strlen(input);
    numBytes = (inputLen + 1) / 3;
    if ((numBytes > GPONCTL_PASSWORD_SIZE) || (((inputLen + 1) % 3) != 0))
    {
        /* Invalid len. */
        printf("inputLen=%d\n", inputLen);
        return 1;
    ...
    for (i = 0; i < numBytes; i++)
    {
        ...
        /* Invalid case. */
        if (isHexNumber(pToken) == 0 || (strlen(pToken) > 2))
        {
            ret = 1;
            break;
        }
...
out:
    ...
    if (ret == GPONCTL_SUCCESS)
    {
        ...
        else if (info.initOperState == BCM_PLOAM_OSTATE_EMERGENCY_STOP_O7)
        {
            initStateName = "EMERGENCY STOP";
        }
        else
        {
            initStateName = "INVALID initial state";
        }
        printf("\n   Start ONU with operational state: %s (0%d)\n\n", initStateName, info.initOperState);
    }
    else
    {
        printf("\n   gponCtl_startAdminState failed, ret=%d\n\n", ret);
    }
...
int setDWELL_TIMERHandler(PGPON_OPTION_INFO pOptions, int nNumOptions) {
  int ret = 0;
  BCM_Ploam_DWELL_TIMERInfo info;
  ...
  while (nNumOptions)
  {
    if (!strcasecmp(pOptions->pOptName, "--dwell_timer"))
    {
      if (pOptions->nNumParms != 1)
      {
          fprintf(stderr,
                  "%s: specify dwell_timer timeout value\n",
                  g_gponctl_PgmName);
          return GPONCTL_INVALID_OPTION;
      }
      info.dwell_timer = strtoul(pOptions->pParms[0], NULL, 0);
    }
    else    /* should never reach here */
    {
      fprintf(stderr, "%s: invalid option [%s]\n",
  ...
  ret = gponCtl_setDWELL_TIMER(&info);

  if (ret != GPONCTL_SUCCESS)
      goto out;

  printf("\n======== DWELL TIMER ========\n\n");
  printf("   DWELL TIMER: %u ms\n", info.dwell_timer);
  printf("\n===========================\n\n");
...
int  getDWELL_TIMERHandler(PGPON_OPTION_INFO pOptions, int nNumOptions) {
  ...
  ret = gponCtl_getDWELL_TIMER(&info);

  if (ret != GPONCTL_SUCCESS)
      goto out;

  printf("\n======== DWELL TIMER ========\n\n");
  printf("   DWELL_TIMER: %u ms\n", info.dwell_timer);
  printf("\n===========================\n\n");
...
static int setTO6Handler(PGPON_OPTION_INFO pOptions, int nNumOptions) {
  int ret = 0;
  BCM_Ploam_TO6Info info;
  ...
  while (nNumOptions)
  {
    if (!strcasecmp(pOptions->pOptName, "--to6"))
    {
      if (pOptions->nNumParms != 1)
      {
          fprintf(stderr,
                  "%s: specify TO6 timeout value\n",
                  g_gponctl_PgmName);
          return GPONCTL_INVALID_OPTION;
      }
      info.to6 = strtoul(pOptions->pParms[0], NULL, 0);
    }
    else    /* should never reach here */
    {
      fprintf(stderr, "%s: invalid option [%s]\n",
...
static int  getTO6Handler(PGPON_OPTION_INFO pOptions, int nNumOptions) {
  int ret = 0;
  BCM_Ploam_TO6Info info;
    ...
    if (ret == GPONCTL_SUCCESS)
    {
        ...
        switch (info.operState)
        {
            ...
            case BCM_PLOAM_OSTATE_DS_TUNING_O8:
                printf("   Operational    state: DS TUNING (O%d)\n", info.operState);
                break;
            case BCM_PLOAM_OSTATE_US_TUNING_O9:
                printf("   Operational    state: US TUNING (O%d)\n", info.operState);
                break;
            default:
                printf("\n   INVALID State : (0%d)\n\n", info.operState);
                break;
...
int getSnPwdHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    if (ret == GPONCTL_SUCCESS)
    {
        ...
        for (i = 0; i < GPONCTL_SERIAL_NUMBER_SIZE - 1; i++)
            printf("%02X-", info.serialNumber[i]);
        printf("%02X\n", info.serialNumber[GPONCTL_SERIAL_NUMBER_SIZE - 1]);
        printf("   Password     : ");
        for (i = 0; i < GPONCTL_PASSWORD_SIZE - 1; i++)
        {
        ...
        printf("%02X\n", info.password[GPONCTL_PASSWORD_SIZE - 1]);
        printf("\n==========================================\n\n");
    }
...
int setSnPwdHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    while (nNumOptions)
    {
        ...
        else if (!strcasecmp(pOptions->pOptName, "--pwd"))
        {
            ...
            memset(&info.password, 0, BCM_PLOAM_PASSWORD_SIZE_BYTES);
...
int getEventHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    if (ret == GPONCTL_SUCCESS)
    {
        ...
        if (eventStatusInfo.eventBitmap & BCM_PLOAM_EVENT_ENCR_STATE_CHANGED)
            printf("   GEM Port Encryption state change : YES\n");
        else
            printf("   GEM Port Encryption state change : NO\n");
        printf("\n=====================================\n\n");
        ...
        if (eventStatusInfo.eventMask & BCM_PLOAM_EVENT_GEM_STATE_CHANGE)
            printf("   GEM Port Operational state change : YES\n");
        else
            printf("   GEM Port Operational state change : NO\n");
        if (eventStatusInfo.eventMask & BCM_PLOAM_EVENT_ENCR_STATE_CHANGED)
            printf("   GEM Port Encryption state change : YES\n");
        else
            printf("   GEM Port Encryption state change : NO\n");
        printf("\n=====================================\n\n");
...
/*
 *------------------------------------------------------------------------------
 * Function Name: getStatsHandler
 * Description  : Processes the BCM_PLOAM_IOC_GET_STATS command.
 * Returns      : 0 - success, non-0 - error
 *------------------------------------------------------------------------------
 */
int getStatsHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    if (!strcasecmp(pOptions->pOptName, "--reset"))
    {
        ...
        gstats.reset = strtoul(pOptions->pParms[0], NULL, 0);
        if (gstats.reset > 1)
        {
            fprintf(stderr,
                    "%s: invalid reset %u\n",
                    g_gponctl_PgmName, gstats.reset);
            return GPONCTL_INVALID_PARAMETER;
        }
    }
    else    /* should never reach here */
...
out:
    ret = gponCtl_getStats(&gstats);
    if (ret == GPONCTL_SUCCESS)
    {
        printf("\n=============== GTC Counters ================\n\n");
        if (gstats.reset)
            printf("Statistic counters are reset\n");
        else
        {
            printf("   BIP error counter                                 %u\n", gstats.bipErr);
            printf("   PSBd HEC error count                              %u\n", gstats.psbdHecErr);
            printf("   XGTC HEC error count                              %u\n", gstats.xgtcHecErr);
            printf("   XGEM encryption key error count                   %u\n", gstats.gemKeyErr);
            printf("   XGEM HEC error count                              %u\n", gstats.gemHecErr);
            printf("   FEC corrected bits                                %u\n", gstats.fecCorrectedBits);
            printf("   FEC corrected symbols                             %u\n", gstats.fecCorrectedSym);
            printf("   FEC corrected codewords                           %u\n", gstats.fecCorrectedCw);
            printf("   FEC uncorrectable codewords                       %u\n", gstats.fecUcCw);

            printf("   PLOAM CRC or MIC errors                           %u\n", gstats.crcErr);
            printf("   Valid ONU id PLOAM counter                        %u\n", gstats.rxOnuId);
            printf("   Broadcast PLOAM counter                           %u\n", gstats.rxBroadcast);
            printf("   Rx unknown PLOAM counter                          %u\n", gstats.rxUnknown);
            printf("   Total Rx PLOAM counter                            %u\n", gstats.rxPloam);
            printf("   Rx profile PLOAMs counter                         %u\n", gstats.rxProfiles);
            printf("   Rx ranging PLOAMs counter                         %u\n", gstats.rxRanging);
            printf("   Rx deactivate PLOAMs counter                      %u\n", gstats.rxDeactivate);
            printf("   Rx disable PLOAMs counter                         %u\n", gstats.rxDisable);
            printf("   Rx registration request PLOAM counter             %u\n", gstats.rxRegReq);
            printf("   Rx assign alloc id PLOAM counter                  %u\n", gstats.rxAllocId);
            printf("   Rx key control PLOAM counter                      %u\n", gstats.rxKeyCntrl);
            printf("   Rx sleep allow PLOAM counter                      %u\n", gstats.rxSleep);

            printf("   Tx illegal access PLOAM counter                   %u\n", gstats.txIllegalAccess);
            printf("   Tx idle PLOAM counter                             %u\n", gstats.txIdle);
            printf("   Tx PLOAM counter                                  %u\n", gstats.txPloam);
            printf("   Tx registration PLOAM counter                     %u\n", gstats.txReg);
            printf("   Tx acknowledge PLOAM counter                      %u\n", gstats.txAck);
            printf("   Tx key report PLOAM counter                       %u\n", gstats.txKeyRep);
            printf("   Tx sleep request PLOAM counter                    %u\n", gstats.txSleepRequest);

            printf("   Successful activations                            %u\n", gstats.successAct);
        }
        printf("\n=============================================\n\n");
    }
...
int getGemPortCountersHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    if (ret == GPONCTL_SUCCESS)
    {
        ...
        printf("   Received fragments           : %llu\n", counters.rxFragments);
        printf("   Received frames              : %llu\n", counters.rxFrames);
        printf("   Received dropped frames      : %llu\n", counters.rxDroppedFrames);
        printf("   Transmitted bytes            : %llu\n", counters.txBytes);
        printf("   Transmitted fragments        : %llu\n", counters.txFragments);
        printf("   Transmitted frames           : %llu\n", counters.txFrames);
        printf("   Transmit dropped frames      : %llu\n", counters.txDroppedFrames);
        printf("   Accepted Multicast Frames    : %llu\n", counters.rxMcastAcceptedFrames);
        printf("   Dropped Multicast Frames     : %llu\n", counters.rxMcastDroppedFrames);
        printf("\n==============================================\n\n");
    }
...
int configGemPortHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    while (nNumOptions)
    {
        ...
        else if (!strcasecmp(pOptions->pOptName, "--dsonly"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify dsonly flag <0..1>\n",
                        g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.isDsOnly = strtoul(pOptions->pParms[0], NULL, 0);
#ifdef GPONCTL_VALIDATE_PARAMS
            if (info.isDsOnly > 1)
            {
                fprintf(stderr,
                        "%s: invalid dsonly flag %d\n",
                        g_gponctl_PgmName, info.isDsOnly);
                return GPONCTL_INVALID_PARAMETER;
            }
#endif
        }
        ...
        else if (!strcasecmp(pOptions->pOptName, "--encring"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify encryption key ring <0..3>\n",
                        g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.encRing = strtoul(pOptions->pParms[0], NULL, 0);
#ifdef GPONCTL_VALIDATE_PARAMS
            if (info.encRing > 3)
            {
                fprintf(stderr,
                        "%s: invalid encryption key ring %d\n",
                        g_gponctl_PgmName, info.encRing);
                return GPONCTL_INVALID_PARAMETER;
            }
#endif
        }
...
/*
 *------------------------------------------------------------------------------
 * Function Name: setMcastEncryptionKeysHandler
 * Description  : Processes the BCM_PLOAM_IOC_SET_MCAST_ENCRYPTION_KEY command.
 * Returns      : 0 - success, non-0 - error
 *------------------------------------------------------------------------------
 */
int setMcastEncryptionKeysHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    while (nNumOptions)
    {
        if (!strcasecmp(pOptions->pOptName, "--keyIdx"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify key index <0..3>\n",
                        g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.key_idx = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--encrypt"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify encrypt flag <0|1>\n",
                        g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.is_encrypted = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--key"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s:key data <xx-xx-..-xx (up to 16)>\n",
                        g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            if (omciDataStrToNum(pOptions->pParms[0], info.key,
              BCM_PLOAM_ENCRYPTION_KEY_SIZE_BYTES))
            {
                fprintf(stderr,
    ...
    if (ret == GPONCTL_SUCCESS)
    {
        printf("\n====== Multicast Encryption Key Configuration =======\n\n");
        printf("   Key index               : %d\n", info.key_idx);
        printf("   Encrypted               : %d\n", info.is_encrypted);
        printf("   Encryption key: ");
        for (i = 0; i < BCM_PLOAM_ENCRYPTION_KEY_SIZE_BYTES; i++)
...
out:
    ...
    if (ret == GPONCTL_SUCCESS)
    {
        ...
        printf("   Baseline packets           : %u\n", counters.rxBaseLinePackets);
        printf("   Extended packets           : %u\n", counters.rxExtendedPackets);
        printf("   Packets with CRC/MIC errs. : %u\n", counters.rxMicCrcErrCnt);
        printf("\n=====================================\n\n");
    }
...
int cfgOmciMirrorHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    while (nNumOptions)
    {
        if (!strcasecmp(pOptions->pOptName, "--enable"))
        {
            ...
            {
                fprintf(stderr,
                        "%s: identify OMCI mirror enabled <0..1>\n",
                        g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.enable = strtoul(pOptions->pParms[0], NULL, 0);
        ...
        else if (!strcasecmp(pOptions->pOptName, "--portIndex"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify LAN port index <0..n>\n",
                        g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.portIndex = strtoul(pOptions->pParms[0], NULL, 0);
            bSetValid = 1;
        }
        else /* should never reach here */
        {
    ...
    if (!bSetValid)
    {
        fprintf(stderr, "%s: ERROR !! Missing mandatory params\n",
               g_gponctl_PgmName);
        return GPONCTL_INVALID_NUMBER_OF_OPTIONS;
    }

    ret = gponCtl_cfgOmciMirror(&info);
...
/*
 *------------------------------------------------------------------------------
 * Function Name: decfgTcontAllocIdHandler
 * Description  : Processes the BCM_PLOAM_IOC_DECFG_TCONT_ALLOCID command.
 * Returns      : 0 - success, non-0 - error
 *------------------------------------------------------------------------------
 */
int decfgTcontAllocIdHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    ret = gponCtl_deconfigTcontAllocId(&info);

    if (ret == GPONCTL_SUCCESS)
    {
        printf("\n====== TCONT deconfiguration =======\n\n");
        printf("   TCONT index             : %d\n", info.tcontIdx);
        printf("   AllocID                 : 0x%X\n", info.allocID);
        printf("\n\n=====================================\n\n");
    }
...
/*
 *------------------------------------------------------------------------------
 * Function Name: setOmciCtrlMskHandler
 * Description  : Processes the BCM_PLOAM_IOC_SET_MSK command.
 * Returns      : 0 - success, non-0 - error
 *------------------------------------------------------------------------------
 */
int setOmciCtrlMskHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    BCM_PLoam_OmciCtrlMsk omciMsk;
    int ret = 0;

    while (nNumOptions)
    {
        if (!strcasecmp(pOptions->pOptName, "--valid"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
            ...
            omciMsk.isValid = strtoul(pOptions->pParms[0], NULL, 0);
            if (omciMsk.isValid > 1)
            {
                omciMsk.isValid = 1;
            }
        }
        else if (!strcasecmp(pOptions->pOptName, "--key"))
        {
            ...
            if (omciDataStrToNum(pOptions->pParms[0], omciMsk.msk, BCM_PLOAM_ENCRYPTION_KEY_SIZE_BYTES))
            {
                fprintf(stderr,
                        "%s: invalid key data %s\n",
                        g_gponctl_PgmName, pOptions->pParms[0]);
    ...
    ret = gponCtl_setOmciCtrlMasterSessionKey(&omciMsk);
...
/*
 *------------------------------------------------------------------------------
 * Function Name: getKeyEncryptionKeyHandler
 * Description  : Processes the BCM_PLOAM_IOC_GET_KEK command.
 * Returns      : 0 - success, non-0 - error
 *------------------------------------------------------------------------------
 */
int getKeyEncryptionKeyHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    BCM_Ploam_Kek kekInfo;
    int i;
    int ret;

    memset(&kekInfo, 0, sizeof(kekInfo));
    ret = gponCtl_getKeyEncryptionKey(&kekInfo);
...
int  getPWMParamsHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    BCM_Ploam_PowerManagementParams info;
    int ret = gponCtl_getPowerManagementParams(&info);

    if (ret == GPONCTL_SUCCESS)
    {
        printf("\n======= PON POWER MANAGEMENT =======\n\n");
        printf("  mode_cap:     %d\n", info.mode_cap);
        printf("  mode_conf:    %d\n", info.mode_conf);
        printf("  itransinit:   %d\n", info.itransinit);
        printf("  itxinit:      %d\n", info.itxinit);
        ...
        printf("  iaware:       %d\n", info.iaware);
        printf("  ihold:        %d\n", info.ihold);
        printf("  ilowpower_doze:   %d\n", info.ilowpower_doze);
        printf("  ilowpower_wsleep: %d\n", info.ilowpower_wsleep);
        printf("\n======================\n\n");
    }
...
int  setPWMParamsHandler(PGPON_OPTION_INFO pOptions, int nNumOptions)
{
    ...
    ret = gponCtl_getPowerManagementParams(&info);

    while (nNumOptions)
    {
        if (!strcasecmp(pOptions->pOptName, "--mode_cap"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify mode_cap <0...7: bit0=doze, bit1=sleep, bit2=wsleep>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.mode_cap = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--mode_conf"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify mode_conf <0...7: bit0=doze, bit1=sleep, bit2=wsleep>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.mode_conf = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--itransinit"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify itransinit <0...0xFFFFFFFF>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.itransinit = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--itxinit"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify itxinit <0...0xFFFFFFFF>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.itxinit = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--ilowpower"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify ilowpower <0...0xFFFFFFFFFFFFFFFF>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.ilowpower = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--irxoff"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify irxoff <0...0xFFFFFFFFFFFFFFFF>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.irxoff = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--iaware"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify iaware <0...0xFFFFFFFFFFFFFFFF>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.iaware = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--ihold"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify ihold <0...0xFFFFFFFF>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.ihold = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--ilowpower_doze"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify ilowpower_doze <0...0xFFFFFFFFFFFFFFFF>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.ilowpower_doze = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else if (!strcasecmp(pOptions->pOptName, "--ilowpower_wsleep"))
        {
            if (pOptions->nNumParms != 1)
            {
                fprintf(stderr,
                        "%s: identify ilowpower_wsleep <0...0xFFFFFFFFFFFFFFFF>\n", g_gponctl_PgmName);
                return GPONCTL_INVALID_OPTION;
            }
            info.ilowpower_wsleep = strtoul(pOptions->pParms[0], NULL, 0);
        }
        else    /* should never reach here */
        {
            fprintf(stderr, "%s: invalid option [%s]\n",
    ...
    ret = gponCtl_setPowerManagementParams(&info);
