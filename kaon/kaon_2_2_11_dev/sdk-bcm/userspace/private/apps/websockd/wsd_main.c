...
/* Common append response function for DPI and flow stat. */
void wsd_append_response(struct wsd_cmn_state *state, char *data)
{
    struct wsd_cmn_response *el = state->responses;
    struct wsd_cmn_response *response;
 
    response = malloc(sizeof(*response));
    if (!response) {
        cmsLog_error("couldn't allocate response");
        return;
    }
    ...
    response->data = malloc(response->size + LWS_SEND_BUFFER_PRE_PADDING +
      LWS_SEND_BUFFER_POST_PADDING);
    if (!response->data) {
        cmsLog_error("unable to allocate buffer for sending to websocket");
        free(response);
        return;
    }
 
    cmsLog_debug("rsp size %d", response->size);
...
/* Common server writable callback function for DPI and flow stat. */
int wsd_callback_serverwritable(struct wsd_cmn_state *state, struct lws *wsi)
{
    ...
    cmsLog_debug("write size %d", resp->size - resp->pos);
