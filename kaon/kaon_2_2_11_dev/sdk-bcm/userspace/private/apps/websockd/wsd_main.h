...
struct wsd_cmn_response {
  int size;
  int pos;
  int send_count;
  unsigned char *data;
  struct wsd_cmn_response *next;
};

struct wsd_cmn_state {
   struct psd_session_key   session;
   struct wsd_cmn_response *responses;
   struct lws              *wsi;
};
