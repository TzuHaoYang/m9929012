...
#if defined(CHIP_6818) || defined(CHIP_63268)
#include <board.h>
#endif
...
#if !defined(CHIP_6818) && !defined(CHIP_63268) && !defined(STANDALONE)
static void * tpcap_mapphys(unsigned int addr)
{
...
{
   printf(
      "    tpcap [--bursttype <burst_type>] [--stoptype <0-no, 1-CRC, 2-timeout,3-bad probe result> ] [--nsamples <val>] [--port <val>] [--direction <0-Tx, 1-Rx, 3-loopback, 4-All>] \n"
	  ...
	  "          [--noAssert] [--nbursts <val>] [--burstStopType <burst_stop_type>] [--onlyAssert <0-disable,1-generate only assert wo/tpcap>] \n" 
...
#endif
{
   ...
   while (argc)
   {
	    if( !dump && !strcmp(*argv, "--capture_wait_mode") )
      {
         argv++;
         argc--;
	  ...
	  else if( !dump && !strcmp(*argv, "--onlyAssert") )
      {
         argv++;
         argc--;

         if (argc){
            OnlyAssert = strtol(*argv, NULL, 0);
		 } 	
         else
         {
            printhelp();
         ...
         if ((OnlyAssert != 0) && (OnlyAssert != 1 ))
            printf("WARNING: OnlyAssert is boolean value\n");
      }
   ...
   if (!dump)
   {
	  ...
	  if (port ==0){
		dp.type_2= (CaptureBurstType<< 8) | ((phy_probe & 0x7) <<1) | (OnlyAssert & 0x1);
	  }
	  else{
		  dp.type_2= (CaptureBurstType<< 8) | ((0x7) <<1) | (OnlyAssert & 0x1);
	  }
		  


      if (stopBurstType != (1 << 31)){
