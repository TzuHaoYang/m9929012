...
static char* writeIp(struct sockaddr *ip)
{
...
static int sntp(char *ntpserver, int tz_idx)
{
    ...
    if (getaddrinfo(ntpserver, NULL, &hints, &res0) != 0)
    {
        cmsLog_notice("dns_lookup(%s) failed", ntpserver);
        res_init(); /* refresh dns caches (resolv.conf) */
    ...
    for (res = res0; res; res = res->ai_next)
    {
        ...
        if( ( s = socket( res->ai_family, SOCK_DGRAM, 0 ) ) == -1 )
        {
            cmsLog_error( "socket: %s", strerror(errno) );
            continue;
        }
        ...
        {        
            ...
            memcpy(&to, res->ai_addr, saLen);
            ((struct sockaddr_in *)&to)->sin_port = htons( 123 );
        }
        else
        {        
            saLen = sizeof(struct sockaddr_in6);
            memcpy(&to, res->ai_addr, saLen);
            ((struct sockaddr_in6 *)&to)->sin6_port = htons( 123 );
        }

        retval =  sendto( s, msg, sizeof( msg ), 0, (struct sockaddr *) &to, saLen );
        if( retval == -1 ) 
        {
            cmsLog_notice( "sendto: %s", strerror(errno) );
        }
...
int main(int argc, char **argv ) {
    ...
    while ((c= getopt(argc, argv, "v:s:t:")) != -1)
    {
        switch (c)
        {
            case 'v':
