...
MOCAD_S void mocad_mps_key_compute(struct mocad_ctx *ctx, unsigned char *peer_public_key, int is_nn, unsigned char *nn_guid, unsigned char *mpskey) {
  ...
  HMAC_CTX * p_hmac_ctx;
  p_hmac_ctx = HMAC_CTX_new();

  HMAC_Init_ex(p_hmac_ctx, dhkey, dhkeylen, EVP_sha256(), NULL);
  ...
  HMAC_Update(p_hmac_ctx, nn_key+1, key_len); // +1 to skip the tag

  // NN's GUID
  HMAC_Update(p_hmac_ctx, nn_guid, 8);
  ...
  HMAC_Update(p_hmac_ctx, nc_key+1, key_len); // +1 to skip the tag

  HMAC_Final(p_hmac_ctx, output, &outputlen);
