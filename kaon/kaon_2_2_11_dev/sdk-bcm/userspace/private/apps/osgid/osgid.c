...
void osgidSendReplyToRequestApp(REQUEST_DATA *requestdata)
{
   ...
   strcpy(msgPayload->username,requestdata->username);
   strcpy(msgPayload->password,requestdata->password);
...
void osgidSendReplyListToRequestApp(pLIST_TYPE pList)
{
   ...
   while(pEntry != NULL)
   {
      ...
      strcpy(msgPayload->username,pRequestData->username);
      strcpy(msgPayload->password,pRequestData->password);
...
CmsRet osgidProcessInstallRequest(REQUEST_DATA *reqdata)
{
   ...
   if (!cmsUtl_strcmp(reqdata->operation,SW_MODULES_OPERATION_INSTALL))
   {
      ...
      ret = modsw_addDuEntry(reqdata->UUID, reqdata->version,
                             duid, reqdata->URL, 
                             reqdata->username, reqdata->password, 
                             reqdata->executionEnv, FALSE,
                             newDuid, sizeof(newDuid)-1);
      if (ret != CMSRET_SUCCESS)
      {
      ...
      modsw_setDuStatus(reqdata->UUID, reqdata->version, reqdata->executionEnv, MDMVS_X_BROADCOM_COM_DOWNLOADING);

      spdRet = beepPkg_startDownload(reqdata->URL,reqdata->username,reqdata->password,bundleDir);
      ret = modsw_convertToCmsRet(spdRet);
      if (ret != CMSRET_SUCCESS)
      {
         /* set DU status to download failed, let ACS delete the entry */
         modsw_setDuStatus(reqdata->UUID, reqdata->version, reqdata->executionEnv, MDMVS_X_BROADCOM_COM_DNLDFAILED);
         return ret;
      }
   }
    ...
    * to install it.
    */
   modsw_setDuStatus(reqdata->UUID, reqdata->version, reqdata->executionEnv, MDMVS_INSTALLING);
...
CmsRet osgidProcessUpdateRequest(REQUEST_DATA *reqdata)
{
   ...
   while ((ret = cmsObj_getNext(MDMOID_DU, &idStack, (void **)&duObject)) == CMSRET_SUCCESS)
   {
      ...
      else if (action == OSGID_UPDATE_ALL_MATCH_URL)
      {
         ...
         {
            /*queue the req data into processingList*/
            strcpy(reqdata->username,duObject->X_BROADCOM_COM_Username);
            strcpy(reqdata->password,duObject->X_BROADCOM_COM_Password);
                            ...
                            char *destdir, char *version, void *pDu)
{
   ...
   spdRet = beepPkg_startDownload(url,username,password,destdir);
   ret = modsw_convertToCmsRet(spdRet);
...
void osgidAddEUinstance(pLB_DATA pData, char *euFullPathStr, UINT32 euFullPathStrLen, char *duDescription)
{
   ...
   ret = modsw_addEuEntryLocked(euIdStr, aliasStr, nameBuf, OSGI_NAME,
              execEnvFullPath, NULL, NULL, pData->bundleId,
              pData->description, pData->version, pData->description,
                                     ...
                                     &mngrEid) == CMSRET_SUCCESS)
      {
         if (mngrEid == EID_OSGID)
         {
            ...
            else
            {
               ...
               strcpy(pRequestData->URL,duObject->URL);
               strcpy(pRequestData->username,duObject->X_BROADCOM_COM_Username);
               strcpy(pRequestData->password,duObject->X_BROADCOM_COM_Password);
