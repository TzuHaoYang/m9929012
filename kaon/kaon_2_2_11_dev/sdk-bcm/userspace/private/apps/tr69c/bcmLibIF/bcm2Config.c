...
extern CmsRet tr69c_fullPathToPathDescriptor(const char *fullpath, MdmPathDescriptor *pathDesc);
extern CmsRet tr69c_pathDescriptorToStdFullPath(const MdmPathDescriptor *pathDesc, char **fullpath);
...
char *getFullPathToIpvxAddrLocked_dev2(UINT32 ipvx, const char *ipAddr)
{
   ...
   if (found)
   {
      ret = tr69c_pathDescriptorToStdFullPath(&pathDesc, &fullPath);
...
void clearModemConnectionURL_dev2(void)
{
   ...
   ret = cmsObj_get(stdOid2EeOid(MDMOID_DEV2_MANAGEMENT_SERVER), &iidStack, 0, (void **) &obj);
   if (ret != CMSRET_SUCCESS)
   {
...
void setDefaultActiveNotificationLocked_dev2(PhlSetParamAttr_t **pSetParmAttrList, int *numEntries)
{
   ...
   *numEntries = num;
   ...
   tr69c_fullPathToPathDescriptor("Device.ManagementServer.ConnectionRequestURL",
...
#ifdef SUPPORT_XMPP
   /* Device.ManagementServer.ConnReqJabberId */
   tr69c_fullPathToPathDescriptor("Device.ManagementServer.ConnReqJabberID",
                                   &(pSetParamAttr->pathDesc));
   pSetParamAttr->attributes.notificationChange = 1;
   pSetParamAttr->attributes.notification = NDA_TR69_ACTIVE_NOTIFICATION;
   pSetParamAttr->attributes.accessBitMask = NDA_ACCESS_TR69C;
...
void updateTr69cCfgInfo_dev2(void)
{
   ...
   if ((ret = cmsObj_get(stdOid2EeOid(MDMOID_DEV2_MANAGEMENT_SERVER), &iidStack, 0, (void *) &acsCfg)) != CMSRET_SUCCESS)
   {
   ...
   {
      /* if acsURL is not configured, don't bother updating other info */
