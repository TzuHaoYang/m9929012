...
extern MdmObjectId stdOid2EeOid (MdmObjectId oldOid);
extern char* tr69c_getScratchPadKey(eScratchPadKeyType keyType);
...
CmsRet downloadComplete(DownloadReq *r, char *buf)
{
   ...
   if ( buf ) {
      ...
      else
      {
         ...
         ret = cmsImg_writeValidatedImage_noReboot(buf, r->fileSize, format, msgHandle);
         ...
         {
            /*
             * In the modem, we will call reboot immediately
             * On the desktop, we will still be here.
             */
...
void saveTR69StatusItems(void)
{
   ...
   key_acsstate = tr69c_getScratchPadKey(SPKT_ACSSTATE);
   if (NULL == key_acsstate)
   {
      cmsLog_error("invalid SP key");
   }
   else if (cmsPsp_set(key_acsstate, &gwState, sizeof(GWStateData)) != CMSRET_SUCCESS)
   {
...
void retrieveTR69StatusItems(void)
{
   ...
   {
      count = cmsPsp_get(key_acsstate, objValue, sizeof(GWStateData));
   }
   ...
   if (acsState.dataModel != objValue->dataModel)
   {
      ...
      cmsPsp_set(key_acsstate,NULL,0);
      return;
   }
...
void updateCredentialsInfo(void)
{
   ...
   {
      path.oid = stdOid2EeOid(MDMOID_DEV2_MANAGEMENT_SERVER);
   }
   else
   {
...
CmsRet setMSrvrURL(const char *value)
{
   ...
   if (acsState.dataModel == DATA_MODEL_TR181)
   {
      paramValue.pathDesc.oid = stdOid2EeOid(MDMOID_DEV2_MANAGEMENT_SERVER);
   }
   else
   {
   ...
   if (saveEntryCount > 0)
   {
      ...
      for (i = 0, j = 0; i < TRANSFER_QUEUE_SIZE && j < saveEntryCount; i++)
      {
         ...
         if (q->state == eTransferNotYetStarted) 
         {
            ...
            // calculate how many seconds passed since received request then save
            if ((q->delaySec - (sysInfo.uptime - q->baseTime)) > 0)
               pSaved->delaySec = q->delaySec - (sysInfo.uptime - q->baseTime);
            else
               pSaved->delaySec = 1;
      ...
      key_transfer = tr69c_getScratchPadKey(SPKT_TRANSFER);
      if (NULL == key_transfer)
      {
         cmsLog_error("invalid SP key");
      }
      else if ((ret = cmsPsp_set(key_transfer, pSaveList, saveSz)) != CMSRET_SUCCESS)
      {
...
CmsRet tr69RetrieveTransferListFromStore(DownloadReqInfo *list, UINT16 *size)
{
   ...
   count = cmsPsp_get(key_transfer, buf, bufSz);
   if (count < 0)
   {
      cmsLog_error("error during scratchpad read of %s (buffer too small?), count=%d", key_transfer, count);
      *size = 0;
   }
   else if (count == 0)
   {
      /* could not find SPKT_TRANSFER in scratch pad or other error */
      cmsLog_debug("could not find %s in scratch pad", key_transfer);
      *size = 0;
   }
   else
   {
      cmsLog_debug("read %d bytes from scratch pad for %s", count, key_transfer);
      *size = (UINT16) count;
...
CmsRet tr69SaveConfigFileInfo(DownloadReq *r)
{
   ...
   key_vendorcfg = tr69c_getScratchPadKey(SPKT_VENDOR_CFG_TOKEN);
   if (NULL == key_vendorcfg)
   {
      cmsLog_error("invalid SP key");
   }
   else if ((ret = cmsPsp_set(key_vendorcfg, &vendorConfig, saveSz)) != CMSRET_SUCCESS)
   {
...
void retrieveClearTR69VendorConfigInfo(void)
{
   ...
   count = cmsPsp_get(key_vendorcfg, pVendorConfig, sizeof(DownloadVendorConfigInfo));
   if (count == 0)
   {
      cmsLog_debug("No existing %s info found in scratch pad.", key_vendorcfg);
   ...
   ret = cmsPsp_set(key_vendorcfg,NULL,0);
