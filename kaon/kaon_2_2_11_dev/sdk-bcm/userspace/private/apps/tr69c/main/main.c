...
eXMPPStatus g_XMPPStatus = XMPP_NONE;
CmsEntityId g_eIdTR69C = EID_TR69C;
...
void usage(char *progName)
{
   ...
   printf("       I: tr69c instance ID. 1 is default one\n");
   exit(1);
}
...
char* tr69c_getScratchPadKey(eScratchPadKeyType keyType)
{
   if (EID_TR69C == g_eIdTR69C)
   {
      ...
      {
         case SPKT_ACSSTATE:            
            return SPK_TR69C_1_ACSSTATE;
         case SPKT_TRANSFER:            
            return SPK_TR69C_1_TRANSFER;
         case SPKT_VENDOR_CFG_TOKEN:            
            return SPK_TR69C_1_VENDOR_CFG_TOKEN;
         default:
            cmsLog_error("unknown keyType=%d!", keyType);
            return NULL;
      }
   }
   else if (EID_TR69C_2 == g_eIdTR69C)
   {
      ...
      {
         case SPKT_ACSSTATE:
            return SPK_TR69C_2_ACSSTATE;
            
         /* TODO:: currently, no requirement on queue transfer in tr69c_2, so disable
                *  this due to Scratch Pad size limitation(8k). If needs to support full function in 
                *  in tr69c_2 in further, it can be enabled after expand Scratch Pad size limitation.
                */
#if 0
         case SPKT_TRANSFER:            
            return SPK_TR69C_2_TRANSFER;
         case SPKT_VENDOR_CFG_TOKEN:
            return SPK_TR69C_2_VENDOR_CFG_TOKEN;
#else
         case SPKT_TRANSFER:
         case SPKT_VENDOR_CFG_TOKEN:
            /* use notice level cmslog to avoid periodic noisy error prints in console*/
            cmsLog_notice("unknown keyType=%d!", keyType);
            return NULL;
#endif
         default:
            cmsLog_error("unknown keyType=%d!", keyType);
   ...
   {
      cmsLog_error("unknown g_eIdTR69C=%d!", g_eIdTR69C);
      return NULL;
   }
}
