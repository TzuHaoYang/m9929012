...
#endif
{
 ...
 sprintf(cmd,"iptables -w -t nat -D PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to %s",langw, primary);
      		bcmsystem(cmd);
 sprintf(cmd,"iptables -w -t nat -A PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to %s",langw, secondary);
      		bcmsystem(cmd);
		onsecondary=1;
	    }
...
void do_query()
{
   ...
   sprintf(cmd, "iptables -w -t nat -D PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to 128.9.0.107 2>/dev/null", langw);
      		bcmsystem(cmd);

		    sprintf(cmd,"iptables -w -t nat -D PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to %s 2>/dev/null",langw, primary);
      		    bcmsystem(cmd);
		    ...
		    bzero(cmd,128);
      		    sprintf(cmd,"iptables -w -t nat -D PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to %s 2>/dev/null",langw, secondary);
      		    bcmsystem(cmd);				  
   ...
   sprintf(cmd, "iptables -w -t nat -D PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to 128.9.0.107 2>/dev/null", langw);
		...
		bzero(cmd,128);
		 ...
		 /*added command to clear dns ip_conntrack*/
 		 ...
 		 bzero(cmd,128);			 
      		 sprintf(cmd, "iptables -w -t nat -A PREROUTING -i br0 -d %s -p udp --dport 53 -j DNAT --to %s",langw, primary);
      		bcmsystem(cmd);
		onsecondary=0;
	}
}
