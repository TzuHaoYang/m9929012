...
static int RegisterHandler( POPTION_INFO pOptions, int nNumOptions );
static int UnregisterHandler( POPTION_INFO pOptions, int nNumOptions );
...
/***************************************************************************
 * The following defines the commands supported by the spuctl userspace app:
 *  start      - Invokes the underlying SPU driver to register its supported
 *               crypto algorithms to the kernel and add ip xfrm entries and
 *               launch strongSwan to *start* the process of setting up
 *               IPSec tunnels as specified in the configuration files of
 *               the corresponding tools.
 *             - NOTE: This command should only be used when the platform
 *               does not depend on CMS to administrate the setup of IPSec
 *               tunnels.
 *  stop       - Stops strongSwan tool and remove ip xfrm entries, which
 *               tear down all IPSec tunnels managed by the tools and
 *               invokes the underlying SPU driver to unregister its
 *               supported crypto algorithms from the kernel.
 *             - NOTE: This command should only be used when the platform
 *               does not depend on CMS to administrate the teardown of
 *               IPSec tunnels.
 *  register   - Invokes the underlying SPU driver to register its supported
 *               crypto algorithms to the kernel *only* (e.g. both the ip
 *               xfrm and strongSwan tools are not invoked by this command.)
 *             - NOTE: This command is expected to get invoked at system 
 *               bootup time (via scripting in init.d) alongside with CMS to
 *               bring the underlying SPU driver to a state as expected by
 *               CMS where all supported crypto algorithms should have been
 *               registered to the kernel.
 *  unregister - Invokes the underlying SPU driver to unregister its
 *               supported crypto algorithms from the kernel *only* (e.g.
 *               both ip xfrm and strongSwan tools are not invoked.)
 *  test       - Kicks off the test function as supported by the SPU driver.
 *  --help     - Dumps the list of commands supported by this tool.
 ***************************************************************************/
...
static int StartHandler( POPTION_INFO pOptions, int nNumOptions )
{
    ...
    system("/var/ipsec/setkey.sh");
    system("ipsec start 2>/dev/null");
 ...
 * Description  : Processes the spuctl register command.
 * Returns      : 0 - success, non-0 - error
 ***************************************************************************/
static int RegisterHandler( POPTION_INFO pOptions, int nNumOptions )
{
    ...
    if ( err )
    {
        printf("Register spu command failed\n");
    }
    return( err );
} /* RegisterHandler */
 ...
 * Description  : Processes the spuctl unregister command.
 * Returns      : 0 - success, non-0 - error
 ***************************************************************************/
static int UnregisterHandler( POPTION_INFO pOptions, int nNumOptions )
{
    ...
    if ( err )
    {
        printf("Unregister spu command failed\n");
    }
    return( err );
} /* UnregisterHandler */
...
{
    fprintf( stderr,
    "Usage: %s start      -> Register ALGs and Start SPU Device\n"
    "       %s stop       -> Unregister ALGs and Stop SPU Device\n"
    "       %s register   -> Register ALGs for SPU Device\n"
    "       %s unregister -> Unregister ALGs for SPU Device\n"
