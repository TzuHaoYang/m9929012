...
\n\
Options:\n\
[-p probe_id]   Pre-defined probe configuration to be used, 1 to 3\n\
...
GCAP_GEN int GCAP_119_main(int argc, char **argv)
{
    ...
    for (i=1; i < argc; i++)
    {
      ...
      {
         rxd_req.probe_id = atoi(argv[i+1])-1;
         probe_set = 1;
         i++; 
      }
    ...
    while((ret = getopt(argc, argv, "hi:p:n:")) != -1) 
    {
        ...
        case 'p':
            ...
            // NOTE: the GCAP probe_ids are 1..3, the API probe_ids are 0..2

            rxd_req.probe_id = atoi(optarg)-1;
            probe_set = 1;
            break;
        case 'n':
    ...
    else if ((rxd_req.probe_id < 0) || (rxd_req.probe_id > 2))
    {
        printf( "Error! Invalid probe id: %d\n", rxd_req.probe_id+1);
        return(-3);
    }

    // Probe ID 0 and 1 are for primary channel, Probe ID 2 is for the secondary channel
    if (rxd_req.probe_id == 2)
       rxd_req.channel_id = 1;
    else
