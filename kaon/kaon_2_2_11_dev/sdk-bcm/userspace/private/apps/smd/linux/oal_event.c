...
static void processIsAppRunning(DlsInfo * dInfo, CmsMsgHeader **msg);
static void processIsAppActive(DlsInfo * dInfo, CmsMsgHeader **msg);
...
void processLaunchConfirmation(DlsInfo *dInfo)
{
   ...
   {
      dInfo->msgQueue = (CmsMsgHeader *) dInfo->msgQueue->next;
      msg->next = 0L;
...
void terminateApp(DlsInfo **dInfo)
{
   ...
   case EID_PPP:
      /*
       * Special case for PPP.  SIGWINCH tells ppp to to erase its session info
       * from the persistent scratch pad.  Technically, we shouldn't do this
       * if the link goes down, but in practice, any impact on ppp reconnect
       * times will be overshadowed by DSL training.
       */
...
void collectApp(DlsInfo **dInfo)
{
   ...
   else
   {
      ...
      { 
        /*
         * There have been issues with PPP not shutting down gracefully.
         * It should be fixed now, but just in case, use a shorter timeout.
         */
         collectInfo.timeout = 2 * MSECS_IN_SEC; 
      }

      if ( (*dInfo)->eInfo->eid == EID_RADVD )
      {
         collectInfo.timeout = 2 * MSECS_IN_SEC;

         if (prctl_collectProcess(&collectInfo, &procInfo) != CMSRET_SUCCESS)
         {
            /* One restart may not finish completely and the other comes. */
            prctl_terminateProcessForcefully((*dInfo)->pid);
            collectInfo.timeout = 3 * MSECS_IN_SEC;
         }
         else
         {
            done = TRUE;
      ...
      if (!done && (ret = prctl_collectProcess(&collectInfo, &procInfo)) != CMSRET_SUCCESS)
      {
...
void queueMsg(DlsInfo *dInfo, CmsMsgHeader **msg)
{
   ...
   else
   {
      ...
      while (msgHeader->next != 0L)
      {
         msgHeader = (CmsMsgHeader *) msgHeader->next;
      }
      msgHeader->next = (uint64_t)(*msg);
   }
   ...
   if ((*msg)->dst == EID_SMD)
   {
      ...
      else
      {
         ...
         case CMS_MSG_IS_APP_RUNNING:
            processIsAppRunning(dInfo, msg);
            break;
            
         case CMS_MSG_IS_APP_ACTIVE:
            processIsAppActive(dInfo, msg);
            break;
...
void processValidateConfigFile(DlsInfo *dInfo, CmsMsgHeader **msg)
{
   ...
   // Let cmsMgm_validateConfigBuf do the locking for us (full auto-lock).
   ret = cmsMgm_validateConfigBuf((char *) ((*msg) + 1), (*msg)->dataLength);
...
void processIsAppRunning(DlsInfo *dInfo, CmsMsgHeader **msg)
{
   DlsInfo *targetDinfo;
...
void processIsAppActive(DlsInfo *dInfo, CmsMsgHeader **msg)
{
   DlsInfo *targetDinfo;
   CmsEntityId targetEid;
   CmsRet rv;
   ...
   if (targetDinfo)
   {
      if (targetDinfo->state == DLS_RUNNING || targetDinfo->state == DLS_LAUNCHED)
          rv = CMSRET_SUCCESS;
   }
