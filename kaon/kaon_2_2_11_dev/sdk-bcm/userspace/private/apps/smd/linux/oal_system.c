...
#else
   /*
    * On the actual Gateway, block SIGINT because user might press control-c to
    * stop a ping command or something.
    */
   signal(SIGINT, SIG_IGN);

   /*
    * When busybox reboots, it will send a SIGTERM to all processes.
    * Smd should ignore it and stay alive so that other apps do not notice smd
