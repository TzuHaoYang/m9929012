...
void processLanHostInfoMsg_igd(CmsMsgHeader *msg)
{
    ...
    {
        cmsLog_notice("could not find bridge ifname %s", body->ifName);
    ...
    if (body->deleteHost)
    {
        ...
        if ((ret = rutLan_getHostEntryByMacAddr(&iidStack, body->macAddr, &searchIidStack, &hostEntry)) == CMSRET_SUCCESS)
        {
    ...
    else
    {
        ...
        else
        {
            ...
            else if((ret= cmsObj_get(MDMOID_LAN_HOST_ENTRY, &iidStack, OGF_NO_VALUE_UPDATE, (void **) &hostEntry))!=CMSRET_SUCCESS) {

                cmsLog_error("could not get instance of lan host returned %d", ret);
                goto out;
            }
        }

        /* both new and update need to update all the field of the hostEntry when it turns from none active to active */
