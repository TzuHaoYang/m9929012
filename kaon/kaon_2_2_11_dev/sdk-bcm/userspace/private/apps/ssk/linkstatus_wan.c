...
void matchRdpaWanType(const char *wanTypeUp)
{
   ...
   if (token)
   {
       bcm_enet_map_ifname_to_unit_port(token, &unit, &port);
       sprintf(wanemac, "EMAC%d", port);
...
#define PON_STATUS_CHANGED(u, s) \
        ((u && cmsUtl_strcmp(s, MDMVS_UP)) || \
        ((!u) && (!cmsUtl_strcmp(s, MDMVS_UP))));

#ifndef EPON_SFU
static CmsRet updatePonInterfaceConfig(InstanceIdStack *wanDevIidStack, UBOOL8 isUp)
{
    WanPonIntfObject *wanPonIntfCfg = NULL;
    InstanceIdStack wanPonIntfIidStack = EMPTY_INSTANCE_ID_STACK;
    UBOOL8 isPonIntfChanged;
    CmsRet ret = CMSRET_SUCCESS;

    while (cmsObj_getNextInSubTreeFlags(MDMOID_WAN_PON_INTF,
         wanDevIidStack, &wanPonIntfIidStack, OGF_NO_VALUE_UPDATE, (void **) &wanPonIntfCfg) == CMSRET_SUCCESS)
    {
        isPonIntfChanged = PON_STATUS_CHANGED(isUp, wanPonIntfCfg->status);
        if (isPonIntfChanged)
        {
            CMSMEM_REPLACE_STRING(wanPonIntfCfg->status, isUp?MDMVS_UP:MDMVS_DISABLED);
            if ((ret = cmsObj_set(wanPonIntfCfg, &wanPonIntfIidStack)) != CMSRET_SUCCESS)
            {
                cmsLog_error("Failed to updatePonIntfConifg. ret = %d", ret);
...
static inline UBOOL8 isWanConnectionFound(SINT32 wanVlanId, SINT32 wanPbit, SINT32 vlanId, SINT32 pbit)
{
    /* take care of don't care filter */
    if (((wanVlanId == -1) && (wanPbit == -1)) ||
          ((vlanId == -1) && (pbit == -1)))
        return TRUE;

    if ((wanVlanId == -1) || (vlanId == -1))
        return (wanPbit == pbit);

    if ((wanPbit == -1) || (pbit == -1))
        return (wanVlanId == vlanId);

    return ((wanVlanId == vlanId) && (wanPbit == pbit));
}

/*
* Update the status of all WANIPConnection/WANPPPConnection under same parentIid(IGD.WANDevice.{i}.WANConnectionDevice.{i}.)
*/
static CmsRet updatePonWanServiceStatus(InstanceIdStack *parentIidStack, SINT32 vlanId, SINT32 pbit, UBOOL8 isUp)
{
    InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
    ...
    cmsLog_debug("Eneter::isUp=%s, vlanId=%d, pbits=%d", isUp?"TRUE":"FALSE", vlanId, pbit);
    cmsLog_debug("Eneter::WanConnectionDevice iid=%s", cmsMdm_dumpIidStack(parentIidStack));

    while (cmsObj_getNextInSubTreeFlags(MDMOID_WAN_IP_CONN,
        parentIidStack, &iidStack, OGF_NO_VALUE_UPDATE, (void **) &ipConn) == CMSRET_SUCCESS)
    {
        found = isWanConnectionFound(ipConn->X_BROADCOM_COM_VlanMuxID, ipConn->X_BROADCOM_COM_VlanMux8021p,
                                     vlanId, pbit);

        cmsLog_debug("WanIpConnObject, ifName=%s, status=%s, found=%s",
            ipConn->X_BROADCOM_COM_IfName, ipConn->connectionStatus, found?"TRUE":"FALSE");
        ...
        if (found)
        {
            if (isUp)
            {
                ...
                {
                    cmsLog_debug("Activate pon interface %s and send link up message", ipConn->X_BROADCOM_COM_IfName);
            ...
            else
            {
                ...
                {
                    cmsLog_debug("Deactivate pon interface %s and send link down message", ipConn->X_BROADCOM_COM_IfName);
    ...
    while (cmsObj_getNextInSubTreeFlags(MDMOID_WAN_PPP_CONN,
        parentIidStack, &iidStack, OGF_NO_VALUE_UPDATE, (void **) &pppConn) == CMSRET_SUCCESS)
    {
        found = isWanConnectionFound(pppConn->X_BROADCOM_COM_VlanMuxID, pppConn->X_BROADCOM_COM_VlanMux8021p,
                                     vlanId, pbit);

        cmsLog_debug("WanPppConnObject, ifName=%s, status=%s, found=%d",
           pppConn->X_BROADCOM_COM_IfName, pppConn->connectionStatus, found?"TRUE":"FALSE");
        ...
        if (found)
        {
            if (isUp)
            {
                ...
                {
                    cmsLog_debug("Activate pon interface %s and send link up message", pppConn->X_BROADCOM_COM_IfName);
                    sendStatusMsgToSmd(CMS_MSG_WAN_LINK_UP, pppConn->X_BROADCOM_COM_IfName);
                    updateSingleWanConnStatusLocked(&iidStack, (void *)pppConn, TRUE);
                }
            }
            else
            {
                ...
                {
                    cmsLog_debug("Deactivate pon interface %s and send link down message", pppConn->X_BROADCOM_COM_IfName);
...
static CmsRet updateGponLinkConfig(InstanceIdStack *iid, WanGponLinkCfgObject *gponLinkCfg, UBOOL8 isUp)
{
    CmsRet ret = CMSRET_SUCCESS;
    ...
    if ((ret = cmsObj_set(gponLinkCfg, iid)) != CMSRET_SUCCESS)
    {
        cmsLog_error("Failed to set WanGponLinkCfgObject. ret = %d", ret);
    }

    return ret;
}

static CmsRet updateGponServiceStatus(const OmciServiceMsgBody *info)
{
    WanConnDeviceObject *wanConnDevObj = NULL;
    WanGponLinkCfgObject *wanGponLinkCfgObj = NULL;
    InstanceIdStack wanDevIidStack = EMPTY_INSTANCE_ID_STACK;
    InstanceIdStack wanConnDevIidStack = EMPTY_INSTANCE_ID_STACK;
    InstanceIdStack wanGponLinkIidStack = EMPTY_INSTANCE_ID_STACK;
    CmsRet ret = CMSRET_SUCCESS;
    SINT32 vlanId;
    SINT32 pbits;
    UBOOL8 isUp;
    UBOOL8 isGponLinkChanged;
    UINT16 connectionNum;
    extern UINT16 rutWan_getNumberOfActiveConnections(const InstanceIdStack *parentIidStack);

    if ((ret = dalGpon_getGponWanIidStatck(&wanDevIidStack)) != CMSRET_SUCCESS)
    {
        cmsLog_error("Failed to getGponWanIidStatck. ret = %d", ret);
        return ret;
    }

    vlanId = info->serviceParams.vlanId;
    pbits = info->serviceParams.pbits;
    isUp = (info->serviceParams.serviceStatus == TRUE);

    if (isUp)
    {
        if ((ret = updatePonInterfaceConfig(&wanDevIidStack, isUp)) != CMSRET_SUCCESS)
        {
            cmsLog_error("Failed to updatePonInterfaceConfig. ret = %d", ret);
            return ret;
        }
    }

    while (cmsObj_getNextInSubTreeFlags(MDMOID_WAN_CONN_DEVICE,
        &wanDevIidStack, &wanConnDevIidStack, OGF_NO_VALUE_UPDATE, (void **) &wanConnDevObj) == CMSRET_SUCCESS)
    {
        while (cmsObj_getNextInSubTreeFlags(MDMOID_WAN_GPON_LINK_CFG,
            &wanConnDevIidStack, &wanGponLinkIidStack, OGF_NO_VALUE_UPDATE, (void **) &wanGponLinkCfgObj) == CMSRET_SUCCESS)
        {
            if (!cmsUtl_strcmp(wanGponLinkCfgObj->ifName, info->l2Ifname))
            {
                /* Update WAN IP/PPP connection status under current GPON link Config */
                ret = updatePonWanServiceStatus(&wanConnDevIidStack, vlanId, pbits, isUp);

                /* Update status of current GPON link Config */
                isGponLinkChanged = PON_STATUS_CHANGED(isUp, wanGponLinkCfgObj->linkStatus);
                cmsLog_debug("isGponLinkChanged %d, isUp %d", isGponLinkChanged, isUp);
                if(isGponLinkChanged)
                {
                    if (isUp)
                    {
                        ret = updateGponLinkConfig(&wanGponLinkIidStack, wanGponLinkCfgObj, isUp);
                    }
                    else
                    {
                        connectionNum = rutWan_getNumberOfActiveConnections(&wanGponLinkIidStack);
                        if (connectionNum == 0)
                            ret = updateGponLinkConfig(&wanGponLinkIidStack, wanGponLinkCfgObj, isUp);
                    }
                }
        ...
        cmsObj_free((void **) &wanConnDevObj);
    }

    if (!isUp)
    {
        connectionNum = rutWan_getNumberOfActiveConnections(&wanDevIidStack);
        if (connectionNum == 0)
        {
            ret = updatePonInterfaceConfig(&wanDevIidStack, isUp);
        }
    }
    return ret;
}
...
void processGponWanServiceStatusChange_igd(CmsMsgHeader *msg)
{
    ...
    /* Function updateGponServiceStatus() triggers the retrieval/update on many objects
     * lock all zones to be safe.
     */
    ...
    ret = updateGponServiceStatus(info);
...
#ifdef EPON_HGU
static CmsRet updateEponLinkConfig(InstanceIdStack *iid, WanEponLinkCfgObject *eponLinkCfg, UBOOL8 isUp)
{
    CmsRet ret = CMSRET_SUCCESS;
    ...
    if ((ret = cmsObj_set(eponLinkCfg, iid)) != CMSRET_SUCCESS)
    {
        cmsLog_error("Failed to set WanEponLinkCfgObject. ret = %d", ret);
    }

    return ret;
}

static CmsRet updateEponServiceStatus(const EponLinkStatusMsgBody *info)
{
    WanConnDeviceObject *wanConnDevObj = NULL;
    WanEponLinkCfgObject *wanEponLinkCfgObj = NULL;
    InstanceIdStack wanDevIidStack = EMPTY_INSTANCE_ID_STACK;
    InstanceIdStack wanConnDevIidStack = EMPTY_INSTANCE_ID_STACK;
    InstanceIdStack wanEponLinkIidStack = EMPTY_INSTANCE_ID_STACK;
    CmsRet ret = CMSRET_SUCCESS;
    SINT32 vlanId;
    SINT32 pbits;
    UBOOL8 isUp;
    UBOOL8 isEponLinkChanged;

    if ((ret = dalEpon_getEponWanIidStatck(&wanDevIidStack)) != CMSRET_SUCCESS)
    {
        cmsLog_error("Failed to getEponWanIidStatck. ret = %d", ret);
        return ret;
    }

    vlanId = info->vlanId;
    pbits= info->pbits;
    isUp = (info->linkStatus == EPON_LINK_STATUS_UP);
    ...
    while (cmsObj_getNextInSubTreeFlags(MDMOID_WAN_CONN_DEVICE,
        &wanDevIidStack, &wanConnDevIidStack, OGF_NO_VALUE_UPDATE, (void **) &wanConnDevObj) == CMSRET_SUCCESS)
    {
        while (cmsObj_getNextInSubTreeFlags(MDMOID_WAN_EPON_LINK_CFG, 
            &wanConnDevIidStack, &wanEponLinkIidStack, OGF_NO_VALUE_UPDATE, (void **) &wanEponLinkCfgObj) == CMSRET_SUCCESS)
        {
            if (!cmsUtl_strcmp(wanEponLinkCfgObj->ifName, info->l2Ifname))
            {
                isEponLinkChanged = PON_STATUS_CHANGED(isUp, wanEponLinkCfgObj->linkStatus);
                if(isEponLinkChanged)
                {
                    if ((ret = updateEponLinkConfig(&wanEponLinkIidStack, wanEponLinkCfgObj, isUp)) == CMSRET_SUCCESS)
                    {
                        ret = updatePonWanServiceStatus(&wanConnDevIidStack, vlanId, pbits, isUp);
                    }
                }
            }
            cmsObj_free((void **) &wanEponLinkCfgObj);
        }
        cmsObj_free((void **) &wanConnDevObj);
    }
...
#else //EPON_SFU
static CmsRet updateEponIntfConifg(InstanceIdStack *iid, WanEponIntfObject *eponIntfCfg, UBOOL8 isUp)
{
    CmsRet ret = CMSRET_SUCCESS;
    ...
    if (isUp)
        ...
        CMSMEM_REPLACE_STRING(eponIntfCfg->status, MDMVS_DISABLED);

    if ((ret = cmsObj_set(eponIntfCfg, iid)) != CMSRET_SUCCESS)
    {
...
static CmsRet updateEponServiceStatus(const EponLinkStatusMsgBody *info)
{
    ...
    WanEponIntfObject *wanEponIntfCfg = NULL;
    InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
    UBOOL8 isEponIntfChanged;
    UBOOL8 isUp;

    // Find the EPON interface config object for the given interface name
    if (dalEpon_getEponIntfByIfName((char *)info->l2Ifname, &iidStack, &wanEponIntfCfg) == FALSE)
    {
        return CMSRET_INTERNAL_ERROR;
    }

    isUp = (info->linkStatus == EPON_LINK_STATUS_UP);
    isEponIntfChanged = PON_STATUS_CHANGED(isUp, wanEponIntfCfg->status);

    //Update EPON interface config and WAN connection services status
    if(isEponIntfChanged)
    {
        if ((ret = updateEponIntfConifg(&iidStack, wanEponIntfCfg, isUp)) != CMSRET_SUCCESS)
        {
            cmsLog_error("Failed to set WanEponIntfObject. ret=%d", ret);
        }
        else
        ...
        if ((ret = cmsObj_getAncestorFlags(MDMOID_WAN_DEV,
            MDMOID_WAN_EPON_INTF, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&wanDev)) != CMSRET_SUCCESS)
        {
            cmsLog_error("Fail to get cmsObj_getAncestor(MDMOID_WAN_CONN_DEVICE). ret=%d", ret);
        }
        else
        {
            updateWanConnStatusInSubtreeLocked(&iidStack, isUp);
        }
    ...
    cmsObj_free((void **)&wanEponIntfCfg);
    return ret;
}
#endif
...
void processEponWanLinkChange_igd(CmsMsgHeader *msg)
{
    ...
    if ((ret = updateEponServiceStatus(info)) != CMSRET_SUCCESS)
    {
        cmsLog_error("updateEponServiceStatus failed, ret=%d", ret);
    }

    cmsLck_releaseLock();

    /* Give eponapp a response after all is done */
    replyMsg.type = msg->type;
    replyMsg.src = EID_SSK;
    replyMsg.dst = msg->src;
