...
static CmsRet voice_GetIpFamily(int *isIpv6)
{
   char ipAddrFamily[BUFLEN_16]={0};
   CmsRet ret;

   ret = dalVoice_GetIpFamily( NULL, ipAddrFamily, sizeof(ipAddrFamily) );
...
#endif // defined(BRCM_VOICE_SUPPORT) && VOICE_IPV6_SUPPORT
...
void processDhcpcStateChanged_igd(CmsMsgHeader *msg)
{
   ...
   /* Processing below may trigger the retrieval/update on many objects,
                    ...
                    == CMSRET_SUCCESS)
            {
               if (ipHost->managedEntityId == bcmIpHost->managedEntityId)
               {
                  ...
                  if (inet_aton(dhcpcInfo->ip, &inAddr) != 0)
                     ipHost->currentAddress = ntohl(inAddr.s_addr);
                  if (inet_aton(dhcpcInfo->mask, &inAddr) != 0)
                     ipHost->currentMask = ntohl(inAddr.s_addr);
                  if (inet_aton(dhcpcInfo->gateway, &inAddr) != 0)
                     ipHost->currentGateway = ntohl(inAddr.s_addr);
                  ...
                  if ((ptr = strstr(dhcpcInfo->nameserver, ",")))
                  {
                      ...
                      {
                          ipHost->currentPrimaryDns = ntohl(inAddr.s_addr);
                          if (inet_aton(&ptr[1], &inAddr) != 0)
                              ipHost->currentSecondaryDns= ntohl(inAddr.s_addr);
                      }
                  }
         ...
         else  /* IFADDRCONF_REMOVE: prefix remove */
         {
            ...
            if (ipConn)
            {
   			    if (cmsUtl_strcmp(ipConn->X_BROADCOM_COM_IPv6SitePrefix, dhcp6cInfo->sitePrefix) == 0)
   			    {
   			       ...
   			       cmsLog_debug("Remove current Prefix");
			    ...
			    else if (cmsUtl_strcmp(ipConn->X_BROADCOM_COM_IPv6SitePrefixOld, dhcp6cInfo->sitePrefix) == 0)
			    {
				    CMSMEM_FREE_BUF_AND_NULL_PTR(ipConn->X_BROADCOM_COM_IPv6SitePrefixOld);
				    ipConn->X_BROADCOM_COM_IPv6SitePrefixVltimeOld = 0;
			    }
   		    }
   		    else
   		    {
				...
				else if (cmsUtl_strcmp(pppConn->X_BROADCOM_COM_IPv6SitePrefixOld, dhcp6cInfo->sitePrefix) == 0)
			    {
				   CMSMEM_FREE_BUF_AND_NULL_PTR(pppConn->X_BROADCOM_COM_IPv6SitePrefixOld);
				   pppConn->X_BROADCOM_COM_IPv6SitePrefixVltimeOld = 0;
			    }
   		    }
         }
      }
...
void processRAStatus6Info_igd(const CmsMsgHeader *msg)
{
   ...
   if (found)
   {
      if (ipConn)
      {
         ...
         if (!IS_EMPTY_STRING(raInfo->pio_prefix))
         {
             *raPrefix = '\0'; 
             sprintf(raPrefix, "%s/%u", raInfo->pio_prefix, raInfo->pio_prefixLen);
             REPLACE_STRING_IF_NOT_EQUAL(ipConn->X_BROADCOM_COM_IPv6RaPrefix, raPrefix);
         }
...
void processPppStateChanged_igd(CmsMsgHeader *msg)
{
   ...
   {
#ifdef DMP_X_BROADCOM_COM_L2TPAC_1  
      if(!cmsUtl_strncmp(pppConn->name, "L2tp", 4) && pid > 0)
      {
	      if(!cmsUtl_strcmp(pppConn->X_BROADCOM_COM_IfName, pppInfo->ifname))
	      {
