...
static CmsRet sendMdmInitializedMsg(void);
...
static void processPostActivating(CmsMsgHeader *msg);
static void processMdmInitDone(void);
...
static void processEponSetAcs(CmsMsgHeader *msg);
#endif

static UBOOL8 initLinkCheckDone = FALSE;
...
SINT32 ssk_main()
{
   ...
   CmsTimestamp start_tms1, end_tms1;
   UINT32 deltaMs1 = 0;
   ...
   while (keepLooping)
   {
      ...
      */
      ...
      if (FD_ISSET(commFd, &readFds))
      {
         ...
         case CMS_MSG_RASTATUS6_HOST6_INFO:
            processLanHostInfoMsg(msg);
            break;
         ...
         case CMS_MSG_RESTART_VOICE_CALLMGR:
            processVoiceRestartCmgr();
            break;

         case CMS_MSG_REBOOT_VOICE:
            processVoiceReboot();
            break;
         ...
         case CMS_MSG_EPON_GET_QUEUE_CONFIG:
            ...
            break;
         case CMS_MSG_EPON_SET_ACS:
            processEponSetAcs(msg);
            break;
         ...
         case CMS_MSG_WIFI_UPDATE_ASSOCIATEDDEVICE:
            processAssociatedDeviceUpdated(msg);
            break;
#endif
         ...
         cmsTms_get(&end_tms1);
         deltaMs1 = cmsTms_deltaInMilliSeconds(&end_tms1, &start_tms1);
         if (deltaMs1 >= (2 * MSECS_IN_SEC))
         {
             cmsLog_notice("Long SSK processing time %d ms, type=0x%x",
               deltaMs1, msg->type);
         }
...
static void processMdmInitDone()
{
   CmsRet ret;

   cmsLog_debug("Got MDM_INIT_DONE (initLinkCheckDone=%d)", initLinkCheckDone);
   if (!initLinkCheckDone)
   {
      ...
      initLinkCheckDone = TRUE;
      ret = sendMdmInitializedMsg();
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("Could not send MDM_INITIALIZED to smd");
      }
   }
   else
   {
      cmsLog_debug("initLinkCheck already done?!?  Msg ignored.");
   }
...
static CmsRet sendMdmInitializedMsg(void)
{
   ...
   cmsLog_notice("Sending MDM_INITIALIZED to smd...");
   ...
   *dmPtr = (UINT32) cmsMdm_isDataModelDevice2();
   ...
   {
      cmsLog_error("send of MDM_INITIALIZED event msg failed. ret=%d", ret);
   }
...
void updateLinkStatus_igd(const char *intfName)
{
   ...
   while (!gotLock)
   {
      ret = cmsLck_acquireAllZoneLocksWithBackoff(0, SSK_LOCK_TIMEOUT);
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("could not get lock, ret=%d, sleep 1 and try again...", ret);
         sleep(1);
      }
      else
      {
   ...
   // Link status changes can take longer to process.
   ...
   if (!found)
   {
      ...
      checkLanLinkStatusLocked_igd(intfName);
   }
...
void processSpeedServiceComplete(CmsMsgHeader *msg __attribute__((unused)))
{
   ...
   rc = fscanf(result, "%s %s %u %u %u %u %u %u %u %s",
...
#ifdef SUPPORT_WEB_SOCKETS
   {
      ...
      cmsMsg.type = CMS_MSG_SPDSVC_DIAG_EVENT;
      cmsMsg.src = EID_SSK;
      ...
      {
         cmsLog_error("could not send out CMS_MSG_SPDSVC_DIAG_EVENT to requestId %d, ret=%d", ret, EID_WEBSOCKD);
      }
      else
      {
         cmsLog_notice("sent out CMS_MSG_SPDSVC_DIAG_EVENT to requesterId %d", EID_WEBSOCKD);
      }
   }
...
extern CmsRet addPersistentWanEthInterfaceObject(const char *ifName,
  const char *pGMACPortList);
extern CmsRet delLanEthInterfaceObject(const char *ifName);
#endif
#ifdef DMP_DEVICE2_ETHERNETINTERFACE_1
extern CmsRet addPersistentWanEthInterfaceObject_dev2(const char *ifName);
#endif /* DMP_DEVICE2_BASELINE_1 */
...
#elif defined(DMP_DEVICE2_ETHERNETINTERFACE_1)
    ret = addPersistentWanEthInterfaceObject_dev2(if_name);
...
void processPostActivating(CmsMsgHeader *msg)
{
   MdmPostActNodeInfo *info = (MdmPostActNodeInfo *) (msg + 1);

   if (info->subType != MDM_POST_ACT_TYPE_FILTER)
   {
      cmsLog_error("Unsupported subtype %d", info->subType);
      return;
   }

#if defined(BCM_PON) && defined(DMP_X_BROADCOM_COM_RDPA_1)
   {
      ...
      {
         if (filtersCfgObj->ready == FALSE)
         {
            cmsLog_debug("mark hardware ready!!");
            filtersCfgObj->ready = TRUE;
            if ((ret = cmsObj_set(filtersCfgObj, &iidStack)) != CMSRET_SUCCESS)
            {
               cmsLog_error("set of filtersCfg failed, ret=%d", ret);
            }
         }
      ...
      {
         cmsLog_error("Get of filtersCfg failed, ret=%d", ret);
      }

      cmsLck_releaseLock();
   }
#else
   cmsLog_error("POST_ACTIVATING msg has no effect on non-PON system");
#endif
...
void processEponSetQueueConfig(CmsMsgHeader *msg)
{
   ...
   for (i = 0; i < queueConfig->bcmQueueCfg.portNum; i++)
   {
       ...
       {
           cmsLog_error("Failed to add MDMOID_DOWN_PORT inst %d, ret=%d", i, ret);
           goto exit;
       }
...
void processEponSetAcs(CmsMsgHeader *msg)
{
   CmsMsgHeader replyMsg = EMPTY_MSG_HEADER;
   CmsRet ret;

#if defined(BRCM_PKTCBL_SUPPORT) && defined(SUPPORT_TR69C)
   EponSetAcsMsgBody *acsConfig = (EponSetAcsMsgBody *)(msg + 1);
   ...
   setAcsParams(acsConfig->acsURL, acsConfig->acsProvisioningCode,
                   acsConfig->cwmpRetryMinimumWaitInterval,
                   acsConfig->cwmpRetryIntervalMultiplier);

   replyMsg.wordData = ret;
   cmsLck_releaseLock();
#else
   replyMsg.wordData = CMSRET_METHOD_NOT_SUPPORTED;
#endif
