...
void processDhcpcStateChanged_dev2(CmsMsgHeader *msg)
{
   ...
   if (dhcpcInfo->addressAssigned)
   {
      ...
      rutDhcp_getNtpserversFromOption42(ipIntfObj->name, dhcpcInfo->ntpserver, 
                                        sizeof(dhcpcInfo->ntpserver));
...
#if defined(BRCM_PKTCBL_SUPPORT)
      rutDhcp_getAcsUrlFromOption125(ipIntfObj->name, dhcpcInfo->acsURL, CMS_MAX_ACS_URL_LENGTH);
#endif // defined(BRCM_PKTCBL_SUPPORT)
      ...
      if (cmsUtl_strcmp(ipIntfObj->name, TR69C_2_ASSOCIATED_WAN_IF_NAME) == 0)
      {
             ...
             dhcpcInfo->cwmpRetryIntervalMultiplier != 0)
         {
            cmsLog_debug("got acsURL=%s provisioningCode=%s MinWaitInterval=%d intMult=%d",
                         ...
                         dhcpcInfo->cwmpRetryMinimumWaitInterval,
                         dhcpcInfo->cwmpRetryIntervalMultiplier);
            setAcs2Params(dhcpcInfo->acsURL, dhcpcInfo->acsProvisioningCode,
                         dhcpcInfo->cwmpRetryMinimumWaitInterval,
                         dhcpcInfo->cwmpRetryIntervalMultiplier);
         }
      }
      ...
      initVoiceOnIntfUpLocked_dev2(CMS_AF_SELECT_IPV4, ipIntfObj->name, 1);
#endif
   }
   else
   {
...
void processPppStateChanged_dev2(CmsMsgHeader *msg)
{
   ...
   while (!found && (cmsObj_getNextFlags(MDMOID_DEV2_PPP_INTERFACE, &pppIntfIidStack,
          OGF_NO_VALUE_UPDATE, (void **)&pppIntfObj) == CMSRET_SUCCESS))
   {
      if (!cmsUtl_strcmp(pppIntfObj->name, "PPPoL2tpAc") && pid > 0)
      {
	     found = TRUE;
	     cmsLog_debug("found PPPoL2tpAc interface");
         cmsLck_releaseLock();
         return; /* no action needed for L2TP PPP case, just return */
      }
      else
      {
         /*
   ...
   case BCM_PPPOE_CLIENT_STATE_DOWN:
      ...
      /* connectionStatus is what drives the ppp state machine in the rcl */
      sskConn_setPppConnStatusByObjLocked(&pppIntfIidStack, pppIntfObj, MDMVS_DISCONNECTED);

      /*
       * Tell all layers above the PPP layer that its lower layer has gone down.
       * Normally, this is the IP.Interface object,
       * where it will set the service state machines to SERVICEDOWN.
       * When we go to SERVICEDOWN state, the dynamically created
       * IPv4Address object will be deleted in the teardown step.
       * Note we do not set the PPP object's status to LOWERLAYER down because
       * we don't actually know if the lower layer is down or not.  PPP down
       * could be caused by a failure in the ppp server.
       */
