...
static void checkEthLagLinkStatusLocked_dev2(const char *intfName);
#endif
...
void updateLinkStatus_dev2(const char *intfName)
{
...
#endif
      /*
       * We were given a specific eth lag intf name, so after checking its link
   ...
   if ((intfName == NULL) ||
       (!strncmp(intfName, ETHLAG_IFC_STR, strlen(ETHLAG_IFC_STR))))
   {

#ifdef DMP_DEVICE2_ETHLAG_1
       checkEthLagLinkStatusLocked_dev2(intfName);
#endif
      /*
       * We were given a specific eth lag intf name, so after checking its link
       * status, we can return.
...
void updateBridgeMgmtPortLocked_dev2()
{
                         ...
                         (void **) &brPortObj) == CMSRET_SUCCESS)
   {
    ...
    defined(DMP_DEVICE2_USBINTERFACE_1) || defined(DMP_DEVICE2_MOCA_1) || defined(DMP_DEVICE2_ETHLAG_1)
 ...
 * DMP_DEVICE2_USBINTERFACE_1 || DEVICE2_MOCA 
 * DEVICE2_ETHLAG
 */
...
#ifdef DMP_DEVICE2_ETHLAG_1
void checkEthLagLinkStatusLocked_dev2(const char *intfName)
{
   UBOOL8 found = FALSE;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
...
//   char bridgeIfName[CMS_IFNAME_LENGTH]={0};
    ...
    * function gets called a lot, and reading the enet switch
    * registers can be expensive.  So use OGF_NO_VALUE_UPDATE first to find
    * the exact right ethLagObj without calling the STL handler function which
    * reads from the enet switch.
    */
   while (!found &&
          cmsObj_getNextFlags(MDMOID_LA_G, &iidStack,
                OGF_NO_VALUE_UPDATE, (void **) &ethLagObj) == CMSRET_SUCCESS)
   {

      UBOOL8 isUpstream = ethLagObj->X_BROADCOM_COM_Upstream;
      
      if (intfName)
      {
         if (cmsUtl_strcmp(ethLagObj->name, intfName))
         {
            /* not the intf that has changed status, go to next */
       ...
       */
      cmsLog_debug("doing actual read of %s (upstream=%d)",
                    ethLagObj->name, isUpstream); 
      cmsObj_free((void **) &ethLagObj);
      if (cmsObj_get(MDMOID_LA_G, &iidStack, 0,
      ...
      isLinkStatusChanged = comparePreviousLinkStatus(ethLagObj->name,
                                                      isUpstream,
                                                      ethLagObj->status);
      ...
      if (isLinkStatusChanged)
      {
         ...
         if ( isUpstream)
         {
            if (!cmsUtl_strcmp(ethLagObj->status, MDMVS_UP))
            {
               printf("(ssk) EthLagWan %s link up\n", ethLagObj->name);
               msgType = CMS_MSG_WAN_LINK_UP;
            }
            else
            {
               printf("(ssk) EthLagWan %s link down\n", ethLagObj->name);
               msgType = CMS_MSG_WAN_LINK_DOWN;
            }
         }
         else
         {
            ...
            {
               /* this is a LAN side eth link up msg */
               printf("(ssk) EthLagLan %s link UP\n", ethLagObj->name);
               msgType = CMS_MSG_ETHLAG_LINK_UP;
            }
            else
            {
               printf("(ssk) EthLagLan %s link DOWN\n", ethLagObj->name);
               msgType = CMS_MSG_ETHLAG_LINK_DOWN;
            }
         }

         intfStack_propagateStatusByIidLocked(MDMOID_LA_G,
                                              &iidStack, ethLagObj->status);

         sendStatusMsgToSmd(msgType, ethLagObj->name);

#if 0// todo.. what does mean to bonding ethernet as host TBD.
         if (msgType == CMS_MSG_ETHLAG_LINK_UP)
         {
            updateLanHostEntryActiveStatus_dev2(ethLagObj->name, TRUE);
         }
         else if (msgType == CMS_MSG_ETHLAG_LINK_DOWN)
         {
            updateLanHostEntryActiveStatus_dev2(ethLagObj->name, FALSE);
         }
#endif    
      ...
      cmsObj_free((void **) &ethLagObj);
   } /* while loop over all bond  interfaces */

   cmsLog_debug("Exit intfName=%s", intfName);
...
void checkWifiLinkStatusLocked_dev2(const char *intfName)
{
   ...
   /*
    * Now that we are using the standard Linux netlink mechanism, this
    * function gets called a lot, so use OGF_NO_VALUE_UPDATE first to find
    * the exact right ssidObj without calling the STL handler function
    */
   while (!found &&
          cmsObj_getNextFlags(MDMOID_DEV2_WIFI_SSID, &iidStack,
                OGF_NO_VALUE_UPDATE, (void **) &ssidObj) == CMSRET_SUCCESS)
   {
      if (intfName)
      {
      ...
      /* Now do a real read all the way down to the STL handler function */
      cmsLog_debug("doing actual read of %s", ssidObj->name);
      cmsObj_free((void **) &ssidObj);
      if (cmsObj_get(MDMOID_DEV2_WIFI_SSID, &iidStack, 0,
      ...
      if (isLinkStatusChanged)
      {
         ...
         else
         {
            if (!cmsUtl_strcmp(ssidObj->status, MDMVS_UP))
            {
               ...
               printf("(ssk) Wifi Lan %s link up\n", ssidObj->name);
            }
            else
            {
               msgType = CMS_MSG_WIFI_LINK_DOWN;
               printf("(ssk) Wifi Lan %s link down\n", ssidObj->name);
            }
         }
