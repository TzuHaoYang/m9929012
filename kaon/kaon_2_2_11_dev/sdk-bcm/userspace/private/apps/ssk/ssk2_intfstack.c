                                                  ...
                                                  const char *newStatus)
{
   ...
   {
      cmsLog_debug("in ppp UP special case");
            ...
            !strcmp(newStatus, MDMVS_ERROR))
   {
      ...
      else
      {
          ...
          * seem to be a problem for IPoE interfaces.  See Jira 31621.
          */
         if ((higherLayerPathDesc.oid == MDMOID_DEV2_PTM_LINK) ||
             (higherLayerPathDesc.oid == MDMOID_DEV2_ATM_LINK))
         {
...
UBOOL8 isOnSameStackLocked(const char *higherLayerFullPath, const char *targetFullPath)
{
   ...
   {
      cmsLog_error("Intf Stack inconsistency! could not find obj for %s",
          higherLayerFullPath);
      return FALSE;
   }
...
void setStatusOnPppFirst(const Dev2InterfaceStackObject *pppIntfStackObj)
{
    ...
    * is down.
    */
   findIntfStackEntryLocked(pppIntfStackObj->lowerLayer, NULL,
                            &vlanIntfStackObj, NULL);
