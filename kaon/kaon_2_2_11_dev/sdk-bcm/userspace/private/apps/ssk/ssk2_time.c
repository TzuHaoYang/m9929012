...
void setNtpServers_dev2(const char *ntpServerList)
{
    ...
    if (CMSRET_SUCCESS != (ret = cmsObj_get(MDMOID_DEV2_TIME, &iidStack,
                                            0, (void **) &timeObj)))
    {
    ...
    end = strlen(ntpServerList);
    // The InternetGatewayDevice.Device.Time object allows up to 5 NTP servers
    while (serverNum <= 5)
    {
        start = i;
        len = 0;
...
void processTimeStateChanged_dev2(UINT32 state)
{
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   Dev2TimeObject *timeObj = NULL;
   Dev2DeviceInfoObject *deviceObj = NULL;
   CmsRet ret = CMSRET_SUCCESS;
   ...
   {
      if ((ret = cmsObj_get(MDMOID_DEV2_DEVICE_INFO, &iidStack, 0, (void **) &deviceObj)) == CMSRET_SUCCESS)
      {
