...
void processDhcp6cStateChanged_dev2(const CmsMsgHeader *msg)
{
   ...
   cmsLog_debug("maptAssigned=%d mapeAssigned=%d eaLen=%d v4prefix=%s v6prefix=%s psid=%x BR=%s",
                dhcp6cInfo->maptAssigned, dhcp6cInfo->mapeAssigned, dhcp6cInfo->eaLen, dhcp6cInfo->ruleIPv4Prefix,
   ...
   cmsLog_debug("found the IpIntfObj");
      ...
      rutDhcp6_getAcsUrlFromOption17(ipIntfObj->name, dhcp6cInfo->acsURL, CMS_MAX_ACS_URL_LENGTH);
#endif // defined(BRCM_PKTCBL_SUPPORT)

#if (DMP_X_BROADCOM_COM_MULTIPLE_TR69C_SUPPORT_1 == 2)
      if (cmsUtl_strcmp(ipIntfObj->name, TR69C_1_ASSOCIATED_WAN_IF_NAME) == 0)
...
#if (DMP_X_BROADCOM_COM_MULTIPLE_TR69C_SUPPORT_1 == 2)
   if (cmsUtl_strcmp(ipIntfObj->name, TR69C_2_ASSOCIATED_WAN_IF_NAME) == 0)
   {
      if (strlen(dhcp6cInfo->acsURL) > 0 ||
          strlen(dhcp6cInfo->acsProvisioningCode) > 0 ||
          dhcp6cInfo->cwmpRetryMinimumWaitInterval != 0 ||
          dhcp6cInfo->cwmpRetryIntervalMultiplier != 0)
      {
         cmsLog_debug("got acsURL=%s provisioningCode=%s MinWaitInterval=%d intMult=%d",
                      ...
                      dhcp6cInfo->cwmpRetryMinimumWaitInterval,
                      dhcp6cInfo->cwmpRetryIntervalMultiplier);
         setAcs2Params(dhcp6cInfo->acsURL, dhcp6cInfo->acsProvisioningCode,
                      dhcp6cInfo->cwmpRetryMinimumWaitInterval,
                      dhcp6cInfo->cwmpRetryIntervalMultiplier);
      }
   }
...
#if defined (DMP_TIME_1) || defined (DMP_DEVICE2_TIME_1)
#if defined(BRCM_PKTCBL_SUPPORT)
         rutDhcp6_getNtpserversFromOption56(ipIntfObj->name, dhcp6cInfo->ntpserver, 
                                            sizeof(dhcp6cInfo->ntpserver));
#endif // defined(BRCM_PKTCBL_SUPPORT)
   ...
   /* 
    * dhcp6c receives MAPT or MAPE information
    * TODO: should we update MAPT or MAPE even WAN is not connected?
    */
   if ( connected && (dhcp6cInfo->maptAssigned || dhcp6cInfo->mapeAssigned) && 
        !IS_EMPTY_STRING(dhcp6cInfo->ruleIPv4Prefix) && !IS_EMPTY_STRING(dhcp6cInfo->brIPv6Prefix))
   {
      /* maptAssigned and mapeAssigned can be (0,0) or (1,0) or (0,1) */
      if (dhcp6cInfo->maptAssigned)
      {
         CMSMEM_REPLACE_STRING(dhcp6cRcvObj->mapTransportMode, MDMVS_TRANSLATION);
      }
      if (dhcp6cInfo->mapeAssigned)
      {
         CMSMEM_REPLACE_STRING(dhcp6cRcvObj->mapTransportMode, MDMVS_ENCAPSULATION);
      }

      cmsLog_debug("assigned MAP%c", *dhcp6cRcvObj->mapTransportMode);
      CMSMEM_REPLACE_STRING(dhcp6cRcvObj->mapBRPrefix, dhcp6cInfo->brIPv6Prefix);
      CMSMEM_REPLACE_STRING(dhcp6cRcvObj->mapRuleIPv4Prefix, dhcp6cInfo->ruleIPv4Prefix);
      CMSMEM_REPLACE_STRING(dhcp6cRcvObj->mapRuleIPv6Prefix, dhcp6cInfo->ruleIPv6Prefix);
      dhcp6cRcvObj->mapEALen = dhcp6cInfo->eaLen;
      dhcp6cRcvObj->mapPSIDOffset = dhcp6cInfo->psidOffset;
      dhcp6cRcvObj->mapPSIDLen = dhcp6cInfo->psidLen;
      dhcp6cRcvObj->mapPSID = dhcp6cInfo->psid;
      dhcp6cRcvObj->mapIsFMR = dhcp6cInfo->isFMR;
   }
   ...
   if (connected)
   {
      ...
      initVoiceOnIntfUpLocked_dev2(CMS_AF_SELECT_IPV6, ipIntfObj->name, 1);
#endif
   }
...
void processRAStatus6Info_dev2(const CmsMsgHeader *msg)
{
   ...
   if (found)
   {
      ...
      if (raInfo->pio_prefixLen != 0)
      {
         ...
         char prefix[CMS_IPADDR_LENGTH + BUFLEN_16]={0};
...
//         REPLACE_STRING_IF_NOT_EQUAL(ipConn->X_BROADCOM_COM_IPv6DNSServers, raInfo->dns_servers);
      ...
      cmsLog_debug("sending WAN_CONNECTION_UP msg for IPv6 connection %s", raInfo->ifName);
      setIpv6ServiceStatusByFullPathLocked(fullPathStringPtr, MDMVS_SERVICEUP);
#endif      
