...
static void GdbProcessCommands(void)
{
	...
	while (1) {
		...
		if (!strncmp((char *)&gdbCmd.diagData, gdbConnectPack, sizeof(gdbConnectPack)-1) && bJustConnected == 0 && justReconnected == 0) {
			...
			if ((gdbDetaching != 0) && ('+' == *(char *)&gdbCmd.diagData)) {
			   gdbDetaching = 0;
...
#if 1
			if (!strncmp((char *)&gdbCmd.diagData, gdbDetachPack, sizeof(gdbDetachPack)-1)) {
			  printf("*** GDB socket detaching \n");
			  gdbDetaching = 1;
			  break;
			}
