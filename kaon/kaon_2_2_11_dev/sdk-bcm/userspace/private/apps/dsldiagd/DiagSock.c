...
typedef struct _LogProtoHeaderExt {
	...
	int				bTcp)
{
	...
	*srvIpAddrLen = sizeof(*srvIpAddr);
	if (port == GDB_PORT)
	  ...
	  res = LogRecvWithTimeout(*pSock, (char *) &logHeader, sizeof(LogProtoHeader), srvIpAddr, toMs, bTcp);
	...
	if (port != GDB_PORT) {
	  if ((res < sizeof(LogProtoHeader)) || ((logHeader.logPartyId & DIAG_PARTY_ID_MASK_EX) != LOG_PARTY_SERVER_EX) ||
	  ...
	  *pDiagData = logHeader.logPartyId & (DIAG_DATA_MASK | DIAG_LOCK_SESSION);
	  ...
	  if (bTcp)
	    res = send(*pSock, (char *)&logHeader, sizeof(LogProtoHeader), 0);
	  else {
	    logHeader.data32 = 0x11223344;
