...
static int bmu_reg_fd;
void bmu_reg_open(void) {

    if ((bmu_reg_fd = open (BOARD_DEVICE_NAME, O_RDWR)) < 0) {
        printf ("Can't open /dev/brcmboard ");
    }
}

void bmu_reg_close(void) {
    close(bmu_reg_fd);
}

uint32 bmu_reg_cmd(uint32 *addr, int write, uint32 wrval) {
    char buf[8];
    ...
    *buf32 = wrval;
    ...
    parms.address = (unsigned long)addr;
    parms.size = 4;
    parms.space = BOARD_MEMACCESS_IOCTL_SPACE_REG;
    parms.count = 1;
    parms.buf = buf;
            
    ioctl(bmu_reg_fd, BOARD_IOCTL_MEM_ACCESS, &parms);

    //printf("1-addr %p  val32 %08lx\n", addr, wrval);
    //printf("2-addr %p  val32 %08lx\n", addr, *buf32);
    return *buf32;
}

#define bmuaddr(fi) (BMU_START+offsetof(BmuBlockRegisters, fi))
#define bmureadaddr(ad) bmu_reg_cmd((uint32 *)ad, 0, 0)
#define bmuread(fi) bmu_reg_cmd((uint32 *)bmuaddr(fi), 0, 0)
#define bmuwriteaddr(ad, va) bmu_reg_cmd((uint32 *)ad, 1, va)
#define bmuwrite(fi, va) bmu_reg_cmd((uint32 *)bmuaddr(fi), 1, va)
#define bmuupdate(fi, op) bmuwrite(fi, bmuread(fi) op)

#define bmubatinfoaddr(fi, ba) bmuaddr(DMEM.updateInfo.batteryInfo[0].fi)+ba*sizeof(TBatInfo)
#define bmureadbatinfo(fi, ba) bmureadaddr(bmubatinfoaddr(fi, ba))
#define bmuwritebatinfo(fi, ba, va) bmuwriteaddr(bmubatinfoaddr(fi, ba), va)

#define bmubatinitaddr(fi, ba) bmuaddr(DMEM.initInfo.batChargeParams[0].fi)+ba*sizeof(TBatChargeParams)
#define bmureadbatinit(fi, ba) bmureadaddr(bmubatinitaddr(fi, ba))
#define bmuwritebatinit(fi, ba, va) bmuwriteaddr(bmubatinitaddr(fi, ba), va)
...
static void getupdateinfo(TBMUUpdateInfo *updateInfo)
{
	...
	uint32 pSrc = bmuaddr(DMEM.updateInfo);

	uint32 *pDst = (uint32 *)updateInfo;
	while(size--) {
          *pDst++ = bmureadaddr(pSrc);
          pSrc += 4;
	}
	bmuwrite(DMEM.updateInfo.BMUStatus, 0);
}
...
BOOL SendSimpleCommand( int command, int qual, uint32 *cmdData )
{
	...
	while( bmuread(DMEM.controlInfo.MipsCmd) & CMD_VALID) Sleep(1);
	// send the target command
	if (cmdData) {
          bmuwrite(DMEM.controlInfo.CmdDataMSW, ((*cmdData)>>16) & 0xffff);
          bmuwrite(DMEM.controlInfo.CmdDataLSW, (*cmdData) & 0xffff);
	}
	bmuwrite(DMEM.controlInfo.MipsCmd, (cmd | CMD_VALID));
	// wait for this command to complete
	while( bmuread(DMEM.controlInfo.MipsCmd) & CMD_VALID) Sleep(1);
	// return the value in cmdData MSW/LSW and an indication of success
	if(cmdData) {
          *cmdData = (bmuread(DMEM.controlInfo.CmdDataMSW) << 16) | bmuread(DMEM.controlInfo.CmdDataLSW);
	}
	CmdErrno = (bmuread(DMEM.controlInfo.MipsCmd) >> CMD_ERR_SHIFT) & CMD_ERR_MASK;
...
{
	uint32 otp_low = bmuread(LS.OtpLow);
...
static BOOL pvtEnableChargingAll ( BOOL Enable )
{
	if (Enable)
	{
		...
		bmuupdate(DMEM.controlInfo.BMUControl, & ~CHARGE_DISABLED);
	}
	else
	{
...
static void	SetChargeParams(TBatteryControllerInfo *info, int battery, float targetVolts)
{
	...
	bmuwrite(DMEM.initInfo.batChargeParams[battery].BAT_PQVTERM, pqvterm);
	bmuwrite(DMEM.initInfo.batChargeParams[battery].BAT_VINIT, vinit);
        bmuwrite(DMEM.initInfo.batChargeParams[battery].BAT_FCVTERM, vterm);
	bmuwrite(DMEM.initInfo.batChargeParams[battery].BAT_PQVCHG, vterm);
	bmuwrite(DMEM.initInfo.batChargeParams[battery].BAT_TCVTERM, vterm);
}
...
static BOOL pvtSetChargePercentageSingle ( TBatteryControllerInfo *info, int battery, int Percent )
{
	...
	picoState = bmuread(DMEM.updateInfo.PicoState);
	if (picoState >= STATE_CHARGE_INIT 
	 && picoState <= STATE_CHG_SUSPEND 
	 && (bmureadbatinfo(BatStatus, battery) & BXS_SELECTED)) {
		if(allowDebugPrint)
		{
...
static BOOL pvtEnableDischargeAll ( BOOL Enable )
{
	if (Enable)
	{
		...
		bmuupdate(DMEM.controlInfo.BMUControl, & ~DISCHARGE_DISABLED);
	}
	else
	{
		HDP(ZONE_TEST1,"Setting Pico global charge disable\n");
...
static BOOL pvtReadGUID (TBatteryControllerInfo *info, int which )
{
	...
	{
		info->Batteries[which].guid = (((uint64)bmuread(SMEM.Param32_H)) << 32) | (uint64)bmuread(SMEM.Param32_L);
		return TRUE;
	}
...
static BOOL pvtMakeHWResistanceMeasurement ( int which, float *result )
{
	...
	{
		float ICOS = (float)bmuread(LS.ImpedanceICos);
		float ISIN = (float)bmuread(LS.ImpedanceISin);
		float VCOS = (float)bmuread(LS.ImpedanceVCos);
		float VSIN = (float)bmuread(LS.ImpedanceVSin);
...
{
	bmuwrite(DMEM.updateInfo.HWFaultsAllowed, mask);
...
static uint16 pvtGetFaultMask( void)
{
	...
	return (uint16)bmuread(DMEM.updateInfo.HWFaultsAllowed);
}

static uint16 pvtGetHWFaults( void )
{
	//HalDebugPrint(ZONE_TEST1,"Getting HW Faults\n");
	return (uint16)bmuread(DMEM.updateInfo.HWFaults);
}

static BOOL pvtClearHWFaults (uint32 mask)
{
	mask = (bmuupdate(DMEM.updateInfo.HWFaults, & ~mask));
	//HalDebugPrint(ZONE_TEST1,"Clearing HW Faults (setting to %#4x)\n", mask);
	bmuwrite(DMEM.updateInfo.HWFaults, mask);
	return TRUE;
}
...
{
	bmuupdate(DMEM.controlInfo.BMUControl, | INHIBIT_WATCHDOG);
}
...
static BOOL pvtAdjustCellConfig(TBatteryControllerInfo *info, int whichBat, TPackParams *packParams)
{
	...
	bmuwritebatinit(BAT_PQICHG,        whichBat, (uint32)(((float)packParams->pqiChg)*ADC_IBAT_A_FACTOR) + 1);
	bmuwritebatinit(BAT_FCICHG,        whichBat, (uint32)(((float)packParams->fciChg)*ADC_IBAT_A_FACTOR) + 1);
	bmuwritebatinit(BAT_FCITERM,       whichBat, (uint32)(((float)packParams->fciTerm)*ADC_IBAT_A_FACTOR) + 1);
	bmuwritebatinit(BAT_TCICHG,        whichBat, (uint32)(((float)packParams->fciChg)*ADC_IBAT_A_FACTOR) + 1); // by default, TC uses same charge current as FC
	bmuwritebatinit(BAT_TCITERM,       whichBat, (uint32)(((float)packParams->tciTerm)*ADC_IBAT_A_FACTOR));
	bmuwritebatinit(BAT_COI_LIMIT,     whichBat, (uint32)(((float)packParams->coiLimit)*ADC_IBAT_A_FACTOR) + 1);
	bmuwritebatinit(BAT_DOI_LIMIT_A,   whichBat, (uint32)(((float)packParams->doiLimitA)*ADC_IBAT_A_FACTOR) + 1);
	bmuwritebatinit(BAT_DOI_LIMIT_B,   whichBat, (uint32)(((float)packParams->doiLimitB)*ADC_IBAT_A_FACTOR) + 1);
	bmuwritebatinit(BAT_DOI_LIMIT_C,   whichBat, (uint32)(((float)packParams->doiLimitC)*ADC_IBAT_A_FACTOR) + 1);
	bmuwritebatinit(BAT_DOI_TIMEOUT_A, whichBat, (uint32)packParams->doiTimeA);
	bmuwritebatinit(BAT_DOI_TIMEOUT_B, whichBat, (uint32)packParams->doiTimeB);
	bmuwritebatinit(BAT_PQVTERM,       whichBat, (uint32)VBAT_MILLIVOLTS_TO_ADC(packParams->pqvTerm));
	bmuwritebatinit(BAT_DCVB_ON,       whichBat, (uint32)VBAT_MILLIVOLTS_TO_ADC(packParams->vdepleted));
	bmuwritebatinit(BAT_DCVB_OFF,      whichBat, (uint32)VBAT_MILLIVOLTS_TO_ADC(packParams->dcvb_off)); 
	bmuwritebatinit(BAT_MAX_BAT_CHG,   whichBat, (uint32)VBAT_MILLIVOLTS_TO_ADC(packParams->mavV));
	bmuwritebatinit(BAT_LOW_WARN,      whichBat, (uint32)VBAT_MILLIVOLTS_TO_ADC(packParams->low_warn));
#ifdef PRINT_BATTERY_UPDATE_INFO
	HalDebugPrint(ZONE_TEST1,"AdjustCellConfig on battery %d:\n", whichBat);
	v32 = bmureadbatinit(BAT_PQICHG,whichBat);
	HalDebugPrint(ZONE_TEST1,"   PQICHG  = 0x%04lx (%d mA)\n", v32, (int)(v32/ADC_IBAT_A_FACTOR));
	v32 = bmureadbatinit(BAT_PQVTERM,whichBat);
	HalDebugPrint(ZONE_TEST1,"   PQVTERM = 0x%04lx (%d mV)\n", v32, (int)VBAT_ADC_TO_MILLIVOLTS(v32));
	v32 = bmureadbatinit(BAT_FCICHG,whichBat);
	HalDebugPrint(ZONE_TEST1,"   FCICHG  = 0x%04lx (%d mA)\n", v32, (int)(v32/ADC_IBAT_A_FACTOR));
	v32 = bmureadbatinit(BAT_FCITERM,whichBat);
	HalDebugPrint(ZONE_TEST1,"   FCITERM = 0x%04lx (%d mA)\n", v32, (int)(v32/ADC_IBAT_A_FACTOR));
	v32 = bmureadbatinit(BAT_TCICHG,whichBat);
	HalDebugPrint(ZONE_TEST1,"   TCICHG  = 0x%04lx (%d mA)\n", v32, (int)(v32/ADC_IBAT_A_FACTOR));
	v32 = bmureadbatinit(BAT_TCITERM,whichBat);
	HalDebugPrint(ZONE_TEST1,"   TCITERM = 0x%04lx (%d mA)\n", v32, (int)(v32/ADC_IBAT_A_FACTOR));
	v32 = bmureadbatinit(BAT_COI_LIMIT,whichBat);
	HalDebugPrint(ZONE_TEST1,"   COI LIM = 0x%04lx (%d mA)\n", v32, (int)(v32/ADC_IBAT_A_FACTOR));
	v32 = bmureadbatinit(BAT_DOI_LIMIT_A,whichBat);
	HalDebugPrint(ZONE_TEST1,"   DOI A   = 0x%04lx (%d mA)/%dms\n", v32, (int)(v32/ADC_IBAT_A_FACTOR), (int)bmureadbatinit(BAT_DOI_TIMEOUT_A,whichBat));
	v32 = bmureadbatinit(BAT_DOI_LIMIT_B,whichBat);
	HalDebugPrint(ZONE_TEST1,"   DOI B   = 0x%04lx (%d mA)/%dms\n", v32, (int)(v32/ADC_IBAT_A_FACTOR), (int)bmureadbatinit(BAT_DOI_TIMEOUT_B,whichBat));
	v32 = bmureadbatinit(BAT_DOI_LIMIT_C,whichBat);
	HalDebugPrint(ZONE_TEST1,"   DOI C   = 0x%04lx (%d mA)\n", v32, (int)(v32/ADC_IBAT_A_FACTOR));
	v32 = bmureadbatinit(BAT_DCVB_ON,whichBat);
	HalDebugPrint(ZONE_TEST1,"   DCVB_ON = 0x%04lx (%d mV)\n", v32, (int)VBAT_ADC_TO_MILLIVOLTS(v32));
	v32 = bmureadbatinit(BAT_DCVB_OFF,whichBat);
	HalDebugPrint(ZONE_TEST1,"   DCVB_OFF= 0x%04lx (%d mV)\n", v32, (int)VBAT_ADC_TO_MILLIVOLTS(v32));
	v32 = bmureadbatinit(BAT_MAX_BAT_CHG,whichBat);
	HalDebugPrint(ZONE_TEST1,"   MAX_CHG = 0x%04lx (%d mV)\n", v32, (int)VBAT_ADC_TO_MILLIVOLTS(v32));
	v32 = bmureadbatinit(BAT_LOW_WARN,whichBat);
...
static void pvtAdjustPORValidityTimeout(uint32 pollSecs)
{
	...
	bmuwrite(DMEM.initInfo.POR_VALID_SECS, (pollSecs * 4) + 2);
}
...
static BOOL pvtVerifyPicocode( BOOL printDebugInfo )
{
	...
	for (loop = 0; loop < (size / sizeof(uint32)); loop++)
	{
		if (bmureadaddr(BMU_PICO_IMEM_START+loop) != *pBatteryPicocode++) // *4?
		{
...
static BOOL pvtStartPico(TBatteryControllerInfo *info, TBatteryDriverCalls *calls)
{
	...
	bmuwrite(LS.SoftReset, PICO_HALT);
	bmuwrite(DMEM.controlInfo.BMUControl, 0);	// set BMUControl to zero - this is where Pico will handshake
	bmuwrite(LS.SoftReset, PICO_RUN_FROM_RAM);	// let Pico start up <------------ comment this line out to test "Pico Failed To Start" error
	while( !(bmuread(DMEM.controlInfo.BMUControl) & STARTUP_HANDSHAKE)) 
	{
		Sleep(1);
		if(maxSleep-- <= 0 ) {
			break;
	...
	if (maxSleep > 0) {
		...
		bmuwrite(DMEM.initInfo.IOV_FAULT_LIMIT, VIN_MILLIVOLTS_TO_ADC(NOMINAL_ADAPTER_MV*1.15)); // IOV = 115% Nominal
		bmuwrite(DMEM.initInfo.MVCHG_ON, VIN_MILLIVOLTS_TO_ADC(NOMINAL_ADAPTER_MV*0.95));	// CHG_ON = 95% Nominal
		bmuwrite(DMEM.initInfo.MVCHG_OFF, VIN_MILLIVOLTS_TO_ADC(NOMINAL_ADAPTER_MV*0.92));	// CHG_OFF = 92% Nominal
		bmuwrite(DMEM.initInfo.MVMAX_IDLE, VIN_MILLIVOLTS_TO_ADC(NOMINAL_ADAPTER_MV*1.10));	// MAX_IDLE = 110% Nominal
		bmuwrite(DMEM.initInfo.MVMIN_SYS, VIN_MILLIVOLTS_TO_ADC(NOMINAL_ADAPTER_MV*0.34));	// MIN_SYS = 34% Nominal
		bmuwrite(DMEM.initInfo.MVLOW_MIN, VIN_MILLIVOLTS_TO_ADC(NOMINAL_ADAPTER_MV*0.67));	// MIN_SYS = 67% Nominal
		bmuwrite(DMEM.initInfo.MVLOW_MAX, VIN_MILLIVOLTS_TO_ADC(NOMINAL_ADAPTER_MV*0.80));	// MIN_SYS = 80% Nominal

		HalDebugPrint (ZONE_TEST1, "IOV_FAULT_LIMIT = %lu\n", bmuread(DMEM.initInfo.IOV_FAULT_LIMIT));
		HalDebugPrint (ZONE_TEST1, "MVCHG_ON        = %lu\n", bmuread(DMEM.initInfo.MVCHG_ON));
		HalDebugPrint (ZONE_TEST1, "MVCHG_OFF       = %lu\n", bmuread(DMEM.initInfo.MVCHG_OFF));
		HalDebugPrint (ZONE_TEST1, "MVMAX_IDLE      = %lu\n", bmuread(DMEM.initInfo.MVMAX_IDLE));
		HalDebugPrint (ZONE_TEST1, "MVMIN_SYS       = %lu\n", bmuread(DMEM.initInfo.MVMIN_SYS));
		HalDebugPrint (ZONE_TEST1, "MVLOW_MIN       = %lu\n", bmuread(DMEM.initInfo.MVLOW_MIN));
		HalDebugPrint (ZONE_TEST1, "MVLOW_MAX       = %lu\n", bmuread(DMEM.initInfo.MVLOW_MAX));

		bmuwrite(DMEM.initInfo.SenseNumerator, kBatterySenseNumerator);
		bmuwrite(DMEM.initInfo.SenseDenominator, kBatterySenseDenominator);
		bmuwrite(DMEM.initInfo.MaxIDCurrent, (610 * kBatterySenseNumerator)/kBatterySenseDenominator);
		bmuwrite(DMEM.initInfo.batChargeParams[0].BAT_MAX_BAT_CHG, VBAT_MILLIVOLTS_TO_ADC(max_chg_volts() * 1000)+1);		// allow a 1 ADC count overage
		bmuwrite(DMEM.initInfo.batChargeParams[1].BAT_MAX_BAT_CHG, VBAT_MILLIVOLTS_TO_ADC(max_chg_volts() * 1000)+1);		// allow a 1 ADC count overage
		bmuwrite(DMEM.initInfo.BatSelectDelay, 7);	// PR23030
		...
		bmuupdate(DMEM.controlInfo.BMUControl, & ~(STARTUP_HANDSHAKE));	// release the hounds!
		...
		if(bmuread(DMEM.updateInfo.BMUStatus) & LTE)
		{
...
//  		if(bmuread(DMEM.updateInfo.HWFaults) != 0 || bmuread(DMEM.updateInfo.HWFaultsAllowed) != 0)
//  		{
//  			int ix;
//  			volatile uint32 *DEBUGINFO = (volatile uint32 *)(0xb3a01000 + 0x3a0);
//  			HalDebugPrint(ZONE_TEST1,"***********************************************\n");
//  			HalDebugPrint(ZONE_TEST1,"Pico reports %#04x faults and %#04x allowed\n", bmuread(DMEM.updateInfo.HWFaults), bmuread(DMEM.updateInfo.HWFaultsAllowed));
//  			HalDebugPrint(ZONE_TEST1,"***********************************************\n");
//  		}
		}
	}
	...
	if(result == FALSE)
	{
		...
		Sleep(1000);
		bmuwrite(LS.SoftReset, PICO_HALT);
		bmuwrite(CP.BatteryEnable, 0x10);		// this isn't actually necessary when an LTE error has been detected, but it won't hurt anything either.
	}
	HalDebugPrint(ZONE_TEST1,"DMEM.initInfo.MVCHG_ON: %d\n", (int)bmuread(DMEM.initInfo.MVCHG_ON));
	HalDebugPrint(ZONE_TEST1,"DMEM.initInfo.MVCHG_OFF: %d\n", (int)bmuread(DMEM.initInfo.MVCHG_OFF));
	HalDebugPrint(ZONE_TEST1,"DMEM.initInfo.MVMAX_IDLE: %d\n", (int)bmuread(DMEM.initInfo.MVMAX_IDLE));
...
{
	uint32 status = bmureadaddr(bmuaddr(LS.VbatStatus)+which); // *4?

	while( !(status & 0x10)) {
		status = bmureadaddr(bmuaddr(LS.VbatStatus)+which); // *4?
	}
	return (bmureadaddr(bmuaddr(LS.VbatAvg)+which)) >> 4; // *4?
}
...
static void pvtMimicWarmRestart()
{
	...
	bmuwrite(LS.SoftReset, PICO_HALT);
	if((bmuread(CP.ActiveBat) & 0x00000003) != 0) {
		HalDebugPrint(ZONE_TEST1,"\nb33\n");
		return;		// battery already selected...nothing to do
	}
	// intialize hardware
	bmuwrite(CP.ChargerMode, (1 << 2));	// SLEEP/PWM Disabled
	bmuwrite(CP.RrSgl[0], 0);		// single input
	bmuwrite(CP.RrSgl[1], 1);		// round-robin
	bmuwrite(CP.SlewWindowSizes, 0);	// don't slew window sizes
	bmuwrite(LS.VbatAvgCtrl, 2);		// 4 samples/average
	bmuwrite(LS.RidAAvgCtrl, 2);
	bmuwrite(LS.RidBAvgCtrl, 2);
	val = bmuread(LS.RidAStatus);		// read once to clear
	val = bmuread(LS.RidBStatus);
	...
	if(batCount != 3) {
		bmuwrite(CP.BatteryEnable, batCount | 0x10);
		HalDebugPrint(ZONE_TEST1,"\nb1%ld\n", batCount);
		return;		// a single battery present...select it
	}
	// both batteries present...measure each in turn
	bmuwrite(CP.BatteryEnable, 0x11);
	Sleep(3);
	val = pvtMeasure(0);
	bmuwrite(CP.BatteryEnable, 0x12);
	Sleep(3);
	if( pvtMeasure(0) < val ) {
		bmuwrite(CP.BatteryEnable, 0x11);
		Sleep(3);
...
{
	uint32 wdVal = bmuread(DMEM.updateInfo.PicoWatchdog) & 0xffff;
	bmuwrite(DMEM.updateInfo.PicoWatchdog, 0);
...
void pvtSetPackChargeInhibit(TBatteryInfo *info, int whichBat, BOOL inhibit)
{
	...
	v32 = bmureadbatinfo(BatStatus, whichBat);
	if(inhibit == TRUE) {
		bmuwritebatinfo(BatStatus, whichBat, v32 | BXS_CHG_DISABLED);
...
void pvtSetPackDischargeInhibit(TBatteryInfo *info, int whichBat, BOOL inhibit)
{
	...
	v32 = bmureadbatinfo(BatStatus, whichBat);
	if(inhibit == TRUE) {
		bmuwritebatinfo(BatStatus, whichBat, v32 | BXS_DCHG_DISABLED);
...
{
	int adc = bmureadbatinit(BAT_DCVB_ON, whichBat);
...
BOOL bspBatInit(TBatteryControllerInfo *info, TBatteryDriverCalls *calls)
{
	...
	if(ALLOW_UNTRIMMED_PARTS == FALSE && pvtPartIsTrimmed() == FALSE)
	{
		...
		bmuwrite(LS.SoftReset, PICO_HALT);		// place core into reset
		bmuwrite(CP.BatteryEnable, 0x10);		// deselect all packs
		bmuwrite(CP.ChargerMode, (1 << 3));		// stop discharge
		bmuwrite(CP.ChargerMode, 0);		// place hardware in sleep
		Sleep(3);
		bmuwrite(CP.AcDisable, 0);
		bmuDisabled = TRUE;
...
BOOL bspBatUpdate(TBatteryControllerInfo *info)
{
	...
	info->RawControllerState = currentState = bmuread(DMEM.updateInfo.PicoState);
	...
	ibatADC = bmuread(SMEM.RecentIBat);
	...
	if ((currentState >= STATE_PREQUAL) && (currentState <= STATE_TOPOFF)) 
	{
		...
		bmuupdate(DMEM.controlInfo.BMUControl, | CHG_SUSPENDED);
		...
		updateInfo.batteryInfo[0].Vbat = bmuread(DMEM.updateInfo.batteryInfo[0].Vbat);
		updateInfo.batteryInfo[1].Vbat = bmuread(DMEM.updateInfo.batteryInfo[1].Vbat);
	}
	...
	HDP (ZONE_TEST1, "    control = 0x%04lx\n", bmuread(DMEM.controlInfo.BMUControl));
...
BOOL bspBatReadOWI(TBatteryControllerInfo *info, int which, int page)
{
	...
	if(SendSimpleCommand(CmdReadEPROM,which,&cmdData))
	{
		...
		uint32 sptr = bmuaddr(DMEM.ioData);
		uint32 temp;
		int ix;
		for( ix = 0; ix < 16; ix++) {
			temp = bmureadaddr(sptr+ix); // *4?
...
{
	return (bmuread(DMEM.updateInfo.BMUStatus) & AC_ABSENT) == 0 ? TRUE : FALSE;
}

BatteryControllerState bspCurrentState()
{
	uint16 state = bmuread(DMEM.updateInfo.PicoState);
	if((state == STATE_DISCHARGE) && (bmuread(DMEM.updateInfo.BMUStatus) & FDCHG_IN_PROGRESS)) 
...
void AssertBMUGPO_0(int state)
{
	uint32 gpo = bmuread(LS.GPO);
	uint32 fd = bmuread(LS.FD_EN);
	if(state)
	{
		gpo |= 1;
		gp0_deassert_counter = 3;
	...
	bmuwrite(LS.GPO, gpo);
	bmuwrite(LS.FD_EN, fd | O_SW_GPO);
	bmuwrite(LS.FD_EN, fd);
}
#undef HDP
