                          ...
                          char *varValue)
{
                                    ...
                                    (void **) &containerInfoObj) == CMSRET_SUCCESS)
         {
                           ...
                           (void **) &containerStateObj) == CMSRET_SUCCESS)
            {
                   ...
                   containerInfoObj->execEnvRef != NULL)
               {
                  ...
                  qdmModsw_getExecEnvAliasByFullPathLocked(containerInfoObj->execEnvRef,
                                                           eeName, sizeof(eeName));
                  /* Only add EUs containers that has its eeName match with eeObj->alias */
                  if (cmsUtl_strcmp(eeObj->alias, eeName) == 0)
                  {
                     containerId++;
                              ...
                              (void **) &containerInfoObj) == CMSRET_SUCCESS)
   {
      ...
      {
         /* Look for only non EE container */
         if (containerInfoObj->execEnvRef != NULL)
         {
            /* Get execution environment name from its full path */
            qdmModsw_getExecEnvAliasByFullPathLocked(containerInfoObj->execEnvRef,
                                                     eeName, sizeof(eeName));
            /* Only add EUs containers that has its eeName match with BEEP_HOSTEE_NAME */
            if (cmsUtl_strcmp(eeName, BEEP_HOSTEE_NAME) == 0)
            {
               containerId++;
               p += sprintf(p, "        <tr data-tt-id='%u' data-tt-parent-id='0'>\n", containerId);
               p += sprintf(p, "          <td>%s</td>\n", (containerStateObj->name? containerStateObj->name:""));
