...
ejRefreshGlbWebVar(const char *path)
{
   ...
   else if (!strcasecmp(last, "ntpdcfg.html"))
   {
...
#define WLVISUALIZATION "1"
#else
#define WLVISUALIZATION "0"
#endif

#include "wlsysutil.h"


extern struct nv_pair preload_nvram[];

static void ejGetWlVar(int argc, char **argv, FILE *stream)
{
	char value[WEB_BIG_BUF_SIZE_MAX_WLAN] = {0}; // country list is big
    if(argc == 3) {
        if (strcmp(argv[1], "nvram") == 0)
        {
            int i;
            for(i = 0 ; preload_nvram[i].name != NULL ; i++)
            {
                if (strcmp(argv[2], preload_nvram[i].name) == 0) // match
                {
                    strncpy(value, preload_nvram[i].value, WEB_BIG_BUF_SIZE_MAX_WLAN-1);
                    break;
                }
            }
    ...
    {
       if (strcmp(argv[1], "wlInterface") == 0)
           sprintf(value, "%d", wlgetintfNo() > 0 ? 1 : 0);
       else if (strcmp(argv[1], "wlVersion") == 0)
       {
           int idx = 0;
#ifdef BUILD_BCM_WLAN_DPDCTL
           for (idx = 0; idx < glbWebVar.pmWlIfNum; idx++)
           {
               if (glbWebVar.pmWlDpd[idx] == 0)
                   break;
           }
#endif
           wlgetVer(idx, value);
       }
       else if (strcmp(argv[1], "wlPassPoint") == 0)
           sprintf(value, "1");
       else if (strcmp(argv[1], "wlVisualization") == 0)
           sprintf(value, WLVISUALIZATION);
       else if (strcmp(argv[1], "wliQoS") == 0)
#ifdef SUPPORT_WLAN_IQOS
           sprintf(value, "1");
#else
           sprintf(value, "0");
#endif
...
/* This is basically a big buffer version of ejGetOther */
static void
ejGetOtherV2(int argc, char **argv, FILE *stream)
{
   char value[WEB_BIG_BUF_SIZE_MAX];
...
static void
ejGetObject(int argc, char **argv, FILE *stream)
{
   char value[WEB_BUF_SIZE_MAX];
   ...
   cgiGetObjectVar(argc, argv, value);
...
struct ej_handler ej_handlers[] = {
	...
	{ "ejGetOtherV2", ejGetOtherV2 },
#ifdef DMP_X_BROADCOM_COM_SELT_1
	{ "ejGetSelt", ejGetSelt },
	{ "ejGetSeltResult", ejGetSeltResult },
#endif
	{ "ejFncCmd", ejFncCmd },
	{ "ejGetObject", ejGetObject },
	{ NULL, NULL }
};
