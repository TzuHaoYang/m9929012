             ...
             char *ipv6status, char *ipv6Addr __attribute__((unused)))
{
   ...
   {
      strcpy(protoStr,  BRIDGE_PROTO_STR);
   }
...
void cgiDeviceInfoWanStatusBody_igd(FILE *fs)
{
   ...
   while (cmsObj_getNextFlags(MDMOID_WAN_DEV, &wanDevIid, OGF_NO_VALUE_UPDATE, (void **)&wanDev) == CMSRET_SUCCESS)
   {
      ...
      while (cmsObj_getNextInSubTree(MDMOID_WAN_IP_CONN, &wanDevIid, &iidStack, (void **)&ipCon) == CMSRET_SUCCESS)
      {
         ...
         while (!found &&
                 cmsObj_getNextFlags(MDMOID_DEV2_IP_INTERFACE, &ipIntfIid, OGF_NO_VALUE_UPDATE, (void **)&ipIntf) == CMSRET_SUCCESS)
         {
            if (!cmsUtl_strcmp(ipIntf->name, ipCon->X_BROADCOM_COM_IfName))
            {
               found = TRUE;
               strncpy(ipv6StatusBuf, ipIntf->X_BROADCOM_COM_IPv6ServiceStatus, sizeof(ipv6StatusBuf) - 1);
            }
      ...
      while (cmsObj_getNextInSubTree(MDMOID_WAN_PPP_CONN, &wanDevIid, &iidStack, (void **)&pppCon) == CMSRET_SUCCESS)
      {
       ...
       while (!found &&
               cmsObj_getNextFlags(MDMOID_DEV2_IP_INTERFACE, &ipIntfIid, OGF_NO_VALUE_UPDATE, (void **)&ipIntf) == CMSRET_SUCCESS)
       {
          if (!cmsUtl_strcmp(ipIntf->name, pppCon->X_BROADCOM_COM_IfName))
          {
             found = TRUE;
             strncpy(ipv6StatusBuf, ipIntf->X_BROADCOM_COM_IPv6ServiceStatus, sizeof(ipv6StatusBuf) - 1);
          }
...
void cgiWriteWanSts(FILE *fs, const char *interfaceDisplayStr, const char *serviceStr,
                      UINT64 rxTotalByte, UINT64 rxTotalPacket, 
                      UINT64 rxTotalErr, UINT64 rxTotalDrop,
                      UINT64 rxMulticastByte, UINT64 rxMulticastPacket,
                      UINT64 rxUnicastPacket, UINT64 rxBroadcastPacket,
                      UINT64 txTotalByte, UINT64 txTotalPacket, 
                      UINT64 txTotalErr, UINT64 txTotalDrop,
                      UINT64 txMulticastByte, UINT64 txMulticastPacket,
                      UINT64 txUnicastPacket, UINT64 txBroadcastPacket)
{
   fprintf(fs, "   <tr align='center'>\n");
   fprintf(fs, "      <td class='hd'>%s</td>\n", interfaceDisplayStr);
   fprintf(fs, "      <td class='hd'>%s</td>\n", serviceStr);
 
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) rxTotalByte);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) rxTotalPacket ); 
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) rxTotalErr);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) rxTotalDrop);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) rxMulticastByte);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) rxMulticastPacket);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) rxUnicastPacket);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) rxBroadcastPacket);
   
   
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) txTotalByte);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) txTotalPacket);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) txTotalErr);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) txTotalDrop);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) txMulticastByte);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) txMulticastPacket);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) txUnicastPacket);
   fprintf(fs, "      <td>%llu</td>\n", (long long unsigned int) txBroadcastPacket);
...
#ifdef DMP_X_ITU_ORG_GPON_1 /* aka SUPPORT_OMCI */
   if (omciIpc_sendResetStats(MDMOID_MOCA_STATS, iidStack) != 0)
   {
      cmsLog_error("sendResetStats for oid MDMOID_MOCA_STATS failed");
   }
#else
  void *dummy=NULL;
