...
void writeMapScript(FILE *fs) 
{
   ...
   fprintf(fs, "   var loc = 'tunnelcfg.cmd?action=removemap&rmLst=' + lst;\n\n");
   fprintf(fs, "   loc += '&sessionKey=%d';\n",glbCurrSessionKey);
   fprintf(fs, "   var code = 'location=\"' + loc + '\"';\n");
   fprintf(fs, "   eval(code);\n");
   fprintf(fs, "}\n\n");
...
void cgiMapView(FILE *fs) 
{
   ...
   fprintf(fs, "<b>MAP -- MAP-T/MAP-E Configuration</b><br><br>\n");
   fprintf(fs, "<center>\n");
   fprintf(fs, "<table border='1' cellpadding='4' cellspacing='0'>\n");
   /* write table header */
   fprintf(fs, "   <tr>\n");
   fprintf(fs, "      <td class='hd'>Mechanism</td>\n");
   fprintf(fs, "      <td class='hd'>WAN</td>\n");
   fprintf(fs, "      <td class='hd'>Dynamic</td>\n");
   ...
   while (cmsObj_getNextFlags(MDMOID_DEV2_MAP_DOMAIN, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&domain) == CMSRET_SUCCESS)
   {
      ...
      if ( cmsUtl_strcmp(domain->transportMode, MDMVS_ENCAPSULATION) )
      {
         fprintf(fs, "      <td>MAP-T</td>\n");
      }
      else
      {
         fprintf(fs, "      <td>MAP-E</td>\n");
      }
      ...
      fprintf(fs, "      <td align='center'><input type='checkbox' name='rml' value='%s|%s'></td>\n",
              domain->transportMode, domain->WANInterface);
...
#ifdef SUPPORT_MAP
   else if ( strcmp(action, "addmap") == 0 )
   {
      cgiMapAdd(query, fs);
   }
   else if ( strcmp(action, "removemap") == 0 )
   {
      cgiMapRemove(query, fs);
   }
   else if ( strcmp(action, "viewmap") == 0 )
   {
      cgiMapView(fs);
   }
#endif
...
#ifdef SUPPORT_MAP
void cgiMapAdd(char *query, FILE *fs) 
{
   CmsRet ret = CMSRET_SUCCESS;;
   char mechanism[BUFLEN_24];
   char wanIntf[BUFLEN_32];
   ...
   UBOOL8 dynamic;
   char cmd[WEB_BUF_SIZE_MAX];
   char err[WEB_BUF_SIZE_MAX];

   mechanism[0] = wanIntf[0] = lanIntf[0] = dynamicInfo[0] = cmd[0] = '\0';

   cgiGetValueByName(query, "mapMechanism", mechanism);
   cgiGetValueByName(query, "associatedWanIntf", wanIntf);
   cgiGetValueByName(query, "associatedLanIntf", lanIntf);
   cgiGetValueByName(query, "dynamic", dynamicInfo);
   sprintf(err, "MAP-%c configuration error", *mechanism);

   cmsLog_debug("mechanism=%s, wanIntf=%s, lanIntf=%s dynamicInfo=%s", 
                 mechanism, wanIntf, lanIntf, dynamicInfo);
   ...
   if ( dynamic == 0 )
   {
      ...
      cmsLog_debug("Mechanism<MAP-%c> BRPrefix<%s> ipv6Prefix<%s> ipv4Prefix<%s> psidOffset<%u> psidLen<%u> psid<0x%x> ", *mechanism, BRPrefix, ipv6Prefix, ipv4Prefix, psidOffset, psidLen, psid);

      if ((ret = dalMap_add_dev2(mechanism, wanIntf, lanIntf, dynamic, BRPrefix, ipv6Prefix, ipv4Prefix, psidOffset, psidLen, psid)) != CMSRET_SUCCESS) 
      {
         cmsLog_error("dalMap_add failed, ret=%d", ret);
         sprintf(cmd, "Adding MAP-%c <%s> failed", *mechanism, wanIntf);
         cgiWriteMessagePage(fs, err, cmd, "tunnelcfg.cmd?action=viewmap");
         return;
      }
   }
   else
   {
      cmsLog_debug("config dynamic MAP-%c", *mechanism);

      if ((ret = dalMap_add_dev2(mechanism, wanIntf, lanIntf, dynamic, NULL, NULL, NULL, 0, 0, 0)) != CMSRET_SUCCESS) 
      {
         cmsLog_error("dalMap_add failed, ret=%d", ret);
         sprintf(cmd, "Adding MAP-%c <%s> failed", *mechanism, wanIntf);
         cgiWriteMessagePage(fs, err, cmd, "tunnelcfg.cmd?action=viewmap");
...
void cgiMapRemove(char *query, FILE *fs) 
{
   ...
   mechanism[0] = '\0'; wanIntf[0] = '\0'; lst[0] = '\0'; str[0] = '\0';
   ...
   while ( pToken != NULL ) 
   {
      ...
      if ( cp == NULL ) continue;
      *cp = '\0';
      cmsUtl_strncpy(mechanism, pToken, sizeof(mechanism));
      cmsUtl_strncpy(wanIntf, (cp + 1), sizeof(wanIntf));
      if ((ret = dalMap_delete_dev2(wanIntf, mechanism)) != CMSRET_SUCCESS)
      {
         cmsLog_error("dalMap_delete_dev2 failed for %s (ret=%d)", wanIntf, ret);
         sprintf(str, "dalMap_delete_dev2 failed for %s ", wanIntf);
      }
   ...
   {
      cgiWriteMessagePage(fs, "MAP-T MAP-E Remove Error", str,
                          "tunnelcfg.cmd?action=viewmap");
   }
}
#endif
