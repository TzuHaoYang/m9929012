...
static CmsRet cgiOmci_sendGetNextRequest(void)
{
   ...
   if (i < NUM_GET_NEXT_OBJECTS)
   {
      ...
      // ext msg len, fixed to 4 bytes
      omciReqMsgSetMsgLen(packet, extMsgSet, 4);
      ...
      else
      {
         cmsLog_notice("Send CMS_MSG_OMCI_COMMAND_REQUEST "
           "with msgType = %d, id = %d", OMCI_MSG_TYPE_GETNEXT, i);
         numMsgTx++;
      }
   }
   if (macroState == OMCI_MARCO_ON)
      ...
      for (j = 0; j < numEntries; j++)
      {
         ...
         if (objectInfoListP[j].oid == GetNextObjectsTable[i].oid)
         {
            ...
            if (omciIpc_pathDescriptorToFullPath(&(objectInfoListP[j]), &mdmPath) == 0)
            {
               ...
               cmsUtl_strncat(objectName, BUFLEN_1024, GetNextObjectsTable[i].name);
