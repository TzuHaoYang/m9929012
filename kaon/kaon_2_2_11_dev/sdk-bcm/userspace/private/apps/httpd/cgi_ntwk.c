...
void cgiAdslCfgTones(char *query, FILE *fs) {
   ...
   char cmd[BUFLEN_256*3], map[BUFLEN_256];
...
void cgiConfigDns(char *filename __attribute__((unused)))
{
   ...
   else if (cmsUtl_strcmp(glbWebVar.dnsPrimary, "0.0.0.0"))
   {
      /* primary has to be non "0.0.0.0" , but secondary can be "0.0.0.0" */
...
#define PRINT_TABLE_LINE(parameter, format_specifier, field, value)           \
if (PMD_OPERATION_SUCCESS == -ret || ERR_CAL_PARAM_VALUE_INVALID == -ret)     \
{                                                                             \
   fprintf(fs, "   <tr>\n");                                                  \
   fprintf(fs, "      <td>%d</td>\n", parameter);                             \
   fprintf(fs, "      <td>%d</td>\n", param.len);                             \
   fprintf(fs, "      <td>%s</td>\n", #parameter " " field);                  \
   if (ERR_CAL_PARAM_VALUE_INVALID == -ret)                                   \
   {                                                                          \
...
void cgiPmdSettings(char *query __attribute__((unused)), FILE *fs)
{
   ...
   uint16_t high, low, lb_bucket_sz;
   uint16_t rate, dload;
   uint8_t compensation_enable0, compensation_enable1, compensation_coeff1_q8, compensation_coeff2_q8;
   int8_t die_temp_ref;
   ...
   fprintf(fs, "  var link = document.createElement('a');\n");
   fprintf(fs, "  link.style.display = 'none';\n");
   fprintf(fs, "  document.body.appendChild(link);\n");

   fprintf(fs, "  link.setAttribute('download', 'pmd_calibration');\n");
   fprintf(fs, "  link.setAttribute('href', 'pmdbackupcalibration.conf');\n");
   fprintf(fs, "  link.click();\n");

   fprintf(fs, "  link.setAttribute('download', 'pmd_temp2apd');\n");
   fprintf(fs, "  link.setAttribute('href', 'pmdbackuptemp2apd.conf');\n");
   fprintf(fs, "  link.click();\n");

   fprintf(fs, "  link.setAttribute('download', 'pmd_calibration.json');\n");
   fprintf(fs, "  link.setAttribute('href', 'pmdbackupjson.conf');\n");
   fprintf(fs, "  link.click();\n");

   fprintf(fs, "  document.body.removeChild(link);\n");
   fprintf(fs, "}\n");
   ...
   fprintf(fs, "<form method='post' ENCTYPE='multipart/form-data' action='pmdjsonupload.cgi' onSubmit='return "
      "isInProgress();'>\n");
   fprintf(fs, "<b>Calibration (JSON format)</b>\n");
   fprintf(fs, "<br>\n");
   fprintf(fs, "<b>Step 1:</b> Obtain PMD calibration JSON file from your provider. This file replaces the other two "
      "legacy binary files (Calibration + Temp2Apd).<br>\n");
   ...
   fprintf(fs, "<b>Step 3:</b> Click the \"Update PMD Calibration (JSON)\" button once to upload the new file.<br>\n");
   ...
   fprintf(fs, "   <tr>\n");
   fprintf(fs, "      <td>PMD JSON File Name:&nbsp;\n");
   fprintf(fs, "      </td>\n");
   fprintf(fs, "      <td><input type='file' name='filename' size='15'></td>\n");
   fprintf(fs, "   </tr>\n");
   fprintf(fs, "</table>\n");
   fprintf(fs, "<p align=\"center\"><input type='submit' value='Update PMD Calibration (JSON)'></p>\n");
   fprintf(fs, "</form>\n");

   fprintf(fs, "<form method='post' ENCTYPE='multipart/form-data' action='pmdcalibrationupload.cgi' onSubmit='return "
      "isInProgress();'>\n");
   fprintf(fs, "<b>Calibration (legacy binary format)</b>\n");
   fprintf(fs, "<br>\n");
   fprintf(fs, "<b>Step 1:</b> Obtain legacy PMD calibration binary file from your provider.<br>\n");
   fprintf(fs, "<b>Step 2:</b> Enter the path to the image file location in the box below or<br>\n");
   fprintf(fs, "click the &quot;Choose File&quot; button to locate the file. You may Drag&Drop as well.<br>\n");
   fprintf(fs, "<b>Step 3:</b> Click the \"Update legacy PMD Calibration (binary)\" button once to upload the new file."
      "<br>\n");
   fprintf(fs, "NOTE: The update process takes about 10 Seconds to complete, and your Broadband Router\n");
   fprintf(fs, "will reboot.<br>\n");
   fprintf(fs, "<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">\n");
   ...
   fprintf(fs, "   </tr>\n");
   fprintf(fs, "</table>\n");
   fprintf(fs, "<p align=\"center\"><input type='submit' value='Update legacy PMD Calibration (binary)'></p>\n");
   fprintf(fs, "</form>\n");

   fprintf(fs, "<form method='post' ENCTYPE='multipart/form-data' action='pmdtemp2apdupload.cgi' onSubmit='return "
      "isInProgress();'>\n");
   fprintf(fs, "<b>Temp2Apd (legacy binary format)</b>\n");
   fprintf(fs, "<br>\n");
   fprintf(fs, "<b>Step 1:</b> Obtain legacy PMD Temp2Apd binary file from your provider.<br>\n");
   fprintf(fs, "<b>Step 2:</b> Enter the path to the image file location in the box below or<br>\n");
   fprintf(fs, "click the &quot;Choose File&quot; button to locate the file. You may Drag&Drop as well.<br>\n");
   fprintf(fs, "<b>Step 3:</b> Click the \"Update legacy PMD Temp2Apd (binary)\" button once to upload the new file."
      "<br>\n");
   fprintf(fs, "NOTE: The update process takes about 10 Seconds to complete, and your Broadband Router\n");
   fprintf(fs, "will reboot.<br>\n");
   fprintf(fs, "<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">\n");
   fprintf(fs, "   <tr>\n");
   fprintf(fs, "      <td>PMD Temp2Apd File Name:&nbsp;\n");
   fprintf(fs, "      </td>\n");
   fprintf(fs, "      <td><input type='file' name='filename' size='15'></td>\n");
   fprintf(fs, "   </tr>\n");
   fprintf(fs, "</table>\n");
   fprintf(fs, "<p align=\"center\"><input type='submit' value='Update legacy PMD Temp2Apd (binary)'></p>\n");
   fprintf(fs, "</form>\n");

   fprintf(fs, "Backup PMD Calibration files. JSON, binary and Temp2Apd. You may save the PMD configuration into 3 "
      "files on your PC.<br>\n");
   fprintf(fs, "<p align=\"center\"><input type='button' value='Backup PMD configuration files' onClick='backupClick()'"
      "></p>\n");
   ...
   PARAMETER_READ_AND_PRINT(PMD_CALIBRATION_FILE_VER);
   PARAMETER_READ_AND_PRINT(PMD_FAQ_LEVEL0_DAC);
   PARAMETER_READ_AND_PRINT(PMD_FAQ_LEVEL1_DAC);
   ...
   PRINT_TABLE_FIELD(PMD_LOS_THR, 0x%X, deassert);
   ...
   param.offset = PMD_ADF_LOS_THRESHOLDS;
   ret = ioctl(LaserDevFd, PMD_IOCTL_CAL_FILE_READ, &param);
   assert = val & 0xffff;
   deassert = (val & 0xffff0000) >> 16;
   PRINT_TABLE_FIELD(PMD_ADF_LOS_THRESHOLDS, 0x%X, assert);
   PRINT_TABLE_FIELD(PMD_ADF_LOS_THRESHOLDS, 0x%X, deassert);

   param.offset = PMD_ADF_LOS_LEAKY_BUCKET;
   ret = ioctl(LaserDevFd, PMD_IOCTL_CAL_FILE_READ, &param);
   assert = val & 0xff;
   lb_bucket_sz = (val & 0xff00) >> 8;
   PRINT_TABLE_FIELD(PMD_ADF_LOS_LEAKY_BUCKET, 0x%X, assert);
   PRINT_TABLE_FIELD(PMD_ADF_LOS_LEAKY_BUCKET, 0x%X, lb_bucket_sz);

   param.offset = PMD_COMPENSATION;
   ret = ioctl(LaserDevFd, PMD_IOCTL_CAL_FILE_READ, &param);
   compensation_enable0 = val & 1;
   compensation_enable1 = (val >> 1) & 1;
   die_temp_ref = (val >> 8) & 0xff;
   compensation_coeff1_q8 = (val >> 16) & 0xff;
   compensation_coeff2_q8 = (val >> 24) & 0xff;
   PRINT_TABLE_FIELD(PMD_COMPENSATION, 0x%X, compensation_enable0);
   PRINT_TABLE_FIELD(PMD_COMPENSATION, 0x%X, compensation_enable1);
   PRINT_TABLE_FIELD(PMD_COMPENSATION, 0x%X, die_temp_ref);
   PRINT_TABLE_FIELD(PMD_COMPENSATION, 0x%X, compensation_coeff1_q8);
   PRINT_TABLE_FIELD(PMD_COMPENSATION, 0x%X, compensation_coeff2_q8);
...
close_calibration_table:
   ...
   fflush(fs);
#else
   fprintf(fs, "Not Supported by DESKTOP_LINUX\n");
#endif /* DESKTOP_LINUX */
}
...
static void cgiBackupFile(char *backup_file_path, FILE *fs)
{
   ...
   pmd_calibration_file = fopen(backup_file_path, "r");
   if (NULL == pmd_calibration_file)
   {
      cmsLog_error("failed to open the %s file, errno=%d, %s", backup_file_path, errno, strerror(errno));
   }
   else
   {
...
#ifdef SUPPORT_TR69C_AUTONOMOUS_TRANSFER_COMPLETE
         imageTransferStats.faultCode = CMSRET_INTERNAL_ERROR;
         sprintf(imageTransferStats.faultStr, "Write of %s backup file failed.", backup_file_path);
#endif
         cmsLog_error("write of %s backup file failed, write_count=%u read_count=%u", backup_file_path, write_count,
            read_count);
      }
#ifdef SUPPORT_TR69C_AUTONOMOUS_TRANSFER_COMPLETE
      else
   ...
   CMSLOG_SEC_SET_USER(&logData, &glbWebVar.curUserName[0]);
   CMSLOG_SEC_SET_SRC_IP(&logData, &glbWebVar.pcIpAddr[0]);
   cmsLog_security(LOG_SECURITY_AUTH_RESOURCES, &logData, backup_file_path);
}

void cgiBackupPmdCalibration(char *query __attribute__((unused)), FILE *fs)
{
  cgiBackupFile(PMD_CALIBRATION_FILE_PATH, fs);
}

void cgiBackupPmdTemp2Apd(char *query __attribute__((unused)), FILE *fs)
{
  cgiBackupFile(PMD_TEMP2APD_FILE_PATH, fs);
}

void cgiBackupPmdJson(char *query __attribute__((unused)), FILE *fs)
{
  cgiBackupFile(PMD_CALIBRATION_JSON_FILE_PATH, fs);
}
