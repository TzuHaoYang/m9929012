 ...
 *
 * @return CmsRet enum from among the following:
 *         CMSRET_SUCCESS,
 *         CMSRET_PMD_CALIBRATION_FILE_SUCCESS,
 *         CMSRET_PMD_TEMP2APD_FILE_SUCCESS,
 *         CMSRET_PMD_JSON_FILE_SUCCESS,
                   ...
                   char **imageBufPtrPtr, UINT32 *imageLen, UBOOL8 flashB)
{
    ...
    *imageLen = 0;
    ...
    if (result == CMSRET_SUCCESS && upload_type != WEB_PMD_UPLOAD_CALIBRATION
        && upload_type != WEB_PMD_UPLOAD_TEMP2APD && upload_type != WEB_PMD_UPLOAD_JSON)
    {
    ...
    if (flashB != 0 && upload_type != WEB_PMD_UPLOAD_CALIBRATION && upload_type != WEB_PMD_UPLOAD_TEMP2APD
    ...
    *imageBufPtrPtr = curPtr;
    ...
    if (result == CMSRET_SUCCESS)
    {
       if (upload_type == WEB_PMD_UPLOAD_CALIBRATION)
       {
          result = CMSRET_PMD_CALIBRATION_FILE_SUCCESS;
       }
       else if (upload_type == WEB_PMD_UPLOAD_TEMP2APD)
       {
          result = CMSRET_PMD_TEMP2APD_FILE_SUCCESS;
       }
       else if (upload_type == WEB_PMD_UPLOAD_JSON)
       {
          result = CMSRET_PMD_JSON_FILE_SUCCESS;
       }
    }
...
static void upload_info_page(FILE * stream)
{
...
static void save_pmd_upload(const char *pmd_file_path, char *imagePtr, UINT32 imageLen)
{
    FILE *pmd_file_handler;

    pmd_file_handler = fopen(pmd_file_path, "w");
    if (pmd_file_handler)
    {
        fwrite(imagePtr, imageLen, 1, pmd_file_handler);
        fclose(pmd_file_handler);
    }
    else
    {
        printf("Can't write to %s.\n%s\n", pmd_file_path, strerror(errno));
        cmsLog_debug("Can't write to %s. %s", pmd_file_path, strerror(errno));
    }
}
...
int do_upload_pre(FILE * stream, int content_len, int upload_type)
{
    ...
    if (glbUploadStatus != CMSRET_SUCCESS && glbUploadStatus != CMSRET_PMD_CALIBRATION_FILE_SUCCESS
       && glbUploadStatus != CMSRET_PMD_TEMP2APD_FILE_SUCCESS && glbUploadStatus != CMSRET_PMD_JSON_FILE_SUCCESS)
    {
#ifdef SUPPORT_DEBUG_TOOLS
       /*
    ...
    {
       save_pmd_upload(PMD_CALIBRATION_FILE_PATH, imagePtr, imageLen);
       return WEB_STS_OK;
    }
    else if (glbUploadStatus == CMSRET_PMD_TEMP2APD_FILE_SUCCESS)
    {
       save_pmd_upload(PMD_TEMP2APD_FILE_PATH, imagePtr, imageLen);
       return WEB_STS_OK;
    }
    else if (glbUploadStatus == CMSRET_PMD_JSON_FILE_SUCCESS)
    {
       save_pmd_upload(PMD_CALIBRATION_JSON_FILE_PATH, imagePtr, imageLen);
       return WEB_STS_OK;
    }
    else
    {
...
void do_upload_post(void)
{
   ...
   else
   {
      ...
      modsw_getPreinstallOverwriteLocked(&overwrite);

      if (overwrite)
      {
         modsw_setPreinstallNeededRuntimeLocked(TRUE, FALSE);
