   ...
   (const MdmPathDescriptor *pathDesc)
{
   ...
   ret = cmsMdm_pathDescriptorToFullPathNoEndDot(pathDesc, &ethIntfFullPath);
    ...
    UINT32 shapingBurstSize)
{
                              ...
                              (void **) &shaperObj) == CMSRET_SUCCESS)
   {
      if (cmsUtl_strcmp(shaperObj->interface, ethIntfFullPath) == 0)
      {
         ...
         shaperObj->enable = shapingEnable;
...
void cgiQosPortShapingSavApply_dev2(char *query)
{
                              ...
                              (void **) &ethIntfObj) == CMSRET_SUCCESS)
   {
      if (ethIntfObj->enable && ethIntfObj->upstream)
      {
         ...
         sprintf(paraNameBuf, "%sWanEnable", ethIntfObj->name);
         if (cgiGetValueByName(query, paraNameBuf, cmd) == CGI_STS_OK)
         {
            sscanf(cmd, "%d", (int *)&shapingEnable);
         }
         else
         {
            shouldSetObj = FALSE;
                              ...
                              (void **) &ethIntfObj) == CMSRET_SUCCESS)
   {
      if (ethIntfObj->enable && !ethIntfObj->upstream)
      {
         ...
         sprintf(paraNameBuf, "%sLanEnable", ethIntfObj->name);
         if (cgiGetValueByName(query, paraNameBuf, cmd) == CGI_STS_OK)
         {
            sscanf(cmd, "%d", (int *)&shapingEnable);
         }
    ...
    UINT32 *shapingBurstSize)
{
                              ...
                              (void **) &shaperObj) == CMSRET_SUCCESS)
   {
      ...
      {
         found = TRUE;
         *shapingEnable = shaperObj->enable;
                                   ...
                                   char *varValue)
{
   ...
   p += sprintf(p, "            <table border='1' cellpadding='6' cellspacing='0'>\n");
   p += sprintf(p, "               <tr align='center'>\n");
   p += sprintf(p, "                  <td class='hd'>Interface</td>\n");
   p += sprintf(p, "                  <td class='hd'>Type</td>\n");
   p += sprintf(p, "                  <td class='hd'>Shaping Rate [-1:2147483] (Kbps)</td>\n");
   p += sprintf(p, "                  <td class='hd'>Burst Size (bytes)</td>\n");
   p += sprintf(p, "                  <td class='hd'>Enable</td>\n");
   p += sprintf(p, "               </tr>\n");
                              ...
                              (void **) &ethIntfObj) == CMSRET_SUCCESS)
   {
      if (ethIntfObj->enable && ethIntfObj->upstream)
      {
         ...
         if (shapingEnable == TRUE)
            p += sprintf(p, "                  <td align='center'><input type='checkbox' name='%sWanEnable' checked></td></td>\n",
                         ethIntfObj->name);
         else
            p += sprintf(p, "                  <td align='center'><input type='checkbox' name='%sWanEnable'></td></td>\n",
                         ethIntfObj->name);
         p += sprintf(p, "               </tr>\n");
      }
                              ...
                              (void **) &ethIntfObj) == CMSRET_SUCCESS)
   {
      if (ethIntfObj->enable && !ethIntfObj->upstream)
      {
         ...
         p += sprintf(p, "                  <td><input type='text' name='%sLanSizeCfg' maxlength='16' size='16' value='%d'></td>\n", 
                      ethIntfObj->name, shapingBurstSize);
         if (shapingEnable == TRUE)
            p += sprintf(p, "                  <td align='center'><input type='checkbox' name='%sLanEnable' checked></td>\n",
                         ethIntfObj->name);
         else
            p += sprintf(p, "                  <td align='center'><input type='checkbox' name='%sLanEnable'></td>\n",
                         ethIntfObj->name);
         p += sprintf(p, "               </tr>\n");
      }
