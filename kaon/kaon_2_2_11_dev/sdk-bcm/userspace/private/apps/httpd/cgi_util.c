...
{
    int fd, gpon_optics_type = -1;
    char buf[3] = {};

    fd = open("/proc/" BCM_I2C_PROC_DIR, O_RDWR);
    if (!fd)
        return -1;

    if ( read(fd, buf, sizeof(buf) - 1) > 0 )
    {
        gpon_optics_type = atoi(buf);
    }
    close(fd);


    return BCM_I2C_PON_OPTICS_TYPE_PMD == gpon_optics_type;
}
...
void cgiGetSysInfo(int argc __attribute__((unused)),
   ...
   else if ( cmsUtl_strcmp(argv[2], "uptimeString") == 0 )
   {
      ...
      sprintf(varValue, "%u", devCtl_getSdramSize());
   ...
   else if ( cmsUtl_strcmp(argv[2], "upldSts") == 0 )
   {
      ...
      switch (glbUploadStatus)
      {
         ...
         case CMSRET_PMD_TEMP2APD_FILE_SUCCESS:
            sprintf(varValue, "5");
            break;

         case CMSRET_PMD_JSON_FILE_SUCCESS:
            sprintf(varValue, "6");
            break;
   ...
   else if ( cmsUtl_strcmp(argv[2], "chipId") == 0 )
   {
      ...
      strcpy(varValue, "0");
#endif
   else if ( cmsUtl_strcmp(argv[2], "buildMap") == 0 )
#ifdef SUPPORT_MAP
      strcpy(varValue, "1");
#else
      strcpy(varValue, "0");
      ...
      strcpy(varValue, " style=\"display:none\"");
#endif
   else if ( cmsUtl_strcmp(argv[2], "buildWlanDpd") == 0 )
...
#else
   {
      ...
      strcpy(varValue, "0");
#endif
   else if ( cmsUtl_strcmp(argv[2], "supportarchertm") == 0 )
      ...
      strcpy(varValue, "0");
#endif
else if ( cmsUtl_strcmp(argv[2], "buildEthLag") == 0 )
#if defined(DMP_DEVICE2_ETHLAG_1)
         strcpy(varValue, "1");
#else
         strcpy(varValue, "0");
#endif
   ...
   else if ( cmsUtl_strcmp(argv[2], "buildWanSelect") == 0 )
#ifdef BCM_PON
         strcpy(varValue, "1");
#else
         strcpy(varValue, "0");
#endif
   else if ( cmsUtl_strcmp(argv[2], "buildDataModel") == 0 )
...
void cgiPrintDHCPLeases(FILE *fs)
{
   ...
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   char file6[BUFLEN_64];
   char strAddr6[BUFLEN_48], strState[BUFLEN_16];
#endif
...
#if defined(DMP_DEVICE2_HOSTS_2) && defined(DMP_X_BROADCOM_COM_DEV2_IPV6_1)
   while (CMSRET_SUCCESS == cmsObj_getNext(MDMOID_DEV2_IP_INTERFACE, &iidStack, (void **) &ipIntfObj))
   {
      ...
      sprintf(file6, HOST6_ACTIVES_FILE "_%s", ipIntfObj->name);

      if (NULL != (fp = fopen(file6, "r")))
      {
         while (EOF != fscanf(fp, "%s lladdr %s %s", strAddr6, strMacAddr, strState))
         {
            fprintf(fs,  " <tr><td></td><td>%s</td><td>%s</td><td></td></tr>\n",
                  strMacAddr, strAddr6);
         }
         fclose(fp);
      }
...
#ifdef DMP_X_BROADCOM_COM_GPONWAN_1
void cgiGetOpticalLinkStatus (int argc __attribute__((unused)), char **argv  __attribute__((unused)), char *varValue)
{
   ...
   {
      sprintf(varValue, "OFF");
   }

   return;
}

void cgiGetGponUpstreamFec (int argc __attribute__((unused)), char **argv  __attribute__((unused)), char *varValue)
{
   ...
   if (ret == 0)
   {
      if (info.fecMode & BCM_PLOAM_FEC_MODE_FLAG_US_FEC_ON)
         sprintf(varValue, "Enable");
      else
         sprintf(varValue, "Disable");
   }
   else
   {
      sprintf(varValue, "Disable");
...
void cgiGetGponDownstreamFec (int argc __attribute__((unused)), char **argv  __attribute__((unused)), char *varValue)
{
   BCM_Ploam_GetFecModeInfo info;
   int ret = gponCtl_getFecMode(&info);
