...
static CmsRet cgiXmppSetManagementServer_igd(InstanceIdStack* iidStack,
                                             char *jabberID,
                                             char *username  __attribute__((unused)),
                                             int enable)
{
   InstanceIdStack mngrServerIidStack = EMPTY_INSTANCE_ID_STACK;
   InstanceIdStack xmppConnIidStack = EMPTY_INSTANCE_ID_STACK;
   ManagementServerObject *mngrServerObj = NULL;
   Dev2XmppConnObject *xmppConn = NULL;
   MdmPathDescriptor xmppConnPathDesc;
   char *xmppConnFullPathStr=NULL, *p;
   char newStr[BUFLEN_1024]={0}, tmpJabberID[BUFLEN_1024]={0};
   int strLen = 1024, size = 0;
   ...
   {
      // always pick the newly enabled XMPPConn object jabberID for ConnReqXMPPConnection
      if (enable)
      {
      ...
      /* re-construct all disabled XMPPConn objects to be AllowedJabberIDs */
      p = newStr;
      while (cmsObj_getNext(MDMOID_DEV2_XMPP_CONN, &xmppConnIidStack,
                            (void **) &xmppConn) == CMSRET_SUCCESS)
      {
         if (xmppConn->enable == FALSE)
         {
            qdmXmpp_getJabberIdLocked_dev2(&xmppConnIidStack, tmpJabberID);
            size = snprintf(p, strLen, "%s,", tmpJabberID);
            p += size;
            strLen -= size;
         }
         cmsObj_free((void **) &xmppConn);
      }
      cmsLog_debug("connReqAllowedJabberIDs: %s", newStr);
...
static CmsRet cgiXmppSetManagementServer_dev2(InstanceIdStack* iidStack,
                                              char *jabberID,
                                              char *username  __attribute__((unused)),
                                              int enable)
{
...
static void cgiXmppConnAdd(char *query, FILE *fs) 
{
   ...
   if(cmsObj_getAncestor(MDMOID_DEV2_XMPP_CONN, MDMOID_DEV2_XMPP_CONN_SERVER, &iidStack, (void **)&xmppConn) == CMSRET_SUCCESS)
   {
      ...
      else
      {
         ...
         cgiXmppSetManagementServer_igd(&iidStack, jabberID, username, atoi(enblConn));
#elif defined(SUPPORT_DM_HYBRID)
         cgiXmppSetManagementServer_igd(&iidStack, jabberID, username, atoi(enblConn));
#elif defined(SUPPORT_DM_PURE181)
         cgiXmppSetManagementServer_dev2(&iidStack, jabberID, username, atoi(enblConn));
