...
void cgiGetDiagId_igd(int type, PDIAG_ID pDiagId)
{
   ...
   info[0] = '\0';
#if defined(SUPPORT_DSL) || defined(DMP_ETHERNETWAN_1) 
...
#ifdef DMP_ADSLWAN_1
/** Return TRUE if DSL link is up and also return link type.
 *  New code should use qdmDsl_isXdslLinkUpLocked().
 */
static UBOOL8 isXdslConnUp(WanLinkType *pLinkType)
{
   UBOOL8 linkup = FALSE;
   UBOOL8 found = FALSE;
   WanDevObject      *wanDev  = NULL;
...
void cgiPrintDefaultGatewayDiag_igd(char *varValue)
{
   ...
   {
      cmsLog_debug("getActiveDefaultGateway returned %s", gwIfc);
      if ((strcmp(gwIfc,"&nbsp") != 0) && (gwIfc[0] != '\0'))
      {
         ...
         if (cmsObj_getNext(MDMOID_WAN_DSL_LINK_CFG, &parentIidStack, (void **)&dslLinkCfg) == CMSRET_SUCCESS)
         {
            ...
            {
               cmsLog_debug("ipConn->X_BROADCOM_COM_IfName %s, defaultGateway %s",
                            ipConn->X_BROADCOM_COM_IfName,
                            ipConn->defaultGateway);
               ...
               if ((cmsUtl_strcmp(ipConn->X_BROADCOM_COM_IfName,gwIfc)) == 0)
               {
                  ...
                  {
                     cmsUtl_strncpy(gwIp, ipConn->defaultGateway, sizeof(gwIp));
                  }
               }
            ...
            {
               cmsLog_debug("pppConn->X_BROADCOM_COM_IfName %s, defaultGateway %s",
                            pppConn->X_BROADCOM_COM_IfName,
               ...
               if ((cmsUtl_strcmp(pppConn->X_BROADCOM_COM_IfName,gwIfc)) == 0)
               {
                  ...
                  {
                     cmsUtl_strncpy(gwIp, pppConn->X_BROADCOM_COM_DefaultGateway, sizeof(gwIp));
                  }
               }
         ...
         if (gwIp[0] != '\0')
         {
            ...
            IPPingDiagObject pingParms;

            memset(&pingParms, 0, sizeof(pingParms));
            ...
            cmsLog_debug("gwIfc %s, pingParms.interface %s",gwIfc, pingParms.interface);
            ...
            CMSMEM_FREE_BUF_AND_NULL_PTR(pingParms.diagnosticsState);
            CMSMEM_FREE_BUF_AND_NULL_PTR(pingParms.interface);
            CMSMEM_FREE_BUF_AND_NULL_PTR(pingParms.host);
            
            /* and then read the object after timeout period */
            if (ret == CMSRET_SUCCESS)
            {
               IPPingDiagObject pingResult;
               ...
               {
                  cmsLog_debug("got PingResult state %s successCount %d",
...
void cgiPrintDnsDiag_igd(char *varValue)
{
   ...
   if ((dnsPrimary[0] != '\0') && (strcmp(dnsPrimary,"0.0.0.0") != 0))
   {
      ...
      pingParms.dataBlockSize = 64;
      pingParms.DSCP = 0;
      cmsLog_debug("pinging dnsPrimary at %s", dnsPrimary);
