...
void cgiCertImport(char *query, FILE *fs, int type) 
{
   ...
   {
      if ((ret = rutCert_processImportedCert(certCfg)) != CMSRET_SUCCESS) 
      {
         sprintf(cmd, "Error in imported certificate %s.<br> Status: %d.", name, ret);
         cgiCertErr(fs, cmd, type);
      }
      else 
      {
...
void writeCertScript(FILE *fs, int type) {
   ...
   fprintf(fs, "        alert('This certificate is in use!\\nPlease remove all references to this certificate first.');\n");
   fprintf(fs, "   } else {\n");
