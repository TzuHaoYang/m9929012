...
void cgiWanIfc(char *query, FILE *fs) 
{
   ...
   while (nl != NULL)
   {
    ...
    defined(DMP_X_BROADCOM_COM_GPONWAN_1) || defined(DMP_X_BROADCOM_COM_EPONWAN_1)         || defined(DMP_DEVICE2_ETHLAG_1)
      if (cmsUtl_strstr(nl->name, ETH_IFC_STR) || 
         ...
         cmsUtl_strstr(nl->name, ETHLAG_IFC_STR))
      {
            fprintf(fs, "<script language='javascript'>\n");
            fprintf(fs, "<!-- hide\n");
...
void cgiWanSrvc(char *query, FILE *fs)
{
   ...
   if (iftype == WAN_IFC_ATM || iftype == WAN_IFC_ETH || iftype == WAN_IFC_PTM || iftype == WAN_IFC_WIFI  ||iftype == WAN_IFC_ETHLAG)
   {
      fprintf(fs, "<table border='0' cellpadding='0' cellspacing='0'>\n");
      ...
      if ((iftype == WAN_IFC_ATM || iftype == WAN_IFC_PTM || iftype == WAN_IFC_ETH || iftype == WAN_IFC_WIFI  ||iftype == WAN_IFC_ETHLAG) &&
...
void writeWanSrvcScript(FILE *fs, const WanIfcType iftype)
{
   ...
   *p = '\0';
      ...
      cmsUtl_strstr(glbWebVar.wanL2IfName, ETHLAG_IFC_STR))
   {
...
cmsLog_debug("needIPv6<%d>", needIPv6); //FIXME: no need!!
   ...
   if (needEoAIntf == TRUE)
   {
      strncat(ifcListStr, "No Interface/None|", sizeof(ifcListStr)-1);      
   }
...
void cgiFormLayer3IfName(char *L3IfName)
{
   ...
   if (cmsUtl_strstr(tempL3IfName, ETH_IFC_STR) || 
      ...
      cmsUtl_strstr(tempL3IfName, ETHLAG_IFC_STR))      
   {
...
void cgiFormServiceName(char *serviceName)
{
   char tmpserviceName[CMS_IFNAME_LENGTH*2];
   char serviceNameSufix[CMS_IFNAME_LENGTH];
   char *p;
   ...
   case CMS_WAN_TYPE_DYNAMIC_ETHERNET_IP:
      /* eg. get  ipoe_0_2_35 */
      sprintf(tmpserviceName, "%s_%s", IPOE_IFC_STR,  serviceNameSufix);
      break;
     
   case CMS_WAN_TYPE_IPOA:
      sprintf(tmpserviceName, "%s_%s", IPOA_IFC_STR,  serviceNameSufix);
      break;
     
   case CMS_WAN_TYPE_PPPOE:
   case CMS_WAN_TYPE_ETHERNET_PPPOE:
      sprintf(tmpserviceName, "%s_%s", PPPOE_IFC_STR,  serviceNameSufix);
      break;
     
   case CMS_WAN_TYPE_PPPOA:
      sprintf(tmpserviceName, "%s_%s", PPPOA_IFC_STR,  serviceNameSufix);
      break;
     
   case CMS_WAN_TYPE_BRIDGE:
   case CMS_WAN_TYPE_ETHERNET_BRIDGE:
      sprintf(tmpserviceName, "%s_%s", BRIDGE_IFC_STR,  serviceNameSufix);
      break;
   ...
   sprintf(serviceName, "%s", tmpserviceName);

   if (glbWebVar.connMode == CMS_CONNECTION_MODE_VLANMUX)
   {
      if (glbWebVar.vlanMuxId >= 0)
      {
         sprintf(serviceName, "%s.%d", tmpserviceName, glbWebVar.vlanMuxId);
      }
   }
...
void cgiGetUiWanUpStreamMode(int argc __attribute__((unused)), char **argv __attribute__((unused)), char *varValue)
{
   strcpy(varValue,"0");
   ...
   if (strcmp(wanTypeBuf, MDMVS_EPON) == 0)
      strcpy(varValue,"3");
   else if (strcmp(wanTypeBuf, MDMVS_GPON) == 0)
      strcpy(varValue,"4");
#endif
}
