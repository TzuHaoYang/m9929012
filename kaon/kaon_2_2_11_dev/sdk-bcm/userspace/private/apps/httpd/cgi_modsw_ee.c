...
void cgiModSwEEApply(char *query, FILE *fs, UBOOL8 enable)
{
   ...
   if (ret != CMSRET_SUCCESS)
   {
      if (enable == TRUE)
      {
         beep_debug_write_to_log(BEEP_LOG_MODE_DU, SMReqId,
                                 eeObj->name, "Start Request",
                                 "Could not get ExecEnvObject");
      }
      else
      {
         beep_debug_write_to_log(BEEP_LOG_MODE_DU, SMReqId,
                                 eeObj->name, "Stop Request",
                                 "Could not get ExecEnvObject");
      }
   ...
   ret = modsw_setExecEnvEnableLocked(eeObj->name, eeObj->vendor,
                                      eeObj->version, enable);
   if (ret != CMSRET_SUCCESS)
   {
      if (enable == TRUE)
      {
   ...
   {
      beep_debug_write_to_log(BEEP_LOG_MODE_DU, SMReqId,
                              eeObj->name, "Start Request", "Success");
   }
   else
   {
      beep_debug_write_to_log(BEEP_LOG_MODE_DU, SMReqId,
                              eeObj->name, "Stop Request", "Success");
   }
...
void cgiModSwEEView(FILE *fs, UBOOL8 refresh)
{
   ...
   while (cmsObj_getNextFlags(MDMOID_EXEC_ENV,
                              &iidStack,
                              OGF_NO_VALUE_UPDATE,
                              (void **)&ExecEnv) == CMSRET_SUCCESS)
   {
      UBOOL8 parentUp = TRUE;

      /* Check if the parent EE is up */
      if (!IS_EMPTY_STRING(ExecEnv->parentExecEnv))
      {
         MdmPathDescriptor pathDesc;
         ExecEnvObject *parent = NULL;
         CmsRet ret;

         ret = cmsMdm_fullPathToPathDescriptor(ExecEnv->parentExecEnv, &pathDesc);
         if (ret != CMSRET_SUCCESS)
         {
            cmsLog_error("Could not convert %s", ExecEnv->parentExecEnv);
         }
         else
         {
            ...
            {
               parentUp = !cmsUtl_strcmp(parent->status, MDMVS_UP);
      ...
      {
         /* Enable button */
         fprintf(fs, "      <td align='center'><input type='button' disabled='1' onClick='EnableClick(this, \"%s\")' value='Enable'></td>\n",
                 eeFullPath);
         /* Uninstall button */
      ...
      else
      {
         ...
         /* Uninstall checkbox and Update button are enabled/disabled if EE is NOT HostEE */
         if (cmsUtl_strcmp(ExecEnv->name, BEEP_HOSTEE_NAME))
         {
            /* ExecEnv is Enable */
            if (ExecEnv->enable)
            {
         ...
         /* Uninstall checkbox and Update button are disabled if EE is HostEE */
         else
         {
            /* Uninstall button */
            fprintf(fs, "      <td align='center'><input type='checkbox' disabled='1' name='uninstall' value='%s'></td>\n",
...
void cgiModSwEEInstallApply(char *query, FILE *fs)
{
   ...
   qdmModsw_getExecEnvFullPathByNameLocked(BEEP_HOSTEE_NAME,
...
CmsRet cgiModSwUninstallEEAction(const char *fullPath)
{
   ...
   /* X_BROADCOM_COM_ExecutionEnvList is unique in BEEP */
   while (found == FALSE &&
   ...
   {
      if (cmsUtl_strcmp(duObj->X_BROADCOM_COM_ExecutionEnvList, fullPath) == 0)
      {
         found = TRUE;
   ...
   else
   {
      cmsLog_debug("Send %s to %d EE full path %s",
                   msgPayload->operation, reqMsg->dst, msgPayload->execEnvFullPath);
   }

   beep_debug_write_to_log(BEEP_LOG_MODE_DU, SMReqId,
                           "bbcd", "Uninstall Request", "Success");
...
void cgiModSwEEUpdateApply(char *query, FILE *fs)
{
                               ...
                               (void **)&duObj) == CMSRET_SUCCESS))
   {
      if (cmsUtl_strcmp(duObj->X_BROADCOM_COM_ExecutionEnvList, fullPath) == 0)
      {
         ...
         if (IS_EMPTY_STRING(version) == TRUE)
         {
            cmsUtl_strncpy(msgPayload->version, duObj->version, sizeof(msgPayload->version));
         }
         else
         {
            cmsUtl_strncpy(msgPayload->version, version, sizeof(msgPayload->version));
         }
...
void cgiModSwDUInstallApply(char *query, FILE *fs)
{
   ...
   ret = qdmModsw_getExecEnvFullPathByAliasLocked(EEName,
                                     msgPayload->execEnvFullPath,
                                     sizeof(msgPayload->execEnvFullPath));
   if (ret != CMSRET_SUCCESS)
   {
   ...
   beep_debug_write_to_log(BEEP_LOG_MODE_DU, SMReqId, EEName, "Install Request", "Success");
...
CmsRet cgiModSwUninstallDUAction(const char *DUID)
{
   ...
   /* DUID is unique in BEEP including multiple EEs */
   while (found == FALSE &&
          (cmsObj_getNextFlags(MDMOID_DU,
                               &iidStack,
                               OGF_NO_VALUE_UPDATE,
                               (void **)&duObj) == CMSRET_SUCCESS))
   {
      if(cmsUtl_strcmp(duObj->DUID, DUID) == 0)
      {
         ...
         ret = qdmModsw_getExecEnvAliasByFullPathLocked(duObj->executionEnvRef,
                                                        EEName, sizeof(EEName));
         if (ret != CMSRET_SUCCESS)
         {
            cmsLog_error("could not get EE name for execEnvRef=%s duid=%s",
                         duObj->executionEnvRef, duObj->DUID);
         }
         ...
         {
            cmsLog_error("Failed to send message (ret=%d)", ret);
            beep_debug_write_to_log(BEEP_LOG_MODE_DU, SMReqId, EEName,
                                    "Uninstall Request", "Fail to send message");
         }
         else
         {
...
void writeDUUpdateCfgScript(FILE *fs)
{
    ...
    * btnNext()
    */
   fprintf(fs, "function btnapply(DUID) {\n");
   ...
   fprintf(fs, "     // append version\n");
   fprintf(fs, "     loc += '&DUver=' + encodeUrl(DUver.value);\n");     
         
   fprintf(fs, "     // append duid\n");
   fprintf(fs, "     loc += '&DUduid=' + DUID;\n");    
   fprintf(fs, "  };\n");
...
void cgiModSwDUUpdateCfg(char *query, FILE *fs)
{
   ...
   fprintf(fs, "<tr><td class='hd'>DU UUID:&nbsp;</td>\n");
   fprintf(fs, "<td><input type='text' ReadOnly =true size='32' name='DUuuid' value='%s'></td></tr>\n",UUID);
   fprintf(fs, "<tr><td class='hd'>DU username:&nbsp;</td>\n");
   fprintf(fs, "<td><input type='text' size='32' name='DUusrname'></td></tr>\n");
   fprintf(fs, "<tr><td class='hd'>DU password:&nbsp;</td>\n");
   fprintf(fs, "<td><input type='password' size='32' name='DUpwd'></td></tr>\n");
   fprintf(fs, "<tr><td class='hd'>DU version:&nbsp;</td>\n");
   fprintf(fs, "<td><input type='text' ReadOnly =true size='32' name='DUver' value='%s'></td>\n", version);
   fprintf(fs, "</tr>\n");
   fprintf(fs, "</table><br><br>\n");

   fprintf(fs, "<center>\n");
   fprintf(fs, "<input type='button' onClick='btnapply(\"%s\")' value='Apply/Update'>\n", DUID);
   fprintf(fs, "<input type='button' onClick='btnCancel()' value='Cancel'>\n");
   fprintf(fs, "</center>\n</form>\n</blockquote>\n</body>\n</html>\n");
...
void cgiModSwDUUpdateApply(char *query, FILE *fs)
{
   ...
   char uuid[BUFLEN_40]={0};
   char username[BUFLEN_256]={0};
   char password[BUFLEN_256]={0};
   char version[BUFLEN_32]={0};
   char duid[BUFLEN_64]={0};
   char msg[BUFLEN_256]={0};
   char EEName[BUFLEN_256]={0};
   CmsRet ret = CMSRET_SUCCESS;
   CmsMsgHeader *reqMsg;
   unsigned char *body;
   ...
   cgiGetValueByName(query, "DUduid", duid);

   cmsLog_debug("web install url=%s, uuid=%s, username=%s, password=%s, version=%s duid=%s",
      url,uuid,username,password,version,duid);
                              ...
                              (void **)&duObj) == CMSRET_SUCCESS)
   {
      if (cmsUtl_strcmp(duObj->DUID, duid) == 0)
      {
         ...
         if (ret != CMSRET_SUCCESS)
         {
            CMSMEM_FREE_BUF_AND_NULL_PTR(reqMsg);
            cmsObj_free((void **) &duObj);
            cmsLog_error("could not get mngrEid for execEnvRef=%s",
                         duObj->executionEnvRef);
            sprintf(msg, "could not get mngrEid for execEnvRef=%s.",
                         duObj->executionEnvRef);
            cgiWriteMessagePage(fs, "DU Update Error", msg, "modSwDU.cmd");
            return;
         }
         ...
         {
            cmsLog_debug("Send %s to %d eename=%s uuid=%s", msgPayload->operation,
                         reqMsg->dst, EEName, msgPayload->UUID);

            beep_debug_write_to_log(BEEP_LOG_MODE_DU, SMReqId, EEName, "Update Request", "Success");
         }
      }
   ...
   {
      CMSMEM_FREE_BUF_AND_NULL_PTR(reqMsg);
      cmsLog_error("Could not find duid %s uuid %s version %s",  duid, uuid, version);
      sprintf(msg, "Could not find duid %s uuid %s version %s.", duid, uuid, version);
      cgiWriteMessagePage(fs, "DU Update Error", msg, "modSwDU.cmd");
      return;
   }
...
void cgiModSwEUView(FILE *fs, UBOOL8 refresh)
{
   ...
   while (cmsObj_getNextFlags(MDMOID_EU, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&EU) == CMSRET_SUCCESS)
   {
      ...
      else
      {
         ...
         else if (cmsUtl_strcmp(EU->executionFaultCode, MDMVS_UNSTARTABLE) == 0)
         {
            /* EU fault code is unstartable --> disable Start/Stop button */
            if(cmsUtl_strcmp(EU->status,"Idle") == 0)
               fprintf(fs, "      <td align='center'><input type='button' disabled='1' onClick='startClick(\"%s\")' value='Start'></td>\n", 
                       (EU->EUID ? EU->EUID:"(null)"));
            else
...
static CmsRet cgiModSwGetEuCpusByEuIidStack(InstanceIdStack *pIidStackEu,
                                            char *cpus, UINT32 cpusLength)
{
   CmsRet ret = CMSRET_INVALID_ARGUMENTS;
   char fileName[BUFLEN_1024]={0};
   char *pNewLine = NULL;
   EUObject *objectEu = NULL;
   ExecEnvObject *objectEe = NULL;
   InstanceIdStack iidStack;
   FILE *fp = NULL;

   if (cpus == NULL || cpusLength == 0)
   {
      return ret;
   }
   ...
   /* get EU object */
   ret = cmsObj_get(MDMOID_EU,
                    pIidStackEu,
                    OGF_NO_VALUE_UPDATE,
                    (void **)&objectEu);

   /* do nothing if application does not have EU info */
   if (ret != CMSRET_SUCCESS)
   {
      return ret;
   }
   ...
   /* get EE object */
   ret = qdmModsw_getExecEnvObjectByFullPathLocked(objectEu->executionEnvRef,
                                                   &objectEe,
                                                   &iidStack);

   /* do nothing if application does not have EE info */
   if (ret != CMSRET_SUCCESS)
   {
      /* free EU object */
      cmsObj_free((void **) &objectEu);
      return ret;
   }

   snprintf(fileName, sizeof(fileName),
            "/sys/fs/cgroup/cpuset/lxc/%s/lxc/%s/cpuset.cpus",
            objectEe->X_BROADCOM_COM_ContainerName,
            objectEu->EUID);

   /* free EE object */
   cmsObj_free((void **) &objectEe);
   ...
   {
      /* get cpus info */
      if ( fgets(cpus, cpusLength, fp) ) {
          /* remove the last newline character */
          pNewLine = strrchr(cpus, '\n');
      }
      /* close file descriptor pointer */
      fclose(fp);
   ...
   cmsLog_debug("fileName: <%s>, cpus: <%s>, ret: <%d>", fileName, cpus, ret);

   return ret;
}

void cgiModSwEUManifestView(FILE *fs, 
                            InstanceIdStack *pIidStackManifest)
{
   ManifestObject *objectManifest = NULL;
   CmsRet ret = CMSRET_SUCCESS;

   /* get EU manifest object */
   ret = cmsObj_get(MDMOID_MANIFEST,
                    pIidStackManifest,
                    OGF_NO_VALUE_UPDATE,
                    (void **)&objectManifest);

   /* do nothing if application does not have manifest info */
   if (ret != CMSRET_SUCCESS)
   {
      return;
   }
   ...
   fprintf(fs, "    <div id='manifest' class='tabcontent'><center>\n");

   /* write table */
   fprintf(fs, "    <table id='tblManifest' border='1' cellpadding='4' cellspacing='0'>\n");
   fprintf(fs, "      <tbody>\n");
   fprintf(fs, "        <tr>\n");
   fprintf(fs, "          <td class='hd' width='200'>Supper User</td>\n");
   if (cmsUtl_strcmp(objectManifest->user, SUPER_USER_STRING) == 0)
      fprintf(fs, "          <td>Yes</td>\n");
   else
      fprintf(fs, "          <td>No</td>\n");
   fprintf(fs, "        </tr>\n");
   fprintf(fs, "        <tr>\n");
   fprintf(fs, "          <td class='hd'>Maximum Memory (KBytes)</td>\n");
   fprintf(fs, "          <td>%" PRIu64 "</td>\n", objectManifest->maxMemory);
   fprintf(fs, "        </tr>\n");
   fprintf(fs, "        <tr>\n");
   fprintf(fs, "          <td class='hd'>Maximum Flash Size (KBytes)</td>\n");
   fprintf(fs, "          <td>%d</td>\n", objectManifest->maxFlashSize);
   fprintf(fs, "        </tr>\n");
   fprintf(fs, "        <tr>\n");
   fprintf(fs, "          <td class='hd'>Network Mode</td>\n");
   fprintf(fs, "          <td>%s</td>\n", (objectManifest->networkMode ? objectManifest->networkMode : ""));
   fprintf(fs, "        </tr>\n");
   if (cmsUtl_strcmp(objectManifest->networkMode, MDMVS_LANONLY) == 0)
   {
      fprintf(fs, "        <tr>\n");
      fprintf(fs, "          <td class='hd'>Interface Identifier</td>\n");
      fprintf(fs, "          <td>%u</td>\n", objectManifest->interfaceIdentifier);
      fprintf(fs, "        </tr>\n");
   }
   fprintf(fs, "        <tr>\n");
   fprintf(fs, "          <td class='hd'>Exposed Ports</td>\n");
   fprintf(fs, "          <td>%s</td>\n", (objectManifest->exposedPorts ? objectManifest->exposedPorts : ""));
   fprintf(fs, "        </tr>\n");
   fprintf(fs, "        <tr>\n");
   fprintf(fs, "          <td class='hd'>Realtime Runtime (usecs)</td>\n");
   fprintf(fs, "          <td>%u</td>\n", objectManifest->realtimeRuntime);
   fprintf(fs, "        </tr>\n");
   fprintf(fs, "        <tr>\n");
   fprintf(fs, "          <td class='hd'>CPUs</td>\n");
   if (IS_EMPTY_STRING(objectManifest->cpus))
   {
      char cpus[BUFLEN_64]={0};

      cgiModSwGetEuCpusByEuIidStack(pIidStackManifest, cpus, sizeof(cpus));
      fprintf(fs, "          <td>%s</td>\n", cpus);
   }
   else
   {
   ...
   /* free EU manifest object */
   cmsObj_free((void **) &objectManifest);
}
...
static void writeEuExtensionScript(FILE *fs, const char *EUID, const char *name)
{
   ...
   fprintf(fs, "        document.getElementById('defaultTab').click();\n");
   fprintf(fs, "      });\n\n");
   ...
   fprintf(fs, "      function defaultOpen() {\n");
   fprintf(fs, "        document.getElementById('defaultTab').click();\n");
   fprintf(fs, "      }\n\n");
   fprintf(fs, "    </script>\n");
}
...
void cgiModSwEUExtensionView(char *query, FILE *fs)
{
   ...
   char euFullPath[BUFLEN_256]={0};
   char msg[BUFLEN_256+BUFLEN_64]={0};
   MdmPathDescriptor pathDesc;
   CmsRet ret = CMSRET_SUCCESS;
   ...
   /* write start body */
   fprintf(fs, "  <body onload='defaultOpen()'>\n");
   fprintf(fs, "  <form>\n");
   fprintf(fs, "    <center><h3>Execution Unit Extension %s</h3><br>\n", name);
   
   /* write tab bar */
   fprintf(fs, "    <ul class='tab'>\n");
   fprintf(fs, "      <li><a href='#' class='tablinks' onclick=\"openTab(event, 'client')\" id='defaultTab'>Client</a></li>\n");
   fprintf(fs, "      <li><a href='#' class='tablinks' onclick=\"openTab(event, 'service')\">Service</a></li>\n");
   fprintf(fs, "      <li><a href='#' class='tablinks' onclick=\"openTab(event, 'manifest')\">Manifest</a></li>\n");
   fprintf(fs, "    </ul>\n");
   fprintf(fs, "    <br>\n");
   ...
   cgiModSwEUManifestView(fs, &(pathDesc.iidStack));
...
void cgiModSwLogDU(char *query, FILE *fs)
{
   ...
   ret = cmsUtl_getRunTimePath(BEEP_DEBUG_DU_LOG, logfile, sizeof(logfile));
   if (ret != CMSRET_SUCCESS)
   {
   ...
   else
   {
      ...
      char nameStr[BUFLEN_64];
      char operStr[BUFLEN_32];
      char logMsg[BUFLEN_512], *log;
      ...
      while (fgets(line, sizeof(line), fp) != NULL)
      {
         ...
         sscanf(line, "%[^|]%*c %d | %[^|]%*c %[^|]%*c", dateTime, &reqId, nameStr, operStr);

         log = strrchr(line,'|');
         if(log != NULL)
         {
            log += 2;
            strncpy(logMsg, log, sizeof(logMsg));
         }
...
{
   char clean[BUFLEN_256]={0};
   char logfile[CMS_MAX_FULLPATH_LENGTH]={0};
   FILE *fp = NULL;
   CmsRet ret = CMSRET_SUCCESS;

   ret = cmsUtl_getRunTimePath(BEEP_DEBUG_EU_LOG, logfile, sizeof(logfile));
   if (ret != CMSRET_SUCCESS)
   {
      cmsLog_error("debug logfile name too long (max=%d)", sizeof(logfile));
   }
...
void cgiModSwLogEE(char *query, FILE *fs)
{
   ...
   /* Since EE and DU state change is same now, the DU log will be used and displayed 
   * on both EE and DU "View log" tab for now.  EE and DU log maybe changed on later on so this function
