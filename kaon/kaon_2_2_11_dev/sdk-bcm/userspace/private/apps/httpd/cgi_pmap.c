   ...
   while (pIfName <= (grpedIntf + strlen(grpedIntf))) {
      if ((pIfNameEnd = strstr(pIfName, "|")) != NULL) {
         ...
         if (*ifName != '\0')
         {
            ...
            if (strstr(ifName, WLAN_IFC_STR) != NULL)
...
#ifdef BUILD_BRCM_UNFWLCFG
      if (strstr(wanIfName, WLAN_IFC_STR) != NULL)
          glbWlRestartNeeded = TRUE;
#endif
   }
...
static CmsRet portmapDeleteBridges(char *lst)
{
   ...
   while ( pToken != NULL )
   {
     ...
     cgiGetIntfGroupInfoByName(groupName, &brIdx,
...
void cgiPortMapView(FILE *fs) {
  ...
  fprintf(fs, "<b>Interface Grouping -- A maximum %d entries can be configured</b><br><br>\n",
			PMAP_MAX_ENTRY);
  fprintf(fs, "Interface Grouping supports multiple ports to WAN and bridging groups. Each group\n");
