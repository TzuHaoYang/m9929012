...
void cgiL2tpAcWanCfgView(FILE *fs);
void cgiL2tpAcWanCfgView_igd(FILE *fs);
void cgiL2tpAcWanCfgView_dev2(FILE *fs);

#if defined(SUPPORT_DM_LEGACY98)
#define cgiL2tpAcWanCfgView(f)  cgiL2tpAcWanCfgView_igd((f))
#elif defined(SUPPORT_DM_HYBRID)
#define cgiL2tpAcWanCfgView(f)  cgiL2tpAcWanCfgView_igd((f))
#elif defined(SUPPORT_DM_PURE181)
#define cgiL2tpAcWanCfgView(f)  cgiL2tpAcWanCfgView_dev2((f))
#elif defined(SUPPORT_DM_DETECT)
#define cgiL2tpAcWanCfgView(f)   (cmsMdm_isDataModelDevice2() ? \
                         cgiL2tpAcWanCfgView_dev2((f))   : \
                         cgiL2tpAcWanCfgView_igd((f)))
#endif


static void writeL2tpAcCfgScript(FILE *fs, char *addLoc, char *removeLoc) 
{
...
void cgiL2tpAcWanCfgView_igd(FILE *fs) 
{
   ...
   InstanceIdStack l2tpId = EMPTY_INSTANCE_ID_STACK;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   CmsRet ret;

   /* write Java Script */
   writeL2tpAcCfgScript(fs, "cfgl2tpac", "l2tpacwan");

   /* write table */
   fprintf(fs, "<b>L2TP Client Side PPP Connection</b><br><br>\n");
   fprintf(fs, "Choose Add, Remove to configure a PPP over L2TP WAN Service)<br><br>\n");
   fprintf(fs, "<table border='1' cellpadding='4' cellspacing='0'>\n");
   /* write table header */
   fprintf(fs, "   <tr align='center'>\n");
   fprintf(fs, "      <td class='hd'>Tunnel Name</td>\n");
   fprintf(fs, "      <td class='hd'>LNS Ip Address</td>\n");
   fprintf(fs, "      <td class='hd'>Remove</td>\n");
   fprintf(fs, "   </tr>\n");

   if (dalWan_getWanL2tpIidStatck(&l2tpId) != CMSRET_SUCCESS)
   {
       return;
   }

   while ((ret = cmsObj_getNextInSubTree(MDMOID_L2TP_AC_INTF_CONFIG, &l2tpId, &iidStack, (void **)&L2tpAcIntf)) == CMSRET_SUCCESS)
   {
      if (L2tpAcIntf->tunnelName)
      {
         fprintf(fs, "   <tr align='center'>\n");
         fprintf(fs, "      <td>%s</td>\n", L2tpAcIntf->tunnelName);
         fprintf(fs, "      <td>%s</td>\n", L2tpAcIntf->lnsIpAddress);
...
void cgiL2tpAcWanCfgView_dev2(FILE *fs) 
{
   Dev2PppInterfaceL2tpObject *L2tpAcIntf = NULL;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   UBOOL8 isSet = FALSE;
   CmsRet ret;
   ...
   if ((ret = dalWan_getWanL2tpAcObject(&iidStack, (void **) &L2tpAcIntf)) != CMSRET_SUCCESS)
   {
	  cmsLog_debug("could not get L2tpAcIntf object");
   }
   else
   {
	  if (L2tpAcIntf->tunnelName)
	  {
		 ...
		 isSet = TRUE; /* yes, a L2tpAc wan interface is defined */
	  }
   ...
   /* For now, only allow one L2tpAc  interface */ 
   if (!isSet)
   {
	  fprintf(fs, "<input type='button' onClick='addClick()' value='Add'>\n");
   }
...
static CmsRet cgiL2tpAcWanRemove(char *query, FILE *fs) 
{
   ...
   while (pToken != NULL)
   {
      strcpy(glbWebVar.tunnelName, pToken);
   
      if ((ret = dalL2tpAc_deleteL2tpAcInterface(&glbWebVar)) == CMSRET_REQUEST_DENIED)
      {
      ...
      {
         cmsLog_error("dalL2tpAc_deleteL2tpAcWanInterface failed for failed for %s (ret=%d)", glbWebVar.wanL2IfName, ret);
         return ret;
      }      
...
void cgiL2tpAcWanCfg(char *query, FILE *fs) 
{
   ...
   {
      if (cgiL2tpAcWanRemove(query, fs) != CMSRET_SUCCESS)
      {
         return;
      }
   }

   /* for L2tpAc WAN Interface display */
   cgiL2tpAcWanCfgView(fs);
}
