...
extern CGI_GET_VAR ExtWebVarTable[];
extern CGI_GET_OBJ_VAR ExtWebObjVarTable[];
extern CGI_ITEM ExtCgiSetTable[];
extern CGI_ITEM ExtCgiGetTable[];
extern UINT32 numOfExtWebVar;
extern UINT32 numOfExtWebObj;
#else
CGI_GET_VAR *ExtWebVarTable = NULL;
CGI_GET_OBJ_VAR *ExtWebObjVarTable = NULL;
CGI_ITEM ExtCgiSetTable[] = {{NULL, NULL, CGI_TYPE_NONE}};
CGI_ITEM ExtCgiGetTable[] = {{NULL, NULL, CGI_TYPE_NONE}};
UINT32 numOfExtWebVar = 0;
UINT32 numOfExtWebObj = 0;
#endif
...
CGI_GET_VAR WebVarTable[] = {
...
#endif
#ifdef SUPPORT_IPSEC
   { "ipsHashList", cgiLoadIPSecHashList },
   { "ipsEncryptList", cgiLoadIPSecEncryptList },
#endif
#ifdef SUPPORT_TR69C
...
#ifdef SUPPORT_QOS
#ifdef SUPPORT_WAN_HTML
#if defined(SUPPORT_FAPCTL) || defined(SUPPORT_RDPA) || defined(SUPPORT_BCMTM) || defined(SUPPORT_ARCHERCTL)
   ...
   { "configuredWanEthIntf", cgiGetConfiguredEthWanIntf },
#endif
#ifdef DMP_DEVICE2_ETHLAG_1
   { "configuredWanEthForEthLag", cgiGetConfiguredEthWanForEthlag },
#endif
   ...
   { "uiWanUpStreamMode", cgiGetUiWanUpStreamMode },
#endif
#ifdef DMP_X_BROADCOM_COM_GPONWAN_1
   { "gponL2AvaIntf", cgiGetAvailableL2GponIntf },
   { "opticalLinkStatus", cgiGetOpticalLinkStatus },
   { "gponUpstreamFec", cgiGetGponUpstreamFec },
   { "gponDownstreamFec", cgiGetGponDownstreamFec },
#endif
...
#if defined(BCM_PON)
   { "wan_select_data", cgiWanSelectData},
#endif
};
...
void do_cgi(char *path, FILE *fs) {
   ...
   if ( ext != NULL ) {
      *ext = '\0';
      ...
      else if ( strstr(filename, "ntpdcfg.html") != NULL )
      {
...
void cgiGetVarOther(int argc, char **argv, char *varValue) {
   ...
   for (i=0 ; i < ARRAY_SIZE(WebVarTable); i++ )
   {
      ...
      {
          varTable = WebVarTable;
          break;
      }
   }

   if ((varTable == NULL) && (ExtWebVarTable != NULL))
   {
      for (i=0 ; i < numOfExtWebVar; i++ )
      {
         if (strcmp(argv[1], ExtWebVarTable[i].cgiGetName) == 0)
         {
             varTable = ExtWebVarTable;
             break;
         }
      }
   }
   ...
   (*(varTable[i].cgiGetHdlr))(argc, argv, varValue);
}
...
void cgiGetObjectVar(int argc, char **argv, char *varValue) {
   UINT32 i;
   CGI_GET_OBJ_VAR *objTable = NULL;
   
   if ( argc < 2) return;

   if (ExtWebObjVarTable != NULL)
   {
      for (i=0 ; i < numOfExtWebObj; i++ ) 
      {  
         if ( strcmp(argv[1], ExtWebObjVarTable[i].cgiGetObjectName) == 0 )
         {
            objTable = ExtWebObjVarTable;
            break;
         }
      }
   }
   ...
   (*(objTable[i].cgiGetObjectHdlr))(argc, argv, varValue);
}
...
CGI_ITEM CgiGetTable[] = {
   ...
   { "enblPppDemand", (void *)&glbWebVar.enblPppDemand, CGI_TYPE_NUM },
   { "pppIdleTime", (void *)&glbWebVar.pppIdleTimeLimit, CGI_TYPE_NUM },
   ...
   { "ipsConnNameList", (void *)NULL, CGI_TYPE_IPSEC_LIST },
   { "ipsSPIList", (void *)NULL, CGI_TYPE_SPI_LIST },
#endif
#if defined(DMP_TIME_1) || defined(DMP_DEVICE2_TIME_1)
   ...
   { "ntpdRefresh", (void *)&glbRefresh, CGI_TYPE_NUM },
#endif
#ifdef SUPPORT_CERT
   ...
   { "omciEthMeId5", &glbWebVar.omciEthMeId5, CGI_TYPE_NUM},
   { "omciEthMeId6", &glbWebVar.omciEthMeId6, CGI_TYPE_NUM},
   { "omciEthMeId7", &glbWebVar.omciEthMeId7, CGI_TYPE_NUM},
   { "omciEthMeId8", &glbWebVar.omciEthMeId8, CGI_TYPE_NUM},
   ...
   { "omciDbgMibEnable", &glbWebVar.omciDbgMibEnable, CGI_TYPE_BOOL},
   ...
   { "pmWlIfNum", (void *) &glbWebVar.pmWlIfNum, CGI_TYPE_NUM },
...
#ifdef SUPPORT_DSL
   { "enblDsl", (void *)&glbCgiTrue, CGI_TYPE_NUM },
   { "dslPhyDrvVersion", (void *)glbWebVar.dslPhyDrvVersion, CGI_TYPE_STR },
#else
   { "enblDsl", (void *)&glbCgiFalse, CGI_TYPE_NUM },
#endif
...
#ifdef DMP_DEVICE2_ETHLAG_1
   { "ethLagIfName", &glbWebVar.ethLagIfName, CGI_TYPE_STR},
   { "enbWanEthLag", (void *)&glbWebVar.enbWanEthLag, CGI_TYPE_BOOL },
   { "ethIfName1", &glbWebVar.ethIfName1, CGI_TYPE_STR},
   { "ethIfName2", &glbWebVar.ethIfName2, CGI_TYPE_STR},   
   { "ethLagMode", &glbWebVar.ethLagMode, CGI_TYPE_STR},
   { "xmitHashPolicy", &glbWebVar.xmitHashPolicy, CGI_TYPE_STR},
   { "lacp_rate", &glbWebVar.lacp_rate, CGI_TYPE_STR},
   { "miimon", &glbWebVar.miimon, CGI_TYPE_NUM},
#endif
...
void cgiGetVar(char *varName, char *varValue, FILE *fs __attribute__((unused))) {
   ...
   {
      i = 0;
      for ( ; ExtCgiGetTable[i].variable != NULL; i++ )
      {
         if ( strcmp(varName, ExtCgiGetTable[i].variable) == 0 )
         {
            varTable = ExtCgiGetTable;
            break;
         }
      }
   }
   ...
   if ( varTable != NULL ) {
      ...
      case CGI_TYPE_STR:
         strcpy(varValue, (char *)varTable[i].value);
         break;
      case CGI_TYPE_MARK_STR:
         ...
         break;
      case CGI_TYPE_BOOL:
         sprintf(varValue, "%d", *((UBOOL8 *)varTable[i].value));
         break;
      case CGI_TYPE_SHORT:
         sprintf(varValue, "%d", *((SINT16 *)varTable[i].value));
         break;
      case CGI_TYPE_NUM:
         sprintf(varValue, "%d", *((SINT32 *)varTable[i].value));
         break;
#ifdef SUPPORT_DSL
...
#if defined(DMP_DEVICE2_HOSTS_2) && defined(DMP_X_BROADCOM_COM_DEV2_IPV6_1)
         if ( glbWebVar.enblDhcpSrv == DHCP_SRV_ENABLE || glbWebVar.enblDhcp6sStateful )
#else
      ...
      case CGI_TYPE_CERT_LIST:
         ...
         break;
      case CGI_TYPE_IPSEC_LIST:
         cgiLoadIPSecList(varValue);
         break;
      case CGI_TYPE_SPI_LIST:
         cgiLoadSPIList(varValue);
         break;
 #endif
...
CGI_ITEM CgiSetTable[] = {
   ...
   { "omciEthMeId5", (void *)&glbWebVar.omciEthMeId5, CGI_TYPE_NUM},
   { "omciEthMeId6", (void *)&glbWebVar.omciEthMeId6, CGI_TYPE_NUM},
   { "omciEthMeId7", (void *)&glbWebVar.omciEthMeId7, CGI_TYPE_NUM},
   { "omciEthMeId8", (void *)&glbWebVar.omciEthMeId8, CGI_TYPE_NUM},
   ...
   { "omciDbgMibEnable", (void *)&glbWebVar.omciDbgMibEnable, CGI_TYPE_BOOL},
...
void cgiSetVar(char *varName, char *varValue) {
   ...
   {
      i = 0;
      for ( ; ExtCgiSetTable[i].variable != NULL; i++ )
      {
         if ( strcmp(varName, ExtCgiSetTable[i].variable) == 0 )
         {
            varTable = ExtCgiSetTable;
            break;
         }
      }
   }
   ...
   if ( varTable != NULL ) {
      switch ( varTable[i].type ) {
      case CGI_TYPE_STR:
         strcpy((char *)varTable[i].value, varValue);
         break;
      case CGI_TYPE_BOOL:
         *((UBOOL8 *)varTable[i].value) = atoi(varValue);
         break;
      case CGI_TYPE_SHORT:
         *((SINT16 *)varTable[i].value) = atoi(varValue);
         break;
      case CGI_TYPE_NUM:
         *((SINT32 *)varTable[i].value) = atoi(varValue);
         break;
      case CGI_TYPE_VPI:
      ...
      case CGI_TYPE_PORT:
         ...
         break;
      default:
         cmsLog_error("Cannot handle cgivar type %d", varTable[i].type);
         break;
      }
   }
   else {
