...
extern CGI_CMD ExtWebCmdTable[];
extern UINT32 numOfExtWebCmd;
#else
CGI_CMD *ExtWebCmdTable = NULL;
UINT32 numOfExtWebCmd = 0;
#endif

CGI_CMD WebCmdTable[] = {
   { "arpview", cgiArpView, LOGIN_USER},
   { "pmdbackupjson", cgiBackupPmdJson, LOGIN_SUPPORT},
   { "pmdbackupcalibration", cgiBackupPmdCalibration, LOGIN_SUPPORT},
   { "pmdbackuptemp2apd", cgiBackupPmdTemp2Apd, LOGIN_SUPPORT},
...
{ "ethLagCfg", cgiEthLagCfg, LOGIN_SUPPORT },
#endif
...
#endif
#if defined(BCM_PON)
   { "wan_select_cfg", cgiWanSelectCfg, LOGIN_SUPPORT},
#endif
};
...
void do_cmd_cgi(char *path, FILE *fs) {
   ...
   if ( ext != NULL ) {
      *ext = '\0';
      ...
      for (i=0; i < NUM_WEB_CMDS; i++ )
      {
         ...
         {
             cmdTable = WebCmdTable;
             break;
         }
      }

      if ((cmdTable == NULL) && (ExtWebCmdTable != NULL))
      {
         for (i=0; i < numOfExtWebCmd; i++ )
         {
            if (strstr(filename, ExtWebCmdTable[i].cgiCmdName))
            {
                cmdTable = ExtWebCmdTable;
                break;
            }
         }
      }
   ...
   else
   {
      ...
      (*(cmdTable[i].cgiCmdHdlr))(query, fs);
   }
}
         ...
         case CMD_TABLE:
            ...
            if ((cmdTable == NULL) && (ExtWebCmdTable != NULL))
            {
               for (i=0; i < numOfExtWebCmd; i++) 
               {
                  if (strstr(filename, ExtWebCmdTable[i].cgiCmdName))
                  {
                     cmsLog_debug("Found %s with authLevel %d", ExtWebCmdTable[i].cgiCmdName, WebCmdTable[i].authLevel);
                     cmdTable = ExtWebCmdTable;
                     extFlag = 1;
                     break;
                  }
            ...
            if (specialCaseHandle(cmdTable[i].cgiCmdName, query))
            {
...
UBOOL8 isPageAllowed(const char *path, HttpLoginType authLevel)
{
   ...
   if (strstr(path, ".cmd"))
   {
      ...
      if (i >= NUM_WEB_CMDS + numOfExtWebCmd) 
      {
         // Badly formatted post string or unrecognized page
...
//         cmsLog_debug("cmdName=%s, authLevel=%d, WebCmdTable[i].authLevel = %d", WebCmdTable[i].cgiCmdName, authLevel, WebCmdTable[i].authLevel);
         if (i < NUM_WEB_CMDS)
            ...
            allowed = (authLevel >= ExtWebCmdTable[i - NUM_WEB_CMDS].authLevel);     
      }
   }
