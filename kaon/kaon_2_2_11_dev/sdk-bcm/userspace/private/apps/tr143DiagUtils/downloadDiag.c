...
static void *conn_Fcn(void *data)
{
   ...
   if (strstr(proto, "http"))
      ret = httpDownloadDiag_r(connResult);
   else
      ret = ftpDownloadDiag_r(connResult);
...
int main (int argc, char *argv[])
{
   int ret, ch;
   int connections = -1;
   const char cleanUpCmd[1024] = "rm -f " TR143_DOWNLOAD_RESULT_FILE"*";
   srandom(time(NULL));
   ...
   while ((ch = getopt(argc, argv, "i:c:d:u:D:")) != -1)
      switch (ch) {
      case 'i':
   ...
   else
   {
      ...
      cleanup_and_notify(Complete, EID_DOWNLOAD_DIAG);
   }
   return 0;
}
