...
int archer_dpi_handler(POPTION_INFO pOptions, int nNumOptions)
{
    ...
    else if ( !strcmp( pOptions->pOptName, "--enable" ) )
    {
        ...
        return archer_dpi_mode_set(ARCHER_DPI_MODE_ENABLE);
    }
    else if ( !strcmp( pOptions->pOptName, "--sq" ) )
    {
        if ( pOptions->nNumParms )
        {
            fprintf( stderr, "%s: Too many parameters specified\n", g_PgmName );
        ...
        return archer_dpi_mode_set(ARCHER_DPI_MODE_SERVICE_QUEUE);
    }
    else if ( !strcmp( pOptions->pOptName, "--disable" ) )
    {
        ...
        return archer_dpi_mode_set(ARCHER_DPI_MODE_DISABLE);
    }
...
/*
 *------------------------------------------------------------------------------
 * Function Name: archer_sq_handler
 * Description  : Processes the ARCHER DPI SQ command.
 * Returns      : 0 - success, non-0 - error
 *------------------------------------------------------------------------------
 */
int archer_sq_handler(POPTION_INFO pOptions, int nNumOptions)
{
    if ( nNumOptions == 0 )
    {
        fprintf( stderr, "%s: No options specified\n", g_PgmName );
    ...
    else if ( !strcmp( pOptions->pOptName, "queue-set" ) )
    {
        ...
        {
            fprintf( stderr, "%s: queue-set expects 6 parameters (%d)\n",
        ...
        {
            if (!strcmp(pOptions->pParms[paramIndex], "--qid"))
            {
            ...
            {
                fprintf( stderr, "%s: invalid parameter for queue-set [%s]\n",
                         g_PgmName, pOptions->pParms[paramIndex] );
                return ARCHER_INVALID_OPTION;
            }
        }

        return archer_dpi_sq_queue_set(queue_index, min_kbps, max_kbps);
    }
    else if ( !strcmp( pOptions->pOptName, "queue-get" ) )
    {
        ...
        {
            fprintf( stderr, "%s: queue-get expects 2 parameters (%d)\n",
                     g_PgmName, pOptions->nNumParms );
            return ARCHER_INVALID_PARAMETER;
        }
        ...
        while (paramIndex < pOptions->nNumParms)
        {
            ...
            {
                fprintf( stderr, "%s: invalid parameter for queue-get [%s]\n",
                         g_PgmName, pOptions->pParms[paramIndex] );
                return ARCHER_INVALID_OPTION;
            }
        }

        ret = archer_dpi_sq_queue_get(queue_index, &min_kbps, &max_kbps);
        if(!ret)
        {
            fprintf(stderr, "Archer SQ: queue_index %u, min_kbps %d, max_kbps %d\n",
                    queue_index, min_kbps, max_kbps);
        }
    ...
    else if ( !strcmp( pOptions->pOptName, "port-set" ) )
    {
        ...
        if ( pOptions->nNumParms != 2 )
        {
            fprintf( stderr, "%s: port-set expects 2 parameters (%d)\n",
                     g_PgmName, pOptions->nNumParms );
            return ARCHER_INVALID_PARAMETER;
        }
        ...
        while (paramIndex < pOptions->nNumParms)
        {
            if (!strcmp(pOptions->pParms[paramIndex], "--kbps"))
            {
            ...
            {
                fprintf( stderr, "%s: invalid parameter for port-set [%s]\n",
                         g_PgmName, pOptions->pParms[paramIndex] );
                return ARCHER_INVALID_OPTION;
            }
        }

        return archer_dpi_sq_port_set(kbps);
    }
    else if ( !strcmp( pOptions->pOptName, "port-get" ) )
    {
        ...
        if ( pOptions->nNumParms != 0 )
        {
            fprintf( stderr, "%s: port-get expects 0 parameters (%d)\n",
                     g_PgmName, pOptions->nNumParms );
            return ARCHER_INVALID_PARAMETER;
        }

        ret = archer_dpi_sq_port_get(&kbps);
        if(!ret)
        {
            fprintf(stderr, "Archer SQ: kbps %d\n", kbps);
        }
    ...
    {
        int paramIndex = 0;
        archer_sq_arbiter_t arbiter = ARCHER_SQ_ARBITER_MAX;

        if ( pOptions->nNumParms != 1 )
        {
            fprintf( stderr, "%s: arbiter-set expects 1 parameter (%d)\n",
                     g_PgmName, pOptions->nNumParms );
            return ARCHER_INVALID_PARAMETER;
        }
        ...
        while (paramIndex < pOptions->nNumParms)
        {
            if (!strcmp(pOptions->pParms[paramIndex], "--sp"))
            {
                arbiter = ARCHER_SQ_ARBITER_SP;
                ++paramIndex;
            }
            else if (!strcmp(pOptions->pParms[paramIndex], "--rr"))
            {
            ...
            {
                fprintf( stderr, "%s: invalid parameter for arbiter-set [%s]\n",
                         g_PgmName, pOptions->pParms[paramIndex] );
                return ARCHER_INVALID_OPTION;
            }
        }

        return archer_dpi_sq_arbiter_set(arbiter);
    }
    else if ( !strcmp( pOptions->pOptName, "arbiter-get" ) )
    {
        ...
        if ( pOptions->nNumParms != 0 )
        {
            fprintf( stderr, "%s: arbiter-get expects 0 parameters (%d)\n",
                     g_PgmName, pOptions->nNumParms );
            return ARCHER_INVALID_PARAMETER;
        }

        ret = archer_dpi_sq_arbiter_get(&arbiter);
        if(!ret)
        {
            char *arbiter_name;
            ...
            switch(arbiter)
            {
                case ARCHER_SQ_ARBITER_SP:
                    arbiter_name = "SP";
                    break;

                case ARCHER_SQ_ARBITER_RR:
                    arbiter_name = "RR";
                    break;
            ...
            fprintf(stderr, "Archer SQ: arbiter %s\n", arbiter_name);
        }
...
COMMAND_INFO g_Cmds[] =
{
    ...
    {"dpi",               {"--enable", "--sq", "--disable", "--stats", ""}, archer_dpi_handler},
    {"sq",                {"stats",
                           "queue-set", "queue-get",
                           "port-set", "port-get",
                           "arbiter-set", "arbiter-get", ""}, archer_sq_handler},
...
void archer_help(void)
{
    printf(
        ...
        "     archerctl dpi         [ --enable, --sq, --disable, --stats ]\n"
        "     archerctl sq [ stats ]\n"
        "                  [ queue-set --qid <queue_index> --min_kbps <kbps> --max_kbps <kbps> ]\n"
        "                  [ queue-get --qid <queue_index> ]\n"
        "                  [ port-set --kbps <kbps> ]\n"
        "                  [ port-get ]\n"
        "                  [ arbiter-set [ --sp | --rr ] ]\n"
        "                  [ arbiter-get ]\n"
