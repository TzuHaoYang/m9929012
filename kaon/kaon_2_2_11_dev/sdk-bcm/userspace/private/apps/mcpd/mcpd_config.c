...
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST 
        mcpd_igmp_upstream_interface_root_index[i] = 0;
#endif
        mcpd_igmp_mcast_interface[i][0] = '\0';
#ifdef SUPPORT_MLD
        mcpd_mld_upstream_interface[i][0] = '\0';
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST      
        mcpd_mld_upstream_interface_root_index[i] = 0;
#endif
    ...
    while(fgets(buffer, sizeof(buffer), fp))
    {
        ...
        *line = '\0';
        ...
        else if(strcasecmp(token, "igmp-proxy-interfaces") == 0)
        {
            ...
            {
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST	
                strncpy(mcpd_igmp_upstream_interface[i], ptr1, 2*IFNAMSIZ+1);
#else
        ...
        else if(strcasecmp(token, "igmp-mcast-interfaces") == 0)
        {
            ...
            {
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST
                strncpy(mcpd_igmp_mcast_interface[i], ptr1, 2*IFNAMSIZ+1);
#else
        ...
        else if(strcasecmp(token, "mld-proxy-interfaces") == 0)
        {
            ...
            {
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST 
                strncpy(mcpd_mld_upstream_interface[i], ptr1, IFNAMSIZ*2+1);
#else
        ...
        else if(strcasecmp(token, "mld-mcast-interfaces") == 0)
        {
            ...
            {
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST 
                strncpy(mcpd_mld_mcast_interface[i], ptr1, IFNAMSIZ*2+1);
#else
