                                         ...
                                         int if_type)
{
    ...
    {
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST
        mcpd_interface_update(ifp, addr, addr6, name, index, root_index, if_type);
#else
...
static void mcpd_send_ovs_remove_snoop_entry_msg(t_MCPD_PROTO_TYPE proto,
                                                 int repIfIdx,
                                                 int brIfIdx,
                                                 UINT8 *grpaddr,
                                                 UINT8 *srcaddr,
                                                 UINT16 lan_tci)
{
    if ( proto == MCPD_PROTO_IGMP )
    {
        ...
        grpip = ((struct in_addr *)grpaddr)->s_addr;

        if ( srcaddr )
        {
            srcip = ((struct in_addr *)srcaddr)->s_addr;
        }
        mcpd_ovs_manage_snoop_entry(repIfIdx,
                                    brIfIdx,
                                    grpip, /* Grp IPv4*/
                                    NULL,
                                    srcip, /* Src IPv4*/
...
/* Check if this is the last reporter on the interface for this group/source */
int mcpd_group_src_is_last_rep_on_if(t_MCPD_PROTO_TYPE proto,
                                     t_MCPD_SRC_OBJ* src,
                                     t_MCPD_REP_OBJ* rep)
{
    t_MCPD_SRC_REP_OBJ *srep = NULL;
    ...
    if(!src)
    {
        MCPD_TRACE(MCPD_TRC_ERR, "NULL src");
        return 0;
    }

    /* There are no other reporter objects */
    if(!src->srep)
    {
        MCPD_TRACE(MCPD_TRC_LOG, "NULL reporter objects on src\n");
        return 1;
    }
                      ...
                      srep = (t_MCPD_SRC_REP_OBJ *)srep->next)
    {
        if ( srep->rep->rep_ifi == rep->rep_ifi )
        {
           ...
           if (MCPD_RET_OK != ret)
           {
               MCPD_TRACE(MCPD_TRC_LOG, "found another reporter for this group/source");
               return 0;
           }
        }
    }

    MCPD_TRACE(MCPD_TRC_LOG, "no other reporter found for this group/source");
    return 1;
} /* mcpd_group_src_is_last_rep_on_if */
...
#if defined(CONFIG_BCM_OVS_MCAST)
        /* Check if there are any other reporters for this source on the same interface.
           If there are none, send remove snoop entry message for this Group/Source
           combination to OVS */
        if (mcpd_group_src_is_last_rep_on_if(proto, src, rep))
        {        
            gp = rep->parent_group;
            mcpd_send_ovs_remove_snoop_entry_msg(proto,
                                                 rep->rep_ifi,
                                                 gp->ifp->if_index,
                                                 gp->addr,
                                                 rsrc->src->addr,
