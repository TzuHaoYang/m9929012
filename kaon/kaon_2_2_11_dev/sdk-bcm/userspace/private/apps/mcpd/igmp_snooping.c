...
{
    return MCPD_RET_OK;
} /* mcpd_igmp_snooping_init */

static void mcpd_igmp_set_wan_info(t_MCPD_INTERFACE_OBJ *ifp_in, t_MCPD_WAN_INFO_ARRAY wan_info)
{
   ...
   {
       if ((ifp->if_dir == MCPD_UPSTREAM) && (ifp->proto_enable & MCPD_IPV4_MCAST_ENABLE))
       {
                                               ...
                                               t_MCPD_PKT_INFO *pkt_info)
{
    ...
    if (!(ifp_in->proto_enable & MCPD_IGMP_SNOOPING_ENABLE))
    {
    ...
    mcpd_igmp_set_wan_info(ifp_in, wan_info);

    if ( bcm_mcast_api_update_igmp_snoop(mcpd_router.sock_nl, 
                                         pkt_info->parent_ifi, 
                                         pkt_info->rxdev_ifi,
                                         pkt_info->to_acceldev_ifi,
                                         pkt_info->tci,
                                         pkt_info->lanppp, 
                                         (const struct in_addr *)gp->addr, 
                                         (const struct in_addr *)gp->addr, 
                                         (const struct in_addr *)src, 
                                         (const struct in_addr *)rep->addr, 
...
#if defined(CONFIG_BCM_OVS_MCAST)
    {
        ...
        mcpd_ovs_manage_snoop_entry(pkt_info->rxdev_ifi,
                                    pkt_info->parent_ifi, 
                                    ((const struct in_addr *)gp->addr)->s_addr, /* v4 Group address */
                                    NULL, /* v6 Group address N/A */
                                    ((const struct in_addr *)src)->s_addr, /* v4 Src address */
                                    NULL, /* v6 Src address N/A */
                                               ...
                                               t_MCPD_FLOOD_TYPE type)
{
    ...
    if (mcpd_get_bridge_members(gp->ifp->if_name, ifInBridge, &portNum) != 0 ||
        portNum == 0)
    {
        MCPD_TRACE(MCPD_TRC_ERR, "Get bridge member error");
        return MCPD_RET_GENERR;
    }
    ...
    for (idx = 0; idx < portNum; idx++)
    {
        ...
        MCPD_TRACE(MCPD_TRC_INFO, "updating igmp flooding info for %s,action=%d\n", ifname, mode);
