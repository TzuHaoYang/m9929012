...
extern char mcpd_igmp_upstream_interface[MCPD_MAX_IFS][IFNAMSIZ * 2 + 1];
extern char mcpd_igmp_mcast_interface[MCPD_MAX_IFS][IFNAMSIZ * 2 + 1];
extern unsigned int mcpd_igmp_upstream_interface_root_index[MCPD_MAX_IFS];
#else
...
#ifdef SUPPORT_MLD
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST
extern char mcpd_mld_upstream_interface[MCPD_MAX_IFS][IFNAMSIZ * 2 + 1];
extern char mcpd_mld_mcast_interface[MCPD_MAX_IFS][IFNAMSIZ * 2 + 1];
extern unsigned int mcpd_mld_upstream_interface_root_index[MCPD_MAX_IFS];
#else
...
int mcpd_mcast_interface_lookup(char *s, t_MCPD_PROTO_TYPE proto)
{
    ...
    for (interface = 0; interface < MCPD_MAX_IFS; interface++)
    {
        if ( proto != MCPD_PROTO_MLD )
        {
            ...
            if (strstr(s, "/"))
            {
                if(0 == strncmp(s, mcpd_igmp_mcast_interface[interface], IFNAMSIZ*2+1))
                {
                    return MCPD_TRUE;
                }
            }
            ...
            else
            {
                sprintf(name, "%s/", s);
                len = strlen(name);
                if(0 == strncmp(name, mcpd_igmp_mcast_interface[interface], len))    /* proxy */
                {
                    return MCPD_TRUE;
                }
                else   /* snooping */
                {
                    sprintf(name, "/%s", s);
                    len = strlen(name);
                    pIf = strstr(mcpd_igmp_mcast_interface[interface], name);
                    if(pIf && (len == strlen(pIf)))
                        return MCPD_TRUE;
                }
            }
#endif
        ...
        if ( proto != MCPD_PROTO_IGMP )
        {
            ...
            if (strstr(s, "/"))
            {
                if (0 == strncmp(s, mcpd_mld_mcast_interface[interface], IFNAMSIZ*2+1))
                {
                    return MCPD_TRUE;
                }
            }
            ...
            else
            {
                sprintf(name, "%s/", s);
                len=strlen(name);
                if(0 == strncmp(name, mcpd_mld_mcast_interface[interface], len))
                {
                    return MCPD_TRUE;
                }
                else
                {
                    sprintf(name, "/%s", s);
                    len=strlen(name);
                    pIf = strstr(mcpd_mld_mcast_interface[interface], name);
                    if(pIf && (len == strlen(pIf)))
                    {
                        return MCPD_TRUE;
                    }
...
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST
int mcpd_get_index_by_name(char *ifName, unsigned int *ifIndex) 
{
    unsigned int index;
    int sockfd;
    ...
    if (mcpd_ifname_to_index(sockfd, &index, ifName) < 0 )
    {
        return MCPD_FALSE;
    }
    ...
    *ifIndex = index;
    return MCPD_TRUE;
} /* mcpd_get_index_by_name */

int mcpd_upstream_interface_lookup_root_index(char *s, t_MCPD_PROTO_TYPE proto, unsigned int *index)
{
    int interface;
    char name[IFNAMSIZ+1]; 
    int len;
    ...
    for (interface = 0; interface < MCPD_MAX_IFS; interface++)
    {
        if ( proto != MCPD_PROTO_MLD )
        {
            sprintf(name, "%s/", s);
            len = strlen(name);
            if(0 == strncmp(name, mcpd_igmp_upstream_interface[interface], len))
            {
                *index = mcpd_igmp_upstream_interface_root_index[interface];
                return MCPD_TRUE;
            }
        }
#ifdef SUPPORT_MLD
        if ( proto != MCPD_PROTO_IGMP )
        {
            sprintf(name, "%s/", s);
            len = strlen(name);
            if(0 == strncmp(name, mcpd_mld_upstream_interface[interface], len))
            {
                *index = mcpd_mld_upstream_interface_root_index[interface];
                return MCPD_TRUE;
            }
        }
#endif
    ...
    return MCPD_FALSE;
} /* mcpd_upstream_interface_lookup_root_index */
#endif /* SUPPORT_OVS_BRIDGE_WAN_MCAST */

int mcpd_upstream_interface_lookup(char *s, t_MCPD_PROTO_TYPE proto)
{
    int interface;
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST
    char name[IFNAMSIZ*2+1];
    int len;
    char *rootName;
    unsigned int index;
#endif
    ...
    for (interface = 0; interface < MCPD_MAX_IFS; interface++)
    {
        if ( proto != MCPD_PROTO_MLD )
        {
            ...
            else
            {
                ...
                if(0 == strncmp(name, mcpd_igmp_upstream_interface[interface], len))
                {
                    strncpy(name, mcpd_igmp_upstream_interface[interface], IFNAMSIZ*2+1);
                    strtok(name, "/");
                    rootName = strtok(NULL, "/");
                    if(MCPD_TRUE == mcpd_get_index_by_name(rootName, &index))
                        mcpd_igmp_upstream_interface_root_index[interface] = index;
                    else 
                        mcpd_igmp_upstream_interface_root_index[interface] = 0;
        ...
        if ( proto != MCPD_PROTO_IGMP )
        {
            ...
            else
            {
                ...
                if(0 == strncmp(name, mcpd_mld_upstream_interface[interface], len))
                {
                    strncpy(name, mcpd_mld_upstream_interface[interface], IFNAMSIZ*2+1); 
                    strtok(name, "/");
                    rootName = strtok(NULL, "/");
                    if(MCPD_TRUE == mcpd_get_index_by_name(rootName, &index))
                        mcpd_mld_upstream_interface_root_index[interface] = index;
                    else
                        mcpd_mld_upstream_interface_root_index[interface] = 0;
...
t_MCPD_IFINFO_OBJ *mcpd_get_ifinfo(short flags, short unflags)
{
    ...
    while ( fgets(line, sizeof(line), fh) ) 
    {
        ...
        {
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST
            if(!mcpd_mcast_interface_lookup(p, MCPD_PROTO_MAX))
#endif
            continue;
        }
...
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST
              if(MCPD_TRUE == mcpd_upstream_interface_lookup_root_index(p, MCPD_PROTO_MAX, &rootIndex))
                  haveRootIndex = 1;
#endif 
        ...
        {
#ifndef SUPPORT_OVS_BRIDGE_WAN_MCAST
           ...
           {
               wan_bridge_if = mcpd_bridge_includes_wan_interface(p);
               if( MCPD_FALSE == wan_bridge_if )
               {
                   wan_bridge_if = mcpd_mcast_interface_lookup(p, MCPD_PROTO_MAX);
               }
           }
#endif
        }
...
int mcpd_is_wan_interface(char *ifname)
{
    ...
    {
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST
        if(mcpd_upstream_interface_lookup(ifname, MCPD_PROTO_MAX) == MCPD_TRUE)
            return MCPD_TRUE;
#endif
        return MCPD_FALSE;
    }
...
#ifdef SUPPORT_OVS_BRIDGE_WAN_MCAST   //for bridge
   if ( mcpd_ifindex_to_name(mcpd_router.sock_igmp, ifIndex, ifName) )
   {
      sprintf(mcastName,"%s/%s", brName, ifName);
      if( mcpd_mcast_interface_lookup(mcastName, MCPD_PROTO_MAX) == MCPD_TRUE )
         return MCPD_TRUE;
    }
#endif
