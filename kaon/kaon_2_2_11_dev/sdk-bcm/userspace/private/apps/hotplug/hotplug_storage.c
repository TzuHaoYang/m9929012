...
#define DEVNAME_MAX_LEN   8

extern void *g_msgHandle;
void notify_smd(int disk, int partition, char *devname, int msgType)
{
   CmsMsgHeader *msg;
   char *data;
   void *msgBuf;
   ...
   msgDataLen =  sizeof(disk) + sizeof(partition) + DEVNAME_MAX_LEN;
   ...
   if (msgBuf != NULL) {
      ...
      *((int *)data + 1) = partition;
      ptr = (char *)((int *)data + 2);
      strncpy(ptr, devname, DEVNAME_MAX_LEN-1);
...
int hotplug_storage_handler(void)
{
    ...
    if(!strcmp(action,"add"))  
    {
        ...
        if(retv == 0)
        {
            ...
            notify_smd(disk,partition,devname,CMS_MSG_STORAGE_ADD_LOGICAL_VOLUME);
        }
#endif
    }
    else if (!strcmp(action,"remove"))
    {
        ...
        notify_smd(disk,partition,devname,CMS_MSG_STORAGE_REMOVE_LOGICAL_VOLUME);
#endif
