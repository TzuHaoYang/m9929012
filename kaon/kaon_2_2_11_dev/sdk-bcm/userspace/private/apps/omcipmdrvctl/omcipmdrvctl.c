 ...
 */
#ifdef BUILD_STATIC
int omcipmdrvctl_main(int argc, char **argv)
#else
...
#endif
{
    ...
    strncpy( g_PgmName, *argv, MAX_PGM_NAME_LEN - 1 );
    if ( argc == 1 )
    {
        usage( );
        exit( nExitCode );
    ...
    while ( argc && nExitCode == 0 )
    {
        ...
        {
            nExitCode = OMCIPMDRVCTL_INVALID_COMMAND;
...
{
    int nRet = OMCIPMDRVCTL_SUCCESS, nNumOptInfo = 0, nArgType = 0;
    ...
    *pnArgNext = 0;
    ...
    do
    {
        ...
        case ARG_TYPE_OPTION:
            ...
            {
                nRet = OMCIPMDRVCTL_INVALID_NUMBER_OF_OPTIONS;
        ...
        case ARG_TYPE_PARAMETER:
            ...
            else
            {
                if ( pCurrOpt )
                {
                    nRet = OMCIPMDRVCTL_INVALID_OPTION;
    ...
    } while ( (nRet == OMCIPMDRVCTL_SUCCESS) && (nArgType!=ARG_TYPE_COMMAND) );

    if ( nRet == OMCIPMDRVCTL_SUCCESS )
