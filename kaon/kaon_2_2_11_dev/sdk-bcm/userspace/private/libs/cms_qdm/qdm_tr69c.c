...
CmsRet qdmTr69c_getManagementServerXmppCfgLocked_igd(char *connectionPath, char *allowedJabberIds, char *connReqJabberId)
{
   ...
   {
      cmsUtl_strcpy(connectionPath, obj->connReqXMPPConnection);
      cmsUtl_strcpy(allowedJabberIds,obj->connReqAllowedJabberIDs);
      cmsUtl_strcpy(connReqJabberId,obj->connReqJabberID);
      cmsObj_free((void **) &obj);
   }
   return ret;   
}
