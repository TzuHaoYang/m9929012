...
UBOOL8 qdmQos_isQosEnabled_dev2(void)
{
   ...
   if ((ret = cmsObj_get(MDMOID_DEV2_QOS, &iidStack, 0, (void **) &qosObj)) != CMSRET_SUCCESS)
   {
      cmsLog_error("cmsObj_get MDMOID_DEV2_QOS  returns error. ret=%d", ret);
   }
   else
   {
      enabled=qosObj->X_BROADCOM_COM_Enable;
                                             ...
                                             CmsQosPolicerInfo *pInfo)
{
   ...
   pInfo->usPolicerInfo = pObj->X_BROADCOM_COM_UsPolicerInfo;
   pInfo->dsPolicerInfo = pObj->X_BROADCOM_COM_DsPolicerInfo;
...
UBOOL8 qdmQos_isLanSwitchPortToLanSwitchPortClassificationPresentLocked_dev2(UINT32 excludeClassKey)
{
                    ...
                    OGF_NO_VALUE_UPDATE, (void **)&cObj) == CMSRET_SUCCESS)
   {
          ...
          (cObj->X_BROADCOM_COM_ClassQueue > 0))
      {
         ...
         else if (!cmsUtl_strcmp(cObj->interface, MDMVS_LOCAL))
         {
            strcpy(ingressIntfNameBuf, MDMVS_LOCAL);
            ingressUpstream = FALSE;
         }
         else if (!cmsUtl_strcmp(cObj->interface, MDMVS_WAN))
         {
            strcpy(ingressIntfNameBuf, MDMVS_WAN);
            ingressUpstream = TRUE;
         }
         else
         {
