   ...
   *isAtm = TRUE;
...

   {
      if (dslLineObj->enable && !cmsUtl_strcmp(dslLineObj->status, MDMVS_UP))
      {
         ...
         lineNumber = dslLineObj->X_BROADCOM_COM_BondingLineNumber;
      }
      cmsObj_free((void **) &dslLineObj);
   }
   ...
   if (!found)
   {
      /* is DSL Line is not up, try FAST Line */
      ...
      {
         if (fastLineObj->enable && !cmsUtl_strcmp(fastLineObj->status, MDMVS_UP))
         {
            found = TRUE;
            *isVdsl = TRUE;
            lineNumber = fastLineObj->X_BROADCOM_COM_BondingLineNumber;
         }
...
UBOOL8 qdmDsl_isXdslLinkUpLocked_dev2()
{
   ...
   INIT_INSTANCE_ID_STACK(&iidStack);
   while (cmsObj_getNext(MDMOID_DEV2_FAST_LINE, &iidStack, (void **)&fastLineObj) == CMSRET_SUCCESS)
   {
      if (fastLineObj->enable && !cmsUtl_strcmp(fastLineObj->status, MDMVS_UP))
      {
         cmsObj_free((void **) &fastLineObj);
