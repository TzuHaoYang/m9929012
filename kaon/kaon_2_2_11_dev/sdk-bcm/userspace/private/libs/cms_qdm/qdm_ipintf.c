...
CmsRet qdmIpIntf_getDefaultLanIntfNameLocked_igd(char *ifName)
{
   ...
   while ((found == FALSE) &&
          (ret = cmsObj_getNext(MDMOID_LAN_IP_INTF, &iidStack,
                                (void **) &ipIntfObj)) == CMSRET_SUCCESS)
   {
      cmsLog_debug("Found %s IPAddress=%s",
                   ipIntfObj->X_BROADCOM_COM_IfName,
                   ipIntfObj->IPInterfaceIPAddress);

      found = TRUE;
      sprintf(ifName, "%s", ipIntfObj->X_BROADCOM_COM_IfName);
                                                ...
                                                char *ipAddress)
{
   ...
   if (pppConnObj)
   {
      ...
      if (!found &&
          ...
          !IS_EMPTY_STRING(pppConnObj->externalIPAddress))
      {
   ...
   if (ipConnObj)
   {
      ...
      if (!found &&
          ...
          !IS_EMPTY_STRING(ipConnObj->externalIPAddress))
      {
...
/* 
 * This function is currently used by MAP-T (or MAP-E) for hybrid data model.
                                                  ...
                                                  UBOOL8 *isStatic)
{
   ...
   else
   {
       ...
       * called by non-MAP-T (or non-MAP-E) purpose.
       */
      ret = CMSRET_INVALID_ARGUMENTS;
   }
