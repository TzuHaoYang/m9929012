   ...
   if (layer2)
   {
     ...
     INIT_INSTANCE_ID_STACK(&iidStack);
      ...
      INIT_INSTANCE_ID_STACK(&iidStack);
      {
          char wlifname[8] = {0};
          int wl_index = 0;
          if(!found && sscanf(intfName,"wds%d",&wl_index) == 1) {
          ...
          {
              found = (0 == cmsUtl_strcmp(((Dev2WifiSsidObject *)mdmObj)->name, wlifname[0]?wlifname:intfName));
              if (found)
              {
     ...
     INIT_INSTANCE_ID_STACK(&iidStack);
             ...
             ((ret = cmsObj_getNextFlags(MDMOID_DEV2_USB_INTERFACE, &iidStack, OGF_NO_VALUE_UPDATE, &mdmObj)) == CMSRET_SUCCESS))
      {
         ...
         cmsObj_free(&mdmObj);
           ...
           INIT_INSTANCE_ID_STACK(&iidStack);
            while ((!found) &&
                   ((ret = cmsObj_getNextFlags(MDMOID_LA_G, &iidStack, OGF_NO_VALUE_UPDATE, &mdmObj)) == CMSRET_SUCCESS))
            {
               found = (0 == cmsUtl_strcmp(((Dev2EthLAGObject*)mdmObj)->name, intfName));
                if (found)
               {
                  pathDesc.oid = MDMOID_LA_G;
                  pathDesc.iidStack = iidStack;
               }          
               cmsObj_free(&mdmObj);
            }
                                                  ...
                                                  char *intfname, UINT32 intfnameLen)
{
   ...
   ret = cmsPhl_getParamValueFlags(&pathDesc, OGF_NO_VALUE_UPDATE, &pParamValue);
   if (ret != CMSRET_SUCCESS)
   {
...
#endif
#ifdef DMP_DEVICE2_ETHLAG_1
       pathDesc.oid == MDMOID_LA_G ||            /* EthLAG is level 2 */
#endif
...
#ifdef DMP_DEVICE2_ETHLAG_1 
   else if (pathDesc.oid == MDMOID_LA_G)
   {
      Dev2EthLAGObject *ethLagObj=NULL;

      ret = cmsObj_get(MDMOID_LA_G, &pathDesc.iidStack,
                       OGF_NO_VALUE_UPDATE, (void **)&ethLagObj);
      if (ret != CMSRET_SUCCESS)
      {
         cmsLog_error("Could not get EthLAG obj at %s, ret=%d", l2FullPath, ret);
      }
      else
      {
         isUpstream = ethLagObj->X_BROADCOM_COM_Upstream;
                                           ...
                                           UBOOL8 *found, MdmObjectId oid)
{
   ...
   ret = cmsPhl_getParamValueFlags(&pathDescLower, OGF_NO_VALUE_UPDATE, &pParamValue);
...
static UINT32 getAllIntfNames(char *ifNamesBuf, UINT32 len,
                              UBOOL8 layer2 __attribute__((unused)),
                              UBOOL8 upstream __attribute__((unused)))
{
...
#ifdef DMP_DEVICE2_ETHERNETINTERFACE_1
   {
      ...
      {
         if (ethIntfObj->name != NULL &&
             ethIntfObj->upstream == upstream)
         {
            ADD_COMMA_SEP_STRING_TO_BUF(ifNamesBuf, ethIntfObj->name);
         }
         cmsObj_free((void **) &ethIntfObj);
      }
   }
...
#ifdef DMP_DEVICE2_WIFISSID_1
   {
      InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK;
      Dev2WifiSsidObject *ssidObj=NULL;
      while ((cmsObj_getNextFlags(MDMOID_DEV2_WIFI_SSID,
                                  &iidStack, OGF_NO_VALUE_UPDATE,
                                  (void **) &ssidObj)) == CMSRET_SUCCESS)
      {
         if (ssidObj->name != NULL &&
             ssidObj->enable &&
             ssidObj->X_BROADCOM_COM_Upstream == upstream)
         {
            ADD_COMMA_SEP_STRING_TO_BUF(ifNamesBuf, ssidObj->name);
         }
         cmsObj_free((void **) &ssidObj);
      }
   }
#endif /* DMP_DEVICE2_WIFISSID_1  */
   ...
   if (layer2 && upstream)
   {
      ...
      while ((cmsObj_getNextFlags(MDMOID_DEV2_PTM_LINK,
                                  &iidStack, OGF_NO_VALUE_UPDATE,
                                  (void **) &ptmLinkObj)) == CMSRET_SUCCESS)
      {
         if (ptmLinkObj->name != NULL)
         {
            ADD_COMMA_SEP_STRING_TO_BUF(ifNamesBuf, ptmLinkObj->name);
         }
   ...
   // TODO: there are other types of interfaces to cover, but this is good
   // enough for now.  Fill in as needed later.
   return count;
}
...
{
   // Layer2, not upstream.
   return (getAllIntfNames(ifNamesBuf, len, TRUE, FALSE));
}

UINT32 qdmIntf_getAllWanL2IntfNames_dev2(char *ifNamesBuf, UINT32 len)
{
   // Layer 2, Upstream.
   return (getAllIntfNames(ifNamesBuf, len, TRUE, TRUE));
}
...
CmsRet qdmIntf_getBridgeNameByIntfName_dev2(const char *intfname, char *bridgeifcName)
{
   ...
   {
       /* find the matched Interface and get the bridge interface name */
       while (!found &&
              (ret = cmsObj_getNext(MDMOID_DEV2_IP_INTERFACE, &iidStack,
                                   (void **)&ipIntfObj)) == CMSRET_SUCCESS)
       {
          ...
          {
             found = TRUE;
             if(ipIntfObj->X_BROADCOM_COM_BridgeName)
             {
                 strcpy(bridgeifcName, ipIntfObj->X_BROADCOM_COM_BridgeName);
                 ret = CMSRET_SUCCESS;
             }
          }
          cmsObj_free((void **) &ipIntfObj);
