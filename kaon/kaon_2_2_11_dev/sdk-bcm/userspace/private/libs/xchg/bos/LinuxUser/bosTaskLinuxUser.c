...
BOS_STATUS bosTaskProceedToShutdown( BOS_TASK_ID *taskid )
{
   ...
   while (counter--)
   {
      status = pthread_cancel( *(pthread_t*) taskid);
      if (status)
      {
         bosLogInfo("Canceled task %d failed. Iteration %d", *(pthread_t*) taskid, counter );
         bosSleep(100);
      }
      else
         break;
   ...
   bosLogInfo("Canceled task %d", *(pthread_t*) taskid);

   return status ? BOS_STATUS_ERR : BOS_STATUS_OK;
}
/***************************************************************************/
BOS_STATUS bosTaskWaitForDone( BOS_TASK_ID *taskid )
{
   int counter, status;
   counter =3;
   bosLogInfo("Wait for task %d", *(pthread_t*) taskid);
   while (counter--)
   {
      status = pthread_join( *(pthread_t*) taskid, NULL);      
      if (status)
      {
         bosLogInfo("Wait for task %d failed. Iteration %d", *(pthread_t*) taskid, counter );
         bosSleep(100);
      }
      else
         break;
