...
static int _bcm_ipc_socket_send(bcmIpcMsgQueue_t *queue, UINT8 *buf, UINT32 buf_length)
{
    ...
    return (rc == (int)buf_length) ? 0 : -ENOENT;
}
 ...
 */
static int _bcm_ipc_transparent_unpack(bcmIpcMsgQueue_t *queue __attribute__((unused)),
  UINT8 *buf, UINT32 buf_length, bcmIpcMsg_t **msg)
{
    bcmIpcMsg_t *m;
    int i;
    ...
    for(i = 0; i< (int)sizeof(bcmIpcMsg_t); i++)
    {
    ...
    if (m->size != buf_length - sizeof(bcmIpcMsg_t))
    {
        bcmIpcApi_error("Received message is insane. Expected data length %u, got %u\n",
            m->size, buf_length - (UINT32)sizeof(bcmIpcMsg_t));
        free(m);
        return -EINVAL;
    }
...
static int bcm_ipc_msg_queue_udp_socket_open(bcmIpcMsgQueue_t *queue)
{
    ...
    int rc;
#ifdef CC_BCM_IPC_API_DEBUG
    UINT8 *p;
#endif /* CC_BCM_IPC_API_DEBUG */
    ...
    if (queue->q.parm.remote_ep_address)
    {
        ...
        for (rc = 0; rc < (int)queue->ep_extra_data_size; rc++)
