...
static void mocaos_write32(MoCAOS_Handle handle, unsigned int addr, unsigned int value, unsigned int mask)
{
    int rc;
    unsigned int x;
    char cli[64];

    addr -= 0x10600000; // subtract MoCA base addr because bmoca adds it back in

    rc = MoCAOS_ReadMem(handle, (unsigned char *)&x, 4, addr);
    if (rc) goto noioctl;
    ...
    rc = MoCAOS_WriteMem(handle, (unsigned char *)&x, 4, addr);
    if (rc) goto noioctl;
...
noioctl:
    // fallback
    addr += 0x10600000;
    sprintf(cli, "bbsictl 0x%X 0x%X 0x%X", addr, value, mask);
    system(cli);
}
...
int MoCAOS_WolCtrl(MoCAOS_Handle handle, int enable)
{
    ...
    if (ioctl(ctx->device_fd, MOCA_IOCTL_WOL, enable) < 0)
        return(errno);
...
void MoCAOS_EnableDataIf(MoCAOS_Handle handle, char *ifname, int enable)
{
    ...
    else if (enable == 2) // disable interface
    {
       ...
       if ((ctx->kdrv_info.chip_id & 0xFFFE0000) == 0x68020000)
       {
          ...
          mocaos_write32(handle, 0x10800808, 0x2000, 0x2004);

          // Flush TX FIFO - set bit
          mocaos_write32(handle, 0x10800b34, 0x1, 0x1);
       }
#endif
    }
    else // enable interface
    {
       ...
       if ((ctx->kdrv_info.chip_id & 0xFFFE0000) == 0x68020000)
       {
          ...
          mocaos_write32(handle, 0x10800808, 0x3, 0x2003);

          // Flush TX FIFO - clear bit
          mocaos_write32(handle, 0x10800b34, 0x0, 0x1);
       }
#
#endif
    }
...
int MoCAOS_SetLedMode(MoCAOS_Handle handle, uint32_t led_mode)
{
   ...
   {
      // This is a 7xxx chip with a 680x chip connected via SPI
       mocaos_write32(handle, 0x10800024, led_mode==1?0:0x30, 0x30);
   }
#endif
...
int MoCAOS_SetLoopback(MoCAOS_Handle handle, uint32_t enable, uint32_t device_id)
{
   ...
   {
      // This is a 7xxx chip with a 680x chip connected via SPI
      mocaos_write32(handle, 0x10800808, enable? 0x8000:0, 0x8000);
   }
#endif
   return 0;
}
