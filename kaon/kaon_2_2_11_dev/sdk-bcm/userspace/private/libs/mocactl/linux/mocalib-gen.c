...
const struct moca_ie_names moca_ie_names[] = {
   ...
   {IE_HW_AGC_ENABLE_P2P, "HW_AGC_ENABLE_P2P"},
   ...
   {IE_UNKNOWN_RATELIMIT_EN, "UNKNOWN_RATELIMIT_EN"},
   ...
   // Group network
   {IE_NETWORK_STATE, "NETWORK_STATE"},
...
{
   *no_ifg6 = MOCA_NO_IFG6_DEF;
}

MOCALIB_GEN_SET_FUNCTION void moca_set_bonding_defaults(uint32_t *bonding, uint32_t flags)
{
   if ((flags & MOCA_BAND_D_LOW_FLAG)) {
      *bonding = MOCA_BONDING_BAND_D_LOW_DEF;
   }
   else if ((flags & MOCA_3390B0_FLAG)) {
      *bonding = MOCA_BONDING_3390B0_DEF;
   }
...
MOCALIB_GEN_SET_FUNCTION void moca_set_hw_agc_enable_p2p_defaults(uint32_t *hw_agc_enable_p2p, uint32_t flags)
{
   *hw_agc_enable_p2p = MOCA_HW_AGC_ENABLE_P2P_DEF;
}
...
{
   *unknown_ratelimit_en = MOCA_UNKNOWN_RATELIMIT_EN_DEF;
}
...
MOCALIB_GEN_SWAP_FUNCTION void moca_adm_stats_swap(struct moca_adm_stats * x)
{
   ...
   x->nc_started_nn = BE16(x->nc_started_nn);
   x->nc_succeeded_nn = BE16(x->nc_succeeded_nn);
   x->nc_dropped_en = BE16(x->nc_dropped_en);
}
...
{
      if ((uint32_t) BE32(val) > MOCA_NO_IFG6_MAX)
         return MOCA_NO_IFG6_ERR;
...
{
      if (flags & MOCA_3390B0_FLAG) {
         if ((uint32_t) BE32(val) < MOCA_BONDING_3390B0_MIN)
...
{
      if (flags & MOCA_6803C0_FLAG) {
         if ((uint32_t) BE32(val) > MOCA_USE_EXT_DATA_MEM_6803C0_MAX)
...
MOCALIB_GEN_RANGE_FUNCTION int __moca_hw_agc_enable_p2p_check(uint32_t val, uint32_t flags)
{
      if ((uint32_t) BE32(val) > MOCA_HW_AGC_ENABLE_P2P_MAX)
         return MOCA_HW_AGC_ENABLE_P2P_ERR;
...
{
      if ((uint32_t) BE32(enable) > MOCA_UNKNOWN_RATELIMIT_EN_MAX)
         return MOCA_UNKNOWN_RATELIMIT_EN_ERR;
...
{
   return(moca_get(vctx, IE_NO_IFG6, val, sizeof(*val)));
}

MOCALIB_GEN_SET_FUNCTION int moca_set_no_ifg6(void *vctx, uint32_t val)
{
   return(moca_set(vctx, IE_NO_IFG6, &val, sizeof(val)));
}

MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_no_ifg6 (char *cli, uint32_t * in, uint32_t * max_len)
{
   char *c = cli;
   uint32_t orig_max_len = *max_len;
   uint32_t bytes_written;

   bytes_written = snprintf(c, *max_len, "--no_ifg6 ");
   if ((bytes_written >= *max_len) || (bytes_written == 0)) goto overrun;
   else *max_len -= bytes_written;
...
MOCALIB_GEN_GET_FUNCTION int __moca_get_hw_agc_enable_p2p(void *vctx, uint32_t *val)
{
   return(moca_get(vctx, IE_HW_AGC_ENABLE_P2P, val, sizeof(*val)));
}

MOCALIB_GEN_SET_FUNCTION int __moca_set_hw_agc_enable_p2p(void *vctx, uint32_t val)
{
   return(moca_set(vctx, IE_HW_AGC_ENABLE_P2P, &val, sizeof(val)));
}

MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_hw_agc_enable_p2p (char *cli, uint32_t * in, uint32_t * max_len)
{
   char *c = cli;
   uint32_t orig_max_len = *max_len;
   uint32_t bytes_written;

   bytes_written = snprintf(c, *max_len, "--hw_agc_enable_p2p ");
   if ((bytes_written >= *max_len) || (bytes_written == 0)) goto overrun;
   else *max_len -= bytes_written;
...
MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_max_tx_power_tune (char *cli, struct moca_max_tx_power_tune * in, uint32_t * max_len)
{
   ...
   for (i = 0; i < 86 + 1; i++) {
      if ( (i == (86)) || ((i != 0) && (in->offset[i] != last_value)) ) 
      {
...
MOCALIB_GEN_GET_FUNCTION int __moca_get_reg(void *vctx, uint32_t addr, uint32_t *val)
{
   return(moca_get_inout(vctx, IE_REG, &addr, sizeof(addr), val, sizeof(*val)));
}

MOCALIB_GEN_SET_FUNCTION int __moca_set_reg(void *vctx, const struct moca_reg *in)
{
   return(moca_set(vctx, IE_REG, in, sizeof(*in)));
}
...
MOCALIB_GEN_NVRAM_FUNCTION static void moca_nv_write_no_ifg6( void * handle, uint32_t config_flags, char *cli, uint32_t * max_len)
{
   int ret=0;
   uint32_t default_no_ifg6;
   uint32_t no_ifg6;
   memset(&default_no_ifg6, 0, sizeof(default_no_ifg6));
   memset(&no_ifg6, 0, sizeof(no_ifg6));
   (void)ret; //eliminate unused variable warning

      ret = moca_get_no_ifg6(handle, &no_ifg6);
      moca_set_no_ifg6_defaults(&default_no_ifg6, config_flags);

      if ((ret == 0) && (memcmp(&default_no_ifg6, &no_ifg6, sizeof(no_ifg6))!=0))
         moca_nv_print_no_ifg6(cli, &no_ifg6, max_len);
}
...
MOCALIB_GEN_NVRAM_FUNCTION static void moca_nv_write_hw_agc_enable_p2p( void * handle, uint32_t config_flags, char *cli, uint32_t * max_len)
{
   int ret=0;
   uint32_t default_hw_agc_enable_p2p;
   uint32_t hw_agc_enable_p2p;
   memset(&default_hw_agc_enable_p2p, 0, sizeof(default_hw_agc_enable_p2p));
   memset(&hw_agc_enable_p2p, 0, sizeof(hw_agc_enable_p2p));
   (void)ret; //eliminate unused variable warning

      ret = __moca_get_hw_agc_enable_p2p(handle, &hw_agc_enable_p2p);
      moca_set_hw_agc_enable_p2p_defaults(&default_hw_agc_enable_p2p, config_flags);

      if ((ret == 0) && (memcmp(&default_hw_agc_enable_p2p, &hw_agc_enable_p2p, sizeof(hw_agc_enable_p2p))!=0))
         moca_nv_print_hw_agc_enable_p2p(cli, &hw_agc_enable_p2p, max_len);
}
...
MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_rlapm_table_50 (char *cli, struct moca_rlapm_table_50 * in, uint32_t * max_len)
{
   ...
   for (i = 0; i < 66 + 1; i++) {
      if ( (i == (66)) || ((i != 0) && (in->rlapmtable[i] != last_value)) ) 
      {
...
MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_snr_margin_rs (char *cli, struct moca_snr_margin_rs * in, uint32_t * max_len)
{
   ...
   for (i = 0; i < 10 + 1; i++) {
      if ( (i == (10)) || ((i != 0) && (in->offsets[i] != last_value)) ) 
      {
...
MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_sapm_table_50 (char *cli, struct moca_sapm_table_50 * in, uint32_t * max_len)
{
   ...
   for (i = 0; i < 256 + 1; i++) {
      if ( (i == (256)) || ((i != 0) && (in->val[i] != last_value)) ) 
      {
...
MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_sapm_table_100 (char *cli, struct moca_sapm_table_100 * in, uint32_t * max_len)
{
   ...
   for (i = 0; i < 512 + 1; i++) {
      if ( (i == (512)) || ((i != 0) && (in->val[i] != last_value)) ) 
      {
         bytes_written = snprintf (c + strlen(c), *max_len, "val ");
         if ((bytes_written >= *max_len) || (bytes_written == 0)) goto overrun;
         else *max_len -= bytes_written;
...
{
   return(moca_get(vctx, IE_UNKNOWN_RATELIMIT_EN, enable, sizeof(*enable)));
}

MOCALIB_GEN_SET_FUNCTION int moca_set_unknown_ratelimit_en(void *vctx, uint32_t enable)
{
   return(moca_set(vctx, IE_UNKNOWN_RATELIMIT_EN, &enable, sizeof(enable)));
}

MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_unknown_ratelimit_en (char *cli, uint32_t * in, uint32_t * max_len)
{
   char *c = cli;
   uint32_t orig_max_len = *max_len;
   uint32_t bytes_written;

   bytes_written = snprintf(c, *max_len, "--unknown_ratelimit_en ");
   if ((bytes_written >= *max_len) || (bytes_written == 0)) goto overrun;
   else *max_len -= bytes_written;
...
MOCALIB_GEN_NVRAM_FUNCTION static void moca_nv_write_unknown_ratelimit_en( void * handle, uint32_t config_flags, char *cli, uint32_t * max_len)
{
   int ret=0;
   uint32_t default_unknown_ratelimit_en;
   uint32_t unknown_ratelimit_en;
   memset(&default_unknown_ratelimit_en, 0, sizeof(default_unknown_ratelimit_en));
   memset(&unknown_ratelimit_en, 0, sizeof(unknown_ratelimit_en));
   (void)ret; //eliminate unused variable warning

      ret = moca_get_unknown_ratelimit_en(handle, &unknown_ratelimit_en);
      moca_set_unknown_ratelimit_en_defaults(&default_unknown_ratelimit_en, config_flags);

      if ((ret == 0) && (memcmp(&default_unknown_ratelimit_en, &unknown_ratelimit_en, sizeof(unknown_ratelimit_en))!=0))
         moca_nv_print_unknown_ratelimit_en(cli, &unknown_ratelimit_en, max_len);
}
...
{
   return(moca_get(vctx, IE_NETWORK_STATE, state, sizeof(*state)));
}
...
MOCALIB_GEN_NVRAM_FUNCTION void moca_nv_print_if_access_table (char *cli, struct moca_if_access_table * in, uint32_t * max_len)
{
   ...
   for (i = 0; i < 16 + 1; i++) {
      if ( (i == 16) || ((i != 0) && !MOCA_MACADDR_COMPARE(&in->mac_addr[i], &last_value)) ) 
      {
...
MOCALIB_GEN_NVRAM_FUNCTION void moca_write_nvram( void * handle, char *c, uint32_t max_len)
{
   ...
   *c = '\0';
   ...
   moca_nv_write_hw_agc_enable_p2p(handle, config_flags, c + strlen(c), &max_len);
   ...
   moca_nv_write_no_ifg6(handle, config_flags, c + strlen(c), &max_len);
   ...
   moca_nv_write_unknown_ratelimit_en(handle, config_flags, c + strlen(c), &max_len);
