...
CmsRet menuBackupSettings(void)
{
   ...
   sprintf(cmd, "bcmbusybox tftp -p -f %s -t c %s", backupSettingParam[1].param, backupSettingParam[0].param);
   prctl_runCommandInShellBlocking(cmd);
   CliUtil::promptContinue();
   return CLI_STS_OK;
#endif
...
CmsRet menuUpdateSettings(void)
{
   ...
   sprintf(cmd, "bcmbusybox tftp -g -t c -f %s %s", updateSettingParam[1].param, updateSettingParam[0].param);
   prctl_runCommandInShellBlocking(cmd);
   CliUtil::promptContinue();
   return CLI_STS_OK;;
#endif
...
CmsRet menuUpdateSoftware(void)
{
   ...
   sprintf(cmd, "bcmbusybox tftp -g -t i -f %s %s", updateSoftwareParam[1].param, updateSoftwareParam[0].param);
   prctl_runCommandInShellBlocking(cmd);
   CliUtil::promptContinue();
   return CLI_STS_OK;;
#endif   
