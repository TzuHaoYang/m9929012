...
static const CLI_CMD_ITEM cliCmdTable[] = {
   ...
   { "tracelocks", "show MDM lock activity",     PERM1, 0, processTraceLocksCmd },
   { "dumplocks", "dump all MDM lock info",     PERM1, 0, processDumpLocksCmd },
   { "dumpoid", "dump object id info",     PERM1, 0, processDumpOidCmd },
   { "dumpcfg", "inspect config info in flash", PERM2, 0, processDumpCfgCmd },
   { "dumpmdm", "inspect MDM",                  PERM1, 0, processDumpMdmCmd },
   ...
   { "dumpsysinfo", "Dump system info for error report", PERM2, 0, processDumpSysInfoCmd },
   ...
   { "tftp", "Broadcom tftp",                 PERM2, 0,   processBcmTftpCmd },
...
#endif
   { "intfgroup", "Interface Grouping", PERM1, LOCK_NEEDED, processIntfGroupCmd },
};
...
/* Timer varying control value size (code/CVC access start). */
#define SSD_TIME_VARY_CTRL_LEN 26
...
static void dumpXPONObjInfo(void)
{
    ...
    if (obj->oamSelection != 0)
    {
        eponBitVendorStr(obj->oamSelection, (char *)oamSelStr, sizeof(oamSelStr));
    }
    else
    {// default oam mode is based on profile
#ifdef DPOE_OAM_BUILD
        sprintf(oamSelStr, "Default is dpoe");
#else
        sprintf(oamSelStr, "Default is ctc");
#endif
    ...
    if (obj->dpoeCosignerName != NULL)
    {
        printf("DpoeSsdCosignerName:\t%s\n", obj->dpoeCosignerName);
    }

    if (obj->dpoeCosignerTime != NULL)
    {
        UINT8 *buf = NULL;
        UINT32 size = 0;
        CmsRet ret = CMSRET_SUCCESS;
        char cosignerTime[SSD_TIME_VARY_CTRL_LEN + 1];

        ret = cmsUtl_hexStringToBinaryBuf(obj->dpoeCosignerTime,
          &buf, &size);
        if ((ret == CMSRET_SUCCESS) && (buf != NULL))
        {
            if (size == SSD_TIME_VARY_CTRL_LEN)
            {
                memcpy(cosignerTime, buf, size);
                cosignerTime[SSD_TIME_VARY_CTRL_LEN] = '\0';
                printf("DpoeSsdCosignerTime:\t%s\n", cosignerTime);
            }
            cmsMem_free(buf);
        }
    }
...
static void saveXponObjInfo(UINT8 type, UINT8 *data, UINT16 len)
{
    ...
    switch (type)
    {
        ...
        case 10:
            CMSMEM_FREE_BUF_AND_NULL_PTR(obj->dpoeCosignerName);
            if (len != 0)
            {
                obj->dpoeCosignerName = cmsMem_alloc(len + 1, ALLOC_ZEROIZE);
                memcpy(obj->dpoeCosignerName, data, len);
            }
            break;

        case 11:
            CMSMEM_FREE_BUF_AND_NULL_PTR(obj->dpoeCosignerTime);
            if (len != 0)
            {
                char *hexStr = NULL;

                if (cmsUtl_binaryBufToHexString(data, len, &hexStr) == CMSRET_SUCCESS)
                {
                    CMSMEM_REPLACE_STRING(obj->dpoeCosignerTime, hexStr);
...
void cmdEponCfgHelp(int argc)
{
    if ( argc <= 1 )
        fprintf(stdout,
         ...
         "       eponcfg dpoecosname <name>|-clear\n"
         "       eponcfg dpoecostime <YYMMDDhhmmssZYYMMDDhhmmssZ>|-clear\n"
         "       eponcfg --help\n");
    else
...
void processEponCfgCmd(char *cmdLine)
{
    ...
    else if (strcasecmp(argv[0], "date") == 0)
    {
        ...
        if ((strlen(argv[1]) == 8) &&
          checkDateStr((UINT8 *)argv[1], strlen(argv[1])))
        {
            saveXponObjInfo(2, (UINT8 *)argv[1], strlen(argv[1]));
        }
        else
        {
            fprintf(stderr, "eponcfg: invalid value of parameters " \
                "for option 'date'\n");
            return;
        }
    }
    ...
    else if (strcasecmp(argv[0], "dpoecosname") == 0)
    {
        if (argc != 2)
        {
            fprintf(stderr, "eponcfg: invalid number of parameters " \
                "for option 'dpoecosname'\n");
            return;
        }
        if (strcmp(argv[1], "-clear") == 0)
        {
            saveXponObjInfo(10, NULL, 0);
        }
        else
        {
            saveXponObjInfo(10, (UINT8 *)argv[1], strlen(argv[1]));
        }
    }
    else if (strcasecmp(argv[0], "dpoecostime") == 0)
    {
        char *argvp;
        int argvLen;
        ...
        if (strcmp(argv[1], "-clear") == 0)
        {
            saveXponObjInfo(11, NULL, 0);
        }
        else if ((argvLen == 26) && checkDateStr((UINT8 *)argvp, 12) &&
          checkDateStr((UINT8 *)(argvp + 13), 12))
        {
            saveXponObjInfo(11, (UINT8 *)argv[1], argvLen);
        }
        else
        {
            fprintf(stderr, "eponcfg: invalid value of parameters " \
