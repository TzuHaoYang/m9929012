...
CmsRet getPppInfo(PWEB_NTWK_VAR pInfo)
{
   ...
   if (strcasecmp(pppCfgParams[2].param, "2") == 0)
   {
      pInfo->pppAuthMethod = PPP_AUTH_METHOD_PAP;
   }
   else if (strcasecmp(pppCfgParams[2].param, "3") == 0)
   {
      pInfo->pppAuthMethod = PPP_AUTH_METHOD_CHAP;
   }
   else if (strcasecmp(pppCfgParams[2].param, "4") == 0)
   {
   ...
   /* enable IGMP */
   if (strcasecmp(pppCfgParams[3].param, "1") == 0)
   {
      pInfo->enblIgmp = TRUE;
   }
   else
