   ...
   if (!strcasecmp(cmdLine, "show all"))
   {
                            ...
                            (void **)&hostObj) == CMSRET_SUCCESS)
      {
         ...
         if (cmsUtl_isValidIpv4Address(hostObj->IPAddress))
         {
            ...
            if (cmsObj_getNextInSubTreeFlags(MDMOID_DEV2_HOST_IPV6_ADDRESS,
                                             &iidStack,
                                             &iidAddr,
                                             OGF_NO_VALUE_UPDATE,
                                             (void **) &ipv6AddrObj) == CMSRET_SUCCESS)
            {
               printf("%s  %s\n", hostObj->physAddress, ipv6AddrObj->IPAddress);
...
void DslCfgProfileUpdate_dev2(unsigned char lineId)
{
...
#ifdef SUPPORT_DSL_GFAST
    found = FALSE;
    while (!found && (cmsObj_getNext(MDMOID_DEV2_FAST_LINE, &iidStack1, (void **) &fastLineObj) == CMSRET_SUCCESS))
    {
       if (dslLineObj->X_BROADCOM_COM_BondingLineNumber == lineId)
       {
          found = TRUE;
    ...
    if (!found)
    {
       printf("%s: could not get FAST Line Obj for line %d", __FUNCTION__, lineId);
       cmsObj_free((void **) &dslLineObj);
       return;
    }
...
void DslIntfCfgMdmUpdate_dev2(unsigned char lineId)
{
    ...
    rutfast_intfCfgInit_dev2(&adslCfg, fastLineObj);
    cmsRet = cmsObj_set(fastLineObj, &iidStack1);
    if (cmsRet != CMSRET_SUCCESS)
    {
       printf("%s: could not set FAST Line Obj for line %d, ret=%d", __FUNCTION__, lineId,cmsRet);
    }
    cmsObj_free((void **) &fastLineObj);
#endif
