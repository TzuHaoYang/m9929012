...
#ifdef DMP_DEVICE2_ETHERNETINTERFACE_1      
{
         ...
         InstanceIdStack ethLagIidStack = EMPTY_INSTANCE_ID_STACK;
         OpticalInterfaceObject* ethLagIntfObj = NULL;

         while (found == FALSE &&
                cmsObj_getNextFlags(MDMOID_LA_G,
                                    &ethLagIidStack,
                                    OGF_NO_VALUE_UPDATE,
                                    (void **)&ethLagIntfObj) == CMSRET_SUCCESS)
         {
            if (cmsUtl_strcmp(currL2IntfNameBuf, ethLagIntfObj->name) == 0)
            {
               snprintf(linkType, BUFLEN_32-1, "%s", MDMVS_EOA);
               found = TRUE;
            }

            cmsObj_free((void **)&ethLagIntfObj);
         }
...
#endif
      printf("%s\t", ipIntf->status? ipIntf->status : "\t");

      if (cmsUtl_strcmp(ipIntf->status, MDMVS_DORMANT) == 0 || cmsUtl_strcmp(ipIntf->status, MDMVS_UP) == 0)
      {
         printf("\t");
      }
      ...
      if (!ipIntf->X_BROADCOM_COM_BridgeService)      
      {
         ...
         if (qdmIpIntf_getIpvxAddressByNameLocked_dev2(CMS_AF_SELECT_IPV6, ipIntf->name, ipAddress) == CMSRET_SUCCESS)
         {
            printf("%s", ipAddress);
         }
      }
