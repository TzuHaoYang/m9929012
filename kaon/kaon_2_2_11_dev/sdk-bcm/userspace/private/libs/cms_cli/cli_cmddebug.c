...
void processDumpOidCmd(char *cmdLine)
{
   ...
   if (isdigit(cmdLine[0]))
   {
      ...
      oid = atoi(cmdLine);
      entry = cmsMdm_getOidInfoEntry(oid);
      if (entry != NULL)
      {
...
void processDumpMdmCmd(char *cmdLine)
{
   ...
   /*
    * Dump the current MDM into the memory buffer and print that out.
    * (Due to the new CMS auto-locking feature, cmsMgm_writeMdmToBuf an will do
    * the locking for us, so we did not need to acquire the global lock before
...
void processMdmCmd(char *cmdLine)
{
   ...
   if (!strcasecmp(cmdLine, "help") || !strcasecmp(cmdLine, "-h") || !strcasecmp(cmdLine, "--help"))
   {
      ...
      printf("       mdm getov <full path to object>\n");
   ...
   else if (!strncasecmp(cmdLine, "getov", 5))
   {
      ...
      if (ret == CMSRET_SUCCESS) 
      {
         ...
         ret = cmsPhl_getParameterValues(&pathDesc, 1, FALSE,
                                         &getParamValueList, &numGetParamValues);
         if (ret != CMSRET_SUCCESS)
         {
      ...
      {
         if (getParamValueList[idxParam].pathDesc.paramName != NULL)
