   ...
   if (cmsUtl_strcmp(webVar->wanAddr6Type, MDMVS_STATIC) == 0)
   {
      ...
      */
      ...
      else
      {
         ...
         if (!IS_EMPTY_STRING(webVar->wanGtwy6))
         {
            /* Add and set the ipv6 forwarding object */
            if ((ret = cmsDal_addWanGtwy6_dev2(webVar->wanGtwy6, IpIntfPathRef)) != CMSRET_SUCCESS)
            {
               cmsLog_error("cmsDal_addWanGtwy6_dev2 failed. ret=%d", ret);
            }
         }
      }
