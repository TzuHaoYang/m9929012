                       ...
                       UINT32 psidOffset, UINT32 psidLen, UINT32 psid)
{
   ...
   cmsLog_debug("Configure new MAP-%c: %s/%s/%d/%s",
                *mechanism, wanIntf, lanIntf, dynamic, BRPrefix);

   if ( (mechanism == NULL) || (wanIntf == NULL) || (lanIntf == NULL) ||
   ...
   /*
    * FIXME: webUI doesn't allow more than one MAP-T (or MAP-E) configuration.
    * So we don't need to check existing configuration before add.
    */
   ...
   REPLACE_STRING_IF_NOT_EQUAL(domain->transportMode, mechanism);
...
CmsRet dalMap_delete_dev2(const char* wanIntf, const char* mode)
{
   ...
   if ( (wanIntf == NULL) || (mode == NULL) )
   {
      cmsLog_error("Invalid parameters");
      return CMSRET_INVALID_PARAM_VALUE;
   }

   cmsLog_debug("Delete MAP-%c: %s", *mode, wanIntf);
