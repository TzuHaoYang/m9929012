                                           ...
                                           const WEB_NTWK_VAR *glbWebVar)
{
   ...
   {
      char subnetLen[BUFLEN_8]="/64";
      cmsNet_subnetIp6SitePrefix(glbWebVar->ipv6UlaPrefix, 0, 64, addrtmp);
      cmsUtl_strncat(addrtmp, CMS_IPADDR_LENGTH, subnetLen);
...
static void updateUlaByPrefix(const char *ulaPrefix, UBOOL8 add)
{
    ...
    */
   {
      ...
      if (!found)
      {
         ...
         if (glbWebVar->enblRadvdUla)
         {
            ...
            if (glbWebVar->enblRadvdUla)
            {
               updateUlaByPrefix(glbWebVar->ipv6UlaPrefix, 1);
            }
              ...
              )
      {
         ...
         {
            updateUlaByPrefix(ipv6Prefix->prefix, 0);
            updateUlaPrefixToIpObj(glbWebVar->ipv6UlaPrefix);
