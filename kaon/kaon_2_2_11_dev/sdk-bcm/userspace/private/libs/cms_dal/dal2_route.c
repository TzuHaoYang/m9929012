...
CmsRet cmsDal_addWanGtwy6_dev2(const char *gtwy, const char *ipIntfFullPath)
{
   InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK;
   ...
   cmsLog_debug("Enter: gtwy=%s ipIntf=%s", gtwy, ipIntfFullPath);
   /*
    * Don't know why TR181 defines multiple router objects.  Just get the
    * first router object and add the ipv6ForwardingObj entry under that one.
    */
   if ((ret = cmsObj_getNext(MDMOID_DEV2_ROUTER, &iidStack, (void **)&routerObj)) != CMSRET_SUCCESS)
   {
      cmsLog_error("Failed to get MDMOID_DEV2_ROUTER Instance, ret = %d", ret);
   ...
   if ((ret = cmsObj_get(MDMOID_DEV2_IPV6_FORWARDING, &iidStack, 0, (void **) &ipv6ForwardingObj)) != CMSRET_SUCCESS)
   {
      cmsLog_error("Failed to get ipv6ForwardingObj, ret = %d", ret);
      cmsObj_deleteInstance(MDMOID_DEV2_IPV6_FORWARDING, &iidStack);
      return ret;
   }

   ipv6ForwardingObj->enable = TRUE;
   /* Just copy webVar->wanGtwy6 over
    */
   CMSMEM_REPLACE_STRING(ipv6ForwardingObj->nextHop, gtwy);
   CMSMEM_REPLACE_STRING(ipv6ForwardingObj->interface, ipIntfFullPath);

   if ((ret = cmsObj_set(ipv6ForwardingObj, &iidStack)) != CMSRET_SUCCESS)
   {
      cmsLog_error("Failed to set ipv6ForwardingObj. ret=%d", ret);
      cmsObj_deleteInstance(MDMOID_DEV2_IPV6_FORWARDING, &iidStack);
   }
...
void cmsDal_deleteWanGtwy6_dev2(const char *ipIntfFullPath)
{
   UBOOL8 found=FALSE;
   InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK; 
   Dev2Ipv6ForwardingObject *ipv6ForwardingObj=NULL;
                             ...
                             (void **) &ipv6ForwardingObj) == CMSRET_SUCCESS)
   {
      ...
      if (!cmsUtl_strcmp(ipv6ForwardingObj->origin, MDMVS_STATIC) &&
          ...
          !cmsUtl_strcasecmp(ipv6ForwardingObj->interface, ipIntfFullPath))
      {
         CmsRet r2;

         found = TRUE;
         cmsLog_debug("Deleting instance at %s", cmsMdm_dumpIidStack(&iidStack));
         if ((r2 = cmsObj_deleteInstance(MDMOID_DEV2_IPV6_FORWARDING, &iidStack)) != CMSRET_SUCCESS)
         {
            cmsLog_error("Failed to delete  MDMOID_DEV2_IPV6_FORWARDING, ret=%d", r2);
         }
      }
...
CmsRet dalRip_deleteRipInterfaceSetting_dev2(const char *ipIntfFullPath)
{
   ...
   /* This is device 2 rip interface setting object  is only for device 2 data model */
   if (!cmsMdm_isDataModelDevice2())
   {
