...
CmsRet dalVoice_SetDefaults( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   dalVoice_GetNumPhysEndpts( &numPhysEpt );
   ...
   for ( i = 0; i<spNum; i++ )
   {
      ...
      dalVoice_SetSipFailoverEnable(&parmsList, MDMVS_OFF);
      dalVoice_SetSipOptionsEnable(&parmsList, MDMVS_OFF);
      dalVoice_SetSipBackToPrimOption(&parmsList, MDMVS_DISABLED);
      dalVoice_SetSipMode( &parmsList, MDMVS_RFC3261 );
      ...
      for( j = 0; j < numAcc; j++ )
      {
         ...
         {
            /* Assign non FXO ports  in ( accNum % maxfxsports ) order */
            snprintf( temp,TEMP_CHARBUF_SIZE, "%d", ((numPhysEpt-numPhysFxo) > 0) ? (j % (numPhysEpt - numPhysFxo)) : (j) );
...
#ifdef SIPLOAD
   dalVoice_SetVoiceDnsEnable( &parmsList, MDMVS_OFF );
#if VOICE_IPV6_SUPPORT
   if ( isIpv6 )
   {
      dalVoice_SetDnsServerAddrPri( &parmsList, ZERO_ADDRESS_IPV6 );
      dalVoice_SetDnsServerAddrSec( &parmsList, ZERO_ADDRESS_IPV6 );
   }
   else
#endif /* VOICE_IPV6_SUPPORT */
   {
      dalVoice_SetDnsServerAddrPri( &parmsList, ZERO_ADDRESS_IPV4 );
      dalVoice_SetDnsServerAddrSec( &parmsList, ZERO_ADDRESS_IPV4 );
   }
#endif /* SIPLOAD */
...
CmsRet dalVoice_SetVoiceDnsEnable ( DAL_VOICE_PARMS *parms, char *setVal )
{
   ...
   if ( (isOff = strncasecmp(setVal, MDMVS_ON, strlen(MDMVS_ON))) &&
        strncasecmp(setVal, MDMVS_OFF, strlen(MDMVS_OFF)) )
   {
   ...
   {
      voiceObj->X_BROADCOM_COM_VoiceDnsEnable = boolVal;
      if ( (ret = cmsObj_set(voiceObj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "could not set voice obj, ret=%d", ret);
      }
...
CmsRet dalVoice_SetDnsServerAddrPri ( DAL_VOICE_PARMS *parms, char *setVal )
{
   ...
   {
      REPLACE_STRING_IF_NOT_EQUAL(voiceObj->X_BROADCOM_COM_VoiceDnsServerPri, setVal);
      if ( (ret = cmsObj_set(voiceObj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "could not set voice obj, ret=%d", ret);
      }
...
CmsRet dalVoice_SetDnsServerAddrSec ( DAL_VOICE_PARMS *parms, char *setVal )
{
   ...
   {
      REPLACE_STRING_IF_NOT_EQUAL(voiceObj->X_BROADCOM_COM_VoiceDnsServerSec, setVal);
      if ( (ret = cmsObj_set(voiceObj, &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "could not set voice obj, ret=%d", ret);
      }
...
CmsRet dalVoice_SetSipUriForTls( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   sipObj->X_BROADCOM_COM_UseSipUriForTls = setVal;

   cmsLog_debug( "SIP URI for TLS = %d for [vpInst] = [%u]\n", setVal, vpInst);
   /* copy new value from local copy to MDM */
   if ( ( ret = cmsObj_set( sipObj,  &iidStack)) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't set SIP URI for TLS ret = %d\n", ret);
   }
...
CmsRet dalVoice_SetVlCFCallId( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   return ( setVlCFFeatureValue(CFCALLTYPE_ENABLED, vpInst, lineInst, DAL_VOICE_FEATURE_CODE_CID, setValInt));
}

/***************************************************************************
* Function Name: dalVoice_SetVlCFCallIdName
* Description  : Enable or disable incoming caller ID name by the endpoint
*                VoiceProfile.{i}.Line{i}.CallingFeatures.CallerIDEnable = new value
*
* Parameters   : parms->op[0] = vpInst, parms->op[1] = lineInst, value = value to be set
* Returns      : CMSRET_SUCCESS when successful.
...
CmsRet dalVoice_SetVlCFCallIdName( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   return ( setVlCFFeatureValue(CFCALLTYPE_ENABLED, vpInst, lineInst, DAL_VOICE_FEATURE_CODE_CID_NAME, setValInt));
}
...
/***************************************************************************
* Function Name: dalVoice_SetEServiceDSCPMark
*
* Description  : Set emergency service DSCP value 
*                VoiceProfile.{i}.Line.{i}.CallingFeatures.X_BROADCOM_COM_EmergencyDSCPMark
*
* Parameters   : parms->op[0] = vpInst, value = value to be set "yes" or "no"
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetEServiceDSCPMark ( DAL_VOICE_PARMS *parms, char *val)
{
   ...
   int                setVal;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   VoiceLineCallingFeaturesObject *obj = NULL;
   LEVELINFO   info[DAL_VOICE_MAXLEVELS]    = { {MDMOID_VOICE_PROF, vpInst, EMPTY_INSTANCE_ID_STACK},
                                      {MDMOID_VOICE_LINE, lineInst, EMPTY_INSTANCE_ID_STACK},
                                      {MDMOID_VOICE_LINE_CALLING_FEATURES, ANY_INSTANCE, EMPTY_INSTANCE_ID_STACK},
                                      {0, 0, EMPTY_INSTANCE_ID_STACK}, };

   dumpCmdParam(parms, val); /* Debug: display command parameters passed here */
   ...
   {
      obj->X_BROADCOM_COM_EmergencyDSCPMark = setVal;

      cmsLog_debug( "EmergencyDSCP = %d for [lineInst] = [%u]\n", setVal, lineInst);

      /* set new value back to MDM */
      if ( ( ret = cmsObj_set( obj,  &iidStack)) != CMSRET_SUCCESS )
      {
         cmsLog_error( "%s Can't set object for reason (%d)\n",__FUNCTION__, ret);
      }
...
/***************************************************************************
* Function Name: dalVoice_SetEServiceEndAllCall
*
* Description  : Set end all current call and accept incoming emergency call
*                VoiceProfile.{i}.EmergencyService.X_BROADCOM_COM_EndCallAcptIncEmerg
*
* Parameters   : parms->op[0] = vpInst, value = value to be set "yes" or "no"
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetEServiceEndCallAcptIncEmerg ( DAL_VOICE_PARMS *parms, char *val)
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
   ...
   if ( val != NULL && strlen(val) )
   {
      if ( !strncasecmp(val, MDMVS_YES, strlen(MDMVS_YES)))
      {
          setVal = 1;
      }
      else if(!strncasecmp(val, MDMVS_NO, strlen(MDMVS_NO)))
      {
          setVal = 0;
      }
   }
   ...
   /*  Get the Voice line call feature object */
   if ( (ret = getObj(info, &iidStack, OGF_NO_VALUE_UPDATE, (void**)&obj)) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't retrieve calling features object \n" );
      return ( ret );
   ...
   {
      obj->X_BROADCOM_COM_EndCallAcptIncEmerg = setVal;

      cmsLog_debug( "AcptIncEmerg = %d for [lineInst] = [%u]\n", setVal, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetEServiceNoLocInfo
*
* Description  : Set emergency service no location info flag
*                VoiceProfile.{i}.EmergencyService.X_BROADCOM_COM_NoLocationInfo
*
* Parameters   : parms->op[0] = vpInst, value = value to be set "yes" or "no"
*
* Returns      : CMSRET_SUCCESS when successful.
...
CmsRet dalVoice_SetEServiceNoLocInfo ( DAL_VOICE_PARMS *parms, char *val)
{
   ...
   {
      obj->X_BROADCOM_COM_EmergNoLocationInfo = setVal;

      cmsLog_debug( "NolocInfo = %d for [lineInst] = [%u]\n", setVal, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetEServiceAllow3WayCall
*
* Description  : Set emergency service 3way call enable flag
*                VoiceProfile.{i}.EmergencyService.X_BROADCOM_COM_Allow3WayCall
*
* Parameters   : parms->op[0] = vpInst, value = value to be set "yes" or "no"
*
* Returns      : CMSRET_SUCCESS when successful.
...
CmsRet dalVoice_SetEServiceAllow3WayCall ( DAL_VOICE_PARMS *parms, char *val)
{
   ...
   {
      obj->X_BROADCOM_COM_EmergAllow3WayCall = setVal;

      cmsLog_debug( "Allow3wayCall = %d for [lineInst] = [%u]\n", setVal, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetEServiceNwHoldBypass
*
* Description  : Set emergency service network hold bypass check flag
*                VoiceProfile.{i}.EmergencyService.X_BROADCOM_COM_NetworkHoldBypassChk
*
* Parameters   : parms->op[0] = vpInst, value = value to be set "yes" or "no"
*
* Returns      : CMSRET_SUCCESS when successful.
...
CmsRet dalVoice_SetEServiceNwHoldBypass ( DAL_VOICE_PARMS *parms, char *val)
{
   ...
   {
      obj->X_BROADCOM_COM_EmergNetworkHoldBypassChk = setVal;

      cmsLog_debug( "Netwk call hold bypass chk = %d for [lineInst] = [%u]\n", setVal, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetEServiceNwHoldDisable
*
* Description  : Set emergency service network hold disable flag
*                VoiceProfile.{i}.EmergencyService.X_BROADCOM_COM_NetworkHoldDisable
*
* Parameters   : parms->op[0] = vpInst, value = value to be set "yes" or "no"
*
* Returns      : CMSRET_SUCCESS when successful.
...
CmsRet dalVoice_SetEServiceNwHoldDisable ( DAL_VOICE_PARMS *parms, char *val)
{
   ...
   {
      obj->X_BROADCOM_COM_EmergNetworkHoldDisable = setVal;

      cmsLog_debug( "Netwk Call hold diable = %d for [lineInst] = [%u]\n", setVal, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetEServiceNwHoldTime
*
* Description  : Set emergency service network holding time
*                VoiceProfile.{i}.EmergencyService.X_BROADCOM_COM_NetworkHoldTime
*
...
CmsRet dalVoice_SetEServiceNwHoldTime ( DAL_VOICE_PARMS *parms, char *val)
{
   ...
   {
      obj->X_BROADCOM_COM_EmergNetworkHoldTime = setVal;

      cmsLog_debug( "Emergency NetworkHold time = %d for [lineInst] = [%u]\n", setVal, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetEServiceHowlerDuration
*
* Description  : Set emergency service howler tone duration
*                VoiceProfile.{i}.EmergencyService.X_BROADCOM_COM_EmergHowlerDuration
*
* Parameters   : parms->op[0] = vpInst, value = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
...
CmsRet dalVoice_SetEServiceHowlerDuration ( DAL_VOICE_PARMS *parms, char *val)
{
   ...
   {
      obj->X_BROADCOM_COM_EmergHowlerDuration = setVal;

      cmsLog_debug( "Emergency howler duration = %d for [lineInst] = [%u]\n", setVal, lineInst);
...
/***************************************************************************
* Function Name: dalVoice_SetVlResetStats
*
* Description  : Set "ResetStatistics" flag to TRUE to trigger clearing of
*                RTP and call stats for particular account.
*                VoiceService.{i}.VoiceProfile.{i}.Line.{i}.Stats.ResetStatistics
*
* Parameters   : parms->op[0] = vpInst, vparms->op[1] = lineInst
*                val = value to be set
*
* Returns      : CMSRET_SUCCESS when successful.
****************************************************************************/
CmsRet dalVoice_SetVlResetStats( DAL_VOICE_PARMS *parms, char *val )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   VoiceLineStatsObject *obj = NULL;
   LEVELINFO   info[DAL_VOICE_MAXLEVELS]    = { {MDMOID_VOICE_PROF, vpInst, EMPTY_INSTANCE_ID_STACK},
                                      {MDMOID_VOICE_LINE, lineInst, EMPTY_INSTANCE_ID_STACK},
   ...
   if ( val == NULL ||
        0 == strlen(val) ||
        strncasecmp( MDMVS_YES, val, strlen( MDMVS_YES ) ) )
   {
      return CMSRET_INVALID_ARGUMENTS;
   }

   /*  Get the Voice line stats object */
   if ( (ret = getObj(info, &iidStack, OGF_NO_VALUE_UPDATE, (void**)&obj)) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't retrieve calling features object \n" );
      return ( ret );
   ...
   {
      obj->resetStatistics = 1;

      cmsLog_debug( "ResetStatistics = 1 for [lineInst] = [%u]\n", lineInst);
...
/*****************************************************************
**  FUNCTION:       dalVoice_GetVlCallingFeatureObject
**
**  PUROPOSE:       Return voice line calling feature object pointer
**
**  INPUT PARMS:    parms->op[0] = Voice Profile Instance
**                  parms->op[1] = Line Instance
**
**  OUTPUT PARMS:   obj - CallingFeatures pointer
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with NULL pointer
**
*******************************************************************/
CmsRet dalVoice_GetVlCallingFeatureObject ( DAL_VOICE_PARMS *parms,  void **obj )
{
   unsigned int       vpInst      = parms->op[0];
   unsigned int       lineInst    = parms->op[1];
   CmsRet             ret;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   VoiceLineCallingFeaturesObject *callFeatureObj = NULL;
   LEVELINFO   info[DAL_VOICE_MAXLEVELS]    = { {MDMOID_VOICE_PROF, vpInst, EMPTY_INSTANCE_ID_STACK},
                                      {MDMOID_VOICE_LINE, lineInst, EMPTY_INSTANCE_ID_STACK},
                                      {MDMOID_VOICE_LINE_CALLING_FEATURES, ANY_INSTANCE, EMPTY_INSTANCE_ID_STACK},
                                      {0, 0, EMPTY_INSTANCE_ID_STACK}, };

   cmsLog_debug( "%s() line:%d \n",__FUNCTION__, parms->op[1]);

   /*  Get the Voice Profile object */
   if ( (ret = getObj(info, &iidStack, OGF_NO_VALUE_UPDATE, (void**)&callFeatureObj)) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't retrieve calling features object \n" );
      return ( ret );
   }
...
**
**  OUTPUT PARMS:   endCall - end all current call and accept incoming emergency call
**                            "1" as true or "0" as false
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_GetEServiceEndAllCall ( DAL_VOICE_PARMS *parms, char *endCall, unsigned int length )
{
   CmsRet             ret;
   VoiceLineCallingFeaturesObject *obj = NULL;

   ret = dalVoice_GetVlCallingFeatureObject( parms, (void **)&obj );
   if ( ret != CMSRET_SUCCESS || obj == NULL )
   {
      cmsLog_error( "%s failed get EService Object\n",__FUNCTION__);
      return ( ret );
   }

   /* DAL interface protects against this but Network CMS may not */
   snprintf( (char *)endCall, length, "%s", (obj->X_BROADCOM_COM_EndCallAcptIncEmerg)?MDMVS_YES:MDMVS_NO);
   cmsObj_free( (void **) &obj );
...
**                  length       = Maximum length of output string
**
**  OUTPUT PARMS:   chk - network hold bypass check, "1" as true or "0" as false
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_GetEServiceNwHoldBypass ( DAL_VOICE_PARMS *parms, char *chk, unsigned int length )
{
   CmsRet             ret;
   VoiceLineCallingFeaturesObject *obj = NULL;
   ...
   /* DAL interface protects against this but Network CMS may not */
   snprintf( (char *)chk, length, "%s", (obj->X_BROADCOM_COM_EmergNetworkHoldBypassChk)?MDMVS_YES:MDMVS_NO);
   cmsObj_free( (void **) &obj );
...
**                  length       = Maximum length of output string
**
**  OUTPUT PARMS:   allow - allow 3 waycall, "1" as true or "0" as false
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
...
CmsRet dalVoice_GetEServiceNwHoldDisable ( DAL_VOICE_PARMS *parms, char *disable, unsigned int length )
{
   ...
   /* DAL interface protects against this but Network CMS may not */
   snprintf( (char *)disable, length, "%s", (obj->X_BROADCOM_COM_EmergNetworkHoldDisable)?MDMVS_YES:MDMVS_NO);
   cmsObj_free( (void **) &obj );
...
**
*******************************************************************/
CmsRet dalVoice_GetEServiceAllow3WayCall ( DAL_VOICE_PARMS *parms, char *allow, unsigned int length )
{
   CmsRet             ret;
   VoiceLineCallingFeaturesObject *obj = NULL;
   ...
   /* DAL interface protects against this but Network CMS may not */
   snprintf( (char *)allow, length, "%s", (obj->X_BROADCOM_COM_EmergAllow3WayCall)?MDMVS_YES:MDMVS_NO);
   cmsObj_free( (void **) &obj );
...
**                  length       = Maximum length of output string
**
**  OUTPUT PARMS:   loc - no location info, "1" as true or "0" as false
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_GetEServiceNoLocInfo ( DAL_VOICE_PARMS *parms, char *loc, unsigned int length )
{
   CmsRet             ret;
   VoiceLineCallingFeaturesObject *obj = NULL;
   ...
   /* DAL interface protects against this but Network CMS may not */
   snprintf( (char *)loc, length, "%s", (obj->X_BROADCOM_COM_EmergNoLocationInfo)?MDMVS_YES:MDMVS_NO);
   cmsObj_free( (void **) &obj );
...
**
**  PUROPOSE:       Return value of
**                  VoiceProfile.{i}.EmergencyService.X_BROADCOM_COM_EmergencyDSCPMark
**
**  INPUT PARMS:    parms->op[0] = Voice Profile Instance
**                  length       = Maximum length of output string
**
**  OUTPUT PARMS:   dscp - dscp mark
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
...
CmsRet dalVoice_GetEServiceDSCPMark ( DAL_VOICE_PARMS *parms, char *dscp, unsigned int length )
{
   ...
   /* DAL interface protects against this but Network CMS may not */
   snprintf( (char *)dscp, length, "%u", obj->X_BROADCOM_COM_EmergencyDSCPMark);
   cmsObj_free( (void **) &obj );
...
**                  length       = Maximum length of output string
**
**  OUTPUT PARMS:   time - Network Hold time
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
*******************************************************************/
CmsRet dalVoice_GetEServiceNwHoldTime ( DAL_VOICE_PARMS *parms, char *nwtime, unsigned int length )
{
   CmsRet             ret;
   VoiceLineCallingFeaturesObject *obj = NULL;
   ...
   /* DAL interface protects against this but Network CMS may not */
   snprintf( (char *)nwtime, length, "%u", obj->X_BROADCOM_COM_EmergNetworkHoldTime);
   cmsObj_free( (void **) &obj );
...
**                  length       = Maximum length of output string
**
**  OUTPUT PARMS:   duration - howler tone duration
**
**  RETURNS:        CMSRET_SUCCESS - Read Success
**                  other failed, check with reason code
**
...
CmsRet dalVoice_GetEServiceHowlerDuration ( DAL_VOICE_PARMS *parms, char *duration, unsigned int length )
{
   ...
   /* DAL interface protects against this but Network CMS may not */
   snprintf( (char *)duration, length, "%u", obj->X_BROADCOM_COM_EmergHowlerDuration);
   cmsObj_free( (void **) &obj );
...
**
**  PUROPOSE:       Get voice DNS enable flag
*                   (X_BROADCOM_COM_VoiceDnsEnable)
**
**  INPUT PARMS:    None
**
...
CmsRet dalVoice_GetVoiceDnsEnable(DAL_VOICE_PARMS *parms, char *getVal, unsigned int length )
{
   ...
   {
      snprintf( getVal, length, "%u",voiceObj->X_BROADCOM_COM_VoiceDnsEnable );
   }
...
CmsRet dalVoice_GetDnsServerAddrPri(DAL_VOICE_PARMS *parms, char *getVal, unsigned int length )
{
   ...
   {
      if ( voiceObj->X_BROADCOM_COM_VoiceDnsServerPri != NULL )
      {
         snprintf( getVal, length, "%s",voiceObj->X_BROADCOM_COM_VoiceDnsServerPri );
#if VOICE_IPV6_SUPPORT
         if ( (ret = dal_Voice_StripIpv6PrefixLength(voiceObj, getVal)) != CMSRET_SUCCESS )
         {
            cmsLog_error( "Could not strip address prefix length, ret=%d", ret);
      ...
      {
         cmsLog_error( "X_BROADCOM_COM_VoiceDnsServerPri is null");
         snprintf( getVal, length, "undefined" );
      }
   }
...
CmsRet dalVoice_GetDnsServerAddrSec(DAL_VOICE_PARMS *parms, char *getVal, unsigned int length )
{
   ...
   {
      if ( voiceObj->X_BROADCOM_COM_VoiceDnsServerSec != NULL )
      {
         snprintf( getVal, length, "%s",voiceObj->X_BROADCOM_COM_VoiceDnsServerSec );
#if VOICE_IPV6_SUPPORT
         if ( (ret = dal_Voice_StripIpv6PrefixLength(voiceObj, getVal)) != CMSRET_SUCCESS )
         {
            cmsLog_error( "Could not strip address prefix length, ret=%d", ret);
      ...
      {
         cmsLog_error( "X_BROADCOM_COM_VoiceDnsServerSec is null");
         snprintf( getVal, length, "undefined" );
      }
   }
...
CmsRet dalVoice_GetSipUriForTls(DAL_VOICE_PARMS *parms, char *value, unsigned int length )
{
   ...
   /* Copy SIP tag matching value */
   cmsLog_debug( "SIP URI for TLS = %d\n ", sipObj->X_BROADCOM_COM_UseSipUriForTls);
   snprintf( (char*)value, length, "%u",sipObj->X_BROADCOM_COM_UseSipUriForTls );
   cmsObj_free( (void **) &sipObj );
...
/*****************************************************************
**  FUNCTION:       dalVoice_GetVoiceFxsPhyInterfaceList
**
**  PUROPOSE:
**
**  INPUT PARMS:    none
...
CmsRet dalVoice_GetVoiceFxsPhyInterfaceList(DAL_VOICE_PARMS *parms, char *list, unsigned int length )
{
   ...
   /* Iterate through list of all fxs objects in the system */
   while ( (ret = cmsObj_getNextFlags(MDMOID_VOICE_PHY_INTF, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&obj)) == CMSRET_SUCCESS )
   {
      if( obj->description != NULL && strstr( obj->description, MDMVS_FXS ) != NULL)
      {
...
/*****************************************************************
**  FUNCTION:       dalVoice_GetVoiceNoSigPhyInterfaceList
**
**  PUROPOSE:
**
**  INPUT PARMS:    none
...
CmsRet dalVoice_GetVoiceNoSigPhyInterfaceList(DAL_VOICE_PARMS *parms, char *list, unsigned int length )
{
   ...
   while ( (ret = cmsObj_getNextFlags(MDMOID_VOICE_PHY_INTF, &iidStack, OGF_NO_VALUE_UPDATE, (void **)&obj)) == CMSRET_SUCCESS )
   {
      if( obj->description && strstr( obj->description, MDMVS_NOSIG ) != NULL )
      {
        memset(tmp, 0, sizeof(tmp));
        sprintf(tmp, "%u", obj->interfaceID);
        if(strlen(list))
...
CmsRet dalVoice_GetVlCFCallId( DAL_VOICE_PARMS *parms, char *getVal, unsigned int length )
{
   ...
   featureParms.op[0] = parms->op[0];
   featureParms.op[1] = parms->op[1];
   featureParms.op[2] = DAL_VOICE_FEATURE_CODE_CID; /* Incoming CallID Number */
...
CmsRet dalVoice_GetVlCFCallIdName( DAL_VOICE_PARMS *parms, char *getVal, unsigned int length )
{
   ...
   featureParms.op[0] = parms->op[0];
   featureParms.op[1] = parms->op[1];
   featureParms.op[2] = DAL_VOICE_FEATURE_CODE_CID_NAME; /* Incoming CallID Name */
...
/***************************************************************************
* Function Name: dalVoice_mapFxsInterfaceIdToPhyIntfId
* Description  : This returns the Voice Pstn instance number corresponding
*                to a certain Fxo interface Id.
*
* Parameters   : (INPUT) Fxo Interface Id - Fxo Physical interface ID
...
CmsRet dalVoice_mapFxsInterfaceIdToPhyIntfId(int id, int *pid)
{
   ...
   while ( (ret =  cmsObj_getNextInSubTreeFlags(MDMOID_VOICE_PHY_INTF, &iidStack, &subIidStack, OGF_NO_VALUE_UPDATE, &obj)) == CMSRET_SUCCESS)
   {
      if( strstr(((_VoicePhyIntfObject *)obj)->description, MDMVS_FXS ) != NULL)
      {
         if( i <= 0 )
         {
            cmsLog_debug("%s found fxs instance %d", __FUNCTION__, id);
            *pid = (int)((_VoicePhyIntfObject *)obj)->interfaceID;
            cmsObj_free(&obj);
            break;
         }
         i--;
   ...
   cmsLog_debug("%s fxsId:%d mapped to PhyId:%d", __FUNCTION__, id, *pid);
...
CmsRet dalVoice_GetSipMode( DAL_VOICE_PARMS *parms, char *value, unsigned int length )
{
   ...
   /* Copy the value */
   if( sipObj->X_BROADCOM_COM_SipMode != NULL )
   {
      strncpy( (char *) value, sipObj->X_BROADCOM_COM_SipMode, length);
   }
   else
   {
      value[0] = '\0';
...
CmsRet dalVoice_SetSipMode( DAL_VOICE_PARMS *parms, char *value )
{
   ...
   /* set the new value in local copy, after freeing old memory */
   cmsMem_free( sipObj->X_BROADCOM_COM_SipMode );
   sipObj->X_BROADCOM_COM_SipMode = cmsMem_strdup ( value );

   /* copy new value from local copy to MDM */
   if ( ( ret = cmsObj_set( sipObj,  &iidStack)) != CMSRET_SUCCESS )
   {
      cmsLog_error( "Can't set sip mode ret = %d\n", ret );
   }
                                         ...
                                         DAL_VOICE_FEATURE_CODE callFeatId, UINT32 setVal)
{
   switch(callFeatId)
   {
      ...
      case DAL_VOICE_FEATURE_CODE_CID:
         switch(callType)
         {
      ...
      case DAL_VOICE_FEATURE_CODE_CID_NAME:
         switch(callType)
         {
            case CFCALLTYPE_ENABLED:   cfObject->callerIDNameEnable = (UINT8)setVal; break;
            default: break;
         }
         break;
                                         ...
                                         DAL_VOICE_FEATURE_CODE callFeatId, int* getVal)
{
    switch(callFeatId)
    {
       ...
       case DAL_VOICE_FEATURE_CODE_CID_NAME:
           switch(callType)
           {
              case CFCALLTYPE_ENABLED:    *getVal = cfObject->callerIDNameEnable; break;
              default: break;
           }
           break;
...
static CmsRet dumpServiceProviderParams(int spNum)
{
   ...
   ** is irrelevant so we hard-code it. */
   ...
   {
      printf( "      SIP URI for TLS calls: %s\n", atoi(objValue) ? MDMVS_ON:MDMVS_OFF );
   }
...
{
    int numSipAccounts = 0;
    int loopLineInst = 0;
    int i;

    dalVoice_GetNumAccPerSrvProv( 0, &numSipAccounts );
