...
CmsRet dalWan_addIPv4Service_dev2(const WEB_NTWK_VAR *webVar)
{
   ...
   char ethIntfPathRef[MDM_SINGLE_FULLPATH_BUFLEN]={0};
   ...
   if ((p = strchr(wanL2IfName, '/')))
   {
      ...
      *p = '\0';
      ...
      cmsUtl_strstr(wanL2IfName, ETHLAG_IFC_STR))
   {
      /* do nothing for now */
   }
   else
   ...
   /* First get the fullpath to the layer 2 interface name. */
   {
      ...
      cmsUtl_strncpy(ethIntfPathRef, layer2FullPath, sizeof(ethIntfPathRef));
      ...
      )
   {
      ...
      cmsUtl_strncpy(ipIntfLowerLayer, ethIntfPathRef, sizeof(ipIntfLowerLayer));
      cmsUtl_strncpy(ethVlanPathRef, ethIntfPathRef, sizeof(ethVlanPathRef));
   }
   else
   {
      /* For EoA */
      ...
      if (isBridge)
      {
         ...
         ret = dalBridge_addPortToBridge_dev2(ethIntfPathRef,
                                              "br0",
                                              webVar->vlanTpid,
                                              webVar->vlanMuxId,
                                              webVar->vlanMuxPr,
         ...
         {
            char *brPortPathRef = NULL;

            ret = cmsMdm_pathDescriptorToFullPathNoEndDot(&brPortPathDesc, &brPortPathRef);
            if (ret == CMSRET_SUCCESS)
            {
               /* save brPortPathRef as ipIntfLowerLayer. */
               cmsUtl_strncpy(ipIntfLowerLayer, brPortPathRef, sizeof(ipIntfLowerLayer));
               CMSMEM_FREE_BUF_AND_NULL_PTR(brPortPathRef);
            }
            else
            {
               cmsLog_error("pathDescriptorToFullPath failed. DEV2_BRIDGE_PORT, iidStack=%s",
                            cmsMdm_dumpIidStack(&brPortPathDesc.iidStack));
            }
         }
         else
         {
            cmsLog_error("Failed adding bridge port object to br0. ret=%d", ret);
         }
      }
      else
      {
         ret = dalEth_addEthernetLink_dev2(ethIntfPathRef, ethLinkPathRef, sizeof(ethLinkPathRef));
         if (ret == CMSRET_SUCCESS)
         {
            ...
            */
            ...
            {
               cmsLog_error("Failed to add vlan termination object. ret=%d", ret);
            }
         }
         else
         {
         ...
         */
         ...
         if (ret == CMSRET_SUCCESS)
         {
            ...
            if (isBridge)
            {
               ...
               if (ret != CMSRET_SUCCESS)
               {
                  cmsLog_error("Failed to add bridge port object to br0. ret=%d", ret);
               }
            }
#endif
         }
   ...
   */
...
#ifdef DMP_X_BROADCOM_COM_DEV2_IPV6_1
   /* delete statically configured IPv6 default route (if any) */
   cmsDal_deleteWanGtwy6_dev2(ipIntfFullPath);
          ...
          ret == CMSRET_SUCCESS)
   {
      ...
      if (pathDesc.oid == MDMOID_DEV2_BRIDGE_PORT)
      {
         ...
         dalBridge_deleteVlanPort_dev2(&pathDesc.iidStack);
      }
#endif
 ...
 */
      ...
      {
         webVar->enblIpVer = ENBL_IPV4_ONLY;
      }
#ifdef DMP_DEVICE2_IPV6INTERFACE_1
/* ipIntfObj->IPv6Enable is only defined if DMP_DEVICE2_IPV6INTERFACE_1 is defined 
*/
      if (ipIntfObj->IPv4Enable && !ipIntfObj->IPv6Enable)
      {
         webVar->enblIpVer = ENBL_IPV4_ONLY;
      }
      else if (!ipIntfObj->IPv4Enable && ipIntfObj->IPv6Enable)
      {
         webVar->enblIpVer = ENBL_IPV6_ONLY;
      }  
      else if (ipIntfObj->IPv4Enable && ipIntfObj->IPv6Enable)
      {
         webVar->enblIpVer = ENBL_IPV4_IPV6;
      } 
#endif
...
#ifdef DMP_DEVICE2_ETHERNETINTERFACE_1
      {
                                    ...
                                    (void **) &ethIntfObj) == CMSRET_SUCCESS)
         {
            if (ethIntfObj->upstream && ethIntfObj->enable)
            {
               ...
               if ( dalEthLag_isEthIntfPartOfEthLag(ethIntfObj->name))
               {
                  /* If ethIntf is already part of EthLAG object, it cannot be used
                  * as the Eth WAN port
                  */
...
#ifdef DMP_DEVICE2_ETHLAG_1
    return dalEthLag_getInterface(ifList);
#endif /* DMP_DEVICE2_ETHERNETINTERFACE_1 */
   return CMSRET_SUCCESS;
}
...
CmsRet dalWan_getAvailableConId_dev2(const char *l2Ifname, SINT32*outConId)
{
   /* conId is the digit after the l2ifname and '.'.  eg. ptm0.1 where 1 is the conId. */
   return (rutUtil_getAvailVlanIndex_dev2(l2Ifname, outConId));
}

#ifdef DMP_X_BROADCOM_COM_L2TPAC_1
CmsRet rut_getWanL2tpAcObject_dev2(InstanceIdStack *iidStack, Dev2PppInterfaceL2tpObject **L2tpAcIntf);

CmsRet dalWan_getWanL2tpAcObject_dev2(InstanceIdStack *iidStack, void **obj)
{
   return (rut_getWanL2tpAcObject_dev2(iidStack, ( Dev2PppInterfaceL2tpObject **)obj));
}
#endif
...
static CmsRet dalWan_getIpInterface(const char *ifName, Dev2IpInterfaceObject **ipIntfObj, InstanceIdStack *iidStack)
{
   CmsRet ret = CMSRET_OBJECT_NOT_FOUND;
   UBOOL8 found = FALSE;

   cmsLog_debug("Enter %s", ifName);
   while (!found  &&
          (ret = cmsObj_getNext(MDMOID_DEV2_IP_INTERFACE, 
                                iidStack,
                                (void **)ipIntfObj)) == CMSRET_SUCCESS)
   {
      if (!(*ipIntfObj)->X_BROADCOM_COM_BridgeService && !cmsUtl_strcmp((*ipIntfObj)->name, ifName))
      {
         found = TRUE;
      }
      else
   ...
   if (!found)
   {
      cmsLog_error("Cannot find WAN IP interface %s for editing?", ifName);
      return CMSRET_INTERNAL_ERROR;
   }
...
CmsRet  dalWan_editeSafe_dev2(const char *ifName, UBOOL8 IPv4Enable, UBOOL8 IPv6Enable)
{
   Dev2IpInterfaceObject *ipIntfObj = NULL;
   UBOOL8 change = FALSE;
   CmsRet ret = CMSRET_SUCCESS;
   ...
   cmsLog_debug("Enter %s, IPv4Enable %d, IPv6Enable %d", ifName, IPv4Enable, IPv6Enable);
   if (CMSRET_SUCCESS != dalWan_getIpInterface(ifName, &ipIntfObj, &iidStack))
   {
      cmsLog_error("cannot find obj for %s", ifName);
      return CMSRET_INTERNAL_ERROR;
   }

   if (ipIntfObj->IPv4Enable != IPv4Enable)
   {
      ipIntfObj->IPv4Enable = IPv4Enable;
      change = TRUE;
   }
#ifdef DMP_X_BROADCOM_COM_DEV2_IPV6_1
   if (ipIntfObj->IPv6Enable != IPv6Enable)
   {
      ipIntfObj->IPv6Enable = IPv6Enable;
      change = TRUE;
   }
#endif
   if (change)
   {
      cmsLog_debug("Bring %s down first and reactivate it", ifName);
      ipIntfObj->enable = FALSE;
...
CmsRet  dalWan_geteSafe_dev2(const char *ifName, UBOOL8 *IPv4Enable, UBOOL8 *IPv6Enable)
{
   Dev2IpInterfaceObject *ipIntfObj = NULL;
   InstanceIdStack iidStack = EMPTY_INSTANCE_ID_STACK;
   ...
   *IPv4Enable = ipIntfObj->IPv4Enable;
   *IPv6Enable = ipIntfObj->IPv6Enable;
   cmsObj_free((void **)&ipIntfObj);
   cmsLog_debug("Exit %s, IPv4Enable %d, IPv6Enable %d", ifName, *IPv4Enable, *IPv6Enable);
