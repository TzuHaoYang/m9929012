                            ...
                            const char *ifName,const char *action)
{
   ...
   */
   ...
   cmsLog_debug("Add Entered, ifName=%s", ifName);

   if ((ret = qdmIntf_intfnameToFullPathLocked_dev2(ifName,FALSE,&ipIntfFullPath)) != CMSRET_SUCCESS)
   {
      cmsLog_error("Failed to convert ifname %s to fullpath, ret = %d", ifName, ret);
      return ret;
   }
   ...
   if (rutIpt_GetfwExceptionbyFullPath_dev2(ipIntfFullPath, &fwExObj,&iidStack) != CMSRET_SUCCESS)
   {
       ...
       ret = rutIpt_GetfwExceptionbyFullPath_dev2(ipIntfFullPath, &fwExObj, &iidStack);
   ...
   /* Got fwExObj  iidStack and free fwExObj and ipIntfFullPath since we are done with them */
   cmsObj_free((void **)&fwExObj);
   CMSMEM_FREE_BUF_AND_NULL_PTR(ipIntfFullPath);
 
   if ((ret = cmsObj_addInstance(MDMOID_DEV2_FIREWALL_EXCEPTION_RULE,&iidStack)) != CMSRET_SUCCESS)
   {
      cmsLog_error("cmsObj_addInstance MDMOID_DEV2_FIREWALL_EXCEPTION_RULE failed, ret %d", ret);
      return ret;
   }  
   ...
   if ((ret = cmsObj_set((void *)fwExRuleObj,&iidStack)) != CMSRET_SUCCESS)
   {
      cmsObj_deleteInstance(MDMOID_DEV2_FIREWALL_EXCEPTION_RULE, &iidStack);
      cmsLog_error("Failed to set MDMOID_DEV2_FIREWALL_EXCEPTION_RULE. ret = %d", ret);
   }
