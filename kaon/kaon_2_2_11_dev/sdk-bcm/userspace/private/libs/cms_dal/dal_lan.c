...
CmsRet dalLan_setLanDevice_igd(const WEB_NTWK_VAR *webVar)
{
   ...
   char dnsServerList[BUFLEN_128];  /* 2 comma separated list of DNS servers for now */
   UBOOL8 found = FALSE;
   ...
   else
   {
         ...
         {           
            if (webVar->enblDhcpSrv == 1)
            {
               ...
               lanHostCfgObj->DHCPLeaseTime = webVar->dhcpLeasedTime * 60; /* webVar is in minutes, MDM in seconds */
               /* Disable DHCP Relay */
...
void dalLan_getDhcpServerInfo_igd(const char *ifName,
                                  ...
                                  char *relaySrv)
 {
   ...
   *relaySrv = 0;
   ...
   if (leaseTime != NULL)
   {
      *leaseTime = lanHostCfg->DHCPLeaseTime / 60; /* MDM in seconds, webVar in minutes */
      cmsLog_debug("leaseTime=%d hours", (*leaseTime / 60));
   }
