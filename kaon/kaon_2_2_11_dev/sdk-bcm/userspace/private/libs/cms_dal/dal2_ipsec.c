...
extern CmsRet rutIPSec_config_dev2(void);
...
CmsRet dalIPSec_addTunnel_dev2(const PWEB_NTWK_VAR pWebVar)
{
   ...
   while ((ret = cmsObj_getNext
         (MDMOID_DEV2_IPSEC_FILTER, &iidStack, (void **) &filterObj)) == CMSRET_SUCCESS)
   {
      if(!strcmp(pWebVar->ipsConnName, filterObj->X_BROADCOM_COM_TunnelName)) 
      {
...
CmsRet dalIPSec_deleteTunnel_dev2(const char* connName)
{
   ...
   else 
   {
          ...
          CMSRET_SUCCESS)
      {
         ...
         Ikev1CfgObject *ikeObj = NULL;
         if ((ret = cmsObj_get(MDMOID_IKEV1_CFG, &pathDesc.iidStack,
         ...
         {
            if(cmsUtl_strlen(ikeObj->certificateName)) 
            {
               ret = dalCert_decRefCount(ikeObj->certificateName);
               if (ret != CMSRET_SUCCESS)
               {
