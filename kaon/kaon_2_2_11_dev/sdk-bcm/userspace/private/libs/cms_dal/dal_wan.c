...
CmsRet addWanPppConnection(const WEB_NTWK_VAR *webVar, InstanceIdStack *iidStack)
{
   ...
   case CMS_WAN_TYPE_PPPOA:
      ...
      {
         CMSMEM_REPLACE_STRING(wanPppConn->connectionTrigger, MDMVS_ONDEMAND);
         wanPppConn->idleDisconnectTime = webVar->pppIdleTimeLimit;
      }
      else
      {
         CMSMEM_REPLACE_STRING(wanPppConn->connectionTrigger, MDMVS_ALWAYSON);
      }
...
CmsRet dalWan_deleteService_igd(const WEB_NTWK_VAR *webVar)
{
   ...
   {
      ret = dalWan_deleteService_dev2(webVar);
   }
#endif
...
CmsRet dalWan_editInterface_igd(const WEB_NTWK_VAR *webVar)
{
   ...
   if (isPpp)
   {
      ...
      if (cmsUtl_strcmp(pppConn->connectionTrigger, webVar->enblPppDemand?MDMVS_ONDEMAND:MDMVS_ALWAYSON))
      {
         CMSMEM_REPLACE_STRING(pppConn->connectionTrigger , webVar->enblPppDemand?MDMVS_ONDEMAND:MDMVS_ALWAYSON);
         change = TRUE;
      }

      if (pppConn->idleDisconnectTime != (UINT32)webVar->pppIdleTimeLimit)
      {
         pppConn->idleDisconnectTime = (UINT32)webVar->pppIdleTimeLimit;
         change = TRUE;
      }
...
CmsRet restoreDhcpInfo(void)
{
   ...
   cmsObj_free((void **)&lanHostCfg);
   cmsObj_free((void **)&lanHostCfgDefault);
...
#endif
#ifdef DMP_X_BROADCOM_COM_L2TPAC_1
			if (!cmsUtl_strncmp(pppCon->name, "L2tp", 4))
			{
				if ((ret = cmsObj_deleteInstance(MDMOID_WAN_CONN_DEVICE, wanConIid)) != CMSRET_SUCCESS)
				{
				   cmsLog_error("cmsObj_deleteInstance<MDMOID_WAN_CONN_DEVICE> returns error. ret=%d", ret);
				   cmsObj_free((void **)&pppCon);
				   return ret;
				}
			}
...
#ifdef DMP_X_BROADCOM_COM_L2TPAC_1
CmsRet dalWan_getWanL2tpIidStatck(InstanceIdStack *l2tpWanIidStack)
{
   return (rutWl2_getL2tpWanIidStack(l2tpWanIidStack));
}
CmsRet dalWan_getWanL2tpAcObject_igd(InstanceIdStack *iidStack __attribute((unused)), void **obj __attribute((unused)))
{
...
CmsRet dalWan_getWanConInfoForEdit_igd(WEB_NTWK_VAR *webVar)
{
   ...
   else 
   {
      ...
      if (!cmsUtl_strcmp(pppConn->connectionTrigger, MDMVS_ONDEMAND))
      {
         webVar->enblPppDemand = TRUE;
      }
      else
      {
         webVar->enblPppDemand = FALSE;
      }
      webVar->pppIdleTimeLimit = pppConn->idleDisconnectTime;
   }
...
CmsRet cmsDal_getConfiguredWanEthIntf_igd(char *gConfiguredWanEthIntf  __attribute((unused)))
{
   /* TODO */
   CmsRet ret  = CMSRET_SUCCESS;
...
CmsRet  dalWan_editeSafe_igd(const char *ifName, UBOOL8 isIPv4Enabled, UBOOL8 isIPv6Enabled)
{
   UBOOL8 isPpp = FALSE;
   void *wanConnObj=NULL;
   InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK;
   ...
   if (CMSRET_SUCCESS != dalWan_getIpOrPppObjectByIfname(ifName, &iidStack, &wanConnObj, &isPpp))
   {
   ...
   else
   {
      ...
      if (ipConn->X_BROADCOM_COM_IPv4Enabled != isIPv4Enabled) 
      {
         ipConn->X_BROADCOM_COM_IPv4Enabled = isIPv4Enabled;
         change = TRUE;
      }
      if (ipConn->X_BROADCOM_COM_IPv6Enabled != isIPv6Enabled) 
      {
         ipConn->X_BROADCOM_COM_IPv6Enabled = isIPv6Enabled;
         change = TRUE;
      }
   ...
   if (change)
   {
      ...
      */
      ...
      else
      {
         ...
         {             
            /* Reactivate this WAN connection */
            ret = dalWan_enableDisableWanConnObj(ifName, TRUE);
         }
         /* else, keep the WAN down */
      }
   }
   else
   {
...
CmsRet  dalWan_geteSafe_igd(const char *ifName, UBOOL8 *isIPv4Enabled, UBOOL8 *isIPv6Enabled)
{
   UBOOL8 isPpp = FALSE;
   void *wanConnObj=NULL;
   InstanceIdStack iidStack=EMPTY_INSTANCE_ID_STACK;
   ...
   else
   {
      ...
      *isIPv4Enabled = ipConn->X_BROADCOM_COM_IPv4Enabled;
      *isIPv6Enabled = ipConn->X_BROADCOM_COM_IPv6Enabled;     
   }
