    ...
    char *maxAddress, SINT32 *leaseTime, char *relaySrv)
{
          ...
          cmsObj_getNext(MDMOID_DEV2_DHCPV4_SERVER_POOL, &iidStack, (void **)&dhcpv4ServerPoolObj) == CMSRET_SUCCESS)
   {
          ...
          cmsUtl_strcmp(ifName, ifNameBuf) == 0)
      {
         ...
         if (leaseTime != NULL)
         {
            *leaseTime = dhcpv4ServerPoolObj->leaseTime / 60; /* MDM in seconds, webVar in minutes */
         }
      }
                         ...
                         (void **)&dhcpv4ServerPoolObj) == CMSRET_SUCCESS)
   {
          ...
          cmsUtl_strcmp(ifName, bridgeIfName) == 0)
      {
         ...
         if (webVar->enblDhcpSrv == 1)
         {
            ...
            dhcpv4ServerPoolObj->leaseTime = webVar->dhcpLeasedTime * 60; /* webVar is in minutes, MDM in seconds */
...
void dalLan_getLanVlan_dev2(WEB_NTWK_VAR *webVar) 
{
   ...
   cmsLog_debug("Enter: lanName=%s", webVar->lanName);

   webVar->lanVlanEnable = 0;
   webVar->lanVlanTagList[0] = '\0';

   if (IS_EMPTY_STRING(webVar->lanName))
   {
      return;
   }
...
#ifdef DMP_DEVICE2_VLANTERMINATION_1
   {
      ...
      {
         if (cmsUtl_strstr(vlanTerminationObj->name, webVar->lanName) != NULL)
         {
            if (strlen(webVar->lanVlanTagList))
...
#ifdef DMP_DEVICE2_VLANBRIDGE_1
   {
      ...
      char defaultVlanPort[BUFLEN_64];
      UINT32 vlanTpid;
      SINT32 vlanId;
      SINT32 vlanPr;

      qdmIntf_getPathDescFromIntfnameLocked_dev2(webVar->lanName, TRUE, &pathDesc);
      ret = cmsMdm_pathDescriptorToFullPathNoEndDot(&pathDesc, &lanFullPath);
      if (ret != CMSRET_SUCCESS)
      {
      ...
      snprintf(defaultVlanPort, sizeof(defaultVlanPort), "%s.0", webVar->lanName);

      /* Loop thru all non-mgmt ports of the LAN interface excluding
       * the default lanvlan port.
       */
      INIT_INSTANCE_ID_STACK(&iidStack);
                                 ...
                                 (void **)&brPortObj) == CMSRET_SUCCESS)
      {
         if (!brPortObj->managementPort &&
             cmsUtl_strstr(brPortObj->lowerLayers, lanFullPath) != NULL &&
             cmsUtl_strcmp(brPortObj->name, defaultVlanPort) &&
             cmsUtl_strcmp(brPortObj->name, webVar->lanName))
         {
            INIT_PATH_DESCRIPTOR(&pathDesc);
            pathDesc.oid = MDMOID_DEV2_BRIDGE_PORT;
            pathDesc.iidStack = iidStack;
            
            ret = qdmVlan_getVlanInfoLocked_dev2(&pathDesc,
            ...
            {
               if (strlen(webVar->lanVlanTagList))
                  snprintf(strVlanTag, BUFLEN_16, "|%d/%d", vlanId, vlanPr);
               else
                  snprintf(strVlanTag, BUFLEN_16, "%d/%d", vlanId, vlanPr);
               cmsUtl_strncat(webVar->lanVlanTagList, BUFLEN_16, strVlanTag);
...
void dalLan_setLanVlan_dev2(WEB_NTWK_VAR *webVar) 
{
   ...
   snprintf(tmpStr, BUFLEN_128, "%s", webVar->lanVlanTagList);
   ...
   for (i = 0, ptr = tmpStr; ; ptr = NULL, i++)
   {
      ...
      {
         char vlanName[BUFLEN_40];

         dal_createVlanTermination_dev2(lanFullPath, vid, pBit, webVar->lanVlanEnable);
         snprintf(vlanName, BUFLEN_40, "%s.%d", webVar->lanName, vid);
#ifdef DMP_DEVICE2_BRIDGE_1
         /* lanvlan interface shall not join any bridge when created. */
//         dalBridge_addIntfNameToBridge_dev2(vlanName, "br0");
   ...
   {
      if (cmsUtl_strstr(vlanTerminationObj->lowerLayers, lanFullPath) != NULL)
      {
         found = FALSE;
         for (j = 0 ; j < i ; j++)
         { 
            if ((unsigned int)vlanTerminationObj->VLANID == vlans[j])
            {
               found = TRUE;
               break;
            }
...
#ifdef DMP_DEVICE2_VLANBRIDGE_1
   /* Now we have to clean up all bridge ports of the LAN interface that are not
    * included in the vlans[] list.
    */
   {
      ...
      /* Find all non-mgmt bridge ports of the LAN interface excluding
       * the default lan vlan port.
       */
      INIT_INSTANCE_ID_STACK(&iidStack);
      while (cmsObj_getNextFlags(MDMOID_DEV2_BRIDGE_PORT, &iidStack,
                                 OGF_NO_VALUE_UPDATE,
                                 (void **) &brPortObj) == CMSRET_SUCCESS)
      {
         if (!brPortObj->managementPort &&
             ...
             cmsUtl_strcmp(brPortObj->name, webVar->lanName))
         {
            ...
            else
            {
               ...
               if (!found)
               {
                  ...
                  dalBridge_deleteVlanPort_dev2(&iidStack);
                  ...
                  /* since this instance had been deleted, we want to set the iidStack
                   * to the previous instance, so that we can continue to do getNext.
                   */
