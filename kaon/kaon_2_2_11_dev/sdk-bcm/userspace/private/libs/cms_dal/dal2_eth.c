...
CmsRet cmsDal_getConfiguredWanEthIntf_dev2(char *gConfiguredWanEthIntf)
{
                              ...
                              (void **) &ethIntfObj) == CMSRET_SUCCESS)
   {
      if ( ethIntfObj->upstream &&
         ...
         ethIntfObj->enable)
      {
         if (cmsUtl_strlen(gConfiguredWanEthIntf) > 0)
         {
            cmsUtl_strcat(gConfiguredWanEthIntf, ",");
         }
         cmsUtl_strcat(gConfiguredWanEthIntf, ethIntfObj->name);
      }
      cmsObj_free((void **)&ethIntfObj);
   }

   cmsLog_debug("gConfiguredWanEthIntf=%s", gConfiguredWanEthIntf);
