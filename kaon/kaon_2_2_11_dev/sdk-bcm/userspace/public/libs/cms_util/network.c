...
CmsRet cmsUtl_getULAddressByPrefix(const char *prefix, const char *ifName, char *ulAddr, UINT32 *prefixLen)
{
    CmsRet        ret;
    char          prefixAddr[CMS_IPADDR_LENGTH];
    unsigned char macAddr[MAC_ADDR_LEN];

    *macAddr = '\0';
    if ((ret = cmsNet_getMacAddrByIfname(ifName, macAddr)) != CMSRET_SUCCESS)
    {
        cmsLog_error("cmsNet_getMacAddrStringByIfname returns error. ret=%d", ret);
        return ret;
    }

    *prefixAddr = '\0';   
    if ((ret = cmsUtl_parsePrefixAddress(prefix, prefixAddr, prefixLen))!= CMSRET_SUCCESS)
    {
       cmsLog_error("cmsUtl_parsePrefixAddress returns error. ret=%d", ret);
       return ret;
    }
    
    if ((ret = cmsUtl_prefixMacToAddress(prefixAddr, macAddr, ulAddr)) != CMSRET_SUCCESS)
    {
       cmsLog_error("cmsUtl_prefixMacToAddress returns error. ret=%d", ret);
       return ret;
    }
