...
int load_gpon_modules(int sub_type, int speed)
{
    int rc;
...
#ifdef BCM_PON_RDP
    else if (MATCH_VALUE(buf, RDPA_WAN_TYPE_VALUE_AE))
    {
        net_port->port = NET_PORT_LAN_5;
    }
#endif
...
#ifdef BCM_PON_RDP
            break; /* Dynamic AE WAN not supported, no need to load epon driver in GBE mode */
#endif
        case NET_PORT_EPON:
