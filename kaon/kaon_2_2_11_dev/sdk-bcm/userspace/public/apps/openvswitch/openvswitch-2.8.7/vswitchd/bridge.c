...
#include "ofproto/ovs_bcmmcast.h"
#endif
...
#if defined(CONFIG_BCM_OVS_MCAST)
int bridge_manage_mcastsnoopentry(char *brname, ovs_be32 ip4,
...
#if defined(CONFIG_BCM_OVS_MCAST)
    bridge_manage_mcastsnoopentry_hook = bridge_manage_mcastsnoopentry;
#endif
    initialized = true;
}
...
#if defined(CONFIG_BCM_OVS_MCAST)
        ovs_mcpd_brcfg_brdelete(br->name);
#endif
...
#if defined(CONFIG_BCM_OVS_MCAST)
extern int manage_mcast_snooping_entry(struct ofproto *ofproto_, ovs_be32 ip4, 
                                       struct in6_addr *ip6, uint16_t vlan, 
                                       void *port, int isip4, ovs_be32 ip4src, 
                                       struct in6_addr *ip6src, int addentry);
int bridge_manage_mcastsnoopentry(char *brname, ovs_be32 ip4,
                                  struct in6_addr *ip6, uint16_t vlan, 
                                  char *portname, int isip4,
                                  ovs_be32 ip4src, struct in6_addr *ip6src,
                                  int addentry)
{
    ...
    br = bridge_lookup(brname);
    if (br && br->cfg->mcast_snooping_enable)
    {
        struct port *port;

        if ((port = port_lookup(br, portname)) != NULL)
        {
            manage_mcast_snooping_entry(br->ofproto, ip4, ip6, 
                                        vlan, (void *)port, isip4, 
                                        ip4src, ip6src, addentry);
            return 0;
        }
    }
    return -1;
