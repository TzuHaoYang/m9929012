	...
	*tcp_flags = 0;
	...
	for (cpu = 0; cpu < nr_cpu_ids; cpu = cpumask_next(cpu, cpu_possible_mask)) {
		...
		if (stats) {
			...
			/* get stats from flow cache only for CPU 0 */
			if (cpu == 0) {
				stats_updated = ovs_bcm_blog_mega_update_fast_stats(
								(struct sw_flow *)flow, ovs_stats);
			}
#endif
...
void ovs_flow_stats_clear(struct sw_flow *flow)
{
	...
	for (cpu = 0; cpu < nr_cpu_ids; cpu = cpumask_next(cpu, cpu_possible_mask)) {
		...
		if (stats) {
#if defined(CONFIG_BCM_OVS) && defined(CONFIG_BLOG)
			/* clear stats from flow cache only for CPU 0 */
			if (cpu == 0) {
				ovs_bcm_blog_mega_clear_fast_stats(flow);
			}
#endif
			spin_lock_bh(&stats->lock);
			stats->used = 0;
