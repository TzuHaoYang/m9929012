...
void ovs_dp_process_packet(struct sk_buff *skb, struct sw_flow_key *key)
{
	...
	if (unlikely(!flow)) {
		...
		upcall.bcm_pktmetadata.to_acceldev_ifi = -1;
		if (skb->in_dev) 
		{
			upcall.bcm_pktmetadata.to_acceldev_ifi = 
			((struct net_device *)skb->in_dev)->ifindex;
		}
#endif
...
#if defined(CONFIG_BCM_OVS_MCAST)
	/* OVS_PACKET_ATTR_BCM_PKTMETADATA */
	if (upcall_info->bcm_pktmetadata.to_acceldev_ifi != -1)
		size += nla_total_size(sizeof(upcall_info->bcm_pktmetadata));
#endif
...
#if defined(CONFIG_BCM_OVS_MCAST)
	/* Add OVS_PACKET_ATTR_BCM_PKTMETADATA if to_acceldev_ifidx is set */
	if (upcall_info->bcm_pktmetadata.to_acceldev_ifi != -1) {
		if (nla_put(user_skb, 
					OVS_PACKET_ATTR_BCM_PKTMETADATA, 
					sizeof(upcall_info->bcm_pktmetadata), 
					&upcall_info->bcm_pktmetadata)) {
			err = -ENOBUFS;
			goto out;
		}
		pad_packet(dp, user_skb);
...
static int ovs_flow_cmd_new(struct sk_buff *skb, struct genl_info *info)
{
	...
	error = ovs_nla_get_match(net, &match, a[OVS_FLOW_ATTR_KEY],
				  a[OVS_FLOW_ATTR_MASK], log, &blog_match);
#else
	error = ovs_nla_get_match(net, &match, a[OVS_FLOW_ATTR_KEY],
	...
	if (likely(!flow)) {
#if defined(CONFIG_BCM_OVS) && defined(CONFIG_BLOG)
		new_flow->output_count = 
			ovs_bcm_blog_output_ports(acts->actions, acts->actions_len);
		new_flow->blog_match = blog_match;
#endif
...
#if defined(CONFIG_BCM_OVS) && defined(CONFIG_BLOG)
	/* Evict and relearn if an accelerated flow already exists
		since actions have been modified */
	if (flow && flow->blog_key) 
	{
		ovs_bcm_blog_notify(flow);
	}
#endif
	return 0;
