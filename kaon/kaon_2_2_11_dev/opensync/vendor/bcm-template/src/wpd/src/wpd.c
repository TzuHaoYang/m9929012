...
static void wpd_add_proc_2_list(struct wpd_ctx *ctx, const char *arg)
{
    ...
    if (ctx->proc_cnt + 1 > WPD_MAX_PROC_CNT)
    {
        LOGI("Too many arguments in proc list, supported only : %d",
             WPD_MAX_PROC_CNT);
        return;
    }
    c = ctx->proc_cnt;
    strncpy(ctx->proc_list[c], arg, sizeof(ctx->proc_list[c]));
    ctx->proc_cnt++;
}
...
static void wpd_handle_proc_list(struct wpd_ctx *ctx)
{
    ...
    for (i = 0; i < ctx->proc_cnt; i++)
    {
        ...
        LOGI("Sending segmentation fault signal into process %s",
             ctx->proc_list[i]);
        rc = system(cmd);
        if (!WIFEXITED(rc) || WEXITSTATUS(rc) !=0)
            LOGI("Killing %s failed [%d, %d]", ctx->proc_list[i],
