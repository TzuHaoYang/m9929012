 ...
 * logread.c
 *
 * Plume's version of logread utility
 *
 * Copyright (c) 2016 Plume Design, Inc.
 */
...
int
filter_add(filter_t *fp, filter_type_t type, char *match, bool invert, bool sensitive)
{
    filter_ent_t        *ent;
...
bool
filter_match(filter_t *fp, filter_type_t type, const char *what)
{
    filter_ent_t        *ent;
    ...
    if (!what) { // in case NULL was passed in
        return false;
    }
...
process_line(const char *line)
{
...
#if defined(CONFIG_LOGREAD_LOG_HAS_YEAR)
    char                    *yr;
#endif /* defined(CONFIG_LOGREAD_LOG_HAS_YEAR) */
#if defined(CONFIG_LOGREAD_LOG_HAS_LEVEL)
    char                    *llvl;
#endif /* defined(CONFIG_LOGREAD_LOG_HAS_LEVEL) */
#if defined(CONFIG_LOGREAD_LOG_HAS_HOSTNAME)
    char                    *hn;
#endif /* defined(CONFIG_LOGREAD_LOG_HAS_HOSTNAME) */
    ...
    do {
        ...
        if (!filter_match(filter, FILTER_TYPE_PROC, proc)    ||
            !filter_match(filter, FILTER_TYPE_PID, pid)      ||
            !filter_match(filter, FILTER_TYPE_SEVERITY, sev) ||
            !filter_match(filter, FILTER_TYPE_MODULE, mod)   ||
        ...
        if (json_output) { // JSON output for elasticsearch
            if (json_base) {
...
#else  /* not defined(CONFIG_LOGREAD_LOG_HAS_YEAR) */
            snprintf(buf, sizeof(buf)-1, "%s %s %s", m, d, tm); // re-create timestamp
#endif /* not defined(CONFIG_LOGREAD_LOG_HAS_YEAR) */
            json_object_set_new(json,       "@version",   json_integer(1));
            json_object_set_new(json,       "message_tm", json_string(buf));
            if (proc)
                json_object_set_new(json,   "process",    json_string(proc));
            if (pid)
                json_object_set_new(json,   "pid",        json_integer(atoi(pid)));
            if (sev)
                json_object_set_new(json,   "severity",   json_string(sev));
            if (mod)
                json_object_set_new(json,   "module",     json_string(mod));
...
void
tailf_cbk(struct ev_loop *loop, void *ev, int revents)
{
    static char            buf[2048];
    static size_t          len = 0, rlen, llen;

    // Read chunk into buffer past the previously saved partial line
    while((rlen = tailf_read(&tf, &buf[len], (sizeof(buf) - len))) > 0) {
        rlen += len;
        llen = parse_lines(buf, rlen);
        len = (rlen - llen);
        if (len > 0) {
...
void
process_filter(filter_type_t type, char *str, bool invert, bool sensitive)
{
    char *what = str, *next;
    int ret;
...
usage(char *prog)
{
    fprintf(stderr, "Plume logread v%s\n", VERSION);
    fprintf(stderr, "Usage: %s [options]\n\n", prog);
    fprintf(stderr, "Options Available:\n");
    fprintf(stderr, "   -V|--version                   Print version information\n");
    fprintf(stderr, "   -F|--file <filename>           Read from file insted of /var/log/messages\n");
