...
static bool cmd_delayed_reboot(const char *delay, const char *rebootReason)
{
    char cmd[512];

    snprintf(cmd, sizeof(cmd), CONFIG_INSTALL_PREFIX"/scripts/delayed-reboot %s %s", delay, rebootReason);
        ...
        struct schema_Wifi_Test_Config *tconfig)
{
        ...
        strcmp("reboot", tconfig->test_id) == 0)
    {
        ...
        if (delay && cmd_delayed_reboot(delay, "CLOUD"))
        {
