...
#define OOKLA_SELECTION_DETAILS_CMD \
        SPEEDTEST_PATH " -c '" CONFIG_SPEEDTEST_URL "' --selection-details"
...
void pa_cb(pasync_ctx_t *ctx, void *buff, int buff_sz)
{
...
static void on_speedtest_timeout(struct ev_loop *loop, ev_timer *watcher, int revent)
{
    ...
    st_in_progress = false;  /* Not strictly neccessary here, since after
        ...
        const struct st_config *st_config)
{
    ...
    /* Traverse through list and try to select most higly ranked preferred:  */
    for (i=0; i < num; i++)
    {
        if (i == 0)
    ...
    switch (self->mon_type)
    {
        ...
        case OVSDB_UPDATE_MODIFY:
            ...
            if (!strcmp(speedtest_config.test_type, "OOKLA"))
            {
                ...
                ev_timer_init(&st_timeout, on_speedtest_timeout, SPEEDTEST_TIMEOUT, 0.0);
                ...
                /* Speedtest context: must be freed in pasync callback. */
                struct st_context *st_ctx_data = malloc(sizeof(*st_ctx_data));
                *st_ctx_data = st_ctx;

                if (!pasync_ropenx(EV_DEFAULT, speedtest_config.testid, st_ctx_data, st_cmd, pa_cb))
                {
