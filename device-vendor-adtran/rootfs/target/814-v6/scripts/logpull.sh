...
collect_and_pack()
{
    ...
    collect_cmd ovs-ofctl dump-flows $BR_HOME
    ...
    collect_cmd ovs-appctl dpif/dump-flows $BR_HOME
    collect_cmd ovs-appctl fdb/show $BR_HOME

    collect_cmd ovs-appctl mdb/show $BR_HOME
    if [ ! -z $BR_WAN ]; then
        collect_cmd ovs-appctl fdb/show $BR_WAN
        collect_cmd ovs-ofctl dump-flows $BR_WAN
        collect_cmd ovs-appctl mdb/show $BR_WAN
    fi
